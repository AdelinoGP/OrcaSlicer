# Repo-owned recipe — copied from ConanCenter's opencascade/7.6.0 (revision
# a3e21966548327e0bf56728b57f57238) with ONE functional change: tcl/tk removed
# and BUILD_MODULE_Draw disabled. Rationale: tcl 8.6.x cannot be built under
# VS 18 2026 (its nmakehlp/rules.vc bootstrap fails) and ConanCenter serves no
# prebuilt binaries for this compiler; OCCT needs tcl only for the Draw test
# harness, which this project does not use (deps/OCCT/OCCT.cmake likewise
# builds with BUILD_MODULE_Draw=OFF and no tcl).
# Keep every other behavior identical to the center recipe.
...
class OpenCascadeConan(ConanFile):
    name = "opencascade"
    # version pinned in-recipe so `conan export conan/recipes/<pkg>` needs no
    # --version flag (rule pnp.conan auto-exports every repo recipe)
    version = "7.6.0"
    ...
    default_options = {
        ...
        "with_draco": False,
        # tk needs tcl, which this recipe removed (see header); must stay False
        "with_tk": False,
        "with_tbb": False,
        "with_opengl": True,
    ...
    def requirements(self):
        # tcl/tk removed (Draw disabled — see header)
    ...
    def generate(self):
        ...
        # Draw is OCCT's tcl-based test harness; disabled so tcl is not needed
        # (see header). Dependency toolkits of enabled modules still build.
    ...
    def _patch_sources(self):
        ...
        ## tcl — removed (Draw disabled, see header); neutralize the detection
        ## include so OCCT's cmake never looks for tcl.
        replace_in_file(
            self,
            cmakelists,
            "OCCT_INCLUDE_CMAKE_FILE (\"adm/cmake/tcl\")",
            "# tcl detection removed (BUILD_MODULE_Draw=OFF, pnp repo recipe)",
            strict=False,
        )
        ## tk
        if self._link_tk:
...
#pragma comment (lib, "tk"  STRINGIZE2(TCL_MAJOR_VERSION) STRINGIZE2(TCL_MINOR_VERSION) ".lib")""",
    ...
    def _get_modules_from_source_code(self):
        csf_to_conan_dependencies = {
            ...
            # tcl removed (Draw disabled); no built toolkit may reference it
            "CSF_TclLibs": {},
