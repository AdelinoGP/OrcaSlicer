...
class wxWidgetsConan(ConanFile):
    # Repo-owned recipe — copied from ConanCenter's wxwidgets/3.3.2 with ONE
    # functional change: private headers (wx/private, wx/generic/private,
    # wx/msw/private, wx/gtk/private) are installed into the package.
    # OrcaSlicer uses them (accessibility/markuptext); the deps/ build copied
    # them post-install the same way (deps/wxWidgets/wxWidgets.cmake:60-80).
    name = "wxwidgets"
    # version pinned in-recipe so `conan export` needs no --version
    # (rule pnp.conan auto-exports every repo recipe)
    version = "3.3.2"
    ...
    def generate(self):
        ...
        # where wx has NATIVE backends (Credential Manager / Keychain) that
        # need no dependency. OrcaSlicer uses wxSecretStore (cloud login).
        _secretstore_native = str(self.settings.os) in ("Windows", "Macos")
        tc.cache_variables["wxUSE_SECRETSTORE"] = \
            "ON" if (self.options.get_safe("secretstore") or _secretstore_native) else "OFF"
    ...
    def package(self):
        ...
        copy(self, pattern="*setup.h",
             ...
             keep_path=False)
        # pnp repo recipe: install private headers (see class docstring);
        # wx's own cmake install skips them
        for sub in ("private", "generic/private", "msw/private", "gtk/private"):
            copy(self, pattern="*.h",
                 src=os.path.join(self.source_folder, "include", "wx", *sub.split("/")),
                 dst=os.path.join(self.package_folder, "include", "wx", *sub.split("/")))
