...
bool default_already_bound(const std::string& key) { return identity_keys().count(key) > 0 || key == "support_interface_flow"; }

const PnpConfigKeyDef* find(const std::vector<PnpConfigKeyDef>& defs, const std::string& key)
{
    auto it = std::find_if(defs.begin(), defs.end(), [&key](const PnpConfigKeyDef& d) { return d.key == key; });
...
TEST_CASE("pnp config-schema types map onto Orca option types", "[pnp][config_keys]")
{
    ...
    SECTION("every type pnp declares has an Orca counterpart")
    {
        ...
        const Case cases[] = {
            ...
            {"enum", coEnum},
            {"percent", coPercent},
            {"float_or_percent", coFloatOrPercent},
            {"float-list", coFloats},
            {"string-list", coStrings},
        };
...
TEST_CASE("parse_schema turns a config-schema reply into registerable defs", "[pnp][config_keys]")
{
    ...
    const auto was_skipped = [&skipped](const std::string& key) {
        return std::any_of(skipped.begin(), skipped.end(), [&key](const PnpConfigKeys::SkippedKey& s) { return s.key == key; });
    };
    ...
    SECTION("a key declared in both halves is registered once")
    {
        ...
        const auto occurrences = std::count_if(defs.begin(), defs.end(),
...
TEST_CASE("registered pnp keys become first-class Orca keys", "[pnp][config_keys]")
{
    ...
    SECTION("a registered key whose name Orca ignores still deserializes")
    {
        ...
        CHECK(std::find(ctx.unrecogized_keys.begin(), ctx.unrecogized_keys.end(), "support_sharp_tails") == ctx.unrecogized_keys.end());
    }
    ...
    SECTION("an unregistered name Orca ignores is still dropped")
    {
        ...
        CHECK(std::find(ctx.unrecogized_keys.begin(), ctx.unrecogized_keys.end(), "support_remove_small_overhangs") !=
              ctx.unrecogized_keys.end());
    }
...
TEST_CASE("an ignore-set pnp key round-trips through a project 3mf", "[pnp][config_keys]")
{
    ...
    Semver file_version;
    const bool loaded = load_bbs_3mf(test_file.c_str(), &dst_config, &ctxt, &dst_model, &dst_plates, &project_presets, &is_bbl_3mf,
