// [INTENT] Windows-specific application entry point wrapper for OrcaSlicer.
// Handles: GPU selection hints, OpenGL driver detection, DLL loading, and main() dispatch.
// [HAZARD] Platform-specific code (Windows only) - not relevant for cross-platform port.
...
#ifdef SLIC3R_GUI
extern "C" {
// [INTENT] GPU selection hints for Windows hybrid graphics systems.
// NvOptimusEnablement = 1 tells NVIDIA driver to prefer discrete GPU.
// AmdPowerXpressRequestHighPerformance = 1 tells AMD driver to prefer discrete GPU.
// [HAZARD] These are Windows-specific exports recognized by GPU drivers at load time.
...
// [INTENT] OpenGL version detection for Windows - checks if system has adequate OpenGL 2.0 support.
// If not, falls back to MESA software renderer. Creates temporary hidden window to probe GL driver.
// [HAZARD] Windows-specific GL probing - uses WGL (OpenGL extension for Windows).
// [COUPLING] Uses boost::split for string parsing - adds boost dependency for string ops.
#ifdef SLIC3R_GUI
...
public:
    ...
    // [MEMORY] HINSTANCE is Windows DLL handle - must call FreeLibrary() to release.
    HINSTANCE hOpenGL = nullptr;
    bool      success = false;

    // [INTENT] Create hidden window to probe OpenGL capabilities - loads system opengl32.dll.
    // Uses message pump to allow window creation callback to execute.
    bool load_opengl_dll()
    {
        ...
        wc.style         = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"OrcaSlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0,
    ...
    // [INTENT] Unload dynamically loaded opengl32.dll - releases GPU driver resources.
    ...
    // [INTENT] Parse version string to compare against required minimum.
...
// [INTENT] Function pointer type for main DLL entry - stdcall convention on x86.
extern "C" {
...
extern "C" {
// [INTENT] Windows application entry point - either wWinMain (GUI, no console) or wmain (console).
// Handles: argument parsing, OpenGL detection, DLL loading, main function dispatch.
// [MEMORY] Loads OrcaSlicer.dll via LoadLibraryEx - must remain loaded while app runs.
...
#endif
{
    ...
    // [HAZARD] _set_error_mode is Windows-specific - affects assert behavior.
...
#ifdef SLIC3R_GUI
    // [INTENT] Check OpenGL version - if < 2.0, force MESA software renderer.
...
#ifdef SLIC3R_GUI
    // [INTENT] Load MESA software renderer if system OpenGL is insufficient.
    if (load_mesa) {
    ...
    // [INTENT] Load OrcaSlicer.dll from same directory as executable.
    ...
    // [INTENT] Resolve main function from DLL - different name for x86 vs x64.
    orcaslicer_main = (Slic3rMainFunc)
    ...
    // [INTENT] Dispatch to DLL's main function - app execution continues there.
