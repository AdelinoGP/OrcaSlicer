...
enum class VendorType { Unknown = 0, Klipper, Marlin, Marlin_BBL, Klipper_Qidi };
namespace Slic3r {
...
struct AMSComboInfo
{
    ...
    bool empty() { return ams_names.empty(); }
};
...
// [INTENT] Aggregate all user/system preset collections and expose one merge point that builds slicer-ready DynamicPrintConfig.
// [COUPLING] Central dependency hub for Preset, AppConfig, Semver, vendor profile metadata, and import/export format logic.
class PresetBundle
{
public:
    // [INTENT] Compose printer/print/filament/project layers into an execution config consumed by slicing and G-code generation.
    // [STATE] Reads mutable preset state from all incoming collections; output depends on active selections and optional filament remapping.
    ...
    // [STATE] Primary startup mutation path: repopulates all preset collections and selection pointers.
    PresetsConfigSubstitutions load_presets(AppConfig&                           config,
    ...
    // [HAZARD] Import entrypoints accept overwrite callbacks and external files; behavior depends on caller-provided conflict policy.
    PresetsConfigSubstitutions load_user_presets(std::string user, ForwardCompatibilitySubstitutionRule rule);
    PresetsConfigSubstitutions load_user_presets(AppConfig&                                                 config,
    ...
    std::optional<FilamentBaseInfo> get_filament_by_filament_id(const std::string& filament_id,
                                                                const std::string& printer_name = std::string()) const;
    ...
    // [COUPLING] Network behavior switches are derived from preset/vendor state instead of a dedicated networking settings module.
    ...
    PresetsConfigSubstitutions load_project_embedded_presets(std::vector<Preset*>                 project_presets,
    ...
    std::set<std::string> get_printer_names_by_printer_type_and_nozzle(const std::string& printer_type,
    ...
    // [STATE] These collections and active selections are long-lived mutable state shared with most UI and slicing entrypoints.
    ...
    // [MEMORY] VendorProfile ownership lives in this map; preset records keep references/pointers that must not outlive PresetBundle.
    VendorMap vendors;
    ...
    bool has_defauls_only() const { return prints.has_defaults_only() && filaments.has_defaults_only() && printers.has_defaults_only(); }

    // [INTENT] Produce a single resolved config snapshot used as the effective contract with libslic3r core algorithms.
    ...
    //  [COUPLING] JSON vendor import bridges on-disk profile schema with in-memory PresetCollection semantics and cross-vendor inheritance.
    std::pair<PresetsConfigSubstitutions, size_t> load_vendor_configs_from_json(const std::string&                   path,
    ...
    // void                        export_configbundle(const std::string &path, bool export_system_settings = false, bool export_physical_printers
    // = false); BBS: add a function to export current configbundle as default void export_current_configbundle(const std::string &path); BBS:
    // add a function to export system presets for cloud-slicer void export_system_configs(const std::string &path);
    std::vector<std::string> export_current_configs(const std::string&                     path,
                                                    std::function<int(std::string const&)> override_confirm,
                                                    bool                                   include_modify,
    ...
    void update_compatible(PresetSelectCompatibleType select_other_print_if_incompatible,
    ...
    void save_changes_for_preset(const std::string&              new_name,
                                 Preset::Type                    type,
...
private:
    ...
    // [MEMORY] merge_presets moves temporary bundle state into this instance; moved-from references must not be reused by callers.
    ...
    void load_config_file_config(const std::string&   name_or_path,
                                 bool                 is_external,
