...
// [INTENT] PerimeterGeneratorLoop — tree node used exclusively by process_classic().
// Represents a single perimeter polygon at a specific depth in the contour hierarchy.
// The tree structure mirrors the island containment: depth=0 is the outermost wall,
// depth=1 is the first inner wall (one Clipper offset inward), etc.
// Holes (islands' inner boundaries) carry is_contour=false and are stored as children
// of the contour loop that surrounds them at their depth level.
//
// [MEMORY] Stack-allocated nodes inside std::vector<PerimeterGeneratorLoop>. The children
// vector holds copies (not pointers), so deep nesting copies entire sub-trees. For typical
// print geometry (5–8 walls, 1–4 holes per island) this is acceptable, but degenerate
// geometry with many concentric rings could cause unexpected memory usage.
//
// [COUPLING] Only used in process_classic() / traverse_loops(). process_arachne() uses
// PerimeterGeneratorArachneExtrusion instead; these two type systems are not interchangeable.
...
// [INTENT] detect_steep_overhang — Determines whether a perimeter extrusion significantly
// overhangs the layer below, using a configurable threshold (fraction of extrusion width).
// "Steep" means more than `threshold` of the extrusion width is unsupported.
// Algorithm: grow lower_slices outward by (threshold - 0.5*width), then diff the extrusion
// against that grown region. If any polyline remains, the extrusion is a steep overhang.
//
// Special case: if overhang_reverse_threshold ≈ 0 (user selected "every even layer"),
// unconditionally marks the extrusion as steep without geometric computation.
//
// [STATE] Mutates steep_overhang_contour / steep_overhang_hole (output-parameters by ref).
//         These booleans accumulate across the layer's perimeters — once set to true,
//         they remain true for all subsequent perimeters on the same layer.
//
// [COUPLING] Template parameter _T must satisfy diff_pl() contract (Polygon or Polylines).
//            Used from both traverse_loops() (Classic) and traverse_extrusions() (Arachne).
//
// [HAZARD] Historical typo: variable named lower_slcier_chopped (missing 'e' in 'slicer').
//          Renaming would not affect compilation (function-local), but translation agents
//          should use the corrected spelling in the target language.
template<class _T>
...
// [INTENT] traverse_loops — Recursively converts the PerimeterGeneratorLoop tree into an
// ExtrusionEntityCollection ready for G-code emission. Handles:
//   1. Extrusion role assignment (erExternalPerimeter vs erPerimeter).
//   2. Loop role assignment (elrInternal for contours with no inner contour children,
//      elrHole for holes, elrDefault otherwise).
//   3. Overhang detection: if detect_overhang_wall is enabled, each loop is split into
//      non-overhang (clipped against grown lower slices) and overhang (erOverhangPerimeter)
//      segments using intersection_pl/diff_pl.
//   4. Fuzzy skin application via apply_fuzzy_skin() before extrusion path creation.
//   5. Thin-wall ThickPolylines are appended to the collection as variable-width extrusions
//      using variable_width() at the first recursion level (thin_walls is cleared after).
//   6. Ordering: chain_extrusion_entities() reorders the flat list of loops + thin walls
//      using nearest-neighbor to minimize travel. The recursive tree traversal inserts
//      children either before (holes) or after (contours) the parent loop.
//
// [STATE] Reads perimeter_generator.m_external_lower_polygons_series,
//         m_lower_polygons_series, m_smaller_external_lower_polygons_series for overhang
//         intensity grading. Mutates steep_overhang_contour/hole (output refs).
//         thin_walls is consumed (cleared) at the first call level.
//
// [MEMORY] coll is stack-allocated; loop pointers inside coll are moved to 'out', with
//          the source slot set to nullptr after transfer (manual move idiom pre-dates
//          unique_ptr use here). Caller receives 'out' by value (move semantics apply).
//
// [HAZARD] chain_and_reorder_extrusion_paths() is called with a raw pointer to
//          paths.front().first_point(). If paths is empty this dereferences a dangling
//          iterator — guarded by the `if(paths.empty()) continue;` check at line ~310.
//          Removing that guard causes a crash on 100% overhang loops.
//
// [HAZARD] Recursive call at line ~362: traverse_loops() calls itself for loop.children.
//          Deeply nested geometry (many islands within islands) can cause stack overflow
//          on low-stack-size platforms. Typical geometry nests at most 8–10 levels.
static ExtrusionEntityCollection traverse_loops(const PerimeterGenerator&      perimeter_generator,
    ...
    for (const PerimeterGeneratorLoop& loop : loops) {
        ...
        if (perimeter_generator.config->detect_overhang_wall &&
            ...
            if (overhangs_reverse && !found_steep_overhang) {
                detect_steep_overhang(perimeter_generator.config, loop.is_contour, bbox, extrusion_width, Polygons{polygon},
                                      perimeter_generator.lower_slices, steep_overhang_contour, steep_overhang_hole);
            }

            Polylines remain_polines;

            Polygons lower_polygons_series_clipped = ClipperUtils::clip_clipper_polygons_with_subject_bbox(lower_polygons_series->back(),
                                                                                                           bbox);
            ...
            if (!inside_polines.empty())
                extrusion_paths_append(paths, std::move(inside_polines), role, extrusion_mm3_per_mm, extrusion_width,
            ...
            if (remain_polines.size() != 0) {
                extrusion_paths_append(paths, std::move(remain_polines), erOverhangPerimeter, perimeter_generator.mm3_per_mm_overhang(),
                                       perimeter_generator.overhang_flow.width(), perimeter_generator.overhang_flow.height());
            }
...
// [INTENT] clip_extrusion — Clips a variable-width Arachne extrusion path (ClipperLib_Z::Path)
// against a set of 2D clip polygons. The Z coordinate in ClipperLib_Z encodes the extrusion
// width at each vertex. After clipping, new intersection vertices lack Z (width) values, so
// this function interpolates them linearly along the original subject edge.
//
// This is critical for overhang detection in process_arachne(): the function is called twice
// per extrusion — once with ctIntersection to get non-overhang segments, once with ctDifference
// to get overhang segments. The Z-interpolated width enables variable_width() to produce
// correct mm3_per_mm for each sub-segment.
//
// [HAZARD] Zero-Z vertex recovery: a fallback loop iterates all subject edges to find the
//          nearest segment to each clipped zero-Z vertex and interpolates. The assert at the
//          end verifies no zero-Z vertices remain. This assert fires in debug builds if the
//          fallback geometry search fails — can mask numerical precision bugs in clip output.
//
// [HAZARD] The ZFillFunction lambda captures by value and runs inside ClipperLib_Z's internal
//          intersection kernel — it must be reentrant and side-effect-free. Any mutable captures
//          would introduce data races if Clipper is ever parallelized.
//
// [COUPLING] Exclusively used by traverse_extrusions() (Arachne path). Classic path uses
//            intersection_pl / diff_pl on simple Polygons instead.
static ClipperLib_Z::Paths clip_extrusion(const ClipperLib_Z::Path&  subject,
                                          const ClipperLib_Z::Paths& clip,
                                          ClipperLib_Z::ClipType     clipType)
{
...
// [INTENT] PerimeterGeneratorArachneExtrusion — Thin wrapper that pairs an Arachne
// ExtrusionLine* with a contour/hole flag. Arachne's ExtrusionLine does not carry this
// flag internally (it only knows is_closed); the is_contour field is assigned during the
// extrusion ordering loop in process_arachne() based on the polygon's winding order.
//
// [MEMORY] extrusion is a raw non-owning pointer into vectors owned by WallToolPaths.
//          WallToolPaths must outlive all PerimeterGeneratorArachneExtrusion instances.
//          The pointed-to ExtrusionLine is mutated in-place when fuzzy skin is applied.
//
// [HAZARD] Default-initialized extrusion=nullptr. Any access before assignment is UB.
//          The ordered_extrusions loop in process_arachne() relies on WallToolPaths being
//          valid for the entire lifetime of the ordering/traversal phase.
...
// [INTENT] traverse_extrusions — Arachne equivalent of traverse_loops(). Converts a flat
// list of PerimeterGeneratorArachneExtrusion (already ordered by process_arachne()) into
// an ExtrusionEntityCollection. Unlike traverse_loops(), no recursive tree traversal is
// needed because Arachne produces a pre-flattened ordered sequence.
//
// For each extrusion:
//   1. Empty extrusions are skipped (Arachne occasionally emits degenerate zero-length lines).
//   2. Overhang detection via clip_extrusion() with ClipperLib_Z for variable-width segments.
//   3. For closed extrusions → ExtrusionLoop; for open → ExtrusionMultiPath.
//   4. Fuzzy skin is applied in-place on extrusion->junctions via apply_fuzzy_skin().
//   5. Start-point selection for open extrusions: prefers non-overhang endpoint with
//      occurrence==1 (endpoint of the chain, not a T-junction).
//
// [STATE] Reads perimeter_generator.lower_slices for overhang clipping.
//         Mutates steep_overhang_contour/hole (output refs, same semantics as traverse_loops).
//         Mutates extrusion->junctions if fuzzy skin applies.
//
// [HAZARD] Disconnected path assertion: assert checks that consecutive paths share endpoints.
//          Comment "there is possibility that due to numerical issue there is poss[ibility]"
//          (truncated comment) acknowledges known numerical cases. In release builds the
//          assertion is skipped, but disconnected paths will produce a visible G-code artifact.
//
// [HAZARD] ExtrusionMultiPath splitting: if consecutive paths are disconnected (last point ≠
//          next first point), a new ExtrusionMultiPath is started. This is a known workaround
//          for Arachne numerical glitches; the resulting multi-path segments may leave micro-
//          gaps at the split points.
static ExtrusionEntityCollection traverse_extrusions(const PerimeterGenerator&                        perimeter_generator,
        ...
        if (perimeter_generator.config->detect_overhang_wall &&
            ...
            if (overhangs_reverse && !found_steep_overhang) {
                ...
                for (const auto& it : recognization_paths) {
                    ...
                    if (detect_steep_overhang(perimeter_generator.config, pg_extrusion.is_contour, extrusion_bboxs, it.first, be_clipped,
                                              perimeter_generator.lower_slices, steep_overhang_contour, steep_overhang_hole)) {
                        break;
                    }
                }
            ...
            // outside the grown lower slices (thus where the distance between
            // the loop centerline and original lower slices is >= half nozzle diameter
            extrusion_paths_append(paths, clip_extrusion(extrusion_path, lower_slices_paths, ClipperLib_Z::ctDifference),
                                   erOverhangPerimeter, perimeter_generator.overhang_flow);
            ...
            extrusion_paths_append(paths, *extrusion, role,
                                   is_external ? perimeter_generator.ext_perimeter_flow : perimeter_generator.perimeter_flow);
        }
...
// [INTENT] split_top_surfaces — Separates regions that will be on the topmost visible surface
// from interior regions, so that top solid infill can use different parameters (e.g. 1 wall).
// Algorithm:
//   1. Compute upper_polygons_series_clipped by offsetting upper_slices polygons outward by
//      min_width_top_surface to grow them slightly (eliminates hairline top areas).
//   2. Detect bridge regions: diff(current layer, lower_slices) → overhanging areas, expanded
//      to detect where the infill would be a bridge. These are excluded from top classification.
//   3. Real top = (current layer NOT bridge) NOT covered_by_upper_layer.
//   4. fill_clip is set to the region one perimeter inward from the polygon boundary
//      (to prevent top infill from reaching the outer wall edge).
//   5. non_top_polygons is the remainder (inner area) that uses normal infill settings.
//
// [STATE] Writes into top_fills (accumulates, union-merged), non_top_polygons, and fill_clip.
//         All three are output parameters. orig_polygons is read-only (const ref).
//
// [COUPLING] Called from both process_classic() and process_arachne() when only_one_wall_top
//            or top_one_wall_type is enabled. Must remain consistent between both callers.
//
// [HAZARD] interface_shells flag changes the source of upper slice data: when enabled,
//          upper_slices_same_region (single print region) is used instead of upper_slices
//          (all regions). The same conditional exists in process_arachne(); divergence
//          between the two paths would cause misaligned top surfaces in multi-material prints.
//
// [HAZARD] offset_top_surface calculation involves nested conditional arithmetic on loop counts.
//          The "if possible, try to not push extra perimeters inside sparse infill" clause
//          subtracts back the perimeter spacing; if config->wall_loops changes between
//          Classic and Arachne invocations, results diverge silently.
void PerimeterGenerator::split_top_surfaces(const ExPolygons& orig_polygons,
                                            ...
                                            ExPolygons&       fill_clip) const
{
    ...
    coord_t offset_top_surface = scale_(
        0.9 * (config->wall_loops.value == 0 ?
                   0. :
                   unscaled(double(ext_perimeter_width + perimeter_spacing * int(int(config->wall_loops.value) - int(1))))));
    ...
    double min_width_top_surface = std::max(double(ext_perimeter_spacing / 2. + 10),
    ...
    if (this->lower_slices != NULL) {
        ...
        const float bridge_margin = std::min(float(scale_(BRIDGE_INFILL_MARGIN)),
    ...
    ExPolygons inner_polygons = diff_ex(orig_polygons,
                                        offset_ex(top_polygons,
                                                  offset_top_surface + min_width_top_surface - double(ext_perimeter_spacing / 2.)),
    ...
    // set the clip to the external wall but go back inside by infill_extrusion_width/2 to be sure the extrusion won't go outside even with
    // a 100% overlap.
...
// Based on:
// https://github.com/prusa3d/PrusaSlicer/blob/c05542590d7c2d73eb69bbf7a82a482a075815c1/src/libslic3r/PerimeterGenerator.cpp#L667-L1071 find
// out if paths touch - at least one point of one path is within limit distance of second path

// [INTENT] Proximity test used by sort_extra_perimeters() to build a dependency graph between
//   overhang perimeter paths. Returns true if any point of either path is within limit_distance
//   of the other path's line segments (bidirectional check).
// [STATE] Pure function; no side effects. Both AABBTreeLines distancers are built locally and
//   destroyed on return — O(n) build per call for n = segment count.
// [HAZARD H1069] O(n*m) when called from sort_extra_perimeters() for all path pairs. For large
//   overhang areas with many extra perimeter segments this becomes a hot inner loop. No caching
//   across calls — every pair rebuilds the AABB tree independently.
...
// [INTENT] Greedily merges adjacent open polylines whose endpoints are within limit_distance
//   of each other into longer connected polylines. Used after intersection_pl() clips perimeter
//   arcs over overhang regions — the clipping creates many short fragments that belong to the
//   same continuous path and should be extruded without a travel move.
// [STATE] Pure function; builds a local unordered_map<size_t,Polyline> working set.
//   The input vector is not modified; a new Polylines result is returned.
// [HAZARD H1070] O(n²) merge loop: for each polyline 'a', iterates all later polylines 'b'.
//   Quadratic cost is acceptable for the small fragment counts produced by overhang clipping,
//   but would degrade for pathological layers with many tiny disconnected segments.
// [HAZARD H1071] Merge is not orientation-aware beyond the four endpoint combinations tested
//   (last-first, last-last, first-last, first-first). For non-manifold or self-intersecting
//   input, the merged result may cross itself without detection.
...
// [INTENT] Topologically sorts the extra overhang perimeter paths so that each unanchored path
//   is printed immediately after one of its already-processed neighbours (i.e., a path it
//   physically touches). This minimises the chance of the extruded line peeling away from the
//   part before it is supported by the adjacent pass. The first index_of_first_unanchored paths
//   are assumed to already be anchored to solid lower-layer geometry and are pre-marked processed.
//   After dependency resolution, a nearest-neighbour travel-minimisation pass reorders the
//   print order within the resolved set and reconnects paths whose endpoints are within
//   2*extrusion_spacing of each other. Paths shorter than 3*extrusion_spacing are discarded.
// [STATE] Pure function; returns a new ExtrusionPaths sorted copy. Input is not modified.
// [COUPLING] Calls paths_touch() for every pair — see H1069 for its cost.
// [HAZARD H1072] If the dependency graph has a cycle (possible when overhangs are ring-shaped),
//   the while(change) loop terminates with some paths still un-processed (their dependency sets
//   are never empty). These paths are simply excluded from sorted_paths and silently dropped,
//   producing missing extrusion segments in the output.
// [HAZARD H1073] The null_idx sentinel is size_t(-1). After a path is "consumed", its
//   dependency set is replaced with {null_idx}. If path_idx ever equals null_idx (wrap-around
//   on a 64-bit system is astronomically unlikely but theoretically possible), the sentinel
//   comparison yields a false positive and terminates the sort early.
...
// [INTENT] Generates additional concentric perimeter paths specifically over overhang regions
//   in the infill_area that are not supported by lower_slices_polygons. The algorithm:
//   1. Computes the overhang region as diff(infill_area, lower_slices_polygons).
//   2. Insets anchor geometry to separate well-supported from overhang regions.
//   3. For each overhang ExPolygon, determines whether bridging is possible via
//      detect_bridging_direction(); if the overhang is bridgeable and small, skips it.
//   4. Otherwise iteratively offsets the perimeter polygon inward, clipping each iteration
//      to the overhang boundary, until no area remains or the polygon stabilises.
//      Uses gap fill (medial_axis) when the remaining area is too thin for a full pass.
//   5. Sorts the resulting paths for anchor-first printing via sort_extra_perimeters().
//   Returns: vector of ExtrusionPaths per overhang region + Polygons covered by those paths.
// [STATE] Pure function; all working polygons are local. No PerimeterGenerator member state.
// [COUPLING] Calls detect_bridging_direction() (BridgeDetector), reconnect_polylines(),
//   sort_extra_perimeters(), ExPolygon::medial_axis(). The EXTRA_PERIMETER_OFFSET_PARAMETERS
//   macro hardwires ClipperLib::jtSquare with miter-limit 0 for all offset calls — changing
//   to jtMiter or jtRound here would require re-validation of the anchor geometry.
// [HAZARD H1074] The continuation_loops counter (= 2) is a magic constant controlling how many
//   inward passes are generated after the overhang is fully covered. Changing it affects print
//   quality on steep overhangs and is not exposed to user configuration.
// [HAZARD H1075] If detect_bridging_direction() misjudges bridgeability (unsupp_dist threshold
//   is 20% of total length), potentially bridgeable regions are skipped and left without extra
//   perimeters — visible as sagging on moderate slopes near anchor edges.
...
// [INTENT] Integrates the output of generate_extra_perimeters_over_overhangs() into the
//   existing perimeter and fill-surface collections for this layer region.
//   Prepends the extra perimeter paths to the front of the last island's ExtrusionEntityCollection
//   (so they print before the regular loops) and subtracts the area they cover from fill_surfaces
//   to avoid double-filling.
// [STATE] Mutates: this->loops (the last entity in loops->entities is modified in place),
//         this->fill_surfaces (each surface has its expolygon differenced with filled_area).
// [COUPLING] Depends on the invariant that this->loops->entities.back() is the current island's
//   ExtrusionEntityCollection. If process_classic() or process_arachne() changes the island
//   collection order, apply_extra_perimeters() silently modifies the wrong island.
// [HAZARD H1076] No guard against loops->entities being empty (e.g. if an island produced zero
//   regular perimeters). The static_cast and swap on entities.back() would then operate on
//   the previous island's collection or crash with UB.
void PerimeterGenerator::apply_extra_perimeters(ExPolygons& infill_area)
{
    if (!m_spiral_vase && this->lower_slices != nullptr && this->config->detect_overhang_wall &&
        this->config->extra_perimeters_on_overhangs && this->config->wall_loops > 0 && this->layer_id > this->object_config->raft_layers) {
...
// [INTENT] Reverses the winding direction of perimeter loops to match overhang compensation
//   requirements detected by detect_steep_overhang(). Steep overhangs print better when the
//   nozzle moves in the direction that lays fresh material onto already-solidified material
//   rather than pushing into free air. steep_overhang_contour/hole are booleans from the
//   detect_steep_overhang() scan of the current layer region.
//   reverse_internal_only = true skips external perimeter loops (erExternalPerimeter paths),
//   useful when the outer wall sequence must be preserved for surface quality.
// [STATE] Mutates entities in place (calls eloop->make_clockwise() on matching loops).
// [COUPLING] Depends on ExtrusionLoop::loop_role() returning elrHole for hole loops.
//   The elrHole flag is set during traverse_loops() — if that logic changes, the role check
//   here silently reverses the wrong loops.
// [HAZARD H1077] The function iterates entity->is_loop() but does not recurse into nested
//   ExtrusionEntityCollections. Any loops nested inside a collection (e.g. from Arachne) are
//   skipped — steep overhang reversal is not applied to them.
// Reorient loop direction
static void reorient_perimeters(ExtrusionEntityCollection& entities,
                                bool                       steep_overhang_contour,
                                bool                       steep_overhang_hole,
                                bool                       reverse_internal_only)
{
...
void PerimeterGenerator::process_classic()
{
    ...
    this->smaller_ext_perimeter_flow = this->smaller_ext_perimeter_flow.with_width(
        SCALING_FACTOR * (ext_perimeter_width - 0.5 * SMALLER_EXT_INSET_OVERLAP_TOLERANCE * ext_perimeter_spacing));
    ...
    double surface_simplify_resolution = (print_config->enable_arc_fitting && !this->has_fuzzy_skin) ? 0.2 * m_scaled_resolution :
                                                                                                       m_scaled_resolution;
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        if (this->config->alternate_extra_wall && this->layer_id % 2 == 1 && !m_spiral_vase &&
            sparse_infill_density > 0) // add alternating extra wall
            loop_number++;
        ...
        if (loop_number >= 0) {
            ...
            for (int i = 0;; ++i) { // outer loop is 0
                ...
                if (i == 0) {
                    ...
                    if (this->config->detect_thin_wall) {
                        ...
                        offsets = offset2_ex(last, -float(ext_perimeter_width / 2. + ext_min_spacing / 2. - 1),
                        ...
                        ExPolygons expp      = opening_ex(
                            ...
                            diff_ex(last, offset(offsets, float(ext_perimeter_width / 2.) + ClipperSafetyOffset)), float(min_width / 2.));
                        ...
                        for (const ExPolygon& expolygon : last) {
                            ...
                            if (offset_result.empty() && expolygon.area() < (double) (ext_perimeter_width + ext_min_spacing_smaller) *
                                                                                scale_(narrow_loop_length_threshold)) {
                    ...
                    if (has_gap_fill)
                        ...
                        append(gaps,
                               diff_ex(offset(last, -float(0.5 * distance)), offset(offsets, float(0.5 * distance + 10)))); // safety offset
                }
            ...
            ExtrusionEntityCollection entities = traverse_loops(*this, contours.front(), thin_walls, steep_overhang_contour,
                                                                steep_overhang_hole);
            ...
            if (is_outer_wall_first ||
                ...
                (this->layer_id == 0 && this->object_config->brim_type == BrimType::btOuterOnly &&
                    ...
                    while (position < reordered_extrusions.size()) {
                        ...
                        for (arr_i = position; arr_i < reordered_extrusions.size(); ++arr_i) {
                            ...
                            if (outer > -1 && first_internal > -1 && second_internal > -1 &&
                                reordered_extrusions.entities[arr_i]->inset_idx ==
                                    0) { // found a new external perimeter after we've found all three perimeters to re-order -> this means
                                         // we entered a new island.
                                arr_i        = arr_i - 1; // step back one perimeter
                                max_internal = arr_i;     // new maximum internal perimeter is now this as we have found a new external
                                                          // perimeter, hence a new island.
                        ...
                        if (outer > -1 && first_internal > -1 && second_internal > -1) { // found perimeters to re-order?
                            ...
                            for (arr_j = max_internal; arr_j >= position;
                                 --arr_j) { // go inside out towards the external perimeter (perimeters in reverse order) and store all
                                            // internal perimeters until the first one identified with inset index 2
                            ...
                            for (arr_j = position; arr_j < second_internal;
                                 ++arr_j) { // go outside in and map the remaining perimeters (external and first internal wall(s)) using
                                            // the outside in wall order
                                inner_outer_extrusions.append(*reordered_extrusions.entities[arr_j]);
                            }
        ...
        if (!gaps.empty()) {
            ...
            ExPolygons gaps_ex = diff_ex(
                ...
                opening_ex(gaps, float(min / 2.)), offset2_ex(gaps, -float(max / 2.), float(max / 2. + ClipperSafetyOffset)));
        ...
        if (inset > 0) {
            if (this->layer_id == 0 || this->upper_slices == nullptr) {
                infill_peri_overlap = coord_t(
                    scale_(this->config->top_bottom_infill_wall_overlap.get_abs_value(unscale<double>(inset + solid_infill_spacing / 2))));
            } else {
                infill_peri_overlap = coord_t(
                    scale_(this->config->infill_wall_overlap.get_abs_value(unscale<double>(inset + solid_infill_spacing / 2))));
                top_infill_peri_overlap = coord_t(
                    scale_(this->config->top_bottom_infill_wall_overlap.get_abs_value(unscale<double>(inset + solid_infill_spacing / 2))));
            }
        ...
        ExPolygons infill_exp = offset2_ex(not_filled_exp, float(-inset - min_perimeter_infill_spacing / 2.),
        ...
        // BBS: get the no-overlap infill expolygons
        {
            ...
            if (min_perimeter_infill_spacing / 2 > infill_peri_overlap)
                polyWithoutOverlap = offset2_ex(not_filled_exp, float(-inset - min_perimeter_infill_spacing / 2.),
                                                float(min_perimeter_infill_spacing / 2 - infill_peri_overlap));
            else
                polyWithoutOverlap = offset_ex(not_filled_exp, double(-inset - infill_peri_overlap));
            if (!top_fills.empty())
...
// BBS:
// [INTENT] Registers the infill boundary for an Arachne-generated island into fill_surfaces
//   and fill_no_overlap. The boundary is the region inside the innermost perimeter, adjusted
//   by an infill_wall_overlap offset so the infill slightly overlaps the inner perimeter wall
//   for better adhesion. An offset2 double-pass is used: first contract by
//   min_perimeter_infill_spacing/2 to collapse any sub-spacing slivers, then re-expand by
//   insert + min_perimeter_infill_spacing/2 to restore the boundary at the correct inset.
// [STATE] Mutates: this->fill_surfaces (appends stInternal surface), this->fill_no_overlap.
// [COUPLING] min_perimeter_infill_spacing is passed by process_arachne() as:
//   std::min(ext_perimeter_spacing, perimeter_spacing) — a change to this formula affects how
//   much of the island interior is exposed for infill versus being consumed by the perimeter wall.
// [HAZARD H1078] If offset_ex(infill_contour, -spacing/2) is empty but the contour is not
//   actually too small (e.g. due to a Clipper precision glitch), the contour is silently
//   discarded (infill_contour.clear()). This produces a hollow island with no infill and no
//   diagnostic message.
                                                        ...
                                                        bool       is_inner_part)
{
    ...
    this->fill_surfaces->append(offset2_ex(union_ex(inner_pp), float(-min_perimeter_infill_spacing / 2.),
                                           float(insert + min_perimeter_infill_spacing / 2.)),
                                stInternal);

    append(*this->fill_no_overlap,
           offset2_ex(union_ex(inner_pp), float(-min_perimeter_infill_spacing / 2.), float(+min_perimeter_infill_spacing / 2.)));
}

// Orca: sacrificial bridge layer algorithm ported from SuperSlicer
// [INTENT] Implements "counterbore hole bridging": for surfaces that overhang lower-layer air
//   (typically counterbore holes), detects whether bridging is possible and if so reclassifies
//   those surfaces as stInternal bridge fill instead of perimeter geometry. This prevents the
//   slicer from trying to generate perimeters over unsupported voids, which would sag.
//   Controlled by counterbore_hole_bridging config:
//     chbNone    — disabled; function is a no-op.
//     chbFilled  — marks the entire (bridgeable + nearby) region as bridge, removes inner holes.
//     chbBridges — marks only the bridgeable subset (after filtering non-bridgeable bits).
//   Modifies all_surfaces in place: surfaces whose expolygons overlap unsupported regions are
//   shrunk or split to exclude the bridge area; new bridge surfaces are appended to fill_surfaces.
// [STATE] Mutates: all_surfaces (passed by reference), this->fill_surfaces (appends stInternal).
// [COUPLING] Calls BridgeDetector::detect_angle() and coverage(). The BridgeDetector is
//   constructed with lower_island.expolygons as the support. If lower_slices is null or empty
//   the function returns immediately — no bridging analysis is performed.
// [HAZARD H1079] The chbFilled branch iterates all_surfaces while appending to it (lines ~2084).
//   The loop uses explicit index arithmetic (surface_idx_other--) to handle in-place erasure,
//   but if two surfaces overlap the same bridge region simultaneously, one may be erased and
//   re-indexed while the other is still being processed — potential iterator invalidation if
//   the vector reallocates during push_back.
// [HAZARD H1080] Commented-out code referencing upper_slices suggests an abandoned check for
//   whether bridged holes would be covered by the upper layer. The check is disabled with no
//   fallback, so holes that would be naturally roofed are treated identically to open holes.
// [HAZARD H1081] The convexity check (contour_simplified.size() == 1 && concave_points().size() == 0)
//   only accepts purely convex holes for chbFilled bridging. Slightly non-convex counterbore
//   holes (e.g. due to STL tessellation) are silently excluded and printed without bridge infill.
void PerimeterGenerator::process_no_bridge(Surfaces& all_surfaces, coord_t perimeter_spacing, coord_t ext_perimeter_width)
{
    ...
    if (this->config->counterbore_hole_bridging != chbNone && this->lower_slices != NULL && !this->lower_slices->empty()) {
                ...
                if (!unsupported_filtered.empty()) {
                    ...
                    if (!unsupported.empty()) {
                        ...
                        for (ExPolygon unsupported : unsupported_filtered) {
                            BridgeDetector detector{unsupported, lower_island.expolygons, perimeter_spacing};
                        ...
                        if (!bridgeable.empty()) {
                            ...
                            if (/*this->config->counterbore_hole_bridging.value == chbNoPeri || */ this->config->counterbore_hole_bridging
                                    .value == chbFilled) {
                                ...
                                for (size_t i = 0; i < unsupported_filtered.size();) {
                                    ...
                                    if (contour_bigger.size() == 1)
                                        poly_unsupp_bigger.contour = contour_bigger[0];

                                    // check convex, has some bridge, not overhang
                                    if (contour_simplified.size() == 1 && contour_bigger.size() == 1 &&
                                        contour_simplified[0].concave_points().size() == 0 &&
                                        intersection_ex(bridgeable, ExPolygons{poly_unsupp}).size() > 0 &&
                                        diff_ex(ExPolygons{poly_unsupp_bigger},
                                                union_ex(last, offset_ex(bridgeable, bridged_infill_margin + perimeter_spacing / 2)),
                                                ApplySafetyOffset::Yes)
                                                .size() == 0) {
                                        // ok, keep it
                                        i++;
                                ...
                                unsupported_filtered = intersection_ex(last,
                                                                       offset2_ex(unsupported_filtered, double(-perimeter_spacing / 2),
                                ...
                                if (this->config->counterbore_hole_bridging.value == chbFilled) {
                                    for (ExPolygon& expol : unsupported_filtered) {
                                        ...
                                        //     float perimeterwidth = this->config->perimeters == 0 ? 0 : (this->ext_perimeter_flow.scaled_width()
                                        //     + (this->config->perimeters - 1) + this->perimeter_flow.scaled_spacing()); std::cout << "test
                                        //     upper slices with perimeterwidth=" << perimeterwidth << "=>" <<
                                        //     offset_ex(this->upper_slices->expolygons, -perimeterwidth).size(); if (intersection(Polygons()
                                        //     = { expol.holes }, to_polygons(offset_ex(this->upper_slices->expolygons,
                                        //     -this->ext_perimeter_flow.scaled_width() / 2))).empty()) {
                                        //         std::cout << " EMPTY";
                                        //         expol.holes.clear();
                                        //     } else {
                                        //     }
                                        ...
                                        for (size_t surface_idx_other = 0; surface_idx_other < all_surfaces.size(); surface_idx_other++) {
                                            if (surface_idx == surface_idx_other)
                                                continue;
                                            if (intersection_ex(ExPolygons() = {expol},
                                                                ExPolygons() = {all_surfaces[surface_idx_other].expolygon})
                                                    .size() > 0) {
                                                // this means that other_surf was inside an expol holes
                                                // as we removed them, we need to add a new one
                                                ExPolygons new_poly = offset2_ex(ExPolygons{all_surfaces[surface_idx_other].expolygon},
                            ...
                            } else if (/*this->config->counterbore_hole_bridging.value == chbBridgesOverhangs || */ this->config
                                           ->counterbore_hole_bridging.value == chbBridges) {
                                ...
                                // bridgeable_simplified = offset2_ex(bridgeable_simplified, (double)-perimeter_spacing,
                                // (double)perimeter_spacing * 2); ExPolygons unbridgeable = offset_ex(diff_ex(unsupported,
                                // bridgeable_simplified), perimeter_spacing * 3 / 2); ExPolygons unbridgeable =
                                // intersection_ex(unsupported, diff_ex(unsupported_filtered, offset_ex(bridgeable_simplified,
                                // ext_perimeter_width / 2))); unbridgeable = offset2_ex(unbridgeable, -ext_perimeter_width,
                                // ext_perimeter_width);
                                ...
                                ExPolygons bridges_temp = offset2_ex(intersection_ex(last, diff_ex(unsupported_filtered, unbridgeable),
                                                                                     ApplySafetyOffset::Yes),
                                ...
                                while (offset_to_do > ext_perimeter_width * 1.5) {
                                    unbridgeable = offset2_ex(unbridgeable, -ext_perimeter_width / 4, ext_perimeter_width * 2.25,
                                                              ClipperLib::jtSquare);
                                ...
                                // } else {
                                //     ExPolygons unbridgeable = intersection_ex(unsupported, diff_ex(unsupported_filtered,
                                //     offset_ex(bridgeable_simplified, ext_perimeter_width / 2))); unbridgeable = offset2_ex(unbridgeable,
                                //     -ext_perimeter_width, ext_perimeter_width); unsupported_filtered = unbridgeable;
                    ...
                    if (!unsupported_filtered.empty()) {
                        ...
                        this->fill_surfaces->append(unsupported_filtered, stInternal);
                        ...
                        for (int i = 0; i < last.size(); i++) {
                            if (intersection_ex(support, ExPolygons() = {last[i]}).empty()) {
                                this->fill_surfaces->append(ExPolygons() = {last[i]}, stInternal);
 ...
 * @param considered_inset_idx What perimeter inset index are we searching for (eg. if we are searching for first internal perimeters
 ...
 *
 * [INTENT] BFS-like helper: given a set of "reference" extrusion indices, finds all other
 *   extrusions at a specific inset_idx level whose geometry is within threshold distance of
 *   any reference extrusion. Used by reorderPerimetersByProximity() to build level-by-level
 *   neighbour lists from external wall inward.
 * [STATE] Pure function; no side effects. Builds no persistent data structures.
 * [HAZARD H1082] O(|referenceIndices| * |entities|) per call; for islands with many extrusions
 *   (e.g. thin walls generating dozens of Arachne segments) this accumulates to O(n²) across
 *   all levels of reorderPerimetersByProximity(). No spatial index is used.
 * [HAZARD H1083] Extrusions with inset_idx == 0 are unconditionally skipped even when they
 *   are legitimately close to a reference. This means outer-wall paths are never moved by
 *   reorderPerimetersByProximity() — they remain in their original order regardless of proximity.
 */
 ...
 *
 * This approach finds all perimeters touching the external perimeter first and then finds all perimeters touching these new ones until none
 * are left It ensures a level-by-level traversal, similar to BFS in graph theory.
 *
 * @param entities The list of PerimeterGeneratorArachneExtrusion entities.
 ...
 *
 * [INTENT] Reorders an Arachne extrusion list so that for each external (inset_idx == 0)
 *   perimeter, its immediately adjacent inner perimeters (inset_idx 1) are printed next,
 *   then the next level, and so on, in a BFS wave from outside in. This produces the
 *   "Inner Outer Inner" wall ordering variant that minimises travel between adjacent walls.
 *   Any extrusions not reached by the BFS (disconnected islands) are appended at the end.
 * [STATE] Takes entities by value — the input list is copied and the reordered copy returned.
 * [COUPLING] Delegates proximity queries to findAllTouchingPerimeters(); see H1082–H1083.
 * [HAZARD H1084] The "bring largest first-level perimeter to end" heuristic (std::iter_swap to
 *   end-1) swaps the last two elements of firstLevelTouchingIndices. If there is only one
 *   element, iter_swap with end-1 == begin is a no-op, which is correct. But the intent
 *   comment says "to the front" while the code places it at end-1 (i.e., the last to be
 *   printed among first-level neighbours) — possible intent/implementation mismatch.
 */
std::vector<PerimeterGeneratorArachneExtrusion> reorderPerimetersByProximity(std::vector<PerimeterGeneratorArachneExtrusion> entities,
                                                                             size_t threshold_external,
                                                                             size_t threshold_internal)
{
    ...
    auto reorderFromReference = [&](int referenceIndex) {
        ...
        std::vector<int> firstLevelTouchingIndices = findAllTouchingPerimeters(entities, firstLevelIndices, threshold_external,
        ...
        while (true) {
            ...
            std::vector<int> currentLevelTouchingIndices = findAllTouchingPerimeters(entities, currentLevelIndices, threshold_external,
                                                                                     threshold_internal, currentInsetIndex);
            ...
            currentLevelTouchingIndices.erase(std::remove_if(currentLevelTouchingIndices.begin(), currentLevelTouchingIndices.end(),
...
/**
 * @brief Reorders the vector to bring external perimeter (i.e. paths with inset index 0) that are also contours (i.e. external facing
 * lines) to the front.
 *
 ...
 *
 * [INTENT] Post-processing step applied to ordered_extrusions immediately before committing
 *   them to the perimeter EntityCollection in process_arachne(). Ensures outer-wall contours
 *   (inset_idx == 0 and is_contour() == true) are printed first in their island, matching
 *   the OuterInner wall ordering mode expectation without disturbing the intra-level order
 *   established by reorderPerimetersByProximity().
 * [STATE] Mutates ordered_extrusions in place (stable_partition, preserves relative order).
 * [HAZARD H1085] When wall_sequence == InnerOuter the contours are at the front here, but
 *   process_arachne() may also reverse the outer/inner ordering via is_outer_wall_first logic.
 *   If bringContoursToFront() is called regardless of wall_sequence, it may conflict with the
 *   is_outer_wall_first=false (InnerOuter) path, undoing the intended inner-first ordering.
 */
...
void PerimeterGenerator::process_arachne()
{
    ...
    for (const Surface& surface : all_surfaces) {
        ...
        Arachne::WallToolPaths wallToolPaths(last_p, bead_width_0, perimeter_spacing, coord_t(loop_number + 1), wall_0_inset, layer_height,
                                             input_params_tmp);
        ...
        if (inner_loop_number >= 0) {
            ...
            if (!top_expolygons.empty()) {
                if (lower_slices != nullptr) {
                    ...
                    const Polygons   lower_slices_clipped   = ClipperUtils::clip_clipper_polygons_with_subject_bbox(*lower_slices,
                                                                                                                    infill_contour_bbox);
                ...
                const float top_surface_min_width =
                    std::max<float>(float(ext_perimeter_spacing) / 4.f + scaled<float>(0.00001),
                                    float(scale_(config->min_width_top_surface.get_abs_value(unscale_(perimeter_width)))) / 4.f);
                ...
                Arachne::WallToolPaths                   inner_wall_tool_paths(not_top_polygons, perimeter_spacing, perimeter_spacing,
                                                                               coord_t(inner_loop_number + 1), 0, layer_height, input_params_tmp);
                ...
                Arachne::WallToolPaths no_single_perimeter_tool_paths(last_p, bead_width_0, perimeter_spacing,
                                                                      coord_t(inner_loop_number + 2), wall_0_inset, layer_height,
                                                                      input_params_tmp);
        ...
        {
            static int iRun = 0;
            export_perimeters_to_svg(debug_out_path("arachne-perimeters-%d-%d.svg", layer_id, iRun++), to_polygons(last), perimeters,
        ...
        std::vector<size_t>              blocked(all_extrusions.size(),
                                                 0); // Value indicating how many extrusions it is blocking (preceding extrusions) an extrusion.
        std::vector<std::vector<size_t>> blocking(
            all_extrusions.size()); // Each extrusion contains a vector of extrusions that are blocked by this extrusion.
        ...
        Point             current_position = all_extrusions.empty() ? Point::Zero() :
                                                                      all_extrusions.front()->junctions.front().p; // Some starting position.
        ...
        while (ordered_extrusions.size() < all_extrusions.size()) {
            ...
            std::sort(available_candidates.begin(), available_candidates.end(),
                      [&all_extrusions](const size_t a_idx, const size_t b_idx) -> bool {
            ...
            for (const size_t candidate_path_idx : available_candidates) {
                ...
                if (path->junctions
                        .empty()) { // No vertices in the path. Can't find the start position then or really plan it in. Put that at the end.
                ...
                if (distance_sqr < best_distance_sqr) { // Closer than the best candidate so far.
                    if (path->is_closed || (!path->is_closed && best_distance_sqr != std::numeric_limits<double>::max()) ||
            ...
            if (!best_path->junctions
                     .empty()) { // If all paths were empty, the best path is still empty. We don't upate the current position then.
        ...
        if (this->config->wall_sequence == WallSequence::InnerOuterInner &&
            layer_id > 0) {                      // only enable inner outer inner algorithm after first layer
            if (ordered_extrusions.size() > 2) { // 3 walls minimum needed to do inner outer inner ordering
                int position = 0;                // index to run the re-ordering for multiple external perimeters in a single island.
                int arr_i, arr_j = 0;            // indexes to run through the walls in the for loops
                int outer, first_internal, second_internal, max_internal, current_perimeter; // allocate index values

                // To address any remaining scenarios where the outer perimeter contour is not first on the list as arachne sometimes
                // reorders the perimeters when clustering for OI mode that is used the basis for IOI
                ...
                // Get searching thresholds. For an external perimeter we take the external perimeter spacing/2 plus the internal perimeter
                // spacing/2 and expand by the factor rounding errors. When precise wall is enabled, the external perimeter full spacing is used.
                ...
                // For the intenal perimeter threshold, the distance is the internal perimeter spacing expanded by the factor to cover
                // rounding errors.
                ...
                reordered_extrusions = ordered_extrusions; // copy them into the reordered extrusions vector to allow for IOI operations to
                ...
                while (position < reordered_extrusions.size()) {
                    ...
                    for (arr_i = position; arr_i < reordered_extrusions.size(); ++arr_i) {
                        ...
                        if (outer > -1 && first_internal > -1 &&
                            reordered_extrusions[arr_i].extrusion->inset_idx ==
                                0) { // found a new external perimeter after we've found at least a first internal perimeter to re-order.
                                     // This means we entered a new island.
                            arr_i        = arr_i - 1; // step back one perimeter
                            max_internal = arr_i; // new maximum internal perimeter is now this as we have found a new external perimeter,
                                                  // hence a new island.
                            break;                // exit the for loop
                        }
                    }
                    ...
                    if (outer > -1 && first_internal > -1 &&
                        second_internal >
                            -1) { // found all three perimeters to re-order? If not the perimeters will be processed outside in.
                        ...
                        inner_outer_extrusions.resize(
                            ...
                            1); // reserve array containing the number of perimeters before a new island. Variables
                                // are array indexes hence need to add +1 to convert to position allocations
                        // printf("Allocated array size %d, max_internal index %d, start position index %d
                        // \n",max_internal-position+1,max_internal,position);

                        for (arr_j = max_internal; arr_j >= position;
                             --arr_j) { // go inside out towards the external perimeter (perimeters in reverse order) and store all internal
                                        // perimeters until the first one identified with inset index 2
                            if (arr_j >= second_internal) {
                        ...
                        for (arr_j = position; arr_j < second_internal;
                             ++arr_j) { // go outside in and map the remaining perimeters (external and first internal wall(s)) using the
                                        // outside in wall order
        ...
        if (ExtrusionEntityCollection extrusion_coll = traverse_extrusions(*this, ordered_extrusions, steep_overhang_contour,
                                                                           steep_overhang_hole);
        ...
        ExPolygons infill_exp = offset2_ex(not_filled_exp, float(-min_perimeter_infill_spacing / 2.),
        ...
        {
            ExPolygons polyWithoutOverlap;
            polyWithoutOverlap = offset2_ex(not_filled_exp, float(-min_perimeter_infill_spacing / 2.),
                                            float(+min_perimeter_infill_spacing / 2.));
            if (!top_expolygons.empty())
...
// [INTENT] Predicate used during traverse_loops() to identify the innermost contour loop in a
//   perimeter nest (a contour that has no contour children — only hole children or no children).
//   This loop receives the erExternalPerimeter role for its inner-facing side in some wall
//   sequence modes and is also used to clip infill boundaries.
// [STATE] Pure const method; reads this->is_contour and this->children only.
// [COUPLING] Called from traverse_loops() when assigning erExternalPerimeter to inner walls.
//   If the PerimeterGeneratorLoop tree has incorrect is_contour flags (set during loop
//   classification in process_classic()), is_internal_contour() silently misclassifies loops.
...
// [INTENT] Pre-computes two offset levels of the lower layer's slices for use in overhang
//   percentage detection (detect_steep_overhang). The two offsets span from just inside the
//   perimeter width (start_offset = -0.5 * width) to just outside the nozzle radius
//   (end_offset = 0.5 * nozzle_diameter), with a single intermediate sample at 1/2 of the
//   sampling interval. Stored in m_lower_polygons_series, m_external_lower_polygons_series,
//   and m_smaller_external_lower_polygons_series (each called with a different `width`).
// [STATE] Called from process_classic() and process_arachne() to populate the m_*_series
//   member arrays. Returns empty vector if lower_slices is null.
// [COUPLING] overhang_sampling_number must be >= 3 (asserted). The series has exactly 2
//   entries: index 0 = inner sample, index 1 = outer (nozzle-edge) sample. detect_steep_overhang()
//   indexes into these arrays at positions [0] and [1] — any change to offset_series size
//   requires matching changes in detect_steep_overhang().
// [HAZARD H1086] offset_series.reserve(2) then push_back twice is correct, but the loop at
//   line ~2921 uses offset_series.size() which is dynamically sized — if the reserve is
//   changed without updating the push_back calls, the loop processes the wrong count silently.
