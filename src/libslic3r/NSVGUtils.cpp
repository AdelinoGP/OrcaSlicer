// [INTENT] SVG parsing and linearization bridge: converts NSVGimage (nanosvg parsed tree)
// into Slic3r ExPolygon geometry for embossing/SVG-import use.
// Handles both filled shapes (fill_to_expolygons) and stroked shapes (stroke_to_expolygons),
// including dashed strokes. Bezier curves are recursively subdivided to polylines
// via flatten_cubic_bez (de Casteljau algorithm, inspired by nanosvgrast.h).
// [COUPLING] Depends on nanosvg (NSVGimage/NSVGshape/NSVGpath), ClipperUtils (offset/contour),
// and Emboss::heal_polygons for topology repair.
// [STATE] No persistent mutable state; all functions are stateless transforms.
// [HAZARD H1172] The large commented-out save() block (lines 150-249) contains dead code
// that includes <charconv> solely for std::to_chars in that block. If the block is
// re-enabled, it emits PrusaSlicer copyright text in the SVG comment, not OrcaSlicer.
// [HAZARD H1173] stroke_to_expolygons passes `mitter` (= std::pow(tesselation_tolerance, 1/3.))
// as ArcTolerance for jtRound joins, but the variable is named after MiterLimit semantics;
// the formula cube-root is undocumented rationale — may produce visually wrong round joins.
...
ExPolygonsWithIds create_shape_with_ids(const NSVGimage& image, const NSVGLineParams& param)
{
    ...
    for (NSVGshape* shape_ptr = image.shapes; shape_ptr != NULL; shape_ptr = shape_ptr->next, ++shape_id) {
        ...
        bool is_stroke_used = shape.stroke.type != NSVG_PAINT_NONE && shape.strokeWidth > 1e-5f;
...
bool need_flattening(float tessTol, const Vec2f& p1, const Vec2f& p2, const Vec2f& p3, const Vec2f& p4)
{
    // f .. first
    // s .. second
    auto det = [](const Vec2f& f, const Vec2f& s) { return std::fabs(f.x() * s.y() - f.y() * s.x()); };
...
bool is_line(const float* p, float precision)
{
    ...
    return is_approx(p[2], p[0] + dx_3, precision) && is_approx(p[4], p[6] - dx_3, precision) && is_approx(p[3], p[1] + dy_3, precision) &&
...
LinesPath linearize_path(NSVGpath* first_path, const NSVGLineParams& param)
{
    ...
    for (NSVGpath* path = first_path; path != NULL; path = path->next) {
        ...
        for (size_t i = 0; i < path_size; i += 3) {
            ...
            Vec2f p3(p[4], p[5]);
            Vec2f p4(p[6], p[7]);
            flatten_cubic_bez(points, param.tesselation_tolerance, p1 * param.scale, p2 * param.scale, p3 * param.scale, p4 * param.scale,
                              param.max_level);
        }
        assert(!points.empty());
        if (points.empty())
...
struct DashesParam
{
    ...
    explicit DashesParam(const NSVGshape& shape, double scale) : dash_count(shape.strokeDashCount)
    {
        assert(dash_count > 0);
