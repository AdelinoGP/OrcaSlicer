...
bool        is_equal(float lh, float rh) { return abs(lh - rh) <= epson; }

bool is_equal_for_sort(float lh, float rh) { return abs(lh - rh) <= 1e-8; }

bool is_equal(const Vec3f& lh, const Vec3f& rh) { return is_equal(lh[0], rh[0]) && is_equal(lh[1], rh[1]) && is_equal(lh[2], rh[2]); }
    ...
    for (int j = 0; j < 3; ++j) { // loop through facet edges
        ...
        if (a->z() == slice_z && b->z() == slice_z) {
            ...
            if (horizontal) {
                ...
                // (the bottom most edge resp. vertex of a triangle is not owned by the triangle, but the top most edge resp. vertex is part
                // of the triangle in respect to the cutting plane).
...
{
    stl_vertex vertices[3]{transform_vertex_fn(mesh_vertices[indices(0)]), transform_vertex_fn(mesh_vertices[indices(1)]),
                           transform_vertex_fn(mesh_vertices[indices(2)])};
...
static inline std::vector<IntersectionLines> slice_make_lines(const std::vector<stl_vertex>&                  vertices,
                                                              const TransformVertex&                          transform_vertex_fn,
                                                              ...
                                                              const ThrowOnCancel                             throw_on_cancel_fn)
{
    ...
    std::array<std::mutex, 64>     lines_mutex;
    tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())), [&vertices, &transform_vertex_fn, &indices, &face_edge_ids, &zs,
                                                                        &lines, &lines_mutex,
                                                                        throw_on_cancel_fn](const tbb::blocked_range<int>& range) {
...
static inline IntersectionLines slice_make_lines(const std::vector<stl_vertex>&                  mesh_vertices,
                                                 const TransformVertex&                          transform_vertex_fn,
                                                 ...
                                                 FaceFilter                                      face_filter)
{
    ...
    for (int face_idx = 0; face_idx < int(mesh_faces.size()); ++face_idx)
        if (face_filter(face_idx)) {
            ...
            int              idx_vertex_lowest = (vertices[1].z() == min_z) ? 1 : ((vertices[2].z() == min_z) ? 2 : 0);
            IntersectionLine il;
            // Ignore horizontal triangles. Any valid horizontal triangle must have a vertical triangle connected, otherwise the part has
            // zero volume.
            if (min_z != max_z &&
                slice_facet(plane_z, vertices, indices, face_edge_ids[face_idx], idx_vertex_lowest, false, il) == FacetSliceType::Slicing) {
                assert(il.edge_type != IntersectionLine::FacetEdgeType::Horizontal);
    ...
    std::array<std::mutex, 64>& lines_mutex)
{
    ...
    if (min_layer == max_layer || horizontal) {
        ...
        assert(
            horizontal || zs.empty() || max_z < zs.front() || min_z > zs.back() ||
        ...
        if (horizontal && min_layer != zs.end() && *min_layer == min_z) {
...
#else
            // Project the coplanar bottom facing triangles to the plane above the slicing plane to match the behavior of slice_mesh() /
            // slice_mesh_ex(), where the slicing plane slices the top facing surfaces, but misses the bottom facing surfaces.
            if (size_t line_id = ProjectionFromTop ? slice_id : slice_id + 1; ProjectionFromTop || line_id < lines.at_slice.size())
                ...
                for (int iedge = 0; iedge < 3; ++iedge)
                    if (facet_neighbors(iedge) == -1) {
                        ...
                        IntersectionLine il{{to_2d(vertices[i]).cast<coord_t>(), to_2d(vertices[j]).cast<coord_t>()},
                                            indices(i),
                                            indices(j),
                                            -1,
                                            -1,
                                            ProjectionFromTop ? IntersectionLine::FacetEdgeType::Bottom :
                                                                IntersectionLine::FacetEdgeType::Top};
            ...
            for (int iedge = 0; iedge < 3; ++iedge)
                if (facet_neighbors(iedge) == -1) {
                    ...
                    emit_slab_edge(IntersectionLine{{to_2d(vertices[i]).cast<coord_t>(), to_2d(vertices[j]).cast<coord_t>()},
                                                    indices(i),
                                                    indices(j),
                                                    -1,
                                                    -1,
                                                    IntersectionLine::FacetEdgeType::Slab},
            ...
            if (type == FacetSliceType::NoSlice) {
                ...
                if (il.edge_type == IntersectionLine::FacetEdgeType::Top || il.edge_type == IntersectionLine::FacetEdgeType::Bottom) {
                    ...
                    if (neighbor_idx == -1) {
                        ...
                        float                              z        = *it;
                        int num_on_plane = (mesh_vertices[neighbor(0)].z() == z) + (mesh_vertices[neighbor(1)].z() == z) +
                                           (mesh_vertices[neighbor(2)].z() == z);
                ...
                for (int iedge = 0; iedge < 3; ++iedge)
                    if (facet_neighbors(iedge) == -1) {
                        ...
                        if (intersects_this && intersects_prev) {
                            ...
                            Line l(il_prev.edge_a_id == edge_id ? il_prev.a : il_prev.b, il.edge_a_id == edge_id ? il.a : il.b);
                            emit_slab_edge(IntersectionLine{l, -1, -1, edge_id, edge_id + num_edges, IntersectionLine::FacetEdgeType::Slab},
                            ...
                            emit_slab_edge(IntersectionLine{{to_2d(edge_up ? vertices[i] : vertices[j]).cast<coord_t>(),
                                                             il.edge_a_id == edge_id ? il.a : il.b},
                                                            edge_up ? indices(i) : indices(j),
                                                            -1,
                                                            -1,
                                                            edge_id + num_edges,
                                                            IntersectionLine::FacetEdgeType::Slab},
...
#ifndef NDEBUG
                            {
                                auto it_prev = it;
                                --it_prev;
                                assert((vertices[i].z() > *it_prev && vertices[j].z() < *it_prev) ||
                                       (vertices[i].z() < *it_prev && vertices[j].z() > *it_prev));
                            }
#endif // NDEBUG
                            emit_slab_edge(IntersectionLine{{il_prev.edge_a_id == edge_id ? il_prev.a : il_prev.b,
                                                             to_2d(edge_up ? vertices[j] : vertices[i]).cast<coord_t>()},
                                                            -1,
                                                            edge_up ? indices(j) : indices(i),
                                                            edge_id,
                                                            -1,
                                                            IntersectionLine::FacetEdgeType::Slab},
                                           slab_id, ProjectionFromTop != edge_up);
    ...
    const ThrowOnCancel                 throw_on_cancel_fn)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())),
                      [&vertices, &indices, &face_neighbors, &face_edge_ids, num_edges, &face_orientation, &zs, top, bottom, &lines_top,
                       &lines_bottom, &lines_mutex_top, &lines_mutex_bottom, throw_on_cancel_fn](const tbb::blocked_range<int>& range) {
                          for (int face_idx = range.begin(); face_idx < range.end(); ++face_idx) {
                              if ((face_idx & 0x0ffff) == 0)
                                  throw_on_cancel_fn();
                              ...
                              if (top && (fo == FaceOrientation::Up || fo == FaceOrientation::Degenerate)) {
                                  ...
                                  slice_facet_with_slabs<true>(vertices, indices, face_idx, neighbors, edge_ids, num_edges, zs, lines_top,
                                                               lines_mutex_top);
                              }
                              if (bottom && (fo == FaceOrientation::Down || fo == FaceOrientation::Degenerate)) {
                                  ...
                                  slice_facet_with_slabs<false>(vertices, indices, face_idx, neighbors, edge_ids, num_edges, zs,
                                                                lines_bottom, lines_mutex_bottom);
                              }
                          }
                      });
    return out;
...
struct OpenPolyline
{
    ...
    {
        this->length = Slic3r::length(this->points);
    }
    void reverse()
    {
        std::swap(start, end);
        ...
        for (;;) {
            ...
            if (next_line == nullptr) {
                ...
                if ((first_line->edge_a_id != -1 && first_line->edge_a_id == last_line->edge_b_id) ||
                    ...
                    open_polylines.emplace_back(OpenPolyline(IntersectionReference(first_line->a_id, first_line->edge_a_id),
                                                             IntersectionReference(last_line->b_id, last_line->edge_b_id),
                                                             std::move(loop_pts)));
                }
                break;
            }
...
static void chain_open_polylines_exact(std::vector<OpenPolyline>& open_polylines, Polygons& loops, bool try_connect_reversed)
{
    ...
    struct OpenPolylineEnd
    {
        ...
        int id() const
        {
            const IntersectionReference& r = ipref();
            return (r.point_id >= 0) ? r.point_id : -r.edge_id;
        }
    ...
    auto find_polyline_end = [&by_id, by_id_lower](const OpenPolylineEnd& end) -> std::vector<OpenPolylineEnd>::iterator {
        for (auto it = std::lower_bound(by_id.begin(), by_id.end(), end, by_id_lower); it != by_id.end() && it->id() == end.id(); ++it)
            if (*it == end)
                return it;
        return by_id.end();
    };
                                            ...
                                            bool                       try_connect_reversed)
{
    ...
    for (OpenPolyline* opl : sorted_by_length) {
        ...
        for (;;) {
            // Find a line starting where last one finishes, only return non-consumed open polylines (OpenPolylineEndAccessor returns null
            // for consumed).
    ...
    ThrowOnCancel throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(lines.at_slice.size())), [&lines, num_edges, &layers,
                                                                               throw_on_cancel](const tbb::blocked_range<int>& range) {
       ...
       // Edge IDs of end points on in-between lines that touch the layer above are already increased with num_edges.
                ...
                if (!in.empty()) {
                    ...
                    coordf_t    stroke_width = scale_(0.02);
                    {
                        ...
                        SVG svg(debug_out_path("make_slab_loops-in-%d-%d-%s.svg", iRun, line_idx, ProjectionFromTop ? "top" : "bottom")
                                    .c_str(),
                                bbox_svg);
                        svg.arrows = true;
                        for (const IntersectionLine& line : in) {
                            const char* color = line.source == IntersectionLine::Source::BottomPlane ? "red" :
                                                line.source == IntersectionLine::Source::TopPlane    ? "blue" :
                                                                                                       "green";
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        SVG svg(debug_out_path("make_slab_loops-out-%d-%d-%s.svg", iRun, line_idx, ProjectionFromTop ? "top" : "bottom")
                                    .c_str(),
                                bbox_svg);
                        svg.arrows = true;
                        for (const IntersectionLine& line : in) {
                    ...
                    {
                        SVG svg(debug_out_path("make_slab_loops-open-polylines-%d-%d-%s.svg", iRun, line_idx,
                                               ProjectionFromTop ? "top" : "bottom")
                                    .c_str(),
                                bbox_svg);
                        svg.draw(loops, "black");
                        svg.arrows = true;
...
static void make_expolygons(
    const Polygons& loops, const float closing_radius, const float extra_offset, ClipperLib::PolyFillType fill_type, ExPolygons* slices)
{
    /*
    ...
    */
    ...
    expolygons_append(*slices, offset_out > 0 && offset_in < 0 ? offset2_ex(union_ex(loops, fill_type), offset_out, offset_in) :
...
static inline bool is_identity(const Transform3d& trafo) { return trafo.matrix() == Transform3d::Identity().matrix(); }
...
std::vector<Polygons> slice_mesh(const indexed_triangle_set& mesh,
                                 // Unscaled Zs
                                 const std::vector<float>& zs,
                                 const MeshSlicingParams&  params,
        ...
        if (zs.size() <= 1) {
            ...
            if (is_identity(params.trafo)) {
                lines = slice_make_lines(
                    mesh.vertices, [](const Vec3f& p) { return Vec3f(scaled<float>(p.x()), scaled<float>(p.y()), p.z()); }, mesh.indices,
                    face_edge_ids, zs, throw_on_cancel);
            } else {
                // Transform the vertices, scale up in XY, not in Z.
                Transform3f tf = make_trafo_for_slicing(params.trafo);
                lines          = slice_make_lines(
                    mesh.vertices, [tf](const Vec3f& p) { return tf * p; }, mesh.indices, face_edge_ids, zs, throw_on_cancel);
            }
        } else {
            // Copy and scale vertices in XY, don't scale in Z. Possibly apply the transformation.
            lines = slice_make_lines(
                transform_mesh_vertices_for_slicing(mesh, params.trafo), [](const Vec3f& p) { return p; }, mesh.indices, face_edge_ids, zs,
                throw_on_cancel);
        }
    }
...
Polygons slice_mesh(const indexed_triangle_set& mesh,
                    // Unscaled Zs
                    const float              plane_z,
                    const MeshSlicingParams& params)
{
...

    {
        ...
        if (trafo_identity) {
            lines.emplace_back(slice_make_lines(
                mesh.vertices, [](const Vec3f& p) { return Vec3f(scaled<float>(p.x()), scaled<float>(p.y()), p.z()); }, mesh.indices,
                face_edge_ids, plane_z, [&face_mask](int face_idx) { return face_mask[face_idx]; }));
        } else {
            // Transform the vertices, scale up in XY, not in Z.
            lines.emplace_back(slice_make_lines(
                mesh.vertices, [tf](const Vec3f& p) { return tf * p; }, mesh.indices, face_edge_ids, plane_z,
...
std::vector<ExPolygons> slice_mesh_ex(const indexed_triangle_set& mesh,
                                      const std::vector<float>&   zs,
                                      ...
                                      std::function<void()>       throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, layers_p.size()), [&layers_p, &params, &layers,
        ...
        for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
            ...
            Slic3r::make_expolygons(layers_p[layer_id], params.closing_radius, params.extra_offset,
                                    this_mode == MeshSlicingParams::SlicingMode::EvenOdd                ? ClipperLib::pftEvenOdd :
                                    this_mode == MeshSlicingParams::SlicingMode::PositiveLargestContour ? ClipperLib::pftPositive :
...
void slice_mesh_slabs(const indexed_triangle_set& mesh,
                      // Unscaled Zs
                      const std::vector<float>&             zs,
                      ...
                      std::function<void()>                 throw_on_cancel)
{
    ...
    // [RESOLVED Q1] its_face_neighbors_par() rebuilds the face-neighbor table from scratch
    // on every call. The admesh adjacency data (its_neighbors_par) is NOT reused here —
    // it is a separate structure used only by mesh-repair code. See agent_journal.md Q1.
    ...
    std::pair<SlabLines, SlabLines> lines = slice_slabs_make_lines(vertices_transformed, mesh.indices, face_neighbors, face_edge_ids,
                                                                   num_edges, face_orientation, zs, out_top != nullptr,
                                                                   out_bottom != nullptr, throw_on_cancel);
...
static void triangulate_slice(indexed_triangle_set& its,
                              IntersectionLines&    lines,
                              ...
                              const std::map<int, Vec3f>& section_vertices_map)
{
    ...
    std::sort(map_vertex_to_index.begin(), map_vertex_to_index.end(), [](const std::pair<Vec2f, int>& l, const std::pair<Vec2f, int>& r) {
        return l.first.x() < r.first.x() ||
               (is_equal_for_sort(l.first.x(), r.first.x()) &&
                (l.first.y() < r.first.y() || (is_equal_for_sort(l.first.y(), r.first.y()) && l.second < r.second)));
    });
    ...
    //    Remove denegerate triangles, if they happen to be created by merging duplicate vertices.
    {
        ...
        for (; i < int(map_vertex_to_index.size());) {
            ...
            int j = i;
            for (++j; j < int(map_vertex_to_index.size()) && is_equal(ipos.x(), map_vertex_to_index[j].first.x()) &&
                      is_equal(ipos.y(), map_vertex_to_index[j].first.y());
                 ++j) {
        ...
        for (size_t i = 0; i < triangles.size();) {
            ...
            for (size_t j = 0; j < 3; ++j) {
                ...
                auto  it      = lower_bound_by_predicate(map_vertex_to_index.begin(), map_vertex_to_index.end(),
                                                       ...
                                                       return l.first.x() < v.x() ||
                                                              (is_equal_for_sort(l.first.x(), v.x()) && l.first.y() < v.y());
                                                   });
                auto  back_it = it;
                int   idx     = -1;
                bool  exist   = false;
...
void cut_mesh(const indexed_triangle_set& mesh, float z, indexed_triangle_set* upper, indexed_triangle_set* lower, bool triangulate_caps)
{
    ...
    for (int facet_idx = 0; facet_idx < int(mesh.indices.size()); ++facet_idx) {
        ...
        if (z > min_z - EPSILON && z < max_z + EPSILON) {
            ...
            slice_type = slice_facet_for_cut_mesh(z, vertices_scaled, mesh.indices[facet_idx], facets_edge_ids[facet_idx],
                                                  idx_vertex_lowest, is_equal(min_z, max_z), line);
        }
        ...
        if (min_z > z || (is_equal(min_z, z) && max_z > z)) {
            ...
            if (is_equal(vertices[0].z(), z)) {
                ...
                isolated_vertex = (vertices[0].z() > z) == (vertices[1].z() > z) ? 2 :
                                  (vertices[1].z() > z) == (vertices[2].z() > z) ? 0 :
                                                                                   1;
            }
            ...
            auto new_vertex = [upper, lower, &upper_slice_vertices, &lower_slice_vertices](const Vec3f& a, const int ia, const Vec3f& b,
                                                                                           const int ib, const Vec3f& c, const int ic,
                                                                                           const Vec3f& new_pt, bool& is_new_vertex) {
