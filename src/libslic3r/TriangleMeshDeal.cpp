// [INTENT] Loop-subdivision smoothing for TriangleMesh via libigl igl::loop.
// [HAZARD] Commented-out igl::read_triangle_mesh call on line 28 contains a
// hardcoded Windows debug path ("E:/Download/libigl-2.6.0/...") – dead code
// left from development; also `igl::upsample` and
// `igl::false_barycentric_subdivision` are commented out as alternatives.
// The active implementation uses only igl::loop (Loop subdivision).
// [COUPLING] Depends on libigl (igl/loop.h, igl/upsample.h,
// igl/false_barycentric_subdivision.h) and Eigen MatrixXd/MatrixXi.
// [STATE] Pure function; mesh data converted from SoA to Eigen matrices
// and back.
