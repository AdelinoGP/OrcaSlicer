// [INTENT] Preset subsystem public API header.  Defines Preset (profile data + metadata),
// PresetCollection (sorted deque of Presets + selection state), PrinterPresetCollection
// (FFF/SLA dual-default subclass), PhysicalPrinter, PhysicalPrinterCollection, and
// VendorProfile (vendor bundle metadata).
// [COUPLING] Includes PrintConfig.hpp (DynamicPrintConfig), Semver.hpp, ProjectTask.hpp.
//            PresetBundle (PresetBundle.hpp) owns all collections and the VendorMap.
// [STATE] Several static members in Preset.cpp (s_suffix_modified, s_Preset_*_options
//         vectors) are lazily initialised once — not thread-safe on first call if called
...
// [INTENT] Vendor bundle metadata: one VendorProfile per vendor directory loaded from
//          resources/profiles/<VendorID>/ (or user data dir).  Stores printer model
//          hierarchy, variant list, default filament/material names, and display assets.
// [COUPLING] Preset::vendor is a raw pointer into PresetBundle::m_vendors (a VendorMap =
//            std::map<string,VendorProfile>).  std::map guarantees pointer stability on
//            insert/erase, which is why VendorMap uses map not unordered_map.
// [STATE] Immutable after loading — treated as read-only shared data accessible from both
//         the UI thread and the slicing thread (no lock taken when reading).
// [HAZARD] PrinterModel accumulates many BBS-specific display fields (bed_model,
//          bed_texture, hotend_model, image_bed_type, etc.) that have no upstream
//          PrusaSlicer equivalent and must be preserved in any translation.
class VendorProfile
{
public:
    std::string name;
...
// [INTENT] Central profile object: one DynamicPrintConfig + identity/metadata.
// [STATE] The live-edited copy is always m_edited_preset in PresetCollection; the stored
//         Preset in m_presets is the last-saved version.  Never modify m_presets[i].config
//         directly from the UI — always go through m_edited_preset.
// [COUPLING] Tightly coupled to: PresetCollection (owns and manages instances),
//            PresetBundle (calls normalize(), save(), load_*), AppConfig (visibility),
//            cloud sync layer (setting_id / sync_info / user_id fields).
// [MEMORY] vendor is a raw non-owning pointer into PresetBundle::m_vendors (stable due to
//          std::map semantics).  loading_substitutions is also a raw non-owning pointer.
// [HAZARD] BBS cloud sync fields (setting_id, filament_id, user_id, base_id, sync_info,
//          updated_time, key_values) have no upstream PrusaSlicer equivalent and must be
//          carried in any translation.
class Preset
{
public:
    // [INTENT] Discriminates which PresetCollection a Preset belongs to.
    // [HAZARD] TYPE_PHYSICAL_PRINTER (7), TYPE_PLATE (8), and TYPE_MODEL (9) are
    //          non-collection sentinel values (beyond TYPE_COUNT = 6) used only in
    //          PresetsConfigSubstitutions reporting.  Code switching on Type must
    //          handle or explicitly skip these to avoid undefined behaviour.
    enum Type {
        TYPE_INVALID,
        TYPE_PRINT,
        TYPE_SLA_PRINT,
    ...
    // or a Configuration file bundling the Print + Filament + Printer presets (in that case is_external and possibly is_system will be
    // true), or it could be a G-code (again, is_external will be true).
    std::string file;
    ...
    // [HAZARD] The `true` parameter to option<>() means "create if missing" — calling
    //          inherits() on a const Preset via the const_cast overload will silently
    //          insert an empty "inherits" key into the config if it was absent.
    ...
    {
        return cfg.option<ConfigOptionString>("compatible_prints_condition", true)->value;
    }
    ...
    {
        return cfg.option<ConfigOptionString>("compatible_printers_condition", true)->value;
    }
    ...
    {
        return this->config.option<ConfigOptionEnum<PrinterTechnology>>("printer_technology", true)->value;
    }
    ...
    static void get_extruder_names_and_keysets(Type                    type,
    ...
    static double convert_filament_diameter_to_pellet_flow(double filament_diameter) { return 4 / (pow(filament_diameter, 2) * PI); }
...
// [INTENT] Sorted container for all presets of one type (Print / Filament / Printer /
//          SLA Print / SLA Material).  Manages selection state, dirty tracking,
//          load/save from disk, cloud sync, and compatibility filtering.
// [STATE] Key mutable state:
//   m_presets       — sorted deque of on-disk profiles (+ defaults at front)
//   m_edited_preset — live copy accumulating user changes for the selected preset
//   m_saved_preset  — snapshot at last explicit Save, used for dirty-vs-saved detection
//   m_idx_selected  — size_t index; size_t(-1) = no selection (unsigned wraparound sentinel)
// [CONCURRENCY] m_mutex protects load/save operations (load_user_preset holds lock
//               during JSON parsing — potentially long hold).  Reading m_presets or
//               m_edited_preset outside the lock is NOT guarded in most UI call paths.
// [COUPLING] Tightly coupled to PresetBundle (calls normalize, merge_presets, operator=),
//            AppConfig (set_visible_from_appconfig), and cloud sync callback (SyncFunc).
// Collections of presets of the same type (one of the Print, Filament or Printer type).
class PresetCollection
{
public:
    // Initialize the PresetCollection with the "- default -" preset.
    PresetCollection(Preset::Type                     type,
                     const std::vector<std::string>&  keys,
                     const Slic3r::StaticPrintConfig& defaults,
                     const std::string&               default_name = "Default Setting");
    ...
    // [INTENT] begin()/end() skip the default preset(s) (m_presets[0..m_num_default_presets-1]).
    //          Use lbegin() to iterate ALL presets including defaults.
    // [HAZARD] Range-for with begin()/end() silently omits defaults.  Any code that
    //          needs to apply an operation to defaults must use lbegin() explicitly.
    ...
    bool load_user_preset(std::string                          name,
    ...
    void                 load_project_embedded_presets(std::vector<Preset*>&                project_presets,
    ...
    Preset& load_preset(const std::string&        path,
                        const std::string&        name,
                        const DynamicPrintConfig& config,
                        bool                      select       = true,
                        Semver                    file_version = Semver());
    Preset& load_preset(
        const std::string& path, const std::string& name, DynamicPrintConfig&& config, bool select = true, Semver file_version = Semver());

    bool clone_presets(std::vector<Preset const*> const&           presets,
                       std::vector<std::string>&                   failures,
                       std::function<void(Preset&, Preset::Type&)> modifier,
                       bool                                        force_rewritten = false);
    bool clone_presets_for_printer(std::vector<Preset const*> const&       templates,
                                   std::vector<std::string>&               failures,
    ...
    // [HAZARD] get_preset_base() is RECURSIVE — follows the full inheritance chain until it
    //          reaches a preset with no "inherits" field.  A circular reference (A inherits B,
    //          B inherits A) causes unbounded recursion and a stack overflow.  No cycle-
    //          detection exists in the implementation (Preset.cpp).
    const Preset* get_preset_base(const Preset& child) const;
    // Return the selected preset including the user modifications.
    // [INTENT] m_edited_preset is the live-edit buffer: it starts as a copy of the selected
    //          preset and is mutated by the UI.  When the user clicks Save, the edited preset
    //          is written to m_presets[m_idx_selected] and back to disk.
    ...
    {
        return this->get_preset_with_vendor_profile(this->get_edited_preset());
    }
    ...
    {
        assert(idx < m_num_default_presets);
        return m_presets[idx];
    }
    const Preset& default_preset(size_t idx = 0) const
    {
        assert(idx < m_num_default_presets);
        return m_presets[idx];
    }
    ...
    {
        return this->first_compatible_idx([](const Preset&) -> int { return 0; });
    }
    ...
    template<typename PreferedCondition> Preset& first_compatible(PreferedCondition prefered_condition)
    {
        return this->preset(this->first_compatible_idx(prefered_condition));
    }
    ...
    void update_compatible(const PresetWithVendorProfile& active_printer,
                           const PresetWithVendorProfile* active_print,
    ...
    {
        return std::count_if(m_presets.begin(), m_presets.end(), [](const Preset& preset) { return preset.is_visible; });
    }

    // Compare the content of get_selected_preset() with get_edited_preset() configs, return true if they differ.
    bool current_is_dirty() const { return is_dirty(&this->get_edited_preset(), &this->get_selected_preset()); }
    ...
    bool saved_is_dirty() const { return is_dirty(&this->get_edited_preset(), &m_saved_preset); }
...
private:
    ...
    {
        auto it = Slic3r::lower_bound_by_predicate(m_presets.begin() + m_num_default_presets, m_presets.end(), [&name, this](const auto& l) {
            if (m_type == Preset::TYPE_FILAMENT) {
                bool l_generic    = boost::starts_with(l.name, GENERIC_PREFIX);
                bool name_generic = boost::starts_with(name, GENERIC_PREFIX);
                if (l_generic && !name_generic)
                    return true;
                if (!l_generic && name_generic)
                    return false;
            }
    ...
    size_t update_compatible_internal(const PresetWithVendorProfile& active_printer,
                                      const PresetWithVendorProfile* active_print,
...
public:
    ...
    static std::vector<std::string> dirty_options_without_option_list(const Preset*                edited,
...
private:
    ...
    // [INTENT] std::deque is used deliberately (not vector) to guarantee that inserting/
    //          erasing at either end does NOT invalidate pointers/references to existing
    //          elements.  This ensures that raw Preset* pointers cached by UI widgets
    //          remain valid after collection modifications.
    ...
    // [INTENT] m_edited_preset is the "live edit buffer" — it starts as a copy of the
    //          selected preset and is mutated by the UI.  m_presets[m_idx_selected] holds
    //          the last-saved version.  Calling get_edited_preset() vs get_selected_preset()
    ...
    // Selected preset.
    // [HAZARD] m_idx_selected is size_t.  The "no selection" sentinel is size_t(-1)
    //          (unsigned wraparound).  Comparisons like (m_idx_selected < 0) at
    //          Preset.hpp:668 are always false for size_t — BBS added a signed-comparison
    //          guard that works correctly only because of implicit conversion rules.
...
{
public:
    PrinterPresetCollection(Preset::Type                     type,
                            const std::vector<std::string>&  keys,
                            const Slic3r::StaticPrintConfig& defaults,
                            const std::string&               default_name = "Default Printer")
...
// [INTENT] PhysicalPrinter represents a physical device (host + connection config)
//          that can be associated with multiple Preset profiles.  It stores only
//          the connection/host parameters (not slicing params).
// [STATE] preset_names is a std::set<string> of the logical Preset names associated
//         with this physical device.  The set is written into the config as a
//         ConfigOptionStrings("preset_names") for persistence.
// [COUPLING] PhysicalPrinterCollection (below) owns PhysicalPrinter instances.
//            PresetBundle::physical_printers holds the collection.
// [HAZARD] has_print_host_information() in PhysicalPrinter (Preset.hpp) always returns
//          false — it is a stub.  The legacy migration path load_printers_from_presets()
//          is entirely inside #if 0 (permanently disabled).
...
// [INTENT] Sorted deque of PhysicalPrinter instances — the physical device registry.
// [STATE] m_idx_selected (size_t(-1) = no selection) + m_selected_preset (name string)
//         together identify the active (printer, preset) pair.
//         Note: unlike PresetCollection there is no m_edited copy — modifications to the
//         selected printer go directly to m_printers[m_idx_selected].
// [HAZARD] find_printer_internal() uses binary search for case_sensitive=true but falls
//          back to O(N) linear scan for case_sensitive=false.  With many printers this
//          degrades linearly.
// [HAZARD] load_printers_from_presets() is entirely inside #if 0 — permanently disabled.
//          Legacy "Print Host upload" migration never runs.
...
public:
    ...
    // Load ini files of the particular type from the provided directory path.
    void load_printers(const std::string&                   dir_path,
                       const std::string&                   subdir,
                       PresetsConfigSubstitutions&          substitutions,
                       ForwardCompatibilitySubstitutionRule rule);
    ...
    {
        return (m_idx_selected == size_t(-1)) ? std::string() : this->get_selected_printer().name;
    }
    // Returns the config of the selected printer, or nullptr if no printer is selected.
    DynamicPrintConfig* get_selected_printer_config()
    {
        return (m_idx_selected == size_t(-1)) ? nullptr : &(this->get_selected_printer().config);
    }
    // Returns the config of the selected printer, or nullptr if no printer is selected.
    PrinterTechnology get_selected_printer_technology()
    {
        return (m_idx_selected == size_t(-1)) ? PrinterTechnology::ptAny : this->get_selected_printer().printer_technology();
    }
