...
enum FilamentUsageType { SupportOnly, ModelOnly, Hybrid };
...
inline bool is_separable_infill_pattern(InfillPattern pattern)
{
    ...
    case ipOctagramSpiral: return true;
...
enum FanDirection { fdUndefine = 0, fdLeft, fdRight, fdBoth };
...
enum class InputShaperType : unsigned char { Default = 0, MZV, ZV, ZVD, ZVDD, ZVDDD, EI, EI2, TwoHumpEI, EI3, ThreeHumpEI, DAA, Disable };
...
enum FilamentMapMode {
    ...
    fmmAutoForMatch,
    fmmManual,
    fmmNozzleManual, // Fully-manual filament->physical-nozzle mapping (filament_nozzle_map). Kept ordered right after fmmManual so every `<
                     // fmmManual` "is-auto" check stays correct.
    fmmDefault
};

// All auto modes are ordered before fmmManual (see the enum ordering note above).
inline bool is_auto_filament_map_mode(FilamentMapMode mode) { return mode < fmmManual; }
...
enum PrimeVolumeMode { pvmDefault = 0, pvmSaving, pvmFast };
...
extern int get_config_index_base(NozzleVolumeType volume_type,
                                 ExtruderType extruder_type,
                                 int variant_id_1based,
...
extern std::vector<std::string> save_extruder_nozzle_stats_to_string(
    const std::vector<std::map<NozzleVolumeType, int>>& extruder_nozzle_stats);
...
public:
    ...
    ConfigOptionDef* add_pnp_key(const t_config_option_key& opt_key, ConfigOptionType type) { return this->add(opt_key, type); }
...
struct PnpConfigKeyDef
{
    ...
    std::string tooltip;
    std::string sidetext;
    // Schema `advanced` flag (wire 1.1.0+): true -> comExpert, false -> comAdvanced.
    // Decides which mode tier the generated PNP settings page shows the control
    // under; host keys arriving without the flag register at comAdvanced.
    bool advanced{false};
...
// One generated-PNP-page optgroup, as derived from the registered defs.
struct PnpPageGroup
{
    // The schema `group` string, verbatim and untranslated.
    std::string category;
    // Member keys, alphabetical.
    std::vector<std::string> keys;
    // True when the group's name is also an Orca settings-page name (Support,
    // Quality, Speed, Walls). The group still renders on the PNP page; nothing
    // merges. Kept as data so review can see the overlap.
    bool matches_orca_page{false};
};

// Bucket the registered pnp keys into page optgroups: by the def's category
// (the schema `group`), groups ordered by descending key count with
// alphabetical ties, keys alphabetical inside each group. `skip_keys` are
// excluded (the host-injected skip list). GUI-free so the layout stays
...
public:
    ...
    std::vector<int> update_values_to_printer_extruders(DynamicPrintConfig& printer_config,
                                                        int extruder_count,
                                                        ...
                                                        NozzleVolumeType filament_nvt = nvtStandard);
    ...
    void update_filament_config_values_for_multiple_extruders(
...
extern void update_static_print_config_from_dynamic(ConfigBase& config,
                                                    const DynamicPrintConfig& dest_config,
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloat, brim_object_gap))((ConfigOptionFloat, brim_flow_ratio))((ConfigOptionBool, brim_use_efc_outline))(
        (ConfigOptionEnum<BrimType>, brim_type))((ConfigOptionFloat, brim_width))((ConfigOptionFloat, brim_ears_detection_length))(
        (ConfigOptionFloat, brim_ears_max_angle))((ConfigOptionFloat, skirt_start_angle))((ConfigOptionBool, bridge_no_support))(
        (ConfigOptionFloat, elefant_foot_compensation))((ConfigOptionInt, elefant_foot_compensation_layers))((
        ConfigOptionPercent, elefant_foot_layers_density))((ConfigOptionFloat, max_bridge_length))((ConfigOptionFloatOrPercent, line_width))
    ...
    ((ConfigOptionFloat, support_interface_spacing))((ConfigOptionFloatsNullable, support_interface_speed))(
    ...
    ((ConfigOptionFloat, support_base_pattern_spacing))((ConfigOptionFloat, support_expansion))(
        (ConfigOptionFloatsNullable, support_speed))((ConfigOptionEnum<SupportMaterialStyle>, support_style))
    ...
    // Orca
    ((ConfigOptionFloat, wall_maximum_resolution))((ConfigOptionFloat, wall_maximum_deviation))(
    ...
    ((ConfigOptionFloatsNullable, default_acceleration))((ConfigOptionFloatsNullable, outer_wall_acceleration))(
        (ConfigOptionFloatsNullable, inner_wall_acceleration))((ConfigOptionFloatsNullable, top_surface_acceleration))(
        (ConfigOptionFloatsNullable, initial_layer_acceleration))((ConfigOptionFloatsOrPercentsNullable, bridge_acceleration))(
        (ConfigOptionFloatsNullable, travel_acceleration))((ConfigOptionFloatsOrPercentsNullable, sparse_infill_acceleration))(
        (ConfigOptionFloatsOrPercentsNullable, internal_solid_infill_acceleration))

        ((ConfigOptionFloatsNullable, default_jerk))((ConfigOptionFloatsNullable, outer_wall_jerk))(
            (ConfigOptionFloatsNullable, inner_wall_jerk))((ConfigOptionFloatsNullable, infill_jerk))(
            (ConfigOptionFloatsNullable, top_surface_jerk))((ConfigOptionFloatsNullable, initial_layer_jerk))(
            (ConfigOptionFloatsNullable, travel_jerk))((ConfigOptionBool, precise_z_height))((ConfigOptionFloatsNullable,
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloat, bridge_flow))((ConfigOptionFloatOrPercent, bridge_line_width))((ConfigOptionFloat, internal_bridge_flow))(
        (ConfigOptionFloatsNullable, bridge_speed))((ConfigOptionFloatsOrPercentsNullable, internal_bridge_speed))(
        (ConfigOptionEnum<EnsureVerticalShellThickness>, ensure_vertical_shell_thickness))((ConfigOptionPercent, top_surface_density))(
        (ConfigOptionPercent, bottom_surface_density))((ConfigOptionEnum<InfillPattern>, top_surface_pattern))(
        (ConfigOptionEnum<InfillPattern>, bottom_surface_pattern))((ConfigOptionEnum<SurfaceFillOrder>, top_surface_fill_order))(
        (ConfigOptionEnum<SurfaceFillOrder>, bottom_surface_fill_order))((ConfigOptionEnum<InfillPattern>, internal_solid_infill_pattern))(
        (ConfigOptionFloatOrPercent, outer_wall_line_width))((ConfigOptionFloatsNullable, outer_wall_speed))(
        (ConfigOptionFloat, infill_direction))((ConfigOptionFloat, solid_infill_direction))((ConfigOptionFloat, top_layer_direction))(
        (ConfigOptionFloat, bottom_layer_direction))((ConfigOptionString, solid_infill_rotate_template))(
        ...
        (ConfigOptionFloat, lateral_lattice_angle_2))((ConfigOptionFloat, infill_overhang_angle))(
        (ConfigOptionFloat, lightning_overhang_angle))((ConfigOptionFloat, lightning_prune_angle))((ConfigOptionFloat,
                                                                                                    lightning_straightening_angle))(
        (ConfigOptionBool, align_infill_direction_to_model))((ConfigOptionEnum<CenterOfSurfacePattern>, center_of_surface_pattern))(
        (ConfigOptionBool, separated_infills))((ConfigOptionString, extra_solid_infills))((ConfigOptionEnum<FuzzySkinType>, fuzzy_skin))(
        (ConfigOptionFloat, fuzzy_skin_thickness))((ConfigOptionFloat, fuzzy_skin_point_distance))(
        (ConfigOptionBool, fuzzy_skin_first_layer))((ConfigOptionEnum<NoiseType>, fuzzy_skin_noise_type))(
        (ConfigOptionEnum<FuzzySkinMode>, fuzzy_skin_mode))((ConfigOptionFloat, fuzzy_skin_scale))((ConfigOptionInt, fuzzy_skin_octaves))(
        (ConfigOptionFloat, fuzzy_skin_persistence))((ConfigOptionInt, fuzzy_skin_ripples_per_layer))(
        (ConfigOptionPercent, fuzzy_skin_ripple_offset))((ConfigOptionInt, fuzzy_skin_layers_between_ripple_offset))(
        (ConfigOptionFloatsNullable, gap_infill_speed))((ConfigOptionInt, sparse_infill_filament_id))(
        (ConfigOptionFloatOrPercent, sparse_infill_line_width))((ConfigOptionPercent, infill_wall_overlap))(
        (ConfigOptionPercent, top_bottom_infill_wall_overlap))((ConfigOptionFloatsNullable, sparse_infill_speed))(
    ...
    // Orca:
    ((ConfigOptionFloatOrPercent, infill_combination_max_layer_height))((ConfigOptionInt, fill_multiline))((ConfigOptionBool,
                                                                                                            gyroid_optimized))
    // Ironing options
    ...
    ((ConfigOptionBool, detect_overhang_wall))((ConfigOptionInt, outer_wall_filament_id))((ConfigOptionInt, inner_wall_filament_id))(
        (ConfigOptionFloatOrPercent, inner_wall_line_width))((ConfigOptionFloatsNullable, inner_wall_speed))
    // Total number of perimeters.
    ((ConfigOptionInt, wall_loops))((ConfigOptionBool, alternate_extra_wall))((ConfigOptionFloat, minimum_sparse_infill_area))(
        (ConfigOptionInt, internal_solid_filament_id))((ConfigOptionInt, top_surface_filament_id))((ConfigOptionInt,
                                                                                                    bottom_surface_filament_id))(
        (ConfigOptionFloatOrPercent, internal_solid_infill_line_width))((ConfigOptionFloatsNullable, internal_solid_infill_speed))
    // Detect thin walls.
    ((ConfigOptionBool, detect_thin_wall))((ConfigOptionFloatOrPercent, top_surface_line_width))((ConfigOptionInt, top_shell_layers))(
        (ConfigOptionFloat, top_shell_thickness))((ConfigOptionFloat, top_surface_expansion))((ConfigOptionFloat,
                                                                                               top_surface_expansion_margin))(
        (ConfigOptionEnum<TopSurfaceExpansionDirection>, top_surface_expansion_direction))((ConfigOptionFloatsNullable, top_surface_speed))
    // BBS
    ((ConfigOptionBoolsNullable, enable_overhang_speed))((ConfigOptionFloatsOrPercentsNullable, overhang_1_4_speed))(
        (ConfigOptionFloatsOrPercentsNullable, overhang_2_4_speed))((ConfigOptionFloatsOrPercentsNullable, overhang_3_4_speed))(
        (ConfigOptionFloatsOrPercentsNullable, overhang_4_4_speed))((ConfigOptionBool, only_one_wall_top))
    ...
    ((ConfigOptionFloatOrPercent, min_width_top_surface))((ConfigOptionBool, only_one_wall_first_layer))(
        ...
        (ConfigOptionPercent, bridge_density))((ConfigOptionFloat, filter_out_gap_fill))((ConfigOptionFloatsOrPercentsNullable,
                                                                                          small_perimeter_speed))(
        (ConfigOptionFloatsNullable, small_perimeter_threshold))((ConfigOptionFloatsOrPercentsNullable, small_support_perimeter_speed))(
        (ConfigOptionFloatsNullable, small_support_perimeter_threshold))((ConfigOptionFloat, top_solid_infill_flow_ratio))(
    ...
    ((ConfigOptionBool, make_overhang_printable))((ConfigOptionBool, extra_perimeters_on_overhangs))(
        (ConfigOptionBoolsNullable, slowdown_for_curled_perimeters))((ConfigOptionBool, hole_to_polyhole))(
        (ConfigOptionFloatOrPercent, hole_to_polyhole_threshold))((ConfigOptionBool,
                                                                   hole_to_polyhole_twisted))((ConfigOptionInt, hole_to_polyhole_max_edges))
    ...
    ((ConfigOptionBool, zaa_enabled))((ConfigOptionBool, zaa_dont_alternate_fill_direction))((ConfigOptionFloat, zaa_min_z))(
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionFloat, machine_max_force_Y))((ConfigOptionFloat, machine_bed_mass_Y))((ConfigOptionFloat, machine_max_printed_mass))
    ...
    // Orca: Input shaping
    ((ConfigOptionBool, input_shaping_emit))((ConfigOptionEnum<InputShaperType>, input_shaping_type))((ConfigOptionFloat,
                                                                                                       input_shaping_freq_x))(
        (ConfigOptionFloat, input_shaping_freq_y))((ConfigOptionFloat, input_shaping_damp_x))((ConfigOptionFloat, input_shaping_damp_y)))
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    ((ConfigOptionInt, part_cooling_fan_min_pwm))((ConfigOptionFloats, filament_diameter))(
        (ConfigOptionBoolsNullable, filament_adaptive_volumetric_speed))((ConfigOptionStrings, volumetric_speed_coefficients))(
        (ConfigOptionInts, filament_adhesiveness_category))((ConfigOptionFloats, filament_density))((ConfigOptionStrings, filament_type))(
        (ConfigOptionBools, filament_soluble))((ConfigOptionStrings, filament_ids))((ConfigOptionStrings, filament_colour))(
        (ConfigOptionStrings, filament_vendor))((ConfigOptionBools, filament_is_support))((ConfigOptionInts, filament_printable))(
        (ConfigOptionInts, filament_extruder_compatibility))((ConfigOptionFloats, filament_change_length))(
    ...
    ((ConfigOptionFloats, filament_max_volumetric_speed))((ConfigOptionInts, required_nozzle_HRC))((ConfigOptionEnum<FilamentMapMode>,
                                                                                                    filament_map_mode))(
        (ConfigOptionInts, filament_map))((ConfigOptionInts, filament_volume_map))((ConfigOptionInts, filament_nozzle_map))(
    ...
    ((ConfigOptionStrings, filament_extruder_variant))((ConfigOptionInts, filament_self_index))(
        (ConfigOptionBool, support_object_skip_flush))((ConfigOptionEnum<BedTempFormula>, bed_temperature_formula))(
        (ConfigOptionInts, physical_extruder_map))((ConfigOptionIntsNullable, nozzle_flush_dataset))(
    ...
    ((ConfigOptionPercents, retract_after_wipe))
        ...
        ((ConfigOptionFloats, retraction_length))((ConfigOptionFloats, retract_length_toolchange))(
            (ConfigOptionInt, enable_long_retraction_when_cut))((ConfigOptionFloats, retraction_distances_when_cut))(
            (ConfigOptionBools, long_retractions_when_cut))((ConfigOptionFloatsNullable, retraction_distances_when_ec))(
            (ConfigOptionBoolsNullable, long_retractions_when_ec))((ConfigOptionFloats, z_hop))
    // BBS
    ((ConfigOptionEnumsGeneric, z_hop_types))((ConfigOptionFloats, travel_slope))((ConfigOptionFloats, retract_lift_above))(
        ...
        (ConfigOptionEnum<ToolChangeOrderingType>, toolchange_ordering))((ConfigOptionBool, wipe_tower_no_sparse_layers))(
        (ConfigOptionString, change_filament_gcode))((ConfigOptionString, change_extrusion_role_gcode))(
        (ConfigOptionString, process_change_extrusion_role_gcode))((ConfigOptionStrings, filament_change_extrusion_role_gcode))(
        (ConfigOptionFloatsNullable, travel_speed))((ConfigOptionFloatsNullable, travel_speed_z))((ConfigOptionBool, silent_mode))(
        (ConfigOptionString, machine_pause_gcode))((ConfigOptionString, template_custom_gcode))
    // BBS
    ((ConfigOptionEnumsGenericNullable,
      nozzle_type))((ConfigOptionInt, nozzle_hrc))((ConfigOptionBool, auxiliary_fan))((ConfigOptionEnum<FanDirection>, fan_direction))(
        (ConfigOptionBool, support_air_filtration))((ConfigOptionBool, support_cooling_filter))((ConfigOptionBool, cooling_filter_enabled))(
        (ConfigOptionEnum<PrinterStructure>, printer_structure))((ConfigOptionBool, support_chamber_temp_control))(
        (ConfigOptionEnumsGeneric, extruder_type))((ConfigOptionEnumsGeneric, nozzle_volume_type))(
        (ConfigOptionStrings, extruder_ams_count))((ConfigOptionStrings, extruder_nozzle_stats))((ConfigOptionInts, printer_extruder_id))(
        (ConfigOptionInt, master_extruder_id))((ConfigOptionStrings, printer_extruder_variant))
    ...
    ((ConfigOptionBool, use_firmware_retraction))((ConfigOptionBool, use_relative_e_distances))((ConfigOptionBool, accel_to_decel_enable))(
        (ConfigOptionPercent, accel_to_decel_factor))((ConfigOptionFloatsOrPercentsNullable, initial_layer_travel_speed))(
        (ConfigOptionFloatsOrPercentsNullable, initial_layer_travel_acceleration))(
        (ConfigOptionFloatsOrPercentsNullable, initial_layer_travel_jerk))((ConfigOptionBool, bbl_calib_mark_logo))((ConfigOptionBool,
                                                                                                                     disable_m73))
    ...
    ((ConfigOptionFloat, cooling_tube_retraction))((ConfigOptionFloat, cooling_tube_length))(
        ...
        (ConfigOptionFloats, filament_toolchange_delay))((ConfigOptionInts, filament_cooling_moves))(
        (ConfigOptionFloats, filament_cooling_initial_speed))((ConfigOptionFloats, filament_minimal_purge_on_wipe_tower))(
        (ConfigOptionFloatsNullable, filament_cooling_before_tower))((ConfigOptionFloats, filament_tower_interface_pre_extrusion_dist))(
        ...
        (ConfigOptionFloats, filament_cooling_final_speed))((ConfigOptionStrings, filament_ramming_parameters))(
        (ConfigOptionBools, filament_multitool_ramming))((ConfigOptionFloats, filament_multitool_ramming_volume))(
        (ConfigOptionFloats, filament_multitool_ramming_flow))((ConfigOptionFloats, filament_stamping_loading_speed))(
        (ConfigOptionFloats, filament_stamping_distance))((ConfigOptionEnum<WipeTowerType>, wipe_tower_type))(
        (ConfigOptionBool, purge_in_prime_tower))((ConfigOptionBool, enable_filament_ramming))(
        (ConfigOptionBool, tool_change_on_wipe_tower))((ConfigOptionBool, support_multi_bed_types))((ConfigOptionBool, use_3mf))
    ...
    ((ConfigOptionFloat, machine_hotend_change_time))((ConfigOptionFloat, machine_prepare_compensation_time))(
        (ConfigOptionBool, enable_pre_heating))((ConfigOptionFloatsNullable, hotend_cooling_rate))(
        (ConfigOptionFloatsNullable, hotend_heating_rate))((ConfigOptionFloats, filament_change_length_nc))(
        (ConfigOptionFloatsNullable, filament_ramming_travel_time))((ConfigOptionIntsNullable, filament_pre_cooling_temperature))(
        ...
        (ConfigOptionFloatsNullable, filament_retract_length_nc))((ConfigOptionIntsNullable, extruder_max_nozzle_count))
    ...
    // ams chamber
    ((ConfigOptionStrings, filament_dev_ams_drying_ams_limitations))((ConfigOptionFloats, filament_dev_ams_drying_temperature))(
        (ConfigOptionFloats, filament_dev_ams_drying_time))((ConfigOptionFloats, filament_dev_ams_drying_heat_distortion_temperature))(
        (ConfigOptionFloats, filament_dev_chamber_drying_bed_temperature))((ConfigOptionFloats, filament_dev_chamber_drying_time))(
...
PRINT_CONFIG_CLASS_DERIVED_DEFINE(
    ...
    // BBS
    ((ConfigOptionInts, additional_cooling_fan_speed))((ConfigOptionInts, close_additional_fan_first_x_layers))(
        (ConfigOptionInts, additional_fan_full_speed_layer))((ConfigOptionFloats, first_x_layer_fan_speed))(
        (ConfigOptionBool, reduce_crossing_wall))((ConfigOptionFloatOrPercent, max_travel_detour_distance))(
        (ConfigOptionPoints, printable_area))((ConfigOptionPointsGroups, extruder_printable_area))((ConfigOptionBool,
                                                                                                    support_parallel_printheads))(
        (ConfigOptionInt, parallel_printheads_count))((ConfigOptionStrings, parallel_printheads_bed_exclude_areas))
    ...
    ((ConfigOptionFloat, extruder_clearance_radius))((ConfigOptionFloat, nozzle_height))((ConfigOptionStrings, extruder_colour))(
        ...
        (ConfigOptionBools, activate_air_filtration))((ConfigOptionBools, activate_air_filtration_during_print))(
        (ConfigOptionBools, activate_air_filtration_on_completion))((ConfigOptionInts, during_print_exhaust_fan_speed))(
        (ConfigOptionInts, complete_print_exhaust_fan_speed))((ConfigOptionFloatOrPercent, initial_layer_line_width))(
        (ConfigOptionFloat, initial_layer_print_height))((ConfigOptionFloatsNullable, initial_layer_speed))
    ...
    ((ConfigOptionInts, initial_layer_fan_speed))((ConfigOptionFloats, fan_max_speed))((ConfigOptionFloats, max_layer_height))(
    ...
    ((ConfigOptionFloats, filament_prime_volume_nc))((ConfigOptionFloatsNullable, filament_preheat_temperature_delta))((ConfigOptionFloats,
                                                                                                                        flush_multiplier))
    ...
    ((ConfigOptionEnum<PrimeVolumeMode>, prime_volume_mode))((ConfigOptionFloats, flush_multiplier_fast))((ConfigOptionFloat, z_offset))
    // BBS: project filaments
    ((ConfigOptionFloats, filament_colour_new))
    // BBS: not in any preset, calculated before slicing
    ((ConfigOptionFloatsNullable, nozzle_volume))((ConfigOptionPoints, start_end_points))((ConfigOptionEnum<TimelapseType>, timelapse_type))
    // Corexy farthest-point timelapse (default false → inert for existing printers)
    ((ConfigOptionBool, farthest_point_timelapse))((ConfigOptionString, thumbnails))
    // BBS: move from PrintObjectConfig
    ((ConfigOptionBool, independent_support_layer_height))((ConfigOptionBool, combine_brims))
    // SoftFever
    ...
    ((ConfigOptionStrings, filament_notes))((ConfigOptionString, notes))((ConfigOptionString, printer_notes))
        ...
        ((ConfigOptionBools, activate_chamber_temp_control))((ConfigOptionInts, chamber_temperature))((ConfigOptionInts,
