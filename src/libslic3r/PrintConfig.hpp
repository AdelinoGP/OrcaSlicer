...
struct PnpConfigKeyDef
{
    ...
    std::string                 tooltip;
    std::string                 sidetext;
    // Schema `advanced` flag (wire 1.1.0+): true -> comExpert, false ->
    // comAdvanced. Decides which mode tier the generated PNP settings page
    // shows the control under; host keys arriving without the flag register
    // at comAdvanced.
    bool                        advanced { false };
