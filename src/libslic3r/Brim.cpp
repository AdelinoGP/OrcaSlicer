// [INTENT] Brim.cpp implements first-layer adhesion lines ("brim") around print
//          objects.  This is a heavily BBS/Orca-extended version of the original
//          PrusaSlicer brim generator.  Key extensions over upstream:
//
//   1. Per-extruder brim generation ordered by (object, extruder) pairs
//   2. Auto-brim width via structural heuristics (second moment of area, height,
//      adhesion coefficient per filament type) — BBS contribution
//   3. Painted brim ears: user-placed anchor discs placed at BrimPoints stored in
//      model metadata — BBS/Bambu-specific feature
//   4. Auto brim ears: convex/concave vertex detection (ported from SuperSlicer)
//   5. EFC (Elephant Foot Compensation) outline snapping for brim/ear placement
//      so brim follows the post-compensation footprint — ORCA contribution
//   6. Per-extruder printable-area clipping to prevent brim outside reachable zone
//   7. Per-object wipe tower no-brim exclusion zone
//
// [COUPLING] Reads from: Print, PrintObject, Layer, LayerRegion, Model, ModelVolume,
//            MaterialType, PrintConfig.  Writes to: brimMap, supportBrimMap
//            (caller-owned maps), PrintObject::firstLayerObjectBrimBoundingBox.
//            All polygon operations use ClipperUtils (Clipper2 wrappers).
//
// [CONCURRENCY] make_brim() is single-threaded except for tbb::parallel_for in
//          makeBrimInfill() which parallelises polyline chaining per brim loop.
//          No shared mutable state inside the parallel body.
...
// [INTENT] append_and_translate (ExPolygon/dst-only overload) — appends src to
//          dst then translates newly appended entries by the instance world shift.
//          Plate offset excluded; applied globally at end of make_brim().
//          Used for accumulating no_brim_area, holes, objectIslands.
...
// [INTENT] append_and_translate (per-object brimAreaMap overload) — shifts src,
//          then subtracts already-accumulated brim areas (diff_ex) to avoid
//          overlapping brim polygons between instances, then stores the result
//          into brimAreaMap[object_id].  The diff ensures that earlier instances
//          claim their territory first; later instances get the remainder only.
// [HAZARD H749] The diff against dst (the global brim_area accumulator) can
//          leave gaps when instances of different objects interleave: if object A
//          instance 1 runs before object B instance 1, object B may get less brim
//          area near object A, even if its configuration requests a full brim width.
static void append_and_translate(
    ExPolygons& dst, const ExPolygons& src, const PrintInstance& instance, const Print& print, std::map<ObjectID, ExPolygons>& brimAreaMap)
{
...
{
    return object.config().brim_use_efc_outline.value && object.config().elefant_foot_compensation.value > 0. &&
           object.config().elefant_foot_compensation_layers.value > 0 && object.config().raft_layers.value == 0;
}
...
static bool closest_point_on_matching_island(const ExPolygons& raw_outline,
                                             const ExPolygons& efc_outline,
                                             const Point&      from,
                                             Point&            closest_out)
{
...
// [INTENT] Returns the adhesion coefficient for the print object, used to scale
//          auto-brim width.  The coefficient is looked up from Model::extruderParamsMap
//          using the filament type name, then forwarded to MaterialType::get_adhesion_coefficient().
//          Default value is 1.0 (neutral); high-adhesion filaments like PLA may be < 1.0,
//          low-adhesion (PP, PEEK, PVDF) > 1.0.
// [HAZARD H750] Only the last matching volume/extruder combination wins — the loop
//          overwrites adhesionCoeff on each match.  For multi-material objects with
//          different adhesion coefficients per volume, only the last found value is used.
// [HAZARD H751] Dead code: the block after "return adhesionCoeff" is a commented-out
//          Python-style list of filament names — leftover from an earlier design.
//          It confuses code readers but is harmless.
...
// [INTENT] Computes the second moment of area (moment of inertia) of a polygon
//          using the shoelace-based formula:
//            I = Σ a_i * (p1² + p1·p2 + p2²) / 12  for each edge (p1→p2)
//          where a_i = cross(p1, p2) is the signed area contribution.
//          Result sm.x = I_xx (about x-axis), sm.y = I_yy (about y-axis).
//          Used downstream by configBrimWidthByVolumeGroups to estimate
//          overturning stiffness of the first-layer cross-section.
// [HAZARD H752] Polygon coordinates are in scaled integer units (×1e6 mm).
//          The second moment is therefore in (scaled units)^4.  The caller
//          must apply SCALING_FACTOR^4 to convert to mm^4 before using.
//          Missing this conversion would give wildly wrong brim widths.
bool compSecondMoment(Polygon poly, Vec2d& sm)
{
    ...
    if (poly.points.size() >= 3) {
        ...
        for (const Point& p : poly.points) {
            ...
            sm += Vec2d((p1.y() * p1.y() + p1.y() * p2.y() + p2.y() * p2.y()), (p1.x() * p1.x() + p1.x() * p2.x() + p2.x() * p2.x())) * a /
                  12;
            p1 = p2;
        }
        return true;
...
// [INTENT] Auto-brim width heuristic for a volume group.  The formula combines:
//   1. height_to_area: ratio of (object height)² to (second moment of area × cross-section)
//      scaled by max print speed — tall, slender objects with high speed get wider brims.
//   2. thermal length factor: thermalLength × 8 / thermalLengthRef × min(h,30)/30 —
//      thermally sensitive filaments on small footprints get extra brim regardless of height.
//   3. Final brim_width = adhesionCoeff × min(min(max(h2a×speed, thermal), 18), 1.5×thermalLen)
//   4. Small brims (< 5 mm and < 1.5× thermal length) are zeroed out — avoid micro-brims.
//   5. Large brims capped at 18 mm.
//
// [HAZARD H752] SCALING_FACTOR^4 applied to Ixx/Iyy to convert from scaled integer units
//          to mm^4 — must remain consistent with how compSecondMoment accumulates values.
// [HAZARD H753] height_to_area divides by Ixx/Iyy.  If the polygon degenerates (e.g.,
//          single very thin sliver), Ixx or Iyy may be near-zero, causing division by a
//          number very close to zero — extremely large height_to_area and thus 18mm brim cap.
// [HAZARD H754] The constant 1920 in height_to_area is an empirically-tuned denominator
//          (BBS-internal) with no documented derivation.  Changing max speed units from mm/s
//          to mm/min would break the formula silently.
double configBrimWidthByVolumeGroups(
    double adhesion, double maxSpeed, const std::vector<ModelVolume*> modelVolumePtrs, const ExPolygons& expolys, double& groupHeight)
{
    ...
    double height_to_area = std::max(height / Ixx * (bbox2.size()(1) * SCALING_FACTOR), height / Iyy * (bbox2.size()(0) * SCALING_FACTOR)) *
                            height / 1920;
    double brim_width = adhesion * std::min(std::min(std::max(height_to_area * maxSpeed,
...
static ExPolygons make_brim_ears_auto(
    const ExPolygons& obj_expoly, coord_t size_ear, coord_t ear_detection_length, coordf_t brim_ears_max_angle, bool is_outer_brim)
{
    ...
    for (const ExPolygon& poly : obj_expoly) {
        ...
        append(pt_ears,
               is_outer_brim ? decimated_polygon.convex_points(angle_threshold) : decimated_polygon.concave_points(angle_threshold));
    }
    ...
    for (auto& pt : brim_ear_points) {
        ...
        if (use_efc_outline) {
            ...
            if (!efc_outline->empty()) {
                ...
                if (closest_point_on_matching_island(raw_outline, *efc_outline, Point(pt_x, pt_y), closest_point)) {
...
// [INTENT] outer_inner_brim_area is the central brim polygon computation engine.
//          It iterates over all (extruder, object) pairs in objPrintVec (the order
//          objects are printed) and builds per-object brim area ExPolygons, which
//          are accumulated into brimAreaMap and supportBrimAreaMap.
//
//          Algorithm outline:
//   1. For each extruder × object pair:
//      a. Compute brim_slices (raw lslices or EFC-adjusted bottom layer)
//      b. For each volume group in firstLayerObjGroups():
//         - Auto-brim: compute brim_width via configBrimWidthByVolumeGroups()
//         - For each first-layer ExPolygon in the volume group:
//           * Outer brim: offset contour outward by brim_width_mod, diff inner gap
//           * Inner brim: shrink hole inward by brim_width, intersect with hole boundary
//           * Ear brim (painted or auto): substitute disc patterns instead of offsets
//      c. Translate per-instance and store in brimAreaMap/supportBrimAreaMap
//   2. Clip all brim areas outside per-extruder printable zones (30 mm border expansion)
//   3. Clip wipe tower footprint from no_brim_area
//   4. Connectivity filter: remove brim polygons not adjacent to an object island
//      or another brim polygon (prevents free-floating strands)
//
// [COUPLING] Reads: Print.objects(), PrintObject.firstLayerObjGroups(),
//            PrintObject.support_layers(), PrintObject.layers().front()->lslices.
//            Writes: brimAreaMap[id], supportBrimAreaMap[id] (accumulated by reference).
//
// [HAZARD H749] Per-instance diff against global brim_area accumulator causes
//          later instances to receive less brim than earlier instances of different
//          objects — ordering dependency in objPrintVec.
// [HAZARD H755] Connectivity filter uses offset by 2×flow_spacing and intersection
//          checks.  For very small isolated objects this may zero out brim entirely
//          even when a brim was requested (the object island doesn't overlap the
//          expanded brim area within 2×spacing).
// [HAZARD H756] Support brim code path has large commented-out blocks — brim is
//          explicitly NOT generated for supports (comment "Brim will not be generated
//          for supports").  Callers relying on supportBrimAreaMap being non-empty
//          for support adhesion will be surprised.
                                        ...
                                        std::vector<unsigned int>&                      printExtruders)
{
    ...
    for (unsigned int extruderNo : printExtruders) {
        ...
        for (const auto& objectWithExtruder : objPrintVec) {
            ...
            const bool has_outer_brim = brim_type == btOuterOnly || brim_type == btOuterAndInner || brim_type == btAutoBrim ||
                                        use_auto_brim_ears || use_brim_ears;
            ...
            if (objectWithExtruder.second == extruderNo && brimToWrite.at(object->id()).obj) {
                ...
                for (const auto& volumeGroup : object->firstLayerObjGroups()) {
                    ...
                    for (auto& volumeID : volumeGroup.volume_ids) {
                        ...
                        if (currentModelVolumePtr != nullptr)
                            groupVolumePtrs.push_back(currentModelVolumePtr);
                    }
                    if (groupVolumePtrs.empty())
                        continue;
                    double groupHeight = 0.;
                    // config brim width in auto-brim mode
                    if (has_brim_auto) {
                        double brimWidthRaw = configBrimWidthByVolumeGroups(adhesion, maxSpeed, groupVolumePtrs, volumeGroup.slices,
                                                                            groupHeight);
                    ...
                    for (const ExPolygon& ex_poly : *volume_group_slices) {
                        ...
                        if (brim_width < scale_(5.) && has_brim_auto && groupHeight > 10.) {
                            brim_width_mod = ex_poly.area() / ex_poly.contour.length() < scaled_half_min_adh_length &&
                                                     brim_width < scaled_flow_width ?
                                                 brim_width + scaled_additional_brim_width :
                                                 brim_width;
                        } else {
                        ...
                        if (has_inner_brim) {
                            ...
                            if (use_brim_ears) {
                                ...
                                coord_t size_ear = (brim_width - brim_offset - flow.scaled_spacing());
                                outerExpoly      = make_brim_ears_auto(offset_ex(ex_poly_holes_reversed, -brim_offset), size_ear,
                                                                       ear_detection_length, brim_ears_max_angle, false);
                            } else {
            ...
            if (support_material_extruder == extruderNo && brimToWrite.at(object->id()).sup) {
                if (!object->support_layers().empty() && object->support_layers().front()->support_type == stInnerNormal) {
                    for (const Polygon& support_contour : object->support_layers().front()->support_fills.polygons_covered_by_spacing()) {
                        ...
                        /*
                        if (has_outer_brim) {
                            append(brim_area_support, diff_ex(offset_ex(support_contour, brim_width + brim_offset, jtRound,
                        SCALED_RESOLUTION), offset_ex(support_contour, brim_offset)));
                        }
                ...
                if (!object->support_layers().empty() && object->support_layers().front()->support_type == stInnerTree) {
                    for (const ExPolygon& ex_poly : object->support_layers().front()->lslices) {
                        ...
                        /*
                        if (has_outer_brim) {
                            append(brim_area_support, diff_ex(offset_ex(ex_poly.contour, brim_width_mod + brim_offset, jtRound,
                        SCALED_RESOLUTION), offset_ex(ex_poly.contour, brim_offset)));
                        }
                        if (has_inner_brim)
                            append(brim_area_support, diff_ex(offset_ex(ex_poly.holes, -brim_offset), offset_ex(ex_poly.holes, -brim_width -
                        brim_offset)));
                        */
                        if (!has_outer_brim)
    ...
    for (const PrintObject* object : print.objects()) {
        ...
        auto       iter                  = std::find_if(objPrintVec.begin(), objPrintVec.end(),
                                                        [object](const std::pair<ObjectID, unsigned int>& item) { return item.first == object->id(); });
        ...
        if (iter != objPrintVec.end()) {
            ...
            // extruder_no_brim_area = offset2_ex(extruder_no_brim_area, scaled_flow_width, -scaled_flow_width); // connect scattered small
            // areas to prevent generating very small brims
        }
        ...
        if (brimAreaMap.find(object->id()) != brimAreaMap.end()) {
            ...
            for (int ia = 0; ia != tempArea.size(); ++ia) {
                ...
                for (int iao = 0; iao != tempArea.size(); ++iao)
                    if (iao != ia)
                        otherExPoly.push_back(tempArea[iao]);

                auto offsetedTa = offset_ex(tempArea[ia], print.brim_flow().scaled_spacing() * 2, jtRound, SCALED_RESOLUTION);
                if (!intersection_ex(offsetedTa, objectIslands).empty() || !intersection_ex(offsetedTa, otherExPoly).empty() ||
...
// [INTENT] tryExPolygonOffset produces concentric inward-offset polygon loops
//          from the brim area ExPolygons.  It implements the "snail trail" brim
//          path generation:
//   1. Simplify with Douglas-Peucker at 0.0125/SCALING_FACTOR resolution
//   2. First offset: inward by -0.5 × flow spacing (center of first brim line)
//   3. Loop: collect current polygon as a brim loop, then:
//      a. Offset inward by -1.3 × spacing (oversize step)
//      b. Simplify again
//      c. Offset outward by +0.3 × spacing (net: -1.0 × spacing per iteration)
//      This two-step ±offset is a "robust offset" technique to collapse thin
//      necks while keeping concentric loops from self-intersecting.
//
// [HAZARD H757] The loop terminates only when islands_ex becomes empty (all
//          contours have collapsed).  For very complex polygons with tiny features,
//          this can run many iterations.  There is no maximum iteration cap.
// [HAZARD H758] The 1.3/-0.3 split is a heuristic magic number (not documented).
//          Porting this to a different geometry library requires understanding this
//          intent, not just the literal values.
...
// [INTENT] makeBrimInfill converts the abstract brim ExPolygon area into concrete
//          G-code-ready ExtrusionEntityCollection.  Pipeline:
//   1. tryExPolygonOffset → concentric Polygon loops from brim area
//   2. union_pt_chained_outside_in → outside-in ordering for seam control
//   3. tbb::parallel_for → each loop polyline independently chained
//   4. optimize_polylines_by_reversing → greedy flip to minimize travel distance
//   5. connect_brim_lines → stitch adjacent open ends that don't cross brim boundaries
//   6. Plate offset translation (BBS multi-plate: very large offsets possible)
//   7. extrusion_entities_append_loops_and_paths → wrap in ExtrusionPath/Loop entities
//
// [CONCURRENCY] tbb::parallel_for over per-loop chain_polylines — safe because
//          each iteration writes to its own loops_pl_by_levels[i] slot.
//
// [HAZARD H759] The plate offset is applied AFTER connect_brim_lines.  The
//          connection line collision test uses brim centrelines (before plate offset).
//          This is correct because islands_area (passed to connect_brim_lines) is also
//          pre-plate-offset.  If the caller ever passes post-offset islands_area, lines
//          would be double-offset and connections would miss/hit incorrectly.
ExtrusionEntityCollection makeBrimInfill(const ExPolygons& singleBrimArea, const Print& print, const Polygons& islands_area)
{
    ...
    extrusion_entities_append_loops_and_paths(brim.entities, std::move(all_loops), erBrim, float(flow.mm3_per_mm()), float(flow.width()),
...
// [INTENT] make_brim is the public entry point for brim generation.  It orchestrates:
//   1. outer_inner_brim_area — compute per-object brim ExPolygons (clipped, filtered)
//   2. firstLayerObjectBrimBoundingBox — populate per-object bounding box for UI/G-code
//   3. islands_area — extract flat Polygon list for skirt/hull merging, apply plate offset
//   4. makeBrimInfill per object and per support — convert ExPolygons to ExtrusionEntityCollection
//
// [STATE] Side-effect: sets PrintObject::firstLayerObjectBrimBoundingBox for each object
//         (via const_cast<PrintObject*>).  This is the only mutation of PrintObject from
//         the brim module.
// [HAZARD H760] const_cast on print.get_object() to access firstLayerObjectBrimBoundingBox.
//         If PrintObject is ever shared across threads concurrently, this write is unsynchronized.
               ...
               std::vector<unsigned int>&                      printExtruders)
{
    ...
    Flow                           flow              = print.brim_flow();
    const auto                     scaled_resolution = scaled<double>(print.config().resolution.value);
    ExPolygons islands_area_ex = outer_inner_brim_area(print, float(flow.scaled_spacing()), brimAreaMap, supportBrimAreaMap, objPrintVec,
                                                       printExtruders);
    ...
    for (const ObjectID printObjID : print.print_object_ids()) {
        ...
        const ExPolygons brim_slices = use_brim_efc_outline(*object) ? get_print_object_bottom_layer_expolygons(*object) :
