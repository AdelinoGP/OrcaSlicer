...
// [INTENT] Triangle selection and painting system for GUI mesh editing.
// Allows painting enforcer/blocker regions on 3D meshes for multi-material printing,
// support generation control, and fuzzy skin areas.
// [COUPLING] Uses TriangleMesh, cereal for serialization, depends on GUI for painting.
// [MEMORY] Maintains parallel triangle/vertex arrays with reference counting for proper cleanup.
// [CONCURRENCY] Has data race protection notes - state field placement critical for thread safety.
...
// [INTENT] Triangle face state types for painting/selection system.
// NONE: default state, no special handling
// ENFORCER: force certain operation (e.g., support generation, seam placement)
// BLOCKER: prevent certain operation
// FUZZY_SKIN: special state for fuzzy skinning feature
// Extruder1-16: multi-material painting states
// [HAZARD] Enum values serialized to 3MF - changing values breaks backward compatibility.
// [MEMORY] Stored in 2-6 bits per triangle depending on mode - compact representation.
enum class EnforcerBlockerType : int8_t {
    ...
    // [HAZARD] FUZZY_SKIN aliases ENFORCER value - relies on caller distinguishing by context!
    FUZZY_SKIN = ENFORCER,
...
public:
    // [INTENT] Cursor types for different painting modes in GUI.
    // CIRCLE: 2D circle brush for painting
    // SPHERE: 3D spherical brush
    // POINTER: single-point selection
    // HEIGHT_RANGE: BBS addition - paint by Z height range
    // GAP_FILL: BBS addition - paint gap fill areas
    enum CursorType {
        CIRCLE,
        SPHERE,
        POINTER,
    ...
    public:
        ...
        static std::unique_ptr<Cursor> cursor_factory(const Vec3f&         center,
        ...
        static std::unique_ptr<Cursor> cursor_factory(float                z_world,
                                                      const Vec3f&         camera_pos,
                                                      const float          height,
                                                      const Transform3d&   trafo_matrix,
                                                      const ClippingPlane& clipping_plane)
    ...
    protected:
        explicit SinglePointCursor(const Vec3f&         center_,
                                   const Vec3f&         source_,
                                   float                radius_world,
    ...
    public:
        ...
        static std::unique_ptr<Cursor> cursor_factory(const Vec3f&         first_center,
                                                      ...
                                                      const ClippingPlane& clipping_plane)
        {
            assert(cursor_type == TriangleSelector::CursorType::CIRCLE || cursor_type == TriangleSelector::CursorType::SPHERE);
            if (cursor_type == TriangleSelector::CursorType::SPHERE)
                return std::make_unique<TriangleSelector::Capsule3D>(first_center, second_center, camera_pos, cursor_radius, trafo_matrix,
                                                                     clipping_plane);
            else
                return std::make_unique<TriangleSelector::Capsule2D>(first_center, second_center, camera_pos, cursor_radius, trafo_matrix,
                                                                     clipping_plane);
        }

    protected:
        explicit DoublePointCursor(const Vec3f&         first_center_,
    ...
    public:
        ...
        explicit HeightRange(
            float z_world_, const Vec3f& source_, float height_, const Transform3d& trafo_, const ClippingPlane& clipping_plane_)
    ...
    public:
        Capsule3D() = delete;
        explicit Capsule3D(const Vec3f&         first_center_,
                           const Vec3f&         second_center_,
                           const Vec3f&         source_,
                           float                radius_world,
                           const Transform3d&   trafo_,
    ...
    public:
        Capsule2D() = delete;
        explicit Capsule2D(const Vec3f&         first_center_,
                           const Vec3f&         second_center_,
                           const Vec3f&         source_,
                           float                radius_world,
                           const Transform3d&   trafo_,
    ...
    struct TriangleBitStreamMapping
    {
        ...
        explicit TriangleBitStreamMapping(int triangleIdx, int bitstreamStartIdx)
            : triangle_idx(triangleIdx), bitstream_start_idx(bitstreamStartIdx)
        {}

        friend bool operator==(const TriangleBitStreamMapping& lhs, const TriangleBitStreamMapping& rhs)
        {
            return lhs.triangle_idx == rhs.triangle_idx && lhs.bitstream_start_idx == rhs.bitstream_start_idx;
        }
    ...
    struct TriangleSplittingData
    {
        ...
        {
            return lhs.triangles_to_split == rhs.triangles_to_split && lhs.bitstream == rhs.bitstream && lhs.used_states == rhs.used_states;
        }
    ...
    void                                                  precompute_all_neighbors_recursive(int                   facet_idx,
                                                                                             const Vec3i32&        neighbors,
                                                                                             const Vec3i32&        neighbors_propagated,
                                                                                             std::vector<Vec3i32>& neighbors_out,
                                                                                             std::vector<Vec3i32>& neighbors_normal_out) const;
    ...
    void seed_fill_select_triangles(
        const Vec3f&         hit,                // point where to start
        int                  facet_start,        // facet of the original mesh (unsplit) that the hit point belongs to
        const Transform3d&   trafo_no_translate, // matrix to get from mesh to world without translation
        const ClippingPlane& clp,                // Clipping plane to limit painting to not clipped facets only
        float                seed_fill_angle,    // the maximal angle between two facets to be painted by the same color
        float highlight_by_angle_deg = 0.f, // The maximal angle of overhang. If it is set to a non-zero value, it is possible to paint only
                                            // the triangles of overhang defined by this angle in degrees.
        bool force_reselection =
            false); // force reselection of the triangle mesh even in cases that mouse is pointing on the selected triangle

    void bucket_fill_select_triangles(
        const Vec3f&         hit,             // point where to start
        int                  facet_start,     // facet of the original mesh (unsplit) that the hit point belongs to
        const ClippingPlane& clp,             // Clipping plane to limit painting to not clipped facets only
        float                seed_fill_angle, // BBS: the maximal angle between two facets to be painted by the same color
        bool propagate, // if bucket fill is propagated to neighbor faces or if it fills the only facet of the modified mesh that the hit
                        // point belongs to.
        bool force_reselection =
            false); // force reselection of the triangle mesh even in cases that mouse is pointing on the selected triangle
    ...
    // [INTENT] Serialize triangle selection state to compact bitstream format.
    // Stores: triangle index -> bitstream start position mapping,
    //         bitstream with split information encoded per triangle,
    //         which enforcer/blocker states are actually used.
    // Used for: saving to 3MF, undo/redo, clipboard.
    // [MEMORY] Bitstream uses ~2 bits per triangle + overhead - much smaller than full object.
    // [COUPLING] Uses cereal for serialization.
    TriangleSplittingData serialize() const;

    // [INTENT] Deserialize bitstream and restore triangle selection state.
    // [PARAM] needs_reset: clear existing state before loading
    // [PARAM] max_ebt: limit which states to load (for backward compatibility)
    // [PARAM] to_delete_filament, replace_filament: multi-material remapping
    // [HAZARD] Complex deserialization with multiple optional transformations.
    ...
    // For all triangles selected by seed fill, set new EnforcerBlockerType and remove flag indicating that triangle was selected by seed
    // fill. The operation may merge split triangles if they are being assigned the same color.
...
protected:
    ...
    public:
        ...
        Triangle(int a, int b, int c, int source_triangle, const EnforcerBlockerType init_state)
            : verts_idxs{a, b, c}, source_triangle{source_triangle}, state{init_state}
        {
    ...
    private:
        ...
        // [HAZARD] This ordering is critical for thread-safe painting operations!
        // [MEMORY] Must be last member - relies on compiler not reordering.
        EnforcerBlockerType state;
    };

    struct Vertex
    {
        explicit Vertex(const stl_vertex& vert) : v{vert}, ref_cnt{0} {}
        stl_vertex v;
        int        ref_cnt;
    };
...
private:
    ...
    Vec3i32 child_neighbors_propagated(const Triangle& tr,
                                       const Vec3i32&  neighbors_propagated,
                                       int             child_idx,
    ...
    void get_facets_split_by_tjoints(const Vec3i32&                            vertices,
                                     const Vec3i32&                            neighbors,
                                     std::vector<stl_triangle_vertex_indices>& out_triangles) const;

    void get_seed_fill_contour_recursive(int                   facet_idx,
                                         const Vec3i32&        neighbors,
                                         const Vec3i32&        neighbors_propagated,
