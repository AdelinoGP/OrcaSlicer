// [INTENT] Arc fitting and arc-segment geometry for the BBS arc-welding G-code optimisation.
//          Ported from ArcWelderLib (open-source). Converts sequences of linear points into
//          G2/G3 arc moves to reduce G-code file size and improve firmware motion quality.
// [STATE]  All functions are stateless helpers or operate on value-type Circle/ArcSegment structs.
//          No shared mutable state — safe to call from multiple threads simultaneously.
// [COUPLING] Depends on Geometry.hpp for coordinate types and constants (PI, EPSILON,
//            SCALED_EPSILON, scale_()). ArcSegment extends Circle with direction/angle data.
#include "Circle.hpp"
...
// [INTENT] Collinearity guard: triangle area below this threshold → points treated as on a line.
// [HAZARD H1155] Threshold is in unscaled mm²; code comment says area scales twice, but the
//                check uses scale_(scale_(...)) which is correct. However Parallel_area_threshold
//                (0.0001 mm²) may be too tight for very short point sequences produced by
//                Arc-welder preprocessing — collinear triples may slip through the guard.
// BBS: threshold used to judge collineation
static const double Parallel_area_threshold = 0.0001;

// [INTENT] Circumcircle from three 2-D points using Cramer's rule on the perpendicular bisectors.
// [STATE]  Purely functional — writes result into new_circle, returns false on near-collinear input
//          or radius exceeding max_radius. Points are in scaled integer coordinates (coord_t).
...
// [INTENT] Multi-point arc fit: first tries the midpoint heuristic (O(1)), then falls back
//          to a linear scan over all interior candidate points to find the fit with least
//          squared deviation. O(n) total arc-fit attempts, each O(n) deviation check → O(n²).
// [HAZARD H1156] middle_point for even count (line 66) averages adjacent pair; for odd count
//                it averages points at middle_index-1 and middle_index+1 — this SKIPS the actual
//                middle point (middle_index). This appears to be a port bug from ArcWelderLib.
bool Circle::try_create_circle(const Points& points, const double max_radius, const double tolerance, Circle& new_circle)
{
    ...
    for (int index = 1; index < count - 1; index++) {
        ...
        if (Circle::try_create_circle(points[0], points[index], points[count - 1], max_radius, test_circle) &&
            test_circle.get_deviation_sum_squared(points, tolerance, current_deviation)) {
...
bool ArcSegment::is_point_inside(const Point& point) const
{
    ...
    return (direction == ArcDirection::Arc_Dir_CCW ? radian_delta > 0.0 && radian_delta < angle_radians :
...
bool ArcSegment::try_create_arc(const Points& points,
                                ...
                                double        path_tolerance_percent)
{
    ...
    ArcSegment test_arc;
    if (!ArcSegment::try_create_arc(test_circle, points[0], points[mid_point_index], points[points.size() - 1], test_arc,
                                    approximate_length, path_tolerance_percent))
        return false;
...
bool ArcSegment::try_create_arc(const Circle& c,
...
// [INTENT] Validate that every interior point of `points` lies within the angular slice of the
//          arc (between start and end radii vectors). Also checks that the polygon edges do not
//          cross either bounding radius ray.
// [HAZARD H1158] Loop starts at index = point_count - 2 (second-to-last point), iterates to
//               point_count - 1. This means only the LAST TWO points are actually checked for
//               angular ordering; all interior points [1 .. point_count-3] are skipped. The
//               in-slice containment checks at line 410/426 use `index < point_count - 1` guard
//               correctly, but the `previous_polar > polar_test` monotonicity check (line 416)
//               only fires once. This is almost certainly a port defect from ArcWelderLib where
//               the loop was meant to iterate from 1 to point_count-1.
...
// [HAZARD H1157] calc_arc_radian uses `abs(radian)` (integer abs from <cstdlib>) for the CW
//               branch (line 498) instead of `std::fabs` or `fabsf`. On most compilers this
//               truncates the float to int before taking absolute value, producing wrong arc
//               lengths for arcs with |radian| < 1 radian.
