...
// [INTENT] Platform utility layer: logging (Boost.Log), file I/O (copy/rename/check),
// path management (g_var_dir, g_resources_dir, etc.), UTF-8/locale helpers,
// memory reporting, XML escaping, and TBB thread count control.
// [STATE] All g_*_dir globals are file-scope static std::string; set once at startup,
// read-only during slicing — no mutex.  Thread-safe only if writes complete before
// any parallel reads (assumed by convention, not enforced).
// [COUPLING] Included by nearly every libslic3r translation unit via libslic3r.h.
//            Windows rename/copy paths borrow code from LLVM; Linux copy path borrows
//            from boost::filesystem.
// [HAZARD H1136] copy_file() on Windows: error_message = "Error: " + errCode where
//   errCode is DWORD (uint32_t) — C++ string+integer arithmetic produces integer not
//   message text.  Error message will contain a garbage numeric string, not a human-
//   readable OS error description.  Fix: use std::system_category().message(errCode).
// [HAZARD H1137] normalize_utf8_nfc(): uses `static std::locale locale_utf8` constructed
//   once inside the function body via boost::locale::generator().  If two threads call
//   this function concurrently before the static is initialized (C++11 magic-static
//   init is sequenced, so the construction itself is safe) the locale generator is
//   single-threaded and safe — however, if libslic3r is compiled with a pre-C++11
//   toolchain the initialization is not guarded.  Mark as environment-dependent risk.
...
void trace(unsigned int level, const char* message)
{
    ...
    BOOST_LOG_STREAM_WITH_PARAMS(::boost::log::trivial::logger::get(), (::boost::log::keywords::severity = severity)) << message;
}

void disable_multi_threading()
{
    // [INTENT] Force TBB to use a single thread, useful when running the Shiny profiler
    //          or when deterministic sequential output is required for debugging.
    // [STATE] TBB >= 2021: uses tbb::global_control (stack-allocated; exits scope
    //         immediately — does NOT persist past this call; porting note: must be stored
    //         as a member/static if long-lived behaviour is wanted).
    //         TBB < 2021: allocates tbb::task_scheduler_init on the heap and intentionally
    //         leaks it via the static pointer so the scheduler stays limited for process
    //         lifetime.
...
void set_var_dir(const std::string& dir) { g_var_dir = dir; }
...
void set_resources_dir(const std::string& dir) { g_resources_dir = dir; }

const std::string& resources_dir() { return g_resources_dir; }

// BBS: add temporary dir
static std::string g_temporary_dir;
void               set_temporary_dir(const std::string& dir) { g_temporary_dir = dir; }

const std::string& temporary_dir() { return g_temporary_dir; }

static std::string g_local_dir;

void set_local_dir(const std::string& dir) { g_local_dir = dir; }

const std::string& localization_dir() { return g_local_dir; }

static std::string g_sys_shapes_dir;

void set_sys_shapes_dir(const std::string& dir) { g_sys_shapes_dir = dir; }

const std::string& sys_shapes_dir() { return g_sys_shapes_dir; }

static std::string g_custom_gcodes_dir;

void set_custom_gcodes_dir(const std::string& dir) { g_custom_gcodes_dir = dir; }

const std::string& custom_gcodes_dir() { return g_custom_gcodes_dir; }
...
std::string custom_shapes_dir() { return (boost::filesystem::path(g_data_dir) / "shapes").string(); }

std::string handy_models_dir() { return (boost::filesystem::path(resources_dir()) / "handy_models").string(); }
...
void set_log_path_and_level(const std::string& file, unsigned int level)
{
    ...
    g_log_sink = boost::log::add_file_log(keywords::file_name = full_path.string() + ".%N", keywords::rotation_size = 100 * 1024 * 1024,
...
#define MAP_ERR_TO_COND(x, y) \
    case x: return std::make_error_code(std::errc::y)
    ...
    default: return std::error_code(windows_error_code, std::system_category());
    }
...
std::error_code rename(const std::string& from, const std::string& to)
{
    ...
    for (unsigned retry = 0; retry != 200; ++retry) {
        if (retry != 0)
            ::Sleep(10);
        from_handle = ::CreateFileW((LPWSTR) wide_from.data(), GENERIC_READ | DELETE,
                                    FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
                                    NULL);
        if (from_handle)
            break;
    }
    ...
    for (unsigned retry = 0; retry != 200; ++retry) {
        ...
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(",first rename file from %1% to %2% failed, reason: %3%") % from.c_str() % to.c_str() %
                                        errcode.message();
        ...
        // closes it.
        ScopedFileHandle to_handle(::CreateFileW((LPCWSTR) wide_to.data(), GENERIC_READ | DELETE,
        ...
        if (!to_handle) {
            ...
            // BBS: add some log for error tracing
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(",open dest file %1% failed, reason: %2%") % to.c_str() % errcode.message();
            return errcode;
        }
        ...
        for (unsigned unique_id = 0; unique_id != 200; ++unique_id) {
            ...
            if (errcode) {
                if (errcode == std::make_error_code(std::errc::file_exists) ||
                    ...
                    ScopedFileHandle to_handle2(::CreateFileW((LPCWSTR) wide_to.data(), 0,
                                                              FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
                    ...
                    if (!::GetFileInformationByHandle(to_handle2, &FI2))
                        return map_windows_error(GetLastError());
                    if (FI.nFileIndexHigh != FI2.nFileIndexHigh || FI.nFileIndexLow != FI2.nFileIndexLow ||
...
bool copy_file_linux(const boost::filesystem::path& from, const boost::filesystem::path& to, boost::system::error_code& ec)
{
    ...
    if (to_mode != from_mode && ::fchmod(outfile.fd, from_mode) != 0) {
        if (platform_flavor() == PlatformFlavor::LinuxOnChromium) {
            ...
            BOOST_LOG_TRIVIAL(info) << "copy_file_linux() failed to fchmod() the output file \"" << to.string() << "\" to " << from_mode
                                    << ": " << ec.message() << " This may be expected when writing to a 9p filesystem.";
        } else {
            // Generic linux. Write out an error to console. At least we may get some feedback.
            BOOST_LOG_TRIVIAL(error) << "copy_file_linux() failed to fchmod() the output file \"" << to.string() << "\" to " << from_mode
                                     << ": " << ec.message();
        }
    }
...
CopyFileResult copy_file_inner(const std::string& from, const std::string& to, std::string& error_message)
{
    ...
    static const auto             perms = boost::filesystem::owner_read | boost::filesystem::owner_write | boost::filesystem::group_read |
    ...
    if (ec) {
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("###copy_file from %1% to %2% failed, error: %3% ") % source.string() % target.string() %
                                        error_message;
...
CopyFileResult copy_file(const std::string& from, const std::string& to, std::string& error_message, const bool with_check)
{
    ...
    if (!result) {
        ...
        // [HAZARD H1136 detail] "Error: " + errCode performs integer arithmetic on a
        // DWORD, producing a numeric string like "Error: 5" not "Access is denied."
        // Fix: use map_windows_error(errCode).message() or FormatMessageA().
...
CopyFileResult check_copy(const std::string& origin, const std::string& copy)
{
    ...
    do {
        ...
        if (origin_cnt != copy_cnt || (origin_cnt > 0 && std::memcmp(buffer_origin.data(), buffer_copy.data(), origin_cnt) != 0))
            // Files are different.
            return FAIL_FILES_DIFFERENT;
        fsize -= origin_cnt;
...
bool is_json_file(const std::string& path) { return boost::iends_with(path, ".json"); }

bool is_img_file(const std::string& path) { return boost::iends_with(path, ".png") || boost::iends_with(path, ".svg"); }
...
bool is_gallery_file(const std::string& path, char const* type) { return boost::iends_with(path, type); }

bool is_shapes_dir(const std::string& dir) { return dir == sys_shapes_dir() || dir == custom_shapes_dir() || dir == handy_models_dir(); }
...
{
    // [HAZARD H1137 detail] static locale_utf8 is initialized on first call.
    // C++11 guarantees thread-safe static initialization (magic-statics), so
    // concurrent first-call is safe on conformant compilers.  On MSVC < 2015 or
    // MinGW without __MINGW_REDIRECT it is NOT safe.  If called from a TBB parallel
    // region before any prior sequential call, a data race is possible on
    // non-conformant toolchains.
    // [INTENT] Normalize a UTF-8 string to NFC form via Boost.Locale.
...
std::string header_slic3r_generated() { return std::string(SLIC3R_APP_NAME " " SoftFever_VERSION); }

std::string header_gcodeviewer_generated() { return std::string(GCODEVIEWER_APP_NAME " " SoftFever_VERSION); }
...
// [INTENT] XML-escape the five reserved characters in text content.  is_marked=true
//          passes '<' and '>' through verbatim (used for pre-escaped markup).
// [HAZARD] O(n²): std::string::replace() shifts all bytes after each substitution;
//          for a string with k escapes of average replacement length r, total cost is
//          O(n*k*r).  Acknowledged in the FIXME comment.  Fix: build result with a
//          reserve-then-append pattern (O(n)).
...
std::string log_memory_info(bool ignore_loglevel)
{
    ...
    if (ignore_loglevel || logSeverity <= boost::log::trivial::info) {
        ...
        if (GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS*) &pmc, sizeof(pmc)))
            out = " WorkingSet: " + format_memsize_MB(pmc.WorkingSetSize) + "; PrivateBytes: " + format_memsize_MB(pmc.PrivateUsage) +
                  "; Pagefile(peak): " + format_memsize_MB(pmc.PagefileUsage) + "(" + format_memsize_MB(pmc.PeakPagefileUsage) + ")";
        else
            out += " Used memory: N/A";
    ...
    for (auto& dir_entry : boost::filesystem::directory_iterator(source)) {
        ...
        if (boost::filesystem::is_directory(dir_entry)) {
            ...
            if (cfr != CopyFileResult::SUCCESS) {
                ...
                throw Slic3r::CriticalException(Slic3r::format(("Copying directory %1% to %2% failed: %3%"), source, target, error_message));
            }
        }
    }
    return;
