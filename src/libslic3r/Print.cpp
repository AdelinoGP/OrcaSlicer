...
// [INTENT] Tears down all PrintObject instances and resets the Print to an
// empty state. Called by the UI when the user starts a new project or removes
// all objects from the plate.
//
// [MEMORY] PrintObject heap ownership: m_objects holds raw owning pointers
// allocated by Print::apply(). The explicit delete loop is correct but fragile —
// any exception thrown by a PrintObject destructor will leak subsequent objects.
// A refactored design should use std::unique_ptr<PrintObject> throughout.
//
// [CONCURRENCY] Acquires state_mutex before invalidate_all_steps() to stop
// background slicing tasks that may be iterating m_objects. Callers must not
// hold state_mutex themselves (non-recursive mutex).
void Print::clear()
{
...
// [INTENT] Reports whether any filament in the current wipe tower tool ordering
// is TPU (thermoplastic polyurethane). TPU requires special handling because it
// cannot be retracted and may block the wipe tower mechanism.
//
// [COUPLING] Reads m_wipe_tower_data.tool_ordering, which is populated by
// _make_wipe_tower(). This function must only be called after the wipe tower
// step has been processed; early calls will always return false (empty ordering).
                                               ...
                                               const std::vector<t_config_option_key>& opt_keys)
{
    ...
    static std::unordered_set<std::string> steps_gcode =
        {// BBS
         ...
         "fan_speedup_overhangs", "fan_speedup_time", "filament_colour", "default_filament_colour", "filament_diameter",
         "volumetric_speed_coefficients", "filament_density", "filament_cost", "filament_notes", "outer_wall_acceleration",
         "inner_wall_acceleration", "initial_layer_acceleration", "top_surface_acceleration", "bridge_acceleration", "travel_acceleration",
         ...
         "layer_change_gcode", "time_lapse_gcode", "wrapping_detection_gcode", "fan_min_speed", "fan_max_speed", "printable_height",
         "slow_down_min_speed", "max_volumetric_extrusion_rate_slope", "max_volumetric_extrusion_rate_slope_segment_length",
         ...
         "preheat_steps", "machine_start_gcode", "filament_start_gcode", "change_filament_gcode", "wipe",
         // BBS
         ...
         "filament_long_retractions_when_cut", "filament_retraction_distances_when_cut", "grab_length", "bed_temperature_formula",
         "filament_notes", "process_notes", "printer_notes"};
    ...
    for (const t_config_option_key& opt_key : opt_keys) {
        if (steps_gcode.find(opt_key) != steps_gcode.end()) {
            ...
            steps.emplace_back(psSkirtBrim);
                   ...
                   opt_key == "filament_shrinkage_compensation_z" || opt_key == "resolution" ||
            ...
            osteps.emplace_back(posSlice);
                   ...
                   opt_key == "filament_max_volumetric_speed" || opt_key == "filament_adaptive_volumetric_speed" ||
                   opt_key == "filament_loading_speed" || opt_key == "filament_loading_speed_start" ||
                   opt_key == "filament_unloading_speed" || opt_key == "filament_unloading_speed_start" ||
                   opt_key == "filament_toolchange_delay" || opt_key == "filament_cooling_moves" ||
                   opt_key == "filament_stamping_loading_speed" || opt_key == "filament_stamping_distance" ||
                   opt_key == "filament_cooling_initial_speed" || opt_key == "filament_cooling_final_speed" ||
                   ...
                   opt_key == "filament_max_volumetric_speed" || opt_key == "gcode_flavor" || opt_key == "single_extruder_multi_material" ||
                   ...
                   opt_key == "enable_prime_tower" || opt_key == "enable_wrapping_detection" || opt_key == "prime_tower_enable_framework" ||
                   opt_key == "prime_tower_width" || opt_key == "prime_tower_brim_width" || opt_key == "prime_tower_skip_points" ||
                   opt_key == "prime_tower_flat_ironing" || opt_key == "enable_tower_interface_features" ||
                   opt_key == "first_layer_print_sequence" || opt_key == "other_layers_print_sequence" ||
                   opt_key == "other_layers_print_sequence_nums" || opt_key == "extruder_ams_count" || opt_key == "filament_map_mode" ||
                   opt_key == "filament_map" || opt_key == "filament_adhesiveness_category" ||
                   opt_key == "filament_tower_interface_pre_extrusion_dist" || opt_key == "filament_tower_interface_pre_extrusion_length" ||
                   opt_key == "filament_tower_ironing_area" || opt_key == "filament_tower_interface_purge_volume" ||
                   opt_key == "filament_tower_interface_print_temp" || opt_key == "wipe_tower_bridging" ||
                   opt_key == "wipe_tower_extra_flow" || opt_key == "wipe_tower_no_sparse_layers" || opt_key == "flush_volumes_matrix" ||
                   opt_key == "prime_volume" || opt_key == "flush_into_infill" || opt_key == "flush_into_support" ||
                   ...
                   opt_key == "idle_temperature" || opt_key == "wipe_tower_cone_angle" || opt_key == "wipe_tower_extra_spacing" ||
                   opt_key == "wipe_tower_max_purge_speed" || opt_key == "wipe_tower_wall_type" ||
                   opt_key == "wipe_tower_extra_rib_length" || opt_key == "wipe_tower_rib_width" || opt_key == "wipe_tower_fillet_wall" ||
                   opt_key == "wipe_tower_filament" || opt_key == "wiping_volumes_extruders" || opt_key == "enable_filament_ramming" ||
                   opt_key == "purge_in_prime_tower" || opt_key == "z_offset" || opt_key == "support_multi_bed_types") {
            steps.emplace_back(psWipeTower);
            steps.emplace_back(psSkirtBrim);
        } else if (opt_key == "filament_soluble" || opt_key == "filament_is_support" || opt_key == "filament_printable" ||
                   opt_key == "filament_change_length" || opt_key == "independent_support_layer_height") {
            steps.emplace_back(psWipeTower);
...
// [INTENT] Trivial calibration parameter setter. Stores the Calib_Params
// struct (mode + range) so that the G-code generator can emit calibration
// pattern sequences (pressure advance, flow rate, etc.).
...
// [INTENT] Invalidates a single PrintStep and unconditionally cascades to
// psGCodeExport. Any step change renders the cached G-code stale, so
// psGCodeExport is always dirtied regardless of which step changed.
//
// [COUPLING] Relies on the PrintStep enum ordering defined in PrintBase.hpp.
// psGCodeExport must remain the final step; if new steps are added after it
// the cascade logic will silently break.
...
// [INTENT] Returns true only when ALL PrintObjects have completed the given
// PrintObjectStep. Used by the UI and downstream logic to gate progress on
// the slowest object in the scene.
//
// [CONCURRENCY] Takes state_mutex to ensure atomic read of per-object step
// states. Must not be called while already holding state_mutex.
...
// [INTENT] Collects the full set of 0-based extruder indices required to print
// all objects on this plate. Three sources are unioned:
//   1. Per-region extruders (colour painting, layer-range overrides via PrintRegion)
//   2. Per-volume extruders (ModelVolume::get_extruders — multi-material painting)
//   3. Per-layer-range extruder overrides on ModelObject
//
// [COUPLING] Delegates to PrintRegion::collect_object_printing_extruders() for
// painted-region extruders, meaning changes to region config must remain
// consistent with this call. The result feeds extruders() and subsequently
// the wipe tower tool ordering.
...
// [INTENT] Collects 0-based extruder indices needed for support material.
// If support_filament == 0 (auto) or support_interface_filament == 0 (auto),
// the support could be printed with any object extruder, so ALL object
// extruders are added conservatively. Explicit filament assignments are
// clamped to the valid range to guard against stale configs referencing
// extruders that no longer exist.
// returns 0-based indices of used extruders
...
// [INTENT] Returns the union of all extruder indices needed for the entire
// print: objects + support + custom G-code tool-change events + wipe tower
// filament. This is the canonical "what extruders does this plate use" query;
// the result drives wipe tower tool ordering and filament compatibility checks.
//
// [COUPLING] conside_custom_gcode=true is needed when calling before G-code
// generation; false is used for quick UI checks. The wipe_tower_filament guard
// (config value is 1-based, stored 0-based here) must stay in sync with
// _make_wipe_tower()'s filament index assumptions.
// returns 0-based indices of used extruders
...
// [INTENT] BBS: Returns objects sorted by first-layer bounding box area
// (largest first) for a Z-slice window [start, end]. Also populates per-object
// layer and instance-shift arrays needed by the conflict checker and preview
// renderer.
//
// [COUPLING] The returned index vector parallels layers_of_objects and
// boundingBox_for_objects, which are populated as side-effects. Callers must
// keep these three containers in sync; the sort is applied only to the index
// vector, not to the containers themselves.
// BBS
...
// [INTENT] Validates that all print instances can be printed sequentially
// without the tool head colliding with already-printed objects.
// Two phases:
//   1. Horizontal clearance: convex hull of each instance (expanded by
//      extruder_clearance_radius + object_skirt_offset) must not overlap any
//      other instance or an excluded bed area.
//   2. Vertical clearance: instances printed before a given instance must not
//      exceed the extruder_clearance_height_to_rod threshold if they share Y
//      overlap with that instance, and must not exceed extruder_clearance_height
//      to_lid otherwise.
//
// Returns a StringObjectException (empty string = valid) and optionally
// populates polygons/height_polygons for visualisation in the UI.
//
// [HAZARD H1100] Lines 707–809 contain a large `#if 0` block (~100 lines)
// that implements an automatic sort algorithm for sequential print ordering
// (topological sort based on left-right pairs and height penalties). The block
// is entirely dead code — the active path simply sorts by object_index (UI
// list order). The dead code contains Chinese developer comments explaining
// the intended heuristic, suggesting this was an experimental feature that was
// abandoned mid-development. A refactoring tool must not translate this block
// as live logic; it should be flagged for deletion or moved to a design doc.
//
// [STATE] The function mutates ModelInstance::arrange_order for each instance
// (line 851: const_cast to set arrange_order = k+1). This is a side-effect on
// the model from inside a validation function — unusual coupling that means
// calling this function changes model state even when only querying.
//
// [COUPLING] Calls print.object_skirt_offset() and print.is_all_objects_are_short()
// which themselves query the full Print state, so this function cannot be tested
// in isolation without a fully-configured Print object.
...
// [INTENT] BBS: Static helper for layered (non-sequential) print mode.
// Validates that all object convex hulls avoid excluded bed areas and the
// wrapping-detection area. Also checks wipe tower clearance, using the
// psWipeTower step completion as a flag to decide whether to use an
// approximate rectangular hull or the actual computed mesh polygon.
//
// [COUPLING] Reads Print::wipe_tower_data() (side-effecting const method —
// see H1101 in wipe_tower_data annotation) to obtain tower depth. Calling
// this before psWipeTower completes gives a correct approximate hull but
// with depth=0 (safe: will produce no false positives, may produce false
// negatives for tower overlap).
// BBS
        ...
        for (const ModelVolume* v : inst->print_object->model_object()->volumes) {
            ...
            if (it_convex_hull == map_model_volume_to_convex_hull.end()) {
                auto volume_hull = v->get_convex_hull_2d(Geometry::assemble_transform(Vec3d::Zero(), inst->model_instance->get_rotation(),
                                                                                      inst->model_instance->get_scaling_factor(),
                                                                                      inst->model_instance->get_mirror()));
...
// [INTENT] Classifies a set of filament type strings into a
// FilamentCompatibilityType enum value describing whether they can safely
// be used together on one plate:
//   - HighLowMixed: high-temp (ABS, PC…) mixed with low-temp (PLA, PETG…) →
//     dangerous, may cause nozzle clog
//   - HighMidMixed / LowMidMixed: one side is HighLowCompatible (e.g. PETG-CF)
//   - Compatible: all filaments in the same temperature class
//
// [COUPLING] Delegates classification of individual types to
// get_filament_temp_type() which uses a static JSON-initialized map (H1102).
...
// [INTENT] Binary check: true if none of the provided integer filament-type
// enum values are a HighTemp+LowTemp combination. Used by the UI to quickly
// gate operations without computing the full FilamentCompatibilityType enum.
...
// [INTENT] Given a set of integer FilamentTempType values already in use on
// a print object, returns the single compatible type that could safely be
// added. If both HighTemp and LowTemp are present returns HighLowCompatible
// as the "least restrictive" compatible option (a defensive fallback).
...
StringObjectException Print::check_multi_filament_valid(const Print& print)
{
    ...
    if (print_config.print_sequence == PrintSequence::ByObject) { // use ByObject valid under ByObject print sequence
        ...
        for (const auto& objectID_t : print.print_object_ids()) {
            ...
            if (print_object->has_support_material()) { // extruder used by supports
                ...
                if (print_object->config().support_filament >= 1 &&
                    (unsigned int) print_object->config().support_filament < num_extruders + 1)
                ...
                if (print_object->config().support_interface_filament >= 1 &&
                    (unsigned int) print_object->config().support_interface_filament < num_extruders + 1)
        ...
        if (Compatibility_each_obj.count(FilamentCompatibilityType::HighLowMixed)) { // at least one object has HighLowMixed
            if (enable_mix_printing) {
                ...
                ret.string = L("Printing high-temp and low-temp filaments together may cause nozzle clogging or printer damage. If you "
                               "still want to print, you can enable the option in Preferences.");
        } else if (Compatibility_each_obj.count(FilamentCompatibilityType::LowMidMixed) ||
                   Compatibility_each_obj.count(FilamentCompatibilityType::HighMidMixed)) { // at least one object has other Mixed
            ret.is_warning = true;
            // ret.hypetext   = hypertext;
    ...
    if (compatibility == FilamentCompatibilityType::HighLowMixed) {
        if (enable_mix_printing) {
            ...
            ret.string = L("Printing high-temp and low-temp filaments together may cause nozzle clogging or printer damage. If you still "
                           "want to print, you can enable the option in Preferences.");
        }
...
// BBS: refine seq-print validation logic.....FIXME:StringObjectException *warning can only contain one warning, but there might be many
// warnings, need a vector<StringObjectException>
                    ...
                    if (object->config().support_style == smsTreeOrganic ||
                        ...
                        if (warning) {
                            ...
                            if (object->config().tree_support_wall_count > 1 &&
                                ...
                                warning->string = L(
                                    "For Organic supports, two walls are supported only with the Hollow/Default base pattern.");
                        ...
                        if (object->config().tree_support_tip_diameter < extrusion_width - EPSILON)
                            return {L("Organic support tree tip diameter must not be smaller than support material extrusion width."),
                                    object, "tree_support_tip_diameter"};
                        if (object->config().tree_support_branch_diameter_organic < 2. * extrusion_width - EPSILON)
                            return {L("Organic support branch diameter must not be smaller than 2x support material extrusion width."),
    ...
    if (warning) {
        try {
            ...
            if (warning_key.empty() && m_default_object_config.default_junction_deviation.value > max_junction_deviation &&
                support_max_junction_deviation) {
...
void Print::process(long long* time_cost_with_cache, bool use_cache)
{
    ...
    if (this->set_started(psSkirtBrim)) {
        ...
        if (this->config().print_sequence == PrintSequence::ByObject) {
            ...
            for (print_object_instance_sequential_active = print_object_instances_ordering.begin();
                 print_object_instance_sequential_active != print_object_instances_ordering.end();
            ...
            if (map_mode < FilamentMapMode::fmmManual) {
                filament_maps = ToolOrdering::get_recommended_filament_maps(all_filaments, this, map_mode, physical_unprintables,
...
int Print::get_hrc_by_nozzle_type(const NozzleType& type)
{
    ...
    if (nozzle_type_to_hrc.empty()) {
        ...
        try {
            ...
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": parse " << file_path.string()
                                     << " got a nlohmann::detail::parse_error, reason = " << err.what();
            nozzle_type_to_hrc = {{"hardened_steel", 55}, {"stainless_steel", 20}, {"tungsten_carbide", 85}, {"brass", 2}, {"undefine", 0}};
        }
    }
...
std::vector<std::string> Print::get_incompatible_filaments_by_nozzle(const float                           nozzle_diameter,
                                                                     const std::optional<NozzleVolumeType> nozzle_volume_type)
{
...
void Print::update_filament_maps_to_config(std::vector<int> f_maps)
{
    if (m_config.filament_map.values != f_maps) {
        ...
        m_full_print_config.update_values_to_printer_extruders_for_multiple_filaments(m_full_print_config, filament_options_with_variant,
        ...
        for (auto& opt_key : extruder_retract_keys) {
            ...
            if (opt_new_filament)
                compute_filament_override_value(opt_key, opt_old_machine, opt_new_machine, opt_new_filament, m_full_print_config,
                                                print_diff, filament_overrides, f_maps);
        }
...
void Print::apply_config_for_render(const DynamicConfig& config) { m_config.apply(config); }

std::vector<int> Print::get_filament_maps() const { return m_config.filament_map.values; }

FilamentMapMode Print::get_filament_map_mode() const { return m_config.filament_map_mode; }
...
std::vector<double> Print::get_extruder_printable_height() const { return m_config.extruder_printable_height.values; }
...
const WipeTowerData& Print::wipe_tower_data(size_t filaments_cnt) const
{
    ...
    double filament_change_volume = 0.;
    {
        ...
        if (filament_change_lengths_opt)
            filament_change_lengths = filament_change_lengths_opt->values;
        double              length   = filament_change_lengths.empty() ? 0 :
                                                                         *std::max_element(filament_change_lengths.begin(), filament_change_lengths.end());
        ...
        if (filament_diameter_opt)
            diameters = filament_diameter_opt->values;
    ...
    if (!is_step_done(psWipeTower) && filaments_cnt != 0) {
        ...
        if (m_config.nozzle_diameter.values.size() == 2)
            volume += filament_change_volume * (int) (filaments_cnt / 2);
...
void Print::_make_wipe_tower()
{
    ...
    if (!bUseWipeTower2) {
        ...
        WipeTower wipe_tower(m_config, m_plate_index, m_origin, m_wipe_tower_data.tool_ordering.first_extruder(),
                             ...
                             m_wipe_tower_data.tool_ordering.all_extruders());
        ...
        // to pass to wipe_tower (so that it can use it for planning the layout of the tower)
        {
            ...
            for (size_t nozzle_id = 0; nozzle_id < nozzle_nums; ++nozzle_id) {
                std::vector<float> flush_matrix(
                    cast<float>(get_flush_volumes_matrix(m_config.flush_volumes_matrix.values, nozzle_id, nozzle_nums)));
            ...
            for (auto& layer_tools : m_wipe_tower_data.tool_ordering.layer_tools()) { // for all layers
                ...
                wipe_tower.plan_toolchange((float) layer_tools.print_z, (float) layer_tools.wipe_tower_layer_height, current_filament_id,
                                           current_filament_id);
                ...
                for (const auto filament_id : layer_tools.extruders) {
                    ...
                    if (pre_filament_id != (unsigned int) (-1) && pre_filament_id != filament_id) {
                        ...
                        volume_to_purge = pre_filament_id == -1 ?
                                              0 :
                                              layer_tools.wiping_extrusions().mark_wiping_extrusions(*this, current_filament_id,
                                                                                                     filament_id, volume_to_purge);
                    }

                    // During the filament change, the extruder will extrude an extra length of grab_length for the corresponding detection,
                    // so the purge can reduce this length.
                    ...
                    wipe_tower.plan_toolchange((float) layer_tools.print_z, (float) layer_tools.wipe_tower_layer_height,
                                               current_filament_id, filament_id, m_config.prime_volume, volume_to_purge);
        ...
        m_wipe_tower_data.construct_mesh(wipe_tower.width(), wipe_tower.get_depth(), wipe_tower.get_height(), wipe_tower.get_brim_width(),
                                         config().wipe_tower_wall_type.value == WipeTowerWallType::wtwRib, wipe_tower.get_rib_width(),
                                         wipe_tower.get_rib_length(), config().wipe_tower_fillet_wall.value);
        const Vec3d origin           = this->get_plate_origin();
...
static void convert_layer_region_from_json(const json& j, LayerRegion& layer_region)
{
    ...
    for (int thinfills_entities_index = 0; thinfills_entities_index < thinfills_entities_count; thinfills_entities_index++) {
        ...
        if (!ret) {
            ...
            char error_buf[1024];
            ::sprintf(error_buf, "Error while parsing thin_fills at layer %zu, print_z %f", layer_region.layer()->id(),
        ...
        if (!ret) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(": error parsing perimeters found at layer %1%, print_z %2%") %
                                            layer_region.layer()->id() % layer_region.layer()->print_z;
            char error_buf[1024];
            ::sprintf(error_buf, "Error while parsing perimeters at layer %zu, print_z %f", layer_region.layer()->id(),
                      layer_region.layer()->print_z);
            throw Slic3r::FileIOError(error_buf);
        }
    }
        ...
        if (!ret) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(": error parsing fills found at layer %1%, print_z %2%") %
                                            layer_region.layer()->id() % layer_region.layer()->print_z;
            char error_buf[1024];
            ::sprintf(error_buf, "Error while parsing fills at layer %zu, print_z %f", layer_region.layer()->id(),
                      layer_region.layer()->print_z);
            throw Slic3r::FileIOError(error_buf);
        }
    }
...
void WipeTowerData::construct_mesh(
    float width, float depth, float height, float brim_width, bool is_rib_wipe_tower, float rib_width, float rib_length, bool fillet_wall)
{
