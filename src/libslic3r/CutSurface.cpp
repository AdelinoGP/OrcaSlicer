// [INTENT] CutSurface.cpp — Projects 2D glyph ExPolygons onto a 3D mesh surface using CGAL
// corefine + flood-fill, producing SurfaceCut (triangle patches with outline loops) for
// embossed/debossed text operations. Core pipeline:
//   1. Filter triangles outside AOI bounding box (TBB parallel)
//   2. Convert model + shape to CGAL Surface_mesh
//   3. Corefine shape into model surface (Visitor tracks intersection sources)
//   4. Flood-fill to classify inside/outside faces
//   5. Collect AOIs, diff against negative models, choose best depth, select & merge patches
//
// [STATE] All state is local to the cut_surface() call — no class members, fully functional.
// Exception: DEBUG_OUTPUT_DIR store helpers use file-scoped order counters (non-thread-safe).
//
// [MEMORY] CGAL Surface_mesh objects are large heap structures. Every call to cut_surface()
// allocates at minimum 2*N cgal_models + 1 cgal_shape + N cgal_neg_models. These are
// stack-lifetime locals but can be hundreds of MB for complex models.
//
// [COUPLING] Depends on: Emboss::IProjection, ExPolygons, ExPolygonsIndices, indexed_triangle_set,
// CGAL::Polygon_mesh_processing::corefine/clip, TBB, BoundingBox (Clipper coord_t units).
//
// [CONCURRENCY] TBB parallel_for used only for bounding-box vertex/triangle pre-filtering
// (set_skip_for_out_of_aoi). All CGAL corefine/clip calls are single-threaded per invocation.
// cut_surface() itself is safe to call concurrently from multiple threads (no shared state).
//
// [H1049 P3/Low] #define DEBUG_OUTPUT_DIR is commented out at line 23.
// All ~150 lines of debug store helpers inside #ifdef DEBUG_OUTPUT_DIR are permanently dead code.
// Hardcoded Windows path "C:/data/temp/cutSurface/" embedded in active string literal below.
...
// [INTENT] IntersectingElement — Tracks which contour edge/face of the extruded shape mesh
// intersected the model surface during CGAL corefine. Used in Visitor to annotate new vertices
// on the model mesh so flood_fill_inner can correctly classify inside/outside faces.
//
// [STATE] Stored in EdgeShapeMap and FaceShapeMap property maps on the CGAL shape mesh.
// Pointer to IntersectingElement is stored in VertexShapeMap on the model mesh.
//
// [H1041 P2/Medium] attr bitfield: packs Type (bits 0-2), is_first (bit 3), is_last (bit 4)
// into unsigned char using arithmetic add/subtract instead of bitwise OR/AND operations.
// set_type() does: attr += (int)t - (int)get_type(). If get_type() returns non-undefined
// and t is also non-undefined, this arithmetic is fragile — if the addition overflows the
// 3-bit type field into the is_first/is_last bits (i.e. attr bits 3-4 are set), the
// result is silently wrong. Safe only when called in the specific construction order
// used in to_cgal(ExPolygons). A refactor must replace with proper bitfield operations.
/// <summary>
/// IntersectingElement
///
...
struct IntersectingElement
{
    ...
    {
        attr = static_cast<unsigned char>(attr + (int) t - (int) get_type());
        return *this;
    }
...
VDistances calc_distances(
    const SurfacePatches& patches, const CutMeshes& models, const CutMesh& shapes_mesh, size_t count_shapes_points, float projection_ratio);
...
SurfaceCut merge_patches(/*const*/ SurfacePatches& patches, const std::vector<bool>& mask);
...
// void store(const ProjectionDistances &pds, const VCutAOIs &aois, const CutMeshes &meshes, const std::string &file, float width = 0.2f/*
// [in mm] */);
                               ...
                               float                                    projection_ratio)
{
    ...
    for (priv::CutMesh& cgal_model : cgal_models) {
        priv::CutAOIs cutAOIs = priv::cut_from_model(cgal_model, shapes, cgal_shape, projection_ratio, s2i);
#ifdef DEBUG_OUTPUT_DIR
    ...
    std::vector<bool>         use_patch = priv::select_patches(best_projection, patches, shapes, shapes_bb, s2i, model_cuts, cgal_models,
                                                               projection);
...
indexed_triangle_set Slic3r::cut2model(const SurfaceCut& cut, const Emboss::IProject3d& projection)
{
    ...
    for (const auto& i : cut.indices) {
        ...
        result.indices.emplace_back(i.x() + back_offset, i.z() + back_offset, i.y() + back_offset);
    }
                                   ...
                                   const BoundingBox&          shapes_bb)
{
    ...
    int                                    index = 0;
    for (Point v : {shapes_bb.min, Point{shapes_bb.min.x(), shapes_bb.max.y()}, shapes_bb.max, Point{shapes_bb.max.x(), shapes_bb.min.y()}})
...
indexed_triangle_set Slic3r::its_mask(const indexed_triangle_set& its, const std::vector<bool>& mask)
{
    ...
    for (const stl_triangle_vertex_indices& f : its.indices)
        if (mask[&f - &its.indices.front()])
            result.indices.push_back(stl_triangle_vertex_indices(cvt_vetices[f[0]], cvt_vetices[f[1]], cvt_vetices[f[2]]));

    return result;
}

indexed_triangle_set Slic3r::its_cut_AoI(const indexed_triangle_set& its, const BoundingBox& bb, const Emboss::IProjection& projection)
{
    std::vector<bool> skip_indicies(its.indices.size(), false);
...
priv::CutMesh priv::to_cgal(const indexed_triangle_set& its, const std::vector<bool>& skip_indicies, bool flip)
{
    ...
    for (const stl_vertex& v : vertices) {
        if (!use_vetices[&v - &vertices.front()])
            continue;
    ...
    if (!flip) {
        for (const stl_triangle_vertex_indices& f : indices) {
            if (skip_indicies[&f - &indices.front()])
                continue;
            result.add_face(to_filtrated_vertices_index[f[0]], to_filtrated_vertices_index[f[1]], to_filtrated_vertices_index[f[2]]);
        }
    } else {
        for (const stl_triangle_vertex_indices& f : indices) {
            if (skip_indicies[&f - &indices.front()])
                continue;
            result.add_face(to_filtrated_vertices_index[f[2]], to_filtrated_vertices_index[f[1]], to_filtrated_vertices_index[f[0]]);
        }
    }
...
// [INTENT] to_cgal(ExPolygons) — Converts 2D glyph contours to an extruded CGAL surface mesh
// by projecting each contour point to a front/back pair, creating quads (two triangles each),
// and attaching EdgeShapeMap/FaceShapeMap property maps for corefine intersection tracking.
//
// [H1048 P2/Medium] assert(!exist_duplicit_vertex(result)) at end of function: duplicate
// vertex detection fires ONLY in debug builds. In Release, duplicate vertices (from degenerate
// projections or coincident glyph points) silently pass into the CGAL corefine call.
// CGAL corefine on meshes with duplicate vertices produces undefined/incorrect results
// without raising an exception. A refactor must add a Release-mode guard or pre-deduplicate.
priv::CutMesh priv::to_cgal(const ExPolygons& shapes, const Project& projection)
{
    if (shapes.empty())
        return {};
    ...
    auto            insert_contour = [&projection, &indices, &result, &edge_shape_map, &face_shape_map](const Polygon& polygon) {
        indices.clear();
    ...
    result.reserve(result.number_of_vertices() + 2 * count_point, result.number_of_edges() + 4 * count_point,
...
priv::ModelCutId priv::ModelCut2index::calc_id(uint32_t index) const
{
    ...
    for (size_t model_index = 1; model_index < m_offsets.size(); ++model_index) {
        if (m_offsets[model_index] > index)
            break;
...
struct Visitor : public CGAL::Polygon_mesh_processing::Corefinement::Default_visitor<CutMesh>
{
    Visitor(const CutMesh& object,
        ...
        : object(object)
        , shape(shape)
        , edge_shape_map(edge_shape_map)
        , face_shape_map(face_shape_map)
        , vert_shape_map(vert_shape_map)
        , is_valid(is_valid)
    {}
...
bool is_face_inside(
    HI hi, const CutMesh& mesh, const CutMesh& shape_mesh, const VertexShapeMap& vertex_shape_map, const ExPolygonsIndices& shape2index);
...
/// <param name="mesh">mesh to process</param>
void collect_surface_data(
    std::queue<FI>& process, std::vector<FI>& faces, std::vector<HI>& outlines, FaceTypeMap& face_type_map, const CutMesh& mesh);
...
// [INTENT] Visitor::intersection_point_detected — CGAL corefine callback. Tracks which
// IntersectingElement (edge or face) of the shape mesh each new intersection point comes from.
// Used to annotate new vertices on the model mesh via vert_shape_map, enabling is_face_inside()
// to later determine inside/outside classification by source contour ordering.
//
// [H1040 P2/Medium] When the shape's top or bottom edge (not a side contour edge) intersects
// the model surface, shape_point_index == max() and *is_valid is set false, causing the
// entire cut_from_model() to return {}. This is correct safety behaviour, but the check
// happens per-intersection: all work done up to that point in the corefine call is wasted.
// Additionally, is_valid is only checked AFTER the full corefine call completes (line ~1447),
// not mid-stream — so corefine runs to completion even when invalid. A refactor cannot
// abort corefine early; early exit must be handled by restructuring projection parameters
// before calling cut_surface().
void priv::Visitor::intersection_point_detected(
    std::size_t i_id, int sdim, HI h_f, HI h_e, const CutMesh& tm_f, const CutMesh& tm_e, bool is_target_coplanar, bool is_source_coplanar)
{
    ...
    HI hi, const CutMesh& mesh, const CutMesh& shape_mesh, const VertexShapeMap& vertex_shape_map, const ExPolygonsIndices& shape2index)
{
    ...
    if (i_from == i_to && type_from == type_to) {
        ...
        assert(type_from == IntersectingElement::Type::face_1 || type_from == IntersectingElement::Type::face_2);
        ...
        CGAL::Sign abcp = type_from == IntersectingElement::Type::face_1 ?
                              CGAL::orientation(shape_mesh.point(VI(i)), shape_mesh.point(VI(i + 1)), shape_mesh.point(VI(j)), p) :
                              // type_from == IntersectingElement::Type::face_2
                              CGAL::orientation(shape_mesh.point(VI(j)), shape_mesh.point(VI(i + 1)), shape_mesh.point(VI(j + 1)), p);
...
priv::CutAOIs priv::cut_from_model(
    CutMesh& cgal_model, const ExPolygons& shapes, CutMesh& cgal_shape, float projection_ratio, const ExPolygonsIndices& s2i)
{
    ...
    const auto& p   = CGAL::parameters::visitor(visitor).edge_is_constrained_map(ecm).throw_on_self_intersection(false);
...
void priv::flood_fill_inner(const CutMesh& mesh, FaceTypeMap& face_type_map)
{
    ...
    auto has_inside_neighbor = [&mesh, &face_type_map](FI fi) {
        ...
        do {
            ...
            if (!fi_opposite.is_valid())
                continue;
            if (face_type_map[fi_opposite] == FaceType::inside)
                return true;
        } while (exist_next());
        return false;
    };
    ...
    for (FI fi : mesh.faces()) {
        ...
        while (!process.empty()) {
            ...
            if (process_type == FaceType::inside)
                continue;
...
void priv::collect_surface_data(
    std::queue<FI>& process, std::vector<FI>& faces, std::vector<HI>& outlines, FaceTypeMap& face_type_map, const CutMesh& mesh)
{
    assert(!process.empty());
    assert(faces.empty());
    ...
    while (!process.empty()) {
        ...
        if (fi_type == FaceType::inside_processed)
            continue;
...
void priv::create_reduce_map(ReductionMap& reduction_map, const CutMesh& mesh)
{
    ...
    auto add_reduction = [&] //&reduction_map, &mesh, &face_type_map, &is_reducible_vertex
        (HI hi) {
            ...
            for (FI fi : mesh.faces_around_target(hi)) {
                ...
                if (((n1.x() > 0) != (n2.x() > 0)) || ((n1.y() > 0) != (n2.y() > 0)) || ((n1.z() > 0) != (n2.z() > 0)))
    ...
    for (EI ei : mesh.edges()) {
        ...
        if (is_reducible_vertex(vi))
            add_reduction(hi);

        HI hi_op = mesh.opposite(hi);
        VI vi_op = mesh.target(hi_op);
        if (is_reducible_vertex(vi_op))
            add_reduction(hi_op);
    }
#ifdef DEBUG_OUTPUT_DIR
...
priv::CutAOIs priv::create_cut_area_of_interests(const CutMesh& mesh, const ExPolygons& shapes, FaceTypeMap& face_type_map)
{
    ...
    for (FI fi : mesh.faces()) {
        if (face_type_map[fi] != FaceType::inside)
            continue;
...
float priv::calc_distance(const P3& p, uint32_t pi, const CutMesh& shapes_mesh, float projection_ratio)
{
...
priv::VDistances priv::calc_distances(
    const SurfacePatches& patches, const CutMeshes& models, const CutMesh& shapes_mesh, size_t count_shapes_points, float projection_ratio)
{
    priv::VDistances result(count_shapes_points);
    for (const SurfacePatch& patch : patches) {
        // map is created during intersection by corefine visitor
        const VertexShapeMap& vert_shape_map = models[patch.model_id].property_map<VI, const IntersectingElement*>(vert_shape_map_name).first;
...
uint32_t   get_closest_point_index(
      const SearchData& sd, size_t line_idx, const Vec2d& hit_point, const ExPolygons& shapes, const ExPolygonsIndices& s2i);
...
std::pair<uint32_t, uint32_t> find_closest_point_pair(const ExPolygons&        shapes,
...
void fill_polygon_distances(const ProjectionDistance& pd,
                            uint32_t                  index,
...
void fill_shape_distances(uint32_t                  start_index,
...
ClosePoint find_close_point(const Point&             p,
                            ProjectionDistances&     result,
...
priv::SearchData priv::create_search_data(const ExPolygons& shapes, const std::vector<bool>& mask)
{
    ...
    size_t index     = 0;
    auto   add_lines = [&sd, &index, &mask](const Polygon& poly) {
...
uint32_t priv::get_closest_point_index(
    const SearchData& sd, size_t line_idx, const Vec2d& hit_point, const ExPolygons& shapes, const ExPolygonsIndices& s2i)
{
    ...
    bool   use_index   = abs(line.a[i] - hit_point[i]) > abs(line.b[i] - hit_point[i]);
    size_t point_index = sd.cvt[line_idx];

    // Lambda used only for check result
    [[maybe_unused]] auto is_same = [&s2i, &shapes](const Vec2d& p, size_t i) -> bool {
        auto             id    = s2i.cvt(i);
        const ExPolygon& shape = shapes[id.expolygons_index];
        const Polygon&   poly  = (id.polygon_index == 0) ? shape.contour : shape.holes[id.polygon_index - 1];
    ...
    const ExPolygon& shape                = shapes[id.expolygons_index];
    size_t           count_polygon_points = (id.polygon_index == 0) ? shape.contour.size() : shape.holes[id.polygon_index - 1].size();
                                        ...
                                        const std::vector<bool>& mask)
{
    ...
    if (sd.tree.nodes().size() == 0) {
        ...
        for (uint32_t i = 0; i < mask.size(); i++)
            if (mask[i]) {
                ...
                const Point&    s_p             = ei.is_contour() ? shapes[ei.expolygons_index].contour[ei.point_index] :
                                                                    shapes[ei.expolygons_index].holes[ei.hole_index()][ei.point_index];
                double          square_distance = (p - s_p).cast<double>().squaredNorm();
                if (closest_id >= mask.size() || closest_square_distance > square_distance) {
                    closest_id              = i;
    ...
    Vec2d                   hit_point;
    Vec2d                   p_d         = p.cast<double>();
    [[maybe_unused]] double distance_sq = AABBTreeLines::squared_distance_to_indexed_lines(sd.lines, sd.tree, p_d, line_idx, hit_point);
    ...
    for (size_t shape_index = 0; shape_index < shapes.size(); shape_index++) {
        ...
        auto search_in_polygon = [&index, &cp, &sd, &mask](const Polygon& polygon) {
            for (size_t i = 0; i < polygon.size(); ++i, ++index) {
                ...
                size_t line_idx = std::numeric_limits<size_t>::max();
                Vec2d  hit_point;
                double distance_sq = AABBTreeLines::squared_distance_to_indexed_lines(sd.lines, sd.tree, p_d, line_idx, hit_point,
                                                                                      cp.distance_sq);
...
const priv::ProjectionDistance* priv::get_closest_projection(const ProjectionDistances& distance, float wanted_distance)
{
    // minimal distance
...
void priv::fill_polygon_distances(const ProjectionDistance& pd,
                                  ...
                                  const VDistances&         distances)
{
    const Points& points = (id.polygon_index == 0) ? shape.contour.points : shape.holes[id.polygon_index - 1].points;
    ...
    auto exist_next = [&distances, &act_index, &act_pd, &result](uint32_t nxt_index) {
    ...
    uint32_t finish_index = (index == first_index) ? (last_index - 1) : (index - 1);
    ...
    do {
        uint32_t nxt_index = (act_index == first_index) ? (last_index - 1) : (act_index - 1);
                                ...
                                const VDistances&         distances)
{
    ...
    do {
        ...
        auto find_close_finished_point = [&first_shape_index, &shape, &result](const Point& p) -> ClosePoint {
            uint32_t   index = first_shape_index;
            ClosePoint cp;
            auto       check_finished_points = [&cp, &result, &index, &p](const Points& pts) {
        ...
        uint32_t index                   = first_shape_index;
        auto     check_unfinished_points = [&index, &result, &distances, &find_close_finished_point, &dist_sq, &start_index,
                                        &finished_index](const Points& pts) {
            for (const Point& p : pts) {
                // try find unfinished
                if (result[index].aoi_index != std::numeric_limits<uint32_t>::max() || distances[index].empty()) {
                    ++index;
                    continue;
                }
...
priv::ClosePoint priv::find_close_point(
    const Point& p, ProjectionDistances& result, std::vector<bool>& finished_shapes, const ExPolygonsIndices& s2i, const ExPolygons& shapes)
{
    // result
    ClosePoint cp;
    // for all finished points
    for (uint32_t shape_index = 0; shape_index < shapes.size(); ++shape_index) {
        if (!finished_shapes[shape_index])
            continue;
        const ExPolygon& shape                      = shapes[shape_index];
        uint32_t         index                      = s2i.cvt({shape_index, 0, 0});
        auto             find_close_point_in_points = [&p, &cp, &index, &result](const Points& pts) {
            for (const Point& p_ : pts) {
    ...
    const VDistances& distances, const ExPolygons& shapes, const Point& start, const ExPolygonsIndices& s2i, const SurfacePatches& patches)
{
    ...
    for (const auto& d : distances)
        if (d.empty())
            mask_distances[&d - &distances.front()] = false;
    ...
    do {
        ...
        auto [finished, unfinished] = find_closest_point_pair(shapes, finished_shapes, s2i, mask_distances);

        // detection of end (best doesn't have value)
        if (finished == std::numeric_limits<uint32_t>::max())
            break;
...
struct IntersectionSources
{
                                     ...
                                     bool           is_source_coplanar)
    {
        ...
        if (&tm_e == model) {
            ...
            // not used any more
            if (is_source_coplanar || is_target_coplanar)
                source = {};
        } else {
            source = {h_f, sdim};
...
void priv::create_face_types(FaceTypeMap& map, const CutMesh& tm1, const CutMesh& tm2, const EdgeBoolMap& ecm, const VertexSourceMap& sources)
{
    ...
    for (FI fi : tm1.faces())
        map[fi] = FaceType::not_constrained;
...
bool priv::clip_cut(SurfacePatch& cut, CutMesh clipper)
{
    ...
    const auto& np_tm = CGAL::parameters::visitor(visitor).throw_on_self_intersection(false);
...
BoundingBoxf3 priv::bounding_box(const SurfacePatch& ecut) { return bounding_box(ecut.mesh); }
                                              ...
                                              const ReductionMap*    rmap)
{
    ...
    if (rmap == nullptr) {
        ...
        for (FI fi : fis) {
            ...
            if (exist_reduction && (t[0] == t[1] || t[1] == t[2] || t[2] == t[0]))
                continue;
    ...
    assert((rmap == nullptr && count_faces == cm.faces().size()) || (rmap != nullptr && count_faces >= cm.faces().size()));
...
std::vector<BoundingBoxf3> priv::create_bbs(const VCutAOIs& cuts, const CutMeshes& cut_models)
{
    size_t count = 0;
    for (const CutAOIs& cut : cuts)
        count += cut.size();
...
bool priv::has_bb_intersection(const BoundingBoxf3& bb, size_t model_index, const BBS& bbs, const ModelCut2index& m2i)
{
    ...
    for (size_t bb_index = start; bb_index < end; bb_index++)
        if (bb.intersects(bbs[bb_index]))
            return true;
    return false;
}

bool priv::is_patch_inside_of_model(const SurfacePatch& patch, const Tree& tree, const Project3d& projection)
{
    ...
    assert(((tree.number_of_intersected_primitives(Ray(a, P3(2 * a.x() - b.x(), 2 * a.y() - b.y(), 2 * a.z() - b.z()))) % 2) == 1) == is_in);
    return is_in;
}
...
priv::SurfacePatch priv::separate_patch(const std::vector<FI>& fis, SurfacePatch& patch, const CvtVI2VI& cvt_from)
{
    ...
    for (size_t model_index = 0; model_index < models.size(); ++model_index) {
        ...
        for (size_t cut_index = 0; cut_index < model_cuts.size(); ++cut_index, ++index) {
            ...
            for (size_t model_index2 = 0; model_index2 < models.size(); ++model_index2) {
                ...
                if (model_index == model_index2)
                    continue;
                for (SurfacePatchEx& patch_ex : aoi_patches) {
                    SurfacePatch& patch = patch_ex.patch;
                    if (has_bb_intersection(patch.bb, model_index2, bbs, m2i) && clip_cut(patch, models[model_index2])) {
...
bool priv::is_over_whole_expoly(const SurfacePatch& patch, const ExPolygons& shapes, const VCutAOIs& cutAOIs, const CutMeshes& meshes)
{
    if (!patch.is_whole_aoi)
        return false;
    return is_over_whole_expoly(cutAOIs[patch.model_id][patch.aoi_id], shapes[patch.shape_id], meshes[patch.model_id]);
}

bool priv::is_over_whole_expoly(const CutAOI& cutAOI, const ExPolygon& shape, const CutMesh& mesh)
{
    ...
    for (HI hi : cutAOI.second) {
        ...
        if (i_s == std::numeric_limits<uint32_t>::max() || i_t == std::numeric_limits<uint32_t>::max())
            return false;
        ...
        if (!is_last_polygon_segment && (ie_s->is_last() || (i_s + 1) != i_t))
            return false;

        IntersectingElement::Type t_s = ie_s->get_type();
        IntersectingElement::Type t_t = ie_t->get_type();
        if (t_s == IntersectingElement::Type::undefined || t_t == IntersectingElement::Type::undefined)
            return false;
                                       ...
                                       const Project&             projection)
{
    ...
    for (const ProjectionDistance& d : best_distances) {
        ...
        if (d.patch_index == std::numeric_limits<uint32_t>::max())
            continue;
        if (in_distances[d.patch_index])
            continue;
    ...
    for (size_t shape_index = 0; shape_index < shapes.size(); shape_index++) {
        ...
        for (uint32_t patch_index : shapes_patches[shape_index]) {
            ...
            if (it != used_shape_patches.end() && *it == patch_index)
                continue;
        ...
        std::sort(patch_shapes.begin(), patch_shapes.end(),
                  [](const PatchShape& a, const PatchShape& b) { return a.depth_range_center_distance < b.depth_range_center_distance; });
    ...
    for (HI hi : outlines) {
        ...
        if (loop_move != nullptr) {
            ...
            if (loop_tail != nullptr) {
                ...
                loop_connect->insert(loop_connect->end(), make_move_iterator(loop_tail->begin()), make_move_iterator(loop_tail->end()));
...
Polygons priv::unproject_loops(const SurfacePatch& patch, const Project& projection, Vec2d& depth_range)
{
    ...
    for (const Loop& l : patch.loops) {
        ...
        for (VI vi : l) {
            ...
            if (depth_range[0] > depth)
                depth_range[0] = depth; // min
            if (depth_range[1] < depth)
                depth_range[1] = depth; // max
...
SurfaceCut priv::patch2cut(SurfacePatch& patch)
{
    ...
    CutMesh::Property_map<VI, SurfaceCut::Index> convert_map      = mesh.add_property_map<VI, SurfaceCut::Index>(convert_map_name).first;
    ...
    for (const Loop& loop : patch.loops) {
        ...
        for (VI vi : loop)
            contour.push_back(convert_map[vi]);
    }
...
void priv::append(SurfaceCut& sc, SurfaceCut&& sc_add)
{
    ...
    if (!sc_add.contours.empty()) {
        SurfaceCut::Index offset  = static_cast<SurfaceCut::Index>(sc.vertices.size());
        size_t            require = sc.contours.size() + sc_add.contours.size();
        if (sc.contours.capacity() < require)
            sc.contours.reserve(require);
        for (std::vector<SurfaceCut::Index>& cut : sc_add.contours)
            for (SurfaceCut::Index& i : cut)
                i += offset;
...
void priv::store(const Vec3f& vertex, const Vec3f& normal, const std::string& file, float size)
{
    int    flatten = 20;
    size_t min_i   = 0;
...
namespace priv {
indexed_triangle_set create_indexed_triangle_set(const std::vector<FI>& faces, const CutMesh& mesh);
} // namespace priv

indexed_triangle_set priv::create_indexed_triangle_set(const std::vector<FI>& faces, const CutMesh& mesh)
{
...
{
    auto create_outline_its = [&mesh](const std::vector<HI>& outlines) -> indexed_triangle_set {
...
void priv::store(const ExPolygons& shapes, const std::vector<bool>& mask, const Connections& connections, const std::string& file_svg)
{
    ...
    auto              get_point = [&shapes, &s2i](size_t i) -> Point {
        ...
        const Polygon&   p  = (id.polygon_index == 0) ? s.contour : s.holes[id.polygon_index - 1];
...
unsigned int get_triangle_tip(unsigned int vi1, unsigned int vi2, const indexed_triangle_set& its);
} // namespace priv

unsigned int priv::get_triangle_tip(unsigned int vi1, unsigned int vi2, const indexed_triangle_set& its)
{
...
indexed_triangle_set priv::create_contour_its(const indexed_triangle_set& its, const std::vector<unsigned int>& contour)
{
    static const float   line_width = 0.1f;
    indexed_triangle_set result;
...
void priv::store(const std::vector<indexed_triangle_set>& models, const std::string& obj_filename)
{
...
void priv::store(const std::vector<priv::CutMesh>& models, const std::string& dir)
{
    prepare_dir(dir);
    if (models.empty())
        return;
...
bool Slic3r::corefine_test(const std::string& model_path, const std::string& shape_path)
{
    ...
    if (!CGAL::IO::read_OFF(model_path, model))
        return false;
    if (!CGAL::IO::read_OFF(shape_path, shape))
        return false;
