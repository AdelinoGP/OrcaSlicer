// [INTENT] FilamentGroupUtils.cpp — utility layer for dual-extruder filament assignment.
//   Provides:
//     - Color: RGBA hex-string parsing and comparison helpers.
//     - MachineFilamentInfo: per-AMS-slot metadata (color, type, extruder_id, is_extended, is_support).
//     - build_machine_filaments(): resolves which machine AMS slots are usable given ams_counts.
//     - collect_unprintable_limits(): merges physical and geometric per-extruder unprintable sets.
//     - extract_unprintable_limit_indices(): remaps unprintable filament IDs to used-filament indices.
//     - check_printable(): validates a proposed filament-group assignment against unprintable constraints.
// [STATE] All functions are stateless; data flows through parameters.
//   All sizes default to 2 (for dual extruders). Single-extruder printers use slot 0 only.
// [COUPLING] Called from FilamentGroup.cpp (K-Medoids clustering, see H459 for static globals),
//   and from the GUI filament-assignment panel. Depends on DynamicPrintConfig for filament metadata.
// [CONCURRENCY] Thread-safe: all functions are purely local. No shared state.
// [HAZARD H1146] The hardcoded "default size to 2" pattern appears in at least 4 places:
//   build_full_machine_filaments (line ~106), build_machine_filaments (line ~143),
//   calc_max_group_size (line ~74), collect_unprintable_limits (line ~180).
//   This bakes in the assumption that the machine always has exactly 2 extruder groups.
//   Any future 3+ extruder machine would require touching all four sites — there is no
//   named constant or configuration parameter for this value.
// [HAZARD H1147] build_full_machine_filaments(): `temp.is_extended = (tray_name == "Ext")` —
//   the external-tray detection is a hardcoded string comparison. Any AMS slot whose tray_name
//   differs from exactly "Ext" (case-sensitive) is treated as non-extended, even if it is
//   semantically the external spool position. A port must replicate this exact string match.
...
namespace Slic3r { namespace FilamentGroupUtils {
...
std::string Color::to_hex_str(bool include_alpha) const
{
    ...
    oss << "#" << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(r) << std::setw(2) << static_cast<int>(g)
...
static std::vector<std::vector<MachineFilamentInfo>> build_full_machine_filaments(
    const std::vector<std::vector<DynamicPrintConfig>>& filament_configs)
{
...
std::vector<std::vector<MachineFilamentInfo>> build_machine_filaments(const std::vector<std::vector<DynamicPrintConfig>>& filament_configs,
                                                                      const std::vector<std::map<int, int>>&              ams_counts,
...
bool collect_unprintable_limits(const std::vector<std::set<int>>& physical_unprintables,
                                const std::vector<std::set<int>>& geometric_unprintables,
                                std::vector<std::set<int>>&       unprintable_limits)
{
...
void extract_indices(const std::vector<unsigned int>&  used_filaments,
                     const std::vector<std::set<int>>& unprintable_elems,
...
void extract_unprintable_limit_indices(const std::vector<std::set<int>>& unprintable_elems,
...
}} // namespace Slic3r::FilamentGroupUtils