...
// [INTENT] Z-tagged aliases encode source-contour identity into Clipper2 intersections without changing XY geometry.
...
template<bool Open = false> inline ZPath64 to_zpath64(const Points& path, int64_t z)
{
    // [STATE] No global mutation; ownership remains local and returned by value.
    ZPath64 out;
    if (!path.empty()) {
...
template<bool Open = false> inline ZPath64 to_zpath64(const Clipper2Lib_Z::Path64& path, int64_t z)
{
    ZPath64 out;
    if (!path.empty()) {
        out.reserve(path.size() + (Open ? 1 : 0));
        for (const Clipper2Lib_Z::Point64& p : path)
            out.emplace_back(p.x, p.y, z);
        if (Open)
            out.emplace_back(out.front());
    }
...
template<bool Open = false> inline ZPaths64 to_zpaths64(const VecOfPoints& paths, int64_t z)
{
    ZPaths64 out;
    out.reserve(paths.size());
    for (const Points& path : paths)
        out.emplace_back(to_zpath64<Open>(path, z));
    return out;
}

template<bool Open = false> inline ZPaths64 to_zpaths64(const Clipper2Lib_Z::Paths64& paths, int64_t z)
{
    ZPaths64 out;
    out.reserve(paths.size());
    for (const Clipper2Lib_Z::Path64& path : paths)
        out.emplace_back(to_zpath64<Open>(path, z));
    return out;
}
...
// offset by base_idx.
template<bool Open = false> inline ZPaths64 expolygons_to_zpaths64(const ExPolygons& src, int64_t& base_idx)
{
    // [STATE] base_idx is an output cursor: caller-visible state advances once per ExPolygon.
    ZPaths64 out;
...
template<bool Open = false> inline ZPaths64 expolygons_to_zpaths64_with_same_z(const ExPolygons& src, int64_t z)
{
    ZPaths64 out;
    out.reserve(std::accumulate(src.begin(), src.end(), size_t(0),
                                [](const size_t acc, const ExPolygon& expoly) { return acc + expoly.num_contours(); }));
...
// If Open, then duplicate the first point at the end.
template<bool Open = false> inline Points from_zpath64(const ZPath64& path)
{
    Points out;
    if (!path.empty()) {
        out.reserve(path.size() + (Open ? 1 : 0));
        for (const ZPoint64& p : path)
...
template<bool Open = false> inline void from_zpaths64(const ZPaths64& paths, VecOfPoints& out)
{
    out.reserve(out.size() + paths.size());
    for (const ZPath64& path : paths)
        out.emplace_back(from_zpath64<Open>(path));
}
template<bool Open = false> inline VecOfPoints from_zpaths64(const ZPaths64& paths)
{
    VecOfPoints out;
...
public:
    ...
    {
        // [INTENT] Collapses edge endpoint provenance into either one source id or an intersection pair id.
        ...
        if (new_size == 1) {
            ...
            // [HAZARD] Negative Z is a sentinel index into m_intersections, not a geometric coordinate.
    ...
    {
        return [this](const ZPoint64& e1bot, const ZPoint64& e1top, const ZPoint64& e2bot, const ZPoint64& e2top, ZPoint64& pt) {
...
private:
    // [MEMORY] Reference is non-owning; caller must keep storage alive for visitor lifetime.
    // [COUPLING] This callback contract matches Clipper2's zCallback_ ABI and cannot be freely reshaped.
