...
// [INTENT] Defines the persisted representation for "custom G-code per print height" events
// (color/pause/tool/custom commands) so UI, project save/load, and G-code generation share one
// schema.
// [STATE] Item::from_json() and Info::from_json() mutate existing objects and append to gcodes;
// callers must clear/reinitialize containers when reusing instances.
// [COUPLING] JSON keys/types are a wire-format contract with project files and legacy migration
// code in print configuration.
// [HAZARD] from_json() uses j.at(...) for required fields and throws on malformed payloads;
// mode/type strings silently fall back to Undef/Unknown, mixing hard and soft failure paths.
...
struct Item
{
    ...
    {
        return (rhs.print_z == this->print_z) && (rhs.type == this->type) && (rhs.extruder == this->extruder) &&
               (rhs.color == this->color) && (rhs.extra == this->extra);
    }
    ...
    {
        // [STATE] Per-item parser updates fields in-place; no transactional rollback on throw.
        std::string type_str;
        j.at("type").get_to(type_str);
        std::map<std::string, Type> str2type = {{"ColorChange", ColorChange}, {"PausePrint", PausePrint}, {"ToolChange", ToolChange},
                                                {"Template", Template},       {"Custom", Custom},         {"Unknown", Unknown}};
        type                                 = Unknown;
...
struct Info
{
    ...
    bool operator==(const Info& rhs) const { return (rhs.mode == this->mode) && (rhs.gcodes == this->gcodes); }
    bool operator!=(const Info& rhs) const { return !(*this == rhs); }

    void from_json(const nlohmann::json& j)
    {
        // [STATE] gcodes.reserve()+push_back() appends parsed events in JSON order.
        std::string mode_str;
        if (j.contains("mode"))
