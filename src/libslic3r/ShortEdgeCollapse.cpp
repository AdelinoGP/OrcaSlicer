// [INTENT] Iterative short-edge collapse mesh decimation targeting a triangle count.
// Algorithm: repeatedly shuffles face order (for quality), then for each face
// collapses the shortest qualifying edge below a gradually-increasing threshold,
// removing the collapsed vertex by mapping it to the kept vertex via union-find
// path compression (get_final_index lambda). Vertex to keep/remove is chosen by
// min_vertex_dot_product score (lower = sharper = keep). Non-updated scores save
// compute at the cost of occasional suboptimal collapses late in decimation.
// [STATE] Stateless public interface; all state is local to its_short_edge_collpase().
// [CONCURRENCY] Not thread-safe; operates on a single mesh in-place.
// [COUPLING] NormalUtils::create_normals, its_face_neighbors_par, its_face_normals.
// [HAZARD H1186] Function name is `its_short_edge_collpase` — note the typo ("collpase"
// instead of "collapse"). This is a public API symbol; any port must preserve the typo
// or provide an aliased symbol for binary compatibility.
// [HAZARD H1187] The union-find path compression in get_final_index uses a shared
// flatten_queue vector captured by reference. If this function is ever called
// concurrently on the same mesh (not currently the case) the queue would be corrupted.
// [HAZARD H1188] edge_len growth formula: `edge_len * (1.0f + 1.0 - decimation_ratio)`.
// When decimation_ratio → 1 (many triangles removed), growth factor approaches 1.0
// (no growth), while ratio → 0 (few removed) gives growth factor up to 2x. This is
// intentionally counterintuitive: slow decimation → large edge threshold increase.
// The comment says "if decimation ratio is low, it increases the len up to twice" —
// this is correct but easy to misread. Port must replicate exact formula.
...
void its_short_edge_collpase(indexed_triangle_set& mesh, size_t target_triangle_count)
{
    ...
    // Algorithm uses get_final_index query to get the actual vertex index. The query also updates all mappings on the way, essentially
    ...
    // to determine which vertex to remove and which to keep;  We try to keep the one with larger angle, because it defines the shape
    // "more". The min vertex dot product is lowest dot product of its normal with the normals of faces around it. the lower the dot
    // product, the more we want to keep the vertex NOTE: This score is not updated, even though the decimation does change the mesh. It
    // saves computation time, and there are no strong reasons to update.
    ...
    while (face_indices.size() > target_triangle_count) {
        // simpple func to increase the edge len - if decimation ratio is low, it increases the len up to twice, if decimation ratio is
        // high, increments are low
        ...
        for (const size_t& face_idx : face_indices) {
            ...
            for (size_t edge_idx = 0; edge_idx < 3; ++edge_idx) {
                ...
                if ((mesh.vertices[vertex_index_keep] - mesh.vertices[vertex_index_remove]).squaredNorm() > max_edge_len_squared) {
                    continue;
                }
