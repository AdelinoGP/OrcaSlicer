...
// [INTENT] Slicing.hpp — layer height infrastructure for FFF object slicing.
// Defines SlicingParameters (the fully resolved layer schedule for one object),
// the layer height profile format, and the free-function API for:
//   - Converting per-Z config ranges to a height profile
//   - Adaptive layer height from mesh curvature (@platsch algorithm)
//   - Gaussian smoothing of a layer height profile
//   - Interactive layer height editor actions (increase/decrease/reduce/smooth)
//   - Generating the final [lo,hi] layer boundary vector
//   - Generating the layer-height color texture for the UI editor
//
// [STATE] SlicingParameters is a POD-like value type (static_assert enforces
//   IsTriviallyCopyable). It is computed once per PrintObject in
//   PrintObject::slicing_parameters() and cached. Invalidated when any
//   config key affecting layer heights, raft, or supports changes.
//
// [COUPLING] SlicingParameters is consumed by:
//   - generate_object_layers() → produces PrintObject::m_layers height vector
//   - SupportMaterial (gap computations)
//   - GCode layer generator (raft z-offset)
//   - UI layer height editor (adjust_layer_height_profile / generate_layer_height_texture)
//
// [MEMORY] layer_height_profile is a flat vector<coordf_t> where adjacent
//   pairs [z_i, h_i] encode "at height z_i the layer height is h_i". The
//   vector forms a piecewise-constant (staircase) function. This encoding
//   requires sorted z values and is NOT self-describing — callers must know
//   the encoding convention. See generate_object_layers() for consumption.
//
// [HAZARD] equal_layering() excludes max_suport_layer_height from comparison
//   (commented out). A change to max support layer height does NOT trigger
//   re-generation of object layers via the equal_layering shortcut — support
//   layer mismatches may persist after config changes until a full invalidation.
//
// [HAZARD] equal_layering() also excludes soluble_interface, gap_raft_object,
//   gap_object_support, gap_support_object from comparison (#if 0 block).
//   Changes to these gap values will not invalidate the layer schedule via
//   this comparison path (marked "BBS: not required for equal layer height").
...
// [INTENT] SlicingParameters: fully-resolved layer schedule for one PrintObject.
// Created by create_from_config() which resolves nozzle diameters, raft layers,
// support gaps, and Z shrinkage compensation into absolute mm heights.
// All coordf_t fields are in mm (double). Fields are ZERO-INITIALIZED by default
// member initializers; valid==true only after create_from_config() succeeds.
//
// Key field relationships:
//   object_print_z_max = object_height * shrinkage_compensation_z + raft_z_offset
//   object_print_z_uncompensated_max = object_height + raft_z_offset  (no Z scale)
//   raft_contact_top_z + gap_raft_object = object_print_z_min
//
// [COUPLING] Consumed by generate_object_layers(), SupportMaterial gap logic,
//   GCode raft lift, and UI layer height editor.
// [HAZARD] IsTriviallyCopyable assertion: adding any non-trivial member (e.g., a
//   std::string or std::vector) will cause a compile-time failure here.
struct SlicingParameters
{
    SlicingParameters() = default;

    // Orca: XYZ filament compensation introduced object_shrinkage_compensation
    static SlicingParameters create_from_config(const PrintConfig&               print_config,
...
inline bool equal_layering(const SlicingParameters& sp1, const SlicingParameters& sp2)
{
    assert(sp1.valid);
    assert(sp2.valid);
    return sp1.base_raft_layers == sp2.base_raft_layers && sp1.interface_raft_layers == sp2.interface_raft_layers &&
           sp1.base_raft_layer_height == sp2.base_raft_layer_height && sp1.interface_raft_layer_height == sp2.interface_raft_layer_height &&
           sp1.contact_raft_layer_height == sp2.contact_raft_layer_height && sp1.layer_height == sp2.layer_height &&
           ...
           sp1.first_print_layer_height == sp2.first_print_layer_height && sp1.first_object_layer_height == sp2.first_object_layer_height &&
...
#endif
           sp1.raft_base_top_z == sp2.raft_base_top_z && sp1.raft_interface_top_z == sp2.raft_interface_top_z &&
           sp1.raft_contact_top_z == sp2.raft_contact_top_z && sp1.object_print_z_min == sp2.object_print_z_min;
}
