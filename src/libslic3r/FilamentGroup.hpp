...
// [INTENT] Default per-group capacity used by clustering when machine limits are not provided.
const static int DEFAULT_CLUSTER_SIZE = 16;

// [INTENT] Cost tolerance (in flush-volume units) used to keep near-optimal alternatives.
const static int ABSOLUTE_FLUSH_GAP_TOLERANCE = 5;

namespace Slic3r {
// [INTENT] Collects unique filament ids used across all layers and returns sorted order.
std::vector<unsigned int> collect_sorted_used_filaments(const std::vector<std::vector<unsigned int>>& layer_filaments);

enum FGStrategy { BestCost, BestFit };

enum FGMode { FlushMode, MatchMode };

namespace FilamentGroupUtils {
// [INTENT] Wall-clock stopwatch used to cap heuristic clustering runtime.
...
public:
    void time_machine_start() { start = std::chrono::high_resolution_clock::now(); }
...
// [INTENT] Represents one candidate assignment with objective cost + preference priority.
...
{
    // [STATE] Inputs derived from sliced model data.
    struct ModelInfo
    {
    ...
    // [STATE] Strategy/tuning parameters selected by caller or preset.
    struct GroupInfo
    {
    ...
    // [STATE] Machine capabilities and currently loaded filament inventory.
    struct MachineInfo
    {
...
{
public:
    // [INTENT] Precomputes pairwise distances used by PAM2 filament clustering.
    FlushDistanceEvaluator(const FlushMatrix&                            flush_matrix,
                           const std::vector<unsigned int>&              used_filaments,
...
private:
    // [MEMORY] Dense O(N^2) matrix for O(1) distance lookup during assignment.
...
public:
    // [COUPLING] Context is copied in and then consumed by both exact and heuristic solvers.
...
private:
    // [INTENT] Dispatcher selecting exact vs heuristic min-flush solver.
    std::vector<int> calc_min_flush_group(int* cost = nullptr);
    // [INTENT] Exhaustive bitmask search for small filament counts.
    std::vector<int> calc_min_flush_group_by_enum(const std::vector<unsigned int>& used_filaments, int* cost = nullptr);
    // [INTENT] PAM2 k-medoids heuristic with timeout for larger filament sets.
    std::vector<int> calc_min_flush_group_by_pam2(const std::vector<unsigned int>& used_filaments,
                                                  int*                             cost       = nullptr,
                                                  int                              timeout_ms = 300);
    ...
    std::vector<int>                          seperate_merged_filaments(const std::vector<int>&                          filament_map,
                                                                        const std::unordered_map<int, std::vector<int>>& merged_filaments);

private:
    // [STATE] Mutable working context (may be rebuilt after filament-merging prepass).
    FilamentGroupContext ctx;
    // [STATE] Near-optimal alternatives retained for downstream AMS matching.
...
class KMediods2
{
    ...
    enum INIT_TYPE { Random = 0, Farthest };

public:
    // [INTENT] Specialized 2-way medoid clustering for dual-extruder assignment.
    KMediods2(const int elem_count, const std::shared_ptr<FlushDistanceEvaluator>& evaluator, int default_group_id = 0)
        : m_evaluator{evaluator}, m_elem_count{elem_count}, m_default_group_id{default_group_id}
    {
...
private:
    ...
    std::vector<int> assign_cluster_label(const std::vector<int>&   center,
                                          const std::map<int, int>& unplaceable_limits,
...
protected:
    // [STATE] Candidate heap used to preserve near-best clusterings.
    FilamentGroupUtils::MemoryedGroupHeap memoryed_groups;
    // [COUPLING] Shared evaluator drives both assignment and cost evaluation.
    ...
    // [HAZARD] H1212: hard-coded to 2 clusters; implementation is not generic k-medoids.
    const int m_k = 2;
    int       m_elem_count;
