...
// [INTENT] AppConfig is the persistent key-value state for UI/runtime behavior,
// network integration, and preset-related selections.
// [COUPLING] Section/key names are consumed across GUI and libslic3r modules,
// making this header an implicit cross-module contract.
// [HAZARD] Values are mostly schema-less strings; compatibility depends on
// disciplined migration and defaulting in the .cpp implementation.
...
public:
    ...
    explicit AppConfig() : m_dirty(false), m_orig_version(Semver::invalid()), m_mode(EAppMode::Editor), m_legacy_datadir(false)
    {
        this->reset();
    }
    ...
    {
        // [STATE] Read-only accessor: absence is signaled explicitly by false.
        value.clear();
    ...
    {
        std::string value;
        this->get(section, key, value);
        return value;
    }
    std::string get(const std::string& key) const
    {
        std::string value;
        this->get("app", key, value);
        return value;
    }
    ...
    bool get_bool(const std::string& key) const { return this->get_bool("app", key); }
    void set(const std::string& section, const std::string& key, const std::string& value)
    {
        ...
        if (old != value) {
            old = value;
            // [STATE] Marks this config snapshot as requiring persistence.
            m_dirty = true;
        }
    }
    ...
    void set(const std::string& key, const std::string& value) { this->set("app", key, value); }

    void set_bool(const std::string& key, const bool& value) { this->set("app", key, value); }
    ...
    bool has(const std::string& key) const { return this->has("app", key); }
    ...
    bool has_section(const std::string& section) const { return m_storage.find(section) != m_storage.end(); }
    const std::map<std::string, std::string>& get_section(const std::string& section) const { return m_storage.find(section)->second; }
    void set_section(const std::string& section, const std::map<std::string, std::string>& data) { m_storage[section] = data; }
    void clear_section(const std::string& section) { m_storage[section].clear(); }
    ...
    void set_vendors(const VendorMap& vendors)
    {
        m_vendors = vendors;
        m_dirty   = true;
    }
    void set_vendors(VendorMap&& vendors)
    {
    ...
    bool has_printer_settings(std::string printer) const { return m_printer_settings.find(printer) != m_printer_settings.end(); }
    ...
    {
        // [HAZARD] Stored as loosely typed JSON; semantic validation is deferred
        // to consuming modules and may fail far from the mutation site.
    ...
    void                     set_mouse_device(const std::string& name,
...
private:
    template<typename T> bool get_3dmouse_device_numeric_value(const std::string& device_name, const char* parameter_name, T& out) const
    {
        ...
        if (it_val == it->second.end())
            return false;
        // [HAZARD] Conversion assumes decimal text and performs an unchecked cast
        // to T, so malformed values can silently truncate.
    ...
    // [STATE] Central mutable settings store shared by application and tooling.
    ...
    // [STATE] Dirty bit used to avoid redundant disk writes.
    bool m_dirty;
