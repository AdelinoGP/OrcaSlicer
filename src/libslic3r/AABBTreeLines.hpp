...
// [INTENT] Provides 2D AABB-tree queries over indexed line-segment sets:
//   - Nearest-line-segment distance (signed and unsigned), point+index output
//   - Point-in-polygon test via coordinate-aligned ray casting
//   - All-lines-within-radius query
//   - Segment intersection enumeration (sorted or unsorted)
//   - LinesDistancer<LineType>: a convenience wrapper bundling a line vector + its AABB tree
//     into a single object with a clean query API.
//
// [COUPLING] Built on top of AABBTreeIndirect::Tree<2, Scalar> and its recursive helpers.
// All 2D operations assume integer or floating-point line types satisfying the Slic3r::Line concept
// (fields a, b of type Vec<2,Scalar>; static constexpr int Dim = 2).
//
// [HAZARD] H717 — `point_outside_closed_contours` casts a horizontal ray in both directions and
// checks that BOTH are odd (inside) or BOTH are even (outside).  If they disagree, it retries with
// a vertical ray.  If the vertical ray also disagrees, it returns 0 (indeterminate).  This
// indeterminate result is silently treated as "on boundary" by callers — not as an error.  For
// numerically degenerate geometry (self-intersecting contours, extremely thin slivers) the test
// can return 0 permanently, silently dropping those regions from inside/outside classification.
//
// [HAZARD] H718 — The signed-distance computation in LinesDistancer::distance_from_lines_extra
// multiplies the unsigned distance by the result of outside(), which returns -1 for inside.
// If outside() returns 0 (indeterminate), the signed distance is 0 regardless of the actual
// geometric distance — silently incorrect for near-boundary query points.
//
// [CONCURRENCY] LinesDistancer stores a mutable-free tree and const lines vector — query methods
// are safe to call from multiple threads after construction.  The constructor is not thread-safe.
...
// [INTENT] `get_intersections_with_line` (free function, sorted/unsorted) enumerates all
// line-segment intersections with a query line segment using AABB pruning.  When `sorted=true`,
// results are sorted by squared distance from line.a, ascending — giving left-to-right order
// for horizontal sweep lines or inside/outside toggle traversal.
// [COUPLING] Sorting adds an O(K log K) step where K = number of intersections; for most
// slicing uses K is small (2–6 per contour).  The unsorted variant avoids this cost.
...
// [INTENT] LinesDistancer<LineType> is the primary user-facing API for 2D line-segment proximity
// queries.  It bundles a copied/moved line vector together with its AABB tree, constructed once at
// creation time, and exposes a clean query interface.  After construction the object is immutable
// (tree and lines do not change), making it safe to share across threads for read queries.
//
// [MEMORY] Stores a full copy of the lines vector plus the AABB tree (~2× N nodes).  For large
// contour sets (e.g., millions of support lines), peak memory doubles during construction.
// Consider the move constructor to avoid the extra copy when the input vector can be consumed.
//
// [STATE] `tree` is built eagerly in the constructor; there is no lazy or deferred build path.
// Constructing a LinesDistancer on an empty vector produces an empty tree; all query functions
// handle the empty-tree case by returning sentinel values (infinity, -1, empty vectors).
//
// [COUPLING] `outside()` delegates to `point_outside_closed_contours` — the H717 indeterminate
// result (returns 0) propagates to `distance_from_lines_extra<SIGNED_DISTANCE>` via H718.
