...
// [INTENT] SquareGrid.cpp implements the discrete grid traversal rules used by Arachne's sparse
// indices. These helpers deliberately favor simple integer arithmetic over exact computational-
// geometry coverage tests because they sit on hot lookup paths.
...
SquareGrid::SquareGrid(coord_t cell_size) : cell_size(cell_size) { assert(cell_size > 0U); }

SquareGrid::GridPoint SquareGrid::toGridPoint(const Vec2i64& point) const { return Point(toGridCoord(point.x()), toGridCoord(point.y())); }

SquareGrid::grid_coord_t SquareGrid::toGridCoord(const int64_t& coord) const
{
    // [HAZARD] Integer truncation toward zero is intentional. It biases negative coordinates near
    // the origin into the zero bucket instead of the mathematically floored bucket.
...
bool SquareGrid::processLineCells(const std::pair<Point, Point> line, const std::function<bool(GridPoint)>& process_cell_func) const
{
    ...
    if (end.x() < start.x()) { // make sure X increases between start and end
        std::swap(start, end);
    }
    ...
    /* [INTENT] This line drawing algorithm iterates over the range of Y coordinates, and
    ...
    for (grid_coord_t cell_y = start_cell.y(); cell_y * y_dir <= end_cell.y() * y_dir; cell_y += y_dir) { // for all Y from start to end
        ...
        if (y_diff == 0) {
            ...
            // [HAZARD] The extra adjustment for negative non-divisible quotients compensates for C++
            // division truncation so crossed-cell coverage stays symmetric around the origin.
            x_cell_end = toGridCoord(corresponding_x + ((corresponding_x < 0) && ((area % y_diff) != 0)));
            if (x_cell_end < start_cell.x()) { // process at least one cell!
...
bool SquareGrid::processNearby(const Point& query_pt, coord_t radius, const std::function<bool(const GridPoint&)>& process_func) const
{
    ...
    // [INTENT] Nearby lookup is square, not circular. Callers wanting a true radius filter must do
    // an exact distance test on the returned elements.
...
SquareGrid::grid_coord_t SquareGrid::nonzeroSign(const grid_coord_t z) const { return (z >= 0) - (z < 0); }

coord_t SquareGrid::getCellSize() const { return cell_size; }
