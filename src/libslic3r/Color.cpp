// [INTENT] Color representation utilities: RGB↔HSV conversion, ColorRGB/ColorRGBA value types,
//          hex encode/decode, GL picking encode/decode checksum, and heuristic "opposite colour"
//          generation for UI contrast.
// [STATE]  All free functions are stateless. ColorRGB/ColorRGBA hold float arrays clamped [0,1].
// [CONCURRENCY] Safe — no global mutable state except the Randomizer in opposite().
// [COUPLING] Depends on libslic3r.h for base types only. No geometry dependencies.
#include "libslic3r.h"
#include "Color.hpp"
...
// [INTENT] Internal PRNG wrapper. Creates a fresh mt19937 seeded from hardware random_device
//          on each call to random_float().
// [HAZARD H1159] Randomizer is instantiated locally in opposite() on every call. Each call
//               constructs a new mt19937 seeded from random_device — this is nondeterministic
//               and not reproducible across runs. Slices that use colour contrast for seam
//               selection or similar decisions will produce non-deterministic G-code between
//               identical runs. If determinism is required, seed must come from a fixed source.
class Randomizer
{
...
ColorRGB::ColorRGB(float r, float g, float b) : m_data({std::clamp(r, 0.0f, 1.0f), std::clamp(g, 0.0f, 1.0f), std::clamp(b, 0.0f, 1.0f)}) {}
...
ColorRGB complementary(const ColorRGB& color) { return {1.0f - color.r(), 1.0f - color.g(), 1.0f - color.b()}; }

ColorRGBA complementary(const ColorRGBA& color) { return {1.0f - color.r(), 1.0f - color.g(), 1.0f - color.b(), color.a()}; }
...
ColorRGBA saturate(const ColorRGBA& color, float factor) { return to_rgba(saturate(to_rgb(color), factor), color.a()); }
...
ColorRGBA picking_decode(unsigned int id)
{
    return {
        ...
        float(picking_checksum_alpha_channel(id & 0xff, (id >> 8) & 0xff, (id >> 16) & 0xff)) *
            INV_255 // checksum for validating against unwanted alpha blending and multi sampling
    };
}
