// [INTENT] PrintApply.cpp — implements Print::apply(), the central synchronisation point
//          between the UI model (Model/ModelObject/ModelInstance) and the slicing backend
//          (Print/PrintObject/PrintRegion).  Called from the main thread whenever the user
//          changes any parameter; runs under Print::state_mutex().
//
// [STATE]  The function manages two parallel object lists:
//            m_model (Print's internal copy of ModelObjects)
//            m_objects (PrintObject ptrs, each tied to one ModelObject + trafo)
//          It must reconcile additions, deletions, moves, and in-place modifications,
//          producing a precise invalidation set to restart only affected pipeline steps.
//
// [CONCURRENCY] Print::apply() acquires state_mutex() before touching any step state.
//               The background slicing thread checks cancellation via throw_if_canceled().
//               The INVALID-before-stop race (H819) may cause a narrow window where
//               step state is INVALID but the worker has not yet stopped — see PrintBase.cpp.
//
// [COUPLING] Tightly coupled to:
//   - Model.hpp (ModelObject, ModelVolume, ModelInstance, ObjectID)
//   - Print.hpp (PrintObject, PrintRegion, PrintObjectRegions)
//   - PrintConfig.hpp (PrintRegionConfig, normalize_fdm_*)
//   - ClipperUtils.hpp (diff, intersection) for printable-filament-changed check
//
// [MEMORY]  print_object_regions is ref-counted (ref_cnt_inc/dec).  ModelObjectStatus
//           holds a raw pointer and calls ref_cnt_dec in its destructor — must not
//           be copied.  ModelObjectStatusDB stores by value in a std::set; the
//           non-copyable destructor is safe because std::set never copies elements.
...
/* static */ void model_volume_list_update_supports(ModelObject& model_object_dst, const ModelObject& model_object_new)
{
    ...
    auto model_volume_lower = [](const ModelVolumeWithStatus& mv1, const ModelVolumeWithStatus& mv2) {
        return mv1.first->id() < mv2.first->id();
    };
    auto model_volume_equal = [](const ModelVolumeWithStatus& mv1, const ModelVolumeWithStatus& mv2) {
        return mv1.first->id() == mv2.first->id();
    };
...
static inline void model_volume_list_copy_configs(ModelObject&          model_object_dst,
                                                  const ModelObject&    model_object_src,
                                                                                 ...
                                                                                 const Vec3d&       shrinkage_compensation)
{
    ...
    for (ModelInstance* model_instance : model_object.instances) {
        if (model_instance->is_printable()) {
            ...
            // BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", Line %1%: found object %2%'s instance %3% for print")%__LINE__
            // %model_object.name %index;
        } else {
            // BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", Line %1%: found object %2%'s instance %3% not printable")%__LINE__
            // %model_object.name %index; BOOST_LOG_TRIVIAL(debug) << boost::format(" object printable %1%, instance printable %2%,
            // print_volume_state %3%")%model_object.printable %model_instance->printable %model_instance->print_volume_state;
        }
        index++;
    }
...
static bool layer_height_ranges_equal(const t_layer_config_ranges& lr1, const t_layer_config_ranges& lr2, bool check_layer_height)
{
    ...
    for (const auto& kvp1 : lr1) {
        ...
        if (!kvp2.second.has("layer_height") || !kvp1.second.has("layer_height"))
            return false;
        if (std::abs(kvp1.first.first - kvp2.first.first) > EPSILON || std::abs(kvp1.first.second - kvp2.first.second) > EPSILON ||
            (check_layer_height &&
             std::abs(kvp1.second.option("layer_height")->getFloat() - kvp2.second.option("layer_height")->getFloat()) > EPSILON))
            return false;
    }
    return true;
}
...
// BBS: add plate index
static t_config_option_keys print_config_diffs(const PrintConfig&        current_config,
                                               ...
                                               std::vector<int>&         filament_maps)
{
    ...
    for (const t_config_option_key& opt_key : current_config.keys()) {
        ...
        if (opt_new == nullptr)
            ...
            continue;
        const ConfigOption* opt_new_filament = std::binary_search(extruder_retract_keys.begin(), extruder_retract_keys.end(), opt_key) ?
                                                   new_full_config.option(filament_prefix + opt_key) :
                                                   nullptr;

        if (opt_new_filament != nullptr) {
            compute_filament_override_value(opt_key, opt_old, opt_new, opt_new_filament, new_full_config, print_diff, filament_overrides,
                                            filament_maps);
...
// BBS: add plate index
static t_config_option_keys full_print_config_diffs(const DynamicPrintConfig& current_full_config,
                                                    const DynamicPrintConfig& new_full_config,
                                                    int                       plate_index)
{
...
static bool is_printable_filament_changed(const DynamicPrintConfig& new_full_config, const Polygon& old_poly, const Polygon& new_poly)
{
    if (old_poly != new_poly) {
        ...
        Points               pts;
        for (auto pt : printable_area) {
            pts.emplace_back(Point(scale_(pt.x()), scale_(pt.y())));
        }
...
// [INTENT] LayerRanges — converts the sparse ModelObject::layer_config_ranges map into a
//          dense, sorted, non-overlapping vector of intervals covering [0, DBL_MAX).
//          Overlap resolution: the first range in the sorted input trims subsequent ones.
//          The last interval always has config == nullptr (default region config applies).
// [COUPLING] The returned DynamicPrintConfig* pointers point into ModelObject::layer_config_ranges.
//            The ModelObject must outlive this LayerRanges instance.
class LayerRanges
{
public:
    struct LayerRange
    ...
    const DynamicPrintConfig* config(const t_layer_height_range& range) const
    {
        ...
        if (it == m_ranges.end() || std::abs(it->layer_height_range.first - range.first) > EPSILON ||
...
struct PrintObjectStatus
{
    ...
    PrintObjectStatus(PrintObject* print_object, Status status = Unknown)
        : id(print_object->model_object()->id()), print_object(print_object), trafo(print_object->trafo()), status(status)
    {}
...
public:
    ...
    iterator_range get_range(const ModelObject& model_object) const { return m_db.equal_range(PrintObjectStatus(model_object.id())); }
    ...
    size_t count(const ModelObject& model_object) { return m_db.count(PrintObjectStatus(model_object.id())); }
...
static void transformed_its_bboxes_in_z_ranges(const indexed_triangle_set&                                    its,
                                               const Transform3f&                                             m,
...
void print_objects_regions_invalidate_keep_some_volumes(PrintObjectRegions& print_object_regions,
                                                        ModelVolumePtrs     old_volumes,
                                                        ModelVolumePtrs     new_volumes)
{
        ...
        if (model_volume_solid_or_modifier(*new_volumes[i_new])) {
            ...
            if (i_old != old_volumes.size() && old_volumes[i_old]->id() == new_volumes[i_new]->id()) {
                if (old_volumes[i_old]->get_matrix().isApprox(new_volumes[i_new]->get_matrix())) {
                    ...
                    assert(i_cached_volume < print_object_regions.cached_volume_ids.size() &&
                           print_object_regions.cached_volume_ids[i_cached_volume] == old_volumes[i_old]->id());
    ...
    print_object_regions.cached_volume_ids.erase(print_object_regions.cached_volume_ids.begin() + last_cached_volume,
                                                 print_object_regions.cached_volume_ids.end());
}

// Find a bounding box of a volume's part intersecting layer_range. Such a bounding box will likely be smaller in XY than the full bounding
// box, thus it will intersect with lower number of other volumes.
const PrintObjectRegions::BoundingBox* find_volume_extents(const PrintObjectRegions::LayerRangeRegions& layer_range,
                                                           const ModelVolume&                           volume)
{
    auto it = lower_bound_by_predicate(layer_range.volumes.begin(), layer_range.volumes.end(),
                                       [&volume](const PrintObjectRegions::VolumeExtents& l) { return l.volume_id < volume.id(); });
...
PrintObjectRegions::BoundingBox find_modifier_volume_extents(const PrintObjectRegions::LayerRangeRegions& layer_range,
    ...
    const std::function<void(const PrintRegionConfig&, const PrintRegionConfig&, const t_config_option_keys&)>& callback_invalidate)
{
    ...
    for (PrintObjectRegions::LayerRangeRegions& layer_range : print_object_regions.layer_ranges) {
        ...
        for (PrintObjectRegions::VolumeRegion& region : layer_range.volume_regions)
            if (region.model_volume->is_model_part() || region.model_volume->is_modifier()) {
                auto it_model_volume = lower_bound_by_predicate(model_volumes.begin(), model_volumes.end(), [&region](const ModelVolume* l) {
                    return l->id() < region.model_volume->id();
                });
                    ...
                    for (int parent_region_id = next_region_id - 1; parent_region_id >= 0; --parent_region_id) {
                        ...
                        if (parent_region.model_volume->is_model_part() || parent_region.model_volume->is_modifier()) {
                            ...
                            if (next_region_id < int(layer_range.volume_regions.size()) &&
                                ...
                                ++next_region_id;
                            } else if (PrintObjectRegions::BoundingBox parent_bbox = find_modifier_volume_extents(layer_range,
                                                                                                                  parent_region_id);
                                       parent_bbox.intersects(*bbox))
                                ...
                                if (PrintRegionConfig config = region_config_from_model_volume(parent_region.region->config(), nullptr,
                                                                                               **it_model_volume, num_extruders);
                ...
                PrintRegionConfig cfg = region.parent == -1 ?
                                            region_config_from_model_volume(default_region_config, layer_range.config, **it_model_volume,
                                                                            num_extruders) :
                                            region_config_from_model_volume(layer_range.volume_regions[region.parent].region->config(),
                                                                            nullptr, **it_model_volume, num_extruders);
    ...
    for (const PrintObjectRegions::LayerRangeRegions& layer_range : print_object_regions.layer_ranges) {
        for (const PrintObjectRegions::FuzzySkinPaintedRegion& region : layer_range.fuzzy_skin_painted_regions) {
            ...
            if (cfg.fuzzy_skin.value != FuzzySkinType::Disabled_fuzzy)
    ...
    // Lastly verify, whether some regions were not merged.
    {
        ...
        std::sort(regions.begin(), regions.end(),
                  [](const PrintRegion* l, const PrintRegion* r) { return l->config_hash() < r->config_hash(); });
...
void update_volume_bboxes(std::vector<PrintObjectRegions::LayerRangeRegions>& layer_ranges,
        ...
        for (const ModelVolume* model_volume : model_volumes)
            if (model_volume_solid_or_modifier(*model_volume)) {
                if (std::binary_search(cached_volume_ids.begin(), cached_volume_ids.end(), model_volume->id())) {
                    auto it = lower_bound_by_predicate(volumes_old.begin(), volumes_old.end(),
                                                       [model_volume](PrintObjectRegions::VolumeExtents& l) {
                                                           return l.volume_id < model_volume->id();
                                                       });
                    if (it != volumes_old.end() && it->volume_id == model_volume->id())
                        layer_range.volumes.emplace_back(*it);
                } else
                    layer_range.volumes.push_back(
                        {model_volume->id(), transformed_its_bbox2d(model_volume->mesh().its,
            ...
            if (model_volume_solid_or_modifier(*model_volume)) {
                if (std::binary_search(cached_volume_ids.begin(), cached_volume_ids.end(), model_volume->id())) {
                    for (PrintObjectRegions::LayerRangeRegions& layer_range : layer_ranges) {
                        ...
                        auto        it   = lower_bound_by_predicate(vold.begin(), vold.end(),
                                                                    [model_volume](const PrintObjectRegions::VolumeExtents& l) {
                                                               return l.volume_id < model_volume->id();
                                                           });
                    ...
                    transformed_its_bboxes_in_z_ranges(model_volume->mesh().its, trafo_for_bbox(object_trafo, model_volume->get_matrix()),
                                                       ranges, bboxes, offset);
                    for (PrintObjectRegions::LayerRangeRegions& layer_range : layer_ranges)
                                                         ...
                                                         const bool                       has_painted_fuzzy_skin)
{
    ...
    auto  out                  = print_object_regions_old ? std::unique_ptr<PrintObjectRegions>(print_object_regions_old) :
                                                            std::make_unique<PrintObjectRegions>();
    ...
    const bool is_mm_painted = num_extruders > 1 && std::any_of(model_volumes.cbegin(), model_volumes.cend(),
                                                                [](const ModelVolume* mv) { return mv->is_mm_painted(); });
    update_volume_bboxes(layer_ranges_regions, out->cached_volume_ids, model_volumes, out->trafo_bboxes,
                         is_mm_painted ? 0.f : std::max(0.f, xy_contour_compensation));
                ...
                if (const PrintObjectRegions::BoundingBox* bbox = find_volume_extents(layer_range, volume); bbox) {
                    if (volume.is_model_part()) {
                        ...
                        layer_range.volume_regions.push_back(
                            {&volume, -1,
                             get_create_region(
                                 region_config_from_model_volume(default_region_config, layer_range.config, volume, num_extruders)),
                             bbox});
                        ...
                        for (int parent_region_id = int(layer_range.volume_regions.size()) - 1; parent_region_id >= 0; --parent_region_id) {
                            ...
                            if (parent_volume.is_model_part() || parent_volume.is_modifier())
                                if (PrintObjectRegions::BoundingBox parent_bbox = find_modifier_volume_extents(layer_range,
                                                                                                               parent_region_id);
                                    parent_bbox.intersects(*bbox)) {
                                    // Only create new region for a modifier, which actually modifies config of it's parent.
                                    if (PrintRegionConfig config = region_config_from_model_volume(parent_region.region->config(), nullptr,
                                                                                                   volume, num_extruders);
                                        config != parent_region.region->config()) {
                                        added = true;
                                        layer_range.volume_regions.push_back(
                                            {&volume, parent_region_id, get_create_region(std::move(config)), bbox});
                        ...
                        if (!added && parent_model_part_id >= 0)
                            ...
                            // Store it so that verify_update_print_object_regions() will handle this modifier correctly if its
                            // configuration changes.
                            layer_range.volume_regions.push_back(
                                {&volume, parent_model_part_id, layer_range.volume_regions[parent_model_part_id].region, bbox});
                    }
                }
        }
    }
    ...
    if (has_painted_fuzzy_skin) {
        ...
        for (PrintObjectRegions::LayerRangeRegions& layer_range : layer_ranges_regions) {
            ...
            for (int parent_volume_region_id = 0; parent_volume_region_id < int(layer_range.volume_regions.size());
                ...
                if (const PrintObjectRegions::VolumeRegion& parent_volume_region = layer_range.volume_regions[parent_volume_region_id];
                    parent_volume_region.model_volume->is_model_part() || parent_volume_region.model_volume->is_modifier()) {
                    PrintRegionConfig cfg = parent_volume_region.region->config();
                    if (cfg.fuzzy_skin.value != FuzzySkinType::Disabled_fuzzy)
                        cfg.fuzzy_skin.value = FuzzySkinType::All;
                    layer_range.fuzzy_skin_painted_regions.push_back(
                        {FuzzySkinParentType::VolumeRegion, parent_volume_region_id, get_create_region(std::move(cfg))});
                }
            }

            for (int parent_painted_regions_id = 0; parent_painted_regions_id < int(layer_range.painted_regions.size());
                 ++parent_painted_regions_id) {
                ...
                if (cfg.fuzzy_skin.value != FuzzySkinType::Disabled_fuzzy)
                    cfg.fuzzy_skin.value = FuzzySkinType::All;
                layer_range.fuzzy_skin_painted_regions.push_back(
                    {FuzzySkinParentType::PaintedRegion, parent_painted_regions_id, get_create_region(std::move(cfg))});
            }
            ...
            std::sort(layer_range.fuzzy_skin_painted_regions.begin(), layer_range.fuzzy_skin_painted_regions.end(),
                      [&layer_range](auto& l, auto& r) {
...
// [INTENT] Print::apply() — the central UI→backend synchronisation function.
// Called from the main thread (under state_mutex()) whenever the user modifies
// any parameter or the model.  Produces a precise ApplyStatus:
//   UNCHANGED — nothing changed, background work continues
//   CHANGED   — config changed but no steps were invalidated
//   INVALIDATED — at least one pipeline step was stopped and must re-run
//
// High-level structure (5 phases):
//   Phase 0: Config normalisation & diff computation
//   Phase 1: Model object list synchronisation (add/remove/reorder)
//   Phase 2: PrintObjectStatusDB construction (snapshot of current PrintObjects)
//   Phase 3: ModelObject ↔ PrintObject content sync (volumes, configs, instances)
//   Phase 4: PrintObject generation / trafo matching
//   Phase 5: PrintObjectRegions verification / regeneration
//
// [HAZARD] normalize_fdm_2() is called TWICE — once before Phase 1 and once
//          after Phase 4.  The second pass (line ~1714) handles cases where
//          newly added objects change the used-filament count.
// [CONCURRENCY] state_mutex() is acquired at line ~1325 and held until return.
//               Background thread checks throw_if_canceled() independently.
Print::ApplyStatus Print::apply(const Model& model, DynamicPrintConfig new_full_config)
{
    ...
    // [INTENT] Phase 0a: Ensure the three settings-ID options exist so that
    //          subsequent diff/apply operations can reference them safely.
    ...
    new_full_config.update_values_to_printer_extruders(new_full_config, printer_options_with_variant_1, "printer_extruder_id",
                                                       "printer_extruder_variant");
    new_full_config.update_values_to_printer_extruders(new_full_config, printer_options_with_variant_2, "printer_extruder_id",
                                                       "printer_extruder_variant", 2);
    // update print config related with variants
    new_full_config.update_values_to_printer_extruders(new_full_config, print_options_with_variant, "print_extruder_id",
                                                       "print_extruder_variant");

    m_ori_full_print_config = new_full_config;
    new_full_config.update_values_to_printer_extruders_for_multiple_filaments(new_full_config, filament_options_with_variant,
    ...
    // [INTENT] Phase 0b: Compute all config diffs before acquiring the state mutex.
    //          This allows the expensive diff operations to run outside the critical section.
    //
    // [STATE] print_diff      — keys changed in PrintConfig (drives step invalidation)
    //         full_config_diff — keys changed anywhere in full config (drives psGCodeExport invalidation)
    //         object_diff     — keys changed in default object config
    //         region_diff     — keys changed in default region config
    //
    // [HAZARD] filament_map_mode logic below: when mode < fmmManual, filament_map
    //          changes are silently swallowed (old config updated in-place) so they
    //          do NOT appear in print_diff.  This means a filament_map change in
    //          auto-assign mode will never trigger step invalidation.
    //
        ...
        if (map_mode < fmmManual) {
            ...
            if (old_filament_map.size() == new_filament_map.size()) {
                ...
                for (size_t index = 0; index < old_filament_map.size(); index++) {
                    if ((old_filament_map[index] == new_filament_map[index]) || (used_filament_set.find(index) == used_filament_set.end()))
                        continue;
                    else {
                        same_map = false;
                        break;
    ...
    // [INTENT] Phase 0c: Acquire the state mutex, then apply config diffs.
    //          After this point all step-state changes are protected.
    //
    // [CONCURRENCY] state_mutex() is a std::mutex; held until end of function.
    //               Background thread checks throw_if_canceled() independently.
    //               Any invalidate_*() call here may trigger call_cancel_callback().
    // [STATE] apply_status accumulates the maximum severity across all changes via
    //         update_apply_status(); it is cast to ApplyStatus at return.
    ...
    if (!(print_diff.empty() && object_diff.empty() && region_diff.empty())) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(
                                       ", got print_diff %1%, object_diff %2%, region_diff %3%, set status to APPLY_STATUS_CHANGED") %
                                       print_diff.size() % object_diff.size() % region_diff.size();
    }
    ...
    if (!full_config_diff.empty()) {
        ...
        //  As long as these config options don't reallocate memory when copying, we are safe overriding a value, which is in use by a
        //  worker thread.
    ...
    // [INTENT] Phase 1: Synchronise the model object list.
    //   Three sub-cases based on whether the model identity changed:
    //   a) model.id() != m_model.id() — entire model replaced; delete all PrintObjects,
    //      deep-copy the new model, mark all objects New.
    //   b) model_object_list_extended() — objects were only appended; mark old ones Old,
    //      copy-append the new tails.
    //   c) general reorder/deletion — stop background, rebuild m_model.objects from
    //      scratch matching by ObjectID, mark each as Old/Moved/New/Deleted.
    //      Delete PrintObjects of Deleted model objects immediately.
    //
    // [STATE] print_regions_reshuffled is set true whenever any PrintObject is added
    //         or removed — triggers Phase 5's region rebuild logic.
    // [MEMORY] ModelObject::new_copy() deep-copies the geometry; expensive for large meshes.
        ...
        if (m_model.get_curr_plate_custom_gcodes() != model.get_curr_plate_custom_gcodes()) {
            update_apply_status(
                num_extruders_changed ||
                        ...
                        // to the active print / model state, and then it is reset, so it is being applicable, but empty, thus the effect is
                        // the same.
                        (num_extruders > 1 && custom_per_printz_gcodes_tool_changes_differ(m_model.get_curr_plate_custom_gcodes().gcodes,
                                                                                           model.get_curr_plate_custom_gcodes().gcodes)) ?
    ...
    // [INTENT] Phase 2: Build a snapshot of all current PrintObjects keyed by
    //          ModelObject ID.  This snapshot is used in Phase 3 to find and
    //          reuse matching PrintObjects instead of recreating them.
    // [STATE] print_object_status_db is cleared at the end of Phase 4 after
    //         all reused/deleted entries have been processed.

    // 2) Map print objects including their transformation matrices.
    PrintObjectStatusDB print_object_status_db(m_objects);

    // [INTENT] Phase 3: Synchronise each ModelObject's content into its m_model
    //          counterpart and propagate config/geometry changes to the matching
    //          PrintObjects (if any).
    //
    //   For each ModelObject (compared against its m_model shadow):
    //   - If solid volumes, origin_translation, layer_height_ranges, or layer_height_profile
    //     changed → invalidate ALL steps of every matching PrintObject; mark
    //     print_object_regions Invalid or PartiallyValid.
    //   - If only support volumes changed → invalidate posSupportMaterial only.
    //   - If only seam data changed → invalidate psGCodeExport only.
    //   - If object config changed → invalidate per-config-option.
    //   - If instances changed → copy new instances; possibly invalidate psGCodeExport.
    //
    // [STATE] model_object_status.print_object_regions is loaded here from the
    //         first PrintObject in its range; ref_cnt_inc() is called immediately.
    //         If the object is fully invalidated, print_object_regions is either
    //         cleared or freed and set to nullptr for Phase 5 to rebuild.
    //
    // [COUPLING] model_volume_list_copy_configs(), layer_height_ranges_copy_configs(),
    //            model_object.assign_copy() all perform deep copies of heavy geometry.
    //
    // [HAZARD] transform3d_equal() uses exact float comparison — see top of file.
    ...
    for (size_t idx_model_object = 0; idx_model_object < model.objects.size(); ++idx_model_object) {
        ...
        bool layer_height_ranges_differ = !layer_height_ranges_equal(model_object.layer_config_ranges, model_object_new.layer_config_ranges,
        ...
        if (solid_or_modifier_differ || model_origin_translation_differ || layer_height_ranges_differ ||
            ...
            if (model_object_status.print_object_regions_status == ModelObjectStatus::PrintObjectRegionsStatus::PartiallyValid)
                ...
                print_objects_regions_invalidate_keep_some_volumes(*model_object_status.print_object_regions, model_object.volumes,
                                                                   model_object_new.volumes);
        ...
        if (!solid_or_modifier_differ) {
            ...
            if (!object_diff.empty() || object_config_changed || num_extruders_changed) {
                PrintObjectConfig new_config = PrintObject::object_config_from_model_object(m_default_object_config, model_object,
                                                                                            num_extruders);
            ...
            if (model_object.instances.size() != model_object_new.instances.size() ||
                ...
                for (const ModelInstance* model_instance : model_object_new.instances) {
                    ...
                    model_object.instances.back()->set_model_object(&model_object);
                                   ...
                                   [](auto l, auto r) {
                                       return l->print_volume_state == r->print_volume_state && l->printable == r->printable &&
                ...
                for (auto old_instance = model_object.instances.begin(); old_instance != model_object.instances.end();
                     ++old_instance, ++new_instance) {
                    if (is_printable_filament_changed(new_full_config, (*old_instance)->convex_hull_2d(),
                                                      (*new_instance)->convex_hull_2d())) {
    ...
    // [INTENT] Phase 4: Generate PrintObjects from ModelObjects and their instances.
    //   For each ModelObject, compute the set of distinct trafos
    //   (print_objects_from_model_object) and match them against existing
    //   PrintObjects from the Phase 2 snapshot.  Reuse matching ones (same trafo),
    //   create new ones, and delete orphans.
    //
    //   Matching uses exact float comparison via transform3d_equal() — any
    //   floating-point perturbation (e.g. from shrinkage compensation rounding)
    //   forces a new PrintObject to be created and the old one to be deleted.
    //
    // [STATE] m_objects is rebuilt from scratch into print_objects_new; then
    //         swapped.  The old list is scanned for Unknown/Deleted entries and
    //         those are freed.  print_object_status_db is cleared after this phase.
    //
    // [HAZARD] print_object_apply_config lambda caches config from print_object_last
    //          for efficiency; safe only because all PrintObjects for a given
    //          ModelObject share the same object_config_from_model_object result.
    ...
    // 4) Generate PrintObjects from ModelObjects and their instances.
    {
        ...
        for (ModelObject* model_object : m_model.objects) {
            ...
            auto print_object_apply_config       = [this, &print_object_last, model_object, num_extruders](PrintObject* print_object) {
                print_object->config_apply(print_object_last ? print_object_last->config() :
                                                                     PrintObject::object_config_from_model_object(m_default_object_config,
                                                                                                                  *model_object, num_extruders));
            ...
            if (old.empty()) {
                ...
                for (PrintObjectTrafoAndInstances& print_instances : model_object_status.print_instances) {
                    PrintObject* print_object = new PrintObject(this, model_object, print_instances.trafo,
                                                                std::move(print_instances.instances));
            ...
            std::sort(old.begin(), old.end(),
                      [](const PrintObjectStatus* lhs, const PrintObjectStatus* rhs) { return transform3d_lower(lhs->trafo, rhs->trafo); });
    ...
    if (new_changed_keys.size() > 0) {
        ...
        if (full_config_diff.empty()) {
            // BBS: previous empty
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(" %1%: full_config_diff previous empty, need to apply now.") % __LINE__;
    ...
    // [INTENT] Phase 5: Verify or regenerate PrintObjectRegions.
    //   For each group of PrintObjects sharing the same ModelObject:
    //   a) If regions still Valid: call verify_update_print_object_regions() to
    //      check whether config changes require region split/merge.
    //      If regions need splitting → mark PartiallyValid, set print_regions_reshuffled.
    //      If trafo changed in a way that invalidates bboxes → mark Invalid.
    //   b) If Invalid or PartiallyValid: call generate_print_object_regions()
    //      to rebuild all VolumeRegions, PaintedRegions, and FuzzySkinPaintedRegions
    //      from scratch (possibly reusing old VolumeExtents bboxes).
    //
    // [STATE] print_object_regions is ref-counted and shared across all PrintObjects
    //         that have the same ModelObject.  ref_cnt_inc() called per PrintObject
    //         in the inner loop; ref_cnt_dec() in ModelObjectStatus destructor.
    //
    // [COUPLING] generate_print_object_regions() is an O(V * L) function where V =
    //            number of ModelVolumes and L = number of layer ranges.  For objects
    //            with many modifier stacks and layer ranges this is the dominant cost
    //            of Print::apply().
    //
    // [HAZARD] painting_extruders list is computed here from mmu_segmentation_facets
    //          used_states — if a facet type is not present in ANY volume, it is
    //          silently omitted.  This can result in missing PaintedRegions if the
    //          user paints on a new volume and used_states is stale.
    ...
    for (auto it_print_object = m_objects.begin(); it_print_object != m_objects.end();) {
        ...
        for (++it_print_object_end;
             it_print_object_end != m_objects.end() && (*it_print_object)->model_object() == (*it_print_object_end)->model_object();
             ++it_print_object_end)
        ...
        if (const auto& volumes = print_object.model_object()->volumes;
            num_extruders > 1 && std::find_if(volumes.begin(), volumes.end(),
                                              [](const ModelVolume* v) { return !v->mmu_segmentation_facets.empty(); }) != volumes.end()) {
        ...
        if (model_object_status.print_object_regions_status == ModelObjectStatus::PrintObjectRegionsStatus::Valid) {
            ...
            if (print_object_regions &&
                !trafos_differ_in_rotation_by_z_and_mirroring_by_xy_only(print_object_regions->trafo_bboxes,
                ...
                print_regions_reshuffled                        = true;
                       ...
                       verify_update_print_object_regions(print_object.model_object()->volumes, m_default_region_config, num_extruders,
    ...
    if (print_regions_reshuffled) {
        ...
        struct cmp
        {
            ...
            {
                return l->config_hash() == r->config_hash() && l->config() == r->config();
            }
        };
    ...
    if (apply_status != APPLY_STATUS_UNCHANGED)
        m_modified_count++;
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(" %1%: finished,  this %2%, m_modified_count %3%, apply_status %4%, m_support_used %5%") %
                                   __LINE__ % this % m_modified_count % apply_status % m_support_used;
