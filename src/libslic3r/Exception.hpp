...
// [INTENT] Keep one exception root so worker threads and UI surfaces can uniformly catch slicer-domain failures.
...
// [COUPLING] This severity split is coupled to notification/UI policy, not just algorithmic failure type.
...
public:
    ...
    // [STATE] objectId_ binds a generic error message to a specific model object for per-object reporting.
...
public:
    using Exception::Exception;
    // [MEMORY] Errors are copied by value so this exception safely owns its payload across thread boundaries.
