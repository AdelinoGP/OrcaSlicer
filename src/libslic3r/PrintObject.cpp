...
#include "Timer.hpp"
#define PRINT_OBJECT_TIME_LIMIT_SECONDS(limit) \
...
// [INTENT] PrintObject constructor: bakes the centering offset (m_center_offset) so that
// Clipper polygon math uses smaller integer coordinates, reducing overflow risk.
// Computes z-rotation diff between the canonical first instance and the new instances,
// rotates the bounding box center accordingly, then calls set_instances().
// Called only from the main thread; all ModelObject / ModelInstance data is valid at this point.
    ...
    m_tree_support_preview_cache(nullptr)
{
    ...
    double z_diff = Geometry::rotation_diff_z(model_object->instances.front()->get_rotation(),
                                              instances.front().model_instance->get_rotation());
...
// [INTENT] set_instances(): applies m_center_offset to each instance's shift so that all
// subsequent 2D polygon operations work in centered coordinates. If the instance list
// changed (different count or shifted positions), invalidates psSkirtBrim, psGCodeExport,
// and (on count change) psWipeTower on the owning Print. Back-fills i.print_object = this
// for reverse-lookup from PrintInstance → PrintObject.
// [STATE] m_instances is the authoritative list of all physical placements of this object.
// [COUPLING] Invalidates Print-level steps directly via m_print->invalidate_steps().
PrintBase::ApplyStatus PrintObject::set_instances(PrintInstances&& instances)
{
    ...
    bool                   equal        = equal_length && std::equal(instances.begin(), instances.end(), m_instances.begin(),
                                                                     [](const PrintInstance& lhs, const PrintInstance& rhs) {
                                                return lhs.model_instance == rhs.model_instance && lhs.shift == rhs.shift;
                                            });
    if (!equal) {
        status = PrintBase::APPLY_STATUS_CHANGED;
        if (m_print->invalidate_steps({psSkirtBrim, psGCodeExport}) || (!equal_length && m_print->invalidate_step(psWipeTower)))
...
// [INTENT] all_regions(): convenience accessor returning reference_wrappers to every
// PrintRegion owned by m_shared_regions. Returns empty vector if shared_regions is null
// (object not yet sliced / configured). Used by callers that need to iterate regions
// without mutating them (const correctness via reference_wrapper<const PrintRegion>).
...
// [INTENT] create_polyholes() (free function): generates a regular polygon approximation
// of a circular hole at `center` with `radius`, sized so that a circle of the original
// radius just fits inside (compensates for Clipper integer-coordinate approximation of
// curves). The polygon has n = max(3, round(4*r/nozzle_diam)) edges.
// When `multiple` is true, produces 5 rotated variants (one per nb_polyhole) so adjacent
// layers use slightly different polygon rotations — this reduces the visible seam line from
// the polygon-vs-circle mismatch. Returns the list of Polygon variants.
// [COUPLING] Radius/nozzle_diameter must both be in scaled Clipper units (coord_t).
...
// [INTENT] _transform_hole_to_polyholes(): post-slice correction that replaces circular
// holes (detected by near-convexity and near-equal radii from centroid) with regular
// polygon approximations tuned to nozzle diameter. The polygons are sized to ensure the
// resulting hole is no smaller than intended (outer-fit polygon, not inner-fit).
// Algorithm:
//   Phase 1 (parallel): scan all LayerRegion surfaces with hole_to_polyhole==true;
//     for each convex hole with >8 points and near-uniform radius, record
//     (center, radius, extruder_id, max_variation, twist) keyed by layer index.
//   Phase 2 (serial): group holes with same key across consecutive layers that form a
//     continuous vertical column (≥ 2 layers, or 1 layer at layer 0 for first-layer
//     compensation). Only columns are replaced to avoid one-off noise.
//   Phase 3 (serial): call create_polyholes() per column and write back to both
//     LayerRegion surfaces and the layer lslices expolygons.
// [HAZARD] H1087: Phase 3 writes polygon points back to raw pointers captured during
//   Phase 1. If Phase 2 logic incorrectly matches two distinct holes to the same group,
//   the wrong polygon could be overwritten. The tolerance check (max_variation) mitigates
//   this but cannot fully prevent it on irregular meshes.
// [CONCURRENCY] Phase 1 is parallel (tbb). Phases 2–3 are serial (single-threaded).
//   layerid2center is a vector-of-vectors written in Phase 1 but only by per-layer index,
//   so no data race despite no explicit lock.
void PrintObject::_transform_hole_to_polyholes()
{
    ...
    for (size_t i = 0; i < this->m_layers.size(); i++)
        layerid2center.emplace_back();
    tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()), [this, &layerid2center](const tbb::blocked_range<size_t>& range) {
                    ...
                    for (Surface& surf : layer->m_regions[region_idx]->slices.surfaces) {
                        for (Polygon& hole : surf.expolygon.holes) {
                            ...
                            if (hole.convex_points(PI).empty() && hole.points.size() > 8) {
                                ...
                                coord_t max_variation = std::max(SCALED_EPSILON,
                                                                 scale_(this->m_layers[layer_idx]
                                                                            ->m_regions[region_idx]
                                                                            ->region()
                                                                            .config()
                                                                            .hole_to_polyhole_threshold.get_abs_value(
                                                                                unscaled(diameter_sum / hole.points.size()))));
                                bool    twist =
                                    this->m_layers[layer_idx]->m_regions[region_idx]->region().config().hole_to_polyhole_twisted.value;
                                if (diameter_max - diameter_min < max_variation * 2 &&
                                    diameter_line_max - diameter_line_min < max_variation * 2) {
                                    layerid2center[layer_idx]
                                        .emplace_back(std::tuple<Point, float, int, coord_t,
                                                                 bool>{center, diameter_max,
                                                                       layer->m_regions[region_idx]->region().config().wall_filament.value,
                                                                       max_variation, twist},
                                                      &hole);
                                }
                            }
    ...
    for (size_t layer_idx = 0; layer_idx < this->m_layers.size(); ++layer_idx) {
        for (size_t hole_idx = 0; hole_idx < layerid2center[layer_idx].size(); ++hole_idx) {
            ...
            for (size_t search_layer_idx = layer_idx + 1; search_layer_idx < this->m_layers.size(); ++search_layer_idx) {
                if (layers()[search_layer_idx]->print_z - layers()[search_layer_idx]->height - max_z > EPSILON)
                    break;
...
// [INTENT] detect_extruder_geometric_unprintables(): determines which extruder indices are
// geometrically unprintable for this object, returning one std::set<int> per extruder.
// Two causes are considered:
//   1. Printable height limit (extruder_printable_height config): any layer above the limit
//      whose fill or perimeter uses a given extruder marks that extruder unprintable for
//      the extruder whose height it exceeds.
//   2. Printable area constraint: unprintable polygons from m_print->get_extruder_unprintable_polygons()
//      are transformed to object coordinates and intersected per-layer against fill and
//      perimeter expolygons; matches populate tbb_geometric_unprintables (concurrent set).
// Returns empty sets (one per extruder) for single-extruder prints (early-out at top).
// [CONCURRENCY] Phase 2 uses tbb::parallel_for with tbb::concurrent_unordered_set per
// extruder to avoid locks. Results are merged into std::set<int> after the loop.
// [HAZARD] H1088: unprintable_area_in_obj_coord is computed by subtracting only the
// first instance's shift (m_instances.front()). On a multi-instance object, areas for
// instances beyond the first may not be accurately tested.
std::vector<std::set<int>> PrintObject::detect_extruder_geometric_unprintables() const
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, m_layers.size()), [this, &tbb_geometric_unprintables, &unprintable_area_in_obj_coord,
                                                                    &unprintable_area_bbox](const tbb::blocked_range<int>& range) {
        for (int j = range.begin(); j < range.end(); ++j) {
            ...
            for (auto layerm : layer->regions()) {
                ...
                for (size_t idx = 0; idx < unprintable_area_in_obj_coord.size(); ++idx) {
                    bool do_infill_filament_detect = (solid_infill_filament > 0 &&
                                                      tbb_geometric_unprintables[idx].count(solid_infill_filament - 1) == 0) ||
                                                     (sparse_infill_filament > 0 &&
                                                      tbb_geometric_unprintables[idx].count(sparse_infill_filament - 1) == 0);
...
void PrintObject::make_perimeters()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        ...
        // if (! region.config().extra_perimeters || region.config().wall_loops == 0 || region.config().sparse_infill_density == 0 ||
        // this->layer_count() < 2)
        continue;

        BOOST_LOG_TRIVIAL(debug) << "Generating extra perimeters for region " << region_id << " in parallel - start";
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size() - 1), [this, &region,
                                                                               region_id](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                m_print->throw_if_canceled();
                ...
                for (Surface& slice : layerm.slices.surfaces) {
                    for (;;) {
                        ...
                        const Polygons critical_area       = diff(offset(slice.expolygon, float(-perimeters_thickness)),
...
// [INTENT] prepare_infill(): drives the posPrepareInfill step — the full surface
// classification and fill-region preparation pipeline. Order is significant:
//   1. restore_untyped_slices_no_extra_perimeters() — resets typed surfaces for robust
//      re-running (see GH #7442), preserving extra_perimeters set by make_perimeters().
//   2. detect_surfaces_type()        — classifies stTop/stBottom/stBottomBridge/stInternal
//   3. prepare_fill_surfaces()       — demotes stTop/stBottom when top/bottom layers=0;
//                                      promotes tiny stInternal to stInternalSolid
//   4. discover_vertical_shells()    — ensures N-layer solid shell perpendicular to walls
//   5. discover_horizontal_shells()  — scatters solid surfaces N layers up/down
//      NOTE: Orca moved this BEFORE process_external_surfaces() (vs. upstream order)
//      to allow bridge expansion over more than one perimeter (e.g. Benchy lettering).
//   6. process_external_surfaces()   — grows top/bottom by EXTERNAL_INFILL_MARGIN
//   7. clip_fill_surfaces()          — trims sparse infill if infill_only_where_needed
//   8. bridge_over_infill()          — converts stInternalSolid above sparse to bridging
//   9. combine_infill()              — merges stInternal across N layers for thick infill
// [STATE] m_typed_slices is true after this step completes (set inside detect_surfaces_type).
// [COUPLING] Calls m_print->set_status(25, ...) and m_print->throw_if_canceled() frequently.
void PrintObject::prepare_infill()
{
    ...
    if (m_typed_slices) {
        ...
        // discover_vertical_shells() with more than a single region. If this step does not use Surface::extra_perimeters or
        // Surface::extra_perimeters is always zero, it is safe to reset to the untyped slices before re-runnning detect_surfaces_type().
        for (Layer* layer : m_layers) {
    ...
    //
    // 1) stBottomBridge / stBottom infill is grown by 3mm and clipped by the total infill area. Bridges are detected. The areas may
...
// [INTENT] infill(): drives the posInfill step. Calls prepare_infill() first as
// prerequisite (idempotent if already done). Then tbb::parallel_for over all layers,
// calling Layer::make_fills() per layer passing the adaptive octree pointers and the
// lightning generator. Reports progress at 35%.
// [STATE] m_adaptive_fill_octrees and m_lightning_generator are populated by
//   prepare_adaptive_infill_data() and prepare_lightning_infill_data() respectively,
//   called earlier in Print::process() before infill().
// [CONCURRENCY] make_fills() is called in parallel; each layer is independent.
void PrintObject::infill()
{
    ...
    if (this->set_started(posInfill)) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()), [this, &adaptive_fill_octree = adaptive_fill_octree,
                                                                           &support_fill_octree = support_fill_octree](
...
// [INTENT] ironing(): drives the posIroning step. Iterates all layers (starting at 0
// to support ironing on all surfaces including bottom) via tbb::parallel_for calling
// Layer::make_ironing(). Ironing adds a slow top-surface pass to improve finish quality.
// No prerequisite call — posIroning is listed as a dependency of posInfill in the DAG,
// so by the time ironing() is called, infill is guaranteed complete.
void PrintObject::ironing()
{
    if (this->set_started(posIroning)) {
        ...
        tbb::parallel_for(
            ...
            tbb::blocked_range<size_t>(0, m_layers.size()), [this](const tbb::blocked_range<size_t>& range) {
                for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                    m_print->throw_if_canceled();
...
// BBS
// [INTENT] clear_overhangs_for_lift(): resets loverhangs on every layer to free memory
// or prepare for recomputation. Only executes if this is not a shared object (m_shared_object
// is nullptr for the primary copy; shared copies share layers and must not mutate them).
...
// [INTENT] detect_overhangs_for_lift(): BBS Orca feature — drives posDetectOverhangsForLift.
// Computes per-layer overhang polygons (loverhangs) for automatic Z-hop lift decisions
// during G-code generation. Uses diff_ex(layer.lslices, offset_ex(lower_layer.lslices,
// min_overlap)) to find regions not covered by the layer below. A morphological open/close
// (offset2_ex) removes thin noise. Stores result in Layer::loverhangs and
// Layer::loverhangs_bbox per layer.
// Skips raft layers (starts from num_raft_layers + 1).
// [CONCURRENCY] tbb::parallel_for over layers. tbb::spin_mutex is declared but not used
// in the lambda — likely a leftover from an older version.
// [COUPLING] Reads nozzle_diameter from m_print->config() and line_width from this->config().
void PrintObject::detect_overhangs_for_lift()
{
    if (this->set_started(posDetectOverhangsForLift)) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(num_raft_layers + 1, num_layers), [this, min_overlap, line_width](
                                                                                           const tbb::blocked_range<size_t>& range) {
...
// [INTENT] generate_support_material(): drives the posSupportMaterial step.
// If support is not configured, runs is_support_necessary() (via TreeSupport) and
// emits a NON_CRITICAL warning if significant overhangs are found (sharp tails,
// cantilevers, or large overhangs). No exception is thrown — it's advisory only.
// If support or raft is enabled AND there are at least 2 layers (or raft with any layer),
// calls _generate_support_material() which dispatches to TreeSupport or
// PrintObjectSupportMaterial depending on config.
// [STATE] clear_support_layers() is always called first to reset SupportLayer* ptrs.
// [COUPLING] Emits warning via active_step_add_warning() which routes to the Print's
// status reporting system. is_support_necessary() instantiates TreeSupport internally —
// expensive operation even when support is off.
...
// [INTENT] estimate_curled_extrusions(): drives posEstimateCurledExtrusions.
// Only executes if at least one print region has enable_overhang_speed enabled.
// Calls SupportSpotsGenerator::estimate_malformations() which annotates each layer
...
// [INTENT] simplify_extrusion_path(): drives three separate simplification steps:
//   posSimplifyPath         — walls: calls Layer::simplify_wall_extrusion_path() per layer
//   posSimplifyInfill       — infill: calls Layer::simplify_infill_extrusion_path() per layer
//   posSimplifySupportPath  — support: calls SupportLayer::simplify_support_extrusion_path()
// Each step is independent (separate set_started/set_done guards) and uses tbb::parallel_for.
// All three report progress at 75%. The separation allows individual steps to be
// invalidated and re-run without repeating the others.
// [CONCURRENCY] All three phases are parallel per-layer; layers are independent.
void PrintObject::simplify_extrusion_path()
{
    ...
    if (this->set_started(posSimplifySupportPath)) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_support_layers.size()), [this](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                m_print->throw_if_canceled();
...
// [INTENT] prepare_adaptive_infill_data(): builds the FillAdaptive octrees used by
// adaptive cubic and support-cubic infill patterns. Two octrees are built — one for
// adaptive infill, one for support infill — each at their respective line spacings.
// Process:
//   1. Query adaptive_fill_line_spacing() to get spacings; early-out if both are 0.
//   2. Extract the raw indexed triangle set from the model object and apply
//      trafo_centered() + to_octree() rotation so the mesh is in octree-aligned coords.
//   3. tbb::parallel_for: triangulate bridging surfaces (surfaces_w_bottom_z) into
//      Vec3d point clouds — these act as "overhang hints" for octree density.
//   4. Gather all bridge triangulations and build both octrees via build_octree().
// [CONCURRENCY] Step 3 is parallel. Steps 1–2 and 4 are single-threaded.
// [MEMORY] Returns two OctreePtrs (unique_ptr wrappers). The caller owns them and stores
//   them in PrintObject::m_adaptive_fill_octrees.
    ...
    const std::vector<std::pair<const Surface*, float>>& surfaces_w_bottom_z) const
{
    ...
    return std::make_pair(adaptive_line_spacing ? build_octree(mesh, overhangs.front(), adaptive_line_spacing, false) : OctreePtr(),
                          support_line_spacing ? build_octree(mesh, overhangs.front(), support_line_spacing, true) : OctreePtr());
}

// [INTENT] prepare_lightning_infill_data(): builds the FillLightning::Generator for this
// object if any printing region uses lightning infill (ipLightning) with density > 0.
// Returns a null GeneratorPtr if lightning infill is not needed (most objects).
// The generator is built via FillLightning::build_generator() which requires a
// cancellation callback (throw_if_canceled lambda).
// [MEMORY] Returns a GeneratorPtr (unique_ptr). Stored in PrintObject::m_lightning_generator.
FillLightning::GeneratorPtr PrintObject::prepare_lightning_infill_data()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id)
        if (const PrintRegionConfig& config = this->printing_region(region_id).config();
            config.sparse_infill_density > 0 && config.sparse_infill_pattern == ipLightning) {
...
// [INTENT] clear_layers(): destructs all Layer* in m_layers via raw-pointer delete,
// then clears the vector. Guards against execution on shared objects (m_shared_object != null)
...
// [INTENT] add_layer(): heap-allocates a new Layer and appends it to m_layers.
// Called during the slicing phase to build up the layer stack in ascending Z order.
// Returns the raw pointer to the newly created layer (owned by m_layers).
// [MEMORY] Raw pointer ownership — caller must not delete; clear_layers() owns deletion.
...
// [INTENT] get_support_layer_at_printz() (const + mutable overloads): binary search on
// m_support_layers (sorted ascending by print_z) to find the support layer whose print_z
// is within ±epsilon of the queried z. Returns nullptr if no match found.
// Uses lower_bound_by_predicate for O(log n) search. The mutable overload delegates to
// the const version via const_cast — standard const/non-const accessor pattern.
const SupportLayer* PrintObject::get_support_layer_at_printz(coordf_t print_z, coordf_t epsilon) const
{
    coordf_t limit = print_z - epsilon;
    auto     it    = Slic3r::lower_bound_by_predicate(m_support_layers.begin(), m_support_layers.end(),
                                                      [limit](const SupportLayer* layer) { return layer->print_z < limit; });
...
// [INTENT] clear_support_layers(): destructs all SupportLayer* in m_support_layers and
// clears the vector. Also resets sharp_tails, sharp_tails_height, and cantilevers on every
// Layer (these are populated by TreeSupport analysis and must be cleared with support data).
// Guards against execution on shared objects — same pattern as clear_layers().
// [MEMORY] Raw-pointer ownership; same caveats as clear_layers().
...
// [INTENT] alloc_tree_support_preview_cache(): lazy-initializes m_tree_support_preview_cache
// (a shared_ptr<TreeSupportData>) used for tree-support preview rendering. The TreeSupportData
// is constructed with the xy_distance from config and a fixed collision resolution constant.
// Subsequent calls return the cached shared_ptr without reconstructing.
// [STATE] m_tree_support_preview_cache persists across calls; only reset when the object
// is re-sliced (clear_support_layers clears it indirectly via destructor on re-creation).
...
// [INTENT] add_tree_support_layer(): heap-allocates a SupportLayer for tree support
// (stInnerTree type) and appends to m_support_layers. Used by TreeSupport generator.
// [INTENT] add_support_layer(): heap-allocates a SupportLayer for normal support and
// appends to m_support_layers. interface_id identifies the interface layer index.
// [INTENT] insert_support_layer(): inserts a SupportLayer at an arbitrary position in
// m_support_layers (used when layers must be inserted in sorted Z order mid-generation).
// [MEMORY] All three: raw pointer, owned by m_support_layers; cleared by clear_support_layers().
...
SupportLayerPtrs::iterator PrintObject::insert_support_layer(
    SupportLayerPtrs::iterator pos, size_t id, size_t interface_id, coordf_t height, coordf_t print_z, coordf_t slice_z)
{
...
// [INTENT] invalidate_state_by_config_options(): maps changed config option keys to the
// minimal set of PrintObjectStep invalidations required. This is the "apply" half of the
// step-dependency system — called when the user changes settings in the UI.
// Each option key is categorized into a PrintObjectStep bucket via a long if/else chain.
// After collecting affected steps, calls this->invalidate_steps(steps) which in turn
// calls invalidate_step() per step to cascade the DAG. Returns true if any step was
// actually invalidated (used by Print::apply to detect whether G-code re-export is needed).
// [COUPLING] Some option changes (e.g. brim_type switch to manual) additionally trigger
// posPerimeters invalidation for the first layer wall ordering feature. These special cases
// read old_config / new_config to compare before/after values.
// [HAZARD] H1089: this is a large hand-maintained mapping (~350 lines of if/else). If a
// new config option is added to PrintObjectConfig or PrintRegionConfig without also being
// listed here, the affected step will not be invalidated on change — silent stale output.
bool PrintObject::invalidate_state_by_config_options(const ConfigOptionResolver&             old_config,
                                                     ...
                                                     const std::vector<t_config_option_key>& opt_keys)
{
    ...
    for (const t_config_option_key& opt_key : opt_keys) {
        if (opt_key == "brim_width" || opt_key == "brim_object_gap" || opt_key == "brim_use_efc_outline" || opt_key == "brim_type" ||
            opt_key == "brim_ears_max_angle" ||
            opt_key == "brim_ears_detection_length"
            // BBS: brim generation depends on printing speed
            || opt_key == "outer_wall_speed" || opt_key == "small_perimeter_speed" || opt_key == "small_perimeter_threshold" ||
        ...
        } else if (opt_key == "wall_loops" || opt_key == "alternate_extra_wall" || opt_key == "top_one_wall_type" ||
                   opt_key == "min_width_top_surface" || opt_key == "only_one_wall_first_layer" ||
                   opt_key == "extra_perimeters_on_overhangs" || opt_key == "detect_overhang_wall" ||
                   opt_key == "initial_layer_line_width" || opt_key == "inner_wall_line_width" || opt_key == "infill_wall_overlap" ||
                   opt_key == "top_bottom_infill_wall_overlap" || opt_key == "seam_gap" || opt_key == "role_based_wipe_speed" ||
            ...
            if (this->is_mm_painted() &&
                (opt_key == "filter_out_gap_fill" && (opt_key == "gap_infill_speed" && is_gap_fill_changed_state_due_to_speed())))
                steps.emplace_back(posSlice);
            steps.emplace_back(posPerimeters);
            ...
            if (m_config.support_top_z_distance == 0.) {
                ...
                steps.emplace_back(posSlice);
                   ...
                   opt_key == "support_expansion" || opt_key == "independent_support_layer_height" // Orca
                   ...
                   opt_key == "tree_support_auto_brim" || opt_key == "tree_support_brim_width" || opt_key == "tree_support_top_rate" ||
...
// [INTENT] invalidate_all_steps(): resets all PrintObjectStep states AND all Print-level
// steps, and marks m_slicing_params.valid = false so SlicingParameters are recomputed.
// Used when a fundamental change (e.g. new model loaded) makes all cached results stale.
...
// stBottom       - Part of a region, which is not supported by the same region, but it is supported either by another region, or by a
// soluble interface layer. stInternal     - Part of a region, which is supported by the same region type. If a part of a region is of
// stBottom and stTop, the stBottom wins.
//
// [INTENT] detect_surfaces_type(): core surface classification pass. Runs per-region, parallel
// per-layer. For each layer:
//   - top:    diff of this layer's slices vs upper layer lslices (or vs upper region if interface_shells)
//   - bottom: diff of this layer's slices vs lower layer lslices; stBottom if soluble support, else stBottomBridge
//   - internal: the remainder not in top or bottom
//   - Crack handling: thin membranes where top and bottom overlap get special treatment to
//     avoid incorrectly classifying small features as bottom bridges.
// After the per-region classification, Orca's second-bridge pass (eblApplyToAll /
// eblExternalBridgeOnly) scans layers for stBottomBridge and marks the layer above as
// stInternalAfterExternalBridge, then immediately reclassifies to stBottomBridge (interim
// workaround — see TODO in code). Then slices_to_fill_surfaces_clipped() is called to
// transfer classification to fill_surfaces.
// [STATE] Sets m_typed_slices = true on completion.
// [CONCURRENCY] Main classification loop is tbb::parallel_for per layer per region.
//   The second-bridge pass is also parallel. The fallback interface_shells path uses a
//   surfaces_new buffer to avoid write conflicts between layers.
// [HAZARD] H1090: the Orca second-bridge layer uses stInternalAfterExternalBridge as an
//   interim type then immediately converts it back to stBottomBridge. Any future code path
//   that observes stInternalAfterExternalBridge will never see it in practice, but if the
//   reclassification step were removed, it would propagate unchecked through the pipeline.
void PrintObject::detect_surfaces_type()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        ...
        tbb::parallel_for(
            ...
            [this, region_id, interface_shells, &surfaces_new](const tbb::blocked_range<size_t>& range) {
                ...
                // 3. for tree(auto), interface top layers=0, max bridge length=0, support_critical_regions_only=false (only in this way the
                // bridge is fully supported)
                bool bottom_is_fully_supported = this->has_support() && m_config.support_top_z_distance.value == 0 &&
                                                 is_auto(m_config.support_type.value);
                if (m_config.support_type.value == stNormalAuto)
                    ...
                    bottom_is_fully_supported &= (m_config.support_interface_top_layers.value > 0 &&
                ...
                for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
                    ...
                    // no_perimeter_full_bridge allow to put bridges where there are nothing, hence adding area to slice, that's why we need
                    // to start from the result of PerimeterGenerator.
                    ...
                    if (lower_layer) {
                        ...
                        surfaces_append(bottom,
                                        opening_ex(diff_ex(layerm_slices_surfaces, lower_layer->lslices, ApplySafetyOffset::Yes), offset),
                                        surface_type_bottom_other);
                        ...
                        if (interface_shells) {
                            ...
                            surfaces_append(bottom,
                                            opening_ex(diff_ex(intersection(layerm_slices_surfaces, lower_layer->lslices), // supported
                                                               lower_layer->m_regions[region_id]->slices.surfaces, ApplySafetyOffset::Yes),
                                                       offset),
                                            stBottom);
                        }
#endif
                            ...
                            if (lower_layer) { // Only detect small cracks for non-first layer, because first layer should always be bottom
                                ...
                                for (const auto& crack : cracks) {
                                    if (offset_ex(crack, small_crack_threshold).empty()) {
                                        ...
                                        Surfaces bot_tmp;
                                        for (auto& b : bottom) {
                                            surfaces_append(bot_tmp, diff_ex(b.expolygon, offset_ex(crack, -small_crack_threshold)),
                                                            b.surface_type);
                                        }
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        ...
                        expolygons_with_attributes.emplace_back(
                            std::make_pair(to_expolygons(layerm->slices.surfaces), SVG::ExPolygonAttributes("black")));
                        SVG::export_expolygons(debug_out_path("1_detect_surfaces_type_%d_region%d-layer_%f.svg", iRun++, region_id,
                                                              layer->print_z)
                                                   .c_str(),
                                               expolygons_with_attributes);
                    }
#endif /* SLIC3R_DEBUG_SLICE_PROCESSING */
        ...
        if ((this->config().enable_extra_bridge_layer.value == eblApplyToAll) ||
            (this->config().enable_extra_bridge_layer.value == eblExternalBridgeOnly)) {
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, last), [this, region_id](const tbb::blocked_range<size_t>& range) {
                for (size_t i = range.begin(); i < range.end(); ++i) {
                    ...
                    int          number_of_internal_walls =
                        std::max(0, layerm->m_region->config().wall_loops -
                                        1); // number of internal walls, clamped to a minimum of 0 as a safety precaution
                    float offset_distance = layerm->flow(frExternalPerimeter)
                                                .scaled_width() // shrink down by external perimeter width (effectively filtering out 2x
                                                                // external perimeters wide bridges)
                                            + ((layerm->flow(frPerimeter).scaled_width()) *
                                               number_of_internal_walls); // shrink down by number of external walls * width of them,
                                                                          // effectively filtering out 2x internal perimeter wide bridges
                    // The reason for doing the above filtering is that in pure bridges, the walls are always printed separately as overhang
                    // walls. Here we care about the bridge infill which is distinct and is the remainder of the bridge area minus the
                    // perimeter width on both sides of the bridge itself. This would also skip generation of very short dual bridge layers
                    // (that are shorter than N perimeters), but these are unecessary as the bridge distance is We could reduce this
                    // slightly to account for innacurcies in the clipping operation.
                    // TODO: Monitor GitHub issues to check whether second bridge layers are ommited where they should be generated. If yes,
                    // reduce the filtering distance
    ...
    // Mark the object to have the region slices classified (typed, which also means they are split based on whether they are supported,
    // bridging, top layers etc.)
    m_typed_slices = true;
}

// [INTENT] process_external_surfaces(): expands top/bottom/bridge surfaces outward by
// EXTERNAL_INFILL_MARGIN so that solid infill regions anchor into adjacent perimeters.
// When any region has zero sparse-infill density (sparse = void), also precomputes
// surfaces_covered[] — the lslices of each layer minus any void (zero-density) regions
// shrunk by 0.3 * EXTERNAL_INFILL_MARGIN — to prevent expansion into unsupported voids.
// Then calls LayerRegion::process_external_surfaces() per layer in tbb::parallel_for,
// passing the appropriate surfaces_covered entry.
// [CONCURRENCY] surfaces_covered precomputation is parallel (tbb). Main processing is
// parallel per layer. Each layer region is independent.
// [STATE] Modifies LayerRegion::fill_surfaces in place (expands top/bottom regions).
void PrintObject::process_external_surfaces()
{
    ...
    // Is there any printing region, that has zero infill? If so, then we don't want the expansion to be performed over the complete voids,
    // but only over voids, which are supported by the layer below.
    ...
    if (has_voids && m_layers.size() > 1) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size() - 1), [this, &surfaces_covered, &layer_expansions_and_voids,
                                                                               unsupported_width](const tbb::blocked_range<size_t>& range) {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()),
                          [this, &surfaces_covered, region_id](const tbb::blocked_range<size_t>& range) {
                              for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                                  ...
                                  m_layers[layer_idx]
                                      ->get_region(int(region_id))
                                      ->process_external_surfaces(
                                          // lower layer
                                          (layer_idx == 0) ? nullptr : m_layers[layer_idx - 1],
                                          // lower layer polygons with density > 0%
                                          (layer_idx == 0 || surfaces_covered.empty() || surfaces_covered[layer_idx - 1].empty()) ?
                                              nullptr :
                                              &surfaces_covered[layer_idx - 1]);
                              }
                          });
        m_print->throw_if_canceled();
...
// [INTENT] discover_vertical_shells(): ensures the configured number of solid perimeter
// layers (top_shell_layers / bottom_shell_layers) are honored vertically by converting
// stInternal fill surfaces to stInternalSolid where needed. Works per-region:
//   For each layer with stTop surfaces: project those surfaces N layers downward and union
//     with any existing stInternalSolid — fill them as solid.
//   For each layer with stBottomBridge/stBottom: project N layers upward — same logic.
// The projection approach ("closing" the shell gap) ensures angled surfaces get a full
// solid wall thickness even when individual layers are thin.
// [STATE] Modifies LayerRegion::fill_surfaces in place: converts stInternal → stInternalSolid.
// [CONCURRENCY] Inner per-layer loop is serial within per-region processing. No tbb here
//   because sequential cross-layer reads and writes are required.
// [HAZARD] H1091: This function calls offset2() (shrink+expand) to clean up thin slivers
//   from the solid-region expansion. Extremely thin walls (<= offset distance) can disappear
//   entirely and leave an unintended void in the shell — visible as a gap in solid regions.
void PrintObject::discover_vertical_shells()
{
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers, grain_size), [this, &cache_top_botom_regions](
            ...
            for (size_t idx_layer = range.begin(); idx_layer < range.end(); ++idx_layer) {
                ...
                for (size_t region_id = 0; region_id < num_regions; ++region_id) {
                    ...
                    //                        append(cache.bottom_surfaces, offset(layerm.fill_surfaces.filter_by_types(surfaces_bottom),
                    //                        top_bottom_expansion));
                    ...
                    if (perimeters > 0) {
                        ...
                        perimeter_offset      = std::max(perimeter_offset, 0.5f * float(extflow.scaled_width() + extflow.scaled_spacing()) +
                                                                               (float(perimeters) - 1.f) * flow.scaled_spacing());
                        perimeter_min_spacing = std::min(perimeter_min_spacing,
                                                         float(std::min(extflow.scaled_spacing(), flow.scaled_spacing())));
                    }
                ...
                if (perimeter_offset > 0.) {
                    ...
                    polygons_append(cache.holes,
                                    offset2(layer.lslices, 0.3f * perimeter_min_spacing, -perimeter_offset - 0.3f * perimeter_min_spacing));
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                    {
                        Slic3r::SVG svg(debug_out_path("discover_vertical_shells-extra-holes-%d.svg", debug_idx),
                                        get_extents(layer.lslices));
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        ...
        if (!top_bottom_surfaces_all_regions) {
            // This is either a single material print, or a multi-material print and interface_shells are enabled, meaning that the vertical
...
// #ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                ...
                if (int n_bottom_layers = region_config.bottom_shell_layers.value; n_bottom_layers > 0) {
                    ...
                    for (; i >= 0 && (i > ibottom || bottom_z - m_layers[i]->bottom_z() < region_config.bottom_shell_thickness - EPSILON);
                         --i) {
                    ...
                    if (one_more_layer_below_top_bottom_surfaces)
                        if (i >= 0 && (i > ibottom || bottom_z - m_layers[i]->print_z < region_config.bottom_shell_thickness - EPSILON))
                            combine_holes(cache_top_botom_regions[i].holes);
                }
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                {
                    Slic3r::SVG svg(debug_out_path("discover_vertical_shells-perimeters-before-union-%d.svg", debug_idx),
                                    get_extents(shell));
                    svg.draw(shell);
...
//                    shell = union_(shell, true);
                ...
                const Polygons polygonsInternal = to_polygons(
                ...
                ExPolygons regularized_shell;
                {
                    ...
                    regularized_shell               = shrink_ex(
                        offset2_ex(union_ex(shell),
                                                 ...
                                                 // Then close gaps narrower than 1.2 * line width, such gaps are difficult to fill in with sparse infill.
                                                 narrow_ensure_vertical_wall_thickness_region_radius + narrow_sparse_infill_region_radius,
                                                 ClipperLib::jtSquare),
                        // Finally expand the infill a bit to remove tiny gaps between solid infill and the other regions.
                    ...
                    Polygons internal_volume;
                    {
                        ...
                        Polygons shrinked_upper_slice  = (idx_layer + 1) < m_layers.size() ? to_polygons(m_layers[idx_layer + 1]->lslices) :
                                                                                             Polygons{};
                    ...
                    regularized_shell
                        .erase(std::remove_if(regularized_shell.begin(), regularized_shell.end(),
                                              [&internal_volume, &min_perimeter_infill_spacing, &object_volume](const ExPolygon& p) {
                                                  return (p.area() < min_perimeter_infill_spacing * scaled(1.5) ||
                                                         ...
                                                         diff(internal_volume, expand(to_polygons(p), min_perimeter_infill_spacing)).size() >=
...
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                {
                    SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal-%d.svg", debug_idx), get_extents(shell),
                                           new_internal, "black", "blue", scale_(0.05));
                    SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal_void-%d.svg", debug_idx),
                                           get_extents(shell), new_internal_void, "black", "blue", scale_(0.05));
                    SVG::export_expolygons(debug_out_path("discover_vertical_shells-new_internal_solid-%d.svg", debug_idx),
                                           get_extents(shell), new_internal_solid, "black", "blue", scale_(0.05));
                }
#endif /* SLIC3R_DEBUG_SLICE_PROCESSING */
...
// [INTENT] bridge_over_infill(): identifies stInternalSolid regions that sit directly
// above stInternal (sparse infill) regions, converts them to stInternalBridge type with
// a computed bridge angle, and replaces the surfaces in fill_surfaces.
// Algorithm (high level):
//   1. For each layer, collect stInternalSolid candidate surfaces that are above sparse infill.
//   2. Compute bridge angle for each candidate by analyzing AABB-anchored span directions.
//   3. Gather CandidateSurface records, then apply them: split original stInternalSolid into
//      bridge portion (stInternalBridge) and non-bridge remainder (stays stInternalSolid).
//   4. Lightning infill special case: temporarily backs up and restores fill_surfaces to
//      avoid interaction between bridge detection and the lightning generator's layer data.
//   5. Orca's second internal bridge pass (eblApplyToAll / eblSecondBridgeOnly): after the
//      first bridge is placed, scans for stInternalBridge in layer i and marks the layer
//      above as stSecondInternalBridge, then immediately reclassifies to stInternalBridge
//      (same interim-workaround pattern as detect_surfaces_type()).
// [HAZARD] H1092: lightning infill backup/restore uses std::move to take ownership of
//   fill_surfaces, then moves back. If an exception is thrown between the move-out and
//   move-in, fill_surfaces is left empty — data loss for the lightning layers.
// [HAZARD] H1093: CandidateSurface holds a raw const Surface* pointer into fill_surfaces.
//   If fill_surfaces is reallocated between candidate collection and application (e.g. by
//   a concurrent writer), this pointer is dangling. In practice, no concurrent writer
//   exists here — but the raw pointer is fragile.
// [CONCURRENCY] Main scan loop is tbb::parallel_for (candidate collection). Application
//   of candidates is serial to avoid simultaneous fill_surfaces mutation.
void PrintObject::bridge_over_infill()
{
    ...
    // SECTION to gather and filter surfaces for expanding, and then cluster them by layer
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, this->layers().size()), [po = static_cast<const PrintObject*>(this),
                                                                                 &candidate_surfaces,
                                                                                 has_lightning_infill](tbb::blocked_range<size_t> r) {
            PRINT_OBJECT_TIME_LIMIT_MILLIS(PRINT_OBJECT_TIME_LIMIT_DEFAULT);
            for (size_t lidx = r.begin(); lidx < r.end(); lidx++) {
                ...
                // By expanding the lower layer solids, we avoid making bridges from the tiny internal overhangs that are (very likely)
                // supported by previous layer solids NOTE that we cannot filter out polygons worth bridging by their area, because
                // sometimes there is a very small internal island that will grow into large hole
                lower_layer_solids = shrink(lower_layer_solids, 1 * spacing); // first remove thin regions that will not support anything
                lower_layer_solids = expand(lower_layer_solids,
                                            (1 + expansion_multiplier) *
                                                spacing); // then expand back (opening), and further for parts supported by internal solids
                ...
                for (const LayerRegion* region : layer->regions()) {
                    ...
                    for (const Surface* s : region_internal_solids) {
                        ...
                        if (po->config().dont_filter_internal_bridges.value == ibfNofilter) {
                            ...
                            // The following flag marks those surfaces, which overlap with unuspported area, but at least part of them is
                            // supported. These regions can be filtered by area, because they for sure are touching solids on lower layers,
                            // and it does not make sense to bridge their tiny overhangs
                            ...
                            if (!unsupported.empty() && (!partially_supported || area(unsupported) > 3 * 3 * spacing * spacing)) {
                                ...
                                // after we extracted the part worth briding, we go over the leftovers and merge the tiny ones back, to not
                                // brake the surface too much
                                ...
                                debug_draw(std::to_string(lidx) + "_candidate_processing_" + std::to_string(area(unsupported)),
                                           to_lines(unsupported),
                                           to_lines(intersection(to_polygons(s->expolygon), expand(unsupported, 5 * spacing))),
    ...
    // LIGHTNING INFILL SECTION - If lightning infill is used somewhere, we check the areas that are going to be bridges, and those that
    // rely on the lightning infill under them get expanded. This somewhat helps to ensure that most of the extrusions are anchored to the
    // lightning infill at the ends. It requires modifying this instance of print object in a specific way, so that we do not invalidate the
    // pointers in our surfaces_by_layer structure.
    if (has_lightning_infill) {
        // Prepare backup data for the Layer Region infills. Before modfiyng the layer region, we backup its fill surfaces by moving! them
        // into this map. then a copy is created, modifiyed and passed to lightning infill generator. After generator is created, we restore
        // the original state of the fills again by moving the data from this map back to the layer regions. This ensures that pointers to
        // surfaces stay valid.
    ...
    // cluster layers by depth needed for thick bridges. Each cluster is to be processed by single thread sequentially, so that bridges
    // cannot appear one on another
    ...
    float                            target_flow_height_factor = 0.9f;
    {
        ...
        // prepare inflated filter for each candidate on each layer. layers will be put into single thread cluster if they are close to each
        // other (z-axis-wise) and if the inflated AABB polygons overlap somewhere
        tbb::parallel_for(tbb::blocked_range<size_t>(0, layers_with_candidates.size()),
                          [&layers_with_candidates, &surfaces_by_layer, &layer_area_covered_by_candidates](tbb::blocked_range<size_t> r) {
                              PRINT_OBJECT_TIME_LIMIT_MILLIS(PRINT_OBJECT_TIME_LIMIT_DEFAULT);
                              for (size_t job_idx = r.begin(); job_idx < r.end(); job_idx++) {
    ...
    auto construct_anchored_polygon = [](Polygons bridged_area, Lines anchors, const Flow& bridging_flow, double bridging_angle) {
        ...
        double   aligning_angle = -bridging_angle + PI * 0.5;
        {
            ...
            for (size_t i = 0; i < n_vlines; i++) {
                ...
                std::sort(polygon_sections[i].begin(), polygon_sections[i].end(), [](const Line& a, const Line& b) {
                    if (a == b)
                        return false; // Ensure irreflexivity
                    return a.a.y() < b.b.y();
                });
            }
            ...
            for (size_t job_idx = 0; job_idx < clustered_layers_for_threads[cluster_idx].size(); job_idx++) {
                ...
                for (const CandidateSurface& candidate : surfaces_by_layer[lidx]) {
                    ...
                    if (candidate.region->region().config().internal_bridge_angle > 0)
                        bridging_angle = candidate.region->region().config().internal_bridge_angle.value * PI /
...
#ifdef DEBUG_BRIDGE_OVER_INFILL
                    debug_draw(std::to_string(lidx) + "_" + std::to_string(cluster_idx) + "_" + std::to_string(job_idx) + "_" +
                                   "_expanded_bridging" + std::to_string(r),
    ...
    if (this->m_config.enable_extra_bridge_layer == eblApplyToAll || this->m_config.enable_extra_bridge_layer == eblInternalBridgeOnly) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, this->layers().size() - 1), [this](const tbb::blocked_range<size_t>& r) {
            for (size_t lidx = r.begin(); lidx < r.end(); ++lidx) {
                ...
                // (C) If there is a next layer, identify overlapping stInternal & stInternalSolid areas and convert the overlap to
...
// [INTENT] clamp_exturder_to_default(): sanitises a single extruder-index option
// by resetting out-of-range values to the default extruder (index 1).
// Guards downstream code that assumes extruder indices are within [1, num_extruders].
...
// [INTENT] object_config_from_model_object(): builds a PrintObjectConfig by layering
// the per-object config on top of the print default, then clamping invalid extruder
// indices. Called once during Print::apply() to set the canonical per-object config.
// [COUPLING] Calls clamp_exturder_to_default() for support/interface filaments;
// relies on DynamicPrintConfig::normalize_fdm() to resolve shorthand FDM keys.
...
// [INTENT] apply_to_print_region_config(): merges a DynamicPrintConfig (from a
// ModelVolume, ModelObject, or material) into a PrintRegionConfig by:
//   1. Mapping the generic "extruder" key → sparse_infill_filament, solid_infill_filament,
//      and wall_filament (non-default values only).
//   2. Copying all other matching keys directly.
// Zero-value extruder entries are treated as "default" and skipped.
// [COUPLING] Central config-merge helper called by region_config_from_model_volume()
//   and transitively by slicing_parameters(). Order of calls matters: later calls override earlier.
...
// [INTENT] region_config_from_model_volume(): assembles the final PrintRegionConfig for
// a single ModelVolume by layering (in priority order):
//   1. Parent print-region config (default_or_parent_region_config)
//   2. ModelObject config (for solid volumes only)
//   3. ModelVolume config
//   4. Volume's material config
//   5. Layer-range overrides (for solid volumes only)
// Clamps extruder indices, zeroes infill density below 0.00011 to avoid near-zero
// division, and disables fuzzy skin if geometry thresholds are not met.
// [COUPLING] Calls apply_to_print_region_config() repeatedly; final config feeds
//   PrintRegion creation in Print::apply() and region hash comparison.
                                                  ...
                                                  size_t                    num_extruders)
{
    ...
    if (config.fuzzy_skin.value != FuzzySkinType::None &&
...
// [INTENT] update_slicing_parameters(): lazily initialises m_slicing_params by calling
// SlicingParameters::create_from_config() if the cached params are marked invalid.
// Orca adds an XYZ shrinkage_compensation vector to the slicing parameters computation.
// [STATE] m_slicing_params.valid acts as a dirty flag; once set true here, the params
//   are reused until invalidate_state_by_config_options() clears them again.
...
// [INTENT] slicing_parameters() (static): constructs a one-shot SlicingParameters
// from a raw DynamicPrintConfig (not from a live PrintObject). Used by the layer-height
// profile editor and the UI preview path where no PrintObject has been constructed yet.
// Manually reconstructs the per-object and per-region configs using
// object_config_from_model_object() / region_config_from_model_volume() to gather
// extruder lists, matching the logic in Print::apply().
// Orca: adds object_shrinkage_compensation parameter forwarded to create_from_config().
// [COUPLING] Must stay in sync with Print::apply() config resolution logic;
//   divergence causes height-profile editors to display incorrect layer heights.
...
SlicingParameters PrintObject::slicing_parameters(const DynamicPrintConfig& full_config,
    ...
    for (const ModelVolume* model_volume : model_object.volumes)
        if (model_volume->is_model_part()) {
            PrintRegion::collect_object_printing_extruders(print_config,
                                                           region_config_from_model_volume(default_region_config, nullptr, *model_volume,
                                                                                           filament_extruders),
            ...
            for (const std::pair<const t_layer_height_range, ModelConfig>& range_and_config : model_object.layer_config_ranges)
                if (range_and_config.second.has("wall_filament") || range_and_config.second.has("sparse_infill_filament") ||
                    ...
                    PrintRegion::collect_object_printing_extruders(print_config,
                                                                   region_config_from_model_volume(default_region_config,
                                                                                                   &range_and_config.second.get(),
                                                                                                   *model_volume, filament_extruders),
                                                                   object_config.brim_type != btNoBrim && object_config.brim_width > 0.,
                                                                   object_extruders);
        }
...
// [INTENT] object_extruders(): collects 0-based extruder indices actually used to print
// this object (excluding brim, support, and helper extrusions). Merges extruders
// from all PrintRegions (via collect_object_printing_extruders) plus any per-volume
// extruder assignments from ModelVolume::get_extruders(). De-duplicates the result.
// [COUPLING] Called by update_slicing_parameters() and slicing_parameters() (static);
//   the returned list determines which nozzle diameters constrain layer heights.
...
// [INTENT] update_layer_height_profile(): ensures the layer_height_profile vector
// is a valid pairwise (z, height) sequence that covers the full object height.
// If the profile is empty or its top entry doesn't match the slicing parameters,
// it is regenerated from layer_config_ranges via layer_height_profile_from_ranges().
// Returns true if the profile was regenerated (used to trigger downstream invalidation).
// [HAZARD] H1096: There is a comment noting "the assignment is crashing on ASAN OsX"
//   and the code uses a vector-constructor copy instead of assignment. This workaround
//   hides a potential ownership/aliasing issue that should be investigated in a port.
                                              ...
                                              std::vector<coordf_t>&   layer_height_profile)
{
    ...
    if (!layer_height_profile.empty() &&
        ...
        ((layer_height_profile.size() & 1) != 0 ||
         ...
         std::abs(layer_height_profile[layer_height_profile.size() - 2] - slicing_parameters.object_print_z_uncompensated_max +
                  slicing_parameters.object_print_z_min) > 1e-3))
...
// BBS:
// [INTENT] get_certain_layers(): collects layers whose print_z falls in [start, end]
// and simultaneously computes the union bounding box of their overhang bboxes.
// Appends one LayerPtrs list and one BoundingBox to the out/boundingbox_objects vectors
// (called with one PrintObject per call, so these vectors grow one entry at a time).
// Used by the multi-plate overhang detection pipeline.
void PrintObject::get_certain_layers(float start, float end, std::vector<LayerPtrs>& out, std::vector<BoundingBox>& boundingbox_objects)
{
    ...
    for (const auto& layer : layers()) {
        if (layer->print_z < start)
            continue;

        if (layer->print_z > end + EPSILON)
            break;
...
// [INTENT] get_instances_shift_without_plate_offset(): returns the 2D shift (in
// scaled integer coordinates) for each instance, excluding the plate offset.
// Used by the G-code writer to compute per-instance tool-path offsets independent
// of plate positioning (plate offset is added separately during G-code emission).
...
//
// [INTENT] clip_fill_surfaces(): top-down pass that converts stInternal surfaces to
// stInternalVoid where they are not structurally needed (infill_only_where_needed mode).
// A layer's sparse infill is only kept where it is directly below a top/solid surface
// or where it supports a perimeter. The pass works by projecting needed-support areas
// downward and clipping the internal surfaces against them.
// Idempotent: converts to stInternalVoid rather than removing, so re-running is safe.
// [STATE] Modifies LayerRegion::fill_surfaces (stInternal → stInternalVoid).
void PrintObject::clip_fill_surfaces()
{
    ...
    for (int layer_id = int(m_layers.size()) - 1; layer_id > 0; --layer_id) {
        ...
        upper_internal            = intersection(
            ...
            smooth_outward(closing(upper_internal, closing_radius, ClipperLib::jtSquare, 0.), scaled<coord_t>(0.1)),
...
// [INTENT] discover_horizontal_shells(): serial pass that scatters top/bottom solid surfaces
// into neighboring layers to fulfill the configured top_shell_layers / bottom_shell_layers.
// Works per-region, per-layer in a serial nested loop (top-down and bottom-up sweeps).
// For each layer with stTop/stBottom/stBottomBridge surfaces, the neighboring N layers
// (above for bottom, below for top) get those polygons promoted to stInternalSolid.
// Also handles the extra_solid_infill pattern: injects additional stInternalSolid surfaces
// in layers between the top/bottom shells.
// Uses a `goto EXTERNAL` for early-loop exit when a layer's region needs no changes —
// a non-trivial control flow pattern that a refactoring tool must handle explicitly.
// [STATE] Modifies LayerRegion::fill_surfaces: promotes stInternal → stInternalSolid.
// [CONCURRENCY] Serial. Cross-layer dependencies prevent parallelization without major refactor.
// [HAZARD] H1094: uses `goto EXTERNAL` label for early exit from inner loop. In a refactored
//   port this must be converted to a break/continue with equivalent logic, or a boolean flag.
void PrintObject::discover_horizontal_shells()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        for (size_t i = 0; i < m_layers.size(); ++i) {
            ...
            if (!region_config.extra_solid_infills.value.empty() && check_layer_id_pattern(region_config.extra_solid_infills.value, i)) {
            ...
            for (size_t idx_surface_type = 0; idx_surface_type < 3; ++idx_surface_type) {
                ...
                // Scatter top / bottom regions to other layers. Scattering process is inherently serial, it is difficult to parallelize
                // without locking.
                for (int n = (type == stTop) ? int(i) - 1 : int(i) + 1;
                     (type == stTop) ? (n >= 0 && (int(i) - n < num_solid_layers ||
                                                   print_z - m_layers[n]->print_z < region_config.top_shell_thickness.value - EPSILON)) :
                                       (n < int(m_layers.size()) &&
                                        (n - int(i) < num_solid_layers ||
                    ...
                    if (new_internal_solid.empty()) {
                        ...
                        if (region_config.sparse_infill_density.value == 0 ||
                            region_config.ensure_vertical_shell_thickness.value == evstCriticalOnly ||
                            region_config.ensure_vertical_shell_thickness.value == evstNone) {
                    ...
                    // when spacing is added in Fill.pm
                    {
                        ...
                        float margin = (region_config.ensure_vertical_shell_thickness.value != evstNone ? 3.f : 1.0f) *
                                       layerm->flow(frSolidInfill).scaled_width(); // require at least this size
                        ...
                        if (!too_narrow.empty()) {
                            ...
                            for (const Surface& surface : neighbor_layerm->fill_surfaces.surfaces)
                                if (surface.is_internal() && !surface.is_bridge())
                                    polygons_append(internal, to_polygons(surface.expolygon));
                            polygons_append(new_internal_solid, intersection(expand(too_narrow, +margin),
                                                                             // Discard bridges as they are grown for anchoring and we can't
                                                                             // remove such anchors. (This may happen when a bridge is being
                                                                             // anchored onto a wall where little space remains after the
                                                                             // bridge is grown, and that little space is an internal solid
                                                                             // shell so it triggers this too_narrow logic.)
                                                                             internal));
                    ...
                    for (SurfacesPtr& group : top_bottom_groups)
                        neighbor_layerm->fill_surfaces.append(diff_ex(group, polygons_internal),
...
// fill_surfaces but we only turn them into VOID surfaces, thus preserving the boundaries.
//
// [INTENT] combine_infill(): implements the "infill every N layers" feature by merging
// stInternal fill surfaces from N consecutive layers into a single thicker surface on
// one designated layer, and converting the skipped layers' surfaces to stInternalVoid.
// The merged Surface gets Surface::thickness and Surface::thickness_layers set to encode
// the multi-layer height — the fill generator uses these to compute extrusion width for
// the thicker paths.
// Per-region, serial per-layer-group. Groups are non-overlapping windows of N layers.
// [STATE] Modifies LayerRegion::fill_surfaces: adds combined surfaces, marks skipped
//   layers as stInternalVoid.
// [HAZARD] H1095: Surface::thickness is a float field that doubles as an encoding for
//   combined-infill height. This field is also used for other purposes elsewhere (e.g.
//   bridging). A refactored tool must distinguish the combined-infill usage from other uses.
void PrintObject::combine_infill()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        ...
        const double infill_combination_max_layer_height = region.config().infill_combination_max_layer_height.get_abs_value(
            nozzle_diameter);
        nozzle_diameter = infill_combination_max_layer_height > 0 ? std::min(infill_combination_max_layer_height, nozzle_diameter) :
                                                                    nozzle_diameter;
        ...
        for (size_t layer_idx = 0; layer_idx < m_layers.size(); ++layer_idx) {
            ...
            float clearance_offset = 0.5f * layerms.back()->flow(frPerimeter).scaled_width() +
                                     // Because fill areas for rectilinear and honeycomb are grown
                                     // later to overlap perimeters, we need to counteract that too.
                                     ((infill_pattern == ipRectilinear || infill_pattern == ipMonotonic || infill_pattern == ipGrid ||
                                       infill_pattern == ipLateralLattice || infill_pattern == ipLine || infill_pattern == ipHoneycomb ||
                                       infill_pattern == ipLateralHoneycomb) ?
                                          1.5f :
                                          0.5f) *
            ...
            for (LayerRegion* layerm : layerms) {
                ...
                if (layerm == layerms.back()) {
                    ...
                    // Save void surfaces.
                    layerm->fill_surfaces.append(intersection_ex(internal, intersection_with_clearance), stInternalVoid);
                }
            }
        }
    }
...
// [INTENT] _generate_support_material(): dispatches to either the tree-support
// generator (TreeSupport) or the classic grid-based support generator
// (PrintObjectSupportMaterial) depending on the support_type config option.
// Thin gateway function — all real logic lives in the two support subsystems.
// [COUPLING] Both code paths consume m_slicing_params (must be valid before call).
//   TreeSupport exposes a throw_on_cancel lambda to integrate with the cancellation token.
...
// [INTENT] remove_bridges_from_contacts<T>(): removes known-bridged regions from
// the overhang contact area so that support is not generated under bridges that
// can self-span. Operates on either Polygons or ExPolygons (template param PolysType).
// Two bridge sources are considered:
//   1. Straight perimeter overhang segments that are supported at both endpoints.
//   2. fill_surfaces entries of type stBottomBridge with a resolved bridge angle.
// Long bridges can optionally be broken into shorter spans (break_bridge flag).
// [HAZARD] H1097: typeid() runtime dispatch used to down-cast overhang_regions to
//   Polygons* or ExPolygons* — fragile if additional PolysType specialisations are added.
//   A refactored port should use template specialization or std::variant instead.
// [COUPLING] Reads layerm->perimeters (must be populated), layerm->fill_surfaces
//   (must have stBottomBridge surfaces), and layerm->unsupported_bridge_edges.
template<typename PolysType>
void PrintObject::remove_bridges_from_contacts(const Layer* lower_layer,
                ...
                if (bbox_size[0] < max_bridge_length && bbox_size[1] < max_bridge_length)
                    ...
                    if (break_bridge) {
                        ...
                        if (fabs(bridge_direction(0)) >
                            fabs(bridge_direction(
                                1))) { // cut bridge along x-axis if bridge direction is aligned to x-axis more than to y-axis
                            ...
                            for (int x = x0 + step; x < x1; x += step) {
                                Polygon poly;
                                poly.points = {Point(x - grid_lw, y0), Point(x + grid_lw, y0), Point(x + grid_lw, y1),
                                               Point(x - grid_lw, y1)};
                            ...
                            for (int y = y0 + step; y < y1; y += step) {
                                Polygon poly;
                                poly.points = {Point(x0, y - grid_lw), Point(x0, y + grid_lw), Point(x1, y + grid_lw),
                                               Point(x1, y - grid_lw)};
                                holes.emplace_back(poly);
                            }
                        }
...
template void PrintObject::remove_bridges_from_contacts<ExPolygons>(const Layer* lower_layer,
                                                                    const Layer* current_layer,
                                                                    float        extrusion_width,
                                                                    ExPolygons*  overhang_regions,
                                                                    float        max_bridge_length,
                                                                    bool         break_bridge);
template void PrintObject::remove_bridges_from_contacts<Polygons>(const Layer* lower_layer,
                                                                  const Layer* current_layer,
                                                                  float        extrusion_width,
...
// [INTENT] is_support_necessary(): heuristic analysis pass that determines whether
// the object needs support by running a full TreeSupport overhang detection pass
// (with support type forced to stTreeAuto for maximum sensitivity), then inspecting
// the resulting TreeSupport state flags.
// Returns SharpTail if sharp tails are detected, Cantilever if a cantilever exceeds
// the distance threshold, or NoNeedSupp otherwise.
// [HAZARD] H1098: Runs a full slice + overhang detection cycle then discards support
//   layers; this is expensive and should not be called in the hot slicing path.
//   In the UI it is called from a background thread for the support-advisor icon.
// [STATE] Calls clear_support_layers() after detection — discards any previously
//   computed support layers as a side effect.
...
// [INTENT] project_triangles_to_slabs(): parallel TBB kernel that projects each
// triangle of a painted custom-facets mesh onto the layer slabs it intersects.
// For each triangle, it determines the first and last layer slab that the triangle
// spans (by z-range), then interpolates sub-polygons at each slice boundary.
// The result is a per-layer vector of Polygons representing the 2D footprint of the
// painted facets — used for support enforcer/blocker and seam painting projection.
// [CONCURRENCY] tbb::parallel_for over triangles; output is written into independent
//   per-triangle slots (projections_of_triangles[idx]) so no locking is required.
//   Final merge into `out` is serial.
// [MEMORY] LightPolygon reserves 5 points per polygon (max for a triangle-slab
//   intersection); preallocated to reduce heap allocations inside the parallel loop.
static void project_triangles_to_slabs(
    ConstLayerPtrsAdaptor layers, const indexed_triangle_set& custom_facets, const Transform3f& tr, bool seam, std::vector<Polygons>& out)
{
                      ...
                      [&custom_facets, &tr, tr_det_sign, seam, layers, &projections_of_triangles](const tbb::blocked_range<size_t>& range) {
                          for (size_t idx = range.begin(); idx < range.end(); ++idx) {
                              ...
                              auto it = std::lower_bound(layers.begin(), layers.end(), facet[0].z() + EPSILON,
                                                         [](const Layer* l1, float z) { return l1->slice_z < z; });
                              ...
                              if (first_layer_id == last_layer_id) {
                                  // The triangle fits just a single slab, just project it. This also avoids division by zero for horizontal
                                  // triangles.
                                  ...
                                  bool add_below = dz < float(2. * EPSILON) && first_layer_id > 0 &&
                                                   layers[first_layer_id - 1]->slice_z > facet[0].z() - EPSILON;
    ...
    for (auto& trg : projections_of_triangles) {
        ...
        for (LightPolygon& poly : trg.polygons) {
            ...
            //                if (cross2(Vec2d((poly.pts[1] - poly.pts[0]).cast<double>()), Vec2d((poly.pts[2] -
            //                poly.pts[1]).cast<double>())) < 0)
...
// [INTENT] project_and_append_custom_facets(): outer loop over model volumes that
// dispatches per-volume painted facets to the correct projection method:
//   - Seam painting: calls project_triangles_to_slabs() (downward-facing triangle
//     projection tracking layer slab boundaries).
//   - Support enforcer/blocker painting: calls slice_mesh_slabs() (upward-facing
//     polygon projection via mesh slicing) and merges results layer-by-layer.
// Output `out` accumulates projected polygons for all volumes; it grows incrementally.
// [COUPLING] Called from GCode/SupportMaterial generation; consumers must union the
//   output polygons per layer before use (raw append, not unioned here).
void PrintObject::project_and_append_custom_facets(bool                                  seam,
                                                   ...
                                                   std::vector<std::pair<Vec3f, Vec3f>>* vertical_points) const
{
    for (const ModelVolume* mv : this->model_object()->volumes)
        if (mv->is_model_part()) {
            ...
            if (!custom_facets.indices.empty()) {
                if (seam)
                    project_triangles_to_slabs(this->layers(), custom_facets, (this->trafo_centered() * mv->get_matrix()).cast<float>(),
                                               seam, out);
                else {
                    std::vector<Polygons> projected;
                    // Support blockers or enforcers. Project downward facing painted areas upwards to their respective slicing plane.
                    slice_mesh_slabs(custom_facets, zs_from_layers(this->layers()), this->trafo_centered() * mv->get_matrix(), nullptr,
                                     &projected, vertical_points, []() {});
...
// [INTENT] get_layer_at_printz() / get_first_layer_bellow_printz() / get_layer_idx_get_printz()
// / get_layer_at_bottomz(): binary-search helpers over m_layers using lower_bound_by_predicate.
// Const and mutable overloads co-exist; mutable versions delegate via const_cast.
// get_layer_at_printz(z): exact match only — returns nullptr if no layer has print_z == z.
// get_layer_at_printz(z, epsilon): fuzzy match within ±epsilon — used for tolerance-aware lookup.
// get_first_layer_bellow_printz(z, epsilon): returns the first layer strictly below z+epsilon.
// get_layer_idx_get_printz(z, epsilon): returns 0-based index instead of pointer (-1 if none).
// get_layer_at_bottomz(z, epsilon): linear scan matching layer bottom_z — O(n), not binary search.
// [HAZARD] H1099: get_layer_at_bottomz() uses a linear scan over all layers rather than
//   binary search; for large layer counts this is O(n) where O(log n) is possible.
const Layer* PrintObject::get_layer_at_printz(coordf_t print_z) const
{
    auto it = Slic3r::lower_bound_by_predicate(m_layers.begin(), m_layers.end(),
                                               [print_z](const Layer* layer) { return layer->print_z < print_z; });
...
const Layer* PrintObject::get_layer_at_printz(coordf_t print_z, coordf_t epsilon) const
{
    coordf_t limit = print_z - epsilon;
    auto     it    = Slic3r::lower_bound_by_predicate(m_layers.begin(), m_layers.end(),
                                                      [limit](const Layer* layer) { return layer->print_z < limit; });
