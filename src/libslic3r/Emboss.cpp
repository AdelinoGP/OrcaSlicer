// [INTENT] Implementation of the stb_truetype-based emboss pipeline:
//   font file loading → glyph rasterization → polygon healing → text layout → 3D projection.
// [COUPLING] stb_truetype (C header, malloc/free), Clipper (union/diff/offset), CGAL Triangulation,
//            Boost.Nowide (wide-string conversion), admesh (stl_file via ScopeGuard).
// [CONCURRENCY] All public Emboss:: functions are called from plater job threads.
//               FontFileWithCache::cache is accessed ONLY from plater job thread (see H1023 in .hpp).
// [STATE] Module-level statics: MAX_HEAL_ITERATION_OF_TEXT, SHAPE_SCALE, ASCENT_CENTER.
//         These are compile-time constants — safe for concurrent read.
...
// [HAZARD H1026 P0/Critical] THIRD COORDINATE SYSTEM: glyph shapes are stored in integer coord_t
// units where 1 coord_t unit = SHAPE_SCALE mm = 0.001 mm.
// This is DISTINCT from both:
//   (a) regular Slic3r scaled coordinates (1 unit = 1e-6 mm = 1 nm), and
//   (b) unscaled mm coordinates.
// Glyph ExPolygon Points are divided by SHAPE_SCALE (i.e. multiplied by 1000 vs print units).
// Mixing glyph-scale coords with print-scale coords silently produces 1000x wrong geometry.
// Conversion: glyph_coord * SHAPE_SCALE * font_scale_to_mm = final mm position.
...
void remove_bad(ExPolygons& expolygons)
{
    expolygons.erase(std::remove_if(expolygons.begin(), expolygons.end(), [](const ExPolygon& p) { return p.contour.size() < 3; }),
                     expolygons.end());
...
bool Emboss::divide_segments_for_close_point(ExPolygons& expolygons, double distance)
{
    ...
    std::sort(divs.begin(), divs.end(), [](const Div& d1, const Div& d2) { return d1.second > d2.second; });
...
HealedExPolygons Emboss::heal_polygons(const Polygons& shape, bool is_non_zero, unsigned int max_iteration)
{
    ...
    ClipperLib::PolyFillType fill_type      = is_non_zero ? ClipperLib::pftNonZero : ClipperLib::pftEvenOdd;
...
bool Emboss::heal_expolygons(ExPolygons& shape, unsigned max_iteration) { return ::heal_dupl_inter(shape, max_iteration); }
...
Points get_unique_intersections(const Slic3r::IntersectionsLines& intersections)
{
    ...
    std::transform(intersections.begin(), intersections.end(), std::back_inserter(result), [](const Slic3r::IntersectionLines& i) {
        return Point(std::floor(i.intersection.x()), std::floor(i.intersection.y()));
    });
...
bool heal_dupl_inter(ExPolygons& shape, unsigned max_iteration)
{
    ...
    // [HAZARD H1027 P1/High] heal_dupl_inter() fallback: when max_iteration is exhausted and
    // the shape still has duplicates or intersections, each offending ExPolygon is replaced
    // by create_bounding_rect() — a hollow rectangle approximating the original outline.
    // This silently corrupts glyph geometry. The assert(false) fires in debug builds only.
    // A refactoring must surface this failure as an error result instead of silently continuing.
...
void remove_small_islands(ExPolygons& expolygons, double minimal_area)
{
    ...
    for (ExPolygon& expoly : expolygons) {
        ...
        auto      it = std::remove_if(holes.begin(), holes.end(), [&minimal_area](const Polygon& p) { return -p.area() < minimal_area; });
...
std::optional<Glyph> get_glyph(const stbtt_fontinfo& font_info, int unicode_letter, float flatness)
{
    ...
    // [HAZARD H1032 P2/Medium] stb_truetype returns malloc-allocated arrays.
    // ScopeGuard wraps `free()` in a C++ RAII destructor — mixing C heap with C++ object
    // lifetimes. This is safe here but requires careful ownership: do NOT copy `vertices`
    // or `points` pointers out of this scope. A refactoring must replace with std::unique_ptr
    // with a custom deleter: unique_ptr<stbtt_vertex, decltype(&free)>(ptr, free).
    ...
    stbtt__point* points = stbtt_FlattenCurves(vertices, num_verts, flatness, &contour_lengths, &num_countour_int, font_info.userdata);
    if (!points)
...
const Glyph* get_glyph(int unicode, const FontFile& font, const FontProp& font_prop, Glyphs& cache, fontinfo_opt& font_info_opt)
{
    ...
    // The TODO to derive it from printer config has never been resolved.
    // This flatness controls curve-to-line approximation quality: a finer printer could
    // benefit from higher resolution (smaller value). Refactoring must expose this via FontProp.
...
Point to_point(const stbtt__point& point)
{
    return Point(static_cast<int>(std::round(point.x / SHAPE_SCALE)), static_cast<int>(std::round(point.y / SHAPE_SCALE)));
}
...
{
    return {boost::nowide::narrow(name.c_str()), boost::nowide::narrow(path.c_str()), EmbossStyle::Type::file_path, FontProp()};
}
} // namespace
...
// [INTENT] Windows font list acquisition. Three strategies exist but only one is called.
// [HAZARD H1028 P2/Medium] get_font_list_by_enumeration() and get_font_list_by_folder()
// are compiled on Windows but NEVER called from get_font_list(). They are permanently
// unreachable dead code on Windows. Non-Windows always returns empty (not implemented).
// A refactoring should either remove the dead enumeration functions or expose a strategy
// parameter. Note: get_font_list_by_register() is the sole live path on Windows.
...
EmbossStyles Emboss::get_font_list_by_register()
{
    ...
    do {
        ...
        result = RegEnumValue(hKey, valueIndex, font_name, &font_name_size, 0, &valueType, fileTTF_name, &fileTTF_name_size);
        valueIndex++;
        if (result != ERROR_SUCCESS || valueType != REG_SZ)
            continue;
...
// [HAZARD H1031 P3/Low] EnumFamCallBack is a Win32 CALLBACK-qualified free function.
// "Fix global function" TODO is never addressed. UNREFERENCED_PARAMETER(lpntm) after
// the early-return `return true` is permanently unreachable dead code.
// This function is itself dead code: get_font_list_by_enumeration() is never called
// from get_font_list(). Refactoring: convert to a lambda passed via wrapper struct.
...
std::unique_ptr<FontFile> Emboss::create_font_file(std::unique_ptr<std::vector<unsigned char>> data)
{
...
std::optional<Glyph> Emboss::letter2glyph(const FontFile& font, unsigned int font_index, int letter, float flatness)
{
    if (!is_valid(font, font_index))
        return {};
...
// [HAZARD H1030 P3/Low] Cancellation checked every 10 characters only.
// For a text job processing a long string, there is up to 10-character latency before
// a cancel request is honoured. This is a design compromise, not a bug, but refactoring
// should expose CANCEL_CHECK as a parameter or use a periodic timer instead.
...
HealedExPolygons Emboss::text2shapes(FontFileWithCache&           font_with_cache,
                                     const char*                  text,
...
bool Emboss::is_italic(const FontFile& font, unsigned int font_index)
{
    ...
    int         name_id = 2;
    int         length;
    const char* value = stbtt_GetFontNameString(info, &length, STBTT_PLATFORM_ID_MICROSOFT, STBTT_MS_EID_UNICODE_BMP, STBTT_MS_LANG_ENGLISH,
                                                name_id);
    ...
    // lower case
    std::transform(value_str.begin(), value_str.end(), value_str.begin(), [](unsigned char c) { return std::tolower(c); });
...
indexed_triangle_set polygons2model_unique(const ExPolygons& shape2d, const IProjection& projection, const Points& points)
{
    ...
    result.vertices.insert(result.vertices.end(), std::make_move_iterator(back_points.begin()), std::make_move_iterator(back_points.end()));
    ...
    for (const Vec3i32& t : shape_triangles)
        result.indices.emplace_back(t.x() + count_point, t.y() + count_point, t.z() + count_point);

    // quads around - zig zag by triangles
    size_t polygon_offset = 0;
    auto   add_quads      = [&polygon_offset, &result, &count_point](const Polygon& polygon) {
                                             ...
                                             const Points&      duplicits)
{
    ...
    // quads around - zig zag by triangles
    size_t polygon_offset = 0;
    auto   add_quads      = [&polygon_offset, &result, count_point, &changes](const Polygon& polygon) {
        uint32_t polygon_points = polygon.points.size();
        // previous index
...
indexed_triangle_set Emboss::polygons2model(const ExPolygons& shape2d, const IProjection& projection)
{
...
std::optional<Vec2d> Emboss::ProjectZ::unproject(const Vec3d& p, double* depth) const { return Vec2d(p.x(), p.y()); }
...
Vec3d Emboss::suggest_up(const Vec3d normal, double up_limit)
{
    ...
    Vec3d wanted_up_side = (std::fabs(normal.z()) > up_limit) ? Vec3d::UnitY() : Vec3d::UnitZ();
...
Transform3d Emboss::create_transformation_onto_surface(const Vec3d& position, const Vec3d& normal, double up_limit)
{
    ...
    if (std::fabs(diff_view.x()) > 1. || std::fabs(diff_view.y()) > 1. || std::fabs(diff_view.z()) > 1.) // oposit direction
        angle_up *= -1.;
...
Vec3d Emboss::OrthoProject::project(const Vec3d& point) const { return point + m_direction; }
...
bool point_in_distance(const Coord2&          distance_sq,
...
float get_align_y_offset(FontProp::VerticalAlign align, unsigned count_lines, const FontFile& ff, const FontProp& fp)
{
    ...
    default: return -line_center + line_height * (count_lines - 1) / 2.;
    }
}
...
void align_shape(ExPolygonsWithIds& shapes, const std::wstring& text, const FontProp& prop, const FontFile& font)
{
    ...
    Point offset(get_align_x_offset(prop.align.first, shape_bb, get_line_bb(0)), y_offset);
...
// [HAZARD H1033 P2/Medium] PERMANENTLY DEAD CODE BLOCK.
// `#define REMOVE_SPIKES` is commented out at the top of this file (`//#define REMOVE_SPIKES`)
// and is never defined in the build system (CMakeLists, compiler flags, or presets).
// The entire spike-removal implementation below (remove_spikes(Polygon&), remove_spikes(Polygons&),
// remove_spikes(ExPolygons&)) is compiled out on every build. The declarations in the anonymous
// namespace at the top are also conditionally compiled and thus also dead.
// Refactoring: decide to either permanently remove this block or enable and integrate it.
#ifdef REMOVE_SPIKES
...
void remove_spikes(Polygon& polygon, const SpikeDesc& spike_desc)
{
    ...
    for (const SpikeHeal& h : heals) {
        ...
        case Type::add:
            ...
            break;
        case Type::erase: pts.erase(pts.begin() + h.index); break;
        case Type::move: pts[h.index] = h.b; break;
        default: break;
        }
    }
}
