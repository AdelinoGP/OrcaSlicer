// [INTENT] Measure.cpp: 3D surface feature extraction and distance/angle measurement engine.
// Used by the interactive measurement tool in the GUI. Two-phase design:
//   Phase 1 (constructor): update_planes() — flood-fill triangles by equal normals,
//     extract plane border polygons in parallel via TBB. O(F log F) per plane.
//   Phase 2 (lazy): extract_features() — per-plane, detect circles/edges from borders
//     using RANSAC circle fitting and angle comparison. Triggered on first access.
// [STATE] MeasuringImpl: m_planes (PlaneData[]), m_face_to_plane (size_t[]), m_its (copy).
//         features_extracted flag per plane — lazy evaluation guard.
// [CONCURRENCY] update_planes() uses tbb::parallel_for across planes (border extraction).
//   extract_features() is called from get_feature() which is presumably on the GUI thread.
//   If get_feature() and any TBB worker overlap, m_planes data race exists.
// [COUPLING] References SurfaceMesh (CGAL-style half-edge mesh), Geometry::circle_ransac,
//   MeasureUtils. The SurfaceMesh is constructed from indexed_triangle_set on every
//   MeasuringImpl construction — O(F) rebuild cost.
...
namespace Slic3r { namespace Measure {
...
static std::tuple<Vec3d, double, double> get_center_and_radius(const std::vector<Vec3d>& points,
                                                               ...
                                                               const Transform3d&        trafo_inv)
{
    ...
    const int iter = points.size() < 10 ? 2 : points.size() < 100 ? 4 : 6;
...
static std::array<Vec3d, 3> orthonormal_basis(const Vec3d& v)
{
    ...
    switch (index) {
    case 0: {
        ret[0] = Vec3d(ret[2].y(), -ret[2].x(), 0.0).normalized();
        break;
    }
    case 1: {
        ret[0] = Vec3d(0.0, ret[2].z(), -ret[2].y()).normalized();
        break;
    }
    case 2: {
        ret[0] = Vec3d(-ret[2].z(), 0.0, ret[2].x()).normalized();
        break;
    }
    }
...
// [INTENT] update_planes: Phase 1 of feature extraction.
// Flood-fill: group co-normal adjacent triangles into PlaneData entries (O(F log F)).
// Then for each plane, walk the half-edge mesh to extract border polygon chains in parallel.
// [HAZARD H1125] Uses `goto PLANE_FAILURE` inside a TBB parallel lambda — acceptable since
// the goto exits only within the enclosing scope, but structurally hostile to porting.
// The PLANE_FAILURE path silently discards all borders for that plane (borders.clear()).
// Any mesh with T-junctions, non-manifold edges, or degenerate faces silently produces
// a plane with no detected features — no error is reported.
// [HAZARD H1126] is_same_normal tolerance is hardcoded at 0.001 per component.
// Two triangles that are "almost" co-planar but beyond this threshold get separate planes.
// For curved surfaces this produces many tiny planes, degrading performance.
...
// [INTENT] extract_features: Phase 2 (lazy). For a single plane, classify its border
// polygons as circles, circular arcs, polygon edges, or straight edges.
// Algorithm:
//   1. For each border: if >4 points, try RANSAC circle fit (get_center_and_radius).
//      If err < 0.05 and equal edge lengths, classify whole border as Circle or Polygon.
//   2. Otherwise: compute interior angles at all border vertices, find runs of equal angles
//      (= circular arc candidates), validate via RANSAC and arc-length threshold (>90°).
//   3. Remaining edges merged into straight Edge features.
//   4. Final feature: the Plane itself (always last in surface_features[]).
// [STATE] After extract_features(), plane.borders is cleared (memory released).
//   plane.features_extracted = true prevents re-extraction.
// [HAZARD H1127] extract_features() is NOT thread-safe: called lazily from get_feature()
...
void MeasuringImpl::extract_features(int plane_idx)
{
    ...
    for (const std::vector<Vec3d>& border : plane.borders) {
        ...
        if (border.size() > 4) {
            ...
            if (err < 0.05) {
                ...
                bool lengths_match = std::all_of(border.begin() + 2, border.end(), [is_polygon](const Vec3d& pt) {
                    return Slic3r::is_approx((pt - *((&pt) - 1)).squaredNorm(), (*((&pt) - 1) - *((&pt) - 2)).squaredNorm(),
                ...
                if (lengths_match && (is_polygon || border.size() > 8)) {
                    if (is_polygon) {
                        ...
                        plane.surface_features.emplace_back(
                            SurfaceFeature(SurfaceFeatureType::Circle, center, plane.normal, std::nullopt, radius));
                    }
                    done = true;
                }
            }
        ...
        if (!done) {
            ...
            assert(
                std::all_of(edges.begin(), edges.end(), [](const SurfaceFeature& f) { return f.get_type() == SurfaceFeatureType::Edge; }));
            ...
            assert(std::all_of(circles.begin(), circles.end(),
                               [](const SurfaceFeature& f) { return f.get_type() == SurfaceFeatureType::Circle; }));
            assert(
                std::all_of(edges.begin(), edges.end(), [](const SurfaceFeature& f) { return f.get_type() == SurfaceFeatureType::Edge; }));
    ...
    cog /= double(counter);
    plane.surface_features.emplace_back(
        SurfaceFeature(SurfaceFeatureType::Plane, plane.normal, cog, std::optional<Vec3d>(), plane_idx + 0.0001));
...
// [INTENT] get_feature: given a face index and 3D hover point, return the closest
// SurfaceFeature within feature_hover_limit (0.5mm) of the point.
// Calls extract_features() lazily if not yet done for this plane.
// Falls back to returning the whole Plane if no sub-feature is within hover range.
// [HAZARD H1128] Hardcoded debug check `if (face_idx == 7516 || face_idx == 7517)`
// with `std::cout << ""` — leftover debug code that fires on every hover event for those faces.
// [STATE] Returns features already transformed to world space via world_tran.
//   The feature's origin_surface_feature stores the pre-transform version for snapping.
// [COUPLING] const_cast to access surface_features.back() from a const PlaneData& is unnecessary.
std::optional<SurfaceFeature> MeasuringImpl::get_feature(size_t             face_idx,
                                                         const Vec3d&       point,
                                                         const Transform3d& world_tran,
                                                         bool               only_select_plane)
{
...
int MeasuringImpl::get_num_of_planes() const { return (m_planes.size()); }
...
const indexed_triangle_set& MeasuringImpl::get_its() const { return this->m_its; }

Measuring::Measuring(const indexed_triangle_set& its) : priv{std::make_unique<MeasuringImpl>(its)} {}

Measuring::~Measuring() {}

std::optional<SurfaceFeature> Measuring::get_feature(size_t             face_idx,
                                                     const Vec3d&       point,
                                                     const Transform3d& world_tran,
...
int Measuring::get_num_of_planes() const { return priv->get_num_of_planes(); }

const std::vector<int>& Measuring::get_plane_triangle_indices(int idx) const { return priv->get_plane_triangle_indices(idx); }

const std::vector<SurfaceFeature>& Measuring::get_plane_features(unsigned int plane_id) const { return priv->get_plane_features(plane_id); }

const indexed_triangle_set& Measuring::get_its() const { return priv->get_its(); }
...
static AngleAndEdges angle_plane_plane(const std::tuple<int, Vec3d, Vec3d>& p1, const std::tuple<int, Vec3d, Vec3d>& p2)
{
    ...
    const Eigen::ParametrizedLine<double, 3> intersection_line = Eigen::ParametrizedLine<double, 3>(intersection_line_origin,
    ...
    const std::pair<Vec3d, Vec3d> edge_on_plane1 = {origin1_proj + radius * edge_on_plane1_unit,
                                                    origin1_proj + 2.0 * radius * edge_on_plane1_unit};
    const std::pair<Vec3d, Vec3d> edge_on_plane2 = {origin2_proj + radius * edge_on_plane2_unit,
...
// [INTENT] get_measurement: dispatcher for pairwise distance/angle measurements.
// All feature-type combinations (Point-Point, Point-Edge, Point-Circle, Point-Plane,
// Edge-Edge, Edge-Circle, Edge-Plane, Circle-Circle, Circle-Plane, Plane-Plane) are
// handled via a large if/else chain. The pair is canonicalized by type index (swap if needed)
// so lower-enum type is always f1.
// [HAZARD H1129] The function dispatches on 10 type combinations. Not all combinations are
// implemented — get_feature() checks `if (res.distance_strict)` and skips unimplemented ones.
// This silent skip-on-null is the only guard; no ASSERT or error log for missing combos.
// [COUPLING] deal_circle_result bool changes the semantics of Point-Circle distance:
//   false → closest point on circle circumference; true → distance to circle center.
//   This flag is caller-controlled with no documentation of intended use cases.
    ...
    if (f1.get_type() == SurfaceFeatureType::Point) {
        if (f2.get_type() == SurfaceFeatureType::Point) {
            ...
            if (dist_start_sq < len_sq && dist_end_sq < len_sq) {
                ...
                result.distance_strict = std::make_optional(
                    DistAndPoints{std::sqrt(std::min(dist_start_sq, dist_end_sq) + sqr(dist_inf)), f1.get_point(), s_is_closer ? s : e});
            }
            ...
            result.distance_infinite = std::make_optional(
                DistAndPoints{plane.absDistance(f1.get_point()), f1.get_point(), plane.projection(f1.get_point())}); // TODO
        ...
        if (f2.get_type() == SurfaceFeatureType::Edge) {
            ...
            auto add_point_edge_distance = [&distances](const Vec3d& v, const std::pair<Vec3d, Vec3d>& e) {
                const MeasurementResult res      = get_measurement(SurfaceFeature(v),
            ...
            auto it                  = std::min_element(distances.begin(), distances.end(),
                                                        [](const DistAndPoints& item1, const DistAndPoints& item2) { return item1.dist < item2.dist; });
        ...
        if (f2.get_type() == SurfaceFeatureType::Circle) {
            ...
            const bool coplanar = are_parallel(normal1, normal2) &&
                                  Eigen::Hyperplane<double, 3>(normal1, center).absDistance(origin2) < EPSILON;
            if (!coplanar) {
                auto                       plane_features = f2.world_plane_features;
                std::vector<DistAndPoints> distances;
                for (const SurfaceFeature& sf : *plane_features) {
                ...
                if (!distances.empty()) {
                    ...
                    result.distance_infinite = std::make_optional(
                        DistAndPoints{plane.absDistance(center), center, plane.projection(center)});
                }
            } else {
...
// [INTENT] can_set_xyz_distance: returns true if the two feature types support
// decomposed XYZ distance display (currently only Point-Point and Circle-Circle).
// [COUPLING] Only 2 of 10 type-combinations return true — used by GUI to enable/disable
// the XYZ distance breakdown widget. Any new type combinations must be added here explicitly.
...
// [INTENT] get_assembly_action: given two surface features, compute the AssemblyAction struct
// that drives the "assembly" GUI panel (align plane to plane, etc.).
// Currently only handles Plane-Plane combinations; all other combos return a default-constructed
// AssemblyAction with all flags false.
// [COUPLING] Directly drives GUI widget enable/disable state — any new feature-type combination
// capable of assembly operations must be added here as an explicit branch.
