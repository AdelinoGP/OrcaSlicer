...
enum MappingResult { MAPPING_RESULT_DEFAULT = 0, MAPPING_RESULT_TYPE_MISMATCH = 1, MAPPING_RESULT_EXCEED = 2 };
...
struct FilamentInfo
{
    ...
    float       distance{0.f};
    int         ctype = 0;
    // [STATE] Mapping fields are mutated by cloud-printer reconciliation logic after slicing completes.
...
public:
    ...
    // [INTENT] Snapshot per-plate material usage and output paths so upload/reporting can run without re-reading G-code.
...
public:
    ...
    // [COUPLING] Parent pointer links cloud task transport state with local plate-specific slice metadata.
    ...
    // [MEMORY] Non-owning raw pointer; lifetime is managed by the containing BBLTask object graph.
...
public:
    ...
    // [MEMORY] Owns pointers to dynamically allocated subtasks/slice-info records; cleanup policy is implemented out-of-line in .cpp.
    // [HAZARD] Raw-pointer containers make ownership transfer implicit and fragile during refactors.
...
public:
    ...
    // [MEMORY] Profile keeps raw task pointers and forwards them to project-level orchestration; destructor is trivial here.
...
public:
    ...
    // [MEMORY] Project aggregates profile pointers; ownership/lifetime contracts are external and must be preserved by callers.
