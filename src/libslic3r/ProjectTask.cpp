// [INTENT] Cloud-platform data structures for Bambu Lab's project/task/subtask
// hierarchy used by the OrcaSlicer cloud-print UI. Handles JSON parsing of
// server responses for BBLProfile, BBLTask, BBLSubTask, and BBLProject objects.
// Two JSON parsers are used: nlohmann/json (BBLSubTask) and boost::property_tree
// (BBLTask) — inconsistency inherited from Bambu Studio lineage.
// [STATE] All state is instance data on the BBL* classes; no file-level statics.
// [COUPLING] Depends on nlohmann/json, boost::property_tree, boost::uuid (included
// but uuid not actually used in this file — leftover include).
// [HAZARD H1184] BBLTask::parse_content_json uses raw `new BBLSubTask` / `delete subtasks[i]`
// manual memory management inside the subtasks vector — mixing raw ownership and
// vector storage. If an exception is thrown mid-loop after some subtasks are already
// pushed, the previously created subtasks that were deleted by the loop are not
// covered by any RAII; however the exception is caught at the try/catch boundary,
// leaving subtasks in a partially cleared state with no cleanup guarantee for the
// newly allocated items. Port should use unique_ptr or smart pointers.
// [HAZARD H1185] BBLSubTask::parse_status falls back to TASK_CREATED for any
// unrecognised status string — unknown future server states silently become "created".
#include "libslic3r.h"
#include "Time.hpp"
#include "Thread.hpp"
