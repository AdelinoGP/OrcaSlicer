// [INTENT] Implementation of FuzzySkin surface-texture feature.
// Applies configurable noise displacement to outer perimeters layer by layer.
// Supports 5 noise types (Perlin, Billow, RidgedMulti, Voronoi, Uniform) via
// the libnoise library.  Handles both classic Polygon and Arachne ExtrusionLine
// perimeters, with per-region splitting for multi-material prints.
//
// [CONCURRENCY] Thread-safe: noise module is created per-call on the stack;
// random_value() uses thread_local state.  No shared mutable state.
//
// [MEMORY] Polygon/junction lists are grown on the stack; `out` vectors are
// reserved to input size before filling.  Peak overhead is O(N) per polygon.
...
void fuzzy_polyline(Points& poly, bool closed, coordf_t slice_z, const FuzzySkinConfig& cfg)
{
    ...
    const double min_dist_between_points = cfg.point_distance * 3. /
...
void fuzzy_extrusion_line(Arachne::ExtrusionJunctions& ext_lines, coordf_t slice_z, const FuzzySkinConfig& cfg, bool closed)
{
    ...
    const double min_dist_between_points = cfg.point_distance * 3. /
                                           4.; // hardcoded: the point distance may vary between 3/4 and 5/4 the supplied value
    const double range_random_point_dist = cfg.point_distance / 2.;
    const double min_extrusion_width =
        0.01; // workaround for many print options. Need overwrite formula with the layer height parameter. The width must more than >>>
              // layer_height * (1 - 0.25 * PI) * 1.05 <<< (last num is the coeff of overlay error case)
    double dist_left_over = random_value() *
                            (min_dist_between_points / 2.); // the distance to be traversed on the line before making the first new point
    ...
    for (auto& p1 : ext_lines) {
        ...
        for (; p0pa_dist < p0p1_size; p0pa_dist += min_dist_between_points + random_value() * range_random_point_dist) {
            ...
            case FuzzySkinMode::Combined:
                ...
                out.emplace_back(pa + (perp(p0p1).cast<double>().normalized() * ((rad - p1.w) / 2)).cast<coord_t>(), rad,
                                 p1.perimeter_index); // 0.05 - minimum width of extruded line
                break;
            }
        }
        dist_left_over = p0pa_dist - p0p1_size;
