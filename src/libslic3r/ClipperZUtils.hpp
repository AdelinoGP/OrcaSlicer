...
namespace Slic3r { namespace ClipperZUtils {

// [INTENT] Z-enabled Clipper1 wrappers encode contour provenance in Z while preserving integer XY clipping semantics.
...
template<bool Open = false> inline ZPath to_zpath(const Points& path, coord_t z)
{
    // [STATE] Pure conversion utility; no shared state touched.
    ZPath out;
    if (!path.empty()) {
...
template<bool Open = false> inline ZPaths to_zpaths(const VecOfPoints& paths, coord_t z)
{
    ZPaths out;
...
// If Open, then duplicate the first point of each path at its end.
template<bool Open = false> inline ZPaths expolygons_to_zpaths(const ExPolygons& src, coord_t& base_idx)
{
    // [STATE] base_idx is a caller-owned running identifier for source expolygon grouping.
    ZPaths out;
...
// Convert a single path to path with a given Z coordinate.
// If Open, then duplicate the first point at the end.
template<bool Open = false> inline Points from_zpath(const ZPoints& path)
{
    Points out;
    if (!path.empty()) {
        out.reserve((path.size() + Open) ? 1 : 0);
...
// Convert multiple paths to paths with a given Z coordinate.
// If Open, then duplicate the first point of each path at its end.
template<bool Open = false> inline void from_zpaths(const ZPaths& paths, VecOfPoints& out)
{
    out.reserve(out.size() + paths.size());
    for (const ZPoints& path : paths)
        out.emplace_back(from_zpath<Open>(path));
}
template<bool Open = false> inline VecOfPoints from_zpaths(const ZPaths& paths)
{
    VecOfPoints out;
...
public:
    ...
    {
        // [INTENT] Preserve origin tracing through clipping intersections for later post-processing reconstruction.
        ...
        if (begin + 1 == end) {
            ...
            if (begin + 2 <= end) {
                ...
                // [HAZARD] Negative Z here is metadata, not height; consumers must decode before geometric use.
    ...
    {
        return [this](const ZPoint& e1bot, const ZPoint& e1top, const ZPoint& e2bot, const ZPoint& e2top, ZPoint& pt) {
...
private:
    // [MEMORY] Non-owning reference; lifetime is externally managed by caller.
    // [COUPLING] Data contract is constrained by ClipperLib_Z callback signature and sentinel convention.
    std::vector<std::pair<coord_t, coord_t>>& m_intersections;
};

}} // namespace Slic3r::ClipperZUtils
