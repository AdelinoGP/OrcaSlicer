...
// [INTENT] EdgeGrid is a raster acceleration structure for polygon/segment queries.
//          It indexes contour segments into uniform grid cells to speed up distance,
//          intersection, and inside/outside tests used by supports and path planning.
// [COUPLING] Consumes core geometry primitives (Point/Line/Polygon/ExPolygon) and
//            shares fixed-point coordinate conventions with the rest of libslic3r.
// [MEMORY] Contour stores non-owning pointers into external point buffers; callers
//          must keep source polygon/polyline storage alive for Grid lifetime.
// [CONCURRENCY] Grid construction and query methods are unsynchronized; treat Grid
//               instances as thread-confined or externally synchronized.
...
public:
    ...
    Lines get_segments() const
    {
        ...
        if (this->num_segments() > 2) {
            ...
            if (!m_open)
                lines.push_back(Line(this->back(), this->front()));
        }
        return lines;
    }
...
private:
    ...
    // [MEMORY] Borrowed span, not owning container. `m_end` is one-past-last.
...
public:
    ...
    // [INTENT] Fill in a rough m_signed_distance_field from the edge grid.
    ...
    // [INTENT] Integer-grid traversal (Bresenham-like) over all cells touched by a line.
    //          Early exit is supported by returning false from visitor.
    // [HAZARD] Integer arithmetic assumes coordinates fit into int64 intermediates after
    //          multiplying by `m_resolution`; extremely large build volumes can overflow.
    ...
    std::pair<std::vector<std::pair<size_t, size_t>>::const_iterator, std::vector<std::pair<size_t, size_t>>::const_iterator> cell_data_range(
        coord_t row, coord_t col) const
    {
...
protected:
    ...
    bool cell_inside_or_crossing(int r, int c) const
    {
        ...
        return (cell.begin < cell.end) || (!m_signed_distance_field.empty() && m_signed_distance_field[r * (m_cols + 1) + c] <= 0.f);
    }

    // [STATE] Core grid metadata; set by create* and reused by query methods.
    ...
    // [MEMORY] Referencing the source contours.
    ...
    // [INTENT] Distance field derived from the edge grid, seed filled by the
    // Danielsson chamfer metric.
    // May be empty.
