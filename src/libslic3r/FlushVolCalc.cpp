// [INTENT] Flush-volume calculator for multi-material filament changes. Estimates the volume
//          of purge material needed when switching from one filament colour to another based on
//          either an ML-trained dataset (FlushPredictor) or a hand-tuned HSV-distance formula.
// [STATE]  FlushVolCalculator is a simple value-type object holding config parameters.
//          g_max_flush_volume is a translation-unit constant (not shared mutable state).
// [COUPLING] Depends on ColorSpaceConvert.hpp for RGB2HSV, and FlushVolPredictor for ML path.
// [MEMORY]  No heap allocation; GenericFlushPredictor is created on the stack each call.
#include <cmath>
#include <assert.h>
...
// [INTENT] Hard cap on flush volume. OrcaSlicer increased this from the upstream BBS value of
//          900 mm³ to 20000 mm³ to support demanding filament transitions (e.g. dark→white PA).
...
static float to_radians(float degree) { return degree / 180.f * M_PI; }

static float get_luminance(float r, float g, float b) { return r * 0.3 + g * 0.59 + b * 0.11; }
...
bool FlushVolCalculator::get_flush_vol_from_data(unsigned char src_r,
                                                 unsigned char src_g,
                                                 unsigned char src_b,
                                                 unsigned char dst_r,
                                                 unsigned char dst_g,
...
int FlushVolCalculator::calc_flush_vol_rgb(
    unsigned char src_r, unsigned char src_g, unsigned char src_b, unsigned char dst_r, unsigned char dst_g, unsigned char dst_b)
{
    float flush_volume;
...
int FlushVolCalculator::calc_flush_vol(unsigned char src_a,
                                       unsigned char src_r,
                                       unsigned char src_g,
                                       unsigned char src_b,
                                       unsigned char dst_a,
                                       ...
                                       unsigned char dst_b)
{
    ...
    // [HAZARD H1165] Variable names are backwards relative to their values:
    //               dark_color_thres = 180/255 ≈ 0.706 → luminance ABOVE this is LIGHT (bright),
    //               light_color_thres = 75/255 ≈ 0.294 → luminance BELOW this is DARK.
    //               is_from_dark actually tests "is from LIGHT colour";
    //               is_to_light actually tests "is to DARK colour". The 1.3× correction for
    //               light→dark transitions is perceptually correct but the naming inverts meaning.
