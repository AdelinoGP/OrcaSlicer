...
// [INTENT] MultiPoint.cpp implements all non-inlined MultiPoint and MultiPoint3 methods:
//   geometric transforms (scale, translate, rotate), simplification algorithms
//   (Douglas-Peucker, Visvalingam-Whyatt), intersection queries, bounding-box helpers,
//   and the Orca IOI wall-ordering distance utilities.
// [COUPLING] No direct dependency on Print or GCode layers — pure geometry.
...
// [INTENT] Non-uniform scale — truncation to coord_t is intentional (integer geometry)
// but accumulates rounding error if called in loops. See H1050.
...
// [INTENT] Rotation around origin using pre-computed cos/sin.
// Rounds to nearest integer coord_t — standard rounding for scaled-unit geometry.
...
{
    // [INTENT] Find closest vertex within epsilon distance. Falls back to exact scan
    //   when epsilon==0. Note: only scans vertices, not edge interiors (H1051).
    if (scaled_epsilon == 0)
...
BoundingBox MultiPoint::bounding_box() const { return BoundingBox(this->points); }
...
bool MultiPoint::intersection(const Line& line, Point* intersection) const
{
    ...
    for (Lines::const_iterator it = lines.begin(); it != lines.end(); ++it) {
        if (it->intersection(line, intersection))
            return true;
    }
    return false;
}
...
// [INTENT] Ramer-Douglas-Peucker simplification. Iterative stack-based implementation —
//   avoids deep recursion on large polylines. Tolerance is in squared scaled units.
//   Preserves first and last points unconditionally.
// [HAZARD] H1055 P3/Low: Dead SVG debug block guarded by #if 0 (lines 221–235). Not
//   harmful but clutters the file and may mislead readers.
...
// [INTENT] Visvalingam-Whyatt simplification: removes points whose effective triangle
//   area (with neighbours) is below tolerance. Area-based — better than DP for preserving
//   visually significant small-angle turns on smooth curves.
// [HAZARD] H1053 P2/Medium: calls std::make_heap() after EVERY node update — O(n) per
//   update → O(n²) total. Author notes this in a comment ("For some reason, std::make_heap()
//   is the fastest way to resort the heap. Probably needs testing."). A proper heap with
//   decrease-key would give O(n log n).
// [MEMORY] Uses raw new/delete for vis_node objects via node_list + heap vectors.
//   Leak-safe as long as the while(!heap.empty()) loop runs to completion, but any
//   early exception (e.g., bad_alloc) would leak all allocated nodes.
...
struct vis_node
{
    vis_node(const size_t& idx, const size_t& _prev_idx, const size_t& _next_idx, const double& _area)
        : pt_idx(idx), prev_idx(_prev_idx), next_idx(_next_idx), area(_area)
    {}
...
// [INTENT] Builds a lower convex hull with a concaveness tolerance: points that would
...
//   Minor but propagated to all call sites.
    ...
    if (n >= 3) {
        ...
        for (int i = 0; i < n; ++i) {
            while (k >= 2 && pts[i].ccw(hull[k - 2], hull[k - 1]) <= 0 &&
                   -pts[i].ccw(hull[k - 2], hull[k - 1]) / (pts[i] - hull[k - 1]).norm() < tolerence)
                --k;
            hull[k++] = pts[i];
        }
        hull.resize(k);
...
// Orca: Distancing function used by IOI wall ordering algorithm for arachne
// [INTENT] minimumDistanceBetweenLinesDefinedByPoints: Computes the minimum Euclidean distance
//   between two polylines A and B by checking all (segment, point) pairs in both directions:
//   A-segments vs B-points and B-segments vs A-points.
//   Used by the Arachne IOI (Inward-Ordered Inset) wall ordering algorithm to determine
//   spatial proximity between wall loops before sorting them by distance.
// [HAZARD] Asymmetric coverage: only point-to-segment distances are checked, not
//   segment-to-segment minimum distances (i.e., two segments that are closest at their
//   interiors but whose endpoints are all far apart will return an inflated distance).
//   For nearly-parallel adjacent walls this is unlikely to matter in practice.
// [COUPLING] Calls file-local squaredDistanceToLineSegment() (defined just above).
//   Caller (Arachne wall ordering) is in PerimeterGenerator or WallToolPaths.
/**
...
void MultiPoint3::translate(const Point& vector) { this->translate(vector(0), vector(1)); }
...
BoundingBox3 MultiPoint3::bounding_box() const { return BoundingBox3(points); }
...
BoundingBox get_extents(const MultiPoint& mp) { return BoundingBox(mp.points); }
...
BoundingBox get_extents_rotated(const MultiPoint& mp, double angle) { return get_extents_rotated(mp.points, angle); }
