// [INTENT] Debug SVG renderer for 2D geometry (polygons, polylines, points,
// surfaces, thick lines).  Not used in production G-code pipeline; only
// compiled in for debug visualisation.
// [HAZARD H1189] draw_grid (BBS addition, line 324): horizontal lines use
// end_pt initialised as Point(bbox.max(1), bbox.min(1)) – the x-coordinate is
// bbox.max(1) (the Y-max value) instead of bbox.max(0) (X-max).  This is a
// copy-paste bug: horizontal lines will have wrong x-extent when bbox is not
// square.  The vertical-lines loop below it is correct.
// [STATE] Writes directly to a FILE* opened by open(); no threading.
// [COUPLING] Depends on ClipperLib path types, all Slic3r 2D geometry types.
#include "SVG.hpp"
#include <iostream>
...
bool SVG::open(const char* afilename)
{
    ...
    fprintf(this->f, "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
                     ...
                     "<svg height=\"2000\" width=\"2000\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:svg=\"http://www.w3.org/2000/svg\" "
                     "xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n"
                     "   <marker id=\"endArrow\" markerHeight=\"8\" markerUnits=\"strokeWidth\" markerWidth=\"10\" orient=\"auto\" "
                     "refX=\"1\" refY=\"5\" viewBox=\"0 0 10 10\">\n"
...
bool SVG::open(const char* afilename, const BoundingBox& bbox, const coord_t bbox_offset, bool aflipY)
{
    ...
    fprintf(this->f,
            ...
            "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.0//EN\" \"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd\">\n"
            "<svg height=\"%f\" width=\"%f\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:svg=\"http://www.w3.org/2000/svg\" "
            "xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n"
            "   <marker id=\"endArrow\" markerHeight=\"8\" markerUnits=\"strokeWidth\" markerWidth=\"10\" orient=\"auto\" refX=\"1\" "
...
{
    fprintf(this->f, "   <line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" style=\"stroke: %s; stroke-width: %f\"",
            to_svg_x(line.a(0) - origin(0)), to_svg_y(line.a(1) - origin(1)), to_svg_x(line.b(0) - origin(0)),
            to_svg_y(line.b(1) - origin(1)), stroke.c_str(), (stroke_width == 0) ? 1.f : to_svg_coord(stroke_width));
    if (this->arrows)
...
void SVG::draw(const ThickLine& line, const std::string& fill, const std::string& stroke, coordf_t stroke_width)
{
    ...
    fprintf(this->f, "   <polygon points=\"%f,%f %f,%f %f,%f %f,%f\" style=\"fill:%s; stroke: %s; stroke-width: %f\"/>\n",
            to_svg_x(line.a(0) - da * perp(0) - origin(0)), to_svg_y(line.a(1) - da * perp(1) - origin(1)),
            to_svg_x(line.b(0) - db * perp(0) - origin(0)), to_svg_y(line.b(1) - db * perp(1) - origin(1)),
            to_svg_x(line.b(0) + db * perp(0) - origin(0)), to_svg_y(line.b(1) + db * perp(1) - origin(1)),
            to_svg_x(line.a(0) + da * perp(0) - origin(0)), to_svg_y(line.a(1) + da * perp(1) - origin(1)), fill.c_str(), stroke.c_str(),
...
void SVG::draw(const Surface& surface, std::string fill, const float fill_opacity) { draw(surface.expolygon, fill, fill_opacity); }
...
void SVG::draw(const Point& point, std::string fill, coord_t iradius)
{
    ...
    svg << "   <circle cx=\"" << to_svg_x(point(0) - origin(0)) << "\" cy=\"" << to_svg_y(point(1) - origin(1)) << "\" r=\"" << radius
        << "\" "
...
void SVG::path(const std::string& d, bool fill, coordf_t stroke_width, const float fill_opacity)
{
    ...
    fprintf(this->f, "   <path d=\"%s\" style=\"fill: %s; stroke: %s; stroke-width: %f; fill-type: evenodd\" %s fill-opacity=\"%f\" />\n",
            d.c_str(), fill ? this->fill.c_str() : "none", this->stroke.c_str(), lineWidth,
            (this->arrows && !fill) ? " marker-end=\"url(#endArrow)\"" : "", fill_opacity);
}
...
{
    fprintf(this->f, "<text x=\"%f\" y=\"%f\" font-family=\"sans-serif\" font-size=\"%dpx\" fill=\"%s\">%s</text>",
            to_svg_x(pt(0) - origin(0)), to_svg_y(pt(1) - origin(1)), font_size * 10, color, text);
}

void SVG::draw_legend(const Point& pt, const char* text, const char* color)
{
    fprintf(this->f, "<circle cx=\"%f\" cy=\"%f\" r=\"10\" fill=\"%s\"/>", to_svg_x(pt(0) - origin(0)), to_svg_y(pt(1) - origin(1)), color);
    fprintf(this->f, "<text x=\"%f\" y=\"%f\" font-family=\"sans-serif\" font-size=\"10px\" fill=\"%s\">%s</text>",
            to_svg_x(pt(0) - origin(0)) + 20.f, to_svg_y(pt(1) - origin(1)), "black", text);
}
...
void SVG::add_comment(const std::string comment) { fprintf(this->f, "<!-- %s -->\n", comment.c_str()); }
...
void SVG::export_expolygons(const char*               path,
                            const BoundingBox&        bbox,
                            const Slic3r::ExPolygons& expolygons,
                            std::string               stroke_outer,
                            std::string               stroke_holes,
                            ...
                            const std::vector<std::pair<Slic3r::ExPolygons, ExPolygonAttributes>>& expolygons_with_attributes)
{
    ...
    size_t num_legend = std::count_if(expolygons_with_attributes.begin(), expolygons_with_attributes.end(),
                                      [](const auto& v) { return !v.second.legend.empty(); });
