// [INTENT] PlaceholderParser: expands template strings used in start/end/toolchange G-code.
// Templates contain [variable_name] (simple substitution) and {expression} (full expression
// language: arithmetic, comparison, if/else, for/endfor, while/endwhile, repeat, assignment).
// Used from GCode.cpp for every custom G-code block and from the UI preview engine.
//
// [STATE] Two main data stores:
//   m_config          — DynamicConfig holding all resolved print/printer/filament options
//   m_external_config — optional override config (checked first on all symbol lookups)
// The parser/evaluator is a Boost.Spirit Qi grammar (qi::grammar) built in the
// client::macro_processor struct. Grammar compilation happens at construction time (static).
//
// [COUPLING] Closely coupled to DynamicPrintConfig (reads all option types via ConfigOption
// polymorphism), Flow (for FloatOrPercent "ratio_over" dependency resolution), and the
// GCode module which calls process() for every custom G-code block per layer/toolchange.
//
// [HAZARD] iso8859_1 encoding workaround: boost::spirit::ascii crashes on negative char values
// for UTF-8 sequences. The workaround uses iso8859_1 instead of UTF-8 — non-ASCII characters
// in template strings may produce incorrect token boundaries or garbled output for locales
// outside Latin-1 range. The FIXME comment at line 172 acknowledges this.
//
// [HAZARD] ratio_over resolution loop (line 1059): resolves FloatOrPercent dependencies
// by following the "ratio_over" chain. The loop is unbounded — a circular "ratio_over"
// dependency in PrintConfigDef would cause an infinite loop. No cycle detection exists.
//
// [HAZARD] apply_env_variables() reads `environ` at construction time for all SLIC3R_*
// environment variables. On macOS, this uses _NSGetEnviron(); on Windows, _environ.
// If the environment changes between construction and use (rare but possible for
// fork/exec scenarios), the cached env values are stale.
...
// [INTENT] Constructor: seeds the parser's symbol table with version, environment variables,
// timestamp, and username. These are available as [version], [timestamp], etc. in templates.
...
{
    const char* user = boost::nowide::getenv("USER")     ? boost::nowide::getenv("USER") :
                       boost::nowide::getenv("USERNAME") ? boost::nowide::getenv("USERNAME") :
                                                           "unknown";
...
void PlaceholderParser::apply_config(DynamicPrintConfig&& rhs) { m_config += std::move(rhs); }
...
// Don't use boost::spirit::ascii, as it crashes internally due to indexing with negative char values for UTF8 characters into some 7bit
...
// [INTENT] expr: dynamically-typed value node used throughout the Boost.Spirit parse tree.
// Carries one of TYPE_EMPTY|TYPE_BOOL|TYPE_INT|TYPE_DOUBLE|TYPE_STRING.
// TYPE_EMPTY is the "skip" sentinel — when the evaluator is inside a suppressed branch
// (if/else block that should not execute), all operations short-circuit when lhs.type()==TYPE_EMPTY.
// [MEMORY] The string case (TYPE_STRING) owns a heap-allocated std::string* in the union Data.
// All constructors/assignment operators/destructor are manually managed — no RAII.
// A port must implement deep-copy, move, and explicit deallocation semantics or use std::variant.
// [HAZARD] The union Data uses memcpy to copy non-string members (void set(const Data& rhs)).
// This is undefined behaviour for non-trivially-copyable types, but works in practice here
// because bool/int/double are trivially copyable and the string case is special-cased.
struct expr
{
    expr() {}
    ...
    explicit expr(bool b, const Iterator& it_begin, const Iterator& it_end) : m_type(TYPE_BOOL), it_range(it_begin, it_end)
    {
        m_data.b = b;
    }
    ...
    explicit expr(double d, const Iterator& it_begin, const Iterator& it_end) : m_type(TYPE_DOUBLE), it_range(it_begin, it_end)
    {
        m_data.d = d;
    }
    ...
    explicit expr(const std::string& s, const Iterator& it_begin, const Iterator& it_end) : m_type(TYPE_STRING), it_range(it_begin, it_end)
    {
    ...
    expr unary_minus(const Iterator start_pos) const
    {
        ...
        case TYPE_EMPTY:
            // Inside an if / else block to be skipped.
            return expr();
        case TYPE_INT: return expr(-this->i(), start_pos, this->it_range.end());
        case TYPE_DOUBLE: return expr(-this->d(), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot apply unary minus operator.");
        }
        assert(false);
    ...
    expr unary_integer(const Iterator start_pos) const
    {
        ...
        case TYPE_EMPTY:
            ...
            return expr();
        case TYPE_INT: return expr(this->i(), start_pos, this->it_range.end());
        case TYPE_DOUBLE: return expr(static_cast<int>(this->d()), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot convert to integer.");
        }
        assert(false);
        // Suppress compiler warnings.
        return expr();
    ...
    expr round(const Iterator start_pos) const
    {
        ...
        case TYPE_EMPTY:
            ...
            return expr();
        case TYPE_INT: return expr(this->i(), start_pos, this->it_range.end());
        case TYPE_DOUBLE: return expr(static_cast<int>(std::round(this->d())), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot round a non-numeric value.");
        }
        assert(false);
        // Suppress compiler warnings.
        return expr();
    ...
    expr floor(const Iterator start_pos) const
    {
        switch (this->type()) {
        case TYPE_INT: return expr(this->i(), start_pos, this->it_range.end());
        case TYPE_DOUBLE: return expr(static_cast<int>(std::floor(this->d())), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot floor a non-numeric value.");
        }
        assert(false);
        // Suppress compiler warnings.
        return expr();
    ...
    expr ceil(const Iterator start_pos) const
    {
        switch (this->type()) {
        case TYPE_INT: return expr(this->i(), start_pos, this->it_range.end());
        case TYPE_DOUBLE: return expr(static_cast<int>(std::ceil(this->d())), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot ceil a non-numeric value.");
        }
        assert(false);
        // Suppress compiler warnings.
        return expr();
    ...
    expr unary_not(const Iterator start_pos) const
    {
        ...
        case TYPE_EMPTY:
            // Inside an if / else block to be skipped.
            return expr();
        case TYPE_BOOL: return expr(!this->b(), start_pos, this->it_range.end());
        default: this->throw_exception("Cannot apply a not operator.");
        }
        assert(false);
        // Suppress compiler warnings.
        return expr();
    ...
    static void compare_op(expr& lhs, expr& rhs, char op, bool invert)
    {
        ...
        if (lhs.numeric_type() && rhs.numeric_type()) {
            ...
            switch (op) {
            case '=':
                value = (lhs.type() == TYPE_DOUBLE || rhs.type() == TYPE_DOUBLE) ? (std::abs(lhs.as_d() - rhs.as_d()) < 1e-8) :
                                                                                   (lhs.i() == rhs.i());
                break;
            case '<':
                value = (lhs.type() == TYPE_DOUBLE || rhs.type() == TYPE_DOUBLE) ? (lhs.as_d() < rhs.as_d()) : (lhs.i() < rhs.i());
                break;
            case '>':
            default:
                value = (lhs.type() == TYPE_DOUBLE || rhs.type() == TYPE_DOUBLE) ? (lhs.as_d() > rhs.as_d()) : (lhs.i() > rhs.i());
            ...
            // Both type are bool.
            if (op != '=')
                boost::throw_exception(qi::expectation_failure<Iterator>(lhs.it_range.begin(), rhs.it_range.end(),
                                                                         spirit::info("*Cannot compare the types.")));
            value = lhs.b() == rhs.b();
            ...
            boost::throw_exception(
                qi::expectation_failure<Iterator>(lhs.it_range.begin(), rhs.it_range.end(), spirit::info("*Cannot compare the types.")));
        }
        lhs.reset();
    ...
    // Store the result into param1.
    // param3 is optional
    template<bool leading_zeros> static void digits(expr& param1, expr& param2, expr& param3)
    {
        if (param1.type() == TYPE_EMPTY)
            // Inside an if / else block to be skipped
            return;
    ...
    static void regex_op(const expr& lhs, IteratorRange& rhs, char op, expr& out)
    {
        ...
        try {
            ...
            boost::throw_exception(
                qi::expectation_failure<Iterator>(rhs.begin(), rhs.end(),
                                                  spirit::info(std::string("*Regular expression compilation failed: ") + ex.what())));
        }
    }
    ...
    static void                      one_of_test_init(expr& out) { out.set_b(false); }
    template<bool RegEx> static void one_of_test(const expr& match, const expr& pattern, expr& out)
    {
    ...
    static void logical_op(expr& lhs, expr& rhs, char op)
    {
        ...
        if (lhs.type() == TYPE_BOOL && rhs.type() == TYPE_BOOL) {
            ...
            boost::throw_exception(
                qi::expectation_failure<Iterator>(lhs.it_range.begin(), rhs.it_range.end(),
                                                  spirit::info("*Cannot apply logical operation to non-boolean operators.")));
        }
    ...
    {
        boost::throw_exception(
            qi::expectation_failure<Iterator>(this->it_range.begin(), this->it_range.end(), spirit::info(std::string("*") + message)));
    }
...
// [INTENT] MyContext: the runtime evaluation context passed through every rule action.
// Holds all config sources (external_config, config, config_override, config_outputs),
// local and global variable stores, extruder index, RNG, and the conditional skip depth.
// [STATE] m_depth_suppressed > 0 means the evaluator is inside a skipped conditional branch.
// All action functions check ctx->skipping() first and return early to skip side effects.
// [COUPLING] Inherits ConfigOptionResolver — the grammar resolves variable names by calling
// optptr() which chains external_config → config → config_override lookups.
// [CONCURRENCY] MyContext is stack-allocated per process() call, not shared across threads.
struct MyContext : public ConfigOptionResolver
{
    ...
    static void legacy_variable_expansion2(const MyContext* ctx,
    ...
    // [INTENT] scalar_variable_to_expr: maps each ConfigOption scalar type to an expr value.
    // FloatOrPercent handling is the most complex case: follows the ratio_over dependency chain
    // (potentially multiple levels) to resolve percent-relative values to absolute floats.
    // [HAZARD] The for(;;) loop at line ~1120 has no cycle detection — a circular ratio_over chain
    // in PrintConfigDef would produce an infinite loop. See file-header [HAZARD] note.
    static void scalar_variable_to_expr(const MyContext* ctx, OptWithPos& opt, expr& output)
    {
            ...
            if (boost::ends_with(opt_key, "line_width")) {
                ...
                for (;;) {
                    ...
                    if (boost::ends_with(opt_def->ratio_over, "line_width")) {
                        ...
                        v *= Flow::extrusion_width(opt_def->ratio_over, static_cast<const ConfigOptionFloatOrPercent*>(opt_parent), *ctx,
        ...
        default: ctx->throw_exception("Unsupported scalar variable type", opt.it_range);
        }
    }
    ...
    static void vector_element_to_expr(const MyContext* ctx, OptWithPos& opt, expr& output)
    {
        ...
        if (!opt.has_index()) {
            ...
            default: ctx->throw_exception("Unsupported vector variable type", opt.it_range);
            }
        }
    }
    ...
    static void scalar_variable_assign_scalar(const MyContext* ctx, OptWithPos& lhs, const expr& rhs)
    {
        ...
        case coInt:
            ...
            break;
        case coString: static_cast<ConfigOptionString*>(wropt)->value = rhs.to_string(); break;
        case coPercent:
            check_numeric(rhs);
        ...
        case coBool:
            ...
            break;
        default: ctx->throw_exception("Unsupported output scalar variable type", lhs.it_range);
        }
    }
    ...
    static void vector_variable_element_assign_scalar(const MyContext* ctx, OptWithPos& lhs, const expr& rhs)
    {
        ...
        case coInts:
            ...
            break;
        case coStrings: static_cast<ConfigOptionStrings*>(vec)->values[lhs.index] = rhs.to_string(); break;
        case coPercents:
            check_numeric(rhs);
        ...
        case coBools:
            ...
            break;
        default: ctx->throw_exception("Unsupported output vector variable type", lhs.it_range);
        }
    }
    ...
    static void vector_variable_assign_expr_with_count(const MyContext* ctx, OptWithPos& lhs, const expr& rhs_count, const expr& rhs_value)
    {
        ...
        case coInts:
            ...
            break;
        case coStrings: static_cast<ConfigOptionStrings*>(opt)->values.assign(count, rhs_value.to_string()); break;
        case coBools:
    ...
    static void vector_variable_new_from_array(
        const MyContext* ctx, bool global_variable, NewOldVariable& lhs, const expr& rhs_count, const expr& rhs_value)
    {
        if (ctx->skipping()) {
        } else if (lhs.opt) {
    ...
    static void vector_variable_assign_initializer_list(const MyContext* ctx, OptWithPos& lhs, const std::vector<expr>& il)
    {
        ...
        case coInts:
            ...
            break;
        case coStrings: fill_vector_from_initializer_list<ConfigOptionStrings>(opt, il, [](auto& v) { return v.to_string(); }); break;
        case coBools:
            for (auto& i : il)
    ...
    static void vector_variable_new_from_initializer_list(const MyContext*         ctx,
                                                          ...
                                                          const std::vector<expr>& il)
    {
        ...
        if (lhs.opt) {
            ...
            if (il.empty())
                ctx->throw_exception("Cannot create vector variable from an empty initializer list, because its type cannot be deduced.",
                                     lhs.it_range);
            ...
            if (num_string > 0)
                ...
                if (num_double + num_int > 0)
                    ctx->throw_exception("Right side is not valid: Mixing numeric and boolean types.",
                                         IteratorRange{il.front().it_range.begin(), il.back().it_range.end()});
    ...
    static bool is_vector_variable_reference(const OptWithPos& var) { return !var.empty() && !var.has_index() && var.opt->is_vector(); }

    // Called when checking whether the NewOldVariable could be assigned a vectir right hand side.
    static bool could_be_vector_variable_reference(const NewOldVariable& var) { return var.opt == nullptr || var.opt->is_vector(); }
    ...
    static void copy_vector_variable_to_vector_variable(const MyContext* ctx, OptWithPos& lhs, const OptWithPos& rhs)
    {
        ...
        if (rhs.opt->is_nil())
            ctx->throw_exception("Some elements of the right hand side vector variable of optional values are undefined (nil)",
                                 rhs.it_range);
        if (lhs.opt->type() != rhs.opt->type()) {
            // Vector types are not compatible.
            switch (lhs.opt->type()) {
            case coFloats: ctx->throw_exception("Left hand side is a float vector, while the right hand side is not.", lhs.it_range);
            case coInts: ctx->throw_exception("Left hand side is an int vector, while the right hand side is not.", lhs.it_range);
            case coStrings: ctx->throw_exception("Left hand side is a string vector, while the right hand side is not.", lhs.it_range);
            case coBools: ctx->throw_exception("Left hand side is a bool vector, while the right hand side is not.", lhs.it_range);
            default: ctx->throw_exception("Left hand side / right hand side vectors are not compatible.", lhs.it_range);
            }
        }
    ...
    static void process_error_message(const MyContext*           context,
...
struct InterpolateTableContext
{
    ...
    static void evaluate(const expr& expr_x, const InterpolateTableContext& table, expr& out)
    {
        ...
        for (size_t i = 1; i < table.table.size(); ++i) {
            ...
            if (x0 > x1)
                boost::throw_exception(qi::expectation_failure(table.table[i - 1].it_range_x.begin(), table.table[i].it_range_x.end(),
                                                               spirit::info("X coordinates of the table must be increasing")));
...
struct utf8_char_parser : qi::primitive_parser<utf8_char_parser>
{
    ...
    template<typename Context> spirit::info what(Context&) const { return spirit::info("unicode_char"); }
};
...
struct ascii_char_skipper_parser : public utf8_char_parser
{
    ...
    bool parse(Iterator& first, Iterator const& last, Context& context, Skipper const& skipper, Attribute& attr) const
    {
        ...
        char c = *first;
        if (it - first > 1 || c < 0)
            MyContext::throw_exception(
                "Non-ASCII7 characters are only allowed inside text blocks and string literals, not inside code blocks.",
                IteratorRange(first, it));
        if (c == '\r' || c == '\n' || c == '\t' || c == ' ') {
    ...
    // This function is called during error handling to create a human readable string for the error context.
    template<typename Context> spirit::info what(Context&) const { return spirit::info("ASCII7_char"); }
};

struct FactorActions
{
    static void set_start_pos(Iterator& start_pos, expr& out) { out.it_range = IteratorRange(start_pos, start_pos); }
    ...
    {
        auto begin_pos = out.it_range.begin();
        out            = expr(std::move(value), begin_pos, end_pos);
    }
    static void minus_(expr& value, expr& out) { out = value.unary_minus(out.it_range.begin()); }
    static void not_(expr& value, expr& out) { out = value.unary_not(out.it_range.begin()); }
    static void to_int(expr& value, expr& out) { out = value.unary_integer(out.it_range.begin()); }
    static void round(expr& value, expr& out) { out = value.round(out.it_range.begin()); }
    static void floor(expr& value, expr& out) { out = value.floor(out.it_range.begin()); }
    static void ceil(expr& value, expr& out) { out = value.ceil(out.it_range.begin()); }
...
// [INTENT] macro_processor: the Boost.Spirit Qi grammar that parses and evaluates OrcaSlicer
// template strings. The grammar is compiled once at construction (static instance) and reused.
// Grammar entry point is `start`, which dispatches to `text_block` (full macro mode) or
// `conditional_expression` (boolean-only mode, used by evaluate_boolean_expression()).
// [STATE] The grammar object is stateless after construction — all mutable state lives in
// MyContext, which is passed as an inherited attribute (_r1) through every rule.
// [COUPLING] All semantic actions are static member functions of MyContext or FactorActions,
// bound via boost::phoenix px::bind. The grammar cannot be ported without replacing the entire
// Boost.Spirit machinery with a custom parser or external grammar generator.
// [HAZARD] The grammar is declared as a file-scope static (g_macro_processor_instance, line ~2377).
// Boost.Spirit grammar construction is not always thread-safe for all target platforms — if
// multiple threads first call process() simultaneously before the static is initialized,
// there is a potential race on static initialization (C++11 guarantees thread-safe static init,
// but Boost.Spirit internally uses non-thread-safe tables in some versions).
struct macro_processor : qi::grammar<Iterator, std::string(const MyContext*), qi::locals<bool>, skipper>
{
    macro_processor() : macro_processor::base_type(start)
    {
        ...
        start = ((eps(px::bind(&MyContext::evaluate_full_macro, _r1)) > text_block(_r1)[_val = _1]) |
                 conditional_expression(_r1)[px::bind(&expr::evaluate_boolean_to_string, _1, _val)]) > eoi;
        start.name("start");
        qi::on_error<qi::fail>(start, px::bind(&MyContext::process_error_message, _r1, _4, _1, _2, _3));

        text_block = *(text[_val += _1]
        ...
        macros = +(block(_r1)[_val += _1] | (statement(_r1) > (+lit(';') | &lit('}')))[_val += _1] | +lit(';'));
        macros.name("macro");
        ...
        if_macros = kw["then"] > *(block(_r1)[_val += _1] |
                                   (statement(_r1) > (+lit(';') | &(kw["elsif"] | kw["else"] | kw["endif"])))[_val += _1] | +lit(';'));
        ...
        block = (kw["if"] > if_else_output(_r1)[_val = _1])
            // (kw["switch"] ...
            ;
        block.name("block");

        // Statements require a separating semicolon.
        statement = (assignment_statement(_r1)[_val = _1]) | (new_variable_statement(_r1)[_val = _1]) |
        ...
        if_else_output = eps[_a = true] > (bool_expr_eval(_r1)[px::bind(&MyContext::block_enter, _r1, _1)] >
                                           (if_text_block(_r1) | if_macros(_r1)))[px::bind(&MyContext::block_exit, _r1, _1, _a, _2, _val)] >
                         *((kw["elsif"] > bool_expr_eval(_r1)[px::bind(&MyContext::block_enter, _r1, _1&& _a)] >
                            (if_text_block(_r1) | if_macros(_r1)))[px::bind(&MyContext::block_exit, _r1, _1, _a, _2, _val)]) >
                         -(kw["else"] > eps[px::bind(&MyContext::block_enter, _r1, _a)] >
                           (if_text_block(_r1) | else_macros(_r1)))[px::bind(&MyContext::block_exit, _r1, _a, _a, _1, _val)] > kw["endif"];
        ...
        */
        ...
        legacy_variable_expansion = (identifier >> &lit(']'))[px::bind(&MyContext::legacy_variable_expansion, _r1, _1, _val)] |
        ...
        identifier = !kw[keywords] >> raw[lexeme[(alpha | '_') >> *(alnum | '_')]];
        identifier.name("identifier");

        conditional_expression = logical_or_expression(_r1)[_val = _1] >>
                                 -('?' > eps[px::bind(&expr::evaluate_boolean, _val, _a)] > eps[px::bind(&MyContext::block_enter, _r1, _a)] >
                                   conditional_expression(_r1)[px::bind(&MyContext::block_exit_ternary, _r1, _a, _1, _val)] > ':' >
                                   eps[px::bind(&MyContext::block_enter, _r1, !_a)] >
                                   conditional_expression(_r1)[px::bind(&MyContext::block_exit_ternary, _r1, !_a, _1, _val)]);
        conditional_expression.name("conditional_expression");

        logical_or_expression = logical_and_expression(_r1)[_val = _1] >>
                                *(((kw["or"] | "||") > logical_and_expression(_r1))[px::bind(&expr::logical_or, _val, _1)]);
        logical_or_expression.name("logical_or_expression");

        logical_and_expression = equality_expression(_r1)[_val = _1] >>
                                 *(((kw["and"] | "&&") > equality_expression(_r1))[px::bind(&expr::logical_and, _val, _1)]);
        logical_and_expression.name("logical_and_expression");

        equality_expression = relational_expression(_r1)[_val = _1] >>
        ...
        relational_expression = additive_expression(_r1)[_val = _1] >>
        ...
        additive_expression = multiplicative_expression(_r1)[_val = _1] >> *((lit('+') > multiplicative_expression(_r1))[_val += _1] |
                                                                             (lit('-') > multiplicative_expression(_r1))[_val -= _1]);
        additive_expression.name("additive_expression");

        multiplicative_expression = unary_expression(_r1)[_val = _1] >>
                                    *((lit('*') > unary_expression(_r1))[_val *= _1] | (lit('/') > unary_expression(_r1))[_val /= _1] |
                                      (lit('%') > unary_expression(_r1))[_val %= _1]);
        multiplicative_expression.name("multiplicative_expression");

        assignment_statement =
            (variable_reference(_r1)[_a = _1] >> '=') >
            ( // Consumes also '(' conditional_expression ')', that means enclosing an expression into braces makes it a single value vector
              // initializer.
                initializer_list(_r1)[px::bind(&MyContext::vector_variable_assign_initializer_list, _r1, _a, _1)]
                // Process it before conditional_expression, as conditional_expression requires a vector reference to be augmented with an
                // index. Only process such variable references, which return a naked vector variable.
                ...
                | conditional_expression(_r1)[px::bind(&MyContext::scalar_variable_assign_scalar_expression, _r1, _a, _1)] |
        ...
        new_variable_statement =
            (kw["local"][_a = false] | kw["global"][_a = true]) > identifier[px::bind(&MyContext::new_old_variable, _r1, _a, _1, _b)] >
            lit('=') >
            ( // Consumes also '(' conditional_expression ')', that means enclosing an expression into braces makes it a single value vector
              // initializer.
                ...
                // Would NOT consume '(' conditional_expression ')' because such value was consumed with the expression above.
                | conditional_expression(_r1)[px::bind(&MyContext::scalar_variable_new_from_scalar_expression, _r1, _a, _b, _1)] |
                (kw["repeat"] > "(" > additive_expression(_r1) > "," > conditional_expression(_r1) >
        ...
        unary_expression = iter_pos[px::bind(&FactorActions::set_start_pos, _1, _val)] >>
                           (variable_reference(_r1)[px::bind(&MyContext::variable_value, _r1, _1, _val)] |
                            ...
                            (kw["filament_change"] > '(' > conditional_expression(_r1) >
                             ')')[px::bind(&MyContext::filament_change, _r1, _1)] |
                            (kw["digits"] > '(' > conditional_expression(_r1)[_val = _1] > ',' > conditional_expression(_r1) >
                             optional_parameter(_r1))[px::bind(&expr::digits<false>, _val, _2, _3)] |
                            (kw["zdigits"] > '(' > conditional_expression(_r1)[_val = _1] > ',' > conditional_expression(_r1) >
                             optional_parameter(_r1))[px::bind(&expr::digits<true>, _val, _2, _3)] |
        ...
        one_of.name("one_of");
        one_of_list = eps[px::bind(&expr::one_of_test_init, _val)] >
        ...
        interpolate_table = (unary_expression(_r1)[_a = _1] > ',' >
                             interpolate_table_list(_r1, _a))[px::bind(&InterpolateTableContext::evaluate, _a, _2, _val)];
        interpolate_table.name("interpolate_table");
        interpolate_table_list = eps[px::bind(&InterpolateTableContext::init, _r2)] >
                                 (*((lit('(') > unary_expression(_r1) > ',' > unary_expression(_r1) >
                                     ')')[px::bind(&InterpolateTableContext::add_pair, _1, _2, _val)] >>
                                    -lit(',')));
        interpolate_table.name("interpolate_table_list");

        optional_parameter = iter_pos[px::bind(&FactorActions::set_start_pos, _1, _val)] >>
                             (lit(')')[px::bind(&FactorActions::noexpr, _val)] | (lit(',') > conditional_expression(_r1) > ')')[_val = _1]);
        optional_parameter.name("optional_parameter");

        variable_reference = variable(_r1)[_a = _1] >>
                             (('[' > additive_expression(_r1)[px::bind(&MyContext::evaluate_index, _1, _b)] > ']' >
                               iter_pos)[px::bind(&MyContext::store_variable_index, _r1, _a, _b, _2, _val)] |
                              eps[_val = _a]);
        ...
        keywords.add("and")("digits")("zdigits")("empty")("if")("int")("is_nil")("local")
            //("inf")
            ("else")("elsif")("endif")("false")("global")("interpolate_table")("min")("max")("random")("filament_change")("repeat")(
                "round")("floor")("ceil")("not")("one_of")("or")("size")("true");
...
// [INTENT] process_macro: thin wrapper that calls Boost.Spirit phrase_parse() with the
// static grammar instance and MyContext. Throws PlaceholderParserError on any parse/runtime error.
// The static g_macro_processor_instance is initialized once at startup (file-scope static).
...
// [INTENT] process(): public entry point for template expansion. Assembles MyContext from
// the stored config, optional override config, optional output config, and extruder id.
// Called from GCode.cpp for every custom G-code block (start/end/toolchange/layer change).
// [CONCURRENCY] Fully re-entrant: all state is in local MyContext. Multiple threads can call
// process() simultaneously on the same PlaceholderParser instance without data races,
// EXCEPT for the static g_macro_processor_instance initialization (see above).
std::string PlaceholderParser::process(const std::string&   templ,
...
bool PlaceholderParser::evaluate_boolean_expression(const std::string&   templ,
