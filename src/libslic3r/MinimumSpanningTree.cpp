// [INTENT] Prim's algorithm MST on a set of 2D integer-coordinate points. Used by the
//          Arachne perimeter generator to order beads and connections along skeletal paths.
// [STATE]  Result stored as an unordered_map keyed by Point value (not pointer). Thread-safe
//          if different instances are used per thread.
// [COUPLING] Depends on libslic3r.h for coord types. No geometry algorithms beyond distance.
// [HAZARD H1169] smallest_distance and smallest_distance_to are keyed by `const Point*` —
//               raw pointers into `vertices_list` (local vector). If vertices_list is ever
//               reallocated these keys become dangling. As written, vertices_list is built
//               once and never modified, so this is safe — but fragile under refactoring.
// [HAZARD H1170] prim() is O(V²): inner loop over smallest_distance for each vertex is O(V).
//               The in-code TODO acknowledges this and notes a heap-based O(V log E) version
//               is possible. For large bed-fill patterns with hundreds of connection points
...
inline double dot_with_unscale(const Point a, const Point b) { return unscale_(a(0)) * unscale_(b(0)) + unscale_(a(1)) * unscale_(b(1)); }
...
auto MinimumSpanningTree::prim(std::vector<Point> vertices) const -> AdjacencyGraph_t
{
    ...
    while (result.size() < vertices_list.size()) // All of the vertices need to be in the tree at the end.
    {
        ...
        // This search is O(V) right now, which can be made down to O(log(V)). This reduces the overall time complexity from O(V*V) to
        // O(V*log(E)). However that requires an implementation of a heap that supports the decreaseKey operation, which is not in the std
        // library.
        // TODO: Implement this?
        ...
        const auto closest = std::min_element(smallest_distance.begin(), smallest_distance.end(),
                                              [](const MapValue& a, const MapValue& b) { return a.second < b.second; });
...
std::vector<Point> MinimumSpanningTree::leaves() const
{
    ...
    for (std::pair<Point, std::vector<Edge>> node : adjacency_graph) {
        if (node.second.size() <=
            1) // Leaves are nodes that have only one adjacent edge, or just the one node if the tree contains one node.
        {
