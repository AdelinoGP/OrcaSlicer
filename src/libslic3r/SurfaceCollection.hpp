// [INTENT] SurfaceCollection — ordered bag of Surface objects for a single LayerRegion.
//          Provides typed filter/keep/remove operations used by PrintObject slicing,
//          perimeter generation, and fill generation stages.
//
// [STATE]  surfaces: public std::vector<Surface>.  No mutex; instances are owned and
//          mutated exclusively by the thread processing their parent LayerRegion.
//
// [COUPLING] filter_by_type() returns SurfacesPtr (raw const pointers into surfaces).
//            Callers must NOT mutate or reallocate the surfaces vector while holding
//            a SurfacesPtr — the pointers will dangle.
//
// [MEMORY] All mutation methods (keep_type/remove_type) use a stable in-place partition
//          + erase pattern to avoid copying; O(n) time, O(1) extra space.
...
public:
    ...
    template<class T> bool any_internal_contains(const T& item) const
    {
        for (const Surface& surface : this->surfaces)
            if (surface.is_internal() && surface.expolygon.contains(item))
                return true;
        return false;
    }
    ...
    {
        for (const Surface& surface : this->surfaces)
            if (surface.is_bottom() && surface.expolygon.contains(item))
                return true;
        return false;
    }
    ...
    {
        clear();
        this->append(std::move(src), surfaceType);
    }
    void set(ExPolygons&& src, const Surface& surfaceTempl)
    {
        clear();
        this->append(std::move(src), surfaceTempl);
    }
    void set(Surfaces&& src)
    {
        clear();
        this->append(std::move(src));
    }
