...
{
public:
    // [INTENT] Strong wrappers prevent accidental arithmetic with plain ints at callsites
    // and make version bump intent explicit (major/minor/patch).
    struct Major
    {
        const int i;
        Major(int i) : i(i) {}
    ...
    Semver(int major, int minor, int patch, const char* metadata = nullptr, const char* prerelease = nullptr) : ver(semver_zero())
    {
        ver.major = major;
        ver.minor = minor;
        ver.patch = patch;
    ...
    {
        // [COUPLING] Delegates semantic-version grammar and normalization to external `semver` C library.
    ...
    {
        static semver_t ver = {std::numeric_limits<int>::max(), std::numeric_limits<int>::max(), std::numeric_limits<int>::max(), nullptr,
                               nullptr};
        return Semver(ver);
    }
    ...
    // [MEMORY] Deep-copy constructor duplicates semver-owned C strings to preserve value semantics.
    Semver(const Semver& other) : ver(::semver_copy(&other.ver)) {}

    Semver& operator=(Semver&& other)
    {
        // [MEMORY] Release existing C-allocated metadata/prerelease before stealing source payload.
        ::semver_free(&ver);
        ver       = other.ver;
    ...
    {
        // [MEMORY] Copy-assignment performs deep copy through semver_copy, preserving independent ownership.
        ::semver_free(&ver);
        ver = ::semver_copy(&other.ver);
        return *this;
    }

    // [MEMORY] RAII guard for C-library allocations inside semver_t.
    ...
    {
        // [HAZARD] Uses C `free`/`strdup` ownership conventions; mixing allocators across libraries would be unsafe.
        if (ver.metadata)
            free(ver.metadata);
    ...
    {
        // BBS: version format
        //  [COUPLING] Bambu-specific patch split (patch/100 + patch%100) defines persisted/user-visible version contract.
        std::string res;
        int         patch_1 = ver.patch / 100;
        int         patch_2 = ver.patch % 100;
        res                 = (boost::format("%1%.%2%.%3%.%4%") % ver.major % ver.minor % patch_1 % patch_2).str();

        if (ver.prerelease != nullptr) {
            res += '-';
            res += ver.prerelease;
        }
        if (ver.metadata != nullptr) {
            res += '+';
            res += ver.metadata;
        }
        return res;
...
private:
    // [STATE] Internal storage mirrors C semver_t and remains mutable through setters/operators.
    semver_t ver;
