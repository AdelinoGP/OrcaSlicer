// [INTENT] FillRectilinear.cpp — Core rectilinear/monotonic/zigzag/crosshatch infill engine.
// This file implements the most complex infill algorithms in OrcaSlicer. Key patterns:
//   - FillRectilinear / FillAlignedRectilinear / FillMonotonic / FillMonotonicLines
//   - FillGrid, FillTriangles, FillStars, FillCubic, FillQuarterCubic
//   - FillZigZag, FillCrossZag, FillLockedZag (Orca additions)
//
// [INTENT] Core algorithm overview (rectilinear family):
//   1. Rotate the polygon so that fill lines become vertical.
//   2. Build ExPolygonWithOffset: two offset levels of the input polygon (outer = perimeter join,
//      inner = actual infill boundary). These double-offset polygons are used to generate
//      linking segments between infill lines without re-crossing perimeters.
//   3. slice_region_by_vertical_lines(): cast N vertical scan lines through the offset polygon,
//      computing rational intersection y-coordinates (pos_p/pos_q) with each polygon edge.
//      Store as SegmentIntersection records sorted by y.
//   4. connect_segment_intersections_by_contours(): link adjacent intersection points via
//      perimeter walks (horizontal links) and vertical runs within the same vertical line.
//      Links are classified as Valid, Invalid, or TooLong.
//   5a. (monotonic mode) generate_montonous_regions() → connect_monotonic_regions() →
//       chain_monotonic_regions() → polylines_from_paths(): Groups vertical segments into
//       monotonic "regions" (all segments can be connected left-to-right in the same direction).
//       The chaining uses a distance matrix (MonotonicRegionLink) optimised via Held-Karp-style
//       nearest-neighbour or brute-force (≤8 regions).
//   5b. (normal mode) traverse_graph_generate_polylines(): greedy graph traversal emitting
//       polylines by following horizontal/vertical links.
//   6. Rotate all output polylines back by the original rotation angle.
//
// [STATE] All output appended to the caller-provided polylines_out vector (output param).
// [COUPLING] Heavy dependency on ClipperUtils for polygon offsets; ExPolygon / Polygon types.
// [CONCURRENCY] No internal parallelism. Called from TBB threads in PrintObject::make_fills().
// [HAZARD] "monotonic" in the function name "generate_montonous_regions" is a TYPO in the codebase
//          (should be "monotonous" or "monotonic"). Do not "fix" during annotation pass.
...
// [INTENT] Compute the arc length along a polygon contour between two intersection points.
// Needed to decide which direction (CW or CCW) is shorter when walking the perimeter to link
// two consecutive infill line endpoints. The walking sum accumulates in floating-point (coordf_t)
// even though polygon coordinates are integer (coord_t), so small epsilon errors are possible.
// [HAZARD] Linear scan O(|seg2 - seg1|) — large polygons with many vertices and tightly spaced
// infill lines can make this hot. Called inside connect_segment_intersections_by_contours().
// [INTENT] segment_length(): Compute the arc length along a polygon contour walking forward from
// point p1 on segment seg1 to point p2 on segment seg2. Direction is always forward (increasing
// segment index, with wrap-around). Caller decides which direction is shorter by calling this
// twice (forward and reversed).
// [MEMORY] Pure read-only traversal of poly.points — no allocation.
// [HAZARD] O(|seg2 - seg1|) scan per call. Called inside connect_segment_intersections_by_contours()
//          for every intersection point × 2 directions — can be O(P * N) total where P = polygon
//          vertices and N = intersection count. Dense infill on complex polygons may be slow.
...
// [INTENT] polygon_segment_append(): Emit all polygon vertices between seg1 and seg2 (exclusive of
// seg2) into an output Points vector. Used when walking the perimeter forward to connect two
// infill intersection points without re-crossing an infill line.
// [HAZARD] If seg1 == seg2 nothing is emitted (documented — caller must handle this case).
...
// Only insert intermediate points between seg1 and seg2.
// [INTENT] polygon_segment_append_reversed(): Same as polygon_segment_append() but walks the
// contour in the backward (CCW→CW) direction. Used when a perimeter segment must be traversed
// in reverse to connect infill endpoints.
// [HAZARD] The comment "it could be, that seg1 == seg2. In that case, append the complete loop."
//          is misleading: when seg1 < seg2 we fall into the else branch and emit the complete loop.
//          This is correct for the algorithm but surprising to readers.
...
// [STATE] SegmentIntersection — fundamental node of the scan-line graph.
// Each instance records where one vertical infill line crosses one polygon segment.
// Fields iContour / iSegment identify the polygon edge; pos_p / pos_q store the y-coordinate
// as an exact rational number (numerator int64 / denominator uint32) to avoid floating-point
// comparison errors during sort. The LinkType / LinkQuality enums encode connectivity to
// neighboring intersections (horizontal = across adjacent vline, Up/Down = along same vline).
// consumed_vertical_up / consumed_perimeter_right are mutation flags cleared during path traversal.
// [MEMORY] Tightly packed with bit-grouped booleans at the end (~40 bytes/instance).
// [HAZARD] pos_q must never be 0; asserted in operator< but not defensively in pos().
// [INTENT] operator< uses a 96-bit integer comparison trick (split 64-bit × 32-bit into two
// 64-bit products) to compare rationals pos_p/pos_q without overflow or FP loss.
...
struct SegmentIntersection
{
    ...
    int         vertical_up() const { return this->has_left_vertical_up() ? this->left_vertical_up() : this->right_vertical_up(); }
...
// [INTENT] ExPolygonWithOffset holds the source expolygon plus two Clipper-offset shells.
// polygons_outer (aoffset1) = perimeter-join boundary; polygons_inner (aoffset2) = actual infill
// region. The double-offset design gives the scan-line engine two different contour sets:
//   - OUTER_LOW / OUTER_HIGH intersections mark the outer boundary of an infill vertical segment.
//   - INNER_LOW / INNER_HIGH intersections mark the inner boundary, used for connecting adjacent
//     infill lines by walking along the perimeter contour.
// [COUPLING] Takes ExPolygon + angle + two offset coords; calls Clipper offset internally.
// [MEMORY] Stores up to 3 full polygon copies (src, outer, inner). For complex models this
// can be several MB per object layer per infill surface.
// [HAZARD] If remove_sticks() or remove_small() strip too many contours, n_contours_inner == 0
// and fill_surface_by_lines() returns early without emitting any infill — silent zero-fill.
...
{
public:
    ExPolygonWithOffset(const ExPolygon& expolygon,
                        float            angle,
                        coord_t          aoffset1,
    ...
    Polygon& contour(size_t idx) { return is_contour_outer(idx) ? polygons_outer[idx] : polygons_inner[idx - n_contours_outer]; }

    bool is_contour_ccw(size_t idx) const { return polygons_ccw[idx]; }

    BoundingBox bounding_box_src() const { return get_extents(polygons_src); }
    BoundingBox bounding_box_outer() const { return get_extents(polygons_outer); }
    BoundingBox bounding_box_inner() const { return get_extents(polygons_inner); }
...
static inline bool intersection_on_prev_next_vertical_line_valid(const std::vector<SegmentedIntersectionLine>& segs,
...
static inline bool intersection_on_prev_vertical_line_valid(const std::vector<SegmentedIntersectionLine>& segs,
                                                            size_t                                        iVerticalLine,
                                                            size_t                                        iIntersection)
{
...
static inline bool intersection_on_next_vertical_line_valid(const std::vector<SegmentedIntersectionLine>& segs,
                                                            size_t                                        iVerticalLine,
                                                            size_t                                        iIntersection)
{
...
static inline coordf_t measure_perimeter_horizontal_segment_length(const ExPolygonWithOffset&                    poly_with_offset,
                                                                   ...
                                                                   size_t                                        iIntersection2)
{
    ...
    return it.is_low() ? segment_length(poly, it.iSegment, p1, it2.iSegment, p2) : segment_length(poly, it2.iSegment, p2, it.iSegment, p1);
}
...
static inline void emit_perimeter_prev_next_segment(const ExPolygonWithOffset&                    poly_with_offset,
                                                    const std::vector<SegmentedIntersectionLine>& segs,
                                                    size_t                                        iVerticalLine,
...
static inline coordf_t measure_perimeter_segment_on_vertical_line_length(const ExPolygonWithOffset&                    poly_with_offset,
                                                                         const std::vector<SegmentedIntersectionLine>& segs,
                                                                         size_t                                        iVerticalLine,
                                                                         size_t                                        iIntersection,
                                                                         size_t                                        iIntersection2,
                                                                         bool                                          forward)
{
    ...
    return forward ? segment_length(poly, itsct.iSegment, p1, itsct2.iSegment, p2) :
...
// The first point (the point of iIntersection) will not be inserted,
// the last point will be inserted.
static inline void emit_perimeter_segment_on_vertical_line(const ExPolygonWithOffset&                    poly_with_offset,
                                                           const std::vector<SegmentedIntersectionLine>& segs,
                                                           size_t                                        iVerticalLine,
                                                           size_t                                        iInnerContour,
                                                           size_t                                        iIntersection,
...
enum DirectionMask { DIR_FORWARD = 1, DIR_BACKWARD = 2 };

// [INTENT] slice_region_by_vertical_lines() — the scan-line engine.
// Casts n_vlines equally-spaced vertical lines through all contours of poly_with_offset,
// records every intersection as a SegmentIntersection, sorts them by y (using the exact rational
// comparator), classifies them as OUTER_LOW/OUTER_HIGH/INNER_LOW/INNER_HIGH, and removes
// degenerate duplicates (tangent vertices, zero-height slivers).
// Output: vector<SegmentedIntersectionLine> — one entry per vertical line, sorted intersections.
// [HAZARD] Under INFILL_DEBUG_OUTPUT mode the validation block uses try/catch around
// ASSERT_THROW macros — on assertion failure it exports an SVG then re-throws. In production
// builds ASSERT_THROW is a plain assert + throw InfillFailedException, so geometry errors
// propagate to fill_surface_by_lines() which silently swallows them (returns true).
// [MEMORY] Intersections are accumulated per vline in a flat vector; no reallocation guard.
// Worst case: O(n_contour_points × n_vlines) intersection objects in one call.
// [STATE] All pos_q values are set to 1 for vertex-exact hits, and to the horizontal span
// denominator otherwise — ensures the rational comparator is valid even for axis-touching points.
static std::vector<SegmentedIntersectionLine> slice_region_by_vertical_lines(const ExPolygonWithOffset& poly_with_offset,
                                                                             size_t                     n_vlines,
                                                                             coord_t                    x0,
        ...
        for (size_t i = 0; i < sil.intersections.size(); ++i) {
            ...
            if (j > 0) {
                ...
                if (iContour == is2.iContour && is.pos_q == 1 && is2.pos_q == 1) {
                    // Two successive intersection points on a vertical line with the same contour, both points are end points of their
                    // respective contour segments.
    ...
    do { \
        ...
        for (size_t i_seg = 0; i_seg < segs.size(); ++i_seg) {
            ...
            for (size_t i = 0; i < sil.intersections.size();) {
                ...
                ASSERT_THROW(sil.intersections[j].type == SegmentIntersection::INNER_LOW ||
...
#undef INFILL_DEBUG_ASSERT
        ...
        for (size_t i_seg = 0; i_seg < segs.size(); ++i_seg) {
            ...
            for (size_t i = 0; i < sil.intersections.size();) {
                ...
                if (!(sil.intersections[j].type == SegmentIntersection::INNER_LOW ||
                      sil.intersections[j].type == SegmentIntersection::OUTER_HIGH)) {
...
// [INTENT] connect_segment_intersections_by_contours() — graph edge builder.
// For every intersection point on every vertical line, finds the nearest preceding (prev) and
// succeeding (next) intersection on the same polygon contour, either on adjacent vertical lines
// (horizontal link) or on the same vertical line (Up/Down vertical link). Populates
// SegmentIntersection::{prev_on_contour, next_on_contour, *_type, *_quality}.
// [HAZARD] The inner search loops are O(n) per intersection point (explicitly noted with
// "FIXME this has O(n) time complexity" in the source). For surfaces with many inner contours
// (e.g. thin walls sliced with many offset layers) this becomes O(n²) total — a known bottleneck.
// [COUPLING] Reads FillParams::dont_connect() and link_max_length to downgrade Valid links to
// TooLong — this is how user-controlled "don't connect infill" propagates through the graph.
// [STATE] All LinkQuality::Invalid symmetry fixups are applied at the end of each vline loop to
// ensure backward/forward links agree — missing this step causes traversal bugs.
static void connect_segment_intersections_by_contours(const ExPolygonWithOffset&              poly_with_offset,
                                                      ...
                                                      const coord_t                           link_max_length)
{
    for (size_t i_vline = 0; i_vline < segs.size(); ++i_vline) {
        ...
        for (int i_intersection = 0; i_intersection < int(il.intersections.size()); ++i_intersection) {
            ...
            itsct.prev_on_contour_type = same_prev ? (iprev < i_intersection ? SegmentIntersection::LinkType::Down :
                                                                               SegmentIntersection::LinkType::Up) :
                                                     SegmentIntersection::LinkType::Horizontal;
            itsct.next_on_contour      = inext;
            itsct.next_on_contour_type = same_next ? (inext < i_intersection ? SegmentIntersection::LinkType::Down :
                                                                               SegmentIntersection::LinkType::Up) :
                                                     SegmentIntersection::LinkType::Horizontal;
            ...
            // If both iprev and inext are on this vline, then there must not be any intersection with the previous or next contour and we
            // will not trace this contour when generating infill.
            ...
            if (params.dont_connect()) {
                ...
                if (itsct.prev_on_contour_quality == SegmentIntersection::LinkQuality::Valid &&
                    (same_prev ? measure_perimeter_segment_on_vertical_line_length(poly_with_offset, segs, i_vline, iprev, i_intersection,
                                                                                   forward) :
                                 measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline - 1, iprev, i_intersection)) >
                        link_max_length)
                    itsct.prev_on_contour_quality = SegmentIntersection::LinkQuality::TooLong;
                if (itsct.next_on_contour_quality == SegmentIntersection::LinkQuality::Valid &&
                    (same_next ? measure_perimeter_segment_on_vertical_line_length(poly_with_offset, segs, i_vline, i_intersection, inext,
                                                                                   forward) :
                                 measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline, i_intersection, inext)) >
                        link_max_length)
                    itsct.next_on_contour_quality = SegmentIntersection::LinkQuality::TooLong;
            }
        }
...
// Find the last INNER_HIGH intersection starting with INNER_LOW, that is followed by OUTER_HIGH intersection, traversing vertical up
// contours if enabled. Such intersection shall always exist.
static const SegmentIntersection& end_of_vertical_run(const SegmentedIntersectionLine& il, const SegmentIntersection& start)
{
    ...
    for (;;) {
        ...
        if (up == -1 || (end->has_left_vertical_up() ? end->prev_on_contour_quality : end->next_on_contour_quality) !=
                            SegmentIntersection::LinkQuality::Valid)
            break;
...
// [INTENT] traverse_graph_generate_polylines() — greedy left-to-right graph traversal.
// Sweeps vertical lines left-to-right. At each unconsumed OUTER_LOW/OUTER_HIGH pair, starts a
// new polyline. From an INNER_HIGH or INNER_LOW end-point, attempts three connection strategies
// in priority order:
//   1) Horizontal: follow inner perimeter to adjacent left or right vline.
//   2) Vertical-outside: follow inner perimeter up/down within the same vline.
//   3) Terminate: emit current polyline, restart search.
// Used by all non-monotonic rectilinear fill variants (FillRectilinear, FillGrid, etc.).
// [STATE] Uses consumed_vertical_up and consumed_perimeter_right flags on SegmentIntersections
// to prevent re-traversal. The `goto found` is a C++ optimization to break multi-level loops.
// [COUPLING] consistent_pattern flag (used by ZigZag) forces alternating forward/backward passes
// per vline to maintain layer-to-layer pattern alignment.
// [HAZARD] The greedy take-shortest heuristic (dist_next < dist_prev) at try_connect may produce
// non-optimal polyline ordering but is deterministic and fast.
                ...
                if (!vline.intersections.empty()) {
                    ...
                    for (int i = 0; i < int(vline.intersections.size()); ++i) {
                        ...
                        if (intrsctn.is_outer()) {
                            ...
                            const bool consumed = intrsctn.is_low() ? intrsctn.consumed_vertical_up :
        ...
        if (try_connect) {
            ...
            if (intersection_horizontal_valid) {
                ...
                if (intersection_prev_valid && intersection_next_valid) {
                    ...
                    coordf_t dist_prev = measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline - 1, i_prev,
                                                                                     i_intersection);
                    coordf_t dist_next = measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline, i_intersection,
                                                                                     i_next);
                    take_next          = dist_next < dist_prev;
                }
                polyline_current->points.emplace_back(vline.pos, it->pos());
                emit_perimeter_prev_next_segment(poly_with_offset, segs, i_vline, it->iContour, i_intersection, take_next ? i_next : i_prev,
                                                 *polyline_current, take_next);
                // FIXME consume the left / right connecting segments at the other end of this line? Currently it is not critical because a
                // perimeter segment is not followed if the vertical segment at the other side has already been consumed.
            ...
            auto vertical_link_quality = (i_vertical == -1 || vline.intersections[i_vertical + (going_up ? 0 : -1)].consumed_vertical_up) ?
                                             SegmentIntersection::LinkQuality::Invalid :
            ...
            if (vertical_link_quality != SegmentIntersection::LinkQuality::Invalid) {
                ...
                if (vertical_link_quality == SegmentIntersection::LinkQuality::Valid)
                    ...
                    emit_perimeter_segment_on_vertical_line(poly_with_offset, segs, i_vline, it->iContour, i_intersection, i_vertical,
                                                            *polyline_current,
                                                            going_up ? it->has_left_vertical_up() : it->has_right_vertical_down());
                else {
                ...
                // Mark both the left and right connecting segment as consumed, because one cannot go to this intersection point as it has
                // been consumed. If there are any outer intersection points skipped (bypassed) by the contour, mark them as processed.
                if (going_up)
...
// [INTENT] MonotonicRegion — a maximal rectangular band of infill lines that can be traversed
// monotonically (always increasing x). Defined by left and right boundary (vline index +
// low/high intersection indices). flips==true means the region has an odd number of lines,
// causing the exit side (right) to be opposite to the entry (left).
// [STATE] left_neighbors / right_neighbors are pointers into the same regions vector —
// they form a DAG encoding left-to-right dependencies for the ACO traversal.
// Under NDEBUG, uses boost::container::small_vector<4> to avoid heap allocation for the common
// case of ≤4 neighbors; in debug builds uses std::vector for better debugger support.
// [MEMORY] len1/len2 are precomputed float lengths for forward/flipped traversal to avoid
// recomputation in the ACO inner loop.
...
// [INTENT] AntPathMatrix — lazy-computed dense 2D matrix of (region×2) × (region×2) AntPath
// entries. The ×2 factor encodes each region's flipped/not-flipped orientation state.
// Entries are computed on first access (length == -1 sentinel). If consecutive regions share
// a direct perimeter connection, path.length is measured along the contour; otherwise Euclidean.
// [MEMORY] O(4 × n_regions²) AntPath entries — for surfaces with many MonotonicRegions this can
// be large. The comment suggests sparse representation (hashing) as a future improvement.
// [STATE] pheromone field on each AntPath is updated in-place during ACO iterations.
// Pheromone update is NOT thread-safe — ACO is entirely single-threaded.
...
{
public:
    AntPathMatrix(const std::vector<MonotonicRegion>&           regions,
                  const ExPolygonWithOffset&                    poly_with_offset,
                  const std::vector<SegmentedIntersectionLine>& segs,
        ...
        if (path.length == -1.) {
            ...
            if (region_from.right.vline + 1 == region_from.left.vline) {
                ...
                if (i_right == i_to && vline_from.intersections[i_from].next_on_contour_quality == SegmentIntersection::LinkQuality::Valid) {
                    ...
                    path.length = unscale<float>(
                        measure_perimeter_horizontal_segment_length(m_poly_with_offset, m_segs, region_from.right.vline, i_from, i_to));
                }
            }
            if (path.length == -1.) {
...
static SegmentIntersection* overlap_bottom(SegmentIntersection&       start,
                                           SegmentIntersection&       end,
                                           SegmentedIntersectionLine& vline_this,
                                           SegmentedIntersectionLine& vline_other,
                                           SegmentIntersection::Side  side)
{
...
static SegmentIntersection* overlap_top(SegmentIntersection&       start,
                                        SegmentIntersection&       end,
                                        SegmentedIntersectionLine& vline_this,
                                        SegmentedIntersectionLine& vline_other,
                                        SegmentIntersection::Side  side)
...
static std::pair<SegmentIntersection*, SegmentIntersection*> left_overlap(SegmentIntersection&       start,
                                                                          SegmentIntersection&       end,
                                                                          SegmentedIntersectionLine& vline_this,
                                                                          SegmentedIntersectionLine& vline_left)
{
...
static std::pair<SegmentIntersection*, SegmentIntersection*> left_overlap(std::pair<SegmentIntersection*, SegmentIntersection*>& start_end,
                                                                          SegmentedIntersectionLine&                             vline_this,
                                                                          SegmentedIntersectionLine&                             vline_left)
{
...
static std::pair<SegmentIntersection*, SegmentIntersection*> right_overlap(SegmentIntersection&       start,
                                                                           SegmentIntersection&       end,
                                                                           SegmentedIntersectionLine& vline_this,
                                                                           SegmentedIntersectionLine& vline_right)
{
    std::pair<SegmentIntersection*, SegmentIntersection*> out(nullptr, nullptr);
...
static std::pair<SegmentIntersection*, SegmentIntersection*> right_overlap(std::pair<SegmentIntersection*, SegmentIntersection*>& start_end,
                                                                           SegmentedIntersectionLine& vline_this,
                                                                           SegmentedIntersectionLine& vline_right)
{
    assert((start_end.first == nullptr) == (start_end.second == nullptr));
...
// [INTENT] generate_montonous_regions() — partition scan-line output into MonotonicRegions.
// [HAZARD] Function name is a TYPO: "montonous" should be "monotonous". Do NOT fix — it is
// referenced by name elsewhere in the translation unit and in BBS history.
// Algorithm: seed sweep left-to-right; for each unconsumed INNER_LOW/INNER_HIGH pair, grows
// a region rightward as long as the adjacent vline has exactly one overlapping counterpart
// (1-to-1 correspondence). Breaks if the adjacent segment fans out (1-to-many) or back
// (many-to-1). The flips flag tracks parity of line count (odd = zig-zag exits opposite side).
// [STATE] consumed_vertical_up flags on SegmentIntersections are set here to prevent
// overlapping regions. These same flags are also used later in traverse_graph_generate_polylines.
...
#define SLIC3R_DEBUG_MONOTONOUS_REGIONS
    ...
    auto                                          test_overlap = [&consumed](int segment, int low, int high) {
        for (const std::pair<int, int>& interval : consumed[segment])
            if ((low >= interval.first && low <= interval.second) || (interval.first >= low && interval.first <= high))
                return true;
...
#ifdef INFILL_DEBUG_OUTPUT
static void export_monotonous_regions_to_svg(const ExPolygonWithOffset&                    poly_with_offset,
                                             const std::vector<SegmentedIntersectionLine>& segs,
                                             ...
                                             const std::string&                            path)
{
    ...
    for (const SegmentedIntersectionLine& sil : segs) {
        for (size_t i = 0; i + 1 < sil.intersections.size(); ++i)
            if (sil.intersections[i].type == SegmentIntersection::INNER_LOW &&
                sil.intersections[i + 1].type == SegmentIntersection::INNER_HIGH) {
                ...
                emit_perimeter_segment_on_vertical_line(poly_with_offset, segs, &sil - segs.data(), it->iContour,
                                                        it - sil.intersections.data(), it->vertical_up(), polyline,
                                                        it->has_left_vertical_up());
...
static float montonous_region_path_length(const MonotonicRegion&                        region,
                                          bool                                          dir,
                                          const ExPolygonWithOffset&                    poly_with_offset,
                                          const std::vector<SegmentedIntersectionLine>& segs)
{
    ...
    for (;;) {
        ...
        const SegmentIntersection*       right       = going_up ? &vertical_run_top(vline_right, vline_right.intersections[iright]) :
                                                                  &vertical_run_bottom(vline_right, vline_right.intersections[iright]);
        ...
        if (inext == i_intersection && it->next_on_contour_quality == SegmentIntersection::LinkQuality::Valid) {
            ...
            total_length += 0.5f * float(measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline,
                                                                                     it - vline.intersections.data(), inext));
...
static void connect_monotonic_regions(std::vector<MonotonicRegion>&           regions,
                                      const ExPolygonWithOffset&              poly_with_offset,
                                      std::vector<SegmentedIntersectionLine>& segs)
{
    ...
    for (MonotonicRegion& region : regions) {
        if (region.left.vline > 0) {
            ...
            auto [lbegin, lend] = left_overlap(vline.intersections[region.left.low], vline.intersections[region.left.high], vline,
                                               vline_left);
            if (lbegin != nullptr) {
                for (;;) {
        ...
        if (region.right.vline + 1 < int(segs.size())) {
            ...
            auto [rbegin, rend] = right_overlap(vline.intersections[region.right.low], vline.intersections[region.right.high], vline,
                                                vline_right);
            if (rbegin != nullptr) {
                for (;;) {
    ...
    for (MonotonicRegion& region : regions) {
        for (MonotonicRegion* neighbor : region.left_neighbors) {
            ...
            assert(std::find(neighbor->right_neighbors.begin(), neighbor->right_neighbors.end(), &region) !=
                   neighbor->right_neighbors.end());
        }
        for (MonotonicRegion* neighbor : region.right_neighbors) {
...
void monotonic_3_opt(std::vector<MonotonicRegionLink>& path, const std::vector<SegmentedIntersectionLine>& segs)
{
    ...
    //
    // Branch & bound with KD-tree may be used with the shorter path constraint, but the precedence constraint will have to be recalculated
    // for each shorter path candidate found, which has a quadratic cost for a dense precedence graph. For a sparse precedence graph the
    // precedence constraint verification will be cheaper.
    //
    // On the other side, if the full search space is traversed as in the diploma thesis by Raad Salman (page 24, Algorithm 6.1
    // Lexicographic Path Preserving 3-opt), then the precedence constraint verification is amortized inside the O(n^3) loop. Now which is
    // better for our task?
    //
...
template<typename... TArgs> inline void print_ant(const std::string& fmt, TArgs&&... args)
{
...
// [INTENT] chain_monotonic_regions() — Ant Colony Optimization (ACO) to sequence MonotonicRegions.
// Implements the ACS (Ant Colony System) variant. Produces an ordered traversal of all regions
// that respects left-to-right dependency ordering (a region cannot be printed before all of its
// left neighbors). ACO parameters are hardcoded: 25 rounds, 10 ants, evaporation=0.1,
// diversification=0.1, alpha=1, beta=2. Initial pheromone is set to 10% of greedy path cost.
// [HAZARD] monotonic_3_opt() is called inside the ACO loop but its body is a STUB — the function
// contains only comments and a TODO, no actual 3-opt optimization. The call is a no-op.
// This means the ACO result is never locally optimized; the comment "Perform 3-opt" is misleading.
// [STATE] pheromone values in AntPathMatrix are updated in-place during each ant iteration
// (diversification) and at end of each round (evaporation + reinforcement). These mutations
// are visible to subsequent ants in the same round — this is intentional ACS behavior.
// [CONCURRENCY] NOT thread-safe. std::mt19937_64 rng is passed by reference from caller;
// all mutation is single-threaded within fill_surface_by_lines().
// Find a run through monotonic infill blocks using an 'Ant colony" optimization method.
// http://www.scholarpedia.org/article/Ant_colony_optimization
static std::vector<MonotonicRegionLink> chain_monotonic_regions(std::vector<MonotonicRegion>&                 regions,
                                                                const ExPolygonWithOffset&                    poly_with_offset,
                                                                const std::vector<SegmentedIntersectionLine>& segs,
                                                                std::mt19937_64&                              rng)
{
    // Number of left neighbors (regions that this region depends on, this region cannot be printed before the regions left of it are
    // printed) + self.
                    ...
                    for (const MonotonicRegion* left : region.left_neighbors) {
                        ...
                        if (regions_processed[iprev]) {
                            ...
                            if (left == path.back().region) {
                                ...
                                // this item will be processed in the next round by traversing path.back().region->right_neighbors before
...
#endif // __APPLE__
    ](AntPath& path) { return pow(path.pheromone, pheromone_alpha) * pow(path.visibility, pheromone_beta); };
    ...
    for (int round = 0; round < num_rounds && num_rounds_no_change < num_rounds_no_change_exit; ++round) {
        ...
        for (int ant = 0; ant < num_ants; ++ant) {
            ...
            *(queue.begin() + first_idx) = std::move(queue.back());
            ...
            assert(validate_unprocessed());
            print_ant("\tRegion (%1%:%2%,%3%) (%4%:%5%,%6%)", path.back().region->left.vline,
                      path.back().flipped ? path.back().region->left.high : path.back().region->left.low,
                      path.back().flipped ? path.back().region->left.low : path.back().region->left.high, path.back().region->right.vline,
            ...
            while (!queue.empty() || !path.back().region->right_neighbors.empty()) {
                ...
                if (dice < probability_take_best) {
                    ...
                    take_path = std::max_element(next_candidates.begin(), next_candidates.end(),
                                                 [](auto& l, auto& r) { return l.probability < r.probability; });
                    ...
                    float total_probability = std::accumulate(next_candidates.begin(), next_candidates.end(), 0.f,
                                                              [](const float l, const NextCandidate& r) { return l + r.probability; });
                ...
                // Move the other right neighbors with satisified constraints to the queue.
                for (std::vector<NextCandidate>::iterator it_next_candidate = next_candidates.begin();
                     it_next_candidate != next_candidates.begin() + num_direct_neighbors; ++it_next_candidate)
                ...
                assert(left_neighbors_unprocessed[next_region - regions.data()] == 1);
                left_neighbors_unprocessed[next_region - regions.data()] = 0;
                print_ant("\tRegion (%1%:%2%,%3%) (%4%:%5%,%6%) length to prev %7%", next_region->left.vline,
                          next_dir ? next_region->left.high : next_region->left.low,
                          next_dir ? next_region->left.low : next_region->left.high, next_region->right.vline,
                          next_dir == next_region->flips ? next_region->right.high : next_region->right.low,
                          next_dir == next_region->flips ? next_region->right.low : next_region->right.high, take_path->link->length);
                ...
                take_path->link->pheromone = (1.f - pheromone_diversification) * take_path->link->pheromone +
            ...
            // Measure path length.
            assert(!path.empty());
            float path_length = std::accumulate(path.begin(), path.end() - 1, path.back().region->length(path.back().flipped),
                                                [&path_matrix](const float l, const MonotonicRegionLink& r) {
                                                    const MonotonicRegionLink& next = *(&r + 1);
                                                    return l + r.region->length(r.flipped) +
                                                           path_matrix(*r.region, r.flipped, *next.region, next.flipped).length;
                                                });
                                 ...
                                 Polylines&                                    polylines_out)
{
    ...
    auto      finish_polyline = [&polyline, &polylines_out]() {
        ...
        if (polyline->points.size() <= 1 ||
            (polyline->points.size() == 2 && std::abs(polyline->points.front().x() - polyline->points.back().x()) < SCALED_EPSILON &&
    ...
    for (const MonotonicRegionLink& path_segment : path) {
        ...
        if (polyline != nullptr && &path_segment != path.data()) {
            ...
            if (i_vline_prev + 1 == i_vline) {
                if (ip_prev->right_horizontal() == i_intersection &&
                    ip_prev->next_on_contour_quality == SegmentIntersection::LinkQuality::Valid) {
                    // Emit a horizontal connection contour.
                    emit_perimeter_prev_next_segment(poly_with_offset, segs, i_vline_prev, ip_prev->iContour, i_intersection_prev,
                                                     i_intersection, *polyline, true);
                    extended = true;
                }
            }
            if (!extended) {
        ...
        for (;;) {
            ...
            if (going_up) {
                ...
                for (;;) {
                    ...
                    if (inext == -1 || it->vertical_up_quality() != SegmentIntersection::LinkQuality::Valid)
                        break;
                    assert(it->iContour == vline.intersections[inext].iContour);
                    emit_perimeter_segment_on_vertical_line(poly_with_offset, segs, i_vline, it->iContour, it - vline.intersections.data(),
                                                            inext, *polyline, it->has_left_vertical_up());
                    it = vline.intersections.data() + inext;
                }
            } else {
                // Going down.
                ...
                for (;;) {
                    ...
                    assert(it->iContour == vline.intersections[inext].iContour);
                    emit_perimeter_segment_on_vertical_line(poly_with_offset, segs, i_vline, it->iContour, it - vline.intersections.data(),
                                                            inext, *polyline, it->has_right_vertical_down());
                    it = vline.intersections.data() + inext;
                }
            }
            ...
            if (inext == i_intersection && it->next_on_contour_quality == SegmentIntersection::LinkQuality::Valid) {
                // Emit a horizontal connection contour.
                emit_perimeter_prev_next_segment(poly_with_offset, segs, i_vline, it->iContour, it - vline.intersections.data(), inext,
                                                 *polyline, true);
            } else {
                // Finish the current vertical line,
                going_up ? ++it : --it;
...
// [INTENT] fill_surface_by_lines() — top-level orchestrator for the scan-line infill engine.
// Coordinates the full pipeline: rotate → build ExPolygonWithOffset → compute vline grid →
// slice_region_by_vertical_lines → connect_segment_intersections_by_contours → (optional:
// pinch_contours + generate_montonous_regions + connect_monotonic_regions +
// chain_monotonic_regions + polylines_from_paths) or traverse_graph_generate_polylines →
// rotate back.
// [STATE] n_polylines_out_initial marks where this call's output starts in polylines_out, so
// only those new polylines are rotated back. The caller-provided vector is appended to in-place.
// [COUPLING] params.monotonic switches between the ACO monotonic path and the greedy traversal.
// params.horiz_move adjusts x0 — used by zig-zag layers to shift the pattern horizontally.
// params.symmetric_infill_y_axis applies a Y-axis mirror to the final rotated polylines (Orca add).
// [HAZARD] INFILL_OVERLAP_OVER_SPACING = 0.45 is a hardcoded magic constant. Changing it alters
// how far infill lines overlap with perimeters — empirically tuned but not documented.
bool FillRectilinear::fill_surface_by_lines(
    const Surface* surface, const FillParams& params, float angleBase, float pattern_shift, Polylines& polylines_out)
{
    ...
    ExPolygonWithOffset poly_with_offset(surface->expolygon, -rotate_vector.first,
    ...
    if (params.full_infill() && !params.dont_adjust) {
        ...
        bounding_box.merge(align_to_grid(bounding_box.min, Point(line_spacing, line_spacing), refpt));
    }
    ...
    for (size_t i_seg = 0; i_seg < segs.size(); ++i_seg) {
        ...
        for (size_t i = 0; i < sil.intersections.size();) {
            ...
            if (i + 1 == j) {
                ...
                svg.draw(Line(Point(sil.pos, sil.intersections[i + 1].pos()), Point(sil.pos, sil.intersections[j - 1].pos())),
                         (j - i + 1 > 4) ? "yellow" : "magenta");
    ...
    if (monotonic_infill) {
        ...
        {
            static int iRun;
            export_monotonous_regions_to_svg(poly_with_offset, segs, regions,
                                             debug_out_path("%s-%03d.svg", "MontonousRegions-initial", iRun++));
        }
#endif // INFILL_DEBUG_OUTPUT
...
void make_fill_lines(const ExPolygonWithOffset& poly_with_offset,
                     Point                      refpt,
                     double                     angle,
                     coord_t                    x_margin,
                     coord_t                    line_spacing,
                     ...
                     Polylines&                 fill_lines)
{
    ...
    for (const SegmentedIntersectionLine& vline :
         slice_region_by_vertical_lines(poly_with_offset, n_vlines, bounding_box.min.x(), line_spacing))
        if (vline.pos >= x_min) {
            if (vline.pos > x_max)
                break;
...
static inline void remove_overlapped(Polylines& polylines, coord_t line_width)
{
    ...
    for (const Polyline& line : polylines) {
        ...
        for (const Polyline& existing : cleaned) {
            ...
            if (std::abs(mp1.y() - mp2.y()) > tolerance && std::abs(mp1.x() - mp2.x()) > tolerance)
                continue;
...
// [INTENT] fill_surface_by_multilines() — multi-pass infill for patterns like Grid, Triangles,
// Stars, Cubic that need multiple sweep angles. Calls make_fill_lines() (a simplified scan-line
// pass that uses only OUTER_LOW/OUTER_HIGH intersections) once per SweepParams entry, collects
// all lines, then applies multiline_fill() for lateral offset grouping, clips to polygon,
// and optionally connects via connect_infill(). Unlike fill_surface_by_lines(), does NOT support
// monotonic ordering or inner contour connections.
// [COUPLING] params.density is divided by sweep_params.size() before processing — caller must
// pass the full desired density; this function applies the per-pass fraction internally.
// [HAZARD] remove_overlapped() uses a midpoint-distance O(n²) deduplication — for high-density
// infill with many lines this can be slow.
bool FillRectilinear::fill_surface_by_multilines(const Surface*                            surface,
                                                 FillParams                                params,
                                                 ...
                                                 Polylines&                                polylines_out)
{
    ...
    ExPolygonWithOffset poly_with_offset_base(surface->expolygon, 0,
                                              float(scale_(
                                                  this->overlap +
                                                  0.5 * params.multiline *
                                                      this->spacing))); // increase offset to crop infill lines when using multiline infill
...
// [INTENT] fill_surface_trapezoidal() — Orca-specific experimental trapezoidal grid/triangular
// infill (multiline > 1). Generates explicit trapezoid geometry by constructing paired lines
// at ±offset from each base infill line, producing a filled-band effect instead of single lines.
// Pattern_type: 0=grid (45° base angle), 1=triangular (90° base angle).
// [COUPLING] Distinct from fill_surface_by_multilines: generates explicit trapezoid polygons
// rather than reusing the scan-line engine. Clips against contracted surface polygon.
// [HAZARD] Period computation uses integer coords cast from float — potential rounding at high
// densities. The sqrt(2)/sqrt(3) period factors are hardcoded geometry constants.
...
// [INTENT] FillRectilinear::fill_surface() — routes to fill_surface_by_lines() for rectilinear,
// zigzag, crosszag, and locked-zag patterns, and to fill_surface_by_multilines() for multiline
// variants. Errors from the inner function are logged but silently swallowed (return true).
Polylines FillRectilinear::fill_surface(const Surface* surface, const FillParams& params)
{
    ...
    if (params.full_infill() || params.multiline == 1 || params.pattern == ipCrossZag || params.pattern == ipZigZag ||
...
Polylines FillGrid::fill_surface(const Surface* surface, const FillParams& params)
{
    ...
    if (params.multiline > 1) {
        ...
        if (!this->fill_surface_trapezoidal(surface, params, {{0.f, 0.f}, {float(M_PI / 2.), 0.f}}, polylines_out, 0))
            BOOST_LOG_TRIVIAL(error) << "FillGrid::fill_surface_trapezoidal() failed.";

    } else {
        if (!this->fill_surface_by_multilines(surface, params, {{0.f, 0.f}, {float(M_PI / 2.), 0.f}}, polylines_out))
...
Polylines FillTriangles::fill_surface(const Surface* surface, const FillParams& params)
{
    ...
    if (params.multiline > 1) {
        // Experimental trapezoidal grid
        if (!this->fill_surface_trapezoidal(surface, params, {{0.f, 0.f}, {float(M_PI / 2.), 0.f}}, polylines_out, 1))
            BOOST_LOG_TRIVIAL(error) << "FillGrid::fill_surface_trapezoidal() failed.";

    } else {
        if (!this->fill_surface_by_multilines(surface, params, {{0.f, 0.f}, {float(M_PI / 3.), 0.f}, {float(2. * M_PI / 3.), 0.}},
                                              polylines_out))
...
{
    Polylines polylines_out;
    if (!this->fill_surface_by_multilines(surface, params,
                                          {{0.f, 0.f},
                                           {float(M_PI / 3.), 0.f},
                                           {float(2. * M_PI / 3.), float((3. / 2.) * this->spacing * params.multiline / params.density)}},
                                          polylines_out))
...
Polylines FillLateralHoneycomb::fill_surface(const Surface* surface, const FillParams& params)
{
    ...
    if (single_line) {
        ...
        if (!fill_surface_by_multilines(surface, multiline_params, {{half_pi, horizontal_offset}}, polylines_out))
        ...
        if (!fill_surface_by_multilines(surface, multiline_params,
...
Polylines FillSupportBase::fill_surface(const Surface* surface, const FillParams& params)
{
    ...
    if (poly_with_offset.n_contours > 0) {
        ...
        connect_base_support(std::move(fill_lines), poly_with_offset.polygons_outer, poly_with_offset.bounding_box_outer(), polylines_out,
                             this->spacing, params);
...
// [INTENT] sample_grid_pattern() — used by Lightning infill to generate candidate anchor points.
// Reuses slice_region_by_vertical_lines() with zero offsets (no inner contour) to find evenly
// spaced grid points inside the polygon. Uses the global layer bounding box (not per-expolygon)
// to guarantee minimum spacing between points in adjacent ExPolygons on the same layer.
// [COUPLING] Caller (FillLightning) depends on exact spacing guarantee — inter-point distance
// always ≥ spacing — to set anchor positions for the tree growth algorithm.
Points sample_grid_pattern(const ExPolygon& expolygon, coord_t spacing, const BoundingBox& global_bounding_box)
{
    ExPolygonWithOffset                    poly_with_offset(expolygon, 0, 0, 0);
    std::vector<SegmentedIntersectionLine> segs = slice_region_by_vertical_lines(poly_with_offset,
                                                                                 (global_bounding_box.max.x() -
                                                                                  global_bounding_box.min.x() + spacing - 1) /
                                                                                     spacing,
                                                                                 global_bounding_box.min.x(), spacing);
...
// [INTENT] FillLockedZag::fill_surface_locked_zag() — Orca-specific dual-density infill.
// Partitions the surface into a "skeleton" (inner region, inset by skin_infill_depth) and
// "skin" (outer annulus). Each region can have different infill density, derived from
// lock_param.skeleton_density_params and lock_param.skin_density_params maps (keyed by density).
// Skeleton uses zigzag (fill_surface() via self), skin uses rectilinear (params.locked_zag=false).
// [COUPLING] lock_param is populated by the caller (PrintObject) from per-region config.
// The skeleton_flow_params / skin_flow_params maps hold per-density Flow objects for variable
// extrusion width — this is what makes "locked zag" produce thicker skeleton lines.
// [STATE] multi_width_polyline output is a vector<pair<Polylines, Flow>> — each subvector has its
// own flow, assembled into separate ExtrusionPath entities by fill_surface_extrusion().
// [HAZARD] The overlap region (intersection_ex with offset_ex(..., overlap_threshold)) causes
// skeleton lines to intentionally extend into the skin region. If overlap_threshold is large
// relative to spacing, skeleton and skin lines can double-print, over-extruding the boundary.
