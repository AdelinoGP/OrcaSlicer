...
namespace Slic3r {
// [INTENT] Shared color value types used across rendering, preset serialization, and UI-driven
// color configuration. Components are normalized floats in [0, 1] to keep conversions between
// OpenGL-style pipelines and text config formats straightforward.
// [STATE] ColorRGB/ColorRGBA are mutable value objects; setters clamp in place.
// [COUPLING] decode_*/encode_* declarations couple this header to string-config parsing paths
// implemented in Color.cpp and consumed by preset/config subsystems.
// [HAZARD] Equality for RGBA uses color_is_equal() tolerance semantics while RGB uses exact
// std::array comparison. Translators must preserve this mixed behavior to avoid regressions.
...
// Produce an alpha channel checksum for the red green blue components. The alpha channel may then be used to verify, whether the rgb
// components were not interpolated by alpha blending or multi sampling.
