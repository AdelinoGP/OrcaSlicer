// [INTENT] PrintConfig.cpp is the runtime configuration registry for OrcaSlicer.
// It has three major responsibilities:
//   1. Enum serialization: defines string↔integer maps for every enum type
//      (GCodeFlavor, InfillPattern, SupportMaterialStyle, etc.) via the
//      CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS macro, and registers them with the
//      templated ConfigOptionEnum<T> system so they can be serialized to/from
//      JSON/INI profile files.
//   2. Option definition: PrintConfigDef::init_common_params(),
//      init_fff_params(), init_sla_params() register every user-visible slicing
//      parameter: label, tooltip, unit, range, default, GUI mode (comSimple /
//      comAdvanced / comDeveloper), and category.
//   3. Utility functions: get_bed_shape(), get_bed_excluded_area(),
//      NozzleType/ExtruderType string helpers, filament-map helpers, etc.
// [COUPLING] The definitions here are consumed by:
//   - DynamicPrintConfig (Config.hpp) for generic parameter storage/lookup
//   - PrintConfig.hpp struct fields (typed accessors)
//   - The GUI (slic3r/GUI/) for parameter rendering and validation
//   - Profile I/O (bbs_3mf.cpp, Config.cpp) for preset serialization
// [STATE] PrintConfigDef is a singleton (one global instance created at startup).
//   All `init_*` calls run in the constructor; no lazy initialisation.
// [HAZARD] H770 — This file is 10,836 lines; it is the largest single source file
//   in the repository. Any automated refactoring tool that naively inlines or
//   splits this file risks breaking the singleton guarantee or the enum-map
//   registration order that the serialization layer depends on.
// [HAZARD] H771 — The L() macro is an identity marker for l10n extraction tools,
//   NOT a runtime translation call. _(s) IS the runtime translator. Confusing the
//   two in a refactor will break either the translation pipeline or the UI strings.
// [MEMORY] ConfigOptionDef* pointers returned by this->add() are owned by the
//   t_optiondef_map inside PrintConfigDef. Callers must never delete them.
...
namespace {
// [INTENT] Helper: splits a string by `separator` and deduplicates the tokens
// into a std::set. Used by filament/extruder profile matching logic.
...
const std::vector<std::string> filament_extruder_override_keys = {
    ...
    "filament_retract_lift_enforce", "filament_retraction_speed", "filament_deretraction_speed",
    ...
    // percents
    "filament_retract_before_wipe", "filament_long_retractions_when_cut", "filament_retraction_distances_when_cut"};
...
static t_config_enum_values s_keys_map_AuthorizationType{{"key", atKeyPassword}, {"user", atUserPassword}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(AuthorizationType)

static t_config_enum_values
    s_keys_map_GCodeFlavor{{"marlin", gcfMarlinLegacy},     {"reprap", gcfRepRapSprinter}, {"reprapfirmware", gcfRepRapFirmware},
                           {"repetier", gcfRepetier},       {"teacup", gcfTeacup},         {"makerware", gcfMakerWare},
                           {"marlin2", gcfMarlinFirmware},  {"sailfish", gcfSailfish},     {"klipper", gcfKlipper},
                           {"smoothie", gcfSmoothie},       {"mach3", gcfMach3},           {"machinekit", gcfMachinekit},
                           {"no-extrusion", gcfNoExtrusion}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(GCodeFlavor)

static t_config_enum_values s_keys_map_BedTempFormula{{"by_first_filament", int(BedTempFormula::btfFirstFilament)},
                                                      {"by_highest_temp", int(BedTempFormula::btfHighestTemp)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(BedTempFormula)

// Orca
static t_config_enum_values s_keys_map_PowerLossRecoveryMode{{"printer_configuration", int(PowerLossRecoveryMode::PrinterConfiguration)},
...
static t_config_enum_values s_keys_map_FuzzySkinType{{"none", int(FuzzySkinType::None)},
...
static t_config_enum_values s_keys_map_NoiseType{{"classic", int(NoiseType::Classic)},
...
static t_config_enum_values s_keys_map_FuzzySkinMode{{"displacement", int(FuzzySkinMode::Displacement)},
...
static t_config_enum_values s_keys_map_InfillPattern{{"monotonic", ipMonotonic},
...
static t_config_enum_values s_keys_map_IroningType{{"no ironing", int(IroningType::NoIroning)},
...
// BBS
static t_config_enum_values s_keys_map_WallSequence{{"inner wall/outer wall", int(WallSequence::InnerOuter)},
...
// BBS
static t_config_enum_values s_keys_map_PrintSequence{{"by layer", int(PrintSequence::ByLayer)}, {"by object", int(PrintSequence::ByObject)}};
...
static t_config_enum_values s_keys_map_SlicingMode{{"regular", int(SlicingMode::Regular)},
                                                   {"even_odd", int(SlicingMode::EvenOdd)},
                                                   {"close_holes", int(SlicingMode::CloseHoles)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SlicingMode)

static t_config_enum_values s_keys_map_SupportMaterialPattern{
    {"rectilinear", smpRectilinear}, {"rectilinear-grid", smpRectilinearGrid},
    {"honeycomb", smpHoneycomb},     {"lightning", smpLightning},
    {"default", smpDefault},         {"hollow", smpNone},
};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SupportMaterialPattern)

static t_config_enum_values s_keys_map_SupportMaterialStyle{{"default", smsDefault},
                                                            {"grid", smsGrid},
                                                            {"snug", smsSnug},
...
static t_config_enum_values s_keys_map_SupportMaterialInterfacePattern{{"auto", smipAuto},
...
static t_config_enum_values s_keys_map_SeamPosition{{"nearest", spNearest},
                                                    {"aligned", spAligned},
...
static const t_config_enum_values s_keys_map_SLADisplayOrientation = {{"landscape", sladoLandscape}, {"portrait", sladoPortrait}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SLADisplayOrientation)

static const t_config_enum_values s_keys_map_SLAPillarConnectionMode = {{"zigzag", slapcmZigZag},
                                                                        {"cross", slapcmCross},
                                                                        {"dynamic", slapcmDynamic}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SLAPillarConnectionMode)

static const t_config_enum_values s_keys_map_SLAMaterialSpeed = {{"slow", slamsSlow}, {"fast", slamsFast}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SLAMaterialSpeed);

static const t_config_enum_values s_keys_map_BrimType = {
    {"no_brim", btNoBrim},     {"outer_only", btOuterOnly}, {"inner_only", btInnerOnly}, {"outer_and_inner", btOuterAndInner},
...
static const t_config_enum_values s_keys_map_SkirtType = {{"combined", stCombined}, {"perobject", stPerObject}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(SkirtType)

static const t_config_enum_values s_keys_map_DraftShield = {{"disabled", dsDisabled}, {"enabled", dsEnabled}};
...
static const t_config_enum_values s_keys_map_OverhangFanThreshold = {{"0%", Overhang_threshold_none}, {"10%", Overhang_threshold_1_4},
                                                                     {"25%", Overhang_threshold_2_4}, {"50%", Overhang_threshold_3_4},
                                                                     {"75%", Overhang_threshold_4_4}, {"95%", Overhang_threshold_bridge}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(OverhangFanThreshold)

// BBS
static const t_config_enum_values s_keys_map_BedType = {{"Default Plate", btDefault},  {"SuperTack Plate", btSuperTack},
                                                        {"Cool Plate", btPC},          {"Engineering Plate", btEP},
                                                        {"High Temp Plate", btPEI},    {"Textured PEI Plate", btPTE},
...
static t_config_enum_values s_keys_map_NozzleType{{"undefine", int(NozzleType::ntUndefine)},
...
static t_config_enum_values s_keys_map_PrinterStructure{{"undefine", int(PrinterStructure::psUndefine)},
...
static t_config_enum_values s_keys_map_PerimeterGeneratorType{{"classic", int(PerimeterGeneratorType::Classic)},
                                                              {"arachne", int(PerimeterGeneratorType::Arachne)}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(PerimeterGeneratorType)

static const t_config_enum_values s_keys_map_ZHopType = {{"Auto Lift", zhtAuto},
                                                         {"Normal Lift", zhtNormal},
                                                         {"Slope Lift", zhtSlope},
                                                         {"Spiral Lift", zhtSpiral}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(ZHopType)

static const t_config_enum_values s_keys_map_RetractLiftEnforceType = {{"All Surfaces", rletAllSurfaces},
...
static const t_config_enum_values s_keys_map_ExtruderType = {{"Direct Drive", etDirectDrive}, {"Bowden", etBowden}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(ExtruderType)

static const t_config_enum_values s_keys_map_NozzleVolumeType = {{"Standard", nvtStandard}, {"High Flow", nvtHighFlow}};
CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(NozzleVolumeType)

static const t_config_enum_values s_keys_map_FilamentMapMode = {{"Auto For Flush", fmmAutoForFlush},
...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L(
        "Unprintable area in XY plane. For example, X1 Series printers use the front left corner to cut filament during filament change. "
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip     = L("Maximum detour distance for avoiding crossing wall. "
                             ...
                             "Detour length could be specified either as an absolute value or as percentage (for example 50%) of a direct travel "
                             "path. Zero to disable.");
    ...
    def->tooltip = L(
        "Enable this option to allow adjustment of the part cooling fan speed for specifically for overhangs, internal and external "
    ...
    def->tooltip = L(
        "Use this part cooling fan speed when printing bridges or overhang walls with an overhang threshold that exceeds "
    ...
    def->tooltip = L(
        "When the overhang exceeds this specified threshold, force the cooling fan to run at the 'Overhang Fan Speed' set below. "
    ...
    def->tooltip  = L("Decrease this value slightly (for example 0.9) to reduce the amount of material for bridge, to improve sag.\n\n"
                       "The actual bridge flow used is calculated by multiplying this value with the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2.0;
    def->mode     = comAdvanced;
    ...
    def->category = L("Quality");
    def->tooltip  = L("This value governs the thickness of the internal bridge layer. This is the first layer over sparse infill. Decrease "
                       "this value slightly (for example 0.9) to improve surface quality over sparse infill."
                       "\n\nThe actual internal bridge flow used is calculated by multiplying this value with the bridge flow ratio, the "
    ...
    def->tooltip  = L("This factor affects the amount of material for top solid infill. "
                       ...
                       "The actual top surface flow used is calculated by multiplying this value with the filament flow ratio, and if set, "
    ...
    def->tooltip  = L("This factor affects the amount of material for bottom solid infill.\n\n"
                       "The actual bottom solid infill flow used is calculated by multiplying this value with the filament flow ratio, and "
    ...
    def->tooltip  = L(
        "This factor affects the amount of material on the first layer for the extrusion path roles listed in this section.\n\n"
         "For the first layer, the actual flow ratio for each path role (does not affect brims and skirts) will be multiplied by this "
         "value.");
    def->min  = 0;
    def->max  = 2;
    def->mode = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for outer walls.\n\n"
                       "The actual outer wall flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for inner walls.\n\n"
                       "The actual inner wall flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for overhangs.\n\n"
                       "The actual overhang flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for sparse infill.\n\n"
                       "The actual sparse infill flow used is calculated by multiplying this value by the filament flow ratio, and if set, "
                       "the object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for internal solid infill.\n\n"
                       "The actual internal solid infill flow used is calculated by multiplying this value by the filament flow ratio, and "
                       "if set, the object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for filling the gaps.\n\n"
                       "The actual gap filling flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for support.\n\n"
                       "The actual support flow used is calculated by multiplying this value by the filament flow ratio, and if set, the "
                       "object's flow ratio.");
    def->min      = 0;
    def->max      = 2;
    def->mode     = comAdvanced;
    ...
    def->tooltip  = L("This factor affects the amount of material for the support interface.\n\n"
                       "The actual support interface flow used is calculated by multiplying this value by the filament flow ratio, and if "
    ...
    // xgettext:no-c-format, no-boost-format
    def->tooltip = L(
        "If a top surface has to be printed and it's partially covered by another layer, it won't be considered at a top layer where its "
        "width is below this value."
        ...
        "\nWarning: If enabled, artifacts can be created if you have some thin features on the next layer, like letters. Set this setting "
        "to 0 to remove these artifacts.");
    def->sidetext    = L("mm or %");
    ...
    def->category   = L("Quality");
    def->tooltip    = L("Extrude perimeters that have a part over an overhang in the reverse direction on even layers. This alternating "
                           "pattern can drastically improve steep overhangs.\n\nThis setting can also help reduce part warping due to the "
                           "reduction of stresses in the part walls.");
    def->mode       = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip    = L(
        "Apply the reverse perimeters logic only on internal perimeters.\n\n"
           ...
           "it is recommended to set the Reverse Threshold to 0 so that all internal walls print in alternating directions on even layers "
           "irrespective of their overhang degree.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category   = L("Speed");
    def->tooltip    = L("Speed of internal bridges. If the value is expressed as a percentage, it will be calculated based on the "
                           "bridge_speed. Default value is 150%.");
    def->sidetext   = L("mm/s or %");
    ...
    def->tooltip  = L(
        "This option enables the generation of an extra bridge layer over internal and/or external bridges.\n\n"
         ...
         "The extra bridge layer results in reduced pillowing on top surfaces, as well as reduced separation of the external bridge layer "
         "from its surrounding perimeters.\n\n"
         "It is generally recommended to set this to at least 'External bridge only', unless specific issues with the sliced model are "
         "found.\n\n"
         "Options:\n"
         "1. Disabled - does not generate second bridge layers. This is the default and is set for compatibility purposes\n"
         "2. External bridge only - generates second bridge layers for external-facing bridges only. Please note that small bridges that "
         "are shorter "
         ...
         "If multiple regions in the same island, with varying bridge angles are present, the last region of that island will be selected "
         "as the angle reference\n"
    ...
    def->category = L("Strength");
    def->tooltip = L("Line pattern of internal solid infill. if the detect narrow internal solid infill be enabled, the concentric pattern "
    ...
    def->tooltip  = L(
        "Print sequence of the internal (inner) and external (outer) walls.\n\n"
         ...
         "However, this option results in slightly reduced surface quality as the external perimeter is deformed by being squashed to the "
         "internal perimeter.\n\n"
         "Use Inner/Outer/Inner for the best external surface finish and dimensional accuracy as the external wall is printed undisturbed "
         "from an internal perimeter. "
    ...
    def->tooltip  = L("Order of wall/infill. When the tickbox is unchecked the walls are printed first, which works best in most "
                       "cases.\n\nPrinting infill first may help with extreme overhangs as the walls have the neighbouring infill to adhere "
                       "to. However, the infill will slightly push out the printed walls where it is attached to them, resulting in a worse "
                       "external surface finish. It can also cause the infill to shine through the external surfaces of the part.");
    def->category = L("Quality");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool{false});
    ...
    def->category = L("Quality");
    def->tooltip = L("The direction which the wall loops are extruded when looking down from the top.\n\nBy default all walls are extruded "
                     "in counter-clockwise, unless Reverse on even is enabled. Set this to any option other than Auto will force the wall "
    ...
    def->tooltip = L("With increasing print speeds (and hence increasing volumetric flow through the nozzle) and increasing accelerations, "
                     ...
                     "that does not cause too much bulging on features with lower flow speed and accelerations while also not causing gaps "
                     "on faster features.\n\n"
                     "This feature aims to address this limitation by modeling the response of your printer's extrusion system depending "
                     "on the volumetric flow speed and acceleration it is printing at. Internally, it generates a fitted model that can "
                     "extrapolate the needed pressure "
                     "advance for any given volumetric flow speed and acceleration, which is then emitted to the printer depending on the "
    ...
    // xgettext:no-c-format, no-boost-format
    def->tooltip = L(
        "Add sets of pressure advance (PA) values, the volumetric flow speeds and accelerations they were measured at, separated by a "
        "comma. "
        ...
        "print speed in your profile (usually its the sparse or solid infill). Then run them for the same speeds for the slowest and "
        "fastest print accelerations, "
    ...
    def->tooltip = L(
        "Enable adaptive PA for overhangs as well as when flow changes within the same feature. This is an experimental option, "
        "as if the PA profile is not set accurately, it will cause uniformity issues on the external surfaces before and after "
        "overhangs.\n");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBools{false});
    ...
    def->tooltip = L(
        "Time to unload old filament when switch filament. It's usually applicable for single-extruder multi-material machines. "
        "For tool changers or multi-tool machines, it's typically 0. For statistics only.");
    def->sidetext = L("s"); // seconds, CIS languages need translation
    def->min      = 0;
    def->mode     = comAdvanced;
    ...
    def->tooltip = L("This option determines how the bed temperature is set during slicing: based on the temperature of the first filament "
                     "or the highest temperature of the printed filaments.");
    def->mode    = comAdvanced;
    ...
    /*
        ...
        higher packing density -> more material extruded by single turn -> higher pellet_flow_coefficient -> treated as if a filament of
       larger diameter is being used All other calculations remain the same for slicing.
    */
    ...
    def->tooltip = L("Pellet flow coefficient is empirically derived and allows for volume calculation for pellet printers.\n\nInternally "
                     "it is converted to filament_diameter. All other volume calculations remain the same.\n\nfilament_diameter = sqrt( (4 "
                     "* pellet_flow_coefficient) / PI )");
    def->min     = 0;
    ...
    def->tooltip  = L("Enter the shrinkage percentage that the filament will get after cooling (94% if you measure 94mm instead of 100mm). "
                       "The part will be scaled in XY to compensate. For multi-material prints, ensure filament shrinkage matches across all "
                       "used filaments\n"
    ...
    def->tooltip  = L("Print temperature for prime tower interface layer (where different materials meet). If set to -1, use max "
    ...
    def->set_default_value(
        new ConfigOptionStrings{"120 100 6.6 6.8 7.2 7.6 7.9 8.2 8.7 9.4 9.9 10.0|"
                                " 0.05 6.6 0.45 6.8 0.95 7.8 1.45 8.3 1.95 9.7 2.45 10 2.95 7.6 3.45 7.6 3.95 7.6 4.45 7.6 4.95 7.6"});

    def          = this->add("filament_multitool_ramming", coBools);
    def->label   = L("Enable ramming for multi-tool setups");
    def->tooltip = L(
        "Perform ramming when using multi-tool printer (i.e. when the 'Single Extruder Multimaterial' in Printer Settings is unchecked). "
    ...
    def->tooltip  = L("When changing the extruder, it is recommended to extrude a certain length of filament from the original extruder. "
                       "This helps minimize nozzle oozing.");
    def->sidetext = L("mm"); // milimeters, CIS languages need translation
    def->min      = 0;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloats{10});
    ...
    def->category = L("Strength");
    // xgettext:no-c-format, no-boost-format
    def->tooltip  = L("Density of internal sparse infill, 100% turns all sparse infill into solid infill and internal solid infill pattern "
                       "will be used.");
    def->sidetext = "%";
    def->min      = 0;
    def->max      = 100;
    ...
    def->category = L("Strength");
    def->tooltip  = L("Aligns infill and surface fill directions to follow the model's orientation on the build plate. When enabled, fill "
                       "directions rotate with the model to maintain optimal strength characteristics.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Strength");
    def->tooltip  = L("Insert solid infill at specific layers. Use N to insert every Nth layer, N#K to insert K consecutive solid layers "
                       "every N layers (K is optional, e.g. '5#' equals '5#1'), or a comma-separated list (e.g. 1,7,9) to insert at explicit "
                       "layers. Layers are 1-based.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionString());
    ...
    def->label      = L("Bridge");
    def->category   = L("Speed");
    def->tooltip    = L("Acceleration of bridges. If the value is expressed as a percentage (e.g. 50%), it will be calculated based on the "
    ...
    def->category = L("Speed");
    def->tooltip = L("Acceleration of sparse infill. If the value is expressed as a percentage (e.g. 100%), it will be calculated based on "
    ...
    def->tooltip = L(
        "This part cooling fan speed is applied when printing support interfaces. Setting this parameter to a higher than regular speed "
    ...
    def->tooltip       = L("Fuzzy skin generation mode. Works only with Arachne!\n"
                                 ...
                                 "Combined: Joint mode [Displacement] + [Extrusion]. The appearance of the walls is similar to [Displacement] Mode, "
                                 "but it leaves no pores between the perimeters.\n\n"
                                 "Attention! The [Extrusion] and [Combined] modes works only the fuzzy_skin_thickness parameter not more than the "
                                 "thickness of printed loop. "
                                 "At the same time, the width of the extrusion for a particular layer should also not be below a certain level. "
                                 "It is usually equal 15-25%% of a layer height. Therefore, the maximum fuzzy skin thickness with a perimeter width of "
                                 "0.4 mm and a layer height of 0.2 mm will be 0.4-(0.2*0.25)=±0.35mm! "
                                 "If you enter a higher parameter than this, the error Flow::spacing() will displayed, and the model will not be "
                                 "sliced. You can choose this number until this error is repeated.");
    ...
    def->category = L("Others");
    def->tooltip  = L("The number of octaves of coherent noise to use. Higher values increase the detail of the noise, but also increase "
                       "computation time.");
    def->min      = 1;
    def->max      = 10;
    def->mode     = comAdvanced;
    ...
    def->tooltip = L(
        "Choose how to control power loss recovery. When set to Printer configuration, the slicer will not emit power loss recovery G-code "
        "and will leave the printer's configuration unchanged. Applicable to Bambu Lab or Marlin 2 firmware based printers.");
    def->mode          = comAdvanced;
    ...
    def->tooltip = L(
        "Start the fan this number of seconds earlier than its target start time (you can use fractional seconds)."
    ...
    def->category = L("Strength");
    def->tooltip  = L(
        "This parameter adds a rotation of solid infill direction to each layer according to the specified template. "
    ...
    def->tooltip  = L("The remaining part of the model contour after removing a certain depth from the surface is called the skeleton. "
                       ...
                       "When two regions have the same sparse infill settings but different skeleton densities, their skeleton areas will "
                       "develop overlapping sections. "
    ...
    def->tooltip  = L("The portion of the model's outer surface within a certain depth range is called the skin. "
                       "This parameter is used to adjust the density of this section. "
                       "When two regions have the same sparse infill settings but different skin densities, this area will not be split into "
    ...
    def->tooltip  = L("Maximum layer height for the combined sparse infill.\n\n"
                       "Set it to 0 or 100% to use the nozzle diameter (for maximum reduction in print time) or a value of ~80% to maximize "
                       "sparse infill strength.\n\n"
                       "The number of layers over which infill is combined is derived by dividing this value with the layer height and "
    ...
    def->tooltip  = L("Generate interlocking beam structure at the locations where different filaments touch. This improves the adhesion "
                       "between filaments, especially models printed in different materials.");
    def->category = L("Advanced");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip  = L("The height of the beams of the interlocking structure, measured in number of layers. Less layers is stronger, but "
    ...
    def->tooltip  = L("The distance from the boundary between filaments to generate interlocking structure, measured in cells. Too few "
                       "cells will result in poor adhesion.");
    def->min      = 1;
    def->category = L("Advanced");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionInt(2));
...

    {
        ...
        std::vector<AxisDefault> axes{// name, max_feedrate,  max_acceleration, max_jerk
    ...
    def->tooltip  = L("A lower value results in smoother extrusion rate transitions. "
                       ...
                       "Default value of 3 works well for most cases. If your printer is stuttering, increase this value to reduce the "
    ...
    def->tooltip = L("Applies extrusion rate smoothing only on external perimeters and overhangs. This can help reduce artefacts due to "
                     "sharp speed transitions on externally visible "
    ...
    def->label   = L("Fan speed");
    def->tooltip = L(
        "Speed of auxiliary part cooling fan. Auxiliary fan will run at this speed during printing except the first several layers "
        "which is defined by no cooling layers.\nPlease enable auxiliary_fan in printer settings to use this feature. G-code command: M106 "
        "P2 S(0-255)");
    def->sidetext = "%";
    def->min      = 0;
    def->max      = 100;
    ...
    def->category = L("Strength");
    def->tooltip  = L("This setting adds an extra wall to every other layer. This way the infill gets wedged vertically between the walls, "
                       "resulting in stronger prints.\n\n"
                       "When this option is enabled, the ensure vertical shell thickness option needs to be disabled.\n\n"
                       "Using lightning infill together with this option is not recommended as there is limited infill to anchor the extra "
                       "perimeters to.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip  = L("If this value is positive, Z-hop will only come into effect when Z is above the parameter: \"Z-hop lower boundary\" "
                       "and is below this value.");
    def->sidetext = L("mm"); // milimeters, CIS languages need translation
    def->mode     = comAdvanced;
    def->min      = 0;
    def->set_default_value(new ConfigOptionFloats{0.});
    ...
    // internal use only, don't need translation
    def->label   = "Type";
    def->tooltip = "This setting is only used for initial value of manual calibration of pressure advance. Bowden extruder usually has "
                   "larger PA value. This setting doesn't influence normal slicing.";
    ...
    def->category = L("Quality");
    def->tooltip = L("In order to reduce the visibility of the seam in a closed loop extrusion, the loop is interrupted and shortened by a "
                     "specified amount.\n"
                     "This amount can be specified in millimeters or as a percentage of the current extruder diameter. The default value "
    ...
    def->tooltip  = L("The wipe speed is determined by the speed of the current extrusion role. "
                       "e.g. if a wipe action is executed immediately following an outer wall extrusion, the speed of the outer wall "
                       "extrusion will be utilized for the wipe action.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(true));
    ...
    def->category = L("Quality");
    def->tooltip  = L("To minimize the visibility of the seam in a closed loop extrusion, a small inward movement is executed before the "
    ...
    def->category = L("Quality");
    def->tooltip  = L(
        "To minimize visibility of potential overextrusion at the start of an external perimeter when printing with "
    ...
    def->tooltip = L("Limits the skirt/draft shield loops to one wall after the first layer. This is useful, on occasion, to conserve "
                     "filament but may cause the draft shield/skirt to warp / crack.");
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip    = L("Minimum filament extrusion length in mm when printing the skirt. Zero means this feature is disabled.\n\n"
                           ...
                           "Final number of loops is not taking into account while arranging or validating objects distance. Increase loop "
                           "number in such case.");
    def->min        = 0;
    def->sidetext   = L("mm"); // milimeters, CIS languages need translation
    def->mode       = comAdvanced;
    ...
    def->tooltip = L("To reduce the waiting time after tool change, Orca can preheat the next tool while the current tool is still in use. "
                     "This setting specifies the time in seconds to preheat the next tool. Orca will insert a M104 command to preheat the "
                     "tool in advance.");
    def->sidetext = L("s"); // seconds, CIS languages need translation
    def->min      = 0;
    def->max      = 120;
...
//    def->min = 0;
    ...
    def->category    = L("Support");
    def->tooltip     = L("If threshold angle is zero, support will be generated for overhangs whose overlap is below the threshold. The "
                             "smaller this value is, the steeper the overhang that can be printed without support.");
    def->sidetext    = L("mm or %");
    def->min         = 0;
    def->max         = 100;
    def->max_literal = 0.5;
    ...
    def->tooltip = L("Enable this option for automated chamber temperature control. "
                     "This option activates the emitting of an M191 command before the \"machine_start_gcode\"\n which sets the chamber "
                     "temperature and waits until it is reached. "
                     "In addition, it emits an M141 command at the end of the print to turn off the chamber heater, if present.\n\n"
                     "This option relies on the firmware supporting the M191 and M141 commands either via macros or natively and is "
                     "usually used when an active chamber heater is installed.");
    def->mode    = comSimple;
    def->set_default_value(new ConfigOptionBools{false});
    ...
    def->tooltip  = L("Describe how long the nozzle will move along the last path when retracting.\n\n"
                       ...
                       "Setting a value in the retract amount before wipe setting below will perform any excess retraction before the wipe, "
                       "else it will be performed after.");
    def->sidetext = L("mm"); // milimeters, CIS languages need translation
    def->min      = 0;
    def->mode     = comAdvanced;
    ...
    def->set_default_value(
        new ConfigOptionFloats{0.f, 280.f, 280.f, 280.f, 280.f, 0.f, 280.f, 280.f, 280.f, 280.f, 0.f, 280.f, 280.f, 280.f, 280.f, 0.f});
    ...
    def->tooltip  = L("The maximum print speed when purging in the wipe tower and printing the wipe tower sparse layers. "
                       "When purging, if the sparse infill speed or calculated speed from the filament max volumetric speed is lower, the "
                       "lowest will be used instead.\n\n"
                       "When printing the sparse layers, if the internal perimeter speed or calculated speed from the filament max "
                       "volumetric speed is lower, the lowest will be used instead.\n\n"
                       "Increasing this speed may affect the tower's stability as well as increase the force with which the nozzle collides "
                       "with any blobs that may have formed on the wipe tower.\n\n"
                       "Before increasing this parameter beyond the default of 90 mm/s, make sure your printer can reliably bridge at the "
                       "increased speeds and that ooze when tool changing is well controlled.\n\n"
    ...
    def->tooltip = L("When interface-layer temperature boost is active, set the nozzle back to print temperature at the start of the prime "
                     "tower so it cools down during the tower.");
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Quality");
    def->tooltip  = L(
        "Adjust this value to prevent short, unclosed walls from being printed, which could increase print time. "
         ...
         "'One wall threshold' is only visible if this setting is set above the default value of 0.5, or if single-wall top surfaces is "
         "enabled.");
    def->sidetext = L("mm"); // milimeters, CIS languages need translation
    def->mode     = comAdvanced;
    def->min      = 0.0;
    ...
    for (auto& opt_key : filament_extruder_override_keys) {
        ...
        if (opt_key == "filament_retraction_length" || opt_key == "filament_z_hop" || opt_key == "filament_long_retractions_when_cut" ||
        ...
        case coFloats:
            def->set_default_value(
                new ConfigOptionFloatsNullable(static_cast<const ConfigOptionFloats*>(it_opt->second.default_value.get())->values));
            break;
        case coPercents:
            def->set_default_value(
                new ConfigOptionPercentsNullable(static_cast<const ConfigOptionPercents*>(it_opt->second.default_value.get())->values));
            break;
        case coBools:
            def->set_default_value(
                new ConfigOptionBoolsNullable(static_cast<const ConfigOptionBools*>(it_opt->second.default_value.get())->values));
            break;
        case coEnums:
            def->set_default_value(new ConfigOptionEnumsGenericNullable(
...
void PrintConfigDef::init_extruder_option_keys()
{
    ...
    m_extruder_option_keys = {"extruder_type",
                              "nozzle_diameter",
    ...
    m_extruder_retract_keys = {"deretraction_speed",
                               "long_retractions_when_cut",
                               "retract_before_wipe",
                               "retract_lift_above",
                               "retract_lift_below",
...
void PrintConfigDef::init_filament_option_keys()
{
    ...
    m_filament_retract_keys = {"deretraction_speed",
                               "long_retractions_when_cut",
                               "retract_before_wipe",
                               "retract_lift_above",
                               "retract_lift_below",
...
void PrintConfigDef::handle_legacy(t_config_option_key& opt_key, std::string& value)
{
    ...
    } else if (opt_key == "nozzle_volume_type" || opt_key == "default_nozzle_volume_type" || opt_key == "printer_extruder_variant" ||
               opt_key == "print_extruder_variant" || opt_key == "filament_extruder_variant" || opt_key == "extruder_variant_list") {
...
std::set<std::string> filament_options_with_variant =
    {"filament_flow_ratio", "filament_max_volumetric_speed",
     ...
     "filament_extruder_variant", "filament_retraction_length", "filament_z_hop", "filament_z_hop_types", "filament_retract_lift_above",
     "filament_retract_lift_below", "filament_retract_lift_enforce", "filament_retract_restart_extra", "filament_retraction_speed",
     "filament_deretraction_speed", "filament_retraction_minimum_travel", "filament_retract_when_changing_layer", "filament_wipe",
     // BBS
     "filament_wipe_distance", "filament_retract_before_wipe", "filament_long_retractions_when_cut",
     "filament_retraction_distances_when_cut", "long_retractions_when_ec", "retraction_distances_when_ec",
     "nozzle_temperature_initial_layer", "nozzle_temperature", "filament_flush_volumetric_speed", "filament_flush_temp",
     "volumetric_speed_coefficients", "filament_adaptive_volumetric_speed", "filament_ironing_flow", "filament_ironing_spacing",
     "filament_ironing_inset", "filament_ironing_speed"};

// Parameters that are the same as the number of extruders
std::set<std::string> printer_extruder_options = {"extruder_type",           "nozzle_diameter",           "default_nozzle_volume_type",
                                                  "extruder_printable_area", "extruder_printable_height", "min_layer_height",
                                                  "max_layer_height"};

std::set<std::string> printer_options_with_variant_1 = {"nozzle_volume",
                                                        "retraction_length",
                                                        "z_hop",
                                                        "travel_slope",
                                                        "retract_lift_above",
...
std::set<std::string> printer_options_with_variant_2 = {"machine_max_acceleration_x",
...
double min_object_distance(const ConfigBase& cfg)
{
    ...
    if (printer_technology == ptSLA)
        ...
        if (!ecr_opt || !co_opt)
            ...
            ret = ((co_opt->value == PrintSequence::ByObject) && ecr_opt->value > duplicate_distance) ? ecr_opt->value : duplicate_distance;
        }
    }
...
t_config_option_keys DynamicPrintConfig::normalize_fdm_2(int num_objects, int used_filaments)
{
    ...
    if (used_filaments > 0 && ept_opt != nullptr) {
        ...
        if (!is_smooth_timelapse && !enable_wrapping &&
            (used_filaments == 1 || (ps_opt->value == PrintSequence::ByObject && num_objects > 1))) {
            if (ept_opt->value) {
                ept_opt->value = false;
...
static void extend_extruder_variant(DynamicPrintConfig& config, const unsigned int num_extruders)
{
    ...
    extruder_variant_opt->resize(num_extruders,
                                 extruder_variant_opt); // Use the first option as the default value, so all extruders have the same variant
    ...
    for (int i = 0; i < num_extruders; i++) {
        ...
        if (!variants_list.empty()) {
            ...
            printer_extruder_variant_opt->values.insert(printer_extruder_variant_opt->values.end(), variants_list.begin(),
                                                        variants_list.end());
        }
    }
}
...
void DynamicPrintConfig::set_num_extruders(unsigned int num_extruders)
{
    ...
    for (const std::string& key : print_config_def.extruder_option_keys()) {
        if (key == "default_filament_profile")
            // Don't resize this field, as it is presented to the user at the "Dependencies" page of the Printer profile and we don't want
            // to present empty fields there, if not defined by the system profile.
            continue;
                                     ...
                                     std::vector<int>&         f_maps)
{
    ...
    if (opt_key == "long_retractions_when_cut" &&
    ...
    if (opt_key == "retraction_distances_when_cut" &&
        new_full_config.option<ConfigOptionInt>("enable_long_retraction_when_cut")->value != LongRectrationLevel::EnableFilament) {
...
std::map<std::string, std::string> validate(const FullPrintConfig& cfg, bool under_cli)
{
    ...
    if (cfg.use_firmware_retraction.value && cfg.gcode_flavor.value != gcfKlipper && cfg.gcode_flavor.value != gcfSmoothie &&
        cfg.gcode_flavor.value != gcfRepRapSprinter && cfg.gcode_flavor.value != gcfRepRapFirmware &&
        cfg.gcode_flavor.value != gcfMarlinLegacy && cfg.gcode_flavor.value != gcfMarlinFirmware &&
        cfg.gcode_flavor.value != gcfMachinekit && cfg.gcode_flavor.value != gcfRepetier)
    ...
    if (cfg.extruder_clearance_height_to_rod <= 0) {
        error_message.emplace("extruder_clearance_height_to_rod",
                              L("invalid value ") + std::to_string(cfg.extruder_clearance_height_to_rod));
    }
    if (cfg.extruder_clearance_height_to_lid <= 0) {
        error_message.emplace("extruder_clearance_height_to_lid",
                              L("invalid value ") + std::to_string(cfg.extruder_clearance_height_to_lid));
    }
    ...
    if (cfg.spiral_mode && under_cli) {
        ...
        if (cfg.sparse_infill_density > 0) {
            error_message.emplace("sparse_infill_density",
                                  L("Invalid value when spiral vase mode is enabled: ") + std::to_string(cfg.sparse_infill_density));
            // return "Spiral vase mode can only print hollow objects, so you need to set Fill density to 0";
        }

        if (cfg.top_shell_layers > 0) {
            error_message.emplace("top_shell_layers",
                                  L("Invalid value when spiral vase mode is enabled: ") + std::to_string(cfg.top_shell_layers));
            // return "Spiral vase mode is not compatible with top solid layers";
        }

        if (cfg.enable_support) {
            error_message.emplace("enable_support",
                                  L("Invalid value when spiral vase mode is enabled: ") + std::to_string(cfg.enable_support));
            // return "Spiral vase mode is not compatible with support";
        }
        if (cfg.enforce_support_layers > 0) {
            error_message.emplace("enforce_support_layers",
                                  L("Invalid value when spiral vase mode is enabled: ") + std::to_string(cfg.enforce_support_layers));
            // return "Spiral vase mode is not compatible with support";
        }
    }
    ...
    // extrusion widths
    {
        ...
        const char* widths[] = {"outer_wall_line_width",     "inner_wall_line_width",
                                "sparse_infill_line_width",  "internal_solid_infill_line_width",
                                "top_surface_line_width",    "support_line_width",
                                "initial_layer_line_width",  "skin_infill_line_width",
    ...
    for (const std::string& opt_key : cfg.keys()) {
        ...
        if (out_of_range) {
            if (error_message.find(opt_key) == error_message.end())
                error_message.emplace(opt_key, opt->serialize() + L(" not in range ") + "[" + std::to_string(optdef->min) + "," +
                                                   std::to_string(optdef->max) + "]");
...
PRINT_CONFIG_CACHE_INITIALIZE((PrintObjectConfig,
                               PrintRegionConfig,
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    */
    ...
    def->tooltip = L("Load and store settings at the given directory. This is useful for maintaining different profiles or including "
...
OtherSlicingStatesConfigDef::OtherSlicingStatesConfigDef()
{
    ...
    new_def("in_head_wrap_detect_zone", coBool, "In head wrap detect zone",
            "Indicates if the first layer overlaps with the head wrap zone.");
}
...
TemperaturesConfigDef::TemperaturesConfigDef()
{
    ...
    new_def("bed_temperature", coInts, "Bed temperature", "Vector of bed temperatures for each extruder/filament.") new_def(
        "bed_temperature_initial_layer", coInts, "Initial layer bed temperature",
        "Vector of initial layer bed temperatures for each extruder/filament. Provides the same value as first_layer_bed_temperature.")
        new_def("bed_temperature_initial_layer_single", coInt, "Initial layer bed temperature (initial extruder)",
                "Initial layer bed temperature for the initial extruder. Same as bed_temperature_initial_layer[initial_extruder]")
            new_def("chamber_temperature", coInts, "Chamber temperature", "Vector of chamber temperatures for each extruder/filament.")
                new_def("overall_chamber_temperature", coInt, "Overall chamber temperature",
                        "Overall chamber temperature. This value is the maximum chamber temperature of any extruder/filament used.")
                    new_def("first_layer_bed_temperature", coInts, "First layer bed temperature",
                            "Vector of first layer bed temperatures for each extruder/filament. Provides the same value as "
                            "bed_temperature_initial_layer.") new_def("first_layer_temperature", coInts, "First layer temperature",
                                                                      "Vector of first layer temperatures for each extruder/filament.")
}
...
const std::map<t_custom_gcode_key, t_config_option_keys>& custom_gcode_specific_placeholders() { return s_CustomGcodeSpecificPlaceholders; }
...
CustomGcodeSpecificConfigDef::CustomGcodeSpecificConfigDef()
{
    ...
    // change_filament_gcode
    new_def("previous_extruder", coInt, "Previous extruder",
            "Index of the extruder that is being unloaded. The index is zero based (first extruder has index 0).");
    new_def("next_extruder", coInt, "Next extruder",
            "Index of the extruder that is being loaded. The index is zero based (first extruder has index 0).");
    ...
    new_def("old_retract_length_toolchange", coFloat, "Old retract length toolchange",
            "The toolchange retraction length of the previous filament.");
    new_def("new_retract_length_toolchange", coFloat, "New retract length toolchange",
            "The toolchange retraction length of the new filament.");
    ...
    std::string extrusion_role_types =
        "Possible Values:\n[\"Perimeter\", \"ExternalPerimeter\", "
        "\"OverhangPerimeter\", \"InternalInfill\", \"SolidInfill\", \"TopSolidInfill\", \"BottomSurface\", \"BridgeInfill\", \"GapFill\", "
        "\"Ironing\", "
    ...
    new_def("last_extrusion_role", coString, "Last extrusion role",
            "The previously used extrusion role/type\nPossible Values:\n" + extrusion_role_types);
}
...
float       get_real_skirt_dist(const DynamicPrintConfig& cfg) { return has_skirt(cfg) ? cfg.opt_float("skirt_distance") : 0; }
...
bool is_XL_printer(const PrintConfig& cfg) { return is_XL_printer(cfg.printer_notes.value); }
} // namespace Slic3r
