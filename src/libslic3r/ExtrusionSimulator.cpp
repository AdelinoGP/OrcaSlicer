...
// [INTENT] Pixel-rasterizer for extrusion width simulation. Converts an ExtrusionPath
// into a floating-point accumulator grid (A2f) whose cells hold material height, then
// colour-maps the result into an RGBA image for the GUI preview.
// [STATE] Uses Boost.Geometry types (point_xy, box, multi_array) throughout — NOT Eigen
// or Clipper. All internal vector/matrix types are V2f/V2i/V2d/B2f/B2i/A2f/A2uc aliases.
// [COUPLING] Consumed by ExtrusionSimulator (public API in ExtrusionSimulator.hpp).
//            Has no TBB parallelism — purely sequential.
// [HAZARD H1133] ExtrusionSimulatorImpl uses raw new/delete pimpl pattern (ctor: new,
//   dtor: delete pimpl; pimpl = NULL). Not exception-safe; any throw between construction
//   and destruction leaks. Port target should use unique_ptr<ExtrusionSimulatorImpl>.
...
template<typename T> inline void operator+=(boost::geometry::model::d2::point_xy<T>& v1, const boost::geometry::model::d2::point_xy<T>& v2)
{
...
template<typename T> inline void operator*=(boost::geometry::model::d2::point_xy<T>& v, const T c)
{
    boost::geometry::multiply_value(v, c);
}

template<typename T> inline void operator/=(boost::geometry::model::d2::point_xy<T>& v, const T c) { boost::geometry::divide_value(v, c); }

template<typename T>
inline typename boost::geometry::model::d2::point_xy<T> operator+(const boost::geometry::model::d2::point_xy<T>& v1,
...
template<typename T>
inline boost::geometry::model::d2::point_xy<T> operator-(const boost::geometry::model::d2::point_xy<T>& v1,
                                                         const boost::geometry::model::d2::point_xy<T>& v2)
{
    boost::geometry::model::d2::point_xy<T> out(v1);
    out -= v2;
...
template<typename T> inline boost::geometry::model::d2::point_xy<T> operator*(const boost::geometry::model::d2::point_xy<T>& v, const T c)
{
...
template<typename T>
inline typename boost::geometry::model::d2::point_xy<T> operator*(const T c, const boost::geometry::model::d2::point_xy<T>& v)
{
    boost::geometry::model::d2::point_xy<T> out(v);
    out *= c;
    return out;
...
template<typename T>
inline typename boost::geometry::model::d2::point_xy<T> operator/(const boost::geometry::model::d2::point_xy<T>& v, const T c)
{
    boost::geometry::model::d2::point_xy<T> out(v);
    out /= c;
    return out;
...
template<typename T> inline T dot(const boost::geometry::model::d2::point_xy<T>& v1, const boost::geometry::model::d2::point_xy<T>& v2)
{
    return boost::geometry::dot_product(v1, v2);
}

template<typename T> inline T dot(const boost::geometry::model::d2::point_xy<T>& v) { return boost::geometry::dot_product(v, v); }

template<typename T> inline T cross(const boost::geometry::model::d2::point_xy<T>& v1, const boost::geometry::model::d2::point_xy<T>& v2)
{
    return v1.x() * v2.y() - v2.x() * v1.y();
}

// Euclidian measure
template<typename T> inline T l2(const boost::geometry::model::d2::point_xy<T>& v) { return std::sqrt(dot(v)); }

// Euclidian measure
template<typename T> inline T mag(const boost::geometry::model::d2::point_xy<T>& v) { return l2(v); }

template<typename T>
inline T dist2_to_line(const boost::geometry::model::d2::point_xy<T>& p0,
...
template<typename T>
int line_circle_intersection(const boost::geometry::model::d2::point_xy<T>& p0,
                             const boost::geometry::model::d2::point_xy<T>& p1,
...
template<typename T>
int clip_rect_by_AABB(boost::geometry::model::d2::point_xy<T>                                     rect[8],
...
// [INTENT] clip_circle_by_AABB: Clips a circle against an AABB polygon, producing a
// polygon approximation. The circular arc is approximated by its chord (linear segment),
// NOT analytically integrated — area calculation is approximate.
// [HAZARD H1132] Large /* ... */ commented-out block below (circle_AABB_intersection_area)
//   is a dead alternative implementation. It also contains a syntax error
//   (boost::geometry::point_in_box used as function object incorrectly). Must not be
//   compiled; remove on porting. The #if 0 span-based circle fill in gcode_spread_points
//   is similarly dead code — replaced by the active pixel-loop in the #else branch.
template<typename T>
int clip_circle_by_AABB(const boost::geometry::model::d2::point_xy<T>&                              center,
                        const T                                                                     radius,
                        const boost::geometry::model::box<boost::geometry::model::d2::point_xy<T>>& aabb,
                        ...
                        bool                                                                        result_arc[8])
{
    ...
    V2T rect[4] = {aabb.min_corner(), V2T(aabb.max_corner().x(), aabb.min_corner().y()), aabb.max_corner(),
...
template<typename T> inline T polyArea(const boost::geometry::model::d2::point_xy<T>* poly, int n)
{
    T area = T(0);
    for (int i = 1; i + 1 < n; ++i)
...
template<typename T> boost::geometry::model::d2::point_xy<T> polyCentroid(const boost::geometry::model::d2::point_xy<T>* poly, int n)
{
...
// [INTENT] gcode_paint_layer: Rasterizes an extrusion polyline into the float accumulator
// grid (A2f). For each line segment, builds a bounding rectangle, clips it against each
// pixel cell using Sutherland-Hodgman (clip_rect_by_AABB), and accumulates
// area * thickness into acc[j][i]. This represents deposited material height per cell.
// [STATE] `thickness` parameter is passed in as 0.5f from extrude_to_accumulator —
// see HAZARD H1130 below.
void gcode_paint_layer(const std::vector<V2f>& polyline, float width, float thickness, A2f& acc)
{
    ...
    for (size_t iLine = 1; iLine != polyline.size(); ++iLine) {
        ...
        B2i bboxLinei(V2i(std::clamp(int(floor(bboxLine.min_corner().x())), 0, nc - 1),
        ...
        // printf("bboxLinei %d,%d %d,%d\n", bboxLinei.min_corner().x(), bboxLinei.min_corner().y(), bboxLinei.max_corner().x(),
        // bboxLinei.max_corner().y());
#ifdef _DEBUG
...
struct Cell
{
    ...
    bool operator<(const Cell& c2) const { return this->excess_height < c2.excess_height; }
};
...
// [INTENT] gcode_spread_points: Spreads excess material across cells under each extrusion
// circle. Two modes: SpreadFull (average height over all cells) and SpreadExcess
// (only push excess material down to deficit cells, using a prefix-sum sort).
// [STATE] The #if 0 block above the #else is dead span-based circle-fill code
// (replaced by the active pixel-loop in the #else branch). See HAZARD H1132.
// [MEMORY] cells/areas_sum vectors are pre-allocated to n_cells_max = ceil(rmax*2+2)^2;
// no dynamic realloc during inner loops.
void gcode_spread_points(A2f& acc, const A2f& mask, const ExtrusionPoints& points, ExtrusionSimulationType simulationType)
{
    ...
    for (ExtrusionPoints::const_iterator it = points.begin(); it != points.end(); ++it) {
        ...
        B2i bboxi(V2i(std::clamp(int(floor(bbox.min_corner().x())), 0, nc - 1), std::clamp(int(floor(bbox.min_corner().y())), 0, nr - 1)),
                  V2i(std::clamp(int(ceil(bbox.max_corner().x())), 0, nc - 1), std::clamp(int(ceil(bbox.max_corner().y())), 0, nr - 1)));
        /*
        ...
        */
        ...
        //		if (true) {
        //		printf("volume_total: %f, volume_full: %f, fill factor: %f\n", volume_total, volume_full, 100.f - 100.f * volume_total /
        // volume_full); 		printf("volume_full: %f, volume_excess+deficit: %f, volume_excess: %f, volume_deficit: %f\n", volume_full,
        // volume_excess+volume_deficit, volume_excess, volume_deficit);
        if (simulationType == ExtrusionSimulationSpreadFull || volume_total <= volume_full) {
            ...
            for (size_t i = 0; i < n_cells; ++i) {
                ...
                acc[cell.idx.y()][cell.idx.x()] = (1.f - cell.fraction_covered) * cell.volume +
...
void ExtrusionSimulator::set_image_size(const Point& image_size)
{
    ...
    if (this->image_size.x() == image_size.x() && this->image_size.y() == image_size.y())
        return;
    ...
    // [HAZARD H1131] Debug pattern leftover in production code. Every time the image size
    // changes, even pixel columns are filled with full-opacity red (R=255, A=255).
    // This is visible on screen until evaluate_accumulator() overwrites the pixels.
    // The FIXME comment acknowledges the issue but no fix has been applied across
    // multiple upstream generations. Port target should remove this block entirely
    // and initialize image_data to transparent black (all zeros).
...
void ExtrusionSimulator::set_bounding_box(const BoundingBox& bbox) { this->bbox = bbox; }

const void* ExtrusionSimulator::image_ptr() const { return (pimpl->image_data.empty()) ? NULL : (void*) &pimpl->image_data.front(); }
...
void ExtrusionSimulator::extrude_to_accumulator(const ExtrusionPath& path, const Point& shift, ExtrusionSimulationType simulationType)
{
    ...
    // [HAZARD H1130] path.height is read from the ExtrusionPath but immediately discarded.
    // ept.height is hardcoded to 0.5f, and gcode_paint_layer is called with 0.5f as
    // thickness. The commented-out line "//float h = scale_(path.height) * scalex;" shows
    // the intent to use actual layer height, but it was never enabled. All simulation
    // results are therefore based on a fixed 0.5mm height regardless of actual layer
    // height. Port target must decide whether to preserve this approximation or fix it.
...
void ExtrusionSimulator::evaluate_accumulator(ExtrusionSimulationType simulationType)
{
    ...
    // [INTENT] Optionally spread material, then colour-map accumulator → RGBA image.
    // [HAZARD H1132 continued] The #if 1 / #else block in the colour-mapping loop
    // is a compile-time display-mode switch between accumulator and mask visualisation.
    // The #else branch (mask display) is permanently dead. Port target should remove
    // the conditional and always use pimpl->accumulator[r][c].
    Point sz = viewport.size();
