...
// [INTENT] QEM (Quadric Error Metrics) mesh decimation. Reduces an indexed_triangle_set
// to triangle_count triangles by iteratively collapsing edges with minimum quadric error.
// Algorithm: Garland & Heckbert 1997.
// [STATE] Two TBB parallel_for passes in init() (normals + errors); quadric sum and
// edge-info construction are sequential. Main collapse loop is single-threaded.
// [COUPLING] Entry point: its_quadric_edge_collapse() in QuadricEdgeCollapse.hpp.
//            cancel/progress via throw_on_cancel / status_fn callbacks.
// [MEMORY] MutablePriorityQueue (miniheap, 32-element pages) owns Error objects keyed
//          by triangle index. ti_2_mpqi maps triangle index → priority queue index.
...
public:
    ...
    {
        T det = m[a11] * m[a22] * m[a33] + m[a13] * m[a21] * m[a32] + m[a12] * m[a23] * m[a31] - m[a13] * m[a22] * m[a31] -
...
struct Error
{
    ...
    Error(float value, uint32_t triangle_index) : value(value), triangle_index(triangle_index) {}
    Error() = default;
};
...
struct TriangleInfo
{
    ...
    // [HAZARD H1134] Deletion encoded by setting n.x() > 2.f (valid normals are in [-1,1]).
    // Identical aliasing pattern to TriangleSelector.cpp H1103.
    // Porting risk: any language/renderer that stores or inspects the normal field will misread
    // deleted triangles as degenerate geometry rather than tombstoned entries.  Replace with an
    // explicit `bool deleted` flag or a side-channel bitset.
...
struct CopyEdgeInfo
{
    ...
    uint32_t count;
    uint32_t move;
    CopyEdgeInfo(uint32_t start, uint32_t count, uint32_t move) : start(start), count(count), move(move) {}
};
...
std::tuple<TriangleInfos, VertexInfos, EdgeInfos, Errors> init(const indexed_triangle_set& its,
                                                               ...
                                                               StatusFn&                   status_fn);
std::optional<uint32_t>                                   find_triangle_index1(
                                      uint32_t vi, const VertexInfo& v_info, uint32_t ti, const EdgeInfos& e_infos, const Indices& indices);
void reorder_edges(EdgeInfos& e_infos, const VertexInfo& v_info, uint32_t ti0, uint32_t ti1);
bool is_flipped(const Vec3f&                new_vertex,
                uint32_t                    ti0,
                uint32_t                    ti1,
...
bool create_no_volume(uint32_t          vi0,
                      uint32_t          vi1,
                      uint32_t          ti0,
                      uint32_t          ti1,
...
void  change_neighbors(EdgeInfos&      e_infos,
                       VertexInfos&    v_infos,
                       uint32_t        ti0,
                       uint32_t        ti1,
                       uint32_t        vi0,
...
void store_surround(const char*                 obj_filename,
                    ...
                    const EdgeInfos&            e_infos);
bool check_neighbors(const indexed_triangle_set& its, const TriangleInfos& t_infos, const VertexInfos& v_infos, const EdgeInfos& e_infos);
...
{
    // check input
    if (triangle_count >= its.indices.size())
        return;
    float maximal_error = (max_error == nullptr) ? std::numeric_limits<float>::max() : *max_error;
    if (maximal_error <= 0.f)
        return;
    if (throw_on_cancel == nullptr)
        throw_on_cancel = []() {};
    ...
    uint32_t status_mod = std::max(uint32_t(16), count_triangle_to_reduce / (100 - status_init_size));
    ...
    while (actual_triangle_count > triangle_count && !mpq.empty()) {
        ...
        auto     ti1_opt = (v_info0.count < v_info1.count) ? find_triangle_index1(vi1, v_info0, ti0, e_infos, its.indices) :
        ...
        if (!ti1_opt.has_value() || // edge has only one triangle
            degenerate(vi0, ti0, ti1, v_info1, e_infos, its.indices) || degenerate(vi1, ti0, ti1, v_info0, e_infos, its.indices) ||
            ...
            Vec3i32 ord    = (errors[0] < errors[1]) ?
                                 ((errors[0] < errors[2]) ? ((errors[1] < errors[2]) ? Vec3i32(0, 1, 2) : Vec3i32(0, 2, 1)) :
                                                            Vec3i32(2, 0, 1)) :
                                 ((errors[1] < errors[2]) ? ((errors[0] < errors[2]) ? Vec3i32(1, 0, 2) : Vec3i32(1, 2, 0)) : Vec3i32(2, 1, 0));
        ...
        change_neighbors(e_infos, v_infos, ti0, ti1, vi0, vi1, vi_top0, t1, ceis, e_infos_swap);
...
Vec3d QuadricEdgeCollapse::create_normal(const Triangle& triangle, const Vertices& vertices)
{
...
double QuadricEdgeCollapse::calculate_determinant(const SymMat& q) { return q.det(0, 1, 2, 1, 4, 5, 2, 5, 7); }
...
std::array<double, 3> QuadricEdgeCollapse::vertices_error(const SymMat& q, const std::array<Vec3d, 3>& vertices)
{
    return {vertex_error(q, vertices[0]), vertex_error(q, vertices[1]), vertex_error(q, vertices[2])};
}

double QuadricEdgeCollapse::calculate_error(uint32_t id_v1, uint32_t id_v2, const SymMat& q, const Vertices& vertices)
{
...
Vec3f QuadricEdgeCollapse::calculate_vertex(uint32_t id_v1, uint32_t id_v2, const SymMat& q, const Vertices& vertices)
{
    double det = calculate_determinant(q);
    if (std::abs(det) < std::numeric_limits<double>::epsilon()) {
        // can't divide by zero
...
SymMat QuadricEdgeCollapse::create_quadric(const Triangle& t, const Vec3d& n, const Vertices& vertices)
{
...
std::tuple<TriangleInfos, VertexInfos, EdgeInfos, Errors> QuadricEdgeCollapse::init(const indexed_triangle_set& its,
                                                                                    ...
                                                                                    StatusFn&                   status_fn)
{
    // [INTENT] Build all auxiliary data for the collapse loop:
    //   Pass 1 (parallel): compute per-triangle normals + quadrics into triangle_quadrics[].
    //   Pass 2 (sequential): accumulate per-vertex quadric sums (v_info.q) + count neighbor triangles.
    //   Pass 3 (sequential): set v_info.start offsets (prefix-sum over counts).
    //   Pass 4 (parallel): compute per-triangle minimum-edge errors → errors[].
    //   Pass 5 (sequential): fill e_infos[] reference array (vertex → triangle adjacency).
    // [STATE] triangle_quadrics[] is a temporary; destroyed at end of inner scope.
    //         e_infos has size = indices.size()*3; every vertex's edge block is contiguous
    //         and addressed via v_info.{start,count}.
    // [CONCURRENCY] Passes 1 and 4 use tbb::parallel_for over triangle indices.
    //               throw_on_cancel() is called inside the parallel lambdas at 10^6-triangle
    //               intervals — safe only if the callback itself is thread-safe.
    //               Passes 2, 3, 5 are strictly sequential; no TBB.
    int           status_offset = 0;
...
std::optional<uint32_t> QuadricEdgeCollapse::find_triangle_index1(
    uint32_t vi, const VertexInfo& v_info, uint32_t ti0, const EdgeInfos& e_infos, const Indices& indices)
{
    ...
    for (uint32_t ei = v_info.start; ei < end; ++ei) {
        ...
        if (t[(e_info.edge + 1) % 3] == vi_coord || t[(e_info.edge + 2) % 3] == vi_coord)
...
void QuadricEdgeCollapse::reorder_edges(EdgeInfos& e_infos, const VertexInfo& v_info, uint32_t ti0, uint32_t ti1)
{
...
bool QuadricEdgeCollapse::is_flipped(const Vec3f&                new_vertex,
                                     ...
                                     const indexed_triangle_set& its)
{
    // [INTENT] Guard: reject a collapse if moving a vertex to new_vertex would flip any
    //          neighbouring triangle's normal more than ~80° (dot_thr = 0.2) or produce a
    //          degenerate triangle (parallel edges → |dot| ≈ 1).
    // [STATE] dot_thr = 0.2f is a hardcoded magic number (≈80°); no config exposure.
...
bool QuadricEdgeCollapse::degenerate(
    uint32_t vi, uint32_t ti0, uint32_t ti1, const VertexInfo& v_info, const EdgeInfos& e_infos, const Indices& indices)
{
    // [INTENT] Guard: reject a collapse if any triangle in v_info's neighbourhood already
    //          contains vertex index vi — collapsing would create a triangle with two
    //          identical vertex indices (degenerate face).
    ...
    for (size_t ei = v_info.start; ei < v_info_end; ++ei) {
        ...
        for (size_t i = 0; i < 3; ++i)
            if (static_cast<uint32_t>(t[i]) == vi)
                return true;
    }
    return false;
}
                                           ...
                                           const Indices&    indices)
{
    ...
    for (size_t ei0 = v_info0.start; ei0 < v_info0_end; ++ei0) {
        ...
        for (size_t ei1 = v_info1.start; ei1 < v_info1_end; ++ei1) {
            ...
            size_t          t1i     = 0;
            for (; t1i < 3; ++t1i)
                if (static_cast<uint32_t>(t1[t1i]) == t0vi1)
                    break;
            if (t1i >= 3)
                continue; // without vertex index from triangle 0
            // check if second index is same too
            ++t1i;
            if (t1i == 3)
                t1i = 0; // triangle loop(modulo 3)
            if (static_cast<uint32_t>(t1[t1i]) == vi1) {
                ++t1i;
                if (t1i == 3)
                    t1i = 0; // triangle loop(modulo 3)
            }
            if (static_cast<uint32_t>(t1[t1i]) == t0vi0)
                return true;
        }
    }
    return false;
...
Vec3d QuadricEdgeCollapse::calculate_3errors(const Triangle& t, const Vertices& vertices, const VertexInfos& v_infos)
{
    Vec3d error;
    for (size_t j = 0; j < 3; ++j) {
        size_t   j2  = (j == 2) ? 0 : (j + 1);
...
Error QuadricEdgeCollapse::calculate_error(
    uint32_t ti, const Triangle& t, const Vertices& vertices, const VertexInfos& v_infos, unsigned char& min_index)
{
    Vec3d error = calculate_3errors(t, vertices, v_infos);
    // select min error
    min_index = (error[0] < error[1]) ? ((error[0] < error[2]) ? 0 : 2) : ((error[1] < error[2]) ? 1 : 2);
    return Error(static_cast<float>(error[min_index]), ti);
}

void QuadricEdgeCollapse::remove_triangle(EdgeInfos& e_infos, VertexInfo& v_info, uint32_t ti)
{
...
void QuadricEdgeCollapse::change_neighbors(EdgeInfos&      e_infos,
                                           ...
                                           EdgeInfos&      e_infos1)
{
    // [INTENT] Merge the edge-info adjacency blocks of vi1 into vi0 after collapsing edge
    //          (vi0,vi1).  The two degenerated triangles (ti0, ti1) are tombstoned.
    //          The flat e_infos[] array is updated in-place: blocks for vertices between
    //          vi0+1 and vi1 may be shifted right by `need` slots to make room.
    // [HAZARD H1135] Most complex mutation in the file.  The algorithm walks v_infos[]
    //   scanning for contiguous free slots between vertex blocks; it then shifts ranges
    //   rightward in reverse order (infos[]) to avoid clobbering unread data.
    //   Invariants required: (a) vi0 < vi1 (enforced by caller swap), (b) e_infos1 is a
    //   scratch buffer large enough for v_info1.count-2 entries, (c) v_info0.start+count
    //   never exceeds e_infos.size().
    //   Any porting that linearises or reorganises v_infos / e_infos (e.g. flattening to a
    //   hash map) must replicate this in-place shift logic exactly or replace it with a
    //   dynamic adjacency list per vertex.
    // [MEMORY] infos and e_infos1 are caller-provided scratch vectors; cleared+reserved
    //          here on each call to amortise allocations.
    assert(vi0 < vi1);
    ...
    uint32_t need = (new_triangle_count < v_info0.count) ? 0 : (new_triangle_count - v_info0.count);
...
{
    // [INTENT] Final compaction pass: rebuild its.vertices and its.indices in-place by
    //          squeezing out deleted entries.  Uses e_infos to patch vertex indices in
    //          surviving triangles before copying vertices forward.
    // [STATE] Modifies its.vertices and its.indices directly via erase().
    //         Does NOT touch e_infos or v_infos — they are abandoned after this call.
    uint32_t vi_new = 0;
                                          ...
                                          const EdgeInfos&            e_infos)
{
    ...
    for (size_t vi = 0; vi < its.vertices.size(); vi++) {
        ...
        std::copy(e_infos.begin() + v_info.start, e_infos.begin() + v_info.start + v_info.count, std::back_inserter(eis));
        auto compare = [](const EdgeInfo& ei1, const EdgeInfo& ei2) { return ei1.t_index < ei2.t_index; };
        std::sort(eis.begin(), eis.end(), compare);
        std::sort(e_infos2.begin() + v_info2.start, e_infos2.begin() + v_info2.start + v_info2.count, compare);
