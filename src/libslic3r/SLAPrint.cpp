...
// [INTENT] SLAPrint.cpp — implements the SLA print FSM: configuration diffing,
//   model object synchronization, per-object step invalidation, and the top-level
//   process() loop.
//
// Key design decisions documented here:
//   - apply(): diffs four config layers (print/printer/material/object) and
//     propagates changes to invalidation chains.
//   - sla_trafo(): builds the object-level Transform3d from instances.front() only.
//     See [HAZARD H806] — all instances are assumed to share X/Y rotation and scale.
//   - invalidate_state_by_config_options(): classifies opt_keys into three buckets
//     (steps_full, steps_rasterize, steps_ignore). Unknown keys trigger assert(false)
//     in debug, silently pass in release — see [HAZARD H807].
//   - process(): two-phase per-object loop (level1: slaposHollowing..slaposPad,
//     level2: slaposSliceSupports) followed by two print-level steps.
//     No TBB at this level; each step may use TBB internally.
//
// [CONCURRENCY] state_mutex() (from PrintBase) guards all step state transitions.
//   The background thread must not access m_objects or m_printer_input without the lock.
//
// [MEMORY] SLAPrintObjects are heap-allocated and manually deleted in apply() and clear().
//   No shared_ptr; lifetimes are strictly controlled by SLAPrint.
...
// [INTENT] is_zero_elevation: true when pad_around_object is on — object sits on build plate,
//   elevation = 0. Drives object_elevation_mm suppression in make_support_cfg().
bool is_zero_elevation(const SLAPrintObjectConfig& c) { return c.pad_enable.getBool() && c.pad_around_object.getBool(); }

// [INTENT] make_support_cfg: converts SLAPrintObjectConfig to sla::SupportTreeConfig.
//   All dimension params are in mm (float/double). head_fallback_radius uses percent scaling.
//   bridge_slope is converted from degrees to radians here.
// [COUPLING] Called by validate() for constraint checks and by SLAPrintSteps::support_tree().
sla::SupportTreeConfig make_support_cfg(const SLAPrintObjectConfig& c)
{
    ...
    scfg.head_fallback_radius_mm     = 0.01 * c.support_small_pillar_diameter_percent.getFloat() * pillar_r;
    ...
    case slapcmZigZag: scfg.pillar_connection_mode = sla::PillarConnectionMode::zigzag; break;
    case slapcmCross: scfg.pillar_connection_mode = sla::PillarConnectionMode::cross; break;
    case slapcmDynamic: scfg.pillar_connection_mode = sla::PillarConnectionMode::dynamic; break;
    }
    ...
    scfg.pillar_base_safety_distance_mm = c.support_base_safety_distance.getFloat() < EPSILON ? scfg.safety_distance_mm :
                                                                                                c.support_base_safety_distance.getFloat();
...
sla::PadConfig::EmbedObject builtin_pad_cfg(const SLAPrintObjectConfig& c)
{
    ...
    if (ret.enabled) {
        ...
        ret.stick_penetration_mm = c.pad_object_connector_penetration.getFloat();
    }
...
// [INTENT] sla_trafo: builds the per-object Transform3d used for slicing.
//   Strips X and Y offset (sets to 0) and strips Z rotation (sets to 0) so that
//   the returned trafo represents only: Z offset, XY/Z scale/mirror, and X/Y rotation.
//   The removed X/Y offset and Z rotation become Instance::shift and Instance::rotation.
// [COUPLING] Only uses instances.front() — assumes all instances share rotation/scale.
//   This is a valid SLA constraint (one orientation per object) but is not enforced at
//   the data level. See [HAZARD H806] in SLAPrint.hpp.
// [COUPLING] Calls relative_correction() to bake printer+material XYZ scale correction
//   directly into the transform matrix.
    ...
    if (!model_object.instances.empty()) {
        ...
        for (ModelInstance* model_instance : model_object.instances)
            if (model_instance->is_printable()) {
                instances.emplace_back(model_instance->id(), Point::new_scale(model_instance->get_offset(X), model_instance->get_offset(Y)),
...
// [INTENT] apply(): the entry point for model/config changes from the UI.
//   Three-phase update:
//     1. Diff four config layers (print/printer/material/object) — propagates invalidation.
//     2. Sync ModelObjects: handles New / Old / Moved / Deleted status transitions.
//     3. Map ModelObjects to SLAPrintObjects: reuse existing or construct new ones.
// [STATE] Holds state_mutex() for the entire method to prevent background process()
//   from reading m_objects while they are being reorganized.
// [MEMORY] SLAPrintObjects are heap-allocated with `new SLAPrintObject(...)`.
//   Deleted PrintObjects are freed inline; no RAII (no unique_ptr).
// [COUPLING] Calls sla_trafo() to set the transform on each PrintObject — triggers
//   m_transformed_rmesh invalidation (the mesh cache is lazily re-computed on next access).
SLAPrint::ApplyStatus SLAPrint::apply(const Model& model, DynamicPrintConfig config)
{
    ...
    if (m_printer)
        m_printer->apply(m_printer_config);
        ...
        if (it_status->status != ModelObjectStatus::New) {
            ...
            bool sla_trafo_differs  = model_object.instances.empty() != model_object_new.instances.empty() ||
                                     (!model_object.instances.empty() && (!sla_trafo(model_object).isApprox(sla_trafo(model_object_new)) ||
                                                                          model_object.instances.front()->is_left_handed() !=
                                                                              model_object_new.instances.front()->is_left_handed()));
...
StringObjectException SLAPrint::validate(StringObjectException*                  exception,
                                         ...
                                         std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    for (SLAPrintObject* po : m_objects) {
        ...
        if (supports_en && mo->sla_points_status == sla::PointsStatus::UserModified && mo->sla_support_points.empty())
        ...
        if (supports_en && builtinpad.enabled && cfg.pillar_base_safety_distance_mm < builtinpad.object_gap_mm) {
            return {L("The endings of the support pillars will be deployed on the "
...
// [INTENT] process(): top-level SLA slicing driver.
//   Two-level object step ordering:
//     level1 (slaposHollowing..slaposPad): process ALL objects before advancing to level2.
//     level2 (slaposSliceSupports): run after level1 so GUI preview can show support slices
//     sooner while geometry is still being rasterized.
//   Followed by two print-level steps: slapsMergeSlicesAndEval, slapsRasterize.
// [CONCURRENCY] Serial at this level; each step (hollow_model, slice_model, etc.) uses
//   TBB internally. Cancellation is checked between every step via throw_if_canceled().
// [STATE] m_stepmask[step] gates whether a step is executed. set_task() manipulates these
//   masks for priority / partial processing; finalize() resets them all to true.
// [COUPLING] Steps::execute() dispatches to the concrete step implementation.
//   The `bench` struct is either a real Benchmark (SLAPRINT_DO_BENCHMARK) or a no-op stub.
void SLAPrint::process(long long* time_cost_with_cache, bool use_cache)
{
    ...
    std::vector<SLAPrintObjectStep> level1_obj_steps = {slaposHollowing,     slaposDrillHoles,  slaposObjectSlice,
                                                        slaposSupportPoints, slaposSupportTree, slaposPad};

    // and then slice all supports to allow preview to be displayed ASAP
    std::vector<SLAPrintObjectStep> level2_obj_steps = {slaposSliceSupports};
    ...
    struct
    {
        void   start() {}
        void   stop() {}
        double getElapsedSec() { return .0; }
    } bench;
#endif

    std::array<double, slaposCount + slapsCount> step_times{};

    auto apply_steps_on_objects = [this, &st, &printsteps, &step_times, &bench](const std::vector<SLAPrintObjectStep>& steps) {
        double incr = 0;
        for (SLAPrintObject* po : m_objects) {
    ...
    for (double t : step_times)
        csvbenchstr += std::to_string(t) + ";";

    std::cout << "Performance stats: \n" << csvbenchstr << std::endl;
#endif
}

// [INTENT] invalidate_state_by_config_options: classifies changed config keys into
//   three buckets and propagates invalidation:
//     steps_full       → invalidates slapsMergeSlicesAndEval + slaposObjectSlice + all objects
//     steps_rasterize  → only invalidates slapsMergeSlicesAndEval (raster params, display dims)
//     steps_ignore     → no invalidation (read-only printer metadata, bottle costs, etc.)
// [HAZARD H807] Unrecognized keys hit `assert(false)` — debug only.
//   In release builds new config keys silently fall through with no invalidation.
//   Any config key added to PrintConfig.hpp without a matching entry here will be silently lost.
bool SLAPrint::invalidate_state_by_config_options(const std::vector<t_config_option_key>& opt_keys, bool& invalidate_all_model_objects)
{
    ...
    static std::unordered_set<std::string> steps_full = {"initial_layer_height",  "material_correction",       "material_correction_x",
                                                         "material_correction_y", "material_correction_z",     "relative_correction",
                                                         "relative_correction_x", "relative_correction_y",     "relative_correction_z",
                                                         "absolute_correction",   "elefant_foot_compensation", "elefant_foot_min_width",
                                                         "gamma_correction"};
    ...
    static std::unordered_set<std::string> steps_rasterize =
        {"min_exposure_time",     "max_exposure_time", "exposure_time",      "min_initial_exposure_time", "max_initial_exposure_time",
         "initial_exposure_time", "display_width",     "display_height",     "display_pixels_x",          "display_pixels_y",
         "display_mirror_x",      "display_mirror_y",  "display_orientation"};

    static std::unordered_set<std::string> steps_ignore = {"printable_area", "printable_height", "printer_technology", "filename_format",
                                                           "fast_tilt_time", "slow_tilt_time",   "area_fill",          "bottle_cost",
                                                           "bottle_volume",  "bottle_weight",    "material_density"};
...
SLAPrintObject::SLAPrintObject(SLAPrint* print, ModelObject* model_object)
    : Inherited(print, model_object), m_stepmask(slaposCount, true), m_transformed_rmesh([this](TriangleMesh& obj) {
...
bool SLAPrintObject::invalidate_state_by_config_options(const std::vector<t_config_option_key>& opt_keys)
{
    ...
    bool                            invalidated = false;
    for (const t_config_option_key& opt_key : opt_keys) {
        if (opt_key == "hollowing_enable" || opt_key == "hollowing_min_thickness" || opt_key == "hollowing_quality" ||
            opt_key == "hollowing_closing_distance") {
            steps.emplace_back(slaposHollowing);
        } else if (opt_key == "layer_height" || opt_key == "faded_layers" || opt_key == "pad_enable" || opt_key == "pad_wall_thickness" ||
                   opt_key == "supports_enable" || opt_key == "support_object_elevation" || opt_key == "pad_around_object" ||
                   opt_key == "pad_around_object_everywhere" || opt_key == "slice_closing_radius") {
            steps.emplace_back(slaposObjectSlice);
        } else if (opt_key == "support_points_density_relative" || opt_key == "support_points_minimal_distance") {
            steps.emplace_back(slaposSupportPoints);
        } else if (opt_key == "support_head_front_diameter" || opt_key == "support_head_penetration" || opt_key == "support_head_width" ||
                   opt_key == "support_pillar_diameter" || opt_key == "support_small_pillar_diameter_percent" ||
                   opt_key == "support_max_bridges_on_pillar" || opt_key == "support_pillar_connection_mode" ||
                   opt_key == "support_buildplate_only" || opt_key == "support_base_diameter" || opt_key == "support_base_height" ||
                   opt_key == "support_critical_angle" || opt_key == "support_max_bridge_length" ||
                   opt_key == "support_max_pillar_link_distance" || opt_key == "support_base_safety_distance") {
            steps.emplace_back(slaposSupportTree);
        } else if (opt_key == "pad_wall_height" || opt_key == "pad_brim_size" || opt_key == "pad_max_merge_distance" ||
                   opt_key == "pad_wall_slope" || opt_key == "pad_edge_radius" || opt_key == "pad_object_gap" ||
                   opt_key == "pad_object_connector_stride" || opt_key == "pad_object_connector_width" ||
...
bool SLAPrintObject::invalidate_all_steps() { return Inherited::invalidate_all_steps() | m_print->invalidate_all_steps(); }

double SLAPrintObject::get_elevation() const
{
    if (is_zero_elevation(m_config))
        return 0.;
...
const ExPolygons& SliceRecord::get_slice(SliceOrigin o) const
{
    ...
    const std::vector<ExPolygons>& v = o == soModel ? m_po->get_model_slices() : m_po->get_support_slices();
...
{
    switch (step) {
    case slaposDrillHoles: return m_hollowing_data && !m_hollowing_data->hollow_mesh_with_holes.empty();
    case slaposSupportTree: return !this->support_mesh().empty();
    case slaposPad: return !this->pad_mesh().empty();
...
{
    switch (step) {
    case slaposSupportTree: return this->support_mesh();
    case slaposPad: return this->pad_mesh();
    case slaposDrillHoles:
        if (m_hollowing_data)
            return get_mesh_to_print();
        [[fallthrough]];
    default: return TriangleMesh();
    }
}
...
{
    if (m_hollowing_data && m_hollowing_data->interior && m_config.hollowing_enable.getBool())
...
// [INTENT] transformed_drainhole_points: applies the object trafo to all drain hole positions
//   and normals, then extends each hole out of the mesh surface by HoleStickOutLength to avoid
//   numerical issues when the hole mouth is placed on a nearly-flat surface.
// [MEMORY] Returns by value (copies all DrainHole entries from model_object).
// [COUPLING] Uses instances.front() for scaling — consistent with sla_trafo() assumption.
// [MATH] Normal transform uses covector formula: divide each component by sc(i)^2
//   to correctly transform surface normals under non-uniform scaling.
//   This is mathematically correct (normals are covectors, transform as M^{-T}).
//   The double application (tr * normal - translation) followed by scale^-2 division
//   is intentional — but the reasoning is entirely undocumented in original code.
sla::DrainHoles SLAPrintObject::transformed_drainhole_points() const
{
    ...
    for (sla::DrainHole& hl : pts) {
        ...
        hl.normal = Vec3f(hl.normal(0) / (sc(0) * sc(0)), hl.normal(1) / (sc(1) * sc(1)), hl.normal(2) / (sc(2) * sc(2)));
...
{
    DynamicConfig config;
    for (const char* key : {"print_time", "total_cost", "total_weight", "objects_used_material", "support_used_material"})
...
void SLAPrint::StatusReporter::operator()(SLAPrint& p, double st, const std::string& msg, unsigned flags, const std::string& logmsg)
{
    m_st = st;
    BOOST_LOG_TRIVIAL(info) << st << "% " << msg << (logmsg.empty() ? "" : ": ") << logmsg << log_memory_info();
