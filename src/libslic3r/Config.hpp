// [INTENT] Central configuration type hierarchy for OrcaSlicer.
// All user-visible settings (print, filament, printer) are stored and transported as ConfigOption
// subclasses.  Two concrete stores exist:
//   - DynamicConfig: heap-allocated polymorphic map (string -> ConfigOption*); used at UI layer.
//   - StaticConfig:  compile-time struct layout with virtual optptr() dispatch; used in hot
//                    slicing paths for direct field access.
// ConfigBase is the abstract interface shared by both.  ConfigDef holds static metadata only
// (no values); each DynamicConfig may optionally reference a ConfigDef via def().
//
// [STATE] All ConfigDef objects and their serialization_key_ordinals are initialised once from
// the main thread at program start via PrintConfigDef; after that they are effectively immutable
// read-only globals.
//
// [COUPLING] ConfigOption subtypes are registered with cereal at the bottom of Config.cpp.
// Any new concrete subtype MUST be added to that block or polymorphic serialisation will silently
// lose data.
//
// [HAZARD] The coVectorType bitmask (0x4000) encodes scalar vs vector in the enum integer; any
// translation that stores ConfigOptionType as a plain integer MUST preserve the bitmask logic or
// all type-dispatch switch statements will produce wrong branches.
...
{
    return is_approx(l.value, r.value) && l.percent == r.percent;
}
inline bool operator!=(const FloatOrPercent& l, const FloatOrPercent& r) throw() { return !(l == r); }
inline bool operator<(const FloatOrPercent& l, const FloatOrPercent& r) throw()
{
    return l.value < r.value || (is_approx(l.value, r.value) && int(l.percent) < int(r.percent));
}
} // namespace Slic3r
...
// [INTENT] Discriminated type tag for all ConfigOption subclasses.
// Encoding rule: scalar types are plain integers 1-11; vector variants = scalar | coVectorType.
// coVectorType (0x4000) is a bitmask sentinel — never a standalone type.
// Translation hazard: store as an opaque integer only; do NOT use in arithmetic except
// via (type & coVectorType) and (type - coVectorType) idioms used throughout the codebase.
// [COUPLING] coPointsGroups and coIntsGroups are BBS-added compound types with no scalar
// counterpart; they live at ordinals 10/11 in the vector space only.
enum ConfigOptionType {
    ...
    // [COUPLING] BBS-only compound types; no scalar equivalents.
...
// [INTENT] CRTP template base for all single-valued options (bool, int, float, string, point, enum).
// Holds a single public field `value` of type T.
// [COUPLING] cereal serialisation via friend serialize() — translators must preserve the
// base-class chain serialisation (cereal::base_class) or saved projects will lose data.
...
public:
    ...
    virtual void set_with_restore_2(
        const ConfigOptionVectorBase* rhs, std::vector<int>& restore_index, int start, int len, bool skip_error = false) = 0;
...
// [INTENT] CRTP template base for all vector-valued options (bools, ints, floats, strings, points).
// Holds a public `std::vector<T> values` field.
// [HAZARD] get_at(i) silently clamps out-of-range index to values.front() (index 0).
//   This is the intentional "default to first extruder" fallback used throughout slicing.
//   Translation: in checked-index languages this must be implemented as explicit clamping,
//   NOT as a bounds-checked access — otherwise single-extruder configs break multi-extruder paths.
// [HAZARD] H826: resize() comment says "duplicate last value" but code calls values.front().
//   The comment at line 675 (now offset) says "back" but the code says "front()".
//   Do not "fix" this — it is relied upon; translators must replicate front()-duplication.
...
public:
    ...
    virtual void set_with_restore_2(
        const ConfigOptionVectorBase* rhs, std::vector<int>& restore_index, int start, int len, bool skip_error = false) override
    {
        if (rhs->type() == this->type()) {
            // backup original ones
            std::vector<T> backup_values = this->values;
    ...
    // [HAZARD] Silent out-of-bounds clamp: if i >= values.size(), returns values.front().
    // This acts as the "default extruder" fallback throughout the slicing pipeline.
    // Translation: must replicate front()-clamp, NOT throw or crash on out-of-range access.
    ...
    // [HAZARD] H826: comment says "last" but code uses front(). Do NOT change to back().
...
// [INTENT] Per-extruder float vector option, templated on NULLABLE.
// Non-nullable (NULLABLE=false) uses standard double comparison; nullable uses quiet_NaN() as the nil sentinel.
// [STATE] nil_value() == std::numeric_limits<double>::quiet_NaN(). NaN-equality is tested via std::isnan(), NOT ==.
// [HAZARD] Comparing NaN with == always returns false; is_nil() and vectors_equal() must use std::isnan() explicitly.
// Translation: any language with strict IEEE-754 NaN semantics must replicate the isnan checks — do NOT use ==.
...
// [INTENT] Per-extruder int vector option, templated on NULLABLE.
// nil_value() == std::numeric_limits<int>::max() (INT_MAX = 2147483647).
// [HAZARD] INT_MAX as nil sentinel means a legitimate value of 2147483647 cannot be stored — translation must replicate sentinel.
// Translation: any language without unsigned-overflow guarantees must allocate an extra "is_nil" bit array instead.
...
// [INTENT] Stores a percentage value as a raw double (e.g., 50.0 represents 50%, NOT 0.5).
// get_abs_value(ratio) divides by 100 to convert to fraction.
// [HAZARD] Translation: do NOT store as 0.0–1.0 fraction — all callers pass the raw % value through get_abs_value().
// Serializes with trailing "%" character; deserializer ignores the trailing "%" (it's optional).
...
// [INTENT] Stores a vector of Vec2d points (printer bed shape, etc.).
// [HAZARD] H825: cereal serialization uses saveBinary/loadBinary on raw Vec2d memory — NOT portable across
// architectures with different endianness or different sizeof(double). Text (serialize/deserialize) uses "NxM"
// format and IS portable. Any cross-platform project serialization relies only on the text path, but binary
// caches (cereal) will corrupt silently on mixed-endian systems or when sizeof(double) differs.
...
// [INTENT] Boolean vector option using unsigned char (NOT bool) as the underlying storage type.
// This allows a special nil sentinel value (nil_value() == 0xFF = 255) for nullable variants.
// Storing as unsigned char avoids the std::vector<bool> specialisation (which uses bit packing).
// [HAZARD] H824: the non-const get_at() uses reinterpret_cast<bool*>(&unsigned_char). This is UB if
// sizeof(bool) != 1 on the target platform (C++ standard allows bool to be larger than 1 byte).
// In practice, all current compilers make bool 1 byte, but this is not guaranteed by the standard.
// Translation: in any language that stores booleans as multi-byte values, this reinterpret cast is incorrect.
...
public:
    ...
    {
        assert(!this->values.empty());
        // [HAZARD] H824: reinterpret_cast<bool*> on unsigned char* is UB if sizeof(bool) != 1.
        // The comment above ("FIXME this smells") acknowledges this. All current compilers use 1-byte bool,
        // but the standard does not guarantee it. Translating to any language with multi-byte bool is unsafe.
...
// [INTENT] Compile-time typed enum option. The enum type T is known at compile-time.
// Serializes to the enum name string (e.g., "rectilinear") via ConfigOptionEnum<T>::get_enum_names().
// [COUPLING] get_enum_names() and get_enum_values() are defined externally (in PrintConfig.cpp) per enum type T
// via explicit template specialisations. The mapping must be kept in sync with the enum definition.
// [STATE] ConfigOptionEnum<T> stores the T value directly in ConfigOptionSingle<T>::value.
// Translation: replicate the name<->int mapping tables; reordering enum members changes serialized names.
...
// [INTENT] Runtime-dispatch enum option: the enum type T is NOT known at compile-time.
// [STATE] keys_map is a raw non-owning pointer to the enum name->int map; ConfigOptionEnumGeneric does NOT own it.
// The map's lifetime must exceed this option's lifetime — typically guaranteed because both live for the program duration.
// [COUPLING] Interoperable with ConfigOptionEnum<T>: operator== and set() call rhs.getInt() allowing cross-type assignment.
...
// BBS
// [INTENT] BBS-specific addition: a vector of enum values (coEnums type) with runtime-dispatch keys_map.
// [COUPLING] Inherits from ConfigOptionInts (= ConfigOptionIntsTempl<false>, non-nullable) NOT from
// ConfigOptionIntsTempl<NULLABLE>. The NULLABLE template parameter therefore has no effect on the base class
// nil behaviour — only the overridden nullable() method returns NULLABLE. This is an inconsistency.
// [HAZARD] nil_value() calls ConfigOptionInts::nil_value() which returns INT_MAX — but the non-nullable base
// class does NOT treat INT_MAX specially during serialization, so nullable NIL values will serialize as "2147483647".
// [STATE] keys_map is a non-owning raw pointer, same lifetime requirements as ConfigOptionEnumGeneric.
...
public:
    ...
    // 0 is an invalid key.
    // [STATE] Assigned from a static local counter inside ConfigDef::add() — monotonically increasing across ALL
    // ConfigDef instances in the process. Used as the key in by_serialization_key_ordinal reverse-lookup map.
    // [HAZARD] Not thread-safe — comment in ConfigDef::add() says "shall be initialized from the main thread".
    // Any parallel initialisation of ConfigDef instances (e.g., two independent presets loaded concurrently) would race.
...
// [INTENT] Static (shared, immutable after init) schema object. Holds all option definitions and two lookup maps:
//   options — by string key (t_config_option_key → ConfigOptionDef)
//   by_serialization_key_ordinal — by monotonic ordinal (size_t → const ConfigOptionDef*)
// [STATE] Typically instantiated once per configuration scope (e.g., one global PrintConfigDef) during startup.
// [COUPLING] Referenced by ConfigBase::def() — all ConfigBase instances point to the same ConfigDef.
// [HAZARD] ConfigDef::add() uses a static local counter — see serialization_key_ordinal annotation above.
class ConfigDef
{
public:
...
// [INTENT] Abstract base for all config stores — static (PrintConfig) and dynamic (DynamicConfig).
// Defines three pure virtual contracts:
//   def()    — returns the static schema (ConfigDef); DynamicConfig returns nullptr by default
//   optptr() — find/create a ConfigOption by key
//   keys()   — enumerate all held option keys
// [COUPLING] Inherits from ConfigOptionResolver (read-only lookup). ConfigBase adds write/deserialize.
// [STATE] handle_legacy() and handle_legacy_composite() are overridable hooks called during deserialization
// to remap obsolete option names/values to current names. Both are no-ops in the base class.
// [MEMORY] Does NOT own its ConfigDef (def() returns pointer to shared static object).
...
// [INTENT] Heap-allocated, polymorphic config store. Options are held as std::map<string, unique_ptr<ConfigOption>>.
// This allows storing any subset of options without a fixed schema.
// [STATE] def() returns nullptr by default — DynamicConfig has no schema constraint unless a subclass overrides def().
// [MEMORY] Each ConfigOption value is heap-allocated and owned via unique_ptr. clone() is used for copy.
// [COUPLING] optptr(key, create=true) lazy-creates options using ConfigDef::create_default_option() when def() is non-null.
// [HAZARD] set_key_value() accepts raw ConfigOption* and takes ownership — caller must NOT free the pointer after passing.
...
// [INTENT] Compile-time struct layout config store. Each option maps to a concrete C++ member field in a
// derived struct (e.g., PrintConfig, PrintObjectConfig). optptr() returns pointers to those struct members.
// [STATE] The ConfigDef is shared/static (same as DynamicConfig), but actual values live in concrete struct fields —
// not heap-allocated. This makes StaticConfig very efficient for per-parameter access in the slicing hot path.
// [COUPLING] StaticConfig and DynamicConfig both inherit virtually from ConfigBase — diamond inheritance is intentional
// to allow a class like FullPrintConfig to combine both static and dynamic option sets via virtual base.
// [MEMORY] No heap allocation for option values — they are direct struct members.
