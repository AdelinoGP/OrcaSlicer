// [INTENT] Preset subsystem implementation.  Covers:
//   - VendorProfile loading (from_ini / from_json)
//   - Preset serialization/deserialization (save/load/normalize)
//   - PresetCollection management (sorted deque, selection, dirty tracking)
//   - Cloud-sync helpers (set_sync_info_and_save, get_differed_values_to_update)
//   - PhysicalPrinter / PhysicalPrinterCollection I/O
//   - PresetUtils namespace (bed model/texture/hotend asset resolution)
// [STATE] Module-level statics:
//   s_suffix_modified  — static std::string, lazy-init, NOT thread-safe on first call
//   s_Preset_print_options / s_Preset_filament_options / s_Preset_printer_options etc.
//     — static vectors, initialised once by static local in accessor functions,
//       NOT thread-safe if called concurrently before first initialisation
// [COUPLING] Calls into Config.hpp (DynamicPrintConfig), PrintConfig.hpp (PrintConfigDef),
//            PresetBundle.hpp (called from Preset.cpp for context), AppConfig.hpp,
//            and the BBL cloud JSON schema.
// [CONCURRENCY] PresetCollection::m_mutex is taken only in load_user_preset() and
//               save_user_presets().  Most other methods are NOT locked — caller must
//               ensure single-threaded access or external locking.
#include <cassert>
...
ConfigFileType guess_config_file_type(const ptree& tree)
{
    ...
    for (const ptree::value_type& v : tree) {
        if (v.second.empty()) {
            if (
                ...
                ++app_config;
            else if (v.first == "nozzle_diameter" || v.first == "filament_diameter")
                ++config;
        } else if (boost::algorithm::starts_with(v.first, "print:") || boost::algorithm::starts_with(v.first, "filament:") ||
                   boost::algorithm::starts_with(v.first, "printer:") || v.first == "settings")
            ++bundle;
    ...
    return (app_config > bundle && app_config > config) ? CONFIG_FILE_TYPE_APP_CONFIG :
           (bundle > config)                            ? CONFIG_FILE_TYPE_CONFIG_BUNDLE :
                                                          CONFIG_FILE_TYPE_CONFIG;
}

// [INTENT] VendorProfile::from_ini(path) is the thin file-reading overload that reads
//          the INI into a ptree and delegates to the heavier from_ini(tree, path, load_all).
// [COUPLING] Reads legacy PrusaSlicer-style vendor bundle .ini files; BBS JSON bundles
//            are loaded through a separate path in PresetBundle.
...
void extend_default_config_length(DynamicPrintConfig& config, const bool set_nil_to_default, const DynamicPrintConfig& defaults)
{
    ...
    auto replace_nil_and_resize = [&](const std::string& key, int length) {
        ...
        if (set_nil_to_default && raw_ptr->is_nil() && defaults.has(key) &&
            std::find(filament_extruder_override_keys.begin(), filament_extruder_override_keys.end(), key) ==
                filament_extruder_override_keys.end()) {
            opt_vec->clear();
...
// [INTENT] VendorProfile::from_ini(tree, path, load_all) is the heavy parser that builds
//          a VendorProfile from a pre-parsed Boost property_tree.  It reads the [vendor]
//          section (name, config_version, config_update_url, changelog_url), then iterates
//          every [printer_model:*] section to populate VendorProfile::models[], then loads
//          default_filaments and default_sla_materials.  Called by the thin overload above
//          and directly by PresetBundle when loading bundled vendor profiles.
// [COUPLING] Reads legacy PrusaSlicer-style .ini bundle format (not the BBS JSON format).
//            Technology field parsed via ConfigOptionEnum<PrinterTechnology>; SLA printers
//            are NOT filtered out here (the #if 0 guard at line ~299 is dead code).
//            BBL-specific family inference from pre_family_model_map is applied when
//            vendor name == "BBL" and no explicit family is given.
// [HAZARD] get_or_throw() throws Slic3r::RuntimeError if a required key is missing —
//          callers must wrap in try/catch.  load_all=false is a short-circuit that skips
//          all model/material loading; the returned VendorProfile has only name/version/URLs.
VendorProfile VendorProfile::from_ini(const ptree& tree, const boost::filesystem::path& path, bool load_all)
{
    ...
    auto get_or_throw = [&](const ptree& tree, const std::string& key) -> ptree::const_assoc_iterator {
        ...
        if (res == tree.not_found()) {
            throw Slic3r::RuntimeError(
                (boost::format("Vendor Config Bundle `%1%` is not valid: Missing secion or key: `%2%`.") % id % key).str());
        }
        return res;
    };
    ...
    if (!config_version) {
        throw Slic3r::RuntimeError(
            (boost::format("Vendor Config Bundle `%1%` is not valid: Cannot parse config_version: `%2%`.") % id % config_version_str).str());
    } else {
    ...
    for (auto& section : tree) {
        if (boost::starts_with(section.first, printer_model_key)) {
            ...
            if (!ConfigOptionEnum<PrinterTechnology>::from_string(technology_field, model.technology)) {
                BOOST_LOG_TRIVIAL(error) << boost::format("Vendor bundle: `%1%`: Invalid printer technology field: `%2%`") % id %
                                                technology_field;
            ...
            if (model.family.empty() && res.name == "BBL") {
                ...
                if (from_pre_map != pre_family_model_map.end()) {
                    model.family = from_pre_map->second;
                }
            }
#if 0
            ...
            if (Slic3r::unescape_strings_cstyle(default_materials_field, model.default_materials)) {
                ...
                BOOST_LOG_TRIVIAL(error) << boost::format("Vendor bundle: `%1%`: Malformed default_materials field: `%2%`") % id %
                                                default_materials_field;
            }
...
const std::string& Preset::suffix_modified() { return g_suffix_modified; }

void Preset::update_suffix_modified(const std::string& new_suffix_modified) { g_suffix_modified = new_suffix_modified; }
// Remove an optional "(modified)" suffix from a name.
// This converts a UI name to a unique preset identifier.
std::string Preset::remove_suffix_modified(const std::string& name)
{
    return boost::algorithm::starts_with(name, g_suffix_modified) ? name.substr(g_suffix_modified.size()) : name;
}

// [INTENT] normalize() is called after every preset load to ensure all per-extruder /
//          per-filament vector config options are resized to match the detected extruder
//          count.  It is the authoritative post-load fixup step.
// [HAZARD] The extruder count is derived from "filament_diameter" (multi-material path)
//          or "nozzle_diameter" (single-extruder SEMM path).  If neither option is
//          present the count defaults to 1.  Any translator must preserve this two-
//          branch resolution order.
// [COUPLING] Calls config.set_num_filaments() or config.set_num_extruders() which
//            resize all registered per-extruder options simultaneously.
...
// [INTENT] remove_invalid_keys() strips any config keys not present in default_config,
//          returning a comma-separated list of removed keys for diagnostic logging.
//          Called after loading user presets to guard against stale/renamed option keys
//          that would otherwise persist in memory with no associated definition.
// [HAZARD] Silently erases unknown keys — no error is thrown.  Callers that rely on
//          round-trip fidelity (e.g., cloud sync) will lose user data for removed keys
//          with only a log warning as evidence.
...
{
    switch (type) {
    case Preset::Type::TYPE_FILAMENT: return PRESET_FILAMENT_NAME;
    case Preset::Type::TYPE_PRINT: return PRESET_PRINT_NAME;
    case Preset::Type::TYPE_PRINTER: return PRESET_PRINTER_NAME;
    case Preset::Type::TYPE_PHYSICAL_PRINTER: return "physical_printer";
    case Preset::Type::TYPE_INVALID: return "invalid";
...
{
    switch (type) {
    case Preset::Type::TYPE_FILAMENT: return PRESET_IOT_FILAMENT_TYPE;
    case Preset::Type::TYPE_PRINT: return PRESET_IOT_PRINT_TYPE;
    case Preset::Type::TYPE_PRINTER: return PRESET_IOT_PRINTER_TYPE;
...
// [INTENT] Diff-save: when parent_config != nullptr, only the keys that DIFFER from
//          parent_config are written to the JSON file.  This implements the inheritance
//          compression that keeps user files small and parent-tracking intact.
//          When parent_config == nullptr, all keys are written (full save).
// [HAZARD] is_project_embedded presets are silently skipped — they exist only in RAM.
//          Callers that call save() on a project-embedded preset expecting a file write
//          will get no error and no file.
void Preset::save(DynamicPrintConfig* parent_config)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " save config for: " << this->name << " and filament_id: " << filament_id
                            << " and base_id: " << this->base_id;
...
{
    return (this->is_dirty ? g_suffix_modified : "") + ((no_alias || this->alias.empty()) ? this->name : this->alias);
}
    ...
    if (!has_compatible_printers && !condition.empty()) {
        try {
            ...
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(": parsing error of compatible_printers_condition %1%: %2%") %
                                              active_printer.preset.name % err.what();
            return true;
        }
    }
...
std::string Preset::get_filament_type(std::string& display_filament_type) { return config.get_filament_type(display_filament_type); }
...
void Preset::get_extruder_names_and_keysets(Type                    type,
                                            std::string&            extruder_id_name,
                                            std::string&            extruder_variant_name,
                                            std::set<std::string>** p_key_set1,
                                            std::set<std::string>** p_key_set2)
{
...
// [INTENT] Canonical key lists that define which DynamicPrintConfig keys belong to
//          each preset type.  These are the authoritative "schema" for each preset
//          category — only keys in these lists are saved/loaded/diffed for that type.
// [HAZARD] These are file-scope static globals initialised at program start (before
//          main()).  Adding or removing keys changes the serialization schema; any
//          existing preset file that contains a key NOT in this list will trigger the
//          forward-compatibility substitution mechanism when loaded.
// [HAZARD] Preset::printer_options() returns a MERGED list (s_Preset_printer_options +
//          s_Preset_machine_limits_options) built via a static local lambda — that
//          merged list is initialised exactly once (C++11 guaranteed), so it is safe
//          after first call, but NOT safe if two threads race on the very first call
//          (pre-C++17 compilers could have a race on static local initialisation).
...
static std::vector<std::string> s_Preset_filament_options{
    ...
    "volumetric_speed_coefficients", "filament_type", "filament_soluble", "filament_is_support", "filament_printable",
    "filament_max_volumetric_speed", "filament_adaptive_volumetric_speed", "filament_flow_ratio", "filament_density",
    "filament_adhesiveness_category", "filament_cost", "filament_minimal_purge_on_wipe_tower",
    "filament_tower_interface_pre_extrusion_dist", "filament_tower_interface_pre_extrusion_length", "filament_tower_ironing_area",
    "filament_tower_interface_purge_volume", "filament_tower_interface_print_temp", "nozzle_temperature",
    ...
    "fan_max_speed", "enable_overhang_bridge_fan", "overhang_fan_speed", "overhang_fan_threshold", "close_fan_the_first_x_layers",
    "full_fan_speed_layer", "fan_cooling_layer_time", "slow_down_layer_time", "slow_down_min_speed", "filament_start_gcode",
    "filament_end_gcode",
    // exhaust fan control
    "activate_air_filtration", "during_print_exhaust_fan_speed", "complete_print_exhaust_fan_speed",
    // Retract overrides
    "filament_retraction_length", "filament_z_hop", "filament_z_hop_types", "filament_retract_lift_above", "filament_retract_lift_below",
    ...
    // SoftFever
    "enable_pressure_advance", "pressure_advance", "adaptive_pressure_advance", "adaptive_pressure_advance_model",
    "adaptive_pressure_advance_overhangs", "adaptive_pressure_advance_bridges", "chamber_temperature", "filament_shrink",
    "filament_shrinkage_compensation_z", "support_material_interface_fan_speed", "internal_bridge_fan_speed",
    "filament_notes" /*,"filament_seam_gap"*/, "ironing_fan_speed",
    // Filament ironing overrides
    "filament_ironing_flow", "filament_ironing_spacing", "filament_ironing_inset", "filament_ironing_speed", "filament_loading_speed",
    "filament_loading_speed_start", "filament_unloading_speed", "filament_unloading_speed_start", "filament_toolchange_delay",
    "filament_cooling_moves", "filament_stamping_loading_speed", "filament_stamping_distance", "filament_cooling_initial_speed",
    "filament_cooling_final_speed", "filament_ramming_parameters", "filament_multitool_ramming", "filament_multitool_ramming_volume",
    "filament_multitool_ramming_flow", "activate_chamber_temp_control", "filament_long_retractions_when_cut",
    "filament_retraction_distances_when_cut", "idle_temperature",
    // BBS filament change length while the extruder color
    "filament_change_length", "filament_flush_volumetric_speed", "filament_flush_temp", "long_retractions_when_ec",
...
static std::vector<std::string> s_Preset_printer_options{"printer_technology",
                                                         "printable_area",
...
static std::vector<std::string> s_Preset_sla_print_options{"layer_height", "faded_layers", "supports_enable", "support_head_front_diameter",
                                                           "support_head_penetration", "support_head_width", "support_pillar_diameter",
                                                           "support_small_pillar_diameter_percent", "support_max_bridges_on_pillar",
                                                           "support_pillar_connection_mode", "support_buildplate_only",
                                                           "support_pillar_widening_factor", "support_base_diameter", "support_base_height",
                                                           "support_base_safety_distance", "support_critical_angle",
                                                           "support_max_bridge_length", "support_max_pillar_link_distance",
                                                           "support_object_elevation", "support_points_density_relative",
                                                           "support_points_minimal_distance", "slice_closing_radius", "pad_enable",
                                                           "pad_wall_thickness", "pad_wall_height", "pad_brim_size",
                                                           "pad_max_merge_distance",
                                                           // "pad_edge_radius",
                                                           "pad_wall_slope", "pad_object_gap", "pad_around_object",
                                                           "pad_around_object_everywhere", "pad_object_connector_stride",
                                                           "pad_object_connector_width", "pad_object_connector_penetration",
                                                           "hollowing_enable", "hollowing_min_thickness", "hollowing_quality",
                                                           "hollowing_closing_distance", "filename_format", "default_sla_print_profile",
                                                           "compatible_printers", "compatible_printers_condition", "inherits"};

static std::vector<std::string> s_Preset_sla_material_options{"material_colour",
                                                              "material_type",
...
static std::vector<std::string> s_Preset_sla_printer_options{"printer_technology",
                                                             "printable_area",
                                                             "bed_custom_texture",
                                                             "bed_custom_model",
                                                             "printable_height",
...
PresetCollection::PresetCollection(Preset::Type                     type,
                                   const std::vector<std::string>&  keys,
                                   const Slic3r::StaticPrintConfig& defaults,
                                   const std::string&               default_name)
    : m_type(type), m_edited_preset(type, "", false), m_saved_preset(type, "", false), m_idx_selected(0)
{
...
void PresetCollection::add_default_preset(const std::vector<std::string>&  keys,
                                          const Slic3r::StaticPrintConfig& defaults,
...
// [INTENT] load_presets() is the main directory scanner for a preset collection.
//          It recurses into the "base/" subdirectory first (custom root presets), then
//          iterates all *.json files in the target directory, skipping duplicates.
//          Each JSON is parsed via DynamicPrintConfig::load_from_json(); the result is
//          inserted into m_presets in sorted order for binary-search lookups.
// [HAZARD] Corrupt JSON files are SILENTLY DELETED — both the .json and .info files are
//          removed from disk on load failure with only a log error, no user notification.
// [HAZARD] Missing or unparseable version field causes silent skip (no error, no log at
//          WARNING level) — presets with invalid versions are lost without trace.
// [COUPLING] Calls Preset::get_extruder_names_and_keysets() to resolve multi-extruder
//            variant key names per preset type; calls load_info() for the .info sidecar.
//            Interacts with find_preset() for duplicate detection.
void PresetCollection::load_presets(const std::string&                   dir_path,
                                    ...
                                    ForwardCompatibilitySubstitutionRule substitution_rule)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(" enter, load presets from %1%, current type %2%") % dir % Preset::get_type_string(m_type);
            ...
            try {
                ...
                try {
                    ...
                    if (!config_substitutions.empty())
                        substitutions.push_back({preset.name, m_type, PresetConfigSubstitutions::Source::UserFile, preset.file,
                                                 std::move(config_substitutions)});
                    if (!reason.empty()) {
                    ...
                    if (inherit_preset) {
                        ...
                        if ((inherits_config2 && !inherits_config2->value.empty())) {
                            BOOST_LOG_TRIVIAL(error)
                                << boost::format("can not find parent %1% for config %2%!") % inherits_config2->value % preset.file;
                            ++m_errors;
                            continue;
                        }
                        // We support custom root preset now
                        // Find a default preset for the config. The PrintPresetCollection provides different default preset based on the
                        // "printer_technology" field.
                    ...
                    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                             << boost::format(
                                                    ", preset type %1%, name %2%, path %3%, is_system %4%, is_default %5% is_visible %6%") %
                                                    Preset::get_type_string(m_type) % preset.name % preset.file % preset.is_system %
                                                    preset.is_default % preset.is_visible;
                    // add alias for custom filament preset
                    set_custom_preset_alias(preset);
                } catch (const std::ifstream::failure& err) {
                    ++m_errors;
                    BOOST_LOG_TRIVIAL(error) << boost::format("The user-config cannot be loaded: %1%. Reason: %2%") % preset.file %
                                                    err.what();
                    fs::path file_path(preset.file);
                    if (fs::exists(file_path))
                        fs::remove(file_path);
                    ...
                    ++m_errors;
                    BOOST_LOG_TRIVIAL(error) << boost::format("Failed loading the user-config file: %1%. Reason: %2%") % preset.file %
                                                    err.what();
...
// [INTENT] get_preset_differed_for_save() builds a transient Preset* whose config
//          contains ONLY the keys that differ from the parent preset's config, plus the
//          required extruder identity keys.  Used by the cloud-sync path to minimise
//          upload payload.  Returns nullptr for system/default presets (nothing to save).
// [HAZARD] Allocates a Preset* on the heap — caller owns and must delete.  Missing
//          delete would be a leak; passing nullptr return to save() skips the diff logic.
// [COUPLING] Calls config.diff() (shallow), then set_with_nil() for per-extruder vectors
//            using key_set1/key_set2 distinction (variant-level vs. full-vector).
//            Closely mirrors get_differed_values_to_update() but returns a Preset object
//            rather than a flat key_values map.
...
int PresetCollection::get_differed_values_to_update(Preset& preset, std::map<std::string, std::string>& key_values)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " uploading user preset name is: " << preset.name
                            << "and create filament_id is: " << preset.filament_id << " and base_id is: " << preset.base_id;
...
// [INTENT] load_project_embedded_presets() ingests the in-memory Preset* list extracted
//          from a 3MF project file, merging them into this PresetCollection under the
//          is_project_embedded=true flag.  Project-embedded presets shadow user presets
//          during the session but are never written to the user preset directory.
// [COUPLING] Reads the same JSON format as load_presets() but sources data from the
//            project_presets vector (already parsed by 3mf.cpp) rather than disk files.
// [HAZARD] Project-embedded presets with the same name as an existing user preset will
//          UPDATE the existing record — the original user preset data is overwritten in
//          memory for the duration of the session.  Closing the project does NOT
//          automatically restore the user preset; reset_project_embedded_presets() must
//          be called explicitly.
void PresetCollection::load_project_embedded_presets(std::vector<Preset*>&                project_presets,
                                                     ...
                                                     ForwardCompatibilitySubstitutionRule rule)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(" enter, type %1% , total preset counts %2%") % Preset::get_type_string(m_type) %
    ...
    for (it = project_presets.begin(); it != project_presets.end(); it++) {
        Preset* preset = *it;
        if (preset->type != Preset::get_type_from_string(type))
            continue;
        ...
        try {
            ...
            if (preset->loading_substitutions && !preset->loading_substitutions->empty()) {
                substitutions.push_back({preset->name, m_type, PresetConfigSubstitutions::Source::ProjectFile, preset->name,
                                         std::move(*(preset->loading_substitutions))});
            ...
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(
                                            ", %1% got preset, name %2%, path %3%, is_system %4%, is_default %5% is_visible %6%") %
                                            Preset::get_type_string(m_type) % preset->name % preset->file % preset->is_system %
                                            preset->is_default % preset->is_visible;
        } catch (const std::runtime_error& err) {
            errors_cummulative += err.what();
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(" finished, %1% got %2% presets, errors_cummulative %3%") % Preset::get_type_string(m_type) %
                                    presets_loaded.size() % errors_cummulative;
    if (!errors_cummulative.empty())
        throw Slic3r::RuntimeError(errors_cummulative);
}
...
bool PresetCollection::reset_project_embedded_presets()
{
    ...
    while (it != m_presets.end()) {
        ...
        if (it->is_project_embedded) {
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(" type %1% , delete preset %2%") % Preset::get_type_string(m_type) % it->name;
...
// [INTENT] set_sync_info_and_save() updates cloud sync metadata for a named preset:
//          sets sync_info, setting_id, updated_time, and propagates base_id to children.
//          Holds m_mutex lock for the entire operation.
// [HAZARD] Line 2069 original (now offset by annotations): the expression
//          `preset->sync_info == "update"` returns a bool (0 or 1) which is then
//          used as the ternary condition.  This means the ternary always evaluates to
//          one of two Preset::save() calls, never a no-op — the logic is correct but
//          the readability is misleading (should be an if-else).
...
int PresetCollection::get_user_presets(PresetBundle* preset_bundle, std::vector<Preset>& result_presets)
{
    ...
    for (Preset& preset : m_presets) {
        if (!preset.is_user())
            continue;
        if (preset.base_id.empty() && preset.inherits() != "")
            continue;
    ...
    for (auto it = m_presets.begin(); it != m_presets.end(); it++) {
        ...
        if (!parent_preset) {
            ...
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(" can not find parent preset for %1% , inherits %2%") % preset->name % inherits;
            continue;
        }

        if (preset->base_id.empty())
            preset->base_id = parent_preset->setting_id;
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << preset->name << " filament_id: " << preset->filament_id
...
// [INTENT] Load a single user preset from a pre-parsed key-value map (not a file path).
//          Called during startup and cloud sync when the preset data has already been
//          fetched as JSON and split into a map<string,string>.
// [CONCURRENCY] Acquires m_mutex (via lock()) and holds it across the entire function
//               body including JSON parsing / config apply — potentially a long critical
//               section that blocks any concurrent load/save on the same collection.
// [HAZARD] If an exception is thrown inside the locked section, the mutex may not be
//          released (no RAII guard — manual lock()/unlock() calls).  Wrapped in a
//          try-catch but the catch re-throws through errors_cummulative string, and
//          unlock() is called in the normal path only — exception path is unguarded.
bool PresetCollection::load_user_preset(std::string                          name,
                                        std::map<std::string, std::string>   preset_values,
                                        PresetsConfigSubstitutions&          substitutions,
                                        ForwardCompatibilitySubstitutionRule rule)
{
    std::string errors_cummulative;
    ...
    if (!cloud_version) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                   << boost::format("invalid version %1%, not loading for user preset %2%") % version_str % name;
        return false;
    }
    ...
    if ((iter != m_presets.end()) && (iter->name == name)) {
        ...
        if ((cloud_update_time == 0) || (cloud_update_time <= iter->updated_time)) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(
                                           "preset %1%'s update_time is eqaul or newer, cloud  update_time %2%, local update_time %3%") %
                                           name % cloud_update_time % iter->updated_time;
            unlock();
            return false;
        } else {
    ...
    try {
        ...
        if (inherit_preset) {
            ...
            if (inherits_config2 && !inherits_config2->value.empty()) {
                ...
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(", can not find inherit preset for user preset %1%, just skip") % name;
                unlock();
                return false;
            }
            // Find a default preset for the config. The PrintPresetCollection provides different default preset based on the
        ...
        if (need_update) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(", update the user preset %1% from cloud, type %2%, setting_id %3%, base_id %4%, "
                                                     "sync_info %5% inherits %6%, filament_id %7%") %
                                           iter->name % Preset::get_type_string(m_type) % iter->setting_id % iter->base_id %
                                           iter->sync_info % iter->inherits() % iter->filament_id;
        } else {
            // create a new one
            Preset preset(m_type, name, false);
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(", insert a new user preset %1%, type %2%, setting_id %3%, base_id %4%, sync_info %5% "
                                                     "inherits %6%, filament_id %7%") %
                                           preset.name % Preset::get_type_string(m_type) % preset.setting_id % preset.base_id %
                                           preset.sync_info % preset.inherits() % preset.filament_id;
            if (cur_index <= m_idx_selected) {
                m_idx_selected++;
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(", increase m_idx_selected to %1%, due to user preset inserted") % m_idx_selected;
            }
        }
    } catch (const std::runtime_error& err) {
        errors_cummulative += err.what();
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(" finished, load user preset %1% , type %2%, errors_cummulative %3%") % name %
                                    Preset::get_type_string(m_type) % errors_cummulative;
...
bool PresetCollection::validate_preset(const std::string& preset_name, std::string& inherit_name)
{
    ...
    if (!found) {
        if (!inherit_name.empty()) {
            ...
            if (found)
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": preset_name %1%, inherit_name %2%, found inherit in list") % preset_name %
                                               inherit_name;
            else
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(
                                                  ": preset_name %1%, inherit_name %2%, can not found preset and inherit in list") %
                                                  preset_name % inherit_name;
        } else {
...
Preset& PresetCollection::load_preset(
    const std::string& path, const std::string& name, const DynamicPrintConfig& config, bool select, Semver file_version)
{
...
static bool profile_print_params_same(const DynamicPrintConfig& cfg_old, const DynamicPrintConfig& cfg_new)
{
    ...
    for (const char* key : {"compatible_prints", "compatible_prints_condition", "compatible_printers", "compatible_printers_condition",
    ...
    const std::string filament_id)
{
    ...
    keys.erase(std::remove_if(keys.begin(), keys.end(),
                              [](std::string& val) {
                                  return val == "print_host" || val == "print_host_webui" || val == "printhost_apikey" ||
                                         val == "printhost_cafile" || val == "printhost_user" || val == "printhost_password" ||
                                         val == "printhost_port";
                              }),
               keys.end());
    ...
    if (!inherits.empty() && (different_settings_list.size() > 0)) {
        ...
        if (iter != m_presets.end() && iter->name == inherits) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": change preset %1% inherit %2% 's value to %3% 's values") % original_name %
                                           inherits % path;
            cfg.update_non_diff_values_to_base_config(iter->config, keys, different_settings_list, extruder_id_name, extruder_variant_name,
                                                      *key_set1, *key_set2);
        }
    ...
    // BBS: add config related logs
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(" enter, type %1% , path %2%, name %3%, original_name %4%, inherits %5%") %
                                    Preset::get_type_string(m_type) % path % name % original_name % inherits;
    if (select == LoadAndSelect::Never) {
        ...
        if ((edited.name == original_name || edited.name == inherits) && profile_print_params_same(edited.config, cfg)) {
            ...
            // BBS: add config related logs
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(" Just point to that already selected and edited profile %1%") % edited.name;
    ...
    if (found && profile_print_params_same(it->config, cfg)) {
        ...
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                 << boost::format(
                                        " The preset exists and it matches the values stored inside config. using original_name %1%") %
                                        original_name;
    ...
    if (!found && select != LoadAndSelect::Never && !inherits.empty()) {
        ...
        if (found && profile_print_params_same(it->config, cfg)) {
            ...
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(
                                            " The preset exists and it matches the values stored inside config. using inherits %1%") %
                                            inherits;
            return std::make_pair(&(*it), false);
        }
    }
    if (found) {
        ...
        if (select == LoadAndSelect::Always) {
            ...
            // BBS: add config related logs
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(" Select the existing preset %1% and override it with new values") % it->name;
    ...
    for (size_t idx = 0;; ++idx) {
        ...
        if (profile_print_params_same(it->config, cfg)) {
            ...
            // BBS: add config related logs
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(" The preset %1% exists and it matches the values stored inside config.") % new_name;
            return std::make_pair(&(*it), false);
        }
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(
                                    ", type %1% added a preset, name %2%, path %3%, is_system %4%, is_default %5% is_external %6%") %
                                    Preset::get_type_string(m_type) % preset.name % preset.file % preset.is_system % preset.is_default %
                                    preset.is_external;
    return std::make_pair(&preset, false);
}

Preset& PresetCollection::load_preset(
    const std::string& path, const std::string& name, DynamicPrintConfig&& config, bool select, Semver file_version)
{
    lock();
    ...
    // BBS: add config related logs
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(", preset type %1%, name %2%, path %3%, is_system %4%, is_default %5% is_visible %6%") %
                                    Preset::get_type_string(m_type) % preset.name % preset.file % preset.is_system % preset.is_default %
                                    preset.is_visible;
    return preset;
}

// [INTENT] clone_presets() is the core preset-duplication engine.  It copies each preset
//          in the input list, strips all cloud/system metadata (vendor, renamed_from,
//          setting_id, inherits, base_id), resets flags to user-owned visible state, then
//          applies the caller-supplied modifier lambda to rename and further customize the
//          clone.  Clones are written to disk via save(nullptr) (full save, no diff).
// [CONCURRENCY] Acquires m_mutex (via lock()) for the insertion loop — safe for concurrent
//               access to m_presets.  The pre-insertion name-clash check outside the lock
//               creates a TOCTOU window: the same name could be inserted by another thread
//               between the check and the lock.
// [HAZARD] force_rewritten=false returns false without modifying anything if ANY name
//          already exists — it does not do partial insertion for the non-conflicting names.
// [COUPLING] Calls set_custom_preset_alias() and select_preset_by_name() — UI state is
//            updated as a side effect of the clone operation.
bool PresetCollection::clone_presets(std::vector<Preset const*> const&           presets,
                                     std::vector<std::string>&                   failures,
                                     std::function<void(Preset&, Preset::Type&)> modifier,
...
bool PresetCollection::clone_presets_for_printer(std::vector<Preset const*> const&       templates,
                                                 ...
                                                 bool                                    force_rewritten)
{
    return clone_presets(
        templates, failures,
        [printer, create_filament_id](Preset& preset, Preset::Type& type) {
                                                  ...
                                                  bool                      force_rewritten)
{
    ...
    return clone_presets(
        presets, failures,
        [&filament_name, &filament_id, &dynamic_config, &compatible_printers](Preset& preset, Preset::Type& type) {
...
std::map<std::string, std::vector<Preset const*>> PresetCollection::get_filament_presets() const
{
    ...
    for (auto& preset : m_presets) {
        if (preset.is_user()) {
            if (preset.inherits() == "") {
                filament_presets[preset.filament_id].push_back(&preset);
            }
            continue;
        }
        if (get_preset_base(preset) == &preset) {
...
// [INTENT] get_preset_base() recursively walks the inherits chain to find the ultimate
//          base preset (system or user root) for a given child preset.  Used by cloud
//          sync and filament_id management to identify the canonical identity anchor.
// [HAZARD] UNBOUNDED RECURSION — no cycle detection.  A preset A inheriting from B
//          which inherits from A will overflow the stack.  The system does not validate
//          the inherits graph on load, so circular chains are possible via hand-edited
//          JSON files.  (Registered as H828.)
// [COUPLING] Calls find_preset2() (which does alias-aware lookup) rather than plain
//            find_preset() — critical for presets whose parent was renamed.
...
const std::string& PresetCollection::get_preset_name_by_alias(const std::string& alias) const
{
    for (
        ...
        auto it = Slic3r::lower_bound_by_predicate(m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(),
                                                   [&alias](auto& l) { return l.first < alias; });
        ...
        for (const std::string& preset_name : it->second) {
            if (auto it_preset = this->find_preset_internal(preset_name);
                it_preset != m_presets.end() && it_preset->name == preset_name && it_preset->is_visible &&
                (it_preset->is_compatible || size_t(it_preset - m_presets.begin()) == m_idx_selected))
...
bool PresetCollection::is_alias_exist(const std::string& alias, Preset* preset)
{
    ...
    if (m_map_alias_to_profile_name.end() == it)
        return false;
    if (!preset)
        return true;
    ...
    for (const std::string& printer_name : compatible_printers->values) {
        ...
        if (m_printer_hold_alias.end() != printer_iter) {
            ...
            if (m_printer_hold_alias[printer_name].end() != alias_iter) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << " The alias already exists: " << alias
                                        << " and the preset name: " << preset->name;
                return true;
            }
        }
    }
...
const std::string& PresetCollection::get_suffix_modified() { return g_suffix_modified; }
...
size_t PresetCollection::update_compatible_internal(const PresetWithVendorProfile& active_printer,
                                                    const PresetWithVendorProfile* active_print,
                                                    PresetSelectCompatibleType     unselect_if_incompatible)
{
    ...
    if (active_print)
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": active printer %1%, print %2%, unselect_if_incompatible %3%") %
                                       active_printer.preset.name % active_print->preset.name % (int) unselect_if_incompatible;
    else
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": active printer %1%, unselect_if_incompatible %2%") % active_printer.preset.name %
                                       (int) unselect_if_incompatible;
    for (size_t idx_preset = m_num_default_presets; idx_preset < m_presets.size(); ++idx_preset) {
        ...
        if (!preset_edited.is_compatible && selected &&
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(":  type %1% , previous selected %2% becomes uncompatible, will select later") %
                                           Preset::get_type_string(m_type) % m_idx_selected;
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": type %1% returned m_idx_selected %2%, some_compatible %3%") %
                                   Preset::get_type_string(m_type) % m_idx_selected % some_compatible;
...
template<class T>
void add_correct_opts_to_diff(
    const std::string& opt_key, t_config_option_keys& vec, const ConfigBase& other, const ConfigBase& this_c, bool strict)
{
...
// [INTENT] deep_diff() is the per-extruder-aware config comparator.  Unlike the shallow
//          config.diff() it expands vector options element-by-element, emitting keys with
//          a "#N" suffix (e.g., "nozzle_diameter#0") for per-extruder differences.
//          Used by is_dirty() and dirty_options() when deep_compare=true.
// [COUPLING] Handles special-cased keys: printable_area, bed_exclude_area, compatible_prints,
//            compatible_printers, thumbnails, wrapping_exclude_area are treated as atomic
//            scalars.  "default_filament_profile" is intentionally skipped (never dirty).
//            "thumbnails" has a special semantic comparison path via GCodeThumbnails.
// [HAZARD] coEnums is mapped to add_correct_opts_to_diff<ConfigOptionInts> — enum type
//          identity is lost in the comparison.  Any new coXxx type added to ConfigOptionType
//          will fall through to the default: branch (atomic diff) unless explicitly cased.
        ...
        if (this_opt != nullptr && other_opt != nullptr && *this_opt != *other_opt) {
            ...
            if (opt_key == "printable_area" || opt_key == "bed_exclude_area" || opt_key == "compatible_prints" ||
                ...
                case coFloatsOrPercents:
                    add_correct_opts_to_diff<ConfigOptionFloatsOrPercents>(opt_key, diff, config_other, config_this, strict);
                    break;
...
std::vector<std::string> PresetCollection::dirty_options(const Preset* edited, const Preset* reference, const bool deep_compare /*= false*/)
{
    ...
    if (edited != nullptr && reference != nullptr) {
        // Only compares options existing in both configs.
        changed = deep_compare ? deep_diff(edited->config, reference->config) : reference->config.diff(edited->config);
        // The "compatible_printers" option key is handled differently from the others:
        // It is not mandatory. If the key is missing, it means it is compatible with any printer.
        // If the key exists and it is empty, it means it is compatible with no printer.
        for (auto& opt_key : optional_keys)
...
std::vector<std::string> PresetCollection::dirty_options_without_option_list(const Preset*                edited,
                                                                             const Preset*                reference,
                                                                             const std::set<std::string>& option_ignore_list,
...
Preset& PresetCollection::select_preset(size_t idx)
{
    ...
    if (!m_presets[idx].is_visible) {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": %1% set %2%, idx %3% to visible") % Preset::get_type_string(m_type) %
                                       m_presets[idx].name % idx;
        m_presets[idx].is_visible = true;
    }
    // BBS: add config related logs
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": %1% select success, m_idx_selected %2%, name %3%, is_system %4%, is_default %5%") %
                                   Preset::get_type_string(m_type) % m_idx_selected % m_edited_preset.name % m_edited_preset.is_system %
                                   m_edited_preset.is_default;
...
bool PresetCollection::select_preset_by_name(const std::string& name_w_suffix, bool force)
{
    // BBS: add config related logs
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": %1%, try to select by name %2%, force %3%") % Preset::get_type_string(m_type) %
                                   name_w_suffix % force;
    ...
    if (m_idx_selected != idx || force) {
        ...
        // BBS: add config related logs
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": %1%, select %2%, success") % Preset::get_type_string(m_type) % name_w_suffix;
        return true;
    }
    ...
    if (m_idx_selected == idx)
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": %1%, select %2%, failed") % Preset::get_type_string(m_type) % name_w_suffix;
    return false;
}
...
{
    if (m_type == Preset::Type::TYPE_FILAMENT && preset.config.has(BBL_JSON_KEY_INHERITS) &&
            ...
            if (printer_filament_alias.end() == alias_iter) {
                ...
                if (remove) {
                    ...
                    if (auto alias_iter = m_map_alias_to_profile_name.find(alias); alias_iter != m_map_alias_to_profile_name.end()) {
                        ...
                        if (presets.empty()) {
                            m_map_alias_to_profile_name.erase(alias);
                        }
                    }
                }
            }
        }
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << " preset name : " << preset.name << " remove action: " << remove
                                << " insert success: " << insert_success << " remove success: " << remove_success << " alias: " << alias;
    }
}
...
std::string PresetCollection::path_for_preset(const Preset& preset) const { return path_from_name(preset.name, is_base_preset(preset)); }
...
std::string PhysicalPrinter::separator() { return " * "; }

static std::vector<std::string> s_PhysicalPrinter_opts{"preset_name", // temporary option to compatibility with older Slicer
                                                       "preset_names", "printer_technology", "bbl_use_printhost", "host_type",
                                                       "printer_agent", "print_host", "print_host_webui", "printhost_apikey",
                                                       "printhost_cafile", "printhost_port", "printhost_authorization_type",
                                                       // HTTP digest authentization (RFC 2617)
                                                       "printhost_user", "printhost_password", "printhost_ssl_ignore_revoke"};

const std::vector<std::string>& PhysicalPrinter::printer_options() { return s_PhysicalPrinter_opts; }
...
// [INTENT] has_print_host_information() is a stub that unconditionally returns false.
//          Originally intended to check whether a DynamicPrintConfig contains any
//          non-empty "Print Host upload" options (host, apikey, cafile, etc.) for the
//          purpose of filtering physical printers.  The check was never implemented.
// [HAZARD] Always returns false — no physical printer is ever filtered based on print-host
//          info.  Any caller that depends on this method for security/filtering decisions
//          will silently pass all printers through.  (Registered as H831.)
bool PhysicalPrinter::has_print_host_information(const DynamicPrintConfig& config) { return false; }

const std::set<std::string>& PhysicalPrinter::get_preset_names() const { return preset_names; }
...
void PhysicalPrinter::reset_presets() { return preset_names.clear(); }

bool PhysicalPrinter::add_preset(const std::string& preset_name) { return preset_names.emplace(preset_name).second; }

bool PhysicalPrinter::delete_preset(const std::string& preset_name) { return preset_names.erase(preset_name) > 0; }
...
void PhysicalPrinter::set_name(const std::string& name) { this->name = name; }

std::string PhysicalPrinter::get_full_name(std::string preset_name) const { return name + separator() + preset_name; }
...
// Throws an exception on error.
void PhysicalPrinterCollection::load_printers(const std::string&                   dir_path,
                                              const std::string&                   subdir,
                                              PresetsConfigSubstitutions&          substitutions,
                                              ForwardCompatibilitySubstitutionRule substitution_rule)
{
        ...
        if (Slic3r::is_json_file(file_name)) {
            ...
            try {
                ...
                try {
                    ...
                    if (!config_substitutions.empty())
                        substitutions.push_back({name, Preset::TYPE_PHYSICAL_PRINTER, PresetConfigSubstitutions::Source::UserFile,
                                                 printer.file, std::move(config_substitutions)});
...
void PhysicalPrinterCollection::load_printer(
    const std::string& path, const std::string& name, DynamicPrintConfig&& config, bool select, bool save /* = false*/)
{
...
// [INTENT] load_printers_from_presets() was the legacy migration path that created
//          PhysicalPrinter entries from Preset configs that contained Print-Host upload
//          fields (host, apikey, cafile).  The entire body is wrapped in #if 0, making
//          this a permanently disabled no-op.  (Registered as H833.)
// [HAZARD] Dead code block — physical printers are never auto-migrated from old presets.
//          Any user who had Print-Host info in their old preset.ini config will silently
//          lose it on upgrade without ever seeing a migration error.
...
// [INTENT] find_printer_internal() is the binary-search iterator for PhysicalPrinterCollection.
//          Case-sensitive search uses std::lower_bound (O(log N)); case-insensitive search
//          uses a linear scan (O(N)).
// [HAZARD] The O(N) linear fallback for case-insensitive lookup becomes a performance
//          bottleneck for large collections.  Since physical printers are stored sorted
//          (case-sensitively), a case-insensitive binary search would need a custom
//          comparator — the current fallback accepts the cost silently.
std::deque<PhysicalPrinter>::iterator PhysicalPrinterCollection::find_printer_internal(const std::string& name,
...
void PhysicalPrinterCollection::select_printer(const PhysicalPrinter& printer) { return select_printer(printer.name); }

bool PhysicalPrinterCollection::has_selection() const { return m_idx_selected != size_t(-1); }
...
{
    return m_idx_selected == size_t(it - m_printers.begin()) && m_selected_preset == preset_name;
}

namespace PresetUtils {
// [INTENT] system_printer_bed_model/bed_texture/hotend_model resolve the vendor asset
//          paths for the currently selected printer preset.  Each uses a two-tier lookup:
//          data_dir() first (user's customized copy), falling back to resources_dir()
//          (bundled default).  hotend_model additionally falls back to the bundled
//          generic hotend.stl if no vendor-specific model exists.
// [COUPLING] All three functions call system_printer_model() which dereferences
//            preset.vendor — if vendor is nullptr (user preset without system parent),
//            all three silently return an empty string / the generic fallback path.
const VendorProfile::PrinterModel* system_printer_model(const Preset& preset)
{
    ...
    if (preset.vendor != nullptr) {
        ...
        if (printer_model != nullptr && !printer_model->value.empty()) {
            auto it = std::find_if(preset.vendor->models.begin(), preset.vendor->models.end(),
                                   [printer_model](const VendorProfile::PrinterModel& pm) { return pm.id == printer_model->value; });
