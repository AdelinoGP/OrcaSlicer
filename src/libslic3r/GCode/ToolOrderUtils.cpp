// [INTENT] ToolOrderUtils.cpp — implements the min-flush tool-ordering algorithms.
//
// MinCostMaxFlow (MCMF) — SPFA-based bipartite matching solver:
//   - spfa(source, sink): runs Shortest Path Faster Algorithm (Bellman-Ford with queue)
//     to find a minimum-cost augmenting path in the residual graph.
//   - Returns false when sink is unreachable (no augmenting path → flow is maximum).
//   - Flow augmentation: traces back via prev[] and increments edge flows.
//   - The `edges[prev_edge ^ 1]` XOR trick relies on forward/reverse edges being
//     stored in consecutive even/odd positions in the edges vector [HAZARD H852].
//   - solve(): repeatedly calls spfa until no augmenting path exists, then reads
//     back the matching from left→right edges with flow > 0.
//
// MaxFlowSolver — BFS-based plain max-flow (Edmonds-Karp) without cost:
//   - Used for feasibility-only matching (does not minimise flush volume).
//   - Same XOR trick for reverse edges [HAZARD H852].
//
// solve_extruder_order (Hamiltonian shortest path via bitmask DP):
//   - State: bitmask of visited extruders × last extruder visited.
//   - Transition: cache[state | (1<<t)][t] = min(cache[state][m] + wipe[m][t])
//   - Complexity: O(2^N × N²) time, O(2^N × N) space (float).
//   - [HAZARD H850]: at N=20 this is ~6 GB of cache memory.
//
// solve_extruder_order_with_forcast (brute-force over curr × next permutations):
//   - Uses std::next_permutation to enumerate all orderings of curr and next layers.
//   - Outer loop: all orderings of curr_layer_extruders (N! iterations)
//   - Inner loop: all orderings of next_layer_extruders (M! iterations)
//   - Total: O(N! × M!) — safe only for N,M ≤ 5. [HAZARD H851].
//   - Best ordering is the one minimising sum(wipe[prev→curr]) + sum(wipe[prev→next]).
//   - Tie-break by total filament change count (minimise unnecessary tool changes).
//
// solve_extruder_order_with_greedy (nearest-neighbour heuristic):
//   - O(N²), used as fallback for >20 extruders.
//   - At each step, picks the unvisited extruder with minimum wipe cost from current.
//   - Not optimal but guaranteed O(1) memory beyond inputs.
//
// reorder_filaments_for_minimum_flush_volume:
//   - Groups filaments by AMS group (groups[0] / groups[1]).
//   - For each layer per group, computes best filament order via get_extruders_order.
//   - Caches results per (curr_set, next_set, prev_extruder) using a uint128_t
//     bit-packed hash key (boost::multiprecision::uint128_t). Bit layout:
//       bits 0-31:   curr layer extruder set (bit i = extruder i present)
//       bits 32-63:  next layer extruder set (only when use_forcast=true)
//       bits 64-95:  prev extruder (bit 64+i = extruder i was last)
//   - Interleaves group 0 and group 1 sequences per layer, tracking last_group_id
//     to alternate groups and minimise cross-group flush transitions.
//
// [HAZARD H852] edges[prev_edge ^ 1] reverse-edge XOR trick: correct ONLY if
// edges are always added in pairs (forward at even index, reverse at odd).
// add_edge() always pushes two edges: this invariant holds as long as add_edge is
// the only way to add edges. A single push_back outside add_edge would break all
// subsequent XOR lookups silently.
//
// [HAZARD H853] MinCostMaxFlow::get_distance: the if (l_nodes[idx_in_left] == -1)
// branch has dead code (the sum/divide block after `return 0`). The TODO comment
// says "test more here". The dead code computes an average flush volume for a
// "virtual" start node — this was clearly intended as fallback logic but was
// disabled with an early return. If l_nodes ever contains -1 in production, the
// distance will silently be 0 rather than the intended average.
...
MaxFlowSolver::MaxFlowSolver(const std::vector<int>&                          u_nodes,
...
GeneralMinCostSolver::GeneralMinCostSolver(const std::vector<std::vector<float>>& matrix_,
...
std::vector<int> GeneralMinCostSolver::solve() { return m_solver->solve(); }

MinFlushFlowSolver::~MinFlushFlowSolver() {}

MinFlushFlowSolver::MinFlushFlowSolver(const std::vector<std::vector<float>>&           matrix_,
                                       const std::vector<int>&                          u_nodes,
                                       const std::vector<int>&                          v_nodes,
                                       const std::unordered_map<int, std::vector<int>>& uv_link_limits,
                                       const std::unordered_map<int, std::vector<int>>& uv_unlink_limits,
...
std::vector<int> MinFlushFlowSolver::solve() { return m_solver->solve(); }

MatchModeGroupSolver::~MatchModeGroupSolver() {}

MatchModeGroupSolver::MatchModeGroupSolver(const std::vector<std::vector<float>>&           matrix_,
                                           const std::vector<int>&                          u_nodes,
                                           const std::vector<int>&                          v_nodes,
                                           const std::vector<int>&                          v_capacity,
                                           const std::unordered_map<int, std::vector<int>>& uv_unlink_limits)
...
std::vector<int> MatchModeGroupSolver::solve() { return m_solver->solve(); }
                                                                   ...
                                                                   float*                                 min_cost)
{
    ...
    auto get_filament_change_count = [](const std::vector<unsigned int>& curr_seq, const std::vector<unsigned int>& next_seq,
                                        const std::optional<unsigned int>& start_extruder_id) {
        int  count            = 0;
                ...
                if (state >> target & 1) {
                    for (unsigned int mid_point = 0; mid_point < all_extruders.size(); ++mid_point) {
                        if (state >> mid_point & 1) {
                            auto tmp = cache[state - (1 << target)][mid_point] +
                                       wipe_volumes[all_extruders[mid_point]][all_extruders[target]];
...
template<class T>
static std::vector<T> collect_filaments_in_groups(const std::unordered_set<unsigned int>& group,
                                               ...
                                               std::vector<std::vector<unsigned int>>*                    filament_sequences)
{
    ...
    auto extruders_to_hash_key = [](const std::vector<unsigned int>& curr_layer_extruders,
                                    const std::vector<unsigned int>& next_layer_extruders, const std::optional<unsigned int>& prev_extruder,
        ...
        for (size_t layer = 0; layer < layer_filaments.size(); ++layer) {
            ...
            if (auto iter = custom_layer_sequence_map.find(layer); iter != custom_layer_sequence_map.end()) {
                ...
                for (auto& f : sequence_in_group) {
                    if (prev) {
                        tmp_cost += flush_matrix[idx][*prev][f];
                    }
                    prev = f;
                }
                cost += tmp_cost;
            ...
            bool                      use_forcast = (filament_used_in_group.size() <= max_n_with_forcast &&
                                filament_used_in_group_next_layer.size() <= max_n_with_forcast);
            float                     tmp_cost    = 0;
            std::vector<unsigned int> sequence;
            uint128_t hash_key = extruders_to_hash_key(filament_used_in_group, filament_used_in_group_next_layer, current_extruder_id,
                                                       use_forcast);
            if (auto iter = caches.find(hash_key); iter != caches.end()) {
                tmp_cost = iter->second.first;
                sequence = iter->second.second;
            } else {
                sequence         = get_extruders_order(flush_matrix[idx], filament_used_in_group, filament_used_in_group_next_layer,
                                                       current_extruder_id, use_forcast, &tmp_cost);
