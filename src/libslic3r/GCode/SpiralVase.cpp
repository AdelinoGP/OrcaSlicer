// [INTENT] SpiralVase.cpp — implements the Z-ramp spiral vase algorithm.
//
// Algorithm overview (process_layer):
//   Pass 1: Clone the GCodeReader and re-parse the layer to compute
//           total_layer_length (sum of XY extrusion move distances) and
//           layer_height (sum of Z increments on this layer).
//
//   Pass 2: Re-parse with the real reader, rewriting each G1 move:
//     - Initial Z-only move → replaced with Z=previous_layer_top (no advance)
//     - Extrusion moves → Z linearly interpolated from layer_bottom to layer_top
//       based on cumulative XY progress (len / total_layer_length)
//     - Travel moves → silently dropped (comment in code explains the seam rationale)
//     - transition_in (first spiral layer): E scaled from starting_flow_ratio→100%
//     - transition_out (last spiral layer): duplicates moves into transition_gcode
//       with E scaled 100%→finishing_flow_ratio, appended at the end
//
// Smooth spiral extension (m_smooth_spiral):
//   XY of each point is blended with the nearest point on the previous layer's
//   polyline using a lerp factor = 1 - len/total_layer_length. This distributes
//   the layer-start seam over the full revolution.
//
// [MEMORY] m_previous_layer is raw new/delete. current_layer is also raw new,
// assigned to m_previous_layer at end of each call. See [HAZARD H847].
//
// [HAZARD H848] Pass 1 clones the GCodeReader by value (`GCodeReader r = m_reader`).
// The comment says "FIXME: This copies the GCodeConfig of the reader." The copy is
// O(size of config) but happens per layer — could be replaced by a lightweight
// reset-and-replay if performance matters.
//
// [HAZARD H849] transition_out only works with relative E distances
// (m_config.use_relative_e_distances.value). With absolute E, the E scaling on the
// cloned transition lines will be wrong (it would scale the absolute position, not
// the extrusion amount). This is documented with a FIXME in the code.
//
// [COUPLING] Calls line.set(Z/X/Y/E) using GCodeLine::set(), which has raw-string
// splice hazards described in H837/H838.
...
std::string SpiralVase::process_layer(const std::string& gcode, bool last_layer)
{
    ...
    m_reader.parse_buffer(gcode, [&new_gcode, &z, total_layer_length, layer_height, transition_in, &len, &current_layer, &previous_layer,
                                  &transition_gcode, transition_out, smooth_spiral, &max_xy_dist_for_smoothing, &last_point,
                                  starting_flowrate, finishing_flowrate](GCodeReader& reader, GCodeReader::GCodeLine line) {
        if (line.cmd_is("G1")) {
            // Orca: Filter out retractions at layer change
            if (line.retracting(reader) || (line.extruding(reader) && line.dist_XY(reader) < EPSILON))
                return;
