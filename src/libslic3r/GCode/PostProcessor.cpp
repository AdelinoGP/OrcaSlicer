// [INTENT] PostProcessor.cpp — platform-specific child process launcher and
// G-code line-number inserter.
//
// Platform split:
//   WIN32: Uses Win32 CreateProcessW / WaitForSingleObject. Scripts are dispatched
//          via their native type:
//            *.pl  → bundled perl.exe in <slicer_dir>/perl/
//            *.bat → cmd.exe /C
//            other → direct execution (e.g. Python, PowerShell)
//          Command-line quoting uses the correct backslash-doubling algorithm from
//          the MSDN "Everyone Quotes Command Line Arguments The Wrong Way" blog post.
//   POSIX: Uses boost::process::child with the user's $SHELL (fallback /bin/sh -c).
//          G-code path is single-quote-escaped to handle spaces and special chars.
//
// [HAZARD H844] WIN32: WaitForSingleObject(INFINITE) — no timeout. A hanging
// post-processing script will hang the slicer indefinitely.
//
// [HAZARD H845] POSIX: $SHELL expansion is not validated. If $SHELL points to an
// unusual binary (e.g. fish, csh, tcsh), the `-c <command>` convention may not work.
// No fallback to /bin/bash is attempted beyond the /bin/sh default.
//
// [HAZARD H846] WIN32: The environment is captured via GetEnvironmentStrings() at
// the time run_post_process_scripts() is called. If the print config sets env vars
// (config.setenv_()) AFTER this capture, the child process will not see them.
// The current code calls setenv_() before run_script(), and GetEnvironmentStrings()
// is called inside execute_process_winapi(), so the order is actually correct —
// but this ordering dependency is fragile and undocumented.
            ...
            for (std::string script : lines) {
                ...
                if (!boost::filesystem::exists(gcode_file)) {
                    const std::string msg = (boost::format(
                                                 _(L("Post-processing script %1% failed.\n\n"
