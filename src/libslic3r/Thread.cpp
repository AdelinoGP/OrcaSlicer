// [INTENT] Platform-abstracted thread naming utilities + TBB worker thread
// initialisation (name + locale).
// [HAZARD H1190] name_tbb_thread_pool_threads_set_locale (line 213): the
// guard `static bool initialized` is a plain bool written without any atomic
// or mutex protection.  If two threads call this simultaneously before
// initialization completes, both could proceed past the guard and run the
// tbb::parallel_for twice, causing double locale initialisation.  The comment
// on line 36 ("Not thread safe!") explicitly documents that the Windows
// API-lookup function has the same issue; both must be called from the main
// thread before spawning workers.
// [STATE] Four static Windows globals (s_hKernel32, s_fnSet/GetThreadDescription,
// s_SetGetThreadDescriptionInitialized) and one static initialized bool.
// [CONCURRENCY] All static state is intentionally initialised once from main
// thread; racy if violated.
// [COUPLING] Depends on TBB task_arena, boost::thread, platform pthread/Win32.
#ifdef _WIN32
#include <windows.h>
...
bool set_current_thread_name(const char* thread_name) { return WindowsSetThreadName(::GetCurrentThread(), thread_name); }
...
void save_main_thread_id() { g_main_thread_id = boost::this_thread::get_id(); }

// Retrieve the cached main (UI) thread ID.
boost::thread::id get_main_thread_id() { return g_main_thread_id; }

// Checks whether the main (UI) thread is active.
bool is_main_thread_active() { return get_main_thread_id() == boost::this_thread::get_id(); }
...
void name_tbb_thread_pool_threads_set_locale()
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, nthreads, 1),
