// [INTENT] FlushVolPredictor.cpp — ML-table-driven flush volume prediction for filament changes.
//   Two layers:
//     1. FlushPredict namespace: color utilities — RGB↔LAB conversion and CIEDE2000 color distance.
//     2. FlushVolPredictor class: loads a precomputed flush-volume data file (TSV with RGB pairs
//        and empirically measured flush volumes), looks up by closest color match.
//     3. GenericFlushPredictor: thin facade over a static map of FlushVolPredictor instances
//        (one per dataset_value: 0=standard, 1=dual-standard, 2=dual-highflow).
// [STATE] FlushVolPredictor is immutable after construction (all data loaded from file).
//   `predictor_instances` is a file-scope static map — loaded lazily on first use per dataset value.
// [CONCURRENCY] `predictor_instances` is an unprotected static global. Two threads constructing
//   GenericFlushPredictor with the same dataset_value simultaneously will race on:
//     predictor_instances[dataset_value] = FlushVolPredictor(path) — a data race.
//   [HAZARD H1144] This race is the same lazy-init pattern as H1109 (PresetBundle), H1102 (Print).
//   Fix: std::call_once or a mutex around the insert.
// [COUPLING] Called from PrintConfig.cpp and GUI filament-change logic to populate
//   flush volume suggestions in the UI and in G-code flush tower calculations.
// [HAZARD H1145] FlushVolPredictor::predict() does a linear scan through m_colors to find the
//   closest color to `from` and `to` separately. The scan uses is_similar_color() with a fixed
//   default threshold (not passed — uses the default 8.0 from FlushVolPredictor.hpp). The first
//   match is returned (not the best match). For colors that are equidistant from two palette
//   entries, the match is determined by palette order in the data file — non-deterministic across
//   file versions. A port should use a nearest-neighbor search (e.g. k-d tree) and return the
//   globally closest color rather than the first within threshold.
...
static double rad_to_deg(double rad) { return 180.0 / M_PI * rad; }

static double deg_to_rad(double deg) { return deg * M_PI / 180.0; }

// [INTENT] Convert sRGB (uint8 0–255) to CIE L*a*b* (CIELAB D65 illuminant).
//   Uses standard sRGB gamma linearization and the D65 white point (XN=95.0489, YN=100, ZN=108.884).
//   XYZ reference values are hardcoded — port must match exactly or color distances will shift.
...
// [INTENT] Compute CIEDE2000 perceptual color distance between two CIELab colors.
//   Implements the full CIEDE2000 formula (Luo, Cui, Rigg 2001) including the RT rotational term.
//   K_L = K_C = K_H = 1.0 (parametric factors — standard values).
//   Returns a float distance; threshold for "similar" colors is typically 8.0 (see header default).
// [STATE] Uses a file-scope static `pow_25_to_7 = pow(25,7)` — computed once at function scope
//   (actually static local, initialized on first call; thread-safe in C++11).
float calc_color_distance(const LABColor& lab1, const LABColor& lab2)
{
    ...
    const double T = 1 - 0.17 * cos(p_H_mean - deg_to_rad(30)) + 0.24 * cos(2 * p_H_mean) + 0.32 * cos(3 * p_H_mean + deg_to_rad(6)) -
                     0.2 * cos(4 * p_H_mean - deg_to_rad(63));
    ...
    double de = sqrt(pow(delta_L / (K_L * S_L), 2) + pow(delta_C / (K_C * S_C), 2) + pow(delta_H / (K_H * S_H), 2) +
...
int FlushVolPredictor::get_min_flush_volume()
{
    ...
    return static_cast<int>(
        std::min_element(m_flush_map.begin(), m_flush_map.end(), [](const auto& a, const auto& b) { return a.second < b.second; })->second);
}
...
// [STATE] File-scope static map: lazily populated, one FlushVolPredictor per dataset_value.
//   dataset_value 0 = standard, 1 = dual-standard, 2 = dual-highflow.
// [HAZARD H1144] No mutex. Concurrent construction of GenericFlushPredictor with the same
//   dataset_value races on this map. See file-level header.
static std::unordered_map<int, FlushVolPredictor> predictor_instances;

// [INTENT] Facade: find-or-load the FlushVolPredictor for dataset_value, cache in static map.
//   Path is determined by dataset_value with hardcoded suffix strings — no configurability.
// [HAZARD H1144] (see above — map insert is not mutex-protected)
