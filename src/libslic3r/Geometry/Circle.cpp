...
// [INTENT] Circle.cpp: implementations of the non-template circle-fitting functions:
//   - circle_center_taubin_newton (overloads for Points and Vec2ds)
//   - circle_taubin_newton
//   - circle_ransac
// [COUPLING] Consumed by GCode arc-detection, SeamPlacer, and Arachne geometry helpers.
...
// [INTENT] Integer-point overload: unscales to mm before delegating to the double overload.
// [COUPLING] Thin adapter; all arithmetic is in the Vec2d overload below.
...
/// lie on.
// [INTENT] Taubin-Newton circle fit: computes normalized 2nd-order moments of the input, builds
// the characteristic polynomial (Chernov eq 5.60), and uses Newton's method to find the root.
// The root gives the algebraic distance minimizer, from which center coordinates are extracted.
// [COMPLEXITY] O(n) for moment accumulation + O(cycles) for Newton iterations (usually ~5).
// [HAZARD] H576 (Low): If Newton's method diverges (|ynew| > |yold|), returns NaN center.
//   Callers must check std::isnan on the result or wrap in circle_taubin_newton which
//   propagates the invalid state as Circled::make_invalid().
// [MEMORY] O(1) extra — all accumulation in local doubles.
Vec2d circle_center_taubin_newton(const Vec2ds::const_iterator& input_begin, const Vec2ds::const_iterator& input_end, size_t cycles)
{
    ...
    for (size_t i = 0; i < cycles; ++i) {
        ...
        if (std::abs((xnew - xold) / xnew) < 1e-12)
...
Circled circle_taubin_newton(const Vec2ds& input, size_t cycles)
{
    ...
    if (input.size() < 3) {
        ...
        out.radius = std::accumulate(input.begin(), input.end(), 0.,
                                     [&out](double acc, const Vec2d& pt) { return (pt - out.center).norm() + acc; });
        out.radius /= double(input.size());
    }
    return out;
}

// [INTENT] RANSAC circle fit: randomly sample 3-point subsets, compute circumcenter, average
// radius over all points, track best-by-max-deviation winner.
// [HAZARD] H572 (Medium): std::mt19937 default-constructed (seed 0) — deterministic, NOT random.
//   See Circle.hpp declaration for full hazard text.
// [COMPLEXITY] O(iterations * n) — each iteration evaluates all n points.
// [MEMORY] O(n) for the samples vector (at most 3 elements, but vector allocated).
//   Port note: replace vector<Vec2d> samples with std::array<Vec2d, 3> to avoid heap.
Circled circle_ransac(const Vec2ds& input, size_t iterations, double* min_error)
{
    ...
    for (size_t iter = 0; iter < iterations; ++iter) {
        ...
        c.radius = std::accumulate(input.begin(), input.end(), 0.,
                                   [&c](double acc, const Vec2d& pt) { return (pt - c.center).norm() + acc; });
