...
// [INTENT] MedialAxis.cpp: Medial Axis Transform (MAT) skeleton extraction from ExPolygon.
// Pipeline:
//   1. Convert ExPolygon to Lines.
//   2. Construct VoronoiDiagram from lines (with validity repair if needed).
//   3. annotate_inside_outside() marks which VD vertices/edges are inside the polygon.
//   4. Iterate all primary, finite edges; validate width and orientation via validate_edge().
//   5. Chain validated edges into ThickPolylines via process_edge_neighbors().
//
// [COUPLING] Depends on VoronoiDiagram (Voronoi.hpp + Voronoi.cpp) for construction and repair.
//   Uses ClipperUtils::closing_ex() for morphological closing in repair fallback path.
// [MEMORY] m_edge_data is sized edges.size()/2; built fresh on each build() call.
//   ThickPolylines output is allocated by caller and filled via emplace_back.
// [CONCURRENCY] Not thread-safe — build() modifies m_vd (repairs in place) and m_edge_data.
// [HAZARD] H580 (Low): Debug build (SLIC3R_DEBUG) embeds a copy of boost::polygon::voronoi_visual_utils
//   inside namespace boost::polygon. If the actual Boost version ships its own voronoi_visual_utils,
//   this causes an ODR violation (two definitions of the same class in the same namespace).
//   Port note: move the debug visualization code into a separate debug-only file.
...
inline point_type retrieve_point(const std::vector<segment_type>& segments, const cell_type& cell)
{
    ...
    return (cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? low(segments[cell.source_index()]) :
                                                                                             high(segments[cell.source_index()]);
}

inline void clip_infinite_edge(const std::vector<segment_type>& segments,
                               ...
                               std::vector<point_type>*         clipped_edge)
{
    ...
    if (edge.vertex0() == NULL) {
        clipped_edge->push_back(point_type(origin.x() - direction.x() * koef, origin.y() - direction.y() * koef));
    } else {
...
inline void sample_curved_edge(const std::vector<segment_type>& segments,
                               const edge_type&                 edge,
                               std::vector<point_type>&         sampled_edge,
                               coordinate_type                  max_dist)
{
    point_type   point   = edge.cell()->contains_point() ? retrieve_point(segments, *edge.cell()) :
...
void dump_voronoi_to_svg(const Lines&                                         lines,
                         /* const */ boost::polygon::voronoi_diagram<double>& vd,
                         const ThickPolylines*                                polylines,
                         const char*                                          path)
{
    ...
    for (boost::polygon::voronoi_diagram<double>::const_edge_iterator it = vd.edges().begin(); it != vd.edges().end(); ++it) {
        ...
        for (std::size_t i = 0; i + 1 < samples.size(); ++i)
            svg.draw(Line(Point(coord_t(samples[i].x()), coord_t(samples[i].y())),
                          Point(coord_t(samples[i + 1].x()), coord_t(samples[i + 1].y()))),
                     color, voronoiLineWidth);
    }
#endif
...
template<typename VD, typename SEGMENTS>
inline std::pair<typename VD::coord_type, typename VD::coord_type> measure_edge_thickness(const VD&                     vd,
...
template<typename VD> class Lines2VDSegments
{
public:
    Lines2VDSegments(const Lines& alines) : lines(alines) {}
    typename VD::segment_type operator[](size_t idx) const
    {
        return typename VD::segment_type(typename VD::point_type(typename VD::coord_type(lines[idx].a(0)),
                                                                 typename VD::coord_type(lines[idx].a(1))),
                                         typename VD::point_type(typename VD::coord_type(lines[idx].b(0)),
                                                                 typename VD::coord_type(lines[idx].b(1))));
    }
...
// [INTENT] build() orchestrates the MAT extraction pipeline:
//   1. Construct VD; if invalid, repair via morphological closing and re-try once.
//   2. annotate_inside_outside() labels VD vertices/edges as inside/outside the polygon.
//   3. Allocate m_edge_data (one entry per edge pair); validate and populate each edge.
//   4. Chain validated edges into ThickPolylines: seed from first active edge, grow forward
//      and backward via process_edge_neighbors(), then emit.
// [HAZARD] H581 (Medium): The repair path (closing_ex + re-construct) may change the polygon
//   topology by merging very close points. The resulting VD will be of a modified polygon,
//   not the original. Downstream width filtering is then relative to the repaired shape.
//   Port note: log or flag repair so callers can detect quality degradation.
...
void MedialAxis::process_edge_neighbors(const VD::edge_type* edge, ThickPolyline* polyline)
{
    for (;;) {
        ...
        if (num_neighbors == 1) {
            if (std::pair<EdgeData&, bool> neighbor_data = this->edge_data(*first_neighbor); neighbor_data.first.active) {
...
// [INTENT] validate_edge: determine if a Voronoi edge belongs to the MAT skeleton.
// Two-step filter:
//   1. Width filter: compute cross-section thickness at both edge endpoints (distance to nearest
//      boundary * 2). Accept if at least one endpoint's width is in [min_width, max_width].
//   2. Angle filter (for two-segment cells only): compute orientation angle between the two
//      generating segments. Reject if angle is too far from PI (not facing / not narrow region),
//      unless the edge is very short (< min_width), in which case keep regardless.
// [COUPLING] read m_lines[], m_min_width, m_max_width; writes edge_data()->width_start/end.
// [HAZARD] H582 (Low): The angle threshold (PI/8 = 22.5°) is hardcoded. For non-standard
//   extrusion geometries this may prune valid thin-wall features. No way to configure at runtime.
bool MedialAxis::validate_edge(const VD::edge_type* edge)
{
    ...
    const Line line({edge->vertex0()->x(), edge->vertex0()->y()}, {edge->vertex1()->x(), edge->vertex1()->y()});
    ...
    */
    ...
    coordf_t w0 = cell_r->contains_segment() ? segment_r.distance_to(line.a) * 2 :
                                               (retrieve_endpoint(cell_r) - line.a).cast<double>().norm() * 2;

    coordf_t w1 = cell_l->contains_segment() ? segment_l.distance_to(line.b) * 2 :
    ...
    if ((w0 >= m_min_width || w1 >= m_min_width) && (w0 <= m_max_width || w1 <= m_max_width)) {
