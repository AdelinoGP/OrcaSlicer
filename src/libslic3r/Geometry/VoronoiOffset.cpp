...
//
// [INTENT] Implements Voronoi-based polygon offsetting for Slic3r.
// Pipeline:
//   1. annotate_inside_outside()   — classify every VD vertex/edge/cell as
//      Inside/Outside/OnContour/Boundary using signed half-plane geometry.
//   2. signed_vertex_distances()   — compute signed distance from each VD
//      vertex to the nearest input site (negative inside, positive outside).
//   3. edge_offset_contour_intersections() — find where the iso-curve at
//      |offset_distance| crosses each VD half-edge.
//   4. offset()                    — trace those intersection points into
//      closed Polygons, discretizing circular arcs at point sites.
//   5. skeleton_edges_rough()      — mark VD edges that are part of the
//      medial axis skeleton above a dr/dl threshold.
//
// [MEMORY] All functions are stateless; they return vectors or mutate VD
//   color fields in-place.  Peak memory is O(E) for intersection point tables.
// [CONCURRENCY] Not thread-safe: annotation writes VD color fields globally.
// [COUPLING] Relies on boost::polygon VD layout (twin halfedges consecutive,
//   color() fields castable to enum, contiguous containers).
// [HAZARD] VORONOI_DEBUG_OUT: debug build dumps SVG files to debug_out_path().
//   Disabled by default (#define VORONOI_DEBUG_OUT to enable).
...
namespace detail {
// [INTENT] Intersect a ray (start=pt, direction=v) with a circle (center, radius r).
//   Returns the parameter t >= 0 such that pt + t*v lies on the circle.
//   Used exclusively for unbounded (infinite) VD edges that start inside
//   the offset circle and extend outward.
// [HAZARD] Asserts pt is inside and pt+v is outside the circle — caller must
//   supply a direction large enough to exit the circle.
...
// [INTENT] Return 0, 1, or 2 points equidistant from pt1, pt2, and at distance d
//   from each. Arises when computing offset contour intersections on Point-Point
//   VD edges (the bisector of two input segment endpoints).
// [HAZARD] xy_swap trick: coordinates are swapped when |cx| < |cy| to avoid
//   division by near-zero cx. Coordinates are swapped back before return.
...
Intersections point_point_equal_distance_points(const Point& pt1, const Point& pt2, const double d)
{
    ...
    double        e = 0.5 / cx;
    double        f = 0.5 * cy + u;
    Intersections out{cnt, {Vec2d(-e * (v * w - cl), v), Vec2d(-e * (w * f - cl), f)}};
    if (xy_swapped) {
...
Intersections line_point_equal_distance_points(const Line& line, const Point& ipt, const double d)
{
    ...
    double        g = f - u;
    double        h = f + u;
    Intersections out{cnt, {Vec2d((-b * g + e) / a, g), Vec2d((-b * h + e) / a, h)}};
    if (xy_swapped) {
        std::swap(out.pts[0].x(), out.pts[0].y());
        std::swap(out.pts[1].x(), out.pts[1].y());
    }
    ...
    assert(std::abs(Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(), out.pts[0]) - d) <
           SCALED_EPSILON);
    assert(std::abs(Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(), out.pts[1]) - d) <
           SCALED_EPSILON);
...
template<typename VertexType> inline bool vertex_equal_to_point(const VertexType& vertex, const Point& ipt)
{
...
bool vertex_equal_to_point(const VD::vertex_type* vertex, const Point& ipt) { return vertex_equal_to_point(*vertex, ipt); }
...
double dist_to_site(const Lines& lines, const VD::cell_type& cell, const Vec2d& point)
{
    ...
    return cell.contains_point() ?
               (((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b).cast<double>() - point)
                   .norm() :
    ...
    for (const VD::cell_type& cell : vd.cells()) {
        ...
        do {
            ...
            }
            {
                ...
                switch (vc) {
                case VertexCategory::Inside: ++num_vertices_inside; break;
                case VertexCategory::Outside: ++num_vertices_outside; break;
                case VertexCategory::OnContour: ++num_vertices_on_contour; break;
                default: assert(false);
                }
            }
            {
                ...
                switch (cc) {
                case CellCategory::Boundary: assert(cc_other != CellCategory::Boundary || cell_other->contains_segment()); break;
                case CellCategory::Inside: assert(cc_other == CellCategory::Inside || cc_other == CellCategory::Boundary); break;
                case CellCategory::Outside: assert(cc_other == CellCategory::Outside || cc_other == CellCategory::Boundary); break;
...
bool verify_offset_intersection_points(const VD&                 vd,
                                       const Lines&              lines,
...
// [INTENT] Zero all color fields on VD vertices, edges, and cells to Unknown.
//   Called before annotate_inside_outside() to ensure a clean annotation slate.
// [STATE] Mutates VD color fields in-place via const_cast (VD is logically const but
//   color fields are annotation state).
// [HAZARD] const_cast is intentional but exposes a hidden mutation footgun for callers
//   that hold const VD references.
...
// [INTENT] Classify every VD vertex, edge, and cell as Inside / Outside / OnContour / Boundary.
//   Four-phase algorithm:
//     1. on_site check: vertices coinciding with input contour points → OnContour.
//     2. Secondary-edge endpoints: one endpoint guaranteed on contour → OnContour.
//     3. Segment-site half-plane: cross product of vertex vs. line site → Inside/Outside
//        for all edges incident on at least one segment cell; propagates to cells.
//     4. Seed-fill BFS: remaining Point-Point edges inherit category from annotated neighbours.
// [STATE] Mutates all color fields of vd in-place.
// [CONCURRENCY] Not thread-safe: writes to shared VD state.
// [HAZARD] H590: assert(side != 0.) on line 894 fires on degenerate inputs where a VD
//   vertex lies exactly on the extension of an input line; aborts debug builds.
// [COUPLING] Relies on boost::polygon VD invariants: secondary edges connect point↔segment
//   cells, twin halfedges are consecutive in vd.edges(), vertex0/1 ownership is consistent.
void annotate_inside_outside(VD& vd, const Lines& lines)
{
    ...
    auto annotate_vertex = [](const VD::vertex_type* vertex, VertexCategory new_vertex_category) {
        ...
        assert(new_vertex_category == VertexCategory::Inside || new_vertex_category == VertexCategory::Outside ||
    ...
    auto annotate_edge = [](const VD::edge_type* edge, EdgeCategory new_edge_category) {
        ...
        case EdgeCategory::PointsOutside:
            // assert(edge->vertex0() != nullptr);
            break;
        case EdgeCategory::PointsToContour: assert(edge->vertex1() != nullptr); break;
        default: assert(false);
        }
#endif // NDEBUG
    ...
    auto annotate_cell = [](const VD::cell_type* cell, CellCategory new_cell_category) -> bool {
        ...
        assert(new_cell_category == CellCategory::Inside || new_cell_category == CellCategory::Outside ||
    ...
    for (const VD::edge_type& edge : vd.edges()) {
        if (edge.is_secondary() && edge.vertex0() != nullptr) {
            ...
            const Point& pt_on_contour = edge.cell()->contains_point() ? contour_point(*edge.cell(), lines) :
    ...
    for (const VD::edge_type& edge : vd.edges())
        if (edge.vertex1() == nullptr) {
            ...
            // Only one of the two vertices may lie on input contour.
            assert(!edge.is_linear() || vertex_category(edge.vertex0()) != VertexCategory::OnContour ||
            ...
            if (line) {
                ...
                if (!on_contour && cell == edge.cell() && edge.twin()->cell()->contains_segment()) {
                    ...
                    if (pt_on_contour) {
                        ...
                        auto                   on_contour = [&pt_on_contour](const VD::vertex_type* v) {
                            return std::abs(v->x() - pt_on_contour->x()) < 0.5001 && std::abs(v->y() - pt_on_contour->y()) < 0.5001;
                        };
                        assert(!on_contour(v0) && !on_contour(v1));
                    }
                }
                ...
                if (on_contour && v1_category == VertexCategory::OnContour) {
                    ...
                    annotate_cell(cell2, (on_contour && cell2->contains_segment()) ?
...
// [INTENT] Compute the signed Euclidean distance from each VD vertex to the nearest
//   input site. Sign convention: negative = inside polygon, zero = OnContour, positive = outside.
//   Distance is measured to the closest input geometry (point or segment) reachable via
//   the incident VD cell. Prefers point cells over segment cells for vertices surrounded
//   by mixed cells to avoid foot-point projection errors.
// [STATE] Read-only on vd and lines; returns a new vector indexed by vertex position.
// [COUPLING] vd must already be annotated by annotate_inside_outside().
// [HAZARD] If all incident cells contain segments (no point cell adjacent) the function
//   falls back to projecting the vertex onto the line of the last traversed cell's segment —
//   this is correct but relies on the Voronoi invariant that every vertex has at most one
//   equidistant segment.
    ...
    for (const VD::vertex_type& vertex : vd.vertices()) {
        ...
        if (vc == VertexCategory::OnContour) {
            ...
            if (point_cell == 0) {
                ...
                dist = Geometry::ray_point_distance<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(), vertex_point(vertex));
            } else {
...
// [INTENT] For each VD half-edge, find the point (if any) where the iso-distance curve
//   at |offset_distance| crosses that edge. Returns a Vec2d per edge: NaN.x = no crossing,
//   finite x/y = crossing point, .y==1 = edge marked visited (twin processed first).
//
//   Three edge types handled:
//     a) Infinite edges (v1==nullptr): monotone distance; one crossing possible.
//        Point-Point case: ray-circle intersection via first_circle_segment_intersection_parameter().
//        Point-Segment secondary case: normal direction on segment at endpoint.
//     b) Linear bisectors of two segments and secondary edges: distance is linear → simple lerp.
//     c) Point-Segment or Point-Point finite edges: distance is non-monotone (parabolic or
//        hyperbolic); may produce 0, 1, or 2 crossings. Calls line_point_equal_distance_points()
//        or point_point_equal_distance_points() for closed-form solutions.
//
// [STATE] Read-only on vd/lines/vertex_distances; returns new vector of crossing points.
// [HAZARD] H589: open loops (next_edge==nullptr) are silently discarded (no returned error);
//   only logged via VORONOI_DEBUG_OUT.
// [HAZARD] Non-monotone edges with numerical discrepancies between expected and found
//   intersection counts are handled by fallback (artificial split point at dmin vertex).
// [COUPLING] Index arithmetic: uses pointer subtraction from vd.edges().front() — requires
//   contiguous boost VD storage.
        ...
        if (v1 == nullptr) {
            ...
            if (offset_distance > dmin) {
                ...
                if (cell->contains_point() && cell2->contains_point()) {
                    ...
                    assert(line.a == ipt || line.b == ipt);
                    out[edge_idx] = ipt.cast<double>() +
                                    offset_distance * Vec2d(line.b.y() - line.a.y(), line.a.x() - line.b.x()).normalized();
                }
            ...
            if (bisector || edge.is_secondary()) {
                ...
                // The distance function along the Voronoi edge may either have one maximum at one vertex and one minimum at the other
                // vertex, or it may have two maxima at the vertices and a minimum somewhere along the Voronoi edge, and this Voronoi edge
                ...
                if (num_intersections > 0) {
                    ...
                    if (point_vs_segment) {
                        assert(cell->contains_point() || cell2->contains_point());
                        intersections = detail::line_point_equal_distance_points(cell->contains_segment() ? line0 : line1, pt0,
                                                                                 offset_distance);
                    } else {
                    ...
                    // the number of intersections found may not be in accord to the number of intersections expected from evaluating
                    // simpler expressions. Adjust the result to the number of intersection points expected.
...
// [INTENT] Trace the iso-distance curves computed by edge_offset_contour_intersections()
//   into closed Polygons representing the offset contour(s).
//
//   Algorithm:
//     - For each unconsumed intersection point (seed), follow next_offset_edge() traversal
//       around VD cells until the loop closes back at the seed edge.
//     - Between consecutive intersection points that lie in a Point-site cell, a circular arc
//       is discretized with angle_step derived from discretization_error / offset_distance.
//     - Segment-site transitions produce linear segments (no arc).
//     - Intersection points are consumed (NaN-stamped) to prevent re-use as seeds.
//     - Output: outer contours CCW, holes CW (matches Clipper / Slic3r polygon conventions).
//
// [STATE] Modifies edge_points (local copy) in-place to mark consumed intersections.
// [MEMORY] Peak memory: O(E) for edge_points + O(V_output) for polygon vertices.
// [HAZARD] H589: if next_offset_edge() returns nullptr (open loop) the assert fires in
//   debug builds and loop is silently skipped in release builds — produces incomplete contours.
// [COUPLING] offset_distance sign convention: negative = inward offset, positive = outward.
//   The inside_offset flag is set once at entry and used to select the correct VD edges.
                ...
                double                          discretization_error)
{
    ...
    ++irun;
    {
        ...
        for (const VD::edge_type& edge : vd.edges())
            if (edge_category(edge) == (offset_distance > 0 ? EdgeCategory::PointsOutside : EdgeCategory::PointsInside) &&
                ...
                if (v1 == nullptr) {
                    ...
                    const Line&          line1 = lines[cell2->source_index()];
                    if (cell->contains_point() && cell2->contains_point()) {
                        const Point& pt0 = (cell->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line0.a :
                                                                                                                              line0.b;
                        const Point& pt1 = (cell2->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line1.a :
                                                                                                                               line1.b;
                        ...
                        const Point& ipt = cell->contains_segment() ?
                                               ((cell2->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ?
                                                    line1.a :
                                                    line1.b) :
                                               ((cell->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line0.a :
                                                                                                                                   line0.b);
                        // Infinite edge starts at an input contour, therefore there is always an intersection with an offset curve.
                        const Line& line = cell->contains_segment() ? line0 : line1;
                        assert(line.a == ipt || line.b == ipt);
        ...
        dump_voronoi_to_svg(debug_out_path("voronoi-offset-candidates1-%d.svg", irun).c_str(), vd, Points(), lines, Polygons(),
                            helper_lines);
    }
#endif // VORONOI_DEBUG_OUT
    ...
    Lines helper_lines;
    {
        for (const VD::edge_type& edge : vd.edges())
            if (edge_offset_has_intersection(edge_points[&edge - front_edge]))
                helper_lines.emplace_back(
                    Line(Point(edge.vertex0()->x(), edge.vertex0()->y()), Point(edge_points[&edge - front_edge].cast<coord_t>())));
        dump_voronoi_to_svg(debug_out_path("voronoi-offset-candidates2-%d.svg", irun).c_str(), vd, Points(), lines, Polygons(),
                            helper_lines);
    }
#endif // VORONOI_DEBUG_OUT
        ...
        if (edge_offset_has_intersection(last_pt)) {
            ...
            do {
                ...
                if (next_edge == nullptr) {
                    ...
                    dump_voronoi_to_svg(debug_out_path("voronoi-offset-open-loop-%d.svg", irun).c_str(), vd, Points(), lines, Polygons(),
                                        hl);
                }
#endif // VORONOI_DEBUG_OUT
                assert(next_edge);
...
#ifndef NDEBUG
                {
                    ...
                    if (std::max(err, err2) >= SCALED_EPSILON) {
                        Lines helper_lines;
                        dump_voronoi_to_svg(debug_out_path("voronoi-offset-incorrect_pt-%d.svg", irun).c_str(), vd, Points(), lines,
                                            Polygons(), to_lines(poly));
                    }
#endif // VORONOI_DEBUG_OUT
                ...
                if (cell->contains_point()) {
                    ...
                    // The extracted arc will have the same orientation. As the Voronoi regions are convex, the angle covered by the arc
                    // will be convex as well.
                    const Line&  line0  = lines[cell->source_index()];
                    const Vec2d& center = ((cell->source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line0.a :
                                                                                                                              line0.b)
                                              .cast<double>();
...
// [INTENT] Convenience wrapper: calls annotate_inside_outside() and signed_vertex_distances()
//   internally, then delegates to the 5-arg offset(). Suitable for one-shot use when the
//   caller does not need pre-computed distances.
// [HAZARD] H588: uses const_cast<VD&>(vd) to mutate VD color fields. Not thread-safe if
//   caller holds other const refs to the same VD. The VD is left annotated after the call.
...
// [INTENT] Identify VD edges that are part of the medial-axis skeleton above a dr/dl
//   threshold derived from threshold_alpha (angle of the sharpest corner to include).
//
//   Three cases per edge:
//     a) Segment-Segment bisector: dr/dl is constant along the bisector; compare once
//        against threshold_cos_alpha. Whole edge is either skeleton or not.
//     b) Point-Segment bisector (parabolic): parabola has a minimum dr/dl at the apex;
//        returns two threshold crossing positions (may be on the edge or NaN if outside).
//     c) Point-Point bisector (linear): hyperbola with minimum at midpoint; same threshold
//        crossing logic via point_point_dr_dl_thresholds().
//
//   Output per half-edge: Vec2d(nan,nan) = not skeleton; finite point = start position of
//   skeleton segment from that vertex. "Position 1" is never assigned.
//
// [STATE] Read-only on vd/lines; returns new skeleton-start vector.
// [COUPLING] vd must be annotated (annotate_inside_outside() called first).
// [HAZARD] Secondary and unbounded edges are always skipped; caller must not rely on them
//   appearing in skeleton output.
std::vector<Vec2d> skeleton_edges_rough(const VD&    vd,
                                        const Lines& lines,
    ...
    for (const VD::edge_type& edge : vd.edges()) {
        ...
        if (cell->contains_segment() && cell2->contains_segment()) {
            ...
            if (cell->contains_point() != cell2->contains_point()) {
                ...
                const Point& pt0  = cell->contains_point() ? contour_point(*cell, line0) : contour_point(*cell2, line1);
                const Line&  line = cell->contains_segment() ? line0 : line1;
                std::tie(out[edge_idx], out[edge_idx2]) = detail::point_segment_dr_dl_thresholds(pt0, line, vertex_point(v0),
                                                                                                 vertex_point(v1),
                ...
                const Point& pt0                        = contour_point(*cell, line0);
                const Point& pt1                        = contour_point(*cell2, line1);
                std::tie(out[edge_idx], out[edge_idx2]) = detail::point_point_dr_dl_thresholds(pt0, pt1, vertex_point(v0), vertex_point(v1),
                                                                                               threshold_tan_alpha_half);
            }
        }
    }
