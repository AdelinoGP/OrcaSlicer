// [INTENT] Implementation of IndexedMesh: AABB-tree ray-casting, nearest
// surface-point, and normal-computation for SLA support generation.
// [COUPLING] AABBTreeIndirect (libIGL/Embree).  Hole-aware path guarded by
// SLIC3R_HOLE_RAYCASTER which is unconditionally #undef'd in the header
// (dead code — H922).
...
// [INTENT] Pimpl for the AABB tree implementation, keeping libIGL headers
// out of IndexedMesh.hpp consumers.
// [STATE] m_triangle_ray_epsilon: computed once at init.  With
// calculate_epsilon=true it scales as l^2 (l = average edge length mm).
// Large CAD meshes with avg edge > 10 mm produce epsilon > 0.1 — may cause
// missed or false hits on thin features (see H921 family).
...
IndexedMesh::IndexedMesh(const indexed_triangle_set& tmesh, bool calculate_epsilon) : m_aabb(new AABBImpl()), m_tm(&tmesh)
{
    init(tmesh, calculate_epsilon);
}

IndexedMesh::IndexedMesh(const TriangleMesh& mesh, bool calculate_epsilon) : m_aabb(new AABBImpl()), m_tm(&mesh.its)
{
...
IndexedMesh::IndexedMesh(const IndexedMesh& other)
    : m_tm(other.m_tm), m_ground_level(other.m_ground_level), m_aabb(new AABBImpl(*other.m_aabb))
{}
...
const std::vector<Vec3f>& IndexedMesh::vertices() const { return m_tm->vertices; }

const std::vector<Vec3i32>& IndexedMesh::indices() const { return m_tm->indices; }

const Vec3f& IndexedMesh::vertices(size_t idx) const { return m_tm->vertices[idx]; }

const Vec3i32& IndexedMesh::indices(size_t idx) const { return m_tm->indices[idx]; }

Vec3d IndexedMesh::normal_by_face_id(int face_id) const { return its_unnormalized_normal(*m_tm, face_id).cast<double>().normalized(); }

IndexedMesh::hit_result IndexedMesh::query_ray_hit(const Vec3d& s, const Vec3d& dir) const
{
...
std::vector<IndexedMesh::hit_result> IndexedMesh::query_ray_hits(const Vec3d& s, const Vec3d& dir) const
{
    ...
    // [HAZARD H923] DUPLICATE HIT REMOVAL DISABLED (P2/Medium)
    // The erase/unique block below was disabled by a BBS note (STUDIO-2591).
    // Meshes with overlapping faces produce duplicate hits.  Any caller relying
    // on parity-counting (even number = outside, odd = inside) will get wrong
    // inside/outside decisions when duplicates are present.
...
#ifdef SLIC3R_HOLE_RAYCASTER
IndexedMesh::hit_result IndexedMesh::filter_hits(const std::vector<IndexedMesh::hit_result>& object_hits) const
{
...
static bool point_on_edge(const Vec3d& p, const Vec3d& e1, const Vec3d& e2, double eps = 0.05)
{
                 ...
                 const std::vector<unsigned>& pt_indices)
{
    ...
    ccr::for_each(size_t(0), range.size(), [&ret, &mesh, &points, thr, eps, &range](size_t ridx) {
        thr();
        ...
        // [HAZARD H924] O(N×M) NEIGHBOUR SEARCH FOR EDGE/VERTEX HITS (P1/High)
        // When a support point lands on a vertex (ic>=0) or an edge (ia>=0,ib>=0),
        // ALL triangles in the mesh are iterated to find sharing triangles.
        // No adjacency structure is used.  For a mesh with F faces, each
        // edge/vertex-touching point costs O(F).  For large meshes (>100k faces)
        // with many such points this is extremely slow.  A port should pre-build
        // a vertex->face adjacency map to bring this to O(degree).
        ...
        if (ic >= 0) { // The point is right on a vertex of the triangle
            ...
            for (size_t n = 0; n < mesh.indices().size(); ++n) {
                thr();
                Vec3i32 ni = mesh.indices(n);
                if ((ni(X) == ia || ni(Y) == ia || ni(Z) == ia) && (ni(X) == ib || ni(Y) == ib || ni(Z) == ib))
                    neigh.emplace_back(n);
            }
        }
        ...
        std::sort(neighnorms.begin(), neighnorms.end(), [](const Vec3d& v1, const Vec3d& v2) { return v1.sum() < v2.sum(); });

        auto lend = std::unique(neighnorms.begin(), neighnorms.end(), [](const Vec3d& n1, const Vec3d& n2) {
