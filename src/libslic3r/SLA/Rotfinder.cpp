// [INTENT] SLA rotation optimiser implementation. Evaluates three scoring
// strategies over X/Y rotation space to find the minimum-support orientation:
//   1. get_misalginment_score  — maximise alignment with XYZ planes (fewer support
//      points by virtue of flat faces); uses brute-force 2D grid search.
//   2. get_supportedness_score — minimise downward-facing overhangs weighted by area.
//   3. find_min_z_height_rotation — minimise bounding-box Z height.
// All three are exposed via the three public entry-point functions at the
// bottom of the file. Parallelism via TBB execution::reduce / for_each.
//
// [STATE] All scoring functions are pure (mesh + transform → scalar). Only
// shared mutable state lives in RotfinderBoilerplate::status (int, updated by
// statusfn() called inside the TBB lambda — benign race; only used for progress).
//
// [CONCURRENCY] TBB parallel reduce used for face-score accumulation. The
// 'status' integer is updated without a lock (acceptable: it is only a progress
// counter read by the UI thread at a coarse granularity).
//
// [COUPLING] Calls SLAPrintObjectConfig for elevation/pad-around-object decisions.
// Geometry::Transformation used to decompose a Transform3f into Euler angles —
// relies on internal Geometry conventions (roll/pitch/yaw order).
...
std::array<Vec3f, 3> get_triangle_vertices(const TriangleMesh& mesh, size_t faceidx)
{
    ...
    return {mesh.its.vertices[face(0)], mesh.its.vertices[face(1)], mesh.its.vertices[face(2)]};
}

std::array<Vec3f, 3> get_transformed_triangle(const TriangleMesh& mesh, const Transform3f& tr, size_t faceidx)
{
...
template<class T, class AccessFn> T sum_score(AccessFn&& accessfn, size_t facecount, size_t Nthreads)
{
    T      initv     = 0.;
...
// [HAZARD H929] Function name typo: "misalginment" (missing 'n').
// Severity: P3/Low. This is a file-scope anonymous-namespace symbol so the
// typo does not escape to external linkage; however the public-facing entry
// point `find_best_misalignment_rotation` delegates here and the comment
// "Try to guess..." refers to support points — the docstring is wrong too.
// When refactoring: rename to get_misalignment_score.
//
// [HAZARD H940] sum_score<int_fast64_t> accumulates scaled face scores.
// `scaled<int_fast64_t>(score)` maps float mm² values into the integer
// coordinate space (×1e6). For a mesh with 10⁶ faces of 1 mm² each the
// accumulated value approaches 10^18 which is within int_fast64_t range
// (max ~9.2×10^18), but for very high-polygon meshes it can overflow silently
// in release builds. Severity: P2/Medium.
...
double get_misalginment_score(const TriangleMesh& mesh, const Transform3f& tr)
{
    ...
    auto accessfn = [&mesh, &tr](size_t fi) {
        ...
        float score = fc.area * (std::abs(fc.normal.dot(Vec3f::UnitX())) + std::abs(fc.normal.dot(Vec3f::UnitY())) +
...
float find_ground_level(const TriangleMesh& mesh, const Transform3f& tr, size_t threads)
{
    size_t vsize = mesh.its.vertices.size();

    auto minfn = [](float a, float b) { return std::min(a, b); };

    auto accessfn = [&mesh, &tr](size_t vi) { return (tr * mesh.its.vertices[vi]).z(); };
...
float get_supportedness_onfloor_score(const TriangleMesh& mesh, const Transform3f& tr)
{
    if (mesh.its.vertices.empty())
        return std::nan("");
...
std::vector<XYRotation> get_chull_rotations(const TriangleMesh& mesh, size_t max_count)
{
    ...
    if (!max_count)
        max_count = inputs.size();
    std::sort(inputs.begin(), inputs.end(), [](const RotArea& ra, const RotArea& rb) { return ra.area > rb.area; });

    auto ret = reserve_vector<XYRotation>(std::min(max_count, inputs.size()));
    for (const RotArea& ra : inputs)
        ret.emplace_back(ra.rot);

    return ret;
}

// Find the best score from a set of function inputs. Evaluate for every point.
template<size_t N, class Fn, class It, class StopCond> std::array<double, N> find_min_score(Fn&& fn, It from, It to, StopCond&& stopfn)
{
    ...
    execution::for_each(
        ex_tbb, size_t(0), dist,
        [&stopfn, &scores, &fn, &from](size_t i) {
            if (stopfn())
                return;
...
template<unsigned MAX_ITER> struct RotfinderBoilerplate
{
    ...
    // optimization.
    // [HAZARD H941] Only instances[0] is used. If a ModelObject has multiple
    // instances with different scales or mirrors, those are silently ignored.
    // Severity: P2/Medium. Refactor: decide whether to use a representative
    // instance or merge/average all instance transforms.
    static TriangleMesh get_mesh_to_rotate(const ModelObject& mo)
    {
        ...
        Transform3d    trafo_instance = Geometry::assemble_transform(offset, rotation, mi->get_scaling_factor(), mi->get_mirror());
    ...
    RotfinderBoilerplate(const ModelObject& mo, const RotOptimizeParams& p)
        : mesh{get_mesh_to_rotate(mo)}, params{p}, max_tries(p.accuracy() * MAX_TRIES)
    {}
...
Vec2d find_best_misalignment_rotation(const ModelObject& mo, const RotOptimizeParams& params)
{
    ...
    opt::Optimizer<opt::AlgBruteForce> solver(opt::StopCriteria{}.max_iterations(bp.max_tries).stop_condition([&bp] {
        return bp.stopcond();
    }),
                                              gridsize);
...
Vec2d find_least_supports_rotation(const ModelObject& mo, const RotOptimizeParams& params)
{
    ...
    if (is_on_floor(pocfg)) {
        ...
        rot = find_min_score<2>(objfn, inputs.begin(), inputs.end(), [&bp] { return bp.stopcond(); });
...
Vec2d find_min_z_height_rotation(const ModelObject& mo, const RotOptimizeParams& params)
{
    ...
    XYRotation rot = find_min_score<2>(objfn, inputs.begin(), inputs.end(), [&bp] { return bp.stopcond(); });
