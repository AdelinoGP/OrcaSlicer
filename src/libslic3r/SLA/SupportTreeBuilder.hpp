// [INTENT] SupportTreeBuilder accumulates support-tree geometry (heads,
// pillars, junctions, bridges, crossbridges, diffbridges, pedestals, anchors,
// pad). Implements SupportTree's virtual interface. Each add_*() method is
// thread-safe via SpinningMutex; merged_mesh() caches the tessellated result
// in m_meshcache and is invalidated whenever a part is added.
//
// [STATE]
//   m_heads / m_pillars / m_junctions / m_bridges / m_crossbridges /
//   m_diffbridges / m_pedestals / m_anchors — append-only collections.
//   m_head_indices — maps external head-ID → m_heads[] index (support-point IDs
//     can be sparse; this extra indirection handles gaps).
//   m_meshcache / m_meshcache_valid — lazy merged mesh, protected by m_mutex.
//   m_model_height — updated in merged_mesh(), holds Z extent of support mesh.
//
// [CONCURRENCY] SpinningMutex (ccr::SpinningMutex) guards all add_*() calls
// and mesh cache access. merged_mesh() acquires no lock (it is const and
// checks m_meshcache_valid without a lock first — benign double-check because
// worst case is rebuilding cache twice). Real data races are on the cache flag
// itself; practically safe because merge_and_cleanup() is called single-
// threaded after all add_*() calls.
//
// [HAZARD H932] add_anchor() sets:
//   m_anchors.back().id = long(m_junctions.size() - 1)
// This assigns the JUNCTION count as the anchor ID, not the anchor count.
// The anchor ID should be `long(m_anchors.size() - 1)`. Severity: P1/High —
// anchor IDs will be wrong whenever junctions and anchors are not 1:1.
//
// [HAZARD H933] Move and copy constructors omit m_junctions, m_diffbridges,
// m_pedestals, m_anchors. These four collections are silently dropped when a
// SupportTreeBuilder is moved or copied. Severity: P1/High.
//
// [HAZARD H934] merge_and_cleanup() clears only m_heads, m_head_indices,
// m_pillars, m_junctions, m_bridges — it omits m_crossbridges, m_diffbridges,
// m_pedestals, m_anchors. Those collections remain in memory after cleanup,
// leaking potentially large vectors. Severity: P2/Medium.
...
template<class Vec> double distance(const Vec& p) { return std::sqrt(p.transpose() * p); }
...
struct Head : public SupportTreeNode
{
    ...
    inline double real_width() const { return 2 * r_pin_mm + width_mm + 2 * r_back_mm; }

    inline double fullwidth() const { return real_width() - penetration_mm; }

    inline Vec3d junction_point() const { return pos + (fullwidth() - r_back_mm) * dir; }
...
struct Pillar : public SupportTreeNode
{
    ...
    Pillar(const Vec3d& endp, double h, double radius = 1.) : height{h}, r(radius), endpt(endp), starts_from_head(false) {}

    Vec3d startpoint() const { return {endpt.x(), endpt.y(), endpt.z() + height}; }
...
struct Pedestal : public SupportTreeNode
{
    ...
    Pedestal(const Vec3d& p, double h, double rbottom, double rtop) : pos{p}, height{h}, r_bottom{rbottom}, r_top{rtop} {}
};

// This is the thing that anchors a pillar or bridge to the model body.
// It is actually a reverse pinhead.
struct Anchor : public Head
{
    using Head::Head;
};
...
struct Bridge : public SupportTreeNode
{
    ...
    Bridge(const Vec3d& j1, const Vec3d& j2, double r_mm = 0.8) : r{r_mm}, startp{j1}, endp{j2} {}
...
struct DiffBridge : public Bridge
{
    ...
    DiffBridge(const Vec3d& p_s, const Vec3d& p_e, double r_s, double r_e) : Bridge{p_s, p_e, r_s}, end_r{r_e} {}
};
...
class SupportTreeBuilder : public SupportTree
{
    ...
    template<class BridgeT, class... Args> const BridgeT& _add_bridge(std::vector<BridgeT>& br, Args&&... args)
    {
...
public:
    ...
    template<class... Args> const Anchor& add_anchor(Args&&... args)
    {
        ...
        // [HAZARD H932] BUG: ID assigned from m_junctions.size()-1, not
        // m_anchors.size()-1. Anchor IDs will be wrong when junction and
        // anchor counts diverge. Severity: P1/High.
        m_anchors.back().id = long(m_junctions.size() - 1);
        m_meshcache_valid   = false;
    ...
    const Bridge& add_bridge(const Vec3d& s, const Vec3d& e, double r) { return _add_bridge(m_bridges, s, e, r); }
    ...
    {
        std::lock_guard<Mutex> lk(m_mutex);
        assert(id >= 0 && size_t(id) < m_pillars.size() && size_t(id) < std::numeric_limits<size_t>::max());
    ...
    const indexed_triangle_set& add_pad(const ExPolygons& modelbase, const PadConfig& pcfg) override;

    void remove_pad() override { m_pad = Pad(); }

    virtual const indexed_triangle_set& retrieve_mesh(MeshType meshtype = MeshType::Support) const override;
};
