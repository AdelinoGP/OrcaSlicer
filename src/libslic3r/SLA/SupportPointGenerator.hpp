// [INTENT] Auto-support-point generator for SLA prints. Given a stack of
// ExPolygon slices and their Z heights, it:
//   1. Builds a layer-island graph (Structure/MyLayer) with overlap links.
//   2. Propagates "support force" downward through the island graph.
//   3. For each island with a support deficit, samples Poisson-disk points.
//   4. Projects the 2D sample points onto the 3D mesh surface.
// Entry point: SupportPointGenerator constructor (calls execute() → process()).
//
// [STATE]
//   m_output         — accumulated output SupportPoints, appended-only.
//   m_rng            — mt19937 PRNG. Seeded from std::random_device unless
//                      test code calls seed() directly.
//   m_emesh          — non-owning reference to IndexedMesh (for projection).
//   m_config         — algorithm parameters (density, minimal_distance, ...).
//
// [CONCURRENCY] process()/make_layers() use ccr_par::for_each (TBB parallel)
// for layer initialisation and overlap linking. The output vector m_output is
// populated only in add_support_points() which is called single-threaded from
// the per-layer loop in process(). No lock needed on m_output.
//
// [HAZARD H944] pixel_area in make_layers() is hardcoded to pow(0.047f,2.f)
// (Anycubic Photon pixel size). The FIXME comment requests fetching from
// printer config but this is never done. Result: minimum island area filter
// is wrong for any printer with different pixel dimensions. Severity: P2/Medium.
//
// [HAZARD H945] Island overlap linking is O(N²) per adjacent layer pair
// (explicit FIXME comment in source). Very slow for layers with many tiny
// islands. Severity: P2/Medium.
//
// [HAZARD H946] sample_expolygon_boundary() `rng` parameter is declared but
// unused (`/*rng*/` comment in function signature). The function does not use
// random sampling — it uses equally-spaced points. The parameter signature is
// misleading; callers passing an rng get no effect. Severity: P3/Low.
...
public:
    struct Config
    {
        ...
        // Originally calibrated to 7.7f, reduced density by Tamas to 70% which is 11.1 (7.7 / 0.7) to adjust for new algorithm changes in
        // tm_suppt_gen_improve
        inline float support_force() const
        {
            return 11.1f / density_relative;
        }                                                  // a force one point can support       (arbitrary force unit)
        inline float tear_pressure() const { return 1.f; } // pressure that the display exerts    (the force unit per mm2)
    };

    SupportPointGenerator(const IndexedMesh&             emesh,
    ...
    struct PointGrid3D
    {
        ...
        {
            return Vec3i32(int(floor(pos.x() / cell_size.x())), int(floor(pos.y() / cell_size.y())), int(floor(pos.z() / cell_size.z())));
        }
    ...
    void execute(const std::vector<ExPolygons>& slices, const std::vector<float>& heights);
...
private:
    void uniformly_cover(
        const ExPolygons& islands, Structure& structure, float deficit, PointGrid3D& grid3d, IslandCoverageFlags flags = icfNone);
