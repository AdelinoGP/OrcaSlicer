// [INTENT] Implements SupportTreeBuilder constructors, operator=, add_pillar_base,
// merged_mesh, full_height, merge_and_cleanup, retrieve_mesh, and Pad constructor.
// This is the hub that tessellates and caches the complete support mesh.
//
// [HAZARD H933] Move and copy constructors/operator= OMIT four collections:
//   m_junctions, m_diffbridges, m_pedestals, m_anchors.
// These are silently dropped on move/copy. Severity: P1/High.
// When refactoring: add all four to each constructor initialiser list and each
// operator= body, or use the default compiler-generated copy/move (= default).
//
// [HAZARD H934] merge_and_cleanup() clears only 5 of 9 collections:
//   m_heads, m_head_indices, m_pillars, m_junctions, m_bridges.
// The following are NOT cleared: m_crossbridges, m_diffbridges, m_pedestals,
// m_anchors — they remain allocated after cleanup, leaking memory.
// Severity: P2/Medium. When refactoring add explicit clear for the remaining
// four collections.
#define NOMINMAX
...
Head::Head(double r_big_mm, double r_small_mm, double length_mm, double penetration, const Vec3d& direction, const Vec3d& offset)
    : dir(direction), pos(offset), r_back_mm(r_big_mm), r_pin_mm(r_small_mm), width_mm(length_mm), penetration_mm(penetration)
{}
...
Pad::Pad(const indexed_triangle_set& support_mesh,
         ...
         ThrowOnCancel               thr)
    : cfg(pcfg), zlevel(ground_level + pcfg.full_height() - pcfg.required_elevation())
{
    thr();
    ...
    for (auto& p : tmesh.vertices)
        p += offs;

    its_merge_vertices(tmesh);
}

const indexed_triangle_set& SupportTreeBuilder::add_pad(const ExPolygons& modelbase, const PadConfig& cfg)
{
...
void SupportTreeBuilder::add_pillar_base(long pid, double baseheight, double radius)
{
    ...
    m_pedestals.emplace_back(pll.endpt, std::min(baseheight, pll.height), std::max(radius, pll.r), pll.r);
...
const indexed_triangle_set& SupportTreeBuilder::merged_mesh(size_t steps) const
{
    ...
    for (auto& head : m_heads) {
        if (ctl().stopcondition())
            break;
        if (head.is_valid())
            its_merge(merged, get_mesh(head, steps));
    }
