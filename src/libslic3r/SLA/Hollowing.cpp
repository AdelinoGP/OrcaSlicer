// [INTENT] Implementation of SLA hollowing: converts solid mesh to a shelled
// interior using OpenVDB signed-distance field, generates drain holes, removes
// internal triangles, and cuts drain-hole cross-sections into slice layers.
// [COUPLING] Heavy dependencies: OpenVDB (voxelization), QuadricEdgeCollapse
// (mesh simplification), TriangleMeshSlicer (hole slice cutout), Clipper.
#include <functional>
#include <optional>
...
// [INTENT] Interior: all state produced by the OpenVDB hollowing pass.
// [STATE] gridptr: signed-distance FloatGrid at voxel_scale.
// [HAZARD H925] NON-THREAD-SAFE MUTABLE ACCESSOR (P1/High)
// Interior::accessor is mutable.  reset_accessor() explicitly says "Not a
// thread safe call!".  Concurrent get_distance_raw() calls from multiple
// threads on the same Interior produce a data race on the accessor.
// [MEMORY] gridptr is a shared_ptr (OpenVDB).  Copying Interior shares the
// grid reference — mutations via one copy affect all others.
struct Interior
{
...
indexed_triangle_set& get_mesh(Interior& interior) { return interior.mesh; }

const indexed_triangle_set& get_mesh(const Interior& interior) { return interior.mesh; }

static InteriorPtr generate_interior_verbose(
    const TriangleMesh& mesh, const JobController& ctl, double min_thickness, double voxel_scale, double closing_dist)
{
...
InteriorPtr generate_interior(const TriangleMesh& mesh, const HollowingConfig& hc, const JobController& ctl)
{
    ...
    InteriorPtr interior = generate_interior_verbose(mesh, ctl, hc.min_thickness, voxel_scale, hc.closing_distance);
...
{
    return (pos == sp.pos) && (normal == sp.normal) && is_approx(radius, sp.radius) && is_approx(height, sp.height);
}
...
bool DrainHole::get_intersections(const Vec3f& s, const Vec3f& dir, std::array<std::pair<float, Vec3d>, 2>& out) const
{
                    ...
                    std::function<void(void)> thr)
{
    ...
    if (obj_slices.size() != hole_slices.size())
        BOOST_LOG_TRIVIAL(warning) << "Sliced object and drain-holes layer count does not match!";
...
{
    if (mesh.empty() || interior.mesh.empty())
        return;
...
static double get_distance_raw(const Vec3f& p, const Interior& interior)
{
    ...
    if (!interior.accessor)
        interior.reset_accessor();

    auto v       = (p * interior.voxel_scale).cast<double>();
    auto grididx = interior.gridptr->transform().worldToIndexCellCentered({v.x(), v.y(), v.z()});
...
static double get_distance(const TriangleBubble& b, const Interior& interior)
{
    ...
    return (D > 0. && R >= interior.nb_out) || (D < 0. && R >= interior.nb_in) || ((D - R) < 0. && 2 * R > interior.thickness) ?
               std::nan("") :
...
template<class Fn> void divide_triangle(const DivFace& face, Fn&& visitor)
{
    std::array<Vec3f, 3> edges = {(face.verts[0] - face.verts[1]), (face.verts[1] - face.verts[2]), (face.verts[2] - face.verts[0])};

    std::array<size_t, 3> edgeidx = {0, 1, 2};

    std::sort(edgeidx.begin(), edgeidx.end(), [&edges](size_t e1, size_t e2) { return edges[e1].squaredNorm() > edges[e2].squaredNorm(); });
...
void remove_inside_triangles(TriangleMesh& mesh, const Interior& interior, const std::vector<bool>& exclude_mask)
{
    ...
    auto is_excluded      = [&exclude_mask, use_exclude_mask](size_t face_id) { return use_exclude_mask && exclude_mask[face_id]; };
    ...
    struct MeshMods
    {
        ...
        struct
        {
            ...
            const std::array<Vec3f, 3>& operator[](size_t idx) const { return data[idx]; }
        ...
        size_t to_remove_cnt() const { return std::accumulate(to_remove.begin(), to_remove.end(), size_t(0)); }
    ...
    auto new_faces = reserve_vector<Vec3i32>(faces.size() + mesh_mods.new_triangles.size());
    ...
    BOOST_LOG_TRIVIAL(info) << "Trimming: " << mesh_mods.to_remove_cnt() << " triangles removed";
    BOOST_LOG_TRIVIAL(info) << "Trimming: " << mesh_mods.new_triangles.size() << " triangles added";
