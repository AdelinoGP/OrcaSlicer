// [INTENT] Generic cubic / bicubic interpolation kernels (linear hat, Catmull-Rom,
// B-spline). Used in the SLA bed-levelling tilt-correction code to interpolate
// a 2D correction surface at arbitrary (x,y) sample positions.
//
// [HAZARD H942] Line ~152 in cubic_interpolate: the boundary-extension lambda
//   auto f = [&F, w](x) { ... }
// is missing the parameter type declaration `int x`. This is ill-formed
// standard C++. It compiles only under some MSVC/Clang extensions that accept
// a bare identifier as a parameter name in a lambda. Severity: P1/High —
// compilation may fail on strict-C++17 platforms.
// When refactoring write: [&F, w](int x) { ... }
//
// [HAZARD H943] Line ~176 in bicubic_interpolate: the boundary-extension lambda
//   auto f = [&f, w, h](int x, int y) { ... }
// captures `f` by reference in its own initialiser — this is self-referential
// lambda capture which is undefined behaviour (UB) in C++17 (the variable `f`
// is not yet constructed at the point its address is taken). On most platforms
// the lambda never calls itself recursively so it happens to work, but it is
// technically UB. Severity: P1/High.
// When refactoring use a named helper function or std::function + recursive assign.
#ifndef BICUBIC_HPP
#define BICUBIC_HPP
...
template<typename KERNEL> struct CubicKernel
{
...
static float cubic_interpolate(const Eigen::ArrayBase<Derived>& F, const typename KERNEL::FloatType pt, const typename KERNEL::FloatType dx)
{
    ...
    // [HAZARD H942] Lambda missing parameter type: `(x)` should be `(int x)`.
    // Ill-formed C++17; compiles only with non-conforming extensions.
    // Severity: P1/High.
    auto f = [&F, w](x) { return F[BicubicInternal::clamp(x, 0, w - 1)]; } return KERNEL::interpolate(f(ix - 1), f(ix), f(ix + 1),
                                                                                                      f(ix + 2), s);
}
...
static float bicubic_interpolate(const Eigen::MatrixBase<Derived>&                                        F,
                                 ...
                                 const typename KERNEL::FloatType                                         dx)
{
    typedef typename KERNEL::FloatType T;
    const int                          w  = F.cols();
    const int                          h  = F.rows();
    ...
    // Transition region. Extend with a constant function.
    // [HAZARD H943] Self-referential lambda capture: `[&f, ...]` captures `f`
    // by reference while `f` is still being constructed — undefined behaviour
    // per C++17. The lambda never recurses here so it works by accident.
    // Severity: P1/High. Refactor: replace with a regular static helper
    // function that closes over F, w, h by reference.
    auto f =
        [&f, w, h](int x, int y) {
