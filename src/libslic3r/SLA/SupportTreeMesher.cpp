// [INTENT] Implements the four primitive triangle-mesh generators declared in
// SupportTreeMesher.hpp: sphere(), cylinder(), pinhead(), halfcone().
// These produce closed, watertight indexed_triangle_set meshes used as
// building blocks for the SLA support tree.
//
// [STATE] All functions are pure (no shared mutable state). Each returns a
// new indexed_triangle_set by value.
//
// [MEMORY] O(steps) vertices per primitive. No dynamic allocation beyond
// the vectors inside indexed_triangle_set. All temporaries are stack-local.
//
// [COUPLING] No dependencies on SupportTree state; can be unit-tested in
// isolation given only the primitive parameter structs.
...
indexed_triangle_set sphere(double rho, Portion portion, double fa)
{
    ...
    if (sbegin == 0)
        vertices.emplace_back(Vec3f(0.f, 0.f, float(-rho + increment * sbegin * 2. * rho)));
    ...
    for (size_t i = 0; i < ring.size(); i++) {
        ...
        if (sbegin == 0)
            (i == 0) ? facets.emplace_back(coord_t(ring.size()), 0, 1) : facets.emplace_back(id - 1, 0, id);
        ++id;
    }
...
indexed_triangle_set cylinder(double r, double h, size_t ssteps, const Vec3d& sp)
{
    ...
    points.emplace_back(jp.cast<float>());
    int ci = int(points.size() - 1);
    for (int i = 0; i < steps - 1; ++i)
        indices.emplace_back(i + offs + 1, i + offs, ci);

    indices.emplace_back(offs, steps + offs - 1, ci);

    points.emplace_back(endp.cast<float>());
    ci = int(points.size() - 1);
    for (int i = 0; i < steps - 1; ++i)
...
indexed_triangle_set pinhead(double r_pin, double r_back, double length, size_t steps)
{
    ...
    for (size_t idx1 = s1.vertices.size() - steps, idx2 = s1.vertices.size(); idx1 < s1.vertices.size() - 1; idx1++, idx2++) {
