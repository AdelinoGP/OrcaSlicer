...
SupportPointGenerator::SupportPointGenerator(const sla::IndexedMesh&        emesh,
...
SupportPointGenerator::SupportPointGenerator(const IndexedMesh&                   emesh,
...
void SupportPointGenerator::execute(const std::vector<ExPolygons>& slices, const std::vector<float>& heights)
{
...
void SupportPointGenerator::project_onto_mesh(std::vector<sla::SupportPoint>& points) const
{
    ...
    ccr_par::for_each(
        size_t(0), points.size(),
        [this, &points](size_t idx) {
            if ((idx % 16) == 0)
...
static std::vector<SupportPointGenerator::MyLayer> make_layers(const std::vector<ExPolygons>& slices,
                                                               const std::vector<float>&      heights,
                                                               std::function<void(void)>      throw_on_cancel)
{
    ...
    // [HAZARD H944] pixel_area hardcoded to pow(0.047f, 2.f) — the Anycubic
    // Photon pixel pitch. The FIXME comment requests computing from printer
    // config but this is never implemented. All printers with different pixel
    // dimensions will use the wrong minimum island area threshold.
    // Severity: P2/Medium. Refactor: pass printer config or pixel_area as a
    // parameter to make_layers().
    // FIXME: calculate actual pixel area from printer config:
    // const float pixel_area = pow(wxGetApp().preset_bundle->project_config.option<ConfigOptionFloat>("display_width") /
    // wxGetApp().preset_bundle->project_config.option<ConfigOptionInt>("display_pixels_x"), 2.f); //
    const float pixel_area = pow(0.047f, 2.f);

    ccr_par::for_each(
        size_t(0), layers.size(),
        [&layers, &slices, &heights, pixel_area, throw_on_cancel](size_t layer_id) {
            ...
            for (const ExPolygon& island : islands) {
                ...
                if (area >= pixel_area)
                    ...
                    layer.islands.emplace_back(layer, island, get_extents(island.contour), unscaled<float>(island.contour.centroid()), area,
                                               height);
            }
        },
        32 /*gransize*/);
    ...
    ccr_par::for_each(
        ...
        [&layers, &heights, throw_on_cancel](size_t layer_id) {
            ...
            // [HAZARD H945] O(N²) island overlap linking — explicit FIXME.
            // For layers with many small islands this is prohibitively slow.
            // Severity: P2/Medium. Refactor: spatial index on bbox to skip
            // non-overlapping island pairs in O(log N) per query.
    ...
    for (unsigned int layer_id = 0; layer_id < layers.size(); ++layer_id) {
        ...
        for (Structure& top : layer_top->islands)
            for (Structure::Link& bottom_link : top.islands_below) {
                ...
                //                  bottom.supports_force *= std::min(1.f, 1.f - std::min(1.f, (1600.f * layer_height) * centroids_dist *
                //                  centroids_dist / bottom.area));
                float& support_force = support_force_bottom[&bottom - layer_bottom->islands.data()];
                // FIXME this condition does not reflect a bifurcation into a one large island and one tiny island well, it incorrectly resets
...
std::vector<Vec2f> sample_expolygon(const ExPolygon& expoly, float samples_per_mm2, std::mt19937& rng)
{
    ...
    if (!triangles.empty()) {
        ...
        for (size_t i = 0; i < num_samples; ++i) {
            ...
            size_t idx_triangle = std::min<size_t>(std::upper_bound(areas.begin(), areas.end(), (float) r) - areas.begin(),
                                                   areas.size() - 1) *
                                  3;
...
// [HAZARD H946] `rng` parameter is declared but unused (commented out as
// `/*rng*/`). This function samples equally-spaced polygon edge points —
// fully deterministic. The misleading rng parameter implies randomness that
// doesn't exist. Severity: P3/Low. Refactor: remove the rng parameter.
                                                           ...
                                                           REFUSE_FUNCTION           refuse_function)
{
    ...
    struct CellIDHash
    {
        std::size_t operator()(const Vec2i32& cell_id) const { return std::hash<int>()(cell_id.x()) ^ std::hash<int>()(cell_id.y() * 593); }
    };
...
void SupportPointGenerator::uniformly_cover(
    const ExPolygons& islands, Structure& structure, float deficit, PointGrid3D& grid3d, IslandCoverageFlags flags)
{
    ...
    // FIXME share the random generator. The random generator may be not so cheap to initialize, also we don't want the random generator to
    // be restarted for each polygon.

    std::vector<Vec2f> raw_samples = flags & icfWithBoundary ?
                                         sample_expolygon_with_boundary(islands, samples_per_mm2, 5.f / poisson_radius, m_rng) :
                                         sample_expolygon(islands, samples_per_mm2, m_rng);

    std::vector<Vec2f> poisson_samples;
    for (size_t iter = 0; iter < 4; ++iter) {
        poisson_samples = poisson_disk_from_samples(raw_samples, poisson_radius, [&structure, &grid3d, min_spacing](const Vec2f& pos) {
...
void remove_bottom_points(std::vector<SupportPoint>& pts, float lvl)
{
    ...
    auto endit = std::remove_if(pts.begin(), pts.end(), [lvl](const sla::SupportPoint& sp) { return sp.pos.z() <= lvl; });
...
void SupportPointGenerator::output_structures(const std::vector<Structure>& structures)
{
    for (unsigned int i = 0; i < structures.size(); ++i) {
        std::stringstream ss;
        ss << structures[i].unique_id.count() << "_" << std::setw(10) << std::setfill('0') << 1000 + (int) structures[i].height / 1000
           << ".png";
