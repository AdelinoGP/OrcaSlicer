// [INTENT] Implements the SupportTreeBuildsteps state-machine steps that
// populate the SupportTreeBuilder with actual geometry. Key steps:
//   filter()           — discard bad support points, assign normals.
//   add_pinheads()     — [HAZARD H938] EMPTY STUB — does nothing (see below).
//   classify()         — split heads into ground-facing vs model-facing.
//   routing_to_ground()— create pillars / bridged pillars for ground-facing.
//   routing_to_model() — create anchor backs for model-facing heads.
//   interconnect_pillars() — add cross-bridges between nearby pillars.
//   merge_result()     — trigger mesh tessellation and cache.
//
// [HAZARD H938] add_pinheads() is a COMPLETE EMPTY STUB (body: `{}`).
// The function is supposed to construct Head objects from support points that
// passed the filter step, but the entire implementation is absent.
// Severity: P0/Critical — without this step the builder has no heads, so
// classify/routing/meshing produce empty geometry. In practice filter()
// populates m_iheads and the classify step reads them via m_builder.head(),
// which implies the Head objects are created somewhere else or were added
// in a prior design and this step became a vestige. Any refactoring must
// re-examine where Head construction actually happens vs where it was intended.
...
{
    return StopCriteria{}.rel_score_diff(cfg.optimizer_rel_score_diff).max_iterations(cfg.optimizer_max_iterations);
}

template<class C, class Hit = IndexedMesh::hit_result> static Hit min_hit(const C& hits)
{
    auto mit = std::min_element(hits.begin(), hits.end(), [](const Hit& h1, const Hit& h2) { return h1.distance() < h2.distance(); });

    return *mit;
}

SupportTreeBuildsteps::SupportTreeBuildsteps(SupportTreeBuilder& builder, const SupportableMesh& sm)
    : m_cfg(sm.cfg)
    , m_mesh(sm.emesh)
...
bool SupportTreeBuildsteps::execute(SupportTreeBuilder& builder, const SupportableMesh& sm)
{
    ...
    if (sm.cfg.ground_facing_only) {
        program[ROUTING_NONGROUND] = []() { BOOST_LOG_TRIVIAL(info) << "Skipping model-facing supports as requested."; };
    }

    // Let's define a simple automaton that will run our program.
    auto progress = [&builder, &pc]() {
        static const std::array<std::string, NUM_STEPS> stepstr{"Starting",
                                                                "Filtering",
                                                                "Generate pinheads",
                                                                "Classification",
                                                                "Routing to ground",
        ...
        static const std::array<unsigned, NUM_STEPS> stepstate{0, 10, 30, 50, 60, 70, 80, 99, 100, 0};
...
IndexedMesh::hit_result SupportTreeBuildsteps::pinhead_mesh_intersect(
    const Vec3d& s, const Vec3d& dir, double r_pin, double r_back, double width, double sd)
{
...
IndexedMesh::hit_result SupportTreeBuildsteps::bridge_mesh_intersect(const Vec3d& src, const Vec3d& dir, double r, double sd)
{
    static const size_t SAMPLES = 8;
    ...
    ccr::for_each(size_t(0), hits.size(), [this, r, src, /*ins_check,*/ &ring, dir, sd, &hits](size_t i) {
        Hit& hit = hits[i];
...
bool SupportTreeBuildsteps::interconnect(const Pillar& pillar, const Pillar& nextpillar)
{
    ...
    if (slower(Z) - elower(Z) < 0)
        return false;
    if (supper(Z) - eupper(Z) < 0)
        return false;

    double pillar_dist     = distance(Vec2d{slower(X), slower(Y)}, Vec2d{supper(X), supper(Y)});
    double bridge_distance = pillar_dist / std::cos(-m_cfg.bridge_slope);
    double zstep           = pillar_dist * std::tan(-m_cfg.bridge_slope);

    if (pillar_dist < 2 * m_cfg.head_back_radius_mm || pillar_dist > m_cfg.max_pillar_link_distance_mm)
        return false;
    ...
    bool docrosses = pcm == PillarConnectionMode::cross || (pcm == PillarConnectionMode::dynamic && pillar_dist > 2 * m_cfg.base_radius_mm);
    ...
    while (ej(Z) >= eupper(Z) /*endz*/) {
        ...
        if (docrosses) {
            ...
            if (sjback(Z) <= slower(Z) && ejback(Z) >= eupper(Z) &&
                bridge_mesh_distance(sjback, dirv(sjback, ejback), pillar.r) >= bridge_distance) {
...
{
    auto nearpillar = [this, nearpillar_id]() -> const Pillar& { return m_builder.pillar(nearpillar_id); };
    ...
    if (t < distance(bridgestart, bridgeend))
        return false;
...
bool SupportTreeBuildsteps::create_ground_pillar(const Vec3d& hjp, const Vec3d& sourcedir, double radius, long head_id)
{
    ...
    auto eval_limits = [this, &radius, &can_add_base, &gndlvl, &gap_dist, &jp_gnd](bool base_en = true) {
    ...
    if (radius < m_cfg.head_back_radius_mm && jp.z() - gndlvl > 20 * radius) {
        std::optional<DiffBridge> diffbr = search_widening_path(jp, dir, radius, m_cfg.head_back_radius_mm);

        if (diffbr && diffbr->endp.z() > jp_gnd) {
            auto& br = m_builder.add_diffbridge(*diffbr);
            if (head_id >= 0)
                m_builder.head(head_id).bridge_id = br.id;
            endp   = diffbr->endp;
            radius = diffbr->end_r;
    ...
    if (m_cfg.object_elevation_mm < EPSILON) {
        ...
        while (((dlast = std::sqrt(m_mesh.squared_distance(to_floor(nexp)))) < gap_dist ||
                !std::isinf(bridge_mesh_distance(nexp, DOWN, radius))) &&
               t < tmax) {
            t += radius;
            nexp = endp + t * d;
        }
    ...
    pillar_id = head_id >= 0 && !non_head ? m_builder.add_pillar(head_id, h) : m_builder.add_pillar(gp, h, radius);

    if (can_add_base)
        add_pillar_base(pillar_id);

    if (pillar_id >= 0) // Save the pillar endpoint in the spatial index
        m_pillar_index.guarded_insert(m_builder.pillar(pillar_id).endpt, unsigned(pillar_id));

    return true;
}

std::optional<DiffBridge> SupportTreeBuildsteps::search_widening_path(const Vec3d& jp, const Vec3d& dir, double radius, double new_radius)
{
    ...
    auto oresult = solver.to_max().optimize(
        [this, jp, radius, new_radius](const opt::Input<3>& input) {
            ...
            double ret  = pinhead_mesh_intersect(jp, d, radius, new_radius, t).distance();
        ...
        bounds({{PI - m_cfg.bridge_slope, PI}, // Must not exceed the slope limit
                {-PI, PI},                     // azimuth can be a full search
                {radius + m_cfg.head_back_radius_mm, fallback_ratio * m_cfg.max_bridge_length_mm}}));
...
void SupportTreeBuildsteps::filter()
{
    ...
    auto nmls = sla::normals(m_points, m_mesh, m_cfg.head_front_radius_mm, m_thr, filtered_indices);
    ...
    heads.reserve(m_support_pts.size());
    for (const SupportPoint& sp : m_support_pts) {
        m_thr();
        heads.emplace_back(std::nan(""), sp.head_front_radius, 0., m_cfg.head_penetration_mm,
                           Vec3d::Zero(),        // dir
    ...
    filterfn = [this, &nmls, &heads, &filterfn](unsigned fidx, size_t i, double back_r) {
        ...
        double w = lmin + 2 * back_r + 2 * m_cfg.head_front_radius_mm - m_cfg.head_penetration_mm;
        ...
        IndexedMesh::hit_result t = pinhead_mesh_intersect(hp, nn, pin_r, back_r, w);
        ...
        if (t.distance() < w) {
            ...
            auto oresult = solver.to_max().optimize(
                [this, pin_r, back_r, hp](const opt::Input<3>& input) {
                    ...
                    return pinhead_mesh_intersect(hp, dir, pin_r, back_r, l).distance();
                },
    ...
    ccr::for_each(size_t(0), filtered_indices.size(),
                  [this, &filterfn, &filtered_indices](size_t i) { filterfn(filtered_indices[i], i, m_cfg.head_back_radius_mm); });
...
// [HAZARD H938] COMPLETE EMPTY STUB — entire function body is absent.
// Severity: P0/Critical. Pinheads are supposed to be constructed here from the
// filtered support points in m_iheads. Without this step the builder has no
// Head objects, making classify() and all routing steps operate on empty data.
// This is either dead design (pinhead creation migrated to filter()) or a
// severe regression. Refactoring note: audit filter() to see if Head creation
// moved there; if so, document and remove this stub.
...
void SupportTreeBuildsteps::classify()
{
    ...
    for (unsigned i : m_iheads) {
        ...
        if (std::isinf(hit.distance()))
            ...
            m_iheads_onmodel.emplace_back(i);
    ...
    auto predicate = [this](const PointIndexEl& e1, const PointIndexEl& e2) {
        double d2d = distance(to_2d(e1.first), to_2d(e2.first));
        double d3d = distance(e1.first, e2.first);
        return d2d < 2 * m_cfg.base_radius_mm && d3d < m_cfg.max_bridge_length_mm;
    };

    m_pillar_clusters = cluster(ground_head_indices, pointfn, predicate, m_cfg.max_bridges_on_pillar);
}
...
void SupportTreeBuildsteps::routing_to_ground()
{
    ...
    for (auto& cl : m_pillar_clusters) {
        ...
        if (!create_ground_pillar(h.junction_point(), h.dir, h.r_back_mm, h.id)) {
            BOOST_LOG_TRIVIAL(warning) << "Pillar cannot be created for support point id: " << hid;
    ...
    for (auto cl : m_pillar_clusters) {
        ...
        if (!q.empty()) {
            ...
            for (auto c : cl) {
                ...
                if (!connect_to_nearpillar(sidehead, centerpillarID) && !search_pillar_and_connect(sidehead)) {
...
{
    if (connect_to_ground(head, head.dir))
        return true;
...
bool SupportTreeBuildsteps::connect_to_model_body(Head& head)
{
    ...
    if (it == m_head_to_ground_scans.end())
        return false;
    ...
    Vec3d  hitp    = std::abs(hitdiff) < 2 * head.r_back_mm ? center_hit.position() : hit.position();
    ...
    m_builder.add_anchor(head.r_back_mm, head.r_pin_mm, w, m_cfg.head_penetration_mm, taildir, hitp);
    ...
    while (nearest_id < 0 && !spindex.empty()) {
        ...
        if (nearest_id >= 0) {
            if (size_t(nearest_id) < m_builder.pillarcount()) {
                if (!connect_to_nearpillar(source, nearest_id) || m_builder.pillar(nearest_id).r < source.r_back_mm) {
...
void SupportTreeBuildsteps::routing_to_model()
{
    ...
    ccr::for_each(m_iheads_onmodel.begin(), m_iheads_onmodel.end(), [this](const unsigned idx) {
        m_thr();

        auto& head = m_builder.head(idx);

        // Search nearby pillar
        if (search_pillar_and_connect(head)) {
            return;
        }

        // Cannot connect to nearby pillar. We will try to search for
        // a route to the ground.
        if (connect_to_ground(head)) {
            return;
        }

        // No route to the ground, so connect to the model body as a last resort
        if (connect_to_model_body(head)) {
            return;
        }

        // We have failed to route this head.
        BOOST_LOG_TRIVIAL(warning) << "Failed to route model facing support point. ID: " << idx;
...
void SupportTreeBuildsteps::interconnect_pillars()
{
    ...
    auto cascadefn = [this, d, &pairs, min_height_ratio, H1](const PointIndexEl& el) {
        ...
        if (pillar.links >= neighbors)
            return;

        double max_d = d * pillar.r / m_cfg.head_back_radius_mm;
        // Query all remaining points within reach
        auto qres = m_pillar_index.query([qp, max_d](const PointIndexEl& e) { return distance(e.first, qp) < max_d; });

        // sort the result by distance (have to check if this is needed)
        std::sort(qres.begin(), qres.end(),
                  [qp](const PointIndexEl& e1, const PointIndexEl& e2) { return distance(e1.first, qp) < distance(e2.first, qp); });

        for (auto& re : qres) { // process the queried neighbors

            if (re.second == el.second)
                continue; // Skip self
            ...
            if (interconnect(pillar, neighborpillar)) {
                ...
                if (pillar.height < H1 || neighborpillar.height / pillar.height > min_height_ratio)
                    m_builder.increment_links(pillar);

                if (neighborpillar.height < H1 || pillar.height / neighborpillar.height > min_height_ratio)
    ...
    for (size_t pid = 0; pid < pillarcount; pid++) {
        ...
        double min_dist = m_cfg.pillar_base_safety_distance_mm + m_cfg.base_radius_mm + EPSILON;

        while (!found && alpha < 2 * PI) {
            for (unsigned n = 0; n < needpillars && (!n || canplace[n - 1]); n++) {
                ...
                canplace[n] = std::isinf(hr.distance()) && std::sqrt(m_mesh.squared_distance(gndsp)) > min_dist;
            }

            found = std::all_of(canplace.begin(), canplace.end(), [](bool v) { return v; });
        ...
        if (found)
            for (unsigned n = 0; n < needpillars; n++) {
                ...
                if (interconnect(pillar(), p)) {
                    ...
                    double t = bridge_mesh_distance(pillarsp, dirv(pillarsp, s), pillar().r);
        ...
        if (!newpills.empty()) {
            for (auto it = newpills.begin(), nx = std::next(it); nx != newpills.end(); ++it, ++nx) {
