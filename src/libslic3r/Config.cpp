#include "Config.hpp"
// [INTENT] Config.cpp implements the Config option type system runtime logic:
//   - ConfigOptionDef utility methods (create_empty_option, create_default_option)
//   - ConfigDef::add() — static schema registration with monotonic ordinal assignment
//   - ConfigBase serialisation/deserialisation pipeline (set_deserialize_raw, load_from_json, load_from_gcode_*)
//   - ConfigBase accessor helpers (get_abs_value, null_nullables)
//   - DynamicConfig option map management (optptr, diff_report)
//   - CEREAL_REGISTER_TYPE block for polymorphic serialisation
// [COUPLING] Pulls in PrintConfig.hpp solely for GCodeFlavor / gcfMarlin in legacy forward-compat conversion.
// [STATE] All CEREAL_REGISTER_TYPE macros at file scope (outside namespace) register the full option type hierarchy.
#include "format.hpp"
#include "Utils.hpp"
...
ConfigOption* ConfigOptionDef::create_default_option() const
{
    if (this->default_value) {
        ...
        if (type == coEnums) {
            // [HAZARD] H823: Memory leak. `dft` is cloned at the next line, then the two inner return paths at lines
            // ~305 and ~309 return early without executing `delete dft` at the end of this block.
            // The `delete dft` statement is therefore UNREACHABLE dead code.
            // The leak is one ConfigOption clone per coEnums option resolved during DynamicConfig::optptr(create=true).
            ...
            delete dft; // [HAZARD] H823: Dead code — unreachable. dft is always leaked.
        }
        delete dft;
    }
...
// [STATE] serialization_key_ordinal_last is a static local — one shared counter for ALL ConfigDef::add() calls globally.
// [HAZARD] Not thread-safe — caller contract requires all ConfigDef initialisation from the main thread (comment above).
// Any concurrent ConfigDef construction (e.g., lazy init of two preset types in parallel) would produce duplicate ordinals.
...
// [INTENT] Core deserialization dispatcher. Handles three paths:
//   1. alias resolution — looks up alternate names for renamed options
//   2. shortcut expansion — one key maps to multiple keys (e.g., "solid_layers" → "top_shell_layers" + "bottom_shell_layers")
//   3. forward-compatibility substitution — on coBools/coEnum/coBool parse failure, substitutes default value and logs
// [COUPLING] Calls this->option(opt_key, true) which may lazy-create the option via DynamicConfig::optptr(create=true).
// [HAZARD] The coBools special path uses a heuristic ("looks like enum value") to decide whether to substitute.
//   This heuristic does NOT apply to coFloat/coInt/coString — those fail silently with success=false.
...
double ConfigBase::get_abs_value(const t_config_option_key& opt_key) const
{
    ...
    // [HAZARD] H821: Dead code — this throw is UNREACHABLE. The return statement above exits unconditionally.
    // Any case where the type is not coFloat/coInt/coBool/coFloatOrPercent/coPercent exits via the earlier
    // nullptr check (throws runtime_error) or returns at line 772/776. Translation: drop this dead throw.
...
// [INTENT] Format-dispatching load entry point. Determines format by file extension.
// Supports: G-code (.gcode) → load_from_gcode_file(); JSON (.json) → load_from_json().
// [HAZARD] INI format is explicitly disabled: the else branch logs an error and returns empty substitutions.
//   load_from_ini() is commented out — no INI config file is loadable via this path.
// [COUPLING] is_gcode_file() and is_json_file() are extension-based string checks.
// BBS: add json support
...
// [INTENT] Full BBS JSON config loader. Parses nlohmann::json, applies forward-compat hacks, then calls set_deserialize_raw per key.
// [COUPLING] Contains hardcoded BBS forward-compat substitutions:
//   - "hybrid(auto)" support_type renamed: mutates different_settings_to_system to note the change
//   - "wall_infill_order" enum values renamed inline
// [HAZARD] These hardcoded string mutations are tightly coupled to BBS preset version history — any future rename requires
// parallel updates here AND in the preset schema. Translation: these must be replicated as explicit migration rules.
// [STATE] parse_str_arr is a recursive lambda for nested JSON array flattening into comma-separated strings.
...
// [INTENT] Parses G-code file footer comments ("key = value" lines) into config options.
// Walks the G-code buffer in REVERSE — from the end — to stop at the first non-config line.
// [HAZARD] Uses calloc()/free() (raw C allocation) to allocate a temporary buffer for regex_replace output.
// std::regex_replace writes into that C-allocated buffer via a raw pointer — this is NOT a C++ string.
// If the replacement output exceeds the allocated size (unlikely but theoretically possible if the replacement
// is longer than the original), this produces a buffer overrun.
// [HAZARD] This function is `static` (internal linkage) but takes config by reference — it modifies the caller's
// ConfigBase directly. Not thread-safe if called concurrently on the same ConfigBase instance.
...
// [INTENT] Sets all nullable option values to their nil sentinel.
// Used when preparing a "blank" config that marks all fields as unset (inherited from system preset).
// [HAZARD] H822: calls opt->deserialize("nil", ForwardCompatibilitySubstitutionRule::Disable).
// The deserialize() signature is: bool deserialize(const string&, bool append = false).
// ForwardCompatibilitySubstitutionRule::Disable is an enum value, NOT a bool — but it implicitly converts
// to bool (true, since Disable != 0). So this call always passes append=true, not append=false.
// This means existing values are NOT cleared before nil is appended — for vectors, nil is APPENDED
// rather than replacing all values. This is likely a bug masked by callers who only call null_nullables()
// on freshly constructed (empty) configs.
...
// [INTENT] The write path for DynamicConfig: find or lazy-create a ConfigOption for the given key.
// When create=true and the key is not found, calls ConfigOptionDef::create_default_option() to allocate
// a new heap option, then inserts it using emplace_hint() with the iterator from the failed find() call
// (a valid hint for std::map in C++11+ when the element doesn't exist at that iterator position).
// [MEMORY] Returned pointer is owned by this->options (via unique_ptr) — caller must NOT delete it.
// [HAZARD] When def() is nullptr (DynamicConfig default), create=true throws NoDefinitionException.
//   When def() is non-null but opt_key is not defined, returns nullptr (silently) rather than throwing.
...
// [INTENT] CEREAL_REGISTER_TYPE block: registers the full ConfigOption polymorphic hierarchy with the cereal
// serialisation library. This block is OUTSIDE the Slic3r namespace — cereal macros must be at global scope.
// [STATE] This block is ACTIVE (unlike the Model hierarchy which was inside #if 0 — see H527).
//   All concrete option types and their POLYMORPHIC_RELATION chains are registered here.
// [HAZARD] Any new ConfigOption subclass that is serialised via cereal MUST be added to both the REGISTER_TYPE
//   list AND the REGISTER_POLYMORPHIC_RELATION chain, or polymorphic deserialisation will fail at runtime.
// [COUPLING] This block tightly couples Config.cpp to the full ConfigOption type hierarchy at compile time.
