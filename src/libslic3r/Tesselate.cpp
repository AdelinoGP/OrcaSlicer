// [INTENT] 2D polygon triangulation for 3D mesh cap faces using the GLU
// tessellator (glu-libtess).  Converts scaled ExPolygons → unscaled Vec3d
// triangle lists suitable for OpenGL rendering or mesh construction.
// [STATE] GluTessWrapper is a per-call stack object; no shared mutable state.
// [MEMORY] m_intersection_points uses std::deque so that push_back does NOT
// invalidate existing pointers – the GLU combine callback stores a raw pointer
// into the deque back to the tessellator.  A port MUST preserve pointer
// stability; switching to std::vector would be a use-after-realloc bug.
// [HAZARD] tessError callback (lines 165-170) is fully commented out: GL
...
public:
    ...
    ~GluTessWrapper() { gluDeleteTess(m_tesselator); }
...
private:
    ...
    static void tessCombineCB(const GLdouble  newVertex[3],
