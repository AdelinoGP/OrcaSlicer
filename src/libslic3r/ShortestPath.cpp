// [HAZARD H559] Dead debug artifact: this #if 0 block was a performance debugging stub.
//               The pragma optimize("", off) and NDEBUG undef hint this was written to debug
//               a release-mode crash. Left here as-is; has no effect on production builds.
#if 0
#pragma optimize("", off)
#undef NDEBUG
#undef assert
...
// [INTENT] Traveling Salesman Problem (TSP) approximation for ordering extrusion segments
//          to minimize total travel (non-extruding) distance.
//
// Three algorithms are provided, escalating in quality:
//   1. chain_segments_closest_point       — naive nearest-neighbor fallback
//   2. chain_segments_greedy_constrained_reversals_  (V1) — multi-fragment greedy + union-find cycle detection
//   3. chain_segments_greedy_constrained_reversals2_ (V2) — V1 + chain flipping with cost tracking
//
// Post-improvement:
//   - improve_ordering_by_two_exchanges_with_segment_flipping — 2-opt on FlipEdge array (up to 100 iters)
//
// [CONCURRENCY] None of these functions hold any shared state. They are called from worker
//               threads inside TBB parallel_for regions (infill generation, perimeter ordering).
//               Thread safety relies entirely on the caller providing non-aliased data.
//
// [MEMORY] All internal structures (end_points, queues, chains) are stack-local to each call.
//          No heap allocation persists beyond the function call.
namespace Slic3r {

// [INTENT] Algorithm 1 (fallback): Naive O(n²) nearest-neighbor TSP.
//          Visits each segment start exactly once; always produces a valid result even when
//          reversal is constrained (CouldReverseFunc returns false for some segments).
//          Used as the last-resort fallback inside V1 and V2 when the greedy approach fails.
// [STATE] end_points must be pre-allocated with 2×num_segments entries (start+end per segment).
//         Segment pairs are stored at indices [2i, 2i+1]; bit-0 of index encodes start (0) vs end (1).
// [HAZARD] chain_id field of end_points is mutated in place to mark visited points.
//          After return, all chain_id fields == 1 except the last unconnected endpoint.
...
std::vector<std::pair<size_t, bool>> chain_segments_closest_point(std::vector<EndPointType>& end_points,
                                                                  KDTreeType&                kdtree,
                                                                  CouldReverseFunc&          could_reverse_func,
                                                                  EndPointType&              first_point)
{
    ...
    for (int iter = (int) num_segments - 2; iter >= 0; --iter) {
        ...
        size_t next_idx = find_closest_point(kdtree, this_point.pos, [this_idx, &end_points, &could_reverse_func](size_t idx) {
...
// [INTENT] Algorithm 2 (V1): Multi-fragment greedy TSP with union-find cycle detection.
//          Each iteration takes the cheapest available edge that:
//            (a) does not create a bifurcation (a node with 2 outgoing or 2 incoming edges), and
//            (b) does not close a premature cycle (checked via EquivalentChains union-find).
//          REVERSE_COULD_FAIL=true: if CouldReverseFunc rejects a segment reversal,
//          the algorithm falls back first to reverse-order traversal, then to the naive V0.
// [STATE] EquivalentChains is a path-compressed union-find structure built lazily.
//         end_points is indexed as [2i]=start, [2i+1]=end of segment i.
// [HAZARD H556] num_iter = num_segments * 16 — guard against endless loop if stale
//               queue entries keep re-firing. If the limit is hit, first_point remains
//               nullptr and the output reconstruction loop exits immediately, returning
//               an empty `out`. Caller does not check for empty result.
// [COUPLING] Called from chain_segments_greedy_constrained_reversals (public wrapper).
//
...
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals_(SegmentEndPointFunc end_point_func,
                                                                                  CouldReverseFunc    could_reverse_func,
                                                                                  size_t              num_segments,
                                                                                  const PointType*    start_near)
{
    ...
    if (num_segments == 0) {
        ...
        out.emplace_back(0, could_reverse_func(0) && start_near != nullptr &&
                                (end_point_func(0, false) - *start_near).template cast<double>().squaredNorm() <
                                    (end_point_func(0, true) - *start_near).template cast<double>().squaredNorm());
    } else {
        ...
        public:
            ...
            EquivalentChains(size_t reserve)
            {
                m_equivalent_with.reserve(reserve);
                m_equivalent_with.emplace_back(0);
            }
        ...
        for (EndPoint& end_point : end_points) {
            ...
            if (&end_point != first_point) {
                ...
                // Find the closest point to this end_point, which lies on a different extrusion path (filtered by the lambda).
                // Ignore the starting point as the starting point is considered to be occupied, no end point coud connect to it.
                size_t next_idx = find_closest_point(kdtree, end_point.pos, [this_idx, first_point_idx](size_t idx) {
                    return idx != first_point_idx && (idx ^ this_idx) > 1;
                });
                assert(next_idx < end_points.size());
        ...
        auto queue = make_mutable_priority_queue<EndPoint*, false>([](EndPoint* ep, size_t idx) { ep->heap_idx = idx; },
                                                                   [](EndPoint* l, EndPoint* r) {
                                                                       return l->distance_out < r->distance_out;
                                                                   });
            ...
            if (valid) {
                ...
                size_t next_idx = find_closest_point(kdtree, end_point1.pos, [&end_points, &equivalent_chain, this_idx](size_t idx) {
                    ...
                    if ((idx ^ this_idx) <= 1 || end_points[idx].chain_id != 0)
                        ...
                        // cannot connect to an already connected point (ideally those would be removed from the KD tree, but the update is
                        // difficult).
                        return false;
        ...
        if (REVERSE_COULD_FAIL) {
            ...
            if (failed)
                ...
                out = chain_segments_closest_point<EndPoint, decltype(kdtree), CouldReverseFunc>(end_points, kdtree, could_reverse_func,
...
// [INTENT] Helper for V2: re-evaluate a single end point's best candidate after
//          a chain flip may have changed costs. Updates distance_out and re-inserts
//          into the priority queue (or removes if the point should no longer be queued).
// [STATE] Called after every successful connection that causes chain flipping.
//         first_point_idx: index of the absolute start point; we must never flip edges
//         that include the start point.
// [HAZARD] This function modifies end_point.edge_candidate and end_point.distance_out
//          in place. If the KD tree contains stale points (already connected), the
//          lambda filter rejects them but does NOT remove them from the tree — the KD
//          tree is never pruned (noted as FIXME in V1 as well).
template<typename QueueType, typename KDTreeType, typename ChainsType, typename EndPointType>
void update_end_point_in_queue(QueueType&                 queue,
                               const KDTreeType&          kdtree,
                               ChainsType&                chains,
                               std::vector<EndPointType>& end_points,
                               EndPointType&              end_point,
                               size_t                     first_point_idx,
                               const EndPointType*        first_point)
{
    ...
    assert(first_point_idx == this_idx || first_point_idx == (this_idx ^ 1) || end_point.chain_id == 0 ||
           end_point.opposite(end_points).chain_id == 0);
    ...
    if (first_point_idx == this_idx || (end_point.chain_id > 0 && first_point_idx == (this_idx ^ 1))) {
        ...
        size_t next_idx = find_closest_point(kdtree, end_point.pos,
                                             [&end_points, &chains, this_idx, first_point_idx, first_point, this_chain](size_t idx) {
                                                 ...
                                                 assert((end_points[this_idx].chain_id == 0 && end_points[this_idx].edge_out == nullptr) ||
                                                        (end_points[this_idx ^ 1].chain_id == 0 &&
                                                         end_points[this_idx ^ 1].edge_out == nullptr));
                                                 ...
                                                 if (chain2a > 0 && chain2b > 0)
                                                     // Only unconnected end point or a point next to an unconnected end point may be
                                                     // connected to. Ideally those would be removed from the KD tree, but the update is
                                                     // difficult.
                                                     return false;
                                                 ...
                                                 if (this_chain == chain2)
                                                     ...
                                                     return this_chain == 0;
                                                 // Don't connect to a segment requiring flipping if the segment starts or ends with the
                                                 // first point.
                                                 if (chain2a > 0) {
        ...
        assert(next_idx < end_points.size());
        assert(chains.equivalent(end_points[next_idx].chain_id) != chains.equivalent(end_points[next_idx ^ 1].chain_id) ||
               end_points[next_idx].chain_id == 0);
...
// [INTENT] Algorithm 3 (V2): V1 + chain-flip cost tracking.
//          Extends V1 by maintaining Chain structs that remember both forward and flipped
//          cumulative costs. When connecting end points requires flipping an existing chain,
//          the flip penalty (cost_flipped - cost) is added to the candidate distance so the
//          priority queue naturally prefers cheaper flips.
//          After a connection, the merged chain's endpoints are updated in the queue via
//          update_end_point_in_queue().
// [HAZARD H556] Same iteration guard as V1: num_iter = num_segments * 16. If limit hit,
//               first_point stays nullptr, output reconstruction loop immediately exits
//               with an empty result. The caller (chain_segments_greedy_constrained_reversals2)
//               does not detect this.
// [MEMORY] Chains vector grows to ~num_segments entries; each Chain stores two doubles,
//          two EndPoint pointers, and a size_t. For 100k segments: ~4 MB.
// [COUPLING] Called from chain_segments_greedy_constrained_reversals2 and chain_segments_greedy2.
template<typename PointType, typename SegmentEndPointFunc, bool REVERSE_COULD_FAIL, typename CouldReverseFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals2_(SegmentEndPointFunc end_point_func,
                                                                                   CouldReverseFunc    could_reverse_func,
                                                                                   size_t              num_segments,
                                                                                   const PointType*    start_near)
{
    ...
    if (num_segments == 0) {
        ...
        // Just sort the end points so that the first point visited is closest to start_near.
        out.emplace_back(0, start_near != nullptr && (end_point_func(0, true) - *start_near).template cast<double>().squaredNorm() <
                                                         (end_point_func(0, false) - *start_near).template cast<double>().squaredNorm());
    } else {
        // End points of segments for the KD tree closest point search.
        // A single end point is inserted into the search structure for loops, two end points are entered for open paths.
        ...
        //
        // Unique chain IDs are assigned to paths. If paths are connected, end points will not have their chain IDs updated, but the chain
        // IDs will remember an "equivalent" chain ID, which is the lowest ID of all the IDs in the path, and the lowest ID is equivalent to
        // itself. Chain IDs are indexed starting with 1.
        //
        ...
        // Initialize a heap of end points sorted by the lowest distance to the next valid point of a path.
        auto queue = make_mutable_priority_queue<EndPoint*, true>([](EndPoint* ep, size_t idx) { ep->heap_idx = idx; },
                                                                  [](EndPoint* l, EndPoint* r) {
                                                                      return l->distance_out < r->distance_out;
                                                                  });
        ...
        for (size_t num_connections_to_end = num_segments - 1; num_iter > 0; --num_iter) {
            ...
            if (end_point2->chain_id > 0 && end_point2_other->chain_id > 0) {
                ...
                if (end_point1_chain_id == end_point2_chain_id && end_point1_chain_id != 0)
                    ...
                    // Verify whether end_point1.distance_out still matches the current state of the two end points to be connected and
                    // their chains. Namely, the other chain may have been flipped in the meantime.
            ...
            if (valid) {
                ...
                assert(chain1 == nullptr || (chain1_flip ? (chain1->begin == end_point1_other || chain1->end == end_point1_other) :
                                                           (chain1->begin == end_point1 || chain1->end == end_point1)));
                assert(chain2 == nullptr || (chain2_flip ? (chain2->begin == end_point2_other || chain2->end == end_point2_other) :
                                                           (chain2->begin == end_point2 || chain2->end == end_point2)));
                if (chain1_flip)
                ...
                Chain& chain    = chains.chain(chain_id);
                {
                    ...
                    chain_dst.cost  = (chain1 == 0 ? 0. : chain1->cost) + (chain2 == 0 ? 0. : chain2->cost) +
                                     (end_point2->pos - end_point1->pos).norm();
                    chain_dst.cost_flipped = (chain1 == 0 ? 0. : chain1->cost_flipped) + (chain2 == 0 ? 0. : chain2->cost_flipped) +
                                             (end_point2_other->pos - end_point1_other->pos).norm();
                ...
                if (--num_connections_to_end == 0) {
                    ...
                    update_end_point_in_queue(queue, kdtree, chains, end_points, chain.begin->opposite(end_points), first_point_idx,
                                              first_point);
                    update_end_point_in_queue(queue, kdtree, chains, end_points, chain.end->opposite(end_points), first_point_idx,
                                              first_point);
                    if (chain1_flip)
                    ...
                    assert(&chain.begin->opposite(end_points) != first_point &&
                           (chain.begin == first_point ? chain.begin->opposite(end_points).heap_idx_invalid() :
                                                         chain.begin->opposite(end_points).heap_idx < queue.size()));
                    assert(&chain.end->opposite(end_points) != first_point &&
                           (chain.end == first_point ? chain.end->opposite(end_points).heap_idx_invalid() :
                                                       chain.end->opposite(end_points).heap_idx < queue.size()));
                }
            } else {
                // This edge forms a loop. Update end_point1 and try another one.
...
// [INTENT] Public wrappers: thin dispatch layer that selects the appropriate internal
//          algorithm variant and exposes a simpler interface.
//   chain_segments_greedy_constrained_reversals  → V1, REVERSE_COULD_FAIL=true
//   chain_segments_greedy                        → V1, REVERSE_COULD_FAIL=false (all reversible)
//   chain_segments_greedy_constrained_reversals2 → V2, REVERSE_COULD_FAIL=true
//   chain_segments_greedy2                       → V2, REVERSE_COULD_FAIL=false
// [COUPLING] These templates are instantiated only within this .cpp file.
template<typename PointType, typename SegmentEndPointFunc, typename CouldReverseFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals(SegmentEndPointFunc end_point_func,
                                                                                 CouldReverseFunc    could_reverse_func,
                                                                                 size_t              num_segments,
                                                                                 const PointType*    start_near)
{
    return chain_segments_greedy_constrained_reversals_<PointType, SegmentEndPointFunc, true, CouldReverseFunc>(end_point_func,
                                                                                                                could_reverse_func,
                                                                                                                num_segments, start_near);
}

template<typename PointType, typename SegmentEndPointFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy(SegmentEndPointFunc end_point_func,
                                                           size_t              num_segments,
                                                           const PointType*    start_near)
{
    auto could_reverse_func = [](size_t /* idx */) -> bool { return true; };
    return chain_segments_greedy_constrained_reversals_<PointType, SegmentEndPointFunc, false,
                                                        decltype(could_reverse_func)>(end_point_func, could_reverse_func, num_segments,
                                                                                      start_near);
}

template<typename PointType, typename SegmentEndPointFunc, typename CouldReverseFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals2(SegmentEndPointFunc end_point_func,
                                                                                  CouldReverseFunc    could_reverse_func,
                                                                                  size_t              num_segments,
                                                                                  const PointType*    start_near)
{
    return chain_segments_greedy_constrained_reversals2_<PointType, SegmentEndPointFunc, true, CouldReverseFunc>(end_point_func,
                                                                                                                 could_reverse_func,
                                                                                                                 num_segments, start_near);
}

template<typename PointType, typename SegmentEndPointFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy2(SegmentEndPointFunc end_point_func,
                                                            size_t              num_segments,
                                                            const PointType*    start_near)
{
    auto could_reverse_func = [](size_t /* idx */) -> bool { return true; };
    return chain_segments_greedy_constrained_reversals2_<PointType, SegmentEndPointFunc, false,
                                                         decltype(could_reverse_func)>(end_point_func, could_reverse_func, num_segments,
                                                                                       start_near);
}

// [INTENT] Public API: chain ExtrusionEntity pointers by nearest-neighbor V1 TSP.
//          Returns pairs of (original_index, reversed_flag).
//          Loops always have reversed=false (start==end, reversal is meaningless).
// [HAZARD H553] chain_and_reorder_extrusion_entities: the erase-remove_if lambda casts
//               every entity to ExtrusionEntityCollection* unconditionally, then calls
//               ->empty(). If any entity is a plain ExtrusionPath or ExtrusionLoop (not a
//               collection), this is UB — the downcast is unchecked. In practice, perimeter
//               generation only places collections here, but this assumption is fragile.
// [COUPLING] Calls chain_segments_greedy_constrained_reversals (V1 with reversal constraints).
std::vector<std::pair<size_t, bool>> chain_extrusion_entities(std::vector<ExtrusionEntity*>& entities, const Point* start_near)
{
    auto segment_end_point = [&entities](size_t idx, bool first_point) -> const Point& {
        return first_point ? entities[idx]->first_point() : entities[idx]->last_point();
    };
    auto could_reverse = [&entities](size_t idx) {
        const ExtrusionEntity* ee = entities[idx];
        return ee->is_loop() || ee->can_reverse();
    };
    std::vector<std::pair<size_t, bool>> out =
        chain_segments_greedy_constrained_reversals<Point, decltype(segment_end_point), decltype(could_reverse)>(segment_end_point,
                                                                                                                 could_reverse,
                                                                                                                 entities.size(),
                                                                                                                 start_near);
...
// [INTENT] Apply a chain ordering to the entities vector in-place.
//          Calls ExtrusionEntity::reverse() on entities flagged for reversal.
// [COUPLING] chain must have been produced by chain_extrusion_entities on the same entities vector.
...
void chain_and_reorder_extrusion_entities(std::vector<ExtrusionEntity*>& entities, const Point* start_near)
{
    ...
    entities.erase(std::remove_if(entities.begin(), entities.end(),
                                  [](ExtrusionEntity* entity) { return static_cast<ExtrusionEntityCollection*>(entity)->empty(); }),
                   entities.end());
...
{
    auto segment_end_point = [&extrusion_paths](size_t idx, bool first_point) -> const Point& {
        return first_point ? extrusion_paths[idx].first_point() : extrusion_paths[idx].last_point();
    };
    return chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point, extrusion_paths.size(), start_near);
}

// [INTENT] Apply a chain ordering to ExtrustionPath vector. Moves-constructs into output
//          to avoid copies, reverses flagged paths.
// [HAZARD H555] Declaration/definition mismatch: header declares `chain` as
//               `std::vector<std::pair<size_t, bool>> chain` (by value),
//               but this definition takes it as `const std::vector<...>&` (const ref).
//               Compiles because callers only include the header; the linker matches by
//               mangled name. No behavior difference, but breaks ODR expectations and
//               will cause a link error if TUs include both headers.
...
// [INTENT] Order ExPolygons by bounding-box centroid using chain_points (V1 greedy).
// [HAZARD H557] Uses bbox.center() as the representative point. Two ExPolygons with
//               identical bounding-box centroids but different shapes get non-deterministic
//               ordering — they tie in the KD tree nearest-neighbor lookup, and the result
//               depends on insertion order. Affects brim and support layer ordering.
...
std::vector<size_t> chain_points(const Points& points, Point* start_near)
{
    ...
    std::vector<std::pair<size_t, bool>> ordered = chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point,
                                                                                                             points.size(), start_near);
...
// [HAZARD H559] Dead code block #1: improve_ordering_by_segment_flipping.
//               Segment-flipping post-improvement with a MutablePriorityHeap sorted by
//               decreasing connection distance. Was replaced by reorder_by_two_exchanges_with_segment_flipping.
//               Preserve as algorithm reference — shows the evolution from single-flip to 2-opt.
#if 0
...
// [INTENT] FlipEdge: a directed segment (p1→p2) plus its source index.
//          Stored in the live `edges` array used by the 2-opt post-processor.
//          `flip()` swaps p1/p2 to indicate the segment is traversed in reverse.
// [STATE] After reorder_by_two_exchanges_with_segment_flipping, p1 is the entry point
//         and p2 is the exit point for each segment. source_index maps back to the
//         original Polyline for move-construction in improve_ordering_by_two_exchanges.
struct FlipEdge
{
...
static inline ConnectionCost operator-(const ConnectionCost& lhs, const ConnectionCost& rhs)
{
    return ConnectionCost(lhs.cost - rhs.cost, lhs.cost_flipped - rhs.cost_flipped);
}

// [INTENT] minimum_crossover_cost (3-span, LIVE): Given three contiguous spans that together cover
//          the full edge sequence, enumerate all 2^6 = 64 flip/reverse combinations plus 3 span
//          permutations (192 candidates) and return the (cost, flip_min) pair with lowest cost.
//          This is the inner kernel of the 2-opt TSP post-improvement loop.
// [STATE]  `cost_current` = baseline traversal cost; return value has cost_min <= cost_current
//          (if no improvement found, flip_min == 0 and cost_min == cost_current).
// [CONCURRENCY] Pure function — no shared mutable state.
// [HAZARD H560] The 3-span permutation index is encoded as (i >> 6) in do_crossover's switch.
//          Possible values are 0, 1, 2.  The caller packs up to i + (2 << 6) = i + 128, so
//          flip_min can reach 127 + 128 = 255 when i=127, permutation=2.
//          do_crossover's default: branch asserts (i >> 6) == 2; if flip_min > 2*64+63 this fires
//          in debug (silent UB in release).  Currently safe since max flip_min = 2*64+63 = 191.
static inline std::pair<double, size_t> minimum_crossover_cost(const std::vector<FlipEdge>&     edges,
                                                               const std::pair<size_t, size_t>& span1,
                                                               const ConnectionCost&            cost1,
                                                               const std::pair<size_t, size_t>& span2,
                                                               const ConnectionCost&            cost2,
                                                               const std::pair<size_t, size_t>& span3,
                                                               const ConnectionCost&            cost3,
                                                               const double                     cost_current)
{
    auto connection_cost = [&edges](const std::pair<size_t, size_t>& span1, const ConnectionCost& cost1, bool reversed1, bool flipped1,
        ...
        auto first_point = [&edges](const std::pair<size_t, size_t>& span, bool flipped) {
            return flipped ? edges[span.first].p2 : edges[span.first].p1;
        };
        auto last_point = [&edges](const std::pair<size_t, size_t>& span, bool flipped) {
            return flipped ? edges[span.second - 1].p1 : edges[span.second - 1].p2;
        };
        auto point = [first_point, last_point](const std::pair<size_t, size_t>& span, bool start, bool flipped) {
            return start ? first_point(span, flipped) : last_point(span, flipped);
        };
        ...
        return simple_span_ignore(span1, reversed1) || simple_span_ignore(span2, reversed2) || simple_span_ignore(span3, reversed3) ?
    ...
    for (size_t i = 0; i < (1 << 6); ++i) {
        ...
        double c1 = (i == 0) ? cost_current :
                               connection_cost(span1, cost1, (i & 1) != 0, (i & (1 << 1)) != 0, span2, cost2, (i & (1 << 2)) != 0,
                                               (i & (1 << 3)) != 0, span3, cost3, (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        double c2 = connection_cost(span1, cost1, (i & 1) != 0, (i & (1 << 1)) != 0, span3, cost3, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0,
                                    span2, cost2, (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        double c3 = connection_cost(span2, cost2, (i & 1) != 0, (i & (1 << 1)) != 0, span1, cost1, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0,
                                    span3, cost3, (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        if (c1 < cost_min) {
            cost_min = c1;
            flip_min = i;
        }
...
// [INTENT] minimum_crossover_cost (4-span variant, DEAD CODE H559): 4-opt extension that considers
//          four span permutations × 2^8 flip combinations = 12 × 256 = 3072 candidates per call.
//          Gated by #if 0 because it is "currently not used, too slow".
//          Preserved as algorithm reference material — do NOT delete.
// [HAZARD H559] Dead code. Compile-tested only when #if 0 is toggled.
#if 0
...
// [INTENT] do_crossover (3-span, LIVE): Apply the crossover selected by minimum_crossover_cost.
//          Rearranges edges_in → edges_out according to span permutation encoded in `i`:
//            bits [5:0] = reversed/flipped flags for each span
//            bits [7:6] = span permutation (0 = 1-2-3, 1 = 1-3-2, 2 = 2-1-3)
// [MEMORY] edges_out must be pre-allocated with the same size as edges_in (asserted).
// [HAZARD H560] switch default: asserts (i >> 6) == 2 in DEBUG; in RELEASE, if i >> 6 > 2
//               this silently executes do_it with the wrong permutation (span2 first).
//               Safe in practice because minimum_crossover_cost limits flip_min to at most
//               (2 << 6) + 63 = 191, so i >> 6 is always 0, 1, or 2.
static inline void do_crossover(const std::vector<FlipEdge>&     edges_in,
                                std::vector<FlipEdge>&           edges_out,
                                const std::pair<size_t, size_t>& span1,
                                const std::pair<size_t, size_t>& span2,
                                const std::pair<size_t, size_t>& span3,
                                size_t                           i)
{
    ...
    auto do_it = [&edges_in, &edges_out](const std::pair<size_t, size_t>& span1, bool reversed1, bool flipped1,
    ...
    switch (i >> 6) {
    case 0:
        do_it(span1, (i & 1) != 0, (i & (1 << 1)) != 0, span2, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span3, (i & (1 << 4)) != 0,
              (i & (1 << 5)) != 0);
        break;
    case 1:
        do_it(span1, (i & 1) != 0, (i & (1 << 1)) != 0, span3, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span2, (i & (1 << 4)) != 0,
              (i & (1 << 5)) != 0);
        break;
    default:
        assert((i >> 6) == 2);
        do_it(span2, (i & 1) != 0, (i & (1 << 1)) != 0, span1, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span3, (i & (1 << 4)) != 0,
              (i & (1 << 5)) != 0);
    }
    assert(edges_in.size() == edges_out.size());
}

// [INTENT] do_crossover (4-span variant, DEAD CODE H559): Apply a 4-opt crossover.
//          Switch dispatches on (i >> 8) across 12 span permutations.
//          Not compiled (#if 0) — preserved as algorithm reference material.
// [HAZARD H559] Dead code. Do NOT delete.
#if 0
...
// [INTENT] reorder_by_two_exchanges_with_segment_flipping: 2-opt TSP post-improvement on FlipEdge
//          sequences. Iteratively finds the pair of crossover positions that most reduces total
//          chain travel distance, including trying all 3 span permutations × 64 flip combinations.
//          Terminates when no improvement is found or after max_iterations iterations.
// [HAZARD H558] max_iterations = min(n, 100). If the iteration limit is reached without
//          convergence, crossover1_pos_final / crossover2_pos_final remain SIZE_MAX and
//          crossover_flip_final = 0, so the loop body falls into the "giving up" break — the
//          output edges are simply the best found so far, which may not be locally optimal.
//          This is a deliberate trade-off but is not documented in the original code.
// [COMPLEXITY] O(min(n,100) × (n log n + n²)) worst case. Expected better since short-circuit
//          break fires as soon as improvement is found.
...
static inline void reorder_by_two_exchanges_with_segment_flipping(std::vector<FlipEdge>& edges)
{
    ...
    for (size_t iter = 0; iter < max_iterations; ++iter) {
        ...
        std::sort(connection_lengths.begin(), connection_lengths.end(),
                  [](const std::pair<double, size_t>& l, const std::pair<double, size_t>& r) { return l.first > r.first; });
        ...
        for (const std::pair<double, size_t>& first_crossover_candidate : connection_lengths) {
            ...
            for (size_t j = 1; j < connections.size(); ++j)
                if (!connection_tried[j]) {
                    ...
                    if (a > b)
                        std::swap(a, b);
                    std::pair<double, size_t> cost_and_flip = minimum_crossover_cost(edges, std::make_pair(size_t(0), a),
                                                                                     connections[a - 1], std::make_pair(a, b),
                                                                                     connections[b - 1] - connections[a],
                                                                                     std::make_pair(b, edges.size()),
                                                                                     connections.back() - connections[b],
        ...
        if (crossover_flip_final > 0) {
            // Pair of cross over positions and flip / reverse constellation has been found, which improves the total cost of the
            // connection. Perform a crossover.
            if (crossover1_pos_final > crossover2_pos_final)
                std::swap(crossover1_pos_final, crossover2_pos_final);
            do_crossover(edges, edges_tmp, std::make_pair(size_t(0), crossover1_pos_final),
                         std::make_pair(crossover1_pos_final, crossover2_pos_final), std::make_pair(crossover2_pos_final, edges.size()),
...
// [INTENT] reorder_by_three_exchanges_with_segment_flipping (3-opt v1, DEAD CODE H559):
//          Cubic search — for each long edge, tries all (j, k) pairs of crossover positions
//          (O(n³) inner loop per iteration). Disabled due to excessive runtime.
//          Preserved as algorithm reference material.
// [HAZARD H559] Dead code. Do NOT delete.
#if 0
...
// [INTENT] FourOptCosts: Compact wrapper giving indexed (flipped or not) access to four
//          ConnectionCost objects, used only by the dead `minimum_crossover_cost` /
//          `reorder_by_three_exchanges_with_segment_flipping2` code path.
// [STATE]  Holds raw pointers to ConnectionCosts; lifetime must exceed this wrapper.
// [HAZARD H559] Only instantiated inside dead #if 0 blocks.
class FourOptCosts
{
public:
...
// [INTENT] minimum_crossover_cost (Eigen matrix variant, DEAD CODE H559): Uses a precomputed
//          16×16 Eigen distance matrix between span endpoint pairs. Intended to accelerate the
//          4-opt search by avoiding repeated norm() calls. Disabled because the 4-opt outer loop
//          is itself O(n³) and "currently not used, too slow".
// [HAZARD H559] Dead code. Do NOT delete.
#if 0
static inline std::pair<double, size_t> minimum_crossover_cost(
...
// [INTENT] reorder_by_three_exchanges_with_segment_flipping2 (4-opt v2 with Eigen matrix, DEAD CODE H559):
//          Uses the precomputed Eigen distance matrix to speed up 4-opt candidate enumeration.
//          Still O(n³) inner loop — disabled as too slow.
//          Preserved as algorithm reference material.
// [HAZARD H559] Dead code. Do NOT delete.
// Currently not used, too slow.
...
// [INTENT] improve_ordering_by_two_exchanges_with_segment_flipping: Public wrapper that lifts
//          Polylines → FlipEdge vector, delegates to reorder_by_two_exchanges_with_segment_flipping,
//          then reconstructs Polylines (move-constructing from source, reversing if needed).
//          Called by chain_polylines (infill) and brim ordering.
// [STATE]  `fixed_start` = true means the first polyline is pinned; passed to outer caller but
//          NOT passed down to reorder_by_two_exchanges_with_segment_flipping — only affects debug SVG.
//          The actual 2-opt does not honor a fixed start: any segment may end up first.
// [COUPLING] Consumes `polylines` via move; output replaces it in-place.
// Flip the sequences of polylines to lower the total length of connecting lines.
...
static inline void improve_ordering_by_two_exchanges_with_segment_flipping(Polylines& polylines, bool fixed_start)
{
    ...
    std::transform(polylines.begin(), polylines.end(), std::back_inserter(edges), [&polylines](const Polyline& pl) {
        return FlipEdge(pl.first_point().cast<double>(), pl.last_point().cast<double>(), &pl - polylines.data());
    });
#if 1
    reorder_by_two_exchanges_with_segment_flipping(edges);
#else
...
// [INTENT] chain_polylines: Public API for ordering a set of polylines to minimize travel distance.
//          Pipeline: V2 greedy nearest-neighbor chaining (chain_segments_greedy2) → optional
//          2-opt post-improvement (improve_ordering_by_two_exchanges_with_segment_flipping).
//          2-opt step is skipped when start_near != nullptr (fixed-start scenario, e.g. seam).
// [STATE]  Input polylines are consumed (rvalue-ref); output is an owned Polylines.
// [COUPLING] Called by FillBase, brim generation, and other callers that produce unordered polylines.
...
Polylines chain_polylines(Polylines&& polylines, const Point* start_near)
{
    ...
    if (!polylines.empty()) {
        ...
        std::vector<std::pair<size_t, bool>> ordered = chain_segments_greedy2<Point, decltype(segment_end_point)>(segment_end_point,
                                                                                                                  polylines.size(),
                                                                                                                  start_near);
        out.reserve(polylines.size());
        for (auto& segment_and_reversal : ordered) {
...
// [INTENT] chain_print_object_instances (vector overload): Orders all print instances from
//          multiple PrintObjects using V1 nearest-neighbor greedy chaining.
//          Each instance's `shift` is used as its representative point.
//          Returns a flat ordered list of const PrintInstance pointers.
// [COUPLING] Called by the G-code generator to sequence multi-object prints.
// [HAZARD H557] No 2-opt post-improvement — only greedy NN. Instance ordering quality
//          degrades for many widely-spaced objects.
// BBS
std::vector<const PrintInstance*> chain_print_object_instances(const std::vector<const PrintObject*>& print_objects, const Point* start_near)
{
    ...
    auto segment_end_point = [&object_reference_points](size_t idx, bool /* first_point */) -> const Point& {
        return object_reference_points[idx];
    };
    std::vector<std::pair<size_t, bool>> ordered = chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point,
                                                                                                             instances.size(), start_near);
...
// [INTENT] chain_print_object_instances (Print overload): Convenience wrapper that forwards
//          all objects from the Print to the vector overload with start_near=nullptr.
// [COUPLING] Used by GCode.cpp single-plate print sequencing.
...
// [INTENT] chain_lines: Chain a set of disconnected line segments into polylines by joining
//          endpoints that fall within `point_distance_epsilon` of each other.
//          Uses ClosestPointInRadiusLookup (an R-tree-like structure) for O(n log n) lookup.
//          Each seed line initiates a polyline that is extended forward, then reversed and
//          extended backward.  Joined endpoints are averaged to reduce quantization noise.
// [HAZARD H554] `point_distance_epsilon2` is declared `static const double` inside the function
//          body — this is an ODR risk if the translation unit is ever split or if the function
//          is instantiated in multiple TUs. Safe in the current single-TU build but fragile.
// [STATE]  `line_consumed` tracks which lines have been incorporated; lines not reachable from
//          any seed (isolated lines) each start their own polyline.
// [COUPLING] Called by geometry utilities and support material for line merging.
