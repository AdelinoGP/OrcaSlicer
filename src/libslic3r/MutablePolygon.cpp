// [INTENT] MutablePolygon.cpp — doubly-linked circular polygon smoothing kernel.
//   Provides three overloads of remove_duplicates() (exact, epsilon-distance, angle-constrained)
//   and the smooth_outward() corner-smoothing algorithm adapted from Cura.
//   Used by Arachne variable-width toolpath post-processing to clean up generated polygons
//   before they are converted to ExtrusionPaths.
// [STATE] All functions operate on MutablePolygon by reference (in-place mutation).
//   MutablePolygon is a singly-allocated circular linked list; iterators are invalidated only
//   by remove() calls on different iterators (not on the iterator being incremented).
// [COUPLING] smooth_outward() is the primary public entry point; remove_duplicates() is called
//   as a pre-pass inside smooth_outward() and also directly from Arachne path post-processing.
// [CONCURRENCY] No shared state. All functions are purely thread-local; safe to call in parallel
//   if each thread owns a distinct MutablePolygon.
...
// [INTENT] Remove exact duplicate adjacent points (coord_t equality). Walks the circular list
//   once; removes node and continues from the next surviving node. O(n) single pass.
// [STATE] May reduce polygon to empty (size 0) if all points are identical.
...
// [INTENT] Remove nearly-duplicate adjacent points within squared Euclidean distance eps².
//   eps is in the same units as the polygon coordinates (scaled coord_t space).
// [STATE] Same lifecycle as exact-duplicate variant above.
...
// [INTENT] Angle-constrained deduplication: remove adjacent point if it is within scaled_eps
//   of its predecessor AND the turn angle at that point is less than max_angle (i.e. nearly collinear
//   so removing it does not significantly change the polygon shape).
//   Used before smooth_outward() in Arachne to pre-clean near-collinear micro-segments.
// [STATE] Polygon is cleared (not just shrunk) if it degenerates below 3 points.
...
// [INTENT] clip_narrow_corner() — Cura smooth_corner_complex() port (Tim Kuipers).
//   After a concave corner at p1 has been removed (it0..it2 remain), attempt to close a narrow
//   crack by walking forward (it2→next) and backward (it0←prev) simultaneously as long as the
//   new clipping diagonal remains shorter than shortcut_length.
//   Terminates when: (a) one side is blocked by a convex turn, (b) one side overshoots
//   shortcut_length, or (c) the polygon degenerates to ≤2 points (closed hole) or a tiny triangle.
//   On over-shoot, clips the final trapezoid with a circle-line intersection.
// [STATE] Modifies MutablePolygon in-place; updates it0 and it2 via out-params.
//   Returns true if polygon was fully consumed (empty or tiny CCW triangle → caller should return).
// [COUPLING] Only called from smooth_outward(). unprocessed_range is a MutablePolygon::range
//   tracking which source points still need visiting; remove_back/remove_front advance the range.
// [HAZARD] The diagonal-choice heuristic at line ~131 ("take direction closer to p1") is
//   documented with //FIXME — author acknowledges it may not be optimal for all concave cracks.
...
// Now try to close a concave crack by walking left from it0 and right from it2 as long as the new clipping edge is smaller than
// shortcut_length and the new clipping edge is still inside the polygon (it is a diagonal, it does not intersect polygon boundary). Once
// the traversal stops (always at a clipping edge shorter than shortcut_length), the final trapezoid is clipped with a new clipping edge of
// shortcut_length. Return true if a hole was completely closed (degenerated to an empty polygon) or a single CCW triangle was left, which
// is not to be simplified any further. it0, it2 are updated to the final clipping edge.
static bool clip_narrow_corner(const Vec2i64             p1,
...
// [INTENT] smooth_outward() — Cura smooth_outward() port (Tim Kuipers).
//   Iterates every source vertex of the polygon exactly once (via MutablePolygon::range).
//   At each concave corner (cross2(v1,v2) > 0, angle > 135°):
//     1. Removes the corner point.
//     2. If the resulting shortcut |p0→p2| < clip_dist_scaled: tries to close a narrow crack
//        (calls clip_narrow_corner).
//     3. If |p0→p2| > clip_dist_scaled: trims the obtuse corner — either fully clips the shorter
//        arm, or bisects and inserts two new points (perpendicular-to-bisector clip).
//   Finalizes by checking if the last remaining 3-point (triangle) polygon is a CW degenerate hole
//   and clears it if its height < clip_dist_scaled.
// [STATE] Pre-processes with remove_duplicates(0.01 mm). Polygon may be cleared entirely.
//   'unprocessed_range' tracks which vertices have not yet been visited; advancing it past deleted
//   nodes is the key invariant ensuring each source vertex is visited exactly once.
// [COUPLING] Called from Arachne PerimeterGenerator after variable-width path generation,
//   to smooth the per-loop MutablePolygon before extrusion-path construction.
// [CONCURRENCY] Thread-local; no shared state.
