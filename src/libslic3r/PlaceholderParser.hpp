...
public:
    ...
    struct ContextData
    {
        // [STATE] PRNG state is intentionally persisted between process() calls to support deterministic script-side randomness streams.
        std::mt19937 rng;
        ...
        // [MEMORY] Owned optional config blob; lifetime is tied to ContextData so parser instances can remain stateless/thread-safe.
    ...
    // [COUPLING] Evaluator semantics are shared with custom G-code placeholders and print-host script expansion paths.
    std::string process(const std::string&   templ,
    ...
    std::string process(const std::string&   templ,
                        ...
                        ContextData*         context             = nullptr) const
    {
    ...
    // Throws Slic3r::PlaceholderParserError on syntax or runtime error.
    static bool evaluate_boolean_expression(const std::string&   templ,
...
private:
    ...
    // [STATE] Mutable symbol table for one parser instance; callers refresh it via apply_config()/set() before each rendering pass.
    DynamicConfig m_config;
    // [MEMORY] Non-owning pointer: caller must keep external_config alive for the entire PlaceholderParser lifetime.
    // [HAZARD] Dangling external_config would surface as late parser lookup crashes far from callsite.
