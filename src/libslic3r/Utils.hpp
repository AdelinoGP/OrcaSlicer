...
// [INTENT] General utility functions and helpers used throughout the slicer.
// Provides: logging, file/path operations, string manipulation, memory formatting, CLI codes.
// [COUPLING] Heavy boost dependencies: filesystem, algorithm, date_time, system.
// [HAZARD] Also depends on OpenSSL (MD5) - adds cryptographic dependency.
...
// [INTENT] Round division - / divisor with dividend rounding to nearest.
// [HAZARD] Integer division with addition can overflow for large values.
...
// [INTENT] Type-safe vector conversion with overflow checking.
// [HAZARD] Throws std::overflow_error or std::underflow_error on boundary violation.
// [MEMORY] Creates new vector - does not modify source.
...
inline bool is_bambustudio_open(const std::string& url)
{
    return boost::starts_with(url, "bambustudio://open") || boost::starts_with(url, "bambustudioopen://");
}
inline bool is_cura_open(const std::string& url) { return boost::starts_with(url, "cura://open"); }
inline bool is_supported_open_protocol(const std::string& url)
{
    return is_orca_open(url) || is_prusaslicer_open(url) || is_bambustudio_open(url) || is_cura_open(url);
}
...
template<class T>
size_t next_highest_power_of_2(T v,
...
template<class VectorType> void reserve_power_of_2(VectorType& vector, size_t n) { vector.reserve(next_highest_power_of_2(n)); }

template<class VectorType> void reserve_more(VectorType& vector, size_t n) { vector.reserve(vector.size() + n); }
...
template<typename INDEX_TYPE> inline INDEX_TYPE prev_idx_modulo(INDEX_TYPE idx, const INDEX_TYPE count)
{
    if (idx == 0)
        idx = count;
    return --idx;
...
template<typename INDEX_TYPE> inline INDEX_TYPE next_idx_modulo(INDEX_TYPE idx, const INDEX_TYPE count)
{
    if (++idx == count)
        idx = 0;
    return idx;
...
template<typename INDEX_TYPE> inline INDEX_TYPE round_up_divide(const INDEX_TYPE dividend, const INDEX_TYPE divisor)
{
...
// [INTENT] RAII wrapper for arbitrary cleanup actions - similar to std::scope_exit.
// Useful for cleanup that must run even when exceptions are thrown.
// [MEMORY] Closure stored as std::function - heap allocation possible for large captures.
// [PATTERN] Commonly used for: file handle cleanup, resetting flags, unlocking mutexes.
class ScopeGuard
{
public:
...
public:
    ...
    // [INTENT] Executes closure on destruction - ensures cleanup runs.
    ~ScopeGuard()
    {
        if (closure) {
            closure();
...
inline std::string filter_characters(const std::string& str, const std::string& filterChars)
{
    ...
    auto removeFunc = [&filterChars](char ch) { return filterChars.find(ch) != std::string::npos; };
