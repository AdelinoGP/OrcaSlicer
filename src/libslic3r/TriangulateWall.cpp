// [INTENT] Wall triangulation between two polygons at different Z heights.
// [HAZARD] ENTIRE FILE IS DEAD CODE – 100% commented out.  The Ring /
// Triangulator implementation (score-minimising greedy pairing between upper
// and lower polygon rings) was replaced by the simpler wall_strip approach in
// SlicesToTriangleMesh.cpp.  The file compiles to an empty namespace body.
// A port can omit this file entirely but should document the algorithm it
// contained for completeness (greedy ring synchronisation by squared distance).
