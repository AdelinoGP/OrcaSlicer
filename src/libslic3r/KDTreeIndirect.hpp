...
template<size_t ANumDimensions, typename ACoordType, typename ACoordinateFn> class KDTreeIndirect
{
public:
    ...
    // [COUPLING] Stores only indices; coordinate functor ties tree nodes to caller-owned point storage.
    ...
    KDTreeIndirect& operator=(KDTreeIndirect&& rhs)
    {
        ...
        coordinate = std::move(rhs.coordinate);
        return *this;
    }
    ...
    void build(std::vector<size_t>& indices)
    {
        if (indices.empty())
            ...
            // [MEMORY] Dense array layout improves cache locality; unused slots are marked with npos sentinels.
        ...
        // [STATE] Input order is consumed and discarded so callers cannot rely on post-build index vector content.
        indices.clear();
    }
    ...
    {
        // [INTENT] Uses split-plane distance to prune subtrees that cannot contain a closer point.
        ...
        return (dist * dist < search_radius + CoordType(EPSILON)) ?
                   ...
                   (dist > CoordType(0)) ? (unsigned int) (VisitorReturnMask::CONTINUE_RIGHT) :
                                           (unsigned int) (VisitorReturnMask::CONTINUE_LEFT);
    }

    // Visitor is supposed to return a bit mask of VisitorReturnMask.
    template<typename Visitor> void visit(Visitor& visitor) const { visit_recursive(0, 0, visitor); }
...
private:
    ...
    void build_recursive(std::vector<size_t>& input, size_t node, const size_t dimension, const size_t left, const size_t right)
    {
        ...
        // [INTENT] Median partitioning maintains near-balanced depth for predictable nearest-neighbor query cost.
    ...
    {
        // [HAZARD] Quickselect is unstable on equal keys, so tie ordering may differ across platforms/builds.
        while (left < right) {
            size_t    center = (left + right) / 2;
            CoordType pivot;
            {
    ...
    template<typename Visitor> void visit_recursive(size_t node, size_t dimension, Visitor& visitor) const
    {
        assert(!m_nodes.empty());
        if (node >= m_nodes.size() || m_nodes[node] == npos)
            return;

        // [CONCURRENCY] Traversal itself is read-only, but external point storage must stay immutable during visit().
...
template<size_t K, typename PointType, typename FilterFn, size_t D, typename CoordT, typename CoordFn>
std::array<size_t, K> find_closest_points(const KDTreeIndirect<D, CoordT, CoordFn>& kdtree, const PointType& point, FilterFn filter)
{
    // [INTENT] Keeps a fixed-size sorted candidate set to avoid heap churn during hot nearest-neighbor queries.
    ...
    struct Visitor
    {
        ...
        Visitor(const Tree& kdtree, const PointType& point, FilterFn filter) : kdtree(kdtree), point(point), filter(filter)
        {
            results.fill(std::make_pair(Tree::npos, std::numeric_limits<CoordT>::max()));
        }
        unsigned int operator()(size_t idx, size_t dimension)
        {
            if (this->filter(idx)) {
                ...
                auto it  = std::lower_bound(results.begin(), results.end(), res, [](auto& r1, auto& r2) { return r1.second < r2.second; });
...
std::array<size_t, K> find_closest_points(const KDTreeIndirect<D, CoordT, CoordFn>& kdtree, const PointType& point)
{
    return find_closest_points<K>(kdtree, point, [](size_t) { return true; });
}

template<typename PointType, typename FilterFn, size_t D, typename CoordT, typename CoordFn>
...
std::vector<size_t> find_nearby_points(const KDTreeIndirectType&                     kdtree,
                                       ...
                                       FilterFn                                      filter)
{
    // [INTENT] Radius search reuses the same pruning logic to collect all indices inside a spherical neighborhood.
    ...
    struct Visitor
    {
        ...
        Visitor(const KDTreeIndirectType& kdtree, const PointType& center, const CoordType& max_distance, FilterFn filter)
...
std::vector<size_t> find_nearby_points(const KDTreeIndirectType&                     kdtree,
                                       ...
                                       const typename KDTreeIndirectType::CoordType& max_distance)
{
    return find_nearby_points(kdtree, center, max_distance, [](size_t) { return true; });
}

// Find nearby points (spherical neighbourhood) using Euclidian metrics.
template<typename KDTreeIndirectType, typename PointType, typename FilterFn>
std::vector<size_t> find_nearby_points(const KDTreeIndirectType& kdtree, const PointType& bb_min, const PointType& bb_max, FilterFn filter)
{
    struct Visitor
    {
        const KDTreeIndirectType& kdtree;
        ...
        Visitor(const KDTreeIndirectType& kdtree, const PointType& bbmin, const PointType& bbmax, FilterFn filter)
            : kdtree(kdtree), bb_min{bbmin}, bb_max{bbmax}, filter(filter)
        {}
        unsigned int operator()(size_t idx, size_t dimension)
        {
            unsigned int ret = static_cast<unsigned int>(VisitorReturnMask::CONTINUE_LEFT) |
