...
// [INTENT] Common types, settings aggregates, and utility functions shared by the
// tree support pipeline (TreeSupport.cpp, TreeSupport3D.cpp, TreeModelVolumes.cpp).
// Defines two large config structs:
//   TreeSupportMeshGroupSettings — raw config extraction from PrintObject (per-object)
//   TreeSupportSettings          — derived algorithmic constants computed once from above
// Also defines:
//   InterfacePlacer              — mutex-guarded interface/contact layer allocator
//   InterfacePreference          — enum for support-interface overlap resolution
//   LineStatus                   — enum for branch reachability classification
//   layer_z / layer_idx_ceil / layer_idx_floor / layer_initialize — Z↔layer-index helpers
//
// [STATE] TreeSupportSettings::soluble is an inline static (global) bool shared across
//   all instances in the process (H894). Multi-object prints mixing soluble/non-soluble
//   support will have the last writer's value applied to all.
//
// [COUPLING] TreeSupportMeshGroupSettings constructor reads PrintObjectConfig,
//   SlicingParameters, and PrintConfig directly — tight coupling to the print config
//   schema. Any config field rename requires updating the constructor.
...
struct TreeSupportMeshGroupSettings
{
    ...
    explicit TreeSupportMeshGroupSettings(const PrintObject& print_object)
    {
        ...
        for (size_t region_id = 0; region_id < print_object.num_printing_regions(); ++region_id) {
            ...
            external_perimeter_width  = std::max<coordf_t>(external_perimeter_width,
                                                           region.flow(print_object, frExternalPerimeter, config.layer_height).width());
        }
        ...
        this->support_bottom_height = this->support_bottom_enable ?
                                          (config.support_interface_bottom_layers.value > 0 ? config.support_interface_bottom_layers.value :
                                                                                              config.support_interface_top_layers.value) *
                                              this->layer_height :
                                          0;
        ...
        this->support_tree_angle_slow            = std::clamp<double>(config.tree_support_angle_slow * M_PI / 180., 0.,
        ...
        this->support_tree_tip_diameter = std::clamp(scaled<coord_t>(config.tree_support_tip_diameter.value), (coord_t) 0,
    ...
    // The thickness of the support floors. This controls the number of dense layers that are printed on top of places of a model on which
    // support rests.
    ...
    //  When checking where there's model above and below the support, take steps of the given height. Lower values will slice slower, while
    //  higher values may cause normal support to be printed in some places where there should have been support interface.
    ...
    // Amount of offset applied to all support polygons in each layer. Positive values can smooth out the support areas and result in more
    // sturdy support.
    ...
    // The maximum angle of the branches, when the branches have to avoid the model. Use a lower angle to make them more vertical and more
    // stable. Use a higher angle to be able to have more reach. minimum: 0, minimum warning: 20, maximum: 89, maximum warning": 85
    double support_tree_angle{60. * M_PI / 180.};
    // Tree Support Branch Diameter Angle
    // The angle of the branches' diameter as they gradually become thicker towards the bottom. An angle of 0 will cause the branches to
    // have uniform thickness over their length. A bit of an angle can increase stability of the tree support. minimum: 0, maximum: 89.9999,
    // maximum warning: 15
    ...
    // The diameter of the thinnest branches of tree support. Thicker branches are more sturdy. Branches towards the base will be thicker
    // than this. minimum: 0.001, minimum warning: support_line_width * 2
    ...
    // The preferred angle of the branches, when they do not have to avoid the model. Use a lower angle to make them more vertical and more
    // stable. Use a higher angle for branches to merge faster. minimum: 0, minimum warning: 10, maximum: support_tree_angle, maximum
    // warning: support_tree_angle-1
    double support_tree_angle_slow{50. * M_PI / 180.};
    // Tree Support Diameter Increase To Model
    // The most the diameter of a branch that has to connect to the model may increase by merging with branches that could reach the
    // buildplate. Increasing this reduces print time, but increases the area of support that rests on model minimum: 0
    ...
    // How tall a branch has to be if it is placed on the model. Prevents small blobs of support. This setting is ignored when a branch is
    // supporting a support roof. minimum: 0, maximum warning: 5
    ...
    // A higher value results in better overhangs but the supports are harder to remove, thus it is recommended to enable top support
    // interfaces instead of a high branch density value if dense interfaces are needed. 5%-35%
    ...
    // minimum: min_wall_line_width, minimum warning: min_wall_line_width+0.05, maximum_value: support_tree_branch_diameter, value:
    // support_line_width
...
/*!
 * \brief This struct contains settings used in the tree support. Thanks to this most functions do not need to know of meshes etc. Also
...
public:
    ...
    explicit TreeSupportSettings(const TreeSupportMeshGroupSettings& mesh_group_settings, const SlicingParameters& slicing_params)
        ...
        , // The actual radius is 50 microns larger as the resulting branches will be increased by 50 microns to avoid rounding errors
          // effectively increasing the xydistance
        ...
        , support_bottom_layers(
              mesh_group_settings.support_bottom_enable ? (mesh_group_settings.support_bottom_height + layer_height / 2) / layer_height : 0)
        ,
        ...
        , increase_radius_until_layer(increase_radius_until_radius <= branch_radius ?
                                          ...
                                          (increase_radius_until_radius - branch_radius) / branch_radius_increase_per_layer)
        ...
        , min_feature_size(mesh_group_settings.min_feature_size)
    {
        ...
        if (TreeSupportSettings::soluble) {
            // safeOffsetInc can only work in steps of the size xy_min_distance in the worst case => xy_min_distance has to be a bit larger
            // than 0 in this worst case and should be large enough for performance to not suffer extremely When for all meshes the z bottom
            // and top distance is more than one layer though the worst case is xy_min_distance + min_feature_size This is not the best
            // solution, but the only one to ensure areas can not lag though walls at high maximum_move_distance.
        ...
        //            "support_area_overwrite_interface_area", InterfacePreference::SupportAreaOverwritesInterface }, {
        //            "interface_area_overwrite_support_area", InterfacePreference::InterfaceAreaOverwritesSupport }, {
        //            "support_lines_overwrite_interface_area", InterfacePreference::SupportLinesOverwriteInterface }, {
        //            "interface_lines_overwrite_support_area", InterfacePreference::InterfaceLinesOverwriteSupport }, { "nothing",
        //            InterfacePreference::Nothing } }; interface_preference =
        //            interface_map.at(mesh_group_settings.get<std::string>("support_interface_priority"));
    ...
    // [HAZARD] soluble is an inline static (i.e., global) bool shared across ALL
    // TreeSupportSettings instances in the process (H894). In a multi-object print
    // where some objects use soluble support and others do not, the last constructor
    // to run wins. Objects processed before the soluble-object constructor will have
    // had their avoidance zones computed with the wrong value.
...
public:
    // [INTENT] Equality used for cache invalidation in TreeModelVolumes: two
    // TreeSupportSettings are "equal" iff reusing cached avoidance/collision zones
    // is valid (no recomputation needed).
    // [HAZARD] Does NOT compare the embedded `settings` (TreeSupportMeshGroupSettings)
    //   beyond the fields already mirrored into TreeSupportSettings (H895). Two
    //   settings objects that differ only in infill-related fields inside `settings`
    //   will be considered equal, causing incorrect cache hits and wrong infill
    //   generation in interface layers.
    bool operator==(const TreeSupportSettings& other) const
    {
        return branch_radius == other.branch_radius && tip_layers == other.tip_layers &&
               ...
               xy_distance - xy_min_distance ==
                   other.xy_distance - other.xy_min_distance && // if the delta of xy_min_distance and xy_distance is different the
                                                                // collision areas have to be recalculated.
               support_rests_on_model == other.support_rests_on_model &&
               increase_radius_until_layer == other.increase_radius_until_layer && min_dtt_to_model == other.min_dtt_to_model &&
               ...
               z_distance_bottom_layers == other.z_distance_bottom_layers && support_line_width == other.support_line_width &&
               support_line_spacing == other.support_line_spacing &&
               support_roof_line_width ==
                   other.support_roof_line_width && // can not be set on a per-mesh basis currently, so code to enable processing different
                                                    // roof line width in the same iteration seems useless.
               ...
               roof_pattern == other.roof_pattern && // can not be set on a per-mesh basis currently, so code to enable processing different
                                                     // roof patterns in the same iteration seems useless.
               support_roof_angles == other.support_roof_angles &&
               // support_infill_angles == other.support_infill_angles &&
               increase_radius_until_radius == other.increase_radius_until_radius && support_bottom_layers == other.support_bottom_layers &&
               layer_height == other.layer_height && z_distance_top_layers == other.z_distance_top_layers &&
        ...
        // The infill class now wants the settings object and reads a lot of settings, and as the infill class is used to calculate support
        // roof lines for interface-preference. Not all of these may be required to be identical, but as I am not sure, better safe than sorry
#if 0
...
// [INTENT] Dead code — permanently disabled by #if 0.
// [HAZARD] getActualZ() contains an operator precedence bug (H898):
//   `(layer_idx - known_z.size()) * layer_height + known_z.size() ? known_z.back() : 0`
// The `+` binds tighter than `?:`, so the ternary tests the sum as the condition,
// not the array bounds as intended. Re-enabling this block would produce wrong Z
// values whenever the sum evaluates non-zero.
#if 0
    /*!
...
// [INTENT] Returns the minimum polygon area (in scaled² units) below which a support
// polygon is considered negligible and can be discarded without affecting quality.
inline double tiny_area_threshold() { return sqr(scaled<double>(0.001)); }

// [INTENT] Diagnostic error reporter used during tree support generation. Prints to
// stdout unconditionally; on Windows with TREE_SUPPORT_SHOW_ERRORS_WIN32 defined,
// also shows a MessageBoxA for the first critical/non-critical error.
// [HAZARD] Permanently retained "public beta" scaffolding — the `// todo Remove!`
//   comment is from Thomas Rahm's original port and was never addressed (H897).
//   In release builds with no debug console, printf output is silently discarded.
//   The MessageBoxA path is disabled in normal builds so there is no user-visible
//   feedback for internal tree support errors in production.
...
{ // todo Remove!  ONLY FOR PUBLIC BETA!!
    ...
    if (show)
        MessageBoxA(nullptr,
                    std::string("TreeSupport_2 MOD detected an error while generating the tree support.\nPlease report this back to me "
                                "with profile and model.\nRevision 5.0\n" +
                                std::string(message) + "\n" + bugtype)
                        .c_str(),
...
// [INTENT] Converts a layer index to a Z coordinate in mm. For raft layers,
// returns the pre-computed Z from config.raft_layers[]; for object layers
// uses uniform layer_height arithmetic from object_print_z_min.
// [COUPLING] Depends on config.raft_layers being populated correctly by
// TreeSupportSettings constructor (requires raft_layers() > 0 check first).
inline double layer_z(const SlicingParameters& slicing_params, const TreeSupportSettings& config, const size_t layer_idx)
{
    return layer_idx >= config.raft_layers.size() ? slicing_params.object_print_z_min + slicing_params.first_object_layer_height +
                                                        (layer_idx - config.raft_layers.size()) * slicing_params.layer_height :
                                                    config.raft_layers[layer_idx];
}
// [INTENT] Returns the LOWEST (ceiling/most-conservative) layer index whose Z is >= z.
// Used for collision queries: "at which layer does the object first appear above Z?"
inline LayerIndex layer_idx_ceil(const SlicingParameters& slicing_params, const TreeSupportSettings& config, const double z)
{
    return LayerIndex(config.raft_layers.size()) +
           std::max<LayerIndex>(0, ceil((z - slicing_params.object_print_z_min - slicing_params.first_object_layer_height) /
                                        slicing_params.layer_height));
}
// [INTENT] Returns the HIGHEST (floor/most-permissive) layer index whose Z is <= z.
// Symmetric counterpart to layer_idx_ceil; used for queries like "last layer fully below Z".
// [COUPLING] Comment says "Highest collision layer" — meaning the highest index layer that
// is still at or below z, i.e., the floor direction. Naming is potentially confusing;
// in refactoring rename to layer_idx_at_or_below() for clarity.
inline LayerIndex layer_idx_floor(const SlicingParameters& slicing_params, const TreeSupportSettings& config, const double z)
{
    return LayerIndex(config.raft_layers.size()) +
           std::max<LayerIndex>(0, floor((z - slicing_params.object_print_z_min - slicing_params.first_object_layer_height) /
                                         slicing_params.layer_height));
}

// [INTENT] Fills in print_z, bottom_z, and height fields of a newly-allocated
// SupportGeneratorLayer based on layer_idx, delegating to layer_z() for the coordinate.
// [STATE] Layer at idx==0 gets bottom_z=0 (below raft or first object layer).
// [COUPLING] Must be called after layer_storage.allocate*() assigns the layer type;
// layer_initialize only sets geometry, not type.
...
// [INTENT] Thread-UNSAFE convenience: allocates a SupportGeneratorLayer from the arena
// and immediately initializes its geometry. Caller is responsible for holding
// SupportGeneratorLayerStorage's mutex (or guaranteeing single-thread access).
// [CONCURRENCY] "unguarded" suffix signals that NO lock is acquired here.
// Must only be called from within a critical section (e.g., inside InterfacePlacer
// methods that hold m_mutex_layer_storage, or from a single-threaded context).
...
// [INTENT] Thread-SAFE convenience wrapper around layer_allocate_unguarded; acquires
// SupportGeneratorLayerStorage's internal mutex before delegating to allocate().
// Use this variant when the caller does NOT already hold the storage lock.
// [COUPLING] Internally calls SupportGeneratorLayerStorage::allocate() which locks;
// never call from within an existing lock on the same storage (deadlock risk).
inline SupportGeneratorLayer& layer_allocate(SupportGeneratorLayerStorage& layer_storage,
                                             SupporLayerType               layer_type,
                                             const SlicingParameters&      slicing_params,
                                             const TreeSupportSettings&    config,
                                             size_t                        layer_idx)
...
// [INTENT] Manages concurrent writes of interface/contact layers during tree support
// generation. generate_initial_areas() runs per-layer in a TBB parallel_for; multiple
// threads may call add_roof()/add_roofs()/add_roof_build_plate() simultaneously.
// InterfacePlacer serializes writes via m_mutex_layer_storage, then delegates to
// add_roof_unguarded() which does the actual layer allocation + polygon append.
// [MEMORY] Holds non-owning references to externally-owned layer vectors (top_contacts,
// top_interfaces, top_base_interfaces) and the arena (layer_storage). Lifetime of
// InterfacePlacer must not exceed the lifetime of any referenced container.
// [CONCURRENCY] Copy constructor exists so TBB can copy the placer per-task, but all
// copies share the SAME mutex by reference — meaning copies are NOT independent;
// locking one copy blocks all. This is intentional: they share the same output vectors.
// [COUPLING] dtt_roof (distance-to-tip in roof layers) selects which output vector to
// write into: 0 → top_contacts, 1..num_top_interface_layers_only → top_interfaces,
// else → top_base_interfaces. Threshold logic mirrors SupportParameters accessors.
// Used by generate_initial_areas() in parallel by multiple layers.
class InterfacePlacer
{
public:
    InterfacePlacer(const SlicingParameters&      slicing_parameters,
    ...
    {}
    // [CONCURRENCY] Copy constructor shares all references including the output vectors.
    // Copies are NOT isolated workers — they write to the same destination and share
    // the same mutex. TBB copies are safe only because m_mutex_layer_storage is a
    // per-instance std::mutex, not a shared_ptr; each copy has its OWN mutex.
    // [HAZARD] This means copies do NOT share the mutex — each copy has independent
    // locking! Two copies writing to top_contacts[layer_idx] simultaneously would race.
    // Safe only if TBB assigns disjoint layer indices to distinct copies.
    ...
    const SupportParameters&   support_parameters;
    const TreeSupportSettings& config;
    // [INTENT] Escape hatch giving mutable access to top_contacts from outside; used
    // in finalize_interface_and_support_areas() after parallel phase is complete.
    SupportGeneratorLayersPtr& top_contacts_mutable() { return this->top_contacts; }

public:
    // [INTENT] Bulk-insert a vector of roof polygon layers, one per descending layer
    // index from insert_layer_idx downward. Skips empty polygon sets silently.
    // [CONCURRENCY] Acquires m_mutex_layer_storage for the entire loop body.
    ...
    // [INTENT] Single-layer roof insertion; thread-safe wrapper around add_roof_unguarded.
    ...
    // [INTENT] Special-case insertion at layer 0 (build plate contact). Clamps dtt_roof
    // to num_top_interface_layers so it never selects top_base_interfaces for
    // build-plate contacts (they always classify as TopInterface at most).
    ...
    // [INTENT] Core insertion logic — must only be called while m_mutex_layer_storage
    // is held. Selects the output vector by dtt_roof distance, lazily allocates the
    // SupportGeneratorLayer if none exists at insert_layer_idx, then appends polygons.
    // Merging (union) of accumulated polygons is deferred to
    // finalize_interface_and_support_areas() after the parallel phase.
    // [COUPLING] Uses SupporLayerType::TopContact (typo preserved from SupportLayer.hpp)
    // and SupporLayerType::TopInterface — both names carry the canonical typo.
    void add_roof_unguarded(Polygons&& new_roofs, const size_t insert_layer_idx, const size_t dtt_roof)
    {
        ...
        SupportGeneratorLayersPtr& layers = dtt_roof == 0                                                  ? this->top_contacts :
                                            dtt_roof <= support_parameters.num_top_interface_layers_only() ? this->top_interfaces :
...
// [INTENT] Classifies the status of a support line segment during propagation.
// TO_MODEL* variants indicate the line reaches the model surface (not the build plate).
// GRACIOUS = the line reached the model at a gentle enough angle to be acceptable.
// GRACIOUS_SAFE = gracious and within a collision-free margin.
// TO_BP* variants indicate the line reaches the build plate directly.
// TO_BP_SAFE = reaches build plate without touching the model at all.
// INVALID = line was rejected or never assigned (default/error sentinel).
// [COUPLING] Used in TreeSupport3D.cpp drop-to-floor propagation loops; value affects
// which branch handles the segment (model-touching vs. floor-touching paths).
