...
// [INTENT] SupportParameters.hpp — configuration aggregate for the entire support generation system.
// `SupportParameters` is a large POD-like struct that pre-computes all support geometry decisions
// from PrintObject config, PrintConfig, and SlicingParameters at construction time.
// It is passed by const-ref to virtually every function in the support generation pipeline.
//
// Computed values include:
//   - Solubility flags (soluble_interface, soluble_interface_non_soluble_base)
//   - Layer counts (num_top/bottom_interface_layers, num_top/bottom_base_interface_layers)
//   - Flow objects (first_layer_flow, support_material_flow, interface_flow, bottom_interface_flow, raft_interface_flow, ironing_flow)
//   - XY gap distances (gap_xy, gap_xy_first_layer)
//   - Density ratios (interface_density, raft_interface_density, support_density)
//   - Fill patterns (base_fill_pattern, interface_fill_pattern, raft_interface_fill_pattern, contact_fill_pattern)
//   - Raft angles (raft_angle_1st_layer, raft_angle_base, raft_angle_interface)
//   - Tree support wall geometry (with_sheath, tree_branch_diameter_double_wall_area_scaled)
//   - Ironing parameters (ironing, ironing_flow, ironing_spacing, ironing_pattern)
//   - Support style override logic (support_style, handling smsDefault)
//
// [MEMORY] SupportParameters is constructed once per PrintObject and owned by the caller.
//          It stores Flow objects by value. All coordf_t / float fields are value types.
//          No heap allocation beyond Flow internals (if any).
//
// [CONCURRENCY] No mutable state after construction. Safe to pass by const-ref across threads.
//
// [HAZARD] H879: `nozzle_diameter` is obtained via `print_config.nozzle_diameter.get_at(
//          object_config.support_interface_filament - 1)` (line ~161). If
//          `object_config.support_interface_filament.value == 0` (meaning "use active extruder"),
//          the subtraction produces `0 - 1 = SIZE_MAX` (unsigned underflow).
//          `get_at()` clamps out-of-range indices to index 0, so in practice this silently
//          returns the first extruder nozzle diameter instead of asserting.
//          This silent-clamping pattern is the standard `get_at()` convention (documented in H-IDs
//          for other files), but here the -1 offset makes it more likely to trigger.
//
// [HAZARD] H880: Line 40 has a typo: `differnt_support_interface_filament` (missing 'e').
//          The variable is used correctly but any text search or automated refactoring that
//          looks for the canonical spelling `different_support_interface_filament` will not
//          find it.
//
// [HAZARD] H881: `support_layer_height_min` is initialised from `print_config.min_layer_height.values`
//          and then further reduced by iterating over `object.layers()`.  The loop mixes
//          `scaled<coord_t>` units and raw `coordf_t` values: the initial value is in scaled
//          units (`scaled<coord_t>(0.01)`), but the loop body compares with `layer->height`
//          which is in unscaled mm.  If any layer has height < 0.01 mm the comparison
//          `std::min(this->support_layer_height_min, std::max(0.01, layer->height))` will
//          update the field, but the field is in a different unit system than the height.
//          This is a latent unit-system confusion: support_layer_height_min appears to be
//          stored in scaled units at init but used in unscaled context downstream.
//          NOTE: on re-read, `support_layer_height_min` declaration is `coordf_t` (unscaled),
//          but `scaled<coord_t>(0.01)` returns a scaled integer value.  The field receives
...
namespace Slic3r {
// [INTENT] SupportParameters — single-construction configuration aggregate for support generation.
// All fields are computed in the constructor from PrintObject, PrintConfig, SlicingParameters.
// Default constructor is deleted; must be constructed from a PrintObject.
    ...
    SupportParameters(const PrintObject& object)
    {
...

        {
            ...
            this->num_bottom_interface_layers = object_config.support_interface_bottom_layers < 0 ?
                                                    ...
                                                    object_config.support_interface_bottom_layers;
            ...
            if (this->soluble_interface_non_soluble_base) {
                ...
                // Note: support materials (such as Supp.W) can't be used as support base now, so support interface and base are still using
                // different filaments even if support_filament==0
        ...
        for (size_t region_id = 0; region_id < object.num_printing_regions(); ++region_id) {
            ...
            external_perimeter_width  = std::max(external_perimeter_width,
                                                 coordf_t(region.flow(object, frExternalPerimeter, slicing_params.layer_height).width()));
        ...
        this->support_material_bottom_interface_flow = slicing_params.soluble_interface || !object_config.thick_bridges ?
                                                           ...
                                                           Flow::bridging_flow(bridge_flow_ratio *
                                                                                   this->support_material_interface_flow.nozzle_diameter(),
                                                                               this->support_material_interface_flow.nozzle_diameter());

        this->can_merge_support_regions = object_config.support_filament.value == object_config.support_interface_filament.value;
        if (!this->can_merge_support_regions &&
            (object_config.support_filament.value == 0 || object_config.support_interface_filament.value == 0)) {
            // One of the support extruders is of "don't care" type.
            auto object_extruders = object.object_extruders();
            if (object_extruders.size() == 1 &&
                *object_extruders.begin() ==
                    std::max<unsigned int>(object_config.support_filament.value, object_config.support_interface_filament.value))
        ...
        this->interface_spacing = (this->ironing ? 0 : object_config.support_interface_spacing.value) +
                                  this->support_material_interface_flow.spacing();
        this->interface_density = std::min(1., this->support_material_interface_flow.spacing() / this->interface_spacing);
        // Orca: Force solid support interface when using support ironing
        double raft_interface_spacing = (this->ironing ? 0 : object_config.support_interface_spacing.value) +
                                        this->raft_interface_flow.spacing();
        ...
        this->base_fill_pattern                = support_pattern == smpHoneycomb                   ? ipHoneycomb :
        ...
        if (object_config.support_interface_pattern == smipGrid)
            ...
            this->contact_fill_pattern = (object_config.support_interface_pattern == smipAuto && slicing_params.soluble_interface) ||
        ...
        tree_branch_diameter_double_wall_area_scaled = object_config.tree_support_wall_count.value > 1 ?
                                                           0.1 :
                                                       object_config.tree_support_wall_count.value == 0 ?
                                                           0.25 * sqr(scaled<double>(5.0)) * M_PI :
                                                           std::numeric_limits<double>::max();

        support_style = object_config.support_style;
        if (support_style != smsDefault) {
            if ((support_style == smsSnug || support_style == smsGrid) && is_tree(object_config.support_type))
