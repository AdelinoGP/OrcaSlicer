// [INTENT] ExtrusionEntityCollection: owns a heterogeneous vector of heap-allocated
//          ExtrusionEntity* children (paths, loops, multi-paths, nested collections).
//          Acts as both a container and a node in the extrusion entity tree. Used as the
//          top-level store for layer extrusions (LayerRegion::perimeters, fills, etc.).
// [STATE] `entities`: owning vector of raw pointers — clear() and destructor call delete.
//         `no_sort`: when true, chained_path_from() returns *this unchanged (ordering locked).
//         `is_reverse`: private flag controlling can_reverse() — defaults to true.
//          set_reverse() sets it to false (one-way latch, like ExtrusionPath).
// [MEMORY] Deep-copy semantics: copy ctor/operator= call clone() on every entity. Move ctor
//          transfers pointer ownership. This is a frequent source of allocations — large
//          prints copy entire layer entity trees during perimeter generation.
// [CONCURRENCY] Not thread-safe. Per-layer collections are accessed from single-threaded
//               layer processing. No sharing between layers.
// [COUPLING] Depends on ExtrusionEntity.hpp, ShortestPath.hpp (chain_and_reorder). The
//            flatten() method recursively flattens nested collections — callers that
//            iterate entities directly miss nested content unless flattened first.
// [HAZARD H735] `append(ExtrusionEntitiesPtr &&src)` moves pointer ownership but does NOT
//               clone the entities — it takes ownership of the raw pointers from `src`.
//               This is semantically different from `append(const ExtrusionEntitiesPtr &)`,
//               which clones. Mixing the two variants on the same source is a double-free
//               hazard. A port must use a consistent ownership model (e.g. unique_ptr).
...
// [INTENT] filter_by_extrusion_role_in_place: removes entities whose role doesn't match
//          `role` from the vector (in-place, using erase-remove idiom). Does nothing for
//          erMixed. Note: removed elements are NOT deleted — caller retains ownership.
//          filter_by_extrusion_role: returns a filtered shallow copy (shared raw pointers —
//          NOT cloned). Ownership remains with the original vector.
// [HAZARD H736] The filtered copy contains non-owning raw pointers into the original vector.
//               If the original vector is cleared or destroyed, the filtered copy becomes a
//               collection of dangling pointers. The comment in the header warns about this,
//               but the API surface provides no lifetime guarantee mechanism.
...
public:
    ...
    ExtrusionEntityCollection(const ExtrusionEntityCollection& other) : no_sort(other.no_sort), is_reverse(other.is_reverse)
    {
    ...
    /// You should be iterating over flatten().entities if you are interested in the underlying ExtrusionEntities (and don't care about
    /// hierarchy).
    /// \param preserve_ordering Flag to method that will flatten if and only if the underlying collection is sortable when True (default:
    /// False).
