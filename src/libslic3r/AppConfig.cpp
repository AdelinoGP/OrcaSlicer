// [INTENT] AppConfig: singleton persistent application configuration store.
// Manages all user preferences, recent files, vendor/model enablement, calibration data,
// and local machine registry. Two serialization paths exist:
//   - JSON (active, controlled by #define USE_JSON_CONFIG)
//   - INI via boost::property_tree (legacy, still compiled when USE_JSON_CONFIG undefined)
// [STATE] m_storage: map<section, map<key, string>> — all config values are stringly-typed.
//         m_vendors: map<vendor, map<model, set<variant>>> — separate from m_storage.
//         m_filament_presets / m_filament_colors / m_filament_multi_colors / m_filament_color_types:
//           vectors stored outside m_storage — must be kept synchronized on load/save.
// [CONCURRENCY] save() enforces main-thread-only via is_main_thread_active(); no mutex on reads.
//               Any worker thread reading m_storage while main thread calls save() is a data race.
// [COUPLING] Deeply coupled to GUI layer (EAppMode::Editor vs GCodeViewer mode switches ~50 branches).
//            BBL-specific: Bambu IoT environment, AMS sync modes, H2D machine settings baked in.
...
// [INTENT] set_defaults: establish missing keys with their compile-time defaults.
// Called both on fresh install and on load() after parsing to fill gaps from older configs.
// [HAZARD H1118] set_defaults() is ~430 lines with dozens of #ifdef guards for features
// (SUPPORT_AUTO_CENTER, SUPPORT_BACKGROUND_PROCESSING, SUPPORT_REMEMBER_OUTPUT_PATH, etc.)
// that are NOT defined in the OrcaSlicer build — those blocks are always-skipped dead code.
// The active code is interleaved with the dead code, making it hard to see what actually runs.
// [COUPLING] EAppMode branching means Editor and GCodeViewer get different defaults —
// any port must replicate both branches.
...
#ifdef WIN32
// [INTENT] appconfig_md5_hash_line: produce a trailing comment line with MD5 checksum
// of the config data. Used on Windows only to detect file corruption between runs.
// [HAZARD H1119] Uses boost::uuids::detail::md5 which is an internal/unstable namespace.
// Boost docs explicitly state it may change. The comment here acknowledges this.
// Not a portable hashing approach; any port should use a stable crypto library instead.
...
static bool verify_config_file_checksum(boost::nowide::ifstream& ifs)
{
    ...
    if (size_t last_comment_pos = whole_config.find_last_of('#'); last_comment_pos != std::string::npos) {
        // Split read config into two parts, one with checksum, and the second part is part with configuration from the checksum was
        // computed. Verify existence and validity of the MD5 checksum line at the end of the file. When the checksum isn't found, the
        // checksum was not saved correctly, it was removed or it is an older config file without the checksum. If the checksum is
        // incorrect, then the file was either not saved correctly or modified.
        if (std::string_view(whole_config.c_str() + last_comment_pos, whole_config.size() - last_comment_pos) ==
            appconfig_md5_hash_line({whole_config.data(), last_comment_pos}))
            return true;
    }
    return false;
}
...
// [INTENT] AppConfig::load (JSON path, active when USE_JSON_CONFIG defined):
// Parse the JSON config file into m_storage, m_vendors, m_filament_presets,
// m_printer_cali_infos, m_printer_settings, m_local_machines.
// [STATE] Dual path: USE_JSON_CONFIG active → JSON via nlohmann.
//         The legacy INI path (below #else) still compiles but is never reached.
// [HAZARD H1120] The two load() implementations (JSON and INI) have diverged:
//   - JSON path: "presets"/"filaments" stored as array, loaded back with filament_XX key scheme.
//   - INI path: flat key=value, no array concept.
//   Both call set_defaults() at the end, but their deserialization is structurally incompatible.
//   If USE_JSON_CONFIG is ever disabled, all multi-filament preset data will silently vanish.
// [MEMORY] m_filament_presets and m_filament_colors are NOT in m_storage — separate vectors.
//          They are loaded via iter.key() == "filament_presets" / "filament_colors" branches
//          but live in the "app" section's JSON object, not in top-level sections.
// [HAZARD H1121] Windows-only: load() strips content after last '}' to isolate JSON from MD5 line.
//   substr(last_pos+2) assumes exactly one newline between '}' and MD5 comment. If the file
//   was saved with CRLF line endings, last_pos+2 may point mid-line, causing parse failure.
    ...
    try {
        ...
        if (md5_str != right_string)
            BOOST_LOG_TRIVIAL(info)
                << "The configuration file " << AppConfig::loading_path()
        ...
        if (boost::filesystem::exists(backup_path)) {
            ...
            if (back_md5_str != back_right_string) {
                BOOST_LOG_TRIVIAL(error) << format(
                    "Both \"%1%\" and \"%2%\" are corrupted. It isn't possible to restore configuration from the backup.",
                    AppConfig::loading_path(), backup_path);
                backup_ifs.close();
                boost::filesystem::remove(backup_path);
            } else if (std::string error_message; copy_file(backup_path, AppConfig::loading_path(), error_message, false) != SUCCESS) {
                BOOST_LOG_TRIVIAL(error) << format("Configuration file \"%1%\" is corrupted. Failed to restore from backup \"%2%\": %3%",
                                                   AppConfig::loading_path(), backup_path, error_message);
                backup_ifs.close();
                boost::filesystem::remove(backup_path);
            } else {
                BOOST_LOG_TRIVIAL(info) << format(
                    "Configuration file \"%1%\" was corrupted. It has been succesfully restored from the backup \"%2%\".",
                    AppConfig::loading_path(), backup_path);
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            if (it.key() == MODELS_STR) {
                for (auto& j_model : it.value()) {
                    ...
                    if (!unescape_strings_cstyle(j_model["nozzle_diameter"], variants)) {
                        continue;
                    }
                ...
                for (auto& calis_j : it.value()) {
                    ...
                    if (calis_j.contains("cache_flow_rate_calibration_type"))
                        cali_info.cache_flow_rate_calibration_type = static_cast<FlowRatioCalibrationType>(
                            calis_j["cache_flow_rate_calibration_type"].get<int>());
...
// [INTENT] AppConfig::save (JSON path): serialize all in-memory state to JSON config file.
// Uses PID-suffixed temp file + rename for crash safety. On Windows, also writes MD5 checksum
// and makes a .bak backup before renaming (Windows rename is not atomic).
// [CONCURRENCY] Must only be called from main thread; throws CriticalException otherwise.
// [HAZARD H1122] m_filament_presets and m_filament_colors are serialized into the "app" section
// of the JSON (j["app"]["filament_presets"]), NOT as top-level keys — but on load() they are
// read from inside the "app" JSON object. This asymmetry means the round-trip is correct, but
// is fragile: a refactor that moves them to top-level would silently break existing configs.
// [COUPLING] save() hardcodes "header", "calis", "orca_presets", "local_machines",
// "models", and "presets"/"filaments" as special-cased keys — any new structured data
// must add another special-case branch in both save() and load().
void AppConfig::save()
{
    ...
    for (const auto& category : m_storage) {
        ...
        if (category.first == SECTION_FILAMENTS) {
            ...
            for (const auto& kvp : category.second) {
                if (boost::starts_with(kvp.first, "filament") && kvp.first != "filament_colors" && kvp.first != "filament_multi_colors" &&
                    kvp.first != "filament_color_types") {
    ...
    c.close();
    if (c.fail()) {
        BOOST_LOG_TRIVIAL(error) << "Failed to write new configuration to " << path_pid
    ...
    if (copy_file(path_pid, backup_path, error_message, false) != SUCCESS)
        BOOST_LOG_TRIVIAL(error) << "Copying from " << path_pid << " to " << backup_path
                                 << " failed. Failed to create a backup configuration.";
#endif
        ...
        if (boost::filesystem::exists(backup_path)) {
            ...
            if (!verify_config_file_checksum(backup_ifs)) {
                ...
                try {
                    ...
                    BOOST_LOG_TRIVIAL(info) << format(
                        "Failed to parse configuration file \"%1%\" after it has been restored from backup: %2%", AppConfig::loading_path(),
                        ex.what());
                }
            }
        } else
    ...
    for (const auto& section : tree) {
        if (section.second.empty()) {
            ...
            for (const auto& kvp : section.second) {
                if (!boost::starts_with(kvp.first, MODEL_PREFIX)) {
                    continue;
                }
...
// [INTENT] get_variant / set_variant / set_vendors: manage the vendor → model → nozzle-variant
// enablement map (m_vendors). These are saved to the "models" array in JSON.
// [COUPLING] Nozzle diameter variants are stored as a set<string> inside a nested map —
// no structural validation of nozzle values (e.g. "0.4" vs 0.4 float) is performed here.
...
{
    auto iter = std::find_if(m_printer_cali_infos.begin(), m_printer_cali_infos.end(),
                             [&cali_info](const PrinterCaliInfo& cali_info_item) { return cali_info_item.dev_id == cali_info.dev_id; });
...
void AppConfig::set_recent_projects(const std::vector<std::string>& recent_projects)
{
    ...
    if (it == m_storage.end())
        it = m_storage
                 .insert(std::map<std::string, std::map<std::string, std::string>>::value_type("recent_projects",
...
void AppConfig::set_mouse_device(const std::string& name,
                                 ...
                                 bool               invert_roll)
{
    ...
    if (it == m_storage.end())
        it = m_storage
                 .insert(std::map<std::string, std::map<std::string, std::string>>::value_type(key, std::map<std::string, std::string>()))
                 .first;
...
void AppConfig::update_config_dir(const std::string& dir) { this->set("recent", "settings_folder", dir); }
...
void AppConfig::update_last_output_dir(const std::string& dir, const bool removable) { this->set("app", ("last_export_path"), dir); }
...
// [INTENT] get_country_code: map IoT region string to 2-letter code for network API.
// [HAZARD H1123] "Europe" maps to "US" — this is almost certainly a copy-paste bug
// (should map to "EU" or "Others"). Affects network endpoint routing for European users.
// Also has unreachable `return ""` after the else branch which always returns.
...
// [INTENT] save_custom_color_to_config: persist user-defined custom colors to "custom_color_list" section.
// [HAZARD H1124] Uses const_cast to mutate a const map reference returned by get_section():
//   auto data = get_section(...);   // returns const ref
//   auto data_modify = const_cast<std::map<...>*>(&data);
// This casts away const on a local copy — the const_cast is harmless but misleading.
// The intent should just be `auto data = get_section(...)` (non-const copy), but the code
// treats it as if &data points to internal storage. A port should simplify this pattern.
// Keys start at "10" (not "0") for map sort order — magic number with no comment.
...
std::string AppConfig::get_network_plugin_version() const { return get(SETTING_NETWORK_PLUGIN_VERSION); }

void AppConfig::set_network_plugin_version(const std::string& version) { set(SETTING_NETWORK_PLUGIN_VERSION, version); }
...
void AppConfig::clear_skipped_network_versions() { set(SETTING_NETWORK_PLUGIN_SKIPPED_VERSIONS, ""); }

bool AppConfig::is_network_update_prompt_disabled() const { return get_bool(SETTING_NETWORK_PLUGIN_UPDATE_DISABLED); }

void AppConfig::set_network_update_prompt_disabled(bool disabled) { set_bool(SETTING_NETWORK_PLUGIN_UPDATE_DISABLED, disabled); }

bool AppConfig::should_remind_network_update_later() const { return get_bool(SETTING_NETWORK_PLUGIN_REMIND_LATER); }

void AppConfig::set_remind_network_update_later(bool remind) { set_bool(SETTING_NETWORK_PLUGIN_REMIND_LATER, remind); }

void AppConfig::clear_remind_network_update_later() { set_bool(SETTING_NETWORK_PLUGIN_REMIND_LATER, false); }
...
std::string AppConfig::profile_update_url() const { return PROFILE_UPDATE_URL; }

bool AppConfig::exists() { return boost::filesystem::exists(config_path()); }
}; // namespace Slic3r
