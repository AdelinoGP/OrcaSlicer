...
// [INTENT] Token-level data model for the permissive Wavefront OBJ / MTL parser in `objparser.cpp`.
// These structures deliberately preserve source ordering and face/material boundaries instead of building
// `TriangleMesh` directly.
...
struct ObjVertex
{
    // [INTENT] One face-corner reference into the coordinate / texture / normal arrays.
    // `{-1, -1, -1}` is also used as the face delimiter in `ObjData::vertices`.
    int coordIdx;
    int textureCoordIdx;
    int normalIdx;
};
...
{
    return v1.coordIdx == v2.coordIdx && v1.textureCoordIdx == v2.textureCoordIdx && v1.normalIdx == v2.normalIdx;
}

struct ObjUseMtl
{
    // [STATE] `vertexIdxFirst` / `vertexIdxEnd` delimit the span of raw corners under this `usemtl`.
    // `face_start` / `face_end` are derived counters consumed by Orca's face-colour assignment.
    int         vertexIdxFirst;
    int         vertexIdxEnd{-1};
    int         face_start;
    int         face_end{-1};
...
{
    return v1.vertexIdxFirst == v2.vertexIdxFirst && v1.name.compare(v2.name) == 0;
}
...
{
    return v1.vertexIdxFirst == v2.vertexIdxFirst && v1.smoothingGroupID == v2.smoothingGroupID;
}
...
struct ObjData
{
    ...
    // [INTENT] Coordinates store x,y,z and optional r,g,b,a extensions emitted by some OBJ exporters.
    // [HAZARD] `OBJ_VERTEX_LENGTH` is part of the private binary cache ABI.
...
struct MtlData
{
    // Version of the data structure for load / store in the private binary format.
    int version;
    // [INTENT] Material-name lookup table used by higher-level OBJ import when resolving `usemtl` scopes.
    std::unordered_map<std::string, std::shared_ptr<ObjNewMtl>> new_mtl_unmap;
};
// [INTENT] Text parsers plus private binary cache helpers. `OBJ.cpp` builds TriangleMesh / Model on top of them.
