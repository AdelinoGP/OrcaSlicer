// [INTENT] Low-level Wavefront OBJ / MTL tokenizer used by `Format/OBJ.cpp`.
// Unlike the higher-level loader, this file stops at a flat intermediate representation (`ObjData`) that
// preserves raw tables, face boundaries, and material scopes.
//
// [STATE] Parsing appends into caller-provided `ObjData` / `MtlData` containers.
// [COUPLING] `OBJ.cpp` depends on the sentinel-delimited `ObjData::vertices` layout and on the material map
//            produced by `mtlparse()`.
// [HAZARD] The chunked readers ignore several per-line parse failures, so malformed lines may be skipped while
//          import still reports success.
// [HAZARD] `cur_mtl_name` below is global mutable state, making MTL parsing non-reentrant.
...
static bool obj_parseline(const char* line, ObjData& data)
{
    if (*line == 0)
        return true;
    // [INTENT] OBJ syntax is locale-independent and always uses `.` as decimal separator.
    ...
    case '#':
        ...
        break;
    case 'v': {
        // [INTENT] Parse one of OBJ's vertex record families (`v`, `vt`, `vn`, `vp`) into dense arrays so the
        // higher-level importer can resolve face references without reparsing strings.
    ...
    case 'f': {
        // [INTENT] Preserve variable-length faces as a flat stream of corners terminated by an all-`-1` sentinel.
        // This avoids nested allocations per face and lets `OBJ.cpp` triangulate later.
        // face
        EATWS();
        if (*line == 0)
            return false;
        ...
        while (*line != 0) {
            ...
            // [INTENT] OBJ negative indices count backward from the end of each attribute array. Convert them to
            // zero-based absolute indices immediately so downstream code sees one canonical representation.
    ...
    case 'm': {
        if (*(line++) != 't' || *(line++) != 'l' || *(line++) != 'l' || *(line++) != 'i' || *(line++) != 'b')
            return false;
    ...
    case 'u': {
        // [STATE] Material spans are tracked by vertex stream position and derived face counters so the later mesh
        // builder can map emitted triangles back to the active `usemtl` block.
        if (*(line++) != 's' || *(line++) != 'e' || *(line++) != 'm' || *(line++) != 't' || *(line++) != 'l')
            return false;
    ...
    default: BOOST_LOG_TRIVIAL(error) << "ObjParser: Unknown command: " << c1; break;
    }
...
static bool        mtl_parseline(const char* line, MtlData& data)
{
    if (*line == 0)
        return true;
    // [HAZARD] Shared parser state for the current material section; concurrent MTL parses would race here.
    assert(Slic3r::is_decimal_separator_point());
    // Ignore whitespaces at the beginning of the line.
    // FIXME is this a good idea?
    EATWS();
    ...
    case 'n': {
        // [INTENT] `newmtl` opens a mutable material record that subsequent properties populate in-place.
    ...
    case 'm': {
        if (*(line++) != 'a' || *(line++) != 'p' || *(line++) != '_' || *(line++) != 'K' || *(line++) != 'd')
            return false;
        EATWS();
...
bool objparse(const char* path, ObjData& data)
{
    ...
    try {
        ...
        size_t len     = 0;
        size_t lenPrev = 0;
        // [INTENT] Stream large OBJ files a chunk at a time; `lenPrev` carries an incomplete trailing line
        // into the next read so line tokenization stays correct without loading the whole file at once.
        while ((len = ::fread(buf + lenPrev, 1, 65536, pFile)) != 0) {
            ...
            for (size_t i = 0; i < len; ++i)
                if (buf[i] == '\r' || buf[i] == '\n') {
                    ...
                    while (*c == ' ' || *c == '\t')
                        ++c;
                    // [HAZARD] Parse errors are ignored here, so import can succeed with partially populated data.
...
bool mtlparse(const char* path, MtlData& data)
{
    ...
    try {
        ...
        size_t len     = 0;
        size_t lenPrev = 0;
        // [INTENT] Use the same chunked line scanner for MTL sidecars so the two parsers stay behaviorally aligned.
        while ((len = ::fread(buf + lenPrev, 1, 65536, pFile)) != 0) {
            len += lenPrev;
            size_t lastLine = 0;
            for (size_t i = 0; i < len; ++i)
...
bool objparse(std::istream& stream, ObjData& data)
{
    ...
    try {
        ...
        size_t len     = 0;
        size_t lenPrev = 0;
        // [INTENT] Stream overload for callers that already own an input stream; keeps exactly the same line
        // handling semantics as the FILE*-based path.
...
template<typename T> bool savevector(FILE* pFile, const std::vector<T>& v)
{
    size_t cnt = v.size();
    ::fwrite(&cnt, 1, sizeof(cnt), pFile);
    // [HAZARD] This raw dump assumes `T` is trivially copyable and layout-stable. It is only a private cache format.
...
template<typename T> bool savevectornameidx(FILE* pFile, const std::vector<T>& v)
{
    size_t cnt = v.size();
    ::fwrite(&cnt, 1, sizeof(cnt), pFile);
    for (size_t i = 0; i < cnt; ++i) {
...
template<typename T> bool loadvector(FILE* pFile, std::vector<T>& v)
{
    v.clear();
    size_t cnt = 0;
...
template<typename T> bool loadvectornameidx(FILE* pFile, std::vector<T>& v)
{
    v.clear();
    size_t cnt = 0;
    if (::fread(&cnt, sizeof(cnt), 1, pFile) != 1)
...
{
    // [INTENT] Persist the tokenized OBJ representation in a binary cache to avoid repeating text parse work.
    ...
    bool result = savevector(pFile, data.coordinates) && savevector(pFile, data.textureCoordinates) && savevector(pFile, data.normals) &&
                  savevector(pFile, data.parameters) && savevector(pFile, data.mtllibs) && savevectornameidx(pFile, data.usemtls) &&
                  savevectornameidx(pFile, data.objects) && savevectornameidx(pFile, data.groups) &&
                  savevector(pFile, data.smoothingGroups) && savevector(pFile, data.vertices);
...
{
    // [INTENT] Restore the private binary cache produced by `objbinsave()`.
    // [HAZARD] Only cache version 1 is accepted; there is no migration path.
    ...
    bool result = loadvector(pFile, data.coordinates) && loadvector(pFile, data.textureCoordinates) && loadvector(pFile, data.normals) &&
                  loadvector(pFile, data.parameters) && loadvector(pFile, data.mtllibs) && loadvectornameidx(pFile, data.usemtls) &&
                  loadvectornameidx(pFile, data.objects) && loadvectornameidx(pFile, data.groups) &&
                  loadvector(pFile, data.smoothingGroups) && loadvector(pFile, data.vertices);

    ::fclose(pFile);
    return result;
}

template<typename T> bool vectorequal(const std::vector<T>& v1, const std::vector<T>& v2)
{
    if (v1.size() != v2.size())
        return false;
...
{
    // [INTENT] Field-wise equality helper used to validate parser/cache round-trips.
    // FIXME ignore version number
    // version;

    return vectorequal(data1.coordinates, data2.coordinates) && vectorequal(data1.textureCoordinates, data2.textureCoordinates) &&
           vectorequal(data1.normals, data2.normals) && vectorequal(data1.parameters, data2.parameters) &&
           vectorequal(data1.mtllibs, data2.mtllibs) && vectorequal(data1.usemtls, data2.usemtls) &&
           vectorequal(data1.objects, data2.objects) && vectorequal(data1.groups, data2.groups) &&
