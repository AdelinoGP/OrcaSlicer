...
{
    // [MEMORY] The parser context is stack-owned by the load helper, but all raw pointers stored
    //          inside it are borrowed handles into `m_model` containers that may be invalidated if
    //          insertion ordering changes in a refactor.
    AMFParserContext(XML_Parser parser, DynamicPrintConfig* config, ConfigSubstitutionContext* config_substitutions, Model* model)
        : m_parser(parser), m_model(*model), m_config(config), m_config_substitutions(config_substitutions)
    {
        m_path.reserve(12);
    }
    ...
    struct Instance
    {
        ...
        {
            return deltax_set || deltay_set || deltaz_set || rx_set || ry_set || rz_set || scalex_set || scaley_set || scalez_set ||
    ...
    // [STATE] AMF constellation references are resolved indirectly: object ids first map to the
    //         created `ModelObject` index, then deferred instance transforms are replayed in
    //         endDocument() after the full XML stream has been seen.
...
void AMFParserContext::startElement(const char* name, const char** atts)
{
    ...
    case 1:
        if (strcmp(name, "metadata") == 0) {
            ...
            // [INTENT] Material blocks are preserved as `ModelMaterial` attribute bags keyed by the
            //          AMF material id. OrcaSlicer keeps the raw metadata but no longer rebuilds full
            //          print config overrides from it.
    ...
    if (m_path.back() == NODE_TYPE_METADATA) {
        ...
        case 4:
            if (m_path.back() == NODE_TYPE_DELTAX || m_path.back() == NODE_TYPE_DELTAY || m_path.back() == NODE_TYPE_DELTAZ ||
                ...
                m_path.back() == NODE_TYPE_SCALEX || m_path.back() == NODE_TYPE_SCALEY || m_path.back() == NODE_TYPE_SCALEZ ||
                m_path.back() == NODE_TYPE_SCALE || m_path.back() == NODE_TYPE_MIRRORX || m_path.back() == NODE_TYPE_MIRRORY ||
                m_path.back() == NODE_TYPE_MIRRORZ || m_path.back() == NODE_TYPE_PRINTABLE)
...
void AMFParserContext::endElement(const char* /* name */)
{
    ...
    case NODE_TYPE_VOLUME: {
        ...
        {
            // [INTENT] AMF volumes reference the object's shared vertex table by index. The loader
            //          first validates the referenced span, then compacts the span into a standalone
            //          `indexed_triangle_set` because OrcaSlicer stores each `ModelVolume` mesh
            //          independently after import.
            // [HAZARD] This rebasing assumes every face index lies in one contiguous min/max range.
            //          Sparse but valid AMF vertex tables are compacted here; any future importer that
            //          keeps shared vertex ownership must preserve identical face remapping semantics.
            ...
            indexed_triangle_set its{std::move(m_volume_facets),
                                     {m_object_vertices.begin() + min_id, m_object_vertices.begin() + max_id + 1}};
            its_compactify_vertices(its);
            // [COUPLING] Import correctness depends on core mesh utilities from `TriangleMesh` /
            //            `indexed_triangle_set`: AMF itself does not define winding fixes, so the
            //            parser immediately normalizes orientation to the expectations of later mesh
            //            repair and slicing stages.
            if (its_volume(its) < 0)
                its_flip_triangles(its);
            m_volume->set_mesh(std::move(its));
        }

        // [STATE] Legacy Slic3r-specific AMF metadata could mark volumes as modifiers, support
        //         blockers, or custom-configured meshes by writing `slic3r.*` metadata. OrcaSlicer's
        //         active reader no longer applies those branches (see the commented metadata parser
        //         below), so imported volumes default to `MODEL_PART` unless another path changes them.
    ...
    case NODE_TYPE_METADATA:
        /*if ((m_config != nullptr) && strncmp(m_value[0].c_str(), SLIC3R_CONFIG_TYPE, strlen(SLIC3R_CONFIG_TYPE)) == 0) {
            ...
            //m_config_substitutions->substitutions = m_config->load_from_ini_string_commented(std::move(m_value[1].c_str()),
        m_config_substitutions->rule); ConfigBase::load_from_gcode_string_legacy(*m_config, std::move(m_value[1].c_str()),
        *m_config_substitutions);
        ...
        // [HAZARD] The disabled block above documents the old AMF-to-config bridge: `slic3r.*`
        //          metadata could deserialize into `DynamicPrintConfig`, object configs, or volume
        //          configs, and `volume_type` could remap `ModelVolume::type()`. Because that code is
        //          compiled out, modern OrcaSlicer preserves only generic material attributes plus
        //          object/volume names from AMF metadata.
        else */
        if (m_path.size() == 3) {
...
void AMFParserContext::endDocument()
{
    for (const auto& object : m_object_instances_map) {
        ...
        for (const Instance& instance : object.second.instances) {
            ...
            index++;
            if (index > 1) {
                // [INTENT] AMF constellations can instantiate the same object multiple times. OrcaSlicer
                //          models each extra instance as a cloned `ModelObject` containing one instance
                //          transform rather than as a shared mesh with N instance records.
                // [MEMORY] `add_object(*model_object)` performs a deep model-level clone; the duplicate
                //          becomes owned by `m_model` immediately.
            ...
            if (instance.anything_set()) {
                // [STATE] Instance transforms are applied only after parsing finishes so forward
                //         references in `<constellation>` blocks can target objects defined earlier in
                //         the stream without mutating partially built meshes.
                ModelInstance* mi = model_object->add_instance();
                mi->set_offset(Vec3d(instance.deltax_set ? (double) instance.deltax : 0.0,
                                     instance.deltay_set ? (double) instance.deltay : 0.0,
                                     instance.deltaz_set ? (double) instance.deltaz : 0.0));
                mi->set_rotation(Vec3d(instance.rx_set ? (double) instance.rx : 0.0, instance.ry_set ? (double) instance.ry : 0.0,
                                       instance.rz_set ? (double) instance.rz : 0.0));
                mi->set_scaling_factor(Vec3d(instance.scalex_set ? (double) instance.scalex : 1.0,
                                             instance.scaley_set ? (double) instance.scaley : 1.0,
                                             instance.scalez_set ? (double) instance.scalez : 1.0));
                mi->set_mirror(Vec3d(instance.mirrorx_set ? (double) instance.mirrorx : 1.0,
                                     instance.mirrory_set ? (double) instance.mirrory : 1.0,
    ...
    const char* path, DynamicPrintConfig* config, ConfigSubstitutionContext* config_substitutions, Model* model, bool* use_inches)
{
    ...
    for (;;) {
        ...
        if (XML_Parse(parser, buff, len, done) == XML_STATUS_ERROR || ctx.error()) {
            BOOST_LOG_TRIVIAL(error) << "AMF parser: Parse error at line " << int(XML_GetCurrentLineNumber(parser)) << ": "
                                     << ctx.error_message();
            break;
        }
        if (done) {
...
bool extract_model_from_archive(mz_zip_archive&                 archive,
                                ...
                                bool*                           use_inches)
{
    ...
    try {
        res = mz_zip_reader_extract_file_to_callback(
            &archive, stat.m_filename,
            [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n) -> size_t {
                CallbackData* data = (CallbackData*) pOpaque;
                // [INTENT] The ZIP reader never materializes the embedded AMF as a temporary file.
                //          Expat consumes each decompressed chunk directly so large archives avoid an
                //          extra copy before the mesh hits `Model`.
                if (!XML_Parse(data->parser, (const char*) pBuf, (int) n, (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0) ||
                    data->ctx.error()) {
                    char error_buf[1024];
                    ::sprintf(error_buf, "Parsing file %s error at line %d: {%s}", data->stat.m_filename,
                              (int) XML_GetCurrentLineNumber(data->parser), data->ctx.error_message());
    ...
    //{
    //  std::string msg = _(L("The selected amf file has been saved with a newer version of " + std::string(SLIC3R_APP_NAME) + " and is not
    //  compatible.")); throw Slic3r::FileIOError(msg.c_str());
    // const std::string msg = (boost::format(_(L("The selected amf file has been saved with a newer version of %1% and is not
    // compatible."))) % std::string(SLIC3R_APP_NAME)).str(); throw Slic3r::FileIOError(msg);
    //}
...
bool load_amf(const char* path, DynamicPrintConfig* config, ConfigSubstitutionContext* config_substitutions, Model* model, bool* use_inches)
{
    ...
    if (boost::iends_with(path, ".amf")) {
        ...
        return (zip_mask == "PK") ? load_amf_archive(path, config, config_substitutions, model, use_inches) :
...
/*bool store_amf(const char* path, Model* model, const DynamicPrintConfig* config, bool fullpath_sources)
{
    ...
    // Conversion of a floating-point value to text and back is exact as long as at least max_digits10 were used (9 for float, 17 for
double).
    ...
    for (size_t object_id = 0; object_id < model->objects.size(); ++ object_id) {
        ...
        if (!sla_support_points.empty()) {
            ...
            for (size_t i = 0; i < sla_support_points.size(); ++i) {
                if (i != 0)
                    stream << ";";
                stream << sla_support_points[i].pos(0) << ";" << sla_support_points[i].pos(1) << ";" << sla_support_points[i].pos(2) << ";"
<< sla_support_points[i].head_front_radius << ";" << sla_support_points[i].is_new_island;
            }
            stream << "\n    </metadata>\n";
        }
        ...
        for (size_t i_volume = 0; i_volume < object->volumes.size(); ++i_volume) {
            ...
            {
                std::string input_file = xml_escape(fullpath_sources ? volume->source.input_file :
boost::filesystem::path(volume->source.input_file).filename().string()); stream << "        <metadata type=\"slic3r.source_file\">" <<
input_file << "</metadata>\n"; stream << "        <metadata type=\"slic3r.source_object_id\">" << volume->source.object_idx <<
"</metadata>\n"; stream << "        <metadata type=\"slic3r.source_volume_id\">" << volume->source.volume_idx << "</metadata>\n"; stream <<
"        <metadata type=\"slic3r.source_offset_x\">" << volume->source.mesh_offset(0) << "</metadata>\n"; stream << "        <metadata
type=\"slic3r.source_offset_y\">" << volume->source.mesh_offset(1) << "</metadata>\n"; stream << "        <metadata
type=\"slic3r.source_offset_z\">" << volume->source.mesh_offset(2) << "</metadata>\n";
            }
    ...
    std::string internal_amf_filename = boost::ireplace_last_copy(boost::filesystem::path(export_path).filename().string(), ".zip.amf",
".amf"); std::string out = stream.str();
