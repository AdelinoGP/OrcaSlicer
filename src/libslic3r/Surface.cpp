// [INTENT] Bounding-box utilities for Surface / Surfaces / SurfacesPtr and
// debug-export helpers (SVG legend + full SVG dump of a surface set).
// [STATE] Pure functions + debug SVG output; no mutable state.
// [COUPLING] Depends on BoundingBox, Surface (ExPolygon + SurfaceType), SVG.
// [HAZARD] surface_type_to_color_name returns hardcoded RGB strings; a port
// should convert to an enum→colour table rather than string matching.
...
BoundingBox get_extents(const Surface& surface) { return get_extents(surface.expolygon.contour); }
...
Point export_surface_type_legend_to_svg_box_size() { return Point(scale_(1. + 10. * 8.), scale_(3.)); }
