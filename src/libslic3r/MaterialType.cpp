// [INTENT] Static lookup table of 75+ filament material types with temperature, chamber,
//          adhesion, yield strength, and thermal-length properties. Used throughout the slicer
//          to apply material-specific constraints without requiring user configuration.
// [STATE]  all() returns a const reference to a function-local static vector — initialised once,
//          thread-safe after C++11 (magic statics). find() does a linear scan O(n) over ~75 entries.
// [HAZARD H1167] find() returns a raw pointer into the static vector. The vector is never
//               modified after initialisation so the pointer is stable, but callers must not
//               store it across application re-initialisation sequences or plugin/hotreload events.
// [COUPLING] No dependencies beyond <algorithm> and MaterialType.hpp.
...
{
    static const std::vector<MaterialTypeInfo> material_types = {{"ABS", 190, 300, 50, 65, 1, 0.1, 100},
