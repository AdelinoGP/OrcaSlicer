// [INTENT] Reconstruct a watertight (approximately) triangle mesh from a stack
// of 2D ExPolygon slices.  Used for wipe-tower and SLA pad geometry.
// [COUPLING] Depends on Tesselate (GLU tessellator), ClipperUtils (diff_ex),
// TBB for parallel layer processing and parallel reduce merge.
// [HAZARD H1189] FIXME comment at line 114 acknowledges that the repair pass
// (its_merge_vertices + its_remove_degenerate_faces + its_compactify_vertices)
// does NOT fully heal the mesh – cracks remain at layer boundaries due to
// floating-point rounding in unscaled(p).cast<float>().  Porting note: the
// comment on lines 26-29 explains the cast order intentionally mirrors
// Tesselate.cpp so its_merge_vertices can find duplicate vertices via ==.
// The mesh is still used downstream for collision checks and visualisation.
#include <numeric>
...
indexed_triangle_set inline straight_walls(const Polygon& plate, double lo_z, double hi_z) { return wall_strip(plate, lo_z, hi_z); }
...
indexed_triangle_set slices_to_mesh(const std::vector<ExPolygons>& slices, double zmin, const std::vector<float>& grid)
{
    ...
    auto merge_fn = [](const indexed_triangle_set& a, const indexed_triangle_set& b) {
        indexed_triangle_set res{a};
        its_merge(res, b);
        return res;
    };

    auto ret = execution::reduce(ex_tbb, layers.begin(), layers.end(), indexed_triangle_set{}, merge_fn);
