...
int CLI::run(int argc, char **argv)
{
    ...
    for (auto const &opt_key : m_actions) {
        ...
        //} else if (opt_key == "export_gcode" || opt_key == "export_sla" || opt_key == "slice") {
            ...
            // F11 (native-slicing rip-out, fork ticket ticket-008 sec.1): the headless
            // --slice native slicing path (plate loops, Print::load_cached_data,
            // Print::process, Print::export_gcode and the CLI status callbacks feeding
            // them) was removed. pnp_cli is the slicer for this fork; native --slice is
            // no longer supported. Fail cleanly instead of slicing.
            plate_to_slice = m_config.option<ConfigOptionInt>("slice")->value;
            sliced_plate = plate_to_slice;
            BOOST_LOG_TRIVIAL(error) << "native --slice is not supported in this build; use pnp_cli to slice." << std::endl;
            boost::nowide::cerr << "error: native --slice is not supported in this build; use pnp_cli to slice." << std::endl;
            record_exit_reson(outfile_dir, CLI_UNSUPPORTED_OPERATION, 0, cli_errors[CLI_UNSUPPORTED_OPERATION], sliced_info);
            flush_and_exit(CLI_UNSUPPORTED_OPERATION);
        } else {
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    // F11 (native-slicing rip-out sec.1): native --slice is removed (pnp_cli is
    // the slicer), so hide it from --help. The option def still lives in
    // CLIActionsConfigDef so previously-scripted invocations parse and then fail
    // cleanly, but it is no longer advertised.
    cli_actions_config_def.print_cli_help(boost::nowide::cout, false,
        [](const ConfigOptionDef &def) { return def.opt_key != "slice"; });
