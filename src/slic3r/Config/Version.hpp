...
// [INTENT] Configuration bundle version tracking for vendor preset compatibility.
// Used to determine which vendor configs are compatible with which slicer versions.
// [COUPLING] Depends on libslic3r Semver for semantic versioning.

namespace Slic3r { namespace GUI { namespace Config {

// Configuration bundle version.
// [INTENT] Stores version constraints for vendor preset bundles - which slicer versions support them.
struct Version
{
    ...
    // [INTENT] Check if given slicer version falls within supported range.
...
// [INTENT] Index file format for vendor config compatibility - parsed from INI-style files.
class Index
{
public:
    ...
    // Recommended config for a provided slic3r version. Used when checking for slic3r update (slic3r_version is the old one read out from
    // PrusaSlicer.ini)
