...
namespace Slic3r { namespace GUI { namespace Config {
...
{
    this->clear();
    // [STATE] Loading a snapshot always starts from a cleared in-memory view.
    ...
    // [INTENT] Rehydrate snapshot metadata, preset selections, and vendor bundles from INI sections.
...
{
    switch (reason) {
    case Snapshot::SNAPSHOT_UPGRADE: return "upgrade";
    case Snapshot::SNAPSHOT_DOWNGRADE: return "downgrade";
    case Snapshot::SNAPSHOT_BEFORE_ROLLBACK: return "before_rollback";
    case Snapshot::SNAPSHOT_USER: return "user";
...
static constexpr auto snapshot_subdirs = {PRESET_PRINT_NAME,   PRESET_SLA_PRINT_NAME, PRESET_FILAMENT_NAME, PRESET_SLA_MATERIALS_NAME,
                                          PRESET_PRINTER_NAME, "physical_printer",    PRESET_SYSTEM_DIR};
...
bool Snapshot::equal_to_active(const AppConfig& app_config) const
{
    ...
    // as app_config.
    {
        ...
        for (const VendorConfig& vc : this->vendor_configs) {
            ...
            if (it_vendor_models_variants == app_config.vendors().end() || it_vendor_models_variants->second != vc.models_variants_installed)
...
size_t SnapshotDB::load_db()
{
    ...
    for (auto& dir_entry : boost::filesystem::directory_iterator(snapshots_dir))
        if (boost::filesystem::is_directory(dir_entry.status())) {
            ...
            if (dir_entry.path().filename().string() != snapshot.id) {
                errors_cummulative += std::string("Snapshot ID ") + snapshot.id + " does not match the snapshot directory " +
    ...
    std::sort(m_snapshots.begin(), m_snapshots.end(),
              [](const Snapshot& s1, const Snapshot& s2) { return s1.time_captured < s2.time_captured; });
...
void SnapshotDB::update_slic3r_versions(std::vector<Index>& index_db)
{
    for (Snapshot& snapshot : m_snapshots) {
        for (Snapshot::VendorConfig& vendor_config : snapshot.vendor_configs) {
            auto it = std::find_if(index_db.begin(), index_db.end(),
                                   [&vendor_config](const Index& idx) { return idx.vendor() == vendor_config.name; });
            if (it != index_db.end()) {
...
const Snapshot& SnapshotDB::take_snapshot(const AppConfig& app_config, Snapshot::Reason reason, const std::string& comment)
{
    ...
    for (const auto& vendor : app_config.vendors()) {
        ...
        // bundle.load_configbundle((data_dir / PRESET_SYSTEM_DIR / (cfg.name + ".ini")).string(),
        // PresetBundle::LoadConfigBundleAttribute::LoadVendorOnly, ForwardCompatibilitySubstitutionRule::EnableSilent);
        bundle.load_vendor_configs_from_json((data_dir / PRESET_SYSTEM_DIR).string(), cfg.name,
...
SnapshotDB::const_iterator SnapshotDB::snapshot_with_vendor_preset(const std::string& vendor_name, const Semver& config_version)
{
    ...
    for (auto it = m_snapshots.begin(); it != m_snapshots.end(); ++it) {
        ...
        auto            it_vendor_config = std::lower_bound(snapshot.vendor_configs.begin(), snapshot.vendor_configs.end(), key,
                                                            [](const Snapshot::VendorConfig& cfg1, const Snapshot::VendorConfig& cfg2) {
    ...
    try {
        ...
        RichMessageDialog dlg(static_cast<wxWindow*>(wxGetApp().mainframe),
                              _L("OrcaSlicer has encountered an error while taking a configuration snapshot.") + "\n\n" +
                                  from_u8(err.what()) + "\n\n" + from_u8(message),
                              _L("OrcaSlicer error"), wxYES_NO);
        dlg.SetYesNoLabels(_L("Continue"), _L("Abort"));
        return dlg.ShowModal() == wxID_YES;
    }
}

}}} // namespace Slic3r::GUI::Config
