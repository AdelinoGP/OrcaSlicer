...
// [INTENT] Configuration snapshot system for versioning and rollback of user presets.
// Stores complete copies of configuration (presets, vendor bundles) that can be restored.
// Used for: upgrade/downgrade handling, user configuration backups, rollback functionality.
// [COUPLING] GUI module - depends on AppConfig, libslic3r Semver for version comparison.
// [SKIP_GUI] This is a GUI concern, not core slicing functionality - annotation for completeness.

namespace Slic3r {

class AppConfig;

namespace GUI { namespace Config {
...
//		printer/
// 		physical_printer/
// [INTENT] Snapshot stores complete configuration state at a point in time.
// Can be used to restore user configuration after upgrades or to roll back changes.
class Snapshot
{
public:
    // [INTENT] Reason for taking snapshot - used for UI display and filtering.
    enum Reason {
        SNAPSHOT_UNKNOWN,         // Unspecified
        SNAPSHOT_UPGRADE,         // Taken before version upgrade
        SNAPSHOT_DOWNGRADE,       // Taken before version downgrade
        SNAPSHOT_BEFORE_ROLLBACK, // Before rolling back configuration
        SNAPSHOT_USER,            // User-initiated manual snapshot
    };
    ...
    // Return true if the content of the current print / sla_print / filament / sla_material / printer / physical_printer / vendor
    // directories matches the state stored in this snapshot.
...
// [INTENT] Snapshot database singleton - manages all configuration snapshots on disk.
// Implements persistence: snapshots stored as directories with INI files.
// [MEMORY] Maintains in-memory cache of snapshot metadata - loaded lazily on first access.
class SnapshotDB
{
public:
    ...
    // Create a snapshot directory, copy the vendor config bundles, user print / sla_print / filament / sla_material / printer /
    // physical_printer profiles, create an index.
...
// Take snapshot on SnapshotDB::singleton(). If taking snapshot fails, report "message", and present a "Continue" or "Abort" buttons to
// respond. Return true on success and on "Continue" to continue with the process (for example installation of presets).
