...
bool Version::is_slic3r_supported(const Semver& slic3r_version) const
{
    if (!slic3r_version.in_range(min_slic3r_version, max_slic3r_version))
        return false;
    // [INTENT] Reject prerelease bundle data when the running app is less permissive than the bundle tag.
...
bool Version::is_current_slic3r_supported() const { return this->is_slic3r_supported(Slic3r::SEMVER); }

bool Version::is_current_slic3r_downgrade() const { return Slic3r::SEMVER < min_slic3r_version; }
#if 0
...
size_t Index::load(const boost::filesystem::path& path)
{
    ...
    m_path   = path;
    // [STATE] The parser tracks a single in-progress Version record while scanning the flat index file.
    ...
    while (std::getline(ifs, line)) {
#ifndef _MSVCVER
        // On a Unix system, getline does not remove the trailing carriage returns, if the index is shared over a Windows filesystem. Remove
        // them manually.
        ...
        *key_end = 0;
        ...
        if (key_value_pair) {
            if (semver)
                throw file_parser_error("Key cannot be a semantic version", path, idx_line); // Verify validity of the key / value pair.
...
Index::const_iterator Index::recommended(const Semver& slic3r_version) const
{
    ...
    for (const_iterator it = this->begin(); it != this->end(); ++it)
        if (it->is_slic3r_supported(slic3r_version) && (highest == this->end() || highest->config_version < it->config_version))
            highest = it;
    return highest;
}

Index::const_iterator Index::recommended() const { return this->recommended(Slic3r::SEMVER); }
