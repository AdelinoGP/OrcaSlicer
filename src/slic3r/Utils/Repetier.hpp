...
{
public:
    // [INTENT] Repetier adapts PrintHost upload/test flows onto the Repetier HTTP API.
...
private:
    // [STATE] Connection settings are cached from DynamicPrintConfig for each request.
    std::string host;
    std::string apikey;
    std::string cafile;
    std::string port;
