...
// [STATE] These globals are shared with the dynamically loaded Bambu networking layer for log-file coordination,
// making startup order part of the API contract even though the dependency is only visible through externs.
...
// [COUPLING] The callback typedef block is effectively the C++ ABI between GUI/runtime code and the proprietary Bambu
// networking implementation; changing signatures here ripples across plugin boundaries.
// user callbacks
...
struct PrintParams
{
    // [STATE] This struct mirrors the serialized job envelope sent to Bambu services/printers; most fields are plain
    // strings because the external networking layer treats it as a transport DTO instead of a typed domain object.
    /* basic info */
    std::string dev_id;
    std::string task_name;
    std::string project_name;
...
struct NetworkLibraryVersionInfo
{
    ...
    {
        return {v.version, v.version, "", v.display_name, v.url_override ? v.url_override : "", v.is_latest, v.warning ? v.warning : "",
                false};
    }

    static NetworkLibraryVersionInfo from_discovered(const std::string& full_version, const std::string& base, const std::string& sfx)
    {
