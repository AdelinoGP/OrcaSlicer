...
ElegooLink::ElegooLink(DynamicPrintConfig* config) : OctoPrint(config) {}
...
{
    // [INTENT] Elegoo devices may expose either an OctoPrint-compatible REST facade or the vendor SDCP API, so
    // probing falls back from the generic adapter to the proprietary detector before declaring failure.
...
wxString ElegooLink::get_test_ok_msg() const { return _L("Connection to ElegooLink is working correctly."); }
...
#ifdef WIN32
bool ElegooLink::upload_inner_with_resolved_ip(PrintHostUpload                 upload_data,
                                               ...
                                               const boost::asio::ip::address& resolved_addr) const
{
    ...
    // [COUPLING] Even after resolving the printer IP, Elegoo keeps the dual-stack protocol split: OctoPrint-style
    // hosts reuse the base uploader, while native ElegooLink devices switch to chunked SDCP transfer.
    // Elegoo supports both otcoprint and Elegoo link
...
bool ElegooLink::upload_inner_with_host(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    // Otherwise on Windows it contains the resolved IP address of the host.
    wxString test_msg_or_host_ip;
    // [INTENT] Reuse the mature OctoPrint path when possible and only enter the vendor upload loop when the test
    // endpoint proves the printer is speaking the SDCP variant.
    // Elegoo supports both otcoprint and Elegoo link
...
bool ElegooLink::validate_version_text(const boost::optional<std::string>& version_text) const { return true; }
bool ElegooLink::uploadPart(Http&       http,
                            std::string md5,
                            std::string uuid,
                            std::string path,
                            std::string filename,
                            ...
                            InfoFn      info_fn) const
{
    ...
    bool        result = false;
    http.form_clear();
    // [INTENT] Each chunk carries the file MD5, UUID, byte offset, and total length so the printer can verify and
    // reassemble a multi-request upload without a separate session object.
...
bool ElegooLink::loopUpload(std::string url, PrintHostUpload upload_data, ProgressFn progress_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    // [HAZARD] The protocol hardcodes 1 MiB segments and rewrites aggregate progress manually; changing either side
    // of that contract breaks retry behavior and UI progress accounting.
    ...
    if (res) {
        if (upload_data.post_action == PrintHostPostUploadAction::StartPrint) {
            // [COUPLING] A successful transfer is not the end of the workflow: the adapter must open a second
            // WebSocket channel, wait for firmware-side file validation, then issue the SDCP start-print command.
            ...
            try {
                ...
                if (errorString.find("The WebSocket handshake was declined by the remote peer") != std::string::npos) {
                    ...
                    error_fn(_L("The file has been transferred, but some unknown errors occurred. Please check the device page for the "
                                "file and try to start printing again."));
                } else {
...
bool ElegooLink::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    if (resolved_addr.empty()) {
        ...
        BOOST_LOG_TRIVIAL(error) << "ElegooLink failed to resolve hostname " << m_host
                       ...
                       ErrorFn           error_fn) const
{
    ...
    try {
        ...
        std::string jsonString   = R"({
                                        ...
                                        "Data":{
                                            "Cmd":)" +
                                 std::to_string(ElegooLinkCommand::ELEGOO_START_PRINT) + R"(,
                                            "Data":{
                                                "Filename":"/local/)" +
                                 filename + R"(",
                                                "StartLayer":0,
        ...
        // [HAZARD] The start-print payload is assembled as raw JSON text with printer-specific numeric
        // enums and magic field names, so translators should model this as a typed command object.
            ...
            if (*cmd == ElegooLinkCommand::ELEGOO_START_PRINT) {
                ...
                if (ack == ElegooLinkStartPrintAck::SDCP_PRINT_CTRL_ACK_OK) {
                    ...
                    case ElegooLinkStartPrintAck::SDCP_PRINT_CTRL_ACK_BUSY: {
                        error_message = _L(
                            "The printer is busy, Please check the device page for the file and try to start printing again.");
                        break;
                    }
...
bool ElegooLink::checkResult(WebSocketClient& client, ErrorFn error_fn) const
{
    ...
    try {
        ...
        std::cout << "send: " << jsonString << std::endl;
        BOOST_LOG_TRIVIAL(info) << "start print, param: " << jsonString;
        // [INTENT] Poll printer status until the transient "checking file" state disappears so the subsequent
        // start-print command only fires after the firmware has indexed and validated the uploaded job.
        bool needWrite = true;
        // wait 60s
        auto start_time = std::chrono::steady_clock::now();
        do {
