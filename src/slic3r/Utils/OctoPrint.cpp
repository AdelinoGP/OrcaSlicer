...
wxString OctoPrint::get_test_ok_msg() const { return _(L("Connection to OctoPrint is working correctly.")); }

wxString OctoPrint::get_test_failed_msg(wxString& msg) const
{
    return GUI::format_wxstr("%s: %s\n\n%s", _L("Could not connect to OctoPrint"), msg,
...
bool OctoPrint::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    // [INTENT] Windows upload starts with hostname classification because `.local` printers frequently fail a second
    // DNS/mDNS lookup after the initial probe; preserving the resolved IP avoids losing the transfer mid-session.
...
#ifdef WIN32
bool OctoPrint::upload_inner_with_resolved_ip(PrintHostUpload                 upload_data,
                                              ...
                                              const boost::asio::ip::address& resolved_addr) const
{
    ...
    // [COUPLING] The resolved-IP path still has to spoof the original Host header so reverse proxies and vhosts on
    // OctoPrint/PrusaLink continue routing the request to the correct backend.
...
#endif // _WIN32
    {
        // [INTENT] HTTPS and unresolved-host cases stay on the original URL because replacing the authority with a raw
        // IP would invalidate TLS certificates and lose hostname-based routing.
...
SL1Host::SL1Host(DynamicPrintConfig* config) : PrusaLink(config) {}

// SL1Host
const char* SL1Host::get_name() const { return "SL1Host"; }

wxString SL1Host::get_test_ok_msg() const { return _(L("Connection to Prusa SL1 / SL1S is working correctly.")); }
...
wxString PrusaLink::get_test_ok_msg() const { return _(L("Connection to PrusaLink is working correctly.")); }
...
void PrusaLink::set_auth(Http& http) const
{
    ...
    case atKeyPassword: http.header("X-Api-Key", get_apikey()); break;
    case atUserPassword: http.auth_digest(m_username, m_password); break;
    }
...
bool PrusaLink::get_storage(wxArrayString& storage_path, wxArrayString& storage_name) const
{
    ...
    // [INTENT] Storage enumeration filters read-only/full targets before upload so the caller can present only valid
    // destinations instead of probing failures one storage root at a time.
    http.on_error([&](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error getting storage: %2%, HTTP %3%, body: `%4%`") % name % error % status %
                                            body;
            error_msg = L"\n\n" + boost::nowide::widen(error);
            res       = false;
            // If status is 0, the communication with the printer has failed completely (most likely a timeout), if the status is <= 400, it
            // is an error returned by the pritner. If 0, we can show error to the user now, as we know the communication has failed. (res =
            // true will do the trick.) if not 0, we must not show error, as not all printers support api/v1/storage endpoint. So we must be
            // extra careful here, or we might be showing errors on perfectly fine communication.
            if (status == 0)
                res = true;
        })
    ...
    if (res && storage_path.empty()) {
        ...
        // TRN %1% = host
        std::string message = GUI::format(_L("Upload has failed. There is no suitable storage found at %1%."), m_host) +
                              GUI::into_u8(error_msg);
    ...
    http.on_error([&](std::string body, std::string error, unsigned status) {
        ...
        .on_complete([&, this](std::string body, unsigned) {
            ...
            try {
                ...
                // [INTENT] Capability probing decides whether PrusaLink speaks the newer raw PUT upload API or the older
                // multipart POST endpoint so later file transfer can match the printer firmware generation.
...
bool PrusaLink::upload_inner_with_resolved_ip(PrintHostUpload                 upload_data,
                                              ProgressFn                      prorgess_fn,
                                              ErrorFn                         error_fn,
                                              InfoFn                          info_fn,
                                              const boost::asio::ip::address& resolved_addr) const
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%, method: %7%") % name %
                                   upload_data.source_path % url % upload_filename.string() % upload_parent_path.string() %
                                   (upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false") %
                                   (use_put ? "PUT" : "POST");
...
bool PrusaLink::put_inner(
    PrintHostUpload upload_data, std::string url, const std::string& name, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    // [HAZARD] PUT mode pushes the full destination path into the URL itself, so any port must preserve the exact
    // path-escaping rules or printer-side folder resolution will diverge from POST mode.
...
#endif // _WIN32
    set_auth(http);
    // This is ugly, but works. There was an error at PrusaLink side that accepts any string at Print-After-Upload as true, thus False was
    // also triggering print after upload.
    ...
    PrintHostUpload upload_data, std::string url, const std::string& name, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    // [COUPLING] POST mode depends on a virtual hook for printer-specific completion semantics (`print`, `to_print`,
    // `to_queue`), so transport reuse across hosts is only superficial.
...
PrusaConnect::PrusaConnect(DynamicPrintConfig* config) : PrusaLink(config, true) {}
...
wxString PrusaConnect::get_test_ok_msg() const { return _(L("Connection to Prusa Connect is working correctly.")); }
