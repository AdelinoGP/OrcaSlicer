...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type,
                                            const std::vector<wxString> paths_to_open,
                                            bool                        single_instance)
{
    // [COUPLING] Relaunch behavior is tied to packaging layout on each platform (bundle path on macOS, AppImage env on
    // Linux, sibling exe names on Windows), so this helper encodes deployment assumptions rather than pure process API.
#ifdef _WIN32
    wxString path;
    ...
    // Don't call with wxEXEC_HIDE_CONSOLE, OrcaSlicer in GUI mode would just show the splash screen. It would not open the main window
    // though, it would just hang in the background.
    // [HAZARD] Process spawning is intentionally asynchronous everywhere; callers get no child handle or readiness
    // signal, so failures after launch are invisible beyond platform logs.
...
#else // Linux or Unix
    {
        ...
        if (args.empty()) {
            ...
            my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "orca-slicer" : "bambu-gcodeviewer"))
                          .string();
...
{
    wxFileDialog dialog(parent ? parent : wxGetApp().GetTopWindow(), _L("Open G-code file:"),
                        from_u8(wxGetApp().app_config->get_last_dir()), wxString(), file_wildcards(FT_GCODE),
                        wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    if (dialog.ShowModal() == wxID_OK) {
        wxString path = dialog.GetPath();
        // [INTENT] G-code viewer launches in a fresh process so large preview/rasterization state stays isolated from
        // the main slicer session instead of sharing a single UI/runtime heap.
