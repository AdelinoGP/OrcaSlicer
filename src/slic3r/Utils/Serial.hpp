...
{
    // [STATE] Extended scan results retain both OS port naming and optional USB identity hints.
    std::string port;
...
{ return sp1.port == sp2.port && sp1.id_vendor == sp2.id_vendor && sp1.id_product == sp2.id_product && sp1.is_printer == sp2.is_printer; }
...
{
public:
    // [PORTING_HAZARD:P1] The remaining Serial wrapper exists mainly for AVR109 reboot paths, not general printer I/O.
