...
// [INTENT] This file implements the StyleManager which bridges:
// 1) GUI font abstractions (wxFont, ImFont) with
// 2) Core emboss geometry data (FontFile, glyphs).
// The manager handles style CRUD operations, persistence to AppConfig,
// and GPU texture management for style preview images.
// [COUPLING] Direct dependency on:
//   - libslic3r/Emboss (FontFile creation via create_font_file)
//   - libslic3r/AppConfig (section-based key-value storage)
//   - wxWidgets (wxFont)
//   - ImGui (ImFontAtlas)
//   - OpenGL (texture upload)
// [HAZARD] OpenGL texture IDs are raw GLuints stored as void* in ImGui.
// Must call glDeleteTextures when freeing style images.
...
const HorizontalAlignToName horizontal_align_to_name =
    boost::assign::list_of<HorizontalAlignToName::relation>(FontProp::HorizontalAlign::left,
                                                            "left")(FontProp::HorizontalAlign::center,
                                                                    "center")(FontProp::HorizontalAlign::right, "right");
...
const VerticalAlignToName vertical_align_to_name =
    boost::assign::list_of<VerticalAlignToName::relation>(FontProp::VerticalAlign::top, "top")(FontProp::VerticalAlign::center,
                                                                                               "middle")(FontProp::VerticalAlign::bottom,
                                                                                                         "bottom");
} // namespace

void StyleManager::init(AppConfig* app_config)
{
    // [STATE] Stores non-owning pointer to global AppConfig singleton.
    // Used for load/store of all style definitions.
    // [COUPLING] AppConfig must outlive StyleManager - owned by GUI_App.
    ...
    std::optional<size_t> active_index_opt = (app_config != nullptr) ? ::load_style_index(*app_config) : std::optional<size_t>{};

    size_t active_index = 0;
    if (active_index_opt.has_value())
        active_index = *active_index_opt;
    if (active_index >= m_styles.size())
        active_index = 0;
...
bool StyleManager::store_styles_to_app_config(bool use_modification, bool store_active_index)
{
    ...
    if (store_active_index) {
        size_t style_index = exist_stored_style() ? m_style_cache.style_index : m_last_style_index;
...
{
    if (i1 >= m_styles.size() || i2 >= m_styles.size())
        return;
...
bool StyleManager::load_style(const Style& style)
{
    ...
    if (style.type != WxFontUtils::get_current_type())
        return false;
...
bool StyleManager::is_font_changed() const
{
    ...
    if (wx_font_stored.GetFaceName() != wx_font.GetFaceName())
        return true;
...
{
    if (m_style_cache.style_index >= m_styles.size())
        return nullptr;
...
void StyleManager::init_style_images(const Vec2i32& max_size, const std::string& text)
{
        ...
        if (!m_temp_style_images->styles.empty()) {
            ...
            for (StyleManager::StyleImage& image : m_temp_style_images->images) {
                ...
                for (auto& it : m_styles) {
                    if (it.name != style.text || !(it.prop == style.prop))
                        continue;
                    it.image = image;
                    break;
                }
    ...
    for (const Style& style : m_styles) {
        ...
        if (!wx_font_opt.has_value())
            continue;
        std::unique_ptr<FontFile> font_file = WxFontUtils::create_font_file(*wx_font_opt);
        if (font_file == nullptr)
            continue;
        styles.push_back({FontFileWithCache(std::move(font_file)), style.name, style.prop});
    }
...
{
    // [MEMORY] [HAZARD] OpenGL texture cleanup - all style preview images share
    // a single GPU texture atlas. The texture ID is stored as void* in StyleImage.
    // Must call glDeleteTextures exactly once when freeing.
    if (!m_exist_style_images)
        return;
    GLuint tex_id = 0;
...
ImFont* StyleManager::create_imgui_font(const std::string& text, double scale)
{
    ...
    m_style_cache.atlas.Flags |= ImFontAtlasFlags_NoMouseCursors | ImFontAtlasFlags_NoPowerOfTwoHeight;
    ...
    ImFont* font = m_style_cache.atlas.AddFontFromMemoryTTF((void*) buffer.data(), buffer.size(), font_size, &font_config,
...
{
    std::unique_ptr<FontFile> font_file = WxFontUtils::create_font_file(wx_font);
...
bool StyleManager::set_wx_font(const wxFont& wx_font, std::unique_ptr<FontFile> font_file)
{
    ...
    m_style_cache.font_file = FontFileWithCache(std::move(font_file));
...
std::string create_section_name(unsigned index) { return AppConfig::SECTION_EMBOSS_STYLE + ':' + std::to_string(index); }
