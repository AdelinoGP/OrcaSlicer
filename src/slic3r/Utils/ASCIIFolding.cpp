...
// [INTENT] Provide deterministic ASCII folding for every GUI text+path helper so legacy search, sorting, and persistence
// logic behave identically even after a Unity port that lacks wxWidgets normalization utilities.
// [UNITY] In C# this can be replicated with `System.Text.NormalizationForm.FormKD` + `System.Text.Encoding.UTF8` before
// falling back to `Dictionary<char, string>` for digraphs/circled digits and reusing `StringBuilder` for the mutable buffer.
...
// [INTENT] Flatten each incoming Unicode code point into the Lucene-compatible ASCII sequences that legacy GUI helpers expect.
// [STATE] `out` advances as cases write 0-4 ASCII characters so callers can reuse the same buffer without reallocating.
wchar_t* fold_to_ascii(wchar_t c, wchar_t* out)
{
    // [INTENT] Preserve plain ASCII while remapping non-ASCII code points to Lucene's normalized sequences.
    // [PORTING_HAZARD:P3] This massive switch must stay in sync with upstream mappings; generating it from Unicode data is safer.
    if (c < 0x080) {
        *out++ = c;
    } else {
        switch (c) {
...
// [STATE] Iterator overload reuses the pointer-based helper so STL consumers can append folded data without manual buffer math.
...
// [INTENT] Normalize UTF-8 input into ASCII-only buffers so search, sorting, and persistence match across GUI helpers.
// [UNITY] Mirror this using `Encoding.UTF8.GetString(...).Normalize(NormalizationForm.FormKD)` before folding into a `StringBuilder`.
std::string fold_utf8_to_ascii(const std::string& src, bool is_convert_for_filename)
{
    ...
    std::wstring dst;
    // [STATE] `dst` holds the folded result so we can optionally sanitize filenames while keeping the normalized buffer in sync.
    ...
    if (is_convert_for_filename) {
        ...
        if (found != std::string::npos) {
            ...
            std::regex  reg("[\\\\/:*?\"<>|\\0]");
            // [PORTING_HAZARD:P2] This hard-coded Windows invalid-character set + nul arm is platform-specific; Unity should rely on
            // `Path.GetInvalidFileNameChars()`. [UNITY] Replace this section with `new string(filename.Where(c =>
            // !Path.GetInvalidFileNameChars().Contains(c)).ToArray())` and sanitize via `Regex` + `Path.Combine`.
            std::string newFileName = regex_replace(filename, reg, "");
            dstStr                  = dir + "\\" + newFileName;
            // [PORTING_HAZARD:P3] Force-using backslashes assumes Windows paths; Unity ports must use `Path.Combine(dir, newFileName)` plus
