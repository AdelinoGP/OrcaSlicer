...
// [INTENT] Platform-specific implementations for font file creation from wxFont.
// Each platform uses different system APIs to extract TTF file data:
//   - Windows: HFONT handle -> GetFontData WinAPI
//   - macOS: CoreText CTFontCopyAttribute(kCTFontURLAttribute) for file path
//   - Linux: FontConfig FcFontMatch to resolve file path from font properties
// [HAZARD] Platform-specific code must be ported separately. No cross-platform
// abstraction exists for font file access - each OS requires custom implementation.
// [MEMORY] create_font_file returns unique_ptr<FontFile> - ownership transfers to caller.
...
{
    // [INTENT] Critical bridge: converts wxFont (platform font handle) to
    // Emboss::FontFile (raw TTF data). This is the ONLY entry point for
    // loading system fonts into the emboss pipeline.
    // [MEMORY] Returns unique_ptr - caller owns the FontFile.
    // The FontFile contains the raw bytes of the TTF loaded into memory.
    // [HAZARD] Each platform uses different font access:
    //   - Windows: HFONT handle provides in-memory font data via GetFontData
    //   - macOS: Must resolve file path via CoreText, then load from disk
    //   - Linux: Must query FontConfig for file path, then load from disk
    // Porting to a new platform requires implementing font data extraction.
#ifdef _WIN32
...
std::string WxFontUtils::store_wxFont(const wxFont& font)
{
    ...
    BOOST_LOG_TRIVIAL(trace) << "'" << font_descriptor << "' wx string get from GetNativeFontInfoDesc. wxFont " << "IsOk(" << font.IsOk()
                             << "), " << "isNull(" << font.IsNull() << ")" <<
        // "IsFree(" << font.IsFree() << "), " << // on MacOs is no function is free
        "IsFixedWidth(" << font.IsFixedWidth() << "), " << "IsUsingSizeInPixels(" << font.IsUsingSizeInPixels() << "), " << "Encoding("
...
const TypeToFamily WxFontUtils::type_to_family = boost::assign::list_of<TypeToFamily::relation>(wxFONTFAMILY_DEFAULT, "default")(
    wxFONTFAMILY_DECORATIVE,
    "decorative")(wxFONTFAMILY_ROMAN, "roman")(wxFONTFAMILY_SCRIPT, "script")(wxFONTFAMILY_SWISS, "swiss")(wxFONTFAMILY_MODERN,
                                                                                                           "modern")(wxFONTFAMILY_TELETYPE,
                                                                                                                     "teletype");

using TypeToStyle                            = boost::bimap<wxFontStyle, std::string_view>;
const TypeToStyle WxFontUtils::type_to_style = boost::assign::list_of<TypeToStyle::relation>(wxFONTSTYLE_ITALIC,
                                                                                             "italic")(wxFONTSTYLE_SLANT,
                                                                                                       "slant")(wxFONTSTYLE_NORMAL,
                                                                                                                "normal");

using TypeToWeight                             = boost::bimap<wxFontWeight, std::string_view>;
const TypeToWeight WxFontUtils::type_to_weight = boost::assign::list_of<TypeToWeight::relation>(wxFONTWEIGHT_THIN, "thin")(
    wxFONTWEIGHT_EXTRALIGHT, "extraLight")(wxFONTWEIGHT_LIGHT,
                                           "light")(wxFONTWEIGHT_NORMAL,
                                                    "normal")(wxFONTWEIGHT_MEDIUM,
                                                              "medium")(wxFONTWEIGHT_SEMIBOLD,
                                                                        "semibold")(wxFONTWEIGHT_BOLD,
                                                                                    "bold")(wxFONTWEIGHT_EXTRABOLD,
                                                                                            "extraBold")(wxFONTWEIGHT_HEAVY,
                                                                                                         "heavy")(wxFONTWEIGHT_EXTRAHEAVY,
                                                                                                                  "extraHeavy");
...
{
    static std::vector<wxFontStyle> italic_styles = {wxFontStyle::wxFONTSTYLE_ITALIC, wxFontStyle::wxFONTSTYLE_SLANT};
...
{
    static std::vector<wxFontWeight> bold_weight = {wxFontWeight::wxFONTWEIGHT_BOLD, wxFontWeight::wxFONTWEIGHT_HEAVY,
                                                    wxFontWeight::wxFONTWEIGHT_EXTRABOLD, wxFontWeight::wxFONTWEIGHT_EXTRAHEAVY};
