...
// [INTENT] Custom ImGui slider that exposes layer range selection, tick injection, and color-change/filament menus to the wxWidgets UI.
// [STATE] Keeps view state such as selection span, tick cache, custom-gcode window flags, and per-layer timing caches so the Unity port can
// orthogonally bind to ScriptableObject-backed settings. [UNITY] Replace with a UI Toolkit RangeSlider MonoBehaviour that binds to a shared
// ScriptableObject tick registry and uses VisualElement popups for menus; marshal all state mutations through a MainThreadDispatcher to
// keep parity with wxMain thread requirements.
...
bool equivalent_areas(const double& bottom_area, const double& top_area) { return fabs(bottom_area - top_area) <= miscalculation(); }

bool check_color_change(
    PrintObject* object, size_t frst_layer_id, size_t layers_cnt, bool check_overhangs, std::function<bool(Layer*)> break_condition)
{
...
{
    // [INTENT] Preload the icon atlas for one-layer buttons, pause/custom/gcode/delete controls so ImGui can reuse texture IDs without
    // stalling the frame. [OPENGL] Each call sponsors an ImGui `ImTextureID` backed by an uploaded GPU texture and assumes the ImGui
    // context/GL context lives on the wx main thread. [PORTING_HAZARD:P3] Unity must mirror this lazy SVG-to-Texture2D loading; defer new
    // texture creation to Unity's `Addressables` or `Resources` while keeping the menu UI responsive.
    bool result = true;
    if (!is_horizontal()) {
        ...
        result &= IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/one_layer_off_hover.svg", 56, 56,
        ...
        result &= IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/one_layer_on_hover_dark.svg", 56, 56,
        ...
        result &= IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/one_layer_off_hover_dark.svg", 56, 56,
                                                m_one_layer_off_hover_dark_id);
...
int IMSlider::GetActiveValue() const { return m_selection == ssLower ? m_lower_value : m_selection == ssHigher ? m_higher_value : -1; }
...
// [STATE] Rebuilds the tick cache when the serialized custom G-code Info changes and ensures color-change ticks stay in sync with the
// current extruder mode. [EVENT] `post_ticks_changed_event` is triggered whenever a tick batch is cleared so downstream listeners
// (toolbar/menu) can repaint. [UNITY] Map to a `ScriptableObject`-backed tick registry with a `UnityEvent` so the custom range slider
// controller can refresh its tooltip and delete button state. [PORTING_HAZARD:P3] Assumes `wxGetApp()` config watchers and ImGui popups run
// on the main thread when `m_show_menu` toggles.
...
{
    m_draw_mode        = is_sequential_print ? dmSequentialFffPrint : dmRegular;
...
void IMSlider::add_code_as_tick(Type type, int selected_extruder)
{
    ...
    // [EVENT] Notify the user when a color change tick is requested but the template lacks custom gcode.
...
bool IMSlider::switch_one_layer_mode()
{
    ...
    if (!m_is_one_layer) {                    // DEACTIVATE
        ...
        if (!m_one_layer_value || m_one_layer_value > m_max_value || m_one_layer_value < m_min_value) {
            ...
            SetHigherValue(GetHigherValue()); // ORCA use same position with higher value if user changed its position. visible section
                                              // stays same when switching one layer mode with this
    }
...
// [OPENGL] Draws custom groove, handles, and floating label through direct ImGui DrawList calls so the slider blends with the ORCA theme.
// [EVENT] Hooks mouse down to ImGui's ItemHoverable/SetActiveID sequence to translate drag gestures into slider value updates while keeping
// `m_show_menu` gated into right-click handlers. [UNITY] Implement as a UI Toolkit `Slider` with custom `VisualElement` overlay for ticks
// that uses `PointerDownEvent`/`PointerMoveEvent` for the same interactions.
bool IMSlider::horizontal_slider(const char* str_id, int* value, int v_min, int v_max, const ImVec2& size, float scale)
{
    ...
    const ImVec2 groove_start = ImVec2(pos.x + handle_dummy_width, pos.y + size.y - ONE_LAYER_MARGIN.y * m_scale -
                                                                       (ONE_LAYER_BUTTON_SIZE.y / 2) * m_scale * 0.5f -
                                                                       GROOVE_WIDTH * m_scale * 0.5f);
...
void IMSlider::draw_colored_band(const ImRect& groove, const ImRect& slideable_region)
{
    ...
    auto draw_band = [this](const ImU32& clr, const ImRect& band_rc) {
        if (clr == (m_is_dark ? BACKGROUND_COLOR_DARK : BACKGROUND_COLOR_LIGHT)) {
            ...
            ImGui::RenderFrame(band_rc.Min, band_rc.Max, m_is_dark ? GROOVE_COLOR_DARK : GROOVE_COLOR_LIGHT, false,
                               band_rc.GetWidth() * 0.5);
            // cover round corner
            ImGui::RenderFrame(ImVec2(band_rc.Min.x, band_rc.Max.y - band_rc.GetWidth() * 0.5), band_rc.Max,
                               m_is_dark ? GROOVE_COLOR_DARK : GROOVE_COLOR_LIGHT, false);
    ...
    auto draw_main_band = [&main_band, this](const ImU32& clr) {
        if (clr == (m_is_dark ? BACKGROUND_COLOR_DARK : BACKGROUND_COLOR_LIGHT)) {
            ...
            rc.Min += ImVec2(1, 1) * m_scale;
            rc.Max -= ImVec2(1, 1) * m_scale;
            ImGui::RenderFrame(main_band.Min, main_band.Max, m_is_dark ? GROOVE_COLOR_DARK : GROOVE_COLOR_LIGHT, false,
                               main_band.GetWidth() * 0.5);
            ImGui::RenderFrame(rc.Min, rc.Max, clr, false, rc.GetWidth() * 0.5);
        } else {
...
void IMSlider::draw_custom_label_block(const ImVec2 anchor, Type type)
{
    wxString label;
    switch (type) {
    case ColorChange: label = _L("Color"); break;
    case PausePrint: label = _L("Pause"); break;
    case ToolChange: label = _L("Color"); break;
    case Template: label = _L("Template"); break;
    case Custom: label = _L("Custom"); break;
    case Unknown: break;
    default: break;
    }
...
// [EVENT] Draws tick hover boxes, icons, and delete buttons; mouse hovers set `m_tick_value` so other helpers can highlight the same tick.
// [UNITY] Unity should render this as an overlay VisualElement list and feed pointer events into a `TickTooltipController` that exposes
// delete commands on the main thread.
void IMSlider::draw_ticks(const ImRect& slideable_region)
{
    ...
    while (tick_it != m_ticks.ticks.end()) {
        ...
        ImRect tick_hover_box = ImRect(slideable_region.GetCenter().x - tick_box.x / 2, tick_pos - tick_box.y / 2,
                                       slideable_region.GetCenter().x + tick_box.x / 2, tick_pos + tick_box.y / 2);
    ...
    while (tick_it != m_ticks.ticks.end()) {
        ...
        // draw ticks
        ImRect tick_left  = ImRect(slideable_region.GetCenter().x - tick_offset.x, tick_pos - tick_width,
                                   slideable_region.GetCenter().x - tick_offset.y, tick_pos);
        ImRect tick_right = ImRect(slideable_region.GetCenter().x + tick_offset.y, tick_pos - tick_width,
                                   slideable_region.GetCenter().x + tick_offset.x, tick_pos);
...
void IMSlider::show_tooltip(const TickCode& tick)
{
    ...
    switch (tick.type) {
    case CustomGCode::ColorChange: break;
    case CustomGCode::PausePrint: show_tooltip(time_str + _u8L("Pause:") + " \"" + gcode(PausePrint) + "\""); break;
    case CustomGCode::ToolChange: show_tooltip(time_str + _u8L("Change Filament")); break;
    case CustomGCode::Template: show_tooltip(time_str + _u8L("Custom Template:") + " \"" + gcode(Template) + "\""); break;
    case CustomGCode::Custom: show_tooltip(time_str + _u8L("Custom G-code:") + " \"" + tick.extra + "\""); break;
    default: break;
    }
}
...
// [OPENGL] The vertical flow renders dual handles, tick overlays, and the colored filling band per extruder using ImGui's draw lists.
// [EVENT] Drives selection switching, contextual popup toggles, and tick highlighting by monitoring `context.IO.MouseClicked` plus
// `m_tick_value`/`m_tick_rect` state. [PORTING_HAZARD:P3] Unity needs a custom input bridge to simulate ImGui's hover/active semantics and
// to replicate `m_show_menu` gating when right-clicking handles.
bool IMSlider::vertical_slider(const char*     str_id,
                               int*            higher_value,
                               int*            lower_value,
                               ...
                               float           scale)
{
    ...
    const ImVec2 groove_start = ImVec2(pos.x + size.x - ONE_LAYER_MARGIN.x * m_scale - (ONE_LAYER_BUTTON_SIZE.x / 2) * m_scale * 0.5f -
                                           GROOVE_WIDTH * m_scale * 0.5f,
    ...
    ImRect higher_handle     = ImRect(mid_x - handle_radius, higher_handle_pos - handle_radius, mid_x + handle_radius,
                                      higher_handle_pos + handle_radius);

    float  lower_handle_pos = get_pos_from_value(v_min, v_max, *lower_value, lower_slideable_region);
    ImRect lower_handle     = ImRect(mid_x - handle_radius, lower_handle_pos - handle_radius, mid_x + handle_radius,
                                     lower_handle_pos + handle_radius);
    ...
    if (!one_layer_flag) {
        ...
        // update handle position and value
        if (h_selected) {
            value_changed = slider_behavior(id, higher_slideable_region, v_min, v_max, higher_value, &higher_handle,
                                            ImGuiSliderFlags_Vertical, m_tick_value, m_tick_rect);
        }
        if (!h_selected) {
            value_changed = slider_behavior(id, lower_slideable_region, v_min, v_max, lower_value, &lower_handle, ImGuiSliderFlags_Vertical,
        ...
        if (h_selected) {
            window->DrawList->AddCircleFilled(higher_handle_center, handle_radius, handle_border_clr);
            window->DrawList->AddCircleFilled(higher_handle_center, handle_radius - handle_border, handle_clr);
            window->DrawList->AddLine(higher_handle_center + ImVec2(-0.5f * line_length, 0.0f),
                                      higher_handle_center + ImVec2(0.5f * line_length, 0.0f), white_bg, line_width);
            window->DrawList->AddLine(higher_handle_center + ImVec2(0.0f, -0.5f * line_length),
                                      higher_handle_center + ImVec2(0.0f, 0.5f * line_length), white_bg, line_width);
        }
        if (!h_selected) {
            window->DrawList->AddLine(lower_handle_center + ImVec2(-0.5f * line_length, 0.0f),
                                      lower_handle_center + ImVec2(0.5f * line_length, 0.0f), white_bg, line_width);
            window->DrawList->AddLine(lower_handle_center + ImVec2(0.0f, -0.5f * line_length),
                                      lower_handle_center + ImVec2(0.0f, 0.5f * line_length), white_bg, line_width);
        }
    ...
    if (one_layer_flag) {
        ...
        value_changed = slider_behavior(id, one_slideable_region, v_min, v_max, higher_value, &one_handle, ImGuiSliderFlags_Vertical,
                                        m_tick_value, m_tick_rect);
        ...
        if (ImGui::ItemHoverable(one_handle, id) && context.IO.MouseClicked[1])
            m_show_menu = true;
        if ((!ImGui::ItemHoverable(one_handle, id) && context.IO.MouseClicked[1]) || context.IO.MouseClicked[0])
            m_show_menu = false;
        ...
        // draw handle
        window->DrawList->AddLine(ImVec2(mid_x - 0.5 * bar_width, handle_center.y), ImVec2(mid_x + 0.5 * bar_width, handle_center.y),
                                  handle_clr, 2 * line_width);
        window->DrawList->AddCircleFilled(handle_center, handle_radius, handle_border_clr);
        window->DrawList->AddCircleFilled(handle_center, handle_radius - handle_border, handle_clr);
        window->DrawList->AddLine(handle_center + ImVec2(-0.5f * line_length, 0.0f), handle_center + ImVec2(0.5f * line_length, 0.0f),
                                  white_bg, line_width);
        window->DrawList->AddLine(handle_center + ImVec2(0.0f, -0.5f * line_length), handle_center + ImVec2(0.0f, 0.5f * line_length),
                                  white_bg, line_width);
...
// [EVENT] Chooses between horizontal and vertical ImGui windows, updates slider values, and forces extra frames when wheel/mouse events
// mutate the range. [OPENGL] Pushes custom ImGui style colors before drawing each window so the slider draws into the ImGui render pass.
// [UNITY] Represent this as two VisualElement popups (horizontal slider and vertical range slider) backed by a `RangeSliderController` that
// writes into a shared `PrintConfig` ScriptableObject. [PORTING_HAZARD:P3] ImGui window names (`moves_slider`, `laysers_slider`) and
// `ImGuiWindowFlags` are tightly coupled to input focus handling; Unity needs explicit focus hooks to replicate that behavior.
bool IMSlider::render(int canvas_width, int canvas_height)
{
    ...
    int windows_flag = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
                       ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;

    float scale = (float) wxGetApp().em_unit() / 10.0f;

    if (is_horizontal()) {
        ImVec2 size = ImVec2(canvas_width - 2 * std::max(LEFT_MARGIN * m_scale, 0.2f * canvas_width),
                             HORIZONTAL_SLIDER_WINDOW_HEIGHT * m_scale);
        ...
        if (vertical_slider("laysers_slider", &higher_value, &lower_value, higher_label, lower_label, GetMinValue(), GetMaxValue(), size,
                            m_selection, is_one_layer(), scale)) {
        ...
        ImTextureID normal_id = m_is_dark      ? is_one_layer() ? m_one_layer_on_dark_id : m_one_layer_off_dark_id :
                                is_one_layer() ? m_one_layer_on_id :
                                                 m_one_layer_off_id;
        ImTextureID hover_id  = m_is_dark      ? is_one_layer() ? m_one_layer_on_hover_dark_id : m_one_layer_off_hover_dark_id :
                                is_one_layer() ? m_one_layer_on_hover_id :
                                                 m_one_layer_off_hover_id;
...
// [EVENT] Shows a blocking ImGui modal for entering layer-specific custom G-code and keeps focus on the 3D canvas by forcing
// `wxGetApp().plater()->get_current_canvas3D()` focus. [THREAD] Must run on the wx main thread since it manipulates ImGui popups and
// interacts with wx3D canvas focus. [PORTING_HAZARD:P3] Unity must replace this with a `ModalWindow` connected to the Input System to avoid
// cross-thread focus collisions.
void IMSlider::render_input_custom_gcode(std::string custom_gcode)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_TitleBgActive, m_is_dark ? ImVec4(54 / 255.0f, 54 / 255.0f, 60 / 255.0f, 1.00f) :
                                                              ImVec4(245 / 255.0f, 245 / 255.0f, 245 / 255.0f, 1.00f));
    ImGui::GetCurrentContext()->DimBgRatio = 1.0f;
    int windows_flag                       = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize |
                       ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
...
// [EVENT] Opens a modal that accepts a layer number, clamps it, and reuses `do_go_to_layer` to update selection span.
// [THREAD] Runs on the main UI thread because it keeps focus on the wx canvas and reads `wxGetApp()` state.
// [PORTING_HAZARD:P3] Unity must replace this with a guarded IMGUI-style modal (e.g., UI Toolkit Dialog) to avoid hitting native focus
// issues on top of the RenderTexture canvas.
void IMSlider::render_go_to_layer_dialog()
{
    ...
    if (ImGui::BeginPopupModal((_u8L("Jump to Layer")).c_str(), NULL, windows_flag)) {
        imgui.text(_u8L("Please enter the layer number") + " (" + std::to_string(m_min_value + 1) + " - " +
                   std::to_string(m_max_value + 1) + "):");
        if (ImGui::IsMouseClicked(0)) {
            set_focus = false;
        }
        if (set_focus && !ImGui::IsAnyItemActive() && !ImGui::IsMouseClicked(0)) {
...
// [EVENT] Coordinates whether the add/edit tick menu should appear, then delegates to the custom dialogs for custom G-code and layer jumping.
// [STATE] `m_menu_enable` gates whether the menu can open; `m_show_menu` toggles ImGui popups and tooltips.
// [UNITY] Implement as a ContextualMenu VisualElement bound to the RangeSlider controller and use Unity `PopupWindow` for layer dialogs.
...
// [EVENT] Presents the add-tick context menu, including pause, custom G-code, template, jump-to-layer, and filament-change submenu options.
// [STATE] Guards insertion operations with `m_show_menu`, `m_draw_mode`, and `m_can_change_color` so certain buttons disappear when the
// print mode forbids edits. [UNITY] In Unity replace with a `ContextualMenu` VisualElement whose commands mutate the same `TickRegistry`
// and `NotificationManager` via `UnityEvent` callbacks.
void IMSlider::render_add_menu()
{
    ...
    if (ImGui::BeginPopup("slider_add_menu_popup")) {
        ...
        bool hovered          = false;
        {
            ...
            if (hovered) {
                show_tooltip(_u8L("Insert a pause command at the beginning of this layer."));
            }
            ...
            if (hovered) {
                show_tooltip(_u8L("Insert custom G-code at the beginning of this layer."));
            }
            ...
            if (!gcode(Template).empty()) {
                ...
                if (hovered) {
                    show_tooltip(_u8L("Insert template custom G-code at the beginning of this layer."));
                }
            }
        ...
        if (extruder_num > 1) {
            if (!m_can_change_color) {
                ...
                for (int i = 0; i < extruder_num; i++) {
                    ...
                    if (rgba.a() == 0)
                        icon_clr = 0;
                    if (menu_item_with_icon((_u8L("Filament ") + std::to_string(i + 1)).c_str(), "", ImVec2(14, 14) * m_scale, icon_clr,
                                            false, true, &hovered))
                        add_code_as_tick(ToolChange, i + 1);
                    if (hovered) {
                        show_tooltip(_u8L("Change filament at the beginning of this layer."));
                    }
                }
                end_menu();
            }
...
// [EVENT] Opens the edit menu for an existing tick to delete, edit, or replace the filament change, ensuring the menu respects
// `m_can_change_color`. [STATE] `tick` parameter drives which menu items appear (custom gcode vs tool change) and which color-change
// submenu is active. [UNITY] Use a `Menu` VisualElement bound to the tick object with `UnityEvent` handlers to delete or modify the tick
// entry so the slider overlay stays synchronized.
...
void IMSlider::on_change_color_mode(bool is_dark) { m_is_dark = is_dark; }
...
// [EVENT] Converts wx mouse-wheel deltas into slider adjustments and pushes ImGui to schedule an extra frame so the render updates
// immediately. [THREAD] Runs on the wx event thread and assumes ImGui windows still exist (`FindWindowByName`). [PORTING_HAZARD:P3] Tracks
// modifiers differently on macOS and Windows (`WXK_COMMAND`, `WXK_SHIFT`) and hard-codes direction inversion, which Unity must re-implement
// via the Input System. [UNITY] Mirror this with Unity's `MouseScrollWheelEvent` plus `CustomSliderInputBridge` that calls
// `SetHigherValue`/`SetLowerValue` on the main thread.
    ...
    if (m_draw_mode == dmSequentialGCodeView) {
        ...
        if (label_type == ltEstimatedTime) {
            if (m_is_wipe_tower) {
                ...
                return (layer_number == size_t(-1) || layer_number == m_layers_times.size()) ?
                           "" :
                           short_and_splitted_time(get_time_dhms(m_layers_times[layer_number]));
            }
        ...
        if (label_type == ltHeightWithLayer) {
            char   buffer[64];
            size_t layer_number;
            layer_number = m_draw_mode == dmSequentialFffPrint ? (m_values.empty() ? value : value + 1) :
                           m_is_wipe_tower                     ? get_layer_number(value, label_type) + 1 :
