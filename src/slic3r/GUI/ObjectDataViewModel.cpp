...
// [INTENT] ObjectDataViewModel provides the hierarchical data source for the object list (tree view) in the slicer's sidebar.
// It maps the underlying Slic3r::Model structure into a format suitable for wxDataViewCtrl, including plates, objects, volumes, and modifiers.
...
{
    // [STATE] Tracks whether this node can contain children; critical for wxGTK tree control stability.
#ifdef __WXGTK__
...
ObjectDataViewModelNode::ObjectDataViewModelNode(PartPlate* part_plate, wxString name)
    : m_parent(nullptr), m_name(name), m_type(itPlate), m_part_plate(part_plate), m_extruder(wxEmptyString), m_container(true)
{
...
const std::map<InfoItemType, InfoItemAtributes> INFO_ITEMS{
    ...
    {
        InfoItemType::CustomSupports,
        {L("Support painting"), "toolbar_support"},
    },
    //{ InfoItemType::CustomSeam,          {L("Paint-on seam"),           "seam_" },             },
    {
        InfoItemType::MmSegmentation,
        {L("Color painting"), "mmu_segmentation"},
    },
    //{ InfoItemType::Sinking,             {L("Sinking"),                 "objlist_sinking"}, },
    {
        InfoItemType::CutConnectors,
        {L("Cut connectors"), "cut_connectors"},
    },
    {
        InfoItemType::FuzzySkin,
        {L("Paint-on fuzzy skin"), "objlist_fuzzy_skin_paint"},
    },
};
...
ObjectDataViewModelNode::ObjectDataViewModelNode(ObjectDataViewModelNode* parent, const InfoItemType info_type)
    : m_parent(parent), m_type(itInfo), m_info_item_type(info_type), m_extruder(wxEmptyString), m_name(_(INFO_ITEMS.at(info_type).name))
{}

ObjectDataViewModelNode::ObjectDataViewModelNode(ObjectDataViewModelNode* parent, const ItemType type, const int plate_idx)
    : m_parent(parent), m_type(type), m_extruder(wxEmptyString), m_plate_idx(plate_idx)
{
    if (type == itSettings) {
...
void ObjectDataViewModelNode::set_extruder_icon()
{
    if (m_type & (itInstance | itInstanceRoot | itLayerRoot) ||
        ((m_type & itVolume) && m_volume_type != Slic3r::ModelVolumeType::MODEL_PART &&
         m_volume_type != Slic3r::ModelVolumeType::PARAMETER_MODIFIER))
...
void ObjectDataViewModelNode::set_action_icon(bool enable)
{
    ...
    m_action_icon_name = m_type & itPlate              ? undo :
                         ...
                         m_type & (itVolume | itLayer) ? undo :
                                                         /*m_type & itInstance*/ "set_separate_obj";
...
{
    switch (col) {
    case colPrint: m_printable_icon << variant; return true;
    case colHeight: m_variable_height_icon << variant; return true;
    case colName: {
    ...
    // BBS
    case colSupportPaint: m_support_icon << variant; break;
    case colSinking: m_sinking_icon << variant; break;
    case colColorPaint: m_color_icon << variant; break;
    case colEditing: m_action_icon << variant; return true;
    default: printf("MyObjectTreeModel::SetValue: wrong column");
    }
    return false;
}
...
void ObjectDataViewModelNode::SetPlateIdx(const int& idx) { m_plate_idx = idx; }
...
{
    // [INTENT] Adds a new object to the hierarchical tree and links it to its physical plate.
    // [STATE] Uses the underlying model_object configuration to determine initial extruder and plate associations.
    ...
    for (auto plate : m_plates) {
        if (plate->m_part_plate != nullptr && plate->m_part_plate->contain_instance_totally(model_object, 0)) {
            plate_node = plate;
...
static bool append_root_node(ObjectDataViewModelNode* parent_node, ObjectDataViewModelNode** root_node, const ItemType root_type)
{
    ...
    *root_node = inst_root_id < 0 ? new ObjectDataViewModelNode(parent_node, root_type) : parent_node->GetNthChild(inst_root_id);

    if (inst_root_id < 0) {
        if ((root_type & itInstanceRoot) || ((root_type & itLayerRoot) && get_root_idx(parent_node, itInstanceRoot) < 0))
...
wxDataViewItem ObjectDataViewModel::AddInstanceRoot(const wxDataViewItem& parent_item) { return AddRoot(parent_item, itInstanceRoot); }
...
wxDataViewItem ObjectDataViewModel::AddLayersRoot(const wxDataViewItem& parent_item) { return AddRoot(parent_item, itLayerRoot); }
...
wxDataViewItem ObjectDataViewModel::Delete(const wxDataViewItem& item)
{
    ...
    if (id > 0) {
        if (size_t(id) == node_parent->GetChildCount())
            id--;
...
wxDataViewItem ObjectDataViewModel::DeleteLastInstance(const wxDataViewItem& parent_item, size_t num)
{
    ...
    for (int i = inst_cnt - 1; i >= stop; --i) {
        ...
        if (i == 0)
            last_inst_printable = last_instance_node->IsPrintable();
...
wxDataViewItem ObjectDataViewModel::GetItemByVolumeId(int obj_idx, int volume_idx)
{
    ...
    if (parent->GetChildCount() == 0 || (parent->GetChildCount() == 1 && parent->GetNthChild(0)->GetType() & itSettings)) {
        if (volume_idx == 0)
...
wxDataViewItem ObjectDataViewModel::GetItemByLayerId(int obj_idx, int layer_idx) { return GetItemById(obj_idx, layer_idx, itLayerRoot); }
...
int ObjectDataViewModel::GetObjectIdByItem(const wxDataViewItem& item) const { return GetIdByItem(GetObject(item)); }

int ObjectDataViewModel::GetVolumeIdByItem(const wxDataViewItem& item) const { return GetIdByItemAndType(item, itVolume); }

int ObjectDataViewModel::GetInstanceIdByItem(const wxDataViewItem& item) const { return GetIdByItemAndType(item, itInstance); }

int ObjectDataViewModel::GetLayerIdByItem(const wxDataViewItem& item) const { return GetIdByItemAndType(item, itLayer); }
...
void ObjectDataViewModel::GetItemInfo(const wxDataViewItem& item, ItemType& type, int& obj_idx, int& idx)
{
    ...
    ObjectDataViewModelNode* node = static_cast<ObjectDataViewModelNode*>(item.GetID());
    if (!node || node->GetIdx() < -1 ||
        (node->GetIdx() == -1 && !(node->GetType() & (itObject | itSettings | itInstanceRoot | itLayerRoot | itInfo))))
        return;
...
void ObjectDataViewModel::GetValue(wxVariant& variant, const wxDataViewItem& item, unsigned int col) const
{
    ...
    ObjectDataViewModelNode* node = static_cast<ObjectDataViewModelNode*>(item.GetID());
    switch (col) {
    case colPrint: variant << node->m_printable_icon; break;
    case colHeight: variant << node->m_variable_height_icon; break;
    case colName: variant << DataViewBitmapText(node->m_name, node->m_bmp); break;
    case colFilament: variant << DataViewBitmapText(node->m_extruder, node->m_extruder_bmp); break;
    // BBS
    case colSupportPaint: variant << node->m_support_icon; break;
    case colSinking: variant << node->m_sinking_icon; break;
    case colColorPaint: variant << node->m_color_icon; break;
    case colEditing: variant << node->m_action_icon; break;
    default:;
    }
}
...
wxDataViewItem ObjectDataViewModel::ReorganizeChildren(const int current_volume_id, const int new_volume_id, const wxDataViewItem& parent)
{
    auto ret_item = wxDataViewItem(0);
...
wxDataViewItem ObjectDataViewModel::GetSettingsItem(const wxDataViewItem& item) const { return GetItemByType(item, itSettings); }

wxDataViewItem ObjectDataViewModel::GetInstanceRootItem(const wxDataViewItem& item) const { return GetItemByType(item, itInstanceRoot); }

wxDataViewItem ObjectDataViewModel::GetLayerRootItem(const wxDataViewItem& item) const { return GetItemByType(item, itLayerRoot); }
...
void ObjectDataViewModel::UpdateSettingsDigest(const wxDataViewItem& item, const std::vector<std::string>& categories)
{
    if (!item.IsOk())
        return;
    ObjectDataViewModelNode* node = static_cast<ObjectDataViewModelNode*>(item.GetID());
...
wxDataViewItem ObjectDataViewModel::SetPrintableState(PrintIndicator printable,
                                                      int            obj_idx,
                                                      int            subobj_idx /* = -1*/,
...
wxDataViewItem ObjectDataViewModel::SetObjectPrintableState(PrintIndicator printable, wxDataViewItem obj_item)
{
...
void ObjectDataViewModel::Rescale()
{
    ...
    for (wxDataViewItem item : all_items) {
        ...
        switch (node->m_type) {
        case itObject:
        case itVolume: UpdateBitmapForNode(node); break;
        case itLayerRoot: node->m_bmp = create_scaled_bitmap(LayerRootIcon); break;
        case itLayer: node->m_bmp = create_scaled_bitmap(LayerIcon); break;
        case itInfo: node->m_bmp = m_info_bmps.at(node->m_info_item_type); break;
        default: break;
        }
