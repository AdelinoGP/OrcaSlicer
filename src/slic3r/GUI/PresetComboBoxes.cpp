...
// [INTENT] PresetComboBox provides a dropdown for selecting presets, managing types (print, filament, etc.), and rendering icons based on
// compatibility and status. [UNITY] Unity equivalent: TMP_Dropdown wrapper in a MonoBehaviour for selection handling and custom UI item rendering.
PresetComboBox::PresetComboBox(wxWindow* parent, Preset::Type preset_type, const wxSize& size, PresetBundle* preset_bundle /* = nullptr*/)
    : // [UNITY] wxComboBox -> TMP_Dropdown component.
    ::ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, size, 0, nullptr, wxCB_READONLY)
    ,
    // [STATE] Stores the type of presets this combobox handles.
    m_type(preset_type)
    ,
    // [STATE] Tracks the last selected item index.
    m_last_selected(wxNOT_FOUND)
    ,
    // [STATE] Stores the scaling factor (em_unit).
    m_em_unit(em_unit(this))
    ,
    // [STATE] Reference to the PresetBundle for accessing preset collections.
    m_preset_bundle(preset_bundle ? preset_bundle : wxGetApp().preset_bundle)
{
    ...
    fill_width_height();
    // [EVENT] Binds selection changes to OnSelect handler.
    // [UNITY] Unity equivalent: Dropdown.onValueChanged.AddListener.
    Bind(wxEVT_COMBOBOX, &PresetComboBox::OnSelect, this);
}

// [INTENT] Event handler for combo box selection changes. Updates the selected preset and notifies listeners if the selection changed.
// [UNITY] Unity equivalent: Dropdown.onValueChanged handler.
void PresetComboBox::OnSelect(wxCommandEvent& evt)
{
    ...
    if (marker >= LABEL_ITEM_DISABLED && marker < LABEL_ITEM_MAX)
        ...
        // [EVENT] Notify listeners that the selection has changed.
...
void PresetComboBox::set_label_marker(int item, LabelItemType label_item_type) { this->SetClientData(item, (void*) label_item_type); }
...
// [INTENT] Updates the UI selection state and tooltips.
...
// [PORTING_HAZARD:P1] GTK-specific workaround for text fitting and ellipsization; not directly applicable to Unity.
// A workaround for a set of issues related to text fitting into gtk widgets:
#if defined(__WXGTK20__) || defined(__WXGTK3__)
...
int PresetComboBox::update_ams_color()
{
    ...
    auto                color_head = static_cast<ConfigOptionStrings*>(
        cfg->option("filament_colour")->clone()); // single color (the first color if multi-color filament)
    auto color_pack = static_cast<ConfigOptionStrings*>(
        cfg->option("filament_multi_colour")->clone()); // multi color (all colors in all kinds of filament)
    ...
    if (color_str.empty())
        ...
        color_str.erase(color_str.size() - 1);
...
void PresetComboBox::update(std::string select_preset_name)
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i) {
        ...
        bool is_enabled = m_type == Preset::TYPE_PRINTER && printer_technology != ptAny ?
                              preset.printer_technology() == printer_technology :
                              true;
...
bool PresetComboBox::add_ams_filaments(std::string selected, bool alias_name)
{
    ...
    if (is_bbl_vendor_preset && !m_preset_bundle->filament_ams_list.empty()) {
        ...
        for (auto& entry : m_preset_bundle->filament_ams_list) {
            ...
            auto iter = std::find_if(filaments.begin(), filaments.end(), [&filament_id, this](auto& f) {
                return f.is_compatible && m_collection->get_preset_base(f) == &f && f.filament_id == filament_id;
            });
                                  ...
                                  bool               is_system /* = false*/)
{
    ...
    if (bmp == nullptr) {
        ...
        bmps.emplace_back(m_type == Preset::TYPE_PRINTER ? create_scaled_bitmap(main_icon_name, this, 16, !is_enabled) :
                          is_compatible                  ? m_bitmapCompatible.bmp() :
                                                           m_bitmapIncompatible.bmp());
        // Paint a lock at the system presets.
        bmps.emplace_back(is_system ? create_scaled_bitmap(next_icon_name, this, 16, !is_enabled) :
                                      bitmap_cache().mkclear(norm_icon_width, icon_height));
        bmp = bitmap_cache().insert(bitmap_key, bmps);
    }
    ...
    : PresetComboBox(parent, preset_type, wxSize(25 * wxGetApp().em_unit(), 30 * wxGetApp().em_unit() / 10))
{
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        clr_picker = new wxBitmapButton(parent, wxID_ANY, {}, wxDefaultPosition, wxSize(FromDIP(20), FromDIP(20)),
                                        wxBU_EXACTFIT | wxBU_AUTODRAW | wxBORDER_NONE);
...
void PlaterPresetComboBox::show_add_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Add/Remove presets"), "",
        [](wxCommandEvent&) { wxTheApp->CallAfter([]() { run_wizard(ConfigWizard::SP_PRINTERS); }); }, "menu_edit_preset", menu,
...
void PlaterPresetComboBox::show_edit_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Edit preset"), "", [this](wxCommandEvent&) { this->switch_to_tab(); }, "cog", menu, []() { return true; },
        wxGetApp().plater());
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        append_menu_item(
            menu, wxID_ANY, _devL("Change extruder color"), "", [this](wxCommandEvent&) { this->change_extruder_color(); }, "blank_14",
...
wxString PlaterPresetComboBox::get_preset_name(const Preset& preset) { return from_u8(preset.label(false)); }
...
void PlaterPresetComboBox::update()
{
    ...
    const Preset* selected_preset = m_type == Preset::TYPE_FILAMENT ? selected_filament_preset :
                                    has_selection                   ? &m_collection->get_selected_preset() :
                                                                      nullptr;
    ...
    std::vector<std::string> filament_orders = {"Bambu PLA Basic",  "Bambu PLA Matte", "Bambu PETG HF",          "Bambu ABS",
                                                "Bambu PLA Silk",   "Bambu PLA-CF",    "Bambu PLA Galaxy",       "Bambu PLA Metal",
                                                "Bambu PLA Marble", "Bambu PETG-CF",   "Bambu PETG Translucent", "Bambu ABS-GF"};
    ...
    auto add_presets = [this, &preset_descriptions, &filament_orders, &preset_filament_vendors, &first_vendors, &preset_filament_types,
                        &preset_filament_names, &first_types,
                        &selected_in_ams](std::map<wxString, wxBitmap*> const& presets, wxString const& selected, std::string const& group,
                                          wxString const& groupName) {
        if (!presets.empty()) {
            ...
            if (m_type == Preset::TYPE_FILAMENT) {
                ...
                if (group == "System presets" || group == "Unsupported presets")
                    std::sort(list.begin(), list.end(),
                              [&filament_orders, &preset_filament_vendors, &first_vendors, &preset_filament_types, &first_types](auto* l,
                                                                                                                                 auto* r) {
                                  { // Compare order
                ...
                if (groupName == "by_vendor" || groupName == "by_type" || groupName == "") {
                    ...
                    std::sort(list.begin(), list.end(), [&by](auto* l, auto* r) {
                        ...
                        return (l_valid != r_valid) ? l_valid > r_valid : (l_lower != r_lower) ? l_lower < r_lower : l->first < r->first;
                    });
                }
                bool unsupported = group == "Unsupported presets";
                for (auto it : list) {
                    // ORCA add sorting support for vendor / type for user presets
                    auto groupName2 = groupName == "by_type" ?
                                          (preset_filament_types[it->first].empty() ? _L("Unspecified") : preset_filament_types[it->first]) :
                                      groupName == "by_vendor" ?
                                          (preset_filament_vendors[it->first].empty() ? _L("Unspecified") :
            ...
            for (PhysicalPrinterCollection::ConstIterator it = ph_printers.begin(); it != ph_printers.end(); ++it) {
                for (const std::string& preset_name : it->get_preset_names()) {
                    ...
                    if (!preset || !preset->is_visible)
                        continue;
                    std::string main_icon_name, bitmap_key = main_icon_name = preset->printer_technology() == ptSLA ? "sla_printer" :
    m_main_bitmap_name; wxBitmap* bmp = get_bmp(main_icon_name, wide_icons, main_icon_name); assert(bmp);
...
FilamentColor PlaterPresetComboBox::get_cur_color_info()
{
    ...
    if (m_filament_idx < 0 || m_filament_idx >= static_cast<int>(filaments_multi_color.size())) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                   << boost::format(": m_filament_idx %1% out of range %2%") % m_filament_idx %
                                          filaments_multi_color.size();
        return FilamentColor();
    }

    if (m_filament_idx >= static_cast<int>(filament_color_type.size())) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                   << boost::format(": m_filament_idx %1% out of range for color_type %2%") % m_filament_idx %
                                          filament_color_type.size();
        return FilamentColor();
    }
...
void PlaterPresetComboBox::sync_colour_config(const std::vector<std::string>& clrs, bool is_gradient)
{
    ...
    if (m_filament_idx >= multi_colour_opt->values.size())
        multi_colour_opt->values.resize(m_filament_idx + 1);
    if (m_filament_idx >= colour_type_opt->values.size())
        colour_type_opt->values.resize(m_filament_idx + 1);
    if (m_filament_idx >= colour_opt->values.size())
        colour_opt->values.resize(m_filament_idx + 1);
...
void TabPresetComboBox::OnSelect(wxCommandEvent& evt)
{
    ...
    if (marker >= LABEL_ITEM_DISABLED && marker < LABEL_ITEM_MAX) {
        ...
        if (sp != ConfigWizard::SP_WELCOME) {
            wxTheApp->CallAfter([this, sp]() { run_wizard(sp); });
        }
    } else if (on_selection_changed && (m_last_selected != selected_item || m_collection->current_is_dirty())) {
        m_last_selected = selected_item;
        // BBS: ams
...
wxString TabPresetComboBox::get_preset_name(const Preset& preset) { return from_u8(preset.label(true)); }
...
void TabPresetComboBox::update()
{
    ...
    if (!project_embedded_presets.empty()) {
        ...
        for (std::map<wxString, std::pair<wxBitmap*, bool>>::iterator it = project_embedded_presets.begin();
    ...
    for (unsigned int ui_id = 0; ui_id < GetCount(); ++ui_id) {
        ...
        if (preset) {
            ...
            if (old_label != new_label) {
                ...
                if (ui_id == GetSelection())
                    SetToolTip(wxString::FromUTF8(new_label.c_str())); // BBS
            }
        }
    }
#ifdef __APPLE__
