...
/*
[INTENT]
CalibrationCaliPage is a specific step within the calibration wizard that displays
the active printing/calibration status. It provides controls to pause, resume, or
abort the calibration print and shows progress indicators.

[STATE]
- m_cali_mode: Type of calibration (Flow Dynamics, Flow Rate, etc.).
- m_page_type: Specific subtype of the page (Coarse, Fine).
- m_cur_extruder_id: Currently active extruder for the calibration.
- m_is_between_start_and_running: Guard flag for transition between job start and active execution.

[EVENT]
- on_subtask_pause_resume: Dispatches pause/resume commands to MachineObject.
- on_subtask_abort: Shows confirmation dialog and dispatches abort command.
- update(): Polled update that synchronizes UI with MachineObject state.

[UNITY]
- This page should be a prefab within a Unity-based Wizard/Stepper framework.
- PrintingTaskPanel -> Sub-UI-Toolkit component with ProgressBar and Control Buttons.
- CaliPageStepGuide -> Stepper/Breadcrumb UI component.
*/

static const wxString NA_STR = _L("N/A");

CalibrationCaliPage::CalibrationCaliPage(
    wxWindow* parent, CalibMode cali_mode, CaliPageType cali_type, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
...
CalibrationCaliPage::~CalibrationCaliPage()
{
    m_printing_panel->get_pause_resume_button()->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
                                                            wxCommandEventHandler(CalibrationCaliPage::on_subtask_pause_resume), NULL,
                                                            this);
    m_printing_panel->get_abort_button()->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
                                                     wxCommandEventHandler(CalibrationCaliPage::on_subtask_abort), NULL, this);
}
...
void CalibrationCaliPage::create_page(wxWindow* parent)
{
    ...
    m_printing_panel->get_pause_resume_button()->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                                                         wxCommandEventHandler(CalibrationCaliPage::on_subtask_pause_resume), NULL, this);
    m_printing_panel->get_abort_button()->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                                                  wxCommandEventHandler(CalibrationCaliPage::on_subtask_abort), NULL, this);
}
...
void CalibrationCaliPage::clear_last_job_status() { m_is_between_start_and_running = true; }

/*
[STATE]
Synchronizes the printing panel with the current MachineObject state.
Handles various calibration modes (PA Line, Flow Rate, Vol speed Tower) and their
corresponding result-fetching logic (via CalibUtils).
*/
        ...
        if (obj) {
            if (obj->print_status != "RUNNING") {
                BOOST_LOG_TRIVIAL(info) << "on_show_cali_page - machine object status:"
                                        << " dev_id = " << obj->get_dev_id() << ", print_type = " << obj->printer_type
                                        << ", printer_status = " << obj->print_status << ", is_connected = " << obj->is_connected()
                                        << ", m_is_between_start_and_running = " << m_is_between_start_and_running
                                        << ", cali_finished = " << obj->cali_finished << ", cali_version = " << obj->cali_version
                                        << ", cache_flow_ratio = " << obj->cache_flow_ratio << ", sub_task_name = " << obj->subtask_name
                                        << ", gcode_file_name = " << obj->m_gcode_file << ", get_pa_calib_result"
                                        << obj->get_pa_calib_result << ", get_flow_calib_result" << obj->get_flow_calib_result;
            }
        } else {
        ...
        if (obj->is_in_prepare() || obj->print_status == "SLICING") {
            ...
            if (obj->print_status == "FINISH") {
                ...
                if (is_market_task) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << "SHOW_SCORE_BTU: design_id [" << obj->get_modeltask()->design_id << "] print_finish ["
                                            << m_print_finish << "]";
...
void CalibrationCaliPage::on_device_connected(MachineObject* obj) { reset_printing_values(); }
