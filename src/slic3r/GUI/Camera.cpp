...
// [INTENT] Camera implements the logic for the 3D viewport camera, supporting Perspective and Orthographic projection.
// It manages view and projection matrices, camera movement (translation, rotation on sphere), and zoom-to-fit logic.
// [STATE] m_view_matrix, m_projection_matrix, m_target (look-at point), m_distance (from target), m_zoom, m_viewport.
// [UNITY] Replace with Unity Camera component. Orbital movement logic maps to a custom MonoBehaviour.
// [OPENGL] Interacts with GL state via glViewport in apply_viewport().
...
// [INTENT] select_view sets the camera to a predefined standard orientation (e.g., Top, Front, Iso).
// [UNITY] Use Transform.SetPositionAndRotation or pre-recorded view states.
void Camera::select_view(const std::string& direction)
{
    if (direction == "iso") {
        ...
        look_at(m_target - 0.707 * m_distance * Vec3d::UnitY() + 0.707 * m_distance * Vec3d::UnitZ(), m_target,
...
{
    switch (m_type) {
    case EType::Perspective: return m_frustrum_zs.first * (m_projection_matrix.matrix()(0, 2) - 1.0) / m_projection_matrix.matrix()(0, 0);
    default:
    case EType::Ortho:
...
{
    switch (m_type) {
    case EType::Perspective: return m_frustrum_zs.first * (m_projection_matrix.matrix()(1, 2) + 1.0) / m_projection_matrix.matrix()(1, 1);
    default:
    case EType::Ortho:
...
{
    switch (m_type) {
    case EType::Perspective: return 2.0 * m_frustrum_zs.first / m_projection_matrix.matrix()(0, 0);
    default:
    case EType::Ortho: return 2.0 / m_projection_matrix.matrix()(0, 0);
    }
}
...
{
    switch (m_type) {
    case EType::Perspective: return 2.0 * m_frustrum_zs.first / m_projection_matrix.matrix()(1, 1);
    default:
    case EType::Ortho: return 2.0 / m_projection_matrix.matrix()(1, 1);
    }
}
...
{
    switch (m_type) {
    case EType::Perspective: return 2.0 * Geometry::rad2deg(std::atan(1.0 / m_projection_matrix.matrix()(1, 1)));
    default:
    case EType::Ortho: return 0.0;
    };
}

void Camera::set_viewport(int x, int y, unsigned int w, unsigned int h) { m_viewport = {0, 0, int(w), int(h)}; }

void Camera::apply_viewport() const { glsafe(::glViewport(m_viewport[0], m_viewport[1], m_viewport[2], m_viewport[3])); }

// [INTENT] Computes and applies the projection matrix based on current viewport and zoom.
// [UNITY] Unity's Camera handles projection matrix via its own properties (Field of View, Orthographic Size).
// Manual matrix construction should be avoided in Unity unless doing custom rendering effects.
...
// [INTENT] Construct the 4x4 projection matrix manually for OpenGL.
// [UNITY] Use Camera.projectionMatrix if manual control is needed, but prefer standard Camera properties.
void Camera::apply_projection(double left, double right, double bottom, double top, double near_z, double far_z)
{
    ...
    default:
    case EType::Ortho: {
        m_projection_matrix.matrix() << 2.0 * inv_dx, 0.0, 0.0, -(left + right) * inv_dx, 0.0, 2.0 * inv_dy, 0.0, -(bottom + top) * inv_dy,
            0.0, 0.0, -2.0 * inv_dz, -(near_z + far_z) * inv_dz, 0.0, 0.0, 0.0, 1.0;
        break;
    }
    case EType::Perspective: {
        m_projection_matrix.matrix() << 2.0 * near_z * inv_dx, 0.0, (left + right) * inv_dx, 0.0, 0.0, 2.0 * near_z * inv_dy,
            (bottom + top) * inv_dy, 0.0, 0.0, 0.0, -(near_z + far_z) * inv_dz, -2.0 * near_z * far_z * inv_dz, 0.0, 0.0, -1.0, 0.0;
        break;
    }
    }
}

// [INTENT] Orbital rotation around a target point.
// [UNITY] Maps to orbiting a target Transform or point using Quaternion.Euler or RotateAround.
...
void Camera::debug_render() const
{
    ...
    imgui.begin(std::string("Camera statistics"),
...
double Camera::calc_zoom_to_bounding_box_factor(const BoundingBoxf3& box, double margin_factor) const
{
    ...
    const std::vector<Vec3d> vertices = {box.min,
...
// [INTENT] Recompute view matrix to look from 'position' to 'target' with 'up' vector.
// [UNITY] Maps directly to Transform.LookAt or Matrix4x4.LookAt.
...
Vec3d Camera::validate_target(const Vec3d& target) const
{
    ...
    return {std::clamp(target(0), test_box.min(0), test_box.max(0)), std::clamp(target(1), test_box.min(1), test_box.max(1)),
