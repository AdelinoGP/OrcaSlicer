// [ANNOTATED]
// [INTENT] Implementation of the AMS humidity and drying status popup dialog.
// [STATE] Tracks real-time telemetry for humidity levels, percentages, and remaining drying time.
// [UNITY] Map to a retained Unity UI Toolkit VisualElement or Canvas-based Popup prefab.
// [UNITY] Use Unity's Localization system for labels ("Humidity", "Temperature", "Left Time").
// [PORTING_HAZARD:P3] Manual DPI scaling and bitmap management (ScalableBitmap) should be replaced by native Unity UI scaling.
...
uiAmsPercentHumidityDryPopup::uiAmsPercentHumidityDryPopup(wxWindow* parent) : wxDialog(parent, wxID_ANY, "")
{
    Create();
...
{
    if (m_humidity_level != humidiy_level || m_humidity_percent != humidity_percent || m_left_dry_time != left_dry_time ||
