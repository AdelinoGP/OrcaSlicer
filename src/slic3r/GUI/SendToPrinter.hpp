...
// [INTENT] Modal send-to-printer dialog boundary: printer discovery, storage selection, rename handling, and upload progress all live behind
// this declaration. [STATE] Owns the current plate, connection mode, timers, transient transfer jobs, and the mutable selection/UI widgets
// that mirror the active printer. [EVENT] The cpp wires close, refresh, selection, rename, timer, and upload callbacks onto this surface.
// [THREAD] Connection and upload work is kicked off through backend jobs, then marshaled back to the dialog on the UI thread.
// [UNITY] Map this to a modal controller with a ScriptableObject-backed printer/device model and an async upload service.
// [PORTING_HAZARD:P2] Protocol selection, timeout handling, and dialog re-entry are interleaved with widget ownership, so the state machine
// needs to be split carefully.
namespace Slic3r {
...
{
private:
    // [STATE] Workflow flags and cached printer/device identifiers drive the current step and retry behavior.
    void init_bind();
    void init_timer();
...
private:
    ...
    // [STATE] Connection status, transfer handles, and the last refresh timestamp coordinate polling and teardown.
...
public:
    // [INTENT] Public methods expose the dialog lifecycle, status transitions, and event handlers used by the cpp wiring.
    // [EVENT] Rename, selection, timer, close, refresh, and print completion events all route through this interface.
...
private:
    // [THREAD] These helpers create and tear down connection/upload jobs and marshal callbacks back to the dialog thread.
    // [UNITY] This is the seam for a main-thread controller plus async service split; avoid baking network state into view widgets.
