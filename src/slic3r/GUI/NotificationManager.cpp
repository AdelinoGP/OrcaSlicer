...
#if 1
// [INTENT] NotificationManager::NotificationIDProvider manages the allocation and recycling of unique integer IDs for ImGui windows.
// This prevents ID conflicts when notifications are dynamically created and destroyed.
// [UNITY] Not directly applicable as Unity UI Toolkit manages its own element IDs. However, if a similar pooling
// mechanism is needed for performance with custom UI components, a C# class could implement this.
...
void NotificationManager::NotificationIDProvider::release_id(int id) { m_released_ids.push_back(id); }
#else
...
//------PopNotification--------
// [INTENT] PopNotification is a base class for various pop-up notifications displayed in the GUI.
// It manages the notification's data, state, visual appearance, and interaction logic (hover, click, fading).
// [UNITY] This class would be refactored into a C# MonoBehaviour controlling a UI Toolkit VisualElement.
// The rendering logic would move from ImGui calls to UI Toolkit's USS styling and VisualElement API.
// States like fading and hovering would be managed by C# logic or UI Toolkit's state classes.
NotificationManager::PopNotification::PopNotification(const NotificationData& n,
    ...
    : m_data(n)                                         // [STATE] Stores the immutable data associated with the notification.
    , m_id_provider(id_provider)                        // [STATE] Reference to the ID provider for unique ImGui window IDs.
    , m_text1(n.text1)                                  // [STATE] Primary text content of the notification.
    , m_hypertext(n.hypertext)                          // [STATE] Hyperlink text content.
    , m_text2(n.text2)                                  // [STATE] Secondary text content.
    , m_evt_handler(evt_handler)                        // [STATE] wxWidgets event handler for dispatching custom events.
    , m_notification_start(GLCanvas3D::timestamp_now()) // [STATE] Timestamp when the notification was created, for duration tracking.
{
    // [STATE] ImGui colors for different notification levels (Error, Warning, Normal).
    // [UNITY] These colors would be defined in USS variables or ScriptableObjects for theming.
...
// [INTENT] Ensures that UI-related properties like dark mode status and window radius are initialized
// from the current GLCanvas3D settings. This is deferred from the constructor due to dependency on plater().
// [UNITY] Initialization logic would be part of a MonoBehaviour's `Awake` or `Start` method,
// accessing relevant UI settings from a singleton or injected dependency.
void NotificationManager::PopNotification::ensure_ui_inited()
{
    if (!m_is_dark_inited) {
        // [STATE] Retrieves and sets the dark mode status.
        // [UNITY] Accessing global theme settings from a C# manager class.
        m_is_dark        = wxGetApp().plater()->get_current_canvas3D()->get_dark_mode_status();
        m_is_dark_inited = true;
    }

    if (!m_WindowRadius_inited) {
        // [STATE] Initializes the window corner radius based on canvas scaling.
        // [UNITY] UI Toolkit's `border-radius` CSS property would handle this, potentially
        // scaled by a global UI scaling factor.
...
void NotificationManager::PopNotification::set_delete_callback(DeleteCallback callback) { m_on_delete_callback = callback; }
...
void NotificationManager::PopNotification::use_bbl_theme()
{
    ...
    m_is_dark ? push_style_color(ImGuiCol_Border, {62 / 255.f, 62 / 255.f, 69 / 255.f, 1.f}, true, m_current_fade_opacity) :
...
void NotificationManager::PopNotification::render(
    GLCanvas3D& canvas, float initial_y, bool move_from_overlay, float overlay_width, float right_margin)
{
    ...
    if (mouse_pos.x < win_pos.x && mouse_pos.x > win_pos.x - m_window_width && mouse_pos.y > win_pos.y &&
        mouse_pos.y < win_pos.y + m_window_height) {
    ...
    if (imgui.begin(name, window_flags)) {
        ...
        render_close_button(imgui, win_size.x, win_size.y, win_pos.x,
                            win_pos.y); // ORCA draw it after minimize button since its position related to minimize button
    }
    imgui.end();
    ...
    GLCanvas3D& canvas, float initial_y, bool move_from_overlay, float overlay_width, float right_margin)
{
    ...
    use_bbl_theme();
    if (m_data.level == NotificationLevel::SeriousWarningNotificationLevel) { // ORCA match and ensure color usage
    ...
    if (m_data.level == NotificationLevel::ErrorNotificationLevel) { // ORCA match and ensure color usage
...
void NotificationManager::PopNotification::count_lines()
{
    ...
    while (last_end < text.length()) {
        ...
        if (next_hard_end != std::string::npos &&
            ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x < m_window_width - m_window_width_offset) {
            ...
            if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset) {
                ...
                if (next_space > 0 && next_space < text.length()) {
                    ...
                    while (next_space_candidate > 0 &&
                           ImGui::CalcTextSize(text.substr(last_end, next_space_candidate - last_end).c_str()).x <
                ...
                if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width - m_window_width_offset ||
                    ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x <
                        (m_window_width - m_window_width_offset) / 4 * 3) {
                    // BBS
                    wxString wx_text       = from_u8(text.substr(last_end, text.length() - last_end).c_str());
                    float    width_of_char = ImGui::CalcTextSize("a").x;
                    int      letter_count  = (int) ((m_window_width - m_window_width_offset) / width_of_char) /
                                       2; // give a predict value of char count
                    int output_count = 0;
                    while (true) {
    ...
    if (!m_hypertext.empty()) {
        int prev_end = m_endlines.size() > 1 ? m_endlines[m_endlines.size() - 2] :
                                               0; // m_endlines.size() - 2 because we are fitting hypertext instead of last endline
        if (ImGui::CalcTextSize((escape_string_cstyle(text.substr(prev_end, last_end - prev_end)) + m_hypertext).c_str()).x >
            m_window_width - m_window_width_offset) {
...
{
    m_window_height = m_multiline ? std::max(m_lines_count, (size_t) 2) * m_line_height : 2 * m_line_height;
    m_window_height += 1 * m_line_height; // top and bottom
}

void NotificationManager::PopNotification::bbl_render_block_notif_text(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    float       x_offset   = m_left_indentation;
    int         last_end   = 0;
    float       starting_y = (m_lines_count == 2 ? win_size_y / 2 - m_line_height :
                                                   (m_lines_count == 1 ? win_size_y / 2 - m_line_height / 2 : m_line_height / 2));
    ...
    for (size_t i = 0; i < (m_multiline ? m_endlines.size() : std::min(m_endlines.size(), (size_t) 2)); i++) {
        ...
        if (m_endlines.size() > i && m_text1.size() >= m_endlines[i]) {
            ...
            if (pos_start != string::npos && pos_end != string::npos && m_endlines[i] - line.length() >= pos_start &&
                m_endlines[i] <= pos_end) {
...
void NotificationManager::PopNotification::render_text(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    float       x_offset   = m_left_indentation;
    int         last_end   = 0;
...
void NotificationManager::PopNotification::render_hypertext(
    ImGuiWrapper& imgui, const float text_x, const float text_y, const std::string text, bool more)
{
    // invisible button
    ...
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd,
                                        IM_COL32((int) (HyperColor.x * 255), (int) (HyperColor.y * 255), (int) (HyperColor.z * 255),
                                                 (int) (HyperColor.w * 255.f *
                                                        (m_state == EState::FadingOut ? m_current_fade_opacity : 1.f))));
}

void NotificationManager::PopNotification::render_close_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ensure_ui_inited();
    ...
    //	                           true))
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y), ImVec2(win_pos.x, win_pos.y + 2 * m_line_height + 10),
                                   true)) {
...
void NotificationManager::PopNotification::bbl_render_block_notif_buttons(ImGuiWrapper& imgui, ImVec2 win_size, ImVec2 win_pos)
{
    ...
    // m_current_fade_opacity); push_style_color(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f), m_state == EState::FadingOut,
    // m_current_fade_opacity); push_style_color(ImGuiCol_TextSelectedBg, ImVec4(0, .75f, .75f, 1.f), m_state == EState::FadingOut,
...
void NotificationManager::PopNotification::bbl_render_left_sign(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ensure_ui_inited();
    ImDrawList* draw_list       = ImGui::GetWindowDrawList();
    ImVec2      round_rect_pos  = ImVec2(win_pos_x - win_size_x + ImGui::GetStyle().WindowBorderSize,
                                         win_pos_y + ImGui::GetStyle().WindowBorderSize);
...
void NotificationManager::PopNotification::render_left_sign(ImGuiWrapper& imgui)
{
    ...
    //	ImGui::SetCursorPosY(m_window_height / 2 - m_line_height);
    //	imgui.text(text.c_str());
    // } else if (m_data.level == NotificationLevel::PrintInfoNotificationLevel || m_data.level ==
    // NotificationLevel::PrintInfoShortNotificationLevel) { 	std::wstring text; 	text = ImGui::InfoMarker; ImGui::SetCursorPosX(m_line_height
    /// 3); 	ImGui::SetCursorPosY(m_window_height / 2 - m_line_height); 	imgui.text(text.c_str());
    // }
}
void NotificationManager::PopNotification::render_minimize_button(ImGuiWrapper& imgui, const float win_pos_x, const float win_pos_y)
{
    ...
    button_text = m_is_dark ? ImGui::MinimalizeDarkButton : ImGui::MinimalizeButton;
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / 10.f, win_pos_y + m_window_height - 2 * m_line_height + 1),
                                   ImVec2(win_pos_x, win_pos_y + m_window_height), true)) {
        button_text = m_is_dark ? ImGui::MinimalizeHoverDarkButton : ImGui::MinimalizeHoverButton;
    }
    ImVec2 button_pic_size = ImGui::CalcTextSize(into_u8(button_text).c_str());
    ImVec2 button_size(button_pic_size.x * 1.25f, button_pic_size.y * 1.25f);
...
bool NotificationManager::PopNotification::update_state(bool paused, const int64_t delta)
{
    ...
    } else if (m_state != EState::NotFading && m_state != EState::FadingOut && m_state != EState::ClosePending &&
               m_state != EState::Finished && get_duration() != 0 && !paused) {
...
void NotificationManager::ExportFinishedNotification::render_text(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    if (m_eject_pending) {
    ...
    for (size_t i = 0; i < m_lines_count; i++) {
        ...
        if (m_text1.size() >= m_endlines[i]) {
            ...
            if (i == 0 && !m_eject_pending && !m_export_dir_path.empty()) {
                render_hypertext(imgui, x_offset + ImGui::CalcTextSize(line.c_str()).x + ImGui::CalcTextSize("   ").x, starting_y,
                                 _u8L("Open Folder."));
            }
        }
    }
...
void NotificationManager::ExportFinishedNotification::render_close_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::ExportFinishedNotification::render_eject_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - m_line_height * 5.f, win_pos.y),
                                   ImVec2(win_pos.x - m_line_height * 2.5f, win_pos.y + win_size.y), true)) {
    ...
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    ImVec2 lineStart    = ImVec2(win_pos_x - win_size_x + m_left_indentation,
                                 win_pos_y + win_size_y / 2 + (m_multiline ? m_line_height / 2 : 0));
    ImVec2 midPoint     = ImVec2(lineStart.x + (lineEnd.x - lineStart.x) * m_percentage, lineStart.y);
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd,
                                        IM_COL32((int) (gray_color.x * 255), (int) (gray_color.y * 255), (int) (gray_color.z * 255),
                                                 (m_current_fade_opacity * 255.f)),
                                        m_line_height * 0.2f);
    ImGui::GetWindowDrawList()->AddLine(lineStart, midPoint,
                                        IM_COL32((int) (orange_color.x * 255), (int) (orange_color.y * 255), (int) (orange_color.z * 255),
                                                 (m_current_fade_opacity * 255.f)),
                                        m_line_height * 0.2f);
...
void NotificationManager::UpdatedItemsInfoNotification::add_type(InfoItemType type)
{
    ...
    for (it = m_types_and_counts.begin(); it != m_types_and_counts.end(); ++it) {
        ...
        case InfoItemType::CustomSupports:
            text += format(_L_PLURAL("%1$d Object has custom supports.", "%1$d Objects have custom supports.", (*it).second), (*it).second) +
                    "\n";
            break;
        // BBS
        // case InfoItemType::CustomSeam:          text += format(("%1$d Object has custom seam.",			"%1$d Objects have custom
        // seam.",			(*it).second), (*it).second) + "\n"; break;
        case InfoItemType::MmSegmentation:
            ...
            break;
        // BBS
        // case InfoItemType::Sinking:             text += format(("%1$d Object has partial sinking.",		"%1$d Objects have partial
        // sinking.",		(*it).second), (*it).second) + "\n"; break;
        case InfoItemType::CutConnectors:
            text += format(_L_PLURAL("%1$d object was loaded as a part of cut object.", "%1$d objects were loaded as parts of cut object.",
                                     (*it).second),
                           (*it).second) +
                    "\n";
            break;
        case InfoItemType::FuzzySkin:
            text += format(_L_PLURAL("%1$d object was loaded with fuzzy skin painting.",
                                     "%1$d objects were loaded with fuzzy skin painting.", (*it).second),
                           (*it).second) +
                    "\n";
            break;
...
void NotificationManager::URLDownloadNotification::render_close_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_close_button_inner(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y),
                                   ImVec2(win_pos.x, win_pos.y + win_size.y - (m_minimize_b_visible ? 2 * m_line_height : 0)), true)) {
        // Orca: Change based on dark mode
        button_text = m_is_dark ? ImGui::CloseNotifHoverDarkButton : ImGui::CloseNotifHoverButton;
    }
    ImVec2 button_pic_size = ImGui::CalcTextSize(button_text.c_str());
...
void NotificationManager::URLDownloadNotification::render_pause_cancel_buttons_inner(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_pause_button_inner(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
    ...
    std::wstring button_text;
    // Orca: Change based on dark mode
    button_text = m_is_dark ? (m_download_paused ? ImGui::PlayDarkButton : ImGui::PauseDarkButton) :
                              (m_download_paused ? ImGui::PlayButton : ImGui::PauseButton);

    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - m_line_height * 5.f, win_pos.y),
                                   ImVec2(win_pos.x - m_line_height * 2.5f, win_pos.y + win_size.y), true)) {
        // Orca: Change based on dark mode
        button_text = m_is_dark ? (m_download_paused ? ImGui::PlayHoverDarkButton : ImGui::PauseHoverDarkButton) :
                                  (m_download_paused ? ImGui::PlayHoverButton : ImGui::PauseHoverButton);
    }
    ...
    ImGui::SetCursorPosY(win_size.y / 2 - button_size.y);
    if (imgui.button(button_text.c_str(), button_size.x, button_size.y)) {
        trigger_user_action_callback(m_download_paused ? DownloaderUserAction::DownloadUserContinued :
                                                         DownloaderUserAction::DownloadUserPaused);
    }
...
void NotificationManager::URLDownloadNotification::render_open_button_inner(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
...
void NotificationManager::URLDownloadNotification::render_cancel_button_inner(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
...
void NotificationManager::URLDownloadNotification::render_bar(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::PrintHostUploadNotification::render_bar(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    std::string text;
    switch (m_uj_state) {
    case Slic3r::GUI::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_PROGRESS: {
        ...
        std::stringstream stream;
        stream << std::fixed << std::setprecision(2) << (int) (m_percentage * 100) << "% - " << uploaded << " of " << m_file_size
               << "MB uploaded";
        text = stream.str();
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / 2 + win_size_y / 6 - (m_multiline ? 0 : m_line_height / 4));
...
void NotificationManager::ProgressIndicatorNotification::render_cancel_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
...
void NotificationManager::ProgressIndicatorNotification::render_close_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
//------NotificationManager--------
NotificationManager::NotificationManager(wxEvtHandler* evt_handler) : m_evt_handler(evt_handler) {}
...
void NotificationManager::push_delayed_notification(const NotificationType    type,
                                                    ...
                                                    int64_t                   delay_interval)
{
    ...
    assert(it != std::end(basic_notifications));
    if (it != std::end(basic_notifications))
        push_delayed_notification_data(std::make_unique<PopNotification>(*it, m_id_provider, m_evt_handler), condition_callback,
                                       initial_delay, delay_interval);
}
...
void NotificationManager::push_validate_error_notification(StringObjectException const& error)
{
    ...
    if (mo || !error.opt_key.empty()) {
        callback = [id = mo ? mo->id() : 0, opt = error.opt_key](wxEvtHandler*) {
    ...
    if (!error.opt_key.empty())
        link += std::string(" (") + error.opt_key + ")";
    push_notification_data({NotificationType::ValidateError, NotificationLevel::ErrorNotificationLevel, 0,
...
void NotificationManager::push_slicing_warning_notification(const std::string& text,
                                                            bool               gray,
                                                            ModelObject const* obj,
                                                            ObjectID           oid,
                                                            int                warning_step,
                                                            int                warning_msg_id,
...
void NotificationManager::push_slicing_customize_error_notification(NotificationType                   type,
                                                                    NotificationLevel                  level,
                                                                    const std::string&                 text,
                                                                    const std::string&                 hypertext,
                                                                    std::function<bool(wxEvtHandler*)> callback)
...
void NotificationManager::push_flushing_volume_error_notification(NotificationType                   type,
                                                                  NotificationLevel                  level,
                                                                  const std::string&                 text,
                                                                  const std::string&                 hypertext,
                                                                  std::function<bool(wxEvtHandler*)> callback)
...
{
    NotificationData data{NotificationType::SimplifySuggestion,
                          NotificationLevel::PrintInfoNotificationLevel,
                          0,
                          text,
                          hypertext,
                          callback};
...
void NotificationManager::push_exporting_finished_notification(const std::string& path, const std::string& dir_path, bool on_removable)
{
    ...
    NotificationData data{NotificationType::ExportFinished, NotificationLevel::RegularNotificationLevel, on_removable ? 0 : 20,
                          _u8L("Exported successfully") + "\n" + path};
    push_notification_data(std::make_unique<NotificationManager::ExportFinishedNotification>(data, m_id_provider, m_evt_handler,
                                                                                             on_removable, path, dir_path),
                           0);
    set_slicing_progress_hidden();
}
...
void NotificationManager::push_download_URL_progress_notification(size_t                                         id,
                                                                  const std::string&                             text,
                                                                  std::function<bool(DownloaderUserAction, int)> user_action_callback)
{
    // If already exists
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::URLDownload &&
            dynamic_cast<URLDownloadNotification*>(notification.get())->get_download_id() == id) {
            return;
        }
    }
    // push new one
    NotificationData data{NotificationType::URLDownload, NotificationLevel::ProgressBarNotificationLevel, 5, _u8L("Download") + ": " + text};
    push_notification_data(std::make_unique<NotificationManager::URLDownloadNotification>(data, m_id_provider, m_evt_handler, id,
...
void NotificationManager::push_upload_job_notification(
    int id, float filesize, const std::string& filename, const std::string& host, float percentage)
{
    ...
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::PrintHostUpload &&
            dynamic_cast<PrintHostUploadNotification*>(notification.get())->compare_job_id(id)) {
            return;
        }
    }
    ...
    push_notification_data(std::make_unique<NotificationManager::PrintHostUploadNotification>(data, m_id_provider, m_evt_handler, 0, id,
                                                                                              filesize),
                           0);
}
void NotificationManager::set_upload_job_notification_percentage(int                id,
...
void NotificationManager::push_slicing_serious_warning_notification(const std::string& text, std::vector<ModelObject const*> objs)
{
    ...
    push_notification_data({NotificationType::SlicingSeriousWarning, NotificationLevel::SeriousWarningNotificationLevel, 0,
                            _u8L("Serious warning:") + "\n" + text, link, callback},
                           0);
    set_slicing_progress_hidden();
}
...
{
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::SlicingSeriousWarning &&
            notification->compare_text(_u8L("Serious warning:") + "\n" + text)) {
            notification->close();
        }
    }
}
...
void NotificationManager::init_slicing_progress_notification(std::function<bool()> cancel_callback)
{
    ...
    NotificationData data{NotificationType::SlicingProgress,
    ...
    push_notification_data(std::make_unique<NotificationManager::SlicingProgressNotification>(data, m_id_provider, m_evt_handler,
                                                                                              cancel_callback),
                           0);
}
...
void NotificationManager::push_hint_notification(bool open_next)
{
    ...
    if (!open_next) {
        ...
        auto condition = [&self = std::as_const(*this)]() { return self.get_notification_count() == 0; };
        push_delayed_notification_data(std::make_unique<NotificationManager::HintNotification>(data, m_id_provider, m_evt_handler,
                                                                                               open_next),
                                       condition, 500, 30000);
    }
}
...
bool NotificationManager::push_notification_data(std::unique_ptr<NotificationManager::PopNotification> notification, int timestamp)
{
    ...
    if (!notification->is_valid_delete_callback()) {
        auto delete_self = [this](NotificationManager::PopNotification* it) { m_to_delete_after_finish_render = it; };
                                                         ...
                                                         int64_t                                               delay_interval)
{
    ...
    m_waiting_notifications.emplace_back(std::move(notification), condition_callback, initial_delay == 0 ? delay_interval : initial_delay,
                                         delay_interval);
...
void NotificationManager::render_notifications(GLCanvas3D& canvas, float overlay_width, float bottom_margin, float right_margin)
{
    ...
    for (const auto& notification : m_pop_notifications) {
        if (notification->get_data().level == NotificationLevel::ErrorNotificationLevel ||
            notification->get_data().level == NotificationLevel::SeriousWarningNotificationLevel) {
            notification->bbl_render_block_notification(canvas, bottom_up_last_y, m_move_from_overlay && !m_in_preview,
                                                        overlay_width * m_scale, right_margin); // ORCA dont scale margins
            if (notification->get_state() != PopNotification::EState::Finished)
                bottom_up_last_y = notification->get_top() + GAP_WIDTH;
        } else {
            if (notification->get_state() != PopNotification::EState::Hidden &&
                notification->get_state() != PopNotification::EState::Finished) {
                i++;
                notification->render(canvas, bottom_up_last_y, m_move_from_overlay && !m_in_preview, overlay_width * m_scale,
                                     right_margin); // ORCA dont scale margins
                if (notification->get_state() != PopNotification::EState::Finished)
                    bottom_up_last_y = notification->get_top() + GAP_WIDTH;
            }
        }
    ...
    for (const auto& notification : m_pop_notifications) {
        if (notification->get_data().type == NotificationType::SlicingProgress &&
            notification->get_state() != PopNotification::EState::Hidden &&
            notification->get_state() != PopNotification::EState::Finished) {
            ; // assert(i <= 1);
        }
    }
...
bool NotificationManager::update_notifications(GLCanvas3D& canvas)
{
    ...
    for (auto it = m_waiting_notifications.begin(); it != m_waiting_notifications.end();) {
        ...
        if ((*it).remaining_time <= 0) {
            if ((*it).notification &&
                (*it).condition_callback()) { // push notification, erase it from waiting list (frame is scheduled by push)
...
void NotificationManager::sort_notifications()
{
    ...
    std::stable_sort(m_pop_notifications.begin(), m_pop_notifications.end(),
                     [](const std::unique_ptr<PopNotification>& n1, const std::unique_ptr<PopNotification>& n2) {
...
{
    return std::any_of(m_pop_notifications.begin(), m_pop_notifications.end(),
                       [](auto& n) { return n->get_type() == NotificationType::SlicingError; });
}
...
{
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::ExportFinished &&
            dynamic_cast<ExportFinishedNotification*>(notification.get())->m_to_removable)
            notification->close();
    }
}
...
void NotificationManager::bbl_show_objectsinfo_notification(const std::string& text, bool is_warning, bool is_hidden)
{
    ...
    NotificationData data{NotificationType::BBLObjectInfo,
                          NotificationLevel::PrintInfoNotificationLevel,
...
void NotificationManager::bbl_show_plugin_install_notification(const std::string& text)
{
    ...
    NotificationData data{NotificationType::BBLPluginInstallHint,
...
{
    NotificationData data{NotificationType::BBLSliceEmptyLayer, NotificationLevel::WarningNotificationLevel, 0,
                          _u8L("WARNING:") + "\n" + text};
    ...
    for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::BBLSliceEmptyLayer) {
            if (bOverride)
                ...
                NotificationData sdata{NotificationType::BBLSliceEmptyLayer, NotificationLevel::WarningNotificationLevel, 0,
                                       notification->get_text1() + "\n" + text};
...
void NotificationManager::bbl_show_app_newversion_notification() { push_notification(NotificationType::NewAppAvailable); }
...
void NotificationManager::bbl_show_bed_filament_incompatible_notification(const std::string& text)
{
    ...
    push_notification_data({NotificationType::BBLBedFilamentIncompatible, NotificationLevel::ErrorNotificationLevel, 0,
                            _u8L("Error:") + "\n" + text, "Click for more.", callback},
                           0);
}
...
void NotificationManager::bbl_show_sole_text_notification(
    NotificationType sType, const std::string& text, bool bOverride, int level, bool autohide)
{
    ...
    case 1:
        ...
        break;
    case 2:
        nlevel = NotificationLevel::ErrorNotificationLevel;
        break;
        nHideTime = 0;
    case 0:
    default:
        ...
        if (autohide == false)
            nHideTime = BBL_NOTICE_MAX_INTERVAL;
        break;
    }
