// [ANNOTATED]
// [INTENT] Management and telemetry parsing for the multi-material filament system (AMS).
// [STATE] Deep state hierarchy: DevFilaSystem -> DevAms (Unit) -> DevAmsTray (Slot).
// [STATE] Tracks granular slot metadata including RFID tags, K/N factors, humidity, and color.
// [UNITY] Map to a C# AMSService managing a tree of observable unit and tray models.
// [PORTING_HAZARD:P2] Massive nested telemetry parsing logic with manual bit-field extraction and time-based "user pending" overrides.
...
namespace Slic3r {
static int _hex_digit_to_int(const char c)
...
{
    if (m_fila_type == "PLA-S")
        return "Sup.PLA";
    if (m_fila_type == "PA-S")
        return "Sup.PA";
    if (m_fila_type == "ABS-S")
        return "Sup.ABS";
    return m_fila_type;
}
...
{
    if (m_fila_type == "Sup.PLA") {
        return "PLA-S";
    }
    if (m_fila_type == "Sup.PA") {
        return "PA-S";
    }
    if (m_fila_type == "Sup.ABS") {
        return "ABS-S";
    }
    if (m_fila_type == "Support W") {
        return "PLA-S";
    }
    if (m_fila_type == "Support G") {
        return "PA-S";
    }
...
static unordered_map<int, wxString> s_ams_display_formats = {{DevAms::AMS, "AMS-%d"},
    ...
    if (jj.contains("ams")) {
        if (jj["ams"].contains("ams")) {
            ...
            if (jj["ams"].contains("cali_stat")) {
                system->m_ams_cali_stat = jj["ams"]["cali_stat"].get<int>();
            }
                    ...
                    if (it->contains("tray")) {
                        ...
                        for (auto tray_it = (*it)["tray"].begin(); tray_it != (*it)["tray"].end(); tray_it++) {
                            ...
                            if (tray_it->contains("tray_info_idx") && tray_it->contains("tray_type")) {
                                ...
                                std::string type = MachineObject::setting_id_to_type(curr_tray->setting_id,
                                                                                     (*tray_it)["tray_type"].get<std::string>());
                            ...
                            try {
                                if (!ams_id.empty() && !curr_tray->id.empty()) {
                                    ...
                                    if (type_id < 4) {
                                        curr_tray->is_exists = (obj->tray_exist_bits & (1 << (ams_id_int * 4 + tray_id_int))) != 0 ? true :
                                                                                                                                     false;
                                    } else {
                            ...
                            auto diff      = std::chrono::duration_cast<std::chrono::milliseconds>(curr_time -
                                                                                                   obj->extrusion_cali_set_hold_start);
                            if (diff.count() > HOLD_TIMEOUT || diff.count() < 0 || ams_id_int != (obj->extrusion_cali_set_tray_id / 4) ||
