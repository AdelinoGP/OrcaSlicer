// [ANNOTATED]
// [INTENT] Management and telemetry parsing for the printer's extruder system (single/dual toolheads).
// [STATE] Tracks extruder IDs, temperatures, nozzle diameters, and associated AMS slot status.
// [UNITY] Map to a C# ExtruderService managing an observable collection of toolhead models.
// [PORTING_HAZARD:P2] Extensive bit-packed telemetry (V2.0) for toolhead state and AMS slot mapping must be mirrored.
...
NozzleType DevExtder::GetNozzleType() const { return system->Owner()->GetNozzleSystem()->GetNozzle(m_current_nozzle_id).m_nozzle_type; }
...
float DevExtder::GetNozzleDiameter() const { return system->Owner()->GetNozzleSystem()->GetNozzle(m_current_nozzle_id).m_diameter; }

DevExtderSystem::DevExtderSystem(MachineObject* obj) : m_owner(obj)
{
...
std::optional<DevExtder> DevExtderSystem::GetCurrentExtder() const { return GetExtderById(m_current_extder_id); }
