// [ANNOTATED]
// [INTENT] Implementation of the filament-to-AMS-slot mapping algorithm (automatic slot selection).
// [STATE] Builds a distance matrix between sliced filaments and available hardware trays based on color similarity and material type.
// [EVENT] Core logic entry point: ams_filament_mapping.
// [UNITY] Map to a C# FilamentMappingService. Logic is mostly pure and can be ported with standard C# collections.
// [PORTING_HAZARD:P2] Complex optimization heuristic for selecting the "best" slot match; must be mirrored exactly to maintain UX consistency.
...
int DevMappingUtil::ams_filament_mapping(const MachineObject*             obj,
                                         const std::vector<FilamentInfo>& filaments,
                                         std::vector<FilamentInfo>&       result,
                                         std::vector<bool>                map_opt,
                                         std::vector<int>                 exclude_id,
                                         bool                             nozzle_has_ams_then_ignore_ext)
{
    if (filaments.empty())
        return -1;
                ...
                if (distance_map[i][j].is_same_color && distance_map[i][j].is_type_match) {
                    if (min_val > distance_map[i][j].distance) {
                        ...
                        tray_filaments[picked_tar_idx].distance = min_val;
                               ...
                               filaments[picked_src_idx].filament_id != tray_filaments[picked_tar_idx].filament_id &&
