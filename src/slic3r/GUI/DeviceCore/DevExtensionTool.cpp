// [ANNOTATED]
// [INTENT] Parsers for printer extension tools (cutters, lasers, auxiliary fans) telemetry.
// [STATE] Tracks mounting, calibration status, and tool type for printer accessories.
// [UNITY] Map to a C# accessory data model within the PrinterService.
// [PORTING_HAZARD:P3] Uses weak_ptr for safe concurrent access during telemetry parsing; mirror with managed references or thread-safe
// state snapshots in C#.
...
void DevExtensionToolParser::ParseV2_0(const nlohmann::json& extension_tool_json, std::weak_ptr<DevExtensionTool> extension_tool)
{
    if (auto ext_tool = extension_tool.lock()) {
...

        {
            ...
            static std::map<std::string, DevExtensionTool::ToolType> s_type_map = {{"CP00", DevExtensionTool::TOOL_TYPE_CUT_CP00},
