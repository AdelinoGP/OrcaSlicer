// [ANNOTATED]
// [INTENT] Central manager for printer discovery, lifecycle, and state synchronization across local (LAN) and cloud units.
// [STATE] Maintains master lists of local and user-bound MachineObjects; tracks global selection and connectivity status.
// [EVENT] Orchestrates keep-alive heartbeats, periodic telemetry polling, and machine-added/lost notifications.
// [THREAD] Synchronizes access to machine lists via mutexes; drives background refresh cycles using a UI-thread timer.
// [UNITY] Map to a persistent C# DeviceService managing an observable collection of MachineObject models.
// [PORTING_HAZARD:P2] Intricate logic for merging local and cloud discovery results while maintaining stable device IDs and IP mappings.
...
{
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": updating agent for " << localMachineList.size() << " local and " << userMachineList.size()
                            << " user machines";
    m_agent = agent;
...
void DeviceManager::on_machine_alive(std::string json_str)
{
    try {
        ...
        if (it != userMachineList.end()) {
            if (it->second->get_dev_ip() != dev_ip || it->second->bind_state != bind_state || it->second->bind_sec_link != sec_link ||
                it->second->dev_connection_type != connect_type || it->second->bind_ssdp_version != ssdp_version) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " UpdateUserMachineInfo"
                                        << ", dev_id= " << dev_id << ", ip = " << dev_ip << ", printer_name= " << dev_name
        ...
        if (it != localMachineList.end()) {
            ...
            if (obj->get_dev_ip().compare(dev_ip) != 0) {
                if (connection_name.empty()) {
                    BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << obj->get_dev_ip() << " to " << dev_ip;
                    obj->set_dev_ip(dev_ip);
                } else {
                    if (obj->dev_connection_name.empty() || obj->dev_connection_name.compare(connection_name) == 0) {
                        BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << obj->get_dev_ip() << " to " << dev_ip
                                                << " connection_name is " << connection_name;
                        if (obj->dev_connection_name.empty()) {
            ...
            if (obj->wifi_signal != printer_signal || obj->dev_connection_type != connect_type || obj->bind_state != bind_state ||
                obj->bind_sec_link != sec_link || obj->bind_ssdp_version != ssdp_version ||
                obj->printer_type != _parse_printer_type(printer_type_str)) {
                if (obj->dev_connection_type != connect_type || obj->bind_state != bind_state || obj->bind_sec_link != sec_link ||
                    obj->bind_ssdp_version != ssdp_version || obj->printer_type != _parse_printer_type(printer_type_str)) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " New Machine, dev_id= " << dev_id << ", ip = " << dev_ip
                                    << ", printer_name = " << dev_name << ", con_type= " << connect_type << ", signal= " << printer_signal
                                    << ", bind_state= " << bind_state;
        }
...
MachineObject* DeviceManager::insert_local_device(
    const BBLocalMachine& machine, std::string connection_type, std::string bind_state, std::string version, std::string access_code)
{
    MachineObject* obj;
...
{
    BOOST_LOG_TRIVIAL(info) << "set_selected_machine=" << dev_id << " cur_selected=" << selected_machine;
    ...
    if (it != my_machine_list.end()) {
        if (selected_machine == dev_id) {
            ...
            if (it->second->connection_type() != "lan") {
                BOOST_LOG_TRIVIAL(info) << "set_selected_machine: same cloud machine, dev_id =" << dev_id << ", just reset update time";
            ...
            // same dev_id, lan => disconnect and reconnect
                ...
                BOOST_LOG_TRIVIAL(info) << "set_selected_machine: same lan machine, dev_id =" << dev_id << ", disconnect and reconnect";
...
void DeviceManager::OnSelectedMachineChanged(const std::string& /*pre_dev_id*/, const std::string& /*new_dev_id*/)
{
