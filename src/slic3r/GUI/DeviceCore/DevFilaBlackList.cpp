// [ANNOTATED]
// [INTENT] Validation logic for filament compatibility with AMS units and specific printer models.
// [STATE] Loads and queries a static blacklist JSON configuration (filaments_blacklist.json).
// [UNITY] Map to a C# FilamentValidationService using ScriptableObjects for the blacklist data.
// [PORTING_HAZARD:P2] Complex multi-factor matching (vendor, type, suffix, model ID) used to prevent hardware damage; must be mirrored exactly.
                     ...
                     wxString&    wiki_url)
{
    ...
    if (DevFilaBlacklist::filaments_blacklist.contains("blacklist")) {
        for (auto filament_item : DevFilaBlacklist::filaments_blacklist["blacklist"]) {
            ...
            std::vector<std::string> model_ids = filament_item.contains("model_id") ?
                                                     filament_item["model_id"].get<std::vector<std::string>>() :
            ...
            // check model id
            if (!model_ids.empty() && std::find(model_ids.begin(), model_ids.end(), model_id) == model_ids.end()) {
                continue;
            }
            ...
            if (!vendor.empty()) {
                if ((vendor == "bambu lab" && (tag_vendor == vendor)) || (vendor == "third party" && (tag_vendor != "bambu lab"))) {
                    // Do nothing
                } else {
                    continue;
                }
            ...
            if (!type_suffix.empty()) {
                if (tag_type.length() < type_suffix.length()) {
                    continue;
                }
                if ((tag_type.substr(tag_type.length() - type_suffix.length()) != type_suffix)) {
                    continue;
                }
            }
            ...
            L("The rough surface of PLA Glow can accelerate wear on the AMS system, particularly on the internal components of the AMS "
              "Lite.");
...
{
    wxString wiki_url;
    check_filaments_in_blacklist_url(model_id, tag_vendor, tag_type, filament_id, ams_id, slot_id, tag_name, in_blacklist, ac, info,
                                     wiki_url);
}

bool check_filaments_printable(const std::string& tag_vendor,
...
void DevFilaBlacklist::check_filaments_in_blacklist_url(std::string        model_id,
                                                        std::string        tag_vendor,
                                                        std::string        tag_type,
                                                        const std::string& filament_id,
                                                        int                ams_id,
