...
#error \
    "OpenGL backend mismatch: GLEW has EGL support enabled but wxWidgets does not. Ensure GLEW_USE_EGL and wxUSE_GLCANVAS_EGL are both ON or both OFF."
#endif
#if !defined(GLEW_EGL) && defined(wxUSE_GLCANVAS_EGL) && wxUSE_GLCANVAS_EGL
#error \
    "OpenGL backend mismatch: wxWidgets has EGL support enabled but GLEW does not. Ensure GLEW_USE_EGL and wxUSE_GLCANVAS_EGL are both ON or both OFF."
#endif
#endif

// [INTENT] Singleton or manager responsible for OpenGL context initialization, shader management, and capability detection.
// [UNITY] Replace with a dedicated ScriptableObject or MonoBehaviour controller managing Camera + RenderTexture.
...
// [INTENT] Structure for OpenGL capabilities detection.
// [UNITY] Use SystemInfo or QualitySettings for capability querying.
...
bool OpenGLManager::GLInfo::is_mesa() const { return boost::icontains(m_version, "mesa"); }
...
void OpenGLManager::GLInfo::detect() const
{
    ...
    *const_cast<std::string*>(&m_renderer)     = gl_get_string_safe(GL_RENDERER, "N/A");
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("got opengl version %1%, glsl version %2%, vendor %3% , graphics card model %4%") % m_version %
                                   m_glsl_version % m_vendor % m_renderer
                            << std::endl;
...
// [INTENT] Initialize GLEW, check framebuffers, and load shaders.
// [UNITY] Use Addressables or Resources for Shader loading; ensure proper GL context initialization in MonoBehaviour.Start().
bool OpenGLManager::init_gl(bool popup_error)
{
    if (!m_gl_initialized) {
        ...
        // [OPENGL] Setup framebuffer objects based on ARB or Ext.
        ...
        if (!valid_version) {
            ...
            if (popup_error) {
                wxString message = from_u8(
                    (boost::format(_utf8(L("The application cannot run normally because OpenGL version is lower than 3.2.\n")))).str());
                message += "\n";
        ...
        if (valid_version) {
            ...
            if (!result) {
                ...
                if (popup_error) {
                    wxString message = from_u8((boost::format(_utf8(L("Unable to load shaders:\n%s"))) % error).str());
...
wxGLCanvas* OpenGLManager::create_wxglcanvas(wxWindow& parent)
{
    int attribList[] =
        {WX_GL_RGBA, WX_GL_DOUBLEBUFFER,
         ...
         // Requesting an 8 bit alpha channel. Interestingly, the NVIDIA drivers would most likely work with some alpha plane, but
         // glReadPixels would not return the alpha channel on NVIDIA if not requested when the GL context is created.
         WX_GL_MIN_ALPHA, 8, WX_GL_DEPTH_SIZE, 24,
         // BBS: turn on stencil buffer for outline
         WX_GL_STENCIL_SIZE, 8, WX_GL_SAMPLE_BUFFERS, GL_TRUE, WX_GL_SAMPLES, 4, 0};
...
void OpenGLManager::detect_multisample(int* attribList)
{
    ...
    s_multisample           = enable_multisample &&
                            ...
                            platform_flavor() != PlatformFlavor::LinuxOnChromium && wxGLCanvas::IsDisplaySupported(attribList) ?
                                  EMultisampleState::Enabled :
                                  EMultisampleState::Disabled;
