...
static const std::map<int, std::string> error_messages =
    {{SendToPrinterDialog::ERROR_PIPE,
      L("Reconnecting the printer, the operation cannot be completed immediately, please try again later.")},
     ...
     {SendToPrinterDialog::API_VERSION_UNSUPPORT,
      L("The firmware version of the printer is too low. Please update the firmware and try again.")},
...
// [INTENT] Map file-transfer backend error codes back into user-facing copy for the send modal.
// [UNITY] Model this as a view-model lookup table feeding the dialog status banner / validation panel.
    ...
    , m_export_3mf_cancel(false)
{
    ...
    auto timeimg = new wxStaticBitmap(m_scrollable_region, wxID_ANY, create_scaled_bitmap("print-time", this, 18), wxDefaultPosition,
    ...
    auto weightimg = new wxStaticBitmap(m_scrollable_region, wxID_ANY, create_scaled_bitmap("print-weight", this, 18), wxDefaultPosition,
                                        wxSize(FromDIP(18), FromDIP(18)), 0);
    ...
    m_connecting_printer_msg = new wxStaticText(m_connecting_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
    ...
    // bind
    Bind(EVT_SHOW_ERROR_INFO_SEND, [this](auto& e) { show_print_failed_info(true); });
...
// [STATE] The dialog keeps the send workflow as a small state machine: prepare, sending, and finish/error overlays.
// [EVENT] The cancel callback is wired into the progress bar so worker cancellation can unwind from the UI thread.
...
// [INTENT] Capture the plate index up front so later send/export steps operate on the same slice target.
void SendToPrinterDialog::prepare(int print_plate_idx) { m_print_plate_idx = print_plate_idx; }
...
{
    // [EVENT] Closing the modal must cancel worker jobs, stop polling timers, and then finish the dialog with cancel.
    // [PORTING_HAZARD:P2] Unity should treat this as a teardown path with explicit async task disposal before dismissing the modal.
    m_worker->cancel_all();
...
{
    // [INTENT] Drive the full send pipeline: validate printer selection, export slice data if needed, then upload
    // through either the direct file-transfer tunnel or the legacy FTP path.
...
{
    // [THREAD] Printer info fetch is moved off the UI thread, then marshaled back with CallAfter/posted events.
    // [UNITY] Replace this with an async service call plus main-thread continuation before touching UI state.
...
void SendToPrinterDialog::reset_timeout() { timeout_count = 0; }
...
void SendToPrinterDialog::on_timer(wxTimerEvent& event) { update_show_status(); }
...
{
    // [STATE] This method is the status-state machine for the modal; it gates button enablement, banner text, and
    // whether the dialog is in connect / read / send / finish mode.
...
void SendToPrinterDialog::set_default()
{
    ...
    if (data.is_valid()) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " : thumbnail_data invalid."
                                << "current plater: " << m_plater->get_partplate_list().get_curr_plate_index();
    }
...
{
    // [PORTING_HAZARD:P2] Connection setup is split across direct TCP and cloud-mediated tunnel paths with timeout retries.
    // [THREAD] The tunnel callbacks and timeout timer both re-enter the dialog on the UI thread.
    ...
    if (agent) {
        if (m_tcp_try_connect) {
            ...
            agent->get_camera_url(obj->get_dev_id() + "|" + dev_ver + "|" + protocols[1], [this, m = dev_id, v = agent->get_version(),
                                                                                           dv = dev_ver](std::string url) {
...
{
    // [THREAD] Connection result callbacks come back asynchronously and must update the dialog on the UI thread only.
    // [PORTING_HAZARD:P2] Unity should keep the tunnel object and retry policy inside a dedicated network service.
    if (is_success) {
        if (m_url_timer && m_url_timer->IsRunning()) {
            m_url_timer->Stop();
        }
    ...
    m_filetransfer_mediability_job->on_result([this](int res, int resp_ec, std::string json_res, std::vector<std::byte> bin_res) {
        ...
        CallAfter([this, res, resp_ec, json_res] {
            if (res == 0) // 0 is success
            {
                ...
                BOOST_LOG_TRIVIAL(error) << "CreateMediaAbilityJob::" << ":request mediaability failed. res =" << res
                                         << "resp_ec is:" << resp_ec;
                show_status(PrintDialogStatus::PrintStatusPublicInitFailed);
...
{
    // [THREAD] Upload progress and completion are emitted asynchronously and rebound onto the UI thread via CallAfter.
    ...
    m_filetransfer_uploadfile_job->on_result([this](int res, int resp_ec, std::string json_res, std::vector<std::byte> bin_res) { //
        CallAfter([this, res, resp_ec, json_res, bin_res] { UploadFileRessultCallback(res, resp_ec, json_res, bin_res); });
    });
...
{
    // [THREAD] Upload progress arrives from the file-transfer job and is replayed into the progress bar on the UI thread.
    bool     cancelled = false;
    ...
    if (progress == 99) {
        ...
        this->Bind(
            ...
            [this](auto e) {
                show_status(PrintDialogStatus::PrintStatusPublicUploadFiled);
                update_print_status_msg(_L("File upload timed out. Please check if the firmware version supports this operation or verify "
...
{
    // [THREAD] Upload completion also returns on the UI thread; success closes the send flow by posting back into Plater.
    // [PORTING_HAZARD:P2] The completion path still depends on timer cancellation and job ownership cleanup.
    if (m_task_timer && m_task_timer->IsRunning())
        m_task_timer->Stop();
...
// [STATE] Reset owns the dialog's transient session state: timeout timers, selected storage, tunnel jobs, and retries.
