// [ANNOTATED]
// [INTENT] Device print-options dialog for toggling printer-side AI detection, safety, and convenience features.
// [STATE] Mirrors a live `MachineObject` capability set into a large collection of switches, labels, and sensitivity selectors.
// [EVENT] Sends immediate printer commands from UI toggle/combobox handlers instead of staging edits locally.
// [UNITY] Port as a reactive settings panel backed by a device capability/state model rather than direct control-by-control mutation.
// [PORTING_HAZARD:P1] UI visibility, enablement, and device-command dispatch are interleaved, making ordering bugs easy during async state
// refreshes.
    ...
    : DPIDialog(parent, wxID_ANY, _L("Print Options"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_cb_save_remote_print_file_to_storage->Bind(wxEVT_TOGGLEBUTTON, [this](wxCommandEvent& evt) {
        if (obj) {
            obj->command_set_save_remote_print_file_to_storage(m_cb_save_remote_print_file_to_storage->GetValue());
        }
        evt.Skip();
    });
...
PrintOptionsDialog::~PrintOptionsDialog()
{
    ...
    // refine printer function options
    spaghetti_detection_level_list->Disconnect(wxEVT_COMBOBOX,
                                               wxCommandEventHandler(PrintOptionsDialog::set_spaghetti_detection_sensitivity), NULL, this);
    purgechutepileup_detection_level_list->Disconnect(wxEVT_COMBOBOX,
                                                      wxCommandEventHandler(PrintOptionsDialog::set_purgechutepileup_detection_sensitivity),
                                                      NULL, this);
    nozzleclumping_detection_level_list->Disconnect(wxEVT_COMBOBOX,
                                                    wxCommandEventHandler(PrintOptionsDialog::set_nozzleclumping_detection_sensitivity),
                                                    NULL, this);
    airprinting_detection_level_list->Disconnect(wxEVT_COMBOBOX,
                                                 wxCommandEventHandler(PrintOptionsDialog::set_airprinting_detection_sensitivity), NULL,
                                                 this);
}

void PrintOptionsDialog::on_dpi_changed(const wxRect& suggested_rect) { Fit(); }
...
void PrintOptionsDialog::update_options(MachineObject* obj_)
{
    ...
    // [PORTING_HAZARD:P2] Capability-gated show/hide logic is duplicated across many controls; porting should centralize these feature rules.
    if (obj_->is_support_spaghetti_detection || obj_->is_support_purgechutepileup_detection || obj_->is_support_nozzleclumping_detection ||
        obj_->is_support_airprinting_detection) {
    ...
    if (obj_->is_support_build_plate_marker_detect) {
        if (obj_->m_plate_maker_detect_type == MachineObject::POS_CHECK &&
            ...
            text_plate_mark_caption->SetLabel(
                _L("The localization tag of build plate is detected, and printing is paused if the tag is not in predefined range."));
            text_plate_mark_caption->Wrap(FromDIP(400));
        } else if (obj_->m_plate_maker_detect_type == MachineObject::TYPE_POS_CHECK &&
                   (text_plate_mark->GetLabel() != _L("Build Plate Detection"))) {
            text_plate_mark->SetLabel(_L("Build Plate Detection"));
            text_plate_mark_caption->SetLabel(
                _L("Identifies the type and position of the build plate on the heatbed. Pausing printing if a mismatch is detected."));
            text_plate_mark_caption->Wrap(FromDIP(400));
        }
...
wxBoxSizer* PrintOptionsDialog::create_settings_group(wxWindow* parent)
{
    ...
    line_sizer = new wxBoxSizer(wxHORIZONTAL);
    text_ai_detections_caption =
        new Label(ai_refine_panel,
                  _L("Printer will send assistant message or pause printing if any of the following problem is detected."));
    ...
    ai_monitoring_level_list = new ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(100), -1), 0, NULL,
                                            wxCB_READONLY);
    for (auto i = AiMonitorSensitivityLevel::LOW; i < LEVELS_NUM; i = (AiMonitorSensitivityLevel) (i + 1)) {
    ...
    spaghetti_detection_level_list = new ComboBox(ai_refine_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(100), -1), 0,
                                                  NULL, wxCB_READONLY);
    for (auto i = AiMonitorSensitivityLevel::LOW; i < LEVELS_NUM; i = (AiMonitorSensitivityLevel) (i + 1)) {
        wxString level_option = sensitivity_level_to_label_string(i);
    ...
    purgechutepileup_detection_level_list = new ComboBox(ai_refine_panel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                                         wxSize(FromDIP(100), -1), 0, NULL, wxCB_READONLY);
    for (auto i = AiMonitorSensitivityLevel::LOW; i < LEVELS_NUM; i = (AiMonitorSensitivityLevel) (i + 1)) {
        wxString level_option = sensitivity_level_to_label_string(i);
        purgechutepileup_detection_level_list->Append(level_option);
    }
    if (purgechutepileup_detection_level_list->GetCount() > 0) {
        purgechutepileup_detection_level_list->SetSelection(0);
    }
    ...
    line_sizer                             = new wxBoxSizer(wxHORIZONTAL);
    text_nozzleclumping_detection_caption0 = new Label(ai_refine_panel,
                                                       _L("Check if the nozzle is clumping by filaments or other foreign objects."));
    ...
    nozzleclumping_detection_level_list = new ComboBox(ai_refine_panel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                                       wxSize(FromDIP(100), -1), 0, NULL, wxCB_READONLY);
    for (auto i = AiMonitorSensitivityLevel::LOW; i < LEVELS_NUM; i = (AiMonitorSensitivityLevel) (i + 1)) {
        wxString level_option = sensitivity_level_to_label_string(i);
        nozzleclumping_detection_level_list->Append(level_option);
    }
    if (nozzleclumping_detection_level_list->GetCount() > 0) {
        nozzleclumping_detection_level_list->SetSelection(0);
    }
    ...
    line_sizer                          = new wxBoxSizer(wxHORIZONTAL);
    text_airprinting_detection_caption0 = new Label(ai_refine_panel,
                                                    _L("Detects air printing caused by nozzle clogging or filament grinding."));
    ...
    airprinting_detection_level_list = new ComboBox(ai_refine_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(100), -1),
                                                    0, NULL, wxCB_READONLY);
    for (auto i = AiMonitorSensitivityLevel::LOW; i < LEVELS_NUM; i = (AiMonitorSensitivityLevel) (i + 1)) {
        wxString level_option = sensitivity_level_to_label_string(i);
        airprinting_detection_level_list->Append(level_option);
    }
    if (airprinting_detection_level_list->GetCount() > 0) {
        airprinting_detection_level_list->SetSelection(0);
    }
    ...
    text_save_remote_print_file_to_storage_explain =
    ...
    // refine printer function options
    spaghetti_detection_level_list->Connect(wxEVT_COMBOBOX, wxCommandEventHandler(PrintOptionsDialog::set_spaghetti_detection_sensitivity),
                                            NULL, this);
    purgechutepileup_detection_level_list->Connect(wxEVT_COMBOBOX,
                                                   wxCommandEventHandler(PrintOptionsDialog::set_purgechutepileup_detection_sensitivity),
                                                   NULL, this);
    nozzleclumping_detection_level_list->Connect(wxEVT_COMBOBOX,
                                                 wxCommandEventHandler(PrintOptionsDialog::set_nozzleclumping_detection_sensitivity), NULL,
                                                 this);
    airprinting_detection_level_list->Connect(wxEVT_COMBOBOX,
                                              wxCommandEventHandler(PrintOptionsDialog::set_airprinting_detection_sensitivity), NULL, this);
    ...
    : DPIDialog(parent, wxID_ANY, _L("Printer Parts"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    nozzle_type_checkbox = new ComboBox(single_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(180), -1), 0, NULL,
                                        wxCB_READONLY);
    ...
    nozzle_diameter_checkbox = new ComboBox(single_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(180), -1), 0, NULL,
                                            wxCB_READONLY);
    ...
    nozzle_flow_type_checkbox = new ComboBox(single_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(180), -1), 0, NULL,
                                             wxCB_READONLY);
    ...
    multiple_left_nozzle_type_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(180), -1),
                                                      0, NULL, wxCB_READONLY);
    ...
    multiple_left_nozzle_diameter_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                                          wxSize(FromDIP(140), -1), 0, NULL, wxCB_READONLY);
    ...
    multiple_left_nozzle_flow_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(140), -1),
                                                      0, NULL, wxCB_READONLY);
    ...
    multiple_right_nozzle_type_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(180), -1),
                                                       0, NULL, wxCB_READONLY);
    ...
    multiple_right_nozzle_diameter_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                                           wxSize(FromDIP(140), -1), 0, NULL, wxCB_READONLY);
    ...
    multiple_right_nozzle_flow_checkbox = new ComboBox(multiple_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(140), -1),
                                                       0, NULL, wxCB_READONLY);
