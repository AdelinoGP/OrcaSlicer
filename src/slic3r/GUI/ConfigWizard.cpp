...
// [INTENT] Load and parse vendor configuration JSON bundles.
// [UNITY] Use a custom ScriptableObject or JSON parser to load printer profile data.
bool Bundle::load(fs::path source_path, bool ais_in_resources, bool ais_bbl_bundle)
{
    ...
    auto [config_substitutions,
          presets_loaded] = preset_bundle->load_vendor_configs_from_json(parent_path, vendor_name,
...
const Bundle& BundleMap::bbl_bundle() const { return const_cast<BundleMap*>(this)->bbl_bundle(); }
...
PrinterPicker::PrinterPicker(
    wxWindow* parent, const VendorProfile& vendor, wxString title, size_t max_cols, const AppConfig& appconfig, const ModelFilter& filter)
    : wxPanel(parent), vendor_id(vendor.id), width(0)
{
    ...
    for (const auto& model : models) {
        ...
        if (!load_bitmap(GUI::from_u8(Slic3r::data_dir() + "/vendor/" + vendor.id + "/" + model.id + "_thumbnail.png"), bitmap,
                         bitmap_width)) {
            if (!load_bitmap(GUI::from_u8(Slic3r::resources_dir() + "/profiles/" + vendor.id + "/" + model.id + "_thumbnail.png"), bitmap,
                             bitmap_width)) {
                BOOST_LOG_TRIVIAL(warning)
                    << boost::format("Can't find bitmap file `%1%` for vendor `%2%`, printer `%3%`, using placeholder icon instead") %
                           (Slic3r::resources_dir() + "/profiles/" + vendor.id + "/" + model.id + "_thumbnail.png") % vendor.id % model.id;
    ...
    : ConfigWizardPage(parent,
                       from_u8((boost::format(
                                ...
                                SLIC3R_APP_NAME)
                                   .str()),
                       _L("Welcome"))
    , welcome_text(append_text(from_u8((boost::format(_utf8(L("Hello, welcome to %s! This %s helps you with the initial configuration; "
                                                              "just a few settings and you will be ready to print."))) %
                                        SLIC3R_APP_NAME % _utf8(ConfigWizard::name()))
                                           .str())))
    , cbox_reset(append(new wxCheckBox(this, wxID_ANY, _L("Remove user profiles (a snapshot will be taken beforehand)"))))
    , cbox_integrate(
          append(new wxCheckBox(this, wxID_ANY, _L("Perform desktop integration (Sets this binary to be searchable by the system)."))))
{
    welcome_text->Hide();
    cbox_reset->Hide();
    cbox_integrate->Hide();
...
PagePrinters::PagePrinters(
    ConfigWizard* parent, wxString title, wxString shortname, const VendorProfile& vendor, unsigned indent, Technology technology)
    ...
    , install(false) // only used for 3rd party vendors
{
    ...
    for (const auto& family : families) {
        const auto filter = [&](const VendorProfile::PrinterModel& model) {
            return ((model.technology == ptFFF && technology & T_FFF) || (model.technology == ptSLA && technology & T_SLA)) &&
                   model.family == family;
        };
...
{
    // Orca: add custom as default
    if (is_primary_printer_page && (run_reason == ConfigWizard::RR_DATA_EMPTY || run_reason == ConfigWizard::RR_DATA_LEGACY) &&
        printer_pickers.size() > 0 && printer_pickers[0]->vendor_id == PresetBundle::ORCA_DEFAULT_BUNDLE) {
    ...
    , list_profile(new PresetList(this))
{
    ...
    list_printer->Bind(wxEVT_LISTBOX,
                       [this](wxCommandEvent& evt) { update_lists(list_type->GetSelection(), list_vendor->GetSelection(), evt.GetInt()); });
    list_type->Bind(wxEVT_LISTBOX, [this](wxCommandEvent&) { update_lists(list_type->GetSelection(), list_vendor->GetSelection()); });
    list_vendor->Bind(wxEVT_LISTBOX, [this](wxCommandEvent&) { update_lists(list_type->GetSelection(), list_vendor->GetSelection()); });
...
void PageMaterials::clear_compatible_printers_label() { set_compatible_printers_html_window(std::vector<std::string>(), false); }
...
void PageMaterials::on_material_highlighted(int sel_material)
{
    ...
    for (const Preset* printer : materials->printers) {
        for (const Preset* material : matching_materials) {
            if (is_compatible_with_printer(PresetWithVendorProfile(*material, material->vendor),
...
void PageMaterials::sort_list_data(StringList* list, bool add_All_item, bool material_type_ordering)
{
    ...
    if (material_type_ordering) {
        ...
        std::sort(prusa_profiles.begin(), prusa_profiles.end(),
...
void PageMaterials::sort_list_data(PresetList* list, const std::vector<ProfilePrintData>& data)
{
    ...
    for (size_t i = 0; i < prusa_profiles.size(); ++i) {
        list->append(std::string(prusa_profiles[i].name) + (prusa_profiles[i].omnipresent ? "" : " *"),
                     &const_cast<std::string&>(prusa_profiles[i].name.get()));
        list->Check(i, prusa_profiles[i].checked);
    }
    for (size_t i = 0; i < other_profiles.size(); ++i) {
        list->append(std::string(other_profiles[i].name) + (other_profiles[i].omnipresent ? "" : " *"),
                     &const_cast<std::string&>(other_profiles[i].name.get()));
...
PageCustom::PageCustom(ConfigWizard* parent) : ConfigWizardPage(parent, _L("Custom Printer Setup"), _L("Custom Printer"))
{
    ...
    tc_profile_name->Bind(wxEVT_KILL_FOCUS, [this](wxFocusEvent& evt) {
        if (tc_profile_name->GetValue().IsEmpty()) {
            if (profile_name_prev.IsEmpty()) {
                tc_profile_name->SetValue(default_profile_name);
            } else {
                tc_profile_name->SetValue(profile_name_prev);
            }
        } else {
    ...
    , diam_filam(new DiamTextCtrl(this))
{
    ...
    append_text(
        _L("Good precision is required, so use a caliper and do multiple measurements along the filament, then compute the average."));
...
void ConfigWizardIndex::on_paint(wxPaintEvent& evt)
{
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        if (i == item_active || (item_hover >= 0 && i == (size_t) item_hover)) {
            ...
            dc.DrawBitmap(bullet_black.bmp(), x, y + yoff_icon, false);
        } else if (i > item_active) {
            dc.DrawBitmap(bullet_white.bmp(), x, y + yoff_icon, false);
        }
...
void Materials::add_printer(const Preset* preset) { printers.insert(preset); }
...
void ConfigWizard::priv::init_dialog_size()
{
    ...
    wxRect     window_rect(disp_rect.x + disp_rect.width / 20, disp_rect.y + disp_rect.height / 20, 9 * disp_rect.width / 10,
                           9 * disp_rect.height / 10);

    const int width_hint = index->GetSize().GetWidth() +
                           90 * em() /*std::max(90 * em(), (only_sla_mode ? page_msla->get_width() : page_fff->get_width()) + 30 * em())*/
        ;                            // XXX: magic constant, I found no better solution
    ...
    if (!app_config->legacy_datadir()) {
        ...
        for (auto& dir_entry : boost::filesystem::directory_iterator(printer_dir))
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                if (needle == legacy_preset_map.end()) {
                    continue;
                }
                ...
                if (material_name_and_installed.second == "true") {
                    ...
                    if (num_found == 0)
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("Profile %1% was not found in installed vendor Preset Bundles.") % material_name;
                    else if (num_found > 1)
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("Profile %1% was found in %2% vendor Preset Bundles.") % material_name % num_found;
                }
        }
...
void ConfigWizard::priv::create_3rdparty_pages()
{
    for (const auto& pair : bundles) {
        ...
        // Orca: add custom as default
        if (vendor->id == PresetBundle::ORCA_DEFAULT_BUNDLE) {
            continue;
        }
...
void ConfigWizard::priv::update_materials(Technology technology)
{
    if (any_fff_selected && (technology & T_FFF)) {
        ...
        for (const auto& preset : filaments.presets) {
            const auto filter = [preset](const std::pair<std::string, size_t> element) { return preset->alias == element.first; };
            if (std::find_if(filaments.compatibility_counter.begin(), filaments.compatibility_counter.end(), filter) !=
                filaments.compatibility_counter.end()) {
                continue;
            }
            ...
            for (const auto& printer : filaments.printers) {
                ...
                for (size_t i = 0; i < idx_with_same_alias.size() && !compatible; ++i) {
                    ...
                    if (is_compatible_with_printer(PresetWithVendorProfile(prst, prst.vendor),
    ...
    if (any_sla_selected && (technology & T_SLA)) {
        ...
        for (const auto& preset : sla_materials.presets) {
            const auto filter = [preset](const std::pair<std::string, size_t> element) { return preset->alias == element.first; };
            if (std::find_if(sla_materials.compatibility_counter.begin(), sla_materials.compatibility_counter.end(), filter) !=
                sla_materials.compatibility_counter.end()) {
                continue;
            }
            std::vector<size_t> idx_with_same_alias;
                                                                     ...
                                                                     const std::set<const VendorProfile::PrinterModel*>& printer_models)
{
    ...
    if (!models_without_default.empty()) {
        ...
        std::string   message = (technology & T_FFF ?
                                     GUI::format(_L("Following printer profiles has no default filament: %1%Please select one manually."),
                                                 printer_names) :
                                     GUI::format(_L("Following printer profiles has no default material: %1%Please select one manually."),
                                                 printer_names));
...
//
// In case only_for_model_id is set, then the test is done for that particular printer model only, and the default materials are installed
// silently. Otherwise the user is quieried whether to install the missing default materials or not.
//
...
bool ConfigWizard::priv::check_and_install_missing_materials(Technology technology, const std::string& only_for_model_id)
{
    ...
    const auto printer_models_missing_materials = [this, only_for_model_id](PrinterTechnology technology, const std::string& section) {
        const std::map<std::string, std::string>&    appconfig_presets = appconfig_new.has_section(section) ?
                                                                             appconfig_new.get_section(section) :
                    ...
                    if ((only_for_model_id.empty() || only_for_model_id == printer_model->id) &&
                        ...
                        for (const auto& preset : appconfig_presets) {
                            if (preset.second == "1" || preset.second == "true") {
                                ...
                                if (material != nullptr && is_compatible_with_printer(PresetWithVendorProfile(*material, nullptr),
        ...
        assert(printer_models_without_material.empty() || only_for_model_id.empty() ||
               only_for_model_id == (*printer_models_without_material.begin())->id);
        return printer_models_without_material;
    };

    const auto ask_and_select_default_materials =
        [this](const wxString& message, const std::set<const VendorProfile::PrinterModel*>& printer_models, Technology technology) {
    ...
    if (any_fff_selected && (technology & T_FFF)) {
        std::set<const VendorProfile::PrinterModel*> printer_models_without_material =
            printer_models_missing_materials(ptFFF, AppConfig::SECTION_FILAMENTS);
        if (!printer_models_without_material.empty()) {
            if (only_for_model_id.empty())
                ask_and_select_default_materials(_L("The following FFF printer models have no filament selected:") + "\n\n\t" +
                                                 ...
                                                 printer_models_without_material, T_FFF);
            else
    ...
    /* BBS skip SLA
    if (any_sla_selected && (technology & T_SLA)) {
        std::set<const VendorProfile::PrinterModel*> printer_models_without_material = printer_models_missing_materials(ptSLA,
    AppConfig::SECTION_MATERIALS); if (! printer_models_without_material.empty()) { if (only_for_model_id.empty())
                                      ...
                                      bool&                apply_keeped_changes)
{
    ...
    auto get_preferred_printer_technology  = [enabled_vendors, suppress_sla_printer](const std::string& bundle_name, const Bundle& bundle) {
        ...
        if (config != enabled_vendors.end()) {
            for (const auto& model : bundle.vendor_profile->models) {
                if (const auto model_it = config->second.find(model.id); model_it != config->second.end() && model_it->second.size() > 0) {
                    if (pt == ptAny)
                        pt = model.technology;
    ...
    if (preferred_pt = get_preferred_printer_technology("BBL", bundles.bbl_bundle());
        ...
        for (const auto& bundle : bundles) {
            // BBS: set BBL as default
            if (bundle.second.is_bbl_bundle) {
                continue;
            }
    ...
    for (const auto& pair : bundles) {
        if (!pair.second.is_in_resources) {
            continue;
        }
    ...
    if (!check_unsaved_preset_changes)
        if ((check_unsaved_preset_changes = install_bundles.size() > 0))
            header = _L_PLURAL("A new vendor was installed and one of its printers will be activated",
                               "New vendors were installed and one of theirs printers will be activated", install_bundles.size());
    ...
    /*if (snapshot && ! take_config_snapshot_cancel_on_error(*app_config, snapshot_reason, "", _u8L("Do you want to continue changing the
    ...
    // Prusa printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model("BBL", bundles.bbl_bundle(), preferred_variant); preferred_model.empty()) {
        for (const auto& bundle : bundles) {
            if (bundle.second.is_bbl_bundle) {
                continue;
            }
            if (preferred_model = get_preferred_printer_model(bundle.first, bundle.second, preferred_variant); !preferred_model.empty())
                break;
        }
    }
    ...
    if (!check_unsaved_preset_changes) {
        if ((check_unsaved_preset_changes = !first_added_filament.empty() || !first_added_sla_material.empty())) {
            ...
            // BBS
            bool is_filaments_changed = app_config->get_section(AppConfig::SECTION_FILAMENTS) !=
                                        appconfig_new.get_section(AppConfig::SECTION_FILAMENTS);
            ...
            /*bool is_filaments_changed = app_config->get_section(AppConfig::SECTION_FILAMENTS) !=
            appconfig_new.get_section(AppConfig::SECTION_FILAMENTS); bool is_sla_materials_changed =
            app_config->get_section(AppConfig::SECTION_MATERIALS) != appconfig_new.get_section(AppConfig::SECTION_MATERIALS); if
            ((check_unsaved_preset_changes = is_filaments_changed || is_sla_materials_changed)) { header = is_filaments_changed ? _L("Some
            filaments were uninstalled.") : _L("Some SLA materials were uninstalled."); if
            (!wxGetApp().check_and_keep_current_preset_changes(caption, header, act_btns, &apply_keeped_changes)) return false;
            }*/
        }
    }
    ...
    if (!only_sla_mode && page_custom->custom_wanted()) {
        // if unsaved changes was not cheched till this moment
        if (!check_unsaved_preset_changes &&
            !wxGetApp().check_and_keep_current_preset_changes(caption, _L("Custom printer was installed and it will be activated."),
...
bool ConfigWizard::priv::check_fff_selected() { return false; }

bool ConfigWizard::priv::check_sla_selected() { return false; }
...
ConfigWizard::ConfigWizard(wxWindow* parent)
    : DPIDialog(parent,
                wxID_ANY,
                wxString(SLIC3R_APP_NAME) + " - " + _(name()),
                wxDefaultPosition,
                wxDefaultSize,
    ...
    , p(new priv(this))
{
    ...
    p->custom_config.reset(DynamicPrintConfig::new_from_defaults_keys({
        "gcode_flavor", "printable_area", "bed_exclude_area", "bed_custom_texture", "bed_custom_model", "nozzle_diameter",
        "filament_diameter", "thumbnails"
    ...
    p->add_page(p->page_filaments = new PageMaterials(this, &p->filaments, _L("Filament Profiles Selection"), _L("Filaments"), _L("Type:")));
