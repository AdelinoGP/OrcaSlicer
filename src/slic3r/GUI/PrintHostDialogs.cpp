// [ANNOTATED]
// [INTENT] Dialogs for naming uploads sent to printer hosts and for monitoring/canceling queued print-host transfer jobs.
// [STATE] Persists recent upload destinations, optional storage/group selections, and queue dialog column/size preferences.
// [EVENT] Uses wx button, show, and custom queue events to synchronize upload progress with notifications and dialog state.
// [UNITY] Split into upload-form view models plus a queue monitor service that pushes retained UI updates.
// [PORTING_HAZARD:P1] Queue progress/error/cancel handling relies on custom wx events and dialog lifetime guards across asynchronous uploads.
...
fs::path PrintHostSendDialog::filename() const { return into_path(txt_filename->GetValue()); }

PrintHostPostUploadAction PrintHostSendDialog::post_action() const { return post_upload_action; }
...
PrintHostQueueDialog::Event::Event(wxEventType eventType, int winid, size_t job_id) : wxEvent(winid, eventType), job_id(job_id) {}

PrintHostQueueDialog::Event::Event(wxEventType eventType, int winid, size_t job_id, int progress)
    : wxEvent(winid, eventType), job_id(job_id), progress(progress)
{}

PrintHostQueueDialog::Event::Event(wxEventType eventType, int winid, size_t job_id, wxString error)
    : wxEvent(winid, eventType), job_id(job_id), status(std::move(error))
{}

PrintHostQueueDialog::Event::Event(wxEventType eventType, int winid, size_t job_id, wxString tag, wxString status)
    : wxEvent(winid, eventType), job_id(job_id), tag(std::move(tag)), status(std::move(status))
{}

wxEvent* PrintHostQueueDialog::Event::Clone() const { return new Event(*this); }
    ...
    , on_info_evt(this, EVT_PRINTHOST_INFO, &PrintHostQueueDialog::on_info, this)
{
    ...
    job_list->AppendProgressColumn(_L("Progress"), wxDATAVIEW_CELL_INERT, widths[1], wxALIGN_LEFT,
    ...
    Bind(wxEVT_MOVE, [this](wxMoveEvent& evt) { save_user_data(UDT_SIZE | UDT_POSITION | UDT_COLS); });
...
void PrintHostQueueDialog::append_job(const PrintHostJob& job)
{
    ...
    wxGetApp().notification_manager()->push_upload_job_notification(job_list->GetItemCount(), (float) size_i / 1024 / 1024,
                                                                    job.upload_data.upload_path.string(), job.printhost->get_host());
}
...
void PrintHostQueueDialog::on_progress(Event& evt)
{
    ...
    // [PORTING_HAZARD:P2] Queue rows are addressed by mutable positional indices, so any future reordering/filtering must preserve stable job IDs.
    if (evt.progress < 100) {
    ...
    if (evt.progress > 0) {
        ...
        wxGetApp().notification_manager()->set_upload_job_notification_percentage(evt.job_id + 1, into_u8(nm.GetString()),
                                                                                  into_u8(hst.GetString()), evt.progress / 100.f);
    }
}
...
void PrintHostQueueDialog::on_cancel(Event& evt)
{
    ...
    job_list->GetValue(nm, evt.job_id, COL_FILENAME);
    job_list->GetValue(hst, evt.job_id, COL_HOST);
    wxGetApp().notification_manager()->upload_job_notification_show_canceled(evt.job_id + 1, into_u8(nm.GetString()),
