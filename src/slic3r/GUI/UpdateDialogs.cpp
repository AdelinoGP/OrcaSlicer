...
void MsgUpdateSlic3r::on_hyperlink(wxHyperlinkEvent& evt) { wxGetApp().open_browser_with_warning_dialog(evt.GetURL()); }
    ...
    : DPIDialog(wxGetApp().mainframe, wxID_ANY, _L("Configuration update"), wxDefaultPosition, wxDefaultSize, wxCAPTION)
{
    ...
    auto m_text_up_info = new wxStaticText(this, wxID_ANY, _L("A new configuration package is available. Do you want to install it?"),
...
MsgUpdateForced::MsgUpdateForced(const std::vector<Update>& updates)
    : MsgDialog(nullptr,
                _(L("Configuration incompatible")),
                _(L("the configuration package is incompatible with the current application.")) + " ",
                wxOK | wxICON_ERROR)
{
    auto* text = new wxStaticText(this, wxID_ANY,
                                  wxString::Format(_(L("The configuration package is incompatible with the current application.\n"
    ...
    for (const auto& update : updates) {
        ...
        if (!update.comment.empty()) {
            versions->Add(new wxStaticText(this, wxID_ANY,
                                           _(L("Description:"))) /*, 0, wxALIGN_RIGHT*/); // uncoment if align to right (might not look good
                                                                                          // if 1  vedor name is longer than other names)
        ...
        if (!update.change_log.empty()) {
            if (!changelog_textctrl)
                changelog_textctrl = new wxTextCtrl(this, wxID_ANY, from_u8(update.change_log), wxDefaultPosition, wxDefaultSize,
...
//	auto *link = new wxHyperlinkCtrl(this, wxID_ANY, wxString::Format("%s 1.40 configuration update", SLIC3R_APP_NAME),
// CONFIG_UPDATE_WIKI_URL); 	content_sizer->Add(text2); 	content_sizer->Add(link); 	content_sizer->AddSpacer(VERT_SPACING);
//
//	finalize();
// }
//
