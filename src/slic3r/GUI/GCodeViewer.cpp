// [ANNOTATED]
// [INTENT] Primary controller for the 3D G-code preview, integrating libvgcode rendering with the slicer UI.
// [STATE] Manages complex preview modes (Speed, Temperature, Flow, etc.), hierarchical G-code text models, and toolhead markers.
// [EVENT] Orchestrates real-time scrubbing, layer-selection synchronization, and interactive G-code inspection.
// [OPENGL] Bridges slicer data to GPU-based toolpath rendering via libvgcode; manages GL state and shader constants.
// [UNITY] Map to a C# GCodeVisualizationService managing a custom rendering pipeline (Graphics.DrawMeshInstanced).
// [UNITY] All ImGui overlays (Legend, Stats, G-code Window) map to retained Unity UI Toolkit VisualElements.
// [PORTING_HAZARD:P1] High-performance toolpath rendering and large G-code file virtualization (millions of lines) are critical Unity
// porting challenges. [PORTING_HAZARD:P2] Intricate logic for generating 3D toolpath geometry (tubes, caps, corners) from slicing results.
...
int GCodeViewer::SequentialView::ActualSpeedImguiWidget::plot(const char* label, const std::array<float, 2>& frame_size)
{
    ...
    if (size_x > 0.0f && values_count >= values_count_min) {
        ...
        const ImU32 grid_secondary_color = ImGui::GetColorU32(
            ImVec4(0.0f, 150.f / 255.0f, 136.0f / 255.f, 0.5f)); // ORCA color with opacity
        ...
        for (const auto& [level, color] : levels) {
            ...
            window->DrawList->AddLine(ImLerp(inner_bb.Min, ImVec2(inner_bb.Min.x + offset.x, inner_bb.Max.y), ImVec2(0.1f, y)),
                                      ImLerp(inner_bb.Min, ImVec2(inner_bb.Min.x + offset.x, inner_bb.Max.y), ImVec2(0.9f, y)),
                                      ImGuiWrapper::to_ImU32(color), 3.0f);
        }
        ...
        for (int n = 0; n < values_count - 1; ++n) {
            ...
            window->DrawList->AddLine(ImLerp(inner_bb.Min + offset, inner_bb.Max, ImVec2(x, 0.0f)),
                                      ImLerp(inner_bb.Min + offset, inner_bb.Max, ImVec2(x, 1.0f)),
                                      data[n].internal ? grid_secondary_color : grid_main_color);
        }
        ...
        for (int n = 0; n < values_count - 1; ++n) {
            ...
            if (hovered && inner_bb.Contains(io.MousePos)) {
                const float t = ImClamp((io.MousePos.x - inner_bb.Min.x - offset.x) / (inner_bb.Max.x - inner_bb.Min.x - offset.x), 0.0f,
                                        0.9999f);
                if (tp1.x < t && t < tp2.x)
                    idx_hovered = n;
            }
...
void GCodeViewer::SequentialView::Marker::render(int canvas_width, int canvas_height, const libvgcode::EViewType& view_type)
{
    ...
    const Transform3d  model_matrix = (Geometry::translation_transform(
                                           (m_world_position + m_model_z_offset * Vec3f::UnitZ()).cast<double>()) *
                                       Geometry::translation_transform(scale_factor * m_model.get_bounding_box().size().z() *
                                                                       Vec3d::UnitZ()) *
...
{
    switch (type) {
    case libvgcode::EMoveType::Noop: {
        return _u8L("Noop");
    }
    case libvgcode::EMoveType::Retract: {
        return _u8L("Retract");
    }
    case libvgcode::EMoveType::Unretract: {
        return _u8L("Unretract");
    }
    case libvgcode::EMoveType::Seam: {
        return _u8L("Seam");
    }
    case libvgcode::EMoveType::ToolChange: {
        return _u8L("Tool Change");
    }
    case libvgcode::EMoveType::ColorChange: {
        return _u8L("Color Change");
    }
    case libvgcode::EMoveType::PausePrint: {
        return _u8L("Pause Print");
    }
    case libvgcode::EMoveType::CustomGCode: {
        return _u8L("Custom G-code");
    }
    case libvgcode::EMoveType::Travel: {
        return _u8L("Travel");
    }
    case libvgcode::EMoveType::Wipe: {
        return _u8L("Wipe");
    }
    case libvgcode::EMoveType::Extrude: {
        return _u8L("Extrude");
    }
    default: {
        return _u8L("Unknown");
...
static std::string to_string(libvgcode::EGCodeExtrusionRole role)
{
    ...
    case libvgcode::EGCodeExtrusionRole::SupportMaterialInterface: {
        return _u8L("Support interface");
    }
    case libvgcode::EGCodeExtrusionRole::WipeTower: {
        return _u8L("Prime tower");
    }
...
void GCodeViewer::SequentialView::Marker::render_position_window(const libvgcode::Viewer*    viewer,
                                                                 int                         canvas_width,
                                                                 int                         canvas_height,
                                                                 const libvgcode::EViewType& view_type)
{
    ...
    if (viewer != nullptr) {
        ...
        imgui.begin(std::string("ToolPosition"),
        ...
        if (properties_shown) {
            ...
            if (ImGui::BeginTable("Properties", 2)) {
                ...
                append_table_row(_u8L("Type"), [&vertex]() { ImGuiWrapper::text(_u8L(to_string(vertex.type))); });
                ...
                append_table_row(_u8L("Flow rate"),
                                 [&vertex, &buff,
                                  NA_TXT]() { // ORCA use "Flow rate" instead "Volumetric flow Rate" to make window more compact
                                     std::string text;
                                     if (vertex.is_extrusion()) {
                ...
                append_table_row(_u8L("Time"), [viewer, &vertex, &buff, vertex_id]() {
                    ...
                    sprintf(buff, "%s (%.3fs)", get_time_dhms(estimated_time).c_str(),
                            vertex.times[static_cast<size_t>(viewer->get_time_mode())]);
                    const std::string text = std::string(buff);
                    ImGuiWrapper::text(text);
                });
            ...
            if (imgui.button((table_shown ? _L("Hide") : _L("Show")).c_str(), ImVec2(table_btn_width + btn_padding_x * 2.f, 0.f),
                             actual_speed_exist))
            ...
            ImGui::SameLine();
            ImGuiWrapper::text(
                _u8L("Actual speed profile").c_str()); // ORCA show label and plot on external window to make main window more compact
            ...
            if (actual_speed_exist && table_shown) {
                ...
                imgui.set_next_window_pos(ImGui::GetWindowPos().x - 5.f * m_scale /*+ wnd_size.x*/, static_cast<float>(canvas_height),
                ...
                if (ImGui::BeginTable(
                        "ToolPositionTable", 2,
                        ImGuiTableFlags_Borders /*| ImGuiTableFlags_ScrollY*/)) { // ORCA showing scrollbar causes expanding window
                    char buff[1024];
                    ...
                    for (const ActualSpeedImguiWidget::Item& item : m_actual_speed_imgui_widget.data) {
                        ...
                        ImGui::TableNextRow();
                        const ImU32 row_bg_color = ImGui::GetColorU32(item.internal ? ImVec4(0.0f, 150.f / 255.0f, 136.0f / 255.f, 0.15f) :
                                                                                      ImVec4(0.2f, 0.2f, 0.2f, 0.25f)); // ORCA
        ...
        if (ImGui::Button(into_u8(properties_shown ? ImGui::UnfoldButtonIcon : ImGui::FoldButtonIcon).c_str(),
                          ImVec2(24.f, 24.f) * m_scale)) {
        ...
        ImGuiWrapper& imgui = *wxGetApp().imgui();
        // const Size cnv_size = wxGetApp().plater()->get_current_canvas3D()->get_canvas_size();
        // imgui.set_next_window_pos(0.5f * static_cast<float>(cnv_size.get_width()), static_cast<float>(cnv_size.get_height()),
...
void GCodeViewer::SequentialView::GCodeWindow::render(float top, float bottom, float right, uint64_t curr_line_id) const
{
    ...
    imgui.set_next_window_pos(right, top + 6 * m_scale, ImGuiCond_Always, 1.0f,
                              0.0f); // ORCA add a small gap between legend and code viewer
...
void GCodeViewer::SequentialView::render(const bool                  has_render_path,
                                         float                       legend_height,
...
void GCodeViewer::load_as_gcode(const GCodeProcessorResult&       gcode_result,
                                const Print&                      print,
                                ...
                                bool                              only_gcode)
{
    ...
    //             m_cog.add_segment(curr_pos, prev_pos, gcode_result.filament_densities[curr.extruder_id] * curr.mm3_per_mm * (curr_pos -
    //             prev_pos).norm());
    //         }
    //     }
    // #endif // !VGCODE_ENABLE_COG_AND_TOOL_MARKERS
    ...
    const libvgcode::AABox bbox = wxGetApp().is_gcode_viewer() ?
                                      ...
                                      m_viewer.get_extrusion_bounding_box(
                                          {libvgcode::EGCodeExtrusionRole::Perimeter, libvgcode::EGCodeExtrusionRole::ExternalPerimeter,
                                           ...
                                           libvgcode::EGCodeExtrusionRole::InternalBridgeInfill, libvgcode::EGCodeExtrusionRole::Brim,
    ...
    if (m_only_gcode_in_preview) {
        ...
        if (!gcode_result.printable_area.empty()) {
            ...
            wxGetApp().plater()->set_bed_shape(printable_area, bed_exclude_area, wrapping_exclude_area, gcode_result.printable_height,
                                               extruder_areas, extruder_heights, texture, model, gcode_result.printable_area.empty());
        }
        /*else {
    ...
    if (m_conflict_result) {
        m_conflict_result.value().layer = m_viewer.get_layer_id_at(static_cast<float>(m_conflict_result.value()._height));
    }
...
void GCodeViewer::render(int canvas_width, int canvas_height, int right_margin)
{
    ...
    if (is_legend_shown()) {
        ...
        const Size    cnv_size = wxGetApp().plater()->get_current_canvas3D()->get_canvas_size();
        ImGuiPureWrap::set_next_window_pos(static_cast<float>(cnv_size.get_width()), static_cast<float>(cnv_size.get_height()),
                                           ImGuiCond_Always, 1.0f, 1.0f);
        ...
        if (curr_vertex.is_extrusion() || curr_vertex.is_travel() || curr_vertex.is_wipe() ||
            ...
            for (size_t i = start_id; i < end_id; ++i) {
                ...
                const float len = (i > start_id) ?
                                      (libvgcode::convert(v.position) - libvgcode::convert(m_viewer.get_vertex_at(i - 1).position)).norm() :
                                      0.0f;
                total_len += len;
...
{
public:
    explicit ToolpathsObjExporter(const libvgcode::Viewer& viewer) : m_viewer(viewer) {}
...
private:
    ...
    enum CornerType : unsigned char { RightTurn, LeftTurn, Straight };

    CrossSection corner_cross_section(
        const Vec3f& v, const SegmentLocalAxes& axes1, const SegmentLocalAxes& axes2, float width, float height, CornerType& corner_type)
    {
    ...
    void export_segment(FILE&                        f,
                        unsigned char                flags,
                        size_t                       v1_id,
                        const libvgcode::PathVertex& v1,
                        const libvgcode::PathVertex& v2,
                        const libvgcode::PathVertex& v3)
    {
    ...
    void export_material(FILE& f, size_t material_id) { fprintf(&f, "\nusemtl material_%zu\n", material_id + 1); }
    ...
    void export_materials(FILE& f)
    {
        ...
        for (const auto& color : m_colors) {
            fprintf(&f, "\nnewmtl material_%zu\n", ++materials_counter);
            fprintf(&f, "Ka 1 1 1\n");
            fprintf(&f, "Kd %g %g %g\n", static_cast<float>(color[0]) * inv_255, static_cast<float>(color[1]) * inv_255,
    ...
    size_t color_id(size_t vertex_id)
    {
        ...
        const libvgcode::Color       color = (color_top_layer_only && v.layer_id < top_layer_id &&
                                              (!m_viewer.is_spiral_vase_mode() || vertex_id != m_viewer.get_view_enabled_range()[0])) ?
                                                 libvgcode::DUMMY_COLOR :
                                                 m_viewer.get_vertex_color(v);
...
{
    const Camera&           camera                = wxGetApp().plater()->get_camera();
    const libvgcode::Mat4x4 converted_view_matrix = libvgcode::convert(
        static_cast<Matrix4f>(camera.get_view_matrix().matrix().cast<float>()));
    const libvgcode::Mat4x4 converted_projetion_matrix = libvgcode::convert(
        static_cast<Matrix4f>(camera.get_projection_matrix().matrix().cast<float>()));
#if VGCODE_ENABLE_COG_AND_TOOL_MARKERS
    m_viewer.set_cog_marker_scale_factor(m_cog_marker_fixed_screen_size ? 10.0f * m_cog_marker_size * camera.get_inv_zoom() :
                                                                          m_cog_marker_size);
    m_viewer.set_tool_marker_scale_factor(m_tool_marker_fixed_screen_size ? 10.0f * m_tool_marker_size * camera.get_inv_zoom() :
                                                                            m_tool_marker_size);
#endif // VGCODE_ENABLE_COG_AND_TOOL_MARKERS
    ...
    if (is_legend_shown()) {
        ...
        if (ImGui::BeginTable("Data", 2)) {
            ...
            add_range_property_row("layer time logarithmic range",
                                   m_viewer.get_color_range(libvgcode::EViewType::LayerTimeLogarithmic).get_range());
        ...
        if (ImGui::BeginTable("Tool", 2)) {
            ...
            std::array<float, 3>    c     = {static_cast<float>(color[0]) / 255.0f, static_cast<float>(color[1]) / 255.0f,
                                             static_cast<float>(color[2]) / 255.0f};
            if (ImGui::ColorPicker3("##ToolColor", c.data())) {
                m_viewer.set_tool_marker_color(
                    {static_cast<uint8_t>(c[0] * 255.0f), static_cast<uint8_t>(c[1] * 255.0f), static_cast<uint8_t>(c[2] * 255.0f)});
            }
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
    ...
    // title and item data
    {
        ...
        for (auto plate : plate_list.get_nonempty_plate_list()) {
            ...
            const PrintEstimatedStatistics::Mode& plate_time_mode = plate_print_statistics
                                                                        .modes[static_cast<size_t>(m_viewer.get_time_mode())];
...
void GCodeViewer::render_legend_color_arr_recommen(float window_padding)
{
        ...
        if (ImGui::IsMouseHoveringRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), true)) {
            if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
                MessageDialog msg_dlg(nullptr,
                                      _L("Automatically re-slice according to the optimal filament grouping, and the grouping results will "
                                         "be displayed after slicing."),
    ...
    auto draw_dash_line = [&](ImDrawList* draw_list, int dash_length = 5, int gap_length = 3) {
        ...
        for (float i = p1.x; i < p2.x; i += (dash_length + gap_length)) {
            draw_list->AddLine(ImVec2(i, p1.y), ImVec2(i + dash_length, p1.y),
                               ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 0.6f))); // ORCA match color
        }
    };
    ...
    float delta_weight_to_single_ext = stats_by_extruder.stats_by_single_extruder.filament_flush_weight -
                                       stats_by_extruder.stats_by_multi_extruder_curr.filament_flush_weight;
    float delta_weight_to_best       = stats_by_extruder.stats_by_multi_extruder_curr.filament_flush_weight -
                                       stats_by_extruder.stats_by_multi_extruder_best.filament_flush_weight;
    int   delta_change_to_single_ext = stats_by_extruder.stats_by_single_extruder.filament_change_count -
                                       stats_by_extruder.stats_by_multi_extruder_curr.filament_change_count;
    int   delta_change_to_best       = stats_by_extruder.stats_by_multi_extruder_curr.filament_change_count -
    ...
    ImGui::BeginChild("#AMS", ImVec2(0, AMS_container_height), true, ImGuiWindowFlags_AlwaysUseWindowPadding);
    {
        ...
        child_begin_draw_list->AddRectFilled(cursor_pos, ImVec2(cursor_pos.x + half_width, cursor_pos.y + line_height),
        ...
        ImGui::Dummy({window_padding, window_padding});
        imgui.text_wrapped(from_u8(_u8L("Please place filaments on the printer based on grouping result.")),
        ...
        if (any_more_to_best) {
            ...
            if (delta_weight_to_best >= 0 && delta_change_to_best >= 0)
                tip = from_u8((boost::format(_u8L("Increase %1%g filament and %2% changes compared to optimal grouping.")) %
                               number_format(delta_weight_to_best) % delta_change_to_best)
                                  .str());
                ...
                tip = from_u8((boost::format(_u8L("Increase %1%g filament and save %2% changes compared to optimal grouping.")) %
                               number_format(delta_weight_to_best) % std::abs(delta_change_to_best))
                                  .str());
            else if (delta_weight_to_best < 0 && delta_change_to_best >= 0)
                tip = from_u8((boost::format(_u8L("Save %1%g filament and increase %2% changes compared to optimal grouping.")) %
                               number_format(std::abs(delta_weight_to_best)) % delta_change_to_best)
                                  .str());
            ...
            if (delta_weight_to_single_ext >= 0 && delta_change_to_single_ext >= 0)
                tip = from_u8((boost::format(_u8L("Save %1%g filament and %2% changes compared to a printer with one nozzle.")) %
                               number_format(delta_weight_to_single_ext) % delta_change_to_single_ext)
                                  .str());
                ...
                tip = from_u8((boost::format(_u8L("Save %1%g filament and increase %2% changes compared to a printer with one nozzle.")) %
                               number_format(delta_weight_to_single_ext) % std::abs(delta_change_to_single_ext))
                                  .str());
            else if (delta_weight_to_single_ext < 0 && delta_change_to_single_ext >= 0)
                tip = from_u8((boost::format(_u8L("Increase %1%g filament and save %2% changes compared to a printer with one nozzle.")) %
                               number_format(std::abs(delta_weight_to_single_ext)) % delta_change_to_single_ext)
                                  .str());
...
void GCodeViewer::render_legend(float& legend_height, int canvas_width, int canvas_height, int right_margin)
{
    ...
    imgui.set_next_window_pos(float(canvas_width - right_margin * m_scale), 4.0f * m_scale, ImGuiCond_Always, 1.0f,
                              0.0f);                                   // ORCA add a small gap to top to create seperation with main toolbar
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 8.0f * m_scale); // ORCA add window rounding to modernize / match style
    ...
    bool show_estimated_time = time_mode.time > 0.0f &&
                               (curr_view_type == libvgcode::EViewType::FeatureType ||
                                curr_view_type == libvgcode::EViewType::LayerTimeLinear ||
                                curr_view_type == libvgcode::EViewType::LayerTimeLogarithmic ||
    ...
    auto append_item =
        [icon_size, &imgui, imperial_units, &window_padding, &draw_list,
         this](EItemType type, const ColorRGBA& color, const std::vector<std::pair<std::string, float>>& columns_offsets,
            ...
            case EItemType::Line: {
                draw_list->AddLine({pos.x + 1, pos.y + icon_size + 2}, {pos.x + icon_size - 1, pos.y + 4}, ImGuiWrapper::to_ImU32(color),
                                   3.0f);
                break;
            ...
            if (callback) {
                ...
                if (checkbox) {
                    ...
                    // Use calculated position from argument. this method has predictable result compared to alingning button using window
                    // width fixes slowly resizing window and endlessly expanding window when there is a miscalculation on position
    ...
    auto calculate_offsets = [&imgui, max_width, window_padding,
        ...
        const ImGuiStyle&  style = ImGui::GetStyle();
        std::vector<float> offsets;
        // ORCA increase spacing for more readable format. Using direct number requires much less code change in here. GetTextLineHeight for
        // additional spacing for icon_size
    ...
    auto color_print_ranges = [this](unsigned char extruder_id, const std::vector<CustomGCode::Item>& custom_gcode_per_print_z) {
        ...
        for (const auto& item : custom_gcode_per_print_z) {
            ...
            auto                     lower_b = std::lower_bound(zs.begin(), zs.end(), static_cast<float>(item.print_z - epsilon()));
            if (lower_b == zs.end())
                continue;
    ...
    ImGui::SameLine(window_padding *
                    2); // ORCA Ignores item spacing to get perfect window margins since since this part uses dummies for window padding
        ...
        for (libvgcode::EGCodeExtrusionRole role : roles) {
            ...
            if (static_cast<size_t>(role) < libvgcode::GCODE_EXTRUSION_ROLES_COUNT) {
                ...
                ::sprintf(buffer, imperial_units ? "%.2fin" : "%.2fm",
                          model_used_filament_m); // ORCA dont use spacing between value and unit
                used_filaments_length.push_back(buffer);
                ::sprintf(buffer, imperial_units ? "%.2foz" : "%.2fg",
                          model_used_filament_g); // ORCA dont use spacing between value and unit
    ...
    case libvgcode::EViewType::Temperature: {
        ...
        // ORCA: Add Pressure Advance visualization support
    case libvgcode::EViewType::PressureAdvance: {
        imgui.title(_u8L("Pressure Advance"));
        break;
    }
    case libvgcode::EViewType::VolumetricFlowRate: {
    ...
    case libvgcode::EViewType::ActualVolumetricFlowRate: {
        imgui.title(_u8L("Actual volumetric flow rate (mm³/s)"));
        break;
    }
    case libvgcode::EViewType::LayerTimeLinear: {
    ...
    auto append_option_item = [this, append_item](libvgcode::EOptionType type, std::vector<float> offsets) {
        auto append_option_item_with_type = [this, offsets, append_item](libvgcode::EOptionType type, const ColorRGBA& color,
                                                                         const std::string& label, bool visible) {
            append_item(EItemType::Rect, color, {{label, offsets[0]}}, true, offsets.back() /*ORCA checkbox_pos*/, visible,
                        [this, type, visible]() {
        ...
        if (type == libvgcode::EOptionType::Travels) {
            ...
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Travels)),
                                         _u8L("Travel"), visible);
        } else if (type == libvgcode::EOptionType::Seams)
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Seams)), _u8L("Seams"),
                                         visible);
        else if (type == libvgcode::EOptionType::Retractions)
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Retractions)),
                                         _u8L("Retract"), visible);
        else if (type == libvgcode::EOptionType::Unretractions)
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Unretractions)),
                                         _u8L("Unretract"), visible);
        else if (type == libvgcode::EOptionType::ToolChanges)
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::ToolChanges)),
                                         _u8L("Filament Changes"), visible);
        else if (type == libvgcode::EOptionType::Wipes)
            append_option_item_with_type(type, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Wipes)), _u8L("Wipe"),
                                         visible);
    };
    ...
    case libvgcode::EViewType::FeatureType: {
        ...
        for (size_t i = 0; i < roles.size(); ++i) {
            ...
            append_item(EItemType::Rect, libvgcode::convert(m_viewer.get_extrusion_role_color(role)), columns_offsets, true, offsets.back(),
        ...
        for (auto item : options) {
            if (item != libvgcode::EOptionType::Travels) {
                ...
                append_item(EItemType::Rect, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Travels)),
                            columns_offsets, true, offsets.back() /*ORCA checkbox_pos*/, visible, [this, item, visible]() {
    ...
    case libvgcode::EViewType::Height: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::Height), 2);
        break;
    }
    case libvgcode::EViewType::Width: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::Width), 2);
        break;
    }
    case libvgcode::EViewType::Speed: {
        ...
        append_item(EItemType::None, libvgcode::convert(m_viewer.get_option_color(libvgcode::EOptionType::Travels)),
                    {{_u8L("Travel"), offsets[0]}}, true, predictable_icon_pos /*ORCA checkbox_pos*/, travel_visible,
                    [this, travel_visible]() {
    ...
    case libvgcode::EViewType::FanSpeed: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::FanSpeed), 0);
        break;
    }
    case libvgcode::EViewType::Temperature: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::Temperature), 0);
        break;
    }
        // ORCA: Add Pressure Advance visualization support
    case libvgcode::EViewType::PressureAdvance: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::PressureAdvance), 3);
        break;
    }
    case libvgcode::EViewType::LayerTimeLinear: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::LayerTimeLinear), true);
        break;
    }
    case libvgcode::EViewType::LayerTimeLogarithmic: {
        append_range(m_viewer.get_color_range(libvgcode::EViewType::LayerTimeLogarithmic), true);
        break;
    }
    case libvgcode::EViewType::VolumetricFlowRate: {
    ...
    case libvgcode::EViewType::Tool: {
        ...
        for (uint8_t extruder_id : used_extruders_ids) {
            ...
            append_item(EItemType::Rect, libvgcode::convert(m_viewer.get_tool_colors()[extruder_id]),
            ...
            // std::to_string(extruder_id + 1), true, "", 0.0f, 0.0f, offsets, used_filaments_m[extruder_id], used_filaments_g[extruder_id]);
            i++;
        }
        break;
    }
    case libvgcode::EViewType::Summary: {
        ...
        ImGui::SameLine();
        ::sprintf(buf, imperial_units ? "%.2f in / %.2f oz" : "%.2f m / %.2f g", ps.total_used_filament / koef,
                  ps.total_weight / unit_conver);
        imgui.text(buf);
            ...
            if (i < model_used_filaments_m.size() && i < model_used_filaments_g.size()) {
                ...
                float checkbox_pos =
                    std::max(predictable_icon_pos,
                             color_print_offsets[_u8L("Display")]); // ORCA prefer predictable_icon_pos when header not reacing end
                append_item(EItemType::Rect, libvgcode::convert(tool_colors[extruder_idx]), columns_offsets, false, checkbox_pos /*ORCA*/,
                            true, [this, extruder_idx]() {});
            }
            i++;
        }
    ...
    ImGui::Spacing();
    std::string time_title           = m_viewer.get_view_type() == libvgcode::EViewType::FeatureType ? _u8L("Total Estimation") :
                                                                                                       _u8L("Time Estimation");
    ...
    ImGui::Dummy({window_padding, window_padding});
    if (m_nozzle_nums > 1 && (m_viewer.get_view_type() == libvgcode::EViewType::Summary ||
                              m_viewer.get_view_type() == libvgcode::EViewType::ColorPrint)) // ORCA show only on summary and filament tab
