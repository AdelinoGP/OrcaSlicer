...
{
    /* [STATE] Initialize AMS-specific management objects and dev-manager. */
    ...
    m_button_ams_setting = new wxStaticBitmap(m_panel_option_left, wxID_ANY, m_button_ams_setting_normal.bmp(), wxDefaultPosition,
    ...
    if (wxGetApp().app_config->get("language") == "de_DE")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "fr_FR")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "ru_RU")
        m_button_extruder_feed->SetLabel("Load");
    if (wxGetApp().app_config->get("language") == "nl_NL")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "hu_HU")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "ja_JP")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "sv_SE")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "cs_CZ")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "uk_UA")
        m_button_extruder_feed->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "pt_BR")
        m_button_extruder_feed->SetLabel("Load");
    ...
    if (wxGetApp().app_config->get("language") == "de_DE")
        m_button_extruder_back->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "fr_FR")
        m_button_extruder_back->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "ru_RU")
        m_button_extruder_back->SetLabel("Unload");
    if (wxGetApp().app_config->get("language") == "nl_NL")
        m_button_extruder_back->SetFont(Label::Body_9);
    if (wxGetApp().app_config->get("language") == "hu_HU")
        m_button_extruder_back->SetFont(Label::Body_9);
...
void AMSControl::on_retry() { post_event(wxCommandEvent(EVT_AMS_RETRY)); }
...
void AMSControl::EnableUnLoadFilamentBtn(bool enable, const std::string& ams_id, const std::string& can_id, const wxString& tips)
{
    ...
    if (m_button_extruder_back->GetToolTipText() != tips) {
        BOOST_LOG_TRIVIAL(info) << "ams_id=" << ams_id << ", can_id=" << can_id
                                << "  Set Unload Filament Button ToolTip : " << tips.ToUTF8();
...
void AMSControl::msw_rescale()
{
    ...
    if (m_button_extruder_feed)
        m_button_extruder_feed->Rescale(); // ORCA
    if (m_button_extruder_back)
        m_button_extruder_back->Rescale(); // ORCA
    if (m_button_auto_refill)
        m_button_auto_refill->Rescale(); // ORCA
    if (m_button_ams_setting)
...
void AMSControl::CreateAmsDoubleNozzle(const std::string& series_name, const std::string& printer_type)
{
    ...
    for (auto ams_info = m_ams_info.begin(); ams_info != m_ams_info.end(); ams_info++) {
        if (ams_info->cans.size() == GENERIC_AMS_SLOT_NUM) {
            ams_info->nozzle_id == MAIN_EXTRUDER_ID ? m_item_ids[MAIN_EXTRUDER_ID].push_back(ams_info->ams_id) :
            ...
            if (ams_info->nozzle_id == MAIN_EXTRUDER_ID) {
                ...
                if (single_info_right.size() == 2) {
                    single_info_right[0].nozzle_id == MAIN_EXTRUDER_ID ?
                        m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_right[0].ams_id) :
                        m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_right[0].ams_id);
                    single_info_right[1].nozzle_id == MAIN_EXTRUDER_ID ?
                        m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_right[1].ams_id) :
                        m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_right[1].ams_id);
                ...
                if (single_info_left.size() == 2) {
                    single_info_left[0].nozzle_id == MAIN_EXTRUDER_ID ?
                        m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_left[0].ams_id) :
                        m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_left[0].ams_id);
                    single_info_left[1].nozzle_id == MAIN_EXTRUDER_ID ?
                        m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_left[1].ams_id) :
                        m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_left[1].ams_id);
    ...
    single_info_right[0].nozzle_id == MAIN_EXTRUDER_ID ? m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_right[0].ams_id) :
                                                         m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_right[0].ams_id);
    if (single_info_right.size() == 2) {
        single_info_right[1].nozzle_id == MAIN_EXTRUDER_ID ? m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_right[1].ams_id) :
                                                             m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_right[1].ams_id);
        pair_id.push_back(std::make_pair(single_info_right[0].ams_id, single_info_right[1].ams_id));
    }
    AddAms(single_info_right, series_name, printer_type);
    ...
    // wait add
    single_info_left[0].nozzle_id == MAIN_EXTRUDER_ID ? m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_left[0].ams_id) :
                                                        m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_left[0].ams_id);
    if (single_info_left.size() == 2) {
        single_info_left[1].nozzle_id == MAIN_EXTRUDER_ID ? m_item_ids[MAIN_EXTRUDER_ID].push_back(single_info_left[1].ams_id) :
                                                            m_item_ids[DEPUTY_EXTRUDER_ID].push_back(single_info_left[1].ams_id);
        pair_id.push_back(std::make_pair(single_info_left[0].ams_id, single_info_left[1].ams_id));
    }
    AddAmsPreview(single_info_left, AMSPanelPos::LEFT_PANEL);
...
void AMSControl::show_noams_mode() { EnterGenericAMSMode(); }
...
void AMSControl::enable_ams_setting(bool en) { m_button_ams_setting->Enable(en); }
...
std::vector<AMSinfo> AMSControl::GenerateSimulateData()
{
    ...
    std::vector<AMSinfo> single_ams  = {singleams1, singleams2, singleams3, singleams4, singleams5,
                                        singleams6, singleams7, singleams8, singleams9};
    std::vector<AMSinfo> ams_info    = {ams1, singleams2, ams3, singleams4, ams5, singleams6, ams7, singleams8, singleams9};
    return ams_info;
}

/*
 * [INTENT] Updates AMS and printer information.
 * [STATE] Rebuilds the AMS UI if necessary.
 * [THREAD] Should be called on the UI thread as it updates wxWidgets objects.
 */
                           ...
                           bool                 test)
{
    if (!test) {
        ...
        // basic check
        if (m_ams_info.size() == ams_info.size() && m_total_ext_count == data.GetTotalExtderCount() && m_dev_id == dev_id &&
            m_ext_info.size() == ext_info.size()) {
...
AMSRoadShowMode AMSControl::findFirstMode(AMSPanelPos pos)
{
    ...
    if (ams_id.empty() || item == m_ams_item_list.end())
        return init_mode;

    if (item->second->get_can_count() == GENERIC_AMS_SLOT_NUM) {
        if (item->second->get_ams_model() == AMSModel::AMS_LITE)
            return AMSRoadShowMode::AMS_ROAD_MODE_AMS_LITE;
        if (item->second->get_ams_model() == AMSModel::EXT_AMS && item->second->get_ext_type() == AMSModelOriginType::LITE_EXT)
            return AMSRoadShowMode::AMS_ROAD_MODE_AMS_LITE;
        ...
        if (item->second->get_ams_model() == AMSModel::EXT_AMS && item->second->get_ext_type() == AMSModelOriginType::LITE_EXT)
            return AMSRoadShowMode::AMS_ROAD_MODE_AMS_LITE;
        if (item->second->get_ams_model() == AMSModel::N3S_AMS)
            return AMSRoadShowMode::AMS_ROAD_MODE_SINGLE_N3S;
        return AMSRoadShowMode::AMS_ROAD_MODE_SINGLE;
    }
}

void AMSControl::createAmsPanel(wxSimplebook*        parent,
                                int&                 idx,
                                ...
                                int                  total_ext_num)
{
    ...
    if (ams1->get_ext_image()) {
        ams1->get_ext_image()->setTotalExtNum(series_name, printer_type, total_ext_num);
    }

    if (infos.size() == MAX_AMS_NUM_IN_PANEL) { // n3s and ? in a panel
        ams2 = new AmsItem(book_panel, infos[1], infos[1].ams_type, pos);
        if (ams2->get_ext_image()) {
            ams2->get_ext_image()->setTotalExtNum(series_name, printer_type, total_ext_num);
        }
        ...
        if (ams1->get_ext_image()) {
            ams1->get_ext_image()->setShowAmsExt(false);
        }
...
void AMSControl::AddAms(std::vector<AMSinfo> single_info, const std::string& series_name, const std::string& printer_type, AMSPanelPos pos)
{
    ...
    if (m_total_ext_count == 2) {
        if (single_info[0].nozzle_id == MAIN_EXTRUDER_ID) {
            createAmsPanel(m_simplebook_ams_right, m_right_page_index, single_info, series_name, printer_type, AMSPanelPos::RIGHT_PANEL,
                           m_total_ext_count);
        } else if (single_info[0].nozzle_id == DEPUTY_EXTRUDER_ID) {
            createAmsPanel(m_simplebook_ams_left, m_left_page_index, single_info, series_name, printer_type, AMSPanelPos::LEFT_PANEL,
                           m_total_ext_count);
        }
    } else if (m_total_ext_count == 1) {
        if (pos == AMSPanelPos::RIGHT_PANEL) {
...
/*
 * [INTENT] Switches the displayed AMS panel based on the provided AMS ID.
 * [EVENT] Triggered by user interaction (e.g., clicking on an AMS preview).
 * [UNITY] Implement via a controller that enables/disables UI panels in a
 *         Canvas group or swaps content in a UI Toolkit VisualElement.
 */
        ...
        if (item->get_ams_id() == ams_id) {
            ...
            for (auto id : ids) {
                if (id == item->get_ams_id()) {
                    pos == AMSPanelPos::LEFT_PANEL ? m_simplebook_ams_left->SetSelection(item->get_selection()) :
                                                     m_simplebook_ams_right->SetSelection(item->get_selection());
...
void AMSControl::ShowFilamentTip(bool hasams)
{
    ...
    if (hasams) {
        ...
        m_tip_load_info->SetLabelText(
            _L("Choose an AMS slot then press \"Load\" or \"Unload\" button to automatically load or unload filaments."));
    } else {
...
void AMSControl::on_filament_load(wxCommandEvent& event)
{
    ...
    if (filament_id.empty()) {
        MessageDialog
            msg_dlg(nullptr,
                    _L("Filament type is unknown which is required to perform this action. Please set target filament's informations."),
    ...
    for (auto i = 0; i < m_ams_info.size(); i++) {
        if (m_ams_info[i].ams_id == m_current_ams) {
            m_ams_info[i].current_action = AMSAction::AMS_ACTION_LOAD;
        }
    }
...
void AMSControl::on_extrusion_cali(wxCommandEvent& event)
{
    for (auto i = 0; i < m_ams_info.size(); i++) {
        if (m_ams_info[i].ams_id == m_current_ams) {
            m_ams_info[i].current_action = AMSAction::AMS_ACTION_CALI;
        }
    }
...
void AMSControl::on_filament_unload(wxCommandEvent& event)
{
    for (auto i = 0; i < m_ams_info.size(); i++) {
        if (m_ams_info[i].ams_id == m_current_ams) {
            m_ams_info[i].current_action = AMSAction::AMS_ACTION_UNLOAD;
        }
    }
    post_event(SimpleEvent(EVT_AMS_UNLOAD));
}

void AMSControl::auto_refill(wxCommandEvent& event) { post_event(SimpleEvent(EVT_AMS_FILAMENT_BACKUP)); }

void AMSControl::on_ams_setting_click(wxMouseEvent& event) { post_event(SimpleEvent(EVT_AMS_SETTINGS)); }
