...
{
    // [INTENT] Snapshot one AMS unit from the device model into a paint-friendly cache.
    // [STATE] This normalizes tray presence, color, remain, calibration, and humidity into Caninfo rows.
    // [UNITY] Port this as a presenter-side model builder feeding a retained tray view-model tree.
    if (!ams)
        return false;
...
{
    // [INTENT] Treat the virtual/external tray as a one-slot AMS variant with the same cache shape.
    // [PORTING_HAZARD:P2] External trays reuse the same data contract but collapse into a single visual slot.
...
{
    // [INTENT] Convert raw humidity into the discrete asset index expected by the icon set.
    // [UNITY] Keep this as a pure mapping helper so the view can swap sprites without duplicating business rules.
...
{
    // [INTENT] Tiny label widget used by EXT AMS to show the "Ext" marker above the tray stack.
    // [UNITY] This maps cleanly to a cached TextMeshPro label or simple Text element in a retained tray prefab.
...
// [INTENT] Clickable RFID refresh indicator with a loading animation and tray-id event bridge.
// [STATE] Keeps selected/playing/disabled flags plus an 8-frame bitmap loop on the UI thread.
// [EVENT] Posts EVT_AMS_REFRESH_RFID back to the parent; Unity should route this through an explicit button callback.
...
{
    if (m_play_loading || m_disable_mode)
        return;
...
{
    // [THREAD] UI-thread only: the timer invalidates paint, while clicks marshal to wxPostEvent on the same thread.
    if (m_play_loading) {
        return;
    }
...
void AMSrefresh::paintEvent(wxPaintEvent& evt)
{
    ...
    if (!wxWindow::IsEnabled()) {
        colour = StateColor::darkModeColorFor(AMS_CONTROL_GRAY500);
    }
...
{
    // [INTENT] Render one nozzle/extruder silhouette with a filled loading color when material is active.
    // [UNITY] A Unity port can use a sprite mask plus tinted Image overlay instead of repainting bitmaps.
...
{
    // [INTENT] Draw the external-tray badge / total-extruder marker depending on the host AMS configuration.
    // [STATE] Retains the ext position, printer series, and cached sprite keyed by printer type.
    if (over_ext) {
...
void AMSextruder::TurnOff() { m_left_extruder->TurnOff(); }
...
{
    if (m_nozzle_num == nozzle_num && m_series_name == series_name)
        return false;
    m_series_name = series_name;
...
{
    // [INTENT] Paint and manage one filament slot card, including selection, hover, edit affordance, and remaining-material fill.
    // [STATE] Holds the slot cache, hover/selection flags, pass-road overlay state, and cached scalable bitmaps.
    // [UNITY] Model this as a clickable tray card prefab with a separate visual-state controller and data-bound color fill.
...
{
    // [EVENT] The edit hotspot only dispatches when the card is already selected; the current click is a gate, not a direct action.
    // [PORTING_HAZARD:P2] Hit-testing is geometry- and model-dependent, so the Unity version needs the same conditional hotspot logic.
...
{
    // [OPENGL] N/A here: this is CPU-side wxDC composition, but the widget still behaves like a retained custom-painted card.
    // [UNITY] Use a retained prefab with layered images, gradient fills, and a separate overlay for selection/hover chrome.
#ifdef __WXMSW__
    wxSize     size = GetSize();
    wxMemoryDC memdc;
    wxBitmap   bmp(size.x, size.y);
    ...
    if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_THIRDBRAND || m_info.material_state == AMSCanType::AMS_CAN_TYPE_BRAND ||
        ...
        if (m_info.material_name.empty()) {
            ...
            if (has_split) {
                ...
                auto pot_top = wxPoint((libsize.x - line_top_tsize.x) / 2 + FromDIP(3),
                                       (libsize.y - line_top_tsize.y) / 2 - line_top_tsize.y);
    ...
    if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_THIRDBRAND || m_info.material_state == AMSCanType::AMS_CAN_TYPE_BRAND ||
        ...
        if (m_info.material_name.empty() /*&&  m_info.material_state != AMSCanType::AMS_CAN_TYPE_VIRTUAL*/) {
            ...
            if (has_split) {
                ...
                if (!m_show_kn) {
                    auto pot_top = wxPoint((libsize.x - line_top_tsize.x) / 2,
                                           (libsize.y - line_top_tsize.y) / 2 - line_top_tsize.y + FromDIP(6));
                    dc.DrawText(line_top, pot_top);
...
void AMSLib::render_lite_lib(wxDC& dc)
{
    ...
    if (m_info.material_cols.size() > 1) {
        ...
        if (m_ams_model == AMSModel::EXT_AMS) {
            ...
            dc.DrawRoundedRectangle(FromDIP(10), (size.y - libsize.y) / 2 + FromDIP(10), libsize.x - FromDIP(17), libsize.y - FromDIP(20),
                                    0);
        }
    }
    dc.SetPen(wxPen(*wxTRANSPARENT_PEN));
    ...
    if (!m_disable_mode) {
        ...
        if (m_info.material_state != AMSCanType::AMS_CAN_TYPE_EMPTY && m_info.material_state != AMSCanType::AMS_CAN_TYPE_NONE) {
            if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_THIRDBRAND || m_info.material_state == AMSCanType::AMS_CAN_TYPE_VIRTUAL)
                dc.DrawBitmap(temp_bitmap_third.bmp(), (size.x - temp_bitmap_third.GetBmpSize().x) / 2 + FromDIP(2),
                              (size.y - FromDIP(18) - temp_bitmap_third.GetBmpSize().y));
            if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_BRAND)
                dc.DrawBitmap(temp_bitmap_brand.bmp(), (size.x - temp_bitmap_brand.GetBmpSize().x) / 2 + FromDIP(2),
                              (size.y - FromDIP(18) - temp_bitmap_brand.GetBmpSize().y));
        }
    }

    // selected & hover
    if (m_selected) {
        dc.DrawBitmap(tray_bitmap_selected.bmp(), (size.x - tray_bitmap_selected.GetBmpSize().x) / 2,
                      (size.y - tray_bitmap_selected.GetBmpSize().y) / 2);
    } else if (!m_selected && m_hover) {
        dc.DrawBitmap(tray_bitmap_hover.bmp(), (size.x - tray_bitmap_hover.GetBmpSize().x) / 2,
                      (size.y - tray_bitmap_hover.GetBmpSize().y) / 2);
    } else {
...
void AMSLib::render_generic_lib(wxDC& dc)
{
    ...
    if (curr_height != 0) {
        curr_height = std::max(curr_height, FromDIP(2));
    } /*STUDIO-11323 too small to show, enlarge it*/
    dc.SetFont(::Label::Body_13);
        ...
        if (!m_disable_mode) {
            ...
            if (m_info.material_state != AMSCanType::AMS_CAN_TYPE_EMPTY && m_info.material_state != AMSCanType::AMS_CAN_TYPE_NONE) {
                if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_THIRDBRAND ||
                    m_info.material_state == AMSCanType::AMS_CAN_TYPE_VIRTUAL)
                    dc.DrawBitmap(temp_bitmap_third.bmp(), (size.x - temp_bitmap_third.GetBmpSize().x) / 2,
                                  (size.y - FromDIP(10) - temp_bitmap_third.GetBmpSize().y));
                if (m_info.material_state == AMSCanType::AMS_CAN_TYPE_BRAND)
                    dc.DrawBitmap(temp_bitmap_brand.bmp(), (size.x - temp_bitmap_brand.GetBmpSize().x) / 2,
                                  (size.y - FromDIP(10) - temp_bitmap_brand.GetBmpSize().y));
            }
        }
...
{
    // [STATE] Refreshes the slot snapshot from DeviceManager-selected machine state and invalidates transparency caches when colors change.
    ...
    if (info.material_colour.Alpha() != 0 && info.material_colour.Alpha() != 255 && info.material_colour.Alpha() != 254 &&
        m_info.material_colour != info.material_colour) {
    ...
    : AMSRoad()
{
    ...
    Bind(wxEVT_LEFT_UP, [this](wxMouseEvent& e) {
        if (m_canindex == 3 && m_show_humidity) {
            ...
            if (mouse_pos.x > rect.x + GetSize().x - FromDIP(40) && mouse_pos.y > rect.y + GetSize().y - FromDIP(40)) {
...
void AMSRoad::doRender(wxDC& dc)
{
    ...
    if (m_rode_mode == AMSRoadMode::AMS_ROAD_MODE_LEFT) {
        dc.DrawRoundedRectangle(-10, -10, size.x / 2 + 10, size.y * 0.6 + 10, 4);
    }
    ...
    if (m_road_color.Alpha() == 0) {
        dc.SetPen(wxPen(*wxWHITE, m_passroad_width, wxPENSTYLE_SOLID));
    } else {
        dc.SetPen(wxPen(m_road_color, m_passroad_width, wxPENSTYLE_SOLID));
    }
...
{
    // AMS_ROAD_MODE_NONE, AMS_ROAD_MODE_LEFT, AMS_ROAD_MODE_LEFT_RIGHT, AMS_ROAD_MODE_END_TOP, AMS_ROAD_MODE_END_BOTTOM,
    // AMS_ROAD_MODE_END_RIGHT, AMS_ROAD_MODE_LEFT, AMS_ROAD_MODE_LEFT_RIGHT, AMS_ROAD_MODE_END,
    ...
    auto left_right_types = std::vector<AMSPassRoadMode>{AMSPassRoadMode::AMS_ROAD_MODE_NONE, AMSPassRoadMode::AMS_ROAD_MODE_LEFT,
                                                         AMSPassRoadMode::AMS_ROAD_MODE_LEFT_RIGHT};
    auto end_types = std::vector<AMSPassRoadMode>{AMSPassRoadMode::AMS_ROAD_MODE_NONE, AMSPassRoadMode::AMS_ROAD_MODE_END_TOP,
                                                  AMSPassRoadMode::AMS_ROAD_MODE_END_BOTTOM, AMSPassRoadMode::AMS_ROAD_MODE_END_RIGHT};
    ...
    if (m_rode_mode == AMSRoadMode::AMS_ROAD_MODE_LEFT) {
        for (auto i = 0; i < prord_list.size(); i++) {
            ...
            if (iter != left_types.end())
                m_pass_rode_mode.push_back(prord_list[i]);
...
AMSRoadDownPart::AMSRoadDownPart(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size) : AMSRoadDownPart()
{
    /*m_info = info;
    m_canindex = canindex;*/
    // road type
...
void AMSRoadDownPart::create(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size)
{ wxWindow::Create(parent, id, pos, size); }

void AMSRoadDownPart::UpdateLeft(int nozzle_num, AMSRoadShowMode mode)
{
    if (nozzle_num == m_nozzle_num && m_left_rode_mode == mode) {
        return;
    }
...
{
    if (nozzle_num == m_nozzle_num && m_right_rode_mode == mode) {
        return;
    }
...
{
    if (m_left_rode_mode == left_mode && m_right_rode_mode == right_mode) {
        return;
    }
...
void AMSRoadDownPart::doRender(wxDC& dc)
{
    ...
    if (m_left_rode_mode == AMSRoadShowMode::AMS_ROAD_MODE_NONE || m_right_rode_mode == AMSRoadShowMode::AMS_ROAD_MODE_NONE) {
        ...
        case AMSRoadShowMode::AMS_ROAD_MODE_SINGLE_N3S:
            ...
            dc.DrawLine(left_nozzle_pos.x - FromDIP((129)), 0, left_nozzle_pos.x - FromDIP((129)), (size.y / 2));
            break;
        case AMSRoadShowMode::AMS_ROAD_MODE_AMS_LITE: dc.DrawLine(left_nozzle_pos.x, 0, left_nozzle_pos.x, size.y); break;
        default: break;
        }
    ...
    if (m_pass_road_left_step == AMSPassRoadSTEP::AMS_ROAD_STEP_2 || m_pass_road_left_step == AMSPassRoadSTEP::AMS_ROAD_STEP_3) {
        ...
        if (m_left_road_length > 0) {
            if (m_left_rode_mode == AMSRoadShowMode::AMS_ROAD_MODE_AMS_LITE) {
                ...
                dc.DrawLine((left_nozzle_pos.x), (size.y / 2), (left_nozzle_pos.x), (size.y));
                dc.DrawLine(left_nozzle_pos.x - FromDIP(m_left_road_length), (0), left_nozzle_pos.x - FromDIP(m_left_road_length),
                            (size.y / 2));
            }
        }
    }
...
void AMSRoadDownPart::UpdatePassRoad(AMSPanelPos pos, int len, AMSPassRoadSTEP step)
{
    if (m_nozzle_num >= 2) {
        if (pos == AMSPanelPos::LEFT_PANEL) {
            if (m_left_road_length == len && m_pass_road_left_step == step) {
                return;
            }
            ...
            if (m_right_road_length == len && m_pass_road_right_step == step) {
                return;
            }
        ...
        if (pos == AMSPanelPos::LEFT_PANEL) {
            if (m_left_road_length == len && m_pass_road_left_step == step && m_right_road_length == -1) {
                return;
            }
            m_left_road_length    = len;
            m_right_road_length   = -1;
            ...
            if (m_right_road_length == len && m_pass_road_right_step == step && m_left_road_length == -1) {
                return;
            }
            m_right_road_length    = len;
            m_left_road_length     = -1;
...
AMSPreview::AMSPreview(wxWindow* parent, wxWindowID id, AMSinfo amsinfo, AMSModel itemType, const wxPoint& pos, const wxSize& size)
    : AMSPreview()
{
    // [INTENT] Compact preview tile for the whole AMS assembly; it summarizes tray count and selection state in one clickable surface.
    // [UNITY] Map this to a selectable preview card that swaps between the four-slot and single-slot visual prefabs.
    wxWindow::Create(parent, id, pos);
...
void AMSPreview::doRender(wxDC& dc)
{
            ...
            if (iter->material_cols.size() > 1) {
                ...
                if (iter->material_colour.Alpha() == 0) {
                    ...
                    if (iter->material_state == AMSCanType::AMS_CAN_TYPE_EMPTY) {
                        ...
                        dc.DrawLine(rect.GetRight() - FromDIP(1), rect.GetTop() + FromDIP(1), rect.GetLeft() + FromDIP(1),
                                    rect.GetBottom() - FromDIP(1));
                    } else {
    ...
    // single slot
        ...
        if (iter.material_cols.size() > 1) {
            ...
            if (m_ams_item_type == AMSModel::N3S_AMS) {
                auto rect = wxRect(((size.x - AMS_ITEM_CUBE_SIZE.x) / 2), ((size.y - AMS_ITEM_CUBE_SIZE.y) / 2), (AMS_ITEM_CUBE_SIZE.x),
                                   (AMS_ITEM_CUBE_SIZE.y));
                dc.DrawRoundedRectangle(rect, 0);
            } else {
                auto rect = wxRect(((size.x - AMS_ITEM_CUBE_SIZE2.x) / 2), ((size.y - AMS_ITEM_CUBE_SIZE2.y) / 2), (AMS_ITEM_CUBE_SIZE2.x),
                                   (AMS_ITEM_CUBE_SIZE2.y));
        ...
        if (((iter.material_colour.Red() >= 238 && iter.material_colour.Green() >= 238 && iter.material_colour.Blue() >= 238) ||
             iter.material_colour.Alpha() == 0) &&
            m_ams_item_type == AMSModel::EXT_AMS) {
    ...
    if (!wxWindow::IsEnabled()) {
        border_colour = AMS_CONTROL_DISABLE_COLOUR;
    }
...
AMSHumidity::AMSHumidity(wxWindow* parent, wxWindowID id, AMSinfo info, const wxPoint& pos, const wxSize& size) : AMSHumidity()
{
    create(parent, id, pos, wxDefaultSize);

    for (int i = 1; i <= 5; i++) {
        ams_humidity_imgs.push_back(ScalableBitmap(this, "hum_level" + std::to_string(i) + "_light", 20));
    }
    for (int i = 1; i <= 5; i++) {
        ams_humidity_dark_imgs.push_back(ScalableBitmap(this, "hum_level" + std::to_string(i) + "_dark", 20));
    }
    for (int i = 1; i <= 5; i++) {
        ams_humidity_no_num_imgs.push_back(ScalableBitmap(this, "hum_level" + std::to_string(i) + "_no_num_light", 16));
    }
    for (int i = 1; i <= 5; i++) {
        ams_humidity_no_num_dark_imgs.push_back(ScalableBitmap(this, "hum_level" + std::to_string(i) + "_no_num_dark", 16));
    }
...
{
    // [STATE] The humidity card tracks stage, raw percent, and whether drying is active so the text/icon layout can reflow.
    if (m_amsinfo != amsinfo) {
        m_amsinfo = amsinfo;
        update_size();
        Refresh();
...
{
    for (auto& img : ams_humidity_imgs) {
        img.msw_rescale();
    }
    for (auto& img : ams_humidity_dark_imgs) {
        img.msw_rescale();
    }
    for (auto& img : ams_humidity_no_num_imgs) {
        img.msw_rescale();
    }
    for (auto& img : ams_humidity_no_num_dark_imgs) {
        img.msw_rescale();
    }
...
{
    // [INTENT] Root composite for one AMS block: it owns slot cards, refresh affordances, humidity, ext badges, and route overlays.
    // [STATE] Caches the machine model, panel side, tray selection, and child widget lookup tables used by later updates.
    // [UNITY] This wants a retained root prefab with child tray prefabs, route-line renderer, and slot lookup dictionaries.
...
{
    // [EVENT] Each tray cell wires its own click selection and refresh indicator; parent state is updated through wx events.
...
{
    // [PORTING_HAZARD:P2] AMS Lite uses a different spatial packing order and shared refresh glyphs, so the layout is not a 1:1 grid.
    auto amscan = new wxWindow(this, wxID_ANY);
    ...
    switch (canindex) {
    case 0: m_sizer_ams->Add(m_panel_lib, 0, wxALIGN_RIGHT | wxALIGN_BOTTOM, 0); break;
    case 1: m_sizer_ams->Add(m_panel_lib, 0, wxALIGN_RIGHT | wxALIGN_TOP, 0); break;
    case 2: m_sizer_ams->Add(m_panel_lib, 0, wxALIGN_LEFT | wxALIGN_TOP, 0); break;
    case 3: m_sizer_ams->Add(m_panel_lib, 0, wxALIGN_LEFT | wxALIGN_BOTTOM, 0); break;
    default: break;
    }
...
{
    // [STATE] Propagates a fresh AMS snapshot into all child widgets, showing or hiding slots to match tray count changes.
    if (m_info == info) {
        return;
    }
...
{
    // [EVENT] Selection is fan-out state: the chosen tray is highlighted, siblings are cleared, and the parent is notified once.
    for (auto lib_it : m_can_lib_list) {
        AMSLib* lib = lib_it.second;
...
wxColour AmsItem::GetTagColr(wxString canid)
{
    ...
    for (auto lib_it : m_can_lib_list) {
        AMSLib* lib = lib_it.second;
        if (canid == lib->m_info.can_id)
            tag_colour = lib->GetLibColour();
    }
    return tag_colour;
}
...
{
    // [EVENT] This path drives pass-road highlighting from the transport workflow into the tray cards and route overlays.
    if (m_panel_road) {
...
{
    // [EVENT] This is the slot-highlighting bridge for pass-road visualization; it fans out to sibling card overlays.
...
void AmsItem::doRender(wxDC& dc)
{
    ...
    // road for extra
    if (m_ams_model == AMSModel::AMS_LITE) {
        dc.DrawBitmap(m_bitmap_extra_framework.bmp(), (size.x - m_bitmap_extra_framework.GetBmpSize().x) / 2,
                      (size.y - m_bitmap_extra_framework.GetBmpSize().y) / 2 - FromDIP(4));
...
void AmsItem::msw_rescale()
{
    ...
    if (m_humidity != nullptr)
        m_humidity->msw_rescale();
    if (m_ext_text != nullptr)
        m_ext_text->msw_rescale();
    if (m_ext_image != nullptr)
        m_ext_image->msw_rescale(); // ext image upon the lib
