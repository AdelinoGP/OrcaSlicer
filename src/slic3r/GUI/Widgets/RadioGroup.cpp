...
// [INTENT] RadioGroup is a composite radio-style selector: each option is a bitmap + text pair, selection wraps with
// keyboard arrows, and the widget emits wxEVT_COMMAND_RADIOBOX_SELECTED whenever the active item changes.
// [STATE] The control caches labels, paired child widgets, the selected index, enabled state, and hover/focus colors;
// all visible state is rebuilt from that model instead of being stored separately per item.
// [UNITY] Port as a retained radio-list component with one selected index, row/column layout, hover/focus styling,
// and a typed selection-changed callback.
RadioGroup::RadioGroup(wxWindow* parent, const std::vector<wxString>& labels, long direction, int row_col_limit)
...
{
    // [INTENT] Build the paired icon/text rows and wire the interaction model once so the group behaves like a single
    // radio list rather than independent controls.
    // [PORTING_HAZARD:P3] Hover feedback is split across sibling widgets, with mouse-leave hit-testing used to avoid
    // flicker when the pointer crosses between the icon and the label; a Unity port should keep the whole row as one
    ...
    m_text_color = StateColor(std::pair(wxColour("#6B6A6A"), (int) StateColor::Disabled),
                              std::pair(wxColour("#363636"), (int) StateColor::Enabled));

    m_focus_color = StateColor(std::pair(bg, (int) StateColor::NotFocused), std::pair(wxColour("#009688"), (int) StateColor::Focused));
    ...
    for (int i = 0; i < m_item_count; ++i) {
        ...
        rb->Bind(wxEVT_LEFT_DOWN, ([this, i](wxMouseEvent e) {
                     SetSelection(i, true);
                     e.Skip();
                 }));
        rb->Bind(wxEVT_ENTER_WINDOW, ([this, i](wxMouseEvent e) {
                     SetRadioIcon(i, true);
                     e.Skip();
                 }));
        ...
        tx->Bind(wxEVT_BUTTON, ([this, i](wxCommandEvent e) {
                     SetSelection(i, true);
                     e.Skip();
                 }));
        ...
        tx->Bind(wxEVT_CHAR_HOOK, ([this, tx](wxKeyEvent& e) {
                     if (tx->HasFocus()) {
                         ...
                         // [EVENT] Arrow-key navigation wraps inside the group so keyboard focus behaves like one closed radio
                         // list instead of separate independent buttons.
                         if (is_next)
                             SelectNext();
                         else if (is_prev)
                             SelectPrevious();
...
void RadioGroup::SetSelection(int index, bool focus)
{
    if (index >= 0 && index < m_item_count) {
        // [EVENT] Selection changes update focus ownership, refresh every icon from the derived selected/hover state,
        // and then publish wxEVT_COMMAND_RADIOBOX_SELECTED so outside dialogs can mirror the current value.
        int prev_index = m_selectedIndex;
        if (index !=
            prev_index) { // prevent no focusable item on first creation. wxPanel starts taking focus if there is no focusable control
...
int RadioGroup::GetSelection() { return m_selectedIndex; }
...
{
    // [STATE] Visual state is a pure function of enabled + selected + hover, so the sprite choice should stay derived
    // from the model rather than being cached independently in each child widget.
...
{
    // [EVENT] Enabling/disabling is mirrored into child buttons and broadcast through EVT_ENABLE_CHANGED so host
    // panels can react to interactability changes.
    // [UNITY] Model this as a controller-owned interactable flag plus one UI event rather than disabling children ad hoc.
    m_enabled   = enable;
