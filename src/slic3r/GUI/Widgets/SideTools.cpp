...
// [INTENT] SideToolsPanel is the live monitor header strip: it renders the printer name, signal icon, and hover state while keeping the
// status data in local widget state. [STATE] The panel caches printer identity, wifi strength, none-printer mode, hover/click flags, and a
// short interval timer used to suppress repeated clicks. [EVENT] Paint, timer, and mouse bindings all funnel back into Refresh-driven
// redraws instead of mutating the parent layout directly. [UNITY] Port this as a retained header-row controller with image/text slots, a
// hover highlight, and a separate state source for connection status. [PORTING_HAZARD:P2] The current layout depends on wxDC text
// measurements plus hand-tuned pixel offsets, so a Unity port needs explicit truncation/layout logic rather than pixel parity.
...
{
    if (m_dev_name == from_u8(dev_name) && !m_none_printer)
        return;
...
{
    if (last_printer_signal == sign && !m_none_printer)
        return;
...
// [INTENT] start_interval/stop_interval implement a brief click-debounce window so the header row does not immediately re-trigger while the
// pointer is still down. [STATE] The interval bit is owned by the panel and mirrored in the wxTimer lifecycle. [UNITY] Use a short
// coroutine/timer gate or a pressed-state cooldown if the Unity version needs the same anti-repeat behavior.
...
bool SideToolsPanel::is_in_interval() { return m_is_in_interval; }
...
// [INTENT] The renderer branches between connected and no-printer visual states, composing iconography, labels, and status colors from
// cached widget state. [STATE] The draw path depends on m_none_printer, m_dev_name, m_wifi_type, hover state, and the rescaled bitmap
// cache. [UNITY] Model this as a retained row prefab with conditional child visibility and a dedicated text-truncation helper.
// [PORTING_HAZARD:P3] Text shortening is implemented with ad-hoc width probing, so a Unity port should replace it with a deterministic
// layout pass.
...
// [INTENT] doRender is the actual paint routine: it draws the monitor strip in either printer-present or no-printer mode and then layers
// hover chrome on top. [STATE] It consumes the cached bitmaps, current printer text, and wifi signal state, then trims labels to fit the
// remaining pixel budget. [UNITY] Port this as a single retained draw pass inside a controller that owns the layout measurements and the
// per-state icon set. [PORTING_HAZARD:P3] Both icon placement and text truncation are tightly coupled to current DC metrics, so faithful
// Unity rendering needs a layout helper rather than direct bitmap blits.
void SideToolsPanel::doRender(wxDC& dc)
{
    ...
    if (m_none_printer) {
        ...
        dc.SetBrush(
            StateColor::darkModeColorFor(SIDE_TOOLS_BRAND)); // ORCA: Sidebar header background color - Fix for dark mode compability
        dc.DrawRectangle(0, 0, size.x, size.y);
        ...
        if (m_wifi_type == WifiSignal::NONE)
            dc.DrawBitmap(m_wifi_none_img.bmp(), left, (size.y - m_wifi_none_img.GetBmpSize().y) / 2);
        if (m_wifi_type == WifiSignal::WEAK)
            dc.DrawBitmap(m_wifi_weak_img.bmp(), left, (size.y - m_wifi_weak_img.GetBmpSize().y) / 2);
        if (m_wifi_type == WifiSignal::MIDDLE)
            dc.DrawBitmap(m_wifi_middle_img.bmp(), left, (size.y - m_wifi_middle_img.GetBmpSize().y) / 2);
        if (m_wifi_type == WifiSignal::STRONG)
            dc.DrawBitmap(m_wifi_strong_img.bmp(), left, (size.y - m_wifi_strong_img.GetBmpSize().y) / 2);
        if (m_wifi_type == WifiSignal::WIRED)
            dc.DrawBitmap(m_network_wired_img.bmp(), left, (size.y - m_network_wired_img.GetBmpSize().y) / 2);
    }
...
// [EVENT] Mouse enter/leave and button transitions only flip local hover/click flags and request a repaint; they do not dispatch actions
// from this control. [UNITY] Use pointer-enter/exit and pointer-down/up handlers to drive a hover/pressed visual state on the retained row.
...
// [INTENT] SideTools is the composite sidebar module: it stacks the connection banner, expandable error details, and the live
// SideToolsPanel into one vertical block. [STATE] The wrapper owns the banner/button/link widgets, the detail panel, and the expansion bit
// that keeps the error drawer open or closed. [EVENT] The inline toggle handler on the more button shows or hides the detail panel and
// forces the parent tab panel to relayout. [UNITY] Port this as a retained vertical container with a banner row, collapsible detail card,
// and an embedded status-strip child. [PORTING_HAZARD:P2] The current implementation reaches back into m_tabpanel for Refresh/Layout, so
// the Unity version needs an explicit parent-controller contract.
SideTools::SideTools(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size)
{
    ...
    // ORCA standardized HyperLink
    m_link_network_state = new HyperLink(m_side_error_panel, _L("Check the status of current system services"),
                                         wxGetApp().link_to_network_check(), wxST_ELLIPSIZE_END);
    m_link_network_state->SetMaxSize(wxSize(FromDIP(220), -1));
    m_link_network_state->SetFont(::Label::Body_12);

    auto st_title_error_code     = new wxStaticText(m_side_error_panel, wxID_ANY, _L("code"), wxDefaultPosition, wxDefaultSize,
                                                    wxST_ELLIPSIZE_END);
    ...
    auto st_title_error_desc     = new wxStaticText(m_side_error_panel, wxID_ANY, wxT("desc"), wxDefaultPosition, wxDefaultSize,
                                                    wxST_ELLIPSIZE_END);
    ...
    auto st_title_extra_info     = new wxStaticText(m_side_error_panel, wxID_ANY, wxT("info"), wxDefaultPosition, wxDefaultSize,
                                                    wxST_ELLIPSIZE_END);
...
bool SideTools::is_in_interval() { return m_side_tools->is_in_interval(); }

void SideTools::set_current_printer_name(std::string dev_name) { m_side_tools->set_current_printer_name(dev_name); }

void SideTools::set_current_printer_signal(WifiSignal sign) { m_side_tools->set_current_printer_signal(sign); }

void SideTools::set_none_printer_mode() { m_side_tools->set_none_printer_mode(); }

void SideTools::start_interval() { m_side_tools->start_interval(); }
...
// [INTENT] update_status translates printer connectivity state into banner visibility, banner copy, and detailed wifi-signal presentation.
// [STATE] It consumes MachineObject connectivity flags, wired-vs-wireless status, and the current wifi dBm string to choose a signal icon.
// [UNITY] Treat this as a presenter method that feeds a status view-model rather than letting widgets inspect MachineObject directly.
// [PORTING_HAZARD:P3] The dBm parsing is string-based and silently falls back on malformed input, so a Unity port should normalize signal
// strength upstream.
...
// [INTENT] show_status maps monitor bitmasks to one of three user-facing states: no printer, connecting, or disconnected-with-details.
// [STATE] It toggles the banner, error drawer, hyperlink text, and expansion state in response to MonitorStatus flags.
// [UNITY] Model this as a small state machine driving banner/detail subviews, with the error-copy and action link sourced from a status
// payload. [PORTING_HAZARD:P2] Visibility is coordinated through direct Hide/Show/Layout/Fit calls, so the Unity port needs an explicit
// layout state transition instead of imperative relayout.
void SideTools::show_status(int status)
{
    if (((status & (int) MonitorStatus::MONITOR_DISCONNECTED) != 0) || ((status & (int) MonitorStatus::MONITOR_DISCONNECTED_SERVER) != 0)) {
        if ((status & (int) MonitorStatus::MONITOR_DISCONNECTED_SERVER)) {
            ...
            update_connect_err_info(BAMBU_NETWORK_ERR_CONNECTION_TO_SERVER_FAILED, _L("Failed to connect to cloud service"),
            ...
            update_connect_err_info(BAMBU_NETWORK_ERR_CONNECTION_TO_PRINTER_FAILED, _L("Connection to printer failed"),
