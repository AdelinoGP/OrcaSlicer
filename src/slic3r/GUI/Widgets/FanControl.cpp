...
// [STATE] This process-global suppression flag outlives any single widget instance, so one fan prompt can silence warnings for every
// fan-control popup. [PORTING_HAZARD:P2] Unity should scope this decision to the current dialog/session, not a hidden static shared across
// unrelated controls.
...
Description:Fan
**************************************************/
// [INTENT] Fan is the passive readout half of the fan widget: it paints the current speed as a gauge and leaves all mutation to the sibling
// control strip. [STATE] The pre-scaled bitmap set and m_current_speeds are the only live inputs to the draw path. [UNITY] Port this as a
// retained image/text readout with a cached sprite set and a single integer speed model.
Fan::Fan(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size) { create(parent, id, pos, size); }
...
{
    // [INTENT] This is an immediate-mode paint path with a Windows-only double-buffered branch; the widget redraws directly from cached bitmaps
    // instead of keeping a scene graph. [UNITY] Use a retained Image/Text tree and let the UI system handle buffering and invalidation.
#ifdef __WXMSW__
    wxSize     size = GetSize();
    wxMemoryDC memdc;
...
void Fan::doRender(wxDC& dc)
{
    ...
    auto central_point = wxPoint((size.x - m_bitmap_scales[m_current_speeds].GetBmpSize().x) / 2,
                                 (size.y - m_bitmap_scales[m_current_speeds].GetBmpSize().y) / 2 - FromDIP(4));
...
void Fan::DoSetSize(int x, int y, int width, int height, int sizeFlags) { wxWindow::DoSetSize(x, y, width, height, sizeFlags); }

/*************************************************
Description:FanOperate
**************************************************/
// [INTENT] FanOperate is the interactive +/- strip that mutates fan speed, shows the current percentage, and emits discrete add/dec/switch
// events. [STATE] m_min_speeds/m_max_speeds bound the UI affordance, while the global warning-suppression flag can affect every instance.
// [EVENT] Mouse clicks are translated into command events on the widget itself; the parent listens for EVT_FAN_* and syncs machine state.
// [UNITY] Port as a retained stepper/toggle row with explicit command callbacks and a view-model for the speed value.
// [PORTING_HAZARD:P2] Manual screen-space hit testing and global warning suppression couple the widget to wx layout/state in ways a Unity
// prefab should not inherit.
...
{
    // [EVENT] The click target is derived from absolute screen coordinates rather than widget-local hit testing, so the control assumes
    // fixed geometry and DPI-aware widths.
    ...
    if (mouse_pos.x > win_pos.x && mouse_pos.x < (decrease_fir + win_pos.x) && mouse_pos.y > win_pos.y &&
        mouse_pos.y < (win_pos.y + GetSize().y)) {
        decrease_fan_speeds();
        return;
    }

    if (mouse_pos.x > (add_fir + win_pos.x) && mouse_pos.x < (win_pos.x + GetSize().x) && mouse_pos.y > win_pos.y &&
        mouse_pos.y < (win_pos.y + GetSize().y)) {
        add_fan_speeds();
        return;
    }
...
{
    // [STATE] A single modal confirmation gates every fan change while printing, and the user's "change anyway" choice is cached globally
    // by not_show_fan_speed_warning_dlg.
    if (m_obj && !not_show_fan_speed_warning_dlg && m_obj->is_in_printing()) {
        MessageDialog msg_wingow(nullptr, _L("Changing fan speed during printing may affect print quality, please choose carefully."), "",
                                 wxICON_WARNING | wxCANCEL | wxOK);
...
void FanOperate::add_fan_speeds()
{
    ...
    if (m_current_speeds + 1 > m_max_speeds)
        return;
...
**************************************************/
// [INTENT] FanControlNew composes the readout and control strip for one fan/duct entry, switching between manual control, auto, and off
// states. [STATE] m_fan_data mirrors the current device duct topology; m_switch_fan, m_current_speed, and m_show_mode drive icon and
// visibility changes. [EVENT] Child widgets forward switch/add/dec events back through EVT_FAN_CHANGED and the device-control helpers.
// [UNITY] Port as a card prefab with a data-bound label, sprite toggle, and nested stepper child; keep the mode logic in a
// controller/view-model rather than the view. [PORTING_HAZARD:P2] The display mode is inferred from device metadata and protocol support,
// so the Unity UI needs an explicit enum instead of reproducing the current implicit branching.
FanControlNew::FanControlNew(
    wxWindow* parent, const AirDuctData& fan_data, int mode_id, int part_id, wxWindowID id, const wxPoint& pos, const wxSize& size)
    : wxWindow(parent, id, pos, size), m_fan_data(fan_data), m_mode_id(mode_id), m_part_id(part_id)
{
    ...
    m_static_name = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                                     wxST_ELLIPSIZE_END | wxALIGN_CENTER_HORIZONTAL);
    ...
    m_static_status_name = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                                            wxST_ELLIPSIZE_END | wxALIGN_CENTER_HORIZONTAL);
...
void FanControlNew::command_control_fan()
{
    ...
    // [THREAD] This method immediately bridges the UI event into the device fan service; the reply path is intentionally ignored here, so
    // the caller depends on later refresh state. [UNITY] Route this through an async device command service and apply returned state on the
    // main thread after confirmation.
...
{
    // [STATE] The widget recomputes visibility from the active duct mode and the current part's membership in the mode's off/ctrl lists.
    // [PORTING_HAZARD:P3] This implicit mode inference should become an explicit UI state enum in Unity so the view does not re-derive
    // business rules from raw lists.
...
**************************************************/
// [INTENT] FanControlPopupNew is the modal air-duct/fan configuration sheet: it builds mode selectors, per-part fan tiles, and an optional
// cooling-filter submode switch from live machine data. [STATE] m_data caches a snapshot of AirDuctData; the timeout counters debounce
// refreshes, and the maps keep child widgets alive across rebuilds. [EVENT] Clicks are manually hit-tested and re-routed to child controls,
// while EVT_FAN_CHANGED is used as the popup-to-parent completion signal. [UNITY] Port as a modal overlay with recyclable mode chips, part
// cards, and a separate device-control service. [PORTING_HAZARD:P2] Manual screen-space hit testing plus repeated Layout/Fit rebuilds make
// the UI dependent on wx geometry semantics.
...
{
    // [STATE] Rebuilding the popup reuses and replaces per-mode/per-part child widgets, so the maps are the real ownership boundary rather
    // than the sizers.
    ...
    while (iter != m_data.modes.end()) {
        ...
        if (text.empty()) {
            BOOST_LOG_TRIVIAL(error) << "FanControlPopupNew::CreateDuct: radio_btn_name is empty";
        }
...
{
    // [INTENT] Some duct modes expose an extra submode toggle (currently the cooling-filter switch), which is created and destroyed on demand.
    // [UNITY] Model this as an optional nested toggle row that appears only when the active duct mode supports it.
    ...
    if (AIR_DUCT(m_data.curren_mode) == AIR_DUCT::AIR_DUCT_COOLING_FILT && m_data.IsSupportCoolingFilter()) {
        if (!m_cooling_filter_switch_panel) {
            m_cooling_filter_switch_panel = new FanControlNewSwitchPanel(
                m_sub_mode_panel, _L("Filter"),
                _L("Enabling filtration redirects the right fan to filter gas, which may reduce cooling performance."));
            m_cooling_filter_switch_panel->Bind(EVT_FANCTRL_SWITCH, [this](wxCommandEvent& evt) {
                if (m_obj && m_obj->is_in_printing()) {
                    MessageDialog msg_wingow(
                        nullptr,
                        _L("Enabling filtration during printing may reduce cooling and affect print quality. Please choose carefully."), "",
                        wxICON_WARNING | wxCANCEL | wxOK);
                    msg_wingow.SetButtonLabel(wxID_OK, _L("Change Anyway"));
...
{
    // [THREAD] The popup self-throttles refreshes with two timeout counters so repeated device polling does not thrash the UI while a
    // recent command is still settling.
    if (!obj)
        return;
...
{
    // [STATE] This path replaces the cached AirDuctData snapshot and then pushes the new percentages back into every live child control.
    if (m_data == data) {
        return;
    }
...
{
    // [EVENT] The popup manually forwards a single click to whichever mode chip or nested fan toggle contains the screen point.
...
void FanControlPopupNew::on_show(wxShowEvent& evt) { wxGetApp().UpdateDlgDarkUI(this); }

void FanControlPopupNew::command_control_air_duct(int mode_id, int submode)
{
    // [THREAD] The device command is fired immediately and the popup only updates local optimistic state; the JSON reply callback is
    // intentionally discarded here. [UNITY] This should become an async service call with explicit completion/failure state instead of
    // fire-and-forget UI logic.
...
{
    for (const auto& btn_iter : m_mode_switch_btns) {
        btn_iter.second->msw_rescale();
    }
    for (const auto& fan_iter : m_fan_control_list) {
        fan_iter.second->msw_rescale();
    }
}
...
{
    // [EVENT] Mode chips are toggles with a printing-time lockout: the selected mode can be blocked while the printer is actively running.
    /* go check*/
    if (m_obj && m_obj->is_in_printing()) {
        ...
        if (btn && !btn->isSelected()) {
            MessageDialog
                msg_wingow(nullptr,
                           _L("The selected material only supports the current fan mode, and it can't be changed during printing."), "",
                           wxICON_WARNING | wxOK);
...
void FanControlPopupNew::on_fan_changed(const wxCommandEvent& event) { m_fan_set_time_out = time_out; }

void FanControlPopupNew::init_names(MachineObject* obj)
{
    // [STATE] The popup seeds localized labels and printer-specific overrides from device configuration so the visible mode text can change
    // per machine family. Iint fan/door/func/duct name lists
    ...
    label_text[AIR_DUCT::AIR_DUCT_COOLING_FILT] = _L(
        "Cooling mode is suitable for printing PLA/PETG/TPU materials and filters the chamber air.");
    label_text[AIR_DUCT::AIR_DUCT_HEATING_INTERNAL_FILT] = _L(
        "Heating mode is suitable for printing ABS/ASA/PC/PA materials and circulates filters the chamber air.");
    label_text[AIR_DUCT::AIR_DUCT_EXHAUST]      = _L("Exhaust");
    label_text[AIR_DUCT::AIR_DUCT_FULL_COOLING] = _L(
        "Strong cooling mode is suitable for printing PLA/TPU materials. In this mode, the printouts will be fully cooled.");
...
{
    // [UNITY] The fallback path is a localization lookup table keyed by fan function; in Unity this is a data-driven label table, not
    // hard-coded widget text.
    if (m_obj) {
    ...
    static std::map<AIR_FUN, wxString> s_fan_func_name{
        {AIR_FUN::FAN_HEAT_BREAK_0_IDX, _L("Hotend")},    {AIR_FUN::FAN_COOLING_0_AIRDOOR, _L("Parts")},
        {AIR_FUN::FAN_REMOTE_COOLING_0_IDX, _L("Aux")},   {AIR_FUN::FAN_CHAMBER_0_IDX, _L("Exhaust")},
        {AIR_FUN::FAN_HEAT_BREAK_1_IDX, _L("Nozzle1")},   {AIR_FUN::FAN_MC_BOARD_0_IDX, _L("MC Board")},
...
// [INTENT] FanControlNewSwitchPanel is a tiny labeled toggle row used for optional submodes such as the cooling filter.
// [STATE] It owns only the switch bitmap and a boolean; the parent owns the real duct state and command semantics.
// [EVENT] The click handler flips local visuals first, then posts EVT_FANCTRL_SWITCH so the enclosing popup can validate and issue the
// device command. [UNITY] Port as a simple labeled toggle row with a change callback. [PORTING_HAZARD:P3] The optimistic local flip can
// diverge from device state until the parent re-syncs the model.
