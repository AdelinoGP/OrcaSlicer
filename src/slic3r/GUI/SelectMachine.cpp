...
// [INTENT] Send-print modal that owns printer selection, AMS/extruder mapping, status gating, and preview composition.
// [UNITY] Port this as a modal controller with a retained view-model, a printer dropdown, and explicit async status transitions.
// [PORTING_HAZARD:P2] The dialog mixes network polling, manual image compositing, and user confirmation flow in one class.
...
void                  SelectMachineDialog::init_machine_bed_types()
{
    if (MACHINE_BED_TYPE_STRING.size() == 0) {
        MACHINE_BED_TYPE_STRING = {//_L("Auto"),
                                   _L("Bambu Cool Plate") + " / " + _L("PLA Plate"), _L("Bambu Engineering Plate"),
                                   _L("Bambu Smooth PEI Plate") + "/" + _L("High temperature Plate"), _L("Bambu Textured PEI Plate"),
    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
   ...
   m_mode_send->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
    ...
    m_rename_button       = new wxStaticBitmap(m_rename_normal_panel, wxID_ANY, rename_editable->bmp(), wxDefaultPosition,
    ...
    m_bitmap_last_plate   = new wxStaticBitmap(m_basic_panel, wxID_ANY, create_scaled_bitmap("go_last_plate", m_scroll_area, 25),
                                               wxDefaultPosition, wxSize(FromDIP(25), FromDIP(25)), 0);
    ...
    m_bitmap_next_plate   = new wxStaticBitmap(m_basic_panel, wxID_ANY, create_scaled_bitmap("go_next_plate", m_scroll_area, 25),
                                               wxDefaultPosition, wxSize(FromDIP(25), FromDIP(25)), 0);
    ...
    img_ams_backup = new wxStaticBitmap(m_scroll_area, wxID_ANY, create_scaled_bitmap("automatic_material_renewal", this, 16),
                                        wxDefaultPosition, wxSize(FromDIP(16), FromDIP(16)), 0);
    ...
    // auto m_img_mapping_sugs = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("warning", this, 16), wxDefaultPosition,
    ...
    // auto m_img_change_filament_times = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("warning", this, 16), wxDefaultPosition,
    // wxSize(FromDIP(16), FromDIP(16)));
    ...
    auto option_auto_bed_level =
        new PrintOption(m_options_other, _L("Auto Bed Leveling"),
                        _L("This checks the flatness of heatbed. Leveling makes extruded height uniform.\n*Automatic mode: Run a leveling "
                           "check(about 10 seconds). Skip if surface is fine."),
                        ops_auto, "bed_leveling");

    auto option_flow_dynamics_cali = new PrintOption(m_options_other, _L("Flow Dynamics Calibration"),
                                                     _L("This process determines the dynamic flow values to improve overall print "
                                                        "quality.\n*Automatic mode: Skip if the filament was calibrated recently."),
                                                     ops_auto, "flow_cali");

    auto option_nozzle_offset_cali_cali = new PrintOption(m_options_other, _L("Nozzle Offset Calibration"),
                                                          _L("Calibrate nozzle offsets to enhance print quality.\n*Automatic mode: Check "
                                                             "for calibration before printing. Skip if unnecessary."),
    ...
    auto completedimg = new wxStaticBitmap(m_panel_finish, wxID_ANY, create_scaled_bitmap("completed", m_panel_finish, 25),
                                           wxDefaultPosition, wxSize(imgsize, imgsize), 0);
    ...
    m_sw_print_failed_info = new wxScrolledWindow(this, wxID_ANY, wxDefaultPosition,
                                                  wxSize(SELECT_MACHINE_DIALOG_SIMBOOK_SIZE2.x, FromDIP(125)), wxVSCROLL);
    ...
    m_link_network_state = new HyperLink(m_sw_print_failed_info, _L("Check the status of current system services"),
...
{
    // [EVENT] Central event wiring for timer ticks, custom printer/mapping events, and selection/hover behavior.
    // [THREAD] Custom events cross worker/UI boundaries; Unity should marshal these onto the main thread before touching UI.
...
void SelectMachineDialog::update_select_layout(MachineObject* obj)
{
    ...
    if (obj && obj->get_printer_arch() == PrinterArch::ARCH_I3) {
        m_checkbox_list["timelapse"]->setValue("off");
    } /*off timelapse on selected for n series by zhimin.zeng*/
...
{
    // [STATE] Reset the dialog into an editable pre-send state and unblock the shared material controls.
...
{
    // [STATE] Lock the dialog while a send job is active so the printer/map controls cannot drift mid-transfer.
    // disable combobox
...
{
    // [STATE] Show the terminal success page and restore editing after the send flow resolves.
    EnableEditing(true);
...
{
    // [STATE] Keep the per-filament mapping badges synchronized with the latest printer-side AMS assignment result.
    if (result.empty()) {
    ...
    for (auto f = result.begin(); f != result.end(); f++) {
        BOOST_LOG_TRIVIAL(trace) << "ams_mapping f id = " << f->id << ", tray_id = " << f->tray_id << ", color = " << f->color
                                 << ", type = " << f->type;
    ...
    if (nozzle_nums > 1) {
        ...
        if (true /*!can_hybrid_mapping(obj_get_extder_data())*/) {
            ...
            std::merge(m_ams_mapping_result_left.begin(), m_ams_mapping_result_left.end(), m_ams_mapping_result_right.begin(),
                       m_ams_mapping_result_right.end(), m_ams_mapping_result.begin(), [](const FilamentInfo& f1, const FilamentInfo& f2) {
...
bool SelectMachineDialog::can_hybrid_mapping(DevExtderSystem data)
{
    ...
    if (flow_types.size() <= 1 || flow_types.size() != flow_type_of_machine.size())
        return false;
...
bool SelectMachineDialog::is_nozzle_type_match(DevExtderSystem data, wxString& error_message) const
{
    ...
    auto nozzle_volume_type_opt = dynamic_cast<const ConfigOptionEnumsGeneric*>(
        wxGetApp().preset_bundle->project_config.option("nozzle_volume_type"));
        ...
        if (target_machine_nozzle_id < flow_type_of_machine.size()) {
            if (flow_type_of_machine[target_machine_nozzle_id] != used_extruders_flow[it->first]) {
                ...
                error_message = wxString::Format(_L("The nozzle flow setting of %s(%s) doesn't match with the slicing file(%s). "
                                                 ...
                                                 pos, _L(flow_type_of_machine[target_machine_nozzle_id]),
...
void SelectMachineDialog::prepare(int print_plate_idx) { m_print_plate_idx = print_plate_idx; }
...
void SelectMachineDialog::show_status(PrintDialogStatus status, std::vector<wxString> params, wxString wiki_url)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text;
        if (params.size() > 1)
            msg_text = wxString::Format(_L("Filament %s does not match the filament in AMS slot %s. Please update the printer firmware to "
        ...
        try {
            ...
            msg_text = wxString::Format(_L("The selected printer (%s) is incompatible with the print file configuration (%s). Please "
                                           "adjust the printer preset in the prepare page or choose a compatible printer on this page."),
                                        sourcet_print_name, target_print_name);
...
bool SelectMachineDialog::is_nozzle_hrc_matched(const DevExtder* extruder, std::string& filament_type) const
{
    ...
    while (iter != m_materialList.end()) {
        ...
        if (abs(filament_nozzle_hrc) > abs(printer_nozzle_hrc)) {
            ...
            BOOST_LOG_TRIVIAL(info) << "filaments hardness mismatch: filament = " << filament_type
                                    << " printer_nozzle_hrc = " << printer_nozzle_hrc;
            return false;
        }
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent& event)
{
    ...
    if (main_external_spool_filas.size() > 1 || deputy_external_spool_filas.size() > 1) {
        if (!m_ext_change_assist) {
            confirm_text.push_back(ConfirmBeforeSendInfo(_L("More than one filament types have been mapped to the same external spool, "
        ...
        for (const auto& mapping_info : m_ams_mapping_result) {
            ...
            if (!mapped_fila_type.empty() && (mapped_fila_type != mapping_info.type)) {
                confirm_text.push_back(ConfirmBeforeSendInfo(
                    _L("The filament type setting of external spool is different from the filament in the slicing file.")));
                has_slice_warnings = true;
                break;
            }
        }
    ...
    if (!is_same_printer_type && (m_print_type == PrintFromType::FROM_NORMAL)) {
        confirm_text.push_back(
            ConfirmBeforeSendInfo(_L("The printer type selected when generating G-code is not consistent with the currently selected "
    ...
    for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
        ...
        wxString    info;
        wxString    wiki_url;
        DevFilaBlacklist::check_filaments_in_blacklist_url(obj_->printer_type, filament_brand, filament_type,
                                                           m_ams_mapping_result[i].filament_id, ams_id, slot_id, "", in_blacklist, action,
                                                           info, wiki_url);
    ...
    if (has_unknown_filament) {
        has_slice_warnings = true;
        confirm_text.push_back(
            ConfirmBeforeSendInfo(_L("There are some unknown filaments in the AMS mappings. Please check whether they are the required "
    ...
    if (has_slice_warnings) {
        ...
        if (confirm_text.size() > 1 || !is_printing_block) {
            ...
            if (is_printing_block) {
                shown_infos.emplace_back(ConfirmBeforeSendInfo(_L("Please fix the error above, otherwise printing cannot continue."),
                                                               wxEmptyString, ConfirmBeforeSendInfo::InfoLevel::Warning));
            } else {
                shown_infos.emplace_back(
                    ConfirmBeforeSendInfo(_L("Please click the confirm button if you still want to proceed with printing.")));
            }
...
void SelectMachineDialog::update_options_layout()
{
    ...
    for (auto item : children) {
        if (item && item->IsWindow()) {
            shown_options.insert(item->GetWindow());
        }
    }
...
void SelectMachineDialog::update_option_opts(MachineObject* obj)
{
    ...
    if (obj->is_support_bed_leveling == 2) {
        ...
        m_checkbox_list["bed_leveling"]
            ->update_options(ops_auto,
    ...
    if (support_cali_nozzle_offset) {
        m_checkbox_list["nozzle_offset_cali"]->update_options(ops_auto,
                                                              _L("Calibrate nozzle offsets to enhance print quality.\n*Automatic mode: "
                                                                 "Check for calibration before printing. Skip if unnecessary."));
    }
    ...
    if (obj->is_support_pa_calibration) {
        if (obj->GetConfig()->SupportCalibrationPA_FlowAuto() && can_support_pa_auto_cali()) {
            ...
            m_checkbox_list["flow_cali"]
                ->update_options(ops_no_auto, _L("This process determines the dynamic flow values to improve overall print quality."));
        }
    }
...
void SelectMachineDialog::load_option_vals(MachineObject* obj)
{
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " start"
                            << ", dev_id = " << (obj ? obj->get_dev_id() : "NULL") << ", sending mode= " << m_is_in_sending_mode;
...
void SelectMachineDialog::on_send_print()
{
    ...
    m_print_job->set_print_config(MachineBedTypeString[0], (m_checkbox_list["bed_leveling"]->getValue() == "on"),
                                  (m_checkbox_list["flow_cali"]->getValue() == "on"), false, timelapse_option, true, m_ext_change_assist,
                                  m_checkbox_list["bed_leveling"]->getValueInt(), m_checkbox_list["flow_cali"]->getValueInt(),
...
void SelectMachineDialog::clear_ip_address_config(wxCommandEvent& e) { prepare_mode(); }
    ...
    if (selection_data_arr.size() == 8) {
        ...
        for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
            if (m_ams_mapping_result[i].id == wxAtoi(selection_data_arr[5])) {
                ...
                if (m_ams_mapping_result[i].ams_id == std::to_string(VIRTUAL_TRAY_MAIN_ID) ||
                    m_ams_mapping_result[i].ams_id == std::to_string(VIRTUAL_TRAY_DEPUTY_ID)) {
                } else if (m_ams_mapping_result[i].ams_id >= std::to_string(0)) {
                }
            }
            BOOST_LOG_TRIVIAL(trace) << "The ams mapping result: id is " << m_ams_mapping_result[i].id << "tray_id is "
                                     << m_ams_mapping_result[i].tray_id;
        }
...
int SelectMachineDialog::update_print_required_data(Slic3r::DynamicPrintConfig config,
                                                    Slic3r::Model              model,
...
void SelectMachineDialog::reset_timeout() { m_timeout_count = 0; }

static bool _compare_obj_names(MachineObject* obj1, MachineObject* obj2) { return obj1->get_dev_name() < obj2->get_dev_name(); }
 ...
 */
/*******************************************************************/
static void _collect_sorted_machines(Slic3r::DeviceManager* dev_manager, std::vector<MachineObject*>& sorted_machine_objs)
{
    ...
    auto _collect_sorted_objs = [](const std::vector<MachineObject*>& obj_list, std::vector<MachineObject*>& sorted_machine_objs) {
...
{
    // [THREAD] This is a UI-thread polling timer that refreshes printer-dependent tips and the current mapping preview.
    ...
    if (!obj_->GetFilaSystem()->HasAms() || obj_->ams_exist_bits == 0 || !obj_->is_support_filament_backup ||
        !obj_->GetExtderSystem()->HasFilamentBackup() || !obj_->GetFilaSystem()->IsAutoRefillEnabled() || !_HasAms(m_ams_mapping_result)) {
...
{
    // [EVENT] Printer selection is a full state reset: clear warnings, re-query device state, then rebuild the status view.
...
void SelectMachineDialog::update_filament_change_count()
{
    ...
    if (hand_changes_count > 0 || saving_weight > 0) {
        ...
        m_txt_change_filament_times->SetLabel(
            wxString::Format(_L("Cost %dg filament and %d changes more than optimal grouping."), saving_weight, hand_changes_count));
...
static wxString _check_kval_not_default(const MachineObject* obj, const std::vector<FilamentInfo>& mapping_result)
{
    ...
    for (const auto& info : mapping_result) {
        if (!obj->contains_tray(info.ams_id, std::to_string(info.tray_id))) {
            continue;
        }
        if (obj->get_tray(info.ams_id, std::to_string(info.tray_id)).cali_idx == -1) {
            continue;
        } /*-1 means default*/
...
void SelectMachineDialog::update_show_status(MachineObject* obj_)
{
    ...
    /*mode check*/
    if (get_status() == PrintDialogStatus::PrintStatusRefreshingMachineList || get_status() == PrintDialogStatus::PrintStatusSending)
        return;
    ...
    if (is_blocking_printing(obj_)) {
        ...
        return;
    } else if (!obj_->GetConfig()->SupportPrintWithoutSD() &&
               (obj_->GetStorage()->get_sdcard_state() == DevStorage::SdcardState::NO_SDCARD)) {
    ...
    if (obj_->is_lan_mode_printer()) {
        if (obj_->GetStorage()->get_sdcard_state() == DevStorage::SdcardState::NO_SDCARD && !obj_->is_support_print_with_emmc) {
            ...
            return;
                   ...
                   (wxGetApp().app_config->get("allow_abnormal_storage") == "false")) {
            show_status(PrintDialogStatus::PrintStatusLanModeSDcardNotAvailable);
            return;
        }
    }
    ...
    if (nozzle_nums > 1 && m_print_type == FROM_NORMAL) {
        ...
        if (!is_nozzle_type_match(*obj_->GetExtderSystem(), error_message)) {
            ...
            params.emplace_back(_L("Tips: If you changed your nozzle of your printer lately, Please go to 'Device -> Printer parts' to "
                                   "change your nozzle setting."));
    ...
    if (m_print_type == PrintFromType::FROM_NORMAL) {
        ...
        if (!_is_same_nozzle_diameters(obj_, nozzle_diameter, mismatch_nozzle_id)) {
            ...
            msg_params.emplace_back(_L("Tips: If you changed your nozzle of your printer lately, Please go to 'Device -> Printer parts' to "
                                       "change your nozzle setting."));
        ...
        for (const auto& extder : obj_->GetExtderSystem()->GetExtruders()) {
            if (used_nozzle_idxes.count(extder.GetNozzleId()) == 0) {
                continue;
            }
            ...
            if (!is_nozzle_hrc_matched(&extder, filament_type)) {
                ...
                error_msg.emplace_back(
                    wxString::Format(_L("The hardness of current material (%s) exceeds the hardness of %s(%s). Please verify the nozzle or "
                                        "material settings and try again."),
                                     filament_type, _get_nozzle_name(obj_->GetExtderSystem()->GetTotalExtderCount(), extder.GetNozzleId()),
                                     format_steel_name(extder.GetNozzleType())));
    ...
    if (m_print_type == PrintFromType::FROM_NORMAL) {
                    ...
                    if (extder.HasFilamentInExt()) {
                        ...
                        if (filament_info) {
                            ...
                            if (43 - chamber_temp <= 5) {
                                unknown_fila_soften_extruders.insert(extder.GetExtId());
                            }
                        }
                    }
                }
        ...
        if (!high_temp_filaments.empty()) {
            ...
            if (obj_->GetConfig()->HasChamber()) {
                const auto& msg = wxString::Format(_L("[ %s ] requires printing in a high-temperature environment. Please close the door."),
                                                   filament_strs);
                ...
                if (PrePrintChecker::is_error(PrintDialogStatus::PrintStatusFilamentWarningHighChamberTemp)) {
                    return;
                }
            }
        }
        ...
        if (!known_fila_soften_extruders.empty()) {
            const wxString& msg = wxString::Format(_L("The filament on %s may soften. Please unload."),
                                                   _get_ext_loc_str(known_fila_soften_extruders,
                                                                    obj_->GetExtderSystem()->GetTotalExtderCount()));
            show_status(PrintDialogStatus::PrintStatusFilamentWarningHighChamberTempSoft, std::vector<wxString>{msg});
            if (PrePrintChecker::is_error(PrintDialogStatus::PrintStatusFilamentWarningHighChamberTempSoft)) {
                return;
            }
        }
        ...
        if (!unknown_fila_soften_extruders.empty()) {
            const wxString& msg = wxString::Format(_L("The filament on %s is unknown and may soften. Please set filament."),
                                                   _get_ext_loc_str(unknown_fila_soften_extruders,
                                                                    obj_->GetExtderSystem()->GetTotalExtderCount()));
            show_status(PrintDialogStatus::PrintStatusFilamentWarningUnknownHighChamberTempSoft, std::vector<wxString>{msg});
            if (PrePrintChecker::is_error(PrintDialogStatus::PrintStatusFilamentWarningUnknownHighChamberTempSoft)) {
                return;
            }
        }
    }
...
bool SelectMachineDialog::can_support_pa_auto_cali()
{
    ...
    if (!unsupport_auto_cali_filaments.empty()) {
        auto iter = std::find_if(m_filaments.begin(), m_filaments.end(), [&unsupport_auto_cali_filaments](const FilamentInfo& item) {
...
void SelectMachineDialog::Enable_Refresh_Button(bool en) { m_printer_box->EnableRefreshButton(en); }
...
{
    // [STATE] Recompute the send target defaults from the current plate/project, then rebuild the preview and printer list.
    if (m_print_type == PrintFromType::FROM_NORMAL) {
...
{
    // [UNITY] Tooltip text is the mapping affordance here; in Unity this should become an explicit help overlay or inspector hint.
    MaterialHash::iterator iter = m_materialList.begin();
    while (iter != m_materialList.end()) {
        int       id   = iter->first;
        Material* item = iter->second;
...
void SelectMachineDialog::reset_and_sync_ams_list()
{
    ...
    for (auto i = 0; i < extruders.size(); i++) {
        ...
        item->Bind(wxEVT_LEFT_DOWN, [this, item, materials, extruder](wxMouseEvent& e) {
            ...
            if (!m_check_flag || m_print_status == PrintDialogStatus::PrintStatusUnsupportedPrinter) {
                return;
            } /*STUDIO-11301*/
        ...
        if (extruder < materials.size() && extruder >= 0) {
            ...
            info.color = wxString::Format("#%02X%02X%02X%02X", colour_rgb.Red(), colour_rgb.Green(), colour_rgb.Blue(), colour_rgb.Alpha())
                             .ToStdString();
...
{
    // [OPENGL] No GL here, but this is the thumbnail compositing bridge: copy pixels, cache colors, and prepare overlays.
...
{
    // [STATE] Capture pixels that belong to the original thumbnail so later recoloring can preserve the edge outline.
...
wxColour SelectMachineDialog::adjust_color_for_render(const wxColour& color)
{
    ...
    wxColour  render_color((int) (_temp_color_color_[0] * 255.0f), (int) (_temp_color_color_[1] * 255.0f),
                           (int) (_temp_color_color_[2] * 255.0f), (int) (_temp_color_color_[3] * 255.0f));
    return render_color;
}

void SelectMachineDialog::final_deal_edge_pixels_data(ThumbnailData& data)
{
    // [INTENT] Smooth edge pixels after recoloring so the AMS tint preserves a readable thumbnail silhouette.
    if (data.width > 0 && data.height > 0 && m_edge_pixels.size() > 0) {
        for (unsigned int r = 0; r < data.height; ++r) {
            ...
            for (unsigned int c = 0; c < data.width; ++c) {
                ...
                if (exist_rr_up && exist_c_left) {
                    relational_pxs[0] = (unsigned char*) data.pixels.data() + 4 * (rr_up + c_left);
                }
                if (exist_rr_up) {
                    relational_pxs[1] = (unsigned char*) data.pixels.data() + 4 * (rr_up + c);
                }
                if (exist_rr_up && exist_c_right) {
                    relational_pxs[2] = (unsigned char*) data.pixels.data() + 4 * (rr_up + c_right);
                }
                if (exist_c_left) {
                    relational_pxs[3] = (unsigned char*) data.pixels.data() + 4 * (rr + c_left);
                }
                if (exist_c_right) {
                    relational_pxs[4] = (unsigned char*) data.pixels.data() + 4 * (rr + c_right);
                }
                if (exist_rr_down && exist_c_left) {
                    relational_pxs[5] = (unsigned char*) data.pixels.data() + 4 * (rr_down + c_left);
                }
                if (exist_rr_down) {
                    relational_pxs[6] = (unsigned char*) data.pixels.data() + 4 * (rr_down + c);
                }
                if (exist_rr_down && exist_c_right) {
                    relational_pxs[7] = (unsigned char*) data.pixels.data() + 4 * (rr_down + c_right);
                }
...
{
    // [UNITY] In Unity this would be a texture-processing step feeding a preview RawImage; keep the original and recolored variants separate.
    if (input_data.width == 0 || input_data.height == 0 || no_light_data.width == 0 || no_light_data.height == 0) {
...
{
    // [STATE] Repaint the preview thumbnail in-place when a filament color changes or an AMS mapping is edited.
    if (m_cur_colors_in_thumbnail.size() == 0) {
...
void SelectMachineDialog::update_page_turn_state(bool show)
{
    ...
    if (show) {
        if (m_print_plate_idx <= 0) {
            m_bitmap_last_plate->Disable();
        } else {
            m_bitmap_last_plate->Enable();
        }

        if ((m_print_plate_idx + 1) >= m_print_plate_total) {
            m_bitmap_next_plate->Disable();
        } else {
            m_bitmap_next_plate->Enable();
        }
...
{
    // [EVENT] Showing the dialog kicks off timer-driven refresh, selects a default machine, and rebuilds all dependent UI state.
    if (show) {
        EnableEditing(true);
...
void SelectMachineDialog::show_init() { m_ams_mapping_result.clear(); }

SelectMachineDialog::~SelectMachineDialog() { delete m_refresh_timer; }
    ...
    : wxPanel(parent, winid, pos, size)
{
    ...
    // [OPENGL] Custom-painted thumbnail preview panel; Unity should use a textured UI element with a cached composite texture.
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
    ...
    // [INTENT] Compound control for a segmented on/off/auto setting with custom painting and a hover tooltip affordance.
    m_ops        = ops;
    m_param      = param;
    m_full_title = title;
    ...
    m_printoption_tips = new ScalableButton(this, wxID_ANY, "icon_qusetion", wxEmptyString, wxDefaultSize, wxDefaultPosition,
...
void PrintOption::setValue(std::string value) { m_printoption_item->setValue(value); }
...
std::string PrintOption::getValue() { return m_printoption_item->getValue(); }
...
{
    // [EVENT] Translate the click position into a segmented option choice and forward it as a custom dialog event.
    if (!m_enable) {
        return;
    }
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it) {
        ...
        if (text_key == selected_key) {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#009688") :
                                             StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
            ...
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#262E30") :
                                             StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
            dc.SetTextForeground(clr);
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
    ...
    // [UNITY] This is a hand-painted two-state toggle; Unity equivalent is a segmented button or custom UI Toolkit control.
...
void SendModeSwitchButton::on_left_down(wxMouseEvent& evt) { setSelected(!is_selected); }
...
{
    // [INTENT] Build the printer selector header: combo box, refresh button, and bed-type badge live as one reusable subpanel.
    /*printer combobox*/
    ...
    m_button_question = new ScalableButton(this, wxID_ANY, "icon_qusetion", wxEmptyString, wxDefaultSize, wxDefaultPosition,
                                           wxBU_EXACTFIT | wxNO_BORDER, true);
