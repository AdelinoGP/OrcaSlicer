// [ANNOTATED]
// [INTENT] Management of remote file downloads and deep-link URL scheme handling (orcaslicer://).
// [STATE] Tracks active download sessions, their progress, and destination paths.
// [EVENT] Propagates download lifecycle events (progress, completion, error) to the NotificationManager.
// [UNITY] Replace with a C# DownloadService utilizing UnityWebRequest for managed downloads.
// [UNITY] Use Unity's native Deep Linking support for handling orcaslicer:// URL schemes.
// [PORTING_HAZARD:P2] Regex-based protocol parsing for various slicer schemes must be mirrored to maintain cross-app compatibility.
...
Download::Download(int ID, std::string url, wxEvtHandler* evt_handler, const boost::filesystem::path& dest_folder)
    : m_id(ID), m_filename(filename_from_url(url)), m_dest_folder(dest_folder)
{
