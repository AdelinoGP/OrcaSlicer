...
// [INTENT] Compressor batches DXT mipmap levels off the UI thread so texture uploads do not stall frame submission.
// [STATE] m_levels/m_num_levels_compressed/m_abort_compressing track the pending mipmaps and worker lifecycle.
// [THREAD] reset is only invoked on the main (GL) thread after the compressor thread joined, so GPU resources stay safe.
...
// [THREAD] The worker thread can spin up dozens of DXT levels while the UI stays responsive to user input.
// [EVENT] start_compressing only kicks off the thread once mipmaps are staged; the main thread polls via idle events.
...
// [STATE] Polling this helper mirrors the atomic m_num_levels_compressed so UI can push ready mip levels safely.
// [THREAD] Called from the main thread before glTex updates, synchronizing caches with the worker via atomics.
bool GLTexture::Compressor::unsent_compressed_data_available() const
{
    if (m_levels.empty())
        return false;
    // Querying the atomic m_num_levels_compressed value synchronizes processor caches, so that the data of m_levels modified by the worker
    // thread are accessible to the calling thread.
...
// [OPENGL] GPU uploads must happen on the main GL thread; bindings/texsubimage calls here drive the mipmap series.
// [THREAD] Compressor::compress never touches GL, so the main thread marshals the Level buffers with atomics before calling
// send_compressed_data_to_gpu. [UNITY] Model this as a Unity Job producing Texture2D raw data followed by Graphics.UploadTexture on the
// main thread. [PORTING_HAZARD:P1] Unity needs to marshal compressed data back to the render thread via Graphics.UploadTexture or
// AsyncGPUReadback callbacks.
void GLTexture::Compressor::send_compressed_data_to_gpu()
{
    ...
    // Querying the atomic m_num_levels_compressed value synchronizes processor caches, so that the dat of m_levels modified by the worker
    // thread are accessible to the calling thread.
    ...
    for (int i = 0; i < num_compressed; ++i) {
        ...
        if (!level.sent_to_gpu && !level.compressed_data.empty()) {
            glsafe(::glCompressedTexSubImage2D(GL_TEXTURE_2D, (GLint) i, 0, 0, (GLsizei) level.w, (GLsizei) level.h,
                                               GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) level.compressed_data.size(),
                                               (const GLvoid*) level.compressed_data.data()));
...
// [THREAD] Compressor runs on a dedicated std::thread, filling buffers before the main thread grabs them via atomics.
// [EVENT] After each compression batch we wake the idle loop so the scene redraws with the new texture data.
// [UNITY] Unity should map this to a Job System worker that fills byte buffers and dispatches a main-thread Upload.
// [PORTING_HAZARD:P2] Unity must port this to a Job + Main-Thread callback pattern because multithreaded GL-context access is forbidden.
void GLTexture::Compressor::compress()
{
    ...
    for (Level& level : m_levels) {
        ...
        // crashes if doing so, requiring a minimum of 64 bytes and up to a third of the source buffer size, so we set the destination
        // buffer initial size to be half the source buffer size
...
// [INTENT] GLTexture centralizes texture loading/rendering for GUI icons, bed sprites, and label caches.
// [STATE] m_compressor allows asynchronous compression, while m_width/m_height encode GPU allocation metadata.
GLTexture::GLTexture() : m_compressor(*this) {}

// [THREAD] Destruction runs on the GL thread and frees GPU resources before the compressor thread can race with reset().
...
bool GLTexture::load_from_svg_file(
    const std::string& filename, bool use_mipmaps, bool compress, bool apply_anisotropy, unsigned int max_size_px)
{
    reset();
...
// [INTENT] Upload arbitrary pixel buffers when icon atlases or text bitmaps cannot hit a file on disk.
// [STATE] m_width/m_height snapshot the resource, while apply_anisotropy toggles extra driver knobs.
// [OPENGL] Manual glTexImage2D calls + custom mipmap loop keep control over GPU format and filtering.
// [UNITY] Replace this with Texture2D.LoadRawTextureData + Graphics.ApplyTextureSettings before calling Graphics.UploadTexture.
// [PORTING_HAZARD:P3] Unity lacks direct GL state control, so the manual mipmap & anisotropy dance must become TextureImporter or runtime
// parameter tweaks.
bool GLTexture::load_from_raw_data(std::vector<unsigned char> data, unsigned int w, unsigned int h, bool apply_anisotropy)
{
    ...
    glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) m_width, (GLsizei) m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    ...
    if (use_mipmaps) {
        ...
        while (lod_w > 1 || lod_h > 1) {
            ...
            glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA, (GLsizei) lod_w, (GLsizei) lod_h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                  (const void*) data.data()));
        }
...
// [INTENT] Builds a sprite atlas of SVG glyphs for gizmos and UI buttons (hover/pressed/disabled variants).
// [STATE] Cache dimensions rely on sprite_size_px, counts of filenames/states, and dark_mode toggles for palette selection.
// [OPENGL] Single atlas texture upload (possibly compressed) centralizes GPU memory for button glyphs.
// [UNITY] Use the Vector Graphics package to rasterize to Texture2DArray and drive UI Toolkit/SpriteAtlas slicing backed by Texture2D
// arrays. [PORTING_HAZARD:P2] The hand-rolled 1px border padding and manual copy loops must be reimplemented with SpriteAtlas padding rules
// to avoid bleed.
                                                     ...
                                                     bool                                     compress)
{
    ...
    for (const std::string& filename : filenames) {
        ...
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (pressed_data.data()[offset + 0] == 0 && pressed_data.data()[offset + 1] == 0 && pressed_data.data()[offset + 2] == 0) {
        ...
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (hover_data.data()[offset + 0] == 0 && hover_data.data()[offset + 1] == 0 && hover_data.data()[offset + 2] == 0) {
        ...
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_white_only_data.data()[offset + 0] != 0 || sprite_white_only_data.data()[offset + 1] != 0 ||
        ...
        for (int i = 0; i < sprite_n_pixels; ++i) {
            int offset = i * 4;
            if (sprite_gray_only_data.data()[offset + 0] != 0 || sprite_gray_only_data.data()[offset + 1] != 0 ||
        ...
        for (const std::pair<int, bool>& state : states) {
            ...
            for (int j = 0; j < (int) sprite_size_px_ex; ++j) {
                ::memcpy((void*) &data.data()[(state_offset_px + j * m_width) * 4], (const void*) &output_data.data()[j * sprite_stride],
                         sprite_stride);
            }
        }
    ...
    if (compress && OpenGLManager::are_compressed_textures_supported())
        glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width, (GLsizei) m_height, 0, GL_RGBA,
                              GL_UNSIGNED_BYTE, (const void*) data.data()));
    else
        glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) m_width, (GLsizei) m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                              (const void*) data.data()));
    glsafe(::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
...
// [STATE] reset clears the GPU id, dimensions, source path, and forces the compressor to forget pending mipmaps.
// [UNITY] Equivalent Unity flow would call `Destroy(texture)` before replacing `Texture2D` data to avoid leaking native memory.
...
// [INTENT] Infer text bounds via wxFont metrics and bake glyphs into a Texture2D-friendly bitmap.
// [STATE] Captures measured dimensions in m_original_* while the GPU texture rounds up to powers of two for alignment.
// [THREAD] Must run on the UI thread since wxMemoryDC is not thread-safe, so it never executes on compressor threads.
// [UNITY] Unity can capture trusted font metrics via TextMeshPro and RenderTexture, then read back into a Texture2D cached in a
// ScriptableObject. [PORTING_HAZARD:P2] Porting requires re-implementing wx string measurement and multi-line layout logic inside Unity's
// text engine.
...
// [INTENT] Text measurement and glyph rasterization for status labels that need exact bitmaps (like measurements or overlays).
// [STATE] Outputs width/height/line metrics through reference params so calling widgets know the layout.
// [OPENGL] Same upload path as generate_from_text, but also cross-checks for vertical clipping before sending data to GPU.
// [UNITY] Equivalent to using TextMeshProUGUI + RenderTexture, then caching the result into a ScriptableObject for reuse.
// [PORTING_HAZARD:P3] relies on wx DC measurement/multi-line extent; Unity must replicate via TextMeshPro's `textBounds` and `preferredValues`.
bool GLTexture::generate_texture_from_text(
    const std::string& text_str, wxFont& font, int& ww, int& hh, int& hl, wxColor background, wxColor foreground)
{
    if (text_str.empty()) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ":no text string, should not happen\n";
        return false;
...
// [INTENT] Render a quad that overlays the 3D scene with the provided texture region.
// [OPENGL] Enables blending, texture units, and constructs a GLModel-backed triangle mesh to avoid legacy immediate-mode pitfalls.
// [UNITY] This equates to a MeshRenderer/Material pair or Graphics.DrawMesh with a `Texture2D` uniform and alpha blending via URP/HDRP.
// [PORTING_HAZARD:P3] Unity has no direct equivalent to GLTexture::render_sub_texture; you must rebuild the quad mesh + shader state on
// each draw call.
...
// [INTENT] Load PNG assets (bed textures, icons) with optional mipmaps/compression and power-of-two enforcement.
// [STATE] Rescale flags, compression paths, and requires_rescale track whether we changed the incoming image.
// [OPENGL] Manual glTexImage2D calls plus optional compressor queue ensure consistent GPU formats even on weak GPUs.
// [UNITY] Unity would use `Texture2D.LoadImage`, set mipmap/compression via `TextureImporter`, and push to GPU with `Texture2D.Apply`.
// [PORTING_HAZARD:P2] Unity's importer caches textures; replicating dynamic resampling + Compression simultaneously requires careful
// ScriptableObject control.
bool GLTexture::load_from_png(const std::string& filename, bool use_mipmaps, ECompressionType compression_type, bool apply_anisotropy)
{
    ...
    if (use_mipmaps && compression_enabled && OpenGLManager::force_power_of_two_textures()) {
        if (to_squared_power_of_two(boost::filesystem::path(filename).filename().string(), OpenGLManager::get_gl_info().get_max_tex_size(),
                                    m_width, m_height))
    ...
    if (use_mipmaps) {
        ...
        while (lod_w > 1 || lod_h > 1) {
            ...
            if (compression_enabled) {
                if (compression_type == SingleThreaded)
                    glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width, (GLsizei) m_height, 0,
                                          GL_RGBA, GL_UNSIGNED_BYTE, (const void*) data.data()));
                else {
                    // initializes the texture on GPU
                    glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) lod_w, (GLsizei) lod_h, 0,
...
// [INTENT] Rasterize SVG assets via nanoSVG, scaling to max_size_px before pushing to the GPU atlas.
// [STATE] scale_x/scale_y track the requested output ratio while compression/aniso flags determine GL params.
// [OPENGL] Writes RGBA data + optional compressor queue; manual mipmap generation avoids relying on unreliable glGenerateMipmap.
// [UNITY] Porters should leverage Unity Vector Graphics + `SpriteAtlas` + `Texture2DCompressionSettings` to approximate this workflow.
// [PORTING_HAZARD:P2] Unity can't reuse wx image resampling; use `VectorUtils.TessellateScene` + custom RenderTexture capture instead.
bool GLTexture::load_from_svg(const std::string& filename, bool use_mipmaps, bool compress, bool apply_anisotropy, unsigned int max_size_px)
{
    ...
    if (use_mipmaps) {
        ...
        while (lod_w >= 4 && lod_h >= 4) {
            ...
            if (compression_enabled) {
                // initializes the texture on GPU
                glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) lod_w, (GLsizei) lod_h, 0, GL_RGBA,
                                      GL_UNSIGNED_BYTE, 0));
                // and send the uncompressed data to the compressor
                m_compressor.add_level((unsigned int) lod_w, (unsigned int) lod_h, data);
            } else
