// [ANNOTATED]
// [INTENT] Interactive panel for managing and reordering filament color blocks via drag-and-drop.
// [STATE] Custom data object (ColorDataObject) preserves filament ID, material type, and color during drag sessions.
// [EVENT] Hooks into wxWidgets' drag-and-drop protocol (wxDropSource, wxDropTarget) to manage block movement and deletion.
// [UNITY] Replace with a reorderable GridView in UI Toolkit or custom IDragHandler chips in a GridLayoutGroup.
// [PORTING_HAZARD:P3] Hex color parsing and SVG-based block rendering should map to ColorUtility and standard Unity Sprites.
...
public:
    ...
    void set_custom_data_filament_id(int filament_id) { m_data.filament_id = filament_id; }
...
ColorPanel::ColorPanel(DragDropPanel* parent, const wxColour& color, int filament_id, const std::string& type)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(32, 40), wxBORDER_NONE)
    , m_parent(parent)
    , m_color(color)
...
{
public:
    ColorDropSource(wxPanel* parent, wxPanel* color_block, const wxColour& color, const std::string& type, int filament_id)
        : wxDropSource(parent)
    {
...
DragDropPanel::DragDropPanel(wxWindow* parent, const wxString& label, bool is_auto)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE), m_is_auto(is_auto)
{
