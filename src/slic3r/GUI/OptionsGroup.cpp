...
// [INTENT] OptionsGroup: UI component that renders a group of configuration options (fields) within a labeled box.
// [STATE] Maintains a map of fields (m_fields), option definitions (m_options), and layout parameters.
// [EVENT] Field creation and updates triggered by config changes; user interaction updates config via field callbacks.
// [UNITY] Replace with a UI Toolkit VisualElement containing a ListView of custom field VisualElements, bound to a shared config model.
// [PORTING_HAZARD:P2] The dynamic field creation based on ConfigOptionDef::GUIType requires a flexible UI Toolkit field factory.
...
// [INTENT] Build a field for a given Option, delegating to the overload with ConfigOptionDef.
// [EVENT] Called when fields are created or refreshed.
// [UNITY] Map to a factory method that creates appropriate UI Toolkit field VisualElements.
...
const t_field& OptionsGroup::build_field(const t_config_option_key& id, const ConfigOptionDef& opt)
{
    ...
    default:
        ...
        case coPercents:
        case coString:
        case coStrings: m_fields.emplace(id, TextCtrl::Create<TextCtrl>(this->ctrl_parent(), opt, id)); break;
        case coBool:
        case coBools: m_fields.emplace(id, CheckBox::Create<CheckBox>(this->ctrl_parent(), opt, id)); break;
        case coInt:
        case coInts: m_fields.emplace(id, SpinCtrl::Create<SpinCtrl>(this->ctrl_parent(), opt, id)); break;
        case coEnum:
        case coEnums: m_fields.emplace(id, Choice::Create<Choice>(this->ctrl_parent(), opt, id)); break;
        case coPoint:
        case coPoints: m_fields.emplace(id, PointCtrl::Create<PointCtrl>(this->ctrl_parent(), opt, id)); break;
        case coNone: break;
        default: throw Slic3r::LogicError("This control doesn't exist till now"); break;
        }
    }
...
// [INTENT] Construct an OptionsGroup with parent window, title, icon, and layout options.
// [STATE] Initializes parent, title, icon, custom control flag, static box flag, extra column.
// [EVENT] Constructor only; field creation occurs later via build_field.
