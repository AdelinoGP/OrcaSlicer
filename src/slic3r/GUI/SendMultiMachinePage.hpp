...
// [INTENT] SendDeviceItem is the custom-painted row controller for one discovered
// machine. It owns hover/selection feedback and forwards clicks back into the
// dialog instead of relying on standard list-row widgets.
// [OPENGL] none here; the row is immediate-mode wxDC painting with manual text
// clipping and scalable bitmaps.
// [UNITY] Map this to a recycled ListView item prefab with data binding and a
// dedicated selection/highlight state controller.
// [PORTING_HAZARD:P2] The row paints itself and synthesizes mouse gestures, so
// Unity needs explicit hit targets and row lifetime management.
...
class Plater;
// [INTENT] SendMultiMachinePage is the modal send-to-multi-printer workflow.
// It owns the machine roster, AMS mapping UI, rename mode, thumbnail/summary
// header, and send/export actions for the current plate.
// [STATE] The dialog caches discovered devices, the active plate index,
// configuration pointers, AMS/material mapping, checkbox/input widgets, and
// rename/title state that must survive refresh ticks.
// [EVENT] Timer ticks, device row clicks, radio selection, rename actions, and
// send-button handlers all mutate the page state and refresh the machine list.
// [THREAD] Refresh and device discovery are timer/worker driven in the runtime
// implementation; the header boundary needs main-thread marshaling in Unity.
// [UNITY] Split this into a modal controller plus data-driven subviews: a
// service-backed device roster, an AMS mapping panel, and a thumbnail/summary
// header built from reusable UI Toolkit or uGUI components.
// [PORTING_HAZARD:P1] The page mixes persistence, custom row widgets, timer
// refresh, and modal send/export side effects, so Unity should not treat it as
// a single monolithic window.
