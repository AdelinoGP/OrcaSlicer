...
// [STATE] Global alpha/color tweeter thresholds that keep clear volume masks visible and prevent OpenGL rounding from changing border
// colors. [PORTING_HAZARD:P2] Unity normalizes colors differently (linear sRGB) so these hardcoded floats must be re-validated before
// changing rendering precision. [UNITY] Mirror these thresholds in a Shared ScriptableObject palette so both viewport and thumbnail
// renderers agree on transparency.
...
// [INTENT] This TU implements GLVolume / GLVolumeCollection as the core render primitives for the 3D viewport and auxiliary layers.
// [STATE] Each GLVolume tracks selection, hover, modifier, printable/visible flags and relies on GUI::wxGetApp() for the current
// shader/camera. [EVENT] Rendering flows from the wxWidgets paint event -> GUI::wxGetApp().render_scene() so these helpers are invoked per
// frame. [THREAD] All entry points must run on the main UI thread to keep OpenGL context ownership intact; worker threads must marshal
// updates back before calling render(). [OPENGL] The code flips culling, blend state, framebuffers, and shader uniforms per-pass before
// drawing meshes or outlines. [UNITY] Port to Unity by mapping GLVolume to a GameObject with MeshRenderer+Material, populating Mesh with
// GLModel, and scheduling draws via Graphics.DrawMesh with a custom MonoBehaviour. [PORTING_HAZARD:P2] Tight coupling to GUI::wxGetApp(),
// shader lookup, and platform-specific GLFramebuffers means the Unity renderer must re-architect the render loop and state ownership.
const float GLVolume::SinkingContours::HalfWidth = 0.25f;
void        GLVolume::SinkingContours::render()
{
    // [EVENT][OPENGL] Draws the sinking outline whenever the viewport render loop hits a transform that went below the drip line.
    // [UNITY] Mirror by scheduling Graphics.DrawMesh with the outline shader after filling a depth texture from the RenderTexture camera.
    update();
...
// [STATE] Rebuilds the outline mesh lazily when the bounding box drifts or the sinking flag changes. Heavy mesh work stays on the UI thread.
...
{
    // [EVENT][OPENGL] Called during the viewport draw pass to push mesh data through the currently bound shader/shader uniforms.
    // [UNITY] Replace with Graphics.DrawMesh using the cached Mesh + Material per volume; updates occur on Unity's main thread in LateUpdate.
    if (!is_active)
        return;
...
// [INTENT] Drives the opaque/transparent passes, environment map use, and dynamic outline shaders for every frame that draws the model
// volumes. [OPENGL] Manipulates cull-face, blend state, shader uniforms (z_range, clipping, slope) and optionally renders sinking contours
// with a secondary shader. [UNITY] Replace by sorting meshes into separate passes and calling Graphics.DrawMesh from a `SceneRenderer`
// MonoBehaviour; replicate sinking outline pass via a CommandBuffer. [EVENT] Triggered by the viewport paint event whenever the workbench
// requests a redraw. [PORTING_HAZARD:P2] Current logic depends on GUI::wxGetApp() shader automations and assumes framebuffers are
// available, so Unity needs to recreate equivalent render sets around RenderPipeline callbacks.
...
#else
    GLShaderProgram* edges_shader = GUI::OpenGLManager::get_gl_info().is_core_profile() ? GUI::wxGetApp().get_shader("dashed_thick_lines") :
...
bool GLVolumeCollection::check_outside_state(const BuildVolume&              build_volume,
                                             ModelInstanceEPrintVolumeState* out_state,
                                             ObjectFilamentResults*          object_results) const
{
        ...
        if (object_results->mode < FilamentMapMode::fmmManual) {
            ...
            for (int index = 0; index < extruder_count; index++) {
                if (!unprintable_filament_vec[index].empty()) {
                    ...
                    if (index == 0)
                        ...
                        std::set_intersection(conflict_filament_vector.begin(), conflict_filament_vector.end(),
                                              unprintable_filament_vec[index].begin(), unprintable_filament_vec[index].end(),
                                              insert_iterator<vector<int>>(result_filaments, result_filaments.begin()));
            ...
            if (!conflict_filament_vector.empty()) {
                ...
                for (auto& object_map : objects_unprintable_filaments) {
                    ...
                    for (std::map<int, std::set<int>>::iterator extruder_map_iter = obj_extruder_filament_maps.begin();
                         extruder_map_iter != obj_extruder_filament_maps.end(); extruder_map_iter++) {
    ...
    /*for (GLVolume* volume : this->volumes)
    {
        if (! volume->is_modifier && (volume->shader_outside_printer_detection_enabled || (! volume->is_wipe_tower &&
...
// [INTENT] Builds thick-walled geometry for extrusion paths so the GL model can render solid toolpaths without tesselating on the fly.
// [OPENGL] Emits triangle strips/quads and hard normals for sharp turns to keep the shader-driven lighting consistent with the viewport.
// [UNITY] Porters should mirror this by procedurally slicing polylines into Mesh vertices + triangles inside the extrusion preview controller.
...
// [INTENT] Converts extrusion_path geometry into the qverts/tverts used by the GL preview so each stroke has a solid mesh.
// [EVENT] Called during preview rebuilds after slicing or when the data model signals a path change.
// [UNITY] Map this logic to a MeshBuilder component that feeds a MeshRenderer and rebuilds on demand.
