...
// [INTENT] PhysicalPrinterDialog manages printer-specific network host connectivity and printer agent configuration.
// [STATE] Manages printer host configuration (print host URL, API key, network agent type, etc.).
// [EVENT] Binds buttons for host testing, browsing, and saving printer settings.
// [UNITY] Use a UI Toolkit-based dialog. Replace ConfigOptionsGroup with a UI Toolkit-based parameter inspector.
// [PORTING_HAZARD:P2] Requires bridging network agent communication and asynchronous API testing to Unity C# network/task systems.
PhysicalPrinterDialog::PhysicalPrinterDialog(wxWindow* parent)
    : DPIDialog(parent,
                wxID_ANY,
                _L("Physical Printer"),
                wxDefaultPosition,
                wxSize(45 * wxGetApp().em_unit(), -1),
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    m_input_ctrl = new wxTextCtrl(m_input_area, -1, from_u8(preset_name), wxDefaultPosition,
                                  wxSize(wxSize(FromDIP(360), FromDIP(32)).x, -1), 0 | wxBORDER_NONE);
        ...
        if (opt_key == "printer_agent") {
            try {
                ...
                if (selected_idx >= 0 && selected_idx < static_cast<int>(agents.size())) {
                    m_config->set_key_value("printer_agent", new ConfigOptionString(agents[selected_idx].id));
                }
    ...
    if (Http::ca_file_supported()) {
        ...
        line.widget = [ca_file_hint](wxWindow* parent) {
            std::string info = _u8L("HTTPS CA File") + ":\n\t" +
                               (boost::format(
                                    _u8L("On this system, %s uses HTTPS certificates from the system Certificate Store or Keychain.")) %
                                SLIC3R_APP_NAME)
                                   .str() +
    ...
    if (tech == ptFFF) {
        ...
        if (Field* printhost_field = m_optgroup->get_field("print_host"); printhost_field) {
            if (wxTextCtrl* temp = dynamic_cast<TextCtrl*>(printhost_field)->text_ctrl(); temp) {
                ...
                if (current_host == L"https://connect.prusa3d.com" || current_host == L"https://app.obico.io" ||
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
    ...
    int index_in_choice = (printer_change ? std::clamp(last_in_conf - ((int) ht->m_opt.enum_values.size() - (int) types.size()), 0,
                                                       (int) ht->m_opt.enum_values.size() - 1) :
                                            last_in_conf);
    ...
    if ("prusalink" == ht->m_opt.enum_values.at(index_in_choice))
        ...
        int           host_type = std::clamp(index_in_choice + ((int) ht->m_opt.enum_values.size() - (int) types.size()), 0,
                                             (int) ht->m_opt.enum_values.size() - 1);
...
{
    std::vector<std::string> keys = {"print_host",     "print_host_webui",   "printhost_apikey", "printhost_cafile",
...
void PhysicalPrinterDialog::OnOK(wxEvent& event)
{
    ...
    wxGetApp().CallAfter([] { wxGetApp().switch_printer_agent(); });
}
