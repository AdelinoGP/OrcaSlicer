...
PartPlate::PartPlate() : ObjectBase(-1), m_plater(nullptr), m_model(nullptr), m_quadric(nullptr)
{
    assert(this->id().invalid());
    init();
}

PartPlate::PartPlate(PartPlateList*    partplate_list,
                     Vec3d             origin,
                     int               width,
                     int               depth,
                     int               height,
    ...
    : m_partplate_list(partplate_list)
    , m_plater(platerObj)
    , m_model(modelObj)
    , printer_technology(tech)
    , m_origin(origin)
    , m_width(width)
    , m_depth(depth)
    , m_height(height)
...
void PartPlate::reset_bed_type() { m_config.erase("curr_bed_type"); }

void PartPlate::reset_skirt_start_angle() { m_config.erase("skirt_start_angle"); }
...
void PartPlate::calc_vertex_for_icons_background(int icon_count, GLModel &buffer)
{
    ...
    poly.contour.append({ scale_(p(0) + PARTPLATE_ICON_GAP_LEFT), scale_(p(1) - icon_count * (PARTPLATE_ICON_SIZE + PARTPLATE_ICON_GAP_Y) -
PARTPLATE_ICON_GAP_TOP) }); poly.contour.append({ scale_(p(0) + PARTPLATE_ICON_GAP_LEFT + PARTPLATE_ICON_SIZE), scale_(p(1) - icon_count *
(PARTPLATE_ICON_SIZE + PARTPLATE_ICON_GAP_Y)- PARTPLATE_ICON_GAP_TOP) }); poly.contour.append({ scale_(p(0) + PARTPLATE_ICON_GAP_LEFT +
PARTPLATE_ICON_SIZE), scale_(p(1) - PARTPLATE_ICON_GAP_TOP)}); poly.contour.append({ scale_(p(0) + PARTPLATE_ICON_GAP_LEFT), scale_(p(1) -
PARTPLATE_ICON_GAP_TOP) });
...
// [INTENT] Renders the plate background (unselected or selected)
// [OPENGL] glDepthMask used for transparency handling.
// [UNITY] Use transparent material/shader for plate rendering.
void PartPlate::render_background(bool force_default_color)
{
    // return directly for current plate
    if (m_selected && !force_default_color)
        return;
...
void PartPlate::render_logo(bool bottom, bool render_cali)
{
    if (!m_partplate_list->render_bedtype_logo) {
        ...
        if (m_partplate_list->m_logo_texture.get_id() == 0 ||
            m_partplate_list->m_logo_texture.get_source() != m_partplate_list->m_logo_texture_filename) {
            m_partplate_list->m_logo_texture.reset();
            ...
            if (boost::algorithm::iends_with(m_partplate_list->m_logo_texture_filename, ".svg")) {
                ...
                if (!m_partplate_list->m_logo_texture.load_from_svg_file(m_partplate_list->m_logo_texture_filename, true, true, true,
                                                                         logo_tex_size)) {
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                               << boost::format(": load logo texture from %1% failed!") %
                ...
                // starts generating the main texture, compression will run asynchronously
                if (!m_partplate_list->m_logo_texture.load_from_file(m_partplate_list->m_logo_texture_filename, true,
                                                                     GLTexture::MultiThreaded, true)) {
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                               << boost::format(": load logo texture from %1% failed!") %
                                                      m_partplate_list->m_logo_texture_filename;
                    return;
                ...
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(": can not load logo texture from %1%, unsupported format") %
                                                  m_partplate_list->m_logo_texture_filename;
                return;
            }
    ...
    if (!is_single_extruder) {
        if (m_partplate_list->m_allow_bed_type_in_double_nozzle.find(bed_type_idx) ==
            m_partplate_list->m_allow_bed_type_in_double_nozzle.end()) {
            bed_type_idx = 0;
        }
    }
    // render bed textures
        ...
        if (!only_name) {
            ...
            if (m_partplate_list->render_plate_settings) {
                bool has_plate_settings = get_bed_type() != BedType::btDefault || get_print_seq() != PrintSequence::ByDefault ||
                                          !get_first_layer_print_sequence().empty() || !get_other_layers_print_sequence().empty() ||
...
void PartPlate::render_label(GLCanvas3D& canvas) const {
    ...
    imgui.begin(label, ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration |
ImGuiWindowFlags_NoMove); ImGui::BringWindowToDisplayFront(ImGui::GetCurrentWindow()); float win_w = ImGui::GetWindowWidth(); float
label_len = imgui.calc_text_size(label).x; ImGui::SetCursorPosX(0.5f * (win_w - label_len)); ImGui::AlignTextToFramePadding();
    imgui.text(label);

    // force re-render while the windows gets to its final size (it takes several frames)
    if (ImGui::GetWindowContentRegionWidth() + 2.0f * ImGui::GetStyle().WindowPadding.x !=
ImGui::CalcWindowNextAutoFitSize(ImGui::GetCurrentWindow()).x) canvas.request_extra_frame();
...
int PartPlate::picking_id_component(int idx) const { return this->m_plate_index * GRABBER_COUNT + idx; }
...
std::vector<int> PartPlate::get_extruders(bool conside_custom_gcode) const
{
    ...
    if (conside_custom_gcode) {
        ...
        if (const ConfigOptionStrings* color_option = dynamic_cast<const ConfigOptionStrings*>(
                wxGetApp().preset_bundle->project_config.option("filament_colour"))) {
...
bool PartPlate::check_mixture_filament_compatible(const DynamicPrintConfig& config, std::string& error_msg)
{
    ...
    if (incompatible_filament_pairs.empty()) {
    ...
    for (auto filament : used_filaments) {
        ...
        if (filament_idx >= filament_type_opt->values.size())
            filament_idx = 0;
...

    {
        ...
        filament_types.erase(std::remove_if(filament_types.begin(), filament_types.end(),
                                            [&](const std::string& s) { return !seen.insert(s).second; }),
                             filament_types.end());
    }
    ...
    for (size_t i = 0; i < filament_types.size(); i++) {
        ...
        if (it == incompatible_filament_pairs.end())
            continue;
        for (size_t j = i + 1; j < filament_types.size(); ++j) {
            if (it->second.count(filament_types[j])) {
                conflicts.emplace_back(filament_types[i], filament_types[j]);
            }
        }
    }
...
bool PartPlate::check_compatible_of_nozzle_and_filament(const DynamicPrintConfig&       config,
                                                        ...
                                                        std::string&                    error_msg)
{
    ...
    for (auto& filament_preset : filament_presets) {
        selected_filament_alias.insert(get_filament_alias(filament_preset));
    }
    ...
    for (auto volume_type_value : nozzle_volumes) {
        ...
        if (!incompatible_selected.empty())
            incompatible_selected_map[volume_type] = incompatible_selected;
    }
...
/*Vec3d PartPlate::calculate_wipe_tower_size(const DynamicPrintConfig &config, const double w, const double wipe_volume, int
plate_extruder_size, bool use_global_objects) const
{
...
extra_spacing); if (timelapse_enabled || plate_extruder_size > 1) { float min_wipe_tower_depth =
WipeTower::get_limit_depth_by_height(max_height); depth = std::max((double) min_wipe_tower_depth, depth); wipe_tower_size(0) =
...
Vec3d PartPlate::estimate_wipe_tower_size(const DynamicPrintConfig& config,
                                          const double              w,
                                          ...
                                          bool                      enable_wrapping_detection) const
{
    ...
    double extra_spacing   = config.option("prime_tower_infill_gap")->getFloat() / 100.;
    const ConfigOptionEnum<WipeTowerWallType>* use_rib_wall_opt = config.option<ConfigOptionEnum<WipeTowerWallType>>(
    ...
    if (extruder_count == 2)
        volume += filament_change_volume * (int) (plate_extruder_size / 2);
    if (use_rib_wall) {
...
arrangement::ArrangePolygon PartPlate::estimate_wipe_tower_polygon(const DynamicPrintConfig& config,
                                                                   int                       plate_index,
                                                                   Vec3d&                    wt_pos,
                                                                   Vec3d&                    wt_size,
                                                                   int                       extruder_count,
                                                                   ...
                                                                   bool                      use_global_objects) const
{
    ...
    Polygon                     ap({{scaled(x - wp_brim_width), scaled(y - wp_brim_width)},
...
void PartPlate::set_pos_and_size(Vec3d& origin, int width, int depth, int height, bool with_instance_move, bool do_clear)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(
                                   ": plate_id %1%, before, origin {%2%,%3%,%4%}, plate_width %5%, plate_depth %6%, plate_height %7%") %
                                   m_plate_index % m_origin.x() % m_origin.y() % m_origin.z() % m_width % m_depth % m_height;
    BOOST_LOG_TRIVIAL(info)
        << __FUNCTION__
        << boost::format(": with_instance_move %1%, after, origin {%2%,%3%,%4%}, plate_width %5%, plate_depth %6%, plate_height %7%") %
    ...
    if (with_instance_move && m_model) {
        for (std::set<std::pair<int, int>>::iterator it = obj_to_instance_set.begin(); it != obj_to_instance_set.end(); ++it) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": object %1%, instance %2%, moved {%3%,%4%} to {%5%, %6%}") % obj_id % instance_id %
...
bool PartPlate::intersect_instance(int obj_id, int instance_id, BoundingBoxf3* bounding_box)
{
    ...
    if (!valid_instance(obj_id, instance_id)) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(": plate_id %1%, invalid obj_id %2%, instance_id %3%") % m_plate_index % obj_id %
                                        instance_id;
        return false;
    }
...
int PartPlate::add_instance(int obj_id, int instance_id, bool move_position, BoundingBoxf3* bounding_box)
{
    if (!valid_instance(obj_id, instance_id)) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(": plate_id %1%, invalid obj_id %2%, instance_id %3%, move_position %4%") %
                                        m_plate_index % obj_id % instance_id % move_position;
        return -1;
    }
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": plate_id %1%, add instance obj_id %2%, instance_id %3%, move_position %4%") %
                                    m_plate_index % obj_id % instance_id % move_position;
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": plate %1% , m_ready_for_slice changes to %2%") % m_plate_index % m_ready_for_slice;
    return 0;
}
...
int PartPlate::remove_instance(int obj_id, int instance_id)
{
    ...
    if (it != obj_to_instance_set.end()) {
        ...
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                 << boost::format(":plate_id %1%, found obj_id %2%, instance_id %3%") % m_plate_index % obj_id %
                                        instance_id;
        result = 0;
    } else {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                 << boost::format(": plate_id %1%, can not find obj_id %2%, instance_id %3%") % m_plate_index % obj_id %
                                        instance_id;
        result = -1;
        return result;
    }
        ...
        if ((obj_id >= 0) && (obj_id < m_model->objects.size())) {
            ...
            if (need_skip) {
                if (skip_objects.find(instance->loaded_id) != skip_objects.end()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                            << boost::format(
                                                   ": skipped object, loaded_id %1%, name %2%, set to unprintable, no need to duplicate") %
                                                   instance->loaded_id % object->name;
                    continue;
                }
            }
            for (size_t index = 0; index < dup_count; index++) {
                ...
                for (size_t new_instance_id = 0; new_instance_id < newObj->instances.size(); new_instance_id++) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                            << boost::format(": duplicate object into plate: index_pair [%1%,%2%], obj_id %3%") %
                                                   new_obj_id % new_instance_id % newObj->id().id;
                }
            }
        }
    }
            ...
            if (instance->printable) {
                ...
                if (need_skip) {
                    while (skip_objects.find(instance->loaded_id) != skip_objects.end()) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << __FUNCTION__
                            << boost::format(": duplicated id %1% with skip, try new one %2%") % instance->id().id % instance->loaded_id;
                    }
                }
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": set obj %1% instance %2%'s loaded_id to its id %3%, name %4%") % obj_id %
                                               instance_id % instance->loaded_id % object->name;
            }
        }
    }
...
bool PartPlate::set_shape(const Pointfs&              shape,
                          ...
                          float                       height_to_rod)
{
    ...
    if ((m_shape == new_shape) && (m_exclude_area == new_exclude_areas) && (m_height_to_lid == height_to_lid) &&
    ...
    if ((m_shape != new_shape) || (m_exclude_area != new_exclude_areas)) {
        ...
        // ORCA also change bed_icon_count number in calc_vertex_for_icons() after adding or removing icons for circular shaped beds that
        // uses vertical alingment for icons
        bool dual_bbl = false;
        if (m_plater) {
            PresetBundle* preset = wxGetApp().preset_bundle;
...
bool PartPlate::contains(const Vec3d& point) const { return m_bounding_box.contains(point); }

bool PartPlate::contains(const GLVolume& v) const { return m_bounding_box.contains(v.bounding_box()); }

bool PartPlate::contains(const BoundingBoxf3& bb) const
{
    // Allow the objects to protrude below the print bed
    BoundingBoxf3 print_volume(Vec3d(m_bounding_box.min(0), m_bounding_box.min(1), 0.0),
                               Vec3d(m_bounding_box.max(0), m_bounding_box.max(1), 1e3));
...
void PartPlate::set_selected() { m_selected = true; }

void PartPlate::set_unselected() { m_selected = false; }
...
{
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": plate %1% , update slice result from %2% to %3%") % m_plate_index % m_slice_result_valid %
                                   valid;
...
std::string PartPlate::get_temp_config_3mf_path()
{
    if (m_temp_config_3mf_path.empty()) {
        boost::filesystem::path temp_path(m_model->get_backup_path("Metadata"));
        temp_path /= (boost::format(".%1%.%2%_config.3mf") % get_current_pid() % GLOBAL_PLATE_INDEX++).str();
...
int PartPlate::load_gcode_from_file(const std::string& filename)
{
    ...
    // if (error) {
    //	BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format("Failed to rename the output G-code file from %1% to %2%, error code %3%")
    //% filename.c_str() % path.c_str() % error.message(); 	return -1;
    //}
...
void PartPlate::update_first_layer_print_sequence(size_t filament_nums)
{
    ...
    if (!other_layers_seqs.empty()) {
        ...
        for (auto& other_layers_seq : other_layers_seqs) {
            ...
            if (orders.size() > filament_nums) {
                orders.erase(std::remove_if(orders.begin(), orders.end(), [filament_nums](int n) { return n > filament_nums; }),
                             orders.end());
    ...
    if (print_sequence_1st.size() > filament_nums) {
        print_sequence_1st.erase(std::remove_if(print_sequence_1st.begin(), print_sequence_1st.end(),
                                                [filament_nums](int n) { return n > filament_nums; }),
...
void PartPlate::print() const
{
    ...
    BOOST_LOG_TRIVIAL(trace) << __FUNCTION__
                             << boost::format(": plate index %1%, pointer %2%, print_index %3% print pointer %4%") % m_plate_index % this %
                                    m_print_index % m_print;
    BOOST_LOG_TRIVIAL(trace) << boost::format("\t origin {%1%,%2%,%3%}, width %4%,  depth %5%, height %6%") % m_origin.x() % m_origin.y() %
                                    m_origin.z() % m_width % m_depth % m_height;
    BOOST_LOG_TRIVIAL(trace)
        << boost::format(
               "\t m_printable %1%, m_locked %2%, m_ready_for_slice %3%, m_slice_result_valid %4%,  m_tmp_gcode_path %5%, set size %6%") %
...
PartPlateList::PartPlateList(int width, int depth, int height, Plater* platerObj, Model* modelObj, PrinterTechnology tech)
    ...
    , m_model(modelObj)
    , printer_technology(tech)
    , unprintable_plate(this,
                        Vec3d(0.0 + width * (1. + LOGICAL_PART_PLATE_GAP), 0.0, 0.0),
                        width,
                        depth,
                        height,
                        platerObj,
...
{
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":plate_width %1%, plate_depth %2%, plate_height %3%") % width % depth % height;
...
void PartPlateList::set_default_wipe_tower_pos_for_plate(int plate_idx)
{
    ...
    auto printer_structure_opt = wxGetApp().preset_bundle->printers.get_edited_preset().config.option<ConfigOptionEnum<PrinterStructure>>(
        "printer_structure");
...
void PartPlateList::reset_size(int width, int depth, int height, bool reload_objects, bool update_shapes)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":before size: plate_width %1%, plate_depth %2%, plate_height %3%") % m_plate_width %
                                   m_plate_depth % m_plate_height;
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":after size: plate_width %1%, plate_depth %2%, plate_height %3%") % width % depth % height;
    if ((m_plate_width != width) || (m_plate_depth != depth) || (m_plate_height != height)) {
        ...
        if (update_shapes) {
            set_shapes(m_shape, m_exclude_areas, m_wrapping_exclude_areas, m_extruder_areas, m_extruder_heights, m_logo_texture_filename,
                       m_height_to_lid, m_height_to_rod);
        }
        if (reload_objects)
            reload_all_objects();
        else
...
int PartPlateList::duplicate_plate(int index)
{
    ...
    for (ModelObject* object : obj_ptrs) {
        ...
        for (size_t new_instance_id = 0; new_instance_id < object_copy->instances.size(); new_instance_id++) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": duplicate object into plate: index_pair [%1%,%2%], obj_id %3%") % new_obj_id %
                                           new_instance_id % object_copy->id().id;
        }
    }
...
int PartPlateList::destroy_print(int print_index)
{
    ...
    if (print_index >= 0) {
        ...
        if (it2 != m_gcode_result_list.end()) {
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(":delete GCodeResult %1% for print_index %2%") % it2->second % print_index;
            delete it2->second;
...
void PartPlateList::delete_selected_plate() { delete_plate(m_current_plate); }
...
PartPlate* PartPlateList::get_selected_plate()
{
    if (m_current_plate < 0 || m_current_plate >= m_plate_list.size()) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                   << boost::format(":can not find m_current_plate  %1%, size %2%") % m_current_plate % m_plate_list.size();
        return NULL;
    }
...
void PartPlateList::update_plate_cols()
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":m_plate_count %1%, m_plate_cols change to %2%") % m_plate_count % m_plate_cols;
    return;
}
...
int PartPlateList::lock_plate(int index, bool state)
{
    ...
    if (!plate) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(":can not get plate for index %1%, size %2%") % index % m_plate_list.size();
        return -1;
    }
...
int PartPlateList::notify_instance_update(int obj_id, int instance_id, bool is_new)
{
    ...
    auto is_object_config_compatible_with_spiral_vase = [](ModelObject* object) {
        ...
        if (config.has("wall_loops") && config.opt_int("wall_loops") == 1 && config.has("top_shell_layers") &&
            config.opt_int("top_shell_layers") == 0 && config.has("sparse_infill_density") &&
            config.option<ConfigOptionPercent>("sparse_infill_density")->value == 0 && config.has("enable_support") &&
            !config.opt_bool("enable_support") && config.has("enforce_support_layers") && config.opt_int("enforce_support_layers") == 0 &&
            config.has("ensure_vertical_shell_thickness") && config.opt_bool("ensure_vertical_shell_thickness") &&
            config.has("detect_thin_wall") && !config.opt_bool("detect_thin_wall") && config.has("timelapse_type") &&
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        ...
        if (plate->intersect_instance(obj_id, instance_id, &boundingbox)) {
            ...
            if (plate->config()->has("spiral_mode") && plate->config()->opt_bool("spiral_mode") &&
                !is_object_config_compatible_with_spiral_vase(object)) {
                if (!is_new) {
...
int PartPlateList::add_to_plate(int obj_id, int instance_id, int plate_id)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": plate_id %1%, found obj_id %2%, instance_id %3%") % plate_id % obj_id % instance_id;
    index = find_instance(obj_id, instance_id);
    if (index != -1) {
        // found it added before
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(": found it in previous plate %1%") % index;
        ...
        for (j = 0; j < (unsigned int) object->instances.size(); ++j) {
            ...
            for (k = 0; k < (unsigned int) m_plate_list.size(); ++k) {
                ...
                if (plate->intersect_instance(i, j, &boundingbox)) {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                             << boost::format(": found plate_id %1%, for obj_id %2%, instance_id %3%") % k % i % j;
            ...
            if ((k == m_plate_list.size()) && (unprintable_plate.intersect_instance(i, j, &boundingbox))) {
                ...
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(": found in unprintable plate, obj_id %1%, instance_id %2%") % i % j;
            }
        }
    }
    ...
    for (i = 0; i < (unsigned int) m_model->objects.size(); ++i) {
        ...
        for (j = 0; j < (unsigned int) object->instances.size(); ++j) {
            ...
            if (new_plate->intersect_instance(i, j, &boundingbox)) {
                ...
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(": added to plate_id %1%, for obj_id %2%, instance_id %3%") % plate_index % i % j;
...
bool PartPlateList::preprocess_arrange_polygon(int                          obj_index,
                                               int                          instance_index,
                                               ...
                                               bool                         selected)
{
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        if (m_plate_list[i]->contain_instance(obj_index, instance_index)) {
            ...
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(": obj_id %1% instance_id %2% already in plate %3%, locked %4%, row %5%, col %6%\n") %
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": not in any plates, bed_idx %1%, translation(x) %2%, (y) %3%") % arrange_polygon.bed_idx %
                                    unscale<double>(arrange_polygon.translation(X)) % unscale<double>(arrange_polygon.translation(Y));

    return locked;
}

// preprocess a arrangement::ArrangePolygon, return true if it is not in current plate
bool PartPlateList::preprocess_arrange_polygon_other_locked(int                          obj_index,
                                                            int                          instance_index,
                                                            arrangement::ArrangePolygon& arrange_polygon,
                                                            bool                         selected)
{
    ...
    if (selected) {
        ...
        for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
            if (m_plate_list[i]->contain_instance(obj_index, instance_index)) {
                ...
                arrange_polygon.translation(X) -= scaled<double>(plate_stride_x() * arrange_polygon.col);
                arrange_polygon.translation(Y) += scaled<double>(plate_stride_y() * arrange_polygon.row);
                // BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(": obj_id %1% instance_id %2% in plate %3%, locked %4%, row
                // %5%, col %6%\n") % obj_index % instance_index % i % locked % arrange_polygon.row % arrange_polygon.col;
                return locked;
            }
        }
                                             ...
                                             float                         inflation)
{
    ...
    if (m_exclude_areas.size() > 0) {
        ...
        for (int index = 0; index < plate->m_exclude_bounding_box.size(); index++) {
            Polygon ap({{scaled(plate->m_exclude_bounding_box[index].min.x()), scaled(plate->m_exclude_bounding_box[index].min.y())},
...
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": bed_idx %1%, locked_plate %2%, translation(x) %3%, (y) %4%") % arrange_polygon.bed_idx %
                                    arrange_polygon.locked_plate % unscale<double>(arrange_polygon.translation(X)) %
                                    unscale<double>(arrange_polygon.translation(Y));
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        if (m_plate_list[i]->is_locked()) {
            ...
            if (arrange_polygon.bed_idx <= i) {
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(":found in plate_index %1%, bed_idx %2%") % i % arrange_polygon.bed_idx;
                return;
            }
        }
    }
    ...
    while (plate_index != -1) {
        if (arrange_polygon.bed_idx <= plate_index) {
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(":new plate_index %1%, matches bed_idx %2%") % plate_index % arrange_polygon.bed_idx;
            break;
        }
...
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": bed_idx %1%, selected %2%, translation(x) %3%, (y) %4%") % arrange_polygon.bed_idx %
                                    selected % unscale<double>(arrange_polygon.translation(X)) %
                                    unscale<double>(arrange_polygon.translation(Y));
                                                    ...
                                                    std::vector<std::string>& previous_gcode_paths)
{
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        ...
        if ((i < previous_gcode_paths.size()) && !previous_gcode_paths[i].empty() &&
        ...
        if (it != m_print_list.end()) {
            ...
            if (it2 == m_gcode_result_list.end()) {
                ...
                assert(0);
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(":can not find gcode result for plate %1%, print index %2%") % i %
                                                m_plate_list[i]->m_print_index;
                delete it->second;
                m_print_list.erase(it);
            } else {
...
int PartPlateList::rebuild_plates_after_arrangement(bool recycle_plates, bool except_locked, int plate_index)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(":before rebuild, plates count %1%, recycle_plates %2%") % m_plate_list.size() %
                                   recycle_plates;
...
int PartPlateList::store_to_3mf_structure(PlateDataPtrs& plate_data_list, bool with_slice_info, int plate_idx)
{
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": plate %1% before load, width %2%, height %3%, size %4%!") % (i + 1) %
                                       m_plate_list[i]->thumbnail_data.width % m_plate_list[i]->thumbnail_data.height %
                                       m_plate_list[i]->thumbnail_data.pixels.size();
        plate_data_item->plate_thumbnail.load_from(m_plate_list[i]->thumbnail_data);
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": plate %1% after load, width %2%, height %3%, size %4%!") % (i + 1) %
                                       plate_data_item->plate_thumbnail.width % plate_data_item->plate_thumbnail.height %
                                       plate_data_item->plate_thumbnail.pixels.size();
        ...
        if (m_plate_list[i]->obj_to_instance_set.size() > 0) {
            for (std::set<std::pair<int, int>>::iterator it = m_plate_list[i]->obj_to_instance_set.begin();
                 it != m_plate_list[i]->obj_to_instance_set.end(); ++it)
                plate_data_item->objects_and_instances.emplace_back(it->first, it->second);
        }

        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(
                                       ": plate %1%, gcode_filename=%2%, with_slice_info=%3%, slice_valid %4%, object item count %5%.") %
                                       i % m_plate_list[i]->m_gcode_result->filename % with_slice_info %
                                       m_plate_list[i]->is_slice_result_valid() % plate_data_item->objects_and_instances.size();
...
int PartPlateList::load_from_3mf_structure(PlateDataPtrs& plate_data_list, int filament_count)
{
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data_list[i]->plate_index != index) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(":plate index %1% seems invalid, skip it") % plate_data_list[i]->plate_index;
        }
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": plate %1%, gcode_file %2%, is_sliced_valid %3%, toolpath_outside %4%, is_support_used "
                                                 "%5% is_label_object_enabled %6%") %
                                       i % plate_data_list[i]->gcode_file % plate_data_list[i]->is_sliced_valid %
                                       plate_data_list[i]->toolpath_outside % plate_data_list[i]->is_support_used %
        ...
        /*for (std::vector<std::pair<int, int>>::iterator it = plate_data_list[i]->objects_and_instances.begin(); it !=
           plate_data_list[i]->objects_and_instances.end(); ++it) m_plate_list[index]->obj_to_instance_set.insert(std::pair(it->first,
           it->second));*/
        ...
        gcode_result->print_statistics.modes[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal)].time = atoi(
            plate_data_list[i]->gcode_prediction.c_str());
        ...
        if (m_plater && !plate_data_list[i]->thumbnail_file.empty()) {
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": plate %1%, load thumbnail from %2%.") % (i + 1) %
                                           plate_data_list[i]->thumbnail_file;
            if (boost::filesystem::exists(plate_data_list[i]->thumbnail_file)) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": plate %1% after load, width %2%, height %3%, size %4%!") % (i + 1) %
                                               m_plate_list[index]->thumbnail_data.width % m_plate_list[index]->thumbnail_data.height %
                                               m_plate_list[index]->thumbnail_data.pixels.size();
            }
        }
        ...
        if (m_plater && !plate_data_list[i]->no_light_thumbnail_file.empty()) {
            if (boost::filesystem::exists(plate_data_list[i]->no_light_thumbnail_file)) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": plate %1%, load no_light_thumbnail_file from %2%.") % (i + 1) %
                                               plate_data_list[i]->no_light_thumbnail_file;
                m_plate_list[index]->load_thumbnail_data(plate_data_list[i]->no_light_thumbnail_file,
                                                         m_plate_list[index]->no_light_thumbnail_data);
            }
        }
        ...
        if (m_plater && !plate_data_list[i]->top_file.empty()) {
            if (boost::filesystem::exists(plate_data_list[i]->top_file)) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": plate %1%, load top_thumbnail from %2%.") % (i + 1) %
                                               plate_data_list[i]->top_file;
        ...
        if (m_plater && !plate_data_list[i]->pick_file.empty()) {
            if (boost::filesystem::exists(plate_data_list[i]->pick_file)) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": plate %1%, load pick_thumbnail from %2%.") % (i + 1) %
                                               plate_data_list[i]->pick_file;
    ...
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
        if (!m_plate_list[i]->m_gcode_path_from_3mf.empty()) {
            ...
            m_model->update_print_volume_state({m_plate_list[i]->get_shape(), (double) this->m_plate_height,
                                                m_plate_list[i]->get_extruder_areas(), m_plate_list[i]->get_extruder_heights()});
...
void PartPlateList::print() const
{
    BOOST_LOG_TRIVIAL(trace)
        << __FUNCTION__
        << boost::format(
               "PartPlateList %1%, m_plate_count %2%, current_plate %3%, print_count %4%, current print index %5%, plate cols %6%") %
               this % m_plate_count % m_current_plate % m_print_list.size() % m_print_index % m_plate_cols;
    BOOST_LOG_TRIVIAL(trace) << boost::format("m_plate_width %1%, m_plate_depth %2%, m_plate_height %3%, plate count %4%\nplate list:") %
                                    m_plate_width % m_plate_depth % m_plate_height % m_plate_list.size();
    for (unsigned int i = 0; i < (unsigned int) m_plate_list.size(); ++i) {
...
void PartPlateList::init_bed_type_info()
{
    ...
    std::string                 bottom_texture_end_name = bed_texture_maps.find("bottom_texture_end_name") != bed_texture_maps.end() ?
                                                              bed_texture_maps["bottom_texture_end_name"] :
                                                              "";
    std::string                 bottom_texture_rect_str = bed_texture_maps.find("bottom_texture_rect") != bed_texture_maps.end() ?
                                                              bed_texture_maps["bottom_texture_rect"] :
                                                              "";
    std::string                 middle_texture_rect_str = bed_texture_maps.find("middle_texture_rect") != bed_texture_maps.end() ?
                                                              bed_texture_maps["middle_texture_rect"] :
                                                              "";
    std::string                 use_double_extruder_default_texture = bed_texture_maps.find("use_double_extruder_default_texture") !=
                                                              bed_texture_maps.end() ?
                                                                          bed_texture_maps["use_double_extruder_default_texture"] :
                                                                          "";
    ...
    if (use_double_extruder_texture) {
        ...
        if (middle_rect[2] > 0.f) {
            pte_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3],
        ...
        if (middle_rect[2] > 0.f) {
            pei_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3],
    ...
    for (int i = 0; i < (unsigned int) ExtruderOnlyAreaType::btAreaCount; ++i) {
        for (int j = 0; j < extruder_only_area_info[i].parts.size(); j++) {
            ...
            if (boost::filesystem::exists(filename)) {
                ...
                if (!PartPlateList::extruder_only_area_info[i].parts[j].texture->load_from_svg_file(filename, true, false, false,
                                                                                                    logo_tex_size)) {
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(": load logo texture from %1% failed!") % filename;
                }
            } else {
