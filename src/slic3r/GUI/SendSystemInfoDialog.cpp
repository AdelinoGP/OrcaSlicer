...
// [INTENT] First-run / version-gated consent dialog for telemetry-style system info upload.
// [UNITY] Model this as a modal confirmation controller with a read-only preview sheet and explicit opt-in / opt-out actions.
// [PORTING_HAZARD:P2] The payload is assembled from OS, hardware, OpenGL, and installed-library probes, so the Unity port needs a
// privacy-reviewed data contract rather than a 1:1 UI swap.
class SendSystemInfoDialog : public DPIDialog
{
    enum { MIN_WIDTH = 70, MIN_HEIGHT = 34 };
...
private:
    ...
    // [STATE] Cached JSON payload is generated once up front so the preview and send action stay consistent.
    const std::string m_system_info_json;
    // [STATE] Button members are kept alive for DPI-aware rescaling and binding updates.
...
// [INTENT] Lightweight progress modal that blocks the user while the background upload runs.
// [UNITY] Replace with a modal progress overlay / dialog that owns a cancel event and keeps the UI responsive.
...
// [INTENT] Read-only inspector for the exact JSON payload before it is sent.
// [UNITY] Use a scrollable read-only text view with monospace styling and a close/dismiss action.
...
public:
    ...
    {
        auto* text = new wxTextCtrl(this, wxID_ANY, json, wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY | wxTE_DONTWRAP);
...
// [INTENT] Version gate for re-prompting: only show after a new major/minor release so the dialog is not spammed every launch.
// [UNCLEAR] The persisted last-sent version is currently stubbed to SLIC3R_VERSION, so the gate behaves like "show on newer build" only in
// practice. Only show the dialog when this info was not found (e.g. fresh install) or when current version is newer. Only major and minor
...
static bool should_dialog_be_shown()
{
    ...
    if (!show_in_alphas && semver_current.prerelease() && std::string(semver_current.prerelease()).find("alpha") != std::string::npos)
        return false;
...
static std::map<std::string, std::string> get_cpu_info_from_registry()
{
    ...
    while (true) {
        if (RegGetValueA(HKEY_LOCAL_MACHINE, reg_path.c_str(), "ProcessorNameString", RRF_RT_REG_SZ, NULL, &buf, &bufsize) ==
            ERROR_SUCCESS) {
            out["Model"] = buf;
...
static std::map<std::string, std::string> parse_lscpu_etc(const std::string& name, char delimiter)
{
    ...
    if (fp != NULL) {
        while (fgets(cline, max_len, fp) != NULL) {
            ...
            line.erase(std::remove_if(line.begin(), line.end(), [](char c) { return c == '\"' || c == '\r' || c == '\n'; }), line.end());
...
static std::string get_unique_id()
{
    ...
    if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == NO_ERROR) {
        ...
        while (pAdapterInfo) {
            ...
            if (!ethernet_seen) {
                if ((pAdapterInfo->Type == MIB_IF_TYPE_ETHERNET && (ethernet_seen = true)) || pAdapterInfo->Type == IF_TYPE_IEEE80211)
    ...
    if (ioRegistryRoot != MACH_PORT_NULL) {
        CFStringRef uuidCf = (CFStringRef) IORegistryEntryCreateCFProperty(ioRegistryRoot, CFSTR(kIOPlatformUUIDKey), kCFAllocatorDefault,
                                                                           0);
...
// [INTENT] Build the telemetry payload once so the preview, the upload body, and the confirmation dialog all share the same snapshot.
...
{
    // [STATE] Unique-ID hashing intentionally strips most source detail while still allowing duplicate detection in uploads.
    ...
    // [STATE] Locale, distro, hardware, display, and OpenGL fields are gathered from platform APIs to characterize the install.
    // Get system language.
    ...
    // [STATE] CPU model / vendor / core count are platform-specific probes with different parsing paths per OS.
...
// Only get the scaling on Win, it is not reliable on other platforms.
    ...
    // [OPENGL] GL metadata comes from the active context, so Unity needs a dedicated graphics-info probe rather than a UI field rename.
    ...
    // [OPENGL] Extension enumeration is context-dependent and can be very large; keep it in a separate payload section.
    ...
    // [INTENT] Serialize the complete probe snapshot into the transport payload consumed by the send action.
...
SendSystemInfoDialog::SendSystemInfoDialog(wxWindow* parent)
    ...
    GUI::DPIDialog(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
{
    // [STATE] The dialog is sized around the current EM unit and then reflowed again on DPI changes.
    const int em = GUI::wxGetApp().em_unit();

    // [INTENT] Build the versioned title text that explains which app build is asking for consent.
    ...
    std::string app_name;
    {
        ...
        app_name = std::string(SLIC3R_APP_FULL_NAME) + " " + std::to_string(semver.maj()) + "." + std::to_string(semver.min()) + " " +
                   (is_alpha ? "Alpha" :
                    is_beta  ? "Beta" :
                               "");
    }
    ...
    // [UNITY] The visual structure is a single-column modal with a branded header, preview area, and three decision buttons.
    ...
    wxString html        = GUI::format_wxstr("<html><body bgcolor=%1%><font color=%2%>"
                                             "<table><tr><td>"
                                             "<img src = \"" +
                                                 resources_dir() +
                                                 "/images/OrcaSlicer_192px.png\" />"
                                                 "</td><td align=\"left\">" +
                                                 text0 + "<br / ><br / >" + text1 +
                                                 "<br /><br />"
                                                 "</td></tr></table>" +
                                                 "<b>" + label2 + "</b><br />" + text2 + "</font></body></html>",
    ...
    SetSize(std::max(size.GetWidth(), MIN_WIDTH * em), std::max(size.GetHeight(), MIN_HEIGHT * em));

    CenterOnParent();

    // [EVENT] Button handlers either open the JSON preview or terminate the modal with a persisted decision path.
...
// [EVENT] DPI changes must rescale the action buttons and recompute the minimum dialog size.
void SendSystemInfoDialog::on_dpi_changed(const wxRect&)
{
    const int& em = em_unit();
    msw_buttons_rescale(this, em, {m_btn_send->GetId(), m_btn_dont_send->GetId(), m_btn_ask_later->GetId()});
    SetMinSize(wxSize(MIN_WIDTH * em, MIN_HEIGHT * em));
    Fit();
    Refresh();
}

// [THREAD] Upload work is dispatched to a worker thread while the UI stays in a modal progress loop.
// [PORTING_HAZARD:P2] The current design mixes thread joining, modal nesting, and timer-based polling; Unity should prefer an async
// task/coroutine boundary. This actually sends the info.
bool SendSystemInfoDialog::send_info(wxString& message)
{
    std::atomic<int> job_done = false; // Flag to communicate between threads.
    struct Result
    {
        enum { Success, Cancelled, Error } value;
        wxString str;
    } result; // No synchronization needed, UI thread reads only after worker is joined.

    // [THREAD] The HTTP client callbacks mutate shared result state, but only after the worker thread has serialized access through perform_sync().
    ...
    // [STATE] The progress dialog owns the user-visible cancel path while the worker thread performs the network request.
    ...
    dlg.Bind(wxEVT_TIMER, [&dlg, &job_done](wxTimerEvent&) {
        if (job_done)
            dlg.EndModal(0);
    });
    timer.Start(50);
...
// [INTENT] External entry point: show the consent dialog only when the gating policy says this build should prompt.
// [UNITY] This becomes an app-start or settings-triggered modal flow with explicit early-out when telemetry is disabled.
