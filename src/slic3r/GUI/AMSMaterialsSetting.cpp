...
{
    // [INTENT] Build the modal editor for AMS filament metadata, wiring validation and picker events during construction.
    // [PORTING_HAZARD:P1] This setup relies on wx focus-loss, enter-key, and modal dialog sequencing; Unity needs explicit form-state
    // transitions instead.
    ...
    m_comboBox_filament->Connect(wxEVT_COMMAND_COMBOBOX_SELECTED, wxCommandEventHandler(AMSMaterialsSetting::on_select_filament), NULL,
                                 this);

    m_comboBox_cali_result->Connect(wxEVT_COMMAND_COMBOBOX_SELECTED, wxCommandEventHandler(AMSMaterialsSetting::on_select_cali_result),
                                    NULL, this);
}
...
void AMSMaterialsSetting::create_panel_normal(wxWindow* parent)
{
    ...
    m_title_filament = new wxStaticText(parent, wxID_ANY, _L("Filament"), wxDefaultPosition, wxSize(AMS_MATERIALS_SETTING_LABEL_WIDTH, -1),
                                        0);
...
#ifdef __APPLE__
    m_comboBox_filament = new wxComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, AMS_MATERIALS_SETTING_COMBOX_WIDTH, 0, nullptr,
                                         wxCB_READONLY);
#else
    m_comboBox_filament = new ::ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, AMS_MATERIALS_SETTING_COMBOX_WIDTH, 0, nullptr,
                                         wxCB_READONLY);
#endif
    ...
    m_title_temperature             = new wxStaticText(parent, wxID_ANY, _L("Nozzle\nTemperature"), wxDefaultPosition,
                                                       wxSize(AMS_MATERIALS_SETTING_LABEL_WIDTH, -1), 0);
    ...
    m_input_nozzle_max = new ::TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition,
                                         AMS_MATERIALS_SETTING_INPUT_SIZE, wxTE_CENTRE | wxTE_PROCESS_ENTER);
    m_input_nozzle_min = new ::TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition,
                                         AMS_MATERIALS_SETTING_INPUT_SIZE, wxTE_CENTRE | wxTE_PROCESS_ENTER);
...
void AMSMaterialsSetting::create_panel_kn(wxWindow* parent)
{
    ...
    // pa profile
    m_title_pa_profile = new wxStaticText(parent, wxID_ANY, _L("PA Profile"), wxDefaultPosition,
                                          wxSize(AMS_MATERIALS_SETTING_LABEL_WIDTH, -1), 0);
    ...
    m_comboBox_cali_result = new ::ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, AMS_MATERIALS_SETTING_COMBOX_WIDTH, 0,
    ...
    m_input_k_val = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                                  wxTE_CENTRE | wxTE_PROCESS_ENTER);
    ...
    m_input_n_val = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                                  wxTE_CENTRE | wxTE_PROCESS_ENTER);
...
{
    m_comboBox_filament->Disconnect(wxEVT_COMMAND_COMBOBOX_SELECTED, wxCommandEventHandler(AMSMaterialsSetting::on_select_filament), NULL,
                                    this);
    m_comboBox_cali_result->Disconnect(wxEVT_COMMAND_COMBOBOX_SELECTED, wxCommandEventHandler(AMSMaterialsSetting::on_select_cali_result),
                                       NULL, this);
}

void AMSMaterialsSetting::input_min_finish()
{
    if (m_input_nozzle_min->GetTextCtrl()->GetValue().empty())
        return;
...
{
    if (m_input_nozzle_max->GetTextCtrl()->GetValue().empty())
        return;
...
{
    MessageDialog msg_dlg(nullptr, _L("Are you sure you want to clear the filament information?"), wxEmptyString,
    ...
    if (obj) {
        if (m_is_third) {
            obj->command_ams_filament_settings(ams_id, slot_id, ams_filament_id, ams_setting_id, std::string(col_buf), m_filament_type,
...
void AMSMaterialsSetting::on_select_ok(wxCommandEvent& event)
{
            ...
            if (it->filament_id.compare(filament_id) == 0) {
                ...
                if (wxGetApp().app_config->get("skip_ams_blacklist_check") != "true") {
                    ...
                    if (vendor && (vendor->values.size() > 0)) {
                        ...
                        DevFilaBlacklist::check_filaments_in_blacklist(obj->printer_type, vendor_name, filamnt_type, it->filament_id,
                                                                       ams_id, slot_id, it->name, in_blacklist, action, info);
                    }
    ...
    if (obj->cali_version <= -1 && (obj->get_printer_series() != PrinterSeries::SERIES_X1) &&
...
void AMSMaterialsSetting::on_select_close(wxCommandEvent& event) { Close(); }
...
void AMSMaterialsSetting::set_colors(std::vector<wxColour> colors)
{
    ...
    if (!colors.empty()) {
        ...
        for (const auto& clr : colors) {
            fila_color.m_colors.insert(clr);
        }
        fila_color.EndSet(m_clr_picker->ctype);
        auto clr_query = GUI::wxGetApp().get_filament_color_code_query();
        m_clr_name->SetLabelText(clr_query->GetFilaColorName(ams_filament_id, fila_color));
...
void AMSMaterialsSetting::set_ctype(int ctype) { m_clr_picker->ctype = ctype; }
...
void AMSMaterialsSetting::Popup(wxString filament, wxString sn, wxString temp_min, wxString temp_max, wxString k, wxString n)
{
    ...
    std::set<std::string> printer_names =
        preset_bundle->get_printer_names_by_printer_type_and_nozzle(DevPrinterConfigUtil::get_printer_display_name(obj->printer_type),
                                                                    nozzle_diameter_str);
    ...
    // Sort the filaments
    {
        static std::unordered_map<wxString, int> sorted_names{{"Bambu PLA Basic", 0},         {"Bambu PLA Matte", 1},
                                                              {"Bambu PETG HF", 2},           {"Bambu ABS", 3},
                                                              {"Bambu PLA Silk", 4},          {"Bambu PLA-CF", 5},
                                                              {"Bambu PLA Galaxy", 6},        {"Bambu PLA Metal", 7},
                                                              {"Bambu PLA Marble", 8},        {"Bambu PETG-CF", 9},
                                                              {"Bambu PETG Translucent", 10}, {"Bambu ABS-GF", 11}};
        ...
        for (auto it = preset_bundle->filaments.begin(); it != preset_bundle->filaments.end(); it++) {
            if (!m_comboBox_filament->GetValue().IsEmpty()) {
                ...
                if (it->filament_id.compare(filament_id) == 0) {
                    ...
                    if (!it->is_system && !has_compatible_printer)
                        continue;
        ...
        for (auto cali_item : cali_history) {
            if (cali_item.filament_id == ams_filament_id) {
                if (obj->is_multi_extruders() &&
                    (cali_item.extruder_id != extruder_id || cali_item.nozzle_volume_type != nozzle_volume_type)) {
                    continue;
                }
...
ColorPickerPopup::ColorPickerPopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
    m_def_colors.clear();
    ...
    for (wxColour col : m_def_colors) {
        ...
        cp->Bind(wxEVT_LEFT_DOWN, [this, cp](auto& e) {
            ...
            unsigned long  g_col = ((cp->m_colour.Red() & 0xff) << 24) + ((cp->m_colour.Green() & 0xff) << 16) +
                                   ((cp->m_colour.Blue() & 0xff) << 8) + (cp->m_colour.Alpha() & 0xff);
            evt.SetInt(g_col);
    ...
    m_custom_cp->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_HAND); });
    m_custom_cp->Bind(wxEVT_LEAVE_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_ARROW); });
    ...
    m_ts_stbitmap_custom->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_HAND); });
    m_ts_stbitmap_custom->Bind(wxEVT_LEAVE_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_ARROW); });
...
void ColorPickerPopup::on_custom_clr_picker(wxMouseEvent& event)
{
    ...
    if (clr_dialog->ShowModal() == wxID_OK) {
        ...
        picker_color = wxColour(m_clrData->GetColour().Red(), m_clrData->GetColour().Green(), m_clrData->GetColour().Blue(), 255);
        ...
        wxCommandEvent evt(EVT_SELECTED_COLOR);
        unsigned long  g_col = ((picker_color.Red() & 0xff) << 24) + ((picker_color.Green() & 0xff) << 16) +
                               ((picker_color.Blue() & 0xff) << 8) + (picker_color.Alpha() & 0xff);
        evt.SetInt(g_col);
        wxPostEvent(GetParent(), evt);
    }
}
...
void ColorPickerPopup::Popup() { PopupWindow::Popup(); }
