// [INTENT] Declares the plate settings UI, including layer-range widgets that let users customize first-layer and other-layer print order
// per plate. [STATE] The dialog owns transient combobox, drag-canvas, and layer-range widget state that mirrors the current plate settings
// until the caller reads it back. [PORTING_HAZARD:P2] This API exposes raw wx widget pointers and implicit parent-child lifetime, so ports
// need a safer ownership model around the editor state.
...
public:
    ...
    enum class ValueType { Custom, End };
...
public:
    ...
    PlateSettingsDialog(wxWindow*       parent,
    ...
    std::vector<LayerPrintSequence> get_other_layers_print_seq_infos()
    {
        ...
        for (int i = 0; i < layer_seq_infos.size(); i++) {
            LayerPrintSequence item = std::make_pair(std::make_pair(layer_seq_infos[i].begin_layer_number,
                                                                    layer_seq_infos[i].end_layer_number),
                                                     layer_seq_infos[i].print_sequence);
    ...
    bool get_spiral_mode() { return false; }
