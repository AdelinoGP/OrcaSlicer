// [ANNOTATED]
// [INTENT] Modal discovery dialog for Bonjour-advertised printer hosts and a follow-up IP picker for ambiguous hostname resolution.
// [STATE] Owns the live reply set, selection state, and a timer-driven status label while background discovery is active.
// [EVENT] Bridges Bonjour callbacks back onto the wx event loop with custom reply/complete events.
// [UNITY] Split into a discovery service plus a retained-mode device picker UI instead of a modal dialog.
// [PORTING_HAZARD:P1] Discovery completion and reply delivery depend on wx event queue semantics plus manual lifetime guarding across threads.
...
public:
    ...
    BonjourReplyEvent(wxEventType eventType, int winid, BonjourReply&& reply) : wxEvent(winid, eventType), reply(std::move(reply)) {}

    virtual wxEvent* Clone() const { return new BonjourReplyEvent(*this); }
};
    ...
    , tech(tech)
{
    ...
    Bind(EVT_BONJOUR_COMPLETE, [this](wxCommandEvent&) { this->timer_state = 0; });
...
bool BonjourDialog::show_and_lookup()
{
    ...
    // [THREAD][PORTING_HAZARD:P1] Bonjour callbacks outlive the modal scope, so this guard prevents queued wx events from targeting a freed
    // dialog. The background thread needs to queue messages for this dialog and for that it needs a valid pointer to it (mandated by the
    // wxWidgets API). Here we put the pointer under a shared_ptr and protect it by a mutex, so that both threads can access it safely.
...
void BonjourDialog::on_reply(BonjourReplyEvent& e)
{
    ...
    // [PORTING_HAZARD:P2] Rebuilding the entire sorted list on each reply is simple in wxListView, but a Unity port should avoid full UI churn.
...
IPListDialog::IPListDialog(wxWindow*                                    parent,
                           const wxString&                              hostname,
                           const std::vector<boost::asio::ip::address>& ips,
                           size_t&                                      selected_index)
    : wxDialog(parent,
               wxID_ANY,
               _(L("Multiple resolved IP addresses")),
               wxDefaultPosition,
               wxDefaultSize,
    ...
    , m_selected_index(selected_index)
{
    ...
    auto* label = new wxStaticText(
        this, wxID_ANY,
        GUI::format_wxstr(_L("There are several IP addresses resolving to hostname %1%.\nPlease select one that should be used."),
                          hostname));
    vsizer->Add(label, 0, wxEXPAND | wxTOP | wxLEFT | wxRIGHT, em);
