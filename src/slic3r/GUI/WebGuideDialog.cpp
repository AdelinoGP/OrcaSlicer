...
// [STATE] m_ProfileJson is the shared wizard model that the page script mutates,
// and the background loader fills from vendor presets before the UI is told to refresh.
// [UNITY] This should become a ScriptableObject-backed setup model with an async loader
// and a main-thread command dispatcher instead of a free-floating global json blob.
json m_ProfileJson;

// [INTENT] Build the filament list payload consumed by the embedded guide page.
// [STATE] This filters presets through the current app bundle, hiding system/project
// embedded entries while preserving the "action required" sentinel for incomplete data.
// [UNITY] Port this as a typed data source that feeds a retained list view, not as
// a stringified JS callback assembled in view code.
...
{
    // [INTENT] The dialog hosts the first-run web wizard and acts as the bridge
    // between page-driven onboarding and the native config/preset state.
    // [STATE] It keeps a retained GUI_App back-pointer, the current page enum,
    // and a lazily created wxWebView that owns the HTML/JS interaction surface.
    // [UNITY] Model this as a modal controller with an embedded WebView/UIDocument
    // host and explicit lifecycle ownership for the browser surface.
...
#endif
    // [EVENT] The embedded page drives navigation, fullscreen, and script-message
    // callbacks; the native dialog reacts by mutating config and firing app events.
    // [UNITY] Replace this with a typed bridge from web content to a C# controller,
    // with each command validated before it reaches application services.
...
{
    // [THREAD] The worker thread that scans presets must finish before the dialog
    // is torn down; this blocks the UI thread on join() and is a porting hazard.
    // [PORTING_HAZARD:P2] Unity should cancel/await the loader instead of joining
    // on destruction so the modal can close without freezing the main thread.
    m_destroy = true;
...
{
    // [INTENT] Load a new wizard page and restore focus/state around the webview.
    // [UNITY] This becomes a retained browser host navigation call plus a focus
    // handoff to the page root, not direct widget manipulation.
...
{
    // [STATE] The page enum drives which onboarding branch is shown, and the URL
    // also depends on language plus whether the wizard is handling filaments only.
    // [PORTING_HAZARD:P3] Page selection is implicit in a set of query-string targets;
    // Unity should make these branches explicit in state rather than URL encoding.
    m_page = startpage;
...
{
    // [THREAD] The first completed navigation is the handoff point for the async
    // preset scan; after that the browser is shown and layout is refreshed on the UI thread.
    // [UNITY] Keep the page bootstrap async, but move the data load into an awaited
    // service with explicit completion events instead of a raw boost::thread.
...
{
    // [EVENT] The page posts JSON commands here to drive onboarding, profile edits,
    // plugin installation, and finish/cancel actions.
    // [UNITY] Port this as a typed command contract (eg. close, agree, save-region,
    // finish) routed through a main-thread dispatcher; avoid parsing arbitrary json in UI code.
    try {
...
{
    // [STATE] First-run detection is config-backed and intentionally defaults to
    // "true" unless the finish flag is present.
    wxString    strUse;
...
{
    // [INTENT] Persist the wizard selections back into app_config and rebuild the
    // preset bundle state to match the chosen region, printers, filaments, and stealth mode.
    // [PORTING_HAZARD:P2] This writes through multiple global singletons and can trigger
    // side effects in later startup steps, so Unity needs a service layer with explicit commit stages.
...
{
    // [INTENT] Translate the wizard snapshot into installed vendor bundles and the
    // active printer/filament preset selection.
    // [THREAD] The work is synchronous and mutates live config state, so it assumes
    // the caller is already on the UI thread with the modal flow blocked.
    // [UNITY] Split this into a persistence/service layer that stages install/remove
    // operations before committing the resulting settings model.
...
{
    // [THREAD] This background task scans preset JSON files off the UI thread and
    // posts completion back with CallAfter once the aggregate wizard model is ready.
    // [PORTING_HAZARD:P2] The loader reads and mutates shared state via m_destroy,
    // global path variables, and the module-level json blob, so Unity needs a cancellable
    // async load service with explicit state ownership.
    try {
...
{
    // [INTENT] Mirror the current app_config back into the wizard's json model so
    // the page can re-open with the user's current region, printer, and filament state.
    try {
...
{
    // [INTENT] Expand one vendor bundle into machine, filament, and process entries
    // by recursively resolving referenced json fragments and cover images.
    // [UNITY] Replace this recursive filesystem crawl with a data import step that
    // produces a normalized catalog before the UI needs to display it.
...
{
    // [INTENT] Delegate plugin acquisition to the app-level downloader while keeping
    // the dialog responsible only for progress/status reporting.
    // [UNITY] This should become an async service call with a progress callback into
    // the modal controller, not direct dialog-owned transport logic.
...
{
    // [INTENT] Kick off the install phase for the plugin archive that was previously downloaded.
...
{
    // [UNCLEAR] The status UI is still a stub; the intended feedback loop is likely
    // the same progress surface used by the downloader, but the current implementation
    // never renders it.
    // TODO
    return 0;
}
