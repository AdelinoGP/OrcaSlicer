...
// [INTENT] AboutDialogLogo displays the application logo in a dedicated panel.
// [UNITY] Use UnityEngine.UI.Image with custom MonoBehaviour for drawing logic.
AboutDialogLogo::AboutDialogLogo(wxWindow* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
    this->SetBackgroundColour(*wxWHITE);
    // [STATE] logo: The logo bitmap image.
    this->logo = ScalableBitmap(this, Slic3r::var("OrcaSlicer_192px.png"), wxBITMAP_TYPE_PNG);
    this->SetMinSize(this->logo.GetBmpSize());

    // [EVENT] Handle paint events for custom logo drawing.
    this->Bind(wxEVT_PAINT, &AboutDialogLogo::onRepaint, this);
}

// [INTENT] Handles the painting of the logo onto the panel.
// [PORTING_HAZARD:P1] Paint events are platform-specific; requires custom sprite or RenderTexture drawing in Unity.
...
// CopyrightsDialog
// -----------------------------------------
// [INTENT] CopyrightsDialog displays open-source license information.
// [UNITY] Use a Canvas-based modal panel or a custom EditorWindow.
CopyrightsDialog::CopyrightsDialog()
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe),
                wxID_ANY,
                from_u8((boost::format("%1% - %2%") % (wxGetApp().is_editor() ? SLIC3R_APP_FULL_NAME : GCODEVIEWER_APP_NAME) %
                         _utf8(L("Portions copyright")))
                            .str()),
                wxDefaultPosition,
                wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    // [STATE] m_html: The HTML rendering window for license text.
    // [UNITY] Use UI Toolkit's Text component with Rich Text enabled, or a WebView component.
    m_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxSize(40 * em_unit(), 20 * em_unit()), wxHW_SCROLLBAR_AUTO);
    ...
    // [EVENT] Handle link clicks in the HTML window.
...
wxString CopyrightsDialog::get_html_text()
{
    ...
    for (auto& entry : m_entries) {
        text += format_wxstr("%s<br/>", entry.lib_name);

        text += wxString::Format("<a href=\"%s\">%s</a><br/><br/>", entry.link, entry.link);
    }
...
AboutDialog::AboutDialog()
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe),
                wxID_ANY,
                from_u8(
                    (boost::format(_utf8(L("About %s"))) % (wxGetApp().is_editor() ? SLIC3R_APP_FULL_NAME : GCODEVIEWER_APP_NAME)).str()),
                wxDefaultPosition,
                wxDefaultSize,
                /*wxCAPTION*/ wxDEFAULT_DIALOG_STYLE)
{
    ...
    wxStaticText* html_text = new wxStaticText(this, wxID_ANY, "Copyright(C) 2022-2025 Li Jiang All Rights Reserved", wxDefaultPosition,
                                               wxDefaultSize);
    ...
    m_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_NEVER /*NEVER*/);
    {
        ...
        const auto text = from_u8(
            (boost::format("<html>"
                           "<body>"
                           "<p style=\"text-align:left\"><a style=\"color:#009789\" "
                           "href=\"https://github.com/OrcaSlicer/OrcaSlicer\">https://github.com/OrcaSlicer/OrcaSlicer</ a></p>"
                           "</body>"
                           "</html>"))
                .str());
        m_html->SetPage(text);
