...
// [EVENT] Custom events for slicing status and UI updates.
// [UNITY] Replace with C# Events, UnityEvents, or a Message Bus (e.g., ScriptableObject-based event system).
...
// [INTENT] Static properties for sidebar layout and spacing.
// [UNITY] Use Unity UI Toolkit style sheets (.uss) and Flexbox for layout properties.
class SidebarProps
{
public:
    ...
    {
        return 8;
    } // Use as side margins on titlebar. Has less margin on sides to create separation with its content
    ...
    static int WideSpacing()
    {
        return 18;
    } // Use between main elements / control groups for separation or preventing accidental clicks important
    static int ElementSpacing() { return 5; } // Use if elements has relation between them like edit button for combo box etc.
};

// [INTENT] Sidebar panel containing printer, filament, and object settings.
// [UNITY] Map to a dedicated VisualElement panel with its own Controller (MonoBehaviour).
// [STATE] Manages current extruder count, filament presets, and printer status synchronization.
...
private:
    // [STATE] Private implementation (PIMPL) containing wxWidgets-heavy objects.
    // [PORTING_HAZARD:P2] The 'priv' struct likely contains many wxWidgets objects and event handlers that need decomposition into Unity components.
    struct priv;
...
// [INTENT] Main 3D workspace panel - core of the slicer GUI.
// [UNITY] Map to a WorkspaceController or SceneController MonoBehaviour.
// [STATE] Manages the Model, Print objects, and the 3D viewport (GLCanvas3D).
...
public:
    ...
    void     update_obj_preview_thumbnail(
            ModelObject*, int obj_idx, int vol_idx, std::vector<Slic3r::ColorRGBA> colors, int camera_view_angle_type);
    ...
    enum class PrinterWarningType { NOT_CONNECTED, INCONSISTENT, UNINSTALL_FILAMENT, EMPTY_FILAMENT };
    ...
    {
    public:
        SuppressSnapshots(Plater* plater) : m_plater(plater) { m_plater->suppress_snapshots(); }
        ~SuppressSnapshots() { m_plater->allow_snapshots(); }
    ...
    public:
        ...
        ~TakeSnapshot() { m_plater->allow_snapshots(); }
