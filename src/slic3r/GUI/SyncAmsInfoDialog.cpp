...
bool SyncAmsInfoDialog::Show(bool show)
{
    ...
    if (dirty_filament) {
        ...
        m_confirm_title->SetLabel(
            _L("Synchronizing AMS filaments will discard your modified but unsaved filament presets.\nAre you sure you want to continue?"));
...
void SyncAmsInfoDialog::update_select_layout(MachineObject* obj) { m_checkbox_list["use_ams"]->Hide(); }
...
{ // thumbnail
    ...
    init_bitmaps();
    m_swipe_left_button = new ScalableButton(m_two_thumbnail_panel, wxID_ANY, "previous_item", wxEmptyString, wxDefaultSize,
                                             wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER, true, m_bmp_pix_cont);
    m_swipe_left_button->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) {
        if (!m_swipe_left_button_enable) {
            return;
        }
    ...
    view_two_thumbnail_sizer->AddSpacer(FromDIP(24));
    {
        ...
        m_left_image_button     = new wxButton(m_two_image_panel, wxID_ANY, {}, wxDefaultPosition,
                                               wxSize(FromDIP(LEFT_THUMBNAIL_SIZE_WIDTH), FromDIP(LEFT_THUMBNAIL_SIZE_WIDTH)),
    ...
    m_swipe_right_button = new ScalableButton(m_two_thumbnail_panel, wxID_ANY, "next_item", wxEmptyString, wxDefaultSize, wxDefaultPosition,
                                              wxBU_EXACTFIT | wxNO_BORDER, true, m_bmp_pix_cont);
    m_swipe_right_button->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) {
        if (!m_swipe_right_button_enable) {
            return;
        }
    ...
    m_combobox_plate = new ComboBox(m_two_thumbnail_panel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(60), -1), 0, NULL,
                                    wxCB_READONLY);
...
void SyncAmsInfoDialog::updata_ui_when_priner_not_same()
{
    ...
    m_confirm_title->Show();
    m_confirm_title->SetLabel(
        _L("The connected printer does not match the currently selected printer. Please change the selected printer."));

    Layout();
    Fit();
}

// [INTENT][STATE][UNITY][PORTING_HAZARD:P2] Modal AMS-sync dialog statefully owns printer/filament snapshots,
// timer refreshes, and two alternate mapping modes. Unity should split this into a modal controller plus
// data-bound views; the current wxWidgets class mixes layout, validation, and preview mutation.
    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    for (int i = 0; i < BED_TYPE_COUNT; i++) {
        m_bedtype_list.push_back(SelectMachineDialog::MACHINE_BED_TYPE_STRING[i]);
    }
    ...
    { // content
        ...
        m_reset_all_btn = new ScalableButton(m_scrolledWindow, wxID_ANY, "reset_gray", wxEmptyString, wxDefaultSize, wxDefaultPosition,
        ...
        m_colormap_btn->Bind(wxEVT_BUTTON, &SyncAmsInfoDialog::update_when_change_map_mode,
                             this); // update_when_change_map_mode(e.GetSelection());
    ...
    auto option_auto_bed_level = new PrintOption(m_options_other, _L("Auto Bed Leveling"),
                                                 _L("Check heatbed flatness. Leveling makes extruded height uniform.\n*Automatic mode: "
                                                    "Level first (about 10 seconds). Skip if surface is fine."),
    ...
    { // new content//tip confirm ok button
        ...
        m_tip_attention_override  = _L("Replace the project filaments list sequentially based on printer filaments. And unused printer "
                                       "filaments will be automatically added to the end of the list.");
        ...
        m_advanced_options_icon = new wxStaticBitmap(m_scrolledWindow, wxID_ANY,
                                                     create_scaled_bitmap("advanced_option3", m_scrolledWindow, 18), wxDefaultPosition,
        ...
        m_undone_str = _L("After being synced, the project's filament presets and colors will be replaced with the mapped filament types "
                          "and colors. This action cannot be undone.");
        ...
        m_button_cancel->Bind(wxEVT_COMMAND_BUTTON_CLICKED, [this](wxCommandEvent& e) { EndModal(wxID_CANCEL); });
...
{
    // [EVENT][THREAD][UNITY] Central event wiring: timer ticks, close handling, printer-list refresh,
    // and custom sync events all re-enter the dialog on the UI thread.
    // In Unity this wants explicit UI events plus main-thread marshaling instead of raw Bind/CallAfter traffic.
...
void SyncAmsInfoDialog::sync_ams_mapping_result(std::vector<FilamentInfo>& result)
{
    ...
    for (auto f = result.begin(); f != result.end(); f++) {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << "ams_mapping f id = " << f->id << ", tray_id = " << f->tray_id
                                << ", color = " << f->color << ", type = " << f->type;
...
bool SyncAmsInfoDialog::do_ams_mapping(MachineObject* obj_)
{
    ...
    if (nozzle_nums > 1) {
        ...
        filament_result = DevMappingUtil::ams_filament_mapping(obj_, m_filaments, m_ams_mapping_result, map_opt, std::vector<int>(),
                                                               wxGetApp().app_config->get_bool("ams_sync_match_full_use_color_dist") ?
                                                                   false :
                                                                   true);
    }
    // single nozzle
...
void SyncAmsInfoDialog::deal_only_exist_ext_spool(MachineObject* obj_)
{
    ...
    if (m_map_mode == MapModeEnum::ColorMap) {
        m_tip_attention_color_map = only_exist_ext_spool_flag ?
                                        _L("Only synchronize filament type and color, not including slot information.") :
...
void SyncAmsInfoDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
    if (m_print_status != status) {
        ...
        BOOST_LOG_TRIVIAL(info) << "select_machine_dialog: show_status = " << status << "("
                                << PrePrintChecker::get_print_status_info(status) << ")";
    }
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        try {
            ...
            msg_text = wxString::Format(_L("The selected printer (%s) is incompatible with the chosen printer profile in the slicer (%s)."),
        ...
        wxString msg_text = _L(
            "You selected external and AMS filament at the same time in an extruder, you will need manually change external filament.");
        update_print_status_msg(msg_text, true, false);
    }
...
bool SyncAmsInfoDialog::is_same_nozzle_diameters(NozzleType& tag_nozzle_type, float& nozzle_diameter)
{
    ...
    try {
        ...
        const ConfigOptionEnumsGenericNullable* nozzle_type = preset_bundle->printers.get_edited_preset()
        ...
        for (size_t idx = 0; idx < obj_->GetExtderSystem()->GetTotalExtderCount(); ++idx)
            machine_nozzle_types[idx] = obj_->GetExtderSystem()->GetNozzleType(idx);

        auto used_filaments = wxGetApp().plater()->get_partplate_list().get_curr_plate()->get_used_filaments(); // 1 based
        auto filament_maps  = wxGetApp().plater()->get_partplate_list().get_curr_plate()->get_real_filament_maps(
            preset_bundle->project_config); // 1 based
...
bool SyncAmsInfoDialog::is_same_printer_model()
{
    ...
    if (preset_bundle && preset_bundle->printers.get_edited_preset().get_printer_type(preset_bundle) != obj_->printer_type) {
        ...
        BOOST_LOG_TRIVIAL(info) << "printer_model: source = "
...
{
    // [THREAD][STATE][UNITY][PORTING_HAZARD:P3] Fetches remote print info on a worker thread, then posts back
    // to mutate dialog state and repopulate the printer list. Unity should replace this with async/await or a
    // background service that emits a main-thread completion event; the weak_ptr token guards stale callbacks.
...
void SyncAmsInfoDialog::on_set_finish_mapping(wxCommandEvent& evt)
{
    ...
    if (selection_data_arr.size() == 8) {
        ...
        for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
            ...
            BOOST_LOG_TRIVIAL(info) << "The ams mapping result: id is " << m_ams_mapping_result[i].id << "tray_id is "
...
void SyncAmsInfoDialog::on_timer(wxTimerEvent& event)
{
    ...
    if (!obj_ || !obj_->GetFilaSystem()->HasAms() || obj_->ams_exist_bits == 0 || !obj_->is_support_filament_backup ||
        !obj_->GetExtderSystem()->HasFilamentBackup() || !obj_->GetFilaSystem()->IsAutoRefillEnabled() ||
...
{
    // [STATE][INTENT][PORTING_HAZARD:P2] This is the dialog's status gate: it blocks send/sync actions for
    // server, device, nozzle, SD-card, upgrade, and mapping mismatches. Unity should model this as an explicit
    // validation state machine with a separate service; the current method also drives user-visible copy.
    // refreshing return
    ...
    if (is_must_finish_slice_then_connected_printer()) {
        return;
    }
    MachineObject* obj_ = dev->get_selected_machine();
    if (!obj_) {
...
void SyncAmsInfoDialog::set_default(bool hide_some)
{
    ...
    if (m_print_type == PrintFromType::FROM_NORMAL) {
        ...
        m_cur_input_thumbnail_data = m_specify_plate_idx == -1 ?
                                         m_plater->get_partplate_list().get_curr_plate()->thumbnail_data :
...
void SyncAmsInfoDialog::reset_and_sync_ams_list()
{
    ...
    // [INTENT][STATE][UNITY] Rebuilds the custom-painted filament cards from preset/project state and wires a
    // shared popup editor per row. Unity should use a recyclable row collection plus one shared popup controller,
    // not per-item bespoke wx widgets with screen-space hit testing.
    // for black list
    ...
    for (auto i = 0; i < extruders.size(); i++) {
        ...
        if (contronal_index % SYNC_FLEX_GRID_COL == 0) {
            ...
            if (is_first_row) {
                ...
                if (!m_original_in_colormap) {
                    m_original_in_colormap = new wxStaticText(m_filament_panel, wxID_ANY,
                                                              _CTX(L_CONTEXT("Original", "Sync_AMS"), "Sync_AMS") + ":");
        ...
        // [EVENT][STATE][UNITY] Each row is a custom control that opens a single shared popup, updates mapping
        // selection, and reuses cached sync results. Unity should pair a ListView row prefab with a shared popup
        // instead of per-item mouse handlers and manual screen-position placement.
...
void SyncAmsInfoDialog::generate_override_fix_ams_list()
{
    ...
    for (auto i = 0; i < extruders.size(); i++) {
        ...
        if (contronal_index % SYNC_FLEX_GRID_COL == 0) {
            ...
            if (is_first_row) {
                ...
                if (!m_original_in_override) {
                    m_original_in_override = new wxStaticText(m_fix_filament_panel, wxID_ANY,
                                                              _CTX(L_CONTEXT("Original", "Sync_AMS"), "Sync_AMS") + ":");
        ...
        if (use_double_extruder) {
            if (m_filaments_map[extruder] == 1) {
                item = new MaterialSyncItem(m_fix_filament_panel, colour_rgb,
                                            _L(display_materials[extruder])); // m_filament_left_panel//special
...
{
    // [STATE][UNITY][PORTING_HAZARD:P2] CPU-side thumbnail recoloring/compositing uses cached edge pixels and
    // per-filament colors instead of a GPU shader. Unity should likely move this to a texture transform job or
    // shader pass driven by a retained preview model.
...
void SyncAmsInfoDialog::update_thumbnail_data_accord_plate_index(bool allow_clone_ams_color)
{
    ...
    ThumbnailData& input_data = m_specify_plate_idx == -1 ? m_plater->get_partplate_list().get_curr_plate()->thumbnail_data :
                                                            m_plater->get_partplate_list().get_plate(m_specify_plate_idx)->thumbnail_data;
    ThumbnailData& no_light_data = m_specify_plate_idx == -1 ?
                                       m_plater->get_partplate_list().get_curr_plate()->no_light_thumbnail_data :
...
// [INTENT][UNITY] These transient frames are success toasts tied to the sync workflow.
// Unity should use anchored toast/notification overlays rather than modal dialogs for these confirmations.
SyncNozzleAndAmsDialog::SyncNozzleAndAmsDialog(InputInfo& input_info)
    : BaseTransparentDPIFrame(static_cast<wxWindow*>(wxGetApp().mainframe),
                              ...
                              wxGetApp().preset_bundle->get_printer_extruder_count() == 1 ?
                                  _L("Successfully synchronized nozzle information.") :
...
void SyncNozzleAndAmsDialog::deal_cancel() { on_hide(); }
