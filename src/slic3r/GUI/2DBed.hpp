...
// [INTENT] Class Bed_2D provides a 2D preview panel.
// [UNITY] Map to Unity MonoBehaviour for UI representation.
class Bed_2D : public wxPanel
{
    static const int Border = 10;

    // [STATE] Background rendering state
    bool m_user_drawn_background = true;

    // [STATE] UI scale and coordinate state
...
public:
    ...
    static std::vector<Polylines> generate_grid(
        const ExPolygon& poly, const BoundingBox& pp_bbox, const Vec2d& origin, const double& step, const double& scale);
