...
// [INTENT] Build a shareable system summary string for both the dialog body and clipboard export.
// [UNITY] Keep this as a pure report-text formatter behind a view-model/service so the UI can reuse it without wxHtmlWindow coupling.
...
// [INTENT] Gather live memory and undo-stack stats for the process report.
// [STATE] Reads the current plater undo/redo stack and process-wide memory info from the app singleton.
// [PORTING_HAZARD:P2] This assumes main-thread access to app state; Unity should source the same values from a retained model/service
// instead of reaching into a view singleton.
std::string get_mem_info(bool format_as_html)
{
    ...
    out << b_start << "RAM size reserved for the Undo / Redo stack: " << b_end << Slic3r::format_memsize_MB(stack.get_memory_limit())
        << line_end;
    out << b_start << "RAM size occupied by the Undo / Redo stack: " << b_end << Slic3r::format_memsize_MB(stack.memsize()) << line_end
        << line_end;

    return out.str();
}

// [INTENT] Modal system-information dialog with branding, scrollable text panes, and a clipboard export action.
// [STATE] Owns the rescaled logo bitmap, the summary HTML pane, the OpenGL/process info pane, and the copy button.
// [UNITY] Map this to a modal overlay or popup with a retained text model, two scroll views, and a command button.
// [PORTING_HAZARD:P2] The dialog mixes presentation with live process inspection and graphics capability reporting, so the Unity version
// should separate data collection from the view.
SysInfoDialog::SysInfoDialog()
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe),
                wxID_ANY,
                (wxGetApp().is_editor() ? wxString(SLIC3R_APP_FULL_NAME) : wxString(GCODEVIEWER_APP_NAME)) + " - " +
                ...
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    // title
    {
        wxStaticText* title      = new wxStaticText(this, wxID_ANY, wxGetApp().is_editor() ? SLIC3R_APP_FULL_NAME : GCODEVIEWER_APP_NAME,
    ...
    m_opengl_info_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_AUTO);
    {
        ...
        // [STATE] Windows-only blacklist scan augments the report with process-level DLL hygiene details.
        // [PORTING_HAZARD:P3] This is platform-specific inspection logic, so a Unity port should keep the scan in a platform service rather
        // than the view.
        ...
        if (!blacklisted_libraries.empty())
            blacklisted_libraries_message = wxString("<br><b>") + _L("Blacklisted libraries loaded into OrcaSlicer process:") + "</b><br>" +
        ...
        const auto text = GUI::format_wxstr("<html>"
                                            ...
                                            "</body>"
                                            "</html>",
                                            bgr_clr_str, text_clr_str, text_clr_str, blacklisted_libraries_message, get_mem_info(true),
                                            wxGetApp().get_gl_info(false),
...
// [EVENT] DPI-aware refresh path: rescale the logo, rebuild font metrics, and resize the modal contents after a monitor scale change.
// [UNITY] Use a responsive layout pass that reapplies typography and image scaling instead of directly mutating widget bitmaps.
...
// [EVENT] Copy button handler that exports the same report text to the system clipboard.
// [PORTING_HAZARD:P2] Clipboard access is synchronous and main-thread bound here; Unity should route this through the platform clipboard
// API on the UI thread.
...
// [INTENT] Close the modal dialog using an explicit modal result instead of relying on window destruction.
void SysInfoDialog::onCloseDialog(wxEvent&) { this->EndModal(wxID_CLOSE); }
