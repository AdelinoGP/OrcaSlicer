...
// [ANNOTATED]
// [INTENT] CameraPopup is a transient settings overlay for camera-related machine controls (recording, resolution, virtual camera).
// [UNITY] Map to a retained UI Toolkit or uGUI panel/popup anchored to the camera preview.
// [PORTING_HAZARD:P2] PopupWindow handles complex dismissal/focus-loss logic that must be replicated in Unity (click-outside-to-close).

namespace Slic3r { namespace GUI {

// [EVENT] Custom events for camera state changes and hints.
...
public:
    ...
    // [STATE] Supported resolutions for the physical camera.
    enum CameraResolution { RESOLUTION_720P = 0, RESOLUTION_1080P = 1, RESOLUTION_OPTIONS_NUM = 2 };
...
private:
    // [STATE] MachineObject* obj is the non-owning reference to the printer state.
    // [UNITY] Replace with a reference to a MachineObject MonoBehaviour or ScriptableObject.
    MachineObject* m_obj{nullptr};

    // [THREAD] Interval timer for debouncing or periodic state checks.
    wxTimer* m_interval_timer{nullptr};
    bool     m_is_in_interval{false};

    // [STATE] UI elements for recording, virtual camera, and liveview.
...
// [INTENT] CameraItem represents a selectable camera source or item in a list.
