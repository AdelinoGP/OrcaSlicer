...
// [INTENT] Cross-platform media-control shim for the GUI's video playback surfaces.
// It hides backend differences between Windows Media Player, GStreamer, and the app's custom
// BambuSource integration so callers only see the wxMediaCtrl2 facade.
// [UNITY] Unity analogue would be a small native-plugin-backed media service plus a MonoBehaviour
// controller that exposes load/play/stop/error state to UI code.
#ifdef __WIN32__
#include <winuser.h>
...
class WXDLLIMPEXP_MEDIA wxGStreamerMediaBackend : public wxMediaBackendCommonBase
{
public:
...
// [STATE] The wrapper owns no long-lived media resources directly; it forwards to wxMediaCtrl
// / platform backend state and mirrors only the error/loaded flags exposed through the header.
// [PORTING_HAZARD:P2] The Windows path mutates registry-backed media configuration and depends on
// external codecs/components, which does not map cleanly to Unity's managed runtime.
wxMediaCtrl2::wxMediaCtrl2(wxWindow* parent)
{
#ifdef __WIN32__
    // [EVENT] Windows startup probes GPU vendor hints before the media control is created so the
    // playback surface can prefer the discrete adapter when available.
...
#ifdef __LINUX__
    // [THREAD] The GTK backend requires the control to exist before plugin registration; this is
    // effectively a backend-initialization barrier before any UI can load media.
    // [EVENT] The loaded callback flips m_loaded so the UI can distinguish real playback from
    // a still-initializing pipeline.
...
{
    // [INTENT] Load validates platform prerequisites first, then delegates to wxMediaCtrl::Load
    // once the backend-specific setup (registry, codecs, plugin probes) is satisfied.
#ifdef __WIN32__
    // [PORTING_HAZARD:P1] The Windows branch depends on WMP being installed and on a custom CLSID
    // / registry registration for BambuSource; Unity should treat this as an external capability
    // check and a native-plugin contract, not as in-app state.
    InvalidateBestSize();
    if (m_imp == nullptr) {
        static bool notified = false;
        if (!notified)
            CallAfter([] {
                auto res = wxMessageBox(_L("Windows Media Player is required for this task! Do you want to enable 'Windows Media Player' "
                                           "for your operation system?"),
                                        _L("Error"), wxOK | wxCANCEL);
                if (res == wxOK) {
                    wxString url =
    ...
    }
    {
        ...
        if (path.empty() || !wxFile::Exists(path) || clsid != CLSID_BAMBU_SOURCE) {
            if (boost::filesystem::exists(dll_path)) {
                CallAfter([dll_path] {
                    int res = wxMessageBox(_L("BambuSource has not correctly been registered for media playing! Press Yes to re-register "
                                              "it. You will be promoted twice"),
                                           _L("Error"), wxYES_NO);
                    if (res == wxYES) {
                ...
                CallAfter([] {
                    wxMessageBox(_L("Missing BambuSource component registered for media playing! Please re-install BambuStudio or seek "
                                    "after-sales help."),
                                 _L("Error"), wxOK);
                });
            }
            ...
            if (!notified)
                CallAfter([dll_path] {
                    int res = wxMessageBox(
                        _L("Using a BambuSource from a different install, video play may not work correctly! Press Yes to fix it."),
                        _L("Warning"), wxYES_NO | wxICON_WARNING);
                    if (res == wxYES) {
    ...
    url = wxURI(url.BuildURI()
                    .append("&hwnd=")
                    .append(boost::lexical_cast<std::string>(GetHandle()))
                    .append("&tid=")
                    .append(boost::lexical_cast<std::string>(GetCurrentThreadId())));
#endif
#ifdef __WXGTK3__
    // [STATE] GStreamer codec availability is treated as preflight state; if H.264 support is
    // missing, the control emits a failure state instead of attempting playback.
    ...
    if (!hasplugins) {
        CallAfter([] {
            wxMessageBox(_L("Your system is missing H.264 codecs for GStreamer, which are required to play video. (Try installing the "
    ...
    m_error  = 0;
    m_loaded = false;
    // [EVENT] wxMediaCtrl::Load starts the asynchronous media pipeline; completion/error is
    // surfaced later through state change events and platform-specific callbacks.
...
// [INTENT] Thin pass-through controls keep the external API stable while the real behavior lives
// in the backend-specific wxMediaCtrl implementation.
...
// [STATE] GetLastError is the UI-visible failure latch; Linux reads a backend global while other
// platforms use the wrapper's local error code.
...
// [INTENT] Video size is used as a layout hint for the media view; Linux fakes a size until
// playback starts because GStreamer delays real dimensions.
// [UNITY] This would become a preferred-aspect-ratio hint on a RectTransform / layout element,
// not a hard dependency on an actual decoded frame size.
...
// [INTENT] The control reports no intrinsic best size so callers can size it from surrounding UI.
wxSize wxMediaCtrl2::DoGetBestSize() const { return {-1, -1}; }

#ifdef __WIN32__

// [EVENT] Windows messages from the media backend are parsed into error codes and stat telemetry,
// then re-posted to the wx event queue so the UI thread can react safely.
// [THREAD] WM_USER + 1000 is an inter-thread signal path from the backend into the GUI thread.
WXLRESULT wxMediaCtrl2::MSWWindowProc(WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam)
{
