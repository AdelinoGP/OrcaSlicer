...
// [EVENT] Emit these wxWidgets events to drive the main UI from toolbar actions, keeping GL-focus handling in sync.
// [UNITY] Unity will recreate these as MonoBehaviour events or UnityEvents on a ToolbarController, letting each command bind to a
// coroutine/job pipeline. [PORTING_HAZARD:P2] This tight coupling to wx event IDs must be remapped to Unity's execution graph before the
// toolbar can trigger slices, prints, or uploads.
...
// [STATE] The toolbar item defaults keep callbacks inert/visible so items can register their own hooks without leaving uninitialized state behind.

// [STATE] Toggleable options start disabled so UI state only flips when explicit callbacks set toggable=true.
GLToolbarItem::Data::Option::Option() : toggable(false), action_callback(Default_Action_Callback), render_callback(nullptr) {}

// [INTENT] Data constructor zeroes metadata and keeps callbacks bound while the toolbar populates icons.
...
GLToolbarItem::GLToolbarItem(GLToolbarItem::EType type, const GLToolbarItem::Data& data)
    : m_type(type), m_state(Normal), m_data(data), m_last_action_type(Undefined), m_highlight_state(NotHighlighted)
{
...
// BBS: GUI refactor: GLToolbar
// [OPENGL] Draws cached button text from a texture atlas; Unity can mimic this with a TextMeshPro atlas or UI Toolkit label drawing.
void GLToolbarItem::render_text(float left, float right, float bottom, float top) const
{
    ...
    GLTexture::render_sub_texture(m_data.text_texture.get_id(), left, right, bottom, top,
...
// [OPENGL] Render the icon quad with highlight states; this is the core GL draw path Unity must replicate when drawing icons.
void GLToolbarItem::render(unsigned int tex_id,
                           float        left,
                           float        right,
                           float        bottom,
                           float        top,
...
// [OPENGL] Builds the quad for externally provided image data and overlays activation highlights with callbacks.
void GLToolbarItem::render_image(unsigned int tex_id,
                                 float        left,
                                 float        right,
                                 float        bottom,
                                 float        top,
...
BackgroundTexture::Metadata::Metadata() : filename(""), left(0), right(0), top(0), bottom(0) {}
...
// [INTENT] GLToolbar ties icon data, layout, and GL textures to mouse events so the canvas can expose toolbar commands.
GLToolbar::GLToolbar(GLToolbar::EType type, const std::string& name)
    : m_type(type), m_name(name), m_enabled(false), m_icons_texture_dirty(true), m_pressed_toggable_id(-1)
{}
...
// [STATE] Load the background nine-patch texture so we can draw the chrome once per layout change.
...
// [STATE] The arrow texture is reused for highlight indicators so we avoid redundant GL uploads.
...
GLToolbar::Layout::EType GLToolbar::get_layout_type() const { return m_layout.type; }
...
void GLToolbar::select_item(const std::string& name)
{
    ...
    for (GLToolbarItem* item : m_items) {
        if (!item->is_disabled()) {
            ...
            item->set_state((item->get_name() == name) ? (hover ? GLToolbarItem::HoverPressed : GLToolbarItem::Pressed) :
                                                         (hover ? GLToolbarItem::Hover : GLToolbarItem::Normal));
        }
    }
}
...
// [STATE] One helper to recalc visibility/enabled/pressed states; keeps m_pressed_toggable_id consistent for toggled buttons.
...
// [OPENGL] Central render entry ensures icons/texture atlases draw after layout/visibility updates.
// [UNITY] Unity port should replace this with a Canvas + RawImage batch driven by a ToolbarRenderer MonoBehaviour.
void GLToolbar::render(const GLCanvas3D& parent, GLToolbarItem::EType type)
{
    ...
    case Layout::Horizontal: {
        render_horizontal(parent, type);
        break;
    }
    case Layout::Vertical: {
        render_vertical(parent);
        break;
    }
    }
}

// [EVENT] Mouse events run entirely on the UI thread and rely on m_mouse_capture to keep interactions bounded.
// [THREAD] Input handling must stay on the main thread or Unity port will need to marshal via SynchronizationContext.
...
float GLToolbar::get_height_horizontal() const { return (2.0f * m_layout.border + m_layout.icons_size) * m_layout.scale; }

float GLToolbar::get_height_vertical() const { return get_main_size(); }
...
// [EVENT] do_action orchestrates mouse-triggered callbacks and syncs hover/pressed state before dispatching to jobs.
// [PORTING_HAZARD:P2] This mixes GL rendering with state transitions, so Unity will need to split the logic between UI events and the JobController.
        ...
        if (0 <= item_id && item_id < (int) m_items.size()) {
            ...
            if (item != nullptr && !item->is_separator() && !item->is_disabled() && (!check_hover || item->is_hovered())) {
                if ((type == GLToolbarItem::Right && item->is_right_toggable()) ||
                    ...
                    switch (type) {
                    default:
                    case GLToolbarItem::Left: {
                        item->do_left_action();
                        break;
                    }
                    case GLToolbarItem::Right: {
                        item->do_right_action();
                        break;
                    }
                    }
                } else {
...
// [EVENT] Hover tracking keeps icon states in sync with the cursor, so tooltips reflect state without extra timers.
void GLToolbar::update_hover_state(const Vec2d& mouse_pos, GLCanvas3D& parent)
{
    ...
    case Layout::Vertical: {
        update_hover_state_vertical(mouse_pos, parent);
        break;
    }
    }
}
...
void GLToolbar::update_hover_state_horizontal(const Vec2d& mouse_pos, GLCanvas3D& parent)
{
    ...
    const Vec2d scaled_mouse_pos((mouse_pos.x() - 0.5 * (double) cnv_size.get_width()),
                                 (0.5 * (double) cnv_size.get_height() - mouse_pos.y()));
    ...
    for (GLToolbarItem* item : m_items) {
        ...
        if (item->is_separator())
            ...
            bool                        inside = (left <= (float) scaled_mouse_pos.x()) && ((float) scaled_mouse_pos.x() <= right) &&
...
void GLToolbar::update_hover_state_vertical(const Vec2d& mouse_pos, GLCanvas3D& parent)
{
    ...
    for (GLToolbarItem* item : m_items) {
        ...
        if (item->is_separator())
            ...
            GLToolbarItem::EState state  = item->get_state();
            const bool            inside = (left <= (float) scaled_mouse_pos.x()) && ((float) scaled_mouse_pos.x() <= right) &&
                                (bottom <= (float) scaled_mouse_pos.y()) && ((float) scaled_mouse_pos.y() <= top);
...
// [EVENT] contains_mouse transforms raw canvas positions into toolbar item ids so clicks hit the right command.
int GLToolbar::contains_mouse(const Vec2d& mouse_pos, const GLCanvas3D& parent) const
{
    ...
    case Layout::Vertical: {
        return contains_mouse_vertical(mouse_pos, parent);
    }
    }
}
...
int GLToolbar::contains_mouse_horizontal(const Vec2d& mouse_pos, const GLCanvas3D& parent) const
{
    ...
    for (size_t id = 0; id < m_items.size(); ++id) {
        ...
        if (item->is_separator()) {
            ...
            if (left <= (float) scaled_mouse_pos.x() && (float) scaled_mouse_pos.x() <= right && bottom <= (float) scaled_mouse_pos.y() &&
...
// [OPENGL] Nine-slice background uses the configured metadata so the chrome scales with toolbar size.
...
// [OPENGL] Draws the arrow indicator using its own texture so the highlighted item is obvious when multiple toolbars interleave.
void GLToolbar::render_arrow(const GLCanvas3D& parent, GLToolbarItem* highlighted_item)
{
    ...
    if (tex_id != 0 && arr_tex_width > 0.0f && arr_tex_height > 0.0f) {
        ...
        GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_bottom, internal_top,
                                      {{left_uv, top_uv}, {right_uv, top_uv}, {right_uv, bottom_uv}, {left_uv, bottom_uv}});
    }
}

// [OPENGL] Horizontal layout writes all icon quads/UVs into a single atlas, so Unity needs a similar batching or DrawMesh setup to avoid
// per-icon draw calls.
        ...
        if (item->is_separator())
            ...
            if (!item->is_action_with_text_image()) {
                ...
                if ((tex_id == 0) || (tex_width <= 0) || (tex_height <= 0))
                    return;
                item->render(tex_id, left, left + icons_size_x, top - icons_size_y, top, (unsigned int) tex_width,
                             (unsigned int) tex_height, (unsigned int) (m_layout.icons_size * m_layout.scale));
            }
            // BBS: GUI refactor: GLToolbar
            if (item->is_action_with_text()) {
...
// [OPENGL] Vertical layout mirrors icons in a column; porting equivalent will need to mirror the layout math or reuse a UI Toolkit VerticalLayout.
    ...
    for (const GLToolbarItem* item : m_items) {
        ...
        if (item->is_separator())
            ...
            if (item->is_action_with_text_image()) {
                ...
                if ((tex_id == 0) || (tex_width <= 0) || (tex_height <= 0))
                    return;
                item->render_image(tex_id, image_left, image_left + icons_size_x, top - icons_size_y, top, (unsigned int) tex_width,
                                   (unsigned int) tex_height, (unsigned int) (m_layout.icons_size * m_layout.scale));
            } else {
...
// [OPENGL] Builds the sprite atlas for all icons/events; Unity needs a similar atlas generation stage or an equivalent Texture2D array.
// [PORTING_HAZARD:P3] This helper assumes SVG inputs and GLTexture::load_from_svg_files_as_sprites_array, which need to be replaced with
// Unity Texture2D import logic.
...
// [STATE] Periodically refresh visibility to hide/show separators and keep layout dirty when icons hide.
...
// [STATE] Sync enabled/disabled state so toggable buttons and radio groups respect current job state.
...
// BBS: GUI refactor: GLToolbar
// [OPENGL] Build cached text textures so glyph rendering happens beforehand and doesn't stall the render loop.
...
// [OPENGL] Image textures are produced as part of layout prep rather than per-frame draws.
...
// BBS: GUI refactor: GLToolbar
// [STATE] Other systems query the scaled icon size to align custom widgets or highlight buffers.
float GLToolbar::get_scaled_icon_size() { return m_layout.icons_size * m_layout.scale; }
