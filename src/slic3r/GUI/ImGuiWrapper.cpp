...
// [STATE] Mapping of custom wide characters to internal icon identifiers for ImGui rendering.
...
const ImVec4 ImGuiWrapper::COL_GREEN_LIGHT    = {0.f, 156 / 255.f, 136 / 255.f,
                                                 0.25f}; // ORCA used on various places like text selection bg. Replaced with orca color
...
const ImVec4 ImGuiWrapper::COL_TOOLBAR_BG_DARK = {57 / 255.f, 60 / 255.f, 66 / 255.f,
                                                  1.f}; // ORCA color matches with toolbar_background_dark.png
...
int ImGuiWrapper::TOOLBAR_WINDOW_FLAGS = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
                                         ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar;

// [INTENT] Global utility to load SVG data into ThumbnailData for use with ImGui textures.
// [UNITY] Use Unity's Vector Graphics package or a custom SVG importer to generate Textures/Sprites.
bool get_data_from_svg(const std::string& filename, unsigned int max_size_px, ThumbnailData& thumbnail_data)
{
    ...
    nsvgRasterize(rast, image, 0, 0, scale, thumbnail_data.pixels.data(), thumbnail_data.width, thumbnail_data.height,
                  thumbnail_data.width * 4);
...
// [INTENT] Custom slider behavior implementation for ImGui.
// [UNITY] Can be replaced by Unity UI Slider component or a custom IMGUI/UI Toolkit slider.
bool slider_behavior(ImGuiID          id,
                     const ImRect&    region,
                     const ImS32      v_min,
                     const ImS32      v_max,
                     ImS32*           out_value,
                     ...
                     const ImVec4&    fixed_rect /* = ImRect()*/)
{
    ...
    if (context.ActiveId == id) {
        ...
        if (context.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (context.IO.MouseDown[0]) {
                ...
                mouse_pos_ratio           = (region_usable_sz > 0.0f) ?
                                                ImClamp((mouse_abs_pos - region_usable_pos_min) / region_usable_sz, 0.0f, 1.0f) :
                                                0.0f;
                if (axis == ImGuiAxis_Y)
    ...
    ImVec2 new_handle_center = axis == ImGuiAxis_Y ? ImVec2(out_handle->GetCenter().x, handle_pos) :
                                                     ImVec2(handle_pos, out_handle->GetCenter().y);
                     ...
                     const ImVec2& margin)
{
    ...
    window->DrawList->AddImage(user_texture_id, bb.Min + padding + margin, bb.Max - padding - margin, uv0, uv1,
...
{
    // [INTENT] Initialize ImGui context and internal state.
    ImGui::CreateContext();
...
ImGuiWrapper::~ImGuiWrapper()
{
    // [INTENT] Clean up ImGui context and fonts.
    // [OPENGL] Font textures should be destroyed before the context is gone.
...
{
    // [INTENT] Configures ImGui font ranges based on application language.
    // [STATE] Updates m_glyph_ranges and triggers font re-creation.
    if (m_new_frame_open) {
...
{
    // [INTENT] Adjusts ImGui scaling for high-DPI displays.
    // [STATE] Updates m_font_size and m_style_scaling.
...
{
    // [INTENT] Bridges wxWidgets mouse events into ImGui IO state.
    // [EVENT] Propagates position, buttons, and wheel delta.
...
{
    // [INTENT] Bridges wxWidgets key events into ImGui IO state.
    // [EVENT] Propagates characters and modifier keys.
    if (!display_initialized()) {
        return false;
    }
...
{
    // [INTENT] Starts a new ImGui frame.
    // [STATE] Ensures font textures are loaded into the GPU.
    // [UNITY] Corresponds to Update() or OnGUI().
    if (m_new_frame_open) {
        return;
    }
...
{
    // [INTENT] Finalizes the ImGui frame and issues OpenGL draw calls.
    // [OPENGL] Executes render_draw_data with the generated vertex/index buffers.
    ImGui::Render();
...
// BBL: copy & paste form prusa github repo
// (https://github.com/prusa3d/PrusaSlicer/blob/master/src/slic3r/GUI/ImGuiWrapper.cpp#L375C5-L402C6) synchronize key states when the
// application loses the focus it may happen that the key up event is not processed

// synchronize modifier keys
constexpr std::array<std::pair<ImGuiKeyModFlags_, wxKeyCode>, 3> imgui_mod_keys{std::make_pair(ImGuiKeyModFlags_Ctrl, WXK_CONTROL),
...
ImVec2 ImGuiWrapper::calc_text_size(std::string_view text, bool hide_text_after_double_hash, float wrap_width)
{
    return ImGui::CalcTextSize(text.data(), text.data() + text.length(), hide_text_after_double_hash, wrap_width);
}

ImVec2 ImGuiWrapper::calc_text_size(const std::string& text, bool hide_text_after_double_hash, float wrap_width)
{
    return ImGui::CalcTextSize(text.c_str(), NULL, hide_text_after_double_hash, wrap_width);
}

ImVec2 ImGuiWrapper::calc_text_size(const wxString& text, bool hide_text_after_double_hash, float wrap_width)
{
...
void ImGuiWrapper::set_next_window_bg_alpha(float alpha) { ImGui::SetNextWindowBgAlpha(alpha); }

void ImGuiWrapper::set_next_window_size(float x, float y, ImGuiCond cond) { ImGui::SetNextWindowSize(ImVec2(x, y), cond); }

/* BBL style widgets */
bool ImGuiWrapper::bbl_combo_with_filter(const char*                     label,
                                         const std::string&              preview_value,
                                         const std::vector<std::string>& all_items,
                                         ...
                                         float                           item_height)
{
    ...
    ImGui::PopStyleVar();
    ImGui::PopStyleColor();
    ImGui::BBLRenderArrow(window->DrawList,
                          arrow_bb.Min +
                              ImVec2(ImMax(0.0f, (arrow_size.x - g.FontSize) * 0.5f), ImMax(0.0f, (arrow_size.y - g.FontSize) * 0.5f)),
...
bool ImGuiWrapper::bbl_slider_float_style(
    const std::string& label, float* v, float v_min, float v_max, const char* format, float power, bool clamp, const wxString& tooltip)
{
...
bool ImGuiWrapper::bbl_slider_float(
    const std::string& label, float* v, float v_min, float v_max, const char* format, float power, bool clamp, const wxString& tooltip)
{
...
bool ImGuiWrapper::begin(const std::string& name, int flags) { return ImGui::Begin(name.c_str(), nullptr, (ImGuiWindowFlags) flags); }

bool ImGuiWrapper::begin(const wxString& name, int flags) { return begin(into_u8(name), flags); }
...
bool ImGuiWrapper::begin(const wxString& name, bool* close, int flags) { return begin(into_u8(name), close, flags); }

void ImGuiWrapper::end() { ImGui::End(); }
...
ImVec4 ImGuiWrapper::to_ImVec4(const ColorRGB& color) { return {color.r(), color.g(), color.b(), 1.0}; }
...
void ImGuiWrapper::text(const char* label) { ImGui::Text("%s", label); }

void ImGuiWrapper::text(const std::string& label) { ImGuiWrapper::text(label.c_str()); }
...
void ImGuiWrapper::text_colored(const ImVec4& color, const char* label) { ImGui::TextColored(color, "%s", label); }

void ImGuiWrapper::text_colored(const ImVec4& color, const std::string& label) { ImGuiWrapper::text_colored(color, label.c_str()); }
...
void ImGuiWrapper::text_wrapped(const std::string& label, float wrap_width) { this->text_wrapped(label.c_str(), wrap_width); }
...
void ImGuiWrapper::tooltip(const std::string& label, float wrap_width) { tooltip(label.c_str(), wrap_width); }
...
#if ENABLE_ENHANCED_IMGUI_SLIDER_FLOAT
ImVec2 ImGuiWrapper::get_slider_icon_size() const { return this->calc_button_size(std::wstring(&ImGui::SliderFloatEditBtnIcon, 1)); }

bool ImGuiWrapper::slider_float(const char*     label,
                                float*          v,
                                float           v_min,
                                float           v_max,
...
bool ImGuiWrapper::slider_float(const std::string& label,
                                float*             v,
                                float              v_min,
                                float              v_max,
                                const char*        format /* = "%.3f"*/,
...
bool ImGuiWrapper::slider_float(const wxString& label,
                                float*          v,
                                float           v_min,
                                float           v_max,
                                const char*     format /* = "%.3f"*/,
...
#else
bool ImGuiWrapper::slider_float(
    const char* label, float* v, float v_min, float v_max, const char* format /* = "%.3f"*/, float power /* = 1.0f*/, bool clamp /*= true*/)
{
...
bool ImGuiWrapper::image_button(ImTextureID      user_texture_id,
                                const ImVec2&    size,
                                const ImVec2&    uv0,
                                const ImVec2&    uv1,
                                int              frame_padding,
...
bool ImGuiWrapper::combo(const wxString&                 label,
...
bool ImGuiWrapper::combo(const std::string&              label,
                         const std::vector<std::string>& options,
                         int&                            selection,
                         ImGuiComboFlags                 flags /* = 0*/,
                         float                           label_width /* = 0.0f*/,
...
bool ImGuiWrapper::undo_redo_list(const ImVec2& size,
                                  const bool    is_undo,
                                  bool (*items_getter)(const bool, int, const char**),
                                  int& hovered,
                                  int& selected,
                                  int& mouse_wheel)
{
...
static bool selectable(
    const char* label, bool selected, ImGuiSelectableFlags flags = 0, const ImVec2& size_arg = ImVec2(0, 0), bool* out_hovered = nullptr)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool begin_menu(const char* label, bool enabled)
{
    ...
    // Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus and not allow
    ...
    // We are relying on a O(N) search - so O(N log N) over the frame - which seems like the most efficient for the expected small amount of
    // BeginMenu() calls per frame. If somehow this is ever becoming a problem we can switch to use e.g. ImGuiStorage mapping key to last
    // frame used.
    if (g.MenusIdSubmittedThisFrame.contains(id)) {
        if (menu_is_open)
            menu_is_open =
                ImGui::BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
        else
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        window->DC.CursorPos.x += IM_FLOOR(
            style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It
        ...
        ImGui::RenderArrow(window->DrawList, pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, 0.0f), text_col,
                           ImGuiDir_Right);
    }
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL && !(window->Flags & ImGuiWindowFlags_MenuBar)) {
            ...
            tb.y = ta.y + ImMax((tb.y - extra) - ta.y, -100.0f); // triangle is maximum 200 high to limit the slope and the bias toward
                                                                 // large sub-menus // FIXME: Multiply by fb_scale?
    ...
    if (!enabled) // explicitly close if an open menu becomes disabled, facilitate users code a lot in pattern such as 'if
    ...
    if (menu_is_open) {
        ImGui::SetNextWindowPos(popup_pos,
                                ImGuiCond_Always); // Note: this is super misleading! The value will serve as reference for
...
bool menu_item_with_icon(const char* label,
                         const char* shortcut,
                         ...
                         bool*       hovered /* = nullptr*/)
{
    ...
    // We've been using the equivalent of ImGuiSelectableFlags_SetNavIdOnHover on all Selectable() since early Nav system days (commit
    // 43ee5d73), but I am unsure whether this should be kept at all. For now moved it to be an opt-in feature used by menus only.
    ...
    bool pressed;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        // Mimic the exact layout spacing of BeginMenu() to allow MenuItem() inside a menu bar, which is a little misleading but may be
        // useful Note that in this situation: we don't render the shortcut, we render a highlight instead of the selected tick mark.
        float w = label_size.x;
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        ...
        if (icon_size.x != 0 && icon_size.y != 0) {
            ...
            if (icon_color != 0)
                ...
                window->DrawList->AddImage(transparent, icon_pos, icon_pos + icon_size, {0, 0}, {1, 1},
        ...
        if (selected) {
            // ImGui::RenderCheckMark(window->DrawList, pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f,
...
void ImGuiWrapper::search_list(const ImVec2& size_,
                               bool (*items_getter)(int, const char** label, const char** tooltip),
                               char*                         search_str,
                               ...
                               bool                          is_localized)
{
    ...
    process_key_down(ImGuiKey_Enter, [&selected, hovered_id]() { selected = hovered_id; });
...
bool ImGuiWrapper::want_mouse() const { return ImGui::GetIO().WantCaptureMouse; }

bool ImGuiWrapper::want_keyboard() const { return ImGui::GetIO().WantCaptureKeyboard; }

bool ImGuiWrapper::want_text_input() const { return ImGui::GetIO().WantTextInput; }
...
ImFontAtlasCustomRect* ImGuiWrapper::GetTextureCustomRect(const wchar_t& tex_id)
{
    ...
    return (item != m_custom_glyph_rects_ids.end()) ? ImGui::GetIO().Fonts->GetCustomRectByIndex(m_custom_glyph_rects_ids[tex_id]) :
                                                      nullptr;
}

void ImGuiWrapper::disable_background_fadeout_animation() { GImGui->DimBgRatio = 1.0f; }

ImU32 ImGuiWrapper::to_ImU32(const ColorRGBA& color) { return ImGui::GetColorU32({color.r(), color.g(), color.b(), color.a()}); }

ImVec4 ImGuiWrapper::to_ImVec4(const ColorRGBA& color) { return {color.r(), color.g(), color.b(), color.a()}; }

ColorRGBA ImGuiWrapper::from_ImU32(const ImU32& color) { return from_ImVec4(ImGui::ColorConvertU32ToFloat4(color)); }

ColorRGBA ImGuiWrapper::from_ImVec4(const ImVec4& color) { return {color.x, color.y, color.z, color.w}; }
...
bool ImGuiWrapper::input_optional_int(
    const char* label, std::optional<int>& v, int step, int step_fast, ImGuiInputTextFlags flags, int def_val)
{
...
bool ImGuiWrapper::input_optional_float(
    const char* label, std::optional<float>& v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags, float def_val)
{
    auto                              func = [&](float& value) { return ImGui::InputFloat(label, &value, step, step_fast, format, flags); };
    std::function<bool(const float&)> is_default = [def_val](const float& value) -> bool {
        return std::fabs(value - def_val) <= std::numeric_limits<float>::epsilon();
    };
    return input_optional(v, func, is_default, def_val);
}

bool ImGuiWrapper::drag_optional_float(
    const char* label, std::optional<float>& v, float v_speed, float v_min, float v_max, const char* format, float power, float def_val)
{
...
ImVec2 ImGuiWrapper::suggest_location(const ImVec2& dialog_size, const Slic3r::Polygon& interest, const ImVec2& canvas_size)
{
    ...
    Points window_polygon = {offset.cast<coord_t>(), Point(offset.x(), offset.y() + dialog_size.y),
                             Point(offset.x() + dialog_size.x, offset.y() + dialog_size.y), Point(offset.x() + dialog_size.x, offset.y())};
...
bool ImGuiWrapper::is_char_in_ranges(const ImWchar* ranges, unsigned int letter)
{
    for (const ImWchar* range = ranges; range[0] && range[1]; range += 2) {
        ...
        if (from <= letter && letter <= to)
            return true;
        if (letter < to)
            return false; // ranges should be sorted
    }
    return false;
};
...
std::vector<unsigned char> ImGuiWrapper::load_svg(
    const std::string& bitmap_name, unsigned target_width, unsigned target_height, unsigned* outwidth, unsigned* outheight)
{
...
void ImGuiWrapper::on_change_color_mode(bool is_dark) { m_is_dark_mode = is_dark; }
...
void ImGuiWrapper::pop_confirm_button_style() { ImGui::PopStyleColor(6); }
...
void ImGuiWrapper::pop_cancel_button_style() { ImGui::PopStyleColor(6); }
...
void ImGuiWrapper::pop_button_disable_style() { ImGui::PopStyleColor(3); }
...
void ImGuiWrapper::push_radio_style(const float scale)
{
    ...
    if (m_is_dark_mode) {
        ImGui::PushStyleColor(ImGuiCol_CheckMark,
                              to_ImVec4(decode_color_to_float_array("#009688")));                  // ORCA use orca color for radio buttons
...
void ImGuiWrapper::load_fonts_texture()
{
    ...
    //             continue;
    //         }
    //         ImFont* im_font = io.Fonts->AddFontFromFileTTF(sys_font.second.c_str(), m_font_size, &cfg,
    //         ImGui::GetIO().Fonts->GetGlyphRangesBasic()); if (im_font == nullptr) {
...
void ImGuiWrapper::init_style()
{
    ...
    auto set_color = [&](ImGuiCol_ entity, ImVec4 color) { style.Colors[entity] = color; };
...
{
    // [INTENT] Low-level OpenGL rendering of ImGui draw commands.
    // [OPENGL] Manages GPU state: backups current state, sets up blending/scissor/viewport for UI,
    // and restores state after draw calls. Uses "imgui" shader.
    // [PORTING_HAZARD:P2] Raw OpenGL state management is complex here. Unity's CommandBuffer or
    // UI Toolkit handles this abstraction much more cleanly.
    ...
    glsafe(::glGetIntegerv(GL_ACTIVE_TEXTURE, (GLint*) &last_active_texture));
    GLuint last_program;
    glsafe(::glGetIntegerv(GL_CURRENT_PROGRAM, (GLint*) &last_program));
    GLuint last_texture;
    glsafe(::glGetIntegerv(GL_TEXTURE_BINDING_2D, (GLint*) &last_texture));
    ...
    glsafe(::glGetIntegerv(GL_SCISSOR_BOX, last_scissor_box));
    GLenum last_blend_src_rgb;
    glsafe(::glGetIntegerv(GL_BLEND_SRC_RGB, (GLint*) &last_blend_src_rgb));
    GLenum last_blend_dst_rgb;
    glsafe(::glGetIntegerv(GL_BLEND_DST_RGB, (GLint*) &last_blend_dst_rgb));
    GLenum last_blend_src_alpha;
    glsafe(::glGetIntegerv(GL_BLEND_SRC_ALPHA, (GLint*) &last_blend_src_alpha));
    GLenum last_blend_dst_alpha;
    glsafe(::glGetIntegerv(GL_BLEND_DST_ALPHA, (GLint*) &last_blend_dst_alpha));
    GLenum last_blend_equation_rgb;
    glsafe(::glGetIntegerv(GL_BLEND_EQUATION_RGB, (GLint*) &last_blend_equation_rgb));
    GLenum last_blend_equation_alpha;
    glsafe(::glGetIntegerv(GL_BLEND_EQUATION_ALPHA, (GLint*) &last_blend_equation_alpha));
    ...
    ortho_projection << 2.0f / (R - L), 0.0f, 0.0f, (R + L) / (L - R), 0.0f, 2.0f / (T - B), 0.0f, (T + B) / (B - T), 0.0f, 0.0f, -1.0f,
    ...
    for (int n = 0; n < draw_data->CmdListsCount; ++n) {
        ...
        if (position_id != -1) {
            glsafe(::glVertexAttribPointer(position_id, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert),
        ...
        if (color_id != -1) {
            glsafe(::glVertexAttribPointer(color_id, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert),
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; ++cmd_i) {
            ...
            if (pcmd->UserCallback)
                ...
                glsafe(::glScissor((int) clip_min.x, (int) (fb_height - clip_max.y), (int) (clip_max.x - clip_min.x),
                                   (int) (clip_max.y - clip_min.y)));

                // Bind texture, Draw
                glsafe(::glBindTexture(GL_TEXTURE_2D, (GLuint) (intptr_t) pcmd->GetTexID()));
                glsafe(::glDrawElements(GL_TRIANGLES, (GLsizei) pcmd->ElemCount,
...
void ImGuiWrapper::filament_group(const std::string& filament_type, const char* hex_color, unsigned char filament_id, float align_width)
{
    ...
    ImGui::BeginGroup();
    {
        ...
        draw_list->AddImage(transparent, cursor_pos, {cursor_pos.x + img_size.x, cursor_pos.y + img_size.y}, {0, 0}, {1, 1},
                            ImGui::GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
        // image border test
        ...
        ImGui::SetCursorPos(
            {current_cursor.x + (img_size.x - id_text_size.x) * 0.5f, current_cursor.y + (img_size.y - id_text_size.y) * 0.5f});
        ...
        float text_x_offset = is_multiline ? (img_size.x - wrapped_text_size.x) * 0.5f + 2.f :
                                             (img_size.x - wrapped_text_size.x) * 0.5f + 2.f;

        auto cursor_x_before_text = ImGui::GetCursorPosX();
        current_cursor            = ImGui::GetCursorPos();
        ImGui::SetCursorPos({current_cursor.x + text_x_offset, current_cursor.y + text_y_offset});
...
void ImGuiWrapper::sub_title(const std::string& label)
{
    ...
    draw_list->AddLine(ImVec2(cursor_pos.x, cursor_pos.y + 8.0f), ImVec2(cursor_pos.x + available_width, cursor_pos.y + 8.0f),
