...
enum TrayType { NORMAL, THIRD, EMPTY };
...
public:
    ...
    void set_ams_info(
        wxColour col, wxString txt, int ctype = 0, std::vector<wxColour> cols = std::vector<wxColour>(), bool record_back_info = false);
    void reset_ams_info();
...
{
    // [INTENT] Popup that maps sliced filament entries onto live AMS and external spool slots before send/sync actions.
    // [STATE] Owns transient tray widgets plus the currently selected filament/material index, so callers treat it as short-lived UI state.
    // [PORTING_HAZARD:P1] The behavior depends on wx popup dismissal, manual hit testing, and dynamic sizer composition that do not map 1:1
    // to Unity popups.
