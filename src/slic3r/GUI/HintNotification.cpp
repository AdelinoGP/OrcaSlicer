...
// [STATE][INTENT] Preserves the collection of hint IDs already shown so the notification system can avoid repeats across sessions.
// [PORTING_HAZARD:P2] Unity needs to provide its own binary/JSON persistence (e.g., ScriptableObject or PlayerPrefs) since cereal<> is
// unavailable.
...
namespace {
// [OPENGL][UNITY] Centralizes fade-aware ImGui style pushes so the overlay tint matches notification state; in Unity mirror via
// CanvasGroup/Graphic tint adjustments.
...
// [STATE][THREAD] Persists the serialized hint usage list to the user data directory so future sessions know what has already been shown;
// keep this on the UI thread or hand it off to a dedicated background writer to avoid blocking ImGui. [UNITY] Unity should copy this
// semantics into a ScriptableObject stored under Application.persistentDataPath.
...
// [STATE][THREAD] Reads the persisted hint usage from the same file so the runtime cache starts where the previous session left off (must
// tolerate missing files or corrupted versions). [PORTING_HAZARD:P2] Unity will need to map this to `PlayerPrefs` or a JSON blob in
// `Application.persistentDataPath` with version checks.
...
// [STATE][EVENT] Tag evaluation helpers gate hints based on app mode, printer tech, OS, or filament material so Unity can mirror the same
// visibility rules via AppConfig + Preset metadata.
enum TagCheckResult { TagCheckAffirmative, TagCheckNegative, TagCheckNotCompatible };
// [UNITY] Each branch should be replicated with Runtime configuration queries (ModeService, PlatformInfo, PresetModel) because hints only
// fire if these predicates pass.
TagCheckResult tag_check_mode(const std::string& tag)
{
    ...
    if (std::find(allowed_tags.begin(), allowed_tags.end(), tag) != allowed_tags.end()) {
        ...
        if (config_mode == ConfigOptionMode::comSimple)
            return (tag == "simple" ? TagCheckAffirmative : TagCheckNegative);
        else if (config_mode == ConfigOptionMode::comAdvanced)
            return (tag == "advanced" ? TagCheckAffirmative : TagCheckNegative);
...
// [STATE][EVENT] Guards hints by the current printer technology (FFF/MMU/SLA) and extruder count so Unity's PrintConfig/PresetModel can
...
TagCheckResult tag_check_tech(const std::string& tag)
{
    ...
    if (std::find(allowed_tags.begin(), allowed_tags.end(), tag) != allowed_tags.end()) {
        ...
        if (tech == ptFFF) {
            ...
            if (tag == "MMU")
                return (is_mmu ? TagCheckAffirmative : TagCheckNegative);
...
// [STATE][UNITY] Binary filters on Windows/Linux/OSX flags so Unity can reuse its own `Application.platform` checks when showing OS-specific hints.
...
// [STATE][EVENT][PORTING_HAZARD:P3] Matches filament material tags with the currently selected material preset; Unity must expose the same
// `filament_type` option to keep the filters aligned.
...
// [STATE][EVENT] Applies the accumulated tag filters from the hint metadata so the hint decision tree runs the same predicate as wxWidgets;
// Unity should run the same composed checks before enqueueing a notification. return true if NOT in disabled mode.
...
// [STATE][THREAD] Uninitializes the hint cache, clears the loaded data, and resets serialization flags so the next init run starts from a
// clean slate without leaking cross-session state.
...
// [INTENT][STATE][THREAD] Loads hints from the serialized INI and seeds the rotational randomizer; this runs on startup so Unity should
// warm up the hint catalog before showing modals.
...
// [INTENT][STATE][UNITY] Reads per-hint metadata (text, hypertext actions, tag filters, weights) so Unity can re-expose descriptions/click
// handlers in a UI Toolkit overlay. [PORTING_HAZARD:P2] Depends on boost::property_tree and wxGetApp singletons; the port must replicate
// the INI parsing and tag wiring in managed code.
       ...
       // replace <b> and </b> for imgui markers
            ...
            if (dict.find("hypertext_type") != dict.end()) {
                ...
                if (dict["hypertext_type"] == "link") {
                    ...
                    HintData    hint_data{id_string,      text1,
                                       weight,         was_displayed,
                                       hypertext_text, follow_text,
                                       disabled_tags,  enabled_tags,
                                       false,          documentation_link,
                                       img_url,        [hypertext_link]() { launch_browser_if_allowed(hypertext_link); }};
                    ...
                    HintData    hint_data{id_string,      text1,
                                       ...
                                       disabled_tags,  enabled_tags,
                                       false,          documentation_link,
                                       img_url,        [page, item]() { wxGetApp().open_preferences(1, page); }}; // 1 is to modify
                    m_loaded_hints.emplace_back(hint_data);
                    ...
                    HintData    hint_data{id_string,      text1,
                                       ...
                                       img_url,        [item]() { wxGetApp().plater()->canvas3D()->highlight_toolbar_item(item); }};
                    m_loaded_hints.emplace_back(hint_data);
                    ...
                    HintData    hint_data{id_string,      text1,
                                       ...
                                       disabled_tags,  enabled_tags,
                                       true,           documentation_link,
                                       img_url,        [item]() { wxGetApp().plater()->canvas3D()->highlight_gizmo(item); }};
                    m_loaded_hints.emplace_back(hint_data);
                    ...
                    HintData hint_data{id_string,      text1,
                                       ...
                                       disabled_tags,  enabled_tags,
                                       true,           documentation_link,
                                       img_url,        [menu, item]() { wxGetApp().mainframe->open_menubar_item(menu, item); }};
                    m_loaded_hints.emplace_back(hint_data);
                }
            } else {
                // plain text without hypertext
                HintData hint_data{id_string,     text1,        weight, was_displayed,      hypertext_text, follow_text,
                                   disabled_tags, enabled_tags, false,  documentation_link, img_url};
                m_loaded_hints.emplace_back(hint_data);
            }
        }
    }
}
// [EVENT][STATE][THREAD] Drives hint navigation and ensures Unity mirrors the `Next/Prev/Random` actions on the UI dispatcher so the
// selected hint stays current with user clicks.
...
// shown; Unity must replicate the multi-pass filtering and weight accumulation logic on the main thread. [PORTING_HAZARD:P3] Uses `rand` +
// `srand` with time-tied seeds; consider Unity's `Random.state` approach to avoid drift between sessions.
...
// [STATE][THREAD] Lazily loads the serialized `m_used_ids` cache so the hint tracking can answer the "already-shown" question without
...
// [STATE][THREAD] Appends newly shown hint IDs to the in-memory cache so the generator can log there are no repeats within the current run
// before persisting.
...
// icon; Unity should reproduce the same spacing in a `VisualElement` with padding.
...
// [STATE][OPENGL] Measures how many lines will be displayed with the calculated width so ImGui can size the window; Unity should track the
// same counts to mirror multi-line behavior inside TextMeshPro/UIScrollView.
void NotificationManager::HintNotification::count_lines()
{
            ...
            if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset) {
                ...
                if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width - m_window_width_offset ||
                    ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x <
                        (m_window_width - m_window_width_offset) / 5 * 3) {
                    float width_of_a   = ImGui::CalcTextSize("a").x;
                    int   letter_count = (int) ((m_window_width - m_window_width_offset) / width_of_a);
                    while (last_end + letter_count < text.size() &&
                           ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x < m_window_width - m_window_width_offset) {
    ...
    if (!m_text2.empty()) {
        ...
        while (last_end < text.length() - 1) {
            size_t next_hard_end = text.find_first_of('\n', last_end);
            if (next_hard_end != std::string::npos && ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x <
                                                          m_window_width - m_window_width_offset - size_of_last_line) {
                // next line is ended by '/n'
                ...
                if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset - size_of_last_line) {
                    ...
                    // when one word longer than line.
                    if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x >
                            m_window_width - m_window_width_offset - size_of_last_line ||
                        ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x + size_of_last_line <
                            (m_window_width - m_window_width_offset) / 5 * 3) {
                        float width_of_a   = ImGui::CalcTextSize("a").x;
                        int   letter_count = (int) ((m_window_width - m_window_width_offset - size_of_last_line) / width_of_a);
                        while (last_end + letter_count < text.size() && ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x <
                                                                            m_window_width - m_window_width_offset - size_of_last_line) {
                            letter_count += get_utf8_seq_length(text, last_end + letter_count);
                        }
...
// [OPENGL][UNITY] Calculates the next window height based on line count so the ImGui window stays the same size each frame; Unity can mimic
// this with `VisualElement.style.height` updates before repaint.
...
// [EVENT][STATE][UNITY] Handles hypertext clicks only when the runtime filters permit them so Unity can mirror the same `enabled_tags`
// rollout before dispatching to the bound action.
...
// [OPENGL][STATE][UNITY] Renders all hint text lines, hyertext, and follow-up text with ImGui while applying fade colors and width
// wrapping; Unity should mirror this via a custom UI Toolkit overlay plus TextMeshPro layout loops.
...
// [EVENT][STATE][OPENGL][UNITY] Renders the close/minimize arrow buttons with fading colors and tooltips, keeping the UI thread in control
// so Unity can handle the same actions via button callbacks.
void NotificationManager::HintNotification::render_close_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y + win_size.y / 2 - button_pic_size.y),
                                   ImVec2(win_pos.x, win_pos.y + win_size.y / 2 + button_pic_size.y), true)) {
...
// [EVENT][STATE][PORTING_HAZARD:P3][UNITY] Places the preferences button where minimize would live, reusing global app state; Unity should
// route this to its own settings panel via a `Command` pattern.
    ...
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / 7.5f, win_pos_y + m_window_height - 1.5f * m_line_height),
                                   ImVec2(win_pos_x - m_window_width / 15.f, win_pos_y + m_window_height), true)) {
...
void NotificationManager::HintNotification::render_logo(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
// [EVENT][PORTING_HAZARD:P2][UNITY] Shows the documentation icon that funnels through `open_documentation`; Unity must replicate the
// hyperlink hover tooltip and use `Application.OpenURL` guarded by the same suppress flag.
void NotificationManager::HintNotification::render_documentation_button(
    ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    auto   scale = wxGetApp().plater()->get_current_canvas3D()->get_scale();
    ImVec2 win_size(win_size_x, win_size_y);
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - m_line_height * 5.f, win_pos.y + win_size.y / 2 - button_pic_size.y),
                                   ImVec2(win_pos.x - m_line_height * 2.5f, win_pos.y + win_size.y / 2 + button_pic_size.y), true)) {
