// [ANNOTATED]
// [INTENT] Public declaration of the G-code 3D preview system and its interactive components.
// [STATE] Managed state for toolpath rendering modes, layer scrubbing, statistics, and toolhead markers.
// [UNITY] Map GCodeViewer to a core Unity MonoBehaviour controller. Nested classes (Marker, GCodeWindow) map to specialized C# UI/Scene
// components. [PORTING_HAZARD:P3] Heavy reliance on libvgcode types and immediate-mode UI state (ImGui) for legends and overlays.
...
public:
    ...
    struct SequentialView
    {
...
#if ENABLE_ACTUAL_SPEED_DEBUG
            void set_actual_speed_y_range(const std::pair<float, float>& y_range) { m_actual_speed_imgui_widget.y_range = y_range; }
            void set_actual_speed_levels(const std::vector<std::pair<float, ColorRGBA>>& levels)
            { m_actual_speed_imgui_widget.levels = levels; }
            void set_actual_speed_data(const std::vector<ActualSpeedImguiWidget::Item>& data) { m_actual_speed_imgui_widget.data = data; }
#endif // ENABLE_ACTUAL_SPEED_DEBUG
        ...
        void        render(const bool                  has_render_path,
                           float                       legend_height,
...
public:
    ...
    // BBS: add only gcode mode
    void load_as_gcode(const GCodeProcessorResult&       gcode_result,
                       const Print&                      print,
    ...
    // BBS
    //  void _render_calibration_thumbnail_internal(ThumbnailData& thumbnail_data, const ThumbnailsParams& thumbnail_params, PartPlateList&
    //  partplate_list, OpenGLManager& opengl_manager); void _render_calibration_thumbnail_framebuffer(ThumbnailData& thumbnail_data,
    //  unsigned int w, unsigned int h, const ThumbnailsParams& thumbnail_params, PartPlateList& partplate_list, OpenGLManager&
    //  opengl_manager); void render_calibration_thumbnail(ThumbnailData& thumbnail_data, unsigned int w, unsigned int h, const
    //  ThumbnailsParams& thumbnail_params, PartPlateList& partplate_list, OpenGLManager& opengl_manager);
