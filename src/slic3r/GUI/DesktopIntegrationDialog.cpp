// [ANNOTATED]
// [INTENT] Linux-specific implementation for desktop integration (creating .desktop files, icons, and URL handlers).
// [STATE] Determines installation paths using XDG environment variables and system directory discovery.
// [EVENT] Simple button handlers for committing or reverting desktop changes.
// [UNITY] Map to a platform-specific C# Service or Build Post-Processor. UI should map to a standard ModalWindow.
// [PORTING_HAZARD:P2] Deep reliance on Linux-specific shell commands (xdg-mime), environment variables, and POSIX file system APIs.
...
bool DesktopIntegrationDialog::integration_possible() { return true; }
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    // $XDG_DATA_DIRS defines the preference-ordered set of base directories to search for data files in addition to the $XDG_DATA_HOME base
    // directory. The directories in $XDG_DATA_DIRS should be seperated with a colon ':'. If $XDG_DATA_DIRS is either not set or empty, a
    // value equal to /usr/local/share/:/usr/share/ should be used.
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        ...
        if (contains_path_dir(target_candidates[i], "images")) {
            ...
            if (i == target_candidates.size() - 1) {
                ...
                std::string icon_path = GUI::format("%1%/images/OrcaSlicer.png", resources_dir());
                std::string dest_path = GUI::format("%1%/images/%2%OrcaSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
                if (!contains_path_dir(target_dir_icons, "images") || !copy_icon(icon_path, dest_path)) {
    ...
    if (target_dir_icons.empty()) {
        ...
        app_config->set("desktop_integration_icon_slicer_path",
                        GUI::format("%1%/images/%2%OrcaSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    app_config->set("desktop_integration_app_path",
                    GUI::format("%1%/applications/OrcaSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty()) {
            ...
            std::string dest_path = GUI::format("%1%/images/%2%OrcaSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path,
                                                version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        // Desktop file
        std::string desktop_file = GUI::format("[Desktop Entry]\n"
        ...
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. OrcaSlicer desktop "
                                   "file was probably created successfully."));
        }
    }
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration(std::string url_prefix)
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = GUI::format("%1%/applications/OrcaSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix,
                                           version_suffix);
            if (create_desktop_file(path, desktop_file_downloader)) {
                ...
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to OrcaSlicerURLProtocol.desktop file installation failed. failed path: "
                                         << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
    ...
    std::string desktop_path = GUI::format("%1%/applications/OrcaSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix,
                                           version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default OrcaSlicerURLProtocol-%1%%2%.desktop x-scheme-handler/%1%", url_prefix,
                                          version_suffix);
    ...
    : wxDialog(parent, wxID_ANY, _(L("Desktop Integration")), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    btn_perform->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
    ...
    if (can_undo) {
        ...
        btn_undo->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
