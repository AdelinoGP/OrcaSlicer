...
// [INTENT] Constructor sets up the panel.
Bed_2D::Bed_2D(wxWindow* parent)
    : // [UNITY] Use a standard Panel equivalent in Unity UI.
...
// [INTENT] Calculates grid step based on bounding box.
// [UNITY] Use C# logic for grid spacing in UI.
    ...
    const ExPolygon& poly, const BoundingBox& bb, const Vec2d& origin, const double& step, const double& scale)
{
    ...
    for (coord_t x = origin.x(); x >= bb.min(0); x -= step) { // Negative X axis
        (count % 5 ? lines_thin : lines_bold).push_back(Polyline(Point(x, bb.min(1)), Point(x, bb.max(1))));
        count++;
    }
    count = 0;
    ...
    for (coord_t y = origin.y(); y >= bb.min(1); y -= step) { // Negative Y axis
        (count % 5 ? lines_thin : lines_bold).push_back(Polyline(Point(bb.min(0), y), Point(bb.max(0), y)));
        count++;
    }
    count = 0;
...
void Bed_2D::repaint(const std::vector<Vec2d>& shape)
{
    ...
    //	Vec2d end = Eigen::Translation2d(x_end) * Eigen::Rotation2Dd(angle) * Eigen::Translation2d(- x_end) * Eigen::Vector2d(x_end(0) -
    // arrow_len, x_end(1)); 	dc.DrawLine(wxPoint(x_end(0), x_end(1)), wxPoint(end(0), end(1)));
    // }
    ...
    // for (auto angle : { -arrow_angle, arrow_angle }) {  // ORCA dont draw arrows
    //	Vec2d end = Eigen::Translation2d(y_end) * Eigen::Rotation2Dd(angle) * Eigen::Translation2d(- y_end) * Eigen::Vector2d(y_end(0),
    // y_end(1) + arrow_len); 	dc.DrawLine(wxPoint(y_end(0), y_end(1)), wxPoint(end(0), end(1)));
    // }
    ...
    dc.DrawRectangle(wxPoint(origin_label_x, origin_label_y),
                     extent); // ORCA draw a background to origin position text to improve readability when overlaps with grid
