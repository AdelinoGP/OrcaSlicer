// [ANNOTATED]
// [INTENT] Primary declarations for the device management layer, including the monolithic MachineObject state container.
// [STATE] MachineObject aggregates all printer-specific state: sub-systems, connectivity, telemetry, and capabilities.
// [UNITY] Map MachineObject to a rich C# ViewModel with INotifyPropertyChanged support for Unity UI binding.
// [PORTING_HAZARD:P2] Monolithic state object with hundreds of fields; refactor into composed service models in C#.
...
public:
    enum ActiveState { NotActive, Active, UpdateToDate };
...
public:
    ...
    enum LiveviewLocal { LVL_None, LVL_Disable, LVL_Local, LVL_Rtsps, LVL_Rtsp } liveview_local{LVL_None};
    enum LiveviewRemote { LVR_None, LVR_Tutk, LVR_Agora, LVR_TutkAgora } liveview_remote{LVR_None};
    enum FileLocal { FL_None, FL_Local } file_local{FL_None};
    enum FileRemote { FR_None, FR_Tutk, FR_Agora, FR_TutkAgora } file_remote{FR_None};
    ...
    int command_ams_filament_settings(int         ams_id,
                                      int         slot_id,
    ...
    int command_extrusion_cali_set(int         tray_index,
                                   std::string setting_id,
                                   std::string name,
                                   float       k,
                                   float       n,
    ...
    int command_start_pa_calibration(const X1CCalibInfos& pa_data,
                                     int                  mode = 0); // 0: automatic mode; 1: manual mode. default: automatic mode
