...
// [INTENT] GLShaderProgram encapsulates GL program creation/caching so view passes can reuse compiled shaders without leaking GPU handles.
// [THREAD] Construction/destruction must run on the GL-context thread so that `glDeleteProgram` and friends execute safely.
...
bool GLShaderProgram::init_from_files(const std::string&                             name,
                                      const ShaderFilenames&                         filenames,
                                      const std::initializer_list<std::string_view>& defines)
{
    // [INTENT] Build a shader program from disk assets, injecting the caller's defines so renderer variants share the same GLSL blobs.
    // [UNITY] Unity port should pre-author `ShaderVariantCollection` assets and avoid runtime parsing by bundling defines as keywords on
    // `Shader` assets. [PORTING_HAZARD:P2] Unity lacks direct file loading in production builds, so `resources_dir()` lookup must map to
    // `StreamingAssets` and still honor define ordering. [THREAD] Compilation occurs on the main rendering thread while the GL context is
    // current. Load a shader program from file, prepend defs block.
    auto load_from_file = [](const std::string& filename, const std::string& defs) {
        ...
        if (!s.good()) {
            // [EVENT] Failures here generate log events that bubble into the UI diagnostics panel so missing shader files become visible errors.
    ...
    // [STATE] `defines_program` is a replicated macro block that controls shader branching and must stay aligned with cached program variants.
    ...
    bool valid = !sources[static_cast<size_t>(EShaderType::Vertex)].empty() &&
                 !sources[static_cast<size_t>(EShaderType::Fragment)].empty() && sources[static_cast<size_t>(EShaderType::Compute)].empty();
    valid |= !sources[static_cast<size_t>(EShaderType::Compute)].empty() && sources[static_cast<size_t>(EShaderType::Vertex)].empty() &&
             sources[static_cast<size_t>(EShaderType::Fragment)].empty() && sources[static_cast<size_t>(EShaderType::Geometry)].empty() &&
             sources[static_cast<size_t>(EShaderType::TessEvaluation)].empty() &&
             sources[static_cast<size_t>(EShaderType::TessControl)].empty();
...
bool GLShaderProgram::init_from_texts(const std::string& name, const ShaderSources& sources)
{
    ...
    // [OPENGL] Create a GL shader object for each stage; Unity equivalent is importing compile-time variants via `ShaderVariantCollection`
    // rather than `glCreateShader` at runtime. [PORTING_HAZARD:P3] Unity only exposes pipeline stage wiring through asset import, so this
    // manual creation must be reflected by prebuilt shader assets.
    ...
    for (size_t i = 0; i < static_cast<size_t>(EShaderType::Count); ++i) {
        ...
        if (!source.empty()) {
            ...
            if (result)
                ...
                BOOST_LOG_TRIVIAL(error) << "glCreateShader() failed for " << shader_type_as_string(type) << " shader of shader program '"
                                         << name << "'";
    ...
    // [STATE] After linking we discard intermediate shaders because the GL program owns the compiled code; caches remain tied to `m_id`.
    return true;
}
...
{
    assert(m_id > 0);
    // [OPENGL] Bind the compiled shader program for subsequent draw calls; Unity uses `Material.SetPass` or `CommandBuffer.DrawMesh` to
    // achieve the same.
...
{
    // [OPENGL] Unbind program to avoid leaking state between passes.
...
void GLShaderProgram::set_uniform(int id, bool value) const { set_uniform(id, value ? 1 : 0); }
...
void GLShaderProgram::set_uniform(int id, double value) const { set_uniform(id, static_cast<float>(value)); }
...
void GLShaderProgram::set_uniform(int id, const Transform3d& value) const { set_uniform(id, value.cast<float>()); }
...
void GLShaderProgram::set_uniform(int id, const Matrix3d& value) const { set_uniform(id, (Matrix3f) value.cast<float>()); }
...
void GLShaderProgram::set_uniform(int id, const Matrix4d& value) const { set_uniform(id, (Matrix4f) value.cast<float>()); }
...
void GLShaderProgram::set_uniform(int id, const Vec2d& value) const { set_uniform(id, static_cast<Vec2f>(value.cast<float>())); }
...
void GLShaderProgram::set_uniform(int id, const Vec3d& value) const { set_uniform(id, static_cast<Vec3f>(value.cast<float>())); }

void GLShaderProgram::set_uniform(int id, const ColorRGB& value) const { set_uniform(id, value.data(), 3); }

void GLShaderProgram::set_uniform(int id, const ColorRGBA& value) const { set_uniform(id, value.data(), 4); }
...
int GLShaderProgram::get_attrib_location(const char* name) const
{
    ...
    auto it = std::find_if(m_attrib_location_cache.begin(), m_attrib_location_cache.end(),
                           [name](const auto& p) { return p.first == name; });
    if (it != m_attrib_location_cache.end())
        // [STATE] Use cached attribute lookups to avoid repeated GL calls in hot render loops.
        // Attrib ID cached.
        return it->second;

    int id = ::glGetAttribLocation(m_id, name);
    // [OPENGL] Query attribute index once for each program so vertex array setups can reuse the value.
...
int GLShaderProgram::get_uniform_location(const char* name) const
{
    ...
    auto it = std::find_if(m_uniform_location_cache.begin(), m_uniform_location_cache.end(),
                           [name](const auto& p) { return p.first == name; });
    if (it != m_uniform_location_cache.end())
        // [STATE] Uniform cache mirrors Unity's `MaterialPropertyBlock` caching to avoid redundant `glGetUniformLocation` calls.
        // Uniform ID cached.
        return it->second;

    int id = ::glGetUniformLocation(m_id, name);
    // [OPENGL] Location lookups are once per uniform to keep the render loop lean.
