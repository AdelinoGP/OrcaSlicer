...
AuxiliaryList::AuxiliaryList(wxWindow* parent) : wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDV_NO_HEADER)
{
    ...
    // [THREAD] Construction runs on the main UI thread so the tree and toolbar wiring stay locked to a single scheduler for Unity's main
    // loop replacement.
    ...
    // [EVENT][THREAD][UNITY] Enable native drag/drop on the UI thread; Unity will need a DragAndDrop bridge that uses this signal for reordering.

    // Keyboard events
    Bind(wxEVT_CHAR, [this](wxKeyEvent& event) { this->handle_key_event(event); });
    // [THREAD][UNITY] Key events run on the UI thread, so Unity's Input System should marshal them before this handler.
...
AuxiliaryList::~AuxiliaryList()
{
    ...
    // [PORTING_HAZARD:P3] wxDataViewCtrl holds implicit listeners, so Unity must unregister events before nulling the controller to avoid
    // callbacks into disposed logic. [UNITY] Unity should dispose of the matching tree controller and cached state so the selection/mode
    // cleanup mirrors the wx teardown.
}
...
void AuxiliaryList::init_auxiliary()
{
    ...
    // [STATE] Model initialization pulls from the Plater auxiliary temp path so Unity needs to mirror that shared path before exposing tree
    // entries. [PORTING_HAZARD:P3] wxGetApp() is a wxWidgets singleton that Unity lacks; inject the Plater/root controller so the tree
    // still sees the auxiliary temp directory. [UNITY] Expose the same temp directory (e.g., via a ScriptableObject path provider) before
    // Unity's tree populates so both views share the same source. [THREAD] This runs on the UI thread because it reads wxGetApp() state and
    // updates the data view before the tree becomes visible.
}
...
void AuxiliaryList::do_import_file(AuxiliaryModelNode* folder)
{
    ...
    wxString     src_path;
    wxString     dst_path;
    wxFileDialog dialog(this, _L("Choose files"), wxEmptyString, wxEmptyString, wxFileSelectorDefaultWildcardStr,
...
void AuxiliaryList::on_delete(wxCommandEvent& evt)
{
    ...
    // [THREAD][PORTING_HAZARD:P3] Deletion must stay on the UI thread because the shared model is not thread-safe; Unity should marshal the
    // Delete command before updating the tree. [EVENT][STATE][UNITY] Deletes always funnel through this helper so Unity can tie toolbar
    // buttons and hotkeys to the same state change.
}
...
void AuxiliaryList::on_context_menu(wxDataViewEvent& evt)
{
    ...
    if (node == nullptr) {
        append_menu_item(menu, wxID_ANY, _L("New Folder"), wxEmptyString, [this](wxCommandEvent&) { create_new_folder(); });
    } else if (node->IsContainer()) {
        append_menu_item(menu, wxID_ANY, _L("Import File"), wxEmptyString, [this, node](wxCommandEvent&) { do_import_file(node); });
        append_menu_item(menu, wxID_ANY, _L("Delete"), wxEmptyString, [this, item](wxCommandEvent&) { m_auxiliary_model->Delete(item); });
    } else {
        append_menu_item(menu, wxID_ANY, _L("Open"), wxEmptyString,
                         [this, node](wxCommandEvent&) { wxLaunchDefaultApplication(node->path, 0); });
        append_menu_item(menu, wxID_ANY, _L("Delete"), wxEmptyString, [this, item](wxCommandEvent&) { m_auxiliary_model->Delete(item); });
        append_menu_item(menu, wxID_ANY, _L("Rename"), wxEmptyString, [this, item](wxCommandEvent&) {
    ...
    PopupMenu(menu);
    // [EVENT][INTENT][UNITY][STATE][THREAD] Keep context menus aligned with toolbar verbs so Unity can reuse the same helper methods for
    // right-click overlays (menu creation must stay on the UI thread).
}
...
void AuxiliaryList::on_drop(wxDataViewEvent& evt)
{
    ...
    // [EVENT][STATE][THREAD][UNITY][PORTING_HAZARD:P2] Clear the drag sentinel once the move completes so future drops start fresh, avoid
    // stale references, and teach Unity to reset its drag state before the next drop.
}
...
void AuxiliaryList::on_editing_done(wxDataViewEvent& evt)
{
    ...
    if (!is_done)
        evt.Veto();
    // [STATE][UNITY] Accept or veto rename results to keep the UI tree synchronized; Unity should surface validation feedback before
    // closing a text field.
}
...
// [EVENT][STATE][UNITY][PORTING_HAZARD:P3] Double-click launching relies on native shells; Unity should use a cross-platform helper
// (Process.Start or Application.OpenURL) on the main thread and guard non-file containers. [PORTING_HAZARD:P2] Double-click launching must
// confirm the selected node still maps to a file before invoking Process.Start in Unity. [PORTING_HAZARD:P2] Guard stale path references so
// the launcher doesn't throw when selection data changes mid-drag.
void AuxiliaryList::handle_key_event(wxKeyEvent& evt)
{
    if (evt.GetKeyCode() == WXK_DELETE || evt.GetKeyCode() == WXK_BACK)
        m_auxiliary_model->Delete(this->GetSelection());
    // [EVENT][STATE][THREAD][UNITY] Keyboard delete/backspace mirrors the toolbar Delete button so Unity can wire the same hotkeys into
    // this helper via the Input System command map while staying on the UI thread.
