...
// [INTENT] ObjColorDialog provides a UI for mapping colors detected in an imported OBJ file to available printer filaments/extruders.
// It includes color clustering logic and a 3D thumbnail preview to visualize the mapping results.
    ...
    , m_first_extruder_id(in_out.first_extruder_id)
{
    ...
    if (!ok) {
        ...
        m_button_list[wxOK]->Bind(wxEVT_UPDATE_UI, ([this](wxUpdateUIEvent& e) {
                                      if (m_panel_ObjColor->is_ok() == m_button_list[wxOK]->IsEnabled()) {
                                          return;
                                      }
    ...
    if (this->FindWindowById(wxID_OK, this)) {
        // [EVENT] OK button finalizes the color mapping, resets the preview thumbnail instances, and updates the filament UI.
    ...
    , m_first_extruder_id(in_out.first_extruder_id)
{
    ...
    // BBS: for tunning flush volumes
    {
        ...
        m_color_cluster_num_by_user_ebox = new SpinInput(m_page_simple, "", wxEmptyString, wxDefaultPosition, wxSize(FromDIP(45), -1),
                                                         wxTE_PROCESS_ENTER);
        ...
        wxStaticText* recommend_color_cluster_title = new wxStaticText(m_page_simple, wxID_ANY,
        ...
        { // add combox
            ...
            auto plater     = wxGetApp().plater();
            {
                ...
                if (box.min.x() < 0 || box.min.y() < 0 || box.min.z() < 0) {
                    m_thumbnail_offset = Slic3r::Vec3d(box.min.x() < 0 ? -box.min.x() : 0, box.min.y() < 0 ? -box.min.y() : 0,
            ...
            auto          cur_combox = new ComboBox(m_page_simple, wxID_ANY, wxEmptyString, wxPoint(FromDIP(250), FromDIP(310)),
            ...
            m_image_button = new wxButton(m_page_simple, wxID_ANY, {}, wxDefaultPosition,
                                          wxSize(FromDIP(IMAGE_SIZE_WIDTH), FromDIP(IMAGE_SIZE_WIDTH)), wxBORDER_NONE | wxBU_AUTODRAW);
        ...
        wxGridSizer* current_filaments_sizer = new wxGridSizer(current_filament_row, current_filament_col, FromDIP(5),
                                                               FromDIP(7)); //(int rows, int cols, int vgap, int hgap );
...
void ObjColorPanel::msw_rescale()
{
    ...
    /* for (unsigned int i = 0; i < m_color_cluster_icon_list.size(); ++i) {
         auto bitmap = *get_extruder_color_icon(m_cluster_colours[i].GetAsString(wxC2S_HTML_SYNTAX).ToStdString(), std::to_string(i + 1),
     FromDIP(16), FromDIP(16)); m_color_cluster_icon_list[i]->SetBitmap(bitmap);
     }*/
}
...
void ObjColorPanel::set_layout_callback(LayoutChanggeCallback callback) { m_layout_callback = callback; }
...
{
    std::string str = ("R:" + std::to_string(color.Red()) + std::string(" G:") + std::to_string(color.Green()) + std::string(" B:") +
                       std::to_string(color.Blue()) + std::string(" A:") + std::to_string(color.Alpha()));
    return str;
}
...
ComboBox* ObjColorPanel::CreateEditorCtrl(wxWindow* parent, int id) // wxRect labelRect,, const wxVariant &value
{
    ...
    icons.insert(icons.begin(), get_extruder_color_icon(g_undefined_color_in_obj.GetAsString(wxC2S_HTML_SYNTAX).ToStdString(),
                                                        std::to_string(-1), icon_width, icon_height));
    if (icons.empty())
        return nullptr;

    ::ComboBox* c_editor = new ::ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(m_combox_width), -1), 0,
...
void ObjColorPanel::draw_new_table()
{
    ...
    if (!first_draw) {
        ...
        for (size_t i = row; i < old_row; i++) {
            show_sizer(m_row_sizer_list[i], false);
        }
        for (size_t i = 0; i < cluster_count; i++) {
            show_sizer(m_row_col_boxsizer_list[i], true);
        }
        for (size_t i = cluster_count; i < m_color_num_recommend; i++) {
            show_sizer(m_row_col_boxsizer_list[i], false);
        }
        for (size_t ii = 0; ii < row; ii++) {
            for (size_t j = 0; j < col; j++) {
                ...
                m_color_cluster_icon_list[id]->SetBitmap(
                    *get_extruder_color_icon(color.GetAsString(wxC2S_HTML_SYNTAX).ToStdString(), "", FromDIP(16), FromDIP(16)));
            }
        }
    } else { // first draw
        for (size_t ii = 0; ii < row; ii++) {
...
{
    // [INTENT] Executes the color clustering algorithm based on the desired number of colors.
    // [THREAD] Long-running operation; a busy cursor is shown while the algorithm runs on the UI thread.
...
void ObjColorPanel::deal_default_strategy()
{
    ...
    m_warning_text->SetLabelText(_L("Note") + ": " +
                                 _L("The color has been selected, you can choose OK \n to continue or manually adjust it."));
}
...
void ObjColorPanel::deal_thumbnail()
{
    ...
    if (m_obj_in_out.deal_vertex_color) {
        if (m_obj_in_out.filament_ids.size() > 0) {
            m_deal_thumbnail_flag = Model::obj_import_vertex_color_deal(m_obj_in_out.filament_ids, m_obj_in_out.first_extruder_id,
                                                                        m_obj_in_out.model);
        }
    } else {
        if (m_obj_in_out.filament_ids.size() > 0) {
            m_deal_thumbnail_flag = Model::obj_import_face_color_deal(m_obj_in_out.filament_ids, m_obj_in_out.first_extruder_id,
                                                                      m_obj_in_out.model);
        }
    }
    generate_thumbnail();
...
{
    // [OPENGL] Triggers a thumbnail update using a temporary model volume.
    // [UNITY] Map to a dedicated camera and RenderTexture to visualize mapping results on a 3D preview of the OBJ.
...
bool ObjColorPanel::deal_add_btn()
{
    ...
    for (size_t i = 0; i < new_color_size; i++) {
        ...
        new_icons.emplace_back(get_extruder_color_icon(cur_color.GetAsString(wxC2S_HTML_SYNTAX).ToStdString(), std::to_string(new_index),
