...
// [INTENT] Manages 3DConnexion device input, including background thread communication, HID device polling (Win/Linux), driver integration
// (macOS), and UI interaction for configuration. [UNITY] Replace background thread with a dedicated MonoBehaviour or Task-based system for
// device polling, using Unity's Input System Package for HID access or native plugin for specialized drivers. [THREAD] Heavy background
// polling thread interacts with UI via a thread-safe event queue.
class Mouse3DController
{
    ...
    // [STATE] UI and device configuration state.
    struct Params
    {
        // [UNITY] Use ScriptableObject or JSON config for persistence.
        ...
        template<typename Number> struct CustomParameters
        {
            Number scale;
            Number deadzone;
        };
    ...
    // [STATE] Thread-safe event queue for passing data from background thread to the UI thread.
    // [PORTING_HAZARD:P2] Thread-safe queue implementation must be replaced with Unity's cross-thread communication mechanism (e.g.,
    // ConcurrentQueue).
    class State
    {
    public:
        struct QueueItem
        {
            static QueueItem translation(const Vec3d& translation)
            {
                QueueItem out;
                out.vector          = translation;
                out.type_or_buttons = TranslationType;
                return out;
            }
            static QueueItem rotation(const Vec3d& rotation)
            {
                QueueItem out;
                out.vector          = rotation;
                out.type_or_buttons = RotationType;
                return out;
            }
            static QueueItem buttons(unsigned int buttons)
            {
                QueueItem out;
                out.type_or_buttons = buttons;
                return out;
            }
    ...
    private:
        ...
        // detected. We want to filter these out because we are getting the data directly from the device, bypassing the driver, and those
        // mouse wheel events interfere by triggering unwanted zoom in/out of the scene The following variable is used to count the
        // potential mouse wheel events triggered and is updated by: Mouse3DController::collect_input() through the call to the
        // append_rotation() method GLCanvas3D::on_mouse_wheel() through the call to the process_mouse_wheel() method GLCanvas3D::on_idle()
        // through the call to the apply() method
    ...
    public:
        ...
        Vec3d get_first_vector_of_type(unsigned int type) const
        {
            ...
            auto                         it = std::find_if(m_input_queue.begin(), m_input_queue.end(),
                                                           [type](const QueueItem& item) { return item.type_or_buttons == type; });
...
public:
    ...
    // [UNITY] Use OnEnable in the MonoBehaviour for device connection lifecycle.
    void init();
    // Stop the background thread (Windows and Linux).
    // Disconnect from a 3DConnextion driver (OSX).
    // Call save_config() after shutdown().
    // [UNITY] Use OnDisable in the MonoBehaviour for device connection lifecycle.
    void shutdown();
