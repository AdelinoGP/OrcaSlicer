...
// [INTENT] Base class for message/query dialogs. [UNITY] Map to Canvas with a modal Panel/UIDocument.
...
// [INTENT] Generic warning dialog. [UNITY] Map to specialized modal WarningPanel.
class WarningDialog : public MsgDialog
{
public:
    WarningDialog(wxWindow* parent, const wxString& message, const wxString& caption = wxEmptyString, long style = wxOK);
...
//	: wxTextCtrl(parent, id, wxEmptyString, pos, size!=wxDefaultSize ? size : (style == wxLI_HORIZONTAL ? wxSize(10, 1) : wxSize(1, 10)),
// wxSIMPLE_BORDER, wxDefaultValidator, name)
//	{
//		this->Enable(false);
//	}
//	~StaticLine() {}
...
// [INTENT] Generic message dialog. [UNITY] Map to standard MessageBox equivalent.
...
public:
    ...
    virtual bool SetYesNoCancelLabels(const wxMD::ButtonLabel& yes, const wxMD::ButtonLabel& no, const wxMD::ButtonLabel& cancel)
    {
        DoSetCustomLabel(m_yes, yes);
        DoSetCustomLabel(m_no, no);
    ...
    virtual bool SetOKCancelLabels(const wxMD::ButtonLabel& ok, const wxMD::ButtonLabel& cancel)
    {
        DoSetCustomLabel(m_ok, ok);
        DoSetCustomLabel(m_cancel, cancel);
        return true;
    ...
    bool HasCustomLabels() const { return !(m_ok.empty() && m_cancel.empty() && m_help.empty() && m_yes.empty() && m_no.empty()); }
    ...
    wxString GetYesLabel() const { return m_yes.empty() ? GetDefaultYesLabel() : m_yes; }
    wxString GetNoLabel() const { return m_no.empty() ? GetDefaultNoLabel() : m_no; }
    wxString GetOKLabel() const { return m_ok.empty() ? GetDefaultOKLabel() : m_ok; }
    wxString GetCancelLabel() const { return m_cancel.empty() ? GetDefaultCancelLabel() : m_cancel; }
    wxString GetHelpLabel() const { return m_help.empty() ? GetDefaultHelpLabel() : m_help; }
...
protected:
    ...
    void DoSetCustomLabel(wxString& var, const wxMD::ButtonLabel& label) { var = label.GetAsString(); }
...
private:
    ...
    wxString m_yes, m_no, m_ok, m_cancel, m_help;
};
#else
