...
// [INTENT] Renders a button with an explicit screen position.
// [UNITY] Use Unity UI Button with RectTransform or a custom VisualElement.
...
// [INTENT] Wrappers for ImGui menu system.
// [UNITY] Use Unity UI Toolkit Menu or custom nested UI elements.
...
// [INTENT] Central wrapper class for ImGui integration within the wxWidgets/OpenGL environment.
// [STATE] Holds the ImGui context, font textures, scaling factors, and input state.
// [THREAD] Not thread-safe; must be accessed from the main UI thread for rendering and event processing.
// [UNITY] Maps to a MonoBehaviour (e.g., ImGuiManager) that handles NewFrame/Render in the Update/OnGUI loop.
class ImGuiWrapper
{
    const ImWchar* m_glyph_ranges{nullptr};
    const ImWchar* m_glyph_basic_ranges{nullptr};
    // [STATE] Language-specific font settings for CJK and Korean.
    bool m_font_cjk{false};
    bool m_is_korean{false};
    // [STATE] Dynamic scaling and font sizing for DPI awareness.
    float m_font_size{18.0};
    // [STATE] OpenGL texture IDs for fonts.
    // [OPENGL] Managed lifecycle: loaded in new_frame(), destroyed in destructor.
    unsigned m_font_texture{0};
    unsigned m_font_another_texture{0};
    float    m_style_scaling{1.0};
    // [STATE] Bitmask of currently pressed mouse buttons.
    unsigned m_mouse_buttons{0};
    bool     m_disabled{false};
    bool     m_new_frame_open{false};
    // [STATE] Interaction state for the most recently processed slider.
    ...
    // [OPENGL] Queries GLSL version for shader compatibility.
    void read_glsl_version();

    // [INTENT] Updates font ranges and triggers re-initialization of font textures.
    void set_language(const std::string& language);
    void set_display_size(float w, float h);
    void set_scaling(float font_size, float scale_style, float scale_both);

    // [EVENT] Propagates wxWidgets mouse/key events to ImGui IO.
    // [PORTING_HAZARD:P2] Manual synchronization of key/mouse states between wxWidgets and ImGui
    // is error-prone. Unity's Input System should provide a more unified event flow.
    // [UNITY] Replaced by Unity's Input System or InputManager events in the update loop.
    ...
    // [INTENT] Starts a new ImGui frame. Ensures font textures are initialized.
    // [UNITY] Corresponds to the start of Unity's OnGUI or an early Update step.
    void new_frame();

    // [INTENT] Finalizes ImGui rendering commands and executes them via OpenGL.
    // [OPENGL] Issues draw calls to the active OpenGL context.
    // [UNITY] Handled automatically if using a Unity-ImGui package, or requires custom mesh generation.
    void render();
    ...
    /* BBL style widgets */
    // [INTENT] Custom BambuLab-style UI widgets with enhanced styling and tooltips.
    // [UNITY] Should be implemented as custom Prefabs or VisualElement templates.
    bool bbl_combo_with_filter(const char*                     label,
    ...
    bool bbl_slider_float(const std::string& label,
                          float*             v,
                          float              v_min,
                          float              v_max,
    ...
    bool bbl_slider_float_style(const std::string& label,
                                float*             v,
                                float              v_min,
                                float              v_max,
                                const char*        format  = "%.3f",
    ...
    // [INTENT] Standard ImGui window management.
    ...
    // [INTENT] Standard and BBL-themed button widgets.
    ...
    bool   slider_float(const std::string& label,
                        float*             v,
                        float              v_min,
                        float              v_max,
                        const char*        format        = "%.3f",
                        ...
                        const wxString&    tooltip       = {},
                        bool               show_edit_btn = true);
    bool   slider_float(const wxString& label,
                        float*          v,
                        float           v_min,
                        float           v_max,
                        const char*     format        = "%.3f",
    ...
    bool slider_float(
        const std::string& label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f, bool clamp = true);
    bool slider_float(
        const wxString& label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f, bool clamp = true);
    ...
    bool combo(const std::string&              label,
               const std::vector<std::string>& options,
               int&                            selection,
               ImGuiComboFlags                 flags       = 0,
               float                           label_width = 0.0f,
               float                           item_width  = 0.0f);
    bool combo(const wxString&                 label,
               const std::vector<std::string>& options,
               int&                            selection,
               ImGuiComboFlags                 flags       = 0,
               float                           label_width = 0.0f,
               float                           item_width  = 0.0f);
    bool undo_redo_list(const ImVec2& size,
                        const bool    is_undo,
                        bool (*items_getter)(const bool, int, const char**),
                        int& hovered,
                        int& selected,
                        int& mouse_wheel);
    void search_list(const ImVec2& size,
    ...
    static bool input_optional_int(
        const char* label, std::optional<int>& v, int step = 1, int step_fast = 100, ImGuiInputTextFlags flags = 0, int def_val = 0);
    // Extended function ImGui::InputFloat to work with std::optional<float> value near def_val cause release of optional
    static bool input_optional_float(const char*           label,
    ...
    static bool drag_optional_float(const char*           label,
                                    std::optional<float>& v,
                                    float                 v_speed,
                                    float                 v_min,
                                    float                 v_max,
...
private:
    ...
    void                       destroy_font();
    std::vector<unsigned char> load_svg(
