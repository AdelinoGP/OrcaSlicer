// [ANNOTATED]
// [INTENT] Low-level Windows Win32 API hooks for enabling Dark Mode on legacy controls and window chrome.
// [STATE] Global dark-mode enablement and support flags (g_darkModeEnabled, g_darkModeSupported).
// [EVENT] Hooks into Win32 messages (WM_PAINT, WM_UAHDRAWMENU) via subclassing to override system drawing.
// [UNITY] COMPLETELY REDUNDANT. Unity's UI Toolkit (USS) and Canvas systems handle theme styling natively.
// [PORTING_HAZARD:P1] This file is deeply coupled to Windows HWND/HDC and Win32 theme engines. It should be ignored for the Unity port.
...
bool IsEnabled() { return g_darkModeEnabled; }

bool IsSupported() { return g_darkModeSupported; }

bool IsSystemMenuEnabled() { return g_SystemMenuEnabled; }
...
COLORREF GetBackgroundColor() { return IsEnabled() ? RGB(0x2B, 0x2B, 0x2B) : wxSystemSettings::GetColour(wxSYS_COLOUR_MENUBAR).GetRGB(); }
...
COLORREF GetTextColor() { return IsEnabled() ? RGB(0xF0, 0xF0, 0xF0) : wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT).GetRGB(); }

COLORREF GetHotTextColor() { return IsEnabled() ? RGB(0xFF, 0xFF, 0xFF) : wxSystemSettings::GetColour(wxSYS_COLOUR_ACTIVEBORDER).GetRGB(); }

COLORREF GetSofterTextColor() { return IsEnabled() ? RGB(0xF0, 0xF0, 0xF0) : RGB(0x64, 0x64, 0x64); }

COLORREF GetDarkerTextColor() { return RGB(0xC0, 0xC0, 0xC0); }

COLORREF GetEdgeColor() { return RGB(0x80, 0x80, 0x80); }
...
bool UAHWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LRESULT* lr)
{
    ...
    case WM_UAHDRAWMENUITEM: {
        ...
        DrawThemeTextEx(g_menuTheme, pUDMI->um.hdc, MENU_POPUPITEM, iTextStateID, menuString, mii.cch, dwFlags, &pUDMI->dis.rcItem,
                        &dttopts);
...
void renderButton(HWND hwnd, HDC hdc, HTHEME hTheme, int iPartID, int iStateID)
{
    ...
    */
    ...
    if (iPartID == SBP_ARROWBTN) {
        ...
        rcFocus.bottom += 1;
        POINT vertices_up[] = {{left_pos, bottom_pos},
                               {left_pos + triangle_edge, bottom_pos - triangle_edge},
                               {left_pos + 2 * triangle_edge, bottom_pos}};
        ...
        POINT vertices_down[] = {{left_pos, top_pos},
                                 {left_pos + triangle_edge, top_pos + triangle_edge},
                                 {left_pos + 2 * triangle_edge, top_pos}};
...
void paintButton(HWND hwnd, HDC hdc, ButtonData& buttonData)
{
    ...
    if (iStateID != buttonData.iStateID) {
        GetThemeTransitionDuration(buttonData.hTheme, iPartID, buttonData.iStateID, iStateID, TMT_TRANSITIONDURATIONS,
    ...
    HDC              hdcFrom      = nullptr;
    HDC              hdcTo        = nullptr;
    HANIMATIONBUFFER hbpAnimation = BeginBufferedAnimation(hwnd, hdc, &rcClient, BPBF_COMPATIBLEBITMAP, nullptr, &animParams, &hdcFrom,
                                                           &hdcTo);
    if (hbpAnimation) {
        if (hdcFrom) {
        ...
        if (hdcTo) {
            if (iPartID == SBP_ARROWBTN &&
                (buttonData.iStateID == ARROWBTNSTATES::ABS_DOWNHOT || buttonData.iStateID == ARROWBTNSTATES::ABS_UPHOT))
...
LRESULT CALLBACK ButtonSubclass(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    case WM_THEMECHANGED: pButtonData->closeTheme(); break;
    case WM_PRINTCLIENT:
    case WM_PAINT:
    ...
    case WM_MOUSEMOVE: {
        ...
        if (nButtonStyle == BS_AUTO3STATE) {
            ...
            int iStateID = 0;
            if (xPos <= rcClient.top || xPos >= rcClient.bottom || yPos <= rcClient.left || yPos >= rcClient.right)
    ...
    case WM_SIZE:
    case WM_DESTROY: BufferedPaintStopAllAnimations(hWnd); break;
    case WM_ENABLE:
        if (IsEnabled()) {
...
void AutoSubclassAndThemeChildControls(HWND hwndParent, bool subclass, bool theme)
{
    ...
    Params p{IsEnabled() ? L"DarkMode_Explorer" : nullptr, subclass, theme};

    EnumChildWindows(
        hwndParent,
        [](HWND hwnd, LPARAM lParam) WINAPI_LAMBDA {
