// [ANNOTATED]
// [INTENT] Low-level HTTP file retrieval using libcurl, with support for background threading, pausing, and resuming.
// [STATE] Maintains atomic state for download control (cancel/pause) and tracks byte-level progress.
// [EVENT] Queues lifecycle events (COMPLETE, ERROR, PROGRESS) to the UI thread handler.
// [THREAD] Spawns a dedicated std::thread for synchronous curl operations.
// [UNITY] Replace with UnityWebRequest or HttpClient. Use Task-based async/await and CancellationTokenSource.
// [PORTING_HAZARD:P2] Manual range-based resume logic and PID-based temp file management must be updated to modern managed patterns.
