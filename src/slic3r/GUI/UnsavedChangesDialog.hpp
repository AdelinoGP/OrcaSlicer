...
// [INTENT] These types model the preset-diff tree, the modal unsaved-changes
// workflow, and the paired preset-comparison dialog used when switching or
// saving presets with conflicts.
// [STATE] The hierarchy caches toggled nodes, old/new values, icon variants,
// and the selected action so the dialogs can rebuild and close deterministically.
// [UNITY] Port this as a modal controller with a retained diff-tree view-model,
// a separate compare-presets workspace, and command buttons bound to explicit
// transfer/save/discard actions.
// [PORTING_HAZARD:P2] wxDataViewCtrl, custom icon rendering, and GTK container
// semantics are tightly coupled to native tree behavior; Unity needs a retained
// data model instead of directly mirroring widget ownership.
// ----------------------------------------------------------------------------
...
public:
    ...
    // [UNCLEAR] m_container is forced true to satisfy wxGTK's ahead-of-time
    // container requirement; the Unity hypothesis is that expansion state should
    // be derived from the retained node model rather than inferred late.
...
// [INTENT] DiffModel owns the wxDataViewModel bridge that translates preset
// bundles into a toggleable diff tree and keeps the tree in sync with the dialog.
// [STATE] It owns the root preset nodes, the attached control, and the add/delete
// state needed to preserve selection and enablement across refreshes.
// [EVENT] Value changes, delete/clear operations, and item enablement flow back
// through the data-view callbacks and associated control notifications.
// [UNITY] Map this to a retained tree view-model feeding a UI Toolkit TreeView
// (or equivalent) with explicit item binding and selection state.
// [PORTING_HAZARD:P2] wxDataViewModel callback ordering and column-specific enable
// behavior are widget semantics that do not transfer 1:1 to Unity controls.
class DiffModel : public wxDataViewModel
{
    ...
    ModelNode* AddOption(ModelNode* group_node, wxString option_name, wxString old_value, wxString new_value);
    ModelNode* AddOptionWithGroup(
        ModelNode* category_node, wxString group_name, wxString option_name, wxString old_value, wxString new_value);
...
public:
    ...
    wxDataViewItem AddOption(Preset::Type      type,
                             wxString          category_name,
                             wxString          group_name,
                             wxString          option_name,
                             wxString          old_value,
...
// [INTENT] DiffViewCtrl wraps the native data-view control used to render,
// filter, and context-manage the preset-diff tree.
// [STATE] It caches selection emptiness, string truncation state, column widths,
// and per-item metadata used by the context menu and selection queries.
// [EVENT] Context-menu and value-change handlers are the main interaction seam;
// callers query selected/unselected options after the control updates.
// [UNITY] Use a retained TreeView/ListView with explicit row metadata and a
// separate popup/context-menu controller rather than custom wxDataView rendering.
...
public:
    ...
    void Append(const std::string& opt_key,
                Preset::Type       type,
                wxString           category_name,
                wxString           group_name,
                wxString           option_name,
...
enum ActionButtons { TRANSFER = 1, KEEP = 2, SAVE = 4, DONT_SAVE = 8, REMEMBER_CHOISE = 0x10000 };
...
// [INTENT] UnsavedChangesDialog is the modal workflow that resolves unsaved
// preset changes into transfer, discard, or save actions before the caller
// switches state.
// [STATE] It owns the button set, selected exit action, preset-item list, and
// the derived save-to-project / preset-name metadata that survives across subdialogs.
// [EVENT] DPIDialog resize/color hooks, custom button IDs, and the transfer event
// define the dialog's control flow back to the main frame.
// [UNITY] Port as a modal controller with a retained diff tree, a nested save-
// preset flow, and explicit action callbacks instead of posting native events.
// [PORTING_HAZARD:P2] The dialog mixes confirmation, project-embedded preset
// bookkeeping, and save-preset name collection in one flow; Unity should split
// side effects from presentation state.
struct PresetItem
{
    Preset::Type type;
    std::string  opt_key;
...
public:
    ...
    UnsavedChangesDialog(Preset::Type       type,
                         PresetCollection*  dependent_presets,
                         const std::string& new_selected_preset,
...
{
public:
    // [INTENT] FullCompareDialog is a simple detail popup for showing the full
    // old/new value pair when the truncated diff view is not enough.
    // [UNITY] Map to a lightweight modal detail panel or popup anchored to the diff row.
    FullCompareDialog(const wxString& option_name,
...
{
    // [INTENT] DiffPresetDialog is the paired preset-comparison dialog used to
    // inspect two presets, choose transfer sources, and validate compatibility
    // before action.
    // [STATE] It owns both preset bundles, the active view type, the comparison
    // rows, and the optional transfer-selection controls.
    // [EVENT] Button clicks, checkbox toggles, combo-box changes, and DPI/color
    // hooks all reflow the comparison layout and compatibility labels.
    // [UNITY] Port as a comparison workspace with two preset selectors, a shared
    // diff tree, and a dedicated compatibility/transfer control strip.
    // [PORTING_HAZARD:P2] The dialog depends on two live PresetBundle snapshots
    // and on wx-specific enable/visibility choreography; Unity needs a data-bound
    // workspace.
    DiffViewCtrl* m_tree{nullptr};
