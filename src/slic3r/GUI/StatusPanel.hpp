...
// [INTENT] This header defines the monitor/status dashboard for a connected machine.
// It combines print-state summaries, AMS / filament controls, camera and media widgets,
// calibration / safety / options dialogs, and score-upload UI into one retained wxWidgets
// surface.
// [UNITY] Split into a dashboard root controller with child panels for print status,
// camera/media, AMS, calibration, and modal dialogs; keep web requests and device state
// behind services instead of view-owned globals.
// [PORTING_HAZARD:P2] The current design mixes view composition, device commands, and modal
// dialog ownership in one header, so Unity will need a clearer view-model boundary.
...
enum PrintingTaskType { PRINGINT, CALIBRATION, NOT_CLEAR };

enum ExtruderState { FILLED_LOAD, FILLED_UNLOAD, EMPTY_LOAD, EMPTY_UNLOAD };

struct ScoreData
{
    // [STATE] Serialized review / upload payload for the marketplace score dialog.
    int                                           rating_id;
    int                                           design_id;
    std::string                                   model_id;
    int                                           profile_id;
...
{
    // [INTENT] Small custom-painted widget that visualizes nozzle / extruder occupancy and
    // load-unload direction for single, dual, or multi-nozzle machines.
    // [UNITY] Replace with a lightweight custom UI element that swaps sprites based on an
    // explicit extruder-state model; no direct wx paint equivalent exists.
    // [PORTING_HAZARD:P3] The widget depends on several pre-baked bitmap combinations and
    // manual paint logic, so image atlas selection must be centralized in Unity.
...
public:
    ...
    ExtruderImage(
        wxWindow* parent, wxWindowID id, int nozzle_num, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize);
    ~ExtruderImage();
};
...
{
public:
    // [INTENT] Transient status strip for extruder switching / loading feedback, with
    // optional quit and retry actions.
    // [STATE] Holds the live machine pointer, last control time, and button visibility.
    // [UNITY] Model as a compact status card with explicit action buttons and a timestamped
    // state machine.
...
{
public:
    // [INTENT] Modal upload-and-rating dialog for print results and model marketplace scores.
    // [STATE] Owns star selection, comment text, local/remote image mappings, and upload
    // status flags.
    // [THREAD] The dialog coordinates image loading and upload completion state that must
    // return to the UI thread.
    // [UNITY] Convert to a modal controller with a scrollable form, thumbnail grid, and async
    // upload service; image selection state should live in a view-model.
    ScoreDialog(
        wxWindow* parent, int design_id, std::string model_id, int profile_id, int rating_id, bool success_printed, int star_count = 0);
...
protected:
    enum StatusCode { UPLOAD_PROGRESS = 0, UPLOAD_EXIST_ISSUE, UPLOAD_IMG_FAILED, CODE_NUMBER };
    ...
    wxBoxSizer* get_main_sizer(const std::vector<std::pair<wxString, std::string>>& images = std::vector<std::pair<wxString, std::string>>(),
...
{
public:
    // [INTENT] Printable-task summary card used in the status dashboard to show progress,
    // timings, warnings, and print-result actions.
    // [STATE] Keeps thumbnail, progress, skipped-part counters, star-rating state, and the
    // current machine/task binding.
    // [EVENT] Exposes button and click handlers for pause/resume, abort, part-skip, retry,
    // and scoring actions.
    // [UNITY] Implement as a retained summary card with bound action buttons and a separate
    // data model for progress/telemetry updates.
...
{
protected:
    // [INTENT] Shared base layout for monitoring and machine-control pages.
    // It assembles camera/media controls, print-status widgets, temperature and fan controls,
    // axis jog controls, AMS controls, and calibration/filament sections.
    // [UNITY] This should become a scrollable dashboard page composed from reusable panels and
    // widgets, with machine commands routed through a controller/service layer.
    // [PORTING_HAZARD:P2] The base class owns many optional subtrees and virtual event hooks,
    // so Unity should avoid a single monolithic MonoBehaviour.
...
protected:
    // [INTENT] Concrete monitor/status screen for a specific machine, including transient
    // popups, dialogs, thumbnail/web-request state, and update orchestration.
    // [STATE] Caches last-seen machine bits, camera / recording / timelapse toggles,
    // calibration state, score data, and thumbnail/web request lifecycle flags.
    // [THREAD] Thumbnail refresh uses wxWebRequest and related callbacks, so response handling
    // must preserve UI-thread ownership when ported.
    // [UNITY] Use a screen controller backed by machine-state view models plus dedicated
    // popup/dialog services for web requests, camera, and scoring.
...
public:
    ...
    enum ThumbnailState { PLACE_HOLDER = 0, BROKEN_IMG = 1, TASK_THUMBNAIL = 2, SDCARD_THUMBNAIL = 3, STATE_COUNT = 4 };
