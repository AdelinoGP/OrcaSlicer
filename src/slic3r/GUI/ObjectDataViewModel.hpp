...
// [INTENT] This header defines the hierarchical data model for the object list (tree view) in the slicer's sidebar.
// It provides a wxDataViewModel implementation for displaying plates, objects, volumes, instances, settings, and layers
// with icons, extruder assignment, printability indicators, and warning icons.

// [STATE] The model manages:
// - Hierarchical tree of plates > objects > volumes/instances/layers
// - Visual state via bitmaps for icons, extruders, warnings
// - Mapping between UI indices and 3D model indices
// - Printability and variable height indicators

// [UNITY] This entire data model will be replaced with:
// - A C# ScriptableObject or class representing the hierarchical structure
// - UI Toolkit TreeView or ListView with custom item templates
// - Sprite/Texture assets for icons instead of wxBitmap
...
// ----------------------------------------------------------------------------
// [INTENT] ItemType enum defines the different node types in the object tree hierarchy.
// Each type represents a different level: plates, objects, volumes, instances, settings, layers.
enum ItemType {
    itUndef        = 0,
    itPlate        = 1,
    itObject       = 2,
...
// [INTENT] ColumnNumber enum defines the visible columns in the object list tree view.
// Each column shows different attributes: name, height, printability, extruder, painting info.
// [UNITY] Map to ListView columns in UI Toolkit with custom cell visualizers.
enum ColumnNumber {
    colName = 0, // item name
    colHeight,   // variable height
    colPrint,    // printable property
    colFilament, // extruder selection
    // BBS: Bambu-specific columns for support/color painting and sinking
    colSupportPaint,
    colColorPaint,
    colSinking,
...
// [INTENT] ObjectDataViewModelNode represents a single node in the object tree hierarchy.
// It stores all visual and state information for plates, objects, volumes, instances, settings, layers, and info items.
// [STATE] Important member variables:
// - m_parent/m_children: tree structure
// - m_bmp, m_extruder_bmp, m_*_icon: visual state
// - m_type: ItemType enum
// - m_idx, m_plate_idx: indices in model and plate
// - m_printable, m_variable_height: print state indicators
// [UNITY] Replace with a C# class or ScriptableObject that holds the same data but without wxBitmap references.
// Use UnityEngine.UIElements.VisualElement with Sprite references for icons.
class ObjectDataViewModelNode
{
...
public:
    ObjectDataViewModelNode(const wxString& name, const wxString& extruder, const int plate_idx, ModelObject* model_object)
        : m_parent(NULL), m_name(name), m_type(itObject), m_extruder(extruder), m_plate_idx(plate_idx), m_model_object(model_object)
    {
        set_icons();
        init_container();
    }
    ...
    MyObjectTreeModelNodePtrArray& GetChildren() { return m_children; }
    ObjectDataViewModelNode*       GetNthChild(unsigned int n) { return m_children.Item(n); }
    ...
    {
        if (GetChildCount() < 2 || frst_id < 0 || (size_t) frst_id >= GetChildCount() || scnd_id < 0 || (size_t) scnd_id >= GetChildCount())
            return false;
...
// [INTENT] ObjectDataViewModel is the main data model for the object list tree view.
// It inherits from wxDataViewModel and provides the hierarchical data structure and methods
// to manipulate the object tree (add/remove plates, objects, volumes, instances, settings, layers).
// [STATE] Maintains:
// - m_plates, m_objects: vectors of node pointers for top-level items
// - m_*_bmp: cached bitmaps for different node types
// - m_ctrl: associated wxDataViewCtrl for UI updates
// - m_ui_and_3d_volume_maps: mapping between UI volume indices and 3D model indices
// [EVENT] Custom event wxCUSTOMEVT_LAST_VOLUME_IS_DELETED notifies when last volume is deleted.
// [UNITY] Replace with a custom TreeModel implementation in C# that implements INotifyPropertyChanged.
// Use UI Toolkit's TreeView with a custom data source that mirrors this hierarchical structure.
...
public:
    ...
    int                                get_real_volume_index_in_3d(int ui_object_value, int ui_volume_value)
    {
        if (m_ui_and_3d_volume_maps.find(ui_object_value) != m_ui_and_3d_volume_maps.end()) {
            ...
            if (cur_map.find(ui_volume_value) != cur_map.end()) {
                return cur_map[ui_volume_value];
            }
        }
    ...
    wxDataViewItem AddInstanceChild(const wxDataViewItem&    parent_item,
    ...
    wxDataViewItem ReorganizeChildren(const int cur_volume_id, const int new_volume_id, const wxDataViewItem& parent);
    ...
    wxDataViewItem  SetPrintableState(PrintIndicator printable, int obj_idx, int subobj_idx = -1, ItemType subobj_type = itInstance);
