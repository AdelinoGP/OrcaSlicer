// [INTENT] Defines the data structures and classes for representing and rendering a 3D scene.
// It manages GLVolume (individual 3D parts) and GLVolumeCollection (groups of volumes).
// It also provides utilities for converting extrusion paths to renderable vertices.
...
// [INTENT] Represents a single renderable volume in the 3D scene (e.g., a part, support, or modifier).
// [UNITY] Maps to a GameObject with MeshFilter, MeshRenderer, and a metadata MonoBehaviour.
class GLVolume
{
public:
    std::string name;
    ...
    // [OPENGL] Update colors used in shaders based on current application theme/settings.
    ...
    // [STATE] Interaction state for selection and highlighting.
    ...
    // BBS
protected:
    // [STATE] Transformation matrices for the instance and volume.
    // [UNITY] Maps to the Transform component.
...
public:
    ...
    // [STATE] Unique identifier for the volume in the scene.
    // [PORTING_HAZARD:P2] Unity port will need a way to map these IDs to GameObjects for interaction.
    struct CompositeID
    {
        ...
        {
            return object_id == rhs.object_id && volume_id == rhs.volume_id && instance_id == rhs.instance_id;
        }
        ...
        bool operator<(const CompositeID& rhs) const
        {
            return object_id < rhs.object_id ||
                   (object_id == rhs.object_id &&
                    (volume_id < rhs.volume_id || (volume_id == rhs.volume_id && instance_id < rhs.instance_id)));
        }
    };
    ...
    EHoverState hover;
    // [STATE] The underlying geometry model.
    // [UNITY] Maps to Unity Mesh.
    GUI::GLModel model;
    // [EVENT] raycaster used for picking
    // [UNITY] Maps to MeshCollider or custom raycasting.
    ...
    // Bounding box of this volume, in unscaled coordinates.
    BoundingBoxf3 bounding_box() const { return this->model.get_bounding_box(); }
    ...
    {
        m_instance_transformation.set_offset(offset);
        set_bounding_boxes_as_dirty();
    }
    void set_instance_offset(Axis axis, double offset)
    {
        m_instance_transformation.set_offset(axis, offset);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_instance_transformation.set_rotation(rotation);
        set_bounding_boxes_as_dirty();
    }
    void set_instance_rotation(Axis axis, double rotation)
    {
        m_instance_transformation.set_rotation(axis, rotation);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_instance_transformation.set_scaling_factor(scaling_factor);
        set_bounding_boxes_as_dirty();
    }
    void set_instance_scaling_factor(Axis axis, double scaling_factor)
    {
        m_instance_transformation.set_scaling_factor(axis, scaling_factor);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_instance_transformation.set_mirror(mirror);
        set_bounding_boxes_as_dirty();
    }
    void set_instance_mirror(Axis axis, double mirror)
    {
        m_instance_transformation.set_mirror(axis, mirror);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_volume_transformation = transformation;
        set_bounding_boxes_as_dirty();
    }
    void set_volume_transformation(const Transform3d& transform)
    {
        m_volume_transformation.set_matrix(transform);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_volume_transformation.set_offset(offset);
        set_bounding_boxes_as_dirty();
    }
    void set_volume_offset(Axis axis, double offset)
    {
        m_volume_transformation.set_offset(axis, offset);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_volume_transformation.set_rotation(rotation);
        set_bounding_boxes_as_dirty();
    }
    void set_volume_rotation(Axis axis, double rotation)
    {
        m_volume_transformation.set_rotation(axis, rotation);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_volume_transformation.set_scaling_factor(scaling_factor);
        set_bounding_boxes_as_dirty();
    }
    void set_volume_scaling_factor(Axis axis, double scaling_factor)
    {
        m_volume_transformation.set_scaling_factor(axis, scaling_factor);
        set_bounding_boxes_as_dirty();
    }
    ...
    {
        m_volume_transformation.set_mirror(mirror);
        set_bounding_boxes_as_dirty();
    }
    void set_volume_mirror(Axis axis, double mirror)
    {
        m_volume_transformation.set_mirror(axis, mirror);
        set_bounding_boxes_as_dirty();
    }
    ...
    void set_offset_to_assembly(const Vec3d& offset)
    {
        m_offset_to_assembly = offset;
        set_bounding_boxes_as_dirty();
    }
...
// [INTENT] Manages a group of GLVolume instances.
// [UNITY] Maps to a Manager MonoBehaviour or a parent GameObject for scene organization.
class GLVolumeCollection
{
public:
    enum class ERenderType : unsigned char { Opaque, Transparent, All };

    // [STATE] Represents the printable and renderable bounds of the bed.
    struct PrintVolume
    {
...
private:
    ...
    // [OPENGL] Clipping planes used in shaders for cross-section views or gizmos.
    // [UNITY] Maps to Shader global properties or Material properties.
...
public:
    ...
    std::vector<int> load_object(const ModelObject*      model_object,
                                 int                     obj_idx,
    ...
    int load_object_volume(const ModelObject* model_object,
                           int                obj_idx,
                           int                volume_idx,
                           int                instance_idx,
                           const std::string& color_by,
    ...
    void load_object_auxiliary(const SLAPrintObject* print_object,
                               int                   obj_idx,
    ...
    // [OPENGL] Renders all volumes in the collection.
    // [UNITY] Unity's rendering pipeline (SRP/URP) handles this automatically via MeshRenderers.
    // Filter functions and manual rendering loops might be replaced by layer masks or custom RenderFeatures.
    void render(ERenderType type,
                ...
                bool                                 partly_inside_enable = true) const;
    ...
    bool check_outside_state(const Slic3r::BuildVolume&      build_volume,
...
GLVolumeWithIdAndZList volumes_to_render(const GLVolumePtrs&                  volumes,
                                         ...
                                         const Transform3d&                   view_matrix,
                                         std::function<bool(const GLVolume&)> filter_func = nullptr);

// [INTENT] Static utility class for converting extrusion entities (paths, loops) to vertex data for rendering.
// [UNITY] This logic should be ported to a C# utility class or a Job-based mesh generator.
struct _3DScene
{
    static void thick_lines_to_verts(const Lines&               lines,
    ...
    static void thick_lines_to_verts(const Lines3&              lines,
                                     ...
                                     bool                       closed,
                                     GUI::GLModel::Geometry&    geometry);
    static void extrusionentity_to_verts(const ExtrusionPath&    extrusion_path,
                                         float                   print_z,
                                         const Point&            copy,
                                         GUI::GLModel::Geometry& geometry);
    static void extrusionentity_to_verts(const ExtrusionLoop&    extrusion_loop,
                                         float                   print_z,
                                         const Point&            copy,
                                         GUI::GLModel::Geometry& geometry);
    static void extrusionentity_to_verts(const ExtrusionMultiPath& extrusion_multi_path,
                                         float                     print_z,
                                         const Point&              copy,
                                         GUI::GLModel::Geometry&   geometry);
    static void extrusionentity_to_verts(const ExtrusionEntityCollection& extrusion_entity_collection,
                                         float                            print_z,
                                         const Point&                     copy,
                                         GUI::GLModel::Geometry&          geometry);
    static void extrusionentity_to_verts(const ExtrusionEntity*  extrusion_entity,
                                         float                   print_z,
                                         const Point&            copy,
                                         GUI::GLModel::Geometry& geometry);
};
