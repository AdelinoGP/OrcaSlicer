// [ANNOTATED]
// [INTENT] Declarations for custom owner-drawn list item renderers (mixed bitmap+text).
// [STATE] Custom variant objects (DataViewBitmapText) and renderer state for managing inline editors and markup.
// [UNITY] COMPLETELY REDUNDANT. Map to Unity UI Toolkit UXML templates and C# ItemController classes for ListView/TreeView.
// [PORTING_HAZARD:P3] Platform-specific markup and icon rendering abstractions are unnecessary in Unity.
...
{
public:
    DataViewBitmapText(const wxString& text = wxEmptyString, const wxBitmap& bmp = wxNullBitmap) : m_text(text), m_bmp(bmp) {}
    ...
    bool IsSameAs(const DataViewBitmapText& other) const { return m_text == other.m_text && m_bmp.IsSameAs(other.m_bmp); }
...
{
public:
    TextRenderer(wxDataViewCellMode mode = wxDATAVIEW_CELL_INERT, int align = wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL)
