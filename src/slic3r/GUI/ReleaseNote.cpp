...
// [INTENT] Show release-note content in a lightweight modal dialog with Orca branding and DPI-aware layout.
// [STATE] Owns the title label and a scroll container that is populated on demand by update_release_note().
// [UNITY] Map this to a modal UI panel with a scrollable rich-text area plus a shared brand icon asset.
// [PORTING_HAZARD:P3] The current layout is sized by fixed DIP constants and expects wxSizer reflow to keep it readable.
...
{
    // [INTENT] Replace the placeholder body with version-specific notes and refresh the dialog chrome.
    // [STATE] Rebuilds the inner sizer each time, so the control tree is effectively ephemeral content.
    // [UNITY] Use a single scroll view with a recreated text subtree or a markdown document view.
...
// [INTENT] Prompt to install the network plug-in on next launch and delay the action until the user confirms.
// [STATE] Keeps an info header, a note body, and the OK/Cancel choice buttons in a small fixed dialog.
// [UNITY] Model this as a confirmation modal with persistent settings and a deferred install action.
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    operation_tips = new ::Label(this, Label::Body_12, _L("Click OK to update the Network plug-in when Orca Slicer launches next time."),
                                 LB_AUTO_WRAP);
...
{
    // [INTENT] Load plug-in metadata from JSON and project it into the dialog's summary labels.
    // [THREAD] File I/O stays on the UI thread here; Unity should move parsing to a background task if the payload grows.
    // [PORTING_HAZARD:P2] The method assumes a valid JSON schema and bails out on parse errors without fallback UI.
...
// [INTENT] Present the update-to-new-version workflow, including markdown rendering and browser launch for download links.
// [STATE] Owns both the webview-backed markdown page and the stable-update checkbox persisted in app config.
// [UNITY] Use a UI Toolkit scroll panel with a markdown renderer and a settings-backed toggle for update policy.
// [PORTING_HAZARD:P2] The dialog mixes embedded web content, localized labels, and direct default-browser navigation.
...
{
    // [EVENT] Hooks webview load/title/error callbacks so page lifecycle can drive markdown injection and navigation policy.
    // [UNITY] Replace with an explicit browser component or markdown document view with link interception.
...
{
    // [INTENT] Convert markdown content into a browser-consumable script call.
    // [STATE] Keeps the rendered HTML in the webview rather than in native widgets.
    // [PORTING_HAZARD:P3] Script-injection style updates are brittle; Unity should pass the text through a renderer API instead.
...
{
    // [INTENT] Render markdown release notes into HTML and switch the dialog into update-preview mode.
    // [STATE] Caches the generated HTML in html_source and locks the dialog size after content is loaded.
    // [UNITY] Replace with a markdown viewer / rich-text panel that can colorize for theme without HTML injection.
    // [PORTING_HAZARD:P2] The method mixes parsing, theme styling, and view loading in one pass.
    ...
    html_source      = (boost::format("<html><head><style>body { color: %1%; background-color: %2%; font-family: sans-serif; } a { color: "
                                 "#1E90FF }</style></head><body>") %
                        fg_color % bg_color)
                           .str();
    md_html(
        data_buf_in.data(), data_buf_in.length(),
...
// [INTENT] Provide a reusable secondary confirmation shell with configurable button combinations and optional "don't show again" state.
// [STATE] Tracks which button set is active, the parent event target, and whether the checkbox should persist suppression.
// [EVENT] Button clicks are forwarded to the parent via custom wxCommandEvents, then the dialog hides itself.
// [UNITY] Map to a reusable modal confirmation controller with a callback target and a settings-backed suppression toggle.
SecondaryCheckDialog::SecondaryCheckDialog(wxWindow*           parent,
                                           ...
                                           const wxSize&       size,
                                           long                style,
                                           bool                not_show_again_check) // ORCA VisibleButtons instead ButtonStyle
...
{
    // [EVENT] Push the custom completion signal back to the stored parent window instead of closing inline.
    // [THREAD] wxPostEvent keeps the dispatch on the UI loop; Unity should marshal this through the main-thread queue.
    if (event_parent) {
        event.SetString("");
...
{
    // [INTENT] Replace the dialog body with a wrapped text label and resize the content to fit the current message.
    // [STATE] Rebuilds the scroll content tree when the label is first created and then mutates it in place.
    wxBoxSizer* sizer_text_release_note = new wxBoxSizer(wxVERTICAL);
...
{
    // [EVENT] Reapplies button pressed visuals before showing the dialog to avoid stale hover states.
    // [UNITY] Equivalent behavior is a modal panel open animation plus explicit visual-state reset.
#ifdef __APPLE__
...
void SecondaryCheckDialog::update_title_style(wxString                             title,
                                              SecondaryCheckDialog::VisibleButtons style,
                                              wxWindow*                            parent) // ORCA VisibleButtons instead ButtonStyle
{
...
// [INTENT] Show a multi-action print-failure dialog with dynamic buttons and an optional remote image preview.
// [STATE] Keeps a button registry, used-button list, and web-request handle so the dialog can be reconfigured repeatedly.
// [THREAD] The image preview may arrive asynchronously over wxWebRequest and is applied back on the UI thread.
// [UNITY] Use a modal panel with a button template map and an async image loader.
// [PORTING_HAZARD:P2] Button visibility and close-box policy are changed at runtime based on the current failure mode.
...
{
    // [THREAD] Consumes web-request lifecycle events and mutates the dialog image state in response.
    // [UNITY] Replace with async image download completion callbacks that update a Texture2D or Sprite on the main thread.
...
{
    // [INTENT] Rebuild the message body, error code, and optional image preview for the current failure context.
    // [STATE] Keeps the fetched image in a shared wxWebRequest and resizes the dialog to the best-fit content size.
    // [PORTING_HAZARD:P3] The layout depends on content height heuristics and local image cache availability.
...
// [INTENT] Confirm pre-send warnings and optionally let the user update nozzle-related state before proceeding.
// [STATE] Reuses the same confirm/cancel shell as other dialogs but adds a conditional nozzle action and warning color mode.
// [UNITY] Model this as a reusable modal warning dialog with an optional third action button.
...
{
    // [INTENT] Render a single warning paragraph and size the modal to the text height.
    wxBoxSizer* sizer_text_release_note = new wxBoxSizer(wxVERTICAL);
    if (!m_staticText_release_note) {
        m_staticText_release_note      = new Label(m_vebview_release_note, text, LB_AUTO_WRAP);
        wxBoxSizer* top_blank_sizer    = new wxBoxSizer(wxVERTICAL);
...
{
    // [INTENT] Render a structured warning list with optional wiki links and colored severity emphasis.
    // [EVENT] Link labels open the browser and change cursor state on hover.
    // [UNITY] Use a scrollable list of rich-text rows with link handlers and severity styling.
    wxBoxSizer* sizer_text_release_note = new wxBoxSizer(wxVERTICAL);
    m_vebview_release_note->SetSizer(sizer_text_release_note);
...
// [INTENT] Collect printer IP, access code, and optional manual-setup metadata before dispatching a connection test.
// [STATE] Owns the current entry mode, status labels, step icons, worker, timer, and selected machine pointer.
// [THREAD] The network probe runs on a boost thread and posts results back through wx events and CallAfter.
// [UNITY] Replace with a wizard-style controller that advances between forms and runs the probe via async task/coroutine.
// [PORTING_HAZARD:P2] The dialog interleaves validation, worker lifetimes, and device-manager mutation in one flow.
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    comfirm_after_enter_text  = _L(
        "2. If the IP and Access Code below are different from the actual values on your printer, please correct them.");
    comfirm_last_enter_text = _L(
        "3. Please obtain the device SN from the printer side; it is usually found in the device information on the printer screen.");
...
{
    // [STATE] Toggles between direct-IP mode and manual-setup mode while preserving the current step index.
    m_button_manual_setup->Hide();
    if (index == 0) {
...
{
    // [THREAD] Cancels any outstanding worker before ending the modal, preventing stale callbacks into a dead dialog.
    if (m_thread) {
        m_thread->interrupt();
        m_thread->detach();
        delete m_thread;
...
{
    // [INTENT] Validate the current form values, then either emit a simple reconnect event or start the send worker.
    // [THREAD] The send-worker branch hands off to PlaterWorker and configures a cancel callback for cooperative aborts.
    m_test_right_msg->Hide();
    m_test_wrong_msg->Hide();
    m_img_step3->Hide();
    m_tip4->Hide();
    ...
    m_send_job->on_check_ip_address_fail([this](int result) { this->check_ip_address_failed(result); });
...
{
    // [THREAD] Background probe and device insertion logic executes off the UI thread and re-enters via wxPostEvent/CallAfter.
    // [PORTING_HAZARD:P2] The worker mixes network detection, device registration, and dialog completion state.
...
{
    // [EVENT] Live validation enables/disables the connect/manual buttons based on the current field contents.
    // [UNITY] Mirror this with input-field change listeners and a central validation method.
    auto str_ip              = m_input_ip->GetTextCtrl()->GetValue();
    auto str_access_code     = m_input_access_code->GetTextCtrl()->GetValue();
...
// [INTENT] Offer a compact failure dialog with a retry path and an alternate reconnect action.
// [UNITY] Map to a small modal confirmation box with two explicit buttons and no extra state.
