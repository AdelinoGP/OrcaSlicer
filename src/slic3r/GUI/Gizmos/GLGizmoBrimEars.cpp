...
// [STATE] Hover/selection/error palettes reused by render_points keep brim ears visually consistent and highlight editing context.
// [UNITY] Mirror these constants via a shared ScriptableObject so Unity brushes can reuse the same colors across Materials.

// [INTENT] Return the single selected ModelVolume or nullptr so brim ear logic only works on the active object.
// [PORTING_HAZARD:P3] Relies on Selection indices lining up with GLVolume storage; Unity needs to maintain the same metadata.
...
// [INTENT] Build the reusable cylinder mesh used for each brim ear marker so rendering only changes transforms and colors.
// [OPENGL] Unity should host this as a `Mesh` + `Material` combo and reuse it through `Graphics.DrawMesh` to mimic this shared geometry.
GLGizmoBrimEars::GLGizmoBrimEars(GLCanvas3D& parent, const std::string& icon_filename, unsigned int sprite_id)
...
{
    // [INTENT] Describe toolbar controls and shortcuts for the ImGui input panel; the strings feed the tooltip helper via `m_desc`.
    // [STATE] `m_shortcut_key` and sliders are initialized here so Unity can bind a dedicated `KeyboardShortcut` asset and toolbar tooltip.
    // [EVENT] This setup is read by `on_render_input_window` and `show_tooltip_information` when the gizmo is active.
    // [UNITY] Map these descriptors to a UI Toolkit `VisualElement` tree driven by the `BrimEarsPanelController`.
...
{
    // [STATE] Sync the brim cache with the active model so we only operate on the intended object.
    // [EVENT] Unity must call this when selection changes so the cached `m_editing_cache` stays accurate.
...
{
    // [OPENGL] Enable depth/blend when drawing gizmo geometry so brim ears integrate with the scene while writing to the main render
    // thread. [EVENT] Posting EVT_GLCANVAS_RESETGIZMOS keeps kernel in sync when the selection drifts off the current volume.
    ModelObject*     mo        = m_c->selection_info()->model_object();
...
{
    // [STATE] `m_editing_cache` plus `render_hover_point` drive what gets drawn; keep it synced with selection before each frame.
    // [UNITY] Unity should feed an entry list to a `BrimEarRenderer` MonoBehaviour that mirrors this cache and renders with `Graphics.DrawMesh`.
    auto editing_cache = m_editing_cache;
    if (render_hover_point) {
        editing_cache.push_back(*render_hover_point);
    }
...
bool GLGizmoBrimEars::on_mouse(const wxMouseEvent& mouse_event)
{
    ...
    if (mouse_event.LeftDown()) {
        ...
        if (!control_down && selected_object_idx != -1 && gizmo_event(SLAGizmoEventType::RightDown, mouse_pos, false, false, false))
...
{
    // [EVENT] Mouse/keyboard router that returns true when the input is consumed so GLCanvas3D can stop propagation.
    // [STATE] `m_selection_rectangle`, `m_hover_id`, and modifier keys determine whether points are placed, selected, or deleted.
    // [UNITY] Future port should implement this as a `BrimEarGizmoController` component wired to Unity's EventSystem + InputActions.
    ...
    if (action == SLAGizmoEventType::Moving) {
        ...
        if (unproject_on_mesh2(mouse_position, pos_and_normal)) {
            render_hover_point = CacheEntry(BrimPoint(pos_and_normal.first, m_new_point_head_diameter / 2.f), false,
                                            (inverse_trsf * m_world_normal).cast<float>(), true);
        } else {
    ...
    if ((action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::ShiftUp || action == SLAGizmoEventType::AltUp) &&
        ...
        for (unsigned int i = 0; i < m_editing_cache.size(); ++i)
            points.push_back(trafo.get_matrix() * m_editing_cache[i].brim_point.pos.cast<double>());
        ...
        for (size_t idx : points_idxs)
            points_inside.emplace_back(
                (trafo.get_matrix().cast<float>() * (m_editing_cache[idx].brim_point.pos + m_editing_cache[idx].normal)).cast<float>());
...
void GLGizmoBrimEars::on_dragging(const UpdateData& data)
{
    if (m_hover_id != -1) {
        std::pair<Vec3f, Vec3f> pos_and_normal;
        if (!unproject_on_mesh2(data.mouse_pos.cast<double>(), pos_and_normal))
            return;
...
void GLGizmoBrimEars::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    GizmoImguiBegin(get_name(), ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
    ...
    ImGui::SameLine(caption_size);
    ImGui::PushItemWidth(slider_width);
    m_imgui->bbl_slider_float_style("##detection_radius", &m_detection_radius, 0, static_cast<int>(m_detection_radius_max), "%.1f", 1.0f,
                                    true);
    ImGui::SameLine(drag_left_width);
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    ...
    if (m_imgui->button(m_desc["auto_generate"])) {
        auto_generate();
    }
...
{
    // [EVENT] Flushes brim point edits back into the ModelObject and signals a background refresh; Unity should expose this as a
    // `BrimEarDirty` event feeding into its background process queue.
    ModelObject* mo = m_c->selection_info()->model_object();
    if (mo) {
        mo->brim_points.clear();
        for (const CacheEntry& ce : m_editing_cache)
            mo->brim_points.emplace_back(ce.brim_point);
...
void GLGizmoBrimEars::select_point(int i)
{
    if (i == AllPoints || i == NoPoints) {
        ...
        if (i == AllPoints)
            m_new_point_head_diameter = m_editing_cache[0].brim_point.head_front_radius * 2.f;
    } else {
...
void GLGizmoBrimEars::reload_cache()
{
    ...
    for (const BrimPoint& point : mo->brim_points)
        m_editing_cache.emplace_back(point);
    find_single();
}
...
{
    // [INTENT] Slices the first layer to find hulls where brim ears can land, reusing the existing mesh pipeline.
    // [THREAD] This uses `slice_mesh_ex`, so porting to Unity should push it into a `Job` before touching the render thread.
    // [PORTING_HAZARD:P2] The Clipper-based slicing is sequential and expensive; Unity needs either an equivalent geometry resolver or
    // precomputed offsets.
    const Selection&              selection = m_parent.get_selection();
    const Selection::IndicesList& idxs      = selection.get_volume_idxs();
    if (idxs.size() <= 0)
...
bool GLGizmoBrimEars::add_point_to_cache(Vec3f pos, float head_radius, bool selected, Vec3f normal)
{
    ...
    for (int i = 0; i < m_editing_cache.size(); i++) {
        if (m_editing_cache[i].brim_point == point) {
            return false;
        }
    }
...
void GLGizmoBrimEars::on_register_raycasters_for_picking() { update_raycasters(); }
...
void GLGizmoBrimEars::update_raycasters()
{
    ...
    if (m_editing_cache.size() < m_grabbers.size()) {
        ...
        while (remaining > 0) {
            ...
            g.raycasters[0] = m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, id, *m_cylinder.mesh_raycaster,
...
float GLGizmoBrimEars::get_brim_default_radius() const
{
    const double nozzle_diameter =
        wxGetApp().preset_bundle->printers.get_edited_preset().config.option<ConfigOptionFloats>("nozzle_diameter")->get_at(0);
