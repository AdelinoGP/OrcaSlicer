...
// [INTENT] Support painting gizmo bridging triangle selectors, GUI input, and background generation so users can manually adjust FDM supports.
...
{
    // [STATE] Initialize painter-specific tool state and cursor defaults before any render/update begins.
...
{
    // [THREAD] Shutdown sequence is always triggered from the main UI path, so cancel and join the background support generation job
    // safely. [PORTING_HAZARD:P1] Unity needs an explicit CancellationToken/Job handle since this code waits for boost::thread and
    // interacts with wx Print resources. BBS wait the thread
...
{
    // [STATE] Setting up default slope angle state and marking the edit_flags ready for user painting.
...
std::string GLGizmoFdmSupports::on_get_name() const { return _u8L("Support Painting"); }

bool GLGizmoFdmSupports::on_init()
{
    // [EVENT] Registering shortcuts and UI labels that drive support painting interactions so the control panel can show affordances.
    // [STATE] Build a descriptive lookup table upfront rather than recalculating text on every frame.
    // BBS
...
void GLGizmoFdmSupports::render_painter_gizmo()
{
    ...
    // [OPENGL] Enable blending and depth test to paint selection meshes and support previews on top of the scene.
    // [STATE] Guard support volume rendering based on the cached ready flag so we do not draw stale previews.
    // [UNITY] Unity should mirror this by toggling a transparent MeshRenderer + Material when the preview mesh is ready.
...
// BBS
// [EVENT] Map keyboard shortcuts directly to tool icons so the UI stays in sync with cursor state without extra polling.
...
void GLGizmoFdmSupports::on_set_state()
{
    ...
    // [STATE] Track on/off transitions so we can clear threshold state or force mesh persistence when the gizmo toggles.
...
{
    // [UNITY] Equivalent to a UI Toolkit window with slider controls; Unity needs to avoid RTTI by shipping state via serialized fields and
    // event callbacks.
    init_print_instance();
    // [STATE] Bail early if no model is selected so the gizmo stays dormant outside valid selection scopes.
    ...
    const float reset_button_slider_left = m_imgui->calc_text_size(m_desc.at("reset_direction")).x + m_imgui->scaled(1.5f) +
    ...
    const float sliders_left_width = std::max(gap_area_txt_width,
                                              std::max(smart_fill_angle_txt_width,
                                                       std::max(reset_button_slider_left,
                                                                std::max(std::max(cursor_slider_left, clipping_slider_left),
    ...
    // [STATE] tool tips + current icon selection sync to highlight the active brush and update tooltip guidance.
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text,
                              ImVec4(1.f, 1.f, 1.f, 1.f)); // ORCA: Fixes icon rendered without colors while using Light theme
        ...
        if (btn_clicked && m_current_tool != tool_ids[i]) {
            // [EVENT] Fire off selector reset to keep triangle selection state consistent whenever the brush type changes.
    ...
    // [EVENT] Toggling the overhang-only checkbox drives `m_paint_on_overhangs_only`, which short-circuits the Brush drawing logic.
    ...
    if (m_current_tool == ImGui::CircleButtonIcon) {
        ...
        std::string format_str = std::string("%.f") +
                                 I18N::translate_utf8("", "Face angle threshold, placed after the number with no whitespace in between.");
        ImGui::SameLine(sliders_left_width);
        ImGui::PushItemWidth(sliders_width);
    ...
    if (m_imgui->bbl_slider_float_style("##angle_threshold_deg", &m_highlight_by_angle_threshold_deg, 0.f, 90.f, format_str.data(), 1.0f,
                                        true, tooltip)) {
        m_parent.set_slope_normal_angle(90.f - m_highlight_by_angle_threshold_deg);
        if (!m_parent.is_using_slope()) {
            m_parent.use_slope(true);
    ...
    if (m_current_tool != ImGui::GapFillIcon) {
        ...
        if (b_bbl_slider_float || b_drag_input)
    ...
    if (m_current_tool == ImGui::GapFillIcon) {
        if (m_imgui->button(m_desc.at("perform"))) {
            // [EVENT] `Perform` launches a gap-fill adjustment via undo snapshot and re-renders the mesh.
    ...
    if (m_imgui->button(m_desc.at("remove_all"))) {
        // [EVENT] The "Erase all" action resets all triangle selectors and posts a dirty flag so Unity can refresh overlays.
        Plater::TakeSnapshot snapshot(wxGetApp().plater(), "Reset selection", UndoRedo::SnapshotType::GizmoAction);
        ModelObject*         mo  = m_c->selection_info()->model_object();
        int                  idx = -1;
...
{
    // [STATE] Keep TriangleSelectorPatch filters aligned with the brush mode so gap-fill uses a different filter state than the regular brush.
...
void GLGizmoFdmSupports::show_tooltip_information(float caption_max, float x, float y)
{
    ...
    // [EVENT] Tooltips are rendered in response to UI hover events and relay which inputs are bound to each action.
    ...
    if (ImGui::IsItemHovered()) {
        ...
        switch (m_tool_type) {
        case ToolType::BRUSH: tip_items = {"enforce", "block", "remove", "cursor_size", "clipping_of_view"}; break;
        case ToolType::BUCKET_FILL: break;
        case ToolType::SMART_FILL: tip_items = {"enforce", "block", "remove", "smart_fill_angle", "clipping_of_view"}; break;
        case ToolType::GAP_FILL: tip_items = {"gap_area"}; break;
        default: break;
        }
...
{
    // [STATE] Read enable/support settings from object/global configs to determine whether auto-support thresholds apply.
    ...
    SupportType support_type   = obj_cfg.option("support_type") ? obj_cfg.opt_enum<SupportType>("support_type") :
                                                                  glb_cfg.opt_enum<SupportType>("support_type");
    int         support_threshold_angle = obj_cfg.option("support_threshold_angle") ? obj_cfg.opt_int("support_threshold_angle") :
...
{
    // [PORTING_HAZARD:P2] Mesh filtering mutates triangle selector state directly; Unity should mirror this with compute filters +
    // MeshCollider updates instead of raw triangle access.
    ...
    Plater::TakeSnapshot snapshot(wxGetApp().plater(), block ? "Block supports by angle" : "Add supports by angle");
    update_model_object();
    m_parent.set_as_dirty();
}
...
{
    // [EVENT] Propagate painter edits back into the ModelObject and raise the background process event for downstream layers.
    bool         updated = false;
    ModelObject* mo      = m_c->selection_info()->model_object();
    int          idx     = -1;
    for (ModelVolume* mv : mo->volumes) {
...
void GLGizmoFdmSupports::update_from_model_object(bool first_update)
{
    ...
    // [STATE] Rebuild triangle selectors + timestamp cache whenever the underlying ModelObject changes so UI mirrors stored support stamps.
...
{
    // [INTENT] Advertise this painter as the FDM support tool so the manager can route commands correctly.
...
{
    // [EVENT] Names for the undo snapshot help track whether the user is enforcing or blocking supports per button press.
    // BBS remove _L()
...
{
    // [THREAD] Run on the UI thread because it looks up PrintObjects attached to the wx::PrintView; Unity needs to cache the equivalent
    // PrintContext before dispatching worker jobs. [PORTING_HAZARD:P2] This lives deep in wxJobs/Print infrastructure
    // (Print->PrintObject->PrintInstance), so the Unity port must mirror the lifetime or mock it via sanitized snapshots.
...
void GLGizmoFdmSupports::invalid_support_volumes(bool invalid_step)
{
    ...
    // [THREAD] Guard state shared with the background run_thread to avoid race conditions while cancelling or restarting.
    m_volume_valid = false;
...
{
    // [STATE] Compare cached object/timestamp state against the last generated support mesh to avoid redundant regeneration.
    if (!m_support_volume)
        return true;
    ...
    if (m_object_id != m_print_instance.print_object->id().id) {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ",object_id changed from " << m_object_id << " to "
                                << m_print_instance.print_object->id().id << ", need to regenerate";
        return true;
    }
...
{
    // [THREAD] Called from the UI path; switches into background generation via `m_thread` while holding mutex-synchronized flags.
    // [UNITY] Replace the boost::thread/GLVolume pattern with a Unity Task or Job that builds a Mesh + MeshRenderer and invokes
    // `UnityMainThreadDispatcher` when ready. PrintInstance m_print_instance = get_current_print_instance();
    ...
    m_cancel = false;
    // [THREAD] Always re-create the worker thread to execute `run_thread` so cancellation states can be observed safely.
...
{
    // [THREAD] Runs on boost::thread and marshals results back to the UI thread; Unity should use a Task/Job that reports via
    // `MainThreadDispatcher`. [PORTING_HAZARD:P1] Direct PrintObject/GLVolume interaction must be guarded so Unity does not trigger GL
    // calls off the main thread.
    // [UNITY] Model the support preview generation as a `Task` + `MainThreadDispatcher` pair: worker builds the mesh data, dispatcher
    // creates the MeshRenderer.
    try {
        Print* print = m_print_instance.print_object->print();
        ...
        // [OPENGL] GLVolume holds raw GPU geometry; Unity should create a Mesh / MeshRenderer with similar color + alpha.
...
{
    // [STATE] Flip the volume-ready flags so the renderer knows the preview mesh is valid for display.
