...
protected:
    ...
    // [EVENT] Compose the tooltip text for the input window; triggered during mouse hover in the painter overlay.
...
private:
    // [INTENT] Prepare painter-specific resources before the gizmo is activated so the GL overlay has everything it needs.
    bool on_init() override;

    // BBS: remove const
    // [STATE] Re-syncs support settings when the underlying object changes to keep the preview consistent with the project.
    ...
    // [STATE] Cached slider value for the facet angle threshold; drives how aggressive support selection is.
    int m_support_threshold_angle = -1;

    // BBS: add support preview logic
    // [INTENT] Captures a frozen set of print settings so preview generation does not mutate the live Print object.
    ...
    // [THREAD] Recomputes support volume geometry on the dedicated thread before the UI renders it.
    ...
    void run_thread();
    // [STATE] Angle threshold that drives facet selection heuristics.
    ...
    // [OPENGL] Generated preview mesh consumed in `render_painter_gizmo` and updated only when `m_volume_ready` is true.
    ...
    // [STATE] Cancellation flag observed by the worker thread.
    bool m_cancel = false;
    ...
    // [STATE] Snapshotted print instance for deterministic preview generation independent of live edit state.
    ...
    std::mutex m_mutex;
    // [STATE] Counts pending generate requests to avoid redundant worker restarts.
