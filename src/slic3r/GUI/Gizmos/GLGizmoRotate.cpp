...
// [INTENT] Axis-aligned rotate handles overlay the scene to communicate current selection rotation, snapping bounds, and scale marks.
// [STATE] These static offsets, step counts, and snap radii tuners keep the runtime meshes stable until a significant dimension change
// occurs. [PORTING_HAZARD:P2] The GL draw calls downstream rely on immediate-mode shader uniforms and bespoke line models; Unity will need
// to mirror this with MeshRenderer + Material updates per frame. [UNITY] Use a dedicated MonoBehaviour per axis to regenerate child
// MeshFilters/MeshRenderers and update shader properties (view/projection matrices) via a RenderTexture or UI Toolkit overlay with
// GraphicRaycaster for tooltips.

// [STATE] Each instance targets a single axis so hover/pick IDs stay unique and highlight/drag defaults persist across activation.
...
void GLGizmoRotate::set_highlight_color(const ColorRGBA& color) { m_highlight_color = color; }
...
{
    // [EVENT] Tooltip text is recalculated only when grabber hover/drag state changes so UI notices axis-specific angles without extra polling.
    std::string axis;
    switch (m_axis) {
    case X: {
        axis = "X";
...
{
    // [EVENT] Delegate mouse hits to grabbers so hover_id, highlight, and dragging are kept consistent before rendering.
...
bool GLGizmoRotate::on_init()
{
    ...
    m_grabbers.back().extensions = (GLGizmoBase::EGrabberExtension) (int(GLGizmoBase::EGrabberExtension::PosY) |
                                                                     int(GLGizmoBase::EGrabberExtension::NegY));
    return true;
}

void GLGizmoRotate::on_start_dragging()
{
    // [STATE] Re-sync the cached bounding box and radius when dragging starts so the handle aligns with the selection's latest transform.
    init_data_from_selection(m_parent.get_selection());
}

void GLGizmoRotate::on_dragging(const UpdateData& data)
{
    // [STATE] Project the mouse ray into the current axis plane, apply coarse/fine snap radii, and store the normalized rotation while dragging.
...
{
    // [OPENGL] Depth testing, eye-space matrices, and shader uniforms are managed per-frame before drawing the cached circle, scale marks,
    // and grabber highlight. [UNITY] Mirror this with a MeshRenderer + Material that receives camera matrices from a dedicated Gizmo camera
    // and uses a GraphicRaycaster overlay for hover colors instead of immediate GL.
...
{
    // [STATE] Cache bounding box, sphere, and orientation matrix from the current selection so rendering and snapping use consistent
...
{
    // [INTENT] Render the ImGui rotate input dialog to surface the optimizations choices when the gizmo is active.
...
{
    // [STATE] Pulls saved accuracy/method preferences from config so the rotate dialog survives restarts and matches the cached state.
...
{
    // [OPENGL] Lazily rebuild the line loop representing circle outlines whenever the cached radius changes so GPU buffers stay aligned
    // with the selection.
...
{
    // [OPENGL] Builds the scale tick lines for every rotation step so the mesh can be re-used until the radius shifts.
...
{
    // [STATE] Build the inner/outer snap radius indicators once per radius change so snapping regions stay visible on hover.
...
{
    // [OPENGL] A thin axis radial line shows the reference direction; rebuild only when the radius changes to avoid extra buffer churn.
...
{
    // [STATE] Angle arc rebuilds whenever radius or angle deltas occur so the wedge always tracks the current drag phase.
...
{
    // [OPENGL] Draws the spoke from origin to grabber head; rebuild only when cached center shifts to minimize GLModel churn.
    if (!m_grabber_connection.model.is_initialized() || radius_changed ||
        !m_grabber_connection.old_center.isApprox(m_grabbers.front().center)) {
...
{
    // [INTENT] Respect the cached highlight color so the grabber stays consistent while drawing even when the bounding box updates.
...
{
    // [STATE] Axis-dependent rotation offsets adjust the orientation matrix so the local plane stays aligned with the chosen axis.
    Transform3d ret;
...
{
    // [EVENT] Transform the mouse ray into the axis plane so on_dragging can compute a 2D direction vector, including handling parallel cases.
...
{
    // [EVENT] Drag gestures rotate the selection via TransformationType adjustments while the grabbers maintain hover state.
...
{
    // [INTENT] Initialize each axis gizmo and assign axis colors + hotkey so rotate can show all 3 surfaces consistently.
...
{
    // [STATE] When the gizmo is activated, lock coordinate mode to world and reset the cached last volume to avoid stale references when toggling.
    for (GLGizmoRotate& g : m_gizmos)
        g.set_state(m_state);
    if (get_state() == On) {
...
{
    // [THREAD] Track selection changes on the UI thread, refresh cached volume pointers, and set object manipulation rotation without
    // blocking render.
    const Selection& selection = m_parent.get_selection();
    ...
    if (is_wipe_tower) {
        ...
        float wipe_tower_rotation_angle = dynamic_cast<const ConfigOptionFloat*>(config.option("wipe_tower_rotation_angle"))->value;
...
{
    // [STATE] Only allow activation when there is a non-empty, non-wipe-tower selection so rotate cues stay valid.
...
{
    // [OPENGL] Clears depth and renders each axis circle in the correct order so axis-specific hover coloring occurs without z-fighting.
...
{
    // [EVENT] Register the axis grabbers with the scene raycaster so pointer hits always prioritize gizmos drawn on top.
...
{
    // [EVENT] Remove gizmo raycasters and restore the parent raycaster flag to drop overlays when the gizmo hides.
    for (GLGizmoRotate& g : m_gizmos) {
...
GLGizmoRotate3D::RotoptimzeWindow::RotoptimzeWindow(ImGuiWrapper* imgui, State& state, const Alignment& alignment) : m_imgui{imgui}
{
    // [THREAD] Builds the ImGui rotate optimizer UI on the main thread and dispatches RotoptimizeJob via the UI job worker so expensive
    // searches run in background.
    imgui->begin(_L("Optimize orientation"), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);
    // [UNITY] Mirror this as a UI Toolkit (or UIElements) popup window with a ListView/ComboBox and buttons anchored to the viewport toolbar.
    ...
    for (size_t i = 0; i < RotoptimizeJob::get_methods_count(); ++i) {
        float w    = ImGui::CalcTextSize(RotoptimizeJob::get_method_name(i).c_str()).x + padding.x + ImGui::GetFrameHeight();
    ...
    if (ImGui::BeginCombo("", RotoptimizeJob::get_method_name(state.method_id).c_str())) {
        // [PORTING_HAZARD:P3] ImGui combos rely on immediate mode layout; Unity needs a UI Toolkit popup (VisualElement + ListView) or a
        // custom dropdown to match.
        for (size_t i = 0; i < RotoptimizeJob::get_methods_count(); ++i) {
    ...
    if (imgui->button(btn_txt)) {
        // [THREAD] Dispatch a RotoptimizeJob so the orientation optimization happens on the UI job worker instead of blocking render.
        replace_job(wxGetApp().plater()->get_ui_job_worker(), std::make_unique<RotoptimizeJob>());
    }

    imgui->disabled_end();
}

GLGizmoRotate3D::RotoptimzeWindow::~RotoptimzeWindow() { m_imgui->end(); }
