...
{
    // [INTENT] Exit the paint-on mode by restoring parent view controls so other gizmos can reuse the model cleanly.
    // [STATE] Undo the slope override and re-show any hidden objects that this painter temporarily hid.
    // [UNITY] Unity would undo a `GizmoController` overlay and re-enable any `GameObject` renderers hidden for this tool.
    m_parent.use_slope(false);
    m_parent.toggle_model_objects_visibility(true);
}

std::string GLGizmoFuzzySkin::on_get_name() const { return _u8L("Paint-on fuzzy skin"); }

bool GLGizmoFuzzySkin::on_init()
{
    m_shortcut_key = WXK_CONTROL_H;

    // [STATE][INTENT] Build the localized shortcut/help strings once per gizmo instance so the panel text matches the current locale.
...
void GLGizmoFuzzySkin::render_painter_gizmo()
{
    ...
    // [OPENGL] Blend + depth testing are required for the cursor overlay and colored mesh preview to layer over the scene.
    // [STATE] The `selection` collider drives the mesh that gets highlighted every frame.
    // [UNITY] Recreating this in Unity means calling `Graphics.DrawMesh` inside `OnRenderObject` or a custom `GizmoRenderer` with
    // `Blend`/`DepthTest` toggled. [PORTING_HAZARD:P2] wxWidgets expects no other OpenGL state mutators between render passes, so a Unity
    // port must guard shared `RenderTexture` state.
...
{
    // [EVENT] Tooltip contents are driven by ImGui hover detection, so the UI thread only draws this when the cursor stays over the tooltip
    // icon. [UNITY] In Unity this would map to a `GraphicRaycaster` + `TooltipController` showing `TextMeshPro` content.
    ...
    if (ImGui::IsItemHovered()) {
        ...
        case ToolType::BRUSH:
            ...
            break;
        case ToolType::SMART_FILL: tip_items = {"add_fuzzy_skin", "remove_fuzzy_skin", "smart_fill_angle", "clipping_of_view"}; break;
        default: break;
        }
...
{
    // [STATE] Grab the selected model so the gizmo reflects per-object config overrides.
    // [THREAD] Must run on the UI thread because it constructs ImGui widgets and touches selection info.
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (btn_clicked && m_current_tool != tool_ids[i]) {
            // [STATE] `m_current_tool` drives the following UI and brush behavior and keeps ImGui selection in sync.
    ...
    if (m_c->object_clipper()->get_position() == 0.f) {
        ...
        if (m_imgui->button(m_desc.at("reset_direction"))) {
            // [EVENT][THREAD] Resetting the clip direction happens asynchronously so the tooltip UI stays responsive while wxWidgets
            // repositions the cut plane.
    ...
    if (m_imgui->button(m_desc.at("remove_all"))) {
        // [EVENT] Left-clicking "Erase all" triggers an Undo snapshot + reset of the painted mesh.
        // [UNITY] Unity would push this through an `UndoRedoService` + clear the `PaintMask` data stored on a `ScriptableObject`.
    ...
    if (effective_fuzzy_skin_state == FuzzySkinType::Disabled_fuzzy) {
        ...
        // [STATE] Warn the user when fuzzy skin painting is disabled globally; clicking the text restores the `fuzzy_skin` override.
        // [UNITY] Unity would render this line as a hyperlink in a `UIToolkit` `Label` and toggle a `SerializedObject` boolean.
        link_text();
    }
    ImGui::PopStyleVar(2);
    GizmoImguiEnd();
...
{
    // [STATE] Transfer the triangle selectors' painted mask back into each `ModelVolume` so the data persist across saves.
    // [THREAD] After the update we fire a `SimpleEvent` that schedules `BackgroundSlicingProcess` on the UI thread.
    bool         updated = false;
    ModelObject* mo      = m_c->selection_info()->model_object();
    int          idx     = -1;
    for (ModelVolume* mv : mo->volumes) {
...
{
    // [THREAD] Updates run with a `wxBusyCursor` because they deserialize mesh state on the UI thread before painting.
    // [STATE] Each `TriangleSelectorPatch` deserializes from the `fuzzy_skin_facets` cache, so the gizmo mirrors the saved mask.
    wxBusyCursor wait;
...
PainterGizmoType GLGizmoFuzzySkin::get_painter_type() const { return PainterGizmoType::FUZZY_SKIN; }
