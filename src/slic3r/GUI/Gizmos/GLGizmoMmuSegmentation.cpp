...
// [INTENT] GLGizmoMmuSegmentation coordinates MMU color painting: selection state, triangle selectors, and remapping UI.
// [STATE] Caches include per-extruder palettes, remap tables, and the current tool/brush configuration that drive ImGui sliders and mesh
// highlights. [EVENT] Lifecycle hooks from GLGizmoPainterBase (on_init/on_opening/on_shutdown) orchestrate notification, state refresh, and
// cleanup. [UNITY] Port as a PainterGizmoController MonoBehaviour that drives a RenderTexture/mesh highlight pipeline while exposing an UI
// Toolkit window for sliders and remapping controls. [PORTING_HAZARD:P2] Tight coupling to wxApp globals and manual triangle selector VBO
// updates requires refactoring into Unity-friendly services (ScriptableObject settings + jobified mesh updates).

// [EVENT] Called from on_opening so the notification system highlights the filament limit before painting starts.
// [UNITY] Translate to NotificationCenterController.Post(new TooltipNotification("Filament limit")) when the MonoBehaviour is enabled.
    ...
    wxGetApp()
        ...
        ->push_notification(NotificationType::MmSegmentationExceededExtrudersLimit,
                            ...
                            GUI::format(_L("Filament count exceeds the maximum number that painting tool supports. Only the "
                                        ...
                                        GLGizmoMmuSegmentation::EXTRUDERS_LIMIT));
}

// [EVENT] Invoked when the painter gizmo enters the active state; validates extruder count before letting users paint.
// [STATE] Depends on wxApp filament counts so Unity needs to pull from the shared filament registry before enabling the controller.
...
// [STATE] Restores slope flags and model visibility when the gizmo shuts down so the viewer returns to the default interaction mode.
// [THREAD] Runs on the GL thread during gizmo teardown, so Unity will also need to toggle GameObject visibility on the main thread.
...
// [INTENT] Titles the painter window so the Unity overlay can show the same "Color Painting" label when the tool is active.
std::string GLGizmoMmuSegmentation::on_get_name() const { return _u8L("Color Painting"); }

// [STATE] Gate enabling the painter on ptFFF printers with >1 filament so Unity can gray out the action button otherwise.
...
// [EVENT] Activation requires a meaningful selection; reroute to Unity by checking MeshSelectionService before enabling the controller.
...
// [STATE] Synchronizes the toolkit palette with printer-exposed filaments so selections remain coherent across remaps.
// [UNITY] In Unity this should pull from a shared FilamentPalette ScriptableObject and refresh cached color arrays.
...
// [EVENT] Registers the ImGui shortcut hints (`m_desc`) and initializes per-tool metadata so the UI layout can compute widths before
// drawing. [STATE] Builds localization hints and slider captions that Unity's UI Toolkit tree will need to mirror for responsive layout.
...
// [OPENGL] Enables blending/depth once per frame, renders triangle selectors, and draws cursor overlays before restoring GL state.
// [UNITY] Equivalent to updating a MeshRenderer with the triangle selection mesh followed by Graphics.DrawMesh for the highlighted cursor.
...
// [EVENT] Triggered when selection/preset changes occur; keeps triangle selectors, colors, and remap caches synced with wxApp.
// [STATE] Reinitializes palettes only when filament count or colors diverge, minimizing GPU churn for Unity's mesh cache.
...
// [EVENT] Number keys switch the active filament color, mirroring the ImGui palette buttons for quick tactile selection.
// [UNITY] Map to InputAction bindings or keyboard shortcuts that update a serialized currentIndex field on the MonoBehaviour.
// BBS
...
{
    // [INTENT] Build the squared swatches combo used in the filament palette so that Unity can recreate the same selection grid.
    // [UNITY] Translate to a UI Toolkit selectable list with colored boxes and an InputAction for selection so the C# designer sees the
    // same ordering. [UNCLEAR] The second assert compared `extruders_colors` against itself, so double-check the intended consistency guard
    // during porting.
    ...
    if (ImGui::BeginCombo(label.c_str(), "")) {
        for (size_t extruder_idx = 0; extruder_idx < std::min(extruders.size(), GLGizmoMmuSegmentation::EXTRUDERS_LIMIT); ++extruder_idx) {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(start_position,
                                                      ImVec2(start_position.x + height + height / 2, start_position.y + height),
                                                      ImGuiWrapper::to_ImU32(extruders_colors[extruder_idx]));
            ImGui::GetWindowDrawList()->AddRect(start_position, ImVec2(start_position.x + height + height / 2, start_position.y + height),
                                                IM_COL32_BLACK);
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x + height + height / 2, p.y + height),
                                              ImGuiWrapper::to_ImU32(extruders_colors[selection_idx]));
...
{
    // [EVENT] Draws a tooltip when the info icon hovers; Unity should mirror this via UI Toolkit Tooltip/VisualElement pair.
    ...
    if (ImGui::IsItemHovered()) {
        ...
        switch (m_tool_type) {
        case ToolType::BRUSH: tip_items = {"paint", "erase", "cursor_size", "clipping_of_view", "toggle_wireframe"}; break;
        case ToolType::BUCKET_FILL: tip_items = {"paint", "erase", "smart_fill_angle", "clipping_of_view", "toggle_wireframe"}; break;
        case ToolType::SMART_FILL:
            // TODO:
            break;
        case ToolType::GAP_FILL: tip_items = {"gap_area", "toggle_wireframe"}; break;
        default: break;
        }
...
{
    // [INTENT] Drives the ImGui window for the painter, computing width hints, slider offsets, and tool palettes every frame.
    // [EVENT] Called from GLGizmoPainterBase render loop; in Unity this would be a VisualElement window nested inside a UI Toolkit overlay camera.
    ...
    for (const auto& t :
         std::array<std::string, 6>{"paint", "erase", "cursor_size", "smart_fill_angle", "height_range", "clipping_of_view"}) {
    ...
    draw_list->AddRectFilled({pos.x - 10.0f, pos.y - 7.0f}, {pos.x + window_width + ImGui::GetFrameHeight(), pos.y + color_button_high},
                             ImGui::GetColorU32(ImGuiCol_FrameBgActive, 1.0f), 5.0f);
    ...
    for (int extruder_idx = 0; extruder_idx < n_extruder_colors; extruder_idx++) {
        ...
        ImGuiColorEditFlags flags = ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel |
                                    ImGuiColorEditFlags_NoPicker | ImGuiColorEditFlags_NoTooltip;
        if (m_selected_extruder_idx != extruder_idx)
    ...
    tool_ids = {ImGui::CircleButtonIcon, ImGui::SphereButtonIcon, ImGui::TriangleButtonIcon,
                ImGui::HeightRangeIcon,  ImGui::FillButtonIcon,   ImGui::GapFillIcon};
    std::array<wchar_t, 6> icons;
    if (m_is_dark_mode)
        icons = {ImGui::CircleButtonDarkIcon, ImGui::SphereButtonDarkIcon, ImGui::TriangleButtonDarkIcon,
                 ImGui::HeightRangeDarkIcon,  ImGui::FillButtonDarkIcon,   ImGui::GapFillDarkIcon};
    else
        icons = {ImGui::CircleButtonIcon, ImGui::SphereButtonIcon, ImGui::TriangleButtonIcon,
                 ImGui::HeightRangeIcon,  ImGui::FillButtonIcon,   ImGui::GapFillIcon};
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (m_current_tool == tool_ids[i]) {
            ...
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                                  ImVec4(0.f, 0.59f, 0.53f, 0.25f));                        // ORCA use orca color for selected tool / brush
    ...
    if (m_current_tool == ImGui::CircleButtonIcon || m_current_tool == ImGui::SphereButtonIcon) {
        ...
        ImGui::PushItemWidth(sliders_width);
        std::string format_str = std::string("%.2f") + I18N::translate_utf8("mm",
                                                                            "Height range,"
                                                                            "Facet in [cursor z, cursor z + height] will be selected.");
        m_imgui->bbl_slider_float_style("##cursor_height", &m_cursor_height, CursorHeightMin, CursorHeightMax, format_str.data(), 1.0f,
                                        true);
        ...
        ImGui::PushItemWidth(sliders_width);
        std::string format_str = std::string("%.2f") +
                                 I18N::translate_utf8("", "Triangle patch area threshold,"
                                                          "triangle patch will be merged to neighbor if its area is less than threshold");
        m_imgui->bbl_slider_float_style("##gap_area", &TriangleSelectorPatch::gap_area, TriangleSelectorPatch::GapAreaMin,
                                        TriangleSelectorPatch::GapAreaMax, format_str.data(), 1.0f, true);
    ...
    if (m_show_remap_panel) {
        ...
        draw_list->AddRectFilled({p_bg_min.x - 10.0f, p_bg_min.y - 7.0f},
                                 {p_bg_min.x + available_width + ImGui::GetFrameHeight(), p_bg_min.y + remap_panel_high},
                                 ImGui::GetColorU32(ImGuiCol_FrameBgActive, 1.0f), 5.0f);
...
{
    // [THREAD] Runs on the GL thread; updates obj_list/partplate notifications and posts background events so Unity should queue the
    // equivalent to the main thread.
    bool         updated = false;
    ModelObject* mo      = m_c->selection_info()->model_object();
    int          idx     = -1;
    for (ModelVolume* mv : mo->volumes) {
...
{
    // [INTENT] Rebuilds TriangleSelectorPatch objects for every volume based on extruder colors; keeps VBOs in sync for GL rendering.
    // [PORTING_HAZARD:P2] Unity needs to replace TriangleSelectorPatch with MeshCollider + baked Mesh data updated through Mesh API.
...
{
    // [STATE] Refreshes vertex colors when extruder palette changes; Unity will respond by updating MeshRenderer materials/shader colors.
    for (int i = 0; i < m_triangle_selectors.size(); i++) {
...
{
    // [EVENT] Called when the selected object changes; reloads extruder palette and triangle selectors, keeping Unity's MeshCache fresh.
    wxBusyCursor wait;
...
{
    // [STATE] Toggles the gap-fill filter on triangle selectors so Unity can reapply post-processing depending on the brush type.
...
// [INTENT] Lets the gizmo manager classify this tool as the MMU segmentation painter so render/input routing stays specific.
PainterGizmoType GLGizmoMmuSegmentation::get_painter_type() const { return PainterGizmoType::MM_SEGMENTATION; }

// BBS
// [STATE] Cursor highlight color derived from the selected extruder index; Unity should mirror this to tint the cursor overlay.
...
{
    // [EVENT] When the painter is turned off, trigger mesh serialization and notify the renderer, similar to Unity's OnDisable flow.
...
{
    // [EVENT] Provides descriptive undo snapshot labels so Unity can feed its UndoStack with similar context.
...
{
    // [OPENGL] Explicitly deletes VBO/VAO handles so the GL context stays clean when the MMU overlays rebuild.
    // [PORTING_HAZARD:P2] Unity does not expose raw GL handles, so replace this with Mesh cleanup and GC-safe disposal.
...
{
    // [OPENGL] Binds VAO/VBO/element buffer and issues glDrawElements to paint triangles; Unity will swap in MeshRenderer/Graphics.DrawMesh.
    ...
    if (this->triangle_indices_VBO_ids[triangle_indices_idx] != 0 && this->triangle_indices_sizes[triangle_indices_idx] > 0) {
...
{
    // [OPENGL] Uploads vertex buffer data to GPU (VBO + optionally VAO) and clears client-side copy once committed.
#if !SLIC3R_OPENGL_ES
    if (OpenGLManager::get_gl_info().is_core_profile()) {
#endif // !SLIC3R_OPENGL_ES
        assert(this->vertices_VAO_id == 0);
...
{
    // [OPENGL] For each triangle patch, creates an element buffer and flushes indices to GPU, forming the draw batches.
...
{
    // [STATE] Builds the cache of filaments active in the base geometry plus painted triangles so the remap UI filters irrelevant entries.
...
{
    // [INTENT] Constructs the remap panel, showing source colors and popups for target channels so Unity can mirror it with a VisualElement
    // grid and popup windows. [STATE] Consults `m_used_filaments` and `m_extruder_remap` to avoid repainting unused colors.
    ...
    for (size_t src : m_used_filaments) {
        ...
        if (gray * 255.f < 80.f)
            ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + (size.x - txt_sz.x) * 0.5f, pos.y + (size.y - txt_sz.y) * 0.5f),
        ...
        if (src != m_extruder_remap[src]) {
            ...
            ImDrawList* draw_list = ImGui::GetWindowDrawList();
            draw_list->AddTriangleFilled(p, ImVec2(p.x + s, p.y), ImVec2(p.x, p.y + s), ImGuiWrapper::to_ImU32(dst_col));

            // ORCA: Add a thin outline for better contrast when colors are similar
            draw_list->AddTriangle(p, ImVec2(p.x + s, p.y), ImVec2(p.x, p.y + s), outline_col, 0.5f);
        }
        ...
        if (ImGui::BeginPopup(pop_id.c_str())) {
            ...
            for (int dst = 0; dst < (int) n_extr; ++dst) {
                ...
                ImGuiColorEditFlags dst_flags = ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel |
                                                ImGuiColorEditFlags_NoPicker | ImGuiColorEditFlags_NoTooltip;
                // Show border for currently selected destination filament
                if (m_extruder_remap[src] != dst)
                    dst_flags |= ImGuiColorEditFlags_NoBorder;
                ...
                if (dst_gray * 255.f < 80.f)
                    ImGui::GetWindowDrawList()->AddText(ImVec2(dst_pos.x + (dst_size.x - dst_txt_sz.x) * 0.5f,
                                                               dst_pos.y + (dst_size.y - dst_txt_sz.y) * 0.5f),
...
{
    // [EVENT] Applies user remapping decisions to TriangleSelector state and ModelVolume configs, so Unitys remap dialog must mirror this
    // atomic update with undo snapshots. [PORTING_HAZARD:P2] The remap changes volume config and connector states, so verify Unity's config
    // layer exposes the same hooks before rewiring.
    ...
    Plater::TakeSnapshot snapshot(wxGetApp().plater(), "Remap filament assignments", UndoRedo::SnapshotType::GizmoAction);
