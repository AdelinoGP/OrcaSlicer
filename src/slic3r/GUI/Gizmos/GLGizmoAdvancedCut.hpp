...
// [INTENT] Declaration boundary for the advanced cut gizmo: exposes cut-plane editing, connector editing, and cut execution hooks.
// [STATE] Owns the cached plane geometry, connector selection state, buffered inputs, and validation statistics used by the cpp
// implementation. [UNITY] Port as a tool-controller interface plus a serialized view-model; keep render-only state separate from mutable
// edit state. [PORTING_HAZARD:P1] The declaration mixes mesh state, input buffering, and model mutation entry points, so Unity should split
// tool state from view state.
...
class GLGizmoAdvancedCut : public GLGizmoRotate3D
{
    struct Rotate_data
    {
        ...
        Rotate_data(double an, Axis a) : angle(an), ax(a) {}
    };
