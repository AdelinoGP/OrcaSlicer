...
// [INTENT] Wire the SLA gizmo into the shared base so it inherits the toolbar icon and sprite identity before any per-frame state is built.
// [UNITY] Model this as a constructor that binds a controller to its toolbar icon metadata and cached prefab set.
...
{
    // [STATE] Refresh model-specific caches and restore supports visibility whenever the active object or selection changes so the gizmo
    // reflects the newly focused SLA part.
...
{
    // [OPENGL] Draw blend-enabled support glyphs, selection rect, and clipping planes each frame; [STATE] relies on cached
    // cone/sphere/cylinder meshes so the GPU doesn't reinitialize every update, and [UNITY] this would map to a `MonoBehaviour` issuing
    // `Graphics.DrawMesh` while mirroring the clipping plane state via a `MeshRenderer` shader.
...
{
    // [INTENT] Paint support point glyphs by walking either the auto-generated cache or the manual editing cache, coloring by
    // hover/selection and respecting clipping; [OPENGL] this loop configures shader uniforms and toggles front-face winding for left-handed
    // volumes, [STATE] toggles between `m_editing_cache` and `m_normal_cache`, and [UNITY] it maps to a Unity `MonoBehaviour` updating
    // `Graphics.DrawMesh` with per-instance matrices and a shared material.
    const size_t cache_size = m_editing_mode ? m_editing_cache.size() : m_normal_cache.size();

    const bool has_points = (cache_size != 0);
    const bool has_holes  = (!m_c->hollowed_mesh()->get_hollowed_mesh() && !m_c->selection_info()->model_object()->sla_drain_holes.empty());
    ...
    for (size_t i = 0; i < cache_size; ++i) {
        ...
        if (m_editing_mode) {
            ...
            const Transform3d       model_matrix =
                instance_matrix * support_matrix * Transform3d(aa.toRotationMatrix()) *
...
// [STATE][PORTING_HAZARD:P3] Uses the object clipper to avoid rendering/placing supports when the model is sliced away; Unity port needs an
// equivalent plane test before honoring new points.
...
// [EVENT][THREAD] Queries the shared raycaster on the UI thread and respects the clipping plane so point picks stay aligned with the
// visible mesh; Unity should mirror this via a `Collider.Raycast` + clipping mask check.
...
// [EVENT] Handles control/key-driven editing modes, selection rectangle, auto-generation triggers, and clipping adjustments while returning
// true if the gizmo consumed the event; [PORTING_HAZARD:P2] Unity ports must replicate this branching in their input handling layer rather
// than relying on wxWidgets event propagation. The gizmo has an opportunity to react - if it does, it should return true so that the
// Canvas3D is aware that the event was reacted to and stops trying to make different sense of it. If the gizmo concludes that the event was
// not intended for it, it should return false.
bool GLGizmoSlaSupports::gizmo_event(SLAGizmoEventType action, const Vec2d& mouse_position, bool shift_down, bool alt_down, bool control_down)
{
    ...
    if (m_editing_mode) {
        ...
        if (action == SLAGizmoEventType::LeftDown && !m_selection_rectangle.is_dragging() && !shift_down) {
            ...
            if (m_selection_empty) {
                ...
                if (unproject_on_mesh(mouse_position, pos_and_normal)) { // we got an intersection
                    ...
                    m_editing_cache.emplace_back(sla::SupportPoint(pos_and_normal.first, m_new_point_head_diameter / 2.f, false), false,
                                                 pos_and_normal.second);
        ...
        if ((action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::ShiftUp || action == SLAGizmoEventType::AltUp) &&
            ...
            for (size_t idx : points_idxs)
                points_inside.emplace_back(
                    (trafo.get_matrix().cast<float>() * (m_editing_cache[idx].support_point.pos + m_editing_cache[idx].normal))
                        .cast<float>());
...
// [STATE] Walks object/printed preset configs with default fallbacks so the slider panel always has option metadata even when custom values
// are missing.
...
/*
void GLGizmoSlaSupports::find_intersecting_facets(const igl::AABB<Eigen::MatrixXf, 3>* aabb, const Vec3f& normal, double offset,
std::vector<unsigned int>& idxs) const
{
...
// [INTENT][EVENT] Draws the ImGui toolbar that toggles between auto-generation sliders and editing controls; [UNITY] replicate with a UI
// Toolkit window and `Button`/`Slider` components paired with a MonoBehaviour that mirrors `m_editing_mode` state.
...
RENDER_AGAIN:
    ...
    const float settings_sliders_left      = std::max(m_imgui->calc_text_size(m_desc.at("minimal_distance")).x,
                                                      m_imgui->calc_text_size(m_desc.at("points_density")).x) +
                                             m_imgui->scaled(1.f);
    ...
    const float buttons_width_approx       = m_imgui->calc_text_size(m_desc.at("apply_changes")).x +
                                             m_imgui->calc_text_size(m_desc.at("discard_changes")).x + m_imgui->scaled(1.5f);
    ...
    if (m_editing_mode) {
        float diameter_upper_cap = static_cast<ConfigOptionFloat*>(
                                       wxGetApp().preset_bundle->sla_prints.get_edited_preset().config.option("support_pillar_diameter"))
                                       ->value;
        ...
        // :
        //              (m_c->m_model_object->sla_points_status == sla::PointsStatus::AutoGenerated ? _(L("Autogenerated points (no
        //              modifications)")) : (m_c->m_model_object->sla_points_status == sla::PointsStatus::UserModified ? _(L("User-modified
        //              points")) : (m_c->m_model_object->sla_points_status == sla::PointsStatus::Generating ? _(L("Generation in
...
// [STATE] Gates activation to SLA printers with a single-instance selection and no invalid volumes; Unity ports should map to a
// `CanActivate` check that mirrors these constraints.
...
std::string GLGizmoSlaSupports::on_get_name() const { return _u8L("SLA Support Points"); }
...
// [THREAD][PORTING_HAZARD:P2] Dialog spawn happens via `CallAfter` to avoid reentrancy on OSX; Unity will need to marshal similar
// confirmations back onto the main thread when the user toggles the gizmo state.
void GLGizmoSlaSupports::ask_about_changes_call_after(std::function<void()> on_yes, std::function<void()> on_no)
{
    wxGetApp().CallAfter([on_yes, on_no]() {
        ...
        MessageDialog dlg(GUI::wxGetApp().mainframe,
                          _L("Do you want to save your manually "
...
// [STATE][EVENT] Handles activation/deactivation transitions, guarding against losing manual edits by prompting via
// `ask_about_changes_call_after` when the gizmo is toggled off.
...
{ ar(m_new_point_head_diameter, m_normal_cache, m_editing_cache, m_selection_empty); }
...
{
    // [STATE][EVENT] Keep the editing selection mutually consistent and mirror the active point into the diameter slider state so the UI
    // always edits the currently focused support.
    if (!m_editing_mode) {
...
{
    // [STATE] Recompute whether the cache still has any active selection instead of tracking a separate counter; Unity should preserve the
    // same source-of-truth shape to avoid desync between the toolbar and point list.
    if (!m_editing_mode) {
...
{
    // [STATE][PORTING_HAZARD:P2] Cancel is destructive here: it abandons the temporary editing cache and returns to the hidden-supports
    // state, so a Unity port needs an explicit discard path instead of relying on implicit dialog close behavior.
    if (!m_editing_mode) {
...
{
    // [STATE][THREAD] Commit the whole editing cache back into the model before kicking reslicing, because downstream support generation
    // reads the model snapshot rather than this transient UI cache.
...
{
    // [STATE] Rebuild the local cache from either backend-generated supports or the persisted model points so the gizmo can reopen with the
    // same visible state the model last exposed.
    const ModelObject* mo = m_c->selection_info()->model_object();
    m_normal_cache.clear();
...
void GLGizmoSlaSupports::reslice_SLA_supports(bool postpone_error_messages) const
{
    wxGetApp().CallAfter([this, postpone_error_messages]() {
        wxGetApp().plater()->reslice_SLA_supports(*m_c->selection_info()->model_object(), postpone_error_messages);
    });
}

void GLGizmoSlaSupports::get_data_from_backend()
{
    // [THREAD][STATE] Pull generated support points from SLAPrintObject into the front-end cache without mutating the source model, so the
    // background generation pipeline can continue and the UI can refresh on its own cadence.
...
{
    // [EVENT][THREAD] Confirm the destructive regenerate action, then hand reslicing back to the plater on the UI thread.
...
{
    // [STATE][UNITY] Enter a transient edit session by cloning the current support cache and hiding rendered supports; Unity should model
    // this as a modal editing sub-state with its own retained copy of the support list.
...
{
    // [STATE][EVENT] Restore the normal support visibility and clear the quit notification regardless of whether edits were committed.
    if (m_editing_mode) {
...
{
    // [STATE] Compare cache contents directly so the gizmo can decide whether an exit prompt is required without maintaining a separate
    // dirty flag.
...
// [INTENT] Build the static keyboard-shortcut help dialog once so the gizmo can surface its editing cheatsheet without duplicating layout
// code. [UNITY] Port this as a modal help overlay with a fixed shortcut table and localized labels.
SlaGizmoHelpDialog::SlaGizmoHelpDialog()
    : wxDialog(nullptr,
               wxID_ANY,
               _L("SLA gizmo keyboard shortcuts"),
               wxDefaultPosition,
               wxDefaultSize,
               wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    // [INTENT][UNITY] Build a static shortcut reference panel for manual support editing; in Unity this should become a simple modal help
    // overlay with localized label/value rows rather than a dynamically interactive dialog.
