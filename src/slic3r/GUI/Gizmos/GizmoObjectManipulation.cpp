// [ANNOTATED]
// [INTENT] Implementation of the 3D object manipulation HUD (Move, Rotate, Scale).
// [STATE] Managed state for active selection, coordinate systems (World/Local), unit preferences, and buffered transformation values.
// [EVENT] Interactive ImGui-based windows for direct entry of position, rotation, and scale data.
// [UNITY] Map to a Unity UI Toolkit HUD or Canvas overlay. Use Unity's native Transform API for world/local space conversions.
// [PORTING_HAZARD:P2] Complex coordinate space math (World vs Local vs Instance) and its impact on bounding box calculations must be
// mirrored exactly.
...
GizmoObjectManipulation::GizmoObjectManipulation(GLCanvas3D& glcanvas) : m_glcanvas(glcanvas)
{
...
void GizmoObjectManipulation::update_if_dirty()
{
    ...
    enum ManipulationEditorKey { mePosition = 0, meRotation, meScale, meSize };
...
bool GizmoObjectManipulation::render_combo(ImGuiWrapper*                   imgui_wrapper,
...
static const char* label_values[3][3] = {{"##position_x", "##position_y", "##position_z"},
                                         {"##rotation_x", "##rotation_y", "##rotation_z"},
                                         {"##absolute_rotation_x", "##absolute_rotation_y", "##absolute_rotation_z"}};

static const char* label_scale_values[2][3] = {{"##scale_x", "##scale_y", "##scale_z"}, {"##size_x", "##size_y", "##size_z"}};

bool GizmoObjectManipulation::reset_button(
    ImGuiWrapper* imgui_wrapper, float caption_max, float unit_size, float space_size, float end_text_size)
{
    bool        pressed   = false;
...
bool GizmoObjectManipulation::reset_zero_button(
    ImGuiWrapper* imgui_wrapper, float caption_max, float unit_size, float space_size, float end_text_size)
{
    bool        pressed   = false;
...
void GizmoObjectManipulation::do_render_move_window(
    ImGuiWrapper* imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    ...
    float combox_content_size = imgui_wrapper->calc_text_size(_L("Object coordinates")).x * 1.2 + imgui_wrapper->calc_text_size("xxx"sv).x +
                                imgui_wrapper->scaled(3);
    ...
    if (!is_avoid_one_update) {
        for (int i = 0; i < display_position.size(); i++) {
            if (display_position[i] > MAX_NUM)
                display_position[i] = MAX_NUM;
            if (display_position[i] < -MAX_NUM)
                display_position[i] = -MAX_NUM;
        }
...
void GizmoObjectManipulation::do_render_rotate_window(
    ImGuiWrapper* imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    // BBS: GUI refactor: move gizmo to the right
    ...
    if (m_show_clear_rotation) {
        ...
        if (ImGui::IsItemHovered()) {
            float tooltip_size = imgui_wrapper->calc_text_size(_L("Reset current rotation to the value when open the rotation tool.")).x +
                                 3 * space_size;
    ...
    if (m_show_reset_0_rotation) {
        ImGui::SameLine(caption_max + 3 * unit_size + 4 * space_size + end_text_size);
        if (reset_zero_button(imgui_wrapper, caption_max, unit_size, space_size, end_text_size)) {
...
void GizmoObjectManipulation::do_render_scale_input_window(
    ImGuiWrapper* imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    // BBS: GUI refactor: move gizmo to the right
    ...
    // const Selection &selection = m_glcanvas.get_selection();
    // bool uniform_scale_only    = selection.is_multiple_full_object() || selection.is_multiple_full_instance() || selection.is_mixed() ||
    ...
    //    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ",before_index="<<index <<boost::format(",scale %1%, buffered %2%, original_id %3%,
    ...
    if ((scale_sel >= 0)) {
        // for (int index = 0; index < 3; index++)
        //    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ",after_index="<<index <<boost::format(",scale %1%, buffered %2%, original_id %3%,
        //    new_id %4%\n") % this->m_new_scale[index] % m_buffered_scale[index] % m_last_active_item % current_active_id;
        for (int i = 0; i < 3; ++i) {
            if (i != scale_sel)
