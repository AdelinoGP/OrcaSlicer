...
// [INTENT] Overlay that rotates a selected face downwards so the object lies flat before committing the transform.
// [UNITY] Equivalent in Unity would be a MonoBehaviour that tracks the selected MeshInstance, renders face gizmos via Graphics.DrawMesh or
// MeshRenderer overlays, and feeds rotations back to the selection controller.
...
{
    // [EVENT][THREAD] wxWidgets routes mouse events on the UI/gl thread; we handle clicks only when a detected face is hovered.
    if (mouse_event.LeftDown()) {
        if (m_hover_id != -1) {
            Selection& selection = m_parent.get_selection();
            if (selection.is_single_full_instance()) {
                // Rotate the object so the normal points downward:
                // [STATE][PORTING_HAZARD:P2] Unity will need to copy `flattening_rotate` logic so that rotation history and selection cache
                // stay consistent.
...
{
    // [STATE] Refresh cached selection/object references so the gizmo rebuilds planes only when necessary.
...
{
    // [STATE] The flatten gizmo binds Control+F via the base class so the user can trigger consistently across sessions.
...
CommonGizmosDataID GLGizmoFlatten::on_get_requirements() const { return CommonGizmosDataID::SelectionInfo; }

std::string GLGizmoFlatten::on_get_name() const { return _u8L("Lay on face"); }

bool GLGizmoFlatten::on_is_activable() const
{
    // [STATE] Only show the control when the selection is one whole instance because the rotation math expects a single object.
    // [PORTING_HAZARD:P3] GLCanvas3D::do_rotate also assumes this; the Unity port must keep the same precondition or adjust the handler.
...
void GLGizmoFlatten::on_render()
{
    ...
    // [OPENGL][STATE] Start the flat shader and clear depth to render gizmo highlights without modifying the scene content.
    ...
    if (selection.is_single_full_instance()) {
        // [STATE] The cached planes are only valid when one object instance is selected, as downstream loops assume one camera transform.
        const Transform3d& inst_matrix = selection.get_first_volume()->get_instance_transformation().get_matrix();
        const Camera&      camera      = wxGetApp().plater()->get_camera();
        const Transform3d model_matrix = Geometry::translation_transform(selection.get_first_volume()->get_sla_shift_z() * Vec3d::UnitZ()) *
                                         inst_matrix;
        ...
        for (int i = 0; i < (int) m_planes.size(); ++i) {
            // [UNITY] In Unity the equivalent would swap hover materials on MeshRenderers and call Graphics.DrawMesh for each plane mesh.
...
{
    // [EVENT] Gizmo pickers stay on top of the scene so input events hit the overlay before the model.
    ...
    if (!m_planes.empty()) {
        ...
        for (int i = 0; i < (int) m_planes.size(); ++i) {
            // [UNITY] Unity would register Physics.Raycast hits with a MeshCollider using the computed mesh and a dedicated input layer.
...
{
    // [THREAD] The raycasters are managed on the UI/GL thread, so clear them before updating the cached planes.
...
{
    // [STATE][THREAD] Invalidate cached planes when the object or instance changes; always call from the UI thread before re-registering raycasters.
...
// [INTENT] Rebuilds the flatten planes from the current convex hull once per selection change and prepares the GL/mesh-raycaster data.
// [PORTING_HAZARD:P2] This routine manually clusters faces/normals, resizes polygons, and trims tiny facets – the Unity port must either
// replicate or replace it with a runtime mesh-processing job without leaking CPU time on the render thread.
// [THREAD] Runs on the main UI path triggered from `data_changed`; flag this as expensive when porting to Unity and consider scheduling in a Job.
void GLGizmoFlatten::update_planes()
{
    ...
    for (unsigned int polygon_id = 0; polygon_id < m_planes.size(); ++polygon_id) {
        ...
        for (unsigned int i = 0; i < polygon.size(); i++) // Shoelace formula
            area += polygon[i](0) * polygon[i + 1 < polygon.size() ? i + 1 : 0](1) -
                    polygon[i + 1 < polygon.size() ? i + 1 : 0](0) * polygon[i](1);
...
{
    // [STATE] Guard the expensive rebuild by comparing stored volume matrices, scales, and mirrors.
    const ModelObject* mo = m_c->selection_info()->model_object();
    if (m_state != On || !mo || mo->instances.empty())
        return false;

    if (m_planes.empty() || mo != m_old_model_object || mo->volumes.size() != m_volumes_matrices.size())
        return true;
    ...
    for (unsigned int i = 0; i < mo->volumes.size(); ++i)
        if (!mo->volumes[i]->get_matrix().isApprox(m_volumes_matrices[i]) || mo->volumes[i]->type() != m_volumes_types[i])
            return true;
