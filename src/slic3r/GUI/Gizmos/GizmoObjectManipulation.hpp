...
// [INTENT] Class responsible for rendering and handling object manipulation gizmos (move, rotate, scale) in the 3D viewport.
// [UNITY] Use a custom MonoBehaviour (e.g., 'GizmoController') to manage manipulation UI, likely using Unity's 'Handles' or UI Toolkit.
...
public:
    ...
    bool m_imperial_units{false}; // [STATE] Toggle for imperial/metric units.
    bool m_use_object_cs{false};  // [STATE] Toggle for Object/World coordinate system.
    ...
    // [STATE] Flag for deferred UI update.
    bool m_dirty = false;
    // Cached labels for the delayed update, not localized!
    std::string m_new_title_string;           // [STATE] Deferred UI update strings.
    std::string m_new_move_label_string;      // [STATE] Deferred UI update strings.
    std::string m_new_rotate_label_string;    // [STATE] Deferred UI update strings.
    std::string m_new_scale_label_string;     // [STATE] Deferred UI update strings.
    std::string m_new_unit_string;            // [STATE] Deferred UI update strings.
    Vec3d       m_new_position;               // [STATE] Deferred UI update values.
    Vec3d       m_new_rotation;               // [STATE] Deferred UI update values.
    Vec3d       m_new_absolute_rotation;      // [STATE] Deferred UI update values.
    Vec3d       m_new_scale;                  // [STATE] Deferred UI update values.
    Vec3d       m_new_size;                   // [STATE] Deferred UI update values.
    Vec3d       m_unscale_size;               // [STATE] Original object size.
    Vec3d       m_buffered_position;          // [STATE] Buffered values for undo/redo.
    Vec3d       m_buffered_rotation;          // [STATE] Buffered values for undo/redo.
    Vec3d       m_buffered_absolute_rotation; // [STATE] Buffered values for undo/redo.
    Vec3d       m_buffered_scale;             // [STATE] Buffered values for undo/redo.
    Vec3d       m_buffered_size;              // [STATE] Buffered values for undo/redo.
    Vec3d       cs_center;                    // [STATE] Center of coordinate system.
    bool        m_new_enabled{true};          // [STATE] Enabled/disabled state of gizmo.
    bool        m_uniform_scale{true};        // [STATE] Uniform scaling flag.
    // Does the object manipulation panel work in World or Local coordinates?
    ECoordinatesType m_coordinates_type{ECoordinatesType::World}; // [STATE] Coordinate system type.

    bool m_show_reset_0_rotation{false}; // [STATE] Visibility flags for gizmo buttons.
    bool m_show_clear_rotation{false};   // [STATE] Visibility flags for gizmo buttons.
    bool m_show_clear_scale{false};      // [STATE] Visibility flags for gizmo buttons.
    bool m_show_drop_to_bed{false};      // [STATE] Visibility flags for gizmo buttons.
    enum class RotateType { None, Relative, Absolute };
    RotateType m_last_rotate_type{RotateType::None}; // [STATE] Last rotation mode used.
...
public:
    ...
    void set_use_object_cs(bool flag)
    {
        if (m_use_object_cs != flag)
    ...
    bool  render_combo(ImGuiWrapper*                   imgui_wrapper,
