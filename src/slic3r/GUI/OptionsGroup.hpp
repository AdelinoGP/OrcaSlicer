...
// [INTENT] This header defines the OptionsGroup family of classes for constructing UI pages with labeled configuration options.
// [INTENT] OptionsGroup is a generic container that builds a grid of labeled fields from a list of Line objects.
// [INTENT] ConfigOptionsGroup extends OptionsGroup to bind fields to a DynamicPrintConfig instance, handling value changes and
// synchronization. [INTENT] ExtruderOptionsGroup specializes ConfigOptionsGroup for multi-material extruder configurations. [UNITY]
// OptionsGroup maps to a UI Toolkit VisualElement with a custom VisualTreeAsset for layout. [UNITY] ConfigOptionsGroup maps to a C#
// MonoBehaviour that binds UI fields to a ScriptableObject or serialized config data model. [UNITY] Line and Option structures map to data
// classes for UI item descriptions, with factory methods for generating UI elements. [PORTING_HAZARD:P2] The dynamic field factory based on
// ConfigOptionDef::GUIType requires a flexible UI Toolkit field factory with runtime type resolution. [PORTING_HAZARD:P3] wxGridSizer
// layout is replaced by UI Toolkit's USS flexbox/grid layout. [PORTING_HAZARD:P3] The custom drawing via OG_CustomCtrl (see
// OG_CustomCtrl.hpp) will need a UI Toolkit custom VisualElement.
...
// [INTENT] Option wraps a ConfigOptionDef and adds optional side_widget factory for custom UI elements.
// [INTENT] Represents a single configuration option with metadata for UI generation.
// [STATE] opt holds the ConfigOptionDef; opt_id is the unique key; side_widget is an optional custom widget factory.
// [UNITY] Map to a C# class containing ConfigOptionDef data and a Func<GameObject> for custom side widget.
struct Option
{
    ...
    bool operator==(const Option& rhs) const { return (rhs.opt_id == this->opt_id); }

    Option(const ConfigOptionDef& _opt, t_config_option_key id) : opt(_opt), opt_id(id) {}
};
using t_option = std::unique_ptr<Option>; //!

// [INTENT] Line represents a row in the UI, containing either a separator or a label with associated options.
// [INTENT] Holds label text, tooltip, and factory functions for widget creation.
// [STATE] m_options list of Option objects; m_extra_widgets list of side widget factories; m_is_separator flag.
// [EVENT] No direct event handling; widgets created from Line factories will emit events via Field.
// [UNITY] Map to a C# class representing a UI row, with properties for label, tooltip, and lists of Option and widget factory delegates.
...
public:
    ...
    std::string& get_first_option_key() { return m_options[0].opt_id; }

    void append_option(const Option& option) { m_options.push_back(option); }
    void append_widget(const widget_t widget) { m_extra_widgets.push_back(widget); }
    Line(wxString label, wxString tooltip) : label(_(label)), label_tooltip(_(tooltip)) {}
...
// [INTENT] OptionsGroup is a generic UI container that builds a grid of labeled fields from a list of Line objects.
// [INTENT] Manages field creation, value binding, and event propagation for configuration options.
// [STATE] m_lines holds the list of Line objects; m_fields maps option keys to Field instances; sizer is the wxGridSizer layout.
// [EVENT] m_on_change and m_fill_empty_value callbacks for field value changes and focus loss.
// [THREAD] Not thread-safe; UI updates must occur on the main thread.
// [UNITY] Map to a UI Toolkit VisualElement with a custom VisualTreeAsset for layout.
// [UNITY] m_lines can be represented as a List<Line> data model; fields generated by a factory.
// [PORTING_HAZARD:P2] Dynamic field creation based on ConfigOptionDef::GUIType requires a flexible UI Toolkit field factory.
// [PORTING_HAZARD:P3] wxGridSizer layout is replaced by UI Toolkit's USS flexbox/grid layout.
class OptionsGroup
{
public:
    ...
    {
        append_line(create_single_option_line(option, path));
    }
    ...
    OptionsGroup(wxWindow* _parent, const wxString& title, const wxString& icon, bool is_tab_opt = false, column_t extra_clmn = nullptr);
...
// [INTENT] ConfigOptionsGroup extends OptionsGroup to bind fields to a DynamicPrintConfig instance.
// [INTENT] Provides synchronization between UI fields and underlying configuration data model.
// [STATE] m_config pointer to DynamicPrintConfig; m_modelconfig optional ModelConfig pointer; m_opt_map mapping.
// [EVENT] on_change_OG handles field value changes and updates config via change_opt_value.
// [UNITY] Map to a C# MonoBehaviour that binds UI fields to a ScriptableObject or serialized config data model.
// [UNITY] m_config can be a reference to a config ScriptableObject; change_opt_value updates the scriptable object and marks dirty.
// [PORTING_HAZARD:P2] The config change propagation via ModelConfig::touch() must be replicated with a change tracking system.
// [PORTING_HAZARD:P3] wxWindow* parent and sizer layout replaced by UI Toolkit VisualElement hierarchy.
...
public:
    ...
    {}
    ConfigOptionsGroup(wxWindow* parent, const wxString& title, ModelConfig* config, bool is_tab_opt = false, column_t extra_clmn = nullptr)
        : OptionsGroup(parent, title, wxEmptyString, is_tab_opt, extra_clmn), m_config(&config->get()), m_modelconfig(config)
    {}
    ConfigOptionsGroup(wxWindow* parent) : OptionsGroup(parent, wxEmptyString, wxEmptyString, true, nullptr) {}
    ...
    void set_config_category_and_type(const wxString& category, int type)
    {
        m_config_category = category;
        m_config_type     = type;
    }
    void set_config(DynamicPrintConfig* config)
    {
        m_config      = config;
        m_modelconfig = nullptr;
    }
...
// [INTENT] ExtruderOptionsGroup specializes ConfigOptionsGroup for multi-material extruder configurations.
// [INTENT] Overrides on_change_OG to handle vector options across multiple extruders.
// [STATE] Inherits m_config and m_opt_map; additional state for extruder index.
// [EVENT] on_change_OG propagates changes to all extruder indices in the vector option.
// [UNITY] Map to a C# MonoBehaviour that extends ConfigOptionsGroup binding, with logic for multi-extruder config arrays.
// [PORTING_HAZARD:P2] Vector option handling across extruders requires careful replication in Unity's config model.
class ExtruderOptionsGroup : public ConfigOptionsGroup
{
public:
    ExtruderOptionsGroup(wxWindow*           parent,
                         const wxString&     title,
                         const wxString&     icon,
                         DynamicPrintConfig* config     = nullptr, // ORCA: add support for icons
                         bool                is_tab_opt = false,
                         column_t            extra_clmn = nullptr)
...
// [INTENT] ogStaticText is a specialized wxStaticText for displaying informational text within options pages.
// [INTENT] Supports hyperlink generation via SetPathEnd for linking to help pages.
// [STATE] Inherits wxStaticText state; optional hyperlink path.
// [EVENT] FocusText triggers hyperlink activation.
// [UNITY] Map to a UI Toolkit Label with a clickable hyperlink (using UI Toolkit's hyperlink attribute or custom clickable label).
