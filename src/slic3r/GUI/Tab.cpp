...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (opt_key == "enable_prime_tower") {
        ...
        if (!boost::any_cast<bool>(value) && timelapse_enabled) {
            ...
            MessageDialog dlg(wxGetApp().plater(),
                              _L("A prime tower is required for smooth timelapse. There may be flaws on the model without prime tower. Are "
                                 "you sure you want to disable prime tower?"),
        ...
        if (boost::any_cast<bool>(value) && is_precise_z_height) {
            MessageDialog dlg(wxGetApp().plater(),
                              _L("Enabling both precise Z height and the prime tower may cause the size of prime tower to increase. Do you "
                                 "still want to enable?"),
                              _L("Warning"), wxICON_WARNING | wxYES | wxNO);
            if (dlg.ShowModal() == wxID_NO) {
                DynamicPrintConfig new_conf = *m_config;
    ...
    if (opt_key == "timelapse_type") {
        ...
        if (!wipe_tower_enabled && boost::any_cast<int>(value) == (int) TimelapseType::tlSmooth) {
            MessageDialog dlg(wxGetApp().plater(),
                              _L("A prime tower is required for smooth timelapse. There may be flaws on the model without prime tower. Do "
                                 "you want to enable prime tower?"),
                              _L("Warning"), wxICON_WARNING | wxYES | wxNO);
...
bool Tab::validate_custom_gcode(const wxString& title, const std::string& gcode)
{
    ...
    if (invalid) {
        ...
        for (const std::string& keyword : tags)
            lines += ";" + keyword + "\n";
        wxString reports = format_wxstr(_L_PLURAL("Following line %s contains reserved keywords.\nPlease remove it, or will beat G-code "
                                                  "visualization and printing time estimation.",
                                                  "Following lines %s contain reserved keywords.\nPlease remove them, or will beat G-code "
                                                  "visualization and printing time estimation.",
                                                  tags.size()),
                                        lines);
...
void TabFilament::build()
{
    ...
    line = {L("Cool Plate"), L("Bed temperature when the Cool Plate is installed. A value of 0 means the filament does not support "
    ...
    line = {L("Textured Cool Plate"), L("Bed temperature when the Textured Cool Plate is installed. A value of 0 means the filament does "
    ...
    line = {L("Engineering Plate"), L("Bed temperature when the Engineering Plate is installed. A value of 0 means the filament does not "
    ...
    line = {L("Textured PEI Plate"), L("Bed temperature when the Textured PEI Plate is installed. A value of 0 means the filament does not "
    ...
    line            = {L("Max fan speed threshold"),
                       L("Part cooling fan speed will be max when the estimated layer time is shorter than the setting value")};
    line.label_path = "material_cooling#material-part-cooling-fan";
...
void TabFilament::toggle_options()
{
    ...
    if (m_active_page->title() == L("Filament")) {
        ...
        bool support_chamber_temp_control = this->m_preset_bundle->printers.get_edited_preset().config.opt_bool(
    ...
    if (m_active_page->title() == L("Multimaterial")) {
        ...
        toggle_line("retraction_distances_when_ec",
                    is_multi_extruder && is_BBL_printer && m_config->opt_bool("long_retractions_when_ec", extruder_idx),
                    256 + extruder_idx);
    }
}
                ...
                for (const Preset& preset : m_preset_bundle->filaments.get_presets()) {
                    if (preset.is_compatible && !preset.is_default) {
                        ...
                        if (!preset.setting_id.empty()) {
                            m_preset_bundle->filaments.set_sync_info_and_save(preset.name, preset.setting_id, "delete", 0);
                        }
                    }
                }
...
void Tab::delete_preset()
{
    ...
    if (m_type == Preset::TYPE_PRINTER && !physical_printers.empty()) {
        ...
        if (!ph_printers.empty() || !ph_printers_only.empty()) {
            msg += _L_PLURAL("Following preset will be deleted too.", "Following presets will be deleted too.",
    ...
    */
    ...
    deps.btn->Bind(wxEVT_BUTTON, ([this, parent, &deps](wxCommandEvent& e) {
                       ...
                       PresetCollection& depending_presets  = (deps.type == Preset::TYPE_PRINTER) ? m_preset_bundle->printers :
                                                              (printer_technology == ptFFF)       ? m_preset_bundle->prints :
                                                                                                    m_preset_bundle->sla_prints;
    ...
    if (m_extruder_switch) {
        ...
        if (extruder_nums == 2) {
            ...
            for (size_t i = 0; i < nozzle_volumes_def->enum_labels.size(); ++i) {
                if (nozzle_volumes->values[0] == i)
                    left = _L(nozzle_volumes_def->enum_labels[i]);
                if (nozzle_volumes->values[1] == i)
                    right = _L(nozzle_volumes_def->enum_labels[i]);
            }
...
void Tab::switch_excluder(int extruder_id)
{
    ...
    auto get_index_for_extruder =
        [this, &extruders, &nozzle_volumes,
         variant_keys = variant_keys[m_type >= Preset::TYPE_COUNT ? Preset::TYPE_PRINT : m_type]](int extruder_id, int stride = 1) {
            return m_config->get_index_for_extruder(extruder_id + 1, variant_keys.first, ExtruderType(extruders->values[extruder_id]),
                                                    NozzleVolumeType(nozzle_volumes->values[extruder_id]), variant_keys.second, stride);
        };
