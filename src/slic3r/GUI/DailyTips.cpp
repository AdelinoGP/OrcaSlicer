...
// [ANNOTATED]
// [INTENT] Implements the Daily Tips overlay and modal window using Dear ImGui.
// [STATE] Tip metadata (text, images, links) is retrieved from HintDatabase and rendered via ImGui draw lists.
// [UNITY] Replace ImGui HUD with a retained Unity UI (Canvas) or UI Toolkit VisualElement.
// [UNITY] Use TextMeshPro for localized text and automatic wrapping.
// [PORTING_HAZARD:P2] Manual CJK text wrapping via transient wxWidgets Label creation should be replaced by Unity's native text layout engine.
...
DailyTipsDataRenderer::DailyTipsDataRenderer(DailyTipsLayout layout) : m_layout(layout) {}
...
void DailyTipsDataRenderer::load_texture_from_img_url(const std::string url)
{
    ...
    if (!url.empty()) {
        ...
        if (!m_placeholder_texture) {
            ...
            m_placeholder_texture->load_from_file(Slic3r::resources_dir() + "/images/dailytips_placeholder.png", true, GLTexture::None,
                                                  false);
        }
    }
}
...
// [OPENGL] Direct ImGui draw list calls for rendering the tip panel.
// [UNITY] Use Canvas + Image/Text elements instead of immediate-mode rendering.
...
bool DailyTipsDataRenderer::has_image() const { return !m_data.img_url.empty(); }

void DailyTipsDataRenderer::on_change_color_mode(bool is_dark) { m_is_dark = is_dark; }

void DailyTipsDataRenderer::set_fade_opacity(float opacity) { m_fade_opacity = opacity; }

void DailyTipsDataRenderer::render_img(const ImVec2& start_pos, const ImVec2& size) const
{
    if (has_image())
        ImGui::Image((ImTextureID) (intptr_t) m_texture->get_id(), size, ImVec2(0, 0), ImVec2(1, 1),
                     m_is_dark ? ImVec4(0.8, 0.8, 0.8, m_fade_opacity) : ImVec4(1, 1, 1, m_fade_opacity));
    // else {
    //     ImGui::Image((ImTextureID)(intptr_t)m_placeholder_texture->get_id(), size, ImVec2(0, 0), ImVec2(1, 1), m_is_dark ? ImVec4(0.8,
    //     0.8, 0.8, m_fade_opacity) : ImVec4(1, 1, 1, m_fade_opacity));
    // }
}
...
void DailyTipsDataRenderer::render_text(const ImVec2& start_pos, const ImVec2& size) const
{
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, m_is_dark ? ImVec4(1.0f, 1.0f, 1.0f, 0.88f * m_fade_opacity) :
                                                     ImVec4(38 / 255.0f, 46 / 255.0f, 48 / 255.0f, m_fade_opacity));
    // main text
...
void DailyTipsPanel::set_position(const ImVec2& pos) { m_pos = pos; }
...
void DailyTipsPanel::set_can_expand(bool can_expand) { m_can_expand = can_expand; }

ImVec2 DailyTipsPanel::get_size() { return ImVec2(m_width, m_height); }

// [INTENT] Main render loop for the Daily Tips HUD panel.
// [STATE] Handles expanded/collapsed state and position updates.
// [UNITY] Use a persistent UI Panel with a CanvasGroup for fade effects.
void DailyTipsPanel::render()
{
    ...
    int window_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
                       ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
    if (ImGui::BeginChild((std::string("##DailyTipsPanel") + std::to_string(m_uid)).c_str(), ImVec2(m_width, m_height), false,
                          window_flags)) {
        if (m_can_expand) {
            if (m_is_expanded) {
...
void DailyTipsPanel::retrieve_data_from_hint_database(HintDataNavigation nav)
{
    ...
    if (hint_data != nullptr) {
        DailyTipsData data{hint_data->text,        hint_data->documentation_link, hint_data->image_url,
                           hint_data->follow_text, hint_data->hypertext,          hint_data->callback};
...
bool DailyTipsPanel::is_expanded() { return m_is_expanded; }
    ...
    if (ImGui::BeginChild(name.c_str(), size, false, window_flags)) {
        ...
        if (m_can_expand) {
            if (m_is_expanded) {
                ...
                ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
                ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(.0f, .0f, .0f, .0f));
                ImGui::PushStyleColor(ImGuiCol_Text, m_is_dark ? ImColor(230, 230, 230, (int) (255 * m_fade_opacity)).Value :
                                                                 ImColor(38, 46, 48, (int) (255 * m_fade_opacity)).Value);
                ...
                if (ImGui::IsMouseHoveringRect(btn_pos, btn_pos + expand_btn_size, true)) {
                    ...
                    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd,
                                                        m_is_dark ? ImColor(230, 230, 230, (int) (255 * m_fade_opacity)) :
        ...
        // prev button
        ImColor button_text_color = m_is_dark ? ImColor(228, 228, 228, (int) (255 * m_fade_opacity)) :
                                                ImColor(38, 46, 48, (int) (255 * m_fade_opacity));
        ...
        // next button
        button_text_color      = m_is_dark ? ImColor(228, 228, 228, (int) (255 * m_fade_opacity)) :
                                             ImColor(38, 46, 48, (int) (255 * m_fade_opacity));
...
DailyTipsWindow::DailyTipsWindow() { m_panel = new DailyTipsPanel(false, DailyTipsLayout::Vertical); }
...
void DailyTipsWindow::close() { m_show = false; }
