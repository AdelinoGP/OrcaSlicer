...
{
    // [INTENT] Pack four-slot AMS blocks first, then flow one-slot extras into compact rows for the popup body.
    // [UNITY] Port this as a retained layout rebuild over serialized tray view models instead of mutating wxSizer trees directly.
...
MaterialItem::MaterialItem(wxWindow* parent, wxColour mcolour, wxString mname) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
...
void MaterialItem::render(wxDC& dc)
{
    ...
    if (mcolor.Alpha() == 0) {
        material_name_colour = wxColour(0x26, 0x2E, 0x30);
    }
    ...
    if (m_match) {
        dc.DrawText(mapping_txt,
                    wxPoint(GetSize().x / 2 + (GetSize().x / 2 - mapping_txt_size.x) / 2 - FromDIP(8) - FromDIP(LEFT_OFFSET) + m_text_pos_x,
                            m_text_pos_y));
    }
}
...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_ams_cols.size() > 1) {
        ...
        if (m_ams_ctype == 0) {
            ...
            for (int i = 0; i < cols_size; i++) {
                ...
                if (i != cols_size - 1) {
                    dc.DrawRoundedRectangle(x - FromDIP(LEFT_OFFSET), up, ((float) MATERIAL_REC_WHEEL_SIZE.x) / cols_size + FromDIP(3),
                                            MATERIAL_REC_WHEEL_SIZE.y, 3);
                } else {
                    dc.DrawRoundedRectangle(x - FromDIP(LEFT_OFFSET), up, ((float) MATERIAL_REC_WHEEL_SIZE.x) / cols_size - FromDIP(1),
                                            MATERIAL_REC_WHEEL_SIZE.y, 3);
                }
            }
        }
        ...
        if (m_match) {
            ...
            dc.DrawRectangle((size.x / 2 - MATERIAL_REC_WHEEL_SIZE.x) / 2 + FromDIP(3) - FromDIP(LEFT_OFFSET), up,
                             MATERIAL_REC_WHEEL_SIZE.x - FromDIP(1), MATERIAL_REC_WHEEL_SIZE.y);
        }
    }
    ...
    if (m_text_pos_y > 0 && m_match) {
        ...
        if ((acolor.Red() > 160 && acolor.Green() > 160 && acolor.Blue() > 160) &&
            ...
            dc.DrawBitmap(m_arraw_bitmap_white.bmp(), size.x - m_arraw_bitmap_white.GetBmpSize().x - FromDIP(4) - FromDIP(LEFT_OFFSET),
                          m_text_pos_y + FromDIP(3));
        } else {
            dc.DrawBitmap(m_arraw_bitmap_gray.bmp(), size.x - m_arraw_bitmap_gray.GetBmpSize().x - FromDIP(4) - FromDIP(LEFT_OFFSET),
                          m_text_pos_y + FromDIP(3));
        }
    }
...
void MaterialItem::reset_valid_info() { set_ams_info(m_back_ams_coloul, m_back_ams_name, m_back_ams_ctype, m_back_ams_cols); }
...
void MaterialSyncItem::render(wxDC& dc)
{
    ...
    if (m_match) {
        ...
        dc.SetFont(::Label::Head_12);
        dc.DrawText(mapping_txt,
                    wxPoint(GetSize().x / 2 + (GetSize().x / 2 - mapping_txt_size.x) / 2 - FromDIP(8) - FromDIP(real_left_offset),
                            m_text_pos_y));
    } else {
...
void MaterialSyncItem::doRender(wxDC& dc)
{
        ...
        if (m_ams_cols.size() > 1) {
            ...
            if (m_ams_ctype == 0) {
                ...
                for (int i = 0; i < cols_size; i++) {
                    ...
                    float x = left + ((float) MATERIAL_REC_WHEEL_SIZE.x) * i / cols_size;
                    if (i != cols_size - 1) {
                        dc.DrawRoundedRectangle(x - FromDIP(real_left_offset), up,
                                                ((float) MATERIAL_REC_WHEEL_SIZE.x) / cols_size + FromDIP(3), MATERIAL_REC_WHEEL_SIZE.y, 3);
                    } else {
                        dc.DrawRoundedRectangle(x - FromDIP(real_left_offset), up,
                                                ((float) MATERIAL_REC_WHEEL_SIZE.x) / cols_size - FromDIP(1), MATERIAL_REC_WHEEL_SIZE.y, 3);
                    }
                }
            }
        } else {
            dc.SetPen(*wxTRANSPARENT_PEN);
            dc.SetBrush(wxBrush(wxColour(acolor)));
            dc.DrawRectangle((size.x / 2 - MATERIAL_REC_WHEEL_SIZE.x) / 2 + FromDIP(3) - FromDIP(real_left_offset), up,
                             MATERIAL_REC_WHEEL_SIZE.x - FromDIP(1), MATERIAL_REC_WHEEL_SIZE.y);
        }
    } else {
        dc.SetPen(*wxTRANSPARENT_PEN);
    ...
    if (m_text_pos_y > 0 && m_match && m_dropdown_allow_painted) {
        ...
        if ((acolor.Red() > 160 && acolor.Green() > 160 && acolor.Blue() > 160) &&
            (acolor.Red() < 180 && acolor.Green() < 180 && acolor.Blue() < 180)) {
            dc.DrawBitmap(m_arraw_bitmap_white.bmp(), size.x - m_arraw_bitmap_white.GetBmpSize().x - FromDIP(4) - FromDIP(real_left_offset),
                          m_text_pos_y + FromDIP(3));
        } else {
            dc.DrawBitmap(m_arraw_bitmap_gray.bmp(), size.x - m_arraw_bitmap_gray.GetBmpSize().x - FromDIP(4) - FromDIP(real_left_offset),
                          m_text_pos_y + FromDIP(3));
        }
    }
    auto wheel_left = (GetSize().x / 2 - m_ams_wheel_mitem.GetBmpSize().x) / 2 + FromDIP(2);
    ...
    : PopupWindow(parent, wxBORDER_NONE), m_use_in_sync_dialog(use_in_sync_dialog)
{
    ...
    m_reset_btn = new ScalableButton(m_right_first_text_panel, wxID_ANY, wxGetApp().dark_mode() ? "erase_dark" : "erase", wxEmptyString,
                                     wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER, true, 14);
...
void AmsMapingPopup::set_reset_callback(ResetCallback callback) { m_reset_callback = callback; }

void AmsMapingPopup::show_reset_button() { m_reset_btn->Show(); }

void AmsMapingPopup::set_only_show_ext_spool(bool flag) { m_only_show_ext_spool = flag; }

void AmsMapingPopup::msw_rescale()
{
    m_left_extra_slot->msw_rescale();
    m_right_extra_slot->msw_rescale();
    for (auto item : m_mapping_item_list) {
        item->msw_rescale();
    }
    for (auto container : m_amsmapping_container_list) {
        container->msw_rescale();
    }
...
void AmsMapingPopup::update_materials_list(std::vector<std::string> list) { m_materials_list = list; }

void AmsMapingPopup::set_tag_texture(std::string texture) { m_tag_material = texture; }

bool AmsMapingPopup::is_match_material(std::string material) const { return m_tag_material == material ? true : false; }
        ...
        if (pos.x > p_rect.x && pos.y > p_rect.y && pos.x < (p_rect.x + item->GetSize().x) && pos.y < (p_rect.y + item->GetSize().y)) {
            if (item->m_tray_data.type == TrayType::NORMAL) {
                if (!m_ext_mapping_filatype_check && (item->m_ams_id == VIRTUAL_TRAY_MAIN_ID || item->m_ams_id == VIRTUAL_TRAY_DEPUTY_ID)) {
                    ...
                    if (!is_match_material(item->m_tray_data.filament_type)) {
                        return;
                    }
                }
            }

            if (item->m_tray_data.type == TrayType::EMPTY)
                return;
            if ((m_show_type == ShowType::LEFT && item->GetParent()->GetName() == "left") ||
                (m_show_type == ShowType::RIGHT && item->GetParent()->GetName() == "right") || m_show_type == ShowType::LEFT_AND_RIGHT) {
    ...
    if (!m_only_show_ext_spool) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            if (ams_type >= 1 || ams_type <= 3) { // 1:ams 2:ams-lite 3:n3f
                ...
                auto ams_mapping_item_container = new MappingContainer(nozzle_id == 0 ? m_right_marea_panel : m_left_marea_panel, "AMS-1",
                                                                       4);
            ...
            if (item) {
                for (const auto& mapping_res : ams_mapping_result) {
                    if (mapping_res.id == this->m_current_filament_id) {
                        if (mapping_res.ams_id == std::to_string(item->m_ams_id) && mapping_res.slot_id == std::to_string(item->m_slot_id)) {
    ...
    if (!m_only_show_ext_spool) {
        ...
        for (auto ams_iter = ams_list.begin(); ams_iter != ams_list.end(); ams_iter++) {
            ...
            auto ams_mapping_item_container = new MappingContainer(nozzle_id == 0 ? m_right_marea_panel : m_left_marea_panel,
                                                                   ams_iter->second->GetDisplayName(), ams_iter->second->GetSlotCount());
            ams_mapping_item_container->SetName(nozzle_id == 0 ? m_right_marea_panel->GetName() : m_left_marea_panel->GetName());
            ams_mapping_item_container->SetSizer(sizer_mapping_list);
            ams_mapping_item_container->Layout();
    ...
    for (auto i = 0; i < tray_data.size(); i++) {
        ...
        if (tray_data[i].type == NORMAL) {
            if (is_match_material(tray_data[i].filament_type)) {
                ...
                m_mapping_item->set_data(m_tag_material, wxColour(0xEE, 0xEE, 0xEE), tray_data[i].name, remain_detect_flag, tray_data[i],
                                         true);
...
void AmsMapingPopup::add_ext_ams_mapping(TrayData tray_data, MappingItem* item)
{
    ...
    if (tray_data.type == NORMAL) {
        if (is_match_material(tray_data.filament_type)) {
            ...
            item->set_data(m_tag_material, m_ext_mapping_filatype_check ? wxColour(0xEE, 0xEE, 0xEE) : tray_data.colour, tray_data.name,
                           false, tray_data, true);
            m_has_unmatch_filament = true;
        }

        item->Bind(wxEVT_LEFT_DOWN, [this, tray_data, item](wxMouseEvent& e) {
            if (m_ext_mapping_filatype_check && !is_match_material(tray_data.filament_type))
                return;
            item->send_event(m_current_filament_id);
            Dismiss();
        });
...
MappingItem::MappingItem(wxWindow* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
#ifdef __WINDOWS__
    SetDoubleBuffered(true);
#endif //__WINDOWS__
...
void MappingItem::send_event(int fliament_id)
{
    ...
    wxString param = wxString::Format("%d|%d|%d|%d|%s|%d|%d|%d", m_coloul.Red(), m_coloul.Green(), m_coloul.Blue(), m_coloul.Alpha(),
                                      m_tray_index, fliament_id, m_tray_data.ams_id, m_tray_data.slot_id);
...
static void _DrawRemainArea(const MappingItem* item, const TrayData& dd, bool support_remain_dect, wxDC& dc)
{
    ...
    if (!support_remain_dect) {
        to_paint_remain = 100;
    }
    if (0 > to_paint_remain || to_paint_remain > 100) {
        to_paint_remain = 100;
    }
        ...
        if (m_unmatch) {
            if (!m_name.IsEmpty() && (m_name != "-")) {
                const wxString& msg = wxString::Format(
                    _L("Note: the filament type(%s) does not match with the filament type(%s) in the slicing file. "
                ...
                const wxString& msg = wxString::Format(_L("Note: the slot is empty or undefined. If you want to use this slot, you can "
    ...
    if (m_tray_data.material_cols.size() > 1 && !m_unmatch) {
        ...
        if (m_tray_data.ctype == 0) {
            for (int i = 0; i < m_tray_data.material_cols.size() - 1; i++) {
                auto rect = wxRect(left, (size.y - MAPPING_ITEM_REAL_SIZE.y) / 2 + get_remain_area_height(), MAPPING_ITEM_REAL_SIZE.x,
                                   MAPPING_ITEM_REAL_SIZE.y);
            ...
            for (int i = 0; i < cols_size; i++) {
                ...
                dc.DrawRectangle(x, (size.y - MAPPING_ITEM_REAL_SIZE.y) / 2 + get_remain_area_height(),
                                 (float) MAPPING_ITEM_REAL_SIZE.x / cols_size, MAPPING_ITEM_REAL_SIZE.y);
            }
        }
    } else if (color.Alpha() == 0) {
        dc.DrawBitmap(m_transparent_mapping_item.bmp(), 0, (size.y - MAPPING_ITEM_REAL_SIZE.y) / 2 + get_remain_area_height());
    } else {
        dc.DrawRectangle(0, (size.y - MAPPING_ITEM_REAL_SIZE.y) / 2 + get_remain_area_height(), MAPPING_ITEM_REAL_SIZE.x,
                         MAPPING_ITEM_REAL_SIZE.y);
    }
...
AmsMapingTipPopup::AmsMapingTipPopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
    SetBackgroundColour(*wxWHITE);
    ...
    m_tip_enable_ams = new wxStaticText(m_panel_enable_ams, wxID_ANY, _L("Print with filaments in the AMS"), wxDefaultPosition,
                                        wxDefaultSize, 0);
    ...
    auto img_enable_ams = new wxStaticBitmap(m_panel_enable_ams, wxID_ANY, create_scaled_bitmap("monitor_upgrade_ams", this, 108),
                                             wxDefaultPosition, wxSize(FromDIP(118), FromDIP(108)), 0);
    ...
    m_tip_disable_ams = new wxStaticText(m_panel_disable_ams, wxID_ANY, _L("Print with the filament mounted on the back of chassis"),
    ...
    auto img_disable_ams = new wxStaticBitmap(m_panel_disable_ams, wxID_ANY, create_scaled_bitmap("disable_ams_demo_icon", this, 95),
                                              wxDefaultPosition, wxSize(FromDIP(95), FromDIP(109)), 0);
...
AmsHumidityTipPopup::AmsHumidityTipPopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
    SetBackgroundColour(*wxWHITE);
    ...
    curr_humidity_img   = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("hum_level1_light", this, 132), wxDefaultPosition,
                                             wxSize(FromDIP(132), FromDIP(132)), 0);

    m_staticText_note = new Label(
        this, _L("Please change the desiccant when it is too wet. "
    ...
    Bind(wxEVT_LEFT_UP, [this](auto& e) {
        ...
        if (mouse_pos.x > close_left && mouse_pos.y > close_top && mouse_pos.x < close_right && mouse_pos.y < close_bottom) {
            Dismiss();
        }
    });
...
AmsTutorialPopup::AmsTutorialPopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
    ...
    img_top = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("ams_item_examples", this, 30), wxDefaultPosition,
                                 wxSize(FromDIP(50), FromDIP(30)), 0);
    ...
    arrows_top = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("ams_arrow", this, 8), wxDefaultPosition,
                                    wxSize(FromDIP(24), FromDIP(8)), 0);
    ...
    arrows_bottom = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("ams_arrow", this, 8), wxDefaultPosition,
                                       wxSize(FromDIP(24), FromDIP(8)), 0);
    ...
    img_middle = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("ams_item_examples", this, 30), wxDefaultPosition,
                                    wxSize(FromDIP(50), FromDIP(30)), 0);
    ...
    img_botton = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("ams_mapping_examples", this, 87), wxDefaultPosition,
                                    wxDefaultSize, 0);
...
AmsIntroducePopup::AmsIntroducePopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
    ...
    m_img_enable_ams  = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("monitor_upgrade_ams", this, FromDIP(140)),
                                           wxDefaultPosition, wxDefaultSize, 0);
    m_img_disable_ams = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("disable_ams_demo_icon", this, FromDIP(110)),
                                           wxDefaultPosition, wxDefaultSize, 0);
        ...
        if (extder) {
            for (int filam : extder->GetFilamBackup()) {
                ...
                if (is_in_tray || m_tray_used.size() <= 0) {
                    m_groups_sizer->Add(create_backup_group(wxString::Format("%s%d", _L("Group"), group_index + 1), group_info,
                                                            group_material),
                                        0, wxALL, FromDIP(10));
                    group_index++;
                }
...
void AmsRMGroup::doRender(wxDC& dc)
{
    ...
    for (auto iter = m_group_info.rbegin(); iter != m_group_info.rend(); ++iter) {
        ...
        if (tray_color.Alpha() != 0) {
            ...
            if (tray_color == *wxWHITE)
                dc.DrawEllipticArc(x - center_mask_radius, y - center_mask_radius, center_mask_radius * 2, center_mask_radius * 2,
                                   startAngle, endAngle);
        }
    ...
    for (auto iter = m_group_info.rbegin(); iter != m_group_info.rend(); ++iter) {
        ...
        if (tray_color.Alpha() == 0) {
            dc.SetPen(wxPen(wxColour("#CECECE"), 2));
        }
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        ...
        // draw current
        // dc.DrawBitmap(backup_current_use_white.bmp(), x_center - text_size.x / 2 + FromDIP(3), size.y - y_center - text_size.y / 2 +
        // FromDIP(11));
        //...
        startAngle += ev_angle;
    }
    ...
    // draw center icon
    dc.DrawBitmap(bitmap_backup_tips_0.bmp(),
                  wxPoint((size.x - bitmap_backup_tips_0.GetBmpSize().x) / 2, (size.y - bitmap_backup_tips_0.GetBmpSize().y) / 2));
    // dc.DrawBitmap(bitmap_backup_tips_1.bmp(), wxPoint((size.x - bitmap_backup_tips_1.GetBmpSize().x) / 2, (size.y -
    // bitmap_backup_tips_1.GetBmpSize().y) / 2));
...
AmsHumidityLevelList::AmsHumidityLevelList(wxWindow* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
#ifdef __WINDOWS__
    SetDoubleBuffered(true);
#endif //__WINDOWS__
