...
// [INTENT] This notification owns the transient slice-progress overlay shown on top of the 3D canvas.
// [STATE] It tracks the progress state machine, fade opacity, multiline text layout, print-info availability, and the embedded Daily Tips
// subpanel. [UNITY] Port this as a retained HUD controller layered over the main viewport, with a separate panel for Daily Tips and
// explicit state transitions. [PORTING_HAZARD:P2] The overlay mixes progress lifecycle, close/cancel behavior, and late-arriving metadata
// into one immediate-mode widget.
namespace {
...
{
    // [EVENT] Theme changes must propagate to both the notification shell and the nested Daily Tips panel.
...
{
    // [STATE] The first render normalizes line counts, multiline layout, and fade state before the popup is drawn.
...
{
    // [STATE] Percent is a convenience entry point that promotes completion when it reaches 100% and rejects regressions.
    if (percent < 0.f)
...
bool NotificationManager::SlicingProgressNotification::set_progress_state(
    NotificationManager::SlicingProgressNotification::SlicingProgressState state, float percent /* = 0.f*/)
{
    // [STATE] This is the core progress state machine; it resets the shell for a new slice, guards monotonic progress, and clears stale
    // print info on terminal states.
    switch (state) {
...
{
    // [EVENT] Status text changes are pushed back through the notification system so the overlay can re-render its label content.
    switch (m_sp_state) {
    case Slic3r::GUI::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_NO_SLICING:
        m_state = EState::Hidden;
        break;
...
{
    // [STATE] Print metadata is only valid after completion; late arrivals are converted into a completion transition first.
...
{
    // [STATE] Collapsing the sidebar keeps the completed notification visible instead of letting it fade out immediately.
...
{
    // [EVENT] Cancel delegates to the slicer callback; if the callback declines, the overlay falls back to the no-slicing state.
...
{
    // [STATE] Cancelled and completed states stay visible briefly; live progress remains pinned until the surrounding notification system closes it.
...
{
    // [THREAD] The notification itself remains UI-thread owned, but the close/finish lifecycle is driven by the parent notification timer.
...
{
    // [OPENGL] Rendering stays immediate-mode ImGui over the canvas; the widget recomputes layout each frame instead of caching native
    // controls. [UNITY] Port this as a screen-space overlay controller that reflows from state, not a custom-painted native window.
    ...
    // if (m_sp_state == SlicingProgressNotification::SlicingProgressState::SP_COMPLETED || m_sp_state ==
    // SlicingProgressNotification::SlicingProgressState::SP_CANCELLED) 	m_window_width = m_line_height * 25;
    ...
    // const float  dailytips_panel_height = std::min(380.0f * scale, std::max(90.0f, (cnv_size.get_height() - gcodeviewer_height -
    // progress_panel_height - dailytips_child_window_padding.y - initial_y - m_line_height * 4)));
    ...
    m_window_height = progress_panel_height + m_dailytips_panel->get_size().y + progress_child_window_padding.y +
                      dailytips_child_window_padding.y + bottom_padding.y;
    ...
    if (imgui.begin(name, window_flags)) {
        ...
        //	ImVec2 button_pos = m_window_pos + ImVec2(m_window_width - button_size.x - button_right_margin_x, (m_window_height -
        // button_size.y) / 2.0f); 	float  text_left_margin_x = 15.0f * scale; 	ImVec2 text_pos = m_window_pos + ImVec2(text_left_margin_x,
        // m_window_height / 2.0f - m_line_height * 1.2f); 	ImVec2 view_dailytips_text_pos = m_window_pos + ImVec2(text_left_margin_x,
        // m_window_height / 2.0f + m_line_height * 0.2f);
        ...
        if (m_sp_state == SlicingProgressState::SP_CANCELLED || m_sp_state == SlicingProgressState::SP_PROGRESS ||
            m_sp_state == SlicingProgressState::SP_COMPLETED) {
            ...
            if (ImGui::BeginChild(child_name.c_str(), ImVec2(progress_panel_width, progress_panel_height), false, child_window_flags)) {
                ...
                ImVec2 button_pos        = child_window_pos + ImVec2(progress_panel_width - button_size.x,
                                                                     progress_panel_height - text_bottom - button_size.y / 2.0f);
            ...
            ImVec2 separator_max = ImVec2(ImGui::GetCursorScreenPos().x + progress_child_window_padding.x + progress_panel_width,
                                          ImGui::GetCursorScreenPos().y);
            ImGui::GetCurrentWindow()->DrawList->AddLine(separator_min, separator_max,
                                                         ImColor(238, 238, 238, (int) (255 * m_current_fade_opacity)));
...
{
    // [INTENT] Render the title area differently for each terminal/progress state: complete icon, cancelled text, or multiline progress title.
    ImGuiWrapper& imgui     = *wxGetApp().imgui();
    float         scale     = imgui.get_font_size() / 15.0f;
...
{
    // [OPENGL] The progress bar is a draw-list primitive, not a separate widget, so the fill width is recomputed every frame from m_percentage.
    if (m_sp_state != SlicingProgressState::SP_PROGRESS)
        return;
...
{
    // [INTENT] The embedded Daily Tips region is delegated wholesale to its own controller so the notification does not duplicate that UI logic.
    m_dailytips_panel->render();
}

void NotificationManager::SlicingProgressNotification::render_show_dailytips(const ImVec2& pos)
{
    // [EVENT] The link-style affordance only appears after completion or cancel, and it forwards directly into the plater's Daily Tips controller.
...
{
    // [UNITY] In Unity this should become a dedicated tips-button action on the overlay controller, not a global app singleton call.
...
{
    // [EVENT] Cancel is only interactive while slicing is active, and it dispatches back through the callback path rather than mutating
    // slicer state locally.
    if (m_sp_state == SlicingProgressState::SP_PROGRESS) {
        ImGuiWrapper& imgui = *wxGetApp().imgui();
...
{
    // [EVENT] Terminal states swap the affordance from cancel to close, with dark/light icon variants handled entirely in the draw code.
    if (m_sp_state == SlicingProgressState::SP_CANCELLED || m_sp_state == SlicingProgressState::SP_COMPLETED) {
        ImGuiWrapper& imgui = *wxGetApp().imgui();
