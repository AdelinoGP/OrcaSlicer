...
// [INTENT] TipsDialog displays modal dialogs for instructional tips, with a configurable "Don't show again" option.
// [UNITY] Replace with a UI Toolkit modal overlay/dialog system (e.g., UI Toolkit Dialog).
TipsDialog::TipsDialog(wxWindow*                        parent,
                       const wxString&                  title,
                       const wxString&                  description,
                       std::string                      app_key,
                       long                             style,
                       std::map<wxStandardID, wxString> option_map)
    : DPIDialog(parent, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX), m_app_key(app_key)
{
...
{
    if (m_confirm)
        m_confirm->Rescale(); // ORCA
    Fit();
    Refresh();
}

void ParamsPanel::Highlighter::set_timer_owner(wxEvtHandler* owner, int timerid /* = wxID_ANY*/) { m_timer.SetOwner(owner, timerid); }
    ...
    : wxPanel(parent, id, pos, size, style, name)
{
    ...
    if (dynamic_cast<Notebook*>(parent)) {
        ...
        // BBS: new layout
        // m_search_btn = new ScalableButton(m_top_panel, wxID_ANY, "search", wxEmptyString, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT
        // | wxNO_BORDER, true); m_search_btn->SetToolTip(format_wxstr(_L("Search in settings [%1%]"), _L("Ctrl+") + "F");
...
void ParamsPanel::refresh_tabs()
{
    ...
    for (auto tab : tabs_list)
        if (tab->supports_printer_technology(print_tech)) {
            ...
            case Preset::TYPE_SLA_PRINT: m_tab_print = tab; break;

            case Preset::TYPE_FILAMENT:
            case Preset::TYPE_SLA_MATERIAL: m_tab_filament = tab; break;
...
void ParamsPanel::set_active_tab(wxPanel* tab)
{
    ...
    // BBS: open/close tab
    for (auto t : std::vector<std::pair<wxPanel*, wxStaticLine*>>({{m_tab_print, m_staticline_print},
...
{
    if (m_process_icon)
        m_process_icon->msw_rescale();
    if (m_setting_btn)
        m_setting_btn->msw_rescale();
    if (m_search_btn)
        m_search_btn->msw_rescale();
    if (m_compare_btn)
        m_compare_btn->msw_rescale();
    if (m_tips_arrow)
        m_tips_arrow->msw_rescale();
    m_left_sizer->SetMinSize(wxSize(40 * em_unit(this), -1));
    if (m_mode_sizer)
    ...
    for (auto tab :
         {m_tab_print, m_tab_print_plate, m_tab_print_object, m_tab_print_part, m_tab_print_layer, m_tab_filament, m_tab_printer}) {
        if (tab)
            dynamic_cast<Tab*>(tab)->msw_rescale();
    }
...
void ParamsPanel::notify_object_config_changed()
{
    ...
    if (has_config)
        m_mode_region->SetTextColor2(
            StateColor(std::pair{0xfffffe, (int) StateColor::Checked}, std::pair{wxGetApp().get_label_clr_modified(), 0}));
    else
