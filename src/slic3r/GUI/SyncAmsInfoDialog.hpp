...
class CapsuleButton;
// [INTENT] Multi-state AMS sync modal plus two lightweight overlay frames for nozzle/AMS handoff confirmation and completion.
// [STATE] This header carries the dialog's printer selection, AMS mapping, thumbnail preview, and progress/timeout state in one place.
// [UNITY] Split into a modal controller backed by a retained data model, with separate popup/overlay prefabs for the sync confirm and
// completion banners. [PORTING_HAZARD:P2] The current design mixes UI composition, printer refresh, thumbnail compositing, and sync result
// assembly in one class.
class SyncAmsInfoDialog : public DPIDialog
{
    enum PageType { ptColorMap = 0, ptOverride };
    // [STATE] Dialog-level state tracks the active printer, page mode, cancel/timeout flags, and cached AMS mapping results.
    // [THREAD] The shared token, timer, and future/thread includes indicate async refresh work that can outlive one UI event.
...
protected:
    // [STATE] These pointers own the visible wx widgets for the modal shell, layered thumbnails, and mapping controls.
    // [OPENGL] Thumbnail data is CPU-side, but it feeds the rendered preview images that the user sees while remapping colors.
...
public:
    // [EVENT] These methods are the dialog's public lifecycle and event entry points for refresh, cancel, timer ticks, and accept/deny
    // flow. [UNITY] Model them as controller commands plus UI callbacks rather than widget-owned business logic.
...
public:
    // [STATE] SyncInfo is the entry payload for one open session; SyncResult is the durable output that later code consumes after the modal closes.
    struct SyncInfo
    {
        wxPoint dialog_pos;
...
private:
    // [EVENT] Private helpers rebuild the page, handle plate and mode changes, and repaint the control tree after state transitions.
    // [UNITY] This should become a view-model driven UI rebuild path with explicit setters for page mode and plate selection.
...
// [INTENT] Small transparent confirmation frame used when syncing nozzle and AMS selections.
// [UNITY] A borderless overlay with anchored positioning is the closest equivalent; keep the positional hints but make the interaction a
// normal modal callback.
...
// [INTENT] Terminal confirmation frame shown after AMS sync completes.
// [STATE] It only preserves the anchored positioning input needed to recreate the native placement behavior.
