...
// [UNITY] Use UI Toolkit to implement similar Dialog UI
// [INTENT] RammingDialog: Expert-level customization dialog for filament extrusion ramming settings.
RammingDialog::RammingDialog(wxWindow* parent, const std::string& parameters)
    : wxDialog(parent,
               wxID_ANY,
               _(L("Ramming customization")),
               wxDefaultPosition,
               wxDefaultSize,
               wxDEFAULT_DIALOG_STYLE /* | wxRESIZE_BORDER*/)
{
    SetBackgroundColour(*wxWHITE);
    // [STATE] m_panel_ramming: The main panel displaying ramming chart and controls
    ...
    // [EVENT] Handle closing dialog
    this->Bind(wxEVT_CLOSE_WINDOW, [this](wxCloseEvent& e) { EndModal(wxCANCEL); });

    // [EVENT] Handle OK button click, save parameters
    this->Bind(
        wxEVT_BUTTON,
        [this](wxCommandEvent&) {
    ...
    // [PORTING_HAZARD:P1] wxWidgets MessageDialog requires mapping to a Unity-based modal alert system (e.g., UI Toolkit Overlay).
    Slic3r::GUI::MessageDialog
        dlg(this,
            _(L("Ramming denotes the rapid extrusion just before a tool change in a single-extruder MM printer. Its purpose is to "
            ...
            _(L("Warning")), wxOK | wxICON_EXCLAMATION); // .ShowModal();
    dlg.ShowModal();
}
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize /*,wxPoint(50,50), wxSize(800,350),wxBORDER_RAISED*/)
{
    ...
    m_widget_time = new SpinInput(this, wxEmptyString, _L("ms"), wxDefaultPosition, wxSize(scale(120), -1), wxSP_ARROW_KEYS, 0, 5000, 3000,
                                  250);
    m_widget_volume = new SpinInput(this, wxEmptyString, _L("mm³"), wxDefaultPosition, wxSize(scale(120), -1), wxSP_ARROW_KEYS, 0, 10000, 0);
    m_widget_ramming_line_width_multiplicator = new SpinInput(this, wxEmptyString, "%", wxDefaultPosition, wxSize(scale(120), -1),
                                                              wxSP_ARROW_KEYS, 10, 300, 100);
    m_widget_ramming_step_multiplicator       = new SpinInput(this, wxEmptyString, "%", wxDefaultPosition, wxSize(scale(120), -1),
                                                              wxSP_ARROW_KEYS, 10, 300, 100);
    ...
    m_widget_time->Bind(wxEVT_SPINCTRL, [this](wxCommandEvent&) { m_chart->set_xy_range(m_widget_time->GetValue() * 0.001, -1); });
...
wxString WipingDialog::BuildTextObjStr(bool multi_language)
{
    ...
    if (multi_language) {
        auto_flush_tip = _L("Orca would re-calculate your flushing volumes everytime the filaments color changed or filaments changed. You "
                            "could disable the auto-calculate in Orca Slicer > Preferences");
        ...
        auto_flush_tip    = "Orca would re-calculate your flushing volumes everytime the filaments color changed or filaments changed. You "
                            "could disable the auto-calculate in Orca Slicer > Preferences";
    ...
    , m_max_flush_volume(max_flush_volume)
{
    ...
    m_webview    = wxWebView::New(this, wxID_ANY, wxEmptyString, wxDefaultPosition, applied_size, wxWebViewBackendDefault, wxNO_BORDER);
        ...
        try {
            ...
            if (j["msg"].get<std::string>() == "init") {
                ...
                { // limit all matrix value before write to gcode, the limitation is depends on the multipliers
                    ...
                    if (!store_matrixs.empty()) {
                        cols_temp_matrix = store_matrixs[0].size();
                    }
                    if (store_multipliers.size() == store_matrixs.size() && cols_temp_matrix > 0) // nuzzles==nuzzles
                    {
                        for (size_t idx = 0; idx < store_multipliers.size(); ++idx) {
                            ...
                            std::transform(store_matrixs[idx].begin(), store_matrixs[idx].end(), store_matrixs[idx].begin(),
