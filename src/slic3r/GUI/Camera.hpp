...
// [INTENT] Camera structure encapsulates the 3D viewport's viewing parameters, projection logic, and movement behaviors.
// [STATE] m_view_matrix (viewing transform), m_projection_matrix (projection transform), m_target (look-at point),
// m_distance (distance from eye to target), m_zoom (scale factor for ortho/near-plane for perspective).
// [UNITY] Maps to Unity's Camera component. Transform.position and Transform.rotation track the eye's pose.
// The look-at point (m_target) should be tracked by a custom C# CameraController script.
...
struct Camera
{
    ...
    // [INTENT] EType defines the projection mode of the camera.
    enum class EType : unsigned char { Unknown, Ortho, Perspective, Num_types };
    // [INTENT] ViewAngleType defines standardized camera positions for quick navigation.
...
private:
    ...
    // [STATE] Screen-space viewport rectangle (x, y, width, height).
    std::array<int, 4> m_viewport;
    // [STATE] Transformation matrices for rendering and coordinate mapping.
...
private:
    ...
    double calc_zoom_to_volumes_factor(const GLVolumePtrs& volumes,
                                       Vec3d&              center,
                                       double              margin_factor = DefaultZoomToVolumesMarginFactor) const;
