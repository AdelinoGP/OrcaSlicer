...
// [INTENT] Present a tiled view of PrinterFileSystem entries with thumbnails, selection controls, and action buttons plus hover/scroll UX
// in one wxWindow surface. [STATE] Tracks the window's visual density, hover/pressed tracking, and mask bitmaps so render() can rely on
// reusable assets while the timer drives the transient position bar.
static constexpr int SHADOW_WIDTH = 3;

ImageGrid::ImageGrid(wxWindow* parent)
    : wxWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
    , m_buttonBackgroundColor(
          StateColor(std::make_pair(*wxWHITE, (int) StateColor::Pressed), std::make_pair(*wxRED, (int) StateColor::Normal)))
    , m_buttonTextColor(StateColor(std::make_pair(0x3B4446, (int) StateColor::Pressed),
    ...
    , m_model_weight_icon(this, "model_weight", 14)
{
    ...
    // [STATE] The timer pulses keep the position bar visible after wheel activity, so the delayed Refresh runs on the UI thread with the
...
void ImageGrid::SetFileSystem(boost::shared_ptr<PrinterFileSystem> file_sys)
{
    ...
    // [EVENT] Every notification (mode change, metadata refresh, thumbnail ready, download progress) funnels back through changedEvent so
    // the grid invalidates on the main thread. [THREAD] PrinterFileSystem uses worker threads, so we rely on wx events to marshal these
    // updates safely into Refresh()/UpdateFileSystem().
    m_row_count     = 0;
    m_col_count     = 1;
    m_row_offset    = 0;
    m_scroll_offset = 0;
...
void ImageGrid::SetStatus(ScalableBitmap const& icon, wxString const& msg)
{
    ...
    Refresh();
    // [STATE] Records the last PrinterFileSystem error code/text and forces a repaint so the status overlay shows instantly even when
    // downloads fail mid-flight.
}
...
void Slic3r::GUI::ImageGrid::SetGroupMode(int mode)
{
    ...
    int    index = (m_row_offset + 1 < m_row_count || m_row_count == 0) ?
                       ...
                       ((m_file_sys->GetCount() + m_col_count - 1) / m_col_count -
                     (size.y + m_border_size.GetHeight() - 1) / m_cell_size.GetHeight()) *
                        m_col_count;
...
void Slic3r::GUI::ImageGrid::Rescale()
{
    ...
    UpdateFileSystem();
    // [STATE] Rebuilding mask/bitmap caches keeps the virtual grid geometry and button overlays DPI-aware; porters should regenerate
    // Texture2D overlays before repaint.
...
void Slic3r::GUI::ImageGrid::Select(size_t index)
{
    if (m_selecting) {
        ...
        Refresh();
        // [STATE] While multi-selecting, we only toggle selection bitmaps and keep m_row_offset untouched to avoid jumping the viewport.
        return;
    }
...
void Slic3r::GUI::ImageGrid::DoAction(size_t index, int action)
{
    ...
    // [EVENT] Emits a custom EVT_ITEM_ACTION so upper UI layers know when the user triggered actions like Download/Delete/Print; avoids
    // cross-thread calls by reusing wxCommandEvent.
}
...
void Slic3r::GUI::ImageGrid::UpdateFileSystem()
{
    ...
    // [STATE] Cell/border sizes update row/column counts, so Unity's ScrollView virtualization must recompute layout ranges when this state changes.
    UpdateLayout();
}
...
void ImageGrid::UpdateLayout()
{
    ...
    UpdateFocusRange();
    Refresh();
    // [UNITY] UpdateFocusRange keeps Unity's virtualized ScrollView aware of which cells should be cached as Texture2D thumbnails.
}
...
void Slic3r::GUI::ImageGrid::UpdateFocusRange()
{
    ...
    // [STATE] m_file_sys keeps a focused window of file indexes to prefetch thumbnails; this mirrors Unity's VirtualScrollList prewarm.
...
std::pair<int, size_t> Slic3r::GUI::ImageGrid::HitTest(wxPoint const& pt)
{
    ...
    // [EVENT] Translate pointer/X/Y coordinates into item/action/mode hits so mouse handlers can drive selection or toolbar actions;
    // Unity's PointerEventData would use a similar hit-test pass.
    wxPoint off;
    ...
    if (!m_content_rect.Contains(off)) {
        return {HIT_NONE, -1};
    }
    if (!m_selecting) {
        wxRect hover_rect{0, m_content_rect.GetHeight() - m_buttons_background.GetHeight(), m_content_rect.GetWidth(),
                          m_buttons_background.GetHeight()};
        auto&  file = m_file_sys->GetFile(index);
...
void ImageGrid::mouseMoved(wxMouseEvent& event)
{
    if (!m_hovered || m_pressed)
        return;
    // [EVENT] Mouse move updates hover state and tooltips; this mirrors Unity's UI Toolkit pointer events that update hover styles and
    // tooltip text.
...
void ImageGrid::mouseLeaveWindow(wxMouseEvent& event)
{
    if (m_hovered) {
        ...
        // [STATE] Clearing hovered/pressed states prevents stale tooltips or buttons from sticking after the pointer exits the grid.
    }
}
...
void ImageGrid::mouseDown(wxMouseEvent& event)
{
    if (!m_pressed) {
        m_pressed = true;
        auto hit  = HitTest(event.GetPosition());
        // [EVENT] Track the intended target so mouseReleased can differentiate between selecting, action buttons, group mode, or status retries.
        m_hit_type = hit.first;
        m_hit_item = hit.second;
...
void ImageGrid::mouseReleased(wxMouseEvent& event)
{
    ...
    m_pressed = false;
    auto hit  = HitTest(event.GetPosition());
    // [EVENT] Only process release if the cursor is over the same hit target to avoid accidental drags; this matches Unity's click gating logic.
    if (hit == std::make_pair(m_hit_type, m_hit_item)) {
...
void ImageGrid::resize(wxSizeEvent& event) { UpdateLayout(); }
...
void Slic3r::GUI::ImageGrid::changedEvent(wxCommandEvent& evt)
{
    ...
    // [EVENT] File-system change events update layout, status, or thumbnails; they originate from PrinterFileSystem workers but go through
    // this handler to avoid cross-thread UI access.
    if (evt.GetEventType() == EVT_FILE_CHANGED) {
        if (evt.GetInt() == -1)
            m_file_sys->DownloadCheckFiles(wxGetApp().app_config->get("download_path"));
        // [PORTING_HAZARD:P2] DownloadCheckFiles does synchronous IO/scan, so Unity should run it in a Task and post results back via
        // MainThreadDispatcher before calling UpdateFileSystem.
        UpdateFileSystem();
...
void ImageGrid::paintEvent(wxPaintEvent& evt)
{
    ...
    wxPaintDC dc(this);
    // [OPENGL] Custom paint hook runs on the UI thread; it mimics an immediate-mode render pass so Unity should replace it with a
    // PanelRenderer/VisualElement drawing into a Texture2D.
    render(dc);
}
...
size_t Slic3r::GUI::ImageGrid::firstItem(wxSize const& size, wxPoint& off)
{
    ...
    int offy  = (m_row_offset + 1 < m_row_count || m_row_count == 0) ?
                    m_cell_size.GetHeight() - m_border_size.GetHeight() - m_row_offset * m_cell_size.GetHeight() / 4 +
                       m_row_offset / 4 * m_cell_size.GetHeight() :
                    size_y - (size_y + m_border_size.GetHeight() - 1) / m_cell_size.GetHeight() * m_cell_size.GetHeight();
    int index = (m_row_offset + 1 < m_row_count || m_row_count == 0) ?
                    m_row_offset / 4 * m_col_count :
                    ((m_file_sys->GetCount() + m_col_count - 1) / m_col_count -
                     (size_y + m_border_size.GetHeight() - 1) / m_cell_size.GetHeight()) *
                        m_col_count;
    if (m_file_sys->GetGroupMode() == PrinterFileSystem::G_NONE)
        offy += m_title_mask.GetHeight();
    off = wxPoint{offx, offy};
    // [UNCLEAR] firstItem's sign-flipping math tries to keep the viewport pinned during grouping; verify scroll_offset behavior before porting.
    return index;
}
...
wxBitmap Slic3r::GUI::ImageGrid::createAlphaBitmap(wxSize size, wxColour color, int alpha1, int alpha2)
{
    wxImage image(size);
    image.InitAlpha();
    // [PORTING_HAZARD:P3] Manual alpha ramp construction should move to Unity Texture2D.FillTexData with direct alpha channel writes.
...
wxBitmap Slic3r::GUI::ImageGrid::createShadowBorder(wxSize size, wxColour color, int radius, int shadow)
{
    wxImage image(size);
    image.InitAlpha();
    // [PORTING_HAZARD:P3] Shadow drawing is built from alpha-blended strokes; Unity should reuse SpriteRenderer with a shader or a cached
    // shadow texture.
...
wxBitmap Slic3r::GUI::ImageGrid::createCircleBitmap(wxSize size, int borderWidth, int percent, wxColour fillColor, wxColour borderColor)
{
    wxImage image(size);
    image.InitAlpha();
    // [UNITY] Circle progress icons can map to Unity UI Toolkit vector icons or Sprite masks instead of raw pixel loops.
    memset(image.GetAlpha(), 0, size.GetWidth() * size.GetHeight());
    wxBitmap   bmp(std::move(image));
    wxRect     rc(0, 0, size.x, size.y);
    wxMemoryDC memdc;
...
{
    wxSize size = GetClientSize();
    // [OPENGL] Full repaint loop draws backgrounds, thumbnails, overlays, and scroll grips; port to Unity by converting each batch into
    // VisualElement/CanvasRenderer draw calls.
    ...
    size_t start     = index;
    size_t end       = index;
    size_t hit_image = m_selecting              ? size_t(-1) :
                       m_hit_type == HIT_ITEM   ? m_hit_item :
                       m_hit_type == HIT_ACTION ? m_hit_item / 4 :
                                                  size_t(-1);
    ...
    while (off.y < size.y) {
        ...
        while (index < end) {
            ...
            // Draw content
            decltype(&ImageGrid::renderContent1) contentRender[] = {&ImageGrid::renderContent1, &ImageGrid::renderContent1,
            ...
            dc.DrawRectangle(
                {pt.x + m_border_size.GetWidth(), pt.y, m_cell_size.GetWidth() - m_border_size.GetWidth(), m_border_size.GetHeight()});
    ...
    if (m_timer.IsRunning()) {
        int total_height = (m_file_sys->GetCount() + m_col_count - 1) / m_col_count * m_cell_size.GetHeight() + m_cell_size.GetHeight() -
                           m_border_size.GetHeight();
        if (total_height > size.y) {
            int    offset = (m_row_offset + 1 < m_row_count || m_row_count == 0) ? m_row_offset * (m_cell_size.GetHeight() / 4) :
                                                                                   total_height - size.y;
            wxRect rect   = {size.x - 16, offset * size.y / total_height, 8, size.y * size.y / total_height};
...
void Slic3r::GUI::ImageGrid::renderContent1(wxDC& dc, wxPoint const& pt, int index, bool hit)
{
    ...
    // [UNITY] Individual thumbnails plus download/print buttons should become VisualElement templates that swap Texture2D thumbnails and
    // render overlays via UI Toolkit PanelRenderer. Draw thumbnail
    ...
    // dc.GradientFillLinear({pt.x, pt.y, m_border_size.GetWidth(), 60}, wxColour(0x6F, 0x6F, 0x6F, 0x99), wxColour(0x6F, 0x6F, 0x6F, 0), wxBOTTOM);
        ...
        wxRect        rect{pt.x, pt.y + m_content_rect.GetBottom() - m_buttons_background.GetHeight(), m_content_rect.GetWidth(),
                    m_buttons_background.GetHeight()};
        wxArrayString texts;
        if (hit) {
            texts.Add(_L("Delete"));
...
{
    auto& file = m_file_sys->GetFile(index);
    // [UNITY] Model details overlay this thumbnail; port via VisualElement composite with a row for name/time/weight rendered via
    // PanelRenderer and cached Sprite. Draw thumbnail & buttons
...
void Slic3r::GUI::ImageGrid::renderButtons(wxDC& dc, wxArrayString const& texts, wxRect const& rect2, size_t hit, int states)
{
    // Draw background
    {
        // [PORTING_HAZARD:P3] Manual blending via alpha bitmaps needs replacement in Unity with SpriteBatch or UI Toolkit gradients.
    ...
    for (size_t i = 0; i < texts.size(); ++i) {
        ...
        // [UNITY] In Unity, wire each VisualElement button to the index-specific command and highlight via USS rather than manual drawing.
        // rect.Inflate(10, 5);
...
void Slic3r::GUI::ImageGrid::renderText(wxDC& dc, wxString const& text, wxRect const& rect, int states)
{
    ...
    // [UNITY] Text sizing loops mimic Unity's TextMeshPro auto-size adjustments for button labels.
