...
// [ANNOTATED]
// [INTENT] Private implementation details and GUI components for the ConfigWizard.
// This header defines the internal page structures, data models, and the 'priv'
// implementation class for the configuration wizard.
...
// [INTENT] Data structure representing a configuration bundle for a specific vendor.
struct Bundle
{
...
// [INTENT] Map of vendor IDs to their respective configuration bundles.
// [UNITY] Map to a ScriptableObject or Dictionary-based data model in C#.
...
// [INTENT] Structure managing filament/material presets and their compatibility.
struct Materials
{
    Technology technology;
    // [STATE] List of presets available for this technology.
    std::vector<const Preset*> presets;
    // [STATE] Mapping of material aliases to their compatibility counts.
    ...
    bool containts(const Preset* preset) const
    {
        ...
        return std::find_if(presets.begin(), presets.end(), [preset](const Preset* element) { return element == preset; }) != presets.end();
    }

    bool get_omnipresent(const Preset* preset) { return get_printer_counter(preset) == printers.size(); }
    ...
    template<class F> void filter_presets(const Preset* printer, const std::string& type, const std::string& vendor, F cb)
    {
        for (auto preset : presets) {
            ...
            if ((printer == nullptr ||
                 is_compatible_with_printer(PresetWithVendorProfile(prst, prst.vendor), PresetWithVendorProfile(prntr, prntr.vendor))) &&
...
// [INTENT] GUI component for picking printer models from a vendor.
// [UNITY] Use a ScrollView with reusable Checkbox UI Toolkit elements.
...
// [INTENT] Base class for pages in the configuration wizard.
// [UNITY] Map to a common Page UI Toolkit prefab or VisualElement.
struct ConfigWizardPage : wxPanel
{
    ...
    template<class T> T* append(T* thing, int proportion = 0, int flag = wxEXPAND | wxTOP | wxBOTTOM, int border = 10)
    {
...
// [INTENT] Template helper to extend wxListBox/wxCheckListBox with typed client data.
// [UNITY] Use UI Toolkit ListView with generic data binding (IEnumerable<T>).
template<class T, class D> struct DataList : public T
{
    ...
    const D& get_data(int n) { return *reinterpret_cast<const D*>(this->GetClientData(n)); }
...
// [INTENT] Wizard page for selecting materials (filaments/SLA resins).
// [UNITY] Use a multi-column layout with ListViews for filtering by type and vendor.
...
typedef std::map<std::string /* = vendor ID */, std::pair<PagePrinters* /* = FFF page */, PagePrinters* /* = SLA page */>> Pages3rdparty;

// [INTENT] Navigation index for the left side of the ConfigWizard.
// [UNITY] Use a ListView or a vertical layout of clickable items.
...
private:
    ...
    // [STATE] Index of the currently active page.
    size_t  item_active;
    ssize_t item_hover;
    size_t  last_page;
    ...
    // [EVENT] Custom painting for the index markers and labels.
    // [UNITY] Handle via USS or custom VisualElement drawing.
...
// [INTENT] Main implementation class for ConfigWizard, managing state and page transitions.
// [UNITY] Map to a central WizardController MonoBehaviour.
struct ConfigWizard::priv
{
    ConfigWizard*           q;
    ConfigWizard::RunReason run_reason = RR_USER;
    // [STATE] Temporary configuration store during wizard execution.
    ...
    // [STATE] Collection of all possible wizard pages.
    std::vector<ConfigWizardPage*> all_pages;

    priv(ConfigWizard* q) : q(q), filaments(T_FFF), sla_materials(T_SLA) {}

    // [INTENT] Load all available pages based on current mode and configuration.
    void load_pages();
    ...
    // [PORTING_HAZARD:P2] Wizard finalization involves applying config changes to global app state.
    // [UNITY] Ensure atomic application of changes to the ScriptableObject settings.
    bool on_bnt_finish();
...
struct ConfigWizard::priv
{
    ...
    priv(ConfigWizard* q) : q(q), appconfig_new(), filaments(T_FFF), sla_materials(T_SLA) {}
