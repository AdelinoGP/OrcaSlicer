...
public:
    ...
    void Decorate(wxBitmap& bmp)
    {
        ...
        wxBitmap    logo_bmp = *bmp_cache.load_svg(is_dark ? "splash_logo_dark" : "splash_logo", width,
                                                height); // use with full width & height
        ...
        wxRect version_rect(wxPoint(0, int(height * 0.70)), wxPoint(width, int(height * 0.70) + version_ext.GetHeight()));
...
static void register_win32_device_notification_event()
{
    wxWindow::MSWRegisterMessageHandler(WM_DEVICECHANGE, [](wxWindow* win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
        // Some messages are sent to top level windows by default, some messages are sent to only registered windows, and we explictely
        // register on MainFrame only.
    ...
    wxWindow::MSWRegisterMessageHandler(MainFrame::WM_USER_MEDIACHANGED,
                                        [](wxWindow* win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
                                            // Some messages are sent to top level windows by default, some messages are sent to only
                                            // registered windows, and we explictely register on MainFrame only.
                                            auto main_frame = dynamic_cast<MainFrame*>(win);
                                            auto plater     = (main_frame == nullptr) ? nullptr : main_frame->plater();
                                            if (plater == nullptr)
                                                // Maybe some other top level window like a dialog or maybe a pop-up menu?
...
// [INTENT] Post-initialization - called after main window is shown
// [STATE] Handles command-line file args, initializes OpenGL, starts preset sync
// [THREAD] runs on UI thread after main window construction
// [OPENGL] Calls init_opengl() and GLCanvas3D::init()
// [UNITY] Separate into Start() (after Awake) - Unity's scene loading is async
// [PORTING_HAZARD:P2] File loading on startup - Unity needs async Addressables/AssetBundle loading
void GUI_App::post_init()
{
    ...
    if (!this->init_params->input_files.empty()) {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", init with input files, size %1%, input_gcode %2%") %
        ...
        if (this->init_params->input_files.size() == 1 && is_supported_open_protocol(first_url)) {
            start_download(first_url); // [UNITY] Use UnityWebRequestAsyncOperation
            m_open_method = "url";
        } else {
            if (this->init_params->input_gcode) {
                mainframe->select_tab(size_t(MainFrame::tp3DEditor));
                plater_->select_view_3D("3D");
                this->plater()->load_gcode(from_u8(this->init_params->input_files.front())); // [UNITY] Async load
                m_open_method = "gcode";
            } else {
                mainframe->select_tab(size_t(MainFrame::tp3DEditor));
                plater_->select_view_3D("3D");
...
if (!m_networking_need_update && m_agent) {
    ...
    m_agent->set_on_http_error_fn([this](unsigned int status, std::string body) { this->handle_http_error(status, body); });
    m_agent->start_discovery(true, false);
}

// update the plugin tips
CallAfter([this] { mainframe->refresh_plugin_tips(); });
...
if (!log_addr.empty()) {
    ...
    if (boost::filesystem::exists(log_folder)) {
        ...
        std::sort(files_vec.begin(), files_vec.end(),
                  [](std::pair<time_t, std::string>& a, std::pair<time_t, std::string>& b) { return a.first > b.first; });

        while (files_vec.size() > LOG_FILES_MAX_NUM) {
            auto full_path = log_folder / boost::filesystem::path(files_vec[files_vec.size() - 1].second);
            BOOST_LOG_TRIVIAL(info) << "delete log file over " << LOG_FILES_MAX_NUM
                                    << ", filename: " << files_vec[files_vec.size() - 1].second;
            try {
                boost::filesystem::remove(full_path);
            } catch (const std::exception& ex) {
                BOOST_LOG_TRIVIAL(error) << "failed to delete log file: " << files_vec[files_vec.size() - 1].second
                                         << ". Error: " << ex.what();
            }
            files_vec.pop_back();
        }
...
// [INTENT] GUI_App constructor - initializes application components
// [STATE] Initializes app config, download path, webview runtime, ImGui wrapper
// [UNITY] Unity: Replace constructor with MonoBehaviour Awake() or [RuntimeInitializeOnLoadMethod]
// [PORTING_HAZARD:P1] wxApp initialization happens before main window creation - Unity's Awake runs on GameObject creation
GUI_App::GUI_App()
    : wxApp() // [UNITY] Replace with MonoBehaviour component on GameObject
    //, m_app_mode(mode)
    , m_app_mode(EAppMode::Editor)                                         // [UNITY] Remove - Unity apps are single-purpose
    , m_em_unit(10)                                                        // [UNITY] Calculate from Screen.dpi or Canvas.scaleFactor
    , m_imgui(new ImGuiWrapper())                                          // [UNITY] Replace with Unity UI Toolkit (VisualElements) or uGUI
    , m_removable_drive_manager(std::make_unique<RemovableDriveManager>()) // [UNITY] Use Unity's persistentDataPath
    , m_downloader(std::make_unique<Downloader>())                         // [UNITY] Use UnityWebRequest for HTTP
    , m_other_instance_message_handler(std::make_unique<OtherInstanceMessageHandler>()) // [UNITY] Remove - Unity doesn't support IPC
{
    // app config initializes early becasuse it is used in instance checking in OrcaSlicer.cpp
    this->init_app_config();    // [UNITY] Use ScriptableObject or PlayerPrefs
    this->init_download_path(); // [UNITY] Use Application.persistentDataPath/downloads
#if wxUSE_WEBVIEW_EDGE
    this->init_webview_runtime(); // [UNITY] Remove - no WebView2 in Unity
#endif

    reset_to_active(); // [UNITY] Remove or use Application.focusChanged
}

// [INTENT] Application shutdown - cleanup resources before exit
// [STATE] Clears device manager, login dialogs, networking
// [UNITY] Replace with MonoBehaviour OnDestroy() or Application.quitting event
// [PORTING_HAZARD:P1] Explicit deletion order in wxWidgets vs Unity's garbage collection
void GUI_App::shutdown()
{
    ...
    if (login_dlg != nullptr) {
        ...
        delete login_dlg; // [UNITY] Let Unity's GameObject.Destroy() handle this
        login_dlg = nullptr;
    }
...
int GUI_App::download_plugin(std::string name, std::string package_name, InstallProgressFn pro_fn, WasCancelledFn cancel_fn)
{
                    ...
                    if (resource.is_array()) {
                        for (auto iter = resource.begin(); iter != resource.end(); iter++) {
                            ...
                            BOOST_LOG_TRIVIAL(info)
                                << "[download_plugin 1]: get type " << type << ", version " << version.to_string() << ", url " << url;
                            download_url = url;
                        }
                    }
                } else {
    ...
    http.on_progress([this, &pro_fn, cancel_fn, &result, &reported_percent, &err_msg](Slic3r::Http::Progress progress, bool& cancel) {
            int percent = 0;
                    ...
                    if (res == 0) {
                        ...
                        if (res == 0) {
                            ...
                            BOOST_LOG_TRIVIAL(error)
                                << "[install_plugin]Archive read error:" << mz_zip_get_error_string(zip_error) << std::endl;
...
#elif defined(__WXMAC__)
            auto versioned_lib = plugin_folder /
                                 (std::string("lib") + std::string(BAMBU_NETWORK_LIBRARY) + "_" + config_version + ".dylib");
#else
            auto versioned_lib = plugin_folder / (std::string("lib") + std::string(BAMBU_NETWORK_LIBRARY) + "_" + config_version + ".so");
#endif
            BOOST_LOG_TRIVIAL(info) << "[install_plugin] renaming newly extracted " << legacy_lib_path.string() << " to "
                                    << versioned_lib.string();
            boost::system::error_code ec;
...
// [INTENT] Restart networking subsystem - reset agent and reinitialize callbacks
// [THREAD] Runs on UI thread, spawns network discovery threads
// [EVENT] Triggers machine discovery and preset sync
// [UNITY] Replace with Unity's network manager + custom C# networking layer
...
// [INTENT] Process pending wx events with bounded iteration count to prevent infinite loops
// [THREAD] Runs on UI thread, uses wxYield() which pumps messages
// [EVENT] Processes wxWidgets event queue
// [UNITY] Replace with Unity's main thread dispatcher or Application.tick
...
// [INTENT] Wait for network operations to complete with state verification
// [THREAD] Runs on UI thread, polls network state with sleep intervals
// [EVENT] Processes pending events while waiting for idle state
// [UNITY] Replace with Unity's async/await pattern for network operations
...
bool GUI_App::hot_reload_network_plugin()
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": after restart_networking, is_user_login = " << user_logged_in
                            << ", m_agent = " << (m_agent ? "valid" : "null") << ", version = " << loaded_version;
...
std::string GUI_App::get_latest_network_version() const { return Slic3r::get_latest_network_version(); }
...
void GUI_App::show_network_plugin_download_dialog(bool is_update)
{
    ...
    NetworkPluginDownloadDialog dlg(mainframe, mode, current_version, load_error.message, load_error.technical_details);
...
// [INTENT] Set up network agent callbacks for cloud/printer communication
// [EVENT] Register lambda callbacks for: server connect, printer connect, messages, local LAN events
// [THREAD] Callbacks execute on network thread, use CallAfter() to marshal to UI thread
// [UNITY] Replace NetworkAgent with UnityWebRequest + custom C# networking layer
// [PORTING_HAZARD:P1] CallAfter() pattern doesn't exist - use Unity main thread synchronization
                    ...
                    if (obj->is_lan_mode_printer()) {
                        if (state == ConnectStatus::ConnectStatusOk) {
                            ...
                            if (msg == "5") {
                                ...
                                text = wxString::Format(_L("Connect %s failed! [SN:%s, code=%s]"), from_u8(obj->get_dev_name()),
                                                        obj->get_dev_id(), msg);
                                wxGetApp().show_dialog(text);
                            }
                            event.SetInt(-1);
...
std::string GUI_App::get_gl_info(bool for_github) { return OpenGLManager::get_gl_info().to_string(for_github); }
...
// [INTENT] Initialize OpenGL resources - texture, VAO, VBO, shaders
// [OPENGL] Calls OpenGLManager::init_gl() to create GL context and check capabilities
// [THREAD] Must run on UI thread with valid GL context
// [UNITY] Very different - Unity manages GL contexts internally via Graphics API
// [PORTING_HAZARD:P0] Direct OpenGL access not available in Unity C# - use Unity's Graphics/Drawing APIs
bool GUI_App::init_opengl()
{
#ifdef __linux__
    bool status          = m_opengl_mgr.init_gl(); // [UNITY] Remove - Unity handles GL internally
    m_opengl_initialized = true;
    return status;
#else
    return m_opengl_mgr.init_gl(); // [UNITY] Replace with UnityEditor.Graphics or Runtime Graphics APIs
#endif
}
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(
            _L("Orca Slicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
void GUI_App::copy_older_config() { preset_bundle->copy_files(m_older_data_dir_path); }
    ...
    Bind(wxEVT_TIMER, [this, start_subscribe_timer, dev_id](auto& e) {
        ...
        if ((dev_id == obj->get_dev_id()) && obj->is_connecting() && obj->subscribe_counter > 0) {
            ...
            if (wxGetApp().getAgent())
                wxGetApp().getAgent()->set_user_selected_machine(dev_id);
...
// [INTENT] wxWidgets OnInit - application entry point called on startup
// [EVENT] Entry point for wxWidgets application lifecycle
// [THREAD] Runs on main UI thread
// [UNITY] Replace with MonoBehaviour.Awake() or [RuntimeInitializeOnLoadMethod]
// [PORTING_HAZARD:P1] wxWidgets OnInit happens before any windows are created - Unity's scene loading happens after engine init
bool GUI_App::OnInit()
{
    try {
...
{
    void DoLogText(const wxString& msg) override { BOOST_LOG_TRIVIAL(warning) << msg.ToUTF8().data(); }
    ~wxBoostLog() override
    {
...
bool GUI_App::on_init_inner()
{
    ...
    g_log_set_handler(
        ...
        [](const gchar* log_domain, GLogLevelFlags log_level, const gchar* message, gpointer user_data) {
            if (message && (strstr(message, "gtk_widget_set_allocation") || strstr(message, "WIDGET_REALIZED_FOR_EVENT")))
                return;
    ...
    if (is_editor()) {
        ...
        if (!msg.empty() && !ssl_accept) {
            ...
            app_config->set("tls_cert_store_accepted", dlg.IsCheckBoxChecked() ? "yes" : "no");
            app_config->set("tls_accepted_cert_store_location", dlg.IsCheckBoxChecked() ? Slic3r::Http::tls_system_cert_store() : "");
        }
    }
    ...
    if (bool new_dark_color_mode = dark_mode(); init_dark_color_mode != new_dark_color_mode) {
#ifdef __WINDOWS__
    ...
    if (m_last_config_version) {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format("last app version {%1%.%2%.%3%}, current version {%4%.%5%.%6%}") % last_major %
                                       last_minor % last_patch % cur_major % cur_minor % cur_patch;
        if ((last_major != cur_major) || (last_minor != cur_minor) || (last_patch != cur_patch)) {
    ...
    if (is_editor()) {
        ...
        Bind(EVT_SLIC3R_VERSION_ONLINE, [this](const wxCommandEvent& evt) {
            if (this->plater_ != nullptr) {
                ...
                if (!skip_this_version || evt.GetInt() != 0) {
                    ...
                    case wxID_YES: wxLaunchDefaultBrowser(version_info.url); break;
                    case wxID_NO: break;
                    default:;
                    }
                }
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
            ...
            DownloadDialog dialog(this->mainframe, tips, _L("The Orca Slicer needs an upgrade"), false, wxCENTER | wxICON_INFORMATION);
            dialog.SetExtendedMessage(description_text);

            int result = dialog.ShowModal();
            switch (result) {
            case wxID_YES: wxLaunchDefaultBrowser(download_url); break;
            case wxID_NO: wxGetApp().mainframe->Close(true); break;
            default: wxGetApp().mainframe->Close(true);
            }
        });
        ...
        Bind(EVT_SHOW_DIALOG, [this](const wxCommandEvent& evt) {
            ...
            dlg.Bind(wxEVT_DESTROY, [this](auto& e) { m_info_dialog_content = wxEmptyString; });
            dlg.ShowModal();
        });
    } else {
#ifdef __WXMSW__
    ...
    BOOST_LOG_TRIVIAL(info) << "Network plugin mode: "
                            << (NetworkAgent::use_legacy_network ?
                                    ("legacy (version: " + std::string(BAMBU_NETWORK_AGENT_VERSION_LEGACY) + ")") :
                                    ("modern (version: " + configured_version + ")"));
    ...
    /* if (!NetworkAgent::use_legacy_network) {
        ...
        if (debugger_attached) {
            ...
            wxMessageBox("Force using legacy bambu networking plugin because debugger is attached! If the app terminates itself immediately,
please delete installed plugin and try again!");
        }
    } */
    ...
    try {
        ...
        init_params->preset_substitutions = preset_bundle->load_presets(*app_config,
...
void GUI_App::copy_network_if_available()
{
    ...
    if (boost::filesystem::exists(network_library)) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": Copying network library from " << network_library << " to " << network_library_dst
                                << " successfully.";
    ...
    if (boost::filesystem::exists(player_library)) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": Copying player library from " << player_library << " to " << player_library_dst
                                << " successfully.";
    }
    ...
    if (boost::filesystem::exists(live555_library)) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": Copying live555 library from " << live555_library << " to " << live555_library_dst
                                << " successfully.";
    }
    if (boost::filesystem::exists(changelog_file))
...
bool GUI_App::on_init_network(bool try_backup)
{
    ...
    if (should_load_networking_plugin && m_networking_compatible && !NetworkAgent::use_legacy_network) {
        ...
        if (has_network_update_available()) {
            ...
            bool should_prompt = !app_config->is_network_update_prompt_disabled() && !app_config->is_network_version_skipped(latest) &&
...
const wxColour GUI_App::get_label_default_clr_system() { return dark_mode() ? wxColour(115, 220, 103) : wxColour(26, 132, 57); }

const wxColour GUI_App::get_label_default_clr_modified() { return dark_mode() ? wxColour(253, 111, 40) : wxColour(252, 77, 1); }
    ...
    if (wxButton* btn = dynamic_cast<wxButton*>(window)) {
        if (btn->GetWindowStyleFlag() & wxBU_AUTODRAW)
            ...
            if (btn->GetId() == wxID_OK || btn->GetId() == wxID_CANCEL) {
                ...
                // hovering
                btn->Bind(wxEVT_ENTER_WINDOW, [mark_button](wxMouseEvent& event) {
                    mark_button(true);
                    event.Skip();
                });
                btn->Bind(wxEVT_LEAVE_WINDOW, [mark_button, btn](wxMouseEvent& event) {
                    mark_button(is_focused(btn->GetHWND()));
                    event.Skip();
                });
                // focusing
                btn->Bind(wxEVT_SET_FOCUS, [mark_button](wxFocusEvent& event) {
                    mark_button(true);
                    event.Skip();
                });
                btn->Bind(wxEVT_KILL_FOCUS, [mark_button](wxFocusEvent& event) {
                    mark_button(false);
                    event.Skip();
                });
...
void GUI_App::UpdateDarkUIWin(wxWindow* win) { update_dark_children_ui(win); }
...
bool GUI_App::get_side_menu_popup_status() { return m_side_popup_status; }

void GUI_App::set_side_menu_popup_status(bool status) { m_side_popup_status = status; }
...
bool GUI_App::tabs_as_menu() const { return false; }

wxSize GUI_App::get_min_size() const { return wxSize(76 * m_em_unit, 49 * m_em_unit); }
...
{
    try {
        auto iter = std::find_if(dialogStack.begin(), dialogStack.end(),
                                 [](auto dialog) { return dynamic_cast<DownloadProgressDialog*>(dialog) != nullptr; });
...
static void update_scrolls(wxWindow* window)
{
    ...
    while (node) {
        ...
        if (dynamic_cast<wxScrollHelper*>(win) || dynamic_cast<wxTreeCtrl*>(win) || dynamic_cast<wxTextCtrl*>(win))
...
#ifdef _MSW_DARK_MODE
void GUI_App::force_menu_update() { NppDarkMode::SetSystemMenuForApp(app_config->get("sys_menu_enabled") == "1"); }
#endif //_MSW_DARK_MODE
#endif //__WINDOWS__
...
void GUI_App::persist_window_geometry(wxTopLevelWindow* window, bool default_maximized)
{
    ...
    if (window_pos_restore(window, "mainframe", default_maximized)) {
        on_window_geometry(window, [=]() { window_pos_sanitize(window); });
    } else {
        on_window_geometry(window, [=]() { window_pos_center(window); });
    }
}
...
{
    input_file.Clear();
    wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (3MF):"), app_config->get_last_dir(), "",
...
{
    wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose ZIP file") + ":", from_u8(app_config->get_last_dir()), "",
...
{
    input_file.Clear();
    wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (GCODE/3MF):"), app_config->get_last_dir(), "",
...
wxString GUI_App::transition_tridid(int trid_id) const
{
    ...
    wxString maping_dict[] = {"A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M",
...
void GUI_App::request_user_logout()
{
    if (m_agent && m_agent->is_user_login()) {
        ...
        wxGetApp().check_and_keep_current_preset_changes(_L("User logged out"), header, ActionButtons::KEEP | ActionButtons::SAVE,
    ...
    try {
        ...
        if (command.has_value()) {
            ...
            if (command_str.compare("request_project_download") == 0) {
                ...
                CallAfter([this] { get_login_info(); });
            } else if (command_str.compare("homepage_login_or_register") == 0) {
                CallAfter([this] { this->request_login(true); });
            } else if (command_str.compare("homepage_logout") == 0) {
                CallAfter([this] { wxGetApp().request_user_logout(); });
...
void GUI_App::request_open_project(std::string project_id)
{
    ...
    if (project_id == "<new>")
        ...
        plater()->load_project();
    else if (std::find_if_not(project_id.begin(), project_id.end(), [](char c) { return std::isdigit(c); }) == project_id.end())
        ;
...
void GUI_App::request_remove_project(std::string project_id) { mainframe->remove_recent_project(-1, wxString::FromUTF8(project_id)); }
...
void GUI_App::on_http_error(wxCommandEvent& evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(
            nullptr,
            _L("The version of Orca Slicer is too low and needs to be updated to the latest version before it can be used normally."), "",
            wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {}
    }
...
void GUI_App::on_user_login_handle(wxCommandEvent& evt)
{
    ...
    boost::thread update_thread = boost::thread([this, dev] { dev->update_user_machine_list_info(); });
...
void GUI_App::reset_to_active() { last_active_point = std::chrono::system_clock::now(); }
...
void GUI_App::check_new_version(bool show_tips, int by_user)
{
    ...
    std::string query_params = (boost::format("?name=slicer&version=%1%&guide_version=%2%") %
                                VersionInfo::convert_full_version(SLIC3R_VERSION) % VersionInfo::convert_full_version("0.0.0.1"))
                                   .str();
                    ...
                    if (j["message"].get<std::string>() == "success") {
                        if (j.contains("software")) {
                            if (j["software"].empty() && show_tips) {
                                ...
                                if (j["software"].contains("url") && j["software"].contains("version") &&
                                ...
                                CallAfter([this, show_tips, by_user]() { this->check_update(show_tips, by_user); });
                            }
                        }
                    }
                }
...
std::string detect_updater_version() { return SoftFever_VERSION; }
...
std::string build_updater_query(const UpdaterQuery& query)
{
    ...
    std::sort(params.begin(), params.end(), [](const auto& lhs, const auto& rhs) { return lhs.first < rhs.first; });
...
void maybe_attach_updater_signature(Http& http, const std::string& canonical_query, const std::string& request_url)
{
    ...
    if (HMAC(EVP_sha256(), key->data(), static_cast<int>(key->size()), reinterpret_cast<const unsigned char*>(string_to_sign.data()),
             string_to_sign.size(), digest, &digest_length) == nullptr ||
        digest_length == 0)
        return;
        ...
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr,
                                      _L("The certificate has expired. Please check the time settings or update OrcaSlicer and try again."),
                                      "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate is no longer valid and the printing functions are unavailable."), "",
                                      wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(
                    nullptr,
                    _L("Internal error. Please try upgrading the firmware and OrcaSlicer version. If the issue persists, contact support."),
                    "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
...
{
    if (privacy_version_info.version_str.empty() || privacy_version_info.description.empty() || privacy_version_info.url.empty()) {
        return false;
    }
                    ...
                    if (j["message"].get<std::string>() == "success") {
                        if (j.contains("resources")) {
                            for (auto it = j["resources"].begin(); it != j["resources"].end(); it++) {
                                if (it->contains("type")) {
                                    if ((*it)["type"] == std::string("policy/privacy") && it->contains("version") &&
                                        it->contains("description") && it->contains("url") && it->contains("force_update")) {
    ...
    if (setting_id.empty() && preset->sync_info.empty()) {
        ...
        if (!setting_id.empty()) {
            ...
            if (!ret) {
                ...
                BOOST_LOG_TRIVIAL(trace) << "[sync_preset]update: can not generate differed key-values, we need to skip this preset "
                                         << preset->name;
                result = 0;
            }
        } else {
    ...
    if (http_code >= 400 && values_map["code"] == "14") { // Limit
        ...
        CallAfter([this] {
            ...
            if (mainframe == nullptr)
                return;
            auto msg = _L("The number of user presets cached in the cloud has exceeded the upper limit, newly created user presets can "
...
void GUI_App::start_sync_user_preset(bool with_progress_dlg)
{
    ...
    if (with_progress_dlg) {
        ...
        progressFn = [this, dlg](int percent) { CallAfter([=] { dlg->Update(percent, _L("Loading user preset")); }); };
        cancelFn   = [this, dlg]() { return is_closing() || dlg->WasCanceled(); };
...
void GUI_App::stop_http_server() { m_http_server.stop(); }

void GUI_App::switch_staff_pick(bool on) { mainframe->m_webview->SendDesignStaffpick(on); }
...
#ifdef __linux__
static const wxLanguageInfo* linux_get_existing_locale_language(const wxLanguageInfo* language, const wxLanguageInfo* system_language)
{
    ...
    locales.erase(std::remove_if(locales.begin(), locales.end(),
                                 [](const std::string& s) { return !std::regex_match(s, std::regex("^[a-z]+_[A-Z]{2}$")); }),
                  locales.end());
...
int GUI_App::GetSingleChoiceIndex(const wxString& message, const wxString& caption, const wxArrayString& choices, int initialSelection)
{
#ifdef _WIN32
...
bool GUI_App::load_language(wxString language, bool initial)
{
        ...
        if (!language.empty())
            ...
            if (lang_system != wxLANGUAGE_UNKNOWN) {
            ...
            } else {
                {
                    ...
                    temp_locale.Init();
                    // Set the current translation's language to default, otherwise GetBestTranslation() may not work (see the wxWidgets
                    // source code).
                    wxTranslations::Get()->SetLanguage(wxLANGUAGE_DEFAULT);
                    // Let the wxFileTranslationsLoader enumerate all translation dictionaries for PrusaSlicer
                    // and try to match them with the system specific "preferred languages".
                    // There seems to be a support for that on Windows and OSX, while on Linuxes the code just returns
    ...
    if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
        BOOST_LOG_TRIVIAL(trace)
            << boost::format("The following language code requires right to left layout, which is not supported by OrcaSlicer: %1%") %
    ...
    if (!wxLocale::IsAvailable(language_info->Language) && m_language_info_system) {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format("Can't switch language to %1% (missing locales). Using %2% instead.") % original_lang %
                                       language_info->CanonicalName.ToUTF8().data();
    }
#endif
...
Tab* GUI_App::get_plate_tab() { return plate_tab; }

Tab* GUI_App::get_model_tab(bool part) { return model_tabs_list[part ? 1 : 0]; }

Tab* GUI_App::get_layer_tab() { return model_tabs_list[2]; }
...
ConfigOptionMode GUI_App::get_mode()
{
    ...
    return mode == "advanced" ? comAdvanced : mode == "simple" ? comSimple : mode == "develop" ? comDevelop : comSimple;
}
...
//         local_menu->Append(config_id_base + ConfigMenuUpdate, _L("Check for Configuration Updates"), _L("Check for configuration
...
//         Bind(wxEVT_UPDATE_UI, [this](wxUpdateUIEvent& evt) { if (get_mode() == comSimple) evt.Check(true); }, config_id_base +
//         ConfigMenuModeSimple); Bind(wxEVT_UPDATE_UI, [this](wxUpdateUIEvent& evt) { if (get_mode() == comAdvanced) evt.Check(true); },
//         config_id_base + ConfigMenuModeAdvanced);
//
...
// - Taking snapshot
bool GUI_App::check_and_save_current_preset_changes(const wxString& caption,
                                                    const wxString& header,
                                                    bool            remember_choice /* = true*/,
                                                    bool            dont_save_insted_of_discard /* = false*/)
{
...
bool GUI_App::check_and_keep_current_preset_changes(const wxString& caption,
                                                    const wxString& header,
                                                    int             action_buttons,
...
bool GUI_App::can_load_project() { return true; }
...
bool GUI_App::check_print_host_queue()
{
    ...
    MessageDialog dialog(mainframe, message, wxString(SLIC3R_APP_NAME) + " - " + _(L("Ongoing uploads")),
...
std::vector<std::string>& GUI_App::get_delete_cache_presets() { return need_delete_presets; }
...
Sidebar& GUI_App::sidebar() { return plater_->sidebar(); }
...
ObjectSettings* GUI_App::obj_settings() { return sidebar().obj_settings(); }

ObjectList* GUI_App::obj_list() { return sidebar().obj_list(); }

ObjectLayers* GUI_App::obj_layers() { return sidebar().obj_layers(); }
...
Model& GUI_App::model() { return plater_->model(); }

Downloader* GUI_App::downloader() { return m_downloader.get(); }
...
char GUI_App::from_hex(char ch) { return isdigit(ch) ? ch - '0' : tolower(ch) - 'a' + 10; }

std::string GUI_App::url_decode(std::string value) { return Http::url_decode(value); }

std::string GUI_App::url_encode(std::string value) { return Http::url_encode(value); }
...
int GUI_App::extruders_cnt() const
{
    ...
    return preset.printer_technology() == ptSLA ? 1 : preset.config.option<ConfigOptionFloats>("nozzle_diameter")->values.size();
}
...
// BBS
int GUI_App::filaments_cnt() const { return preset_bundle->filament_presets.size(); }
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
{
    ...
    //     if (preset_updater->config_update(app_config->orig_version(), PresetUpdater::UpdateParams::FORCED_BEFORE_WIZARD) ==
...
void GUI_App::gcode_thumbnails_debug()
{
    ...
    wxFileDialog dialog(GetTopWindow(), _L("Select a G-code file:"), "", "", "G-code files (*.gcode)|*.gcode;*.GCODE;",
...
void GUI_App::check_updates(const bool verbose)
{
    ...
    try {
        updater_result = preset_updater->config_update(app_config->orig_version(), verbose ?
                                                                                       PresetUpdater::UpdateParams::SHOW_TEXT_BOX :
...
bool GUI_App::open_browser_with_warning_dialog(const wxString& url, int flags /* = 0*/) { return wxLaunchDefaultBrowser(url, flags); }
    ...
    for (const ModelObject* mo : model_objects) {
        for (auto vol : mo->volumes) {
            ...
            for (auto id : ve) {
                ...
                if (find(model_filaments.begin(), model_filaments.end(), name) != model_filaments.end())
                    return true;
            }
        }
    }
...
bool is_support_filament(int extruder_id, bool strict_check)
{
    ...
    if (!strict_check && (filament_type == "PETG" || filament_type == "PLA")) {
        ...
        if (has_filaments(model_filaments))
            return true;
    }
    if (support_option == nullptr)
        return false;
