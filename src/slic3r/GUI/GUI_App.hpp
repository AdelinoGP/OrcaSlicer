...
public:
    ...
    static std::string get_full_version() { return convert_full_version(SLIC3R_VERSION); }
...
private:
    ...
    EAppMode m_app_mode{EAppMode::Editor}; // [UNITY] Remove - Unity apps are single-purpose
    bool     m_is_recreating_gui{false};   // [UNITY] Remove - Unity scene reload handles this
#ifdef __linux__
    bool m_opengl_initialized{false}; // [UNITY] Remove - Unity manages GL context
#endif
#if defined(__WINDOWS__)
    bool m_is_arm64{false}; // [UNITY] Use Application.platform to detect ARM64
#endif
#if defined(__WINDOWS__)
...
public:
    ...
    void               push_notification(const MachineObject*  obj,
                                         wxString              msg,
                                         wxString              title = wxEmptyString,
                                         UserNotificationStyle style = UserNotificationStyle::UNS_NORMAL);
    ...
    bool                                              check_and_save_current_preset_changes(const wxString& caption,
                                                                                            const wxString& header,
                                                                                            bool            remember_choice = true,
                                                                                            bool            use_dont_save_insted_of_discard = false);
    void                                              apply_keeped_preset_modifications();
    bool                                              check_and_keep_current_preset_changes(const wxString& caption,
                                                                                            const wxString& header,
                                                                                            int             action_buttons,
    ...
    void set_instance_hash(const size_t hash)
    {
        m_instance_hash_int    = hash;
        m_instance_hash_string = std::to_string(hash);
    }
    ...
    {
        return m_opengl_mgr.get_gl_info().is_glsl_version_greater_or_equal_to(major, minor);
    }
