...
static std::map<int, std::string> error_messages = {{1, L("The device cannot handle more conversations. Please retry later.")},
...
// [INTENT] MediaPlayCtrl manages the printer live view interface, including playback state,
// network interaction for stream URLs, and interaction with the underlying video player.
// [UNITY] Replace with a dedicated MonoBehaviour controller in Unity, using VideoPlayer or a custom streaming texture bridge.
MediaPlayCtrl::MediaPlayCtrl(wxWindow* parent, wxMediaCtrl2* media_ctrl, const wxPoint& pos, const wxSize& size)
    : wxPanel(parent, wxID_ANY, pos, size), m_media_ctrl(media_ctrl)
{
    ...
    // [THREAD] Dedicated media processing thread for potentially blocking operations.
    // [PORTING_HAZARD:P2] The dedicated thread uses blocking calls that need mapping to C# async/await.
    m_thread = boost::thread([this] { media_proc(); });
    ...
    char const* device, char const* dev_ver, char const* channel, void* context, void (*callback)(void* context, char const* url))
{
    ...
    device2 += "|\"agora\"|";
    device2 += channel;
    wxGetApp().getAgent()->get_camera_url(device2, [context, callback](std::string url) { callback(context, url.c_str()); });
}
...
void MediaPlayCtrl::Play()
{
    ...
    if (m_lan_proto > MachineObject::LVL_Disable && (m_lan_mode || !m_remote_proto) && !m_disable_lan && !m_lan_ip.empty()) {
        ...
        BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl: "
                                << hide_passwd(hide_id_middle_string(url, url.find(m_lan_ip), m_lan_ip.length()), {m_lan_passwd});
        m_url = url;
        load();
    ...
    if (m_lan_proto <= MachineObject::LVL_Disable && (m_lan_mode || !m_remote_proto)) {
        Stop(m_lan_proto == MachineObject::LVL_None ? _L("A problem occurred. Please update the printer firmware and try again.") :
        ...
        agent->get_camera_url(m_machine + "|" + m_dev_ver + "|" + protocols[m_remote_proto],
                              [this, m = m_machine, v = agent_version, dv = m_dev_ver, token = std::weak_ptr(m_token)](std::string url) {
                                  ...
                                  BOOST_LOG_TRIVIAL(info)
                                      << "MediaPlayCtrl: " << hide_passwd(url, {"?uid=", "authkey=", "passwd=", "license=", "token="});
...
void MediaPlayCtrl::Stop(wxString const& msg, wxString const& msg2)
{
    ...
    if (m_last_state != MEDIASTATE_IDLE) {
        ...
        if (!msg.IsEmpty())
            ...
            if (m_failed_code < 0)
                boost::thread ping_thread = Slic3r::create_thread([] { start_ping_test(); });
#endif
            SetStatus(msg2);
        } else
    ...
    if (last_state != wxMEDIASTATE_PLAYING && m_failed_code != 0 && m_last_failed_codes.find(m_failed_code) == m_last_failed_codes.end() &&
    ...
    m_url.clear();
    ++m_failed_retry;
    bool local = tunnel == "local" || tunnel == "rtsp" || tunnel == "rtsps";
...
void MediaPlayCtrl::ToggleStream()
{
    ...
    if (!get_stream_url(&url)) {
        ...
        if (!start_stream_service(&need_install)) {
            ...
            if (res == wxID_OK) {
                ...
                {
                    MediaPlayCtrl* ctrl;
                    DownloadProgressDialog2(MediaPlayCtrl* ctrl)
                        : DownloadProgressDialog(_L("Downloading Virtual Camera Tools")), ctrl(ctrl)
                    {}
    ...
    if (!url.empty() && wxGetApp().app_config->get("not_show_vcamera_stop_prev") != "1") {
        MessageDialog dlg(this->GetParent(),
                          _L("Another virtual camera is running.\nOrca Slicer supports only a single virtual camera.\nDo you want to stop "
                             "this virtual camera?"),
                          _L("Warning"), wxYES | wxCANCEL | wxICON_INFORMATION);
        dlg.show_dsa_button();
    ...
    if (m_lan_proto > MachineObject::LVL_Disable && (m_lan_mode || !m_remote_proto) && !m_disable_lan && !m_lan_ip.empty()) {
        ...
        url += "&dev_ver=" + m_dev_ver;
        BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl::ToggleStream: "
                                << hide_passwd(hide_id_middle_string(url, url.find(m_lan_ip), m_lan_ip.length()), {m_lan_passwd});
        std::string             file_url = data_dir() + "/cameratools/url.txt";
        boost::nowide::ofstream file(file_url);
    ...
    agent->get_camera_url(m_machine + "|" + m_dev_ver + "|" + protocols[m_remote_proto],
                          [this, m = m_machine, v = agent->get_version(), dv = m_dev_ver](std::string url) {
                              ...
                              BOOST_LOG_TRIVIAL(info) << "MediaPlayCtrl::ToggleStream: "
                                                      << hide_passwd(url, {"?uid=", "authkey=", "passwd=", "license=", "token="});
                              CallAfter([this, m, url] {
                                  if (m != m_machine)
                                      return;
                                  if (url.empty() || !boost::algorithm::starts_with(url, "bambu:///")) {
                                      MessageDialog(this->GetParent(),
                                                    wxString::Format(_L("Virtual camera initialize failed (%s)!"),
                                                                     url.empty() ? _L("Network unreachable") : from_u8(url)),
...
void MediaPlayCtrl::msw_rescale() { m_button_play->Rescale(); }
...
bool MediaPlayCtrl::start_stream_service(bool* need_install)
{
    ...
    try {
        ...
        for (auto dll : {L"BambuSource.dll", L"live555.dll"}) {
            ...
            if (!boost::filesystem::exists(file_dll) ||
        ...
        boost::process::child process_source(file_source, file_url2.ToStdWstring(), boost::process::start_dir(tools_dir),
                                             boost::process::windows::create_no_window, boost::process::std_out > intermediate,
