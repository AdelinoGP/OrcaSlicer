// [ANNOTATED]
// [INTENT] Declaration of the Field widget hierarchy for configuration option binding and editing.
// [STATE] Managed UI state for various input types (text, checkboxes, sliders, color pickers).
// [STATE] UndoValueUIManager tracks the visual state of revert-to-default/system buttons.
// [UNITY] Map Field hierarchy to a set of C# components and Unity UI prefabs.
// [UNITY] Replace wxWidgets event handlers with C# delegates or UnityEvents for value-changed notifications.
// [PORTING_HAZARD:P2] Complex binding logic between UI elements and ConfigOption objects must be abstracted into a robust C# binding system.
...
class UndoValueUIManager
{
    struct UndoValueUI
    {
        // Bitmap and Tooltip text for m_Undo_btn. The wxButton will be updated only if the new wxBitmap pointer differs from the currently
        // rendered one.
        const ScalableBitmap* undo_bitmap{nullptr};
        const wxString*       undo_tooltip{nullptr};
        // Bitmap and Tooltip text for m_Undo_to_sys_btn. The wxButton will be updated only if the new wxBitmap pointer differs from the
    ...
    struct EditValueUI
    {
        // Bitmap and Tooltip text for m_Edit_btn. The wxButton will be updated only if the new wxBitmap pointer differs from the currently
        // rendered one.
...
public:
    ...
    template<class T>
    static t_field Create(wxWindow*                  parent,
                          const ConfigOptionDef&     opt,
                          const t_config_option_key& id) // interface for creating shared objects
    {
