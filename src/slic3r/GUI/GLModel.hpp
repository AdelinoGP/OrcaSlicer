...
// [INTENT] Encapsulates drawable geometry along with the CPU/GPU bridge so GUI modules can render meshes and debug overlays.
// [UNITY] Mirrors a Unity `Mesh` paired with a `MeshRenderer`/`MeshFilter`; the header shapes how Mesh data, bounds, and file metadata
// propagate in Oculus scenes.
class GLModel
{
public:
    struct Geometry
    {
        // [INTENT] Stores the raw vertex/index streams and layout metadata that describe how OpenGL should consume the data.
        // [STATE] `format` chooses vertex topology/layout while `color` represents the tint applied to this batch.
        ...
        // [STATE] Dynamic CPU buffers; length changes while the mesh is rebuilt and resets before upload.
    ...
    struct RenderData
    {
        ...
        // [OPENGL] GPU handles and draw counts; reset when `reset()` clears the buffers so re-creation occurs lazily.
        // [UNITY] Equivalent to caching a `Mesh` instance (`vertices`, `normals`, `triangles`, `uv`), while render calls mirror `Graphics.DrawMesh`.
    };
...
private:
    ...
    // [THREAD] Guards cross-thread builds (worker threads generating geometry) vs. the main GL thread running `render()`.
    // [PORTING_HAZARD:P2] Unity does not allow GL buffer uploads off the main thread; this flag keywords the need for `MainThreadDispatcher`.
    BoundingBoxf3 m_bounding_box;
    // [STATE] Tracks the last computed scene bounds for selection/highlighting.
    std::string m_filename;
    // [STATE] Source identifier shown in diagnostics/selection overlays.
...
public:
    ...
    {
        return m_render_data.vertices_count > 0 ? m_render_data.vertices_count : m_render_data.geometry.vertices_count();
    }
    size_t indices_count() const
    {
        return m_render_data.indices_count > 0 ? m_render_data.indices_count : m_render_data.geometry.indices_count();
    }
    ...
    // [INTENT] Rebuild the CPU geometry from diverse sources before scheduling GPU submission in `send_to_gpu()`.
    // [EVENT] Called by gizmo/tools when the mesh contents change (selection updates, slicing preview rebuilds).
    void init_from(Geometry&& data);
    void init_from(const TriangleMesh& mesh);
    void init_from(const indexed_triangle_set& its);
    void init_from(const Polygons& polygons, float z);
    bool init_from_file(const std::string& filename);
    // [THREAD] File-based initialization and mesh parsing may happen on worker threads; use `disable_render()` to stall the GL draw pass.
    ...
    void reset();
    // [THREAD] `reset()` drops GPU handles/CPU caches and must be called from the GL context to avoid undefined behavior.
    ...
    // [OPENGL] `render()` ensures buffers reach the GPU before drawing; `render_instanced` mimics `Graphics.DrawMeshInstanced` by feeding
    // instance data handles. [UNITY] Use a `Graphics.DrawMeshInstanced` equivalent passing cached `Mesh` and instance transforms on the
    // main thread. [PORTING_HAZARD:P3] Legacy GL draw calls assume implicit VAO/VBO binding order; Unity needs explicit `CommandBuffer`
    // submissions or `GL` plugins to reproduce UI overlays.
...
private:
    // [OPENGL] Uploads the CPU buffers to GL; returns true if the buffers exist+valid so the next render can bind them.
    // [THREAD] Must be invoked on the active GL context; do not call from background jobs without marshalling.
    bool send_to_gpu();
};
...
// [INTENT] Utility factory for GUI helpers so overlays reuse consistent vertex layouts.
// [UNITY] These mesh generators map to `Mesh` construction code and can feed Unity `MeshTopology.Triangles`.
...
// [UNITY] Use `Vector3[]` + `Mesh.SetVertices` + `Mesh.SetIndices` to replicate this shape in Unity.
