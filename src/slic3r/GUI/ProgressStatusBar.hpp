...
/**
 * @brief [INTENT] The ProgressStatusBar class is the widgets occupying the lower area
 ...
 * [UNITY] Map to Unity UI Toolkit VisualElement with a ProgressBar and Button,
 * or a custom MonoBehaviour that manages a UI Slider/Button pair.
 */

class ProgressStatusBar : public ProgressIndicator
{
    // [STATE] wxWidgets status bar that this class wraps (legacy Perl compatibility)
    wxStatusBar* self; // we cheat! It should be the base class but: perl!
    // [STATE] Progress bar gauge widget
    wxGauge* m_prog;
    // [STATE] Cancel button for user interruption
    wxButton* m_cancelbutton;
    // [STATE] Timer for pulse animation and busy indication
...
public:
    ...
    // [EVENT] Cancel callback for when user clicks cancel button
...
private:
    // [STATE] Flag indicating if the progress bar is in busy/pulse mode
    bool m_busy = false;
    // [STATE] Stored cancel callback function
