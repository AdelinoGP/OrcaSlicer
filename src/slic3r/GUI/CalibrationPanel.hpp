...
/*
[INTENT]
Declaration boundary for the calibration dashboard and printer selection popup.

[STATE]
- MObjectPanel: Small renderer for a single machine card.
- SelectMObjectPopup: Floating popup managing the list of available machines.
- CalibrationPanel: Root panel for the calibration tab.

[UNITY]
- CalibrationPanel -> MonoBehaviour or UI Toolkit Panel.
- MObjectPanel -> UI Toolkit Template/VisualElement.
- SelectMObjectPopup -> Anchored UI Toolkit Popup or separate Canvas.
*/
...
{
public:
    CalibrationPanel(wxWindow*      parent,
                     wxWindowID     id    = wxID_ANY,
                     const wxPoint& pos   = wxDefaultPosition,
                     const wxSize&  size  = wxDefaultSize,
                     long           style = wxTAB_TRAVERSAL);
