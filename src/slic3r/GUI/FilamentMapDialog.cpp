// [ANNOTATED]
// [INTENT] Multi-mode dialog for assigning sliced filaments to physical toolheads (left/right).
// [STATE] Managed UI state for switching between Auto (optimized), Custom (manual), and Global default mapping views.
// [EVENT] Triggers slicing workflow validation and updates plater/plate filament maps upon confirmation.
// [UNITY] Map to a Unity UI Toolkit ModalWindow with a tabbed or step-based content area.
// [UNITY] Use a C# SlicingWorkflowService to manage the pre-slice validation and dialog triggers.
// [PORTING_HAZARD:P2] Critical integration point between UI selection and the slicing engine's extruder mapping configuration.
...
static FilamentMapMode get_applied_map_mode(DynamicConfig&   proj_config,
                                            const Plater*    plater_ref,
                                            const PartPlate* partplate_ref,
                                            const bool       sync_plate)
{
    if (sync_plate)
...
static std::vector<int> get_applied_map(DynamicConfig&   proj_config,
                                        const Plater*    plater_ref,
                                        const PartPlate* partplate_ref,
                                        const bool       sync_plate)
{
...
bool try_pop_up_before_slice(bool is_slice_all, Plater* plater_ref, PartPlate* partplate_ref, bool force_pop_up)
{
    ...
    FilamentMapDialog map_dlg(plater_ref, filament_colors, filament_types, applied_maps, filament_lists, applied_mode,
                              plater_ref->get_machine_sync_status(), false, false);
    ...
    , m_filament_map(filament_map)
{
    ...
    if (show_default)
        mode_sizer->Add(m_default_btn, 1, wxALIGN_CENTER | wxLEFT | wxRIGHT, button_padding);
    mode_sizer->AddStretchSpacer();
    ...
    if (show_default)
        panel_sizer->Add(m_default_map_panel, 0, wxALIGN_CENTER | wxEXPAND);
    ...
    if (show_default)
        m_default_btn->Bind(wxEVT_BUTTON, &FilamentMapDialog::on_switch_mode, this);
...
void FilamentMapDialog::on_checkbox(wxCommandEvent& event)
{
    ...
    if (is_checked) {
        MessageDialog dialog(nullptr, _L("No further pop-up will appear. You can reopen it in 'Preferences'"), _L("Tips"),
