...
// [ANNOTATED]
// [INTENT] Declarations for a suite of dialogs managing printer and filament presets.
// [STATE] Classes maintain transient preset state, vendor/model metadata, and UI component mappings.
// [UNITY] Map DPIDialog subclasses to Unity UI Toolkit ModalWindow or VisualElement-based wizard controllers.
// [PORTING_HAZARD:P2] High coupling between UI layer and Preset implementation (libslic3r/Preset.hpp).
// [PORTING_HAZARD:P2] Mixed use of raw Preset* and std::shared_ptr<Preset> requires careful lifetime management in C#.
...
protected:
    enum FilamentOptionType { VENDOR = 0, TYPE, SERIAL, FILAMENT_PRESET, PRESET_FOR_PRINTER, FILAMENT_NAME_COUNT };
...
private:
    ...
    wxBoxSizer*               create_radio_item(wxString                                     title,
                                                wxWindow*                                    parent,
                                                wxString                                     tooltip,
    ...
    template<typename T> void sort_printer_by_nozzle(std::vector<std::pair<std::string, T>>& printer_name_to_filament_preset);
...
enum SuccessType { PRINTER = 0, FILAMENT };
...
private:
    ...
    std::vector<std::pair<::CheckBox*, std::string>>
        m_printer_name; // for filament and peocess preset export, collaborate with m_filament_name_to_presets
    std::unordered_map<std::string, Preset*> m_printer_presets; // first: printer name, second: printer presets have same printer name
    std::unordered_map<std::string, std::vector<Preset*>>
...
private:
    ...
    std::unordered_map<std::string, std::vector<std::shared_ptr<Preset>>>
        m_printer_compatible_filament_presets; // need be used when add presets
};
...
private:
    ...
    wxPanel* get_child_item(
        wxPanel* parent, std::shared_ptr<Preset> preset, std::string printer_name, int preset_index, bool is_last = false);
