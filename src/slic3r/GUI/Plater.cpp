...
// [THREAD] This method checks worker thread status to avoid race conditions.
// [PORTING_HAZARD:P2] Manages background worker threads and state. Unity should use Job System or ThreadPool.
// Restart background processing thread based on a bitmask of UpdateBackgroundProcessReturnState.
...
bool Plater::priv::restart_background_process(unsigned int state)
{
    ...
    if (!this->background_process.empty() && (state & priv::UPDATE_BACKGROUND_PROCESS_INVALID) == 0 &&
        ...
        // [THREAD] Starts the background process, which likely runs in a separate thread.
        // [UNITY] Ensure background work is off the main thread; use Unity Job System or async/await.
...
bool Plater::priv::replace_volume_with_stl(int object_idx, int volume_idx, const fs::path& new_path, const std::string& snapshot)
{
    ...
    try {
        ...
        if (is_step) {
            ...
            auto callback = [&is_user_cancel, linear, angle, split_compound](Slic3r::Step& file, double& linear_value, double& angle_value,
                                                                             bool& is_split) -> int {
                if (wxGetApp().app_config->get_bool("enable_step_mesh_setting")) {
                    StepMeshDialog mesh_dlg(nullptr, file, linear, angle);
                    if (mesh_dlg.ShowModal() == wxID_OK) {
            ...
            new_model = Model::read_from_step(path, LoadStrategy::AddDefaultInstances | LoadStrategy::LoadModel, nullptr, nullptr, callback,
                                              linear, angle, split_compound);
            if (is_user_cancel)
                return false;
        } else {
...
void Plater::priv::set_current_panel(wxPanel* panel, bool no_slice)
{
    ...
    auto do_reslice = [this, no_slice]() {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(
                                       ": from set_current_panel, no_slice %1%, export_in_progress %2%, model_fits %3%, m_is_slicing %4%") %
    ...
    if (evt.cancelled()) {
        ...
        if (exporting_status == ExportingStatus::EXPORTING_TO_REMOVABLE && !has_error) {
            ...
            notification_manager
                ->push_exporting_finished_notification(last_output_path, last_output_dir_path,
...
int Plater::priv::update_print_required_data(Slic3r::DynamicPrintConfig config,
                                             Slic3r::Model              model,
                                             Slic3r::PlateDataPtrs      plate_data_list,
                                             std::string                file_name,
                                             std::string                file_path)
...
void Plater::priv::apply_color_mode()
{
    ...
    wxColour   orca_color = wxColour(59, 68,
...
void Plater::priv::take_snapshot(const std::string& snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    ((this->printer_technology == ptFFF) ? m_last_fff_printer_profile_name :
...
void Plater::calib_pa(const Calib_Params& params)
{
    ...
    switch (params.mode) {
    case CalibMode::Calib_PA_Line: add_model(false, Slic3r::resources_dir() + "/calib/pressure_advance/pressure_advance_test.drc"); break;
...
void Plater::_calib_pa_pattern_gen_gcode()
{
    ...
    for (auto obj : cur_plate->get_objects_on_this_plate()) {
        auto gcode = model().calib_pa_pattern->generate_custom_gcodes(preset_bundle->full_config(), preset_bundle->is_bbl_vendor(), *obj,
...
void Plater::calib_input_shaping_freq(const Calib_Params& params)
{
    ...
    add_model(false, Slic3r::resources_dir() +
                         (params.test_model < 1 ? "/calib/input_shaping/ringing_tower.drc" : "/calib/input_shaping/fast_tower_test.drc"));
    auto       print_config        = &wxGetApp().preset_bundle->prints.get_edited_preset().config;
    auto       filament_config     = &wxGetApp().preset_bundle->filaments.get_edited_preset().config;
    auto       printer_config      = &wxGetApp().preset_bundle->printers.get_edited_preset().config;
...
void Plater::Calib_Cornering(const Calib_Params& params)
{
    ...
    const std::string cornering_model_path = params.test_model == 0 ? "/calib/input_shaping/ringing_tower.drc" :
...
void Plater::update_obj_preview_thumbnail(
    ModelObject* mo, int obj_idx, int vol_idx, std::vector<Slic3r::ColorRGBA> colors, int camera_view_angle_type)
{
    ...
    try {
        ...
        if (dlg.ShowModal() == wxID_OK) {
            ...
            if (non_project_paths.size() + project_paths.size() != selected_paths.size())
                BOOST_LOG_TRIVIAL(error) << "Decompresing of archive did not retrieve all files. Expected files: " << selected_paths.size()
...
void Plater::update(bool conside_update_flag, bool force_background_processing_update)
{
    ...
    unsigned int flag = force_background_processing_update ? (unsigned int) Plater::priv::UpdateParams::FORCE_BACKGROUND_PROCESSING_UPDATE :
                                                             0;
...
static long GetNumberFromUser(
    const wxString& msg, const wxString& prompt, const wxString& title, long value, long min, long max, wxWindow* parent)
{
#ifdef _WIN32
...
void Plater::set_number_of_copies(/*size_t num*/)
{
    ...
    const int num = GetNumberFromUser(" ", _L("Number of copies:"), _L("Copies of the selected object"), model_object->instances.size(), 0,
                                      1000, this);
    if (num < 0)
        return;
...
void Plater::export_gcode(bool prefer_removable)
{
    ...
    fs::path output_path;
    {
        ...
        wxFileDialog dlg(this, (printer_technology() == ptFFF) ? _L("Save G-code file as:") : _L("Save SLA file as:"), start_dir,
                         ...
                         GUI::file_wildcards((printer_technology() == ptFFF) ? FT_GCODE : FT_SL1, ext), wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
        if (dlg.ShowModal() == wxID_OK) {
    ...
    if (!(strategy & SaveStrategy::Backup)) {
        for (int i = 0; i < p->partplate_list.get_plate_count(); i++) {
            ...
            if (p->partplate_list.get_plate(i)->thumbnail_data.is_valid() && using_exported_file()) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": non need to re-generate thumbnail for gcode/exported mode of plate %1%") % i;
            } else {
            ...
            if (p->partplate_list.get_plate(i)->no_light_thumbnail_data.is_valid() && using_exported_file()) {
                ...
                p->generate_thumbnail(p->partplate_list.get_plate(i)->no_light_thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                      THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho, Camera::ViewAngleType::Iso, false,
                                      true);
            }
            ...
            if (top_thumbnail->is_valid() && using_exported_file()) {
                // no need to generate thumbnail
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": non need to re-generate top_thumbnail for gcode/exported mode of plate %1%") %
                                               i;
            } else {
                ...
                p->generate_thumbnail(p->partplate_list.get_plate(i)->top_thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                      THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho, Camera::ViewAngleType::Top_Plate,
                                      false);
            }
            ...
            if (picking_thumbnail->is_valid() && using_exported_file()) {
                // no need to generate thumbnail
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(": non need to re-generate pick_thumbnail for gcode/exported mode of plate %1%") %
                                               i;
            } else {
...
void Plater::send_gcode_legacy(int plate_idx, Export3mfProgressFn proFn, bool use_3mf)
{
    ...
    if (std::string(upload_job.printhost->get_name()) == "PrusaConnect" &&
        ...
        GUI::MessageDialog dlg(nullptr, _L("Is the printer ready? Is the print sheet in place, empty and clean?"), _L("Upload and Print"),
                               wxOK | wxCANCEL);
        if (dlg.ShowModal() != wxID_OK)
            return;
    }
...
int Plater::update_print_required_data(Slic3r::DynamicPrintConfig config,
                                       Slic3r::Model              model,
                                       Slic3r::PlateDataPtrs      plate_data_list,
                                       std::string                file_name,
                                       std::string                file_path)
...
void Plater::update_flush_volume_matrix(size_t old_nozzle_size, size_t new_nozzle_size)
{
    ...
    if (old_nozzle_size < new_nozzle_size) {
        ...
        if (first_flush_volume_mtx.size() == filament_nums * filament_nums * new_nozzle_size) { // load file
            set_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, first_flush_volume_mtx, -1,
                                     new_nozzle_size);
        } else {
...
{
    p->set_bed_shape(make_counter_clockwise(shape), exclude_area, wrapping_exclude_area, printable_height, extruder_areas, extruder_heights,
                     custom_texture, custom_model, force_as_custom);
}
...
void Plater::pop_warning_and_go_to_device_page(wxString printer_name, PrinterWarningType type, const wxString& title)
{
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        ...
        content = wxString::Format(_L("The currently connected printer on the device page is not %s. Please switch to %s before syncing."),
        ...
        content = _L("The filaments on the printer are all unknown types. Please go to the printer screen or software device page to set "
                     "the filament type.");
    }
...
void Plater::validate_current_plate(bool& model_fits, bool& validate_error)
{
    ...
    if (p->printer_technology == ptFFF) {
        ...
        if (!left_unprintable_text.empty()) {
            p->notification_manager->bbl_show_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError,
        ...
        if (!right_unprintable_text.empty()) {
            p->notification_manager->bbl_show_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError,
