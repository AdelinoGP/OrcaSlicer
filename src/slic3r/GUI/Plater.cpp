// [INTENT] Implementation of the Plater class, the central 3D workspace and sidebar controller.
// [UNITY] Map to SceneController (3D workspace) and InspectorPanel (Sidebar).
#include "Plater.hpp"
...
static std::map<BedType, std::string> bed_type_thumbnails = {{BedType::btPC, "bed_cool"},
...
private:
    // [STATE] Vector of labels for info display. [UNITY] List<SlicingInfoEntry> in Unity.
...
// [INTENT] UI representation of an extruder/nozzle settings group, including AMS controls.
// [UNITY] Map to ExtruderPanel MonoBehaviour/Prefab.
...
void Sidebar::priv::jump_to_object(ObjectDataViewModelNode* item) { m_object_list->selected_object(item); }
...
std::vector<int> get_min_flush_volumes(const DynamicPrintConfig& full_config, size_t nozzle_id)
{
    ...
    if (enable_long_retraction_when_cut_opt) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": get enable_long_retraction_when_cut from config, value=%1%") % machine_enabled_level;
    }
...
static bool has_junction_deviation(const DynamicPrintConfig* printer_config)
{
    ...
    return gcode_flavor && gcode_flavor->value == GCodeFlavor::gcfMarlinFirmware && junction_dev && !junction_dev->values.empty() &&
...
public:
    AMSCountPopupWindow(ExtruderGroup* extruder, int index) : PopupWindow(extruder, wxBORDER_NONE | wxPU_CONTAINS_CONTROLS)
    {
        ...
        auto img4 = new ScalableButton(box, wxID_ANY, "ams_4_tray", {}, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER,
                                       false, 44);
        // img4->SetBackgroundColour(*wxWHITE);
        auto img1 = new ScalableButton(box, wxID_ANY, "ams_1_tray", {}, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER,
                                       false, 44);
...
ExtruderGroup::ExtruderGroup(wxWindow* parent, int index, wxString const& title) : StaticGroup(parent, wxID_ANY, title)
{
...
{
    if (ams_4.empty() && ams4.empty() && ams_1.empty() && ams1.empty())
        return;
    auto sync = [obj](std::vector<AMSinfo>& infos, std::vector<DevAms*> const& ams) -> bool {
        std::vector<AMSinfo> infos2;
        for (auto a : ams) {
            AMSinfo ams_info;
            ams_info.parse_ams_info(const_cast<MachineObject*>(obj), a, obj->GetFilaSystem()->IsDetectRemainEnabled(),
                                    obj->is_support_ams_humidity);
...
bool Sidebar::priv::sync_extruder_list(bool& only_external_material)
{
    ...
    //     plater->pop_warning_and_go_to_device_page(printer_name, Plater::PrinterWarningType::INCONSISTENT, _L("Sync printer
    ...
    if (machine_print_name != target_model_id) {
        MessageDialog dlg(this->plater,
                          _L("The currently selected machine preset is inconsistent with the connected printer type.\n"
                             "Are you sure to continue syncing?"),
                          _L("Sync printer information"), wxICON_WARNING | wxYES | wxNO);
    ...
    for (size_t index = 0; index < extruder_nums; ++index) {
        ...
        if (obj->is_nozzle_flow_type_supported()) {
            if (obj->GetExtderSystem()->GetNozzleFlowType(index) == NozzleFlowType::NONE_FLOWTYPE) {
                MessageDialog dlg(this->plater,
                                  _L("There are unset nozzle types. Please set the nozzle types of all extruders before synchronizing."),
...
void Sidebar::priv::update_sync_status(const MachineObject* obj)
{
    ...
    struct ExtruderInfo
    {
        ...
        {
            return abs(diameter - other.diameter) < EPSILON && /*nozzle_volue_type == other.nozzle_volue_type
                                                            &&*/ ams_4 == other.ams_4 && ams_1 == other.ams_1;
        }
    };
    ...
    std::vector<int>          nozzle_volume_types =
        wxGetApp().preset_bundle->project_config.option<ConfigOptionEnumsGeneric>("nozzle_volume_type")->values;
    ...
    for (auto& item : obj->GetFilaSystem()->GetAmsList()) {
        ...
        if (item.second->GetAmsType() == DevAms::N3S) { // N3S
...
Sidebar::Sidebar(Plater* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        h_sizer_title->Add(p->m_printer_connect, 0, wxALIGN_CENTER | wxRIGHT,
                           FromDIP(SidebarProps::WideSpacing())); // used larger margin to prevent accidental clicks
        h_sizer_title->Add(p->m_printer_bbl_sync, 0, wxALIGN_CENTER | wxRIGHT,
                           FromDIP(SidebarProps::WideSpacing())); // used larger margin to prevent accidental clicks
        ...
        p->panel_printer_preset->Bind(wxEVT_SIZE, [this](auto& e) {
            ...
            if (((min_width < narrow_width && min_width > current_width) || (current_width < narrow_width && min_width > narrow_width)) &&
                p->image_printer->IsShown())
        ...
        p->image_printer = new wxStaticBitmap(p->panel_printer_preset, wxID_ANY, bitmap_printer.bmp(), wxDefaultPosition,
                                              FromDIP(PRINTER_THUMBNAIL_SIZE), 0);
        ...
        */
        ...
        for (wxWindow* w :
             std::initializer_list<wxWindow*>{p->panel_printer_preset, p->btn_edit_printer, p->image_printer, p->combo_printer}) {
        ...
        p->panel_nozzle_dia->Bind(wxEVT_LEFT_DOWN, [this](auto& evt) { p->combo_nozzle_dia->wxEvtHandler::ProcessEvent(evt); });
        ...
        p->combo_nozzle_dia = new ComboBox(p->panel_nozzle_dia, wxID_ANY, wxString(""), wxDefaultPosition, wxDefaultSize, 0, nullptr,
                                           wxCB_READONLY);
        ...
        for (wxWindow* w :
             std::initializer_list<wxWindow*>{p->panel_nozzle_dia, p->label_nozzle_title, p->label_nozzle_type, p->combo_nozzle_dia}) {
            w->Bind(wxEVT_ENTER_WINDOW, [this, panel_color](wxMouseEvent& e) {
        ...
        p->combo_printer_bed = new ComboBox(p->panel_printer_bed, wxID_ANY, wxString(""), wxDefaultPosition, wxDefaultSize, 0, nullptr,
                                            wxCB_READONLY);
...

    {
        ...
        p->filament_area_height = std::ceil(filament_count_user * 0.5) * (30 + SidebarProps::ElementSpacing()) -
        ...
        scrolled_sizer->Add(
            p->m_panel_filament_content, 0, wxEXPAND | wxTOP | wxBOTTOM,
            FromDIP(SidebarProps::ContentMarginV())); // ORCA use vertical margin on parent otherwise it shows scrollbar even on 1 filament
    }

    {
        ...
        p->m_search_item = new TextInput(p->m_search_bar, wxEmptyString, wxEmptyString, "", wxDefaultPosition, wxDefaultSize,
                                         0 | wxBORDER_NONE);
        ...
        search_sizer->Add(new wxWindow(p->m_search_bar, wxID_ANY, wxDefaultPosition, wxSize(0, 0)), 0, wxEXPAND | wxLEFT | wxRIGHT,
                          FromDIP(1));
...
void Sidebar::on_enter_image_printer_bed(wxMouseEvent& evt)
{
    ...
    p->big_bed_image_popup->set_bitmap(
        create_scaled_bitmap("big_" + image_path, p->big_bed_image_popup, p->big_bed_image_popup->get_image_px()));
...
void Sidebar::init_filament_combo(PlaterPresetComboBox** combo, const int filament_idx)
{
    *combo = new PlaterPresetComboBox(p->m_panel_filament_content, Slic3r::Preset::TYPE_FILAMENT);
    ...
    combo_and_btn_sizer->Add((*combo)->clr_picker, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT,
                             FromDIP(SidebarProps::ElementSpacing()) - FromDIP(2)); // ElementSpacing - 2 (from combo box))
    ...
    */
    ...
    combo_and_btn_sizer->Add(edit_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT,
                             FromDIP(SidebarProps::ElementSpacing()) - FromDIP(2)); // ElementSpacing - 2 (from combo box))
    ...
    if (side == 1 && filament_idx > 1)
        sizer_filaments->Remove(filament_idx / 2);
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_SLA_PRINT:; // p->combo_sla_print->update();
        break;

    case Preset::TYPE_SLA_MATERIAL:; // p->combo_sla_material->update();
        break;
    ...
    case Preset::TYPE_PRINTER: {
        ...
        if (auto printer_structure_opt = printer_preset.config.option<ConfigOptionEnum<PrinterStructure>>("printer_structure")) {
            wxGetApp().plater()->get_current_canvas3D()->get_arrange_settings().align_to_y_axis = (printer_structure_opt->value ==
                                                                                                   PrinterStructure::psI3);
        } else
...
bool Sidebar::reset_bed_type_combox_choices(bool is_sidebar_init)
{
    ...
    if (pm && bed_type_def && bed_type_def->enum_keys_map) {
        ...
        for (auto item : bed_type_def->enum_labels) {
            index++;
            bool find = std::find(pm->not_support_bed_types.begin(), pm->not_support_bed_types.end(), item) !=
...
void Sidebar::msw_rescale()
{
    ...
    auto image_path = get_cur_select_bed_image();
    p->image_printer_bed->SetBitmap(create_scaled_bitmap(image_path, this, PRINTER_THUMBNAIL_SIZE.GetHeight()));
    if (p->big_bed_image_popup) { // ORCA force rebuild frame. current wxwidget version not supports wxBITMAP_SCALE_FILL flag on
                                  // wxStaticBitmap also     wxImage scaledImage = bit_map.ConvertToImage();
                                  // scaledImage.Rescale(FromDIP(m_image_px), FromDIP(m_image_px), wxIMAGE_QUALITY_HIGH); didnt worked as
                                  // expected and it requires use on set_bitmap. so that will try to scale everytime
...
void Sidebar::search() { p->searcher.search(); }
...
void Sidebar::change_filament(size_t from_id, size_t to_id) { delete_filament(from_id, int(to_id)); }
...
bool Sidebar::is_new_project_in_gcode3mf()
{
    if (p->plater->using_exported_file()) {
        ...
        MessageDialog dlg(wxGetApp().plater(), text, _L("Warning"), wxOK | wxICON_WARNING);
        dlg.ShowModal();
...
std::map<int, DynamicPrintConfig> Sidebar::build_filament_ams_list(MachineObject* obj)
{
    ...
    auto build_tray_config = [](DevAmsTray const& tray, std::string const& name, std::string ams_id, std::string slot_id) {
        ...
        tray_config.set_key_value("filament_colour",
                                  new ConfigOptionStrings{into_u8(wxColour("#" + tray.color).GetAsString(wxC2S_HTML_SYNTAX))});
        ...
        for (int i = 0; i < tray.cols.size(); ++i) {
            tray_config.opt<ConfigOptionStrings>("filament_multi_colour")
                ->values.push_back(into_u8(wxColour("#" + tray.cols[i]).GetAsString(wxC2S_HTML_SYNTAX)));
        }
        return tray_config;
    };
    ...
    for (auto ams : list) {
        ...
        for (auto tray : ams.second->GetTrays()) {
            ...
            filament_ams_list.emplace(extruder + (ams_id * 4 + slot_id), build_tray_config(*tray.second, get_ams_name(ams_id, slot_id),
                                                                                           std::to_string(ams_id), std::to_string(slot_id)));
        }
    }
    return filament_ams_list;
}
...
void Sidebar::update_sync_status(const MachineObject* obj) { p->update_sync_status(obj); }

int Sidebar::get_sidebar_pos_right_x() { return this->GetScreenPosition().x + this->GetSize().x; }
...
{
    if (m_sna_dialog) {
        m_sna_dialog->on_full_screen(e);
    }
    if (m_fna_dialog) {
        m_fna_dialog->on_full_screen(e);
    }
}
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (n == 0) {
        MessageDialog dlg(this, _L("There are no compatible filaments, and sync is not performed.") + detail, _L("Sync filaments with AMS"),
                          wxOK);
        dlg.ShowModal();
        return;
    }
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
                          _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update Orca Slicer or "
                             "restart Orca Slicer to check if there is an update to system presets.") +
                              detail,
    ...
    auto badge_combox_filament = [sync_color_only](PlaterPresetComboBox* c) {
        auto tip = sync_color_only ?
                       _L("Only filament color information has been synchronized from printer.") :
    ...
    if (!merge_info.is_empty() &&
        wxGetApp().app_config->get_bool("enable_merge_color_by_sync_ams")) { // merge same color and preset filament//use same ams
...
void Sidebar::show_SEMM_buttons(bool bshow)
{
    ...
    if (p->m_bpButton_del_filament && p->combos_filament.size() > 1) // ORCA add filament count as condition to prevent showing Flushing
                                                                     // volumes and Del Filament icon visible while only 1 filament exist
        p->m_bpButton_del_filament->Show(bshow);
    if (p->m_flushing_volume_btn && p->combos_filament.size() > 1) // ORCA add filament count as condition to prevent showing Flushing
                                                                   // volumes and Del Filament icon visible while only 1 filament exist
...
wxPanel* Sidebar::scrolled_panel() { return p->scrolled; }

wxPanel* Sidebar::print_panel() { return p->m_panel_print_content; }

wxPanel* Sidebar::filament_panel() { return p->m_panel_filament_content; }
...
void Sidebar::pop_sync_nozzle_and_ams_dialog()
{
    ...
    wxTheApp->CallAfter([this]() {
        ...
        temp_na_info.dialog_pos = big_btn_pt + wxPoint(big_btn_size.x, big_btn_size.y) +
                                  wxPoint(FromDIP(big_btn_size.x / 10.f - 5), FromDIP(big_btn_size.y / 10.f));
...
void Sidebar::update_searcher() { p->searcher.init(get_search_inputs(m_mode)); }
...
Search::OptionsSearcher& Sidebar::get_searcher() { return p->searcher; }

std::string& Sidebar::get_search_line() { return p->searcher.search_string(); }
...
void Sidebar::auto_calc_flushing_volumes_internal(const int modify_id, const int extruder_id)
{
    ...
    std::vector<double> init_matrix = get_flush_volumes_matrix((project_config.option<ConfigOptionFloats>("flush_volumes_matrix"))->values,
                                                               extruder_id, extruder_nums);
                ...
                if (is_to_support) {
                    ...
                    for (int j = 0; j < multi_colours[from_idx].size(); ++j) {
                        ...
                        for (int k = 0; k < multi_colours[modify_id].size(); ++k) {
                            ...
                            int volume         = calculator.calc_flush_vol(from.Alpha(), from.Red(), from.Green(), from.Blue(), to.Alpha(),
    ...
    set_flush_volumes_matrix((project_config.option<ConfigOptionFloats>("flush_volumes_matrix"))->values, matrix, extruder_id,
                             extruder_nums);
}
...
struct Plater::priv
{
    ...
    int save_project_if_dirty(const wxString& reason)
    {
        ...
        if (dirty_state.is_dirty()) {
            ...
            if (mainframe->can_save_as()) {
                ...
                res = MessageDialog(mainframe,
                                    reason + "\n" + format_wxstr(_L("Do you want to save changes to \"%1%\"?"), suggested_project_name),
                                    wxString(SLIC3R_APP_FULL_NAME), wxYES_NO | wxCANCEL)
                          .ShowModal();
                if (res == wxID_YES)
    ...
    {
        assert(m_undo_redo_stack_active != nullptr);
        return *m_undo_redo_stack_active;
    }
    ...
    // Caching last value of show_action_buttons parameter for show_action_buttons(), so that a callback which does not know this state will
    // not override it.
    // mutable bool    			ready_to_slice = { false };
    // Flag indicating that the G-code export targets a removable device, therefore the show_action_buttons() needs to be called at any case
    // when the background processing finishes.
...
Plater::priv::priv(Plater* q, MainFrame* main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {"printable_area", "bed_exclude_area", "wrapping_exclude_area", "extruder_printable_area", "bed_custom_texture",
           "bed_custom_model", "print_sequence", "extruder_clearance_radius", "extruder_clearance_height_to_lid",
           "extruder_clearance_height_to_rod", "nozzle_height", "skirt_type", "skirt_loops", "skirt_speed", "min_skirt_length",
           "skirt_distance", "skirt_start_angle", "brim_width", "brim_object_gap", "brim_use_efc_outline", "brim_type", "nozzle_diameter",
           "single_extruder_multi_material", "preferred_orientation", "enable_prime_tower", "wipe_tower_x", "wipe_tower_y",
           "prime_tower_width", "prime_tower_brim_width", "prime_tower_skip_points", "prime_tower_enable_framework",
           "prime_tower_infill_gap", "prime_volume", "extruder_colour", "filament_colour", "filament_type", "material_colour",
           "printable_height", "extruder_printable_height", "printer_model", "printer_technology",
           // These values are necessary to construct SlicingParameters by the Canvas3D variable layer height editor.
           "layer_height", "initial_layer_print_height", "min_layer_height", "max_layer_height", "brim_width", "wall_loops",
           ...
           "wipe_tower_cone_angle", "wipe_tower_extra_spacing", "wipe_tower_extra_flow", "wipe_tower_max_purge_speed",
           "wipe_tower_wall_type", "wipe_tower_extra_rib_length", "wipe_tower_rib_width", "wipe_tower_fillet_wall", "wipe_tower_filament",
    ...
    , partplate_list(this->q, &model)
{
    ...
    Camera::EType::Ortho); }); background_process.set_slicing_completed_event(EVT_SLICING_COMPLETED);
    ...
    if (wxGetApp().is_editor()) {
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_INCREASE_INSTANCES, [this](Event<int>& evt) {
            if (evt.data == 1)
                this->q->increase_instances();
            else if (this->can_decrease_instances())
                this->q->decrease_instances();
        });
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_COLLAPSE_SIDEBAR,
                            [this](SimpleEvent&) { this->q->collapse_sidebar(!this->q->is_sidebar_collapsed()); });
        view3D_canvas->Bind(EVT_GLCANVAS_RESET_LAYER_HEIGHT_PROFILE,
                            [this](SimpleEvent&) { this->view3D->get_canvas3d()->reset_layer_height_profile(); });
        view3D_canvas->Bind(EVT_GLCANVAS_ADAPTIVE_LAYER_HEIGHT_PROFILE,
                            [this](Event<float>& evt) { this->view3D->get_canvas3d()->adaptive_layer_height_profile(evt.data); });
        view3D_canvas->Bind(EVT_GLCANVAS_SMOOTH_LAYER_HEIGHT_PROFILE,
    ...
    preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_UPDATE, [this](SimpleEvent&) { preview->get_canvas3d()->set_as_dirty(); });
    if (wxGetApp().is_editor()) {
        preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_TAB, [this](SimpleEvent&) { select_next_view_3D(); });
        preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_COLLAPSE_SIDEBAR,
                                        [this](SimpleEvent&) { this->q->collapse_sidebar(!this->q->is_sidebar_collapsed()); });
        preview->get_wxglcanvas()->Bind(EVT_CUSTOMEVT_TICKSCHANGED, [this](wxCommandEvent& event) {
            ...
            if (tick_event_type == Type::ToolChange || tick_event_type == Type::Custom || tick_event_type == Type::Template ||
                tick_event_type == Type::PausePrint) {
    ...
    this->q->Bind(EVT_HID_DEVICE_ATTACHED, [this](HIDDeviceAttachedEvent& evt) { mouse3d_controller.device_attached(evt.data); });
    this->q->Bind(EVT_HID_DEVICE_DETACHED, [this](HIDDeviceAttachedEvent& evt) { mouse3d_controller.device_detached(evt.data); });
#endif /* _WIN32 */
    ...
    if (wxGetApp().is_editor()) {
        ...
        this->q->Bind(EVT_PRESET_UPDATE_AVAILABLE_CLICKED,
                      [](PresetUpdateAvailableClickedEvent&) { wxGetApp().get_preset_updater()->on_update_notification_confirm(); });
        ...
        this->q->Bind(EVT_REMOVABLE_DRIVE_EJECTED, [this](RemovableDriveEjectEvent& evt) {
            if (evt.data.second) {
                ...
                notification_manager->push_notification(
                    ...
                    format(_L("Successfully unmounted. The device %s (%s) can now be safely removed from the computer."),
                           evt.data.first.name, evt.data.first.path));
            } else {
                notification_manager->push_notification(NotificationType::CustomNotification,
                                                        NotificationManager::NotificationLevel::ErrorNotificationLevel,
                                                        format(_L("Ejecting of device %s (%s) has failed."), evt.data.first.name,
                                                               evt.data.first.path));
            }
        });
    ...
    if (wxGetApp().is_editor()) {
        ...
        this->q->Bind(EVT_RESTORE_PROJECT, [this, last = last_backup](wxCommandEvent& e) {
            ...
            if (Slic3r::has_restore_data(last_backup, originfile)) {
                auto result = MessageDialog(this->q, _L("Previous unsaved project detected, do you want to restore it?"),
                                            wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Restore"), wxYES_NO | wxYES_DEFAULT | wxCENTRE)
                                  .ShowModal();
                if (result == wxID_YES) {
    ...
    this->q->Bind(EVT_INSTANCE_GO_TO_FRONT, [this](InstanceGoToFrontEvent&) { bring_instance_forward(); });
...
//    wxWindowUpdateLocker freeze_guard(q);
    ...
    const bool   force_background_processing_restart = this->printer_technology == ptSLA ||
                                                     (flags & (unsigned int) UpdateParams::FORCE_BACKGROUND_PROCESSING_UPDATE);
...
wxColour Plater::get_next_color_for_filament()
{
    ...
    wxColour colors[FILAMENT_SYSTEM_COLORS_NUM] = {
        ...
        wxColour("#00C1AE"), wxColour("#F4E2C1"), wxColour("#ED1C24"), wxColour("#00FF7F"), wxColour("#F26722"), wxColour("#FFEB31"),
        wxColour("#7841CE"), wxColour("#115877"), wxColour("#ED1E79"), wxColour("#2EBDEF"), wxColour("#345B2F"), wxColour("#800080"),
        wxColour("#FA8173"), wxColour("#800000"), wxColour("#F7B763"), wxColour("#A4C41E"),
    };
...
wxString Plater::get_slice_warning_string(GCodeProcessorResult::SliceWarning& warning)
{
    if (warning.msg == BED_TEMP_TOO_HIGH_THAN_FILAMENT) {
        return _L("The current hot bed temperature is relatively high. The nozzle may be clogged when printing this filament in a closed "
                  "enclosure. Please open the front door and/or remove the upper glass.");
    } else if (warning.msg == NOZZLE_HRC_CHECKER) {
        return _L("The nozzle hardness required by the filament is higher than the default nozzle hardness of the printer. Please replace "
        ...
        return _L("Smooth mode for timelapse is enabled, but the prime tower is off, which may cause print defects. Please enable the "
...
// BBS
std::shared_ptr<BBLStatusBar> Plater::priv::statusbar() { return nullptr; }

std::string Plater::priv::get_config(const std::string& key) const { return wxGetApp().app_config->get(key); }
...
std::vector<size_t> Plater::priv::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": load_model %1%, load_config %2%, input_files size %3%") % load_model % load_config %
                                   input_files.size();
    ...
    const int stage_percent[IMPORT_STAGE_MAX + 1]   = {5,  // IMPORT_STAGE_RESTORE
                                                       ...
                                                       100};
    const int step_percent[LOAD_STEP_STAGE_NUM + 1] = {5,  // LOAD_STEP_STAGE_READ_FILE
    ...
    for (size_t i = 0; i < input_files.size(); ++i) {
        ...
        // On Windows, we swap slashes to back slashes, see GH #6803 as read_from_file() does not understand slashes on Windows thus it
        // assignes full path to names of loaded objects.
        path.make_preferred();
#else  // _WIN32
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        if (load_config)
            strategy = strategy | LoadStrategy::CheckVersion;
            ...
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                            << boost::format(
                                                   ", plate_data.size %1%, project_preset.size %2%, is_bbs_3mf %3%, file_version %4% \n") %
                                                   plate_data.size() % project_presets.size() % (en_3mf_file_type == En3mfType::From_BBS) %
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa || (load_model && !load_config)) {
                        ...
                        // [INTENT] Loop through model objects to update extruder IDs and filaments.
                        for (ModelObject* o : model.objects) {
                            if (o->config.option("extruder"))
                        ...
                        while (filament_size < MAXIMUM_EXTRUDER_NUMBER && filament_size < size) {
                            ...
                            wxColour    new_col        = Plater::get_next_color_for_filament();
                            std::string new_color      = new_col.GetAsString(wxC2S_HTML_SYNTAX).ToStdString();
                            // [UNITY] PresetBundle management maps to a ScriptableObject or custom Asset-based configuration system in Unity.
                            wxGetApp().preset_bundle->set_num_filaments(filament_count, new_color);
                            wxGetApp().plater()->on_filament_count_change(filament_count);
                            ++filament_size;
                        }
                    ...
                    // [STATE] Application version checking for 3MF compatibility.
                    // BBS: version check
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_type != LoadType::LoadGeometry)
                            // [UNITY] wxWidgets MessageDialog maps to Unity UI (uGUI/UI Toolkit) or a custom Modal Dialog system.
                    ...
                    //             show_info(q, _L("The 3MF was generated by an old OrcaSlicer, loading geometry data only."), _L("Load
                    //             3MF"));
                    //         else
                    //             show_info(q, _L("The 3MF is not supported by OrcaSlicer, loading geometry data only."), _L("Load 3MF"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with OrcaSlicer 2.3.1-alpha and use the sparse infill rotation template for
                    // non-safe infill patterns
                        ...
                        if (config_substitutions.unrecogized_keys.size() > 0) {
                            wxString text = wxString::Format(
                                _L("The 3MF file version %s is newer than %s's version %s, found the following unrecognized keys:"),
                            ...
                            if (file_version.min() != app_version.min()) {
                                wxString text = wxString::Format(
                                    _L("The 3MF file version %s is newer than %s's version %s, we suggest to upgrade your software."),
                                    file_version.to_string(), std::string(SLIC3R_APP_FULL_NAME), app_version.to_string());
                                text += "\n";
                        ...
                        load_config = false;
                        show_info(q, _L("The 3MF file was generated by an old OrcaSlicer version, loading geometry data only."),
                                  _L("Load 3MF"));
                    } else if (!load_config) {
                        // [INTENT] Reset config except color. Save the extruder information before reset and restore it after.
                        for (ModelObject* model_object : model.objects) {
                            bool has_extruder = model_object->config.has("extruder");
                            int  extruder_id  = -1;
                            // save the extruder information before reset
                            if (has_extruder) {
                                extruder_id = model_object->config.extruder();
                            }
                            ...
                            if (has_extruder) {
                                model_object->config.set("extruder", extruder_id);
                            }
                            ...
                            for (ModelVolume* model_volume : model_object->volumes) {
                                ...
                                if (has_extruder) {
                                    extruder_id = model_volume->config.extruder();
                                }

                                model_volume->config.reset();

                                if (has_extruder) {
                                    model_volume->config.set("extruder", extruder_id);
                                }
                            }
                        }
                    }
                    ...
                    if (plate_data.size() > 0) {
                        ...
                        if (load_config) {
                            if (translate_old) {
                                // set the size back
                                partplate_list.reset_size(current_width + Bed3D::Axes::DefaultTipRadius,
                                                          current_depth + Bed3D::Axes::DefaultTipRadius, current_height, false);
                            }
                            project_filament_count = config_loaded.option<ConfigOptionStrings>("filament_colour")->size();
                            // [STATE] Update slice context and G-code results for the current plate.
                    ...
                    if ((project_presets.size() > 0) && load_config) {
                        ...
                        PresetBundle&              preset_bundle = *wxGetApp().preset_bundle;
                        preset_substitutions                     = preset_bundle.load_project_embedded_presets(project_presets,
                        ...
                        if (!validity.empty()) {
                            ...
                            for (std::map<std::string, std::string>::iterator it = validity.begin(); it != validity.end(); ++it)
                                BOOST_LOG_TRIVIAL(info)
                                    << __FUNCTION__ << ":" << __LINE__ << boost::format("%1%: %2%") % it->first % it->second;
                            //
                            // [UNITY] NotificationManager maps to a global Notification System in Unity.
                    ...
                    if (!config.empty()) {
...

                        {
                            ...
                            if (en_3mf_file_type == En3mfType::From_BBS && file_version < old_version3) {
                                ...
                                {
                                    ConfigOptionFloats* filament_prime_volume_option = config.option<ConfigOptionFloats>(
                                        "filament_prime_volume");
                                    ConfigOptionStrings* filament_colors_option = config.option<ConfigOptionStrings>("filament_colour",
                                                                                                                     true);
                                ...
                                ConfigOptionEnum<WipeTowerWallType>* prime_tower_rib_wall_option =
                                    config.option<ConfigOptionEnum<WipeTowerWallType>>("wipe_tower_wall_type", true);
                                prime_tower_rib_wall_option->value = WipeTowerWallType::wtwRectangle;

                                ConfigOptionPercent* prime_tower_infill_gap_option =
                                    config.option<ConfigOptionPercent>("prime_tower_infill_gap", true);
                                prime_tower_infill_gap_option->value = 100;

                                ConfigOptionInts* filament_adhesiveness_category_option =
                                    config.option<ConfigOptionInts>("filament_adhesiveness_category", true);
                                ...
                                std::vector<std::string>& diff_settings =
                        ...
                        if (choise.empty() || choise != "true") {
                            // [INTENT] Validate presets for modified G-codes, which is a critical safety check.
                            ...
                            if (validated == VALIDATE_PRESETS_MODIFIED_GCODES) {
                                ...
                                for (std::set<std::string>::iterator it = modified_gcodes.begin(); it != modified_gcodes.end(); ++it)
                                    warning_message += "-" + *it + "\n";
                                warning_message += "\n";
                                // show_info(q, _L("The 3MF has the following modified G-code in filament or printer presets:") +
                                // warning_message + _L("Please confirm that all modified G-code is safe to prevent any damage to the
                                // machine!"), _L("Modified G-code"));
                                MessageDialog
                                    dlg(q,
                                        _L("The 3MF has the following modified G-code in filament or printer presets:") + warning_message +
                                            _L("Please confirm that all modified G-code is safe to prevent any damage to the machine!"),
                                        _L("Modified G-code"));
                                dlg.show_dsa_button();
                                ...
                                for (std::set<std::string>::iterator it = modified_gcodes.begin(); it != modified_gcodes.end(); ++it)
                                    warning_message += "-" + *it + "\n";
                                warning_message += "\n";
                                // show_info(q, _L("The 3MF has the following customized filament or printer presets:") + warning_message +
                                // _L("Please confirm that the G-code within these presets is safe to prevent any damage to the machine!"),
                                // _L("Customized Preset"));
                                MessageDialog dlg(q,
                                                  _L("The 3MF has the following customized filament or printer presets:") +
                                                      from_u8(warning_message) +
                                                      _L("Please confirm that the G-code within these presets is safe to prevent any "
                        ...
                        // always load config
                        {
                            ...
                            // [INTENT] Load config model from 3MF.
                            ...
                            //    if (!names.empty()) {
                            //        std::string notif_text = into_u8(_L_PLURAL("The preset below was temporarily installed on the active
                            //        instance of PrusaSlicer",
                            //                                                   "The presets below were temporarily installed on the active
                            //                                                   instance of PrusaSlicer", names.size())) + ":";
                            ...
                            // For exporting from the amf/3mf we shouldn't check printer_presets for the containing information about "Print
                            // Host upload" BBS: add preset combo box re-active logic currently found only needs re-active here
                            // [UNITY] load_current_presets triggers UI refreshes. Unity UI (UI Toolkit/uGUI) should use a reactive
                            ...
                            // do some post process after loading config
                            {
                                ...
                                if (filament_color) {
                                    ...
                                    ConfigOptionStrings* filament_multi_color = proj_cfg.opt<ConfigOptionStrings>("filament_multi_colour",
                                                                                                                  true);
                                    ...
                                    for (size_t i = 0; i < filament_count; i++) {
                                        ...
                                        if (i >= filament_multi_color->values.size() || colors.empty() ||
                                            colors[0] != filament_color->values[i]) {
                                            filament_multi_color->values[i] = filament_color->values[i];
                                        }
                                    }
                                    // Sync filament colour type
                                    ConfigOptionStrings* filament_color_type = proj_cfg.opt<ConfigOptionStrings>("filament_colour_type",
                                                                                                                 true);
                ...
                if (boost::iends_with(path.string(), ".stp") || boost::iends_with(path.string(), ".step")) {
                    ...
                    // [INTENT] STEP file loading with progress updates and cancelation support.
                    ...
                    // [INTENT] Generic model loading from file (STL, OBJ, 3MF, etc.).
        ...
        if (load_model) {
            ...
            if ((!is_project_file) && (!load_old_project)) {
                ...
                if (int deleted_objects = model.removed_objects_with_zero_volume(); deleted_objects > 0) {
                    MessageDialog(q, _L("Objects with zero volume removed"), _L("The volume of the object is zero"),
                ...
                //        model.objects.size()), from_path(filename))
        ...
        for (ModelObject* model_object : model.objects) {
            ...
            if (!model_object->instances.empty())
                // [INTENT] Ensure model instances are positioned on the print bed.
        ...
        if (one_by_one) {
            ...
            // [INTENT] load_model_objects handles adding objects to the internal model and updating UI.
    ...
    if (load_config) {
        // [UNITY] DeviceManager maps to a Network/Hardware Interface service in Unity.
        ...
        if (dev) {
            ...
            if (obj && obj->is_info_ready()) {
                if (obj->GetExtderSystem()->GetTotalExtderCount() > 0) {
                    ...
                    if (opt)
                        preset_nozzle_diameter = static_cast<const ConfigOptionFloats*>(opt)->values[0];
                    ...
                    if (printer_preset.get_current_printer_type(preset_bundle) != machine_type ||
                        !is_approx((float) preset_nozzle_diameter, machine_nozzle_diameter)) {
                        Preset* machine_preset = get_printer_preset(obj);
                        if (machine_preset != nullptr) {
                            ...
                            if (!wxGetApp().app_config->has("sync_after_load_file_show_flag")) {
                                wxString tips = from_u8(
                                    (boost::format(_u8L("Connected printer is %s. It must match the project preset for printing.\n")) %
                                     printer_model)
                                        .str());

                                tips += _L("Do you want to sync the printer information and automatically switch the preset?");
                                // [UNITY] wxWidgets TipsDialog maps to Unity UI (uGUI/UI Toolkit) or a custom Modal Dialog system.
                                TipsDialog dlg(wxGetApp().mainframe, _L("Tips"), tips, "sync_after_load_file_show_flag", wxYES_NO);
                                if (dlg.ShowModal() == wxID_YES) {
                                    sync_printer_info = true;
                                }
                            } else {
    ...
    if (load_model && load_config) {
        if (model.objects.empty()) {
            ...
            if (first_plate && first_plate->is_slice_result_valid()) {
                ...
                // [EVENT] Tab switching logic (select_view_3D). Mapped to Unity UI Tab management.
                // select plate 0 as default
    ...
    if (tolal_model_count <= 0 && !q->m_exported_file) {
        dlg.Hide();
        if (!is_user_cancel) {
            MessageDialog msg(wxGetApp().mainframe, _L("The file does not contain any geometry data."), _L("Warning"),
                              wxYES | wxICON_WARNING);
...
// [INTENT] load_model_objects core logic for adding objects to the internal model, centering, and scaling.
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z, bool split_object)
{
    ...
    for (ModelObject* model_object : model_objects) {
        ...
        // [PORTING_HAZARD:P2] Interactive scaling dialog during load can block the main thread. Unity UI should use async dialogs.
        ...
        for (size_t i = 0; i < object->instances.size(); ++i) {
            ...
            if (max_ratio > 10000) {
                MessageDialog
                    dlg(q, _L("Your object appears to be too large, do you want to scale it down to fit the print bed automatically?"),
                        _L("Object too large"), wxICON_QUESTION | wxYES);
                int answer = dlg.ShowModal();
    ...
    for (auto& instance : new_instances) {
        ...
        if (plate_empty)
            ...
            // [INTENT] Find nearest empty cell to put the copied object for auto-placement.
    ...
    if (obj_idxs.size() > 1) {
        ...
        // [UNITY] obj_list()->add_objects_to_list maps to updating a TreeView or ScrollView in Unity UI.
...
// [INTENT] logic for proposing filenames for export based on project name or selected objects.
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    case FT_3MF:
    case FT_GCODE:
    case FT_OBJ: wildcard = file_wildcards(file_type); break;
    ...
    // [UNITY] wxWidgets wxFileDialog maps to a native file picker or Unity's EditorUtility.SaveFilePanel (though not for runtime). In
    // runtime, use a specialized file browser package for cross-platform support.
    wxFileDialog dlg(q, dlg_title,
                     is_shapes_dir(out_dir) ? from_u8(wxGetApp().app_config->get_last_dir()) : from_path(output_file.parent_path()),
                     from_path(output_file.filename()), wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT | wxPD_APP_MODAL);
...
const Selection& Plater::priv::get_selection() const { return view3D->get_canvas3d()->get_selection(); }

Selection& Plater::priv::get_selection() { return view3D->get_canvas3d()->get_selection(); }

Selection& Plater::priv::get_curr_selection() { return get_current_canvas3D()->get_selection(); }
...
// [EVENT] selection_changed handler. Propagates selection changes to the 3D viewport and tool states.
...
// [EVENT] object_list_changed handler. Updates the slice/print button status and notifies parameter panels.
void Plater::priv::object_list_changed()
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": can_slice %1%, model_fits= %2%, export_in_progress %3%, has_printable_instances %4% ") %
                                   can_slice % model_fits % export_in_progress % part_plate->has_printable_instances();
    // [UNITY] main_frame->update_slice_print_status propagates state to the top-level UI panels.
...
{
    // [UNITY] TakeSnapshot for Undo/Redo system. Runtime Unity port requires a custom Command/Memento pattern implementation.
    SingleSnapshot ss(q);
...
void Plater::priv::deselect_all() { view3D->deselect_all(); }

void Plater::priv::exit_gizmo() { view3D->exit_gizmo(); }
...
// [INTENT] Reset project state. Clears models, partplates, G-code results, and project embedded presets.
void Plater::priv::reset(bool apply_presets_change)
{
    ...
    // [UNITY] m_aui_mgr.SavePerspective() for UI layout persistence. Unity uses Layout assets or JSON/XML for runtime layout configuration.
    // Save window layout
    if (sidebar_layout.is_enabled) {
        // Reset show state
        auto& sidebar = m_aui_mgr.GetPane(this->sidebar);
...
void Plater::priv::center_selection() { view3D->center_selected(); }
...
// [INTENT] Split object into multiple parts. Clones model, performs split, and reloads objects into the scene.
...
void Plater::priv::split_volume() { wxGetApp().obj_list()->split(); }
...
// [THREAD] schedule_background_process uses a wxTimer (500ms) to debounce reslicing and UI updates. Unity port should use a similar
// debouncing logic with Coroutines or a custom Timer.
...
// [INTENT] schedule_auto_reslice_if_needed core logic for auto-slicing after a configuration or model change.
void Plater::priv::schedule_auto_reslice_if_needed()
{
    ...
    auto_reslice_pending = true;
    auto_reslice_timer.Stop();
    // [EVENT] trigger_auto_reslice_now uses wxGetApp().CallAfter to ensure main thread execution of the reslicing trigger.
...
// [INTENT] Get filament info from connected hardware via DeviceManager. Maps to a hardware abstraction layer in Unity.
...
void Plater::priv::update_print_volume_state()
{
    ...
    BuildVolume build_volume(pp->get_shape(), this->bed.build_volume().printable_height(), this->bed.build_volume().extruder_areas(),
                             this->bed.build_volume().extruder_heights());
...
void Plater::priv::process_validation_warning(StringObjectException const& warning) const
{
    if (warning.string.empty())
        ...
        auto        action_fn = (mo || !warning.opt_key.empty()) ?
                                    [id = mo ? mo->id() : 0, opt = warning.opt_key](wxEvtHandler*) {
                                 auto& objects = wxGetApp().model().objects;
                                 auto  iter = id.id ? std::find_if(objects.begin(), objects.end(), [id](auto o) { return o->id() == id; }) :
                                                             objects.end();
                                 if (iter != objects.end()) {
                                     wxGetApp().mainframe->select_tab(MainFrame::tp3DEditor);
        ...
        notification_manager->push_notification(NotificationType::ValidateWarning,
                                                NotificationManager::NotificationLevel::WarningNotificationLevel,
                                                _u8L("WARNING:") + "\n" + text, hypertext, action_fn);
    }
}

// [INTENT] update_background_process - the heart of the slicing state machine synchronization between UI and background worker. Applies new
// config to background tasks and handles invalidations. Update background processing thread from the current config and Model. Returns a
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages, bool switch_print)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(
                                   ": enter, force_validation=%1% postpone_error_messages=%2%, switch_print=%3%, was_running=%4%") %
                                   force_validation % postpone_error_messages % switch_print % was_running;
    if (switch_print) {
    ...
    background_process.fff_print()->set_check_multi_filaments_compatibility(
        wxGetApp().app_config->get("enable_high_low_temp_mixed_printing") == "false");
    ...
    if (preset_bundle->get_printer_extruder_count() > 1) {
        ...
        // [THREAD] background_process.apply is where the configuration is applied to the background slicing process.
    ...
    if ((invalidated != Print::APPLY_STATUS_UNCHANGED || force_validation) && !background_process.empty()) {
        ...
        // [INTENT] Validate the print state and show errors/warnings as notifications. Critical for UI feedback.
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(
                                   ", Line %1%: was_running = %2%, running %3%, invalidated=%4%, return_state=%5%, internal_cancel=%6%") %
                                   __LINE__ % was_running % this->background_process.running() % invalidated % return_state %
    ...
    if (was_running && !this->background_process.running() && (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0) {
        if (invalidated != Print::APPLY_STATUS_UNCHANGED || this->background_process.is_internal_cancelled()) {
            ...
            SlicingProcessCompletedEvent evt(EVT_PROCESS_COMPLETED, 0, SlicingProcessCompletedEvent::Cancelled, nullptr);
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
    ...
    if ((return_state & UPDATE_BACKGROUND_PROCESS_INVALID) != 0) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", Line %1%: set to process_completed_with_error, return_state=%2%") % __LINE__ %
                                       return_state;
    } else {
        // Background data is valid.
        if ((return_state & UPDATE_BACKGROUND_PROCESS_RESTART) != 0 || (return_state & UPDATE_BACKGROUND_PROCESS_REFRESH_SCENE) != 0)
            notification_manager->set_slicing_progress_hidden();

        // BBS: add slice&&print status update logic
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", Line %1%: background data valid, return_state=%2%") % __LINE__ % return_state;
        PartPlate* cur_plate = background_process.get_current_plate();
...
bool Plater::priv::restart_background_process(unsigned int state)
{
    ...
    if (!this->background_process.empty() && (state & priv::UPDATE_BACKGROUND_PROCESS_INVALID) == 0 &&
        (((state & UPDATE_BACKGROUND_PROCESS_FORCE_RESTART) != 0 && !this->background_process.finished()) ||
         (state & UPDATE_BACKGROUND_PROCESS_FORCE_EXPORT) != 0 || (state & UPDATE_BACKGROUND_PROCESS_RESTART) != 0)) {
    ...
    void Plater::priv::process_validation_warning(StringObjectException const& warning) const
    {
        if (warning.string.empty())
            ...
            auto        action_fn = (mo || !warning.opt_key.empty()) ?
                                     ...
                                     auto& objects = wxGetApp().model().objects;
                                     auto  iter    = id.id ?
                                                                std::find_if(objects.begin(), objects.end(), [id](auto o) { return o->id() == id; }) :
                                                                objects.end();
                                     if (iter != objects.end()) {
                                         wxGetApp().mainframe->select_tab(MainFrame::tp3DEditor);
                                         wxGetApp().obj_list()->select_items({{*iter, nullptr}});
    ...
    unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages, bool switch_print)
    {
        ...
        BOOST_LOG_TRIVIAL(info)
            << __FUNCTION__
            << boost::format(", Line %1%: was_running = %2%, running %3%, invalidated=%4%, return_state=%5%, internal_cancel=%6%") %
                   __LINE__ % was_running % this->background_process.running() % invalidated % return_state %
                   this->background_process.is_internal_cancelled();
        if (was_running && !this->background_process.running() && (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0) {
            if (invalidated != Print::APPLY_STATUS_UNCHANGED || this->background_process.is_internal_cancelled()) {
    ...
    unsigned int Plater::priv::update_restart_background_process(bool force_update_scene, bool force_update_preview)
    {
        ...
        if ((partplate_list.get_plate_count() > 1) && !this->background_process.can_switch_print()) {
            ...
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(": plate count %1%, can not switch") % partplate_list.get_plate_count();
            switch_print = false;
        }
        // bitmask of UpdateBackgroundProcessReturnState
    ...
    bool Plater::priv::replace_volume_with_stl(int object_idx, int volume_idx, const fs::path& new_path, const std::string& snapshot)
    {
        ...
        try {
            ...
            if (is_step) {
                ...
                auto callback = [&is_user_cancel, linear, angle, split_compound](Slic3r::Step& file, double& linear_value,
                                                                                 double& angle_value, bool& is_split) -> int {
                    if (wxGetApp().app_config->get_bool("enable_step_mesh_setting")) {
                        StepMeshDialog mesh_dlg(nullptr, file, linear, angle);
                        if (mesh_dlg.ShowModal() == wxID_OK) {
                ...
                new_model = Model::read_from_step(path, LoadStrategy::AddDefaultInstances | LoadStrategy::LoadModel, nullptr, nullptr,
                                                  callback, linear, angle, split_compound);
                if (is_user_cancel)
                    return false;
            } else {
        ...
        if (new_model.objects.size() > 1 || new_model.objects.front()->volumes.size() > 1) {
            MessageDialog dlg(q, _L("Unable to replace with more than one volume"), _L("Error during replace"),
        ...
        new_volume->translate(new_volume->get_transformation().get_matrix_no_offset() *
    ...
    void Plater::priv::replace_with_stl()
    {
        ...
        title += ":";
        wxFileDialog dialog(q, title, "", from_u8(input_path.filename().string()), file_wildcards(FT_MODEL),
        ...
        if (out_path.empty()) {
            MessageDialog dlg(q, _L("File for the replace wasn't selected"), _L("Error during replace"),
                              wxOK | wxOK_DEFAULT | wxICON_WARNING);
            dlg.ShowModal();
            return;
        }
    ...
    void Plater::priv::replace_all_with_stl()
    {
        ...
        fs::path out_path = dialog.GetPath().ToUTF8().data();
        if (out_path.empty()) {
            MessageDialog dlg(q, _L("Directory for the replace wasn't selected"), _L("Error during replace"),
                              wxOK | wxOK_DEFAULT | wxICON_WARNING);
            dlg.ShowModal();
            return;
        }
    ...
    void Plater::priv::reload_from_disk()
    {
        ...
        selected_volumes.erase(std::unique(selected_volumes.begin(), selected_volumes.end(),
        ...
        struct SelectedVolume
        {
            ...
            bool operator<(const SelectedVolume& other) const
            {
                return object_idx < other.object_idx || (object_idx == other.object_idx && volume_idx < other.volume_idx);
            }
            ...
            if (!volume->source.input_file.empty()) {
                if (fs::exists(volume->source.input_file))
                    ...
                    if (!found)
                       ...
                       !volume->source.is_from_builtin_objects)
...
#endif // __APPLE__
            title += ":";
            wxFileDialog dialog(q, title, "", from_u8(search.filename().string()), file_wildcards(FT_MODEL),
                                wxFD_OPEN | wxFD_FILE_MUST_EXIST);
            if (dialog.ShowModal() != wxID_OK)
                return;
        ...
        for (size_t i = 0; i < input_paths.size(); ++i) {
            ...
            for (auto [obj_idx, vol_idx] : selected_volumes) {
                ...
                bool has_source = !old_volume->source.input_file.empty() &&
                                  boost::algorithm::iequals(fs::path(old_volume->source.input_file).filename().string(),
        ...
        for (auto [src, dest] : replace_paths) {
            for (auto [obj_idx, vol_idx] : selected_volumes) {
                if (boost::algorithm::iequals(model.objects[obj_idx]->volumes[vol_idx]->source.input_file, src.string()))
                    // When an error occurs, either the dest parsing error occurs, or the number of objects in the dest is greater than 1
                    // and cannot be replaced, and cannot be replaced in this loop.
    ...
    void Plater::priv::set_current_panel(wxPanel * panel, bool no_slice)
    {
        ...
        auto do_reslice = [this, no_slice]() {
            ...
            BOOST_LOG_TRIVIAL(info)
                << __FUNCTION__
                << boost::format(": from set_current_panel, no_slice %1%, export_in_progress %2%, model_fits %3%, m_is_slicing %4%") %
                       no_slice % export_in_progress % model_fits % m_is_slicing;
        ...
        if (current_panel == view3D) {
            if (old_panel == preview)
                ...
                for (unsigned int idx : select_idxs) {
                    ...
                    auto real_idx = view3d_selection.query_real_volume_idx_from_other_view(v->object_idx(), v->instance_idx(),
                                                                                           v->volume_idx());
                    if (real_idx >= 0) {
            ...
            if (old_panel == view3D) {
                ...
                for (unsigned int idx : select_idxs) {
                    ...
                    auto real_idx = assemble_selection.query_real_volume_idx_from_other_view(v->object_idx(), v->instance_idx(),
                                                                                             v->volume_idx());
                    if (real_idx >= 0) {
        ...
        if (keys_map) {
            ...
            if (new_bed_type != btCount) {
                ...
                if (old_bed_type != new_bed_type) {
                    ...
                    app_config->set_printer_setting(wxGetApp().preset_bundle->printers.get_selected_preset_name(), "curr_bed_type",
                                                    std::to_string(int(new_bed_type)));
    ...
    void Plater::priv::on_select_preset(wxCommandEvent & evt)
    {
        ...
        std::string preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias(preset_type, Preset::remove_suffix_modified(
                                                                                                      wx_name.ToUTF8().data()));
    ...
    void Plater::priv::on_slicing_update(SlicingStatusEvent & evt)
    {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                 << boost::format(": event_type %1%, percent %2%, text %3%") % evt.GetEventType() % evt.status.percent %
                                        evt.status.text;
        ...
        if (evt.status.flags &
            ...
            for (auto const& warning : state.warnings) {
                if (warning.current) {
                    ...
                    notification_manager->push_slicing_warning_notification(warning.message, false, model_object, object_id, warning_step,
                                                                            warning.message_id, notif_level);
    ...
    void Plater::priv::on_slicing_completed(wxCommandEvent & evt)
    {
        ...
        if (m_slice_all && (m_cur_slice_plate < (partplate_list.get_plate_count() - 1))) {
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format("slicing all, finished plate %1%, will continue next.") % m_cur_slice_plate;
            return;
        }
    ...
    {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                 << boost::format(": enter, m_ignore_event %1%, status %2%") % m_ignore_event % evt.status();
        ...
        if (evt.error()) {
            ...
            if (evt.critical_error()) {
                ...
                for (auto oid : message.second) {
                    ...
                    if (print_object) {
                        ptrs.push_back(print_object->model_object());
                    }
                }
    ...
    this->sidebar->show_sliced_info_sizer(evt.success());
        ...
        if (evt.cancelled()) {
            ...
            if (!has_error && preview) {
                ...
                generate_calibration_thumbnail(*calibration_data, PartPlate::cali_thumbnail_width, PartPlate::cali_thumbnail_height,
                calibration_params); preview->get_canvas3d()->reset_gcode_toolpaths();*/
        ...
        if (is_finished) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(":slicing all, plate %1% finished, start next slice...") % m_cur_slice_plate;
            m_cur_slice_plate++;
    ...
    int Plater::priv::update_print_required_data(Slic3r::DynamicPrintConfig config, Slic3r::Model model,
                                                 Slic3r::PlateDataPtrs plate_data_list, std::string file_name, std::string file_path)
    {
        if (!m_select_machine_dlg)
            m_select_machine_dlg = new SelectMachineDialog(q);
    ...
    {
        if (!m_send_to_sdcard_dlg)
            m_send_to_sdcard_dlg = new SendToPrinterDialog(q);
        if (isall) {
    ...
    void Plater::priv::on_action_split_objects(SimpleEvent&) { split_object(); }

    void Plater::priv::on_action_split_volumes(SimpleEvent&) { split_volume(); }
    ...
    {
        notification_manager->bbl_show_plugin_install_notification(
            into_u8(_L("The network plug-in was not detected. Network related features are unavailable.")));
    }

    void Plater::priv::show_preview_only_hint(wxCommandEvent & event)
    {
        notification_manager->bbl_show_preview_only_notification(
            into_u8(_L("Preview only mode:\nThe loaded file contains G-code only, cannot enter the Prepare page.")));
    }
    ...
    void Plater::priv::apply_color_mode()
    {
        ...
        wxColour   orca_color =
            wxColour(59, 68,
                     70); // wxColour(ColorRGBA::ORCA().r_uchar(), ColorRGBA::ORCA().g_uchar(), ColorRGBA::ORCA().b_uchar());
    ...
    void Plater::priv::on_right_click(RBtnEvent & evt)
    {
        ...
        if (obj_idx == -1) {                     // no one or several object are selected
            ...
            if (printer_technology == ptSLA)
                ...
                const bool is_some_full_instances = selection.is_single_full_instance() || selection.is_single_full_object() ||
                ...
                if (current_panel == assemble_view) {
                    menu = is_some_full_instances ? menus.assemble_object_menu() :
                           is_part                ? menus.assemble_part_menu() :
                                                    menus.assemble_multi_selection_menu();
                } else {
    ...
    void Plater::priv::on_3dcanvas_mouse_dragging_started(SimpleEvent&) { view3D->get_canvas3d()->reset_sequential_print_clearance(); }
    ...
    void Plater::priv::generate_thumbnail(ThumbnailData & data, unsigned int w, unsigned int h, const ThumbnailsParams& thumbnail_params,
                                          Camera::EType camera_type, Camera::ViewAngleType camera_view_angle_type, bool for_picking,
                                          bool ban_light)
    {
        ...
        if (!m_project_folder.empty()) {
            if (!only_filename) {
                if (export_all) {
                    ...
                    auto full_filename = m_project_folder /
                                         std::string((m_project_name + from_u8(plate_index_str) + extension).mb_str(wxConvUTF8));
                    return from_path(full_filename);
                }
            } else {
                if (export_all)
            ...
            if (only_filename) {
                if (!model.objects.empty() && m_project_name == _L("Untitled"))
                    return wxString(fs::path(model.objects.front()->name).replace_extension().c_str()) + from_u8(plate_index_str) +
                           extension;
    ...
    wxString Plater::priv::get_project_name() { return m_project_name; }
    ...
    void Plater::set_prepare_state(int state) { p->m_job_prepare_state = state; }

    int Plater::get_prepare_state() { return p->m_job_prepare_state; }
    ...
    int Plater::get_send_calibration_finished_event() { return EVT_SEND_CALIBRATION_FINISHED; }

    int Plater::get_print_finished_event() { return EVT_PRINT_FINISHED; }

    int Plater::get_send_finished_event() { return EVT_SEND_FINISHED; }

    int Plater::get_publish_finished_event() { return EVT_PUBLISH_FINISHED; }
    ...
    bool Plater::priv::init_collapse_toolbar()
    {
        ...
        item.sprite_id            = 0;
        item.left.action_callback = []() { wxGetApp().plater()->collapse_sidebar(!wxGetApp().plater()->is_sidebar_collapsed()); };
...
void Plater::update_partplate()
{
    ...
    void Plater::priv::reset_gcode_toolpaths() { preview->get_canvas3d()->reset_gcode_toolpaths(); }
    ...
    bool Plater::priv::can_split(bool to_objects) const { return sidebar->obj_list()->is_splittable(to_objects); }
    ...
    bool Plater::priv::can_scale_to_print_volume() const
    {
        ...
        return !sidebar->obj_list()->has_selected_cut_object() && !view3D->get_canvas3d()->get_selection().is_empty() &&
    ...
    bool Plater::priv::can_reload_from_disk() const
    {
        ...
        struct SelectedVolume
        {
            ...
            bool operator==(const SelectedVolume& other) const
            {
                return (object_idx == other.object_idx) && (volume_idx == other.volume_idx);
            }
        };
        std::vector<SelectedVolume> selected_volumes;
    ...
    {
        if (q != nullptr) {
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << ":recevied publish event\n";
        }

        if (!m_publish_dlg)
            m_publish_dlg = new PublishDialog(q);
        if (show) {
            m_publish_dlg->reset();
    ...
    // BBS: add bed exclude area
    void Plater::priv::set_bed_shape(const Pointfs& shape, const Pointfs& exclude_areas, const Pointfs& wrapping_exclude_areas,
                                     const double printable_height, std::vector<Pointfs> extruder_areas,
                                     std::vector<double> extruder_heights, const std::string& custom_texture,
                                     const std::string& custom_model, bool force_as_custom)
    {
        ...
        bool  new_shape      = bed.set_shape(shape, printable_height, extruder_areas, extruder_heights, custom_model, force_as_custom,
                                             shape_position);
        ...
        if (new_shape) {
            ...
            partplate_list.reset_size(max.x() - min.x() - Bed3D::Axes::DefaultTipRadius, max.y() - min.y() - Bed3D::Axes::DefaultTipRadius,
                                      z);
            partplate_list.set_shapes(shape, exclude_areas, wrapping_exclude_areas, extruder_areas, extruder_heights, custom_texture,
    ...
    bool Plater::priv::can_delete() const { return !get_selection().is_empty() && !get_selection().is_wipe_tower(); }

    bool Plater::priv::can_delete_all() const { return !model.objects.empty(); }

    bool Plater::priv::can_add_plate() const { return q->get_partplate_list().get_plate_count() < PartPlateList::MAX_PLATES_COUNT; }
    ...
    bool Plater::priv::can_simplify() const
    {
        ...
        if (get_selected_object_idx() < 0)
            return false;
        // is already opened?
        if (q->get_view3D_canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::EType::Simplify)
            return false;
        return true;
    }
    ...
    {
        if (!m_worker.is_idle() || q->get_view3D_canvas3D()->get_gizmos_manager().is_in_editing_mode())
            return false;

        int obj_idx = get_selected_object_idx();
        return (0 <= obj_idx) && (obj_idx < (int) model.objects.size()) && !sidebar->obj_list()->has_selected_cut_object() &&
               std::all_of(model.objects[obj_idx]->instances.begin(), model.objects[obj_idx]->instances.end(),
                           [](auto& inst) { return inst->printable; });
    }
    ...
    bool Plater::priv::can_split_to_objects() const { return q->can_split(true); }

    bool Plater::priv::can_split_to_volumes() const { return (printer_technology != ptSLA) && q->can_split(false); }

    bool Plater::priv::can_arrange() const { return !model.objects.empty() && m_worker.is_idle(); }
    ...
    bool Plater::priv::can_layers_editing() const { return layers_height_allowed(); }
    ...
    void Plater::priv::on_modify_filament(SimpleEvent & evt)
    {
        ...
        if (wxID_EDIT == res) {
            ...
            wxGetApp().params_dialog()->Popup();
            tab->restore_last_select_item();
            // Opening Studio and directly accessing the Filament settings interface through the edit preset button will not take effect and
            // requires manual settings.
            tab->set_just_edit(true);
            tab->select_preset(need_edit_preset->name);
            // when some preset have modified, if the printer is not need_edit_preset_name compatible printer, the preset will jump to other
            // preset, need select again
            if (!need_edit_preset->is_compatible)
                tab->select_preset(need_edit_preset->name);
        }
    }

    void Plater::priv::on_add_filament(SimpleEvent & evt) { sidebar->add_filament(); }

    void Plater::priv::on_delete_filament(SimpleEvent & evt) { sidebar->delete_filament(); }

    void Plater::priv::on_add_custom_filament(ColorEvent & evt) { sidebar->add_custom_filament(evt.data); }
    ...
    void Plater::priv::take_snapshot(const std::string& snapshot_name, const UndoRedo::SnapshotType snapshot_type)
    {
        ...
        //  This is a workaround until we refactor the Wipe Tower position / orientation to live solely inside the Model, not in the Print
        //  config. BBS: add partplate logic
        if (this->printer_technology == ptFFF) {
        ...
        const GLGizmosManager& gizmos = get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
                                            assemble_view->get_canvas3d()->get_gizmos_manager() :
                                            view3D->get_canvas3d()->get_gizmos_manager();

        if (snapshot_type == UndoRedo::SnapshotType::ProjectSeparator)
            this->undo_redo_stack().clear();
        this->undo_redo_stack().take_snapshot(snapshot_name, model,
                                              get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
                                                  assemble_view->get_canvas3d()->get_selection() :
                                                  view3D->get_canvas3d()->get_selection(),
                                              gizmos, partplate_list, snapshot_data);
        if (snapshot_type == UndoRedo::SnapshotType::LeavingGizmoWithAction) {
            // Filter all but the last UndoRedo::SnapshotType::GizmoAction in a row between the last UndoRedo::SnapshotType::EnteringGizmo
            // and UndoRedo::SnapshotType::LeavingGizmoWithAction. The remaining snapshot will be renamed to a more generic name, depending
            // on what gizmo is being left.
        ...
        ((this->printer_technology == ptFFF) ?
             m_last_fff_printer_profile_name :
             m_last_sla_printer_profile_name) = wxGetApp().preset_bundle->printers.get_selected_preset_name();
        BOOST_LOG_TRIVIAL(info) << "Undo / Redo snapshot taken: " << snapshot_name
                                << ", Undo / Redo stack memory: " << Slic3r::format_memsize_MB(this->undo_redo_stack().memsize())
                                << log_memory_info();
    }
    ...
    void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
    {
        ...
        if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
                this->undo_redo_stack().undo(model,
                                             get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
                                                 assemble_view->get_canvas3d()->get_selection() :
                                                 this->view3D->get_canvas3d()->get_selection(),
                                             get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
                                                 assemble_view->get_canvas3d()->get_gizmos_manager() :
                                                 this->view3D->get_canvas3d()->get_gizmos_manager(),
                                             this->partplate_list, top_snapshot_data, it_snapshot->timestamp) :
                this->undo_redo_stack().redo(model,
                                             get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
                                                 assemble_view->get_canvas3d()->get_gizmos_manager() :
                                                 this->view3D->get_canvas3d()->get_gizmos_manager(),
                                             this->partplate_list, it_snapshot->timestamp)) {
            if (printer_technology_changed) {
                // Switch to the other printer technology. Switch to the last printer active for that particular technology.
                AppConfig* app_config = wxGetApp().app_config;
                app_config->set("presets", PRESET_PRINTER_NAME,
                ...
                // crash by removing selection first.
            ...
            //  This is a workaround until we refactor the Wipe Tower position / orientation to live solely inside the Model, not in the
            //  Print config. BBS: add partplate logic
            if (this->printer_technology == ptFFF) {
                const DynamicPrintConfig& config   = wxGetApp().preset_bundle->prints.get_edited_preset().config;
                const DynamicPrintConfig& proj_cfg = wxGetApp().preset_bundle->project_config;
    ...
    {
        get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
            assemble_view->get_canvas3d()->get_selection().clear() :
    ...
    int Plater::new_project(bool skip_confirm, bool silent, const wxString& project_name)
    {
        ...
        auto check = [this, &transfer_preset_changes](bool yes_or_no) {
            ...
            wxString header               = _L("Some presets are modified.") + "\n" +
                              (yes_or_no ?
                                   _L("You can keep the modified presets to the new project or discard them") :
            ...
            bool result = wxGetApp().check_and_keep_current_preset_changes(_L("Creating a new project"), header, act_buttons,
                                                                           &transfer_preset_changes);
    ...
    void Plater::import_model_id(wxString download_info)
    {
        ...
        int            percent = 0;
        ProgressDialog dlg(dlg_title, wxString(' ', 100) + "\n\n\n\n",
                           100,  // range
                           this, // parent
    ...
    void Plater::download_project(const wxString& project_id) { return; }
    ...
    bool Plater::up_to_date(bool saved, bool backup)
    {
        ...
        return p->model.objects.empty() || (p->up_to_date(saved, backup) && !Slic3r::has_other_changes(backup));
    }
    ...
    void Plater::_calib_pa_pattern(const Calib_Params& params)
    {
        ...
        if (accels.empty()) {
            accels.assign({accel});
            const auto msg{_L("INFO:") + "\n" + _L("No accelerations provided for calibration. Use default acceleration value ") +
                           std::to_string(long(accel)) + _L(u8"mm/s²")};
        ...
        if (!has_junction_deviation(printer_config) &&
            print_config.option<ConfigOptionFloat>("default_jerk")->value > 0) {          // we have set a jerk value
        ...
        if (speeds.empty()) {
            double speed = CalibPressureAdvance::find_optimal_PA_speed(wxGetApp().preset_bundle->full_config(),
            ...
            speeds.assign({speed});
            const auto msg{_L("INFO:") + "\n" + _L("No speeds provided for calibration. Use default optimal speed ") +
                           std::to_string(long(speed)) + _L("mm/s")};
            get_notification_manager()->push_notification(msg.ToStdString());
        ...
        CalibPressureAdvancePattern pa_pattern(params, full_config, is_bbl_machine, *cube, cur_plate->get_origin());
        ...
        cube->scale((pa_pattern.print_size_x() + 4) / cube_bb.size().x(), (pa_pattern.print_size_y() + 4) / cube_bb.size().y(),
        ...
        cube->scale(pa_pattern.handle_xy_size() / cube_bb.size().x(), pa_pattern.handle_xy_size() / cube_bb.size().y(),
                    pa_pattern.max_layer_z() / cube_bb.size().z());
    ...
    void Plater::_calib_pa_pattern_gen_gcode()
    {
        ...
        for (auto obj : cur_plate->get_objects_on_this_plate()) {
            auto gcode = model().calib_pa_pattern->generate_custom_gcodes(preset_bundle->full_config(), preset_bundle->is_bbl_vendor(),
                                                                          *obj, cur_plate->get_origin());
    ...
    void Plater::_calib_pa_tower(const Calib_Params& params)
    {
        ...
        auto wall_speed = CalibPressureAdvance::find_optimal_PA_speed(full_config, full_config.get_abs_value("line_width", nozzle_diameter),
    ...
    void Plater::calib_flowrate(bool is_linear, int pass)
    {
        ...
        if (is_linear) {
            if (pass == 1)
                ...
                add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "Orca-LinearFlow_fine.3mf")
                                     .string());
        } else {
            if (pass == 1)
                add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "flowrate-test-pass1.3mf")
                                     .string());
            else
                add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "flowrate-test-pass2.3mf")
                                     .string());
        }
    ...
    void Plater::calib_input_shaping_freq(const Calib_Params& params)
    {
        ...
        add_model(false, Slic3r::resources_dir() + (params.test_model < 1 ? "/calib/input_shaping/ringing_tower.drc" :
                                                                            "/calib/input_shaping/fast_tower_test.drc"));
        auto       print_config        = &wxGetApp().preset_bundle->prints.get_edited_preset().config;
        auto       filament_config     = &wxGetApp().preset_bundle->filaments.get_edited_preset().config;
        auto       printer_config      = &wxGetApp().preset_bundle->printers.get_edited_preset().config;
        ...
        if (has_junction_deviation(printer_config)) {
            printer_config
                ->set_key_value("machine_max_junction_deviation",
                                new ConfigOptionFloats{
                                    (std::max(printer_config->option<ConfigOptionFloats>("machine_max_junction_deviation")->values.front(),
                                              0.25))});
            print_config->set_key_value("default_junction_deviation", new ConfigOptionFloat(0));
        } else {
            const double jerk_value = (gcode_flavor_option && gcode_flavor_option->value == GCodeFlavor::gcfKlipper) ? 5.0 : 10.0;
            printer_config->set_key_value("machine_max_jerk_x",
                                          new ConfigOptionFloats{
                                              std::max(printer_config->option<ConfigOptionFloats>("machine_max_jerk_x")->values.front(),
                                                       jerk_value)});
            printer_config->set_key_value("machine_max_jerk_y",
                                          new ConfigOptionFloats{
                                              std::max(printer_config->option<ConfigOptionFloats>("machine_max_jerk_y")->values.front(),
                                                       jerk_value)});
    ...
    void Plater::Calib_Cornering(const Calib_Params& params)
    {
        ...
        const std::string cornering_model_path = params.test_model == 0 ?
                                                     ...
                                                     (params.test_model == 1 ? "/calib/input_shaping/fast_tower_test.drc" :
                                                                               "/calib/cornering/SCV-V2.drc");
    ...
    void Plater::reload_print() { p->preview->reload_print(); }

    // BBS
    wxString Plater::get_project_name() { return p->get_project_name(); }
    ...
    void Plater::update_all_plate_thumbnails(bool force_update)
    {
        for (int i = 0; i < get_partplate_list().get_plate_count(); i++) {
            ...
            if (force_update || !plate->thumbnail_data.is_valid()) {
                get_view3D_canvas3D()->render_thumbnail(plate->thumbnail_data, plate->plate_thumbnail_width, plate->plate_thumbnail_height,
                                                        thumbnail_params, Camera::EType::Ortho);
            }
            if (force_update || !plate->no_light_thumbnail_data.is_valid()) {
                get_view3D_canvas3D()->render_thumbnail(plate->no_light_thumbnail_data, plate->plate_thumbnail_width,
                                                        plate->plate_thumbnail_height, thumbnail_params, Camera::EType::Ortho,
                                                        Camera::ViewAngleType::Iso, false, true);
            }
        }
    }

    void Plater::update_obj_preview_thumbnail(ModelObject * mo, int obj_idx, int vol_idx, std::vector<Slic3r::ColorRGBA> colors,
                                              int camera_view_angle_type)
    {
        ...
        get_view3D_canvas3D()->render_thumbnail(plate->obj_preview_thumbnail_data, colors, plate->plate_thumbnail_width,
                                                plate->plate_thumbnail_height, thumbnail_params, model_objects, cur_volumes,
                                                Camera::EType::Ortho, (Camera::ViewAngleType) camera_view_angle_type, false, false);
    }
            ...
            if (dlg.ShowModal() == wxID_OK) {
                    ...
                    for (mz_uint i = 0; i < num_entries; ++i) {
                        if (mz_zip_reader_file_stat(&archive, i, &stat)) {
                            ...
                            try {
                                ...
                                mz_bool res = mz_zip_reader_extract_to_mem(&archive, stat.m_file_index, (void*) buffer.data(),
                                                                           (size_t) stat.m_uncomp_size, 0);
                                if (res == 0) {
                                    // TRN: First argument = path to file, second argument = error description
                                    wxString error_log = GUI::format_wxstr(_L("Failed to unzip file to %1%: %2%"), final_path.string(),
                                ...
                                if (!fs::exists(final_path)) {
                                    wxString error_log =
                                        GUI::format_wxstr(_L("Failed to find unzipped file at %1%. Unzipping of file has failed."),
                                                          final_path.string());
                                    BOOST_LOG_TRIVIAL(error) << error_log;
                                    show_error(nullptr, error_log);
                                    break;
                                ...
                                if (/*(boost::algorithm::iends_with(filename, ".3mf") && !is_project_3mf(final_path.string())) ||*/
                                    (boost::algorithm::iends_with(filename, ".amf") &&
                                     !boost::algorithm::iends_with(filename, ".zip.amf"))) {
                                    non_project_paths.emplace_back(final_path);
                                    break;
                                }
                ...
                if (non_project_paths.size() + project_paths.size() != selected_paths.size())
                    BOOST_LOG_TRIVIAL(error) << "Decompresing of archive did not retrieve all files. Expected files: "
                                             << selected_paths.size()
                                             << " Decopressed files: " << non_project_paths.size() + project_paths.size();
            } else {
                close_zip_reader(&archive);
                return false;
            }
...
#define PROJECT_DROP_DIALOG_SELECT_PLANE_SIZE wxSize(FromDIP(350), FromDIP(120))
    ...
    ProjectDropDialog::ProjectDropDialog(const std::string& filename)
        : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe), wxID_ANY, from_u8((boost::format(_utf8(L("Drop project file")))).str()),
        ...
        , m_action(2)
    {
        ...
        radio_group->Bind(wxEVT_COMMAND_RADIOBOX_SELECTED,
                          [this, radio_group](wxCommandEvent& e) { set_action(radio_group->GetSelection() + 1); });
        ...
        //
        // auto dont_show_again = create_remember_checkbox(_L("Remember my choice."), this, _L("This option can be changed later in
        // preferences, under 'Load Behaviour'.")); m_sizer_left->Add(dont_show_again, 0, wxALL, 5);
        //
    ...
    void ProjectDropDialog::on_select_cancel(wxCommandEvent & event) { EndModal(wxID_CANCEL); }
    ...
    bool Plater::load_files(const wxArrayString& filenames)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": normal_paths %1%, gcode_paths %2%") % normal_paths.size() % gcode_paths.size();
        ...
        std::sort(normal_paths.begin(), normal_paths.end(),
                  [](fs::path obj1, fs::path obj2) { return obj1.filename().string() < obj2.filename().string(); });

        auto loadfiles_type  = LoadFilesType::NoFile;
        auto amf_files_count = get_3mf_file_count(normal_paths);

        if (normal_paths.size() > 1 && amf_files_count < normal_paths.size()) {
            loadfiles_type = LoadFilesType::Multiple3MFOther;
        }
        if (normal_paths.size() > 1 && amf_files_count == normal_paths.size()) {
            loadfiles_type = LoadFilesType::Multiple3MF;
        ...
        auto handle_zips =
            [this](vector<fs::path>& paths) { // NOLINT(*-no-recursion) - Recursion is intended and should be managed properly
                bool res = false;
        ...
        case LoadFilesType::Single3MF: open_3mf_file(normal_paths[0]); break;
    ...
    bool Plater::open_3mf_file(const fs::path& file_path)
    {
        ...
        bool load_setting_ask_when_relevant = wxGetApp().app_config->get(SETTING_PROJECT_LOAD_BEHAVIOUR) ==
                                              OPTION_PROJECT_LOAD_BEHAVIOUR_ASK_WHEN_RELEVANT;
        LoadType load_type = determine_load_type(filename, (not_empty_plate && load_setting_ask_when_relevant) ?
                                                               OPTION_PROJECT_LOAD_BEHAVIOUR_ALWAYS_ASK :
                                                               "");
    ...
    void Plater::update(bool conside_update_flag, bool force_background_processing_update)
    {
        ...
        unsigned int flag = force_background_processing_update ?
                                (unsigned int) Plater::priv::UpdateParams::FORCE_BACKGROUND_PROCESSING_UPDATE :
                                0;
    ...
    void Plater::reset_with_confirm()
    {
        if (p->model.objects.empty() ||
            MessageDialog(static_cast<wxWindow*>(this), _L("All objects will be removed, continue?"),
    ...
    int GUI::Plater::close_with_confirm(std::function<bool(bool)> second_check)
    {
        if (up_to_date(false, false)) {
            if (second_check && !second_check(false))
                return wxID_CANCEL;
        ...
        if (second_check && !second_check(result == wxID_YES))
            return wxID_CANCEL;
    ...
    void Plater::delete_all_objects_from_model() { p->delete_all_objects_from_model(); }
    ...
    static long GetNumberFromUser(const wxString& msg, const wxString& prompt, const wxString& title, long value, long min, long max,
                                  wxWindow* parent)
    {
#ifdef _WIN32
    ...
    void Plater::set_number_of_copies(/*size_t num*/)
    {
        ...
        const int num = GetNumberFromUser(" ", _L("Number of copies:"), _L("Copies of the selected object"), model_object->instances.size(),
                                          0, 1000, this);
        if (num < 0)
            return;
    ...
    bool Plater::is_selection_empty() const { return p->get_selection().is_empty() || p->get_selection().is_wipe_tower(); }

    void Plater::scale_selection_to_fit_print_volume() { p->scale_selection_to_fit_print_volume(); }
    ...
    void Plater::convert_unit(ConversionType conv_type)
    {
        ...
        TakeSnapshot snapshot(this, conv_type == ConversionType::CONV_FROM_INCH  ? "Convert from imperial units" :
                                    ...
                                    conv_type == ConversionType::CONV_FROM_METER ? "Convert from meters" :
                                                                                   "Revert conversion from meters");
        wxBusyCursor wait;
    ...
    void Plater::export_gcode(bool prefer_removable)
    {
        ...
        fs::path output_path;
        {
            ...
            wxFileDialog dlg(this, (printer_technology() == ptFFF) ? _L("Save G-code file as:") : _L("Save SLA file as:"), start_dir,
        ...
        if (!output_path.empty()) {
            ...
            // is_path_on_removable_drive() is called with the "true" parameter to update its internal database as the user may have
            // shuffled the external drives while the dialog was open.
    ...
    void Plater::send_to_printer(bool isall) { p->on_action_send_to_printer(isall); }
    ...
    void Plater::export_gcode_3mf(bool export_all)
    {
        ...
        fs::path output_path;
        {
            ...
            wxFileDialog dlg(this, _L("Save Sliced file as:"), start_dir, from_path(default_output_file.filename()),
                             GUI::file_wildcards(FT_GCODE_3MF, ""), wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
            if (dlg.ShowModal() == wxID_OK) {
                output_path = into_path(dlg.GetPath());
    ...
    Preset* get_printer_preset(const MachineObject* obj)
    {
        ...
        for (auto printer_it = preset_bundle->printers.begin(); printer_it != preset_bundle->printers.end(); printer_it++) {
            ...
            if (model_id.compare(printer_type) == 0 && printer_nozzle_vals &&
                abs(printer_nozzle_vals->get_at(0) - machine_nozzle_diameter) < 1e-3) {
            ...
            if (popup_warning) {
                if (!only_warning) {
                    if (DevPrinterConfigUtil::get_printer_can_set_nozzle(obj->get_show_printer_type())) {
                        ...
                        MessageDialog dlg(wxGetApp().plater(), _L("The nozzle type is not set. Please check."), _L("Warning"),
                                          wxOK | wxICON_WARNING);
                        dlg.ShowModal();
                    }
                    ...
                    auto printer_name =
                        get_selected_printer_name_in_combox(); // wxString(obj->get_preset_printer_model_name(machine_print_name))
                    pop_warning_and_go_to_device_page(printer_name, Plater::PrinterWarningType::NOT_CONNECTED,
                                                      _L("Sync printer information"));
                }
            }
            return false;
    ...
    TriangleMesh Plater::combine_mesh_fff(const ModelObject& mo, int instance_id, std::function<void(const std::string&)> notify_func)
    {
        ...
        if (auto fail_reason_name = csg::check_csgmesh_booleans(Range{std::begin(csgmesh), std::end(csgmesh)});
            std::get<0>(fail_reason_name) != csg::BooleanFailReason::OK) {
            ...
            std::map<csg::BooleanFailReason, std::string> fail_reasons =
                {{csg::BooleanFailReason::OK, "OK"},
                 ...
                 {csg::BooleanFailReason::NoIntersection,
                  Slic3r::format(_u8L("Reason: \"%1%\" and another part have no intersection."), name)}};
...
#define EXPORT_WITH_BOOLEAN 0
    void Plater::export_stl(bool extended, bool selection_only, bool multi_stls, FileType file_type)
    {
        ...
        auto mesh_to_export_fff_no_boolean = [this](const ModelObject& mo, int instance_id) {
            ...
            if (mesh.empty()) {
                get_notification_manager()->push_plater_error_notification(_u8L("Unable to perform boolean operation on model meshes. "
        ...
        auto mesh_to_export_sla = [&, this](const ModelObject& mo, int instance_id) {
            ...
            if (auto m = object->get_mesh_to_print(); m.empty())
                mesh = combine_mesh_fff(mo, instance_id, [this](const std::string& msg) {
                    return get_notification_manager()->push_general_error_notification(msg);
                });
            else {
                ...
                for (const SLAPrintObject::Instance& obj_instance : obj_instances) {
                    ...
                    if (it != object->model_object()->instances.end()) {
                        ...
                        const Transform3d& inst_transform =
                            one_inst_only ? Transform3d::Identity() :
    ...
    // BBS: backup
    int Plater::export_3mf(const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx,
                           Export3mfProgressFn proFn)
    {
        ...
        BOOST_LOG_TRIVIAL(trace) << __FUNCTION__
                                 << boost::format(": path=%1%, backup=%2%, export_plate_idx=%3%, SaveStrategy=%4%") % output_path.string() %
                                        (strategy & SaveStrategy::Backup) % export_plate_idx % (unsigned int) strategy;

        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": path=%1%, backup=%2%, export_plate_idx=%3%, SaveStrategy=%4%") % std::string("") %
                                       (strategy & SaveStrategy::Backup) % export_plate_idx % (unsigned int) strategy;
        ...
        if (!(strategy & SaveStrategy::Backup)) {
            for (int i = 0; i < p->partplate_list.get_plate_count(); i++) {
                ...
                if (p->partplate_list.get_plate(i)->thumbnail_data.is_valid() && using_exported_file()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                            << boost::format(": non need to re-generate thumbnail for gcode/exported mode of plate %1%") %
                                                   i;
                } else {
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(": re-generate thumbnail for plate %1%") % i;
                    const ThumbnailsParams thumbnail_params = {{}, false, true, true, true, i};
                    p->generate_thumbnail(p->partplate_list.get_plate(i)->thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                          THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho);
                }
                ...
                if (p->partplate_list.get_plate(i)->no_light_thumbnail_data.is_valid() && using_exported_file()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(": re-generate thumbnail for plate %1%") % i;
                    const ThumbnailsParams thumbnail_params = {{}, false, true, true, true, i};
                    p->generate_thumbnail(p->partplate_list.get_plate(i)->no_light_thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                          THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho, Camera::ViewAngleType::Iso,
                                          false, true);
                }
                ...
                if (top_thumbnail->is_valid() && using_exported_file()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                            << boost::format(
                                                   ": non need to re-generate top_thumbnail for gcode/exported mode of plate %1%") %
                                                   i;
                } else {
                    ...
                    p->generate_thumbnail(p->partplate_list.get_plate(i)->top_thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                          THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho,
                                          Camera::ViewAngleType::Top_Plate, false);
                }
                ...
                if (picking_thumbnail->is_valid() && using_exported_file()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                            << boost::format(
                                                   ": non need to re-generate pick_thumbnail for gcode/exported mode of plate %1%") %
                                                   i;
                } else {
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(": re-generate pick_thumbnail for plate %1%") % i;
                    const ThumbnailsParams thumbnail_params = {{}, false, true, false, true, i};
                    p->generate_thumbnail(p->partplate_list.get_plate(i)->pick_thumbnail_data, THUMBNAIL_SIZE_3MF.first,
                                          THUMBNAIL_SIZE_3MF.second, thumbnail_params, Camera::EType::Ortho,
                                          Camera::ViewAngleType::Top_Plate, true, true);
                }
            ...
            if (p->partplate_list.get_curr_plate()->is_slice_result_valid()) {
                ...
                // p->generate_calibration_thumbnail(*calibration_data, PartPlate::cali_thumbnail_width, PartPlate::cali_thumbnail_height,
                // calibration_params);
                if (using_exported_file()) {
                    // do nothing
                } else
        ...
        p->partplate_list.store_to_3mf_structure(plate_data_list,
                                                 (strategy & SaveStrategy::WithGcode || strategy & SaveStrategy::WithSliceInfo),
                                                 export_plate_idx);
    ...
    void Plater::publish_project() { return; }

    void Plater::reload_from_disk() { p->reload_from_disk(); }

    void Plater::replace_with_stl() { p->replace_with_stl(); }

    void Plater::replace_all_with_stl() { p->replace_all_with_stl(); }

    void Plater::reload_all_from_disk() { p->reload_all_from_disk(); }

    bool Plater::has_toolpaths_to_export() const { return p->preview->get_canvas3d()->has_toolpaths_to_export(); }
    ...
    bool Plater::is_empty_project() { return model().objects.empty(); }
    ...
    {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", Line %1%: enter, process_completed_with_error=%2%") % __LINE__ %
                                       p->process_completed_with_error;
        ...
        if (!stop_queue(this->get_ui_job_worker(), timeout_ms)) {
            BOOST_LOG_TRIVIAL(error) << "Could not stop UI job within " << timeout_ms << " milliseconds timeout!";
            return;
        }
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(", Line %1%: restart background,state=%2%, result=%3%") % __LINE__ % state % result;
        if ((state & priv::UPDATE_BACKGROUND_PROCESS_INVALID) != 0) {
            // BBS: add logs
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(": state %1% is UPDATE_BACKGROUND_PROCESS_INVALID, can not slice") % state;
        ...
        if ((!result) && p->m_slice_all && (p->m_cur_slice_plate < (p->partplate_list.get_plate_count() - 1))) {
            // slice next
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                       << boost::format(": in slicing all, current plate %1% already sliced, skip to next") %
                                              p->m_cur_slice_plate;
        ...
        if (p->background_process.running()) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": background process changes to not_idle, set ready_to_slice back to true");
        } else {
            ...
            p->main_frame->update_slice_print_status(MainFrame::eEventSliceUpdate, false);
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": background process in idle state, use previous result, clean_gcode_toolpaths=%1%") %
                                           clean_gcode_toolpaths;
        }
    ...
    void Plater::record_slice_preset(std::string action)
    {
        ...
        try {
            ...
            j["support_type"] = ConfigOptionEnum<SupportType>::get_enum_names().at(
                print_preset.config.opt_enum<SupportType>("support_type"));
            j["sparse_infill_pattern"] = ConfigOptionEnum<InfillPattern>::get_enum_names().at(
                print_preset.config.opt_enum<InfillPattern>("sparse_infill_pattern"));
            ...
            if (p->background_process.fff_print()) {
                ...
                if (full_config.has("different_settings_to_system")) {
                    std::vector<std::string> different_values = full_config.option<ConfigOptionStrings>("different_settings_to_system")
                                                                    ->values;
    ...
    void Plater::send_gcode_legacy(int plate_idx, Export3mfProgressFn proFn, bool use_3mf)
    {
...

        {
            ...
            if (host_type == htElegooLink) {
                pDlg = std::make_unique<ElegooPrintHostSendDialog>(default_output_file, upload_job.printhost->get_post_upload_actions(),
    ...
    void Plater::take_snapshot(const std::string& snapshot_name, UndoRedo::SnapshotType snapshot_type)
    {
    ...
    // BBS: single snapshot
    void Plater::single_snapshots_enter(SingleSnapshot * single) { p->single_snapshots_enter(single); }
    ...
    int Plater::update_print_required_data(Slic3r::DynamicPrintConfig config, Slic3r::Model model, Slic3r::PlateDataPtrs plate_data_list,
    ...
    void Plater::on_bed_type_change(BedType bed_type) { sidebar().on_bed_type_change(bed_type); }
    ...
    void Plater::on_config_change(const DynamicPrintConfig& config)
    {
        ...
        for (auto opt_key : diff_keys) {
            ...
            // BBS: add bed_exclude_area
            else if (opt_key == "printable_area" || opt_key == "bed_exclude_area" || opt_key == "bed_custom_texture" ||
                     opt_key == "bed_custom_model" || opt_key == "extruder_clearance_height_to_lid" ||
                ...
                bed_shape_changed = true;
                update_scheduled  = true;
            } else if (boost::starts_with(opt_key, "enable_prime_tower") || boost::starts_with(opt_key, "prime_tower") ||
    ...
    void Plater::update_flush_volume_matrix(size_t old_nozzle_size, size_t new_nozzle_size)
    {
        ...
        if (old_nozzle_size < new_nozzle_size) {
            std::vector<double> first_flush_volume_mtx =
                get_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, -1, old_nozzle_size);
            if (first_flush_volume_mtx.size() == filament_nums * filament_nums * new_nozzle_size) { // load file
                set_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, first_flush_volume_mtx,
                                         -1, new_nozzle_size);
            } else {
                ...
                set_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, flush_volume_mtx, -1,
                                         new_nozzle_size);
            }
            ...
            for (size_t i = 0; i < new_nozzle_size; ++i) {
                std::vector<double> flush_volume_mtx =
                    get_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, -1,
                                             old_nozzle_size);
            ...
            std::vector<double> flush_multipliers = project_config->option<ConfigOptionFloats>("flush_multiplier")->values;
            flush_multipliers.resize(nozzle_nums, 1.f);
            set_flush_volumes_matrix(project_config->option<ConfigOptionFloats>("flush_volumes_matrix")->values, new_flush_volume_mtx, -1,
                                     new_nozzle_size);
            project_config->option<ConfigOptionFloats>("flush_multiplier")->values = flush_multipliers;
        }
    }
    ...
    // BBS: add bed exclude area
    void Plater::set_bed_shape(const Pointfs& shape, const Pointfs& exclude_area, const Pointfs& wrapping_exclude_area,
                               const double printable_height, std::vector<Pointfs> extruder_areas, std::vector<double> extruder_heights,
                               const std::string& custom_texture, const std::string& custom_model, bool force_as_custom) const
    {
        p->set_bed_shape(make_counter_clockwise(shape), exclude_area, wrapping_exclude_area, printable_height, extruder_areas,
                         extruder_heights, custom_texture, custom_model, force_as_custom);
    }
...
// BBS: filament_color logic has been moved out of filament setting
    ...
    {
        // Fill in the printer model key with something which cannot possibly be valid, so that Plater::on_config_change() will update the
        // print bed once a new Printer profile config is loaded.
        p->config->opt_string("printer_model", true) = "bbl_empty";
    }

    void Plater::on_activate() { this->p->show_delayed_error_message(); }
    ...
    std::vector<std::string> Plater::get_filament_colors_render_info() const
    {
        ...
        if (!config->has("filament_multi_colour"))
            return color_packs;
    ...
    std::vector<std::string> Plater::get_filament_color_render_type() const
    {
        ...
        if (!config->has("filament_colour_type"))
            return ctype;
    ...
    wxWindow* Plater::get_select_machine_dialog() { return p->m_select_machine_dlg; }
    ...
    wxString Plater::get_project_filename(const wxString& extension) const { return p->get_project_filename(extension); }
    ...
    void Plater::set_project_filename(const wxString& filename) { p->set_project_filename(filename); }

    bool Plater::is_export_gcode_scheduled() const { return p->background_process.is_export_scheduled(); }

    const Selection& Plater::get_selection() const { return p->get_selection(); }

    int Plater::get_selected_object_idx() { return p->get_selected_object_idx(); }

    bool Plater::is_single_full_object_selection() const { return p->get_selection().is_single_full_object(); }
    ...
    GLCanvas3D* Plater::get_view3D_canvas3D() { return p->view3D->get_canvas3d(); }
    ...
    GLCanvas3D* Plater::get_current_canvas3D(bool exclude_preview) { return p->get_current_canvas3D(exclude_preview); }
    ...
    void Plater::set_current_canvas_as_dirty() { p->set_current_canvas_as_dirty(); }

    void Plater::unbind_canvas_event_handlers() { p->unbind_canvas_event_handlers(); }

    void Plater::reset_canvas_volumes() { p->reset_canvas_volumes(); }

    PrinterTechnology Plater::printer_technology() const { return p->printer_technology; }
    ...
    bool Plater::is_background_process_update_scheduled() const { return this->p->background_process_timer.IsRunning(); }
        ...
        if (type == PrinterWarningType::NOT_CONNECTED) {
            if (device_page) {
                ...
                content = wxString::Format(
                    _L("OrcaSlicer can't connect to %s. Please check if the printer is powered on and connected to the network."),
                    printer_name);
            }
        } else if (type == PrinterWarningType::INCONSISTENT) {
            content =
                wxString::Format(_L("The currently connected printer on the device page is not %s. Please switch to %s before syncing."),
            ...
            content = _L(
                "The filaments on the printer are all unknown types. Please go to the printer screen or software device page to set "
                "the filament type.");
        }
    ...
    bool Plater::is_same_printer_for_connected_and_selected(bool popup_warning)
    {
        ...
        if (wxGetApp().is_blocking_printing()) {
            if (popup_warning) {
                ...
                pop_warning_and_go_to_device_page(printer_name, PrinterWarningType::INCONSISTENT,
    ...
    void Plater::sys_color_changed()
    {
        ...
        if (p->m_select_machine_dlg)
            p->m_select_machine_dlg->sys_color_changed();
...
void Plater::enable_view_toolbar(bool enable)
{
    ...
    bool Plater::init_collapse_toolbar() { return p->init_collapse_toolbar(); }

    const Camera& Plater::get_camera() const { return p->camera; }

    Camera& Plater::get_camera() { return p->camera; }

    // BBS: partplate list related functions
    PartPlateList& Plater::get_partplate_list() { return p->partplate_list; }
    ...
    void Plater::apply_background_progress()
    {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(" %1%: plate %2%, after apply, invalidated= %3%, previous result_valid %4% ") % __LINE__ %
                                       plate_index % invalidated % result_valid;
    ...
    int Plater::select_plate(int plate_index, bool need_slice)
    {
        ...
        if ((!ret) && (p->background_process.can_switch_print())) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(" %1%: plate %2%, after apply, invalidated= %3%, previous result_valid %4% ") %
                                           __LINE__ % plate_index % invalidated % result_valid;
            if (result_valid) {
                if (is_preview_shown()) {
                ...
                if (is_preview_shown()) {
                    if (need_slice) {
                        ...
                        if (model_fits && !validate_err) {
                            ...
                            p->main_frame->update_slice_print_status(MainFrame::eEventPlateUpdate, false);
                            // sometimes the previous print's sliced result is still valid, but the newly added object is laid over the
                            // boundary then the print toolpath will be shown, so we should not refresh print here, only onload shell
                            // refresh_print();
                            p->update_fff_scene_only_shells();
                        }
                    } else {
    ...
    void Plater::validate_current_plate(bool& model_fits, bool& validate_error)
    {
        ...
        if (model_fits) { // TPU check
            bool tpu_valid = cur_plate->check_tpu_printable_status(wxGetApp().preset_bundle->full_config(),
                                                                   wxGetApp().preset_bundle->get_used_tpu_filaments(
                                                                       cur_plate->get_extruders(true)));
            model_fits &= tpu_valid;
        }

        if (model_fits) { // Filament printable check
            wxString filament_printable_error_msg;
            bool     filament_printable = cur_plate->check_filament_printable(wxGetApp().preset_bundle->full_config(),
                                                                              filament_printable_error_msg);
        ...
        if (p->printer_technology == ptFFF) {
            ...
            //             "Please solve the problem by moving it totally on or off the plate, and confirming that the height is within the
            //             build volume.");;
            StringObjectException                  warning;
            Polygons                               polygons;
            std::vector<std::pair<Polygon, float>> height_polygons;
            p->background_process.fff_print()->set_check_multi_filaments_compatibility(
                wxGetApp().app_config->get("enable_high_low_temp_mixed_printing") == "false");
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(": validate err=%1%, warning=%2%, model_fits %3%") % err.string % warning.string %
                                           model_fits;
            ...
            std::string left_unprintable_text  = get_left_extruder_unprintable_text(),
                        right_unprintable_text = get_right_extruder_unprintable_text();
            if (!left_unprintable_text.empty()) {
                p->notification_manager
                    ->bbl_show_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError,
                                                                              left_unprintable_text);
            } else {
                p->notification_manager->bbl_close_filament_map_invalid_notification_before_slice(
                    NotificationType::LeftExtruderUnprintableError);
            }

            if (!right_unprintable_text.empty()) {
                p->notification_manager
                    ->bbl_show_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError,
                                                                              right_unprintable_text);
            } else {
                p->notification_manager->bbl_close_filament_map_invalid_notification_before_slice(
                    NotificationType::RightExtruderUnprintableError);
            }
    ...
    void Plater::open_platesettings_dialog(wxCommandEvent & evt)
    {
        ...
        dlg.Bind(EVT_SET_BED_TYPE_CONFIRM, [this, plate_index, &dlg](wxCommandEvent& e) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format("select bed type %1% for plate %2% at plate side") % bt_sel % plate_index;
            ...
            set_plater_dirty(true);
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format("select print sequence %1% for plate %2% at plate side") % ps_sel % plate_index;
    ...
    void Plater::open_filament_map_setting_dialog(wxCommandEvent & evt)
    {
        ...
        FilamentMapDialog filament_dlg(this, filament_colors, filament_types, plate_filament_maps, curr_plate->get_extruders(true),
                                       plate_filament_map_mode, this->get_machine_sync_status(), false);
    ...
    int Plater::select_plate_by_hover_id(int hover_id, bool right_click, bool isModidyPlateName)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": enter, hover_id %1%, plate_index %2%, action %3%") % hover_id % plate_index % action;
        if (action == 0) {
            // select plate
            ret = p->partplate_list.select_plate(plate_index);
            if (!ret) {
            ...
            if ((!ret) && (p->background_process.can_switch_print())) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(" %1%: after apply, invalidated= %2%, previous result_valid %3% ") % __LINE__ %
                                               invalidated % result_valid;
                if (result_valid) {
                    if (invalidated & PrintBase::APPLY_STATUS_INVALIDATED) {
                        // bool model_fits, validate_err;
                        // validate_current_plate(model_fits, validate_err);
                    ...
                    if (model_fits && part_plate->has_printable_instances()) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                                << boost::format(": will set can_slice to false, has_printable_instances %1%") %
                                                       part_plate->has_printable_instances();
                        // p->view3D->get_canvas3d()->post_event(Event<bool>(EVT_GLCANVAS_ENABLE_ACTION_BUTTONS, false));
                        p->main_frame->update_slice_print_status(MainFrame::eEventPlateUpdate, false);
                    }
                }
    ...
    void Plater::set_bed_position(Vec2d & pos) { p->bed.set_position(pos); }

    // BBS: is the background process slicing currently
    bool Plater::is_background_process_slicing() const { return p->m_is_slicing; }
    ...
    void Plater::show_object_info()
    {
        ...
        if (non_manifold_edges > 0) {
            info_manifold += into_u8("\n" + _L("Tips:") + "\n" +
                                     _L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Orca "
        ...
        if (err.type == StringExceptionType::STRING_EXCEPT_FILAMENT_NOT_MATCH_BED_TYPE) {
            try {
                ...
                if (extruder_id < preset_bundle->filament_presets.size()) {
                    ...
                    err.string =
                        format(_L("Plate %d: %s is not suggested to be used to print filament %s (%s). "
    ...
    unsigned int Plater::get_environment_texture_id() const { return p->environment_texture.get_id(); }
#endif // ENABLE_ENVIRONMENT_MAP

    const BuildVolume& Plater::build_volume() const { return p->bed.build_volume(); }
...
GLToolbar& Plater::get_view_toolbar()
{
    ...
    const GLToolbar& Plater::get_collapse_toolbar() const { return p->collapse_toolbar; }

    GLToolbar& Plater::get_collapse_toolbar() { return p->collapse_toolbar; }

    void Plater::update_preview_bottom_toolbar() { p->update_preview_bottom_toolbar(); }
    ...
    const Mouse3DController& Plater::get_mouse3d_controller() const { return p->mouse3d_controller; }
    ...
    void Plater::show_status_message(std::string s) { BOOST_LOG_TRIVIAL(trace) << "show_status_message:" << s; }
    ...
    void Plater::toggle_render_statistic_dialog() { p->show_render_statistic_dialog = !p->show_render_statistic_dialog; }

    bool Plater::is_render_statistic_dialog_visible() const { return p->show_render_statistic_dialog; }

    void Plater::toggle_show_wireframe() { p->show_wireframe = !p->show_wireframe; }

    bool Plater::is_show_wireframe() const { return p->show_wireframe; }

    void Plater::enable_wireframe(bool status) { p->wireframe_enabled = status; }

    bool Plater::is_wireframe_enabled() const { return p->wireframe_enabled; }
    ...
    void Plater::bring_instance_forward() { p->bring_instance_forward(); }

    bool Plater::need_update() const { return p->need_update(); }

    void Plater::set_need_update(bool need_update) { p->set_need_update(need_update); }
    ...
    bool Plater::PopupObjectTable(int object_id, int volume_id, const wxPoint& position)
    {
        if (dynamic_cast<TabPrinter*>(wxGetApp().get_tab(Preset::TYPE_PRINTER))->m_extruders_count > 1) {
            MessageDialog dlg(this, _L("Currently, the object configuration form cannot be used with a multiple-extruder printer."),
                              _L("Not available"), wxOK | wxICON_WARNING);
            dlg.ShowModal();
            return false;
        }
    ...
    void Plater::update_title_dirty_status() { p->update_title_dirty_status(); }
