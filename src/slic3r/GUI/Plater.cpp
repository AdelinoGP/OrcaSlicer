    ...
    for (auto [src, dest] : replace_paths) {
        for (auto [obj_idx, vol_idx] : selected_volumes) {
            if (boost::algorithm::iequals(model.objects[obj_idx]->volumes[vol_idx]->source.input_file, src.string()))
                // When an error occurs, either the dest parsing error occurs, or the number of objects in the dest is greater than 1 and
                // cannot be replaced, and cannot be replaced in this loop.
...
void Plater::priv::take_snapshot(const std::string& snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    if (snapshot_type == UndoRedo::SnapshotType::LeavingGizmoWithAction) {
        // Filter all but the last UndoRedo::SnapshotType::GizmoAction in a row between the last UndoRedo::SnapshotType::EnteringGizmo and
        // UndoRedo::SnapshotType::LeavingGizmoWithAction. The remaining snapshot will be renamed to a more generic name, depending on what
        // gizmo is being left.
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
        ...
        if (printer_technology_changed) {
            ...
            // Object_list::update_and_show_object_settings_item(), but the Object list still keeps pointer to the old Model. Avoid a crash
            // by removing selection first.
...
void Plater::priv::update_after_undo_redo(const UndoRedo::Snapshot& snapshot, bool /* temp_snapshot_was_taken */)
{
    ...
    // Release the old snapshots always, as it may have happened, that some of the triangle meshes got deserialized from the snapshot, while
    // some triangle meshes may have gotten released from the scene or the background processing, therefore now being calculated into the
    ...
    if (wxGetApp().get_mode() == comSimple && model_has_advanced_features(this->model)) {
        ...
        // There is a little risk of surprising the user, as he already must have had the advanced or advanced mode active for such a
        // snapshot to be taken.
        Slic3r::GUI::wxGetApp().save_mode(comAdvanced);
        view3D->set_as_dirty();
    }

    // this->update() above was called with POSTPONE_VALIDATION_ERROR_MESSAGE, so that if an error message was generated when updating the
    // back end, it would not open immediately, but it would be saved to be show later. Let's do it now. We do not want to display the
    // message box earlier, because on Windows & OSX the message box takes over the message queue pump, which in turn executes the rendering
    // function before a full update after the Undo / Redo jump.
    ...
    , m_action(2)
{
    ...
    //
    // auto dont_show_again = create_remember_checkbox(_L("Remember my choice."), this, _L("This option can be changed later in preferences,
    // under 'Load Behaviour'.")); m_sizer_left->Add(dont_show_again, 0, wxALL, 5);
    //
...
{
    // Fill in the printer model key with something which cannot possibly be valid, so that Plater::on_config_change() will update the print
    // bed once a new Printer profile config is loaded.
