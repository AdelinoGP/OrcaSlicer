...
// [INTENT] Processes command line arguments to detect if this instance should send its parameters to another instance.
...

{
    ...
    BOOST_LOG_TRIVIAL(debug) << "single instance: " << (ret.should_send.has_value() ? (*ret.should_send ? "true" : "false") : "undefined")
    ...
    if ( !checker->IsAnotherRunning() ) */
    {
        ...
        if (!dbus_message_append_args(msg, DBUS_TYPE_STRING, &sigval, DBUS_TYPE_INVALID)) {
            BOOST_LOG_TRIVIAL(error)
                << "Ran out of memory while constructing args for DBus message. Message to another instance wont be send.";
...
bool instance_check(int argc, char** argv, bool app_config_single_instance)
{
    ...
    if (ec.value() >
        0) { // canonical was not able to find the executable (can happen with appimage on some systems. Does it fail on Fuse file systems?)
        ec.clear();
        // Compose path with boost canonical of folder and filename
        hashed_path = std::hash<std::string>{}(
            boost::filesystem::canonical(boost::filesystem::system_complete(argv[0]).parent_path(), ec).string() + "/" +
            boost::filesystem::system_complete(argv[0]).filename().string());
        if (ec.value() > 0) {
    ...
    if (instance_check_internal::get_lock(lock_name + ".lock", data_dir() + "/cache/") && *cla.should_send) {
        ...
        BOOST_LOG_TRIVIAL(error)
            << "Instance check: Another instance found. This instance will terminate. Lock file of current running instance is located at "
            << data_dir() <<
#ifdef _WIN32
            "\\cache\\"
#else // mac & linx
...
void OtherInstanceMessageHandler::init(wxEvtHandler* callback_evt_handler)
{
    ...
    // [INTENT] Starts a worker thread for background message listening if BACKGROUND_MESSAGE_LISTENER is enabled.
    // [THREAD] Starts a worker thread for background message listening.
    // [UNITY] Use a custom Thread, Task, or Job System implementation in Unity.
    m_thread = boost::thread((boost::bind(&OtherInstanceMessageHandler::listen, this)));
#endif // BACKGROUND_MESSAGE_LISTENER
}
// [INTENT] Handles received IPC message and posts wxWidgets events to the main application.
// [UNITY] Use Unity's EventSystem or C# delegates to broadcast events to the main application controller.
...
{
    DBusMessage* reply;
    const char*  introspection_data = " <!DOCTYPE node PUBLIC \"-//freedesktop//DTD D-BUS Object Introspection 1.0//EN\" "
