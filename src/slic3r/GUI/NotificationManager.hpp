...
#include "ImGuiWrapper.hpp" // [INTENT] Include for ImGui types and rendering functionalities. [UNITY] ImGui elements will be replaced by UI Toolkit VisualElements or UGUI components.

// [INTENT] This header defines the NotificationManager class, which is responsible for managing and displaying various types of in-app
// notifications. It handles the lifecycle of notifications, including creation, update, rendering, and dismissal. It uses wxWidgets for
// event handling and ImGui for rendering.
//
// [UNITY] In Unity, this would likely map to a central notification service or manager (`NotificationService` MonoBehaviour or
// ScriptableObject) which orchestrates the display of UI Toolkit or Canvas-based notifications. Each `NotificationType` would correspond to
// a distinct UI element prefab or a `VisualElement` template. Event handling (wxEvtHandler, std::function callbacks) would be replaced by
// Unity Events or C# events/delegates. ImGui rendering calls would be replaced by UI Toolkit or UGUI rendering logic.
//
// [STATE] `m_pop_notifications` stores active notifications, `m_waiting_notifications` holds delayed notifications.
// `m_initialized` tracks if the manager is ready to display notifications.
// `m_evt_handler` is the target for wxWidgets events, facilitating cross-thread communication.
// `m_id_provider` manages unique IDs for ImGui windows to prevent conflicts during rendering.
class NotificationManager
{
public:
    // [INTENT] Defines different levels of importance for notifications, influencing their display order, persistence, and visual style.
    // [UNITY] Maps directly to different visual styles and priority handling for UI elements in Unity (e.g., different prefabs, sorting
    // layers, or distinct `VisualElement` styles).
    ...
    void push_notification(NotificationType                   type,
    ...
    void push_delayed_notification(const NotificationType    type,
    ...
    void push_slicing_warning_notification(const std::string& text,
                                           bool               gray,
    ...
    void push_flushing_volume_error_notification(NotificationType                   type,
                                                 NotificationLevel                  level,
                                                 const std::string&                 text,
                                                 const std::string&                 hypertext = "",
                                                 std::function<bool(wxEvtHandler*)> callback  = std::function<bool(wxEvtHandler*)>());
    ...
    void push_slicing_customize_error_notification(NotificationType                   type,
                                                   NotificationLevel                  level,
                                                   const std::string&                 text,
                                                   const std::string&                 hypertext = "",
                                                   std::function<bool(wxEvtHandler*)> callback  = std::function<bool(wxEvtHandler*)>());
    ...
    void push_simplify_suggestion_notification(const std::string&                 text,
                                               ObjectID                           object_id,
                                               const std::string&                 hypertext = "",
                                               std::function<bool(wxEvtHandler*)> callback  = std::function<bool(wxEvtHandler*)>());
    ...
    void push_download_URL_progress_notification(size_t                                         id,
                                                 const std::string&                             text,
...
private:
    ...
    public:
        ...
        virtual void bbl_render_block_notification(
        ...
        void         set_hovered()
        {
            if (m_state != EState::Finished && m_state != EState::ClosePending && m_state != EState::Hidden && m_state != EState::Unknown)
    ...
    protected:
        ...
        virtual void render_hypertext(ImGuiWrapper& imgui, const float text_x, const float text_y, const std::string text, bool more = false);
        virtual void bbl_render_block_notif_text(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        virtual void bbl_render_block_notif_buttons(ImGuiWrapper& imgui, ImVec2 win_size, ImVec2 win_pos);
        virtual void bbl_render_block_notif_left_sign(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        // Left sign could be error or warning sign
        virtual void bbl_render_left_sign(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);

        virtual void render_left_sign(ImGuiWrapper& imgui);
        virtual void render_minimize_button(ImGuiWrapper& imgui, const float win_pos_x, const float win_pos_y);
    ...
    {
    public:
        PlaterWarningNotification(const NotificationData& n, NotificationIDProvider& id_provider, wxEvtHandler* evt_handler)
            : PopNotification(n, id_provider, evt_handler)
        {}
        void close() override;
        void real_close()
    ...
    {
    public:
        ProgressBarNotification(const NotificationData& n, NotificationIDProvider& id_provider, wxEvtHandler* evt_handler)
            : PopNotification(n, id_provider, evt_handler)
        {}
    ...
    {
    public:
        URLDownloadNotification(const NotificationData&                        n,
            ...
            : ProgressBarNotification(n, id_provider, evt_handler), m_download_id(download_id), m_user_action_callback(user_action_callback)
        {}
        ...
        void   set_user_action_callback(std::function<bool(DownloaderUserAction, int)> user_action_callback)
        {
    ...
    protected:
        void render_close_button(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) override;
        void render_close_button_inner(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        void render_pause_cancel_buttons_inner(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        void render_open_button_inner(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        void render_cancel_button_inner(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        void render_pause_button_inner(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
        void render_bar(
            ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) override;
    ...
    {
    public:
        enum class UploadJobState { PB_PROGRESS, PB_ERROR, PB_CANCELLED, PB_COMPLETED };
        PrintHostUploadNotification(const NotificationData& n,
                                    NotificationIDProvider& id_provider,
                                    wxEvtHandler*           evt_handler,
                                    float                   percentage,
                                    int                     job_id,
                                    float                   filesize)
            : ProgressBarNotification(n, id_provider, evt_handler), m_job_id(job_id), m_file_size(filesize)
        {
            m_has_cancel_button = true;
            set_percentage(percentage);
        }
        static std::string get_upload_job_text(int id, const std::string& filename, const std::string& host)
        {
            return /*"[" + std::to_string(id) + "] " + */ filename + " -> " + host;
        }
        void set_percentage(float percent) override;
        void cancel()
        {
            m_uj_state          = UploadJobState::PB_CANCELLED;
    ...
    {
    public:
        ExportFinishedNotification(const NotificationData& n,
                                   NotificationIDProvider& id_provider,
                                   wxEvtHandler*           evt_handler,
                                   bool                    to_removable,
    ...
    {
    public:
        UpdatedItemsInfoNotification(const NotificationData& n,
                                     NotificationIDProvider& id_provider,
                                     wxEvtHandler*           evt_handler,
                                     InfoItemType            info_item_type)
            : PopNotification(n, id_provider, evt_handler)
        {
    ...
    struct DelayedNotification
    {
        ...
        DelayedNotification(std::unique_ptr<PopNotification> n, std::function<bool(void)> cb, int64_t r, int64_t d)
            : notification(std::move(n)), condition_callback(cb), remaining_time(r), delay_interval(d)
        {}
    };
    ...
    // Note that notification object is constructed when being added to the waiting list, but there are no updates called on it and its
    // timer is reset at regular push. Also note that no control of same notification is done during push_delayed_notification_data but
    // if waiting notif fails to push, it continues waiting. If delay_interval is 0, notification is pushed only after initial_delay no
    // matter the result.
    void push_delayed_notification_data(std::unique_ptr<NotificationManager::PopNotification> notification,
                                        std::function<bool(void)>                             condition_callback,
                                        int64_t                                               initial_delay,
                                        int64_t                                               delay_interval);
    ...
    const std::vector<NotificationType> m_multiple_types = {NotificationType::CustomNotification, NotificationType::PlaterWarning,
                                                            NotificationType::ProgressBar, NotificationType::PrintHostUpload,
                                                            NotificationType::SimplifySuggestion};
    // prepared (basic) notifications
    //  non-static so its not loaded too early. If static, the translations wont load correctly.
    const std::vector<NotificationData> basic_notifications = {
        NotificationData{NotificationType::Mouse3dDisconnected, NotificationLevel::RegularNotificationLevel, 10,
                         _u8L("3D Mouse disconnected.")},
        NotificationData{NotificationType::PresetUpdateAvailable, NotificationLevel::ImportantNotificationLevel, BBL_NOTICE_MAX_INTERVAL,
                         _u8L("Configuration can update now."), _u8L("Detail."),
        ...
        NotificationData{NotificationType::EmptyColorChangeCode, NotificationLevel::PrintInfoNotificationLevel, 10,
                         std::string(_devL("value can not be empty when you add a G-code for color change.\nPlease check the \"Color "
                                           "Change G-code\" in \"Printer Settings > Custom G-code\"")
                                         .mb_str())},
        ...
        NotificationData{NotificationType::BBLPluginUpdateAvailable, NotificationLevel::ImportantNotificationLevel, BBL_NOTICE_MAX_INTERVAL,
                         _u8L("New network plug-in available."), _u8L("Details"),
                         [](wxEvtHandler* evnthndlr) {
        ...
        NotificationData{NotificationType::BBLPrinterConfigUpdateAvailable, NotificationLevel::ImportantNotificationLevel,
                         BBL_NOTICE_MAX_INTERVAL, _u8L("New printer config available."), _u8L("Details"),
                         [](wxEvtHandler* evnthndlr) {
                             if (evnthndlr != nullptr)
                                 wxPostEvent(evnthndlr,
                                             PrinterConfigUpdateAvailableClickedEvent(EVT_PRINTER_CONFIG_UPDATE_AVAILABLE_CLICKED));
                             return true;
                         }},
        ...
        NotificationData{NotificationType::BBLUserPresetExceedLimit, NotificationLevel::WarningNotificationLevel, BBL_NOTICE_MAX_INTERVAL,
                         _u8L("The number of user presets cached in the cloud has exceeded the upper limit, newly created user presets can "
                              "only be used locally."),
                         _u8L("Wiki Guide"),
                         [](wxEvtHandler* evnthndlr) {
                             wxLaunchDefaultBrowser(
                                 "https://wiki.bambulab.com/en/software/bambu-studio/3rd-party-printer-profile#cloud-user-presets-limit");
                             return false;
                         }},
        ...
        NotificationData{NotificationType::NewAppAvailable, NotificationLevel::ImportantNotificationLevel, 20,
                         _u8L("Software has New version."), _u8L("Goto download page."),
                         [](wxEvtHandler* evnthndlr) {
                             // BBS set feishu release page by default
                             wxGetApp().open_browser_with_warning_dialog("https://www.thingiverse.com/");
                             return true;
                         }},
        //{NotificationType::NewAppAvailable, NotificationLevel::ImportantNotificationLevel, 20,  _u8L("New vesion of PrusaSlicer is
        // available.", _u8L("Download page.") }, {NotificationType::LoadingFailed, NotificationLevel::RegularNotificationLevel, 20,
        //_u8L("Loading of model has Failed") }, {NotificationType::DeviceEjected, NotificationLevel::RegularNotificationLevel, 10,
        // _u8L("Removable device has been safely ejected")} // if we want changeble text (like here name of device), we need to do it
