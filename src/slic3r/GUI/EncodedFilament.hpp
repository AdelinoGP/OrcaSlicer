// [ANNOTATED]
// [INTENT] Declarations for the filament color metadata service and associated data models.
// [STATE] Custom structures for HSV-based color comparison and multi-color filament categorization.
// [UNITY] Replace wxColourToHSV with Unity's Color.RGBToHSV. Port sorting logic to C# IComparer.
// [PORTING_HAZARD:P2] Custom color set comparison logic (operator<) is used for database indexing and must be mirrored exactly.
...
struct wxColorSorter
{
    std::size_t operator()(const wxColour& lhs_it, const wxColour& rhs_it) const noexcept
    {
        ...
        if (ha.v != hb.v)
            return ha.v < hb.v;
        if (lhs_it.Alpha() != rhs_it.Alpha())
            return lhs_it.Alpha() < rhs_it.Alpha();
        return false;
    }
};
...
public:
    bool operator<(const FilamentColor& other) const
    {
        ...
        if (m_color_type != other.m_color_type) {
            return m_color_type < other.m_color_type;
        }
        if (m_colors == other.m_colors) {
            return false;
        }
...
public:
    ...
    FilamentColorCode(const wxString&                          color_code,
