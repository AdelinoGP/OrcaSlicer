...
// [INTENT] The main worker method runs the desktop OAuth authorization-code flow without blocking the UI thread.
// [STATE] The job owns a short-lived local callback server plus a shared result object that survives until finalize posts completion.
// [THREAD] This method is executed on a worker thread while the callback handler and HTTP exchange feed results back through a thread-safe
// queue. [UNITY] Port this to an async service that starts a localhost listener, awaits the browser callback, then marshals the completion
// back onto the main thread. [PORTING_HAZARD:P2] The flow depends on desktop localhost redirects and wx job finalization; Unity needs an
// explicit platform-safe auth bridge and callback lifetime model.
void OAuthJob::process(Ctl& ctl)
{
    ...
    // [EVENT] The callback handler is the cross-thread handoff point where the browser redirect becomes an OAuthResult pushed back to the
    // worker. [UNITY] Equivalent to an HttpListener/embedded web callback that resolves a TaskCompletionSource once the redirect arrives.
    local_authorization_server.set_request_handler([this, queue](const std::string& url) -> std::shared_ptr<HttpServer::Response> {
        if (boost::contains(url, "/callback")) {
            ...
            OAuthResult r;
            // [INTENT] Exchange the authorization code for tokens synchronously inside the callback path so the browser receives a final
            // redirect only after auth succeeds/fails. [PORTING_HAZARD:P2] Unity should avoid doing this blocking network call directly
            // inside the listener callback and instead await a request task before completing the browser flow.
    ...
    // [STATE] The local server stays alive until we either receive a callback result or the job is canceled.
...
// [INTENT] Finalize shuts down the callback server on the UI thread and emits the completion event that lets dialogs resume their login
// flow. [THREAD] This method is executed on the main UI thread after the worker loop exits. [EVENT] `EVT_OAUTH_COMPLETE_MESSAGE` is the
// only UI-facing completion signal; downstream dialogs rely on it instead of directly observing the worker thread. [UNITY] Replace this
// with a main-thread callback/event from the auth service rather than a wx event posted to a window handle.
