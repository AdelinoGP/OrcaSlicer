...
// [INTENT] Updates render colors from ImGui settings.
// [UNITY] Use Unity's Color/Material system or UI Toolkit theme colors.
...
void GLCanvas3D::load_render_colors() { RenderColor::colors[RenderCol_3D_Background] = ImGuiWrapper::to_ImVec4(DEFAULT_BG_LIGHT_COLOR); }
...
{
    static std::string object_limited_text = _u8L(
...
{
    static std::string object_clashed_text = _u8L(
...
void GLCanvas3D::LayersEditing::show_tooltip_information(const GLCanvas3D&            canvas,
...
void GLCanvas3D::LayersEditing::render_variable_layer_height_dialog(const GLCanvas3D& canvas)
{
    ...
    imgui.begin(_L("Variable layer height"),
                ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
    ...
    if (imgui.button(_L("Smooth")))
        wxPostEvent((wxEvtHandler*) canvas.get_wxglcanvas(),
                    HeightProfileSmoothEvent(EVT_GLCANVAS_SMOOTH_LAYER_HEIGHT_PROFILE, m_smooth_params));
    ImGui::SameLine();
    text_align = std::max(text_align, ImGui::GetCursorPosX());
    ImGui::SetCursorPosX(text_align);
    ImGui::AlignTextToFramePadding();
    ...
    std::map<wxString, wxString> captions_texts = {{_L("Left mouse button") + ":", _L("Add detail")},
...
bool GLCanvas3D::LayersEditing::is_initialized() const { return wxGetApp().get_shader("variable_layer_height") != nullptr; }
        ...
        if (z != -1000.0f) {
            ...
            for (size_t i = m_layer_height_profile.size() - 2; i >= 2; i -= 2) {
                ...
                if (zi_1 <= z && z <= zi) {
                    float dz = zi - zi_1;
                    h        = (dz != 0.0f) ?
                                   static_cast<float>(lerp(m_layer_height_profile[i - 1], m_layer_height_profile[i + 1], (z - zi_1) / dz)) :
...
void GLCanvas3D::LayersEditing::render_profile(const GLCanvas3D& canvas)
{
    ...
    if (!m_profile.baseline.is_initialized() || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        // vertices
        const float axis_x = 2.0f *
                             ((cnv_width - THICKNESS_BAR_WIDTH + float(m_slicing_parameters->layer_height) * scale_x) * cnv_inv_width -
                              0.5f);
    ...
    if (!m_profile.profile.is_initialized() || m_profile.old_layer_height_profile != m_layer_height_profile) {
        ...
        for (unsigned int i = 0; i < (unsigned int) m_layer_height_profile.size(); i += 2) {
            init_data.add_vertex(
                Vec2f(2.0f * ((cnv_width - THICKNESS_BAR_WIDTH + float(m_layer_height_profile[i + 1]) * scale_x) * cnv_inv_width - 0.5f),
...
void GLCanvas3D::LayersEditing::render_volumes(const GLCanvas3D& canvas, const GLVolumeCollection& volumes)
{
    ...
    ScopeGuard       guard([current_shader]() {
    ...
    glsafe(::glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0, half_w, half_h, GL_RGBA, GL_UNSIGNED_BYTE,
                             m_layers_texture.data.data() + m_layers_texture.width * m_layers_texture.height * 4));
...
void GLCanvas3D::LayersEditing::adjust_layer_height_profile()
{
    ...
    Slic3r::adjust_layer_height_profile(*m_model_object, *m_slicing_parameters, m_layer_height_profile, this->last_z, this->strength,
                                        this->band_width, this->last_action);
    m_layers_texture.valid = false;
}
...
void GLCanvas3D::LayersEditing::generate_layer_height_texture()
{
    ...
    m_layers_texture.cells         = Slic3r::generate_layer_height_texture(*m_slicing_parameters,
                                                                           Slic3r::generate_object_layers(*m_slicing_parameters,
                                                                                                          m_layer_height_profile, false),
                                                                           m_layers_texture.data.data(), m_layers_texture.height,
                                                                           m_layers_texture.width, level_of_detail_2nd_level);
...
void GLCanvas3D::Labels::render(const std::vector<const ModelInstance*>& sorted_instances) const
{
    ...
    if (sorted_instances.size() > 0) {
        for (size_t i = 0; i < sorted_instances.size(); ++i) {
            ...
            std::vector<Owner>::iterator it = std::find_if(owners.begin(), owners.end(),
                                                           [id](const Owner& owner) { return owner.model_instance_id == id; });
            if (it != owners.end())
    ...
    for (const Owner& owner : owners) {
        ...
        imgui.begin(owner.title, ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration |
        ...
        if (ImGui::GetWindowContentRegionWidth() + 2.0f * ImGui::GetStyle().WindowPadding.x !=
            ImGui::CalcWindowNextAutoFitSize(ImGui::GetCurrentWindow()).x)
...
void GLCanvas3D::Tooltip::render(const Vec2d& mouse_position, GLCanvas3D& canvas)
{
    ...
    const float alpha = (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_start_time).count() <
                         500) ?
                            0.01f :
                            1.0f;
    ...
    imgui.begin(wxString("canvas_tooltip"), ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_NoMove |
    ...
    if (alpha < 1.0f || ImGui::GetWindowContentRegionWidth() + 2.0f * ImGui::GetStyle().WindowPadding.x !=
                            ImGui::CalcWindowNextAutoFitSize(ImGui::GetCurrentWindow()).x)
        imgui.set_requires_extra_frame();
#else
    if (alpha < 1.0f || ImGui::GetWindowContentRegionWidth() + 2.0f * ImGui::GetStyle().WindowPadding.x !=
...
void GLCanvas3D::SequentialPrintClearance::set_polygons(const Polygons&                               polygons,
                                                        const std::vector<std::pair<Polygon, float>>& height_polygons)
{
    // BBS: add height limit logic
...
void GLCanvas3D::load_arrange_settings()
{
    ...
    std::string dist_fff_seq_print_str = wxGetApp().app_config->get("arrange", "min_object_distance_seq_print_fff");

    std::string dist_sla_str = wxGetApp().app_config->get("arrange", "min_object_distance_sla");

    std::string en_rot_fff_str = wxGetApp().app_config->get("arrange", "enable_rotation_fff");

    std::string en_rot_fff_seqp_str = wxGetApp().app_config->get("arrange", "enable_rotation_seq_print");

    std::string en_rot_sla_str = wxGetApp().app_config->get("arrange", "enable_rotation_sla");

    std::string en_allow_multiple_materials_str = wxGetApp().app_config->get("arrange", "allow_multi_materials_on_same_plate");

    std::string en_avoid_region_str = wxGetApp().app_config->get("arrange", "avoid_extrusion_cali_region");
    ...
    if (!en_allow_multiple_materials_str.empty())
        m_arrange_settings_fff.allow_multi_materials_on_same_plate = (en_allow_multiple_materials_str == "1" ||
                                                                      en_allow_multiple_materials_str == "true");
...
BoundingBoxf3 GLCanvas3D::_get_current_partplate_print_volume()
{
    ...
    if (m_process && m_config) {
        ...
        BoundingBoxf3 print_volume({plate_bb.min(0), plate_bb.min(1), 0.0},
                                   {plate_bb.max(0), plate_bb.max(1), m_config->opt_float("printable_height")});
...
static bool construct_error_string(ObjectFilamentResults& object_result, std::string& error_string)
{
    ...
    if (!object_result.partly_outside_objects.empty()) {
        ...
        error_string += _u8L("Please solve the problem by moving it totally on or off the plate, and confirming that the height is within "
                             "the build volume.\n");
        return true;
    }
    return false;
    ...
    for (size_t idx = 0; idx < tips.size(); ++idx) {
        ...
        if ((idx == 0 && left_unprintable_objects.size() > 1) || (idx == 1 && right_unprintable_objects.size() > 1))
            ...
            model_prefix = (boost::format(_u8L("The position or size of the model %s exceeds the %s's printable range.")) %
                            object_result.object_filaments.front().object->name % nozzle_name)
                               .str();
        ...
        if (idx == 0)
            tips[idx] += (boost::format(_u8L("Left nozzle: X:%1%-%2%, Y:%3%-%4%, Z:%5%-%6%\n")) % format_number(left_x_min) %
                          format_number(left_x_max) % format_number(left_y_min) % format_number(left_y_max) % format_number(left_z_min) %
                          format_number(left_z_max))
                             .str();
        else
            tips[idx] += (boost::format(_u8L("Right nozzle: X:%1%-%2%, Y:%3%-%4%, Z:%5%-%6%")) % format_number(right_x_min) %
                          format_number(right_x_max) % format_number(right_y_min) % format_number(right_y_max) %
                          format_number(right_z_min) % format_number(right_z_max))
                             .str();
...
void GLCanvas3D::toggle_sla_auxiliaries_visibility(bool visible, const ModelObject* mo, int instance_idx)
{
    ...
    for (GLVolume* vol : m_volumes.volumes) {
        ...
        if ((mo == nullptr || m_model->objects[vol->composite_id.object_id] == mo) &&
            (instance_idx == -1 || vol->composite_id.instance_id == instance_idx) && vol->composite_id.volume_id < 0) {
            vol->is_active = visible;
            auto it        = std::find_if(raycasters->begin(), raycasters->end(), [vol](std::shared_ptr<SceneRaycasterItem> item) {
...
Camera& GLCanvas3D::get_camera() { return camera; }

void GLCanvas3D::set_color_by(const std::string& value) { m_color_by = value; }

void GLCanvas3D::refresh_camera_scene_box() { wxGetApp().plater()->get_camera().set_scene_box(scene_bounding_box()); }
...
BoundingBoxf3 GLCanvas3D::volumes_bounding_box(bool current_plate_only) const
{
    ...
    if (is_limit) {
        if (current_plate_only) {
            ...
            auto        horizontal_radius = 0.5 * sqrt(std::pow(plate_list_box.min[0] - plate_list_box.max[0], 2) +
                                                       std::pow(plate_list_box.min[1] - plate_list_box.max[1], 2));
            const float scale             = 2;
...
void GLCanvas3D::enable_select_plate_toolbar(bool enable) { m_sel_plate_toolbar.set_enabled(enable); }
...
void GLCanvas3D::zoom_to_gcode() { _zoom_to_box(m_gcode_viewer.get_paths_bounding_box(), 1.05); }
...
bool GLCanvas3D::make_current_for_postinit() { return _set_current(); }
...
void GLCanvas3D::render(bool only_init)
{
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        if (m_canvas_type == ECanvasType::CanvasPreview &&
            m_gcode_viewer.has_data()) { // ORCA only shift position of notifiations when sliders / gcode_viewer exist
        ...
        wxGetApp().plater()->get_notification_manager()->render_notifications(*this, get_overlay_window_width(), bottom_margin,
                                                                              right_margin);
                                  ...
                                  bool                    ban_light)
{
    ...
    render_thumbnail(thumbnail_data, w, h, thumbnail_params, model_objects, m_volumes, camera_type, camera_view_angle_type, for_picking,
                     ban_light);
}
                                  ...
                                  bool                      ban_light)
{
    ...
    case OpenGLManager::EFramebufferType::Arb: {
        render_thumbnail_framebuffer(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(), model_objects,
                                     volumes, colors, shader, camera_type, camera_view_angle_type, for_picking, ban_light);
        break;
    }
    ...
    default: {
        render_thumbnail_legacy(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(), model_objects, volumes,
                                colors, shader, camera_type, camera_view_angle_type, for_picking, ban_light);
        break;
    }
    }
}
                                  ...
                                  bool                      ban_light)
{
    ...
    case OpenGLManager::EFramebufferType::Arb: {
        render_thumbnail_framebuffer(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(), model_objects,
                                     volumes, extruder_colors, shader, camera_type, camera_view_angle_type, for_picking, ban_light);
        break;
    }
    case OpenGLManager::EFramebufferType::Ext: {
        render_thumbnail_framebuffer_ext(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(), model_objects,
    ...
    default: {
        render_thumbnail_legacy(thumbnail_data, w, h, thumbnail_params, wxGetApp().plater()->get_partplate_list(), model_objects, volumes,
                                extruder_colors, shader, camera_type, camera_view_angle_type, for_picking, ban_light);
        break;
    }
    }
}
...
void GLCanvas3D::update_plate_thumbnails() { _update_imgui_select_plate_toolbar(); }
...
void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh)
{
    ...
    {
        ModelVolumeState(const GLVolume* volume) : model_volume(nullptr), geometry_id(volume->geometry_id), volume_idx(-1) {}
        ModelVolumeState(const ModelVolume* model_volume, const ObjectID& instance_id, const GLVolume::CompositeID& composite_id)
            : model_volume(model_volume)
            , geometry_id(std::make_pair(model_volume->id().id, instance_id.id))
    ...
    {
        GLVolumeState() : volume_idx(size_t(-1)) {}
        GLVolumeState(const GLVolume* volume, unsigned int volume_idx) : composite_id(volume->composite_id), volume_idx(volume_idx) {}
        GLVolumeState(const GLVolume::CompositeID& composite_id) : composite_id(composite_id), volume_idx(size_t(-1)) {}
            ...
            for (int volume_idx = 0; volume_idx < (int) model_object->volumes.size(); ++volume_idx) {
                ...
                if (m_canvas_type == ECanvasType::CanvasAssembleView) {
                    if (model_volume->is_model_part())
                        model_volume_state.emplace_back(model_volume, model_instance->id(),
                                                        GLVolume::CompositeID(object_idx, volume_idx, instance_idx));
                } else {
                    model_volume_state.emplace_back(model_volume, model_instance->id(),
                                                    GLVolume::CompositeID(object_idx, volume_idx, instance_idx));
                }
    ...
    for (size_t volume_id = 0; volume_id < m_volumes.volumes.size(); ++volume_id) {
        ...
        if (mvs == nullptr || force_full_scene_refresh) {
            ...
            if (mvs->model_volume != nullptr) {
                ...
                if (m_canvas_type != ECanvasType::CanvasAssembleView) {
                    volume->set_instance_transformation(
                        mvs->model_volume->get_object()->instances[mvs->composite_id.instance_id]->get_transformation());
                    ...
                    volume->set_instance_transformation(
                        mvs->model_volume->get_object()->instances[mvs->composite_id.instance_id]->get_assemble_transformation());
                    volume->set_volume_transformation(mvs->model_volume->get_transformation());
                    // updates volumes convex hull
                    if (mvs->model_volume->is_model_part() && !volume->convex_hull())
                        // Model volume was likely changed from modifier or support blocker / enforcer to a model part.
                        // Only model parts require convex hulls.
                        volume->set_convex_hull(mvs->model_volume->get_convex_hull_shared_ptr());
                    volume->set_offset_to_assembly(
                        mvs->model_volume->get_object()->instances[mvs->composite_id.instance_id]->get_offset_to_assembly());
                }
            }
        }
    }
        ...
        for (const SLAPrintObject* print_object : sla_print->objects()) {
                ...
                for (size_t istep = 0; istep < sla_steps.size(); ++istep)
                    if (sla_steps[istep] == slaposDrillHoles) {
                        ...
                        if (it->new_geometry()) {
                            ...
                            m_volumes.volumes[it->volume_idx]->composite_id =
                                GLVolume::CompositeID(object_idx, m_volumes.volumes[it->volume_idx]->volume_idx(), instance_idx);
                            m_volumes.volumes[it->volume_idx]->set_instance_transformation(
                                model_object->instances[instance_idx]->get_transformation());
                        }
                    }
            }
            ...
            for (size_t istep = 0; istep < sla_steps.size(); ++istep)
                if (!instances[istep].empty())
                    m_volumes.load_object_auxiliary(print_object, object_idx, instances[istep], sla_steps[istep],
                                                    state.step[istep].timestamp);
        }

        // Shift-up all volumes of the object so that it has the right elevation with respect to the print bed
        for (GLVolume* volume : m_volumes.volumes)
            if (volume->object_idx() < (int) m_model->objects.size() &&
                m_model->objects[volume->object_idx()]->instances[volume->instance_idx()]->is_printable())
    ...
    if (printer_technology == ptFFF && m_config->has("filament_colour") && (m_canvas_type != ECanvasType::CanvasAssembleView)) {
        ...
        if (wt && (need_wipe_tower || filaments_count > 1) && !wxGetApp().plater()->only_gcode_mode() &&
            !wxGetApp().plater()->is_gcode_3mf()) {
            for (int plate_id = 0; plate_id < n_plates; plate_id++) {
                ...
                if (!need_wipe_tower && part_plate->get_extruders(true).size() < 2)
                    continue;
                ...
                Vec3d wipe_tower_size = ppl.get_plate(plate_id)->estimate_wipe_tower_size(print_cfg, w, v, nozzle_nums, 0, false,
                                                                                          dynamic_cast<const ConfigOptionBool*>(
                                                                                              dconfig.option("enable_wrapping_detection"))
                                                                                              ->value);

                {
                    ...
                    if (!current_print->is_step_done(psWipeTower) || !current_print->wipe_tower_data().wipe_tower_mesh_data) {
                        ...
                        {
                            int volume_idx_wipe_tower_new = m_volumes.load_wipe_tower_preview(1000 + plate_id, x + plate_origin(0),
                                                                                              y + plate_origin(1),
                                                                                              (float) wipe_tower_size(0),
                                                                                              (float) wipe_tower_size(1),
                                                                                              (float) wipe_tower_size(2), a,
                        ...
                        BoundingBox   plate_bbox2d = BoundingBox(scaled(Vec2f(plate_bbox.min[0], plate_bbox.min[1])),
                                                                 scaled(Vec2f(plate_bbox.max[0], plate_bbox.max[1])));
                        ...
                        int           volume_idx_wipe_tower_new = m_volumes.load_real_wipe_tower_preview(
                            ...
                            current_print->wipe_tower_data().wipe_tower_mesh_data->real_brim_mesh, true, a,
                            /*!print->is_step_done(psWipeTower)*/ true, m_initialized);
                        int volume_idx_wipe_tower_old = volume_idxs_wipe_tower_old[plate_id];
                        if (volume_idx_wipe_tower_old != -1)
                            map_glvolume_old_to_new[volume_idx_wipe_tower_old] = volume_idx_wipe_tower_new;
                    }
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        if (!m_volumes.empty()) {
            ...
            bool       wipe_tower_outside   = m_volumes
                                          .check_wipe_tower_outside_state(m_bed.build_volume(),
            ...
            auto unprintable_flag = construct_extruder_unprintable_error(object_results, get_left_extruder_unprintable_text(),
            ...
            bool tpu_valid = cur_plate->check_tpu_printable_status(full_config_temp, wxGetApp().preset_bundle->get_used_tpu_filaments(
            ...
            bool filament_nozzle_compatible = cur_plate->check_compatible_of_nozzle_and_filament(full_config_temp,
                                                                                                 wxGetApp().preset_bundle->filament_presets,
                                                                                                 get_nozzle_filament_incompatible_text());
            _set_warning_notification(EWarning::NozzleFilamentIncompatible, !filament_nozzle_compatible);

            bool filament_mixture_compatible = cur_plate->check_mixture_filament_compatible(full_config_temp,
                                                                                            get_filament_mixture_warning_text());
...
void GLCanvas3D::set_shell_transparence(float alpha) { m_gcode_viewer.set_shell_transparency(alpha); }
                                    ...
                                    bool                            only_gcode)
{
    ...
    m_gcode_viewer.load_as_gcode(gcode_result, *this->fff_print(), str_tool_colors, str_color_print_colors,
                                 wxGetApp().plater()->build_volume(), exclude_bounding_box, wxGetApp().get_mode(), only_gcode);
...
void GLCanvas3D::on_char(wxKeyEvent& evt)
{
    ...
    if (keyCode == WXK_ESCAPE && (_deactivate_arrange_menu() || _deactivate_orient_menu()))
        return;
...
#endif /* __APPLE__ */
        {
            m_labels.show(!m_labels.is_shown());
            m_dirty = true;
            break;
        }
        case '0': {
...
public:
    TranslationProcessor(UpAction up_action, DownAction down_action) : m_up_action(up_action), m_down_action(down_action) {}
    ...
    if (thiz != this) {
        ...
        translationProcessor = TranslationProcessor(
            ...
            [this](const Vec3d& direction, bool slow, bool camera_space) {
                ...
                Vec3d displacement;
                if (camera_space) {
                    Eigen::Matrix<double, 3, 3, Eigen::DontAlign> inv_view_3x3 =
                        wxGetApp().plater()->get_camera().get_view_matrix().inverse().matrix().block(0, 0, 3, 3);
...
//                    set_cursor(Standard);
                    ...
                    if (m_canvas_type == CanvasPreview) {
                        ...
                        if ((evt.CmdDown() || evt.ShiftDown()) && evt.GetKeyCode() == 'G') {
                            ...
                            if (m_layers_slider->GetSelection() == ssHigher) {
                                new_pos = keyCode == WXK_UP ? m_layers_slider->GetHigherValue() + increment :
                                                              m_layers_slider->GetHigherValue() - increment;
                                m_layers_slider->SetHigherValue(new_pos);
                                m_moves_slider->SetHigherValue(m_moves_slider->GetMaxValue());
                            } else if (m_layers_slider->GetSelection() == ssLower) {
                                new_pos = keyCode == WXK_UP ? m_layers_slider->GetLowerValue() + increment :
                                                              m_layers_slider->GetLowerValue() - increment;
                                m_layers_slider->SetLowerValue(new_pos);
                            }
                        } else if (keyCode == WXK_LEFT) {
                            if (m_moves_slider->GetHigherValue() == m_moves_slider->GetMinValue() &&
                                (m_layers_slider->GetHigherValue() > m_layers_slider->GetMinValue())) {
                            ...
                            if (m_moves_slider->GetHigherValue() == m_moves_slider->GetMaxValue() &&
                                (m_layers_slider->GetHigherValue() < m_layers_slider->GetMaxValue())) {
                                m_layers_slider->SetHigherValue(m_layers_slider->GetHigherValue() + 1);
    ...
    if (keyCode != WXK_TAB && keyCode != WXK_LEFT && keyCode != WXK_UP && keyCode != WXK_RIGHT && keyCode != WXK_DOWN) {
...
void GLCanvas3D::on_mouse_wheel(wxMouseEvent& evt)
{
...
#ifdef __WXMSW__
    // For some reason the Idle event is not being generated after the mouse scroll event in case of scrolling with the two fingers on the
    // touch pad, if the event is not allowed to be passed further. evt.Skip() used to trigger the needed screen refresh, but it does no
    // more. wxWakeUpIdle() seem to work now.
    wxWakeUpIdle();
#endif /* __WXMSW__ */
    ...
    if (is_layers_editing_enabled()) {
        ...
        if (object_idx_selected != -1) {
            ...
            if (m_layers_editing.bar_rect_contains(*this, (float) evt.GetX(), (float) evt.GetY())) {
                ...
                m_layers_editing.band_width = std::max(std::min(m_layers_editing.band_width * (1.0f + 0.1f * (float) evt.GetWheelRotation() /
                                                                                                          (float) evt.GetWheelDelta()),
                                                                10.0f),
                                                       1.5f);
                if (m_canvas != nullptr)
                    m_canvas->Refresh();
    ...
    if (m_canvas_type == CanvasAssembleView && (evt.AltDown() || evt.CmdDown())) {
        ...
        if (evt.AltDown()) {
            ...
            clp_dist      = rotation < 0.f ? std::max(0., clp_dist - 0.01) : std::min(1., clp_dist + 0.01);
            m_gizmos.m_assemble_view_data->model_objects_clipper()->set_position(clp_dist, true);
        } else if (evt.CmdDown()) {
            m_explosion_ratio = rotation < 0.f ? std::max(1., m_explosion_ratio - 0.01) : std::min(3., m_explosion_ratio + 0.01);
...
void GLCanvas3D::on_mouse(wxMouseEvent& evt)
{
    ...
    if (!mouse_in_layer_editing && m_gizmos.on_mouse(evt)) {
        ...
        if (evt.Dragging()) {
            ...
            if (current_printer_technology() == ptFFF && (fff_print()->config().print_sequence == PrintSequence::ByObject)) {
                ...
                if (c == GLGizmosManager::EType::Move || c == GLGizmosManager::EType::Scale || c == GLGizmosManager::EType::Rotate)
                    show_sinking_contours();
            }
        } else if (evt.LeftUp() && m_gizmos.get_current_type() == GLGizmosManager::EType::Scale &&
    ...
    if (m_mouse.drag.move_requires_threshold && m_mouse.is_move_start_threshold_position_2D_defined() &&
        m_mouse.is_move_threshold_met(pos)) {
    ...
    if (evt.Entering()) {
            ...
            if (evt.LeftDown() && evt.ShiftDown() && m_picking_enabled && m_layers_editing.state != LayersEditing::Editing) {
                if (/*m_gizmos.get_current_type() != GLGizmosManager::SlaSupports
                    ...
                    m_rectangle_selection.start_dragging(m_mouse.position,
                                                         evt.ShiftDown() ? GLSelectionRectangle::Select : GLSelectionRectangle::Deselect);
                    m_dirty = true;
                }
            } else {
        ...
        if (m_canvas_type != ECanvasType::CanvasAssembleView) {
            if (!m_mouse.drag.move_requires_threshold) {
                ...
                // we do not want to translate objects if the user just clicked on an object while pressing shift to remove it from the
                // selection and then drag
                if (m_selection.contains_volume(get_first_hover_volume_idx())) {
                    ...
                    if (std::abs(camera.get_dir_forward()(2)) < EPSILON) {
                        ...
                        // finds the intersection of the mouse ray with the plane parallel to the camera viewport and passing throught the
                        // starting position use ray-plane intersection see i.e.
                        // https://en.wikipedia.org/wiki/Line%E2%80%93plane_intersection algebric form in our case plane normal and ray
                        // direction are the same (orthogonal view) when moving to perspective camera the negative z unit axis of the camera
                        // needs to be transformed in world space and used as plane normal
            ...
            if ((any_gizmo_active || swap_mouse_buttons || m_hover_volume_idxs.empty()) && m_mouse.is_start_position_3D_defined()) {
                ...
                if (this->m_canvas_type == ECanvasType::CanvasAssembleView || m_gizmos.get_current_type() == GLGizmosManager::FdmSupports ||
                    ...
                    if (wxGetApp().app_config->get_bool("use_free_camera"))
                        ...
                        // Forces camera right vector to be parallel to XY plane in case it has been misaligned using the 3D mouse free
                        // rotation. It is cheaper to call this function right away instead of testing
                        // wxGetApp().plater()->get_mouse3d_controller().connected(), which checks an atomics (flushes CPU caches). See GH
                        // issue #3816.
                        bool rotate_limit = current_printer_technology() != ptSLA;
        ...
        if (m_layers_editing.state != LayersEditing::Unknown) {
            ...
            m_rectangle_selection.stop_dragging();
        } else if (evt.LeftUp() && !m_mouse.ignore_left_up && !m_mouse.dragging && m_hover_volume_idxs.empty() &&
                   m_hover_plate_idxs.empty() && !is_layers_editing_enabled()) {
    ...
    if (evt.LeftDClick() && !m_hover_volume_idxs.empty() &&
        (type == GLGizmosManager::EType::Undefined || type == GLGizmosManager::EType::Move || type == GLGizmosManager::EType::Rotate ||
         type == GLGizmosManager::EType::Scale || type == GLGizmosManager::EType::Emboss || type == GLGizmosManager::EType::Svg)) {
...
GLCanvas3D::WipeTowerInfo GLCanvas3D::get_wipe_tower_info(int plate_idx) const
{
    ...
    for (const GLVolume* vol : m_volumes.volumes) {
        if (vol->is_wipe_tower && vol->object_idx() - 1000 == plate_idx) {
            ...
            if (wt_brim_width < 0)
                wt_brim_width = WipeTower::get_auto_brim_by_height((float) bb.max.z());
            ...
            if (brim_width < 0)
                brim_width = WipeTower::get_auto_brim_by_height((float) bb.max.z());
...
std::vector<Vec2f> GLCanvas3D::get_empty_cells(const Vec2f start_point, const Vec2f step)
{
    ...
    for (size_t i = 0; i < m_model->objects.size(); ++i) {
        ...
        Polygon        hull_2d         = model_object->convex_hull_2d(
            Geometry::assemble_transform({0.0, 0.0, model_instance0->get_offset().z()}, model_instance0->get_rotation(),
    ...
    std::sort(cells_ret.begin(), cells_ret.end(),
              [start](const Vec2f& cell1, const Vec2f& cell2) { return (cell1 - start).norm() < (cell2 - start).norm(); });
    return cells_ret;
}
...
void GLCanvas3D::update_sequential_clearance()
{
    ...
    if (m_sequential_print_clearance_first_displacement) {
        ...
        if (fff_print()->is_all_objects_are_short())
            ...
            shrink_factor = static_cast<float>(
                scale_(0.5 * fff_print()->config().extruder_clearance_radius.value + object_skirt_offset - 0.1));
        ...
        for (size_t i = 0; i < m_model->objects.size(); ++i) {
            ModelObject*   model_object    = m_model->objects[i];
            ModelInstance* model_instance0 = model_object->instances.front();
            Polygon        hull_no_offset  = model_object->convex_hull_2d(
                Geometry::assemble_transform({0.0, 0.0, model_instance0->get_offset().z()}, model_instance0->get_rotation(),
                                                     model_instance0->get_scaling_factor(), model_instance0->get_mirror()));
            auto    tmp     = offset(hull_no_offset,
                                     // Shrink the extruder_clearance_radius a tiny bit, so that if the object arrangement algorithm placed the
                                     // objects exactly by satisfying the extruder_clearance_radius, this test will not trigger collision.
                                     shrink_factor, jtRound, mitter_limit);
    ...
    std::sort(convex_and_bounding_boxes.begin(), convex_and_bounding_boxes.end(), [](auto& l, auto& r) {
...
bool GLCanvas3D::_render_orient_menu(float left, float right, float bottom, float top)
{
    ...
    // if (imgui->slider_float(_L("Overhang Angle"), &settings.overhang_angle, angle_min, 90.0f, "%5.2f") || angle_min >
    // settings.overhang_angle) {
...
bool GLCanvas3D::_render_arrange_menu(float left, float right, float bottom, float top)
{
    ...
    imgui->begin(_L("Arrange options"),
    ...
    bool b_Spacing = imgui->bbl_slider_float_style("##Spacing", &settings.distance, dist_min, 100.0f, "%5.2f") ||
    ...
    // Align to Y axis. Only enable this option when auto rotation not enabled
    {
        ...
        if (settings_out.enable_rotation == true) {
            imgui->disabled_end();
        }
    }
    ...
    if (imgui->button(_L("Reset"))) {
        ...
        // BBS: add specific arrange settings
        if (seq_print)
            settings_out.is_seq_print = true;

        if (auto printer_structure_opt =
                wxGetApp().preset_bundle->printers.get_edited_preset().config.option<ConfigOptionEnum<PrinterStructure>>(
                    "printer_structure")) {
    ...
    if (result.changed) {
        ...
        if (result.dragging) {
            ...
            case 22: // front
                ...
                camera.auto_type(Camera::EType::Ortho);
                break;
            default:
...
void GLCanvas3D::render_thumbnail_internal(ThumbnailData&            thumbnail_data,
                                           const ThumbnailsParams&   thumbnail_params,
                                           ...
                                           bool                      ban_light)
{
    ...
    if (thumbnail_params.use_plate_box) {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format(
                                       "render_thumbnail: plate_idx %1% volumes size %2%, shader %3%, use_top_view=%4%, for_picking=%5%") %
                                       plate_idx % visible_volumes.size() % shader % (int) camera_view_angle_type % for_picking;
    } else {
...
void GLCanvas3D::render_thumbnail_framebuffer(ThumbnailData&            thumbnail_data,
                                              unsigned int              w,
                                              unsigned int              h,
                                              const ThumbnailsParams&   thumbnail_params,
                                              PartPlateList&            partplate_list,
                                              ...
                                              bool                      ban_light)
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("render_thumbnail prepare: w %1%, h %2%, max_samples  %3%, render_fbo %4%") % w % h %
                                   max_samples % render_fbo;
    GLuint render_tex        = 0;
...
void GLCanvas3D::render_thumbnail_framebuffer_ext(ThumbnailData&            thumbnail_data,
                                                  unsigned int              w,
                                                  unsigned int              h,
                                                  const ThumbnailsParams&   thumbnail_params,
                                                  PartPlateList&            partplate_list,
...
void GLCanvas3D::render_thumbnail_legacy(ThumbnailData&            thumbnail_data,
                                         unsigned int              w,
                                         unsigned int              h,
                                         const ThumbnailsParams&   thumbnail_params,
                                         PartPlateList&            partplate_list,
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    item.left.action_callback = [this]() {
        if (m_canvas != nullptr) {
            ...
            if (agent)
                agent->track_update_property("auto_orient", std::to_string(++auto_orient_count));
        }
    };
    if (!m_main_toolbar.add_item(item))
        return false;
    ...
    item.tooltip = _utf8(L("Arrange all objects")) + " [A]\n" + _utf8(L("Arrange objects on selected plates")) + " [" + _utf8(L("Shift+")) +
                   "A]";
    item.sprite_id++;
...
{
    bool result = true;
    if (!m_sel_plate_toolbar.is_enabled() || m_sel_plate_toolbar.is_render_finish)
        return false;
...
bool GLCanvas3D::_init_assemble_view_toolbar()
{
    ...
    item.enabling_callback    = []() -> bool { return wxGetApp().plater()->has_assmeble_view(); };
...
bool GLCanvas3D::_init_collapse_toolbar() { return wxGetApp().plater()->init_collapse_toolbar(); }
...
void GLCanvas3D::_picking_pass()
{
    ...
    if (hit.is_valid()) {
        ...
        case SceneRaycaster::EType::FallbackGizmo: {
            const Size& cnv_size = get_canvas_size();
            const bool  inside   = 0 <= m_mouse.position.x() && m_mouse.position.x() < cnv_size.get_width() && 0 <= m_mouse.position.y() &&
                                m_mouse.position.y() < cnv_size.get_height();
    ...
    case SceneRaycaster::EType::Gizmo: {
        object_type = "Gizmo element";
        break;
    }
    case SceneRaycaster::EType::FallbackGizmo: {
        object_type = "Gizmo2 element";
        break;
    }
    case SceneRaycaster::EType::Volume: {
    ...
    auto add_strings_row_to_table = [&imgui](const std::string& col_1, const ImVec4& col_1_color, const std::string& col_2,
                                             const ImVec4& col_2_color, const std::string& col_3 = "",
                                             const ImVec4& col_3_color = ImGui::GetStyleColorVec4(ImGuiCol_Text)) {
        ImGui::TableNextRow();
    ...
    if (hit.type != SceneRaycaster::EType::None) {
        if (ImGui::BeginTable("Hit", 2)) {
            add_strings_row_to_table("Object ID", ImGuiWrapper::COL_ORANGE_LIGHT, std::to_string(hit.raycaster_id),
    ...
    if (ImGui::BeginTable("Raycasters", 2)) {
        ...
        add_strings_row_to_table("Gizmo elements", ImGuiWrapper::COL_ORANGE_LIGHT, std::string(buf),
                                 ImGui::GetStyleColorVec4(ImGuiCol_Text));
        sprintf(buf, "%d (%d)", (int) m_scene_raycaster.fallback_gizmos_count(), (int) m_scene_raycaster.active_fallback_gizmos_count());
        add_strings_row_to_table("Gizmo2 elements", ImGuiWrapper::COL_ORANGE_LIGHT, std::string(buf),
                                 ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }
        ...
        if (ImGui::BeginTable("GizmoRaycasters", 3)) {
            for (size_t i = 0; i < gizmo_raycasters->size(); ++i) {
                add_strings_row_to_table(std::to_string(i), ImGuiWrapper::COL_ORANGE_LIGHT,
                                         std::to_string(
                                             SceneRaycaster::decode_id(SceneRaycaster::EType::Gizmo, (*gizmo_raycasters)[i]->get_id())),
                                         ImGui::GetStyleColorVec4(ImGuiCol_Text),
                                         to_string(Geometry::Transformation((*gizmo_raycasters)[i]->get_transform()).get_offset()),
                                         ImGui::GetStyleColorVec4(ImGuiCol_Text));
            }
            ImGui::EndTable();
        }
    ...
    std::vector<std::shared_ptr<SceneRaycasterItem>>* gizmo2_raycasters = m_scene_raycaster.get_raycasters(
        SceneRaycaster::EType::FallbackGizmo);
    if (gizmo2_raycasters != nullptr && !gizmo2_raycasters->empty()) {
        ...
        if (ImGui::BeginTable("Gizmo2Raycasters", 3)) {
            for (size_t i = 0; i < gizmo2_raycasters->size(); ++i) {
                add_strings_row_to_table(std::to_string(i), ImGuiWrapper::COL_ORANGE_LIGHT,
                                         std::to_string(SceneRaycaster::decode_id(SceneRaycaster::EType::FallbackGizmo,
                                                                                  (*gizmo2_raycasters)[i]->get_id())),
                                         ImGui::GetStyleColorVec4(ImGuiCol_Text),
                                         to_string(Geometry::Transformation((*gizmo2_raycasters)[i]->get_transform()).get_offset()),
                                         ImGui::GetStyleColorVec4(ImGuiCol_Text));
            }
            ImGui::EndTable();
        }
...
void GLCanvas3D::_rectangular_selection_picking_pass()
{
    ...
    if (m_picking_enabled) {
        ...
        if (use_framebuffer) {
            ...
            framebuffer_camera.apply_projection(rect_near_left, rect_near_right, rect_near_bottom, rect_near_top, camera->get_near_z(),
                                                camera->get_far_z());
...
void GLCanvas3D::_render_background()
{
    ...
    if (wxGetApp().is_editor()) {
        use_error_color = m_dynamic_background_enabled && (current_printer_technology() != ptSLA || !m_volumes.empty());
...
void GLCanvas3D::_render_platelist(const Transform3d& view_matrix,
...
void GLCanvas3D::_render_objects(GLVolumeCollection::ERenderType type, bool with_outline)
{
    ...
    if (const BuildVolume& build_volume = m_bed.build_volume(); build_volume.valid()) {
        ...
        case BuildVolume_Type::Circle: {
            m_volumes.set_print_volume(
                {1, // Circle
                 {unscaled<float>(build_volume.circle().center.x()), unscaled<float>(build_volume.circle().center.y()),
        ...
        case BuildVolume_Type::Custom: {
            m_volumes.set_print_volume({static_cast<int>(type), {-FLT_MAX, -FLT_MAX, FLT_MAX, FLT_MAX}, {-FLT_MAX, FLT_MAX}});
        }
        }
    ...
    if (shader != nullptr) {
        ...
        case GLVolumeCollection::ERenderType::Opaque: {
            ...
            if (dynamic_cast<GLGizmoPainterBase*>(gm.get_current()) == nullptr) {
                if (m_picking_enabled && m_layers_editing.is_enabled() && (m_layers_editing.last_object_id != -1) &&
                    ...
                    const Camera& camera    = wxGetApp().plater()->get_camera();
                    m_volumes.render(type, false, camera.get_view_matrix(), camera.get_projection_matrix(), cvn_size,
                                     [object_id](const GLVolume& volume) {
                    ...
                    const Camera& camera = wxGetApp().plater()->get_camera();
                    m_volumes.render(
                        type, m_picking_enabled, camera.get_view_matrix(), camera.get_projection_matrix(), cvn_size,
        ...
        case GLVolumeCollection::ERenderType::Transparent: {
            ...
            // BBS:add assemble view related logic
            m_volumes.render(
                type, false, camera.get_view_matrix(), camera.get_projection_matrix(), cvn_size,
...
void GLCanvas3D::_render_gcode(int canvas_width, int canvas_height)
{
    ...
    if (layers_slider->is_dirty()) {
        ...
        if (m_gcode_viewer.has_data()) {
            m_gcode_viewer.set_layers_z_range(
                {static_cast<unsigned int>(layers_slider->GetLowerValue()), static_cast<unsigned int>(layers_slider->GetHigherValue())});
        }
    ...
    if (moves_slider->is_dirty()) {
        ...
        m_gcode_viewer.update_sequential_view_current((moves_slider->GetLowerValueD() - 1.0),
                                                      static_cast<unsigned int>(moves_slider->GetHigherValueD() - 1.0));
        post_event(SimpleEvent(EVT_GLCANVAS_UPDATE));
    }
}
...
void GLCanvas3D::_render_sequential_clearance()
{
    if (m_gizmos.is_dragging())
        return;
    auto type = m_gizmos.get_current_type();
    if (type == GLGizmosManager::EType::Undefined || can_sequential_clearance_show_in_gizmo()) {
...
void GLCanvas3D::_check_and_update_toolbar_icon_scale()
{
    ...
    float top_tb_width = m_main_toolbar.get_width() + m_gizmos.get_scaled_total_width() + m_assemble_view_toolbar.get_width() +
                         m_separator_toolbar.get_width() + collapse_toolbar_width * 2;
    int items_cnt = m_main_toolbar.get_visible_items_cnt() + m_gizmos.get_selectable_icons_cnt() +
                    m_assemble_view_toolbar.get_visible_items_cnt() + m_separator_toolbar.get_visible_items_cnt() +
                    collapse_toolbar.get_visible_items_cnt();
    ...
    // items_cnt = m_main_toolbar.get_visible_items_cnt() + m_gizmos.get_selectable_icons_cnt() + 3; // +3 means a place for top and view
    // toolbars and separators in gizmos toolbar

    // calculate scale needed for items in the gizmos toolbar
    items_cnt = m_main_toolbar.get_visible_items_cnt() + m_gizmos.get_selectable_icons_cnt() +
                m_assemble_view_toolbar.get_visible_items_cnt();
            ...
            if (ImGui::Button("Export")) {
                ...
                for (int i = 0; i < RenderCol_Count; i++) {
                    ...
                    if (!output_only_modified || memcmp(&col, &RenderColor::colors[i], sizeof(ImVec4)) != 0)
                        ImGui::LogText("RenderColor::colors[%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE, name,
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    float window_height_calc = (item_count * (button_height + (margin_size + button_margin) * 2.0f) +
                                (item_count - 1) * ImGui::GetStyle().ItemSpacing.y + ImGui::GetStyle().WindowPadding.y * 2.0f);
    ...
    float window_width       = button_width + (margin_size + button_margin + ImGui::GetStyle().WindowPadding.x) * 2 +
                         (show_scroll ? scrollbar_size : 0);
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, m_is_dark ? ImVec4(.9f, .9f, .9f, 1) :
                                                     ImVec4(.3f, .3f, .3f, 1)); // ORCA Plate number text > Add support for dark mode
    ImGui::PushStyleColor(ImGuiCol_WindowBg, window_bg);
    ImGui::PushStyleColor(ImGuiCol_ScrollbarBg,
                          ImVec4(0.f, 0.f, 0.f,
                                 0.f)); // ORCA using background color with opacity creates a second color. This prevents secondary color
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding,
                        show_scroll ? (4.0f * f_scale) : (button_radius + margin_size + frame_padding + ImGui::GetStyle().WindowPadding.x));
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, button_radius + margin_size);

    imgui.set_next_window_pos(canvas_w * 0 + 5.0f * f_scale, canvas_h * 0 + y_offset, ImGuiCond_Always, 0,
                              0); // ORCA Add slight gap on left edge so toolbar looks like floating and it creates separation with sidebar
    imgui.set_next_window_size(window_width, window_height, ImGuiCond_Always);

    if (show_scroll)
        imgui.begin(_L("Select Plate"),
                    ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse);
    else
        imgui.begin(_L("Select Plate"), ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove |
    ...
    if (m_sel_plate_toolbar.show_stats_item) {
        ...
        if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::UNSLICED ||
            all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICING) {
            btn_texture_id = (ImTextureID) (intptr_t) (m_process->running() ? all_plates_stats_item->image_slicing.get_id() :
                                                                              all_plates_stats_item->image_idle.get_id());
    ...
    for (int i = 0; i < m_sel_plate_toolbar.m_items.size(); i++) {
        ...
        ImVec2 text_start_pos = ImVec2(start_pos.x + 4.0f * f_scale,
                                       start_pos.y + 2.0f * f_scale); // ORCA move close to corner to prevent overlapping with preview
...
void GLCanvas3D::_render_return_toolbar() const
{
    ...
    if (ImGui::ImageTextButton(real_size, _utf8(L("Return")).c_str(), m_return_toolbar.get_return_texture_id(), button_icon_size, uv0, uv1,
                               -1, bg_col, tint_col, margin)) {
        if (m_canvas != nullptr)
...
void GLCanvas3D::_render_canvas_toolbar()
{
    ...
    Vec2i32 pos      = {m_canvas_toolbar_pos[0] + margin.x, get_canvas_size().get_height() - margin.y};
    ...
    imgui.begin(_L("Canvas Toolbar"), ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoScrollbar |
                                          ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse); //

    ImTextureID m_normal_id = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MENU_DARK :
                                                                       GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MENU);
    ImTextureID m_hover_id  = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MENU_DARK_HOVER :
                                                                       GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_MENU_HOVER);

    if (ImGui::ImageButton3(m_normal_id, m_hover_id, btn_size)) {
        if (!ImGui::IsPopupOpen("CanvasToolbarMenu")) {
            ImGui::SetNextWindowPos(ImVec2(pos[0] + padding.x, pos[1] - padding.y - (zoom_btn ? (btn_size.y + spacing.y) : 0.f)),
                                    ImGuiCond_Always, ImVec2(0, 1)); // pivot bottom-left
    ...
    if (zoom_btn) {
        ...
        ImTextureID z_normal_id = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_ZOOM_DARK :
                                                                           GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_ZOOM);
        ImTextureID z_hover_id  = m_gizmos.get_icon_texture_id(m_is_dark ? GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_ZOOM_DARK_HOVER :
                                                                           GLGizmosManager::MENU_ICON_NAME::IC_CANVAS_ZOOM_HOVER);
    ...
    if (ImGui::BeginPopup("CanvasToolbarMenu")) {
        ...
        auto create_menu_item = [this, sc](const std::string& name, bool enable, bool condition, const std::function<void()>& action) {
            ...
            ImGui::TextColored(enable ? ImVec4(1, 1, 1, 1) : ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled), "%s",
                               into_u8(condition ? ImGui::VisibleIcon : ImGui::HiddenIcon).c_str());
        };

        create_menu_item(_utf8(L("3D Navigator")),
                         m_canvas_type != ECanvasType::CanvasAssembleView, // not work on assembly
                         wxGetApp().show_3d_navigator(), [this] {
        ...
        create_menu_item(_utf8(L("Zoom button")),
                         true, // work on all
                         wxGetApp().show_canvas_zoom_button(), [this] {
        ...
        create_menu_item(_utf8(L("Overhangs")),
                         ...
                         p->is_view3D_overhang_shown(), [this, p] { p->show_view3D_overhang(!p->is_view3D_overhang_shown()); });

        create_menu_item(_utf8(L("Outline")),
                         m_canvas_type != ECanvasType::CanvasPreview, // not work on preview
                         wxGetApp().show_outline(), [this] { wxGetApp().toggle_show_outline(); });

        ImGui::Separator();

        create_menu_item(_utf8(L("Perspective")),
                         true, // work on all
                         cfg->get_bool("use_perspective_camera"), [this, &cfg] {
        ...
        create_menu_item(_utf8(L("Axes")),
                         m_canvas_type != ECanvasType::CanvasAssembleView, // not work on assembly
                         m_show_world_axes, [this] { toggle_world_axes_visibility(false); });

        create_menu_item(_utf8(L("Gridlines")),
                         m_canvas_type != ECanvasType::CanvasAssembleView, // not work on assembly
                         wxGetApp().show_plate_gridlines(), [this] { wxGetApp().toggle_show_plate_gridlines(); });

        ImGui::Separator();

        create_menu_item(_utf8(L("Labels")),
                         m_canvas_type == ECanvasType::CanvasView3D, // work only on prepare
                         p->are_view3D_labels_shown(), [this, p] { p->show_view3D_labels(!p->are_view3D_labels_shown()); });
...
void GLCanvas3D::_render_paint_toolbar() const
{
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg,
                          m_is_dark ? ImGuiWrapper::COL_TOOLBAR_BG_DARK : ImGuiWrapper::COL_TOOLBAR_BG); // ORCA Toolbar color
    ...
    imgui.begin(_L("Paint Toolbar"), ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar |
    ...
    const ImRect  right_arrow_button       = ImRect(ImGui::GetCurrentWindow()->Pos + ImGui::GetWindowSize() - arrow_button_size,
    ...
    for (int i = 0; i < extruder_num; i++) {
        ...
        if (ImGui::Button(("##filament_button" + std::to_string(i)).c_str(), button_size)) {
            if (!ImGui::IsMouseHoveringRect(left_arrow_button.Min, left_arrow_button.Max) &&
                !ImGui::IsMouseHoveringRect(right_arrow_button.Min, right_arrow_button.Max))
    ...
    if (ImGui::GetWindowWidth() == constraint_window_width) {
        ...
        ImGui::BBLRenderArrow(draw_list,
                              left_arrow_button.GetCenter() - ImVec2(draw_list->_Data->FontSize, draw_list->_Data->FontSize) * 0.5f,
                              arrow_color, ImGuiDir_Left, 2.0f);
        ...
        ImGui::BBLRenderArrow(draw_list,
                              right_arrow_button.GetCenter() - ImVec2(draw_list->_Data->FontSize, draw_list->_Data->FontSize) * 0.5f,
                              arrow_color, ImGuiDir_Right, 2.0f);
    }
...
void GLCanvas3D::_render_assemble_control()
{
    ...
    const float text_size_x    = std::max(imgui->calc_text_size(_L("Reset direction")).x + 2 * ImGui::GetStyle().FramePadding.x,
                                          std::max(imgui->calc_text_size(_L("Explosion Ratio")).x,
                                                   imgui->calc_text_size(_L("Section View")).x));
    ...
    imgui->begin(_L("Assemble Control"),
                 ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar);
    ...
    float same_line_width = tip_icon_size;
    {
        ...
        if (clp_dist == 0.f) {
            ...
            if (imgui->button(_L("Reset direction"))) {
                wxGetApp().CallAfter([this]() { m_gizmos.m_assemble_view_data->model_objects_clipper()->set_position(-1., false); });
            }
        }
...
void GLCanvas3D::_render_assemble_info() const
{
    ...
    imgui->begin(_L("Assembly Info"),
                 ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
    ...
    for (int i = 0; i < 3; ++i) {
        if (!m_camera_target.axis[i].is_initialized() || target_changed) {
            ...
            init_data.format = {GLModel::Geometry::EPrimitiveType::Lines, GLModel::Geometry::EVertexLayout::P3,
                                GLModel::Geometry::EIndexType::USHORT};
...
void GLCanvas3D::_render_sla_slices()
{
    ...
    for (unsigned int i = 0; i < (unsigned int) print_objects.size(); ++i) {
        ...
        if ((!bottom_obj_triangles.is_initialized() || !bottom_sup_triangles.is_initialized() || !top_obj_triangles.is_initialized() ||
             !top_sup_triangles.is_initialized()) &&
        ...
        if (shader != nullptr) {
            ...
            for (const SLAPrintObject::Instance& inst : obj->instances()) {
                ...
                const Transform3d view_model_matrix = camera.get_view_matrix() *
                                                      Geometry::assemble_transform(Vec3d(unscale<double>(inst.shift.x()),
                                                                                         unscale<double>(inst.shift.y()), 0.0),
                                                                                   inst.rotation * Vec3d::UnitZ(), Vec3d::Ones(),
                                                                                   obj->is_left_handed() ? Vec3d(-1.0f, 1.0f, 1.0f) :
                                                                                                           Vec3d::Ones());
...
void GLCanvas3D::_update_volumes_hover_state()
{
    ...
    for (int i : m_hover_volume_idxs) {
        ...
        // (volume->is_modifier && !selection_modifiers_only && !is_ctrl_pressed) -> allows hovering on selected modifiers belonging to
        // selection of type Instance
        bool select = (!volume.selected || (volume.is_modifier && !selection_modifiers_only && !ctrl_pressed)) && !alt_pressed;

        if (select || deselect) {
            bool as_volume = volume.is_modifier && hover_from_single_instance && !ctrl_pressed &&
                             ((!deselect) || (deselect && !m_selection.is_single_full_instance() &&
                                              (volume.object_idx() == m_selection.get_object_idx()) &&
                                              (volume.instance_idx() == m_selection.get_instance_idx())));
...
void GLCanvas3D::_perform_layer_editing_action(wxMouseEvent* evt)
{
    ...
    if (evt != nullptr) {
        ...
        m_layers_editing.last_action = evt->ShiftDown() ?
                                           (evt->RightIsDown() ? LAYER_HEIGHT_EDIT_ACTION_SMOOTH : LAYER_HEIGHT_EDIT_ACTION_REDUCE) :
...
Vec3d GLCanvas3D::_mouse_to_3d(const Point& mouse_pos, float* z)
{
    ...
    if (z == nullptr) {
        ...
        Vec3d         out;
        igl::unproject(Vec3d(mouse_pos.x(), viewport[3] - mouse_pos.y(), *z), camera.get_view_matrix().matrix(),
                       camera.get_projection_matrix().matrix(), viewport, out);
        return out;
    }
}

Vec3d GLCanvas3D::_mouse_to_bed_3d(const Point& mouse_pos) { return mouse_ray(mouse_pos).intersect_plane(0.0); }
...
void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
{
    ...
    enum ErrorType { PLATER_WARNING, PLATER_ERROR, SLICING_SERIOUS_WARNING, SLICING_ERROR, SLICING_LIMIT_ERROR, SLICING_HEIGHT_OUTSIDE };
    const static std::vector<std::string> extruder_name_list = {_u8L("left nozzle"),
                                                                _u8L("right nozzle")}; // in ui, we treat extruder as nozzle
    std::string                           text;
    ...
    case EWarning::GCodeConflict: {
        ...
        text = (boost::format(_u8L("Conflicts of G-code paths have been found at layer %d, Z = %.2lfmm. Please separate the conflicted "
                                   "objects farther (%s <-> %s).")) %
    ...
    case EWarning::ToolHeightOutside:
        text  = _u8L("A G-code path goes beyond the max print height.");
    ...
    case EWarning::MultiExtruderPrintableError: {
        for (auto error_iter = m_gcode_viewer.m_gcode_check_result.print_area_error_infos.begin();
             error_iter != m_gcode_viewer.m_gcode_check_result.print_area_error_infos.end(); ++error_iter) {
            ...
            if (error_iter->second.size() == 1) {
                text += (boost::format(_u8L(
                             "Filament %s is placed in the %s, but the generated G-code path exceeds the printable range of the %s.")) %
                         filaments % extruder_name % extruder_name)
                            .str();
            } else {
                text += (boost::format(_u8L(
                             "Filaments %s are placed in the %s, but the generated G-code path exceeds the printable range of the %s.")) %
                         filaments % extruder_name % extruder_name)
                            .str();
            }
        }
    ...
    case EWarning::MultiExtruderHeightOutside: {
        for (auto error_iter = m_gcode_viewer.m_gcode_check_result.print_height_error_infos.begin();
             error_iter != m_gcode_viewer.m_gcode_check_result.print_height_error_infos.end(); ++error_iter) {
            ...
            if (error_iter->second.size() == 1) {
                ...
                text += (boost::format(_u8L(
                             "Filaments %s are placed in the %s, but the generated G-code path exceeds the printable height of the %s.")) %
                         filaments % extruder_name % extruder_name)
                            .str();
            }
        }
    ...
    case EWarning::SlaSupportsOutside:
        text  = ("SLA supports outside the print area were detected.");
        error = ErrorType::PLATER_ERROR;
        break;
    case EWarning::SomethingNotShown: text = _u8L("Only the object being edited is visible."); break;
    case EWarning::ObjectClashed: error = ErrorType::PLATER_ERROR; break;
    case EWarning::ObjectLimited: text = get_object_limited_text(); break;
    ...
    case EWarning::MixUsePLAAndPETG:
        ...
        break;
    case EWarning::PrimeTowerOutside: text = _u8L("The prime tower extends beyond the plate boundary."); break;
    ...
    case EWarning::FlushingVolumeZero:
        text = _u8L(
            "Partial flushing volume set to 0. Multi-color printing may cause color mixing in models. Please readjust flushing settings.");
        error = ErrorType::SLICING_ERROR;
        break;
    }
        ...
        if (warning == EWarning::MixUsePLAAndPETG) {
            if (state) {
                notification_manager.push_slicing_customize_error_notification(
                    NotificationType::BBLMixUsePLAAndPETG, NotificationLevel::WarningNotificationLevel, text, _u8L("Click Wiki for help."),
                    [](wxEvtHandler*) {
                        ...
                        wxGetApp().open_browser_with_warning_dialog(
                            wxString::Format(L"https://wiki.bambulab.com/%s/filament-acc/filament/h2d-pla-and-petg-mutual-support", region));
                        return false;
                    });
            } else
                notification_manager.close_slicing_customize_error_notification(NotificationType::BBLMixUsePLAAndPETG,
                                                                                NotificationLevel::WarningNotificationLevel);
        } else if (warning == EWarning::NozzleFilamentIncompatible) {
            if (state) {
                notification_manager.push_slicing_customize_error_notification(NotificationType::BBLNozzleFilamentIncompatible,
                                                                               NotificationLevel::WarningNotificationLevel, text);
            } else {
                notification_manager.close_slicing_customize_error_notification(NotificationType::BBLNozzleFilamentIncompatible,
                                                                                NotificationLevel::WarningNotificationLevel);
            }
        } else {
            if (state)
        ...
        if (warning == EWarning::LeftExtruderPrintableError) {
            if (state) {
                if (auto left_str = get_left_extruder_unprintable_text(); !left_str.empty())
                    notification_manager
                        .bbl_show_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError, left_str);
            } else {
                notification_manager.bbl_close_filament_map_invalid_notification_before_slice(
            ...
            if (state) {
                if (auto right_str = get_right_extruder_unprintable_text(); !right_str.empty())
                    notification_manager
                        .bbl_show_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError, right_str);
            } else {
                notification_manager.bbl_close_filament_map_invalid_notification_before_slice(
    ...
    case SLICING_SERIOUS_WARNING:
        if (state)
            notification_manager.push_slicing_serious_warning_notification(text, conflictObj ?
                                                                                     std::vector<ModelObject const*>{conflictObj} :
                                                                                     std::vector<ModelObject const*>{});
        else
    ...
    case SLICING_ERROR:
        ...
        if (warning == EWarning::FilamentPrintableError) {
            if (state) {
                ...
                notification_manager.push_slicing_customize_error_notification(NotificationType::BBLFilamentPrintableError,
                                                                               NotificationLevel::ErrorNotificationLevel, text,
                                                                               _u8L("Click here to regroup"), callback);
            } else
                notification_manager.close_slicing_customize_error_notification(NotificationType::BBLFilamentPrintableError,
            ...
            if (state) {
                ...
                notification_manager.push_flushing_volume_error_notification(NotificationType::BBLFlushingVolumeZero,
                                                                             NotificationLevel::WarningNotificationLevel, text,
                                                                             _u8L("Flushing Volume"), callback);
            } else
                notification_manager.close_flushing_volume_error_notification(NotificationType::BBLFlushingVolumeZero,
                                                                              NotificationLevel::WarningNotificationLevel);
        } else {
            if (state)
                notification_manager.push_slicing_error_notification(text, conflictObj ? std::vector<ModelObject const*>{conflictObj} :
                                                                                         std::vector<ModelObject const*>{});
            else
    ...
    case SLICING_HEIGHT_OUTSIDE:
        if (state)
            notification_manager.push_slicing_customize_error_notification(NotificationType::BBLSliceMultiExtruderHeightOutside,
                                                                           NotificationLevel::ErrorNotificationLevel, text);
        else
            notification_manager.close_slicing_customize_error_notification(NotificationType::BBLSliceMultiExtruderHeightOutside,
                                                                            NotificationLevel::ErrorNotificationLevel);
        break;
    default: break;
    }
...
bool GLCanvas3D::is_flushing_matrix_error()
{
    ...
    for (int i = 0; i < config_matrix.size(); i++) {
        ...
        if (row_id != col_id && config_matrix[i] == 0)
            return true;
    }
    return false;
}
...
const Print* GLCanvas3D::fff_print() const { return (m_process == nullptr) ? nullptr : m_process->fff_print(); }

const SLAPrint* GLCanvas3D::sla_print() const { return (m_process == nullptr) ? nullptr : m_process->sla_print(); }
...
void GLCanvas3D::RenderTimer::Notify() { wxPostEvent((wxEvtHandler*) GetOwner(), RenderTimerEvent(EVT_GLCANVAS_RENDER_TIMER, *this)); }
