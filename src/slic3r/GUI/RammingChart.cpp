...
// [EVENT][UNITY] Chart changes are surfaced as a parent-window command event; the Unity port should raise a
// controller event after recomputing the curve so the wipe-tower workflow can refresh dependent summaries.

// [INTENT] Immediate-mode paint routine for the editable ramming chart: it draws the chart frame, sampled curve,
// draggable control points, and axis labels from the current math-space state.
// [UNITY] This maps best to a custom chart control (UI Toolkit or IMGUI) with retained math data and pointer-driven
// interaction; the chart should not be decomposed into standard form widgets.
void Chart::draw()
{
    ...
    // [STATE] The chart frame is derived from current theme colors plus the cached plot rectangle; the port should
    // keep this as a single style source rather than scattering hard-coded sizes across the view.
    ...
    if (visible_area.m_width < 0.499) {
        ...
        dc.DrawText(_(L("NO RAMMING AT ALL")),
                    wxPoint(m_rect.GetLeft() + m_rect.GetWidth() / 2 - legend_side, m_rect.GetBottom() - m_rect.GetHeight() / 2));
        return;
    }

    // [INTENT] Render the sampled polyline and the per-column gradient backdrop from the cached rasterized curve.
    // [PORTING_HAZARD:P2] The view mixes model sampling with pixel-space drawing, so a Unity port should separate
    // spline evaluation from rendering resolution before mapping to a mesh or polyline renderer.
    if (!m_line_to_draw.empty()) {
        for (unsigned int i = 0; i < m_line_to_draw.size() - 2; ++i) {
            ...
            dc.SetPen(wxPen(wxColor(std::min(222, color), 222 - std::max(color - 222, 0), 60),
                            1)); // adding blue color sligtly gives a bit more modern look instead using raw red & green
    ...
    for (auto& button : m_buttons) {
        ...
        label_x     = std::clamp(label_x, m_rect.GetLeft() + (padding * 2),
                                 m_rect.GetRight() - label_width - (padding * 2)); // adjust to fit within chart bounds
...
// [EVENT][STATE][UNITY] Right-click editing removes a control point only when manual manipulation is enabled;
// in Unity this becomes a context-menu or gesture handler on the same chart controller, mutating the curve model
// before recomputing the sampled preview.
...
// [EVENT][STATE] Left-click begins a drag on the hit-tested control point and records the mouse position so drag
// deltas can be applied in screen space.
...
// [EVENT][STATE][THREAD] Mouse-move is the live drag loop: it changes the cursor, clamps motion to the chart,
// updates one or all control points, and then recomputes the sampled line synchronously on the UI thread.
// [UNITY] This should become pointer-drag handling in a single MonoBehaviour/controller; keep the drag state in the
// component rather than in transient event objects.
...
// [EVENT][STATE] Double-clicking in the chart adds a new control point at the clicked math position, sorts the point
// set by X, and rebuilds the sampled curve immediately.
...
// [INTENT] Rebuild the chart's derived data from the current control points: rasterized line samples, total volume,
// and the notification sent to the owning dialog.
// [STATE] This mutates both the polyline cache and the summary volume used elsewhere in the ramming workflow.
// [UNITY] The equivalent in Unity is a curve-model rebuild plus a separate sampled preview cache for rendering and
// export, ideally backed by a serialized settings/model object.
void Chart::recalculate_line()
{
    ...
    if (points.size() > 1) {
        ...
        for (int x = m_rect.GetLeft(); x <= m_rect.GetRight(); ++x) {
            ...
            m_total_volume += (m_rect.GetBottom() - m_line_to_draw.back()) * (visible_area.m_width / m_rect.GetWidth()) *
                              (visible_area.m_height / m_rect.GetHeight());
        }
    }
...
// [INTENT] Export evenly sampled ramming-speed values for downstream consumers that need the time-series rather than
// the on-screen polyline.
// [STATE] Sampling density depends on the current visible width, so the export is coupled to the chart's zoom range.
...
// [INTENT] Return the curve control points in math space for persistence or higher-level synchronization.
// [UNITY] A Unity port should expose the same data through the controller/model layer instead of reading view state
// directly.
...
// [EVENT] Legacy wx event table wiring binds pointer and paint callbacks into the chart widget; Unity should replace
// this with a single component-level pointer/paint pipeline rather than callback macros.
