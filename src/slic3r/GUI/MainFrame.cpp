...
// [EVENT] Global application events for tab selection, networking, and login state.
// [UNITY] Use C# delegates or UnityEvents in a global AppState or ServiceProvider.
...
#ifdef __WXGTK__
// [INTENT] Intercepts mouse events globally to provide resize handles for the borderless window on Linux/GTK.
// [UNITY] NO_DIRECT_EQUIVALENT — Unity's StandaloneWindow handles this or use a custom borderless window plugin.
// [PORTING_HAZARD:P3] GTK-specific low-level event filtering.
class MainFrame::GtkResizeBorderHandler : public wxEventFilter
{
public:
    static constexpr int BORDER_PX = 8;

    explicit GtkResizeBorderHandler(MainFrame* frame) : m_frame(frame) { wxEvtHandler::AddFilter(this); }

    ~GtkResizeBorderHandler() override { wxEvtHandler::RemoveFilter(this); }
    ...
    int FilterEvent(wxEvent& event) override
    {
        ...
        if (t == wxEVT_LEFT_DOWN) {
            gtk_window_begin_resize_drag(GTK_WINDOW(m_frame->m_widget), edge,
                                         1, // left mouse button
                                         mouse.x, mouse.y, gtk_get_current_event_time());
...
static wxIcon main_frame_icon(GUI_App::EAppMode app_mode)
{
    ...
    if (len > 0 && len < MAX_PATH) {
        ...
        }) if (boost::iends_with(path, exe_name)) { path.erase(path.end() - exe_name.size(), path.end()); path += L"prusa-gcodeviewer.exe";
                    break;
                }
        }*/
    }
...
// [INTENT] Constructor for the primary application frame.
// [UNITY] Corresponds to the entry point for the MainUIController (MonoBehaviour.Start/Awake).
MainFrame::MainFrame()
    ...
    , diff_dialog(this)
{
    ...
    */
    ...
    case GUI_App::EAppMode::Editor:
        ...
        break;
    case GUI_App::EAppMode::GCodeViewer: break;
    }
#endif // __APPLE__
    ...
    // [INTENT] Initialize the main notebook-style tab interface and menu structures.
    // [UNITY] Use Unity's UI Toolkit (VisualElements) or a Prefab-based Panel system to initialize tabs.
    init_tabpanel();
    ...
    // [EVENT] Global keyboard shortcuts and window resize event handling.
    // [UNITY] Use Unity's Input System and the Resize event of the Screen or Canvas.
    ...
    Bind(wxEVT_CLOSE_WINDOW, [this](wxCloseEvent& event) {
        ...
        // BBS:
        // if (event.CanVeto() && !wxGetApp().check_and_save_current_preset_changes(_L("Application is closing"), _L("Closing Application
        // while some presets are modified."))) {
        //     event.Veto();
        //     return;
        // }
        auto check = [](bool yes_or_no) {
            if (yes_or_no)
                return true;
            return wxGetApp().check_and_save_current_preset_changes(_L("Application is closing"),
...
// FIXME maybe this is useful for __WXGTK3__ as well?
    ...
    if (m_plater != nullptr) {
        ...
        Bind(EVT_BACKUP_POST, [](wxCommandEvent& e) { Slic3r::run_backup_ui_tasks(); });
        ;
    }
    this->Bind(wxEVT_CHAR_HOOK, [this](wxKeyEvent& evt) {
        ...
        if (evt.CmdDown() && evt.GetKeyCode() == 'Q') {
        ...
        if (evt.CmdDown() && evt.GetKeyCode() == 'F') {
            if (m_plater &&
                (m_tabpanel->GetSelection() == TabPosition::tp3DEditor || m_tabpanel->GetSelection() == TabPosition::tpPreview)) {
...
void MainFrame::bind_diff_dialog()
{
    ...
    auto transfer = [this, get_tab](Preset::Type type) {
        get_tab(type)->transfer_options(diff_dialog.get_left_preset_name(type), diff_dialog.get_right_preset_name(type),
...
// [INTENT] Shutdown process for the main frame, ensuring all background threads and jobs are cancelled.
// [UNITY] Use OnDestroy() or OnDisable() to clean up MonoBehaviours, cancel C# Tasks/Jobs, and save persistent state.
...
void MainFrame::update_title() { return; }
...
// [INTENT] Initialize the main notebook/tabbed interface.
// [UNITY] Use a TabController component managing a list of UI Panel Prefabs or VisualElements.
void MainFrame::init_tabpanel()
{
    ...
    m_tabpanel->AddPage(m_calibration, _L("Calibration"), std::string("tab_calibration_active"), std::string("tab_calibration_active"),
                        false);
...
bool MainFrame::preview_only_hint()
{
    if (m_plater && (m_plater->only_gcode_mode() || (m_plater->using_exported_file()))) {
        BOOST_LOG_TRIVIAL(info) << boost::format("skipped tab switch from %1% to %2% in preview mode") % m_tabpanel->GetSelection() %
                                       tp3DEditor;

        ConfirmBeforeSendDialog confirm_dlg(this, wxID_ANY, _L("Warning"));
        confirm_dlg.Bind(EVT_SECONDARY_CHECK_CONFIRM, [this](wxCommandEvent& e) { preview_only_to_editor = true; });
...
bool MainFrame::can_open_project() const { return (m_plater && !m_plater->is_background_process_slicing()); }
...
{
    return (m_plater != nullptr) && !m_plater->get_view3D_canvas3D()->get_gizmos_manager().is_in_editing_mode(false) &&
...
{
    return (m_plater != nullptr) && !m_plater->get_view3D_canvas3D()->get_gizmos_manager().is_in_editing_mode(false) &&
           !m_plater->using_exported_file() && !m_plater->only_gcode_mode();
}

void MainFrame::save_project() { save_project_as(m_plater->get_project_filename(".3mf")); }
...
bool MainFrame::can_upload() const { return true; }

bool MainFrame::can_export_model() const { return (m_plater != nullptr) && !m_plater->model().objects.empty(); }

bool MainFrame::can_export_toolpaths() const
{
    return (m_plater != nullptr) && (m_plater->printer_technology() == ptFFF) && m_plater->is_preview_shown() &&
           m_plater->is_preview_loaded() && m_plater->has_toolpaths_to_export();
}
...
bool MainFrame::can_clone() const { return can_select() && !m_plater->is_selection_empty(); }
...
wxBoxSizer* MainFrame::create_side_tools()
{
    ...
    slice_panel->Bind(wxEVT_LEAVE_WINDOW, [this](auto& event) { m_filament_group_popup->tryClose(); });
    ...
    m_slice_btn->Bind(wxEVT_LEAVE_WINDOW, [this](auto& event) { m_filament_group_popup->tryClose(); });
#endif
...
void MainFrame::update_slice_print_status(SlicePrintEventType event, bool can_slice, bool can_print)
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(" m_slice_select %1%: can_slice= %2%, can_print %3%, enable_slice %4%, enable_print %5% ") %
                                   m_slice_select % can_slice % can_print % enable_slice % enable_print;
...
static wxMenu* generate_help_menu()
{
    ...
    append_menu_item(helpMenu, wxID_ANY, about_title, about_title, [](wxCommandEvent&) { Slic3r::GUI::about(); });
#endif
...
{
#ifndef __WINDOWS__
    append_menu_item(publish_menu, wxID_ANY, _L("Upload Models"), _L("Upload Models"), [](wxCommandEvent&) {
    ...
    append_menu_item(publish_menu, wxID_ANY, _L("Download Models"), _L("Download Models"), [](wxCommandEvent&) {
        if (!wxGetApp().getAgent()) {
            BOOST_LOG_TRIVIAL(info) << "publish: no agent";
            return;
        }
...
static void add_common_view_menu_items(wxMenu* view_menu, MainFrame* mainFrame, std::function<bool(void)> can_change_view)
{
    ...
    append_menu_item(
        view_menu, wxID_ANY, _L("Default View") + "\t" + ctrl + "0", _L("Default View"),
    ...
    append_menu_item(
        view_menu, wxID_ANY, _L("Top") + "\t" + ctrl + "1", _L("Top View"), [mainFrame](wxCommandEvent&) { mainFrame->select_view("top"); },
        "", nullptr, [can_change_view]() { return can_change_view(); }, mainFrame);
    // TRN To be shown in the main menu View->Bottom
    append_menu_item(
        view_menu, wxID_ANY, _L("Bottom") + "\t" + ctrl + "2", _L("Bottom View"),
        [mainFrame](wxCommandEvent&) { mainFrame->select_view("bottom"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Front") + "\t" + ctrl + "3", _L("Front View"),
        [mainFrame](wxCommandEvent&) { mainFrame->select_view("front"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Rear") + "\t" + ctrl + "4", _L("Rear View"),
        [mainFrame](wxCommandEvent&) { mainFrame->select_view("rear"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _CTX(L_CONTEXT("Left", "Camera"), "Camera") + "\t" + ctrl + "5", _L("Left View"),
        [mainFrame](wxCommandEvent&) { mainFrame->select_view("left"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _CTX(L_CONTEXT("Right", "Camera"), "Camera") + "\t" + ctrl + "6", _L("Right View"),
        [mainFrame](wxCommandEvent&) { mainFrame->select_view("right"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
}
...
void MainFrame::init_menubar_as_editor()
{
    ...
    wxMenu* fileMenu = new wxMenu;
    {
        ...
        // New Window
        append_menu_item(
            fileMenu, wxID_ANY, _L("New Window"), _L("Start a new window"), [](wxCommandEvent&) { start_new_slicer(); }, "", nullptr,
            [this] { return m_plater != nullptr && wxGetApp().app_config->get("app", "single_instance") == "false"; }, this);
#endif
        // New Project
        append_menu_item(
            fileMenu, wxID_ANY, _L("New Project") + "\t" + ctrl + "N", _L("Start a new project"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->new_project();
            },
        ...
        Bind(
            wxEVT_UPDATE_UI, [this](wxUpdateUIEvent& evt) { evt.Enable(can_open_project() && (m_recent_projects.GetCount() > 0)); },
...
#ifndef __APPLE__
        append_menu_item(
            fileMenu, wxID_ANY, _L("Save Project as") + dots + "\t" + ctrl + shift + "S", _L("Save current project as"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->save_project(true);
            },
...
#endif
        append_menu_item(
            import_menu, wxID_ANY, _L("Import Zip Archive") + dots, _L("Load models contained within a zip archive"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->import_zip_archive();
            },
            "menu_import", nullptr, [this]() { return can_add_models(); });
        append_menu_item(
        ...
        // BBS export as STL
        append_menu_item(
            export_menu, wxID_ANY, _L("Export all objects as one STL") + dots, _L("Export all objects as one STL"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->export_stl();
            },
        ...
        append_menu_item(
            ...
            "menu_export_stl", nullptr, [this]() { return can_export_model(); }, this);
        append_menu_item(
            export_menu, wxID_ANY, _L("Export all objects as one DRC") + dots, _L("Export all objects as one DRC"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->export_stl(false, false, false, FT_DRC);
            },
            "menu_export_stl", nullptr, [this]() { return can_export_model(); }, this);
        append_menu_item(
            export_menu, wxID_ANY, _L("Export all objects as DRCs") + dots, _L("Export all objects as DRCs"),
            [this](wxCommandEvent&) {
                if (m_plater)
                    m_plater->export_stl(false, false, true, FT_DRC);
            },
            "menu_export_stl", nullptr, [this]() { return can_export_model(); }, this);
        append_menu_item(
        ...
        append_menu_item(
            export_menu, wxID_ANY, _L("Export toolpaths as OBJ") + dots, _L("Export toolpaths as OBJ"),
            [this](wxCommandEvent&) {
                if (m_plater != nullptr)
                    m_plater->export_toolpaths_to_obj();
            },
            "menu_export_toolpaths", nullptr, [this]() { return can_export_toolpaths(); }, this);
...
#ifndef __APPLE__
        append_menu_item(
            fileMenu, wxID_EXIT, _L("Quit"), wxString::Format(_L("Quit")), [this](wxCommandEvent&) { Close(false); }, "menu_exit", nullptr);
#else
        append_menu_item(
            fileMenu, wxID_EXIT, _L("Quit"), wxString::Format(_L("Quit")), [this](wxCommandEvent&) { Close(false); }, "", nullptr);
#endif
    }
    ...
    if (m_plater != nullptr) {
        ...
        editMenu->AppendSeparator();
        append_menu_item(
            editMenu, wxID_ANY, _L("Duplicate Current Plate"), _L("Duplicate the current plate"),
            [this](wxCommandEvent&) { m_plater->duplicate_plate(); }, "menu_remove", nullptr, [this]() { return true; }, this);
        editMenu->AppendSeparator();
#else
        // BBS undo
        append_menu_item(
            editMenu, wxID_ANY, _L("Undo") + sep + ctrl_t + "Z", _L("Undo"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
        ...
        // BBS redo
        append_menu_item(
            editMenu, wxID_ANY, _L("Redo") + sep + ctrl_t + "Y", _L("Redo"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        // BBS Cut TODO
        append_menu_item(
            editMenu, wxID_ANY, _L("Cut") + sep + ctrl_t + "X", _L("Cut selection to clipboard"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        // BBS Copy
        append_menu_item(
            editMenu, wxID_ANY, _L("Copy") + sep + ctrl_t + "C", _L("Copy selection to clipboard"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        // BBS Paste
        append_menu_item(
            editMenu, wxID_ANY, _L("Paste") + sep + ctrl_t + "V", _L("Paste clipboard"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        // BBS Clone Selected
        append_menu_item(
            editMenu, wxID_ANY, _L("Clone selected") + "\t" + ctrl + "K", _L("Clone copies of selections"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        append_menu_item(
            editMenu, wxID_ANY, _L("Duplicate Current Plate"), _L("Duplicate the current plate"),
            [this, handle_key_event](wxCommandEvent&) { m_plater->duplicate_plate(); }, "", nullptr, [this]() { return true; }, this);
        editMenu->AppendSeparator();
        ...
        // BBS Select All
        append_menu_item(
            editMenu, wxID_ANY, _L("Select all") + sep + ctrl_t + "A", _L("Selects all objects"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
                e.SetControlDown(true);
        ...
        // BBS Deslect All
        append_menu_item(
            editMenu, wxID_ANY, _L("Deselect all") + sep + _L("Esc"), _L("Deselects all objects"),
            [this, handle_key_event](wxCommandEvent&) {
                wxKeyEvent e;
                e.SetEventType(wxEVT_KEY_DOWN);
        ...
        //     }, nullptr, [this]() {return can_select(); }, [this]() { return wxGetApp().app_config->get("show_model_mesh").compare("true")
        //     == 0; }, this);
        ...
        //     }, nullptr, [this]() {return can_select(); }, [this]() { return
        //     wxGetApp().app_config->get("show_model_shadow").compare("true") == 0; }, this);
        // editMenu->AppendSeparator();
        ...
        //     }, nullptr, [this]() {return can_select(); }, [this]() { return
        //     wxGetApp().app_config->get("show_printable_box").compare("true") == 0; }, this);
    }
    ...
    if (m_plater) {
        ...
        auto       perspective_item = append_menu_radio_item(
            viewMenu, wxID_CAMERA_PERSPECTIVE + camera_id_base, _L("Use Perspective View"), _L("Use Perspective View"),
            [this](wxCommandEvent&) {
            ...
            nullptr);
        // BBS orthogonal view
        auto orthogonal_item = append_menu_radio_item(
            viewMenu, wxID_CAMERA_ORTHOGONAL + camera_id_base, _L("Use Orthogonal View"), _L("Use Orthogonal View"),
            [this](wxCommandEvent&) {
        ...
        this->Bind(
            wxEVT_UPDATE_UI,
            [viewMenu, camera_id_base](wxUpdateUIEvent& evt) {
        ...
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Auto Perspective"),
            _L("Automatically switch between orthographic and perspective when changing from top/bottom/side views."),
            [this](wxCommandEvent&) {
        ...
        viewMenu->AppendSeparator();
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show &G-code Window") + sep + "C", _L("Show G-code window in Preview scene."),
            [this](wxCommandEvent&) {
                wxGetApp().toggle_show_gcode_window();
                m_plater->get_current_canvas3D()->post_event(SimpleEvent(wxEVT_PAINT));
            },
            this, [this]() { return m_tabpanel->GetSelection() == tpPreview; }, [this]() { return wxGetApp().show_gcode_window(); }, this);
        ...
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show Gridlines"), _L("Show Gridlines on plate"),
            [this](wxCommandEvent&) {
        ...
        viewMenu->AppendSeparator();
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show &Labels") + "\t" + ctrl + "E", _L("Show object labels in 3D scene."),
            [this](wxCommandEvent&) {
                m_plater->show_view3D_labels(!m_plater->are_view3D_labels_shown());
                m_plater->get_current_canvas3D()->post_event(SimpleEvent(wxEVT_PAINT));
            },
            this, [this]() { return m_plater->is_view3D_shown(); }, [this]() { return m_plater->are_view3D_labels_shown(); }, this);

        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show &Overhang"), _L("Show object overhang highlight in 3D scene."),
            [this](wxCommandEvent&) {
        ...
        /*viewMenu->AppendSeparator();
        append_menu_check_item(viewMenu, wxID_ANY, _L("Show &Wireframe") + "\t" + ctrl + shift + _L("Enter"), _L("Show wireframes in 3D
        scene."), [this](wxCommandEvent&) { m_plater->toggle_show_wireframe();
        m_plater->get_current_canvas3D()->post_event(SimpleEvent(wxEVT_PAINT)); }, this, [this]() { return m_plater->is_wireframe_enabled();
        }, [this]() { return m_plater->is_show_wireframe(); }, this);*/
    ...
    append_menu_item(
        parent_menu, wxID_ANY, _L(about_title), "", [this](wxCommandEvent&) { Slic3r::GUI::about(); }, "", nullptr, []() { return true; },
        this, 0);
    append_menu_item(
    ...
    // Temperature
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Temperature"), _L("Temperature Calibration"),
        [this](wxCommandEvent&) {
            if (!m_temp_calib_dlg)
    ...
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Max flowrate"), _L("Max flowrate"),
        [this](wxCommandEvent&) {
            if (!m_vol_test_dlg)
    ...
    // Pressure Advance
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Pressure advance"), _L("Pressure advance"),
        [this](wxCommandEvent&) {
            if (!m_pa_calib_dlg)
    ...
    append_menu_item(
        flowrate_menu, wxID_ANY, _L("YOLO (Recommended)"), _L("Orca YOLO flowrate calibration, 0.01 step"),
        [this](wxCommandEvent&) {
            if (m_plater)
        ...
        this);
    append_menu_item(
        flowrate_menu, wxID_ANY, _L("YOLO (perfectionist version)"), _L("Orca YOLO flowrate calibration, 0.005 step"),
        [this](wxCommandEvent&) {
            if (m_plater)
    ...
    // Retraction test
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Retraction test"), _L("Retraction test"),
        [this](wxCommandEvent&) {
    ...
    // Cornering
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Cornering"), _L("Cornering calibration"),
        [this](wxCommandEvent&) {
    ...
    // VFA
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("VFA"), _L("VFA"),
        [this](wxCommandEvent&) {
            if (!m_vfa_test_dlg)
    ...
    // help
    append_menu_item(
        m_topbar->GetCalibMenu(), wxID_ANY, _L("Tutorial"), _L("Calibration help"),
        [this](wxCommandEvent&) {
            ...
            if (const std::string country_code = wxGetApp().app_config->get_country_code(); country_code == "CN") {
                ...
                url = "https://gitee.com/n0isyfox/orca-slicer-docs/wikis/%E6%A0%A1%E5%87%86/"
                      "%E6%89%93%E5%8D%B0%E5%8F%82%E6%95%B0%E6%A0%A1%E5%87%86";
            }
    ...
    // Max Volumetric Speed
    append_menu_item(
        calib_menu, wxID_ANY, _L("Max flowrate"), _L("Max flowrate"),
        [this](wxCommandEvent&) {
            if (!m_vol_test_dlg)
                m_vol_test_dlg = new MaxVolumetricSpeed_Test_Dlg((wxWindow*) this, wxID_ANY, m_plater);
            m_vol_test_dlg->ShowModal();
    ...
    // Cornering
    append_menu_item(
        calib_menu, wxID_ANY, _L("Cornering"), _L("Cornering calibration"),
        [this](wxCommandEvent&) {
            auto dlg = new Cornering_Test_Dlg((wxWindow*) this, wxID_ANY, m_plater);
            dlg->ShowModal();
            dlg->Destroy();
    ...
    // VFA
    append_menu_item(
        calib_menu, wxID_ANY, _L("VFA"), _L("VFA"),
        [this](wxCommandEvent&) {
            if (!m_vfa_test_dlg)
                m_vfa_test_dlg = new VFA_Test_Dlg((wxWindow*) this, wxID_ANY, m_plater);
            m_vfa_test_dlg->ShowModal();
    ...
    append_menu_item(
        ...
        [this](wxCommandEvent&) { wxLaunchDefaultBrowser("https://www.orcaslicer.com/wiki/Calibration", wxBROWSER_NEW_WINDOW); }, "",
        nullptr,
        [this]() {
            return m_plater->is_view3D_shown();
            ;
...
void MainFrame::export_config()
{
    ...
    wxDirDialog dlg(this, _L("Choose a directory"),
                    from_u8(!m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir()),
    ...
    if (!path.IsEmpty()) {
        ...
        try {
            auto files = wxGetApp().preset_bundle->export_current_configs(
                into_u8(path),
            ...
            MessageDialog dlg(this,
                              wxString::Format(_L_PLURAL("There is %d config exported. (Only non-system configs)",
                                                         "There are %d configs exported. (Only non-system configs)", files.size()),
                                               files.size()),
...
void MainFrame::load_config_file()
{
    ...
    wxFileDialog  dlg(this, _L("Select profile to load:"),
                     !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(), "config.json",
                      "Config files (*.json;*.zip;*.orca_printer;*.orca_filament)|*.json;*.zip;*.orca_printer;*.orca_filament",
    ...
    auto msg = wxString::Format(_L_PLURAL("There is %d config imported. (Only non-system and compatible configs)",
                                          "There are %d configs imported. (Only non-system and compatible configs)", cfiles.size()),
                                cfiles.size());
    if (cfiles.empty())
        msg += _L("\nHint: Make sure you have added the corresponding printer before importing the configs.");
    MessageDialog dlg2(this, msg, _L("Import result"), wxOK);
    dlg2.ShowModal();
}
...
{
    try {
        ConfigSubstitutions config_substitutions = wxGetApp().preset_bundle->load_config_file(path,
                                                                                              ForwardCompatibilitySubstitutionRule::Enable);
...
/*void MainFrame::export_system_configs()
{
    // Ask user for a file name.
    wxDirDialog dlg(this, _L("choose a directory"),
        !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(), wxDD_DEFAULT_STYLE |
wxDD_DIR_MUST_EXIST); wxString path; if (dlg.ShowModal() == wxID_OK) path = dlg.GetPath(); if (!path.IsEmpty()) {
        // Export the config bundle.
    ...
    auto select = [this, tab](bool was_hidden) {
        ...
        /*if (wxGetApp().tabs_as_menu()) {
            if (Tab* cur_tab = dynamic_cast<Tab*>(m_tabpanel->GetPage(new_selection)))
                update_marker_for_tabs_menu((m_layout == ESettingsLayout::Old ? m_menubar : m_settings_dialog.menubar()), cur_tab->title(),
        m_layout == ESettingsLayout::Old); else if (tab == 0 && m_layout == ESettingsLayout::Old)
...
void MainFrame::on_select_default_preset(SimpleEvent& evt)
{
    MessageDialog dialog(this,
                         ...
                         _L("Synchronization"), wxCENTER | wxYES_DEFAULT | wxYES_NO | wxICON_INFORMATION);
...
SettingsDialog::SettingsDialog(MainFrame* mainframe)
    : DPIDialog(NULL,
                wxID_ANY,
                wxString(SLIC3R_APP_NAME) + " - " + _L("Settings"),
                wxDefaultPosition,
                wxDefaultSize,
