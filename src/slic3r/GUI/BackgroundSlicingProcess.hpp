...
// [INTENT] Orchestrates background slicing and G-code generation, managing the worker thread and UI synchronization.
// [UNITY] Reimplement using Unity Job System or C# Task Parallel Library (TPL).
//         UI callbacks should use a main-thread marshaling layer (e.g., async/await with UniTask or standard C# Tasks).
...
public:
    ...
    // [EVENT] The following wxCommandEvent will be sent to the UI thread / Plater window, when the slicing is finished
    // and the background processing will transition into G-code export.
    // The wxCommandEvent is sent to the UI thread asynchronously without waiting for the event to be processed.
    void set_slicing_completed_event(int event_id) { m_event_slicing_completed_id = event_id; }
    // [EVENT] The following wxCommandEvent will be sent to the UI thread / Plater window, when the G-code export is finished.
    // The wxCommandEvent is sent to the UI thread asynchronously without waiting for the event to be processed.
    void set_finished_event(int event_id) { m_event_finished_id = event_id; }
    // [EVENT] The following wxCommandEvent will be sent to the UI thread / Plater window, when the G-code is being exported to
    ...
    // Take the project path (if provided), extract the name of the project, run it through the macro processor and save it next to the
    // project file. If the project_path is empty, just run output_filepath().
    ...
    bool  running() const
    {
        return m_state == STATE_STARTED || m_state == STATE_RUNNING || m_state == STATE_FINISHED || m_state == STATE_CANCELED;
    }
...
private:
    // [INTENT] Background thread main loop.
    void thread_proc();
    // Calls thread_proc(), catches all C++ exceptions and shows them using wxApp::OnUnhandledException().
    void thread_proc_safe() throw();
#ifdef _WIN32
    // Wrapper for Win32 structured exceptions. Win32 structured exception blocks and C++ exception blocks cannot be mixed in the same
    // function. Catch a SEH exception and return its ID or zero if no SEH exception has been catched.
    ...
    // [UNITY] Use System.Threading.Tasks.Task or Unity Job System.
    boost::thread m_thread;
    // [THREAD] Synchronization primitives.
    std::mutex              m_mutex;
    std::condition_variable m_condition;
    // [STATE] Current worker thread state.
    State m_state = STATE_INITIAL;

    // [THREAD] Synchronization structure for executing tasks on the UI thread from the background thread.
    // [UNITY] Replace with async/await marshaling to main thread.
    struct UITask
    {
        enum State {
            Planned,
    ...
    // [THREAD] Execute task from background thread on the UI thread synchronously. Returns true if processed, false if cancelled before
    // executing the task.
    ...
    // [EVENT] wxWidgets command IDs for UI notifications.
