...
// [INTENT] Selection is the mutable edit model behind GLCanvas3D: it owns no scene data, but it
// [STATE] caches pre-edit transforms, bounding volumes, and clipboard copies so drag/rotate/scale
// [UNITY] can become a selection transaction service plus a separate gizmo overlay controller.
// [PORTING_HAZARD:P2] This file mixes undo snapshots, model mutation, object-list refreshes, and GL preview rendering.
...
Selection::VolumeCache::TransformCache::TransformCache(const Geometry::Transformation& transform)
    : position(transform.get_offset()), transform(transform)
{
...
Selection::VolumeCache::VolumeCache(const Geometry::Transformation& volume_transform, const Geometry::Transformation& instance_transform)
    : m_volume(volume_transform), m_instance(instance_transform)
{}
...
Selection::Clipboard::Clipboard() { m_model.reset(new Model); }

void Selection::Clipboard::reset() { m_model->clear_objects(); }

bool Selection::Clipboard::is_empty() const { return m_model->objects.empty(); }

ModelObject* Selection::Clipboard::add_object() { return m_model->add_object(); }
...
const ModelObjectPtrs& Selection::Clipboard::get_objects() const { return m_model->objects; }

Selection::Selection()
    : m_volumes(nullptr), m_model(nullptr), m_enabled(false), m_mode(Instance), m_type(Empty), m_valid(false), m_scale_factor(1.0f)
...
// [OPENGL] Init must run on a live GL context; it lazily builds the shared selection gizmo meshes.
// [UNITY] Replace with persistent mesh/line assets owned by an overlay renderer instead of context-bound GLModel init.
bool Selection::init()
{
...
void Selection::add(unsigned int volume_idx, bool as_single_selection, bool check_for_already_contained)
{
    ...
    // [EVENT] Selection changes are command-like UI actions: they snapshot undo state, mutate the
    // [THREAD] GL volumes, and notify the canvas/sidebar synchronously on the main thread.
...
// BBS
//  [EVENT] Printable-state toggles fan out into the model tree, object list, and canvas highlight state.
//  [UNITY] Model this as a scene-graph command that updates the hierarchy UI after the data change.
...
int Selection::get_object_idx() const { return (m_cache.content.size() == 1) ? m_cache.content.begin()->first : -1; }
        ...
        if (m_valid) {
            for (unsigned int i : m_list) {
                ...
                if (volume.is_modifier)
                    continue;
                Transform3d trafo = volume.get_instance_transformation().get_matrix_no_scaling_factor() *
...
const std::pair<BoundingBoxf3, Transform3d>& Selection::get_bounding_box_in_current_reference_system() const
{
    ...
    if (!m_bounding_box_in_current_reference_system.has_value()) {
        ...
        *const_cast<std::optional<std::pair<BoundingBoxf3, Transform3d>>*>(
            &m_bounding_box_in_current_reference_system) = get_bounding_box_in_reference_system(coordinates_type);
    }
        ...
        if (m_valid && !m_list.empty()) {
            ...
            for (unsigned int i : m_list) {
                ...
                const indexed_triangle_set& its    = (hull != nullptr) ?
                                                         hull->its :
                                                         m_model->objects[volume.object_idx()]->volumes[volume.volume_idx()]->mesh().its;
...
void Selection::move_to_center(const Vec3d& displacement, bool local)
{
    ...
    // BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(": %1%, displacement {%2%, %3%, %4%}") % __LINE__ % displacement(X) %
    ...
    for (unsigned int i : m_list) {
        ...
        if (m_mode == Volume) {
            if (local)
                ...
                const Vec3d local_displacement = (m_cache.volumes_data[i].get_instance_rotation_matrix() *
                                                  m_cache.volumes_data[i].get_instance_scale_matrix() *
    ...
    for (unsigned int i : m_list) {
        ...
        if (m_mode == Instance && !is_wipe_tower()) {
            ...
            if (transformation_type.instance()) {
                ...
                transform_instance_relative(v, volume_data, transformation_type, Geometry::translation_transform(displacement),
            ...
            if (v.is_wipe_tower) { // in world cs
                ...
                bool show_read_wipe_tower = wxGetApp().plater()->get_partplate_list().get_plate(plate_idx)->fff_print()->is_step_done(
                    psWipeTower);
                ...
                actual_displacement      = (m_cache.volumes_data[i].get_instance_rotation_matrix() *
                                            m_cache.volumes_data[i].get_instance_scale_matrix() *
                                            m_cache.volumes_data[i].get_instance_mirror_matrix())
                                               .inverse() *
                                           displacement;
                ...
                BoundingBox tower_bbox2d = BoundingBox(scaled(Vec2f(tower_bbox.min[0], tower_bbox.min[1])),
                                                       scaled(Vec2f(tower_bbox.max[0], tower_bbox.max[1])));
                ...
                const Geometry::Transformation& inst_trafo = volume_data.get_instance_transform();
                v.set_volume_offset(vol_trafo.get_offset() +
                                    inst_trafo.get_scaling_factor_matrix().inverse() * vol_trafo.get_rotation_matrix() * displacement);
            } else {
                ...
                transform_volume_relative(v, volume_data, transformation_type, Geometry::translation_transform(relative_disp),
                                          m_cache.dragging_center);
            }
        }
    }
...
void Selection::rotate(const Vec3d& rotation, TransformationType transformation_type)
{
        ...
        if (m_mode == Instance && !is_wipe_tower()) {
            ...
            if (transformation_type.instance()) {
                ...
                rotation_matrix                         = inst_matrix_no_offset.inverse() * inst_rotation_matrix * rotation_matrix *
                                                          inst_rotation_matrix.inverse() * inst_matrix_no_offset;
                ...
                rotation_matrix        = Geometry::translation_transform(inst_pivot) * rotation_matrix *
                ...
                if (!requires_general_synchronization) {
                    ...
                    const Transform3d new_world_rotation_matrix = (inst_trafo * Geometry::Transformation(rotation_matrix) * vol_trafo)
                                                                      .get_rotation_matrix();
                    if (std::abs((old_world_rotation_matrix * Vec3d::UnitX()).z() - (new_world_rotation_matrix * Vec3d::UnitX()).z()) >
                        EPSILON)
                        requires_general_synchronization = true;
                    else if (std::abs((old_world_rotation_matrix * Vec3d::UnitY()).z() - (new_world_rotation_matrix * Vec3d::UnitY()).z()) >
                             EPSILON)
                        requires_general_synchronization = true;
                    else if (std::abs((old_world_rotation_matrix * Vec3d::UnitZ()).z() - (new_world_rotation_matrix * Vec3d::UnitZ()).z()) >
                             EPSILON)
                        requires_general_synchronization = true;
                }
            }
            ...
            if (!is_single_volume_or_modifier()) {
                ...
                if (transformation_type.instance()) {
                    ...
                    if (transformation_type.local()) {
                        ...
                        rotation_matrix = vol_matrix_no_offset.inverse() * inst_scale_matrix.inverse() * vol_rotation_matrix *
                                          rotation_matrix * vol_rotation_matrix.inverse() * inst_scale_matrix * vol_matrix_no_offset;
                    }
                }
                transform_volume_relative(v, volume_data, transformation_type, rotation_matrix, m_cache.rotation_pivot);
            }
    ...
    if (m_mode == Instance) {
        ...
        const SyncRotationType type = (transformation_type.instance() && requires_general_synchronization) ||
                                              (!transformation_type.instance() && rot_axis_max != 2) || rotation.isApprox(Vec3d::Zero()) ?
                                          SyncRotationType::GENERAL :
                                          SyncRotationType::NONE;
...
void Selection::scale_to_fit_print_volume(const BuildVolume& volume)
{
    ...
    auto fit_circle = [this, fit](const BuildVolume& volume) {
        ...
        const Vec3d  print_center = {unscale<double>(print_circle.center.x()), unscale<double>(print_circle.center.y()),
                                     0.5 * volume.printable_height()};
...
void Selection::scale_to_fit_print_volume(const DynamicPrintConfig& config)
{
    ...
    if (opt != nullptr) {
        ...
        BoundingBoxf3 print_volume({unscale<double>(bed_box_2D.min(0)), unscale<double>(bed_box_2D.min(1)), 0.0},
                                   {unscale<double>(bed_box_2D.max(0)), unscale<double>(bed_box_2D.max(1)),
...
// [INTENT] Transform helpers apply edits relative to the cached pre-drag state so the selection can
// [STATE] preserve rigid-body vs per-volume behavior, selection pivots, and sibling synchronization.
// [UNITY] Keep the edit math in a transaction snapshot; do not accumulate from rendered node transforms.
    ...
    for (unsigned int i : m_list) {
        ...
        if (m_mode == Instance) {
            if (transformation_type.instance()) {
                ...
                v.set_instance_transformation(Geometry::translation_transform(world_inst_pivot) * offset_trafo *
                                              Transform3d(inst_rotation) * scale_trafo * Geometry::translation_transform(-local_inst_pivot));
            } else
                transform_instance_relative(v, volume_data, transformation_type,
                                            Geometry::translation_transform(world_translation) * Geometry::scale_transform(relative_scale),
                                            m_cache.dragging_center);
            ...
            if (!is_single_volume_or_modifier()) {
                assert(transformation_type.world());
                transform_volume_relative(v, volume_data, transformation_type,
                                          Geometry::translation_transform(world_translation) * Geometry::scale_transform(scale),
                                          m_cache.dragging_center);
            } else {
                transformation_type.set_independent();
                Vec3d translation;
                if (transformation_type.local()) {
                    translation = volume_data.get_volume_transform().get_matrix_no_offset().inverse() *
                                  inst_trafo.get_matrix_no_offset().inverse() * world_translation;
                } else if (transformation_type.instance())
                    translation = inst_trafo.get_matrix_no_offset().inverse() * world_translation;
                else
                    translation = world_translation;
                transform_volume_relative(v, volume_data, transformation_type,
                                          Geometry::translation_transform(translation) * Geometry::scale_transform(scale),
                                          m_cache.dragging_center);
            }
        }
    }
...
// [OPENGL] Rendering is immediate-mode overlay work: it reuses cached line meshes and shader selection
// [UNITY] based on camera/profile state, so the port needs a dedicated gizmo/overlay renderer.
void Selection::render(float scale_factor)
{
    ...
    render_bounding_box(box, trafo,
                        wxGetApp().plater()->canvas3D()->get_canvas_type() == GLCanvas3D::ECanvasType::CanvasAssembleView ?
                            ColorRGB::YELLOW() :
                            ColorRGB::WHITE());
...
//  [EVENT] Sidebar hint rendering keys off the edited field name and mirrors the active manipulator axis.
//  [PORTING_HAZARD:P3] The field-name protocol is implicit string routing; Unity should replace it with typed edit handles.
...
bool Selection::requires_local_axes() const { return m_mode == Volume && is_from_single_instance(); }
...
// [INTENT] Clipboard is a deep-copy model buffer for copy/cut/paste across objects, instances, and volumes.
// [PORTING_HAZARD:P3] Pasting reconstructs geometry and repositions it with plate/object heuristics, so provenance matters.
...
// [EVENT] Paste dispatches by clipboard mode and selection mode; it is a UI command, not a generic data merge.
// [UNITY] Use explicit copy/paste command objects that carry the source selection shape and placement rules.
...
void Selection::fill_color(int extruder_id) { wxGetApp().obj_list()->set_extruder_for_selected_items(extruder_id); }
...
std::vector<unsigned int> Selection::get_volume_idxs_from_volume(unsigned int object_idx,
...
void Selection::update_valid() { m_valid = (m_volumes != nullptr) && (m_model != nullptr); }

// [STATE] update_type recomputes the selection taxonomy and cached object/instance membership from m_list.
// [UNCLEAR] The type lattice is legacy-heavy and partially BBS-specific; keep the classification logic verbatim in Unity until behavior is mapped.
                ...
                if (model_volumes_count * instances_count + sla_volumes_count == (unsigned int) m_list.size()) {
                    ...
                    if (model_volumes_count + sla_volumes_count == (unsigned int) m_list.size()) {
                        ...
                        requires_disable = true;
                           ...
                           (selected_instances_count * model_volumes_count + sla_volumes_count == (unsigned int) m_list.size())) {
...
// [OPENGL] Bounding-box overlay geometry is cached as a tiny GLModel and redrawn with dashed-line shaders.
// [UNITY] Replace with a reusable line-mesh or gizmo primitive rather than rebuilding per frame.
...
static ColorRGBA get_color(Axis axis) { return GLGizmoBase::AXES_COLOR[axis]; }
                                           ...
                                           const Transform3d& matrix)
{
    ...
    auto render_sidebar_scale_hint = [this, uniform_scale](Axis axis, GLShaderProgram& shader, const Transform3d& view_matrix,
...
void Selection::render_sidebar_layers_hints(const std::string& sidebar_field, GLShaderProgram& shader)
{
    ...
    m_planes.models[0].set_color((camera_on_top && type == 1) || (!camera_on_top && type == 2) ? SOLID_PLANE_COLOR :
                                                                                                 TRANSPARENT_PLANE_COLOR);
    m_planes.models[0].render();
    m_planes.models[1].set_color((camera_on_top && type == 2) || (!camera_on_top && type == 1) ? SOLID_PLANE_COLOR :
                                                                                                 TRANSPARENT_PLANE_COLOR);
...
static bool is_left_handed(const Transform3d::ConstLinearPart& m) { return m.determinant() < 0; }

static bool is_left_handed(const Transform3d& m) { return is_left_handed(m.linear()); }
...
// [EVENT] Selection edits implicitly fan out to sibling instances so grouped objects stay visually coherent.
// [PORTING_HAZARD:P2] Unity needs an explicit sibling-update step; this behavior is easy to miss if edits only touch the selected node.
...
// [STATE] Bed-clamping keeps every instance sharing the same lowest Z adjustment so selected and unselected siblings remain aligned.
...
// [PORTING_HAZARD:P2] Z clamping is selection-type dependent: volume edits shift only the selected volumes, while instance edits move the
// full instance.
...
bool Selection::is_from_fully_selected_instance(unsigned int volume_idx) const
{
    ...
    struct SameInstance
    {
        ...
        { return (volumes[i]->volume_idx() >= 0) && (volumes[i]->object_idx() == obj_idx) && (volumes[i]->instance_idx() == inst_idx); }
    };
    ...
    unsigned int count = (unsigned int) std::count_if(m_list.begin(), m_list.end(),
                                                      SameInstance(object_idx, volume->instance_idx(), *m_volumes));
...
// [UNITY] Object paste is placement-aware: it picks an empty cell on the active plate and replays the instance transforms there.
// [PORTING_HAZARD:P2] The placement heuristic depends on plate geometry, current selection, and multi-object spacing.
void Selection::paste_objects_from_clipboard()
{
    ...
    for (size_t i = 0; i < src_objects.size(); i++) {
        ...
        if (shift_all(0) != 0 || shift_all(1) != 0) {
            // BBS: if multiple objects are selected, move them as a whole after copy
            if (i == 0)
                empty_cell_all = wxGetApp().plater()->canvas3D()->get_nearest_empty_cell({start_point(0), start_point(1)},
                                                                                         {bbox.size()(0) + 1, bbox.size()(1) + 1});
            auto instance_shift = src_object->instances.front()->get_offset() - src_objects[0]->instances.front()->get_offset();
            displacement        = {shift_all.x() + empty_cell_all.x() + instance_shift.x(),
                                   shift_all.y() + empty_cell_all.y() + instance_shift.y(), start_offset(2)};
        } else {
...
void Selection::transform_instance_relative(GLVolume&          volume,
...
void Selection::transform_volume_relative(GLVolume&          volume,
                                          const VolumeCache& volume_data,
                                          TransformationType transformation_type,
                                          const Transform3d& transform,
                                          const Vec3d&       world_pivot)
{
    ...
    if (transformation_type.world()) {
        const Vec3d       inst_pivot = transformation_type.independent() ? vol_trafo.get_offset() :
                                                                           (Vec3d) (inst_trafo.get_matrix().inverse() * world_pivot);
        const Transform3d inst_matrix_no_offset = inst_trafo.get_matrix_no_offset();
        const Transform3d trafo = Geometry::translation_transform(inst_pivot) * inst_matrix_no_offset.inverse() * transform *
