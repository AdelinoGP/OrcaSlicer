...
// [INTENT] Object table constants drive the per-object settings grid in the Print Preview/Quality dialogs so the column widths, scrollbars,
// [STATE] and cell sizing helpers live in shared globals ensuring the wxWidgets dialog can respect a minimum pixel budget and overflow
// limits. [UNITY] Unity port should map these dimensions into a UI Toolkit `ScrollView` + `GridLayout` combination (or a custom `ListView`
// cell template) and [PORTING_HAZARD:P2] note that the wx table assumes synchronous row sizing and direct pixel metrics, which Unity will
// need to approximate in device-independent units.
...
// ----------------------------------------------------------------------------
void GridCellIconRenderer::Draw(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, const wxRect& rect, int row, int col, bool isSelected)
{
    // [INTENT] Overlay a lock/undo badge whenever the option cell differs from its baseline so the user can rollback the change explicitly.
    // [STATE] The renderer updates `m_icon_row_height`/`m_icon_col_width` on the table for later layout recalculations when change-state
    // toggles. [UNITY] In Unity this translates to a custom `VisualElement` cell template that compares a `SerializedProperty` snapshot to
    // its cached value and draws a `Texture2D` badge via `IMGUIContainer` or UI Toolkit in the `ListView` cell template.
...
wxSize GridCellIconRenderer::GetBestSize(wxGrid& WXUNUSED(grid), wxGridCellAttr& attr, wxDC& dc, int WXUNUSED(row), int WXUNUSED(col))
{
    wxSize size{32, 30};
    return size;
}

GridCellIconRenderer* GridCellIconRenderer::Clone() const { return new GridCellIconRenderer(); }

// ----------------------------------------------------------------------------
// GridCellFilamentsEditor
// ----------------------------------------------------------------------------

GridCellFilamentsEditor::GridCellFilamentsEditor(const wxArrayString& choices, bool allowOthers, std::vector<wxBitmap*>* bitmaps)
    : wxGridCellChoiceEditor(choices, allowOthers), m_icons(bitmaps)
{}

GridCellFilamentsEditor::GridCellFilamentsEditor(size_t count, const wxString choices[], bool allowOthers, std::vector<wxBitmap*>* bitmaps)
...
void GridCellFilamentsEditor::Create(wxWindow* parent, wxWindowID id, wxEvtHandler* evtHandler)
{
    // [INTENT] Build a drop-down that pairs filament swatches (m_icons) with their localized names so scanning table rows stays visual and
    // numeric shortcuts are supported. [EVENT] The editor keeps `wxEVT_COMBOBOX_CLOSEUP` bound during `BeginEdit` so closing the popup
    // completes the grid edit cycle without losing focus.
    int style = wxTE_PROCESS_ENTER | wxTE_PROCESS_TAB | wxBORDER_NONE;

    if (!m_allowOthers)
        style |= wxCB_READONLY;
    ::ComboBox* bitmap_combo = new ComboBox(parent, id, wxEmptyString, wxDefaultPosition, wxSize(((*m_icons)[0])->GetWidth() + 10, -1), 0,
                                            nullptr, CB_NO_DROP_ICON | CB_NO_TEXT | wxCB_READONLY);
    if (m_icons) {
...
void GridCellFilamentsEditor::OnComboCloseUp(wxCommandEvent& evt) { wxGridCellChoiceEditor::OnComboCloseUp(evt); }
...
bool GridCellFilamentsEditor::EndEdit(
...
GridCellChoiceEditor::GridCellChoiceEditor(const wxArrayString& choices) : wxGridCellChoiceEditor(choices) {}

GridCellChoiceEditor::GridCellChoiceEditor(size_t count, const wxString choices[]) : wxGridCellChoiceEditor(count, choices) {}
...
void GridCellChoiceEditor::SetSize(const wxRect& rect) { wxGridCellChoiceEditor::SetSize(rect); }
...
bool GridCellChoiceEditor::EndEdit(
    int WXUNUSED(row), int WXUNUSED(col), const wxGrid* WXUNUSED(grid), const wxString& WXUNUSED(oldval), wxString* newval)
{
    const wxString value = Combo()->GetValue();
    if (value == m_value)
...
void GridCellSupportEditor::DoActivate(int row, int col, wxGrid* grid)
{
    ...
    if ((left_col == right_col) && (top_row == bottom_row)) {
        ...
        return;
    } else if ((left_col == right_col) && (top_row != bottom_row)) {
...
// ----------------------------------------------------------------------------
void GridCellSupportRenderer::Draw(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, const wxRect& rect, int row, int col, bool isSelected)
{
    /*ObjectGridTable *table = dynamic_cast<ObjectGridTable *>(grid.GetTable());
    wxRect text_rect = rect;
...
wxSize GridCellSupportRenderer::GetBestSize(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, int WXUNUSED(row), int WXUNUSED(col))
{
    /*
...
GridCellSupportRenderer* GridCellSupportRenderer::Clone() const { return new GridCellSupportRenderer(); }
...
wxBEGIN_EVENT_TABLE(ObjectGrid, wxGrid) EVT_KEY_DOWN(ObjectGrid::OnKeyDown) EVT_KEY_UP(ObjectGrid::OnKeyUp) EVT_CHAR(ObjectGrid::OnChar)
    EVT_GRID_LABEL_LEFT_CLICK(ObjectGrid::OnColHeadLeftClick) EVT_GRID_RANGE_SELECTED(ObjectGrid::OnRangeSelected) wxEND_EVENT_TABLE()
        ...
        bool ObjectGrid::OnCellLeftClick(wxGridEvent& event, int row, int col, ConfigOptionType type)
{
    ...
    if ((position.x > (col_right_pos - grid_cell_checkbox_size)) && (position.y >= (row_up_pos + row_offset)) &&
        (position.y <= (row_down_pos - row_offset)))
...
void ObjectGrid::OnRangeSelected(wxGridRangeSelectEvent& ev) { ev.Skip(); }
    ...
    if ((event.GetModifiers() & ctrlMask) != 0) {
        ...
        case 'C': {
            ...
            if (iter == blocks.end()) {
                ...
                selection = wxGridBlockCoords(GetGridCursorRow(), GetGridCursorCol(), GetGridCursorRow(), GetGridCursorCol());
            ...
            if (GetGridCursorCol() == ObjectGridTable::col_printable || GetGridCursorCol() == ObjectGridTable::col_enable_support) {
    ...
    if ((src_row_cnt == 1) && (src_col_cnt == 1)) {
        ...
        /*if ((src_col_cnt != 1) || (dst_left_col != src_left_col))
            ...
            if ((count <= 0) || (count != src_row_cnt* src_col_cnt )){
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", can not split pasted data %1%, count %2%, src_row_cnt %3%,
        src_col_cnt %4%")%buf %count %src_row_cnt %src_col_cnt; return;
            }
...
void ObjectGrid::OnKeyUp(wxKeyEvent& event) { event.Skip(); }
...
void ObjectGrid::DrawColLabels(wxDC& dc, const wxArrayInt& cols) { wxGrid::DrawColLabels(dc, cols); }
...
// [INTENT] Owns the wxGrid-backed object/volume settings table, translating grid edits into ModelConfig and Object updates.
// [STATE] Tracks `m_grid_data`, `m_col_data`, `m_selected_cells`, and column metadata so the UI can highlight changed cells and reset
// columns. [EVENT] Hooks into left-clicks, value changes, and range selections to keep configs aligned and to notify
// `wxGetApp().obj_list()` of state shifts. [THREAD] Runs entirely on the GUI thread, so Unity must marshal data through a
// `MainThreadDispatcher` when background workers mutate the config snapshot. [UNITY] Replace with a UI Toolkit `ListView`/`ScrollView`
// pairing and a `ScriptableObject` model list where each `VisualElement` renders the columns described by `ObjectGridCol`.
// [PORTING_HAZARD:P2] The implementation relies on row indices matching `ObjectGrid` order; Unity must keep an ID mapping when using
// asynchronous list data sources.
...
wxString ObjectGridTable::GetTypeName(int row, int col)
{
    ...
    switch (option_type) {
    case coString: return wxGRID_VALUE_STRING;
    case coBool: return wxGRID_VALUE_BOOL;
    case coInt: return wxGRID_VALUE_NUMBER;
    case coFloat:
    case coPercent: return wxGRID_VALUE_FLOAT;
    case coEnum: return wxGRID_VALUE_CHOICE;
    default: break;
    }
...
int ObjectGridTable::GetNumberRows() { return m_grid_data.size() + 1; }

int ObjectGridTable::GetNumberCols() { return m_col_data.size(); }
...
bool ObjectGridTable::CanSetValueAs(int row, int col, const wxString& typeName) { return CanGetValueAs(row, col, typeName); }
...
wxString ObjectGridTable::GetValue(int row, int col)
{
    ...
    if (row == 0) {
        ...
        case col_filaments: return "Filament";
        case col_layer_height: return "Layer height";
        case col_wall_loops: return "Perimeter";
        case col_fill_density: return "Infill density(%)";
        case col_enable_support: return "Support";
        case col_brim_type: return "Brim";
        case col_speed_perimeter: return "Perimeter speed";
...
// [INTENT] Apply a committed grid cell value back into the `Model`/`ModelVolume`, keeping the preview state and object list synchronized.
// [EVENT] Triggered from `SetValueAsBool` and other modifiers when wxGrid reports a change so the backend knows a printable toggle or name
// edit happened. [UNITY] Replace with a `ListViewCellController` that writes to a `ScriptableObject` object list and raises C# callbacks
// for selection/printable updates.
void ObjectGridTable::update_value_to_object(Model* model, ObjectGridRow* grid_row, int col)
{
    ...
    if (!object) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(", object_id %1%, volume_id %2%, can not find modelObject anymore!") %
                                        grid_row->object_id % grid_row->volume_id;
        return;
    }
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(", object_id %1%, volume_id %2%, col %3%, row_type %4%") % grid_row->object_id %
                                   grid_row->volume_id % col % grid_row->row_type;
    ...
    if (grid_row->row_type != row_object) {
        ...
        /* else if (col == col_assemble_name) {
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                    << boost::format(", change object %1%'s printable to %2%") % object->module_name % object->printable;
        }
    }
...
// [EVENT] Called directly by wxGrid when the user edits a cell so the table can validate and dispatch updates.
// [STATE] Guards `row == 0` headers and uses column metadata to decide if the change goes into `ConfigOption`s or `Model` objects.
// [UNITY] Implement as a `ListView` cell commit callback that invokes `ObjectRow.TryCommit(value)` plus a UI refresh on the data source.
...
// [INTENT] Registers the column metadata (printable icon, names, filament choices, etc.) that control which cells are editable, iconified,
// or tied to config-driven values. [STATE] Populates `m_col_data` with `ObjectGridCol` entries describing alignment, whether a column reads
// from `ConfigOption`, and whether a reset icon exists. [UNITY] Translate these descriptors into `ColumnDefinition` scripts used by Unity's
...
void ObjectGridTable::init_cols(ObjectGrid* object_grid)
{
    ...
    col = new ObjectGridCol(coString, "plate_index", " ", true, false, false, false,
                            wxALIGN_CENTRE); // bool only_object, bool icon, bool edit, bool config
    m_col_data.push_back(col);
...
void ObjectGridTable::construct_object_configs(ObjectGrid* object_grid)
{
    ...
    for (int i = 0; i < object_count; i++) {
        ...
        object_grid->layer_height = *(
            get_object_config_value<ConfigOptionFloat>(global_config, object_grid->config, m_col_data[col_layer_height]->key));
        object_grid->ori_layer_height = *(global_config.option<ConfigOptionFloat>(m_col_data[col_layer_height]->key));
        object_grid->wall_loops       = *(
            get_object_config_value<ConfigOptionInt>(global_config, object_grid->config, m_col_data[col_wall_loops]->key));
        object_grid->ori_wall_loops        = *(global_config.option<ConfigOptionInt>(m_col_data[col_wall_loops]->key));
        object_grid->sparse_infill_density = *(
            get_object_config_value<ConfigOptionPercent>(global_config, object_grid->config, m_col_data[col_fill_density]->key));
        object_grid->ori_fill_density = *(global_config.option<ConfigOptionPercent>(m_col_data[col_fill_density]->key));
        object_grid->enable_support   = *(
            get_object_config_value<ConfigOptionBool>(global_config, object_grid->config, m_col_data[col_enable_support]->key));
        object_grid->ori_enable_support = *(global_config.option<ConfigOptionBool>(m_col_data[col_enable_support]->key));
        object_grid->brim_type          = *(
            get_object_config_value<ConfigOptionEnum<BrimType>>(global_config, object_grid->config, m_col_data[col_brim_type]->key));
        object_grid->ori_brim_type   = *(global_config.option<ConfigOptionEnum<BrimType>>(m_col_data[col_brim_type]->key));
        object_grid->speed_perimeter = *(
            get_object_config_value<ConfigOptionFloat>(global_config, object_grid->config, m_col_data[col_speed_perimeter]->key));
        ...
        for (int j = 0; j < volume_count; j++) {
            ...
            // auto extruder_id_ptr = get_volume_config_value<ConfigOptionInt>(filament_config, object_grid->config, volume_grid->config,
            // m_col_data[col_filaments]->key);
            ...
            volume_grid->layer_height = *(get_volume_config_value<ConfigOptionFloat>(global_config, object_grid->config, volume_grid->config,
                                                                                     m_col_data[col_layer_height]->key));
            volume_grid->ori_layer_height = object_grid->layer_height;
            volume_grid->wall_loops = *(get_volume_config_value<ConfigOptionInt>(global_config, object_grid->config, volume_grid->config,
                                                                                 m_col_data[col_wall_loops]->key));
            volume_grid->ori_wall_loops        = object_grid->wall_loops;
            volume_grid->sparse_infill_density = *(get_volume_config_value<ConfigOptionPercent>(global_config, object_grid->config,
                                                                                                volume_grid->config,
                                                                                                m_col_data[col_fill_density]->key));
            volume_grid->ori_fill_density      = object_grid->sparse_infill_density;
            volume_grid->enable_support        = *(get_volume_config_value<ConfigOptionBool>(global_config, object_grid->config,
                                                                                             volume_grid->config,
                                                                                             m_col_data[col_enable_support]->key));
            volume_grid->ori_enable_support    = object_grid->enable_support;
            volume_grid->brim_type             = *(get_volume_config_value<ConfigOptionEnum<BrimType>>(global_config, object_grid->config,
                                                                                                       volume_grid->config,
                                                                                                       m_col_data[col_brim_type]->key));
            volume_grid->ori_brim_type         = object_grid->brim_type;
            volume_grid->speed_perimeter       = *(get_volume_config_value<ConfigOptionFloat>(global_config, object_grid->config,
                                                                                              volume_grid->config,
                                                                                              m_col_data[col_speed_perimeter]->key));
...
void ObjectGridTable::reload_object_data(ObjectGridRow* grid_row, const std::string& category, DynamicPrintConfig& global_config)
{
    ...
    if (category == ObjectGridTable::category_all) {
        ...
        grid_row->sparse_infill_density = *(
            get_object_config_value<ConfigOptionPercent>(global_config, grid_row->config, m_col_data[col_fill_density]->key));
        grid_row->ori_fill_density = *(global_config.option<ConfigOptionPercent>(m_col_data[col_fill_density]->key));
        grid_row->enable_support   = *(
            get_object_config_value<ConfigOptionBool>(global_config, grid_row->config, m_col_data[col_enable_support]->key));
        grid_row->ori_enable_support = *(global_config.option<ConfigOptionBool>(m_col_data[col_enable_support]->key));
        grid_row->brim_type          = *(
            get_object_config_value<ConfigOptionEnum<BrimType>>(global_config, grid_row->config, m_col_data[col_brim_type]->key));
        grid_row->ori_brim_type   = *(global_config.option<ConfigOptionEnum<BrimType>>(m_col_data[col_brim_type]->key));
        grid_row->speed_perimeter = *(
            get_object_config_value<ConfigOptionFloat>(global_config, grid_row->config, m_col_data[col_speed_perimeter]->key));
...
void ObjectGridTable::reload_part_data(ObjectGridRow*      volume_row,
                                       ...
                                       DynamicPrintConfig& global_config)
{
    ...
    if (category == ObjectGridTable::category_all) {
        ...
        volume_row->wall_loops       = *(get_volume_config_value<ConfigOptionInt>(global_config, object_row->config, volume_row->config,
                                                                                  m_col_data[col_wall_loops]->key));
        volume_row->ori_wall_loops   = object_row->wall_loops;
        volume_row->sparse_infill_density = *(get_volume_config_value<ConfigOptionPercent>(global_config, object_row->config,
                                                                                           volume_row->config,
                                                                                           m_col_data[col_fill_density]->key));
        volume_row->ori_fill_density      = object_row->sparse_infill_density;
        volume_row->enable_support     = *(get_volume_config_value<ConfigOptionBool>(global_config, object_row->config, volume_row->config,
                                                                                     m_col_data[col_enable_support]->key));
        volume_row->ori_enable_support = object_row->enable_support;
        volume_row->brim_type = *(get_volume_config_value<ConfigOptionEnum<BrimType>>(global_config, object_row->config, volume_row->config,
                                                                                      m_col_data[col_brim_type]->key));
        volume_row->ori_brim_type   = object_row->brim_type;
        volume_row->speed_perimeter = *(get_volume_config_value<ConfigOptionFloat>(global_config, object_row->config, volume_row->config,
                                                                                   m_col_data[col_speed_perimeter]->key));
                ...
                if (grid_row->row_type == ObjectGridTable::row_volume) {
                    ...
                    case coEnum:
                        if (col == ObjectGridTable::col_filaments) {
                            GridCellFilamentsEditor* filament_editor = new GridCellFilamentsEditor(grid_col->choices, false,
                                                                                                   &m_panel->m_color_bitmaps);
...
void ObjectGridTable::sort_by_col(int col)
{
    ...
    if (col == col_name) {
        if (m_sort_col == col) {
            auto sort_func = [](ObjectGridRow* row1, ObjectGridRow* row2) { return (row2->name.value.compare(row1->name.value) < 0); };
            // sort_by_name();
            sort_row_data(sort_func);
            m_sort_col = -1;
        } else {
            auto sort_func = [](ObjectGridRow* row1, ObjectGridRow* row2) { return (row1->name.value.compare(row2->name.value) < 0); };
            // sort_by_name();
            sort_row_data(sort_func);
            m_sort_col = col;
        }
...
// [EVENT] Handles left-clicks for sort headers and icon resets so the table can revert changed cells and trigger config/model
// notifications. [STATE] When icon columns are clicked, it rewrites cached `ConfigOption` values and notifies the side panel/`obj_list` to
// keep UI in sync. [UNITY] Implement using UI Toolkit's `ListView.onSelectionChanged` plus a `Button` overlay in the cell template that
// commits resets via C# events.
            ...
            if (cur_option != orig_option) {
                ...
                if (grid_col_2->b_from_config) {
                    if (col == col_filaments_reset) {
                        update_filament_to_config(grid_row->config, grid_col_2->key, cur_option, orig_option,
                                                  (grid_row->row_type == row_object));
                        update_volume_values_from_object(row, col - 1);
                    } else
                        update_value_to_config(grid_row->config, grid_col_2->key, cur_option, orig_option);
                    ObjectVolumeID object_volume_id;
                    object_volume_id.object = m_panel->m_model->objects[grid_row->object_id];
                    object_volume_id.volume = (grid_row->row_type == row_object) ? nullptr :
                                                                                   object_volume_id.object->volumes[grid_row->volume_id];
                    if (col == col_filaments_reset)
                        wxGetApp().obj_list()->update_filament_values_for_items(m_panel->m_filaments_count);
                    else
                        wxGetApp().obj_list()->object_config_options_changed(object_volume_id);
                    ...
                    m_panel->m_object_settings->ValueChanged(row, is_object, object, grid_row->config, grid_col_2->category,
                                                             grid_col_2->key);
...
// [EVENT] Mirrors wxGrid `EVT_GRID_CELL_CHANGED`, refreshing the side panel and object list when an editable cell commits a new value.
// [STATE] Skips header rows and reset columns, and only notifies `obj_list()` when values come from the configuration rather than the
// object. [UNITY] Unity should hook `ListView.onSelectionChanged` or manually call this from a `CellEditor` completion callback when the
...
wxBitmap& ObjectGridTable::get_undo_bitmap(bool selected) { return m_panel->m_undo_bitmap; }
...
wxBEGIN_EVENT_TABLE(ObjectTablePanel, wxPanel)
    ...
    EVT_GRID_CELL_LEFT_CLICK(ObjectTablePanel::OnCellLeftClick) EVT_GRID_ROW_SIZE(ObjectTablePanel::OnRowSize)
    ...
    wxEND_EVENT_TABLE()
        ...
        ObjectTablePanel::ObjectTablePanel(wxWindow*       parent,
                                           wxWindowID      id,
                                           const wxPoint&  pos,
                                           const wxSize&   size,
                                           long            style,
    ...
    , m_float_validator(2, nullptr, wxNUM_VAL_ZERO_AS_BLANK)
{
    ...
    /* m_page_text = new wxStaticText(m_side_window, wxID_ANY, wxString(L("Per Object Setting")), wxDefaultPosition, wxSize(-1, 32),
     wxALIGN_CENTRE_HORIZONTAL|wxST_ELLIPSIZE_END); m_page_text->SetMaxSize(wxSize(256, 32)); m_page_text->SetFont(Label::Head_18);
...
int ObjectTablePanel::init_filaments_and_colors()
{
    ...
    if (m_filaments_count <= 0) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                 << boost::format(", can not get filaments, count: %1%, set to default") % m_filaments_count;
    ...
    if (color_count != m_filaments_count) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                   << boost::format(", invalid color count:%1%, extruder count: %2%") % color_count % m_filaments_count;
    }
...
void ObjectTablePanel::load_data()
{
    ...
    m_object_grid->SetColLabelValue(ObjectGridTable::col_plate_index,
                                    wxString::Format("%S%S", _L("Plate"), wxString::FromUTF8("\u2191\u2193")));
    ...
    m_object_grid->SetColLabelValue(ObjectGridTable::col_filaments,
                                    wxString::Format("%S%S", _L("Filament"), wxString::FromUTF8("\u2191\u2193")));
    ...
    for (int i = 0; i < ObjectGridTable::col_max; i++) {
        ...
        case ObjectGridTable::col_printable_reset: m_object_grid->SetColSize(i, FromDIP(0)); break;
        ...
        case ObjectGridTable::col_filaments_reset: m_object_grid->SetColSize(i, FromDIP(0)); break;
        ...
        case ObjectGridTable::col_layer_height_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;
        ...
        case ObjectGridTable::col_wall_loops_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;

        case ObjectGridTable::col_fill_density: m_object_grid->SetColSize(i, m_object_grid->GetColSize(i) - FromDIP(28)); break;

        case ObjectGridTable::col_fill_density_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;

        case ObjectGridTable::col_enable_support: m_object_grid->SetColSize(i, m_object_grid->GetColSize(i) - FromDIP(28)); break;

        case ObjectGridTable::col_enable_support_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;

        case ObjectGridTable::col_brim_type: m_object_grid->SetColSize(i, FromDIP(56)); break;

        case ObjectGridTable::col_brim_type_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;
        ...
        case ObjectGridTable::col_speed_perimeter_reset: m_object_grid->SetColSize(i, FromDIP(28)); break;
        }
    }

    m_object_grid->SetGridLineColour(*wxWHITE);
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(", finished, got %1% rows, %2% cols") % m_object_grid_table->GetNumberRows() %
...
void ObjectTablePanel::OnRowSize(wxGridSizeEvent& ev)
{
    ...
    g_dialog_max_width  = (panel_size.GetWidth() > g_max_size_from_parent.GetWidth()) ? g_max_size_from_parent.GetWidth() :
                                                                                        panel_size.GetWidth();
    g_dialog_max_height = (panel_size.GetHeight() > g_max_size_from_parent.GetHeight()) ? g_max_size_from_parent.GetHeight() :
...
void ObjectTablePanel::OnCellValueChanged(wxGridEvent& ev)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format("cell (%1%, %2%) changed from %3% to %4%") % row % col % ev.GetString() %
                                    m_object_grid->GetCellValue(row, col);
...
void ObjectTablePanel::msw_rescale() { m_object_grid->HideRowLabels(); }
    ...
    , m_plater(platerObj)
{
    ...
    if ((g_dialog_width == 0) || (g_dialog_height == 0)) {
        g_dialog_height = (panel_size.GetHeight() > g_max_size_from_parent.GetHeight()) ? g_max_size_from_parent.GetHeight() :
                                                                                          panel_size.GetHeight();
        g_dialog_width  = (panel_size.GetWidth() > g_max_size_from_parent.GetWidth()) ? g_max_size_from_parent.GetWidth() :
                                                                                        panel_size.GetWidth();
    }
    ...
    // g_dialog_max_height = (panel_size.GetHeight() > g_max_size_from_parent.GetHeight()) ? g_max_size_from_parent.GetHeight() :
    // panel_size.GetHeight(); this->SetMaxSize(wxSize(g_dialog_max_width, g_dialog_max_height)); m_top_sizer->SetSizeHints(this);
...
void ObjectTableDialog::on_sys_color_changed() { Refresh(); }
