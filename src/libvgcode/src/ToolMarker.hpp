...
namespace libvgcode {
// [INTENT] RAII wrapper for arrow geometry on GPU
// [UNITY] Would be: class ToolMarker : IDisposable (manual GPU resource cleanup)
// [PORTING_HAZARD:P1] C++ RAII pattern doesn't translate - Unity requires explicit Dispose()
...
public:
    ToolMarker() = default;
    ~ToolMarker() { shutdown(); }
    // [PORTING_HAZARD:P1] Deleted copy/move - use reference semantics in Unity
    ...
    // [OPENGL] Creates VAO, VBO, IBO from generated geometry
    // [UNITY] Equivalent: GenerateMesh() + GraphicsBuffer.Allocate()
    //
    void init(uint16_t resolution, float tip_radius, float tip_height, float stem_radius, float stem_height);
    //
    // Release gpu buffers.
    // [OPENGL] Calls glDeleteBuffers/glDeleteVertexArrays
    // [UNITY] Equivalent: GraphicsBuffer.Release() in Dispose()
    //
    void shutdown();
    // [OPENGL] Bind VAO and draw indexed triangles
    // [UNITY] Equivalent: Graphics.DrawMeshInstanced()
    void render();

    // [INTENT] Properties for transform and appearance
    // [UNITY] Would be [SerializeField] Vector3 position, float offsetZ, Color color, float alpha
...
private:
    // [STATE] Transform and appearance state
    Vec3  m_position{0.0f, 0.0f, 0.0f};
    float m_offset_z{0.5f};
    Color m_color{255, 255, 255};
    float m_alpha{0.5f};

    // [STATE] GPU state: indices count and buffer handles
    uint16_t     m_indices_count{0};
    unsigned int m_vao_id{0};
    unsigned int m_vbo_id{0};
    unsigned int m_ibo_id{0};
    // [THREAD] Single-threaded GPU ownership (no locking)
