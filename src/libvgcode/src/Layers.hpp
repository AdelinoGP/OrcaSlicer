...
public:
    ...
    {
        return (mode < ETimeMode::COUNT && layer_id < m_items.size()) ? m_items[layer_id].times[static_cast<std::size_t>(mode)] : 0.0f;
    }
    float       get_layer_z(std::size_t layer_id) const { return (layer_id < m_items.size()) ? m_items[layer_id].z : 0.0f; }
...
private:
    // [STATE] Metadata for a single layer.
    // [UNITY] Map to `LayerInfo` struct.
    struct Item
    {
        float                               z{0.0f};
        Range                               range;
