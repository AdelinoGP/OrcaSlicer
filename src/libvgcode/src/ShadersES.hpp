...
static const char* Segments_Vertex_Shader_ES =
    ...
    "  float top_specular = light_top_specular * pow(max(dot(-normalize(eye_position), reflect(-light_top_dir, eye_normal)), 0.0), "
    "light_top_shininess);\n"
    ...
    "#ifdef FIX_TWISTING\n"
    "  int closer_id = (dot(camera_position - pos_a, camera_position - pos_a) < dot(camera_position - pos_b, camera_position - pos_b)) ? "
    "id_a : id_b;\n"
    ...
    "}\n";
// [OPENGL] Fragment shader for segment color output
...
// [OPENGL] Vertex shader for option markers (different lighting model)
static const char* Options_Vertex_Shader_ES =
    ...
    "  gl_Position = projection_matrix * vec4(eye_position, 1.0);\n"
    "}\n";
// [OPENGL] Fragment shader for options (same as segments)
...
// [INTENT] Cog and tool markers are optional features controlled by compile-time flag
// [UNITY] These would be separate shader passes or material variants in Unity
static const char* Cog_Marker_Vertex_Shader_ES =
    ...
    "  gl_Position = projection_matrix * vec4(eye_position, 1.0);\n"
    "}\n";
// [OPENGL] Fragment shader for cog marker with black/white checker pattern
...
static const char* Tool_Marker_Vertex_Shader_ES =
    ...
    "  gl_Position = projection_matrix * vec4(eye_position, 1.0);\n"
    "}\n";
// [OPENGL] Fragment shader for tool marker (pass-through color)
