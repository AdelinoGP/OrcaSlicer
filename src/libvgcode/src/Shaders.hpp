...
namespace libvgcode {
// [INTENT] Desktop OpenGL shader strings for G-code path visualization
// [UNITY] These GLSL shaders need conversion to HLSL/ShaderGraph for Unity SRP
...
// [PORTING_HAZARD:P1] Platform-specific scaling - Unity handles cross-platform uniformly
// [UNCLEAR] Why different scaling factors needed between Windows and Unix?
#ifndef _WIN32
                                           ...
                                           "  float top_diffuse = light_top_diffuse * max(dot(eye_normal, light_top_dir), 0.0);\n"
                                           "  float front_diffuse = light_front_diffuse * max(dot(eye_normal, light_front_dir), 0.0);\n"
                                           "  float top_specular = light_top_specular * pow(max(dot(-normalize(eye_position), "
