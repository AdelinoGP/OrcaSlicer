...
// [INTENT] Implementation of G-code visualization, managing OpenGL resources and GPU data
// [UNITY] Would be: MonoBehaviour that holds references to Meshes, Textures, and Material
// [PORTING_HAZARD:P3] Extensive reliance on OpenGL buffer/texture management
class ViewerImpl
{
public:
    ViewerImpl();
    ...
    const PathVertex& get_vertex_at(size_t id) const { return (id < m_vertices.size()) ? m_vertices[id] : PathVertex::DUMMY_PATH_VERTEX; }
...
private:
    ...
    std::array<ColorRange, COLOR_RANGE_TYPES_COUNT> m_layer_time_range{ColorRange(EColorRangeType::Linear),
