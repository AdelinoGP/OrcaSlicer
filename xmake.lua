...
-- resolved beside the executable); on means resources live under
-- <prefix>/share/OrcaSlicer, which is what distro packages want.
-- CMakeLists.txt:129 also forces SLIC3R_DESKTOP_INTEGRATION off when FHS is on.
option("fhs")
    set_default(false)
    set_description("Assume OrcaSlicer is installed in an FHS directory structure (Linux)")
option_end()

option("prefix")
    set_default("/usr/local")
    set_description("Install prefix for the FHS layout (CMAKE_INSTALL_PREFIX equivalent)")
option_end()
...
rule("pnp.conan")
    on_load(function (target)
        ...
        -- Lockfiles are resolved per platform: a lock records concrete recipe
        -- revisions for one dependency graph, and the graph is not the same
        -- across platforms (Linux pulls GTK/fontconfig/dbus that Windows never
        -- sees). conan.lock is the Windows-resolved graph and stays the
        -- default name for backwards compatibility; other platforms use
        -- conan-<plat>.lock. Whoever has that host generates it with
        --     conan lock create conan/conanfile.py --lockfile="" \
        --         --profile:host=conan/profile_host.txt \
        --         --profile:build=conan/profile_build.txt \
        --         --lockfile-out=conan/conan-<plat>.lock
        -- and commits it. Until then we resolve without a lock, which builds
        -- but is not reproducible — hence the warning.
        local plat = config.plat() or os.host()
        local lockfile = path.join(projectdir, "conan",
            plat == "windows" and "conan.lock" or ("conan-" .. plat .. ".lock"))
        local have_lock = os.isfile(lockfile)
        if not have_lock then
            wprint("pnp.conan: no lockfile for %s (%s); resolving unlocked. " ..
                   "Dependency versions are NOT reproducible on this platform.",
                   plat, path.filename(lockfile))
        end
        ...
        if stale and not _g.pnp_conan_installed then
            ...
            local argv = {"install", path.join(projectdir, "conan", "conanfile.py"),
...
target("OrcaSlicer")
    ...
    -- (a #cmakedefine template, which add_configfiles cannot render):
    --     #cmakedefine SLIC3R_FHS @SLIC3R_FHS@
    --     #define SLIC3R_FHS_RESOURCES "@SLIC3R_FHS_RESOURCES@"
    -- With fhs off this is the portable layout (resources beside the binary);
    -- with fhs on, resources resolve to <prefix>/share/OrcaSlicer, matching
    -- CMakeLists.txt:939-950.
    on_load(function (target)
        import("core.project.config")
        ...
        if has_config("fhs") then
            ...
            content = "/* generated by xmake; FHS layout */\n" ..
                "#define SLIC3R_FHS 1\n" ..
                "#define SLIC3R_FHS_RESOURCES \"" .. prefix .. "/share/OrcaSlicer\"\n"
        else
            content = "/* generated by xmake; portable layout (no FHS) */\n" ..
                "/* #undef SLIC3R_FHS */\n" ..
                "#define SLIC3R_FHS_RESOURCES \"\"\n"
        end
        -- rewrite unconditionally: writing only when absent would silently keep
        -- a stale layout after `xmake f --fhs=y`
        if not os.isfile(out) or io.readfile(out) ~= content then
...
-- xmake appimage -> Linux AppImage, replacing build_appimage.sh.in.
--
-- UNVERIFIED: never run. No Linux host was available during the migration.
-- Ports the AppDir construction from src/dev-utils/platform/unix/build_appimage.sh.in
-- (AppRun, top-level icon, hicolor icon, .desktop, metainfo) but does NOT
-- reproduce that script's `wget` of appimagetool or its /.dockerenv special
-- cases: appimagetool must already be on PATH.
task("appimage")
    on_run(function ()
        import("core.project.config")
        ...
        assert(config.plat() == "linux", "appimage: Linux only (plat is %s)", config.plat())
        local appimagetool = find_tool("appimagetool")
        assert(appimagetool, "appimage: appimagetool not found on PATH")

        -- AppDir is the portable tree with the binary renamed to AppRun
        local staged = layout.assemble()
        local appdir = path.join(config.builddir(), "package", "OrcaSlicer.AppDir")
        os.tryrm(appdir)
        os.vcp(staged, appdir)
        os.mv(path.join(appdir, layout.exe_name()), path.join(appdir, "AppRun"))

        local icon = path.join(appdir, "resources", "images", "OrcaSlicer_192px.png")
        os.vcp(icon, path.join(appdir, "OrcaSlicer.png"))
        os.vcp(icon, path.join(appdir, "usr", "share", "icons", "hicolor",
            "192x192", "apps", "OrcaSlicer.png"))

        io.writefile(path.join(appdir, "com.orcaslicer.OrcaSlicer.desktop"), [[
[Desktop Entry]
Name=OrcaSlicer
Exec=AppRun %F
Icon=OrcaSlicer
...
MimeType=model/stl;application/vnd.ms-3mfdocument;application/prs.wavefront-obj;application/x-amf;
        ...
        os.vcp(path.join(appdir, "com.orcaslicer.OrcaSlicer.desktop"),
            path.join(appdir, "usr", "share", "applications",
                "com.orcaslicer.OrcaSlicer.desktop"))

        local metainfo = path.join(os.projectdir(), "scripts", "flatpak",
            "com.orcaslicer.OrcaSlicer.metainfo.xml")
        if os.isfile(metainfo) then
            local dst = path.join(appdir, "usr", "share", "metainfo")
            os.vcp(metainfo, path.join(dst, "com.orcaslicer.OrcaSlicer.metainfo.xml"))
            -- the shell script installs the same file under both names
    ...
    set_menu {
        usage = "xmake appimage",
        description = "Build a Linux AppImage (UNVERIFIED — requires appimagetool)"
    }
task_end()

-- xmake dmg -> macOS disk image around the .app bundle.
--
-- UNVERIFIED: never run. No Mac was available during the migration.
-- No code signing or notarization — those need an Apple Developer identity.
task("dmg")
    on_run(function ()
        import("core.project.config")
        import("lib.detect.find_tool")
        ...
        assert(config.plat() == "macosx", "dmg: macOS only (plat is %s)", config.plat())
    ...
    set_menu {
        usage = "xmake dmg",
        description = "Build a macOS .dmg (UNVERIFIED — requires hdiutil)"
    }
task_end()
...
target("pnp_config_translator_tests")
    ...
    add_files("tests/pnp/test_pnp_config_translator.cpp",
              ...
              "tests/pnp/test_pnp_preserved_rows.cpp",
