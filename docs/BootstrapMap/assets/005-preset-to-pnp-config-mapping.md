...
> `support_type` added to Tier A 2026-08-11: pnp's claim dedup resolves the `support-generator` holder from the raw sidecar value (`SUPPORT_GENERATOR_CONFIG_KEY`, values `tree(auto)` / `tree(manual)` / legacy `hybrid(auto)` → `com.core.tree-support`, else traditional) — see the 013 third-surface addendum. The row is an identity copy; the raw value was already reaching pnp in the 3MF sidecar, so the tint clearing is the only GUI behavior change.
